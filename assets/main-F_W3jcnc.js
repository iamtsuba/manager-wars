import{s as k,F as Ot,h as vt,j as $i,l as Je,i as Xi,k as Ji,b as Qi}from"./formation-links-CEzN0rcz.js";function ci(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  `,e.querySelectorAll(".auth-tab").forEach(n=>{n.addEventListener("click",()=>{e.querySelectorAll(".auth-tab").forEach(c=>c.classList.remove("active")),n.classList.add("active"),document.getElementById("tab-login").style.display=n.dataset.tab==="login"?"block":"none",document.getElementById("tab-register").style.display=n.dataset.tab==="register"?"block":"none"})}),document.getElementById("login-btn").addEventListener("click",async()=>{const n=document.getElementById("login-email").value.trim(),c=document.getElementById("login-password").value,s=document.getElementById("login-error");if(s.textContent="",!n||!c){s.textContent="Remplissez tous les champs.";return}const a=document.getElementById("login-btn");a.textContent="Connexion…",a.disabled=!0;const{error:d}=await k.auth.signInWithPassword({email:n,password:c});if(a.textContent="Se connecter",a.disabled=!1,d){s.textContent=d.message.includes("Invalid")?"Email ou mot de passe incorrect.":d.message;return}window.location.reload()}),document.getElementById("login-password").addEventListener("keydown",n=>{n.key==="Enter"&&document.getElementById("login-btn").click()}),document.getElementById("reg-btn").addEventListener("click",async()=>{const n=document.getElementById("reg-email").value.trim(),c=document.getElementById("reg-password").value,s=document.getElementById("reg-confirm").value,a=document.getElementById("reg-error");if(a.textContent="",!n||!c||!s){a.textContent="Remplissez tous les champs.";return}if(c.length<6){a.textContent="Mot de passe trop court (min. 6 caractères).";return}if(c!==s){a.textContent="Les mots de passe ne correspondent pas.";return}const d=document.getElementById("reg-btn");d.textContent="Création…",d.disabled=!0;const{error:p}=await k.auth.signUp({email:n,password:c});if(d.textContent="Créer mon compte",d.disabled=!1,p){a.textContent=p.message;return}i("Compte créé ! Connectez-vous.","success",4e3),document.querySelector('[data-tab="login"]').click(),document.getElementById("login-email").value=n})}function Zi(e,{state:t,navigate:i,toast:n,refreshProfile:c}){let s="#1A6B3C",a="#D4A017";e.innerHTML=`
  ...
  `;function d(){var y;const o=document.getElementById("logo-preview"),r=document.getElementById("logo-initials"),u=((y=document.getElementById("setup-club"))==null?void 0:y.value)||"MW",f=u.trim().split(" ").filter(Boolean),g=f.length>=2?(f[0][0]+f[1][0]).toUpperCase():u.slice(0,2).toUpperCase();o&&(o.style.background=a,o.style.borderColor=s),r&&(r.textContent=g,r.style.color=s)}document.getElementById("color1").addEventListener("input",o=>{s=o.target.value,document.getElementById("swatch1").style.background=s,d()}),document.getElementById("color2").addEventListener("input",o=>{a=o.target.value,document.getElementById("swatch2").style.background=a,d()});function p(o){document.querySelectorAll(".setup-step").forEach(r=>r.classList.remove("active")),document.getElementById(`step-${o}`).classList.add("active"),document.getElementById("step-num").textContent=o,document.getElementById("progress-fill").style.width=`${Math.round(o/3*100)}%`,o===3&&d()}document.getElementById("step1-next").addEventListener("click",async()=>{const o=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("step1-error");if(r.textContent="",o.length<3){r.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await k.from("users").select("id").eq("pseudo",o).maybeSingle();if(u){r.textContent="Ce pseudo est déjà pris.";return}p(2)}),document.getElementById("step2-back").addEventListener("click",()=>p(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const o=document.getElementById("setup-club").value.trim(),r=document.getElementById("step2-error");if(r.textContent="",o.length<2){r.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await k.from("users").select("id").eq("club_name",o).maybeSingle();if(u){r.textContent="Ce nom de club est déjà pris.";return}p(3)}),document.getElementById("step3-back").addEventListener("click",()=>p(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const o=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),f=document.getElementById("step3-finish");u.textContent="",f.disabled=!0,f.textContent="Création en cours…";try{const{error:g}=await k.from("users").insert({id:t.user.id,pseudo:o,club_name:r,club_color1:s,club_color2:a,credits:1e4});if(g)throw g;await eo(t.user.id),await c(),n(`Bienvenue ${o} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(g){u.textContent=g.message,f.disabled=!1,f.textContent="🚀 Créer mon profil !"}})}async function eo(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await k.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const mt="#1A6B3C",xt="#cc2222",to="#D4A017",pi="#888";async function io(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </div>`,await _i(i,n),document.getElementById("btn-add-friend").addEventListener("click",()=>no(i,n)),document.getElementById("btn-accept-friend").addEventListener("click",()=>ki(i,n))}async function _i(e,t){const i=e.user.id,{data:n,error:c}=await k.from("friendships").select("id, requester_id, addressee_id").eq("status","accepted").or(`requester_id.eq.${i},addressee_id.eq.${i}`),{count:s}=await k.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",i).eq("status","pending"),a=document.getElementById("pending-badge");a&&(s>0?(a.style.display="flex",a.textContent=s):a.style.display="none");const d=document.getElementById("friends-list");if(!d)return;if(c){console.error("[Friends] Erreur:",c),d.innerHTML=`<div style="color:${xt};text-align:center;padding:16px">Erreur chargement : ${c.message}</div>`;return}const p=(n||[]).map(u=>u.requester_id===i?u.addressee_id:u.requester_id);let o={};if(p.length){const{data:u}=await k.from("users").select("id, pseudo, club_name").in("id",p);(u||[]).forEach(f=>{o[f.id]=f})}const r=(n||[]).map(u=>({friendshipId:u.id,friend:o[u.requester_id===i?u.addressee_id:u.requester_id]||{pseudo:"?"}}));if(!r.length){d.innerHTML=`
    ...
    </div>`,d.querySelectorAll("[data-stats]").forEach(u=>{u.addEventListener("click",()=>ro(e,u.dataset.stats,u.dataset.friendName))}),d.querySelectorAll("[data-match]").forEach(u=>{u.addEventListener("click",()=>t("⚽ Match ami — Bientôt disponible !","info"))})}function oo(e,t){const i=e.club_name||e.pseudo||"?",n=(e.pseudo||"?").slice(0,2).toUpperCase();return`
    ...
    ${Ut()}`,document.body.appendChild(i);const n=i.querySelector("#friend-pseudo-input"),c=i.querySelector("#add-friend-error"),s=()=>i.remove();n.focus(),i.querySelector("#add-cancel").addEventListener("click",s),i.addEventListener("click",a=>{a.target===i&&s()}),i.querySelector("#add-ok").addEventListener("click",async()=>{const a=n.value.trim();if(!a){c.textContent="Entre un pseudo";return}c.textContent="";const{data:d}=await k.from("users").select("id, pseudo").ilike("pseudo",a).single();if(!d){c.textContent="Utilisateur introuvable";return}if(d.id===e.user.id){c.textContent="Tu ne peux pas t'ajouter toi-même";return}const{data:p}=await k.from("friendships").select("id, status").or(`and(requester_id.eq.${e.user.id},addressee_id.eq.${d.id}),and(requester_id.eq.${d.id},addressee_id.eq.${e.user.id})`).single();if(p){const r=p.status==="accepted"?"Vous êtes déjà amis !":p.status==="pending"?"Demande déjà envoyée":"Une demande existe déjà";c.textContent=r;return}const{error:o}=await k.from("friendships").insert({requester_id:e.user.id,addressee_id:d.id,status:"pending"});if(o){c.textContent="Erreur : "+o.message;return}s(),t(`✅ Demande envoyée à ${d.pseudo} !`,"success")})}async function ki(e,t,i=null){const n=e.user.id,{data:c}=await k.from("friendships").select("id, requester_id").eq("addressee_id",n).eq("status","pending").order("created_at",{ascending:!1}),s=(c||[]).map(u=>u.requester_id);let a={};if(s.length){const{data:u}=await k.from("users").select("id, pseudo, club_name").in("id",s);(u||[]).forEach(f=>{a[f.id]=f})}const d=(c||[]).map(u=>({...u,requester:a[u.requester_id]||{pseudo:"?"}})),p=Ht(),o=d||[];p.innerHTML=`
    <div class="popup-box">
    ...
    </style>`}const ao="2026.06.27-1519";async function ui(e,{state:t,navigate:i,toast:n}){var s,a;const c=t.profile;c&&(e.innerHTML=`
  <div class="page">
    <div class="page-body">
  ...
  `,document.body.appendChild(i);const n=()=>i.remove();document.getElementById("diff-cancel").addEventListener("click",n),i.addEventListener("click",c=>{c.target===i&&n()}),i.querySelectorAll("[data-mode]").forEach(c=>{c.addEventListener("click",()=>{n(),e("match",{matchMode:c.dataset.mode})})})}const Ce={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé pour ce match."},"Double attaque":{icon:"⚡",desc:"La note d'attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la note d'un joueur adverse."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function me(e,t){if(!e)return 0;switch(t){case"GK":return Number(e.note_g)||0;case"DEF":return Number(e.note_d)||0;case"MIL":return Number(e.note_m)||0;case"ATT":return Number(e.note_a)||0;default:return 0}}const fi=["ATT","MIL","DEF","GK"];function Ai(e){let t=0;const i=e.length;for(let n=0;n<i;n++)for(let c=n+1;c<i;c++){const s=e[n],a=e[c];if(!s||!a)continue;const d=s._col!=null&&a._col!=null&&s._col===a._col,p=s._col!=null&&a._col!=null&&Math.abs(s._col-a._col)===1,o=fi.indexOf(s._line||s.job),r=fi.indexOf(a._line||a.job),u=Math.abs(o-r)===1;if(!((s._line||s.job)===(a._line||a.job)&&p||d&&u))continue;const y=s.country_code&&a.country_code&&s.country_code===a.country_code,l=s.club_id&&a.club_id&&s.club_id===a.club_id;y&&l?t+=2:(y||l)&&(t+=1)}return t}function Kt(e,t={}){const i=e.reduce((s,a)=>{const d=a._line||a.job;return s+(Number(d==="MIL"?a.note_m:a.note_a)||0)+(a.boost||0)},0),n=Ai(e);let c=i+n;return t.doubleAttack&&(c*=2),t.stolenNote&&(c-=t.stolenNote),{base:i,links:n,total:Math.max(0,c)}}function Vt(e,t={}){const i=e.reduce((s,a)=>{const d=a._line||a.job;let p=0;return d==="GK"?p=Number(a.note_g)||0:d==="MIL"?p=Number(a.note_m)||0:p=Number(a.note_d)||0,s+p+(a.boost||0)},0),n=Ai(e);let c=i+n;return t.stolenNote&&(c-=t.stolenNote),{base:i,links:n,total:Math.max(0,c)}}function Yt(e,t,i={}){return i.shield?{goal:!1,shielded:!0}:{goal:e>t,shielded:!1}}function Ti(e,t,i="easy"){const n=e.filter(a=>!a.used);if(!n.length)return[];const c=[...n].sort((a,d)=>{const p=t==="attack"?me(a,"ATT"):a._line==="GK"?me(a,"GK"):me(a,"DEF");return(t==="attack"?me(d,"ATT"):d._line==="GK"?me(d,"GK"):me(d,"DEF"))-p});let s=i==="easy"?1+Math.floor(Math.random()*2):i==="medium"?2+Math.floor(Math.random()*2):3;return c.slice(0,Math.min(s,c.length,3))}function lo(e,t){const i={vs_ai_easy:{victoire:500,nul:250,defaite:50},vs_ai_medium:{victoire:1e3,nul:500,defaite:50},vs_ai_hard:{victoire:1500,nul:750,defaite:100},vs_ai_club:{victoire:2500,nul:1250,defaite:100}};return(i[e]||i.vs_ai_easy)[t]||0}const Ii={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},ft={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},wt=["GK","DEF","MIL","ATT"],co=["Tous","GK","DEF","MIL","ATT"],po={normal:1e3,pepite:5e3,papyte:5e3,legende:1e4},Wt={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function Li(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";if(!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function yt(e,t){return e&&Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}function St(e,t=""){var u,f;const i=e.player;if(!i)return"";const n=i.job||"ATT",c=ft[n],s=Ii[i.rarity]||"#ccc",a=yt(i,n),d=i.job2?yt(i,i.job2):null,p=i.job2?ft[i.job2]:null,o=Li(i),r=Wt[i.country_code]||i.country_code||"";return`
  <div style="
  ...
  </div>`}async function uo(e,t){const{state:i,navigate:n,toast:c,openModal:s,closeModal:a}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await k.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(l=document.getElementById("direct-sell-form-btn"))==null||l.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${o} pour ${lt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const h=f.find(D=>!D.is_for_sale)||f[0];if(!h){s("Aucune carte à vendre","error");return}const{error:I}=await k.from("cards").delete().eq("id",h.id);if(I){s(I.message,"error");return}await k.from("users").update({credits:(c.profile.credits||0)+lt}).eq("id",c.profile.id),await p(),s(`+${lt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),d("collection")}),(x=document.getElementById("market-sell-form-btn"))==null||x.addEventListener("click",async()=>{const h=parseInt(document.getElementById("sell-price-form").value);if(!h||h<1){s("Prix invalide","error");return}await k.from("cards").update({is_for_sale:!0,sale_price:h}).eq("id",e.id),await k.from("market_listings").insert({seller_id:c.profile.id,card_id:e.id,price:h}),s("Carte mise en vente sur le marché !","success"),a(),d("collection")}),(v=document.getElementById("cancel-sell-form-btn"))==null||v.addEventListener("click",async()=>{await k.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await k.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),s("Annonce retirée","success"),a(),d("collection")})}async function mi(e,t,i,n){var te,$,R,Y,A,K;const{state:c,toast:s,openModal:a,closeModal:d,navigate:p,refreshProfile:o}=n,r=e.player,u=t.filter(G=>G.player.id===r.id),f=u.length,g=po[r.rarity]||1e3,y=r.rarity!=="legende",l=Li(r),x=yt(r,r.job),v=r.job2?yt(r,r.job2):null,h=ft[r.job]||"#1A6B3C",I=r.job2?ft[r.job2]:null,D=Ii[r.rarity]||"#ccc",_=Wt[r.country_code]||r.country_code||"",w=u.map(G=>G.id);let S={};if(w.length){const{data:G}=await k.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",w).order("transferred_at",{ascending:!0});(G||[]).forEach(J=>{S[J.card_id]||(S[J.card_id]=[]),S[J.card_id].push(J)})}const ee=G=>`
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const a=prompt("Nom du deck :",`Deck ${((s==null?void 0:s.length)||0)+1}`);if(!a)return;const{data:d,error:p}=await k.from("decks").insert({owner_id:i.profile.id,name:a}).select().single();if(p){c(p.message,"error");return}c("Deck créé !","success"),xi(d.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(a=>{a.addEventListener("click",()=>xi(a.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(a=>{a.addEventListener("click",async()=>{const d=prompt("Nouveau nom :",a.dataset.name);if(!d||d===a.dataset.name)return;const{error:p}=await k.from("decks").update({name:d}).eq("id",a.dataset.rename);if(p){c(p.message,"error");return}c("Deck renommé !","success"),Bt(e,t)})}),e.querySelectorAll("[data-delete]").forEach(a=>{a.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${a.dataset.name}" ? Cette action est irréversible.`))return;await k.from("deck_cards").delete().eq("deck_id",a.dataset.delete);const{error:d}=await k.from("decks").delete().eq("id",a.dataset.delete);if(d){c(d.message,"error");return}c("Deck supprimé.","success"),Bt(e,t)})})}async function xi(e,t,i){const{state:n,toast:c}=i;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await k.from("decks").select("*").eq("id",e).single(),{data:a}=await k.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",n.profile.id),d=(a||[]).filter(g=>g.card_type==="player"&&g.player),p=(a||[]).filter(g=>g.card_type==="formation"),o=p.map(g=>g.formation).filter(Boolean),{data:r}=await k.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let u=s.formation||"4-4-2";o.length>0&&!o.includes(u)&&(u=o[0]);const f={deckId:e,name:s.name,formation:u,formationCardId:s.formation_card_id,slots:{},subs:[],playerCards:d,formationCards:p,availableFormations:o};(r||[]).forEach(g=>{g.is_starter?f.slots[g.position]=g.card_id:f.subs.includes(g.card_id)||f.subs.push(g.card_id)}),ot(t,f,i)}function ot(e,t,i){var p;const{navigate:n}=i;Et[t.formation];const c=yi(t.formation),s=c.filter(o=>t.slots[o]).length,a=t.availableFormations.length>0?t.availableFormations:Object.keys(Et),d=t.subs.map(o=>t.playerCards.find(r=>r.id===o)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div class="page">
    ...
    <div class="page-body">
      ...
      ${u.length>0?u.map(l=>{var D,_;const x=l.player,v=a==="GK"?x.note_g:a==="DEF"?x.note_d:a==="MIL"?x.note_m:x.note_a,h=Lt(x),I={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[x.rarity];return`<div class="player-option" data-card-id="${l.id}"
          ...
          <div style="width:44px;height:44px;border-radius:8px;overflow:hidden;flex-shrink:0;background:#dde;border:2px solid ${We[a]}">
            ${h?`<img src="${h}" style="width:100%;height:100%;object-fit:cover">`:`<div style="width:100%;height:100%;background:${We[a]};display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:700">${a}</div>`}
          </div>
          <!-- Infos -->
          <div style="flex:1;min-width:0">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(g=document.getElementById("close-selector"))==null||g.addEventListener("click",s),(y=document.getElementById("remove-player"))==null||y.addEventListener("click",()=>{delete t.slots[e],s(),ot(i,t,n)}),document.querySelectorAll(".player-option").forEach(l=>{l.addEventListener("click",()=>{t.slots[e]=l.dataset.cardId,s(),ot(i,t,n)})})}function ho(e,t,i){var p;const{openModal:n,closeModal:c}=i,s=new Set;Object.values(e.slots).filter(Boolean).forEach(o=>{var u;const r=e.playerCards.find(f=>f.id===o);(u=r==null?void 0:r.player)!=null&&u.id&&s.add(r.player.id)}),e.subs.forEach(o=>{var u;const r=e.playerCards.find(f=>f.id===o);(u=r==null?void 0:r.player)!=null&&u.id&&s.add(r.player.id)});const a=new Set,d=e.playerCards.filter(o=>{var r,u,f;return s.has((r=o.player)==null?void 0:r.id)||a.has((u=o.player)==null?void 0:u.id)?!1:(a.add((f=o.player)==null?void 0:f.id),!0)});n("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
  ...
  <div class="page">
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${s.map(d=>{const p=d.cost===0||c>=d.cost;return`<div class="booster-card ${p?"":"disabled"}" data-booster="${d.id}" style="position:relative">
  ...
  `,document.body.appendChild(c);let s=!1;const a=document.getElementById("pack-cut-zone"),d=document.getElementById("pack-blade");let p=!1;const o=A=>A.touches&&A.touches[0]?A.touches[0].clientX:A.clientX;function r(A){s||(p=!0,d.style.opacity="1",u(A))}function u(A){if(!p||s)return;const K=a.getBoundingClientRect(),G=o(A)-K.left,J=Math.max(0,Math.min(1,G/K.width));d.style.width=J*K.width+"px",J>=.82&&g()}function f(){s||(p=!1,d.style.transition="width .2s ease, opacity .2s ease",d.style.width="0",d.style.opacity="0",setTimeout(()=>{s||(d.style.transition="")},220))}function g(){var K;if(s)return;s=!0,p=!1,d.style.width="100%",d.style.opacity="1",(K=document.getElementById("cut-flash"))==null||K.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const A=document.getElementById("cut-hint");A&&(A.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{d.style.opacity="0",document.getElementById("pack-phase").style.display="none",v(0)},620)}a.addEventListener("pointerdown",r),window.addEventListener("pointermove",u),window.addEventListener("pointerup",f),a.addEventListener("touchstart",r,{passive:!0}),window.addEventListener("touchmove",u,{passive:!0}),window.addEventListener("touchend",f);let y=0,l=!1;const x=new Set;function v(A){y=A,document.getElementById("reveal-phase").style.display="flex",h(),I(A,0),ee()}function h(){const A=document.getElementById("card-dots");A&&(A.innerHTML=e.map((K,G)=>`<div class="card-dot" data-i="${G}" style="width:8px;height:8px;border-radius:50%;background:${G===y?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),A.querySelectorAll(".card-dot").forEach(K=>K.addEventListener("click",()=>_(parseInt(K.dataset.i)))))}function I(A,K){var N;const G=e[A],J=document.getElementById("card-counter"),se=document.getElementById("card-track");J&&(J.textContent=`Carte ${A+1} / ${e.length}`);const de=G.card_type==="player"&&((N=G.player)==null?void 0:N.rarity)==="legende",ae=!x.has(A);x.add(A);let M=0;if(G.card_type==="player"&&G.player){const j=G.player,P=j.job||"ATT";M=Number(P==="GK"?j.note_g:P==="DEF"?j.note_d:P==="MIL"?j.note_m:j.note_a)||0}const C=j=>{se.innerHTML=`
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px 16px">
        ${s.map(y=>{const l=a.find(x=>x.gc_type===y.gc_type);return d(y,!!l)}).join("")}
      </div>
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(y=>{y.addEventListener("click",()=>{const l=y.dataset.id,x=s.find(h=>h.id===l);if(!x)return;const v=a.findIndex(h=>h.gc_type===x.gc_type);v>-1?a.splice(v,1):a.length<3&&a.push(x),o()})}),(u=e.querySelector("#gc-launch"))==null||u.addEventListener("click",()=>{r&&p(a)}),(f=e.querySelector("#gc-no-gc"))==null||f.addEventListener("click",()=>p([])),(g=e.querySelector("#gc-reset"))==null||g.addEventListener("click",()=>{a.length&&(a=[],o())})}o()}async function Go(e,t,i){const{state:n,navigate:c}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await k.from("decks").select("id,name,is_active,formation").eq("owner_id",n.profile.id).order("created_at",{ascending:!1});if(!s||s.length===0){Xe(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>c("decks"));return}const a=s.map(r=>r.id),{data:d}=await k.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order");let p=0;function o(){var h,I,D,_,w;const r=s[p],u=(d||[]).filter(S=>S.deck_id===r.id),f=u.filter(S=>{var ee;return S.is_starter&&((ee=S.card)==null?void 0:ee.player)}).map(S=>qt(S.card,S.position)),g=u.find(S=>{var ee;return((ee=S.card)==null?void 0:ee.card_type)==="formation"}),y=r.formation||((h=g==null?void 0:g.card)==null?void 0:h.formation)||"4-4-2",l=f.length>=11?At(f,y):null,x=f.length>=11;Ye(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${s.map((S,ee)=>`<div style="width:7px;height:7px;border-radius:50%;background:${ee===p?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
    ...
    </div>`,function(){const ee=e.querySelector(".deck-preview-wrap svg");ee&&(ee.removeAttribute("width"),ee.removeAttribute("height"),ee.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ee.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(I=document.getElementById("prev-deck"))==null||I.addEventListener("click",()=>{p>0&&(p--,o())}),(D=document.getElementById("next-deck"))==null||D.addEventListener("click",()=>{p<s.length-1&&(p++,o())}),(_=document.getElementById("validate-deck"))==null||_.addEventListener("click",()=>{x&&t.navigate("match",{matchMode:i,deckId:r.id})}),(w=document.getElementById("cancel-deck-select"))==null||w.addEventListener("click",()=>{Re(e),c("home")});const v=document.getElementById("deck-swipe-zone");if(v){let S=0,ee=0;v.addEventListener("touchstart",re=>{S=re.touches[0].clientX,ee=re.touches[0].clientY},{passive:!0}),v.addEventListener("touchend",re=>{const te=re.changedTouches[0].clientX-S,$=re.changedTouches[0].clientY-ee;Math.abs(te)<40||Math.abs(te)<Math.abs($)||(te<0&&p<s.length-1?(p++,o()):te>0&&p>0&&(p--,o()))},{passive:!0})}}o()}function Pi(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function Be(e,t=44,i=58){if(!e)return`<div style="width:${t}px;height:${i}px;border:1.5px dashed rgba(255,255,255,0.2);border-radius:5px"></div>`;const n=typeof e.portrait=="string"&&e.portrait.startsWith("http")?e.portrait:Te(e),c=Pi(e),s=e._line||e.job||"MIL",a=rt[s]||"#555",d={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",p=e.note!==void 0?Number(e.note)||0:(Number(me(e,s))||0)+(e.boost||0),o=Oi(e==null?void 0:e.country_code),r=Math.round(i*.19),u=Math.round(i*.25),f=i-Math.round(i*.19)-Math.round(i*.25),g=e!=null&&e.used?.28:1;return`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${d};background:${a};position:relative;overflow:hidden;flex-shrink:0;opacity:${g}">
    <div style="position:absolute;top:0;left:0;right:0;height:${r}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:center;z-index:2">
  ...
  </div>`}function et(e,t,i){if(!(e!=null&&e.length))return"";const n=e.slice(0,5);let c='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return n.forEach((s,a)=>{if(c+=Be(s,40,52),a<n.length-1){const d=Je(s,n[a+1]);c+=`<div style="width:7px;height:3px;background:${d==="#ff3333"||d==="#cc2222"?"rgba(255,255,255,0.12)":d};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(c+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),c+="</div>",c}function Gt(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function Oi(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function ht(e,t,i,n,c=310,s=310,a=[]){var I;const d=vt[t]||{},p=$i(t)||Ot[t]||[],o={},r=["ATT","MIL","DEF","GK"];for(const D of r)(e[D]||[]).forEach((w,S)=>{o[`${D}${S+1}`]=w});function u(D){const _=d[D];return _?{x:_.x*c,y:_.y*s}:null}let f="";for(const[D,_]of p){const w=u(D),S=u(_);if(!w||!S)continue;const ee=o[D],re=o[_],te=Je(ee,re);te==="#00ff88"||te==="#FFD700"?(f+=`<line x1="${w.x.toFixed(1)}" y1="${w.y.toFixed(1)}" x2="${S.x.toFixed(1)}" y2="${S.y.toFixed(1)}"
  ...
  </div>`}async function Hi(e,t,i,n){var _;const{state:c,navigate:s,toast:a}=t;Ye(e);const d=c.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!d.deckId)return Go(e,t,i);const p=d.deckId;let o,r,u,f;try{const w=await Promise.all([k.from("decks").select("formation,name").eq("id",p).single(),k.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",p).order("slot_order")]);o=w[0].data,u=w[0].error,r=w[1].data,f=w[1].error}catch(w){console.error("[Match] Exception chargement deck:",w),Xe(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>s("home"));return}if(u||f){console.error("[Match] Erreur Supabase:",u||f),Xe(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>s("home"));return}const g=(r||[]).filter(w=>{var S;return w.is_starter&&((S=w.card)==null?void 0:S.player)}).map(w=>qt(w.card,w.position)),y=(r||[]).filter(w=>{var S;return!w.is_starter&&((S=w.card)==null?void 0:S.player)}).map(w=>qt(w.card,w.position));if(g.length<11){Xe(e,"⚠️",`Deck incomplet (${g.length}/11).`,"Compléter",()=>s("decks"));return}const l=(r||[]).find(w=>{var S;return((S=w.card)==null?void 0:S.card_type)==="formation"}),x=(o==null?void 0:o.formation)||((_=l==null?void 0:l.card)==null?void 0:_.formation)||"4-4-2",{data:v,error:h}=await k.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",c.profile.id).eq("card_type","game_changer"),{data:I}=await k.from("gc_definitions").select("*").eq("is_active",!0),D=(v||[]).map(w=>({...w,_gcDef:(I==null?void 0:I.find(S=>S.name===w.gc_type||S.id===w.gc_definition_id))||null}));n({deckId:p,formation:x,starters:g,subsRaw:y,gcCardsEnriched:D,gcDefs:I||[]})}async function Ro(e,t){const{state:i}=t,c=(i.params||{}).matchMode||"vs_ai_easy",s=c.replace("vs_ai_",""),a=c;await Hi(e,t,c,async({deckId:d,formation:p,starters:o,subsRaw:r,gcCardsEnriched:u,gcDefs:f})=>{try{const g=At(o,p),y=await No(p,s),l=async x=>{try{const{data:v,error:h}=await k.from("matches").insert({home_id:i.profile.id,away_id:null,mode:a,home_deck_id:d,status:"in_progress"}).select().single();if(h){console.error("[MatchIA] Erreur création match:",h),Xe(e,"⚠️","Impossible de créer le match ("+h.message+").","Retour",()=>t.navigate("home"));return}const I={gcDefs:f||[],matchId:v==null?void 0:v.id,mode:a,difficulty:s,formation:p,homeTeam:g,aiTeam:y,homeSubs:r,subsUsed:0,maxSubs:Math.min(r.length,3),homeScore:0,aiScore:0,gcCards:x,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Oo(e,I,t)}catch(v){console.error("[MatchIA] Exception launchMatch:",v),Xe(e,"⚠️","Erreur au lancement du match : "+v.message,"Retour",()=>t.navigate("home"))}};if(!u.length){l([]);return}Ni(e,u,l)}catch(g){console.error("[MatchIA] Exception setup:",g),Xe(e,"⚠️","Erreur de préparation du match : "+g.message,"Retour",()=>t.navigate("home"))}})}async function No(e,t){var a,d;const{data:i}=await k.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return Po(e);const n=nt[e]||nt["4-4-2"],c={GK:[],DEF:[],MIL:[],ATT:[]},s=[...i];for(const p of["GK","DEF","MIL","ATT"]){const o=s.filter(y=>y.job===p),r=s.filter(y=>y.job!==p),u=[...o,...r],f=[];for(let y=0;y<n[p];y++){const l=u[y]||s[y];l&&f.push({cardId:"ai-"+l.id+"-"+y,id:l.id,firstname:l.firstname,name:l.surname_encoded,country_code:l.country_code,club_id:l.club_id,job:l.job,job2:l.job2,note_g:Number(l.note_g)||0,note_d:Number(l.note_d)||0,note_m:Number(l.note_m)||0,note_a:Number(l.note_a)||0,rarity:l.rarity,skin:l.skin,hair:l.hair,hair_length:l.hair_length,clubName:((a=l.clubs)==null?void 0:a.encoded_name)||null,clubLogo:((d=l.clubs)==null?void 0:d.logo_url)||null,boost:0,used:!1,_line:p})}const g=at(f.length);f.forEach((y,l)=>{y._col=g[l]}),c[p]=f}return c}function Po(e){const t=nt[e]||nt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let c=0;for(const s of["GK","DEF","MIL","ATT"]){const a=[];for(let p=0;p<t[s];p++){const o=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+c,id:"fake-"+c,firstname:"IA",name:n[c%n.length],country_code:"XX",club_id:null,job:s,job2:null,note_g:s==="GK"?o:2,note_d:s==="DEF"?o:2,note_m:s==="MIL"?o:2,note_a:s==="ATT"?o:2,rarity:"normal",boost:0,used:!1,_line:s}),c++}const d=at(a.length);a.forEach((p,o)=>{p._col=d[o]}),i[s]=a}return i}function Oo(e,t,i){e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
      ...
      <div class="duel-score-line duel-score-line-${h}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
        Score: ${s(l)} + ${a(l)} liens = <b style="color:#fff">${s(l)+a(l)}</b>
      </div>
    </div>`}e.innerHTML=`
  ...
  `,document.body.appendChild(n);let r=!1;const u=()=>{r||(r=!0,n.remove(),i())};n.addEventListener("click",u),setTimeout(u,2e3)}function ct(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function Ct(e,t,i,n=null,c=null){var g,y;if(t.phase!=="attack"){ct("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){ct(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const s=Object.entries(t.homeTeam).flatMap(([l,x])=>(x||[]).filter(v=>v.used).map(v=>({...v,_line:v._line||l}))),a=t.homeSubs.filter(l=>!t.usedSubIds.includes(l.cardId));if(!s.length){ct("Aucun joueur utilisé à remplacer");return}if(!a.length){ct("Aucun remplaçant disponible");return}let d=Math.max(0,s.findIndex(l=>l.cardId===c));const p=((g=s[d])==null?void 0:g._line)||((y=s[d])==null?void 0:y.job);let o=n?Math.max(0,a.findIndex(l=>l.cardId===n)):Math.max(0,a.findIndex(l=>l.job===p)),r=!1;const u=document.createElement("div");u.id="sub-overlay",u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function f(){var _,w,S,ee,re,te;const l=s[d],x=a[o],v=Math.min(130,Math.round((window.innerWidth-90)/2)),h=Math.round(v*1.35),I=$=>`background:rgba(255,255,255,0.12);border:none;color:${$?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${$?"default":"pointer"};flex-shrink:0`;u.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <button id="in-down" style="${I(o>=a.length-1)}" ${o>=a.length-1?"disabled":""}>▼</button>
    ...
    </div>`,document.body.appendChild(g),(y=g.querySelector("#gc-back"))==null||y.addEventListener("click",()=>g.remove()),(l=g.querySelector("#gc-use"))==null||l.addEventListener("click",()=>{g.remove(),Zo(e,t,i,n,c)})}function $t(e,t,i,n,c,s){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let d=[];function p(){var o,r;a.innerHTML=`
    ...
    </div>`,(o=a.querySelector("#gc-picker-close"))==null||o.addEventListener("click",()=>a.remove()),a.querySelectorAll(".gc-pick-item").forEach(u=>{u.addEventListener("click",()=>{const f=u.dataset.cid,g=e.find(l=>l.cardId===f);if(!g)return;const y=d.findIndex(l=>l.cardId===f);y>-1?d.splice(y,1):d.length<t&&d.push(g),p()})}),(r=a.querySelector("#gc-picker-confirm"))==null||r.addEventListener("click",()=>{a.remove(),s(d)})}p(),document.body.appendChild(a)}const Qo={BOOST_STAT:({value:e=1,count:t=1,roles:i=[]},n,c,s)=>{const a=Object.entries(n.homeTeam).filter(([d])=>!i.length||i.includes(d)).flatMap(([d,p])=>p.filter(o=>!o.used).map(o=>({...o,_line:d})));return a.length?($t(a,t,`Choisir ${t} joueur(s) à booster (+${e})`,c,n,d=>{d.forEach(p=>{const o=(n.homeTeam[p._line]||[]).find(r=>r.cardId===p.cardId);o&&(o.boost=(o.boost||0)+e,n.log.push({text:`⚡ +${e} sur ${o.name}`,type:"info"}))}),_e(c,n,s)}),!0):(n.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),_e(c,n,s),!0)},DEBUFF_STAT:({value:e=1,count:t=1,roles:i=[],target:n="ai"},c,s,a)=>{const d=n==="home"?c.homeTeam:c.aiTeam,p=n==="ai"?"adverse":"allié",o=Object.entries(d).filter(([r])=>!i.length||i.includes(r)).flatMap(([r,u])=>u.filter(f=>!f.used).map(f=>({...f,_line:r})));return o.length?($t(o,t,`Choisir ${t} joueur(s) ${p}(s) à débuffer (-${e})`,s,c,r=>{r.forEach(u=>{const g=((n==="home"?c.homeTeam:c.aiTeam)[u._line]||[]).find(y=>y.cardId===u.cardId);g&&(g.boost=(g.boost||0)-e,c.log.push({text:`🎯 -${e} sur ${g.name}${n==="ai"?" (IA)":""}`,type:"info"}))}),_e(s,c,a)}),!0):(c.log.push({text:`🎯 Aucun joueur ${p} disponible`,type:"info"}),_e(s,c,a),!0)},GRAY_PLAYER:({count:e=1,roles:t=[],target:i="ai"},n,c,s)=>{const a=i==="home"?n.homeTeam:n.aiTeam,d=i==="ai"?"adverse":"allié",p=Object.entries(a).filter(([o])=>!t.length||t.includes(o)).flatMap(([o,r])=>r.filter(u=>!u.used).map(u=>({...u,_line:o})));return p.length?($t(p,e,`Choisir ${e} joueur(s) ${d}(s) à exclure`,c,n,o=>{o.forEach(r=>{const f=((i==="home"?n.homeTeam:n.aiTeam)[r._line]||[]).find(g=>g.cardId===r.cardId);f&&(f.used=!0,n.log.push({text:`❌ ${f.name}${i==="ai"?" (IA)":""} exclu !`,type:"info"}))}),_e(c,n,s)}),!0):(n.log.push({text:`❌ Aucun joueur ${d} à exclure`,type:"info"}),_e(c,n,s),!0)},REVIVE_PLAYER:({count:e=1,roles:t=[]},i,n,c)=>{const s=Object.entries(i.homeTeam).filter(([a])=>!t.length||t.includes(a)).flatMap(([a,d])=>d.filter(p=>p.used).map(p=>({...p,_line:a})));return s.length?($t(s,e,`Choisir ${e} joueur(s) à ressusciter`,n,i,a=>{a.forEach(d=>{const p=(i.homeTeam[d._line]||[]).find(o=>o.cardId===d.cardId);p&&(p.used=!1,i.log.push({text:`💫 ${p.name} ressuscité !`,type:"info"}))}),_e(n,i,c)}),!0):(i.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),_e(n,i,c),!0)},REMOVE_GOAL:({},e)=>(e.aiScore>0?(e.aiScore--,e.log.push({text:"🚫 Dernier but IA annulé !",type:"info"})):e.log.push({text:"🚫 Aucun but à annuler",type:"info"}),!1),ADD_GOAL_DRAW:({},e)=>(e.homeScore===e.aiScore?(e.homeScore++,e.log.push({text:"🎯 But bonus (match nul) !",type:"info"})):e.log.push({text:"🎯 But bonus : non applicable (pas de match nul)",type:"info"}),!1),ADD_SUB:({value:e=1},t)=>(t.maxSubs=(t.maxSubs||3)+e,t.log.push({text:`🔄 +${e} remplacement(s) débloqué(s)`,type:"info"}),!1),CUSTOM:()=>!1};function Zo(e,t,i,n,c){n.usedGc.push(e);const s=n.gcDefs||[],a=s.find(p=>p.name===t)||s.find(p=>{var o;return((o=p.name)==null?void 0:o.toLowerCase().trim())===(t==null?void 0:t.toLowerCase().trim())});let d=!1;if(a!=null&&a.effect_type&&a.effect_type!=="CUSTOM"){const p=Qo[a.effect_type];p?p(a.effect_params||{},n,i,c)||(d=!0):(c.toast(`Effet "${a.effect_type}" non implémenté`,"error"),d=!0)}else{switch(t){case"Double attaque":n.modifiers.home.doubleAttack=!0,n.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":n.modifiers.home.shield=!0,n.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const p=Object.entries(n.homeTeam).flatMap(([o,r])=>(r||[]).filter(u=>u.used).map(u=>({...u,_line:o})));p.length?(p[0].used=!1,n.log.push({text:`💫 ${p[0].name} ressuscité !`,type:"info"})):n.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":n.modifiers.ai.stolenNote=(n.modifiers.ai.stolenNote||0)+1,n.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const p=[...n.aiTeam.ATT||[],...n.aiTeam.MIL||[]].filter(o=>!o.used);if(p.length){const o=p.sort((r,u)=>me(u,"ATT")-me(r,"ATT"))[0];o.used=!0,n.log.push({text:`❄️ ${o.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":n.maxSubs++,n.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}d=!0}k.from("cards").delete().eq("id",e).then(()=>{}),d&&_e(i,n,c)}function en(e,t,i){const n=Object.values(t.homeTeam).flat().filter(c=>!c.used);if(!n.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(c=>{c.addEventListener("click",()=>{const s=c.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const d=(t.homeTeam[a]||[]).find(p=>p.cardId===s);if(d){d.boost=(d.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${d.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),_e(e,t,i)})})}function Tt(e,t,i,n,c){const s=document.createElement("div");s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const a=Array.from({length:10},(o,r)=>`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(s);let d=!1;const p=()=>{d||(d=!0,s.remove(),setTimeout(()=>c(),50))};s.addEventListener("click",p),setTimeout(p,3500)}async function gt(e,t,i){var r,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,c=t.homeScore>t.aiScore,s=t.homeScore===t.aiScore,a=c?"victoire":s?"nul":"defaite",d=lo(t.mode,a);t.matchId&&await k.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?n.profile.id:null,home_credits_reward:d,played_at:new Date().toISOString()}).eq("id",t.matchId);const p={credits:(n.profile.credits||0)+d,matches_played:(n.profile.matches_played||0)+1};c?p.wins=(n.profile.wins||0)+1:s?p.draws=(n.profile.draws||0)+1:p.losses=(n.profile.losses||0)+1,await k.from("users").update(p).eq("id",n.profile.id),await i.refreshProfile();const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",o.innerHTML=`
    ...
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}async function on(e,t){await Hi(e,t,"random",({deckId:i,formation:n,starters:c,subsRaw:s,gcCardsEnriched:a,gcDefs:d})=>{const p=o=>nn(e,t,i,n,c,s,o,d||[]);if(!a.length){p([]);return}Ni(e,a,p)})}async function nn(e,t,i,n,c,s,a=[],d=[]){var h;const{state:p,navigate:o,toast:r}=t;let u=!1,f=null;Ye(e),e.innerHTML=`
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      card:cards(id, card_type,
        player:players(id, firstname, surname_encoded, country_code, job, job2,
          note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length, clubs(encoded_name, logo_url)))`).eq("seller_id",i.profile.id).in("status",["active","sold"]).order("listed_at",{ascending:!1}).limit(100),a=(c||[]).filter(o=>o.seller_id!==i.profile.id),d=s||[];d.filter(o=>o.status==="active").length,e.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `;function p(o){const r=document.getElementById("mkt-content"),u=o==="buy"?a:d;if(u.length===0){r.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      </div>`,document.body.appendChild(n);const c=s=>{n.remove(),i(s)};n.querySelector("#buy-ok").addEventListener("click",()=>c(!0)),n.querySelector("#buy-cancel").addEventListener("click",()=>c(!1)),n.addEventListener("click",s=>{s.target===n&&c(!1)})})}async function cn(e,t,i){const{toast:n}=i,{data:c}=await k.from("market_listings").select("card_id").eq("id",e).single();await k.from("market_listings").update({status:"cancelled"}).eq("id",e),c&&await k.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",c.card_id),n("Annonce retirée","success"),Qt(t,i)}function pn(e){return{GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[e]||"#888"}async function un(e,{state:t,navigate:i,toast:n}){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:c}=await k.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100);e.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `}async function fn(e,{state:t,navigate:i,toast:n}){const c=t.profile;if(!c)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await k.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
      home:users!home_id(pseudo,club_name),
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${c.id},away_id.eq.${c.id}`).order("created_at",{ascending:!1}).limit(50),a={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},d=(s||[]).filter(r=>r.status==="finished"),p=(s||[]).filter(r=>r.status==="in_progress");function o(r){const u=r.home_id===c.id;u?r.home_score:r.away_score,u?r.away_score:r.home_score;const f=r.winner_id===c.id,g=r.home_score===r.away_score&&r.status==="finished",y=r.status!=="finished"?"…":g?"N":f?"V":"D",l=r.status!=="finished"||g?"#888":f?"#1A6B3C":"#c0392b";let x=a[r.mode]||r.mode;r.away_id===null&&!x.startsWith("IA")&&(x="IA");const h=r.home_id===c.id?r.away:r.home;let I;r.away_id===null?I=x:h?I=`${h.club_name||h.pseudo} (${h.pseudo})`:I="Adversaire";let D="";r.status==="in_progress"&&Date.now()-new Date(r.created_at).getTime()>3600*1e3&&(D=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const _=new Date(r.created_at),w=_.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+_.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),S=r.status==="finished"?`${r.home_score} - ${r.away_score}`:`${r.home_score||0} - ${r.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
