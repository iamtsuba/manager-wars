import{s as $,l as ae,F as Ge,b as xe,c as st,d as Mt}from"./formation-links-CDBKB_z4.js";function Ye(e,{navigate:t,toast:i}){e.innerHTML=`
    ...
    <div class="page-body">
      ...
      <div>
        ...
        <div class="card-panel" style="padding:0">
          ${o.map(r=>{const l=r.winner_id===c.id,s=r.home_score===r.away_score,p=s?"N":l?"V":"D",u=s?"#888":l?"#1A6B3C":"#c0392b",f={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat"}[r.mode]||r.mode,m=new Date(r.created_at).toLocaleDateString("fr",{month:"short",day:"numeric"});return`<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;border-bottom:1px solid var(--gray-200)">
              <div>
  ...
  </div>`}async function Dt(e,t){const{state:i,navigate:a,toast:c,openModal:o,closeModal:n}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await $.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(m=document.getElementById("direct-sell-form-btn"))==null||m.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${l} pour ${ce.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const b=u.find(w=>!w.is_for_sale)||u[0];if(!b){o("Aucune carte à vendre","error");return}const{error:x}=await $.from("cards").delete().eq("id",b.id);if(x){o(x.message,"error");return}await $.from("users").update({credits:(c.profile.credits||0)+ce}).eq("id",c.profile.id),await r(),o(`+${ce.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),n(),d("collection")}),(h=document.getElementById("market-sell-form-btn"))==null||h.addEventListener("click",async()=>{const b=parseInt(document.getElementById("sell-price-form").value);if(!b||b<1){o("Prix invalide","error");return}await $.from("cards").update({is_for_sale:!0,sale_price:b}).eq("id",e.id),await $.from("market_listings").insert({seller_id:c.profile.id,card_id:e.id,price:b}),o("Carte mise en vente sur le marché !","success"),n(),d("collection")}),(_=document.getElementById("cancel-sell-form-btn"))==null||_.addEventListener("click",async()=>{await $.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await $.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),n(),d("collection")})}function Ze(e,t,i,a){var T,g,v,A,z,j;const{state:c,toast:o,openModal:n,closeModal:d,navigate:r,refreshProfile:l}=a,s=e.player,p=t.filter(S=>S.player.id===s.id),u=p.length,f=Ft[s.rarity]||1e3,y=s.rarity!=="legende",m=gt(s),h=fe(s,s.job),_=s.job2?fe(s,s.job2):null,b=Ee[s.job]||"#1A6B3C",x=s.job2?Ee[s.job2]:null,w=ft[s.rarity]||"#ccc",I=Re[s.country_code]||s.country_code||"";n(`${s.firstname} ${s.surname_encoded}`,`<div style="display:flex;gap:16px;flex-wrap:wrap;justify-content:center">
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const n=prompt("Nom du deck :",`Deck ${((o==null?void 0:o.length)||0)+1}`);if(!n)return;const{data:d,error:r}=await $.from("decks").insert({owner_id:i.profile.id,name:n}).select().single();if(r){c(r.message,"error");return}c("Deck créé !","success"),Qe(d.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(n=>{n.addEventListener("click",()=>Qe(n.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(n=>{n.addEventListener("click",async()=>{const d=prompt("Nouveau nom :",n.dataset.name);if(!d||d===n.dataset.name)return;const{error:r}=await $.from("decks").update({name:d}).eq("id",n.dataset.rename);if(r){c(r.message,"error");return}c("Deck renommé !","success"),ze(e,t)})}),e.querySelectorAll("[data-delete]").forEach(n=>{n.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${n.dataset.name}" ? Cette action est irréversible.`))return;await $.from("deck_cards").delete().eq("deck_id",n.dataset.delete);const{error:d}=await $.from("decks").delete().eq("id",n.dataset.delete);if(d){c(d.message,"error");return}c("Deck supprimé.","success"),ze(e,t)})})}async function Qe(e,t,i){const{state:a,toast:c}=i;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await $.from("decks").select("*").eq("id",e).single(),{data:n}=await $.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",a.profile.id),d=(n||[]).filter(f=>f.card_type==="player"&&f.player),r=(n||[]).filter(f=>f.card_type==="formation"),l=r.map(f=>f.formation).filter(Boolean),{data:s}=await $.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let p=o.formation||"4-4-2";l.length>0&&!l.includes(p)&&(p=l[0]);const u={deckId:e,name:o.name,formation:p,formationCardId:o.formation_card_id,slots:{},subs:[],playerCards:d,formationCards:r,availableFormations:l};(s||[]).forEach(f=>{f.is_starter?u.slots[f.position]=f.card_id:u.subs.includes(f.card_id)||u.subs.push(f.card_id)}),se(t,u,i)}function se(e,t,i){var r;const{navigate:a}=i;Ae[t.formation];const c=et(t.formation),o=c.filter(l=>t.slots[l]).length,n=t.availableFormations.length>0?t.availableFormations:Object.keys(Ae),d=t.subs.map(l=>t.playerCards.find(s=>s.id===l)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div class="page">
  ...
  </div>`,Nt(e,t,c,i),document.getElementById("builder-back").addEventListener("click",()=>a("decks")),document.getElementById("formation-select").addEventListener("change",l=>{t.formation=l.target.value;const s=et(t.formation),p={};s.forEach(u=>{t.slots[u]&&(p[u]=t.slots[u])}),t.slots=p,se(e,t,i)}),document.getElementById("save-deck").addEventListener("click",()=>Ht(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(l=>{l.addEventListener("click",()=>{t.subs=t.subs.filter(s=>s!==l.dataset.removeSub),se(e,t,i)})}),(r=document.getElementById("add-sub-btn"))==null||r.addEventListener("click",()=>{Pt(t,e,i)})}function Nt(e,t,i,a){const c=e.querySelector("#deck-field");if(!c)return;const o=xe[t.formation]||{},n=st(t.formation)||[],d={};for(const _ of i){const b=t.slots[_],x=b?t.playerCards.find(w=>w.id===b):null;d[_]=x?x.player:null}const r=300,l=300,s=48,p=64,u=13,f=16,y=38;function m(_){const b=o[_];return b?{x:b.x*r,y:b.y*l}:null}let h="";for(const[_,b]of n){const x=m(_),w=m(b);if(!x||!w)continue;const I=d[_]?{...d[_],club_id:d[_].club_id,country_code:d[_].country_code,rarity:d[_].rarity}:null,T=d[b]?{...d[b],club_id:d[b].club_id,country_code:d[b].country_code,rarity:d[b].rarity}:null,g=ae(I,T);g==="#ff3333"||g==="#cc2222"?h+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${w.x.toFixed(1)}" y2="${w.y.toFixed(1)}" stroke="${g}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(h+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${w.x.toFixed(1)}" y2="${w.y.toFixed(1)}" stroke="${g}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,h+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${w.x.toFixed(1)}" y2="${w.y.toFixed(1)}" stroke="${g}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const _ of i){const b=m(_);if(!b)continue;const x=d[_],w=_.replace(/\d+/,""),I=te[w]||"#555",T=(b.x-s/2).toFixed(1),g=(b.y-p/2).toFixed(1),v={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[x==null?void 0:x.rarity]||"#aaa";if(x){const A=Te(x),z=mt(x),j=xt(x.country_code),S=Number(w==="GK"?x.note_g:w==="DEF"?x.note_d:w==="MIL"?x.note_m:x.note_a)||0,D=p-u-f;h+=`<defs><clipPath id="dcp-${_}"><rect x="${T}" y="${(b.y-p/2+u).toFixed(1)}" width="${s}" height="${D}"/></clipPath></defs>`,h+=`<rect x="${T}" y="${g}" width="${s}" height="${p}" rx="5" fill="${I}"/>`,A&&(h+=`<image href="${A}" x="${T}" y="${(b.y-p/2+u).toFixed(1)}" width="${s}" height="${D}" clip-path="url(#dcp-${_})" preserveAspectRatio="xMidYMin slice"/>`),h+=`<rect x="${T}" y="${g}" width="${s}" height="${u}" fill="rgba(255,255,255,0.93)"/>`,h+=`<text x="${b.x.toFixed(1)}" y="${(b.y-p/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(x.surname_encoded||"").slice(0,9)}</text>`;const q=(b.y+p/2-f).toFixed(1);h+=`<rect x="${T}" y="${q}" width="${s}" height="${f}" fill="rgba(255,255,255,0.93)"/>`,j&&(h+=`<image href="${j}" x="${(b.x-21).toFixed(1)}" y="${(b.y+p/2-f+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),h+=`<text x="${b.x.toFixed(1)}" y="${(b.y+p/2-f/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${S}</text>`,z&&(h+=`<image href="${z}" x="${(b.x+s/2-14).toFixed(1)}" y="${(b.y+p/2-f+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),h+=`<rect x="${T}" y="${g}" width="${s}" height="${p}" rx="5" fill="none" stroke="${v}" stroke-width="2"/>`}else h+=`<rect x="${T}" y="${g}" width="${s}" height="${p}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,h+=`<text x="${b.x.toFixed(1)}" y="${b.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,h+=`<text x="${b.x.toFixed(1)}" y="${(b.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${w}</text>`;h+=`<rect x="${T}" y="${g}" width="${s}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${_}" style="cursor:pointer"/>`}c.innerHTML=`<svg viewBox="${-y} ${-y} ${r+y*2} ${l+y*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${h}</svg>`,c.querySelectorAll(".deck-slot-hit").forEach(_=>{_.addEventListener("click",()=>Ot(_.dataset.pos,t,e,a))})}function Ot(e,t,i,a){var u,f,y;const{openModal:c,closeModal:o}=a,n=e.replace(/\d+/,""),d=t.slots[e],r=d?t.playerCards.find(m=>m.id===d):null;(u=r==null?void 0:r.player)==null||u.id;const l=new Set;Object.entries(t.slots).forEach(([m,h])=>{var b;if(m===e||!h)return;const _=t.playerCards.find(x=>x.id===h);(b=_==null?void 0:_.player)!=null&&b.id&&l.add(_.player.id)}),t.subs.forEach(m=>{var _;const h=t.playerCards.find(b=>b.id===m);(_=h==null?void 0:h.player)!=null&&_.id&&l.add(h.player.id)});const s=new Set,p=t.playerCards.filter(m=>{const h=m.player;return!(h.job===n||h.job2===n)||l.has(h.id)||s.has(h.id)?!1:(s.add(h.id),!0)});p.sort((m,h)=>{const _=n==="GK"?m.player.note_g:n==="DEF"?m.player.note_d:n==="MIL"?m.player.note_m:m.player.note_a;return(n==="GK"?h.player.note_g:n==="DEF"?h.player.note_d:n==="MIL"?h.player.note_m:h.player.note_a)-_}),c(`Choisir ${n} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${p.length>0?p.map(m=>{var w,I;const h=m.player,_=n==="GK"?h.note_g:n==="DEF"?h.note_d:n==="MIL"?h.note_m:h.note_a,b=Te(h),x={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[h.rarity];return`<div class="player-option" data-card-id="${m.id}"
  ...
  `,document.body.appendChild(a);let c=!1;document.getElementById("pack-visual").addEventListener("click",()=>{if(c)return;c=!0;const f=document.getElementById("pack-visual");f.classList.add("shaking"),setTimeout(()=>{f.classList.add("pack-open"),setTimeout(()=>{document.getElementById("pack-phase").style.display="none",n(0)},600)},500)});let o=0;function n(f){o=f;const y=document.getElementById("reveal-phase");y.style.display="flex",d(f)}function d(f){var T;const y=e[f],m=document.getElementById("card-counter"),h=document.getElementById("single-card-slot"),_=document.getElementById("card-tap-hint");m&&(m.textContent=`Carte ${f+1} / ${e.length}`),_&&(_.textContent=f<e.length-1?"Appuie pour continuer →":"Appuie pour voir toutes tes cartes");const b=it(y),x=y.card_type==="player"&&((T=y.player)==null?void 0:T.rarity)==="legende";h.innerHTML=`
      ...
      </div>`,x&&s();const w=document.getElementById("current-card-wrap");let I=!1;w.addEventListener("click",()=>{if(I)return;I=!0;const g=f+1;g<e.length?(w.style.transition="all 0.25s ease",w.style.transform="translateX(-120%) rotate(-15deg)",w.style.opacity="0",setTimeout(()=>d(g),250)):r()})}function r(){p(),document.getElementById("reveal-phase").style.display="none";const f=document.getElementById("recap-phase");f.style.display="flex";const y=document.getElementById("recap-grid");y.innerHTML=e.map((m,h)=>`
    ...
    </div>`}if(e.card_type==="game_changer"){const o=e._gcDef,n=(o==null?void 0:o.gc_type)==="ultra_game_changer",d={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},r={purple:"#b06ce0",light_blue:"#00d4ef"},l=d[o==null?void 0:o.color]||d.purple,s=r[o==null?void 0:o.color]||r.purple,p=(o==null?void 0:o.name)||e.gc_type||"Game Changer",u=(o==null?void 0:o.effect)||((a=Ce[e.gc_type])==null?void 0:a.desc)||"",f=o!=null&&o.image_url?`/manager-wars/icons/${o.image_url}`:null,y=((c=Ce[e.gc_type])==null?void 0:c.icon)||"⚡";return`<div style="width:170px;height:240px;background:${l};border-radius:14px;border:3px solid ${s};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${s}88;flex-shrink:0">
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const ui="simulation",bt="VOTRE_ZONE_ID";function fi(){switch(ui){case"propellerads":return gi();case"adsense":return xi();default:return qe()}}function qe(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
    ...
    <div style="text-align:center;padding:40px;color:#fff">
  ...
  </div>`,(o=document.getElementById("msg-btn"))==null||o.addEventListener("click",c)}function W(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function je(e){var i,a;const t=e.player;return{cardId:e.id,id:t.id,firstname:t.firstname,name:t.surname_encoded,country_code:t.country_code,club_id:t.club_id,job:t.job,job2:t.job2,note_g:Number(t.note_g)||0,note_d:Number(t.note_d)||0,note_m:Number(t.note_m)||0,note_a:Number(t.note_a)||0,rarity:t.rarity,skin:t.skin,hair:t.hair,hair_length:t.hair_length,clubName:((i=t.clubs)==null?void 0:i.encoded_name)||null,clubLogo:((a=t.clubs)==null?void 0:a.logo_url)||null,boost:0,used:!1,_line:null,_col:null}}function Ne(e){return e===1?[1]:e===2?[0,2]:e===3?[0,1,2]:e===4?[0,1,1,2]:e===5?[0,1,1,1,2]:[1]}function mi(){const e=Math.random()*100;return e<.1?4:e<5?3:e<20?2:1}function wt(e,t){const i=de[t]||de["4-4-2"],a={GK:[],DEF:[],MIL:[],ATT:[]},c=[...e];for(const o of["GK","DEF","MIL","ATT"]){const n=[];for(let r=0;r<i[o];r++){let l=c.findIndex(s=>s.job===o);if(l===-1&&(l=c.findIndex(s=>s.job2===o)),l===-1&&(l=0),c[l]){const s={...c[l],_line:o};n.push(s),c.splice(l,1)}}const d=Ne(n.length);n.forEach((r,l)=>{r._col=d[l]}),a[o]=n}return a}async function yi(e,t){var n,d;const{data:i}=await $.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return hi(e);const a=de[e]||de["4-4-2"],c={GK:[],DEF:[],MIL:[],ATT:[]},o=[...i];for(const r of["GK","DEF","MIL","ATT"]){const l=o.filter(y=>y.job===r),s=o.filter(y=>y.job!==r),p=[...l,...s],u=[];for(let y=0;y<a[r];y++){const m=p[y]||o[y];m&&u.push({cardId:"ai-"+m.id+"-"+y,id:m.id,firstname:m.firstname,name:m.surname_encoded,country_code:m.country_code,club_id:m.club_id,job:m.job,job2:m.job2,note_g:Number(m.note_g)||0,note_d:Number(m.note_d)||0,note_m:Number(m.note_m)||0,note_a:Number(m.note_a)||0,rarity:m.rarity,skin:m.skin,hair:m.hair,hair_length:m.hair_length,clubName:((n=m.clubs)==null?void 0:n.encoded_name)||null,clubLogo:((d=m.clubs)==null?void 0:d.logo_url)||null,boost:0,used:!1,_line:r})}const f=Ne(u.length);u.forEach((y,m)=>{y._col=f[m]}),c[r]=u}return c}function hi(e){const t=de[e]||de["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},a=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let c=0;for(const o of["GK","DEF","MIL","ATT"]){const n=[];for(let r=0;r<t[o];r++){const l=3+Math.floor(Math.random()*5);n.push({cardId:"fake-"+c,id:"fake-"+c,firstname:"IA",name:a[c%a.length],country_code:"XX",club_id:null,job:o,job2:null,note_g:o==="GK"?l:2,note_d:o==="DEF"?l:2,note_m:o==="MIL"?l:2,note_a:o==="ATT"?l:2,rarity:"normal",boost:0,used:!1,_line:o}),c++}const d=Ne(n.length);n.forEach((r,l)=>{r._col=d[l]}),i[o]=n}return i}async function bi(e,t){var T;const{state:i,navigate:a,toast:c}=t,o=i.params||{};e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const n=o.matchMode||"vs_ai_easy",d=n.replace("vs_ai_",""),r=n;if(!o.deckId)return wi(e,t,n);const l=o.deckId,[{data:s},{data:p}]=await Promise.all([$.from("decks").select("formation,name").eq("id",l).single(),$.from("deck_cards").select(`position, is_starter, slot_order,
        card:cards(id, card_type, formation,
          player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
            note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
            clubs(encoded_name,logo_url)))`).eq("deck_id",l).order("slot_order")]),u=(p||[]).filter(g=>{var v;return g.is_starter&&((v=g.card)==null?void 0:v.player)}).map(g=>je(g.card)),f=(p||[]).filter(g=>{var v;return!g.is_starter&&((v=g.card)==null?void 0:v.player)}).map(g=>je(g.card));if(u.length<11){vt(e,"⚠️",`Deck incomplet (${u.length}/11).`,"Compléter",()=>a("decks"));return}const y=(p||[]).find(g=>{var v;return((v=g.card)==null?void 0:v.card_type)==="formation"}),m=(s==null?void 0:s.formation)||((T=y==null?void 0:y.card)==null?void 0:T.formation)||"4-4-2",{data:h}=await $.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",i.profile.id).eq("card_type","game_changer"),{data:_}=await $.from("gc_definitions").select("*").eq("is_active",!0),b=(h||[]).map(g=>({...g,_gcDef:(_==null?void 0:_.find(v=>v.name===g.gc_type||v.id===g.gc_definition_id))||null})),x=wt(u,m),w=await yi(m),I=async g=>{const{data:v}=await $.from("matches").insert({home_id:i.profile.id,away_id:null,mode:r,home_deck_id:l,status:"in_progress"}).select().single(),A={gcDefs:_||[],matchId:v==null?void 0:v.id,mode:r,difficulty:d,formation:m,homeTeam:x,aiTeam:w,homeSubs:f,subsUsed:0,maxSubs:Math.min(f.length,3),homeScore:0,aiScore:0,gcCards:g,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};$i(e,A,t)};if(!b.length){I([]);return}vi(e,b,I)}function vi(e,t,i){const c=new Set,o=t.filter(s=>{const p=s.gc_type||s.id;return c.has(p)?!1:(c.add(p),!0)});let n=[];function d(s,p){const u=s._gcDef,f={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},y={purple:"#9b59b6",light_blue:"#00bcd4"},m=f[u==null?void 0:u.color]||f.purple,h=y[u==null?void 0:u.color]||y.purple;return`<div class="gc-select-card" data-id="${s.id}"
    ...
    </div>`}function r(){var f,y,m;const s=e.querySelector("#gc-confirm-overlay");if(s&&s.remove(),n.length<3)return;const p=document.createElement("div");p.id="gc-confirm-overlay",p.style.cssText="position:absolute;inset:0;background:rgba(10,22,40,0.88);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:32px 24px;z-index:20;backdrop-filter:blur(4px)",p.innerHTML=`
      ...
      <button id="gc-reset" style="width:100%;max-width:320px;padding:14px;border-radius:14px;border:2px solid rgba(212,160,23,0.4);background:rgba(212,160,23,0.1);color:#D4A017;font-size:14px;font-weight:700;cursor:pointer">
        🔄 Changer de GC
      </button>`,(f=p.querySelector("#gc-launch"))==null||f.addEventListener("click",()=>i(n)),(y=p.querySelector("#gc-no-gc"))==null||y.addEventListener("click",()=>i([])),(m=p.querySelector("#gc-reset"))==null||m.addEventListener("click",()=>{n=[],l()});const u=e.querySelector("#gc-screen-wrap");u?u.appendChild(p):e.appendChild(p)}function l(){e.style.overflow="hidden";const s=e.clientHeight||e.getBoundingClientRect().height;s>50&&(e.style.height=s+"px"),e.innerHTML=`
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(p=>{p.addEventListener("click",()=>{const u=p.dataset.id,f=o.find(m=>m.id===u);if(!f)return;const y=n.findIndex(m=>m.gc_type===f.gc_type);y>-1?n.splice(y,1):n.length<3&&n.push(f),l(),n.length===3&&r()})}),n.length===3&&r()}l()}async function wi(e,t,i){const{state:a,navigate:c}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await $.from("decks").select("id,name,is_active,formation").eq("owner_id",a.profile.id).order("created_at",{ascending:!1});if(!o||o.length===0){vt(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>c("decks"));return}const n=o.map(s=>s.id),{data:d}=await $.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",n).order("slot_order");let r=0;function l(){var b,x,w,I,T;const s=o[r],p=(d||[]).filter(g=>g.deck_id===s.id),u=p.filter(g=>{var v;return g.is_starter&&((v=g.card)==null?void 0:v.player)}).map(g=>je(g.card)),f=p.find(g=>{var v;return((v=g.card)==null?void 0:v.card_type)==="formation"}),y=s.formation||((b=f==null?void 0:f.card)==null?void 0:b.formation)||"4-4-2",m=u.length>=11?wt(u,y):null,h=u.length>=11;e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:calc(100dvh - 130px);overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;align-items:center;gap:8px;padding:8px;flex-shrink:0">
        ...
        <div style="flex:1;text-align:center">
          ...
          <div style="font-size:11px;opacity:.6;margin-top:2px">${y} · ${u.length}/11 ${s.is_active?"· ⭐ Actif":""}</div>
        </div>
  ...
  </div>`,setTimeout(()=>_i(e,t,i),5e3)}function _i(e,t,i){const a=t.homeTeam.MIL||[],c=t.aiTeam.MIL||[];function o(p){return p.reduce((u,f)=>u+H(f,"MIL"),0)}function n(p){let u=0;for(let f=0;f<p.length-1;f++){const y=ae(p[f],p[f+1]);y==="#00ff88"?u+=2:y==="#FFD700"&&(u+=1)}return u}const d=o(a)+n(a),r=o(c)+n(c),l=d>=r;function s(p,u,f){return`<div style="text-align:center">
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
  ...
  </div>`,setTimeout(()=>{const p=document.getElementById("score-home"),u=document.getElementById("score-ai"),f=document.getElementById("midfield-result");if(p&&u&&(l?(p.style.fontSize="80px",p.style.color="#FFD700",u.style.opacity="0.25"):(u.style.fontSize="80px",u.style.color="#ff6b6b",p.style.opacity="0.25")),f){const y=mi();f.style.opacity="1",l&&t.clubName,f.innerHTML=`
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${p}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${c?`<img src="${c}" style="width:${p-4}px;height:${p-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,p-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
        ...
        stroke="${v}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const u=48,f=64,y=13,m=16,h={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[b,x]of Object.entries(r)){const w=s(b);if(!w||!x)continue;const I=b.replace(/[0-9]/g,""),T=le[I]||"#555",g=i==="attack"&&["MIL","ATT"].includes(I)&&!x.used||i==="defense"&&["GK","DEF","MIL"].includes(I)&&!x.used,v=a.includes(x.cardId);let A;i==="attack"?A=I==="MIL"?Number(x.note_m)||0:Number(x.note_a)||0:i==="defense"?A=I==="GK"?Number(x.note_g)||0:I==="MIL"?Number(x.note_m)||0:Number(x.note_d)||0:A=Number(I==="GK"?x.note_g:I==="DEF"?x.note_d:I==="MIL"?x.note_m:x.note_a)||0,A=A+(x.boost||0);const z=(w.x-u/2).toFixed(1),j=(w.y-f/2).toFixed(1),S=x.used?.25:1,D=h[x==null?void 0:x.rarity]||h.normal,q=v?"#ff3030":D,U=v?3:(x==null?void 0:x.rarity)==="légende"||(x==null?void 0:x.rarity)==="pépite"?2.5:2,Q=f-y-m;p+=`<defs><clipPath id="cp-${b}"><rect x="${z}" y="${(w.y-f/2+y).toFixed(1)}" width="${u}" height="${Q}"/></clipPath></defs>`,p+=`<rect x="${z}" y="${j}" width="${u}" height="${f}" rx="5" fill="${T}" opacity="${S}"/>`;const X=W(x);X&&!x.used&&(p+=`<image href="${X}" x="${z}" y="${(w.y-f/2+y).toFixed(1)}" width="${u}" height="${Q}" clip-path="url(#cp-${b})" preserveAspectRatio="xMidYMin slice"/>`),p+=`<rect x="${z}" y="${j}" width="${u}" height="${y}" rx="4" fill="rgba(255,255,255,0.92)"/>`,p+=`<text x="${w.x.toFixed(1)}" y="${(w.y-f/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(x.name||"").slice(0,9)}</text>`;const V=(w.y+f/2-m).toFixed(1);if(p+=`<rect x="${z}" y="${V}" width="${u}" height="${m}" fill="rgba(255,255,255,0.92)"/>`,x.used)p+=`<text x="${w.x.toFixed(1)}" y="${(w.y+f/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="13" font-weight="900" fill="rgba(0,0,0,0.4)" font-family="Arial Black">–</text>`;else{const me=Be(x.country_code);me?p+=`<image href="${me}" x="${(w.x-20).toFixed(1)}" y="${(w.y+f/2-m+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:p+=`<text x="${(w.x-13).toFixed(1)}" y="${(w.y+f/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${_t(x.country_code)}</text>`,p+=`<text x="${w.x.toFixed(1)}" y="${(w.y+f/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${A}</text>`;const ye=$t(x);ye?p+=`<image href="${ye}" x="${(w.x+u/2-14).toFixed(1)}" y="${(w.y+f/2-m+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:x.clubName&&(p+=`<text x="${(w.x+14).toFixed(1)}" y="${(w.y+f/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(x.clubName||"").slice(0,3).toUpperCase()}</text>`),x.boost&&(p+=`<rect x="${(w.x+u/2-12).toFixed(1)}" y="${(w.y-f/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,p+=`<text x="${(w.x+u/2-5).toFixed(1)}" y="${(w.y-f/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${x.boost}</text>`)}p+=`<rect x="${z}" y="${j}" width="${u}" height="${f}" rx="5" fill="${v?"rgba(255,255,255,0.12)":"none"}" stroke="${q}" stroke-width="${U}" opacity="${S}"/>`,g&&(p+=`<rect x="${z}" y="${j}" width="${u}" height="${f}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${v?"selected":""}" data-card-id="${x.cardId}" data-role="${I}" style="cursor:pointer"/>`)}const _=38;return`<svg viewBox="${-_} ${-_} ${c+_*2} ${o+_*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${p}
  ...
  `,document.body.appendChild(a);let s=!1;const p=()=>{s||(s=!0,a.remove(),i())};a.addEventListener("click",p),setTimeout(p,2e3)}function pe(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function ot(e,t,i,a=null){if(t.phase!=="attack"){pe("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){pe(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const c=Object.values(t.homeTeam).flat().filter(p=>p.used),o=t.homeSubs.filter(p=>!t.usedSubIds.includes(p.cardId));if(!c.length){pe("Aucun joueur utilisé à remplacer");return}if(!o.length){pe("Aucun remplaçant disponible");return}let n=0,d=Math.max(0,o.findIndex(p=>p.cardId===a)),r=!1;const l=document.createElement("div");l.id="sub-overlay",l.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function s(){var _,b,x,w,I,T;const p=c[n],u=o[d],f=Math.min(130,Math.round((window.innerWidth-90)/2)),y=Math.round(f*1.35),m=g=>`background:rgba(255,255,255,0.12);border:none;color:${g?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${g?"default":"pointer"};flex-shrink:0`;l.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <button id="in-up" style="${m(d===0)}" ${d===0?"disabled":""}>▲</button>
        <div>${u?ne({...u,used:!1,boost:0},f,y):"<div>—</div>"}</div>
        <button id="in-down" style="${m(d>=o.length-1)}" ${d>=o.length-1?"disabled":""}>▼</button>
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
      ${e.map(p=>{const u=p._line||p.job||"MIL",f={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[u]||"#555",y=H(p,u)+(p.boost||0),m=d.find(h=>h.cardId===p.cardId);return`<div class="gc-pick-item" data-cid="${p.cardId}"
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(o);let d=!1;const r=()=>{d||(d=!0,o.remove(),setTimeout(()=>c(),50))};o.addEventListener("click",r),setTimeout(r,3500)}async function re(e,t,i){var s,p;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:a}=i,c=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,n=c?"victoire":o?"nul":"defaite",d=Bt(t.mode,n);t.matchId&&await $.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?a.profile.id:null,home_credits_reward:d,played_at:new Date().toISOString()}).eq("id",t.matchId);const r={credits:(a.profile.credits||0)+d,matches_played:(a.profile.matches_played||0)+1};c?r.wins=(a.profile.wins||0)+1:o?r.draws=(a.profile.draws||0)+1:r.losses=(a.profile.losses||0)+1,await $.from("users").update(r).eq("id",a.profile.id),await i.refreshProfile();const l=document.createElement("div");l.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",l.innerHTML=`
  ...
  `;function d(r){const l=document.getElementById("mkt-content"),s=r==="buy"?n:o;if(s.length===0){l.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${s.map(p=>{var h,_,b;const u=(h=p.card)==null?void 0:h.player;if(!u)return"";const f=u.job==="GK"?u.note_g:u.job==="DEF"?u.note_d:u.job==="MIL"?u.note_m:u.note_a,y=Bi[u.rarity],m=(i.profile.credits||0)>=p.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px">
  ...
  `}async function qi(e,{state:t,navigate:i,toast:a}){const c=t.profile;if(!c)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await $.from("matches").select("id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at").or(`home_id.eq.${c.id},away_id.eq.${c.id}`).order("created_at",{ascending:!1}).limit(50),n={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},d=(o||[]).filter(s=>s.status==="finished"),r=(o||[]).filter(s=>s.status==="in_progress");function l(s){const p=s.home_id===c.id;p?s.home_score:s.away_score,p?s.away_score:s.home_score;const u=s.winner_id===c.id,f=s.home_score===s.away_score&&s.status==="finished",y=s.status!=="finished"?"…":f?"N":u?"V":"D",m=s.status!=="finished"||f?"#888":u?"#1A6B3C":"#c0392b";let h=n[s.mode]||s.mode;s.away_id===null&&!h.startsWith("IA")&&(h="IA");let _="";s.status==="in_progress"&&Date.now()-new Date(s.created_at).getTime()>3600*1e3&&(_=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const b=new Date(s.created_at),x=b.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+b.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),w=s.status==="finished"?`${s.home_score} - ${s.away_score}`:`${s.home_score||0} - ${s.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
