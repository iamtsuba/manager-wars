import{s as A,l as Ze,j as Ii,F as oi,h as zt,i as Zi,k as en,b as tn}from"./formation-links-CEzN0rcz.js";function xi(e,{navigate:t,toast:n}){e.innerHTML=`
  ...
  `;function l(){var $;const i=document.getElementById("logo-preview"),r=document.getElementById("logo-initials"),f=(($=document.getElementById("setup-club"))==null?void 0:$.value)||"MW",m=f.trim().split(" ").filter(Boolean),g=m.length>=2?(m[0][0]+m[1][0]).toUpperCase():f.slice(0,2).toUpperCase();i&&(i.style.background=d,i.style.borderColor=s),r&&(r.textContent=g,r.style.color=s)}document.getElementById("color1").addEventListener("input",i=>{s=i.target.value,document.getElementById("swatch1").style.background=s,l()}),document.getElementById("color2").addEventListener("input",i=>{d=i.target.value,document.getElementById("swatch2").style.background=d,l()});function c(i){document.querySelectorAll(".setup-step").forEach(r=>r.classList.remove("active")),document.getElementById(`step-${i}`).classList.add("active"),document.getElementById("step-num").textContent=i,document.getElementById("progress-fill").style.width=`${Math.round(i/3*100)}%`,i===3&&l()}document.getElementById("step1-next").addEventListener("click",async()=>{const i=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("step1-error");if(r.textContent="",i.length<3){r.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await A.from("users").select("id").eq("pseudo",i).maybeSingle();if(f){r.textContent="Ce pseudo est déjà pris.";return}c(2)}),document.getElementById("step2-back").addEventListener("click",()=>c(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const i=document.getElementById("setup-club").value.trim(),r=document.getElementById("step2-error");if(r.textContent="",i.length<2){r.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await A.from("users").select("id").eq("club_name",i).maybeSingle();if(f){r.textContent="Ce nom de club est déjà pris.";return}c(3)}),document.getElementById("step3-back").addEventListener("click",()=>c(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const i=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),m=document.getElementById("step3-finish");f.textContent="",m.disabled=!0,m.textContent="Création en cours…";try{const{error:g}=await A.from("users").insert({id:t.user.id,pseudo:i,club_name:r,club_color1:s,club_color2:d,credits:1e4});if(g)throw g;await on(t.user.id),await p(),a(`Bienvenue ${i} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(g){f.textContent=g.message,m.disabled=!1,m.textContent="🚀 Créer mon profil !"}})}async function on(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await A.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(n){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",n)}}const Tt="#1A6B3C",It="#cc2222",rn="#D4A017",yi="#888";async function an(e,t){const{state:n,toast:a}=t;e.innerHTML=`
    ...
    ${ai()}`,document.body.appendChild(n);const a=n.querySelector("#friend-pseudo-input"),p=n.querySelector("#add-friend-error"),s=()=>n.remove();a.focus(),n.querySelector("#add-cancel").addEventListener("click",s),n.addEventListener("click",d=>{d.target===n&&s()}),n.querySelector("#add-ok").addEventListener("click",async()=>{const d=a.value.trim();if(!d){p.textContent="Entre un pseudo";return}p.textContent="";const{data:l}=await A.from("users").select("id, pseudo").ilike("pseudo",d).single();if(!l){p.textContent="Utilisateur introuvable";return}if(l.id===e.user.id){p.textContent="Tu ne peux pas t'ajouter toi-même";return}const{data:c}=await A.from("friendships").select("id, status").or(`and(requester_id.eq.${e.user.id},addressee_id.eq.${l.id}),and(requester_id.eq.${l.id},addressee_id.eq.${e.user.id})`).single();if(c){const r=c.status==="accepted"?"Vous êtes déjà amis !":c.status==="pending"?"Demande déjà envoyée":"Une demande existe déjà";p.textContent=r;return}const{error:i}=await A.from("friendships").insert({requester_id:e.user.id,addressee_id:l.id,status:"pending"});if(i){p.textContent="Erreur : "+i.message;return}s(),t(`✅ Demande envoyée à ${l.pseudo} !`,"success")})}async function zi(e,t,n=null,a={}){const p=e.user.id,{data:s}=await A.from("friendships").select("id, requester_id").eq("addressee_id",p).eq("status","pending").order("created_at",{ascending:!1}),d=(s||[]).map(m=>m.requester_id);let l={};if(d.length){const{data:m}=await A.from("users").select("id, pseudo, club_name").in("id",d);(m||[]).forEach(g=>{l[g.id]=g})}const c=(s||[]).map(m=>({...m,requester:l[m.requester_id]||{pseudo:"?"}})),i=ri(),r=c||[];i.innerHTML=`
    <div class="popup-box">
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach($=>{$.addEventListener("click",()=>{const o=$.dataset.id,_=s.find(z=>z.id===o);if(!_)return;const M=d.findIndex(z=>z.gc_type===_.gc_type);M>-1?d.splice(M,1):d.length<3&&d.push(_),i()})}),(f=e.querySelector("#gc-launch"))==null||f.addEventListener("click",()=>{r&&c(d)}),(m=e.querySelector("#gc-no-gc"))==null||m.addEventListener("click",()=>c([])),(g=e.querySelector("#gc-reset"))==null||g.addEventListener("click",()=>{d.length&&(d=[],i())})}i()}function un(e){var a;const t=((a=e==null?void 0:e.state)==null?void 0:a.params)||{},n=t.matchMode||"vs_ai_easy";return n==="friend"?`Match vs ${t.friendName||"Ami"}`:n==="random"?"Match vs Random":`Match vs IA — ${n.replace("vs_ai_","").toUpperCase()}`}async function fn(e,t,n){const{state:a,navigate:p}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await A.from("decks").select("id,name,is_active,formation").eq("owner_id",a.profile.id).order("created_at",{ascending:!1});if(!s||s.length===0){rt(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>p("decks"));return}const d=s.map(r=>r.id),{data:l}=await A.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",d).order("slot_order");let c=0;function i(){var z,B,U,F,j;const r=s[c],f=(l||[]).filter(P=>P.deck_id===r.id),m=f.filter(P=>{var te;return P.is_starter&&((te=P.card)==null?void 0:te.player)}).map(P=>Xt(P.card,P.position)),g=f.find(P=>{var te;return((te=P.card)==null?void 0:te.card_type)==="formation"}),$=r.formation||((z=g==null?void 0:g.card)==null?void 0:z.formation)||"4-4-2",o=m.length>=11?ft(m,$):null,_=m.length>=11;Xe(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${s.map((P,te)=>`<div style="width:7px;height:7px;border-radius:50%;background:${te===c?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
    ...
    </div>`,function(){const te=e.querySelector(".deck-preview-wrap svg");te&&(te.removeAttribute("width"),te.removeAttribute("height"),te.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",te.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(B=document.getElementById("prev-deck"))==null||B.addEventListener("click",()=>{c>0&&(c--,i())}),(U=document.getElementById("next-deck"))==null||U.addEventListener("click",()=>{c<s.length-1&&(c++,i())}),(F=document.getElementById("validate-deck"))==null||F.addEventListener("click",()=>{if(!_)return;const P=t.state.params||{};t.navigate("match",{...P,matchMode:P.matchMode||n,deckId:r.id})}),(j=document.getElementById("cancel-deck-select"))==null||j.addEventListener("click",()=>{Oe(e),p("home")});const M=document.getElementById("deck-swipe-zone");if(M){let P=0,te=0;M.addEventListener("touchstart",ce=>{P=ce.touches[0].clientX,te=ce.touches[0].clientY},{passive:!0}),M.addEventListener("touchend",ce=>{const ne=ce.changedTouches[0].clientX-P,S=ce.changedTouches[0].clientY-te;Math.abs(ne)<40||Math.abs(ne)<Math.abs(S)||(ne<0&&c<s.length-1?(c++,i()):ne>0&&c>0&&(c--,i()))},{passive:!0})}}i()}function Ci(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function Ve(e,t=44,n=58){if(!e)return`<div style="width:${t}px;height:${n}px;border:1.5px dashed rgba(255,255,255,0.2);border-radius:5px"></div>`;const a=typeof e.portrait=="string"&&e.portrait.startsWith("http")?e.portrait:ze(e),p=Ci(e),s=e._line||e.job||"MIL",d=ut[s]||"#555",l={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",c=e.note!==void 0?Number(e.note)||0:(Number(we(e,s))||0)+(e.boost||0),i=ji(e==null?void 0:e.country_code),r=Math.round(n*.19),f=Math.round(n*.25),m=n-Math.round(n*.19)-Math.round(n*.25),g=e!=null&&e.used?.28:1;return`<div style="width:${t}px;height:${n}px;border-radius:5px;border:2px solid ${l};background:${d};position:relative;overflow:hidden;flex-shrink:0;opacity:${g}">
  ...
  </div>`}function it(e,t,n){if(!(e!=null&&e.length))return"";const a=e.slice(0,5);let p='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return a.forEach((s,d)=>{if(p+=Ve(s,40,52),d<a.length-1){const l=Ze(s,a[d+1]);p+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),n!==void 0&&(p+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${n}</div>`),p+="</div>",p}function Jt(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function ji(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function st(e,t,n,a,p=310,s=310,d=[]){var B;const l=zt[t]||{},c=Ii(t)||oi[t]||[],i={},r=["ATT","MIL","DEF","GK"];for(const U of r)(e[U]||[]).forEach((j,P)=>{i[`${U}${P+1}`]=j});function f(U){const F=l[U];return F?{x:F.x*p,y:F.y*s}:null}let m="";for(const[U,F]of c){const j=f(U),P=f(F);if(!j||!P)continue;const te=i[U],ce=i[F],ne=Ze(te,ce);ne==="#00ff88"||ne==="#FFD700"?(m+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${P.x.toFixed(1)}" y2="${P.y.toFixed(1)}"
        stroke="${ne}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,m+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${P.x.toFixed(1)}" y2="${P.y.toFixed(1)}"
        stroke="${ne}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):m+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${P.x.toFixed(1)}" y2="${P.y.toFixed(1)}"
        stroke="${ne}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=48,$=64,o=13,_=16,M={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[U,F]of Object.entries(i)){const j=f(U);if(!j||!F)continue;const P=U.replace(/[0-9]/g,""),te=ut[P]||"#555",ce=d.includes(F.cardId),ne=n==="attack"&&(["MIL","ATT"].includes(P)||ce)&&!F.used||n==="defense"&&["GK","DEF","MIL"].includes(P)&&!F.used,S=a.includes(F.cardId);let Y;n==="attack"?Y=ce?Math.max(1,Number(F.note_a)||0):P==="MIL"?Number(F.note_m)||0:Number(F.note_a)||0:n==="defense"?Y=P==="GK"?Number(F.note_g)||0:P==="MIL"?Number(F.note_m)||0:Number(F.note_d)||0:Y=Number(P==="GK"?F.note_g:P==="DEF"?F.note_d:P==="MIL"?F.note_m:F.note_a)||0,Y=Y+(F.boost||0);const ee=(j.x-g/2).toFixed(1),D=(j.y-$/2).toFixed(1),Q=M[F==null?void 0:F.rarity]||M.normal;if(F.used){const L=typeof import.meta<"u"&&"/manager-wars/"||"/",q=`${typeof window<"u"&&((B=window.location)==null?void 0:B.origin)||""}${L}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");m+=`<rect x="${ee}" y="${D}" width="${g}" height="${$}" rx="5" fill="#161616"/>`,m+=`<image href="${q}" xlink:href="${q}" x="${ee}" y="${D}" width="${g}" height="${$}" preserveAspectRatio="xMidYMid slice"/>`,m+=`<rect x="${ee}" y="${D}" width="${g}" height="${$}" rx="5" fill="none" stroke="${Q}" stroke-width="2" opacity="0.7"/>`,m+=`<rect x="${ee}" y="${D}" width="${g}" height="${$}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${F.cardId}" data-role="${P}" style="cursor:pointer"/>`;continue}const ae=S?.45:1,pe=S?"#FFD700":Q,he=S?3:(F==null?void 0:F.rarity)==="legende"||(F==null?void 0:F.rarity)==="légende"||(F==null?void 0:F.rarity)==="pepite"||(F==null?void 0:F.rarity)==="pépite"?2.5:2,xe=$-o-_;m+=`<defs><clipPath id="cp-${U}"><rect x="${ee}" y="${(j.y-$/2+o).toFixed(1)}" width="${g}" height="${xe}"/></clipPath></defs>`,m+=`<rect x="${ee}" y="${D}" width="${g}" height="${$}" rx="5" fill="${te}" opacity="${ae}"/>`;const ge=ze(F);ge&&(m+=`<image href="${ge}" xlink:href="${ge}" x="${ee}" y="${(j.y-$/2+o).toFixed(1)}" width="${g}" height="${xe}" clip-path="url(#cp-${U})" preserveAspectRatio="xMidYMin slice"/>`),m+=`<rect x="${ee}" y="${D}" width="${g}" height="${o}" rx="4" fill="rgba(255,255,255,0.92)"/>`,m+=`<text x="${j.x.toFixed(1)}" y="${(j.y-$/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(F.name||"").slice(0,9)}</text>`;const O=(j.y+$/2-_).toFixed(1);m+=`<rect x="${ee}" y="${O}" width="${g}" height="${_}" fill="rgba(255,255,255,0.92)"/>`;{const L=Jt(F.country_code);L?m+=`<image href="${L}" xlink:href="${L}" x="${(j.x-20).toFixed(1)}" y="${(j.y+$/2-_+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:m+=`<text x="${(j.x-13).toFixed(1)}" y="${(j.y+$/2-_/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${ji(F.country_code)}</text>`,m+=`<text x="${j.x.toFixed(1)}" y="${(j.y+$/2-_/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${Y}</text>`;const R=Ci(F);R?m+=`<image href="${R}" xlink:href="${R}" x="${(j.x+g/2-14).toFixed(1)}" y="${(j.y+$/2-_+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:F.clubName&&(m+=`<text x="${(j.x+14).toFixed(1)}" y="${(j.y+$/2-_/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(F.clubName||"").slice(0,3).toUpperCase()}</text>`),F.boost&&(m+=`<rect x="${(j.x+g/2-12).toFixed(1)}" y="${(j.y-$/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,m+=`<text x="${(j.x+g/2-5).toFixed(1)}" y="${(j.y-$/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${F.boost}</text>`)}m+=`<rect x="${ee}" y="${D}" width="${g}" height="${$}" rx="5" fill="${S?"rgba(255,255,255,0.12)":"none"}" stroke="${pe}" stroke-width="${he}" opacity="${ae}"/>`,ne&&(m+=`<rect x="${ee}" y="${D}" width="${g}" height="${$}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${S?"selected":""}" data-card-id="${F.cardId}" data-role="${P}" style="cursor:pointer"/>`)}const z=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-z} ${-z} ${p+z*2} ${s+z*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${m}
  ...
  <div style="text-align:center;flex-shrink:0;width:38px">
  ...
  </div>`}async function li(e,t,n,a){var F;const{state:p,navigate:s,toast:d}=t;Xe(e);const l=p.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!l.deckId)return fn(e,t,n);const c=l.deckId;let i,r,f,m;try{const j=await Promise.all([A.from("decks").select("formation,name").eq("id",c).single(),A.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",c).order("slot_order")]);i=j[0].data,f=j[0].error,r=j[1].data,m=j[1].error}catch(j){console.error("[Match] Exception chargement deck:",j),rt(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>s("home"));return}if(f||m){console.error("[Match] Erreur Supabase:",f||m),rt(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>s("home"));return}const g=(r||[]).filter(j=>{var P;return j.is_starter&&((P=j.card)==null?void 0:P.player)}).map(j=>Xt(j.card,j.position)),$=(r||[]).filter(j=>{var P;return!j.is_starter&&((P=j.card)==null?void 0:P.player)}).map(j=>Xt(j.card,j.position));if(g.length<11){rt(e,"⚠️",`Deck incomplet (${g.length}/11).`,"Compléter",()=>s("decks"));return}const o=(r||[]).find(j=>{var P;return((P=j.card)==null?void 0:P.card_type)==="formation"}),_=(i==null?void 0:i.formation)||((F=o==null?void 0:o.card)==null?void 0:F.formation)||"4-4-2",{data:M,error:z}=await A.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",p.profile.id).eq("card_type","game_changer"),{data:B}=await A.from("gc_definitions").select("*").eq("is_active",!0),U=(M||[]).map(j=>({...j,_gcDef:(B==null?void 0:B.find(P=>P.name===j.gc_type||P.id===j.gc_definition_id))||null}));a({deckId:c,formation:_,starters:g,subsRaw:$,gcCardsEnriched:U,gcDefs:B||[]})}async function gn(e,t){var n,a;try{const p=(a=(n=t==null?void 0:t.state)==null?void 0:n.profile)==null?void 0:a.id;try{(A.getChannels?A.getChannels():[]).forEach(d=>{const l=d.topic||"";(l.includes("matchmaking")||l.includes("pvp-match")||l.includes("friend-invite"))&&A.removeChannel(d)})}catch(s){console.warn("[Random] cleanup canaux:",s)}p&&await A.rpc("cancel_matchmaking",{p_user_id:p}).catch(()=>{})}catch{}await li(e,t,"random",({deckId:p,formation:s,starters:d,subsRaw:l,gcCardsEnriched:c,gcDefs:i})=>{const r=f=>mn(e,t,p,s,d,l,f,i||[]);if(!c.length){r([]);return}di(e,c,r)})}async function mn(e,t,n,a,p,s,d=[],l=[]){var z;const{state:c,navigate:i,toast:r}=t;let f=!1,m=null;Xe(e),e.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const g=async(B=!0)=>{f=!0,m&&(A.removeChannel(m),m=null),B&&await A.rpc("cancel_matchmaking",{p_user_id:c.profile.id}).catch(()=>{})};(z=document.getElementById("mm-cancel"))==null||z.addEventListener("click",async()=>{try{await g(!0)}catch{}Oe(e),i("home")});const $=async(B,U,F)=>{if(f)return;f=!0,m&&(A.removeChannel(m),m=null);const j=document.getElementById("mm-status");j&&(j.textContent="Adversaire trouvé !"),await new Promise(P=>setTimeout(P,600)),e.isConnected&&Bi(e,t,B,F,d,l)},{data:o,error:_}=await A.rpc("try_matchmake",{p_user_id:c.profile.id,p_deck_id:n});if(_||!(o!=null&&o.success)){r("Erreur de matchmaking","error"),Oe(e),i("home");return}if(o.matched){$(o.match_id,o.opponent_id,!1);return}const M=document.getElementById("mm-status");M&&(M.textContent="En attente d'un autre joueur..."),m=A.channel("matchmaking-"+c.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${c.profile.id}`},B=>{const U=B.new;U.status==="matched"&&U.match_id&&$(U.match_id,U.matched_with,!0)}).subscribe()}async function xn(e,t,n){const{state:a,navigate:p,toast:s}=t;try{(A.getChannels?A.getChannels():[]).forEach(i=>{const r=i.topic||"";(r.includes("matchmaking")||r.includes("pvp-match")||r.includes("friend-invite"))&&A.removeChannel(i)})}catch{}const{data:d}=await A.from("matches").select("home_id, away_id, status").eq("id",n).single();if(!d){s("Match introuvable","error"),p("home");return}if(d.status==="finished"){s("Ce match est terminé","info"),p("home");return}const l=d.home_id===a.profile.id;Bi(e,t,n,l,[],[])}async function Bi(e,t,n,a,p=[],s=[]){const{state:d,navigate:l,toast:c}=t,i=a?"p1":"p2",r=a?"p2":"p1",f=(p||[]).map(T=>T.id),m=(p||[]).map(T=>({id:T.id,gc_type:T.gc_type,_gcDef:T._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:g}=await A.from("matches").select("*").eq("id",n).single();if(!g){c("Match introuvable","error"),l("home");return}async function $(){const[{data:T},{data:u},{data:x},{data:h}]=await Promise.all([A.rpc("get_deck_for_match",{p_deck_id:g.home_deck_id}),A.rpc("get_deck_for_match",{p_deck_id:g.away_deck_id}),A.from("users").select("id,pseudo,club_name").eq("id",g.home_id).single(),A.from("users").select("id,pseudo,club_name").eq("id",g.away_id).single()]),y=k=>({cardId:k.card_id,position:k.position,id:k.id,firstname:k.firstname,name:k.surname_encoded,country_code:k.country_code,club_id:k.club_id,job:k.job,job2:k.job2,note_g:Number(k.note_g)||0,note_d:Number(k.note_d)||0,note_m:Number(k.note_m)||0,note_a:Number(k.note_a)||0,rarity:k.rarity,skin:k.skin,hair:k.hair,hair_length:k.hair_length,clubName:k.club_encoded_name||null,clubLogo:k.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),b=((T==null?void 0:T.starters)||[]).map(k=>y(k)),v=((u==null?void 0:u.starters)||[]).map(k=>y(k)),w=(T==null?void 0:T.formation)||"4-4-2",I=(u==null?void 0:u.formation)||"4-4-2";return{p1Team:ft(b,w),p2Team:ft(v,I),p1Subs:((T==null?void 0:T.subs)||[]).map(k=>y(k)),p2Subs:((u==null?void 0:u.subs)||[]).map(k=>y(k)),p1Formation:w,p2Formation:I,p1Name:(x==null?void 0:x.club_name)||(x==null?void 0:x.pseudo)||"Joueur 1",p2Name:(h==null?void 0:h.club_name)||(h==null?void 0:h.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:a?f:[],gc_p2:a?[]:f,gcCardsFull_p1:a?m:[],gcCardsFull_p2:a?[]:m,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let o=g.game_state&&Object.keys(g.game_state).length?g.game_state:null;if(!o)if(a){o=await $();const{data:T}=await A.from("matches").select("game_state").eq("id",n).single();!(T!=null&&T.game_state)||!Object.keys(T.game_state).length?await A.from("matches").update({game_state:o,turn_user_id:g.home_id}).eq("id",n):o=T.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let T=0;T<30&&!o;T++){await new Promise(x=>setTimeout(x,400));const{data:u}=await A.from("matches").select("game_state").eq("id",n).single();u!=null&&u.game_state&&Object.keys(u.game_state).length&&(o=u.game_state)}if(!o){c("Erreur de synchronisation","error"),l("home");return}o.gc_p2=f,o.gcCardsFull_p2=m,await A.from("matches").update({game_state:o}).eq("id",n)}let _=!1,M=null,z=!1;const B=new Set,U=new Set;function F(T){var k,E;try{A.removeChannel(j)}catch{}const u=o[i+"Score"]||0,x=o[r+"Score"]||0;let h,y;T.winner_id?(h=T.winner_id===d.profile.id,y=!1):T.forfeit?(h=u>x,y=!1):(y=u===x,h=u>x),(k=document.getElementById("pvp-end-overlay"))==null||k.remove();const b=document.createElement("div");b.id="pvp-end-overlay",b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const v=y?"🤝":h?"🏆":"😞",w=y?"MATCH NUL":h?"VICTOIRE !":"DÉFAITE",I=y?"#fff":h?"#FFD700":"#ff6b6b";b.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(b),(E=b.querySelector("#pvp-end-home"))==null||E.addEventListener("click",()=>{b.remove(),Oe(e),l("home")})}const j=A.channel("pvp-match-"+n).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${n}`},T=>{const u=T.new;try{if(u.status==="finished"||u.forfeit){if(_)return;_=!0,M&&(clearInterval(M),M=null),u.game_state&&(o=u.game_state),F(u);return}if(u.game_state){const x=o;o=u.game_state;const h=o._lastGC;if(h&&h.seq&&!U.has(h.seq)&&(U.add(h.seq),h.by!==i)){xe(h.type,h.by,()=>D());return}const y=x[i+"Score"]||0,b=x[r+"Score"]||0,v=o[i+"Score"]||0,w=o[r+"Score"]||0,I=v>y,k=w>b;if((I||k)&&!B.has(o.round)){B.add(o.round);const E=[...o.log||[]].reverse().find(N=>N.isGoal),C=((E==null?void 0:E.homePlayers)||[]).map(N=>({name:N.name,note:N.note,portrait:N.portrait,job:N.job}));he(C,v,w,I,()=>D());return}D()}}catch(x){console.error("[PvP] crash:",x)}}).subscribe();async function P(T){Object.assign(o,T),o.lastActionAt=new Date().toISOString();const{error:u}=await A.from("matches").update({game_state:o}).eq("id",n);u&&c("Erreur de synchronisation","error");try{D()}catch(x){console.error("[PvP] renderPvpScreen crash:",x)}}async function te(){if(_)return;_=!0,M&&(clearInterval(M),M=null);const T=a?g.away_id:g.home_id,u=a?"p2":"p1",x=a?"p1":"p2",h={...o,[u+"Score"]:3,[x+"Score"]:0,phase:"finished"};try{await A.from("matches").update({status:"finished",forfeit:!0,winner_id:T,home_score:h.p1Score||0,away_score:h.p2Score||0,game_state:h}).eq("id",n)}catch{}try{A.removeChannel(j)}catch{}setTimeout(()=>{Oe(e),l("home")},800)}const ce={BOOST_STAT:({value:T=1,count:u=1,roles:x=[]},h,y)=>{const b=h[i+"Team"],v=Object.entries(b).filter(([w])=>!x.length||x.includes(w)).flatMap(([w,I])=>I.filter(k=>!k.used).map(k=>({...k,_line:w})));if(!v.length){h.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),y(h);return}ne(v,u,`Choisir ${u} joueur(s) à booster (+${T})`,w=>{w.forEach(I=>{const k=(b[I._line]||[]).find(E=>E.cardId===I.cardId);k&&(k.boost=(k.boost||0)+T,h.log.push({text:`⚡ +${T} sur ${k.name}`,type:"info"}))}),h[i+"Team"]=b,y(h)})},DEBUFF_STAT:({value:T=1,count:u=1,roles:x=[],target:h="ai"},y,b)=>{const v=h==="home"?i:r,w=y[v+"Team"],I=h==="home"?"allié":"adverse",k=Object.entries(w).filter(([E])=>!x.length||x.includes(E)).flatMap(([E,C])=>C.filter(N=>!N.used).map(N=>({...N,_line:E})));if(!k.length){y.log.push({text:`🎯 Aucun joueur ${I}`,type:"info"}),b(y);return}ne(k,u,`Choisir ${u} joueur(s) ${I}(s) (-${T})`,E=>{E.forEach(C=>{const N=(w[C._line]||[]).find(W=>W.cardId===C.cardId);N&&(N.boost=(N.boost||0)-T,y.log.push({text:`🎯 -${T} sur ${N.name}`,type:"info"}))}),y[v+"Team"]=w,b(y)})},GRAY_PLAYER:({count:T=1,roles:u=[],target:x="ai"},h,y)=>{const b=x==="home"?i:r,v=h[b+"Team"],w=x==="home"?"allié":"adverse",I=Object.entries(v).filter(([k])=>!u.length||u.includes(k)).flatMap(([k,E])=>E.filter(C=>!C.used).map(C=>({...C,_line:k})));if(!I.length){h.log.push({text:`❌ Aucun joueur ${w}`,type:"info"}),y(h);return}ne(I,T,`Choisir ${T} joueur(s) ${w}(s) à exclure`,k=>{k.forEach(E=>{const C=(v[E._line]||[]).find(N=>N.cardId===E.cardId);C&&(C.used=!0,h.log.push({text:`❌ ${C.name} exclu !`,type:"info"}))}),h[b+"Team"]=v,y(h)})},REVIVE_PLAYER:({count:T=1,roles:u=[]},x,h)=>{const y=x[i+"Team"],b=Object.entries(y).filter(([v])=>!u.length||u.includes(v)).flatMap(([v,w])=>w.filter(I=>I.used).map(I=>({...I,_line:v})));if(!b.length){x.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),h(x);return}ne(b,T,`Choisir ${T} joueur(s) à ressusciter`,v=>{v.forEach(w=>{const I=(y[w._line]||[]).find(k=>k.cardId===w.cardId);I&&(I.used=!1,x.log.push({text:`💫 ${I.name} ressuscité !`,type:"info"}))}),x[i+"Team"]=y,h(x)})},REMOVE_GOAL:({},T,u)=>{const x=r+"Score";T[x]>0?(T[x]--,T.log.push({text:"🚫 Dernier but annulé !",type:"info"})):T.log.push({text:"🚫 Aucun but à annuler",type:"info"}),u(T)},ADD_GOAL_DRAW:({},T,u)=>{T[i+"Score"]===T[r+"Score"]?(T[i+"Score"]++,T.log.push({text:"🎯 But bonus !",type:"info"})):T.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),u(T)},ADD_SUB:({value:T=1},u,x)=>{u.maxSubs=(u.maxSubs||3)+T,u.log.push({text:`🔄 +${T} remplacement(s)`,type:"info"}),x(u)},CUSTOM:({},T,u)=>u(T)};function ne(T,u,x,h){const y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let b=[];function v(){var I,k;const w=T.map(E=>{const C={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[E._line]||"#555",N=we(E,E._line)+(E.boost||0),le=b.find(H=>H.cardId===E.cardId)?"#FFD700":"rgba(255,255,255,0.25)",J=E.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${E.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${le};background:${C};overflow:hidden;cursor:pointer;${J}">
        ...
        </div>`,(I=y.querySelector("#pp-close"))==null||I.addEventListener("click",()=>y.remove()),y.querySelectorAll(".pp-item").forEach(E=>{E.addEventListener("click",()=>{const C=E.dataset.cid,N=T.find(le=>le.cardId===C),W=b.findIndex(le=>le.cardId===C);N&&(W>-1?b.splice(W,1):b.length<u&&b.push(N),v())})}),(k=y.querySelector("#pp-confirm"))==null||k.addEventListener("click",()=>{y.remove(),h(b)})}v(),document.body.appendChild(y)}async function S(T,u){const h=(o["gcCardsFull_"+i]||[]).find(w=>w.id===T),y=(h==null?void 0:h._gcDef)||(o.gcDefs||[]).find(w=>{var I;return w.name===u||((I=w.name)==null?void 0:I.toLowerCase().trim())===(u==null?void 0:u.toLowerCase().trim())}),b=[...o["usedGc_"+i]||[],T],v={type:u,by:i,seq:(o._gcAnimSeq||0)+1};U.add(v.seq),xe(u,i,async()=>{if(y!=null&&y.effect_type&&y.effect_type!=="CUSTOM"){const I=ce[y.effect_type];if(I){const k={...o};I(y.effect_params||{},k,async E=>{E["usedGc_"+i]=b,E._lastGC=v,E._gcAnimSeq=v.seq,await P(E)});return}}const w={...o};switch(u){case"Remplacement+":w.maxSubs=(w.maxSubs||3)+1,w.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const I=r+"Score";w[I]>0&&(w[I]--,w.log.push({text:"🚫 But annulé",type:"info"}));break}}w["usedGc_"+i]=b,w._lastGC=v,w._gcAnimSeq=v.seq,await P(w)})}function Y(T,u){const x="usedCardIds_"+T,h=new Set(o[x]||[]);u.forEach(y=>h.add(y)),o[x]=[...h]}function ee(){for(const T of["p1","p2"]){const u=new Set(o["usedCardIds_"+T]||[]),x=o[T+"Team"];if(x)for(const h of["GK","DEF","MIL","ATT"])(x[h]||[]).forEach(y=>{y.used=u.has(y.cardId)})}}function D(){var Be,Qe,mt,xt,yt,ht;if(o.phase==="reveal")return Q();if(o.phase==="midfield")return pe();if(o.phase==="finished")return He();const T=o[i+"Team"],u=o[r+"Team"];ee();const x=o[i+"Score"],h=o[r+"Score"],y=o[i+"Name"],b=o[r+"Name"],v=o.phase===i+"-attack",w=o.phase===i+"-defense",I=Array.isArray(o["selected_"+i])?o["selected_"+i]:[],k=I.map(X=>X.cardId),E=window.innerWidth>=700,C=o[i+"Subs"]||[],N=o["usedSubIds_"+i]||[],W=C.filter(X=>!N.includes(X.cardId)),le=o["gcCardsFull_"+i]||[],J=o["usedGc_"+i]||[],H=le.filter(X=>!J.includes(X.id)),K=o.boostOwner===i&&!o.boostUsed,Z=!["GK","DEF","MIL","ATT"].some(X=>(u[X]||[]).some(G=>!G.used)),de=[...T.MIL||[],...T.ATT||[]].filter(X=>!X.used),ue=v&&Z&&de.length===0?[...T.GK||[],...T.DEF||[]].filter(X=>!X.used).map(X=>X.cardId):[];function fe(X,G,oe){var Ct,wt;const se=X._gcDef,ye={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[se==null?void 0:se.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Ye={purple:"#b06ce0",light_blue:"#00d4ef"}[se==null?void 0:se.color]||"#b06ce0",We=(se==null?void 0:se.name)||X.gc_type,Je=(se==null?void 0:se.effect)||((Ct=Ge[X.gc_type])==null?void 0:Ct.desc)||"",et=se!=null&&se.image_url?`/manager-wars/icons/${se.image_url}`:null,Mt=((wt=Ge[X.gc_type])==null?void 0:wt.icon)||"⚡",St=Math.round(oe*.22),bt=Math.round(oe*.22),lt=oe-St-bt,vt=We.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${X.id}" data-gc-type="${X.gc_type}"
    ...
    </div>`,i==="p1"&&setTimeout(async()=>{await P({phase:"midfield"})},5e3)}let ae=!1;function pe(){if(ae)return;const T=o[i+"Team"].MIL||[],u=o[r+"Team"].MIL||[];function x(H){return H.reduce((K,Z)=>K+we(Z,"MIL"),0)}function h(H){let K=0;for(let Z=0;Z<H.length-1;Z++){const de=Ze(H[Z],H[Z+1]);de==="#00ff88"?K+=2:de==="#FFD700"&&(K+=1)}return K}const y=x(T)+h(T),b=x(u)+h(u),v=y>=b;function w(H,K,Z){return`<div id="duel-row-${Z}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(b);let I=!1;const k=()=>{I||(I=!0,b.remove(),setTimeout(()=>y(),50))};b.addEventListener("click",k),setTimeout(k,3500)}function xe(T,u,x){var J,H;const h=(o.gcDefs||[]).find(K=>{var Z;return K.name===T||((Z=K.name)==null?void 0:Z.toLowerCase().trim())===(T==null?void 0:T.toLowerCase().trim())}),y={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[h==null?void 0:h.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",b={purple:"#b06ce0",light_blue:"#00d4ef"}[h==null?void 0:h.color]||"#b06ce0",v=(h==null?void 0:h.name)||T,w=(h==null?void 0:h.effect)||((J=Ge[T])==null?void 0:J.desc)||"",I=h!=null&&h.image_url?`/manager-wars/icons/${h.image_url}`:null,k=((H=Ge[T])==null?void 0:H.icon)||"⚡",C=u===i?"Vous":o[u+"Name"]||"Adversaire",N=document.createElement("div");N.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",N.innerHTML=`
      <style>
      ...
      </div>`,document.body.appendChild(x),(h=x.querySelector("#bp-close"))==null||h.addEventListener("click",()=>x.remove()),x.querySelectorAll(".bp-item").forEach(y=>{y.addEventListener("click",async()=>{const b=y.dataset.cid,v=u.find(I=>I.cardId===b);if(!v)return;const w=(T[v._line]||[]).find(I=>I.cardId===b);w&&(w.boost=(w.boost||0)+o.boostValue),x.remove(),await P({[i+"Team"]:T,boostUsed:!0})})})}function L(T=null){var W,le;if(!(o.phase===i+"-attack")){c("Remplacement uniquement avant votre attaque","warning");return}const x=o[i+"Team"],h=o["usedSubIds_"+i]||[],y=o.maxSubs||3;if(h.length>=y){c(`Maximum ${y} remplacements atteint`,"warning");return}const b=Object.entries(x).flatMap(([J,H])=>(H||[]).filter(K=>K.used).map(K=>({...K,_line:J}))),v=(o[i+"Subs"]||[]).filter(J=>!h.includes(J.cardId));if(!b.length){c("Aucun joueur utilisé à remplacer","warning");return}if(!v.length){c("Aucun remplaçant disponible","warning");return}let w=Math.max(0,b.findIndex(J=>J.cardId===T));const I=((W=b[w])==null?void 0:W._line)||((le=b[w])==null?void 0:le.job);let k=Math.max(0,v.findIndex(J=>J.job===I)),E=!1;const C=document.createElement("div");C.id="pvp-sub-overlay",C.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function N(){var fe,me,be,Se,Re,Fe;const J=b[w],H=v[k],K=Math.min(130,Math.round((window.innerWidth-90)/2)),Z=Math.round(K*1.35),de=Ie=>`background:rgba(255,255,255,0.12);border:none;color:${Ie?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Ie?"default":"pointer"};flex-shrink:0`;C.innerHTML=`
      ...
      </div>`,(fe=C.querySelector("#psub-close"))==null||fe.addEventListener("click",()=>C.remove()),(me=C.querySelector("#pout-up"))==null||me.addEventListener("click",()=>{w>0&&(w--,N())}),(be=C.querySelector("#pout-down"))==null||be.addEventListener("click",()=>{w<b.length-1&&(w++,N())}),(Se=C.querySelector("#pin-up"))==null||Se.addEventListener("click",()=>{k>0&&(k--,N())}),(Re=C.querySelector("#pin-down"))==null||Re.addEventListener("click",()=>{k<v.length-1&&(k++,N())});const ue=(Ie,Le,je,De)=>{const Pe=C.querySelector("#"+Ie);if(!Pe)return;let Ue=0;Pe.addEventListener("touchstart",Ke=>{Ue=Ke.touches[0].clientY},{passive:!0}),Pe.addEventListener("touchend",Ke=>{const Ce=Ke.changedTouches[0].clientY-Ue;if(Math.abs(Ce)<30)return;const Be=Le();Ce<0&&Be<De-1?(je(Be+1),N()):Ce>0&&Be>0&&(je(Be-1),N())},{passive:!0})};ue("pin-panel",()=>k,Ie=>k=Ie,v.length),ue("pout-panel",()=>w,Ie=>w=Ie,b.length),(Fe=C.querySelector("#psub-confirm"))==null||Fe.addEventListener("click",async Ie=>{if(Ie.preventDefault(),Ie.stopPropagation(),E)return;E=!0;const Le=b[w],je=v[k];if(!Le||!je)return;const De=Le._line,Pe=(x[De]||[]).findIndex(Ce=>Ce.cardId===Le.cardId);if(Pe===-1){c("Erreur : joueur introuvable","error"),C.remove();return}const Ue={...je,_line:De,position:Le.position,used:!1,boost:0};x[De].splice(Pe,1,Ue);const Ke=[...h,je.cardId];C.remove(),R(Le,je,async()=>{await P({[i+"Team"]:x,[r+"Team"]:o[r+"Team"],["usedSubIds_"+i]:Ke})})})}document.body.appendChild(C),N()}function R(T,u,x){const h={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const b=(I,k,E)=>`<div style="text-align:center">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(y);let v=!1;const w=()=>{v||(v=!0,y.remove(),setTimeout(()=>x(),50))};y.addEventListener("click",w),setTimeout(w,2200)}function q(){var u;const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",T.innerHTML=`
      <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-transform:uppercase">Équipe adverse</div>
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${o[r+"Name"]}</div>
      <div style="width:min(90vw,420px)">${st(o[r+"Team"],o[r+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(T),(u=T.querySelector("#pvp-opp-close"))==null||u.addEventListener("click",()=>T.remove())}function V(){var x;const T=o.log||[],u=document.createElement("div");u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",u.innerHTML=`
      ...
      </div>`,document.body.appendChild(u),(x=u.querySelector("#pvp-hist-close"))==null||x.addEventListener("click",()=>u.remove())}async function re(){if(o.phase!==i+"-attack")return;const T=i==="p1"?"p2":"p1",u=(o.round||0)+1,x=[...o.log||[]];x.push({type:"info",text:`⏭️ ${o[i+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const h=Te(o),y=Ae(T),b=h||!y?"finished":T+"-attack";await P({["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:b,attacker:T,round:u,log:x}),b==="finished"&&await Ne(o)}async function ie(){const T=o[i+"Team"],u=!["GK","DEF","MIL","ATT"].some(b=>(o[r+"Team"][b]||[]).some(v=>!v.used)),x=(o["selected_"+i]||[]).map(b=>{const v=(T[b._role]||[]).find(N=>N.cardId===b.cardId)||b,w=u&&["GK","DEF"].includes(b._role),I=T[b._role]||[],k=I.findIndex(N=>N.cardId===b.cardId),E=ot(I.length),C=k>=0?E[k]:v._col??1;return{...v,_line:b._role,_col:C,...w?{note_a:Math.max(1,Number(v.note_a)||0)}:{}}});if(!x.length)return;const h=Pt(x,o.modifiers[i]||{});Y(i,x.map(b=>b.cardId)),x.forEach(b=>{const v=(T[b._role]||[]).find(w=>w.cardId===b.cardId);v&&(v.used=!0)}),o["selected_"+i]=[],D();const y=[...o.log||[]];if(u){const b=(o[i+"Score"]||0)+1,v=x.map(C=>({name:C.name,note:we(C,C._line||"ATT"),portrait:ze(C),job:C.job}));y.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:v,homeTotal:h.total,aiTotal:0});const w=(o.round||0)+1,I=i==="p1"?"p2":"p1",k={...o,[i+"Team"]:T,[i+"Score"]:b},E=Te(k);B.add(w),he(v,b,o[r+"Score"]||0,!0,async()=>{await P({[i+"Team"]:T,[i+"Score"]:b,["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:E?"finished":I+"-attack",attacker:I,round:w,log:y}),E&&await Ne({...o,[i+"Score"]:b})});return}y.push({type:"pending",text:`⚔️ ${o[i+"Name"]} attaque (${h.total})`}),await P({[i+"Team"]:T,[r+"Team"]:o[r+"Team"],pendingAttack:{...h,players:x,side:i},["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},phase:r+"-defense",log:y})}async function ke(){const T=o[i+"Team"],u=(o["selected_"+i]||[]).map(J=>{const H=(T[J._role]||[]).find(fe=>fe.cardId===J.cardId)||J,K=T[J._role]||[],Z=K.findIndex(fe=>fe.cardId===J.cardId),de=ot(K.length),ue=Z>=0?de[Z]:H._col??1;return{...H,_line:J._role,_col:ue}}),x=Ot(u,o.modifiers[i]||{});Y(i,u.map(J=>J.cardId)),u.forEach(J=>{const H=(T[J._role]||[]).find(K=>K.cardId===J.cardId);H&&(H.used=!0)}),o["selected_"+i]=[],D();const h=Ht(o.pendingAttack.total,x.total,o.modifiers[i]||{}),y=o.pendingAttack.side,b=h==="attack"||(h==null?void 0:h.goal),v=y==="p1"?"p2":"p1",w=(o.round||0)+1,I=(o.pendingAttack.players||[]).map(J=>({name:J.name,note:we(J,J._line||"ATT"),portrait:ze(J),job:J.job})),k=[...o.log||[]];k.push({type:"duel",isGoal:b,homeScored:b&&y===i,text:b?`⚽ BUT de ${o[y+"Name"]} ! (${o.pendingAttack.total} vs ${x.total})`:`✋ Attaque stoppée (${o.pendingAttack.total} vs ${x.total})`,homePlayers:I,aiPlayers:u.map(J=>({name:J.name,note:we(J,J._line||"DEF"),portrait:ze(J),job:J.job})),homeTotal:o.pendingAttack.total,aiTotal:x.total});const E=b?(o[y+"Score"]||0)+1:o[y+"Score"]||0,C={...o,[i+"Team"]:T,[y+"Score"]:E},N=Te(C),W=N?"finished":v+"-attack",le=async()=>{await P({[i+"Team"]:T,[r+"Team"]:o[r+"Team"],[y+"Score"]:E,["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:W,attacker:v,round:w,log:k}),(W==="finished"||N)&&await Ne({...o,[y+"Score"]:E})};if(b){const J=y===i,H=J?E:o[i+"Score"]||0,K=J?o[r+"Score"]||0:E;B.add(w),he(I,H,K,J,le)}else await le()}function Ee(T){return["MIL","ATT"].some(u=>(T[u]||[]).some(x=>!x.used))}function $e(T){return["GK","DEF","MIL","ATT"].some(u=>(T[u]||[]).some(x=>!x.used))}function _e(T){return $e(T)&&!Ee(T)}function Ae(T){const u=o[T+"Team"],x=o[(T==="p1"?"p2":"p1")+"Team"];return!!(Ee(u)||!$e(x)&&_e(u))}function Te(T){const u=["MIL","ATT"].some(k=>(T.p1Team[k]||[]).some(E=>!E.used)),x=["MIL","ATT"].some(k=>(T.p2Team[k]||[]).some(E=>!E.used)),h=$e(T.p1Team),y=$e(T.p2Team);return!u&&!(!y&&h)&&(!x&&!(!h&&y))}function qe(T){const u=T.p1Score||0,x=T.p2Score||0;return u===x?null:u>x?g.home_id:g.away_id}async function Ne(T){try{await A.from("matches").update({status:"finished",winner_id:qe(T),home_score:T.p1Score||0,away_score:T.p2Score||0}).eq("id",n)}catch(u){console.error("[PvP] finishMatch:",u)}}function He(){var y;const T=o[i+"Score"],u=o[r+"Score"],x=T>u,h=T===u;Xe(e),e.innerHTML=`
    ...
    </div>`,(y=document.getElementById("pvp-home"))==null||y.addEventListener("click",()=>{try{A.removeChannel(j)}catch{}Oe(e),l("home")})}D()}const yn="2026.06.28-2301";async function bi(e,{state:t,navigate:n,toast:a}){var s,d;const p=t.profile;p&&(e.innerHTML=`
  <div class="page">
    <div class="page-body">
  ...
  `,e.querySelectorAll("[data-nav]").forEach(l=>{l.addEventListener("click",c=>{c.preventDefault(),n(l.dataset.nav)})}),(s=document.getElementById("nav-rankings"))==null||s.addEventListener("click",()=>n("rankings")),(d=document.getElementById("nav-matches"))==null||d.addEventListener("click",()=>n("matches")),e.querySelectorAll("[data-action]").forEach(l=>{l.addEventListener("click",()=>{l.classList.add("tapped"),setTimeout(()=>l.classList.remove("tapped"),200);const c=l.dataset.action;if(c==="match-ai"){wn(n);return}if(c==="match-random"){n("match",{matchMode:"random"});return}if(c==="match-friend"){n("friends");return}a("Bientôt disponible","info")})}),document.getElementById("logout-btn").addEventListener("click",async()=>{await A.auth.signOut(),window.location.reload()}),qi(t,a),vn(t,a,n),Fi(t,a,n))}async function Fi(e,t,n){const a=document.getElementById("ongoing-match-banner");if(!a)return;const p=e.profile.id,{data:s}=await A.from("matches").select("id, home_id, away_id, status, mode").eq("status","active").or(`home_id.eq.${p},away_id.eq.${p}`).order("created_at",{ascending:!1});if(!(s!=null&&s.length)){a.innerHTML="";return}const d=s.map(c=>c.home_id===p?c.away_id:c.home_id).filter(Boolean);let l={};if(d.length){const{data:c}=await A.from("users").select("id, pseudo, club_name").in("id",d);(c||[]).forEach(i=>{l[i.id]=i.club_name||i.pseudo})}a.innerHTML=s.map(c=>{const i=c.home_id===p?c.away_id:c.home_id,r=l[i]||"Adversaire";return`
    ...
    </div>`,(i=document.getElementById("match-inv-accept"))==null||i.addEventListener("click",()=>{a.innerHTML="",n("match",{matchMode:"friend",friendId:d,friendName:s})}),(r=document.getElementById("match-inv-decline"))==null||r.addEventListener("click",async()=>{await A.from("friend_match_invites").update({status:"declined"}).eq("id",p.id),a.innerHTML="",t("Invitation refusée","info")})}async function qi(e,t){const n=document.getElementById("friend-requests-banner");if(!n)return;const{data:a,error:p}=await A.from("friendships").select("id, requester:users!requester_id(pseudo, club_name)").eq("addressee_id",e.user.id).eq("status","pending");if(p||!(a!=null&&a.length)){n.innerHTML="";return}const s=a.length,d=a.slice(0,2).map(c=>{var i;return((i=c.requester)==null?void 0:i.pseudo)||"?"}).join(", "),l=s>2?` +${s-2}`:"";n.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
  ...
  </div>`}async function kn(e,t){const{state:n,navigate:a,toast:p,openModal:s,closeModal:d}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await A.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),i=(l||[]).filter(O=>O.card_type==="player"&&O.player),r=(l||[]).filter(O=>O.card_type==="game_changer"),f=(l||[]).filter(O=>O.card_type==="formation"),{data:m}=await A.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(m||[]).forEach(O=>{g[O.name]=O});const $=Object.keys(oi),o=Object.keys(Ge),_={};i.forEach(O=>{const L=O.player.id;_[L]=(_[L]||0)+1}),new Set(Object.keys(_).map(O=>String(O)));const M=new Set(f.map(O=>O.formation)),z=new Set(r.map(O=>O.gc_type));let B="player",U="Tous",F="",j=!1;function P(){return[...i].sort((O,L)=>{const R=jt.indexOf(O.player.job),q=jt.indexOf(L.player.job);return R!==q?R-q:(O.player.surname_encoded||"").localeCompare(L.player.surname_encoded||"")})}function te(){return[...c||[]].sort((O,L)=>{const R=jt.indexOf(O.job),q=jt.indexOf(L.job);return R!==q?R-q:(O.surname_encoded||"").localeCompare(L.surname_encoded||"")})}function ce(){return P().filter(O=>{const L=O.player,R=U==="Tous"||L.job===U,q=!F||`${L.firstname} ${L.surname_encoded}`.toLowerCase().includes(F);return R&&q})}function ne(){return te().filter(O=>{const L=U==="Tous"||O.job===U,R=!F||`${O.firstname} ${O.surname_encoded}`.toLowerCase().includes(F);return L&&R})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        <div style="display:flex;justify-content:flex-end">
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{j=!j,S(),Y()})))}function Y(){const O=document.getElementById("col-grid");O&&(B==="player"?he(O):B==="formation"?xe(O):ge(O))}function ee(O,L,R,q,V){V=V||"#7a28b8";const re=document.getElementById("col-grid"),ie=document.getElementById("col-big");if(!re||!ie)return;var ke=0;function Ee(){ie.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+L(O[ke])+"</div>";var $e=ie.querySelector("[data-card-id],[data-form-id],[data-gc-id]");$e&&$e.addEventListener("click",function(){q(O[ke])}),requestAnimationFrame(function(){var _e=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!_e||!ie)){var Ae=ie.clientHeight-8,Te=ie.clientWidth-24,qe=_e.offsetHeight,Ne=_e.offsetWidth;if(qe>0&&Ne>0&&Ae>40){var He=Math.min(Ae/qe,Te/Ne,1);_e.style.transform="scale("+He.toFixed(3)+")",_e.style.transformOrigin="top center"}}}),re.innerHTML=O.map(function(_e,Ae){return'<div class="col-mini-item" data-idx="'+Ae+'" style="flex-shrink:0;cursor:pointer;border-radius:8px;border:1.5px solid '+(Ae===ke?V:"transparent")+';transition:border-color .15s;overflow:hidden">'+R(_e,Ae===ke)+"</div>"}).join(""),re.querySelectorAll(".col-mini-item").forEach(function(_e){_e.addEventListener("click",function(){ke=Number(_e.dataset.idx),Ee(),_e.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}Ee()}function D(O){var L=.54,R=Math.round(140*L),q=Math.round(310*L),V;if(!O||O._fake){var re=O?O.player:null;if(!re)return"";V=vi(re)}else V=Vt(O,"");return'<div style="width:'+R+"px;height:"+q+'px;overflow:hidden;position:relative;flex-shrink:0"><div style="transform:scale('+L+');transform-origin:top left;position:absolute;top:0;left:0;pointer-events:none">'+V+"</div></div>"}function Q(O,L,R){L=L||100,R=R||140;var q=zt[O]||{},V={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},re=Math.max(3,Math.round(L*.06)),ie=Object.entries(q).map(function(Ee){var $e=Ee[0],_e=Ee[1],Ae=$e.replace(/\d+$/,""),Te=V[Ae]||"#888",qe=Math.round(_e.x*L),Ne=Math.round(_e.y*R);return'<circle cx="'+qe+'" cy="'+Ne+'" r="'+re+'" fill="'+Te+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),ke=Math.max(1,Math.round(L/50));return'<svg viewBox="0 0 '+L+" "+R+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+L+'" height="'+R+'" fill="#1A6B3C"/><rect x="'+Math.round(L*.2)+'" y="'+Math.round(R*.02)+'" width="'+Math.round(L*.6)+'" height="'+Math.round(R*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+ke+'"/><line x1="0" y1="'+Math.round(R*.5)+'" x2="'+L+'" y2="'+Math.round(R*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+ke+'"/><ellipse cx="'+Math.round(L*.5)+'" cy="'+Math.round(R*.5)+'" rx="'+Math.round(L*.18)+'" ry="'+Math.round(R*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+ke+'"/><rect x="'+Math.round(L*.2)+'" y="'+Math.round(R*.82)+'" width="'+Math.round(L*.6)+'" height="'+Math.round(R*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+ke+'"/>'+ie+"</svg>"}function ae(O,L,R){var q=R>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+R+"</div>":"",V=L?'data-form-id="'+L.id+'"':"",re=O.length>7?14:O.length>5?16:19,ie=!!L;return"<div "+V+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(ie?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(ie?"":"filter:grayscale(1);opacity:0.5")+'">'+q+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(ie?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+re+"px;font-weight:900;color:"+(ie?"#1A6B3C":"#aaa")+';line-height:1">'+O+'</div></div><div style="flex:1;overflow:hidden;background:'+(ie?"#1A6B3C":"#ccc")+'">'+Q(O,140,220)+"</div></div>"}function pe(O,L){var R=.54,q=Math.round(140*R),V=Math.round(305*R),re=Math.round(V*.22),ie=V-re,ke=O.length>7?5:7,Ee=Q(O,q,ie),$e=L?"1.5px solid #2a7a40":"1px solid #ddd",_e=L?"":"filter:grayscale(1);opacity:0.45;",Ae=L?"#1A6B3C":"#bbb",Te="#fff";return'<div style="width:'+q+"px;height:"+V+"px;border-radius:6px;border:"+$e+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+_e+'"><div style="height:'+re+"px;background:"+Ae+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+ke+"px;font-weight:900;color:"+Te+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(q-4)+'px">'+O+'</span></div><div style="height:'+ie+'px;overflow:hidden;flex-shrink:0">'+Ee+"</div></div>"}function he(O){if(j){const L=ne();if(!L.length){O.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const R=L.map(q=>i.find(V=>V.player.id===q.id)||{_fake:!0,player:q,id:"fake-"+q.id});ee(R,q=>q._fake?vi(q.player):Vt(q,""),q=>q._fake?D({player:q.player,id:"x",_fake:!0}):D(q),q=>{q._fake||wi(q,i,_,t)},"#1A6B3C")}else{const L=ce();if(!L.length){O.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const R={},q=[];L.forEach(V=>{R[V.player.id]||(R[V.player.id]=!0,q.push(V))}),ee(q,V=>{const re=_[V.player.id]||1,ie=re>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${re}</div>`:"",Ee=i.filter($e=>$e.player.id===V.player.id&&$e.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Vt(V,ie+Ee)},V=>D(V),V=>wi(V,i,_,t),"#1A6B3C")}}function xe(O){const L=j?$:[...M];if(!L.length){O.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const R=L.map(q=>({formation:q,card:f.find(V=>V.formation===q)||null,owned:M.has(q)}));ee(R,({formation:q,card:V,owned:re})=>ae(q,re?V:null,re?f.filter(ie=>ie.formation===q).length:0),({formation:q,owned:V})=>pe(q,V),({card:q,owned:V})=>{V&&q&&An(q,f,t,s)},"#1A6B3C")}function ge(O){const L=Object.keys(g),R=j?L.length?L:o:[...z];if(!R.length){O.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const q=R.map(V=>({type:V,gc:Ge[V]||{icon:"⚡",desc:""},def:g[V]||null,owned:z.has(V),card:r.find(re=>re.gc_type===V)||null}));ee(q,({type:V,gc:re,def:ie,owned:ke,card:Ee})=>{const $e=ke?r.filter(x=>x.gc_type===V).length:0,_e=$e>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${$e}</div>`:"",Ae=(ie==null?void 0:ie.gc_type)==="ultra_game_changer",Te={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},qe={purple:"#b06ce0",light_blue:"#00d4ef"},Ne=Te[ie==null?void 0:ie.color]||Te.purple,He=qe[ie==null?void 0:ie.color]||qe.purple,T=(ie==null?void 0:ie.effect)||re.desc||"",u=ie!=null&&ie.image_url?`/manager-wars/icons/${ie.image_url}`:null;return ke&&Ee?`<div data-gc-id="${Ee.id}" data-gc-type="${V}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${He};background:${Ne};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${He}66;cursor:pointer">
          ${_e}
          ...
          <div style="height:150px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${u?`<img src="${u}" style="max-width:120px;max-height:120px;object-fit:contain;border-radius:6px">`:`<span style="font-size:64px">${re.icon}</span>`}
          </div>
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Bt=1e3;function An(e,t,n,a){var $,o,_;const{state:p,toast:s,closeModal:d,navigate:l,refreshProfile:c}=n,i=e.formation,r={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function f(){const M=zt[i]||{},z=oi[i]||[],B=290,U=360,F=20;function j(te){const ce=M[te];return ce?{x:ce.x*B,y:ce.y*U}:null}let P=`<svg width="${B}" height="${U}" viewBox="0 0 ${B} ${U}" xmlns="http://www.w3.org/2000/svg">`;for(const[te,ce]of z){const ne=j(te),S=j(ce);!ne||!S||(P+=`<line x1="${ne.x}" y1="${ne.y}" x2="${S.x}" y2="${S.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const te of Object.keys(M)){const ce=j(te);if(!ce)continue;const ne=te.replace(/\d+/,""),S=r[ne]||"#555";P+=`<circle cx="${ce.x}" cy="${ce.y}" r="${F}" fill="${S}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,P+=`<text x="${ce.x}" y="${ce.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${ne}</text>`}return P+="</svg>",P}const m=t.filter(M=>M.formation===i);m.length;const g=!e.is_for_sale;a(`Formation ${i}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),($=document.getElementById("direct-sell-form-btn"))==null||$.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${i} pour ${Bt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const M=m.find(B=>!B.is_for_sale)||m[0];if(!M){s("Aucune carte à vendre","error");return}const{error:z}=await A.from("cards").delete().eq("id",M.id);if(z){s(z.message,"error");return}await A.from("users").update({credits:(p.profile.credits||0)+Bt}).eq("id",p.profile.id),await c(),s(`+${Bt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),d(),l("collection")}),(o=document.getElementById("market-sell-form-btn"))==null||o.addEventListener("click",async()=>{const M=parseInt(document.getElementById("sell-price-form").value);if(!M||M<1){s("Prix invalide","error");return}await A.from("cards").update({is_for_sale:!0,sale_price:M}).eq("id",e.id),await A.from("market_listings").insert({seller_id:p.profile.id,card_id:e.id,price:M}),s("Carte mise en vente sur le marché !","success"),d(),l("collection")}),(_=document.getElementById("cancel-sell-form-btn"))==null||_.addEventListener("click",async()=>{await A.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await A.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),s("Annonce retirée","success"),d(),l("collection")})}async function wi(e,t,n,a){var Q,ae,pe,he,xe,ge,O;const{state:p,toast:s,openModal:d,closeModal:l,navigate:c,refreshProfile:i}=a,r=e.player,f=t.filter(L=>L.player.id===r.id),m=f.length,g=Math.max(Number(r.note_g)||0,Number(r.note_d)||0,Number(r.note_m)||0,Number(r.note_a)||0),$=r.rarity||"normal",{data:o}=await A.from("sell_price_configs").select("*").eq("rarity",$).lte("note_min",g).gte("note_max",g).order("note_min",{ascending:!1}).limit(1),_=((Q=o==null?void 0:o[0])==null?void 0:Q.price)??$n[$]??1e3,M=r.rarity!=="legende",z=Gi(r),B=Lt(r,r.job),U=r.job2?Lt(r,r.job2):null,F=Et[r.job]||"#1A6B3C",j=r.job2?Et[r.job2]:null,P=Di[r.rarity]||"#ccc",te=ci[r.country_code]||r.country_code||"",ce=f.map(L=>L.id);let ne={};if(ce.length){const{data:L}=await A.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",ce).order("transferred_at",{ascending:!0});(L||[]).forEach(R=>{ne[R.card_id]||(ne[R.card_id]=[]),ne[R.card_id].push(R)})}const S=L=>`
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${f.map((L,R)=>{const q=ne[L.id]||[],V=L.is_for_sale,re=q.length?q[q.length-1]:null;return`
            <div data-card-id="${L.id}" data-card-idx="${R}" class="exemplaire-row"
              ...
              <div style="display:flex;align-items:center;gap:10px">
                ...
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:700">Exemplaire ${R+1}${V?" 🏷️ En vente":""}</div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        <div>
          ...
          <div style="font-weight:700;color:${P}">${r.rarity.toUpperCase()}</div>
        </div>
        <div>
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",r.note_g],["DEF",r.note_d],["MIL",r.note_m],["ATT",r.note_a]].map(([L,R])=>{const q=Et[L],V=Number(R)||0;return`<div style="display:flex;flex-direction:column;align-items:center;gap:2px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(he=document.getElementById("close-detail"))==null||he.addEventListener("click",l);let ee=new Set;const D=()=>{const L=ee.size,R=document.getElementById("sell-action-panel");R&&(R.style.display=L>0?"block":"none",document.getElementById("sell-selected-count").textContent=L,document.getElementById("sell-direct-total").textContent=(L*_).toLocaleString("fr")+" cr.")};document.querySelectorAll(".expl-check").forEach(L=>{L.addEventListener("change",()=>{const R=L.dataset.id;L.checked?ee.add(R):ee.delete(R);const q=L.closest(".exemplaire-row");q&&(q.style.borderColor=L.checked?"#1A6B3C":"#eee"),D()})}),document.querySelectorAll(".exemplaire-row").forEach(L=>{L.addEventListener("click",R=>{if(R.target.closest("button")||R.target.tagName==="INPUT")return;const q=L.querySelector(".expl-check");q&&!q.disabled&&(q.checked=!q.checked,q.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(L=>{L.addEventListener("click",R=>{R.stopPropagation();const q=document.querySelector(`.expl-hist[data-hist="${L.dataset.idx}"]`);q&&(q.style.display=q.style.display==="none"?"flex":"none")})}),(xe=document.getElementById("direct-sell-btn"))==null||xe.addEventListener("click",async()=>{const L=[...ee];if(!L.length)return;const R=L.length*_;if(!confirm(`Vendre ${L.length} carte${L.length>1?"s":""} ${r.surname_encoded} pour ${R.toLocaleString("fr")} crédits ? Action irréversible.`))return;const{error:q}=await A.from("cards").delete().in("id",L);if(q){s(q.message,"error");return}await A.from("users").update({credits:(p.profile.credits||0)+R}).eq("id",p.profile.id),await i();const V=document.getElementById("nav-credits");V&&(V.textContent=`💰 ${(p.profile.credits||0).toLocaleString("fr")}`),s(`+${R.toLocaleString("fr")} crédits ! ${L.length} carte${L.length>1?"s":""} vendue${L.length>1?"s":""}.`,"success"),l(),c("collection")}),(ge=document.getElementById("market-sell-btn"))==null||ge.addEventListener("click",async()=>{var ie;const L=[...ee];if(!L.length){s("Sélectionne au moins un exemplaire","warning");return}const R=parseInt((ie=document.getElementById("sell-market-price"))==null?void 0:ie.value);if(!R||R<1){s("Prix invalide","error");return}const{error:q}=await A.from("cards").update({is_for_sale:!0,sale_price:R}).in("id",L);if(q){s(q.message,"error");return}const V=L.map(ke=>({seller_id:p.profile.id,card_id:ke,price:R,status:"active"})),{error:re}=await A.from("market_listings").insert(V);re&&console.warn("[Market] insert listings:",re.message),s(`${L.length} carte${L.length>1?"s":""} mise${L.length>1?"s":""} en vente à ${R.toLocaleString("fr")} cr. chacune !`,"success"),l(),c("collection")}),(O=document.getElementById("cancel-sell-btn"))==null||O.addEventListener("click",async()=>{await A.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await A.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),s("Annonce retirée","success"),l(),c("collection")})}const Gt={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},at={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Ut(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const n=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${n}.png`}function Ni(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function Ri(e){var n;const t="https://fcnwclxlkytdfjotqkta.supabase.co";return(n=e==null?void 0:e.clubs)!=null&&n.logo_url?e.clubs.logo_url.startsWith("http")?e.clubs.logo_url:`${t}/storage/v1/object/public/assets/clubs/${e.clubs.logo_url}`:null}function Tn(e,t=44,n=58){var g;const a=e?Ut(e):null,p=e?Ri(e):null,s=Ni(e==null?void 0:e.country_code),d=(e==null?void 0:e.job)||"MIL",l=at[d]||"#555",c={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",i=Number(d==="GK"?e==null?void 0:e.note_g:d==="DEF"?e==null?void 0:e.note_d:d==="MIL"?e==null?void 0:e.note_m:e==null?void 0:e.note_a)||0,r=Math.round(n*.19),f=Math.round(n*.25),m=n-r-f;return e?`<div style="width:${t}px;height:${n}px;border-radius:5px;border:2px solid ${c};background:${l};position:relative;overflow:hidden;flex-shrink:0">
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const d=prompt("Nom du deck :",`Deck ${((s==null?void 0:s.length)||0)+1}`);if(!d)return;const{data:l,error:c}=await A.from("decks").insert({owner_id:n.profile.id,name:d}).select().single();if(c){p(c.message,"error");return}p("Deck créé !","success"),_i(l.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(d=>{d.addEventListener("click",()=>_i(d.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(d=>{d.addEventListener("click",async()=>{const l=prompt("Nouveau nom :",d.dataset.name);if(!l||l===d.dataset.name)return;const{error:c}=await A.from("decks").update({name:l}).eq("id",d.dataset.rename);if(c){p(c.message,"error");return}p("Deck renommé !","success"),Qt(e,t)})}),e.querySelectorAll("[data-delete]").forEach(d=>{d.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${d.dataset.name}" ? Cette action est irréversible.`))return;await A.from("deck_cards").delete().eq("deck_id",d.dataset.delete);const{error:l}=await A.from("decks").delete().eq("id",d.dataset.delete);if(l){p(l.message,"error");return}p("Deck supprimé.","success"),Qt(e,t)})})}async function _i(e,t,n){const{state:a,toast:p}=n;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await A.from("decks").select("*").eq("id",e).single(),{data:d}=await A.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",a.profile.id),l=(d||[]).filter(g=>g.card_type==="player"&&g.player),c=(d||[]).filter(g=>g.card_type==="formation"),i=c.map(g=>g.formation).filter(Boolean),{data:r}=await A.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let f=s.formation||"4-4-2";i.length>0&&!i.includes(f)&&(f=i[0]);const m={deckId:e,name:s.name,formation:f,formationCardId:s.formation_card_id,slots:{},subs:[],playerCards:l,formationCards:c,availableFormations:i};(r||[]).forEach(g=>{g.is_starter?m.slots[g.position]=g.card_id:m.subs.includes(g.card_id)||m.subs.push(g.card_id)}),gt(t,m,n)}function gt(e,t,n){var c;const{navigate:a}=n;Gt[t.formation];const p=$i(t.formation),s=p.filter(i=>t.slots[i]).length,d=t.availableFormations.length>0?t.availableFormations:Object.keys(Gt),l=t.subs.map(i=>t.playerCards.find(r=>r.id===i)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div style="height:100%;overflow-y:auto;padding-bottom:80px;background:var(--page-bg)">
  ...
  </div>`,In(e,t,p,n),document.getElementById("builder-back").addEventListener("click",()=>a("decks")),document.getElementById("formation-select").addEventListener("change",i=>{t.formation=i.target.value;const r=$i(t.formation),f={};r.forEach(m=>{t.slots[m]&&(f[m]=t.slots[m])}),t.slots=f,gt(e,t,n)}),document.getElementById("save-deck").addEventListener("click",()=>Mn(t,n)),e.querySelectorAll("[data-remove-sub]").forEach(i=>{i.addEventListener("click",()=>{t.subs=t.subs.filter(r=>r!==i.dataset.removeSub),gt(e,t,n)})}),(c=document.getElementById("add-sub-btn"))==null||c.addEventListener("click",()=>{zn(t,e,n)})}function In(e,t,n,a){const p=e.querySelector("#deck-field");if(!p)return;const s=zt[t.formation]||{},d=Ii(t.formation)||[],l={};for(const M of n){const z=t.slots[M],B=z?t.playerCards.find(U=>U.id===z):null;l[M]=B?B.player:null}const c=300,i=300,r=48,f=64,m=13,g=16,$=38;function o(M){const z=s[M];return z?{x:z.x*c,y:z.y*i}:null}let _="";for(const[M,z]of d){const B=o(M),U=o(z);if(!B||!U)continue;const F=l[M]?{...l[M],club_id:l[M].club_id,country_code:l[M].country_code,rarity:l[M].rarity}:null,j=l[z]?{...l[z],club_id:l[z].club_id,country_code:l[z].country_code,rarity:l[z].rarity}:null,P=Ze(F,j);P==="#ff3333"||P==="#cc2222"?_+=`<line x1="${B.x.toFixed(1)}" y1="${B.y.toFixed(1)}" x2="${U.x.toFixed(1)}" y2="${U.y.toFixed(1)}" stroke="${P}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(_+=`<line x1="${B.x.toFixed(1)}" y1="${B.y.toFixed(1)}" x2="${U.x.toFixed(1)}" y2="${U.y.toFixed(1)}" stroke="${P}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,_+=`<line x1="${B.x.toFixed(1)}" y1="${B.y.toFixed(1)}" x2="${U.x.toFixed(1)}" y2="${U.y.toFixed(1)}" stroke="${P}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const M of n){const z=o(M);if(!z)continue;const B=l[M],U=M.replace(/\d+/,""),F=at[U]||"#555",j=(z.x-r/2).toFixed(1),P=(z.y-f/2).toFixed(1),te={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[B==null?void 0:B.rarity]||"#aaa";if(B){const ce=Ut(B),ne=Ri(B),S=Ni(B.country_code),Y=Number(U==="GK"?B.note_g:U==="DEF"?B.note_d:U==="MIL"?B.note_m:B.note_a)||0,ee=f-m-g;_+=`<defs><clipPath id="dcp-${M}"><rect x="${j}" y="${(z.y-f/2+m).toFixed(1)}" width="${r}" height="${ee}"/></clipPath></defs>`,_+=`<rect x="${j}" y="${P}" width="${r}" height="${f}" rx="5" fill="${F}"/>`,ce&&(_+=`<image href="${ce}" x="${j}" y="${(z.y-f/2+m).toFixed(1)}" width="${r}" height="${ee}" clip-path="url(#dcp-${M})" preserveAspectRatio="xMidYMin slice"/>`),_+=`<rect x="${j}" y="${P}" width="${r}" height="${m}" fill="rgba(255,255,255,0.93)"/>`,_+=`<text x="${z.x.toFixed(1)}" y="${(z.y-f/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(B.surname_encoded||"").slice(0,9)}</text>`;const D=(z.y+f/2-g).toFixed(1);_+=`<rect x="${j}" y="${D}" width="${r}" height="${g}" fill="rgba(255,255,255,0.93)"/>`,S&&(_+=`<image href="${S}" x="${(z.x-21).toFixed(1)}" y="${(z.y+f/2-g+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),_+=`<text x="${z.x.toFixed(1)}" y="${(z.y+f/2-g/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${Y}</text>`,ne&&(_+=`<image href="${ne}" x="${(z.x+r/2-14).toFixed(1)}" y="${(z.y+f/2-g+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),_+=`<rect x="${j}" y="${P}" width="${r}" height="${f}" rx="5" fill="none" stroke="${te}" stroke-width="2"/>`}else _+=`<rect x="${j}" y="${P}" width="${r}" height="${f}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,_+=`<text x="${z.x.toFixed(1)}" y="${z.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,_+=`<text x="${z.x.toFixed(1)}" y="${(z.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${U}</text>`;_+=`<rect x="${j}" y="${P}" width="${r}" height="${f}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${M}" style="cursor:pointer"/>`}p.innerHTML=`<svg viewBox="${-$} ${-$} ${c+$*2} ${i+$*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${_}</svg>`,p.querySelectorAll(".deck-slot-hit").forEach(M=>{M.addEventListener("click",()=>Ln(M.dataset.pos,t,e,a))})}function Ln(e,t,n,a){var m,g,$;const{openModal:p,closeModal:s}=a,d=e.replace(/\d+/,""),l=t.slots[e],c=l?t.playerCards.find(o=>o.id===l):null;(m=c==null?void 0:c.player)==null||m.id;const i=new Set;Object.entries(t.slots).forEach(([o,_])=>{var z;if(o===e||!_)return;const M=t.playerCards.find(B=>B.id===_);(z=M==null?void 0:M.player)!=null&&z.id&&i.add(M.player.id)}),t.subs.forEach(o=>{var M;const _=t.playerCards.find(z=>z.id===o);(M=_==null?void 0:_.player)!=null&&M.id&&i.add(_.player.id)});const r=new Set,f=t.playerCards.filter(o=>{const _=o.player;return!(_.job===d||_.job2===d)||i.has(_.id)||r.has(_.id)?!1:(r.add(_.id),!0)});f.sort((o,_)=>{const M=d==="GK"?o.player.note_g:d==="DEF"?o.player.note_d:d==="MIL"?o.player.note_m:o.player.note_a;return(d==="GK"?_.player.note_g:d==="DEF"?_.player.note_d:d==="MIL"?_.player.note_m:_.player.note_a)-M}),p(`Choisir ${d} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${f.length>0?f.map(o=>{var U,F;const _=o.player,M=d==="GK"?_.note_g:d==="DEF"?_.note_d:d==="MIL"?_.note_m:_.note_a,z=Ut(_),B={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[_.rarity];return`<div class="player-option" data-card-id="${o.id}"
          ...
          <div style="width:44px;height:44px;border-radius:8px;overflow:hidden;flex-shrink:0;background:#dde;border:2px solid ${at[d]}">
            ${z?`<img src="${z}" style="width:100%;height:100%;object-fit:cover">`:`<div style="width:100%;height:100%;background:${at[d]};display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:700">${d}</div>`}
          </div>
          <!-- Infos -->
          <div style="flex:1;min-width:0">
          ...
          <!-- Note -->
          <div style="width:36px;height:36px;border-radius:8px;background:${at[d]};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:16px;border:2px solid ${B};flex-shrink:0">
            ${M}
          </div>
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${o.map((B,U)=>{const F=U<o.length-1?Ze(B,o[U+1]):null,j=!F||F==="#ff3333"||F==="#cc2222",P=F==="#00ff88"?"+2":F==="#FFD700"?"+1":"";return`
          ...
          ${U<o.length-1?`<div class="duel-link duel-link-${z}" data-idx="${U}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${j?"rgba(255,255,255,0.12)":F};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${j?"none":`0 0 8px ${F}`}">
            ${P?`<span style="position:absolute;top:-13px;left:50%;transform:translateX(-50%);font-size:8px;font-weight:900;color:${F}">${P}</span>`:""}
          </div>`:""}
          `}).join("")}
      </div>
      <div class="duel-score-line duel-score-line-${z}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
  </div>`;const f=()=>{const o=(_,M)=>e.querySelectorAll(_).forEach((z,B)=>{setTimeout(()=>{z.style.opacity="1",z.style.transform="translateY(0) scale(1)"},M+B*90)});o(".duel-card-home",150),o(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((_,M)=>{setTimeout(()=>{_.style.opacity="1"},M*70)}),900),setTimeout(()=>{const _=e.querySelector("#vs-label");_&&(_.style.opacity="1",_.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(M=>M.style.opacity="1")},1250),setTimeout(()=>{m("score-home",l,800),m("score-ai",c,800)},1500)};function m(o,_,M){const z=document.getElementById(o);if(!z)return;const B=performance.now(),U=F=>{const j=Math.min(1,(F-B)/M);z.textContent=Math.round(_*(1-Math.pow(1-j,3))),j<1?requestAnimationFrame(U):z.textContent=_};requestAnimationFrame(U)}requestAnimationFrame(f),t.attacker=i?"home":"ai";const g=i?si():null;i&&(t.boostCard={value:g}),t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:a.map(o=>({name:o.name,note:we(o,"MIL"),portrait:ze(o),job:o.job,country_code:o.country_code,rarity:o.rarity,clubName:o.clubName,clubLogo:o.clubLogo})),aiPlayers:p.map(o=>({name:o.name,note:we(o,"MIL"),portrait:ze(o),job:o.job,country_code:o.country_code,rarity:o.rarity,clubName:o.clubName,clubLogo:o.clubLogo})),homeTotal:l,aiTotal:c,text:`Duel milieu : ${t.clubName} ${l} – ${c} IA → ${i?t.clubName+" attaque":"IA attaque"}`});const $=()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",Me(e,t,n),t.attacker==="ai"&&setTimeout(()=>ti(e,t,n),800)};setTimeout(()=>{const o=document.getElementById("score-home"),_=document.getElementById("score-ai"),M=document.getElementById(i?"duel-row-home":"duel-row-ai"),z=document.getElementById(i?"duel-row-ai":"duel-row-home"),B=i?o:_,U=i?_:o;B&&(B.style.fontSize="80px",B.style.color=i?"#FFD700":"#ff6b6b",B.style.animation="duelPulse .5s ease"+(i?", duelGlow 1.5s ease infinite .5s":"")),U&&(U.style.opacity="0.25"),setTimeout(()=>{M&&(M.style.transformOrigin="center",M.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",M.style.zIndex="5"),setTimeout(()=>{var j;const F=document.getElementById("duel-shock");if(F){const P=(j=z||M)==null?void 0:j.getBoundingClientRect(),te=e.querySelector(".match-screen").getBoundingClientRect();P&&(F.style.top=P.top-te.top+P.height/2+"px"),F.style.animation="shockwave .5s ease-out forwards"}z&&(z.style.transformOrigin="center",z.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var j;const F=document.getElementById("duel-finale");F&&(F.innerHTML=`
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,F.style.transition="opacity .45s ease",F.style.opacity="1",F.style.pointerEvents="auto",(j=document.getElementById("start-match-btn"))==null||j.addEventListener("click",$))},600)},700)},2800)}function io(e){if(e.type==="duel"&&(e.homeTotal!=null||e.aiTotal!=null)){const t=(e.homeTotal||0)>=(e.aiTotal||0);return`
    ...
    <div style="display:flex;align-items:center;gap:4px;${t?"flex-direction:row-reverse":""};background:rgba(255,255,255,0.04);border-radius:8px;padding:5px 8px;border:1px solid rgba(255,255,255,0.07)">
      ...
      ${nt(e.inPlayer||{})}
    </div>`}return`<div style="font-size:11px;color:${e.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${e.type==="goal"?700:400};padding:3px 2px">${e.text||""}</div>`}function Me(e,t,n){var B,U,F,j,P,te,ce,ne;const a=t.selected.map(S=>S.cardId),p=t.usedSubIds||[],s=t.homeSubs.filter(S=>!p.includes(S.cardId)),l=Object.values(t.homeTeam).flat().filter(S=>S.used).length>0&&s.length>0&&t.subsUsed<t.maxSubs,c=!["GK","DEF","MIL","ATT"].some(S=>(t.aiTeam[S]||[]).some(Y=>!Y.used)),i=[...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(S=>!S.used),r=t.phase==="attack"&&c&&i.length===0?[...t.homeTeam.GK||[],...t.homeTeam.DEF||[]].filter(S=>!S.used).map(S=>S.cardId):[];t.log[t.log.length-1];const f=t.phase==="ai-attack"||t.phase==="ai-defense",m=t.phase==="attack",g=t.phase==="defense",$=t.phase==="finished",o=t.gcCards.filter(S=>!t.usedGc.includes(S.id)),_=t.boostCard&&!t.boostUsed;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column",e.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const Y=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
          ...
          </div>`}const S=t.log[t.log.length-1];return S?'<div style="padding:2px 4px">'+io(S)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    ${(()=>{const S=window.innerWidth>=700,Y=(O,L,R)=>{var Ne,He;const q=(t.gcDefs||[]).find(T=>T.name===O.gc_type),V={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[q==null?void 0:q.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",re={purple:"#b06ce0",light_blue:"#00d4ef"}[q==null?void 0:q.color]||"#b06ce0",ie=(q==null?void 0:q.name)||O.gc_type,ke=(q==null?void 0:q.effect)||((Ne=Ge[O.gc_type])==null?void 0:Ne.desc)||"",Ee=q!=null&&q.image_url?`/manager-wars/icons/${q.image_url}`:null,$e=((He=Ge[O.gc_type])==null?void 0:He.icon)||"⚡",_e=Math.round(R*.22),Ae=Math.round(R*.22),Te=R-_e-Ae,qe=ie.length>12?7:9;return`<div class="gc-mini" data-gc-id="${O.id}" data-gc-type="${O.gc_type}"
          style="box-sizing:border-box;width:${L}px;height:${R}px;border-radius:10px;border:2px solid ${re};background:${V};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
        ...
        </div>`},ee=(O,L)=>{var R;return`<div id="boost-card"
          style="box-sizing:border-box;width:${O}px;height:${L}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(L*.04)}px;text-align:center;flex-shrink:0">
            ...
            <div style="font-size:${Math.round(L*.09)}px;color:#000;font-weight:900">+${(R=t.boostCard)==null?void 0:R.value}</div>
          </div>`},D=(O,L)=>L?ee(130,175):Y(O,130,175),Q=(O,L)=>L?ee(68,95):Y(O,68,95),ae=S?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",pe=$?`<button id="btn-results" style="${ae};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:f?`<div style="${ae};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:m?`<button id="btn-action" style="${ae};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:g?`<button id="btn-action" style="${ae};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${ae};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,he=m||g?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",xe=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${S?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
        ${s.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':s.map(O=>`<div class="sub-btn-col" data-sub-id="${O.cardId}" style="cursor:pointer;flex-shrink:0">${Ve(O,76,100)}</div>`).join("")}
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(s);let l=!1;const c=()=>{l||(l=!0,s.remove(),setTimeout(()=>p(),50))};s.addEventListener("click",c),setTimeout(c,3500)}async function At(e,t,n){var r,f;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:a}=n,p=t.homeScore>t.aiScore,s=t.homeScore===t.aiScore,d=p?"victoire":s?"nul":"defaite",l=pn(t.mode,d);t.matchId&&await A.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:p?a.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const c={credits:(a.profile.credits||0)+l,matches_played:(a.profile.matches_played||0)+1};p?c.wins=(a.profile.wins||0)+1:s?c.draws=(a.profile.draws||0)+1:c.losses=(a.profile.losses||0)+1,await A.from("users").update(c).eq("id",a.profile.id),await n.refreshProfile();const i=document.createElement("div");i.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",i.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      </div>`,(D=document.getElementById("lobby-cancel"))==null||D.addEventListener("click",async()=>{$=!0,o&&(A.removeChannel(o),o=null),Y&&await A.from("friend_match_invites").update({status:"declined"}).eq("id",Y),Oe(e),f("home")})},M=async(ne,S)=>{$=!0,o&&(A.removeChannel(o),o=null),await new Promise(Y=>setTimeout(Y,600)),e.isConnected&&xo(e,t,ne,S,d,l)},{data:z}=await A.from("friend_match_invites").select("*").eq("status","pending").or(`and(inviter_id.eq.${g},invitee_id.eq.${c}),and(inviter_id.eq.${c},invitee_id.eq.${g})`).order("created_at",{ascending:!1}).limit(1).maybeSingle();let B,U;if(z&&z.inviter_id===c)U=!1,B=z.id,await A.from("friend_match_invites").update({invitee_deck_id:n,invitee_ready:!0,status:"accepted"}).eq("id",B);else{U=!0;const{data:ne,error:S}=await A.from("friend_match_invites").insert({inviter_id:g,invitee_id:c,friend_id:c,inviter_deck_id:n,inviter_ready:!0,status:"pending"}).select().single();if(S||!ne){m("Erreur création invitation","error"),Oe(e),f("home");return}B=ne.id}if(_(!0,!U,B),!U){const ne={home_id:c,away_id:g,home_deck_id:z.inviter_deck_id,away_deck_id:n,status:"active",mode:"friend"},S=await A.from("matches").insert(ne).select().single(),Y=S.data;if(!Y){m("Erreur création match: "+(((ce=S.error)==null?void 0:ce.message)||""),"error"),Oe(e),f("home");return}await A.from("friend_match_invites").update({match_id:Y.id,status:"matched"}).eq("id",B),M(Y.id,!1);return}let F=!1;const j=ne=>{$||F||(ne.status==="matched"&&ne.match_id?(F=!0,clearInterval(P),clearInterval(te),M(ne.match_id,!0)):ne.status==="declined"?(clearInterval(P),clearInterval(te),m(`${i} a décliné l'invitation`,"warning"),Oe(e),f("home")):ne.invitee_ready&&_(!0,!0,B))},P=setInterval(()=>{if($){clearInterval(P);return}A.from("matchmaking_queue").delete().eq("user_id",g).then(()=>{},()=>{})},3e3),te=setInterval(async()=>{if($||F){clearInterval(te);return}const{data:ne}=await A.from("friend_match_invites").select("*").eq("id",B).maybeSingle();ne&&j(ne)},1200);o=A.channel("friend-invite-"+B).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${B}`},ne=>j(ne.new)).subscribe()}async function xo(e,t,n,a,p=[],s=[]){const{state:d,navigate:l,toast:c}=t,i=a?"p1":"p2",r=a?"p2":"p1",f=(p||[]).map(u=>u.id),m=(p||[]).map(u=>({id:u.id,gc_type:u.gc_type,_gcDef:u._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:g}=await A.from("matches").select("*").eq("id",n).single();if(!g){c("Match introuvable","error"),l("home");return}async function $(){const[{data:u},{data:x},{data:h},{data:y}]=await Promise.all([A.rpc("get_deck_for_match",{p_deck_id:g.home_deck_id}),A.rpc("get_deck_for_match",{p_deck_id:g.away_deck_id}),A.from("users").select("id,pseudo,club_name").eq("id",g.home_id).single(),A.from("users").select("id,pseudo,club_name").eq("id",g.away_id).single()]),b=E=>({cardId:E.card_id,position:E.position,id:E.id,firstname:E.firstname,name:E.surname_encoded,country_code:E.country_code,club_id:E.club_id,job:E.job,job2:E.job2,note_g:Number(E.note_g)||0,note_d:Number(E.note_d)||0,note_m:Number(E.note_m)||0,note_a:Number(E.note_a)||0,rarity:E.rarity,skin:E.skin,hair:E.hair,hair_length:E.hair_length,clubName:E.club_encoded_name||null,clubLogo:E.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),v=((u==null?void 0:u.starters)||[]).map(E=>b(E)),w=((x==null?void 0:x.starters)||[]).map(E=>b(E)),I=(u==null?void 0:u.formation)||"4-4-2",k=(x==null?void 0:x.formation)||"4-4-2";return{p1Team:ft(v,I),p2Team:ft(w,k),p1Subs:((u==null?void 0:u.subs)||[]).map(E=>b(E)),p2Subs:((x==null?void 0:x.subs)||[]).map(E=>b(E)),p1Formation:I,p2Formation:k,p1Name:(h==null?void 0:h.club_name)||(h==null?void 0:h.pseudo)||"Joueur 1",p2Name:(y==null?void 0:y.club_name)||(y==null?void 0:y.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:a?f:[],gc_p2:a?[]:f,gcCardsFull_p1:a?m:[],gcCardsFull_p2:a?[]:m,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let o=g.game_state&&Object.keys(g.game_state).length?g.game_state:null;if(!o)if(a){o=await $();const{data:u}=await A.from("matches").select("game_state").eq("id",n).single();!(u!=null&&u.game_state)||!Object.keys(u.game_state).length?await A.from("matches").update({game_state:o,turn_user_id:g.home_id}).eq("id",n):o=u.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let u=0;u<60&&!o;u++){await new Promise(h=>setTimeout(h,400));const{data:x}=await A.from("matches").select("game_state").eq("id",n).single();x!=null&&x.game_state&&Object.keys(x.game_state).length&&(o=x.game_state)}if(!o){c("Erreur de synchronisation","error"),l("home");return}o.gc_p2=f,o.gcCardsFull_p2=m,await A.from("matches").update({game_state:o}).eq("id",n)}let _=!1,M=!1,z=null,B=!1;const U=new Set,F=new Set;function j(u){var E,C;try{A.removeChannel(P)}catch{}const x=o[i+"Score"]||0,h=o[r+"Score"]||0;if(!M){const N=o.p1Score||0,W=o.p2Score||0,le=(o.usedGc_p1||[]).length,J=(o.usedGc_p2||[]).length,H=u.winner_id||(N>W?g.home_id:W>N?g.away_id:null);(H?d.profile.id===H:d.profile.id<(a?g.away_id:g.home_id))&&(M=!0,cn({player1Id:g.home_id,player2Id:g.away_id,score1:N,score2:W,gc1:le,gc2:J}).catch(Z=>console.warn("[FriendMatch] updateStats:",Z)))}let y,b;u.winner_id?(y=u.winner_id===d.profile.id,b=!1):u.forfeit?(y=x>h,b=!1):(b=x===h,y=x>h),(E=document.getElementById("pvp-end-overlay"))==null||E.remove();const v=document.createElement("div");v.id="pvp-end-overlay",v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const w=b?"🤝":y?"🏆":"😞",I=b?"MATCH NUL":y?"VICTOIRE !":"DÉFAITE",k=b?"#fff":y?"#FFD700":"#ff6b6b";v.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(v),(C=v.querySelector("#pvp-end-home"))==null||C.addEventListener("click",()=>{v.remove(),Oe(e),l("home")})}const P=A.channel("pvp-match-"+n).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${n}`},u=>{const x=u.new;try{if(x.status==="finished"||x.forfeit){if(_)return;_=!0,z&&(clearInterval(z),z=null),x.game_state&&(o=x.game_state),j(x);return}if(x.game_state){const h=o;o=x.game_state;const y=o._lastGC;if(y&&y.seq&&!F.has(y.seq)&&(F.add(y.seq),y.by!==i)){ge(y.type,y.by,()=>Q());return}const b=h[i+"Score"]||0,v=h[r+"Score"]||0,w=o[i+"Score"]||0,I=o[r+"Score"]||0,k=w>b,E=I>v;if((k||E)&&!U.has(o.round)){U.add(o.round);const C=[...o.log||[]].reverse().find(W=>W.isGoal),N=((C==null?void 0:C.homePlayers)||[]).map(W=>({name:W.name,note:W.note,portrait:W.portrait,job:W.job}));xe(N,w,I,k,()=>Q());return}Q()}}catch(h){console.error("[PvP] crash:",h)}}).subscribe();async function te(u){Object.assign(o,u),o.lastActionAt=new Date().toISOString();const{error:x}=await A.from("matches").update({game_state:o}).eq("id",n);x&&c("Erreur de synchronisation","error");try{Q()}catch(h){console.error("[PvP] renderPvpScreen crash:",h)}}async function ce(){if(_)return;_=!0,z&&(clearInterval(z),z=null);const u=a?g.away_id:g.home_id,x=a?"p2":"p1",h=a?"p1":"p2",y={...o,[x+"Score"]:3,[h+"Score"]:0,phase:"finished"};try{await A.from("matches").update({status:"finished",forfeit:!0,winner_id:u,home_score:y.p1Score||0,away_score:y.p2Score||0,game_state:y}).eq("id",n)}catch{}try{A.removeChannel(P)}catch{}setTimeout(()=>{Oe(e),l("home")},800)}const ne={BOOST_STAT:({value:u=1,count:x=1,roles:h=[]},y,b)=>{const v=y[i+"Team"],w=Object.entries(v).filter(([I])=>!h.length||h.includes(I)).flatMap(([I,k])=>k.filter(E=>!E.used).map(E=>({...E,_line:I})));if(!w.length){y.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),b(y);return}S(w,x,`Choisir ${x} joueur(s) à booster (+${u})`,I=>{I.forEach(k=>{const E=(v[k._line]||[]).find(C=>C.cardId===k.cardId);E&&(E.boost=(E.boost||0)+u,y.log.push({text:`⚡ +${u} sur ${E.name}`,type:"info"}))}),y[i+"Team"]=v,b(y)})},DEBUFF_STAT:({value:u=1,count:x=1,roles:h=[],target:y="ai"},b,v)=>{const w=y==="home"?i:r,I=b[w+"Team"],k=y==="home"?"allié":"adverse",E=Object.entries(I).filter(([C])=>!h.length||h.includes(C)).flatMap(([C,N])=>N.filter(W=>!W.used).map(W=>({...W,_line:C})));if(!E.length){b.log.push({text:`🎯 Aucun joueur ${k}`,type:"info"}),v(b);return}S(E,x,`Choisir ${x} joueur(s) ${k}(s) (-${u})`,C=>{C.forEach(N=>{const W=(I[N._line]||[]).find(le=>le.cardId===N.cardId);W&&(W.boost=(W.boost||0)-u,b.log.push({text:`🎯 -${u} sur ${W.name}`,type:"info"}))}),b[w+"Team"]=I,v(b)})},GRAY_PLAYER:({count:u=1,roles:x=[],target:h="ai"},y,b)=>{const v=h==="home"?i:r,w=y[v+"Team"],I=h==="home"?"allié":"adverse",k=Object.entries(w).filter(([E])=>!x.length||x.includes(E)).flatMap(([E,C])=>C.filter(N=>!N.used).map(N=>({...N,_line:E})));if(!k.length){y.log.push({text:`❌ Aucun joueur ${I}`,type:"info"}),b(y);return}S(k,u,`Choisir ${u} joueur(s) ${I}(s) à exclure`,E=>{E.forEach(C=>{const N=(w[C._line]||[]).find(W=>W.cardId===C.cardId);N&&(N.used=!0,y.log.push({text:`❌ ${N.name} exclu !`,type:"info"}))}),y[v+"Team"]=w,b(y)})},REVIVE_PLAYER:({count:u=1,roles:x=[]},h,y)=>{const b=h[i+"Team"],v=Object.entries(b).filter(([w])=>!x.length||x.includes(w)).flatMap(([w,I])=>I.filter(k=>k.used).map(k=>({...k,_line:w})));if(!v.length){h.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),y(h);return}S(v,u,`Choisir ${u} joueur(s) à ressusciter`,w=>{w.forEach(I=>{const k=(b[I._line]||[]).find(E=>E.cardId===I.cardId);k&&(k.used=!1,h.log.push({text:`💫 ${k.name} ressuscité !`,type:"info"}))}),h[i+"Team"]=b,y(h)})},REMOVE_GOAL:({},u,x)=>{const h=r+"Score";u[h]>0?(u[h]--,u.log.push({text:"🚫 Dernier but annulé !",type:"info"})):u.log.push({text:"🚫 Aucun but à annuler",type:"info"}),x(u)},ADD_GOAL_DRAW:({},u,x)=>{u[i+"Score"]===u[r+"Score"]?(u[i+"Score"]++,u.log.push({text:"🎯 But bonus !",type:"info"})):u.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),x(u)},ADD_SUB:({value:u=1},x,h)=>{x.maxSubs=(x.maxSubs||3)+u,x.log.push({text:`🔄 +${u} remplacement(s)`,type:"info"}),h(x)},CUSTOM:({},u,x)=>x(u)};function S(u,x,h,y){const b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let v=[];function w(){var k,E;const I=u.map(C=>{const N={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[C._line]||"#555",W=we(C,C._line)+(C.boost||0),J=v.find(K=>K.cardId===C.cardId)?"#FFD700":"rgba(255,255,255,0.25)",H=C.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${C.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${J};background:${N};overflow:hidden;cursor:pointer;${H}">
        ...
        </div>`,(k=b.querySelector("#pp-close"))==null||k.addEventListener("click",()=>b.remove()),b.querySelectorAll(".pp-item").forEach(C=>{C.addEventListener("click",()=>{const N=C.dataset.cid,W=u.find(J=>J.cardId===N),le=v.findIndex(J=>J.cardId===N);W&&(le>-1?v.splice(le,1):v.length<x&&v.push(W),w())})}),(E=b.querySelector("#pp-confirm"))==null||E.addEventListener("click",()=>{b.remove(),y(v)})}w(),document.body.appendChild(b)}async function Y(u,x){const y=(o["gcCardsFull_"+i]||[]).find(I=>I.id===u),b=(y==null?void 0:y._gcDef)||(o.gcDefs||[]).find(I=>{var k;return I.name===x||((k=I.name)==null?void 0:k.toLowerCase().trim())===(x==null?void 0:x.toLowerCase().trim())}),v=[...o["usedGc_"+i]||[],u],w={type:x,by:i,seq:(o._gcAnimSeq||0)+1};F.add(w.seq),ge(x,i,async()=>{if(b!=null&&b.effect_type&&b.effect_type!=="CUSTOM"){const k=ne[b.effect_type];if(k){const E={...o};k(b.effect_params||{},E,async C=>{C["usedGc_"+i]=v,C._lastGC=w,C._gcAnimSeq=w.seq,await te(C)});return}}const I={...o};switch(x){case"Remplacement+":I.maxSubs=(I.maxSubs||3)+1,I.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const k=r+"Score";I[k]>0&&(I[k]--,I.log.push({text:"🚫 But annulé",type:"info"}));break}}I["usedGc_"+i]=v,I._lastGC=w,I._gcAnimSeq=w.seq,await te(I)})}function ee(u,x){const h="usedCardIds_"+u,y=new Set(o[h]||[]);x.forEach(b=>y.add(b)),o[h]=[...y]}function D(){for(const u of["p1","p2"]){const x=new Set(o["usedCardIds_"+u]||[]),h=o[u+"Team"];if(h)for(const y of["GK","DEF","MIL","ATT"])(h[y]||[]).forEach(b=>{b.used=x.has(b.cardId)})}}function Q(){var Qe,mt,xt,yt,ht,X;if(o.phase==="reveal")return ae();if(o.phase==="midfield")return he();if(o.phase==="finished")return T();const u=o[i+"Team"],x=o[r+"Team"];D();const h=o[i+"Score"],y=o[r+"Score"],b=o[i+"Name"],v=o[r+"Name"],w=o.phase===i+"-attack",I=o.phase===i+"-defense",k=Array.isArray(o["selected_"+i])?o["selected_"+i]:[],E=k.map(G=>G.cardId),C=window.innerWidth>=700,N=o[i+"Subs"]||[],W=o["usedSubIds_"+i]||[],le=N.filter(G=>!W.includes(G.cardId)),J=o["gcCardsFull_"+i]||[],H=o["usedGc_"+i]||[],K=J.filter(G=>!H.includes(G.id)),Z=o.boostOwner===i&&!o.boostUsed,de=!["GK","DEF","MIL","ATT"].some(G=>(x[G]||[]).some(oe=>!oe.used)),ue=[...u.MIL||[],...u.ATT||[]].filter(G=>!G.used),fe=w&&de&&ue.length===0?[...u.GK||[],...u.DEF||[]].filter(G=>!G.used).map(G=>G.cardId):[];function me(G,oe,se){var wt,mi;const ye=G._gcDef,Ye={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ye==null?void 0:ye.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",We={purple:"#b06ce0",light_blue:"#00d4ef"}[ye==null?void 0:ye.color]||"#b06ce0",Je=(ye==null?void 0:ye.name)||G.gc_type,et=(ye==null?void 0:ye.effect)||((wt=Ge[G.gc_type])==null?void 0:wt.desc)||"",Mt=ye!=null&&ye.image_url?`/manager-wars/icons/${ye.image_url}`:null,St=((mi=Ge[G.gc_type])==null?void 0:mi.icon)||"⚡",bt=Math.round(se*.22),lt=Math.round(se*.22),vt=se-bt-lt,Ct=Je.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${G.id}" data-gc-type="${G.gc_type}"
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(v);let k=!1;const E=()=>{k||(k=!0,v.remove(),setTimeout(()=>b(),50))};v.addEventListener("click",E),setTimeout(E,3500)}function ge(u,x,h){var H,K;const y=(o.gcDefs||[]).find(Z=>{var de;return Z.name===u||((de=Z.name)==null?void 0:de.toLowerCase().trim())===(u==null?void 0:u.toLowerCase().trim())}),b={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[y==null?void 0:y.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",v={purple:"#b06ce0",light_blue:"#00d4ef"}[y==null?void 0:y.color]||"#b06ce0",w=(y==null?void 0:y.name)||u,I=(y==null?void 0:y.effect)||((H=Ge[u])==null?void 0:H.desc)||"",k=y!=null&&y.image_url?`/manager-wars/icons/${y.image_url}`:null,E=((K=Ge[u])==null?void 0:K.icon)||"⚡",N=x===i?"Vous":o[x+"Name"]||"Adversaire",W=document.createElement("div");W.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",W.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(W);let le=!1;const J=()=>{le||(le=!0,W.remove(),setTimeout(()=>h&&h(),50))};W.addEventListener("click",J),setTimeout(J,3e3)}function O(u,x){var W,le,J,H;const y=(o["gcCardsFull_"+i]||[]).find(K=>K.id===u),b=y==null?void 0:y._gcDef,v={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[b==null?void 0:b.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",w={purple:"#b06ce0",light_blue:"#00d4ef"}[b==null?void 0:b.color]||"#b06ce0",I=(b==null?void 0:b.name)||x,k=(b==null?void 0:b.effect)||((W=Ge[x])==null?void 0:W.desc)||"Carte spéciale.",E=b!=null&&b.image_url?`/manager-wars/icons/${b.image_url}`:null,C=((le=Ge[x])==null?void 0:le.icon)||"⚡",N=document.createElement("div");N.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",N.innerHTML=`
      <div style="width:190px;border-radius:16px;border:3px solid ${w};background:${v};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 40px ${w}66">
        ...
        <div style="height:160px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${E?`<img src="${E}" style="max-width:150px;max-height:150px;object-fit:contain">`:`<span style="font-size:72px">${C}</span>`}
        </div>
        <div style="padding:10px;background:rgba(0,0,0,0.38);text-align:center">
      ...
      </div>`,document.body.appendChild(h),(y=h.querySelector("#bp-close"))==null||y.addEventListener("click",()=>h.remove()),h.querySelectorAll(".bp-item").forEach(b=>{b.addEventListener("click",async()=>{const v=b.dataset.cid,w=x.find(k=>k.cardId===v);if(!w)return;const I=(u[w._line]||[]).find(k=>k.cardId===v);I&&(I.boost=(I.boost||0)+o.boostValue),h.remove(),await te({[i+"Team"]:u,boostUsed:!0})})})}function R(u=null){var le,J;if(!(o.phase===i+"-attack")){c("Remplacement uniquement avant votre attaque","warning");return}const h=o[i+"Team"],y=o["usedSubIds_"+i]||[],b=o.maxSubs||3;if(y.length>=b){c(`Maximum ${b} remplacements atteint`,"warning");return}const v=Object.entries(h).flatMap(([H,K])=>(K||[]).filter(Z=>Z.used).map(Z=>({...Z,_line:H}))),w=(o[i+"Subs"]||[]).filter(H=>!y.includes(H.cardId));if(!v.length){c("Aucun joueur utilisé à remplacer","warning");return}if(!w.length){c("Aucun remplaçant disponible","warning");return}let I=Math.max(0,v.findIndex(H=>H.cardId===u));const k=((le=v[I])==null?void 0:le._line)||((J=v[I])==null?void 0:J.job);let E=Math.max(0,w.findIndex(H=>H.job===k)),C=!1;const N=document.createElement("div");N.id="pvp-sub-overlay",N.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function W(){var me,be,Se,Re,Fe,Ie;const H=v[I],K=w[E],Z=Math.min(130,Math.round((window.innerWidth-90)/2)),de=Math.round(Z*1.35),ue=Le=>`background:rgba(255,255,255,0.12);border:none;color:${Le?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Le?"default":"pointer"};flex-shrink:0`;N.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(b);let w=!1;const I=()=>{w||(w=!0,b.remove(),setTimeout(()=>h(),50))};b.addEventListener("click",I),setTimeout(I,2200)}function V(){var x;const u=document.createElement("div");u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",u.innerHTML=`
      <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-transform:uppercase">Équipe adverse</div>
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${o[r+"Name"]}</div>
      <div style="width:min(90vw,420px)">${st(o[r+"Team"],o[r+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(u),(x=u.querySelector("#pvp-opp-close"))==null||x.addEventListener("click",()=>u.remove())}function re(){var h;const u=o.log||[],x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",x.innerHTML=`
      <div style="display:flex;align-items:center;padding:14px 16px;border-bottom:1px solid rgba(255,255,255,0.1);flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">📋 Historique</div>
        <button id="pvp-hist-close" style="background:none;border:none;color:rgba(255,255,255,0.6);font-size:20px;cursor:pointer">✕</button>
      </div>
      ...
      </div>`,document.body.appendChild(x),(h=x.querySelector("#pvp-hist-close"))==null||h.addEventListener("click",()=>x.remove())}async function ie(){if(o.phase!==i+"-attack")return;const u=i==="p1"?"p2":"p1",x=(o.round||0)+1,h=[...o.log||[]];h.push({type:"info",text:`⏭️ ${o[i+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const y=qe(o),b=Te(u),v=y||!b?"finished":u+"-attack";await te({["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:v,attacker:u,round:x,log:h}),v==="finished"&&await He(o)}async function ke(){const u=o[i+"Team"],x=!["GK","DEF","MIL","ATT"].some(v=>(o[r+"Team"][v]||[]).some(w=>!w.used)),h=(o["selected_"+i]||[]).map(v=>{const w=(u[v._role]||[]).find(W=>W.cardId===v.cardId)||v,I=x&&["GK","DEF"].includes(v._role),k=u[v._role]||[],E=k.findIndex(W=>W.cardId===v.cardId),C=ot(k.length),N=E>=0?C[E]:w._col??1;return{...w,_line:v._role,_col:N,...I?{note_a:Math.max(1,Number(w.note_a)||0)}:{}}});if(!h.length)return;const y=Pt(h,o.modifiers[i]||{});ee(i,h.map(v=>v.cardId)),h.forEach(v=>{const w=(u[v._role]||[]).find(I=>I.cardId===v.cardId);w&&(w.used=!0)}),o["selected_"+i]=[],Q();const b=[...o.log||[]];if(x){const v=(o[i+"Score"]||0)+1,w=h.map(N=>({name:N.name,note:we(N,N._line||"ATT"),portrait:ze(N),job:N.job}));b.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:w,homeTotal:y.total,aiTotal:0});const I=(o.round||0)+1,k=i==="p1"?"p2":"p1",E={...o,[i+"Team"]:u,[i+"Score"]:v},C=qe(E);U.add(I),xe(w,v,o[r+"Score"]||0,!0,async()=>{await te({[i+"Team"]:u,[i+"Score"]:v,["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:C?"finished":k+"-attack",attacker:k,round:I,log:b}),C&&await He({...o,[i+"Score"]:v})});return}b.push({type:"pending",text:`⚔️ ${o[i+"Name"]} attaque (${y.total})`}),await te({[i+"Team"]:u,[r+"Team"]:o[r+"Team"],pendingAttack:{...y,players:h,side:i},["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},phase:r+"-defense",log:b})}async function Ee(){const u=o[i+"Team"],x=(o["selected_"+i]||[]).map(H=>{const K=(u[H._role]||[]).find(me=>me.cardId===H.cardId)||H,Z=u[H._role]||[],de=Z.findIndex(me=>me.cardId===H.cardId),ue=ot(Z.length),fe=de>=0?ue[de]:K._col??1;return{...K,_line:H._role,_col:fe}}),h=Ot(x,o.modifiers[i]||{});ee(i,x.map(H=>H.cardId)),x.forEach(H=>{const K=(u[H._role]||[]).find(Z=>Z.cardId===H.cardId);K&&(K.used=!0)}),o["selected_"+i]=[],Q();const y=Ht(o.pendingAttack.total,h.total,o.modifiers[i]||{}),b=o.pendingAttack.side,v=y==="attack"||(y==null?void 0:y.goal),w=b==="p1"?"p2":"p1",I=(o.round||0)+1,k=(o.pendingAttack.players||[]).map(H=>({name:H.name,note:we(H,H._line||"ATT"),portrait:ze(H),job:H.job})),E=[...o.log||[]];E.push({type:"duel",isGoal:v,homeScored:v&&b===i,text:v?`⚽ BUT de ${o[b+"Name"]} ! (${o.pendingAttack.total} vs ${h.total})`:`✋ Attaque stoppée (${o.pendingAttack.total} vs ${h.total})`,homePlayers:k,aiPlayers:x.map(H=>({name:H.name,note:we(H,H._line||"DEF"),portrait:ze(H),job:H.job})),homeTotal:o.pendingAttack.total,aiTotal:h.total});const C=v?(o[b+"Score"]||0)+1:o[b+"Score"]||0,N={...o,[i+"Team"]:u,[b+"Score"]:C},W=qe(N),le=W?"finished":w+"-attack",J=async()=>{await te({[i+"Team"]:u,[r+"Team"]:o[r+"Team"],[b+"Score"]:C,["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:le,attacker:w,round:I,log:E}),(le==="finished"||W)&&await He({...o,[b+"Score"]:C})};if(v){const H=b===i,K=H?C:o[i+"Score"]||0,Z=H?o[r+"Score"]||0:C;U.add(I),xe(k,K,Z,H,J)}else await J()}function $e(u){return["MIL","ATT"].some(x=>(u[x]||[]).some(h=>!h.used))}function _e(u){return["GK","DEF","MIL","ATT"].some(x=>(u[x]||[]).some(h=>!h.used))}function Ae(u){return _e(u)&&!$e(u)}function Te(u){const x=o[u+"Team"],h=o[(u==="p1"?"p2":"p1")+"Team"];return!!($e(x)||!_e(h)&&Ae(x))}function qe(u){const x=["MIL","ATT"].some(E=>(u.p1Team[E]||[]).some(C=>!C.used)),h=["MIL","ATT"].some(E=>(u.p2Team[E]||[]).some(C=>!C.used)),y=_e(u.p1Team),b=_e(u.p2Team);return!x&&!(!b&&y)&&(!h&&!(!y&&b))}function Ne(u){const x=u.p1Score||0,h=u.p2Score||0;return x===h?null:x>h?g.home_id:g.away_id}async function He(u){try{await A.from("matches").update({status:"finished",winner_id:Ne(u),home_score:u.p1Score||0,away_score:u.p2Score||0}).eq("id",n)}catch(x){console.error("[PvP] finishMatch:",x)}}function T(){var b;const u=o[i+"Score"],x=o[r+"Score"],h=u>x,y=u===x;Xe(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
  ...
  `;function c(i){const r=document.getElementById("mkt-content"),f=i==="buy"?d:l;if(f.length===0){r.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${m.map(g=>{var B,U,F,j;const $=(B=g.card)==null?void 0:B.player;if(!$)return"";const o=$.job==="GK"?$.note_g:$.job==="DEF"?$.note_d:$.job==="MIL"?$.note_m:$.note_a,_=yo[$.rarity],M=(n.profile.credits||0)>=g.price,z=g.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${z?"opacity:0.65":""}">
      ...
      </div>`,document.body.appendChild(a);const p=s=>{a.remove(),n(s)};a.querySelector("#buy-ok").addEventListener("click",()=>p(!0)),a.querySelector("#buy-cancel").addEventListener("click",()=>p(!1)),a.addEventListener("click",s=>{s.target===a&&p(!1)})})}async function wo(e,t,n){const{toast:a}=n,{data:p}=await A.from("market_listings").select("card_id").eq("id",e).single();await A.from("market_listings").update({status:"cancelled"}).eq("id",e),p&&await A.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",p.card_id),a("Annonce retirée","success"),fi(t,n)}function _o(e){return{GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[e]||"#888"}async function $o(e,{state:t,navigate:n,toast:a}){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:p}=await A.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100);e.innerHTML=`
  <div class="page">
    <div class="page-header">
