...
import{s as k,F as hi,h as Pt,j as Yi,l as at,i as Ao,k as zo,b as So}from"./formation-links-Dnttc-Cp.js";const Lo="/";function Li(e,{navigate:t,toast:i}){let s="login";const l=()=>{var a,c,g,n,o,m;const d=s==="login";e.innerHTML=`
    <div style="
    ...
    </style>`,(a=document.getElementById("tab-login-btn"))==null||a.addEventListener("click",()=>{s="login",l()}),(c=document.getElementById("tab-reg-btn"))==null||c.addEventListener("click",()=>{s="register",l()}),d?((g=document.getElementById("login-password"))==null||g.addEventListener("keydown",b=>{var u;b.key==="Enter"&&((u=document.getElementById("login-btn"))==null||u.click())}),(n=document.getElementById("login-btn"))==null||n.addEventListener("click",async()=>{const b=document.getElementById("login-email").value.trim(),u=document.getElementById("login-password").value,A=document.getElementById("login-error");if(A.textContent="",!b||!u){A.textContent="Remplissez tous les champs.";return}const r=document.getElementById("login-btn");r.textContent="⏳ Connexion…",r.disabled=!0;const{error:p}=await k.auth.signInWithPassword({email:b,password:u});if(r.textContent="⚽ Se connecter",r.disabled=!1,p){A.textContent=p.message.includes("Invalid")?"Email ou mot de passe incorrect.":p.message;return}window.location.reload()})):((o=document.getElementById("reg-confirm"))==null||o.addEventListener("keydown",b=>{var u;b.key==="Enter"&&((u=document.getElementById("reg-btn"))==null||u.click())}),(m=document.getElementById("reg-btn"))==null||m.addEventListener("click",async()=>{const b=document.getElementById("reg-email").value.trim(),u=document.getElementById("reg-password").value,A=document.getElementById("reg-confirm").value,r=document.getElementById("reg-error");if(r.textContent="",!b||!u||!A){r.textContent="Remplissez tous les champs.";return}if(u.length<6){r.textContent="Mot de passe trop court (min. 6 caractères).";return}if(u!==A){r.textContent="Les mots de passe ne correspondent pas.";return}const p=document.getElementById("reg-btn");p.textContent="⏳ Création…",p.disabled=!0;const{error:I}=await k.auth.signUp({email:b,password:u});if(p.textContent="🚀 Créer mon compte",p.disabled=!1,I){r.textContent=I.message;return}i("Compte créé ! Connecte-toi pour commencer.","success",4e3),s="login",l(),setTimeout(()=>{const L=document.getElementById("login-email");L&&(L.value=b)},50)}))};l()}function Io(e,{state:t,navigate:i,toast:s,refreshProfile:l}){let d="#1A6B3C",a="#D4A017";e.innerHTML=`
  ...
  `;function c(){var A;const n=document.getElementById("logo-preview"),o=document.getElementById("logo-initials"),m=((A=document.getElementById("setup-club"))==null?void 0:A.value)||"MW",b=m.trim().split(" ").filter(Boolean),u=b.length>=2?(b[0][0]+b[1][0]).toUpperCase():m.slice(0,2).toUpperCase();n&&(n.style.background=a,n.style.borderColor=d),o&&(o.textContent=u,o.style.color=d)}document.getElementById("color1").addEventListener("input",n=>{d=n.target.value,document.getElementById("swatch1").style.background=d,c()}),document.getElementById("color2").addEventListener("input",n=>{a=n.target.value,document.getElementById("swatch2").style.background=a,c()});function g(n){document.querySelectorAll(".setup-step").forEach(o=>o.classList.remove("active")),document.getElementById(`step-${n}`).classList.add("active"),document.getElementById("step-num").textContent=n,document.getElementById("progress-fill").style.width=`${Math.round(n/3*100)}%`,n===3&&c()}document.getElementById("step1-next").addEventListener("click",async()=>{const n=document.getElementById("setup-pseudo").value.trim(),o=document.getElementById("step1-error");if(o.textContent="",n.length<3){o.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:m}=await k.from("users").select("id").eq("pseudo",n).maybeSingle();if(m){o.textContent="Ce pseudo est déjà pris.";return}g(2)}),document.getElementById("step2-back").addEventListener("click",()=>g(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const n=document.getElementById("setup-club").value.trim(),o=document.getElementById("step2-error");if(o.textContent="",n.length<2){o.textContent="Nom trop court (min. 2 caractères).";return}const{data:m}=await k.from("users").select("id").eq("club_name",n).maybeSingle();if(m){o.textContent="Ce nom de club est déjà pris.";return}g(3)}),document.getElementById("step3-back").addEventListener("click",()=>g(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const n=document.getElementById("setup-pseudo").value.trim(),o=document.getElementById("setup-club").value.trim(),m=document.getElementById("step3-error"),b=document.getElementById("step3-finish");m.textContent="",b.disabled=!0,b.textContent="Création en cours…";try{const{error:u}=await k.from("users").insert({id:t.user.id,pseudo:n,club_name:o,club_color1:d,club_color2:a,credits:1e4});if(u)throw u;await Mo(t.user.id),await l(),s(`Bienvenue ${n} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(u){m.textContent=u.message,b.disabled=!1,b.textContent="🚀 Créer mon profil !"}})}async function Mo(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await k.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const jo="modulepreload",Co=function(e){return"/"+e},Ii={},ti=function(t,i,s){let l=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),c=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));l=Promise.allSettled(i.map(g=>{if(g=Co(g),g in Ii)return;Ii[g]=!0;const n=g.endsWith(".css"),o=n?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${g}"]${o}`))return;const m=document.createElement("link");if(m.rel=n?"stylesheet":jo,n||(m.as="script"),m.crossOrigin="",m.href=g,c&&m.setAttribute("nonce",c),document.head.appendChild(m),n)return new Promise((b,u)=>{m.addEventListener("load",b),m.addEventListener("error",()=>u(new Error(`Unable to preload CSS for ${g}`)))})}))}function d(a){const c=new Event("vite:preloadError",{cancelable:!0});if(c.payload=a,window.dispatchEvent(c),!c.defaultPrevented)throw a}return l.then(a=>{for(const c of a||[])c.status==="rejected"&&d(c.reason);return t().catch(d)})},Dt="#1A6B3C",Gt="#cc2222",qo="#D4A017",Mi="#888";async function Bo(e,t){const{state:i,toast:s}=t;e.innerHTML=`
    ...
    </div>`,await Wi(i,s,t),document.getElementById("btn-add-friend").addEventListener("click",()=>Do(i,s)),document.getElementById("btn-accept-friend").addEventListener("click",()=>Ji(i,s,null,t))}async function Wi(e,t,i={}){const{navigate:s}=i,l=e.user.id,{data:d,error:a}=await k.from("friendships").select("id, requester_id, addressee_id").eq("status","accepted").or(`requester_id.eq.${l},addressee_id.eq.${l}`),{count:c}=await k.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",l).eq("status","pending"),g=document.getElementById("pending-badge");g&&(c>0?(g.style.display="flex",g.textContent=c):g.style.display="none");const n=document.getElementById("friends-list");if(!n)return;if(a){console.error("[Friends] Erreur:",a),n.innerHTML=`<div style="color:${Gt};text-align:center;padding:16px">Erreur chargement : ${a.message}</div>`;return}const o=(d||[]).map(u=>u.requester_id===l?u.addressee_id:u.requester_id);let m={};if(o.length){const{data:u}=await k.from("users").select("id, pseudo, club_name, last_seen_at, club_color1, club_color2").in("id",o);(u||[]).forEach(A=>{m[A.id]=A})}const b=(d||[]).map(u=>({friendshipId:u.id,friend:m[u.requester_id===l?u.addressee_id:u.requester_id]||{pseudo:"?"}}));if(!b.length){n.innerHTML=`
    ...
    <div class="popup-box">
      ...
      ${o.length?`<div style="display:flex;flex-direction:column;gap:8px;max-height:50vh;overflow-y:auto;margin-bottom:14px">
            ${o.map(b=>{var u,A,r;return`
              <div style="display:flex;align-items:center;gap:10px;background:#f9f9f9;border-radius:10px;padding:10px 12px">
                <div style="flex:1;font-size:14px;font-weight:700">${((u=b.requester)==null?void 0:u.club_name)||((A=b.requester)==null?void 0:A.pseudo)||"?"}
    ...
    ${wi()}`,document.body.appendChild(n);const m=()=>n.remove();n.querySelector("#pending-close").addEventListener("click",m),n.addEventListener("click",b=>{b.target===n&&m()}),n.querySelectorAll("[data-accept]").forEach(b=>{b.addEventListener("click",async()=>{const{error:u}=await k.from("friendships").update({status:"accepted"}).eq("id",b.dataset.accept);if(u){t("Erreur : "+u.message,"error");return}b.closest("div[style]").remove(),t("✅ Ami accepté !","success"),Wi(e,t,s),i&&i()})}),n.querySelectorAll("[data-decline]").forEach(b=>{b.addEventListener("click",async()=>{await k.from("friendships").delete().eq("id",b.dataset.decline),b.closest("div[style]").remove(),t("Demande refusée","info"),i&&i()})})}async function Go(e,t,i){const s=e.user.id,[l,d]=[s,t].sort(),a=s===l,{data:c}=await k.from("friend_match_stats").select("*").eq("player1_id",l).eq("player2_id",d).single(),g=e.profile.club_name||e.profile.pseudo||"Moi",n=c||{},o=a?n.wins_p1||0:n.wins_p2||0,m=a?n.wins_p2||0:n.wins_p1||0,b=n.draws||0,u=a?n.goals_p1||0:n.goals_p2||0,A=a?n.goals_p2||0:n.goals_p1||0,r=a?n.gc_used_p1||0:n.gc_used_p2||0,p=a?n.gc_used_p2||0:n.gc_used_p1||0,I=n.matches_total||0,L=(H,C,B,j=Dt,W=Gt)=>`
    <div style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:8px;padding:10px 0;border-bottom:1px solid #f0f0f0">
      ...
      <div style="text-align:left;font-size:18px;font-weight:900;color:${W}">${B}</div>
    </div>`,F=vi();F.innerHTML=`
    ...
    </style>`}async function Xi({player1Id:e,player2Id:t,score1:i,score2:s,gc1:l,gc2:d}){const[a,c]=[e,t].sort(),g=e!==a,n=g?s:i,o=g?i:s,m=g?d:l,b=g?l:d,u=n>o?1:0,A=o>n?1:0,r=n===o?1:0,{data:p}=await k.from("friend_match_stats").select("*").eq("player1_id",a).eq("player2_id",c).single();p?await k.from("friend_match_stats").update({wins_p1:p.wins_p1+u,wins_p2:p.wins_p2+A,draws:p.draws+r,goals_p1:p.goals_p1+n,goals_p2:p.goals_p2+o,gc_used_p1:p.gc_used_p1+m,gc_used_p2:p.gc_used_p2+b,matches_total:p.matches_total+1}).eq("player1_id",a).eq("player2_id",c):await k.from("friend_match_stats").insert({player1_id:a,player2_id:c,wins_p1:u,wins_p2:A,draws:r,goals_p1:n,goals_p2:o,gc_used_p1:m,gc_used_p2:b,matches_total:1})}const Po="2026.07.05-0004";async function ji(e,{state:t,navigate:i,toast:s}){var d,a;const l=t.profile;l&&(e.innerHTML=`
  <style>
  ...
  `,e.querySelectorAll("[data-nav]").forEach(c=>{c.addEventListener("click",g=>{g.preventDefault(),i(c.dataset.nav)})}),(d=document.getElementById("nav-rankings"))==null||d.addEventListener("click",()=>i("rankings")),(a=document.getElementById("nav-matches"))==null||a.addEventListener("click",()=>i("matches")),e.querySelectorAll("[data-action]").forEach(c=>{c.addEventListener("click",()=>{c.classList.add("tapped"),setTimeout(()=>c.classList.remove("tapped"),200);const g=c.dataset.action;if(g==="match-ai"){Ho(i);return}if(g==="match-random"){i("match",{matchMode:"random"});return}if(g==="match-friend"){i("friends");return}if(g==="mini-league"){i("mini-league");return}if(g==="ranked"){ctx.toast("Mode Ranked — Bientôt disponible !","info");return}s("Bientôt disponible","info")})}),window.innerWidth<768&&requestAnimationFrame(()=>{var H,C,B,j,W,de;const c=((H=window.visualViewport)==null?void 0:H.height)||window.innerHeight,g=((C=document.querySelector(".top-nav"))==null?void 0:C.offsetHeight)||56,n=((B=document.querySelector(".bottom-nav"))==null?void 0:B.offsetHeight)||60,o=((j=e.querySelector(".hero-compact"))==null?void 0:j.offsetHeight)||52,m=e.querySelector(".ranked-banner");e.querySelector(".play-grid");const b=((de=(W=e.querySelector("#logout-btn"))==null?void 0:W.closest("div"))==null?void 0:de.offsetHeight)||44,u=["friend-requests-banner","match-invite-banner","ongoing-match-banner"].reduce((re,P)=>{var X;return re+(((X=document.getElementById(P))==null?void 0:X.offsetHeight)||0)},0),A=14*5,r=c-g-n-o-b-u-A,p=Math.max(80,Math.round(r*.28)),I=Math.max(160,Math.round(r*.72)),L=Math.floor((I-10)/2);m&&(m.style.minHeight=m.style.maxHeight=p+"px"),e.querySelectorAll(".play-grid .play-card").forEach(re=>{re.style.minHeight=re.style.height=L+"px"});const F=Math.round(L*.55);e.querySelectorAll(".play-card .play-icon").forEach(re=>{re.style.height=F+"px"})}),document.getElementById("logout-btn").addEventListener("click",async()=>{await k.auth.signOut(),window.location.reload()}),Qi(t,s),Oo(t,s,i),Zi(t,s,i))}async function Zi(e,t,i){const s=document.getElementById("ongoing-match-banner");if(!s)return;const l=e.profile.id,{data:d}=await k.from("matches").select("id, home_id, away_id, status, mode").eq("status","active").or(`home_id.eq.${l},away_id.eq.${l}`).order("created_at",{ascending:!1});if(!(d!=null&&d.length)){s.innerHTML="";return}const a=d.map(g=>g.home_id===l?g.away_id:g.home_id).filter(Boolean);let c={};if(a.length){const{data:g}=await k.from("users").select("id, pseudo, club_name").in("id",a);(g||[]).forEach(n=>{c[n.id]=n.club_name||n.pseudo})}s.innerHTML=d.map(g=>{const n=g.home_id===l?g.away_id:g.home_id,o=c[n]||"Adversaire",m=g.mode==="mini_league";return`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      stadium_def:stadium_definitions(id, name, club_id, country_code, image_url,
        club:clubs(encoded_name, logo_url))`).eq("owner_id",i.profile.id),{data:g}=await k.from("players").select(`id, firstname, surname_encoded, country_code, club_id, job, job2,
      note_g, note_d, note_m, note_a, rarity, note_min, note_max, skin, hair, hair_length,
      clubs(encoded_name, logo_url)`).eq("is_active",!0),n=(c||[]).filter(J=>J.card_type==="player"&&J.player),o=(c||[]).filter(J=>J.card_type==="game_changer"),m=(c||[]).filter(J=>J.card_type==="formation"),b=(c||[]).filter(J=>J.card_type==="stadium"),{data:u}=await k.from("gc_definitions").select("name,gc_type,color,effect,image_url"),A={};(u||[]).forEach(J=>{A[J.name]=J});const{data:r}=await k.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),p={};(r||[]).forEach(J=>{p[J.id]=J}),b.forEach(J=>{J.stadium_def&&(p[J.stadium_id]=J.stadium_def)});const I=Object.keys(hi),L=Object.keys(Ve),F={};n.forEach(J=>{const oe=J.player.id;F[oe]=(F[oe]||0)+1}),new Set(Object.keys(F).map(J=>String(J)));const H=new Set(m.map(J=>J.formation)),C=new Set(o.map(J=>J.gc_type));let B="player",j="Tous",W="",de=!1;function re(){return[...n].sort((J,oe)=>{const ge=Kt.indexOf(J.player.job),Y=Kt.indexOf(oe.player.job);return ge!==Y?ge-Y:(J.player.surname_encoded||"").localeCompare(oe.player.surname_encoded||"")})}function P(){return[...g||[]].sort((J,oe)=>{const ge=Kt.indexOf(J.job),Y=Kt.indexOf(oe.job);return ge!==Y?ge-Y:(J.surname_encoded||"").localeCompare(oe.surname_encoded||"")})}function X(){return re().filter(J=>{const oe=J.player,ge=j==="Tous"||oe.job===j,Y=!W||`${oe.firstname} ${oe.surname_encoded}`.toLowerCase().includes(W);return ge&&Y})}function U(){return P().filter(J=>{const oe=j==="Tous"||J.job===j,ge=!W||`${J.firstname} ${J.surname_encoded}`.toLowerCase().includes(W);return oe&&ge})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
  ...
  </div>`;function N(){const J=document.getElementById("col-filters");J&&(B==="player"?(J.innerHTML=`
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Vt=1e3;function Zo(e,t,i,s){var A,r,p;const{state:l,toast:d,closeModal:a,navigate:c,refreshProfile:g}=i,n=e.formation,o={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function m(){const I=Pt[n]||{},L=hi[n]||[],F=290,H=360,C=20;function B(W){const de=I[W];return de?{x:de.x*F,y:de.y*H}:null}let j=`<svg width="${F}" height="${H}" viewBox="0 0 ${F} ${H}" xmlns="http://www.w3.org/2000/svg">`;for(const[W,de]of L){const re=B(W),P=B(de);!re||!P||(j+=`<line x1="${re.x}" y1="${re.y}" x2="${P.x}" y2="${P.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const W of Object.keys(I)){const de=B(W);if(!de)continue;const re=W.replace(/\d+/,""),P=o[re]||"#555";j+=`<circle cx="${de.x}" cy="${de.y}" r="${C}" fill="${P}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,j+=`<text x="${de.x}" y="${de.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${re}</text>`}return j+="</svg>",j}const b=t.filter(I=>I.formation===n);b.length;const u=!e.is_for_sale;s(`Formation ${n}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(A=document.getElementById("direct-sell-form-btn"))==null||A.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${n} pour ${Vt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const I=b.find(F=>!F.is_for_sale)||b[0];if(!I){d("Aucune carte à vendre","error");return}await k.from("market_listings").delete().eq("card_id",I.id),await k.from("transfer_history").delete().eq("card_id",I.id);const{error:L}=await k.from("cards").delete().eq("id",I.id);if(L){d(L.message,"error");return}await k.from("users").update({credits:(l.profile.credits||0)+Vt}).eq("id",l.profile.id),await g(),d(`+${Vt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),c("collection")}),(r=document.getElementById("market-sell-form-btn"))==null||r.addEventListener("click",async()=>{const I=parseInt(document.getElementById("sell-price-form").value);if(!I||I<1){d("Prix invalide","error");return}await k.from("cards").update({is_for_sale:!0,sale_price:I}).eq("id",e.id),await k.from("market_listings").insert({seller_id:l.profile.id,card_id:e.id,price:I}),d("Carte mise en vente sur le marché !","success"),a(),c("collection")}),(p=document.getElementById("cancel-sell-form-btn"))==null||p.addEventListener("click",async()=>{await k.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await k.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),d("Annonce retirée","success"),a(),c("collection")})}async function Fi(e,t,i,s){var _e,Ae,Ie,xe,J,oe,ge;const{state:l,toast:d,openModal:a,closeModal:c,navigate:g,refreshProfile:n}=s,o=e.player,m=t.filter(Y=>Y.player.id===o.id),b=m.length,u=e.evolution_bonus||0,A=Math.max((Number(o.note_g)||0)+(o.job==="GK"||o.job2==="GK"?u:0),(Number(o.note_d)||0)+(o.job==="DEF"||o.job2==="DEF"?u:0),(Number(o.note_m)||0)+(o.job==="MIL"||o.job2==="MIL"?u:0),(Number(o.note_a)||0)+(o.job==="ATT"||o.job2==="ATT"?u:0)),r=o.rarity||"normal",{data:p}=await k.from("sell_price_configs").select("*").eq("rarity",r).lte("note_min",A).gte("note_max",A).order("note_min",{ascending:!1}).limit(1);((_e=p==null?void 0:p[0])==null?void 0:_e.price)??Yo[r];const I=o.rarity!=="legende",L=ao(o),F=((o.rarity==="pepite"||o.rarity==="papyte")&&e.current_note!=null?e.current_note:zt(o,o.job))+u,H=o.rarity==="pepite"||o.rarity==="papyte",C=o.job2?(H?_i(e,ro(o.job2)):zt(o,o.job2))+(zt(o,o.job2)>0?u:0):null,B=qt[o.job]||"#1A6B3C",j=o.job2?qt[o.job2]:null,W=no[o.rarity]||"#ccc",de=Ei[o.country_code]||o.country_code||"",re=e.evolution_bonus||0,X=F+re,U=C||0,N=U>0?U+re:0,ne=m.map(Y=>Y.id);let pe={};if(ne.length){const{data:Y}=await k.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",ne).order("transferred_at",{ascending:!0});(Y||[]).forEach(Z=>{pe[Z.card_id]||(pe[Z.card_id]=[]),pe[Z.card_id].push(Z)})}const ye=Y=>{const Z=Y.transferred_at?new Date(Y.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",K=Y.source==="booster"?"Booster":Y.price?Y.price.toLocaleString("fr"):"—";return`
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;background:#f0fdf4;border-radius:10px;padding:10px 14px">
          <div>
            ...
            <div style="font-size:11px;color:#555;margin-top:2px">
              ${(e.evolution_bonus||0)>0?`+${e.evolution_bonus} appliqué${e.evolution_bonus>1?"s":""} · Note : ${X}${o.job2&&U>0?` / ${N}`:""}`:`Note actuelle : ${X}${o.job2&&U>0?` / ${N}`:""}`}
            </div>
          </div>
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(xe=document.getElementById("close-detail"))==null||xe.addEventListener("click",c);let we=new Set;const ve=()=>{const Y=we.size,Z=document.getElementById("sell-action-panel");if(!Z)return;Z.style.display=Y>0?"block":"none",document.getElementById("sell-selected-count").textContent=Y;const K=document.getElementById("evolve-btn");K&&(K.textContent=`⬆️ Évoluer ${Y>1?"(+"+Y+")":""}`)};document.querySelectorAll(".expl-check").forEach(Y=>{Y.addEventListener("change",()=>{const Z=Y.dataset.id;Y.checked?we.add(Z):we.delete(Z);const K=Y.closest(".exemplaire-row");K&&(K.style.borderColor=Y.checked?"#1A6B3C":"#eee"),ve()})}),document.querySelectorAll(".exemplaire-row").forEach(Y=>{Y.addEventListener("click",Z=>{if(Z.target.closest("button")||Z.target.tagName==="INPUT")return;const K=Y.querySelector(".expl-check");K&&!K.disabled&&(K.checked=!K.checked,K.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(Y=>{Y.addEventListener("click",Z=>{Z.stopPropagation();const K=document.querySelector(`.expl-hist[data-hist="${Y.dataset.idx}"]`);K&&(K.style.display=K.style.display==="none"?"flex":"none")})}),(J=document.getElementById("evolve-btn"))==null||J.addEventListener("click",async()=>{if(b<=1)return;const Y=[...we];if(!Y.length)return;if(we.has(e.id)){const be=document.createElement("div");be.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",be.innerHTML=`
        ...
        <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.3)">
          ...
          <div style="background:#f0fdf4;border-radius:10px;padding:12px;margin-bottom:16px;font-size:13px;color:#333">
            ...
            📈 Note : <strong>${F+(e.evolution_bonus||0)}</strong> → <strong style="color:#1A6B3C">${F+(e.evolution_bonus||0)+Z.length*K}</strong>
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${u}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${l?`<img src="${l}" style="width:${u-4}px;height:${u-4}px;object-fit:contain">`:(r=e==null?void 0:e.clubs)!=null&&r.encoded_name?`<span style="font-size:${Math.max(4,u-8)}px;font-weight:700;color:#333">${(e.clubs.encoded_name||"").slice(0,3).toUpperCase()}</span>`:"<span></span>"}
    </div>
  ...
  <div style="height:100%;overflow-y:auto;padding-bottom:80px;background:var(--page-bg)">
    ...
    <div style="padding:8px 16px;background:#fff;border-bottom:1px solid var(--gray-200);display:flex;align-items:center;gap:10px">
      ...
      ${t.stadiumCards.length>0?`
        <select id="stadium-select" style="padding:6px;border-radius:6px;border:1.5px solid #E87722;font-size:12px;max-width:180px;background:#fff">
          ...
          ${t.stadiumCards.map(o=>{var u;const m=t.stadDefMap[o.stadium_id],b=m?m.name+((u=m.club)!=null&&u.encoded_name?` (${m.club.encoded_name})`:m.country_code?` (${m.country_code})`:""):o.id.slice(0,8);return`<option value="${o.id}" ${t.stadiumCardId===o.id?"selected":""}>${b}</option>`}).join("")}
  ...
  </div>`,en(e,t,l,i),document.getElementById("builder-back").addEventListener("click",()=>s("decks")),document.getElementById("formation-select").addEventListener("change",o=>{t.formation=o.target.value;const m=Gi(t.formation),b={};m.forEach(u=>{t.slots[u]&&(b[u]=t.slots[u])}),t.slots=b,wt(e,t,i)}),(g=document.getElementById("stadium-select"))==null||g.addEventListener("change",o=>{t.stadiumCardId=o.target.value||null,wt(e,t,i)}),document.getElementById("save-deck").addEventListener("click",()=>nn(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(o=>{o.addEventListener("click",()=>{t.subs=t.subs.filter(m=>m!==o.dataset.removeSub),wt(e,t,i)})}),(n=document.getElementById("add-sub-btn"))==null||n.addEventListener("click",()=>{on(t,e,i)})}function en(e,t,i,s){var F,H;const l=e.querySelector("#deck-field");if(!l)return;const d=(F=t.stadiumCards)==null?void 0:F.find(C=>C.id===t.stadiumCardId),a=d&&((H=t.stadDefMap)==null?void 0:H[d.stadium_id])||null,c=Pt[t.formation]||{},g=Yi(t.formation)||[],n={};for(const C of i){const B=t.slots[C],j=B?t.playerCards.find(W=>W.id===B):null;j!=null&&j.player?n[C]={...j.player,_evolution_bonus:j.evolution_bonus||0}:n[C]=null}const o=300,m=300,b=48,u=64,A=13,r=16,p=38;function I(C){const B=c[C];return B?{x:B.x*o,y:B.y*m}:null}let L="";for(const[C,B]of g){const j=I(C),W=I(B);if(!j||!W)continue;const de=n[C]?{...n[C],club_id:n[C].club_id,country_code:n[C].country_code,rarity:n[C].rarity}:null,re=n[B]?{...n[B],club_id:n[B].club_id,country_code:n[B].country_code,rarity:n[B].rarity}:null,P=at(de,re);P==="#ff3333"||P==="#cc2222"?L+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${W.x.toFixed(1)}" y2="${W.y.toFixed(1)}" stroke="${P}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(L+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${W.x.toFixed(1)}" y2="${W.y.toFixed(1)}" stroke="${P}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,L+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${W.x.toFixed(1)}" y2="${W.y.toFixed(1)}" stroke="${P}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const C of i){const B=I(C);if(!B)continue;const j=n[C],W=C.replace(/\d+/,""),de=vt[W]||"#555",re=(B.x-b/2).toFixed(1),P=(B.y-u/2).toFixed(1),X={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[j==null?void 0:j.rarity]||"#aaa";if(j){const U=ii(j),N=lo(j),ne=so(j.country_code),pe=j._evolution_bonus||0,ye=(Number(W==="GK"?j.note_g:W==="DEF"?j.note_d:W==="MIL"?j.note_m:j.note_a)||0)+(W===j.job||W===j.job2?pe:0),Se=a&&(a.club_id&&String(j.club_id)===String(a.club_id)||a.country_code&&j.country_code===a.country_code),we=Se?ye+10:ye,ve=u-A-r;L+=`<defs><clipPath id="dcp-${C}"><rect x="${re}" y="${(B.y-u/2+A).toFixed(1)}" width="${b}" height="${ve}"/></clipPath></defs>`,L+=`<rect x="${re}" y="${P}" width="${b}" height="${u}" rx="5" fill="${de}"/>`,U&&(L+=`<image href="${U}" x="${re}" y="${(B.y-u/2+A).toFixed(1)}" width="${b}" height="${ve}" clip-path="url(#dcp-${C})" preserveAspectRatio="xMidYMin slice"/>`),L+=`<rect x="${re}" y="${P}" width="${b}" height="${A}" fill="rgba(255,255,255,0.93)"/>`,L+=`<text x="${B.x.toFixed(1)}" y="${(B.y-u/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(j.surname_encoded||"").slice(0,9)}</text>`;const _e=(B.y+u/2-r).toFixed(1);L+=`<rect x="${re}" y="${_e}" width="${b}" height="${r}" fill="rgba(255,255,255,0.93)"/>`,ne&&(L+=`<image href="${ne}" x="${(B.x-21).toFixed(1)}" y="${(B.y+u/2-r+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),L+=`<text x="${B.x.toFixed(1)}" y="${(B.y+u/2-r/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${we}</text>`,N&&(L+=`<image href="${N}" x="${(B.x+b/2-14).toFixed(1)}" y="${(B.y+u/2-r+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),L+=`<rect x="${re}" y="${P}" width="${b}" height="${u}" rx="5" fill="none" stroke="${Se?"#E87722":X}" stroke-width="${Se?"2.5":"2"}"/>`,Se&&(L+=`<rect x="${(B.x+b/2-13).toFixed(1)}" y="${P}" width="13" height="9" rx="3" fill="#E87722"/>`,L+=`<text x="${(B.x+b/2-6.5).toFixed(1)}" y="${(B.y-u/2+4.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="900" fill="#fff" font-family="Arial">+10</text>`)}else L+=`<rect x="${re}" y="${P}" width="${b}" height="${u}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,L+=`<text x="${B.x.toFixed(1)}" y="${B.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,L+=`<text x="${B.x.toFixed(1)}" y="${(B.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${W}</text>`;L+=`<rect x="${re}" y="${P}" width="${b}" height="${u}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${C}" style="cursor:pointer"/>`}l.innerHTML=`<svg viewBox="${-p} ${-p} ${o+p*2} ${m+p*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${L}</svg>`,l.querySelectorAll(".deck-slot-hit").forEach(C=>{C.addEventListener("click",()=>tn(C.dataset.pos,t,e,s))})}function tn(e,t,i,s){var b,u,A;const{openModal:l,closeModal:d}=s,a=e.replace(/\d+/,""),c=t.slots[e],g=c?t.playerCards.find(r=>r.id===c):null;(b=g==null?void 0:g.player)==null||b.id;const n=new Set;Object.entries(t.slots).forEach(([r,p])=>{var L;if(r===e||!p)return;const I=t.playerCards.find(F=>F.id===p);(L=I==null?void 0:I.player)!=null&&L.id&&n.add(I.player.id)}),t.subs.forEach(r=>{var I;const p=t.playerCards.find(L=>L.id===r);(I=p==null?void 0:p.player)!=null&&I.id&&n.add(p.player.id)});const o=new Set,m=t.playerCards.filter(r=>{const p=r.player;return!(p.job===a||p.job2===a)||n.has(p.id)||o.has(p.id)?!1:(o.add(p.id),!0)});m.sort((r,p)=>{const I=r.evolution_bonus||0,L=p.evolution_bonus||0,F=(a==="GK"?r.player.note_g:a==="DEF"?r.player.note_d:a==="MIL"?r.player.note_m:r.player.note_a)+(a===r.player.job||a===r.player.job2?I:0);return(a==="GK"?p.player.note_g:a==="DEF"?p.player.note_d:a==="MIL"?p.player.note_m:p.player.note_a)+(a===p.player.job||a===p.player.job2?L:0)-F}),l(`Choisir ${a} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${m.length>0?m.map(r=>{var C,B;const p=r.player,I=r.evolution_bonus||0,L=(a==="GK"?p.note_g:a==="DEF"?p.note_d:a==="MIL"?p.note_m:p.note_a)+(a===p.job||a===p.job2?I:0),F=ii(p),H={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[p.rarity];return`<div class="player-option" data-card-id="${r.id}"
          ...
          <div style="width:44px;height:44px;border-radius:8px;overflow:hidden;flex-shrink:0;background:#dde;border:2px solid ${vt[a]}">
            ${F?`<img src="${F}" style="width:100%;height:100%;object-fit:cover">`:`<div style="width:100%;height:100%;background:${vt[a]};display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:700">${a}</div>`}
          </div>
          <!-- Infos -->
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;align-items:center;gap:6px;margin-top:3px">
              ...
              ${(C=p.clubs)!=null&&C.logo_url?`<img src="${p.clubs.logo_url}" style="width:18px;height:18px;object-fit:contain">`:`<span style="font-size:10px;color:var(--gray-600)">${((B=p.clubs)==null?void 0:B.encoded_name)||"—"}</span>`}
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(u=document.getElementById("close-selector"))==null||u.addEventListener("click",d),(A=document.getElementById("remove-player"))==null||A.addEventListener("click",()=>{delete t.slots[e],d(),wt(i,t,s)}),document.querySelectorAll(".player-option").forEach(r=>{r.addEventListener("click",()=>{t.slots[e]=r.dataset.cardId,d(),wt(i,t,s)})})}function on(e,t,i){var g;const{openModal:s,closeModal:l}=i,d=new Set;Object.keys(e.slots).forEach(n=>{var b;const o=e.slots[n];if(!o)return;const m=e.playerCards.find(u=>u.id===o);(b=m==null?void 0:m.player)!=null&&b.id&&d.add(m.player.id)}),e.subs.forEach(n=>{var m;const o=e.playerCards.find(b=>b.id===n);(m=o==null?void 0:o.player)!=null&&m.id&&d.add(o.player.id)});const a=new Set,c=e.playerCards.filter(n=>{var o,m,b;return d.has((o=n.player)==null?void 0:o.id)||a.has((m=n.player)==null?void 0:m.id)?!1:(a.add((b=n.player)==null?void 0:b.id),!0)});s("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${c.length>0?c.map(n=>{var u;const o=n.player,m=ii(o),b=(o.job==="GK"?o.note_g:o.job==="DEF"?o.note_d:o.job==="MIL"?o.note_m:o.note_a)+(n.evolution_bonus||0);return`<div class="player-option" data-card-id="${n.id}"
          style="display:flex;align-items:center;gap:10px;padding:8px;border:1.5px solid var(--gray-200);border-radius:10px;cursor:pointer">
  ...
  </div>`}function uo(e){var l;const t={};(e.rates||[]).forEach(d=>{t[d.card_type]=(t[d.card_type]||0)+Number(d.percentage||0)});const i=((l=Object.entries(t).sort((d,a)=>a[1]-d[1])[0])==null?void 0:l[0])||"player",s=e.image_url||"booster-players.png";return{id:e.id,img:"/icons/"+s,name:e.name,sub:`${e.card_count} carte(s)`,cost:e.price_type==="credits"&&e.price_credits||0,costLabel:e.price_type==="credits"?`${(e.price_credits||0).toLocaleString("fr")} crédits`:e.price_type==="pub"?"1 pub":"Gratuit",cardCount:e.card_count||5,type:i,isPub:e.price_type==="pub",rates:e.rates||[],allow_duplicates:e.allow_duplicates!==!1,_raw:e}}async function pn(e,{state:t,navigate:i,toast:s}){var m,b;const l=((m=t.profile)==null?void 0:m.credits)||0;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⏳ Chargement...</div>';let d=[];try{d=(await co()).map(uo)}catch(u){console.warn("Erreur chargement boosters DB, fallback hardcodé",u)}d.length||(d=an.map(u=>({...u,rates:[],isPub:u.id==="players_pub"})));const a=await k.from("cards").select("player_id, card_type, formation, stadium_id, gc_type").eq("owner_id",t.profile.id).then(u=>u.data||[]),c=new Set(a.filter(u=>u.card_type==="stadium").map(u=>u.stadium_id)),g=new Set(a.filter(u=>u.card_type==="formation").map(u=>u.formation)),n=new Set(a.filter(u=>u.card_type==="game_changer").map(u=>u.gc_type)),o={};for(const u of d){if(u.allow_duplicates!==!1||!((b=u.rates)!=null&&b.length))continue;const A=[...new Set((u.rates||[]).map(p=>p.card_type))];let r=!1;for(const p of A)if(p==="stadium"){const{data:I}=await k.from("stadium_definitions").select("id");if((I||[]).some(L=>!c.has(L.id))){r=!0;break}}else if(p==="game_changer"){const{data:I}=await k.from("gc_definitions").select("name").eq("is_active",!0);if((I||[]).some(L=>!n.has(L.name))){r=!0;break}}else if(p==="formation"){const{FORMATION_LINKS:I}=await ti(async()=>{const{FORMATION_LINKS:L}=await import("./formation-links-Dnttc-Cp.js").then(F=>F.m);return{FORMATION_LINKS:L}},__vite__mapDeps([0,1]));if(Object.keys(I).some(L=>!g.has(L))){r=!0;break}}else{r=!0;break}r||(o[u.id]=!0)}e.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📦 Boosters</h2>
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${d.map(u=>{const A=u.cost===0||l>=u.cost,r=o[u.id]===!0;return`<div class="booster-card ${!A||r?"disabled":""}" data-booster="${u.id}" style="position:relative">
  ...
  </div>`,e.querySelectorAll(".booster-card:not(.disabled)").forEach(u=>{u.addEventListener("click",async()=>{const A=d.find(r=>r.id===u.dataset.booster);if(A){u.style.opacity="0.5",u.style.pointerEvents="none";try{await un(A,{state:t,toast:s,navigate:i,container:e})}catch(r){s(r.message,"error"),u.style.opacity="",u.style.pointerEvents=""}}})}),e.querySelectorAll(".booster-info-btn").forEach(u=>{u.addEventListener("click",A=>{A.stopPropagation();const r=d.find(p=>p.id===u.dataset.boosterId);xn(r)})})}async function un(e,{state:t,toast:i,navigate:s,container:l}){var m,b;if(e.cost>0&&t.profile.credits<e.cost){i("Crédits insuffisants","error");return}e.isPub&&await bn();const{data:d}=await k.from("cards").select("card_type, player_id, formation").eq("owner_id",t.profile.id),a=new Set((d||[]).filter(u=>u.card_type==="player").map(u=>u.player_id)),c=new Set((d||[]).filter(u=>u.card_type==="formation").map(u=>u.formation));let g=[],n=null;try{if((m=e.rates)!=null&&m.length)g=await gi(t.profile,e);else{const u=e.type||"player";u==="player"?g=await fo(t.profile,e.cardCount,e.cost):u==="game_changer"?g=await go(t.profile,e.cardCount,e.cost):u==="formation"?g=await mo(t.profile,e.cost):g=await gi(t.profile,e)}}catch(u){n=u.message||String(u),console.error("[Booster] Erreur:",u)}if(!(g!=null&&g.length)){const u=document.createElement("div");u.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",u.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
      <div style="font-size:12px;color:rgba(255,255,255,0.6);max-width:320px;word-break:break-all;background:rgba(255,255,255,0.05);padding:10px;border-radius:8px;margin-top:6px">
        ${n||"Vérifie la console (F12) pour plus de détails"}
      </div>
      <button style="margin-top:10px;padding:12px 28px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer" id="anim-close-err">Fermer</button>`,document.body.appendChild(u),(b=u.querySelector("#anim-close-err"))==null||b.addEventListener("click",()=>u.remove());return}g.forEach(u=>{u.card_type==="player"&&u.player?u.isDuplicate=a.has(u.player.id):u.card_type==="formation"&&(u.isDuplicate=c.has(u.formation))});const{data:o}=await k.from("users").select("*").eq("id",t.profile.id).single();o&&(t.profile=o),xo(g,e,s)}function fn(){const e=Math.random()*100;return e<.5?"legende":e<2?"special":e<10?"normal_high":"normal_low"}function mt(e){return Math.max(Number(e.note_g)||0,Number(e.note_d)||0,Number(e.note_m)||0,Number(e.note_a)||0)}function gn(e,t){let i;switch(t){case"legende":i=e.filter(s=>s.rarity==="legende"),i.length||(i=e.filter(s=>s.rarity==="pepite"||s.rarity==="papyte")),i.length||(i=e.filter(s=>mt(s)>=6));break;case"special":i=e.filter(s=>s.rarity==="pepite"||s.rarity==="papyte"),i.length||(i=e.filter(s=>mt(s)>=6));break;case"normal_high":i=e.filter(s=>s.rarity==="normal"&&mt(s)>=6),i.length||(i=e.filter(s=>mt(s)>=6));break;default:i=e.filter(s=>s.rarity==="normal"&&mt(s)>=1&&mt(s)<=5),i.length||(i=e.filter(s=>s.rarity==="normal"));break}return i.length||(i=e),i[Math.floor(Math.random()*i.length)]}async function gi(e,t){if(t.cost>0){const{error:b}=await k.from("users").update({credits:e.credits-t.cost}).eq("id",e.id);if(b)throw b}const i=t.allow_duplicates!==!1;let s=[];const{data:l,error:d}=await k.from("cards").select("player_id, card_type, formation, stadium_id, gc_type").eq("owner_id",e.id);if(d){const{data:b}=await k.from("cards").select("player_id, card_type, formation, gc_type").eq("owner_id",e.id);s=b||[]}else s=l||[];const a=new Set(s.filter(b=>b.card_type==="player").map(b=>b.player_id)),c=new Set(s.filter(b=>b.card_type==="formation").map(b=>b.formation)),g=new Set(s.filter(b=>b.card_type==="game_changer").map(b=>b.gc_type)),n=new Set(s.filter(b=>b.card_type==="stadium").map(b=>b.stadium_id).filter(Boolean)),o=new Set,m=[];for(let b=0;b<(t.cardCount||5);b++){const u=rn(t.rates);if(u){if(u.card_type==="player"){const A=j=>({légende:"legende",pépite:"pepite",pépites:"pepite"})[j]||j,r=u.rarity?A(u.rarity):null;let p=k.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);r&&(p=p.eq("rarity",r));const{data:I}=await p;let L=I||[];if((u.note_min||u.note_max)&&(L=L.filter(j=>{const W=Math.max(Number(j.note_g)||0,Number(j.note_d)||0,Number(j.note_m)||0,Number(j.note_a)||0);return(!u.note_min||W>=u.note_min)&&(!u.note_max||W<=u.note_max)})),L.length||(u.note_min||u.note_max?(L=I||[],console.warn("[Booster] Aucun joueur avec note",u.note_min,"-",u.note_max,"— fallback rareté uniquement")):L=I||[]),!L.length)continue;let F=L.filter(j=>!o.has(j.id));if(i)F.length||(F=L);else if(F=F.filter(j=>!a.has(j.id)),!F.length)continue;const H=F[Math.floor(Math.random()*F.length)];o.add(H.id);const C=a.has(H.id),{data:B}=await k.from("cards").insert({owner_id:e.id,player_id:H.id,card_type:"player"}).select().single();B&&(m.push({...B,player:H,isDuplicate:C}),k.rpc("record_transfer",{p_card_id:B.id,p_player_id:H.id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{}))}else if(u.card_type==="game_changer"){const{data:A}=await k.from("gc_definitions").select("id,name").eq("is_active",!0).eq("gc_type","game_changer"),r=A!=null&&A.length?A:[{name:"Ressusciter"},{name:"Double attaque"},{name:"Bouclier"},{name:"Vol de note"},{name:"Gel"}],p=i?r:r.filter(H=>!g.has(H.name));if(!i&&!p.length)continue;const L=p[Math.floor(Math.random()*p.length)].name,{data:F}=await k.from("cards").insert({owner_id:e.id,card_type:"game_changer",gc_type:L}).select().single();F&&m.push(F)}else if(u.card_type==="formation"){const A=po(),r=i?A:A.filter(F=>!c.has(F));if(!i&&!r.length)continue;const p=r[Math.floor(Math.random()*r.length)],I=c.has(p),{data:L}=await k.from("cards").insert({owner_id:e.id,card_type:"formation",formation:p}).select();L!=null&&L[0]&&m.push({...L[0],isDuplicate:I})}else if(u.card_type==="stadium"){const{data:A,error:r}=await k.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)");if(r){console.error("[Booster] stadium_definitions:",r.message);continue}if(!(A!=null&&A.length)){console.warn("[Booster] Aucun stade en DB");continue}const p=i?A:A.filter(H=>!n.has(H.id));if(!i&&!p.length)continue;const I=p[Math.floor(Math.random()*p.length)],{data:L,error:F}=await k.from("cards").insert({owner_id:e.id,card_type:"stadium",stadium_id:I.id}).select("id,card_type,stadium_id").single();if(F){console.error("[Booster] insert stadium card:",F.message);continue}L&&m.push({...L,rarity:"normal",_stadiumDef:I})}}}return m}async function fo(e,t,i){if(i>0){const{error:n}=await k.from("users").update({credits:e.credits-i}).eq("id",e.id);if(n)throw n}const{data:s}=await k.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(s!=null&&s.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const l=s.filter(n=>n.job==="GK"),d=s.filter(n=>n.job!=="GK"),a=!e.first_booster_opened&&l.length>0,c=[];for(let n=0;n<t;n++){const o=n===0&&a?l:n===0?d:s,m=fn(),b=gn(o,m);b&&c.push(b)}a&&await k.from("users").update({first_booster_opened:!0}).eq("id",e.id);const{data:g}=await k.from("cards").insert(c.map(n=>({owner_id:e.id,player_id:n.id,card_type:"player"}))).select();return(g||[]).forEach((n,o)=>{k.rpc("record_transfer",{p_card_id:n.id,p_player_id:c[o].id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{})}),c.map((n,o)=>({...g[o],player:n}))}async function go(e,t,i){const{error:s}=await k.from("users").update({credits:e.credits-i}).eq("id",e.id);if(s)throw s;const{data:l}=await k.from("gc_definitions").select("id,name,gc_type,color,effect,image_url").eq("is_active",!0),d=l!=null&&l.length?l:Object.keys(fi).map(o=>({name:o,gc_type:"game_changer"})),a=Array.from({length:t},()=>{const o=d[Math.floor(Math.random()*d.length)];return{owner_id:e.id,card_type:"game_changer",gc_type:o.name,gc_definition_id:o.id||null}}),{data:c,error:g}=await k.from("cards").insert(a).select();return g&&console.error("[Booster GC] Erreur insert:",g.message,g),(c||[]).map(o=>{const m=l==null?void 0:l.find(b=>b.name===o.gc_type||b.id===o.gc_definition_id);return{...o,_gcDef:m||null}})}async function mo(e,t){const{error:i}=await k.from("users").update({credits:e.credits-t}).eq("id",e.id);if(i)throw i;const{data:s}=await k.from("cards").select("formation").eq("owner_id",e.id).eq("card_type","formation"),l=new Set((s||[]).map(o=>o.formation)),d=po(),a=d[Math.floor(Math.random()*d.length)],c=l.has(a),{data:g,error:n}=await k.from("cards").insert({owner_id:e.id,card_type:"formation",formation:a}).select();return n&&console.error("[Booster Formation] Erreur insert:",n.message,n),(g||[]).map(o=>({...o,isDuplicate:c}))}function xo(e,t,i,s=null){var X,U;if(!e||e.length===0){const N=document.createElement("div");N.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",N.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
  ...
  `,document.body.appendChild(l);let d=!1;const a=document.getElementById("pack-cut-zone"),c=document.getElementById("pack-blade");let g=!1;const n=N=>N.touches&&N.touches[0]?N.touches[0].clientX:N.clientX;function o(N){d||(g=!0,c.style.opacity="1",m(N))}function m(N){if(!g||d)return;const ne=a.getBoundingClientRect(),pe=n(N)-ne.left,ye=Math.max(0,Math.min(1,pe/ne.width));c.style.width=ye*ne.width+"px",ye>=.82&&u()}function b(){d||(g=!1,c.style.transition="width .2s ease, opacity .2s ease",c.style.width="0",c.style.opacity="0",setTimeout(()=>{d||(c.style.transition="")},220))}function u(){var ne;if(d)return;d=!0,g=!1,c.style.width="100%",c.style.opacity="1",(ne=document.getElementById("cut-flash"))==null||ne.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const N=document.getElementById("cut-hint");N&&(N.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{c.style.opacity="0",document.getElementById("pack-phase").style.display="none",I(0)},620)}a.addEventListener("pointerdown",o),window.addEventListener("pointermove",m),window.addEventListener("pointerup",b),a.addEventListener("touchstart",o,{passive:!0}),window.addEventListener("touchmove",m,{passive:!0}),window.addEventListener("touchend",b);let A=0,r=!1;const p=new Set;function I(N){A=N,document.getElementById("reveal-phase").style.display="flex",L(),F(N,0),W()}function L(){const N=document.getElementById("card-dots");N&&(N.innerHTML=e.map((ne,pe)=>`<div class="card-dot" data-i="${pe}" style="width:8px;height:8px;border-radius:50%;background:${pe===A?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),N.querySelectorAll(".card-dot").forEach(ne=>ne.addEventListener("click",()=>C(parseInt(ne.dataset.i)))))}function F(N,ne){var xe;const pe=e[N],ye=document.getElementById("card-counter"),Se=document.getElementById("card-track");ye&&(ye.textContent=`Carte ${N+1} / ${e.length}`);const we=document.getElementById("reveal-btns");we&&(we.style.display=N===e.length-1?"flex":"none");const ve=pe.card_type==="player"&&((xe=pe.player)==null?void 0:xe.rarity)==="legende",_e=!p.has(N);p.add(N);let Ae=0;if(pe.card_type==="player"&&pe.player){const J=pe.player,oe=J.job||"ATT";Ae=Number(oe==="GK"?J.note_g:oe==="DEF"?J.note_d:oe==="MIL"?J.note_m:J.note_a)||0}const Ie=J=>{Se.innerHTML=`
      ...
      <div style="flex:1;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06);margin:0">
        ${u?`<img src="${u}" style="max-width:120px;max-height:120px;width:auto;height:auto;object-fit:contain;border-radius:6px">`:`<span style="font-size:56px">${A}</span>`}
      </div>
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`}const g=o=>{e.style.overflow="",e.style.height="",e.style.display="",e.style.flexDirection="",i(o)};function n(){var m,b,u;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const o=a.length>0;e.innerHTML=`
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(A=>{A.addEventListener("click",()=>{const r=A.dataset.id,p=d.find(L=>L.id===r);if(!p)return;const I=a.findIndex(L=>L.gc_type===p.gc_type);I>-1?a.splice(I,1):a.length<3&&a.push(p),n()})}),(m=e.querySelector("#gc-launch"))==null||m.addEventListener("click",()=>{o&&g(a)}),(b=e.querySelector("#gc-no-gc"))==null||b.addEventListener("click",()=>g([])),(u=e.querySelector("#gc-reset"))==null||u.addEventListener("click",()=>{a.length&&(a=[],n())})}n()}function wn(e){var s;const t=((s=e==null?void 0:e.state)==null?void 0:s.params)||{},i=t.matchMode||"vs_ai_easy";return i==="friend"?`Match vs ${t.friendName||"Ami"}`:i==="random"?"Match vs Random":`Match vs IA — ${i.replace("vs_ai_","").toUpperCase()}`}async function _n(e,t,i){const{state:s,navigate:l}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await k.from("decks").select(`id,name,is_active,formation,stadium_card_id,
      stadium_card:cards!stadium_card_id(id,stadium_id,
        stadium_def:stadium_definitions(id,name,club_id,country_code,image_url,
          club:clubs(encoded_name,logo_url)))`).eq("owner_id",s.profile.id).order("created_at",{ascending:!1});if(!d||d.length===0){_t(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>l("decks"));return}const a=d.map(b=>b.id),{data:c}=await k.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),g=[...new Set((c||[]).filter(b=>{var u,A;return((u=b.card)==null?void 0:u.card_type)==="stadium"&&((A=b.card)==null?void 0:A.stadium_id)}).map(b=>b.card.stadium_id))],n={};if(g.length){const{data:b}=await k.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",g);(b||[]).forEach(u=>{n[u.id]=u}),(c||[]).forEach(u=>{var A,r;((A=u.card)==null?void 0:A.card_type)==="stadium"&&((r=u.card)!=null&&r.stadium_id)&&(u.card._stadiumDef=n[u.card.stadium_id]||null)})}let o=0;function m(){var C,B,j,W,de,re,P;const b=d[o],u=(c||[]).filter(X=>X.deck_id===b.id),A=u.filter(X=>{var U;return X.is_starter&&((U=X.card)==null?void 0:U.player)}).map(X=>mi(X.card,X.position)),r=u.find(X=>{var U;return((U=X.card)==null?void 0:U.card_type)==="formation"}),p=b.formation||((C=r==null?void 0:r.card)==null?void 0:C.formation)||"4-4-2";let I=A.length>=11?yt(A,p):null,L=((B=b.stadium_card)==null?void 0:B.stadium_def)||null;L&&I&&(I=Ht(I,L));const F=A.length>=11;et(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
  ...
  </div>`}function ot(e,t,i){if(!(e!=null&&e.length))return"";const s=e.slice(0,5);let l='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return s.forEach((d,a)=>{if(l+=Ye(d,40,52),a<s.length-1){const c=at(d,s[a+1]);l+=`<div style="width:7px;height:3px;background:${c==="#ff3333"||c==="#cc2222"?"rgba(255,255,255,0.12)":c};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(l+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),l+="</div>",l}function Zt(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function bo(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function ut(e,t,i,s,l=310,d=310,a=[]){var F;const c=Pt[t]||{},g=Yi(t)||hi[t]||[],n={},o=["ATT","MIL","DEF","GK"];for(const H of o)(e[H]||[]).forEach((B,j)=>{n[`${H}${j+1}`]=B});function m(H){const C=c[H];return C?{x:C.x*l,y:C.y*d}:null}let b="";for(const[H,C]of g){const B=m(H),j=m(C);if(!B||!j)continue;const W=n[H],de=n[C],re=at(W,de);re==="#00ff88"||re==="#FFD700"?(b+=`<line x1="${B.x.toFixed(1)}" y1="${B.y.toFixed(1)}" x2="${j.x.toFixed(1)}" y2="${j.y.toFixed(1)}"
        stroke="${re}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,b+=`<line x1="${B.x.toFixed(1)}" y1="${B.y.toFixed(1)}" x2="${j.x.toFixed(1)}" y2="${j.y.toFixed(1)}"
        stroke="${re}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):b+=`<line x1="${B.x.toFixed(1)}" y1="${B.y.toFixed(1)}" x2="${j.x.toFixed(1)}" y2="${j.y.toFixed(1)}"
        stroke="${re}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const u=48,A=64,r=13,p=16,I={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[H,C]of Object.entries(n)){const B=m(H);if(!B||!C)continue;const j=H.replace(/[0-9]/g,""),W=Mt[j]||"#555",de=a.includes(C.cardId),re=i==="attack"&&(["MIL","ATT"].includes(j)||de)&&!C.used||i==="defense"&&["GK","DEF","MIL"].includes(j)&&!C.used,P=s.includes(C.cardId);let X;i==="attack"?X=de?Math.max(1,Number(C.note_a)||0):j==="MIL"?Number(C.note_m)||0:Number(C.note_a)||0:i==="defense"?X=j==="GK"?Number(C.note_g)||0:j==="MIL"?Number(C.note_m)||0:Number(C.note_d)||0:X=Number(j==="GK"?C.note_g:j==="DEF"?C.note_d:j==="MIL"?C.note_m:C.note_a)||0,X=X+(C.boost||0),C.stadiumBonus&&(i==="attack"&&(j==="ATT"||j==="MIL")||i==="defense"&&(j==="GK"||j==="DEF"||j==="MIL")?X+=10:i||(X+=10));const U=(B.x-u/2).toFixed(1),N=(B.y-A/2).toFixed(1),ne=I[C==null?void 0:C.rarity]||I.normal;if(C.used){const Ae=typeof import.meta<"u"&&"/"||"/",xe=`${typeof window<"u"&&((F=window.location)==null?void 0:F.origin)||""}${Ae}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");b+=`<rect x="${U}" y="${N}" width="${u}" height="${A}" rx="5" fill="#161616"/>`,b+=`<image href="${xe}" xlink:href="${xe}" x="${U}" y="${N}" width="${u}" height="${A}" preserveAspectRatio="xMidYMid slice"/>`,b+=`<rect x="${U}" y="${N}" width="${u}" height="${A}" rx="5" fill="none" stroke="${ne}" stroke-width="2" opacity="0.7"/>`,b+=`<rect x="${U}" y="${N}" width="${u}" height="${A}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${C.cardId}" data-role="${j}" style="cursor:pointer"/>`;continue}const pe=P?.45:1,ye=P?"#FFD700":ne,Se=P?3:(C==null?void 0:C.rarity)==="legende"||(C==null?void 0:C.rarity)==="légende"||(C==null?void 0:C.rarity)==="pepite"||(C==null?void 0:C.rarity)==="pépite"?2.5:2,we=A-r-p;b+=`<defs><clipPath id="cp-${H}"><rect x="${U}" y="${(B.y-A/2+r).toFixed(1)}" width="${u}" height="${we}"/></clipPath></defs>`,b+=`<rect x="${U}" y="${N}" width="${u}" height="${A}" rx="5" fill="${W}" opacity="${pe}"/>`;const ve=Pe(C);ve&&(b+=`<image href="${ve}" xlink:href="${ve}" x="${U}" y="${(B.y-A/2+r).toFixed(1)}" width="${u}" height="${we}" clip-path="url(#cp-${H})" preserveAspectRatio="xMidYMin slice"/>`),b+=`<rect x="${U}" y="${N}" width="${u}" height="${r}" rx="4" fill="rgba(255,255,255,0.92)"/>`,b+=`<text x="${B.x.toFixed(1)}" y="${(B.y-A/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(C.name||"").slice(0,9)}</text>`;const _e=(B.y+A/2-p).toFixed(1);b+=`<rect x="${U}" y="${_e}" width="${u}" height="${p}" fill="rgba(255,255,255,0.92)"/>`;{const Ae=Zt(C.country_code);Ae?b+=`<image href="${Ae}" xlink:href="${Ae}" x="${(B.x-20).toFixed(1)}" y="${(B.y+A/2-p+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:b+=`<text x="${(B.x-13).toFixed(1)}" y="${(B.y+A/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${bo(C.country_code)}</text>`;const Ie=C.stadiumBonus?"#E87722":"#111";b+=`<text x="${B.x.toFixed(1)}" y="${(B.y+A/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="${Ie}" font-family="Arial Black">${X}</text>`;const xe=yo(C);xe?b+=`<image href="${xe}" xlink:href="${xe}" x="${(B.x+u/2-14).toFixed(1)}" y="${(B.y+A/2-p+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:C.clubName&&(b+=`<text x="${(B.x+14).toFixed(1)}" y="${(B.y+A/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(C.clubName||"").slice(0,3).toUpperCase()}</text>`),C.stadiumBonus&&(b+=`<rect x="${(B.x-u/2).toFixed(1)}" y="${(B.y-A/2).toFixed(1)}" width="${u}" height="${A}" rx="5" fill="none" stroke="#E87722" stroke-width="2" opacity="0.8"/>`)}b+=`<rect x="${U}" y="${N}" width="${u}" height="${A}" rx="5" fill="${P?"rgba(255,255,255,0.12)":"none"}" stroke="${ye}" stroke-width="${Se}" opacity="${pe}"/>`,re&&(b+=`<rect x="${U}" y="${N}" width="${u}" height="${A}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${P?"selected":""}" data-card-id="${C.cardId}" data-role="${j}" style="cursor:pointer"/>`)}const L=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-L} ${-L} ${l+L*2} ${d+L*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${b}
  ...
  </div>`}async function ai(e,t,i,s){var B;const{state:l,navigate:d,toast:a}=t;et(e);const c=l.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!c.deckId)return _n(e,t,i);const g=c.deckId;let n,o,m,b;try{const j=await Promise.all([k.from("decks").select("formation,name,stadium_card_id").eq("id",g).single(),k.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",g).order("slot_order")]);n=j[0].data,m=j[0].error,o=j[1].data,b=j[1].error}catch(j){console.error("[Match] Exception chargement deck:",j),_t(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>d("home"));return}if(m||b){console.error("[Match] Erreur Supabase:",m||b),_t(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>d("home"));return}const u=(o||[]).filter(j=>{var W;return j.is_starter&&((W=j.card)==null?void 0:W.player)}).map(j=>mi(j.card,j.position)),A=(o||[]).filter(j=>{var W;return!j.is_starter&&((W=j.card)==null?void 0:W.player)}).map(j=>mi(j.card,j.position));if(u.length<11){_t(e,"⚠️",`Deck incomplet (${u.length}/11).`,"Compléter",()=>d("decks"));return}const r=(o||[]).find(j=>{var W;return((W=j.card)==null?void 0:W.card_type)==="formation"}),p=(n==null?void 0:n.formation)||((B=r==null?void 0:r.card)==null?void 0:B.formation)||"4-4-2",{data:I,error:L}=await k.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",l.profile.id).eq("card_type","game_changer"),{data:F}=await k.from("gc_definitions").select("*").eq("is_active",!0),H=(I||[]).map(j=>({...j,_gcDef:(F==null?void 0:F.find(W=>W.name===j.gc_type||W.id===j.gc_definition_id))||null}));let C=null;if(n!=null&&n.stadium_card_id){const{data:j}=await k.from("cards").select("stadium_id").eq("id",n.stadium_card_id).single();if(j!=null&&j.stadium_id){const{data:W}=await k.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",j.stadium_id).single();C=W||null}}s({deckId:g,formation:p,starters:u,subsRaw:A,gcCardsEnriched:H,gcDefs:F||[],stadiumDef:C})}async function $n(e,t){const{state:i}=t,l=(i.params||{}).matchMode||"vs_ai_easy",d=l.replace("vs_ai_",""),a=l;await ai(e,t,l,async({deckId:c,formation:g,starters:n,subsRaw:o,gcCardsEnriched:m,gcDefs:b,stadiumDef:u})=>{try{let A=yt(n,g);u&&(A=Ht(A,u),oi(o,u));const r=await kn(g,d),p=async I=>{try{const{data:L,error:F}=await k.from("matches").insert({home_id:i.profile.id,away_id:null,mode:a,home_deck_id:c,status:"in_progress"}).select().single();if(F){console.error("[MatchIA] Erreur création match:",F),_t(e,"⚠️","Impossible de créer le match ("+F.message+").","Retour",()=>t.navigate("home"));return}const H={gcDefs:b||[],matchId:L==null?void 0:L.id,mode:a,difficulty:d,formation:g,homeTeam:A,aiTeam:r,homeSubs:o,subsUsed:0,maxSubs:Math.min(o.length,3),homeScore:0,aiScore:0,gcCards:I,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Tn(e,H,t)}catch(L){console.error("[MatchIA] Exception launchMatch:",L),_t(e,"⚠️","Erreur au lancement du match : "+L.message,"Retour",()=>t.navigate("home"))}};if(!m.length){p([]);return}ri(e,m,p)}catch(A){console.error("[MatchIA] Exception setup:",A),_t(e,"⚠️","Erreur de préparation du match : "+A.message,"Retour",()=>t.navigate("home"))}})}async function kn(e,t){var a,c;const{data:i}=await k.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return En(e);const s=It[e]||It["4-4-2"],l={GK:[],DEF:[],MIL:[],ATT:[]},d=[...i];for(const g of["GK","DEF","MIL","ATT"]){const n=d.filter(A=>A.job===g),o=d.filter(A=>A.job!==g),m=[...n,...o],b=[];for(let A=0;A<s[g];A++){const r=m[A]||d[A];r&&b.push({cardId:"ai-"+r.id+"-"+A,id:r.id,firstname:r.firstname,name:r.surname_encoded,country_code:r.country_code,club_id:r.club_id,job:r.job,job2:r.job2,note_g:Number(r.note_g)||0,note_d:Number(r.note_d)||0,note_m:Number(r.note_m)||0,note_a:Number(r.note_a)||0,rarity:r.rarity,skin:r.skin,hair:r.hair,hair_length:r.hair_length,clubName:((a=r.clubs)==null?void 0:a.encoded_name)||null,clubLogo:((c=r.clubs)==null?void 0:c.logo_url)||null,boost:0,used:!1,_line:g})}const u=lt(b.length);b.forEach((A,r)=>{A._col=u[r]}),l[g]=b}return l}function En(e){const t=It[e]||It["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},s=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let l=0;for(const d of["GK","DEF","MIL","ATT"]){const a=[];for(let g=0;g<t[d];g++){const n=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+l,id:"fake-"+l,firstname:"IA",name:s[l%s.length],country_code:"XX",club_id:null,job:d,job2:null,note_g:d==="GK"?n:2,note_d:d==="DEF"?n:2,note_m:d==="MIL"?n:2,note_a:d==="ATT"?n:2,rarity:"normal",boost:0,used:!1,_line:d}),l++}const c=lt(a.length);a.forEach((g,n)=>{g._col=c[n]}),i[d]=a}return i}function Tn(e,t,i){e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${r.map((F,H)=>{const C=H<r.length-1?at(F,r[H+1]):null,B=!C||C==="#ff3333"||C==="#cc2222",j=C==="#00ff88"?"+2":C==="#FFD700"?"+1":"";return`
          <div class="duel-card duel-card-${L}" data-idx="${H}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease, transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
            ${Ye({...F,note:je(F,"MIL"),_line:"MIL"},58,78)}
          </div>
          ${H<r.length-1?`<div class="duel-link duel-link-${L}" data-idx="${H}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${B?"rgba(255,255,255,0.12)":C};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${B?"none":`0 0 8px ${C}`}">
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
  </div>`;const m=()=>{const r=(p,I)=>e.querySelectorAll(p).forEach((L,F)=>{setTimeout(()=>{L.style.opacity="1",L.style.transform="translateY(0) scale(1)"},I+F*90)});r(".duel-card-home",150),r(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((p,I)=>{setTimeout(()=>{p.style.opacity="1"},I*70)}),900),setTimeout(()=>{const p=e.querySelector("#vs-label");p&&(p.style.opacity="1",p.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(I=>I.style.opacity="1")},1250),setTimeout(()=>{b("score-home",c,800),b("score-ai",g,800)},1500)};function b(r,p,I){const L=document.getElementById(r);if(!L)return;const F=performance.now(),H=C=>{const B=Math.min(1,(C-F)/I);L.textContent=Math.round(p*(1-Math.pow(1-B,3))),B<1?requestAnimationFrame(H):L.textContent=p};requestAnimationFrame(H)}requestAnimationFrame(m),t.attacker=n?"home":"ai";const u=n?ni():null;n&&(t.boostCard={value:u}),t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:s.map(r=>({name:r.name,note:je(r,"MIL"),portrait:Pe(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo})),aiPlayers:l.map(r=>({name:r.name,note:je(r,"MIL"),portrait:Pe(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo})),homeTotal:c,aiTotal:g,text:`Duel milieu : ${t.clubName} ${c} – ${g} IA → ${n?t.clubName+" attaque":"IA attaque"}`});const A=()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",Xe(e,t,i),t.attacker==="ai"&&setTimeout(()=>xi(e,t,i),800)};setTimeout(()=>{const r=document.getElementById("score-home"),p=document.getElementById("score-ai"),I=document.getElementById(n?"duel-row-home":"duel-row-ai"),L=document.getElementById(n?"duel-row-ai":"duel-row-home"),F=n?r:p,H=n?p:r;F&&(F.style.fontSize="80px",F.style.color=n?"#FFD700":"#ff6b6b",F.style.animation="duelPulse .5s ease"+(n?", duelGlow 1.5s ease infinite .5s":"")),H&&(H.style.opacity="0.25"),setTimeout(()=>{I&&(I.style.transformOrigin="center",I.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",I.style.zIndex="5"),setTimeout(()=>{var B;const C=document.getElementById("duel-shock");if(C){const j=(B=L||I)==null?void 0:B.getBoundingClientRect(),W=e.querySelector(".match-screen").getBoundingClientRect();j&&(C.style.top=j.top-W.top+j.height/2+"px"),C.style.animation="shockwave .5s ease-out forwards"}L&&(L.style.transformOrigin="center",L.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var B;const C=document.getElementById("duel-finale");C&&(C.innerHTML=`
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,C.style.transition="opacity .45s ease",C.style.opacity="1",C.style.pointerEvents="auto",(B=document.getElementById("start-match-btn"))==null||B.addEventListener("click",A))},600)},700)},2800)}function zn(e){if(e.type==="duel"&&(e.homeTotal!=null||e.aiTotal!=null)){const t=(e.homeTotal||0)>=(e.aiTotal||0);return`
    ...
    <div style="display:flex;align-items:center;gap:4px;${t?"flex-direction:row-reverse":""};background:rgba(255,255,255,0.04);border-radius:8px;padding:5px 8px;border:1px solid rgba(255,255,255,0.07)">
      ...
      ${dt(e.inPlayer||{})}
    </div>`}return`<div style="font-size:11px;color:${e.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${e.type==="goal"?700:400};padding:3px 2px">${e.text||""}</div>`}function Xe(e,t,i){var F,H,C,B,j,W,de,re;const s=t.selected.map(P=>P.cardId),l=t.usedSubIds||[],d=t.homeSubs.filter(P=>!l.includes(P.cardId)),c=Object.values(t.homeTeam).flat().filter(P=>P.used).length>0&&d.length>0&&t.subsUsed<t.maxSubs,g=!["GK","DEF","MIL","ATT"].some(P=>(t.aiTeam[P]||[]).some(X=>!X.used)),n=[...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(P=>!P.used),o=t.phase==="attack"&&g&&n.length===0?[...t.homeTeam.GK||[],...t.homeTeam.DEF||[]].filter(P=>!P.used).map(P=>P.cardId):[];t.log[t.log.length-1];const m=t.phase==="ai-attack"||t.phase==="ai-defense",b=t.phase==="attack",u=t.phase==="defense",A=t.phase==="finished",r=t.gcCards.filter(P=>!t.usedGc.includes(P.id)),p=t.boostCard&&!t.boostUsed;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column",e.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const P=window.innerWidth>=700,X=(_e,Ae,Ie)=>{var be,ze;const xe=(t.gcDefs||[]).find(M=>M.name===_e.gc_type),J={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[xe==null?void 0:xe.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",oe={purple:"#b06ce0",light_blue:"#00d4ef"}[xe==null?void 0:xe.color]||"#b06ce0",ge=(xe==null?void 0:xe.name)||_e.gc_type,Y=(xe==null?void 0:xe.effect)||((be=Ve[_e.gc_type])==null?void 0:be.desc)||"",Z=xe!=null&&xe.image_url?`/icons/${xe.image_url}`:null,K=((ze=Ve[_e.gc_type])==null?void 0:ze.icon)||"⚡",se=Math.round(Ie*.22),he=Math.round(Ie*.22),Le=Ie-se-he,Me=ge.length>12?7:9;return`<div class="gc-mini" data-gc-id="${_e.id}" data-gc-type="${_e.gc_type}"
          ...
          </div>`},N=(_e,Ae)=>Ae?U(130,175):X(_e,130,175),ne=(_e,Ae)=>Ae?U(68,95):X(_e,68,95),pe=P?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",ye=A?`<button id="btn-results" style="${pe};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:m?`<div style="${pe};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:b?`<button id="btn-action" style="${pe};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:u?`<button id="btn-action" style="${pe};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${pe};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,Se=b||u?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",we=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${P?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
  ...
  `,document.body.appendChild(s);let o=!1;const m=()=>{o||(o=!0,s.remove(),i())};s.addEventListener("click",m),setTimeout(m,2e3)}function jt(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function li(e,t,i,s=null,l=null){var u,A;if(t.phase!=="attack"){jt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){jt(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const d=Object.entries(t.homeTeam).flatMap(([r,p])=>(p||[]).filter(I=>I.used).map(I=>({...I,_line:I._line||r}))),a=t.homeSubs.filter(r=>!t.usedSubIds.includes(r.cardId));if(!d.length){jt("Aucun joueur utilisé à remplacer");return}if(!a.length){jt("Aucun remplaçant disponible");return}let c=Math.max(0,d.findIndex(r=>r.cardId===l));const g=((u=d[c])==null?void 0:u._line)||((A=d[c])==null?void 0:A.job);let n=s?Math.max(0,a.findIndex(r=>r.cardId===s)):Math.max(0,a.findIndex(r=>r.job===g)),o=!1;const m=document.createElement("div");m.id="sub-overlay",m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function b(){var C,B,j,W,de,re;const r=d[c],p=a[n],I=Math.min(130,Math.round((window.innerWidth-90)/2)),L=Math.round(I*1.35),F=P=>`background:rgba(255,255,255,0.12);border:none;color:${P?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${P?"default":"pointer"};flex-shrink:0`;m.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <button id="in-down" style="${F(n>=a.length-1)}" ${n>=a.length-1?"disabled":""}>▼</button>
    ...
    </div>`,document.body.appendChild(u),(A=u.querySelector("#gc-back"))==null||A.addEventListener("click",()=>u.remove()),(r=u.querySelector("#gc-use"))==null||r.addEventListener("click",()=>{u.remove(),Bn(e,t,i,s,l)})}function Yt(e,t,i,s,l,d){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let c=[];function g(){var n,o;a.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
      ${e.map(m=>{const b=m._line||m.job||"MIL",u={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[b]||"#555",A=je(m,b)+(m.boost||0),r=c.find(p=>p.cardId===m.cardId);return`<div class="gc-pick-item" data-cid="${m.cardId}"
          style="width:80px;border-radius:8px;border:2.5px solid ${r?"#FFD700":"rgba(255,255,255,0.25)"};background:${u};overflow:hidden;cursor:pointer;flex-shrink:0;${m.used?"opacity:0.3;pointer-events:none":""}">
    ...
    </div>`,(n=a.querySelector("#gc-picker-close"))==null||n.addEventListener("click",()=>a.remove()),a.querySelectorAll(".gc-pick-item").forEach(m=>{m.addEventListener("click",()=>{const b=m.dataset.cid,u=e.find(r=>r.cardId===b);if(!u)return;const A=c.findIndex(r=>r.cardId===b);A>-1?c.splice(A,1):c.length<t&&c.push(u),g()})}),(o=a.querySelector("#gc-picker-confirm"))==null||o.addEventListener("click",()=>{a.remove(),d(c)})}g(),document.body.appendChild(a)}const qn={BOOST_STAT:({value:e=1,count:t=1,roles:i=[]},s,l,d)=>{const a=Object.entries(s.homeTeam).filter(([c])=>!i.length||i.includes(c)).flatMap(([c,g])=>g.filter(n=>!n.used).map(n=>({...n,_line:c})));return a.length?(Yt(a,t,`Choisir ${t} joueur(s) à booster (+${e})`,l,s,c=>{c.forEach(g=>{const n=(s.homeTeam[g._line]||[]).find(o=>o.cardId===g.cardId);n&&(n.boost=(n.boost||0)+e,s.log.push({text:`⚡ +${e} sur ${n.name}`,type:"info"}))}),Xe(l,s,d)}),!0):(s.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),Xe(l,s,d),!0)},DEBUFF_STAT:({value:e=1,count:t=1,roles:i=[],target:s="ai"},l,d,a)=>{const c=s==="home"?l.homeTeam:l.aiTeam,g=s==="ai"?"adverse":"allié",n=Object.entries(c).filter(([o])=>!i.length||i.includes(o)).flatMap(([o,m])=>m.filter(b=>!b.used).map(b=>({...b,_line:o})));return n.length?(Yt(n,t,`Choisir ${t} joueur(s) ${g}(s) à débuffer (-${e})`,d,l,o=>{o.forEach(m=>{const u=((s==="home"?l.homeTeam:l.aiTeam)[m._line]||[]).find(A=>A.cardId===m.cardId);u&&(u.boost=(u.boost||0)-e,l.log.push({text:`🎯 -${e} sur ${u.name}${s==="ai"?" (IA)":""}`,type:"info"}))}),Xe(d,l,a)}),!0):(l.log.push({text:`🎯 Aucun joueur ${g} disponible`,type:"info"}),Xe(d,l,a),!0)},GRAY_PLAYER:({count:e=1,roles:t=[],target:i="ai"},s,l,d)=>{const a=i==="home"?s.homeTeam:s.aiTeam,c=i==="ai"?"adverse":"allié",g=Object.entries(a).filter(([n])=>!t.length||t.includes(n)).flatMap(([n,o])=>o.filter(m=>!m.used).map(m=>({...m,_line:n})));return g.length?(Yt(g,e,`Choisir ${e} joueur(s) ${c}(s) à exclure`,l,s,n=>{n.forEach(o=>{const b=((i==="home"?s.homeTeam:s.aiTeam)[o._line]||[]).find(u=>u.cardId===o.cardId);b&&(b.used=!0,s.log.push({text:`❌ ${b.name}${i==="ai"?" (IA)":""} exclu !`,type:"info"}))}),Xe(l,s,d)}),!0):(s.log.push({text:`❌ Aucun joueur ${c} à exclure`,type:"info"}),Xe(l,s,d),!0)},REVIVE_PLAYER:({count:e=1,roles:t=[]},i,s,l)=>{const d=Object.entries(i.homeTeam).filter(([a])=>!t.length||t.includes(a)).flatMap(([a,c])=>c.filter(g=>g.used).map(g=>({...g,_line:a})));return d.length?(Yt(d,e,`Choisir ${e} joueur(s) à ressusciter`,s,i,a=>{a.forEach(c=>{const g=(i.homeTeam[c._line]||[]).find(n=>n.cardId===c.cardId);g&&(g.used=!1,i.log.push({text:`💫 ${g.name} ressuscité !`,type:"info"}))}),Xe(s,i,l)}),!0):(i.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),Xe(s,i,l),!0)},REMOVE_GOAL:({},e)=>(e.aiScore>0?(e.aiScore--,e.log.push({text:"🚫 Dernier but IA annulé !",type:"info"})):e.log.push({text:"🚫 Aucun but à annuler",type:"info"}),!1),ADD_GOAL_DRAW:({},e)=>(e.homeScore===e.aiScore?(e.homeScore++,e.log.push({text:"🎯 But bonus (match nul) !",type:"info"})):e.log.push({text:"🎯 But bonus : non applicable (pas de match nul)",type:"info"}),!1),ADD_SUB:({value:e=1},t)=>(t.maxSubs=(t.maxSubs||3)+e,t.log.push({text:`🔄 +${e} remplacement(s) débloqué(s)`,type:"info"}),!1),CUSTOM:()=>!1};function Bn(e,t,i,s,l){s.usedGc.push(e);const d=s.gcDefs||[],a=d.find(g=>g.name===t)||d.find(g=>{var n;return((n=g.name)==null?void 0:n.toLowerCase().trim())===(t==null?void 0:t.toLowerCase().trim())});let c=!1;if(a!=null&&a.effect_type&&a.effect_type!=="CUSTOM"){const g=qn[a.effect_type];g?g(a.effect_params||{},s,i,l)||(c=!0):(l.toast(`Effet "${a.effect_type}" non implémenté`,"error"),c=!0)}else{switch(t){case"Double attaque":s.modifiers.home.doubleAttack=!0,s.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":s.modifiers.home.shield=!0,s.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const g=Object.entries(s.homeTeam).flatMap(([n,o])=>(o||[]).filter(m=>m.used).map(m=>({...m,_line:n})));g.length?(g[0].used=!1,s.log.push({text:`💫 ${g[0].name} ressuscité !`,type:"info"})):s.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":s.modifiers.ai.stolenNote=(s.modifiers.ai.stolenNote||0)+1,s.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const g=[...s.aiTeam.ATT||[],...s.aiTeam.MIL||[]].filter(n=>!n.used);if(g.length){const n=g.sort((o,m)=>je(m,"ATT")-je(o,"ATT"))[0];n.used=!0,s.log.push({text:`❄️ ${n.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":s.maxSubs++,s.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}c=!0}k.from("cards").delete().eq("id",e).then(()=>{}),c&&Xe(i,s,l)}function Fn(e,t,i){const s=Object.values(t.homeTeam).flat().filter(l=>!l.used);if(!s.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(l=>{l.addEventListener("click",()=>{const d=l.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const c=(t.homeTeam[a]||[]).find(g=>g.cardId===d);if(c){c.boost=(c.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${c.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),Xe(e,t,i)})})}function Qt(e,t,i,s,l){const d=document.createElement("div");d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const a=Array.from({length:10},(n,o)=>`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(d);let c=!1;const g=()=>{c||(c=!0,d.remove(),setTimeout(()=>l(),50))};d.addEventListener("click",g),setTimeout(g,3500)}async function Bt(e,t,i){var o,m;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:s}=i,l=t.homeScore>t.aiScore,d=t.homeScore===t.aiScore,a=l?"victoire":d?"nul":"defaite",c=Uo(t.mode,a);t.matchId&&await k.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:l?s.profile.id:null,home_credits_reward:c,played_at:new Date().toISOString()}).eq("id",t.matchId);const g={credits:(s.profile.credits||0)+c,matches_played:(s.profile.matches_played||0)+1};l?g.wins=(s.profile.wins||0)+1:d?g.draws=(s.profile.draws||0)+1:g.losses=(s.profile.losses||0)+1,await k.from("users").update(g).eq("id",s.profile.id),await i.refreshProfile();const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",n.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const A=async(H=!0)=>{b=!0,u&&(k.removeChannel(u),u=null),H&&await k.rpc("cancel_matchmaking",{p_user_id:n.profile.id}).catch(()=>{})};(F=document.getElementById("mm-cancel"))==null||F.addEventListener("click",async()=>{try{await A(!0)}catch{}We(e),o("home")});const r=async(H,C,B)=>{if(b)return;b=!0,u&&(k.removeChannel(u),u=null);const j=document.getElementById("mm-status");j&&(j.textContent="Adversaire trouvé !"),await new Promise(W=>setTimeout(W,600)),e.isConnected&&wo(e,t,H,B,a,c)},{data:p,error:I}=await k.rpc("try_matchmake",{p_user_id:n.profile.id,p_deck_id:i});if(I||!(p!=null&&p.success)){m("Erreur de matchmaking","error"),We(e),o("home");return}if(p.matched){r(p.match_id,p.opponent_id,!1);return}const L=document.getElementById("mm-status");L&&(L.textContent="En attente d'un autre joueur..."),u=k.channel("matchmaking-"+n.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${n.profile.id}`},H=>{const C=H.new;C.status==="matched"&&C.match_id&&r(C.match_id,C.matched_with,!0)}).subscribe()}async function Pn(e,t,i){const{state:s,navigate:l,toast:d}=t;try{(k.getChannels?k.getChannels():[]).forEach(n=>{const o=n.topic||"";(o.includes("matchmaking")||o.includes("pvp-match")||o.includes("friend-invite"))&&k.removeChannel(n)})}catch{}const{data:a}=await k.from("matches").select("home_id, away_id, status").eq("id",i).single();if(!a){d("Match introuvable","error"),l("home");return}if(a.status==="finished"){d("Ce match est terminé","info"),l("home");return}const c=a.home_id===s.profile.id;wo(e,t,i,c,[],[])}async function wo(e,t,i,s,l=[],d=[]){const{state:a,navigate:c,toast:g}=t,n=s?"p1":"p2",o=s?"p2":"p1",m=(l||[]).map(M=>M.id),b=(l||[]).map(M=>({id:M.id,gc_type:M.gc_type,_gcDef:M._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:u}=await k.from("matches").select("*").eq("id",i).single();if(!u){g("Match introuvable","error"),c("home");return}async function A(){const[{data:M},{data:D},{data:w},{data:E}]=await Promise.all([k.rpc("get_deck_for_match",{p_deck_id:u.home_deck_id}),k.rpc("get_deck_for_match",{p_deck_id:u.away_deck_id}),k.from("users").select("id,pseudo,club_name").eq("id",u.home_id).single(),k.from("users").select("id,pseudo,club_name").eq("id",u.away_id).single()]),f=T=>{const S=Number(T.evolution_bonus)||0;return{cardId:T.card_id,position:T.position,id:T.id,firstname:T.firstname,name:T.surname_encoded,country_code:T.country_code,club_id:T.club_id,job:T.job,job2:T.job2,note_g:(Number(T.note_g)||0)+(T.job==="GK"||T.job2==="GK"&&Number(T.note_g)>0?S:0),note_d:(Number(T.note_d)||0)+(T.job==="DEF"||T.job2==="DEF"&&Number(T.note_d)>0?S:0),note_m:(Number(T.note_m)||0)+(T.job==="MIL"||T.job2==="MIL"&&Number(T.note_m)>0?S:0),note_a:(Number(T.note_a)||0)+(T.job==="ATT"||T.job2==="ATT"&&Number(T.note_a)>0?S:0),evolution_bonus:S,rarity:T.rarity,skin:T.skin,hair:T.hair,hair_length:T.hair_length,clubName:T.club_encoded_name||null,clubLogo:T.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},x=((M==null?void 0:M.starters)||[]).map(T=>f(T)),y=((D==null?void 0:D.starters)||[]).map(T=>f(T)),h=(M==null?void 0:M.formation)||"4-4-2",v=(D==null?void 0:D.formation)||"4-4-2";let _=yt(x,h),z=yt(y,v);const $=((M==null?void 0:M.subs)||[]).map(T=>f(T));return stadiumDef&&s&&(_=Ht(_,stadiumDef),oi($,stadiumDef)),{p1Team:_,p2Team:z,p1Subs:$,p2Subs:((D==null?void 0:D.subs)||[]).map(T=>f(T)),p1Formation:h,p2Formation:v,p1Name:(w==null?void 0:w.club_name)||(w==null?void 0:w.pseudo)||"Joueur 1",p2Name:(E==null?void 0:E.club_name)||(E==null?void 0:E.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:s?m:[],gc_p2:s?[]:m,gcCardsFull_p1:s?b:[],gcCardsFull_p2:s?[]:b,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:d||[],lastActionAt:new Date().toISOString()}}let r=u.game_state&&Object.keys(u.game_state).length?u.game_state:null;if(!r)if(s){r=await A();const{data:M}=await k.from("matches").select("game_state").eq("id",i).single();!(M!=null&&M.game_state)||!Object.keys(M.game_state).length?await k.from("matches").update({game_state:r,turn_user_id:u.home_id}).eq("id",i):r=M.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let M=0;M<30&&!r;M++){await new Promise(w=>setTimeout(w,400));const{data:D}=await k.from("matches").select("game_state").eq("id",i).single();D!=null&&D.game_state&&Object.keys(D.game_state).length&&(r=D.game_state)}if(!r){g("Erreur de synchronisation","error"),c("home");return}r.gc_p2=m,r.gcCardsFull_p2=b,await k.from("matches").update({game_state:r}).eq("id",i)}let p=!1,I=null,L=!1;const F=new Set,H=new Set;function C(M){var _,z;try{k.removeChannel(B)}catch{}const D=r[n+"Score"]||0,w=r[o+"Score"]||0;let E,f;M.winner_id?(E=M.winner_id===a.profile.id,f=!1):M.forfeit?(E=D>w,f=!1):(f=D===w,E=D>w),(_=document.getElementById("pvp-end-overlay"))==null||_.remove();const x=document.createElement("div");x.id="pvp-end-overlay",x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const y=f?"🤝":E?"🏆":"😞",h=f?"MATCH NUL":E?"VICTOIRE !":"DÉFAITE",v=f?"#fff":E?"#FFD700":"#ff6b6b";x.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(x),(z=x.querySelector("#pvp-end-home"))==null||z.addEventListener("click",()=>{x.remove(),We(e),c("home")})}const B=k.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},M=>{const D=M.new;try{if(D.status==="finished"||D.forfeit){if(p)return;p=!0,I&&(clearInterval(I),I=null),D.game_state&&(r=D.game_state),C(D);return}if(D.game_state){const w=r;r=D.game_state;const E=r._lastGC;if(E&&E.seq&&!H.has(E.seq)&&(H.add(E.seq),E.by!==n)){we(E.type,E.by,()=>N());return}const f=w[n+"Score"]||0,x=w[o+"Score"]||0,y=r[n+"Score"]||0,h=r[o+"Score"]||0,v=y>f,_=h>x;if((v||_)&&!F.has(r.round)){F.add(r.round);const z=[...r.log||[]].reverse().find(T=>T.isGoal),$=((z==null?void 0:z.homePlayers)||[]).map(T=>({name:T.name,note:T.note,portrait:T.portrait,job:T.job}));Se($,y,h,v,()=>N());return}N()}}catch(w){console.error("[PvP] crash:",w)}}).subscribe();async function j(M){Object.assign(r,M),r.lastActionAt=new Date().toISOString();const{error:D}=await k.from("matches").update({game_state:r}).eq("id",i);D&&g("Erreur de synchronisation","error");try{N()}catch(w){console.error("[PvP] renderPvpScreen crash:",w)}}async function W(){if(p)return;p=!0,I&&(clearInterval(I),I=null);const M=s?u.away_id:u.home_id,D=s?"p2":"p1",w=s?"p1":"p2",E={...r,[D+"Score"]:3,[w+"Score"]:0,phase:"finished"};try{await k.from("matches").update({status:"finished",forfeit:!0,winner_id:M,home_score:E.p1Score||0,away_score:E.p2Score||0,game_state:E}).eq("id",i)}catch{}try{k.removeChannel(B)}catch{}setTimeout(()=>{We(e),c("home")},800)}const de={BOOST_STAT:({value:M=1,count:D=1,roles:w=[]},E,f)=>{const x=E[n+"Team"],y=Object.entries(x).filter(([h])=>!w.length||w.includes(h)).flatMap(([h,v])=>v.filter(_=>!_.used).map(_=>({..._,_line:h})));if(!y.length){E.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),f(E);return}re(y,D,`Choisir ${D} joueur(s) à booster (+${M})`,h=>{h.forEach(v=>{const _=(x[v._line]||[]).find(z=>z.cardId===v.cardId);_&&(_.boost=(_.boost||0)+M,E.log.push({text:`⚡ +${M} sur ${_.name}`,type:"info"}))}),E[n+"Team"]=x,f(E)})},DEBUFF_STAT:({value:M=1,count:D=1,roles:w=[],target:E="ai"},f,x)=>{const y=E==="home"?n:o,h=f[y+"Team"],v=E==="home"?"allié":"adverse",_=Object.entries(h).filter(([z])=>!w.length||w.includes(z)).flatMap(([z,$])=>$.map(T=>({...T,_line:z})));if(!_.length){f.log.push({text:`🎯 Aucun joueur ${v}`,type:"info"}),x(f);return}re(_,D,`Choisir ${D} joueur(s) ${v}(s) (-${M})`,z=>{z.forEach($=>{const T=(h[$._line]||[]).find(S=>S.cardId===$.cardId);T&&(T.boost=(T.boost||0)-M,f.log.push({text:`🎯 -${M} sur ${T.name}`,type:"info"}))}),f[y+"Team"]=h,x(f)})},GRAY_PLAYER:({count:M=1,roles:D=[],target:w="ai"},E,f)=>{const x=w==="home"?n:o,y=E[x+"Team"],h=w==="home"?"allié":"adverse",v=Object.entries(y).filter(([_])=>!D.length||D.includes(_)).flatMap(([_,z])=>z.filter($=>!$.used).map($=>({...$,_line:_})));if(!v.length){E.log.push({text:`❌ Aucun joueur ${h}`,type:"info"}),f(E);return}re(v,M,`Choisir ${M} joueur(s) ${h}(s) à exclure`,_=>{const z="usedCardIds_"+x,$=new Set(E[z]||[]);_.forEach(T=>{const S=(y[T._line]||[]).find(q=>q.cardId===T.cardId);S&&(S.used=!0,$.add(T.cardId),E.log.push({text:`❌ ${S.name} exclu !`,type:"info"}))}),E[z]=[...$],E[x+"Team"]=y,f(E)})},REVIVE_PLAYER:({count:M=1,roles:D=[]},w,E)=>{const f=w[n+"Team"],x=Object.entries(f).filter(([y])=>!D.length||D.includes(y)).flatMap(([y,h])=>h.filter(v=>v.used).map(v=>({...v,_line:y})));if(!x.length){w.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),E(w);return}re(x,M,`Choisir ${M} joueur(s) à ressusciter`,y=>{y.forEach(h=>{const v=(f[h._line]||[]).find(_=>_.cardId===h.cardId);v&&(v.used=!1,w.log.push({text:`💫 ${v.name} ressuscité !`,type:"info"}))}),w[n+"Team"]=f,E(w)})},REMOVE_GOAL:({},M,D)=>{const w=o+"Score";M[w]>0?(M[w]--,M.log.push({text:"🚫 Dernier but annulé !",type:"info"})):M.log.push({text:"🚫 Aucun but à annuler",type:"info"}),D(M)},ADD_GOAL_DRAW:({},M,D)=>{M[n+"Score"]===M[o+"Score"]?(M[n+"Score"]++,M.log.push({text:"🎯 But bonus !",type:"info"})):M.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),D(M)},ADD_SUB:({value:M=1},D,w)=>{D.maxSubs=(D.maxSubs||3)+M,D.log.push({text:`🔄 +${M} remplacement(s)`,type:"info"}),w(D)},CUSTOM:({},M,D)=>D(M)};function re(M,D,w,E){const f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let x=[];function y(){var v,_;const h=M.map(z=>{const $={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[z._line]||"#555",T=je(z,z._line)+(z.boost||0),q=x.find(R=>R.cardId===z.cardId)?"#FFD700":"rgba(255,255,255,0.25)",G=z.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${z.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${q};background:${$};overflow:hidden;cursor:pointer;${G}">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(T);let S=!1;const q=()=>{S||(S=!0,T.remove(),setTimeout(()=>w&&w(),50))};T.addEventListener("click",q),setTimeout(q,3e3)}function ve(M,D){var T,S,q,G;const E=(r["gcCardsFull_"+n]||[]).find(R=>R.id===M),f=E==null?void 0:E._gcDef,x={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[f==null?void 0:f.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",y={purple:"#b06ce0",light_blue:"#00d4ef"}[f==null?void 0:f.color]||"#b06ce0",h=(f==null?void 0:f.name)||D,v=(f==null?void 0:f.effect)||((T=Ve[D])==null?void 0:T.desc)||"Carte spéciale.",_=f!=null&&f.image_url?`/icons/${f.image_url}`:null,z=((S=Ve[D])==null?void 0:S.icon)||"⚡",$=document.createElement("div");$.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",$.innerHTML=`
      ...
      </div>`,document.body.appendChild($),(q=$.querySelector("#pvp-gc-back"))==null||q.addEventListener("click",()=>$.remove()),(G=$.querySelector("#pvp-gc-use"))==null||G.addEventListener("click",()=>{$.remove(),P(M,D)})}function _e(){var E;const M=r[n+"Team"],D=Object.entries(M).flatMap(([f,x])=>(x||[]).filter(y=>!y.used).map(y=>({...y,_line:f})));if(!D.length)return;const w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",w.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">⚡ Choisir un joueur pour +${r.boostValue}</div>
        <button id="bp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${D.map(f=>{const x={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[f._line]||"#555",y=je(f,f._line)+(f.boost||0);return`<div class="bp-item" data-cid="${f.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${x};overflow:hidden;cursor:pointer">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(f);let y=!1;const h=()=>{y||(y=!0,f.remove(),setTimeout(()=>w(),50))};f.addEventListener("click",h),setTimeout(h,2200)}function xe(){var D;const M=document.createElement("div");M.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",M.innerHTML=`
      ...
      </div>`,document.body.appendChild(D),(w=D.querySelector("#pvp-hist-close"))==null||w.addEventListener("click",()=>D.remove())}async function oe(){if(r.phase!==n+"-attack")return;const M=n==="p1"?"p2":"p1",D=(r.round||0)+1,w=[...r.log||[]];w.push({type:"info",text:`⏭️ ${r[n+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const E=Le(r),f=he(M),x=E||!f?"finished":M+"-attack";await j({["selected_"+n]:[],modifiers:{...r.modifiers,[n]:{}},pendingAttack:null,phase:x,attacker:M,round:D,log:w}),x==="finished"&&await be(r)}async function ge(){const M=r[n+"Team"],D=!["GK","DEF","MIL","ATT"].some(x=>(r[o+"Team"][x]||[]).some(y=>!y.used)),w=(r["selected_"+n]||[]).map(x=>{const y=(M[x._role]||[]).find(T=>T.cardId===x.cardId)||x,h=D&&["GK","DEF"].includes(x._role),v=M[x._role]||[],_=v.findIndex(T=>T.cardId===x.cardId),z=lt(v.length),$=_>=0?z[_]:y._col??1;return{...y,_line:x._role,_col:$,...h?{note_a:Math.max(1,Number(y.note_a)||0)}:{}}});if(!w.length)return;const E=Nt(w,r.modifiers[n]||{});X(n,w.map(x=>x.cardId)),w.forEach(x=>{const y=(M[x._role]||[]).find(h=>h.cardId===x.cardId);y&&(y.used=!0)}),r["selected_"+n]=[],N();const f=[...r.log||[]];if(D){const x=(r[n+"Score"]||0)+1,y=w.map($=>({name:$.name,note:je($,$._line||"ATT"),portrait:Pe($),job:$.job}));f.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:y,homeTotal:E.total,aiTotal:0});const h=(r.round||0)+1,v=n==="p1"?"p2":"p1",_={...r,[n+"Team"]:M,[n+"Score"]:x},z=Le(_);F.add(h),Se(y,x,r[o+"Score"]||0,!0,async()=>{await j({[n+"Team"]:M,[n+"Score"]:x,["selected_"+n]:[],modifiers:{...r.modifiers,[n]:{}},pendingAttack:null,phase:z?"finished":v+"-attack",attacker:v,round:h,log:f}),z&&await be({...r,[n+"Score"]:x})});return}f.push({type:"pending",text:`⚔️ ${r[n+"Name"]} attaque (${E.total})`}),await j({[n+"Team"]:M,[o+"Team"]:r[o+"Team"],pendingAttack:{...E,players:w,side:n},["selected_"+n]:[],modifiers:{...r.modifiers,[n]:{}},phase:o+"-defense",log:f})}async function Y(){const M=r[n+"Team"],D=(r["selected_"+n]||[]).map(G=>{const R=(M[G._role]||[]).find(ae=>ae.cardId===G.cardId)||G,O=M[G._role]||[],Q=O.findIndex(ae=>ae.cardId===G.cardId),V=lt(O.length),ie=Q>=0?V[Q]:R._col??1;return{...R,_line:G._role,_col:ie}}),w=Rt(D,r.modifiers[n]||{});X(n,D.map(G=>G.cardId)),D.forEach(G=>{const R=(M[G._role]||[]).find(O=>O.cardId===G.cardId);R&&(R.used=!0)}),r["selected_"+n]=[],N();const E=Ot(r.pendingAttack.total,w.total,r.modifiers[n]||{}),f=r.pendingAttack.side,x=E==="attack"||(E==null?void 0:E.goal),y=f==="p1"?"p2":"p1",h=(r.round||0)+1,v=(r.pendingAttack.players||[]).map(G=>({name:G.name,note:je(G,G._line||"ATT"),portrait:Pe(G),job:G.job})),_=[...r.log||[]];_.push({type:"duel",isGoal:x,homeScored:x&&f===n,text:x?`⚽ BUT de ${r[f+"Name"]} ! (${r.pendingAttack.total} vs ${w.total})`:`✋ Attaque stoppée (${r.pendingAttack.total} vs ${w.total})`,homePlayers:v,aiPlayers:D.map(G=>({name:G.name,note:je(G,G._line||"DEF"),portrait:Pe(G),job:G.job})),homeTotal:r.pendingAttack.total,aiTotal:w.total});const z=x?(r[f+"Score"]||0)+1:r[f+"Score"]||0,$={...r,[n+"Team"]:M,[f+"Score"]:z},T=Le($),S=T?"finished":y+"-attack",q=async()=>{await j({[n+"Team"]:M,[o+"Team"]:r[o+"Team"],[f+"Score"]:z,["selected_"+n]:[],modifiers:{...r.modifiers,[n]:{}},pendingAttack:null,phase:S,attacker:y,round:h,log:_}),(S==="finished"||T)&&await be({...r,[f+"Score"]:z})};if(x){const G=f===n,R=G?z:r[n+"Score"]||0,O=G?r[o+"Score"]||0:z;F.add(h),Se(v,R,O,G,q)}else await q()}function Z(M){return["MIL","ATT"].some(D=>(M[D]||[]).some(w=>!w.used))}function K(M){return["GK","DEF","MIL","ATT"].some(D=>(M[D]||[]).some(w=>!w.used))}function se(M){return K(M)&&!Z(M)}function he(M){const D=r[M+"Team"],w=r[(M==="p1"?"p2":"p1")+"Team"];return!!(Z(D)||!K(w)&&se(D))}function Le(M){const D=["MIL","ATT"].some(_=>(M.p1Team[_]||[]).some(z=>!z.used)),w=["MIL","ATT"].some(_=>(M.p2Team[_]||[]).some(z=>!z.used)),E=K(M.p1Team),f=K(M.p2Team);return!D&&!(!f&&E)&&(!w&&!(!E&&f))}function Me(M){const D=M.p1Score||0,w=M.p2Score||0;return D===w?null:D>w?u.home_id:u.away_id}async function be(M){try{const D=Me(M),w=D?u.home_id===D?u.away_id:u.home_id:null;await k.from("matches").update({status:"finished",winner_id:D,home_score:M.p1Score||0,away_score:M.p2Score||0}).eq("id",i),D&&w&&updateEvolutiveCards(D,w).catch(()=>{})}catch(D){console.error("[PvP] finishMatch:",D)}}function ze(){var f;const M=r[n+"Score"],D=r[o+"Score"],w=M>D,E=M===D;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
    ...
    </div>`,(f=document.getElementById("pvp-home"))==null||f.addEventListener("click",()=>{try{k.removeChannel(B)}catch{}We(e),c("home")})}N()}const Nn=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:vo,resumePvpMatch:Pn},Symbol.toStringTag,{value:"Module"}));async function Rn(e,t,i,s){var l,d;try{const a=(d=(l=t==null?void 0:t.state)==null?void 0:l.profile)==null?void 0:d.id;try{(k.getChannels?k.getChannels():[]).forEach(g=>{const n=g.topic||"";(n.includes("matchmaking")||n.includes("pvp-match"))&&k.removeChannel(g)})}catch(c){console.warn("[FriendMatch] cleanup canaux:",c)}a&&(await k.rpc("cancel_matchmaking",{p_user_id:a}).catch(()=>{}),await k.from("matchmaking_queue").delete().eq("user_id",a).then(()=>{},()=>{}))}catch{}await ai(e,t,"random",({deckId:a,formation:c,starters:g,subsRaw:n,gcCardsEnriched:o,gcDefs:m,stadiumDef:b})=>{const u=A=>On(e,t,a,c,g,n,A,m||[],i,s);if(!o.length){u([]);return}ri(e,o,u)})}async function On(e,t,i,s,l,d,a=[],c=[],g,n){var de;const{state:o,navigate:m,toast:b}=t,u=o.profile.id;let A=!1,r=null;et(e);try{await k.rpc("cancel_matchmaking",{p_user_id:u})}catch{}try{await k.from("matchmaking_queue").delete().eq("user_id",u)}catch{}const p=(re,P,X,U)=>{var N;e.innerHTML=`
      ...
      </div>`,(N=document.getElementById("lobby-cancel"))==null||N.addEventListener("click",async()=>{A=!0,r&&(k.removeChannel(r),r=null),X&&await k.from("friend_match_invites").update({status:"declined"}).eq("id",X),We(e),m("home")})},I=async(re,P)=>{A=!0,r&&(k.removeChannel(r),r=null),await new Promise(X=>setTimeout(X,600)),e.isConnected&&Hn(e,t,re,P,a,c)},{data:L}=await k.from("friend_match_invites").select("*").eq("status","pending").or(`and(inviter_id.eq.${u},invitee_id.eq.${g}),and(inviter_id.eq.${g},invitee_id.eq.${u})`).order("created_at",{ascending:!1}).limit(1).maybeSingle();let F,H;if(L&&L.inviter_id===g)H=!1,F=L.id,await k.from("friend_match_invites").update({invitee_deck_id:i,invitee_ready:!0,status:"accepted"}).eq("id",F);else{H=!0;const{data:re,error:P}=await k.from("friend_match_invites").insert({inviter_id:u,invitee_id:g,friend_id:g,inviter_deck_id:i,inviter_ready:!0,status:"pending"}).select().single();if(P||!re){b("Erreur création invitation","error"),We(e),m("home");return}F=re.id}if(p(!0,!H,F),!H){const re={home_id:g,away_id:u,home_deck_id:L.inviter_deck_id,away_deck_id:i,status:"active",mode:"friend"},P=await k.from("matches").insert(re).select().single(),X=P.data;if(!X){b("Erreur création match: "+(((de=P.error)==null?void 0:de.message)||""),"error"),We(e),m("home");return}await k.from("friend_match_invites").update({match_id:X.id,status:"matched"}).eq("id",F),I(X.id,!1);return}let C=!1;const B=re=>{A||C||(re.status==="matched"&&re.match_id?(C=!0,clearInterval(j),clearInterval(W),I(re.match_id,!0)):re.status==="declined"?(clearInterval(j),clearInterval(W),b(`${n} a décliné l'invitation`,"warning"),We(e),m("home")):re.invitee_ready&&p(!0,!0,F))},j=setInterval(()=>{if(A){clearInterval(j);return}k.from("matchmaking_queue").delete().eq("user_id",u).then(()=>{},()=>{})},3e3),W=setInterval(async()=>{if(A||C){clearInterval(W);return}const{data:re}=await k.from("friend_match_invites").select("*").eq("id",F).maybeSingle();re&&B(re)},1200);r=k.channel("friend-invite-"+F).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${F}`},re=>B(re.new)).subscribe()}async function Hn(e,t,i,s,l=[],d=[]){const{state:a,navigate:c,toast:g}=t,n=s?"p1":"p2",o=s?"p2":"p1",m=(l||[]).map(w=>w.id),b=(l||[]).map(w=>({id:w.id,gc_type:w.gc_type,_gcDef:w._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:u}=await k.from("matches").select("*").eq("id",i).single();if(!u){g("Match introuvable","error"),c("home");return}async function A(){const[{data:w},{data:E},{data:f},{data:x}]=await Promise.all([k.rpc("get_deck_for_match",{p_deck_id:u.home_deck_id}),k.rpc("get_deck_for_match",{p_deck_id:u.away_deck_id}),k.from("users").select("id,pseudo,club_name").eq("id",u.home_id).single(),k.from("users").select("id,pseudo,club_name").eq("id",u.away_id).single()]),y=$=>{const T=Number($.evolution_bonus)||0;return{cardId:$.card_id,position:$.position,id:$.id,firstname:$.firstname,name:$.surname_encoded,country_code:$.country_code,club_id:$.club_id,job:$.job,job2:$.job2,note_g:(Number($.note_g)||0)+($.job==="GK"||$.job2==="GK"&&Number($.note_g)>0?T:0),note_d:(Number($.note_d)||0)+($.job==="DEF"||$.job2==="DEF"&&Number($.note_d)>0?T:0),note_m:(Number($.note_m)||0)+($.job==="MIL"||$.job2==="MIL"&&Number($.note_m)>0?T:0),note_a:(Number($.note_a)||0)+($.job==="ATT"||$.job2==="ATT"&&Number($.note_a)>0?T:0),evolution_bonus:T,rarity:$.rarity,skin:$.skin,hair:$.hair,hair_length:$.hair_length,clubName:$.club_encoded_name||null,clubLogo:$.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},h=((w==null?void 0:w.starters)||[]).map($=>y($)),v=((E==null?void 0:E.starters)||[]).map($=>y($)),_=(w==null?void 0:w.formation)||"4-4-2",z=(E==null?void 0:E.formation)||"4-4-2";return{p1Team:(()=>{const $=yt(h,_);return stadiumDef?Ht($,stadiumDef):$})(),p2Team:yt(v,z),p1Subs:(()=>{const $=((w==null?void 0:w.subs)||[]).map(T=>y(T));return stadiumDef&&oi($,stadiumDef),$})(),p2Subs:((E==null?void 0:E.subs)||[]).map($=>y($)),p1Formation:_,p2Formation:z,p1Name:(f==null?void 0:f.club_name)||(f==null?void 0:f.pseudo)||"Joueur 1",p2Name:(x==null?void 0:x.club_name)||(x==null?void 0:x.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:s?m:[],gc_p2:s?[]:m,gcCardsFull_p1:s?b:[],gcCardsFull_p2:s?[]:b,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:d||[],lastActionAt:new Date().toISOString()}}let r=u.game_state&&Object.keys(u.game_state).length?u.game_state:null;if(!r)if(s){r=await A();const{data:w}=await k.from("matches").select("game_state").eq("id",i).single();!(w!=null&&w.game_state)||!Object.keys(w.game_state).length?await k.from("matches").update({game_state:r,turn_user_id:u.home_id}).eq("id",i):r=w.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let w=0;w<60&&!r;w++){await new Promise(f=>setTimeout(f,400));const{data:E}=await k.from("matches").select("game_state").eq("id",i).single();E!=null&&E.game_state&&Object.keys(E.game_state).length&&(r=E.game_state)}if(!r){g("Erreur de synchronisation","error"),c("home");return}r.gc_p2=m,r.gcCardsFull_p2=b,await k.from("matches").update({game_state:r}).eq("id",i)}let p=!1,I=!1,L=!1,F=null,H=!1;const C=new Set,B=new Set;function j(w){var $,T;try{k.removeChannel(W)}catch{}const E=r[n+"Score"]||0,f=r[o+"Score"]||0;if(!L){L=!0;const S=w.winner_id||(E>f?a.profile.id:f>E?"opp":null),q=S===a.profile.id?"win":S?"loss":null;q&&ti(()=>Promise.resolve().then(()=>oo),void 0).then(G=>G.applyOwnEvolution(a.profile.id,q)).catch(()=>{})}if(!I){const S=r.p1Score||0,q=r.p2Score||0,G=(r.usedGc_p1||[]).length,R=(r.usedGc_p2||[]).length,O=w.winner_id||(S>q?u.home_id:q>S?u.away_id:null);(O?a.profile.id===O:a.profile.id<(s?u.away_id:u.home_id))&&(I=!0,Xi({player1Id:u.home_id,player2Id:u.away_id,score1:S,score2:q,gc1:G,gc2:R}).catch(V=>console.warn("[FriendMatch] updateStats:",V)))}let x,y;w.winner_id?(x=w.winner_id===a.profile.id,y=!1):w.forfeit?(x=E>f,y=!1):(y=E===f,x=E>f),($=document.getElementById("pvp-end-overlay"))==null||$.remove();const h=document.createElement("div");h.id="pvp-end-overlay",h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const v=y?"🤝":x?"🏆":"😞",_=y?"MATCH NUL":x?"VICTOIRE !":"DÉFAITE",z=y?"#fff":x?"#FFD700":"#ff6b6b";h.innerHTML=`
      <div style="font-size:64px">${v}</div>
      <div style="font-size:26px;font-weight:900;color:${z}">${_}</div>
      <div style="font-size:18px">${r[n+"Name"]} ${E} – ${f} ${r[o+"Name"]}</div>
      ${w.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${x?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(h),(T=h.querySelector("#pvp-end-home"))==null||T.addEventListener("click",()=>{h.remove(),We(e),c("home")})}const W=k.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},w=>{const E=w.new;try{if(E.status==="finished"||E.forfeit){if(p)return;p=!0,F&&(clearInterval(F),F=null),E.game_state&&(r=E.game_state),j(E);return}if(E.game_state){const f=r;r=E.game_state;const x=r._lastGC;if(x&&x.seq&&!B.has(x.seq)&&(B.add(x.seq),x.by!==n)){_e(x.type,x.by,()=>pe());return}const y=f[n+"Score"]||0,h=f[o+"Score"]||0,v=r[n+"Score"]||0,_=r[o+"Score"]||0,z=v>y,$=_>h;if((z||$)&&!C.has(r.round)){C.add(r.round);const T=[...r.log||[]].reverse().find(q=>q.isGoal),S=((T==null?void 0:T.homePlayers)||[]).map(q=>({name:q.name,note:q.note,portrait:q.portrait,job:q.job}));ve(S,v,_,z,()=>pe());return}pe()}}catch(f){console.error("[PvP] crash:",f)}}).subscribe();async function de(w){Object.assign(r,w),r.lastActionAt=new Date().toISOString();const{error:E}=await k.from("matches").update({game_state:r}).eq("id",i);E&&g("Erreur de synchronisation","error");try{pe()}catch(f){console.error("[PvP] renderPvpScreen crash:",f)}}async function re(){if(p)return;p=!0,F&&(clearInterval(F),F=null);const w=s?u.away_id:u.home_id,E=s?"p2":"p1",f=s?"p1":"p2",x={...r,[E+"Score"]:3,[f+"Score"]:0,phase:"finished"};try{await k.from("matches").update({status:"finished",forfeit:!0,winner_id:w,home_score:x.p1Score||0,away_score:x.p2Score||0,game_state:x}).eq("id",i)}catch{}try{k.removeChannel(W)}catch{}setTimeout(()=>{We(e),c("home")},800)}const P={BOOST_STAT:({value:w=1,count:E=1,roles:f=[]},x,y)=>{const h=x[n+"Team"],v=Object.entries(h).filter(([_])=>!f.length||f.includes(_)).flatMap(([_,z])=>z.filter($=>!$.used).map($=>({...$,_line:_})));if(!v.length){x.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),y(x);return}X(v,E,`Choisir ${E} joueur(s) à booster (+${w})`,_=>{_.forEach(z=>{const $=(h[z._line]||[]).find(T=>T.cardId===z.cardId);$&&($.boost=($.boost||0)+w,x.log.push({text:`⚡ +${w} sur ${$.name}`,type:"info"}))}),x[n+"Team"]=h,y(x)})},DEBUFF_STAT:({value:w=1,count:E=1,roles:f=[],target:x="ai"},y,h)=>{const v=x==="home"?n:o,_=y[v+"Team"],z=x==="home"?"allié":"adverse",$=Object.entries(_).filter(([T])=>!f.length||f.includes(T)).flatMap(([T,S])=>S.map(q=>({...q,_line:T})));if(!$.length){y.log.push({text:`🎯 Aucun joueur ${z}`,type:"info"}),h(y);return}X($,E,`Choisir ${E} joueur(s) ${z}(s) (-${w})`,T=>{T.forEach(S=>{const q=(_[S._line]||[]).find(G=>G.cardId===S.cardId);q&&(q.boost=(q.boost||0)-w,y.log.push({text:`🎯 -${w} sur ${q.name}`,type:"info"}))}),y[v+"Team"]=_,h(y)})},GRAY_PLAYER:({count:w=1,roles:E=[],target:f="ai"},x,y)=>{const h=f==="home"?n:o,v=x[h+"Team"],_=f==="home"?"allié":"adverse",z=Object.entries(v).filter(([$])=>!E.length||E.includes($)).flatMap(([$,T])=>T.filter(S=>!S.used).map(S=>({...S,_line:$})));if(!z.length){x.log.push({text:`❌ Aucun joueur ${_}`,type:"info"}),y(x);return}X(z,w,`Choisir ${w} joueur(s) ${_}(s) à exclure`,$=>{const T="usedCardIds_"+h,S=new Set(x[T]||[]);$.forEach(q=>{const G=(v[q._line]||[]).find(R=>R.cardId===q.cardId);G&&(G.used=!0,S.add(q.cardId),x.log.push({text:`❌ ${G.name} exclu !`,type:"info"}))}),x[T]=[...S],x[h+"Team"]=v,y(x)})},REVIVE_PLAYER:({count:w=1,roles:E=[]},f,x)=>{const y=f[n+"Team"],h=Object.entries(y).filter(([v])=>!E.length||E.includes(v)).flatMap(([v,_])=>_.filter(z=>z.used).map(z=>({...z,_line:v})));if(!h.length){f.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),x(f);return}X(h,w,`Choisir ${w} joueur(s) à ressusciter`,v=>{v.forEach(_=>{const z=(y[_._line]||[]).find($=>$.cardId===_.cardId);z&&(z.used=!1,f.log.push({text:`💫 ${z.name} ressuscité !`,type:"info"}))}),f[n+"Team"]=y,x(f)})},REMOVE_GOAL:({},w,E)=>{const f=o+"Score";w[f]>0?(w[f]--,w.log.push({text:"🚫 Dernier but annulé !",type:"info"})):w.log.push({text:"🚫 Aucun but à annuler",type:"info"}),E(w)},ADD_GOAL_DRAW:({},w,E)=>{w[n+"Score"]===w[o+"Score"]?(w[n+"Score"]++,w.log.push({text:"🎯 But bonus !",type:"info"})):w.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),E(w)},ADD_SUB:({value:w=1},E,f)=>{E.maxSubs=(E.maxSubs||3)+w,E.log.push({text:`🔄 +${w} remplacement(s)`,type:"info"}),f(E)},CUSTOM:({},w,E)=>E(w)};function X(w,E,f,x){const y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let h=[];function v(){var z,$;const _=w.map(T=>{const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[T._line]||"#555",q=je(T,T._line)+(T.boost||0),R=h.find(Q=>Q.cardId===T.cardId)?"#FFD700":"rgba(255,255,255,0.25)",O=T.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${T.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${R};background:${S};overflow:hidden;cursor:pointer;${O}">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(S),(R=S.querySelector("#pvp-gc-back"))==null||R.addEventListener("click",()=>S.remove()),(O=S.querySelector("#pvp-gc-use"))==null||O.addEventListener("click",()=>{S.remove(),U(w,E)})}function Ie(){var x;const w=r[n+"Team"],E=Object.entries(w).flatMap(([y,h])=>(h||[]).filter(v=>!v.used).map(v=>({...v,_line:y})));if(!E.length)return;const f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",f.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">⚡ Choisir un joueur pour +${r.boostValue}</div>
        <button id="bp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
      </div>
      ...
      </div>`,document.body.appendChild(f),(x=f.querySelector("#bp-close"))==null||x.addEventListener("click",()=>f.remove()),f.querySelectorAll(".bp-item").forEach(y=>{y.addEventListener("click",async()=>{const h=y.dataset.cid,v=E.find(z=>z.cardId===h);if(!v)return;const _=(w[v._line]||[]).find(z=>z.cardId===h);_&&(_.boost=(_.boost||0)+r.boostValue),f.remove(),await de({[n+"Team"]:w,boostUsed:!0})})})}function xe(w=null){var G,R;if(!(r.phase===n+"-attack")){g("Remplacement uniquement avant votre attaque","warning");return}const f=r[n+"Team"],x=r["usedSubIds_"+n]||[],y=r.maxSubs||3;if(x.length>=y){g(`Maximum ${y} remplacements atteint`,"warning");return}const h=Object.entries(f).flatMap(([O,Q])=>(Q||[]).filter(V=>V.used).map(V=>({...V,_line:O}))),v=(r[n+"Subs"]||[]).filter(O=>!x.includes(O.cardId));if(!h.length){g("Aucun joueur utilisé à remplacer","warning");return}if(!v.length){g("Aucun remplaçant disponible","warning");return}let _=Math.max(0,h.findIndex(O=>O.cardId===w));const z=((G=h[_])==null?void 0:G._line)||((R=h[_])==null?void 0:R.job);let $=Math.max(0,v.findIndex(O=>O.job===z)),T=!1;const S=document.createElement("div");S.id="pvp-sub-overlay",S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function q(){var me,$e,ke,qe,Ce,Re;const O=h[_],Q=v[$],V=Math.min(130,Math.round((window.innerWidth-90)/2)),ie=Math.round(V*1.35),ae=Fe=>`background:rgba(255,255,255,0.12);border:none;color:${Fe?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Fe?"default":"pointer"};flex-shrink:0`;S.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      </div>`,document.body.appendChild(E),(x=E.querySelector("#pvp-hist-close"))==null||x.addEventListener("click",()=>E.remove())}async function Y(){if(r.phase!==n+"-attack")return;const w=n==="p1"?"p2":"p1",E=(r.round||0)+1,f=[...r.log||[]];f.push({type:"info",text:`⏭️ ${r[n+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const x=be(r),y=Me(w),h=x||!y?"finished":w+"-attack";await de({["selected_"+n]:[],modifiers:{...r.modifiers,[n]:{}},pendingAttack:null,phase:h,attacker:w,round:E,log:f}),h==="finished"&&await M(r)}async function Z(){const w=r[n+"Team"],E=!["GK","DEF","MIL","ATT"].some(h=>(r[o+"Team"][h]||[]).some(v=>!v.used)),f=(r["selected_"+n]||[]).map(h=>{const v=(w[h._role]||[]).find(q=>q.cardId===h.cardId)||h,_=E&&["GK","DEF"].includes(h._role),z=w[h._role]||[],$=z.findIndex(q=>q.cardId===h.cardId),T=lt(z.length),S=$>=0?T[$]:v._col??1;return{...v,_line:h._role,_col:S,..._?{note_a:Math.max(1,Number(v.note_a)||0)}:{}}});if(!f.length)return;const x=Nt(f,r.modifiers[n]||{});N(n,f.map(h=>h.cardId)),f.forEach(h=>{const v=(w[h._role]||[]).find(_=>_.cardId===h.cardId);v&&(v.used=!0)}),r["selected_"+n]=[],pe();const y=[...r.log||[]];if(E){const h=(r[n+"Score"]||0)+1,v=f.map(S=>({name:S.name,note:je(S,S._line||"ATT"),portrait:Pe(S),job:S.job}));y.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:v,homeTotal:x.total,aiTotal:0});const _=(r.round||0)+1,z=n==="p1"?"p2":"p1",$={...r,[n+"Team"]:w,[n+"Score"]:h},T=be($);C.add(_),ve(v,h,r[o+"Score"]||0,!0,async()=>{await de({[n+"Team"]:w,[n+"Score"]:h,["selected_"+n]:[],modifiers:{...r.modifiers,[n]:{}},pendingAttack:null,phase:T?"finished":z+"-attack",attacker:z,round:_,log:y}),T&&await M({...r,[n+"Score"]:h})});return}y.push({type:"pending",text:`⚔️ ${r[n+"Name"]} attaque (${x.total})`}),await de({[n+"Team"]:w,[o+"Team"]:r[o+"Team"],pendingAttack:{...x,players:f,side:n},["selected_"+n]:[],modifiers:{...r.modifiers,[n]:{}},phase:o+"-defense",log:y})}async function K(){const w=r[n+"Team"],E=(r["selected_"+n]||[]).map(O=>{const Q=(w[O._role]||[]).find(me=>me.cardId===O.cardId)||O,V=w[O._role]||[],ie=V.findIndex(me=>me.cardId===O.cardId),ae=lt(V.length),fe=ie>=0?ae[ie]:Q._col??1;return{...Q,_line:O._role,_col:fe}}),f=Rt(E,r.modifiers[n]||{});N(n,E.map(O=>O.cardId)),E.forEach(O=>{const Q=(w[O._role]||[]).find(V=>V.cardId===O.cardId);Q&&(Q.used=!0)}),r["selected_"+n]=[],pe();const x=Ot(r.pendingAttack.total,f.total,r.modifiers[n]||{}),y=r.pendingAttack.side,h=x==="attack"||(x==null?void 0:x.goal),v=y==="p1"?"p2":"p1",_=(r.round||0)+1,z=(r.pendingAttack.players||[]).map(O=>({name:O.name,note:je(O,O._line||"ATT"),portrait:Pe(O),job:O.job})),$=[...r.log||[]];$.push({type:"duel",isGoal:h,homeScored:h&&y===n,text:h?`⚽ BUT de ${r[y+"Name"]} ! (${r.pendingAttack.total} vs ${f.total})`:`✋ Attaque stoppée (${r.pendingAttack.total} vs ${f.total})`,homePlayers:z,aiPlayers:E.map(O=>({name:O.name,note:je(O,O._line||"DEF"),portrait:Pe(O),job:O.job})),homeTotal:r.pendingAttack.total,aiTotal:f.total});const T=h?(r[y+"Score"]||0)+1:r[y+"Score"]||0,S={...r,[n+"Team"]:w,[y+"Score"]:T},q=be(S),G=q?"finished":v+"-attack",R=async()=>{await de({[n+"Team"]:w,[o+"Team"]:r[o+"Team"],[y+"Score"]:T,["selected_"+n]:[],modifiers:{...r.modifiers,[n]:{}},pendingAttack:null,phase:G,attacker:v,round:_,log:$}),(G==="finished"||q)&&await M({...r,[y+"Score"]:T})};if(h){const O=y===n,Q=O?T:r[n+"Score"]||0,V=O?r[o+"Score"]||0:T;C.add(_),ve(z,Q,V,O,R)}else await R()}function se(w){return["MIL","ATT"].some(E=>(w[E]||[]).some(f=>!f.used))}function he(w){return["GK","DEF","MIL","ATT"].some(E=>(w[E]||[]).some(f=>!f.used))}function Le(w){return he(w)&&!se(w)}function Me(w){const E=r[w+"Team"],f=r[(w==="p1"?"p2":"p1")+"Team"];return!!(se(E)||!he(f)&&Le(E))}function be(w){const E=["MIL","ATT"].some($=>(w.p1Team[$]||[]).some(T=>!T.used)),f=["MIL","ATT"].some($=>(w.p2Team[$]||[]).some(T=>!T.used)),x=he(w.p1Team),y=he(w.p2Team);return!E&&!(!y&&x)&&(!f&&!(!x&&y))}function ze(w){const E=w.p1Score||0,f=w.p2Score||0;return E===f?null:E>f?u.home_id:u.away_id}async function M(w){try{const E=ze(w),f=E?u.home_id===E?u.away_id:u.home_id:null;await k.from("matches").update({status:"finished",winner_id:E,home_score:w.p1Score||0,away_score:w.p2Score||0}).eq("id",i),E&&f&&ki(E,f).catch(()=>{})}catch(E){console.error("[PvP] finishMatch:",E)}}function D(){var y;const w=r[n+"Score"],E=r[o+"Score"],f=w>E,x=w===E;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      <button id="pvp-home" style="padding:16px 40px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏠 Retour</button>
    </div>`,(y=document.getElementById("pvp-home"))==null||y.addEventListener("click",()=>{try{k.removeChannel(W)}catch{}We(e),c("home")})}pe()}const Lt="#1A6B3C",xt="#D4A017";async function Un(e,t){var s;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const i=(s=t.state.params)==null?void 0:s.openLeagueId;if(i){t.state.params.openLeagueId=null,await bt(e,t,i);return}await $t(e,t)}async function $t(e,t,i="waiting"){var I;const{state:s}=t,l=s.profile.id,{data:d}=await k.from("mini_league_members").select("league_id").eq("user_id",l),a=(d||[]).map(L=>L.league_id),{data:c,error:g}=await k.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").eq("is_archived",!1).order("created_at",{ascending:!1}).limit(30),n=g?(await k.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").order("created_at",{ascending:!1}).limit(30)).data||[]:c||[],m=(a.length?await k.from("mini_leagues").select("*, mini_league_members(count)").in("id",a).order("created_at",{ascending:!1}):{data:[]}).data||[],b=m.filter(L=>L.status==="waiting"&&!L.is_archived),u=m.filter(L=>L.status==="active"&&!L.is_archived),A=m.filter(L=>L.is_archived||L.status==="finished"),r=n.filter(L=>!a.includes(L.id)),p=[{key:"waiting",label:"🟡 En attente",count:b.length+r.length},{key:"active",label:"🟢 En cours",count:u.length},{key:"archived",label:"📁 Archivées",count:A.length}];e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?Kn(b,r,l):i==="active"?Vn(u,l):Yn(A,l)}
    </div>
  </div>`,(I=document.getElementById("ml-create-btn"))==null||I.addEventListener("click",()=>Wn(e,t)),e.querySelectorAll(".ml-tab").forEach(L=>L.addEventListener("click",()=>$t(e,t,L.dataset.tab))),e.querySelectorAll("[data-league-id]").forEach(L=>L.addEventListener("click",()=>bt(e,t,L.dataset.leagueId))),e.querySelectorAll("[data-join]").forEach(L=>L.addEventListener("click",F=>{F.stopPropagation(),_o(e,t,L.dataset.join,L.dataset.type)})),e.querySelectorAll("[data-delete]").forEach(L=>L.addEventListener("click",F=>{F.stopPropagation(),yi(e,t,L.dataset.delete,L.dataset.name,i)}))}function ei(e,t,i=!1){var a,c;const s=e.creator_id===t,l=e.pot||0,d=((c=(a=e.mini_league_members)==null?void 0:a[0])==null?void 0:c.count)||0;return`<div data-league-id="${e.id}" style="background:#fff;border-radius:12px;padding:14px 16px;box-shadow:0 1px 6px rgba(0,0,0,0.08);cursor:pointer;margin-bottom:8px">
    ...
    <div style="padding:20px;display:flex;flex-direction:column;gap:14px">
      ...
      </div>`,document.body.appendChild(t);const i=t.querySelector("#ml-pwd-input"),s=l=>{t.remove(),e(l)};setTimeout(()=>i==null?void 0:i.focus(),80),t.querySelector("#pwd-cancel").addEventListener("click",()=>s(null)),t.querySelector("#pwd-ok").addEventListener("click",()=>s(i.value.trim())),i.addEventListener("keydown",l=>{l.key==="Enter"&&s(i.value.trim())}),t.addEventListener("click",l=>{l.target===t&&s(null)})})}async function _o(e,t,i,s){const{toast:l,state:d}=t,a=d.profile.id,{data:c}=await k.from("mini_leagues").select("password,status,entry_fee,pot,max_players").eq("id",i).single();if(!c){l("Mini League introuvable","error");return}if(c.status!=="waiting"){l("Cette Mini League a déjà démarré","warning");return}const{count:g}=await k.from("mini_league_members").select("id",{count:"exact",head:!0}).eq("league_id",i);if(g>=c.max_players){l("La Mini League est complète","warning");return}if(s==="private"){const r=await Jn();if(r===null)return;if(c.password!==r){l("Mot de passe incorrect","error");return}}const n=c.entry_fee||100,{data:o}=await k.from("users").select("credits").eq("id",a).single();if(((o==null?void 0:o.credits)||0)<n){l(`Crédits insuffisants — il te faut ${n.toLocaleString("fr")} cr. (solde : ${((o==null?void 0:o.credits)||0).toLocaleString("fr")} cr.)`,"error");return}const{error:m}=await k.from("mini_league_members").insert({league_id:i,user_id:a});if(m){l("Erreur inscription : "+m.message,"error");return}const{error:b}=await k.from("users").update({credits:o.credits-n}).eq("id",a),{error:u}=await k.from("mini_leagues").update({pot:(c.pot||0)+n}).eq("id",i);(b||u)&&console.error("Erreur post-inscription:",b,u),d.profile&&(d.profile.credits=o.credits-n);const A=document.getElementById("nav-credits");A&&(A.textContent=`💰 ${(d.profile.credits||0).toLocaleString("fr")}`),l(`✅ Inscrit ! −${n.toLocaleString("fr")} cr. · Pot : ${((c.pot||0)+n).toLocaleString("fr")} cr.`,"success"),bt(e,t,i)}async function Xn(e,t,i,s){const{toast:l,state:d}=t,a=d.profile.id;if(!confirm(`Te désinscrire et récupérer ${s.toLocaleString("fr")} cr. ?`))return;const{data:c}=await k.from("mini_leagues").select("pot,status").eq("id",i).single();if(!c||c.status!=="waiting"){l("Impossible — la league a déjà démarré","error");return}const{data:g}=await k.from("users").select("credits").eq("id",a).single();await k.from("mini_leagues").update({pot:Math.max(0,(c.pot||0)-s)}).eq("id",i),await k.from("users").update({credits:((g==null?void 0:g.credits)||0)+s}).eq("id",a),await k.from("mini_league_members").delete().eq("league_id",i).eq("user_id",a),d.profile&&(d.profile.credits=((g==null?void 0:g.credits)||0)+s);const n=document.getElementById("nav-credits");n&&(n.textContent=`💰 ${(d.profile.credits||0).toLocaleString("fr")}`),l(`↩️ Désinscrit · +${s.toLocaleString("fr")} cr. remboursés`,"success"),$t(e,t,"waiting")}async function yi(e,t,i,s,l){const{toast:d}=t;if(!confirm(`Supprimer définitivement "${s}" et tous ses matchs/membres ? Action irréversible.`))return;await k.from("mini_league_matches").delete().eq("league_id",i),await k.from("mini_league_members").delete().eq("league_id",i);const{error:a}=await k.from("mini_leagues").delete().eq("id",i);if(a){d("Erreur suppression ("+a.message+")","error");return}d("Mini League supprimée avec succès","success"),$t(e,t,l)}async function Zn(e,t,i){await k.from("mini_leagues").update({is_archived:!0}).eq("id",i),t.toast("Mini League archivée","success"),$t(e,t,"archived")}async function bt(e,t,i){var H,C,B,j,W,de,re,P,X;const{state:s,toast:l}=t,d=s.profile.id,[{data:a},{data:c},{data:g}]=await Promise.all([k.from("mini_leagues").select("*").eq("id",i).single(),k.from("mini_league_members").select("*, user:users(id,pseudo,club_name,club_color1,club_color2)").eq("league_id",i),k.from("mini_league_matches").select("*").eq("league_id",i).order("matchday").order("created_at")]);if(!a){l("Introuvable","error"),await $t(e,t);return}const n=(c||[]).some(U=>U.user_id===d),o=a.creator_id===d,m=(c||[]).map(U=>U.user).filter(Boolean),b=ko(m,g||[]),u=(g||[]).filter(U=>U.matchday===a.current_day),A=a.pot||0,r=a.entry_fee||100,p=u.length>0&&u.every(U=>U.status==="finished"||U.status==="bye"),I=a.current_day>=a.total_days,L=(c||[]).find(U=>U.user_id===d);e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${b.map((U,N)=>{const ne=A>0&&a.status==="finished"?N===0?Math.floor(A*.7):N===1?Math.floor(A*.2):N===2?Math.floor(A*.1):0:0;return`<tr style="border-bottom:1px solid #f5f5f5;${U.userId===d?"background:#f0fdf4;":""}">
    ...
    <div style="background:linear-gradient(160deg,#1a0a2e,#3b1a6e);border-radius:20px;padding:32px 24px;max-width:320px;width:100%;text-align:center;color:#fff;box-shadow:0 8px 40px rgba(0,0,0,0.6)">
      ...
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:20px;background:linear-gradient(180deg,#0a1628,#1a0a2e);padding:24px;text-align:center;color:#fff">
        ...
        <button id="lobby-cancel" style="padding:10px 24px;border-radius:10px;border:1.5px solid rgba(255,255,255,0.2);background:transparent;color:rgba(255,255,255,0.6);font-size:13px;cursor:pointer">Annuler</button>
        <style>@keyframes lobbyPulse{0%,100%{opacity:1}50%{opacity:0.4}}</style>
      </div>`,(W=document.getElementById("lobby-cancel"))==null||W.addEventListener("click",()=>{A=!0,clearInterval(r),We(e),m("mini-league")})},C=async(B,j)=>{A=!0,clearInterval(r),await new Promise(W=>setTimeout(W,600)),e.isConnected&&nr(e,t,B,j,a,c)};if(H(),I){let B=p.match_id;if(!B){const{data:W,error:de}=await k.from("matches").insert({home_id:u,away_id:F,home_deck_id:i,away_deck_id:null,status:"active",mode:"mini_league"}).select().single();if(de||!W){b("Erreur création match","error"),We(e),m("mini-league");return}B=W.id,await k.from("mini_league_matches").update({match_id:B,status:"playing"}).eq("id",g)}const j=B;r=setInterval(async()=>{if(A){clearInterval(r);return}const{data:W}=await k.from("matches").select("away_deck_id").eq("id",j).single();W!=null&&W.away_deck_id&&(clearInterval(r),C(j,!0))},1500)}else{let B=p.match_id;r=setInterval(async()=>{if(A){clearInterval(r);return}const{data:j}=await k.from("mini_league_matches").select("match_id").eq("id",g).single();j!=null&&j.match_id&&(clearInterval(r),B=j.match_id,await k.from("matches").update({away_deck_id:i}).eq("id",B),C(B,!1))},1500)}}async function nr(e,t,i,s,l=[],d=[]){var E;const{state:a,navigate:c,toast:g}=t,n=((E=a.params)==null?void 0:E.leagueId)||null,o=s?"p1":"p2",m=s?"p2":"p1",b=(l||[]).map(f=>f.id),u=(l||[]).map(f=>({id:f.id,gc_type:f.gc_type,_gcDef:f._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:A}=await k.from("matches").select("*").eq("id",i).single();if(!A){g("Match introuvable","error"),c("mini-league");return}async function r(){const[{data:f},{data:x},{data:y},{data:h}]=await Promise.all([k.rpc("get_deck_for_match",{p_deck_id:A.home_deck_id}),k.rpc("get_deck_for_match",{p_deck_id:A.away_deck_id}),k.from("users").select("id,pseudo,club_name").eq("id",A.home_id).single(),k.from("users").select("id,pseudo,club_name").eq("id",A.away_id).single()]),v=S=>{const q=Number(S.evolution_bonus)||0;return{cardId:S.card_id,position:S.position,id:S.id,firstname:S.firstname,name:S.surname_encoded,country_code:S.country_code,club_id:S.club_id,job:S.job,job2:S.job2,note_g:(Number(S.note_g)||0)+(S.job==="GK"||S.job2==="GK"&&Number(S.note_g)>0?q:0),note_d:(Number(S.note_d)||0)+(S.job==="DEF"||S.job2==="DEF"&&Number(S.note_d)>0?q:0),note_m:(Number(S.note_m)||0)+(S.job==="MIL"||S.job2==="MIL"&&Number(S.note_m)>0?q:0),note_a:(Number(S.note_a)||0)+(S.job==="ATT"||S.job2==="ATT"&&Number(S.note_a)>0?q:0),evolution_bonus:q,rarity:S.rarity,skin:S.skin,hair:S.hair,hair_length:S.hair_length,clubName:S.club_encoded_name||null,clubLogo:S.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},_=((f==null?void 0:f.starters)||[]).map(S=>v(S)),z=((x==null?void 0:x.starters)||[]).map(S=>v(S)),$=(f==null?void 0:f.formation)||"4-4-2",T=(x==null?void 0:x.formation)||"4-4-2";return{p1Team:(()=>{const S=yt(_,$);return stadiumDef?Ht(S,stadiumDef):S})(),p2Team:yt(z,T),p1Subs:(()=>{const S=((f==null?void 0:f.subs)||[]).map(q=>v(q));return stadiumDef&&oi(S,stadiumDef),S})(),p2Subs:((x==null?void 0:x.subs)||[]).map(S=>v(S)),p1Formation:$,p2Formation:T,p1Name:(y==null?void 0:y.club_name)||(y==null?void 0:y.pseudo)||"Joueur 1",p2Name:(h==null?void 0:h.club_name)||(h==null?void 0:h.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:s?b:[],gc_p2:s?[]:b,gcCardsFull_p1:s?u:[],gcCardsFull_p2:s?[]:u,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:d||[],lastActionAt:new Date().toISOString()}}let p=A.game_state&&Object.keys(A.game_state).length?A.game_state:null;if(!p)if(s){p=await r();const{data:f}=await k.from("matches").select("game_state").eq("id",i).single();!(f!=null&&f.game_state)||!Object.keys(f.game_state).length?await k.from("matches").update({game_state:p,turn_user_id:A.home_id}).eq("id",i):p=f.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let f=0;f<60&&!p;f++){await new Promise(y=>setTimeout(y,400));const{data:x}=await k.from("matches").select("game_state").eq("id",i).single();x!=null&&x.game_state&&Object.keys(x.game_state).length&&(p=x.game_state)}if(!p){g("Erreur de synchronisation","error"),c("mini-league");return}p.gc_p2=b,p.gcCardsFull_p2=u,await k.from("matches").update({game_state:p}).eq("id",i)}let I=!1,L=!1,F=!1,H=null,C=!1;const B=new Set,j=new Set;function W(f){var S,q;try{k.removeChannel(de)}catch{}const x=p[o+"Score"]||0,y=p[m+"Score"]||0;if(!F){F=!0;const G=f.winner_id||(x>y?a.profile.id:y>x?"opp":null),R=G===a.profile.id?"win":G?"loss":null;R&&ti(()=>Promise.resolve().then(()=>oo),void 0).then(O=>O.applyOwnEvolution(a.profile.id,R)).catch(()=>{})}if(!L){const G=p.p1Score||0,R=p.p2Score||0,O=(p.usedGc_p1||[]).length,Q=(p.usedGc_p2||[]).length,V=f.winner_id||(G>R?A.home_id:R>G?A.away_id:null);(V?a.profile.id===V:a.profile.id<(s?A.away_id:A.home_id))&&(L=!0,Xi({player1Id:A.home_id,player2Id:A.away_id,score1:G,score2:R,gc1:O,gc2:Q}).catch(ae=>console.warn("[FriendMatch] updateStats:",ae)))}let h,v;f.winner_id?(h=f.winner_id===a.profile.id,v=!1):f.forfeit?(h=x>y,v=!1):(v=x===y,h=x>y),(S=document.getElementById("pvp-end-overlay"))==null||S.remove();const _=document.createElement("div");_.id="pvp-end-overlay",_.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const z=v?"🤝":h?"🏆":"😞",$=v?"MATCH NUL":h?"VICTOIRE !":"DÉFAITE",T=v?"#fff":h?"#FFD700":"#ff6b6b";_.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏆 Retour à la Mini League</button>`,document.body.appendChild(_),(q=_.querySelector("#pvp-end-home"))==null||q.addEventListener("click",()=>{_.remove(),We(e),n?c("mini-league",{openLeagueId:n}):c("mini-league")})}const de=k.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},f=>{const x=f.new;try{if(x.status==="finished"||x.forfeit){if(I||(I=!0,H&&(clearInterval(H),H=null),x.game_state&&(p=x.game_state),p.phase==="finished"&&!x.forfeit&&document.getElementById("pvp-home")))return;W(x);return}if(x.game_state){const y=p;p=x.game_state;const h=p._lastGC;if(h&&h.seq&&!j.has(h.seq)&&(j.add(h.seq),h.by!==o)){Ae(h.type,h.by,()=>ye());return}const v=y[o+"Score"]||0,_=y[m+"Score"]||0,z=p[o+"Score"]||0,$=p[m+"Score"]||0,T=z>v,S=$>_;if((T||S)&&!B.has(p.round)){B.add(p.round);const q=[...p.log||[]].reverse().find(R=>R.isGoal),G=((q==null?void 0:q.homePlayers)||[]).map(R=>({name:R.name,note:R.note,portrait:R.portrait,job:R.job}));_e(G,z,$,T,()=>ye());return}ye()}}catch(y){console.error("[PvP] crash:",y)}}).subscribe();async function re(f){Object.assign(p,f),p.lastActionAt=new Date().toISOString();const{error:x}=await k.from("matches").update({game_state:p}).eq("id",i);x&&g("Erreur de synchronisation","error");try{ye()}catch(y){console.error("[PvP] renderPvpScreen crash:",y)}}async function P(){if(I)return;I=!0,H&&(clearInterval(H),H=null);const f=s?A.away_id:A.home_id,x=s?"p2":"p1",y=s?"p1":"p2",h={...p,[x+"Score"]:3,[y+"Score"]:0,phase:"finished"};try{await k.from("matches").update({status:"finished",forfeit:!0,winner_id:f,home_score:h.p1Score||0,away_score:h.p2Score||0,game_state:h}).eq("id",i)}catch{}try{k.removeChannel(de)}catch{}setTimeout(()=>{We(e),c("mini-league")},800)}const X={BOOST_STAT:({value:f=1,count:x=1,roles:y=[]},h,v)=>{const _=h[o+"Team"],z=Object.entries(_).filter(([$])=>!y.length||y.includes($)).flatMap(([$,T])=>T.filter(S=>!S.used).map(S=>({...S,_line:$})));if(!z.length){h.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),v(h);return}U(z,x,`Choisir ${x} joueur(s) à booster (+${f})`,$=>{$.forEach(T=>{const S=(_[T._line]||[]).find(q=>q.cardId===T.cardId);S&&(S.boost=(S.boost||0)+f,h.log.push({text:`⚡ +${f} sur ${S.name}`,type:"info"}))}),h[o+"Team"]=_,v(h)})},DEBUFF_STAT:({value:f=1,count:x=1,roles:y=[],target:h="ai"},v,_)=>{const z=h==="home"?o:m,$=v[z+"Team"],T=h==="home"?"allié":"adverse",S=Object.entries($).filter(([q])=>!y.length||y.includes(q)).flatMap(([q,G])=>G.map(R=>({...R,_line:q})));if(!S.length){v.log.push({text:`🎯 Aucun joueur ${T}`,type:"info"}),_(v);return}U(S,x,`Choisir ${x} joueur(s) ${T}(s) (-${f})`,q=>{q.forEach(G=>{const R=($[G._line]||[]).find(O=>O.cardId===G.cardId);R&&(R.boost=(R.boost||0)-f,v.log.push({text:`🎯 -${f} sur ${R.name}`,type:"info"}))}),v[z+"Team"]=$,_(v)})},GRAY_PLAYER:({count:f=1,roles:x=[],target:y="ai"},h,v)=>{const _=y==="home"?o:m,z=h[_+"Team"],$=y==="home"?"allié":"adverse",T=Object.entries(z).filter(([S])=>!x.length||x.includes(S)).flatMap(([S,q])=>q.filter(G=>!G.used).map(G=>({...G,_line:S})));if(!T.length){h.log.push({text:`❌ Aucun joueur ${$}`,type:"info"}),v(h);return}U(T,f,`Choisir ${f} joueur(s) ${$}(s) à exclure`,S=>{const q="usedCardIds_"+_,G=new Set(h[q]||[]);S.forEach(R=>{const O=(z[R._line]||[]).find(Q=>Q.cardId===R.cardId);O&&(O.used=!0,G.add(R.cardId),h.log.push({text:`❌ ${O.name} exclu !`,type:"info"}))}),h[q]=[...G],h[_+"Team"]=z,v(h)})},REVIVE_PLAYER:({count:f=1,roles:x=[]},y,h)=>{const v=y[o+"Team"],_=Object.entries(v).filter(([z])=>!x.length||x.includes(z)).flatMap(([z,$])=>$.filter(T=>T.used).map(T=>({...T,_line:z})));if(!_.length){y.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),h(y);return}U(_,f,`Choisir ${f} joueur(s) à ressusciter`,z=>{z.forEach($=>{const T=(v[$._line]||[]).find(S=>S.cardId===$.cardId);T&&(T.used=!1,y.log.push({text:`💫 ${T.name} ressuscité !`,type:"info"}))}),y[o+"Team"]=v,h(y)})},REMOVE_GOAL:({},f,x)=>{const y=m+"Score";f[y]>0?(f[y]--,f.log.push({text:"🚫 Dernier but annulé !",type:"info"})):f.log.push({text:"🚫 Aucun but à annuler",type:"info"}),x(f)},ADD_GOAL_DRAW:({},f,x)=>{f[o+"Score"]===f[m+"Score"]?(f[o+"Score"]++,f.log.push({text:"🎯 But bonus !",type:"info"})):f.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),x(f)},ADD_SUB:({value:f=1},x,y)=>{x.maxSubs=(x.maxSubs||3)+f,x.log.push({text:`🔄 +${f} remplacement(s)`,type:"info"}),y(x)},CUSTOM:({},f,x)=>x(f)};function U(f,x,y,h){const v=document.createElement("div");v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let _=[];function z(){var T,S;const $=f.map(q=>{const G={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[q._line]||"#555",R=je(q,q._line)+(q.boost||0),Q=_.find(ie=>ie.cardId===q.cardId)?"#FFD700":"rgba(255,255,255,0.25)",V=q.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${q.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${Q};background:${G};overflow:hidden;cursor:pointer;${V}">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ft(f,p[o+"Formation"],Ne,S,300,300,$e)}
      </div>
    ...
    </div>`,o==="p1"&&setTimeout(async()=>{await re({phase:"midfield"})},5e3)}let we=!1;function ve(){if(we)return;const f=p[o+"Team"].MIL||[],x=p[m+"Team"].MIL||[];function y(ie){return ie.reduce((ae,fe)=>ae+je(fe,"MIL"),0)}function h(ie){let ae=0;for(let fe=0;fe<ie.length-1;fe++){const me=at(ie[fe],ie[fe+1]);me==="#00ff88"?ae+=2:me==="#FFD700"&&(ae+=1)}return ae}const v=y(f)+h(f),_=y(x)+h(x),z=v>=_;function $(ie,ae,fe){return`<div id="duel-row-${fe}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(_);let T=!1;const S=()=>{T||(T=!0,_.remove(),setTimeout(()=>v(),50))};_.addEventListener("click",S),setTimeout(S,3500)}function Ae(f,x,y){var V,ie;const h=(p.gcDefs||[]).find(ae=>{var fe;return ae.name===f||((fe=ae.name)==null?void 0:fe.toLowerCase().trim())===(f==null?void 0:f.toLowerCase().trim())}),v={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[h==null?void 0:h.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",_={purple:"#b06ce0",light_blue:"#00d4ef"}[h==null?void 0:h.color]||"#b06ce0",z=(h==null?void 0:h.name)||f,$=(h==null?void 0:h.effect)||((V=Ve[f])==null?void 0:V.desc)||"",T=h!=null&&h.image_url?`/icons/${h.image_url}`:null,S=((ie=Ve[f])==null?void 0:ie.icon)||"⚡",G=x===o?"Vous":p[x+"Name"]||"Adversaire",R=document.createElement("div");R.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",R.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
      ...
      </div>`,document.body.appendChild(x),(h=x.querySelector("#pvp-hist-close"))==null||h.addEventListener("click",()=>x.remove())}async function Z(){if(p.phase!==o+"-attack")return;const f=o==="p1"?"p2":"p1",x=(p.round||0)+1,y=[...p.log||[]];y.push({type:"info",text:`⏭️ ${p[o+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const h=ze(p),v=be(f),_=h||!v?"finished":f+"-attack";await re({["selected_"+o]:[],modifiers:{...p.modifiers,[o]:{}},pendingAttack:null,phase:_,attacker:f,round:x,log:y}),_==="finished"&&await D(p)}async function K(){const f=p[o+"Team"],x=!["GK","DEF","MIL","ATT"].some(_=>(p[m+"Team"][_]||[]).some(z=>!z.used)),y=(p["selected_"+o]||[]).map(_=>{const z=(f[_._role]||[]).find(R=>R.cardId===_.cardId)||_,$=x&&["GK","DEF"].includes(_._role),T=f[_._role]||[],S=T.findIndex(R=>R.cardId===_.cardId),q=lt(T.length),G=S>=0?q[S]:z._col??1;return{...z,_line:_._role,_col:G,...$?{note_a:Math.max(1,Number(z.note_a)||0)}:{}}});if(!y.length)return;const h=Nt(y,p.modifiers[o]||{});ne(o,y.map(_=>_.cardId)),y.forEach(_=>{const z=(f[_._role]||[]).find($=>$.cardId===_.cardId);z&&(z.used=!0)}),p["selected_"+o]=[],ye();const v=[...p.log||[]];if(x){const _=(p[o+"Score"]||0)+1,z=y.map(G=>({name:G.name,note:je(G,G._line||"ATT"),portrait:Pe(G),job:G.job}));v.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:z,homeTotal:h.total,aiTotal:0});const $=(p.round||0)+1,T=o==="p1"?"p2":"p1",S={...p,[o+"Team"]:f,[o+"Score"]:_},q=ze(S);B.add($),_e(z,_,p[m+"Score"]||0,!0,async()=>{await re({[o+"Team"]:f,[o+"Score"]:_,["selected_"+o]:[],modifiers:{...p.modifiers,[o]:{}},pendingAttack:null,phase:q?"finished":T+"-attack",attacker:T,round:$,log:v}),q&&await D({...p,[o+"Score"]:_})});return}v.push({type:"pending",text:`⚔️ ${p[o+"Name"]} attaque (${h.total})`}),await re({[o+"Team"]:f,[m+"Team"]:p[m+"Team"],pendingAttack:{...h,players:y,side:o},["selected_"+o]:[],modifiers:{...p.modifiers,[o]:{}},phase:m+"-defense",log:v})}async function se(){const f=p[o+"Team"],x=(p["selected_"+o]||[]).map(V=>{const ie=(f[V._role]||[]).find(ke=>ke.cardId===V.cardId)||V,ae=f[V._role]||[],fe=ae.findIndex(ke=>ke.cardId===V.cardId),me=lt(ae.length),$e=fe>=0?me[fe]:ie._col??1;return{...ie,_line:V._role,_col:$e}}),y=Rt(x,p.modifiers[o]||{});ne(o,x.map(V=>V.cardId)),x.forEach(V=>{const ie=(f[V._role]||[]).find(ae=>ae.cardId===V.cardId);ie&&(ie.used=!0)}),p["selected_"+o]=[],ye();const h=Ot(p.pendingAttack.total,y.total,p.modifiers[o]||{}),v=p.pendingAttack.side,_=h==="attack"||(h==null?void 0:h.goal),z=v==="p1"?"p2":"p1",$=(p.round||0)+1,T=(p.pendingAttack.players||[]).map(V=>({name:V.name,note:je(V,V._line||"ATT"),portrait:Pe(V),job:V.job})),S=[...p.log||[]];S.push({type:"duel",isGoal:_,homeScored:_&&v===o,text:_?`⚽ BUT de ${p[v+"Name"]} ! (${p.pendingAttack.total} vs ${y.total})`:`✋ Attaque stoppée (${p.pendingAttack.total} vs ${y.total})`,homePlayers:T,aiPlayers:x.map(V=>({name:V.name,note:je(V,V._line||"DEF"),portrait:Pe(V),job:V.job})),homeTotal:p.pendingAttack.total,aiTotal:y.total});const q=_?(p[v+"Score"]||0)+1:p[v+"Score"]||0,G={...p,[o+"Team"]:f,[v+"Score"]:q},R=ze(G),O=R?"finished":z+"-attack",Q=async()=>{await re({[o+"Team"]:f,[m+"Team"]:p[m+"Team"],[v+"Score"]:q,["selected_"+o]:[],modifiers:{...p.modifiers,[o]:{}},pendingAttack:null,phase:O,attacker:z,round:$,log:S}),(O==="finished"||R)&&await D({...p,[v+"Score"]:q})};if(_){const V=v===o,ie=V?q:p[o+"Score"]||0,ae=V?p[m+"Score"]||0:q;B.add($),_e(T,ie,ae,V,Q)}else await Q()}function he(f){return["MIL","ATT"].some(x=>(f[x]||[]).some(y=>!y.used))}function Le(f){return["GK","DEF","MIL","ATT"].some(x=>(f[x]||[]).some(y=>!y.used))}function Me(f){return Le(f)&&!he(f)}function be(f){const x=p[f+"Team"],y=p[(f==="p1"?"p2":"p1")+"Team"];return!!(he(x)||!Le(y)&&Me(x))}function ze(f){const x=["MIL","ATT"].some(S=>(f.p1Team[S]||[]).some(q=>!q.used)),y=["MIL","ATT"].some(S=>(f.p2Team[S]||[]).some(q=>!q.used)),h=Le(f.p1Team),v=Le(f.p2Team);return!x&&!(!v&&h)&&(!y&&!(!h&&v))}function M(f){const x=f.p1Score||0,y=f.p2Score||0;return x===y?null:x>y?A.home_id:A.away_id}async function D(f){try{const x=M(f),y=x?A.home_id===x?A.away_id:A.home_id:null,h=f.p1Score||0,v=f.p2Score||0;await k.from("matches").update({status:"finished",winner_id:x,home_score:h,away_score:v}).eq("id",i);const{data:_}=await k.from("mini_league_matches").select("id, league_id, matchday").eq("match_id",i).single();if(_){await k.from("mini_league_matches").update({home_score:h,away_score:v,status:"finished"}).eq("id",_.id);const{data:z}=await k.from("mini_league_matches").select("id, status").eq("league_id",_.league_id).eq("matchday",_.matchday);if((z||[]).every(T=>T.status==="finished"||T.status==="bye")){const{data:T}=await k.from("mini_leagues").select("current_day,total_days").eq("id",_.league_id).single();if(T){const S=(T.current_day||0)+1,q=S>(T.total_days||0);await k.from("mini_leagues").update({current_day:q?T.total_days:S,status:q?"finished":"active"}).eq("id",_.league_id)}}}x&&y&&ki(x,y).catch(()=>{})}catch(x){console.error("[ML] finishMatch:",x)}}function w(){var v;if(I&&document.getElementById("pvp-end-overlay"))return;I=!0;const f=p[o+"Score"],x=p[m+"Score"],y=f>x,h=f===x;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function sr(e,t,i){let s=0;const l=document.createElement("div");l.id="tutorial-overlay",l.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const d=()=>{var o,m,b;const c=t[s],g=s===t.length-1,n=Math.round((s+1)/t.length*100);l.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let g="buy";const n=()=>{var r,p,I,L,F,H,C;return{name:(((r=document.getElementById("flt-name"))==null?void 0:r.value)||"").toLowerCase().trim(),club:(((p=document.getElementById("flt-club"))==null?void 0:p.value)||"").toLowerCase().trim(),country:(((I=document.getElementById("flt-country"))==null?void 0:I.value)||"").toLowerCase().trim(),job:((L=document.getElementById("flt-job"))==null?void 0:L.value)||"",rarity:((F=document.getElementById("flt-rarity"))==null?void 0:F.value)||"",note1:parseInt((H=document.getElementById("flt-note1"))==null?void 0:H.value)||0,note2:parseInt((C=document.getElementById("flt-note2"))==null?void 0:C.value)||0}};function o(r){const p=n();return r.filter(I=>{var de,re,P;const L=(de=I.card)==null?void 0:de.player;if(!L)return!1;const F=`${L.firstname} ${L.surname_encoded}`.toLowerCase(),H=(((re=L.clubs)==null?void 0:re.encoded_name)||"").toLowerCase(),C=(L.country_code||"").toLowerCase(),B=((P=I.card)==null?void 0:P.evolution_bonus)||0,j=Tt(L,L.job,B),W=L.job2?Tt(L,L.job2,B):0;return!(p.name&&!F.includes(p.name)||p.club&&!H.includes(p.club)||p.country&&!C.includes(p.country)||p.job&&L.job!==p.job||p.rarity&&L.rarity!==p.rarity||p.note1&&j<p.note1||p.note2&&W<p.note2)})}function m(r){var C,B,j,W;const p=(C=r.card)==null?void 0:C.player;if(!p)return"";const I=((B=r.card)==null?void 0:B.evolution_bonus)||0,L=Tt(p,p.job,I),F=p.job2?Tt(p,p.job2,I):0,H=(i.profile.credits||0)>=r.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:8px;padding:10px 12px;overflow:hidden;padding-left:6px">
      ${Hi(p.rarity)}
      ${Wt(L,p.job)}
      ${Wt(F,p.job2||null)}
      ${Ui(p.country_code)}
    ...
    </div>`}function b(r){var C,B,j,W;const p=(C=r.card)==null?void 0:C.player;if(!p)return"";const I=((B=r.card)==null?void 0:B.evolution_bonus)||0,L=Tt(p,p.job,I),F=p.job2?Tt(p,p.job2,I):0,H=r.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:8px;padding:10px 12px;overflow:hidden;padding-left:6px;${H?"opacity:0.7":""}">
      ${Hi(p.rarity)}
      ${Wt(L,p.job)}
      ${Wt(F,p.job2||null)}
      ${Ui(p.country_code)}
    ...
    </div>`}function u(){const r=document.getElementById("mkt-content"),p=document.getElementById("mkt-filters");if(r){if(p.style.display=g==="buy"?"flex":"none",g==="buy"){const I=o(a);r.innerHTML=I.length?I.map(m).join(""):'<div style="text-align:center;color:#aaa;padding:40px">Aucune carte trouvée.</div>'}else{const I=c.filter(F=>F.status==="active").sort((F,H)=>new Date(H.listed_at)-new Date(F.listed_at)),L=c.filter(F=>F.status==="sold").sort((F,H)=>new Date(H.sold_at||H.listed_at)-new Date(F.sold_at||F.listed_at));r.innerHTML=(I.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:4px 0 6px;text-transform:uppercase;letter-spacing:1px">🟢 En vente (${I.length})</div>`+I.map(b).join(""):"")+(L.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:12px 0 6px;text-transform:uppercase;letter-spacing:1px">✅ Ventes réussies (${L.length})</div>`+L.map(b).join(""):"")+(!I.length&&!L.length?'<div style="text-align:center;color:#aaa;padding:40px">Aucune vente pour le moment.</div>':"")}r.querySelectorAll("[data-buy]").forEach(I=>I.addEventListener("click",()=>pr(I.dataset.buy,a,e,t))),r.querySelectorAll("[data-cancel]").forEach(I=>I.addEventListener("click",()=>fr(I.dataset.cancel,e,t)))}}e.querySelectorAll(".mkt-tab").forEach(r=>{r.addEventListener("click",()=>{g=r.dataset.tab,e.querySelectorAll(".mkt-tab").forEach(p=>{const I=p===r;p.style.background=I?"var(--green)":"#fff",p.style.color=I?"#fff":"var(--gray-600)",p.style.borderColor=I?"var(--green)":"var(--gray-200)"}),u()})});let A;["flt-name","flt-club","flt-country","flt-job","flt-rarity","flt-note1","flt-note2"].forEach(r=>{var p;(p=document.getElementById(r))==null||p.addEventListener("input",()=>{clearTimeout(A),A=setTimeout(u,250)})}),u()}async function pr(e,t,i,s){const{state:l,toast:d,refreshProfile:a}=s,c=t.find(o=>o.id===e);if(!c)return;const g=c.price;if((l.profile.credits||0)<g){d("Crédits insuffisants","error");return}ur(c,async()=>{const{error:o}=await k.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:l.profile.id});if(o){d("Erreur achat : "+o.message,"error");return}await a();const m=document.getElementById("nav-credits");m&&(m.textContent=`💰 ${(l.profile.credits||0).toLocaleString("fr")}`),d("✅ Carte achetée !","success"),await Ai(i,s)})}function ur(e,t){var a;const i=(a=e.card)==null?void 0:a.player,s=i?`${i.firstname} ${i.surname_encoded}`:"cette carte",l=document.createElement("div");l.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",l.innerHTML=`
  ...
  `}async function mr(e,{state:t,navigate:i,toast:s}){const l=t.profile;if(!l)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await k.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
      ...
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${l.id},away_id.eq.${l.id}`).order("created_at",{ascending:!1}).limit(50),a={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},c=(d||[]).filter(o=>o.status==="finished"),g=(d||[]).filter(o=>o.status==="in_progress");function n(o){const m=o.home_id===l.id;m?o.home_score:o.away_score,m?o.away_score:o.home_score;const b=o.winner_id===l.id,u=o.home_score===o.away_score&&o.status==="finished",A=o.status!=="finished"?"…":u?"N":b?"V":"D",r=o.status!=="finished"||u?"#888":b?"#1A6B3C":"#c0392b";let p=a[o.mode]||o.mode;o.away_id===null&&!p.startsWith("IA")&&(p="IA");const L=o.home_id===l.id?o.away:o.home;let F;o.away_id===null?F=p:L?F=`${L.club_name||L.pseudo} (${L.pseudo})`:F="Adversaire";let H="";o.status==="in_progress"&&Date.now()-new Date(o.created_at).getTime()>3600*1e3&&(H=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const C=new Date(o.created_at),B=C.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+C.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),j=o.status==="finished"?`${o.home_score} - ${o.away_score}`:`${o.home_score||0} - ${o.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
        <div style="font-size:13px;font-weight:600">${F}${H}</div>
        <div style="font-size:11px;color:var(--gray-600)">${p} · ${B}${o.status==="in_progress"?" · en cours":""}</div>
      </div>
      <div style="display:flex;align-items:center;gap:8px">
