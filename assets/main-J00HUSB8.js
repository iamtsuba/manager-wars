import{s as $,F as ai,h as Mt,j as Bi,l as Ze,i as un,k as fn,b as gn}from"./formation-links-CEzN0rcz.js";function hi(e,{navigate:t,toast:n}){e.innerHTML=`
  ...
  `;function l(){var z;const i=document.getElementById("logo-preview"),r=document.getElementById("logo-initials"),f=((z=document.getElementById("setup-club"))==null?void 0:z.value)||"MW",m=f.trim().split(" ").filter(Boolean),y=m.length>=2?(m[0][0]+m[1][0]).toUpperCase():f.slice(0,2).toUpperCase();i&&(i.style.background=s,i.style.borderColor=d),r&&(r.textContent=y,r.style.color=d)}document.getElementById("color1").addEventListener("input",i=>{d=i.target.value,document.getElementById("swatch1").style.background=d,l()}),document.getElementById("color2").addEventListener("input",i=>{s=i.target.value,document.getElementById("swatch2").style.background=s,l()});function p(i){document.querySelectorAll(".setup-step").forEach(r=>r.classList.remove("active")),document.getElementById(`step-${i}`).classList.add("active"),document.getElementById("step-num").textContent=i,document.getElementById("progress-fill").style.width=`${Math.round(i/3*100)}%`,i===3&&l()}document.getElementById("step1-next").addEventListener("click",async()=>{const i=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("step1-error");if(r.textContent="",i.length<3){r.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await $.from("users").select("id").eq("pseudo",i).maybeSingle();if(f){r.textContent="Ce pseudo est déjà pris.";return}p(2)}),document.getElementById("step2-back").addEventListener("click",()=>p(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const i=document.getElementById("setup-club").value.trim(),r=document.getElementById("step2-error");if(r.textContent="",i.length<2){r.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await $.from("users").select("id").eq("club_name",i).maybeSingle();if(f){r.textContent="Ce nom de club est déjà pris.";return}p(3)}),document.getElementById("step3-back").addEventListener("click",()=>p(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const i=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),m=document.getElementById("step3-finish");f.textContent="",m.disabled=!0,m.textContent="Création en cours…";try{const{error:y}=await $.from("users").insert({id:t.user.id,pseudo:i,club_name:r,club_color1:d,club_color2:s,credits:1e4});if(y)throw y;await xn(t.user.id),await c(),a(`Bienvenue ${i} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(y){f.textContent=y.message,m.disabled=!1,m.textContent="🚀 Créer mon profil !"}})}async function xn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await $.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(n){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",n)}}const yn="modulepreload",hn=function(e){return"/manager-wars/"+e},bi={},bn=function(t,n,a){let c=Promise.resolve();if(n&&n.length>0){document.getElementsByTagName("link");const s=document.querySelector("meta[property=csp-nonce]"),l=(s==null?void 0:s.nonce)||(s==null?void 0:s.getAttribute("nonce"));c=Promise.allSettled(n.map(p=>{if(p=hn(p),p in bi)return;bi[p]=!0;const i=p.endsWith(".css"),r=i?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${p}"]${r}`))return;const f=document.createElement("link");if(f.rel=i?"stylesheet":yn,i||(f.as="script"),f.crossOrigin="",f.href=p,l&&f.setAttribute("nonce",l),document.head.appendChild(f),i)return new Promise((m,y)=>{f.addEventListener("load",m),f.addEventListener("error",()=>y(new Error(`Unable to preload CSS for ${p}`)))})}))}function d(s){const l=new Event("vite:preloadError",{cancelable:!0});if(l.payload=s,window.dispatchEvent(l),!l.defaultPrevented)throw s}return c.then(s=>{for(const l of s||[])l.status==="rejected"&&d(l.reason);return t().catch(d)})},It="#1A6B3C",Lt="#cc2222",vn="#D4A017",vi="#888";async function wn(e,t){const{state:n,toast:a}=t;e.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${m.map(({friendshipId:y,friend:z})=>_n(z,y)).join("")}
    </div>`,i.querySelectorAll("[data-stats]").forEach(y=>{y.addEventListener("click",()=>kn(e,y.dataset.stats,y.dataset.friendName))}),i.querySelectorAll("[data-match]").forEach(y=>{y.addEventListener("click",()=>{const z=y.dataset.friendId,o=y.dataset.friendName;console.log("[Friends] clic match",{fid:z,fname:o,hasNavigate:typeof a}),typeof a=="function"?a("match",{matchMode:"friend",friendId:z,friendName:o}):t("Erreur navigation","error")})})}function _n(e,t){const n=e.club_name||e.pseudo||"?",a=e.pseudo||"",c=(a||n).slice(0,2).toUpperCase(),d=e.club_color2||It,s=e.club_color1||"#ffffff",l=e.last_seen_at?new Date(e.last_seen_at):null,p=l&&Date.now()-l.getTime()<3*60*1e3;return`
    ...
    </div>`}function $n(e,t){const n=si();n.innerHTML=`
    <div class="popup-box">
    ...
    ${di()}`,document.body.appendChild(i);const f=()=>i.remove();i.querySelector("#pending-close").addEventListener("click",f),i.addEventListener("click",m=>{m.target===i&&f()}),i.querySelectorAll("[data-accept]").forEach(m=>{m.addEventListener("click",async()=>{const{error:y}=await $.from("friendships").update({status:"accepted"}).eq("id",m.dataset.accept);if(y){t("Erreur : "+y.message,"error");return}m.closest("div[style]").remove(),t("✅ Ami accepté !","success"),Fi(e,t,a),n&&n()})}),i.querySelectorAll("[data-decline]").forEach(m=>{m.addEventListener("click",async()=>{await $.from("friendships").delete().eq("id",m.dataset.decline),m.closest("div[style]").remove(),t("Demande refusée","info"),n&&n()})})}async function kn(e,t,n){const a=e.user.id,[c,d]=[a,t].sort(),s=a===c,{data:l}=await $.from("friend_match_stats").select("*").eq("player1_id",c).eq("player2_id",d).single(),p=e.profile.club_name||e.profile.pseudo||"Moi",i=l||{},r=s?i.wins_p1||0:i.wins_p2||0,f=s?i.wins_p2||0:i.wins_p1||0,m=i.draws||0,y=s?i.goals_p1||0:i.goals_p2||0,z=s?i.goals_p2||0:i.goals_p1||0,o=s?i.gc_used_p1||0:i.gc_used_p2||0,v=s?i.gc_used_p2||0:i.gc_used_p1||0,L=i.matches_total||0,T=(R,F,j,N=It,ee=Lt)=>`
    <div style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:8px;padding:10px 0;border-bottom:1px solid #f0f0f0">
      <div style="text-align:right;font-size:18px;font-weight:900;color:${N}">${F}</div>
  ...
  `,e.querySelectorAll("[data-nav]").forEach(l=>{l.addEventListener("click",p=>{p.preventDefault(),n(l.dataset.nav)})}),(d=document.getElementById("nav-rankings"))==null||d.addEventListener("click",()=>n("rankings")),(s=document.getElementById("nav-matches"))==null||s.addEventListener("click",()=>n("matches")),e.querySelectorAll("[data-action]").forEach(l=>{l.addEventListener("click",()=>{l.classList.add("tapped"),setTimeout(()=>l.classList.remove("tapped"),200);const p=l.dataset.action;if(p==="match-ai"){zn(n);return}if(p==="match-random"){n("match",{matchMode:"random"});return}if(p==="match-friend"){n("friends");return}a("Bientôt disponible","info")})}),document.getElementById("logout-btn").addEventListener("click",async()=>{await $.auth.signOut(),window.location.reload()}),Gi(t,a),Ln(t,a,n),qi(t,a,n))}async function qi(e,t,n){const a=document.getElementById("ongoing-match-banner");if(!a)return;const c=e.profile.id,{data:d}=await $.from("matches").select("id, home_id, away_id, status, mode").eq("status","active").or(`home_id.eq.${c},away_id.eq.${c}`).order("created_at",{ascending:!1});if(!(d!=null&&d.length)){a.innerHTML="";return}const s=d.map(p=>p.home_id===c?p.away_id:p.home_id).filter(Boolean);let l={};if(s.length){const{data:p}=await $.from("users").select("id, pseudo, club_name").in("id",s);(p||[]).forEach(i=>{l[i.id]=i.club_name||i.pseudo})}a.innerHTML=d.map(p=>{const i=p.home_id===c?p.away_id:p.home_id,r=l[i]||"Adversaire";return`
      ...
      </div>`}).join(""),a.querySelectorAll("[data-resume]").forEach(p=>{p.addEventListener("click",async()=>{const{resumePvpMatch:i}=await bn(async()=>{const{resumePvpMatch:f}=await Promise.resolve().then(()=>zo);return{resumePvpMatch:f}},void 0),r=document.getElementById("page-content")||document.getElementById("app");i(r,{state:e,navigate:n,toast:t,openModal:null,closeModal:null,refreshProfile:null},p.dataset.resume)})}),a.querySelectorAll("[data-abandon]").forEach(p=>{p.addEventListener("click",()=>{In(async()=>{await Tn(p.dataset.abandon,p.dataset.opp,c),t("Match abandonné (défaite 3-0)","info"),qi(e,t,n)})})})}async function Tn(e,t,n){const{data:a}=await $.from("matches").select("home_id, away_id, game_state").eq("id",e).single();if(!a)return;const c=a.home_id===n,d=c?0:3,s=c?3:0,l=a.game_state||{};l.p1Score=d,l.p2Score=s,l.phase="finished",l.forfeit=!0,await $.from("matches").update({status:"finished",forfeit:!0,winner_id:t,home_score:d,away_score:s,game_state:l}).eq("id",e)}function In(e){const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",t.innerHTML=`
    ...
    </div>`,document.getElementById("friend-req-btn").addEventListener("click",()=>{Di(e,t,()=>Gi(e,t))})}function zn(e){const t=[{mode:"vs_ai_easy",label:"Facile",sub:"Gain 500 cr.",icon:"🟢"},{mode:"vs_ai_medium",label:"Moyen",sub:"Gain 1 000 cr.",icon:"🟡"},{mode:"vs_ai_hard",label:"Difficile",sub:"Gain 1 500 cr.",icon:"🟠"},{mode:"vs_ai_club",label:"Club",sub:"Gain 2 500 cr.",icon:"🔴"}],n=document.createElement("div");n.className="modal-overlay",n.style.zIndex="2000",n.innerHTML=`
  ...
  `,document.body.appendChild(n);const a=()=>n.remove();document.getElementById("diff-cancel").addEventListener("click",a),n.addEventListener("click",c=>{c.target===n&&a()}),n.querySelectorAll("[data-mode]").forEach(c=>{c.addEventListener("click",()=>{a(),e("match",{matchMode:c.dataset.mode})})})}const Ne={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé pour ce match."},"Double attaque":{icon:"⚡",desc:"La note d'attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la note d'un joueur adverse."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function we(e,t){if(!e)return 0;switch(t){case"GK":return Number(e.note_g)||0;case"DEF":return Number(e.note_d)||0;case"MIL":return Number(e.note_m)||0;case"ATT":return Number(e.note_a)||0;default:return 0}}const _i=["ATT","MIL","DEF","GK"];function Ni(e){let t=0;const n=e.length;for(let a=0;a<n;a++)for(let c=a+1;c<n;c++){const d=e[a],s=e[c];if(!d||!s)continue;const l=d._col!=null&&s._col!=null&&d._col===s._col,p=d._col!=null&&s._col!=null&&Math.abs(d._col-s._col)===1,i=_i.indexOf(d._line||d.job),r=_i.indexOf(s._line||s.job),f=Math.abs(i-r)===1;if(!((d._line||d.job)===(s._line||s.job)&&p||l&&f))continue;const z=d.country_code&&s.country_code&&d.country_code===s.country_code,o=d.club_id&&s.club_id&&d.club_id===s.club_id;z&&o?t+=2:(z||o)&&(t+=1)}return t}function Ut(e,t={}){const n=e.reduce((d,s)=>{const l=s._line||s.job;return d+(Number(l==="MIL"?s.note_m:s.note_a)||0)+(s.boost||0)},0),a=Ni(e);let c=n+a;return t.doubleAttack&&(c*=2),t.stolenNote&&(c-=t.stolenNote),{base:n,links:a,total:Math.max(0,c)}}function Kt(e,t={}){const n=e.reduce((d,s)=>{const l=s._line||s.job;let p=0;return l==="GK"?p=Number(s.note_g)||0:l==="MIL"?p=Number(s.note_m)||0:p=Number(s.note_d)||0,d+p+(s.boost||0)},0),a=Ni(e);let c=n+a;return t.stolenNote&&(c-=t.stolenNote),{base:n,links:a,total:Math.max(0,c)}}function Vt(e,t,n={}){return n.shield?{goal:!1,shielded:!0}:{goal:e>t,shielded:!1}}function Pi(e,t,n="easy"){const a=e.filter(s=>!s.used);if(!a.length)return[];const c=[...a].sort((s,l)=>{const p=t==="attack"?we(s,"ATT"):s._line==="GK"?we(s,"GK"):we(s,"DEF");return(t==="attack"?we(l,"ATT"):l._line==="GK"?we(l,"GK"):we(l,"DEF"))-p});let d=n==="easy"?1+Math.floor(Math.random()*2):n==="medium"?2+Math.floor(Math.random()*2):3;return c.slice(0,Math.min(d,c.length,3))}function Mn(e,t){const n={vs_ai_easy:{victoire:500,nul:250,defaite:50},vs_ai_medium:{victoire:1e3,nul:500,defaite:50},vs_ai_hard:{victoire:1500,nul:750,defaite:100},vs_ai_club:{victoire:2500,nul:1250,defaite:100}};return(n[e]||n.vs_ai_easy)[t]||0}const Sn={pepite:{win:2,loss:-1,boosterStart:"min"},papyte:{win:1,loss:-2,boosterStart:"max"}};function Ri(e,t){const n=e.player;if(!n)return 0;const a=n.rarity;if(a!=="pepite"&&a!=="papyte")return Number(n[t])||0;const c=Oi(n),d=Number(n[t])||0;if(d<=0)return 0;const s=n.note_min??1,l=n.note_max??10,i=(e.current_note??c)-c;return Math.min(l,Math.max(s,d+i))}function Oi(e){const t=e.job||"ATT";return Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}async function Hi(e,t){!e||!t||await Promise.all([$i(e,"win"),$i(t,"loss")])}async function $i(e,t){const{data:n}=await $.from("cards").select("id, current_note, player:players(rarity, job, note_g, note_d, note_m, note_a, note_min, note_max)").eq("owner_id",e).eq("card_type","player");if(!(n!=null&&n.length))return;const a=n.filter(c=>{var d,s;return((d=c.player)==null?void 0:d.rarity)==="pepite"||((s=c.player)==null?void 0:s.rarity)==="papyte"});a.length&&await Promise.all(a.map(c=>{const d=Sn[c.player.rarity],s=t==="win"?d.win:d.loss,l=c.player.note_min??1,p=c.player.note_max??10,i=Oi(c.player),r=c.current_note??i,f=Math.min(p,Math.max(l,r+s));return $.from("cards").update({current_note:f}).eq("id",c.id)}))}const Ui={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},At={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},Bt=["GK","DEF","MIL","ATT"],Cn=["Tous","GK","DEF","MIL","ATT"],jn={normal:1e3,pepite:5e3,papyte:5e3,legende:1e4};function ki(e){const t=e.player;if(!t)return null;const n=t.rarity;return(n==="pepite"||n==="papyte")&&e.current_note!=null?e.current_note:Math.max(Number(t.note_g)||0,Number(t.note_d)||0,Number(t.note_m)||0,Number(t.note_a)||0)}function Bn(e){return e.length?e.reduce((t,n)=>ki(n)>ki(t)?n:t,e[0]):e[0]}function Ki(e){return e==="GK"?"note_g":e==="DEF"?"note_d":e==="MIL"?"note_m":"note_a"}const li={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function Vi(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";if(!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const n=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${n}.png`}function zt(e,t){return e&&Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}function Xt(e,t=""){var m,y;const n=e.player;if(!n)return"";const a=n.job||"ATT",c=At[a],d=Ui[n.rarity]||"#ccc",s=n.rarity==="pepite"||n.rarity==="papyte",l=s&&e.current_note!=null?e.current_note:zt(n,a),p=n.job2?s?Ri(e,Ki(n.job2)):zt(n,n.job2):null,i=n.job2?At[n.job2]:null,r=Vi(n),f=li[n.country_code]||n.country_code||"";return`
  <div style="
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
  ...
  </div>`}async function Fn(e,t){const{state:n,navigate:a,toast:c,openModal:d,closeModal:s}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await $.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),i=(l||[]).filter(O=>O.card_type==="player"&&O.player),r=(l||[]).filter(O=>O.card_type==="game_changer"),f=(l||[]).filter(O=>O.card_type==="formation"),{data:m}=await $.from("gc_definitions").select("name,gc_type,color,effect,image_url"),y={};(m||[]).forEach(O=>{y[O.name]=O});const z=Object.keys(ai),o=Object.keys(Ne),v={};i.forEach(O=>{const U=O.player.id;v[U]=(v[U]||0)+1}),new Set(Object.keys(v).map(O=>String(O)));const L=new Set(f.map(O=>O.formation)),T=new Set(r.map(O=>O.gc_type));let C="player",R="Tous",F="",j=!1;function N(){return[...i].sort((O,U)=>{const S=Bt.indexOf(O.player.job),D=Bt.indexOf(U.player.job);return S!==D?S-D:(O.player.surname_encoded||"").localeCompare(U.player.surname_encoded||"")})}function ee(){return[...p||[]].sort((O,U)=>{const S=Bt.indexOf(O.job),D=Bt.indexOf(U.job);return S!==D?S-D:(O.surname_encoded||"").localeCompare(U.surname_encoded||"")})}function le(){return N().filter(O=>{const U=O.player,S=R==="Tous"||U.job===R,D=!F||`${U.firstname} ${U.surname_encoded}`.toLowerCase().includes(F);return S&&D})}function re(){return ee().filter(O=>{const U=R==="Tous"||O.job===R,S=!F||`${O.firstname} ${O.surname_encoded}`.toLowerCase().includes(F);return U&&S})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function M(){const O=document.getElementById("col-filters");O&&(C==="player"?(O.innerHTML=`
          ...
          ${Cn.map(U=>`
            <button class="filter-btn" data-job="${U}"
              style="flex-shrink:0;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;cursor:pointer;
                border:1.5px solid ${U===R?"var(--green)":"var(--gray-200)"};
                background:${U===R?"var(--green)":"#fff"};
                color:${U===R?"#fff":"var(--gray-600)"}">
              ${U}
            </button>`).join("")}
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",U=>{F=U.target.value.toLowerCase(),Y()}),e.querySelectorAll(".filter-btn").forEach(U=>{U.addEventListener("click",()=>{R=U.dataset.job,M(),Y()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{j=!j,M(),Y()})):(O.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{j=!j,M(),Y()})))}function Y(){const O=document.getElementById("col-grid");O&&(C==="player"?he(O):C==="formation"?xe(O):ge(O))}function ie(O,U,S,D,K){const ne=document.getElementById("col-grid"),te=document.getElementById("col-big");if(!ne||!te)return;var _e=0;function $e(){const Ee=window.innerWidth>=768,Me=document.getElementById("col-big"),Ae=document.getElementById("col-grid");Ee&&Me&&(Me.style.minHeight="420px",Me.style.flex="1 1 auto"),Ee&&Ae&&(Ae.style.height=Math.round(310*.76+16)+"px",Ae.style.flexShrink="0",Ae.style.overflowX="auto",Ae.style.overflowY="hidden",Ae.style.alignItems="center",Ae.style.padding="8px 16px"),te.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+U(O[_e])+"</div>";var Te=te.querySelector("[data-card-id],[data-form-id],[data-gc-id]");Te&&Te.addEventListener("click",function(){D(O[_e])}),requestAnimationFrame(function(){var ke=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!ke||!te)){var qe=te.clientHeight-8,Re=te.clientWidth-24,k=ke.offsetHeight,u=ke.offsetWidth;if(k>0&&u>0&&qe>40){var g=Ee?2.2:1,h=Math.min(qe/k,Re/u,g);ke.style.transform="scale("+h.toFixed(3)+")",ke.style.transformOrigin="top center"}}}),ne.innerHTML=O.map(function(ke,qe){var Re=qe===_e,k="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(Re?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+qe+'" style="'+k+'">'+S(ke,Re)+"</div>"}).join(""),ne.querySelectorAll(".col-mini-item").forEach(function(ke){ke.addEventListener("click",function(){_e=Number(ke.dataset.idx),$e(),ke.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}$e()}function q(O){var U=window.innerWidth>=768?.76:.54,S;if(!O||O._fake){var D=O?O.player:null;if(!D)return"";S=Ei(D)}else S=Xt(O,"");return'<div style="display:inline-block;zoom:'+U+';pointer-events:none;line-height:0">'+S+"</div>"}function Q(O,U,S){U=U||100,S=S||140;var D=Mt[O]||{},K={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},ne=Math.max(3,Math.round(U*.06)),te=Object.entries(D).map(function($e){var Ee=$e[0],Me=$e[1],Ae=Ee.replace(/\d+$/,""),Te=K[Ae]||"#888",ke=Math.round(Me.x*U),qe=Math.round(Me.y*S);return'<circle cx="'+ke+'" cy="'+qe+'" r="'+ne+'" fill="'+Te+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),_e=Math.max(1,Math.round(U/50));return'<svg viewBox="0 0 '+U+" "+S+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+U+'" height="'+S+'" fill="#1A6B3C"/><rect x="'+Math.round(U*.2)+'" y="'+Math.round(S*.02)+'" width="'+Math.round(U*.6)+'" height="'+Math.round(S*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+_e+'"/><line x1="0" y1="'+Math.round(S*.5)+'" x2="'+U+'" y2="'+Math.round(S*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+_e+'"/><ellipse cx="'+Math.round(U*.5)+'" cy="'+Math.round(S*.5)+'" rx="'+Math.round(U*.18)+'" ry="'+Math.round(S*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+_e+'"/><rect x="'+Math.round(U*.2)+'" y="'+Math.round(S*.82)+'" width="'+Math.round(U*.6)+'" height="'+Math.round(S*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+_e+'"/>'+te+"</svg>"}function ae(O,U,S){var D=S>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+S+"</div>":"",K=U?'data-form-id="'+U.id+'"':"",ne=O.length>7?14:O.length>5?16:19,te=!!U;return"<div "+K+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(te?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(te?"":"filter:grayscale(1);opacity:0.5")+'">'+D+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(te?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+ne+"px;font-weight:900;color:"+(te?"#1A6B3C":"#aaa")+';line-height:1">'+O+'</div></div><div style="flex:1;overflow:hidden;background:'+(te?"#1A6B3C":"#ccc")+'">'+Q(O,140,220)+"</div></div>"}function pe(O,U){var S=window.innerWidth>=768?.76:.54,D=140,K=305,ne=Math.round(K*.22),te=K-ne,_e=O.length>7?10:13,$e=Q(O,D,te),Ee=U?"1.5px solid #2a7a40":"1px solid #ddd",Me=U?"":"filter:grayscale(1);opacity:0.45;",Ae=U?"#1A6B3C":"#bbb",Te="#fff";return'<div style="display:inline-block;zoom:'+S+';line-height:0;pointer-events:none"><div style="width:'+D+"px;height:"+K+"px;border-radius:6px;border:"+Ee+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+Me+'"><div style="height:'+ne+"px;background:"+Ae+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+_e+"px;font-weight:900;color:"+Te+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(D-4)+'px">'+O+'</span></div><div style="height:'+te+'px;overflow:hidden;flex-shrink:0">'+$e+"</div></div></div>"}function he(O){if(j){const U=re();if(!U.length){O.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const S=U.map(D=>i.find(K=>K.player.id===D.id)||{_fake:!0,player:D,id:"fake-"+D.id});ie(S,D=>D._fake?Ei(D.player):Xt(D,""),D=>D._fake?q({player:D.player,id:"x",_fake:!0}):q(D),D=>{D._fake||Ai(D,i,v,t)})}else{const U=le();if(!U.length){O.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const S={};U.forEach(K=>{const ne=K.player.id;S[ne]||(S[ne]=[]),S[ne].push(K)});const D=Object.values(S).map(K=>Bn(K));ie(D,K=>{const ne=v[K.player.id]||1,te=ne>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${ne}</div>`:"",$e=i.filter(Ee=>Ee.player.id===K.player.id&&Ee.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Xt(K,te+$e)},K=>q(K),K=>Ai(K,i,v,t))}}function xe(O){const U=j?z:[...L];if(!U.length){O.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const S=U.map(D=>({formation:D,card:f.find(K=>K.formation===D)||null,owned:L.has(D)}));ie(S,({formation:D,card:K,owned:ne})=>ae(D,ne?K:null,ne?f.filter(te=>te.formation===D).length:0),({formation:D,owned:K})=>pe(D,K),({card:D,owned:K})=>{K&&D&&qn(D,f,t,d)})}function ge(O){const U=Object.keys(y),S=j?U.length?U:o:[...T];if(!S.length){O.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const D=S.map(K=>({type:K,gc:Ne[K]||{icon:"⚡",desc:""},def:y[K]||null,owned:T.has(K),card:r.find(ne=>ne.gc_type===K)||null}));ie(D,({type:K,gc:ne,def:te,owned:_e,card:$e})=>{const Ee=_e?r.filter(g=>g.gc_type===K).length:0,Me=Ee>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${Ee}</div>`:"",Ae=(te==null?void 0:te.gc_type)==="ultra_game_changer",Te={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},ke={purple:"#b06ce0",light_blue:"#00d4ef"},qe=Te[te==null?void 0:te.color]||Te.purple,Re=ke[te==null?void 0:te.color]||ke.purple,k=(te==null?void 0:te.effect)||ne.desc||"",u=te!=null&&te.image_url?`/manager-wars/icons/${te.image_url}`:null;return _e&&$e?`<div data-gc-id="${$e.id}" data-gc-type="${K}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${Re};background:${qe};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${Re}66;cursor:pointer">
          ${Me}
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Ft=1e3;function qn(e,t,n,a){var z,o,v;const{state:c,toast:d,closeModal:s,navigate:l,refreshProfile:p}=n,i=e.formation,r={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function f(){const L=Mt[i]||{},T=ai[i]||[],C=290,R=360,F=20;function j(ee){const le=L[ee];return le?{x:le.x*C,y:le.y*R}:null}let N=`<svg width="${C}" height="${R}" viewBox="0 0 ${C} ${R}" xmlns="http://www.w3.org/2000/svg">`;for(const[ee,le]of T){const re=j(ee),M=j(le);!re||!M||(N+=`<line x1="${re.x}" y1="${re.y}" x2="${M.x}" y2="${M.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const ee of Object.keys(L)){const le=j(ee);if(!le)continue;const re=ee.replace(/\d+/,""),M=r[re]||"#555";N+=`<circle cx="${le.x}" cy="${le.y}" r="${F}" fill="${M}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,N+=`<text x="${le.x}" y="${le.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${re}</text>`}return N+="</svg>",N}const m=t.filter(L=>L.formation===i);m.length;const y=!e.is_for_sale;a(`Formation ${i}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(z=document.getElementById("direct-sell-form-btn"))==null||z.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${i} pour ${Ft.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const L=m.find(C=>!C.is_for_sale)||m[0];if(!L){d("Aucune carte à vendre","error");return}await $.from("market_listings").delete().eq("card_id",L.id),await $.from("transfer_history").delete().eq("card_id",L.id);const{error:T}=await $.from("cards").delete().eq("id",L.id);if(T){d(T.message,"error");return}await $.from("users").update({credits:(c.profile.credits||0)+Ft}).eq("id",c.profile.id),await p(),d(`+${Ft.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),s(),l("collection")}),(o=document.getElementById("market-sell-form-btn"))==null||o.addEventListener("click",async()=>{const L=parseInt(document.getElementById("sell-price-form").value);if(!L||L<1){d("Prix invalide","error");return}await $.from("cards").update({is_for_sale:!0,sale_price:L}).eq("id",e.id),await $.from("market_listings").insert({seller_id:c.profile.id,card_id:e.id,price:L}),d("Carte mise en vente sur le marché !","success"),s(),l("collection")}),(v=document.getElementById("cancel-sell-form-btn"))==null||v.addEventListener("click",async()=>{await $.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await $.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),d("Annonce retirée","success"),s(),l("collection")})}async function Ai(e,t,n,a){var ae,pe,he,xe,ge,O,U;const{state:c,toast:d,openModal:s,closeModal:l,navigate:p,refreshProfile:i}=a,r=e.player,f=t.filter(S=>S.player.id===r.id),m=f.length,y=Math.max(Number(r.note_g)||0,Number(r.note_d)||0,Number(r.note_m)||0,Number(r.note_a)||0),z=r.rarity||"normal",{data:o}=await $.from("sell_price_configs").select("*").eq("rarity",z).lte("note_min",y).gte("note_max",y).order("note_min",{ascending:!1}).limit(1),v=((ae=o==null?void 0:o[0])==null?void 0:ae.price)??jn[z]??1e3,L=r.rarity!=="legende",T=Vi(r),C=(r.rarity==="pepite"||r.rarity==="papyte")&&e.current_note!=null?e.current_note:zt(r,r.job),R=r.rarity==="pepite"||r.rarity==="papyte",F=r.job2?R?Ri(e,Ki(r.job2)):zt(r,r.job2):null,j=At[r.job]||"#1A6B3C",N=r.job2?At[r.job2]:null,ee=Ui[r.rarity]||"#ccc",le=li[r.country_code]||r.country_code||"",re=f.map(S=>S.id);let M={};if(re.length){const{data:S}=await $.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",re).order("transferred_at",{ascending:!0});(S||[]).forEach(D=>{M[D.card_id]||(M[D.card_id]=[]),M[D.card_id].push(D)})}const Y=S=>{const D=S.transferred_at?new Date(S.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",K=S.source==="booster"?"Booster":S.price?S.price.toLocaleString("fr"):"—";return`
    <div style="display:flex;justify-content:space-between;align-items:flex-start;background:#f9f9f9;border-radius:8px;padding:8px 12px;gap:8px">
      ...
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:13px;font-weight:700;color:${S.source==="booster"?"var(--gray-600)":"var(--yellow)"}">${K}</div>
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${f.map((S,D)=>{const K=M[S.id]||[],ne=S.is_for_sale,te=K.length?K[K.length-1]:null,$e=(r.rarity==="pepite"||r.rarity==="papyte")&&S.current_note!=null?` (☆${S.current_note})`:"";return`
            <div data-card-id="${S.id}" data-card-idx="${D}" class="exemplaire-row"
              style="border:2px solid #eee;border-radius:10px;padding:10px;cursor:pointer;transition:border-color .15s;${ne?"opacity:0.6":""}">
              <div style="display:flex;align-items:center;gap:10px">
                <input type="checkbox" class="expl-check" data-id="${S.id}" ${ne?"disabled":""} style="width:18px;height:18px;cursor:pointer;accent-color:#1A6B3C;flex-shrink:0">
                <div style="flex:1;min-width:0">
      ...
      <div style="width:140px;border-radius:12px;padding:6px;background:${ee};flex-shrink:0">
        <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
          ...
          <div style="position:relative;height:80px;background:#f2e8d2;display:flex;flex-direction:column;align-items:center">
            ...
            ${F!==null?`
            <svg width="32" height="31" viewBox="0 0 32 31" style="position:absolute;top:50px;z-index:2;display:block">
              <polygon points="16,2 19.5,11 30,11 22,17.5 25,27 16,21.5 7,27 10,17.5 2,11 12.5,11" fill="${N}" stroke="white" stroke-width="1.5"/>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",r.note_g],["DEF",r.note_d],["MIL",r.note_m],["ATT",r.note_a]].map(([S,D])=>{const K=At[S],ne=Number(D)||0;return`<div style="display:flex;flex-direction:column;align-items:center;gap:2px">
    ...
    ${ie}
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(xe=document.getElementById("close-detail"))==null||xe.addEventListener("click",l);let q=new Set;const Q=()=>{const S=q.size,D=document.getElementById("sell-action-panel");D&&(D.style.display=S>0?"block":"none",document.getElementById("sell-selected-count").textContent=S,document.getElementById("sell-direct-total").textContent=(S*v).toLocaleString("fr")+" cr.")};document.querySelectorAll(".expl-check").forEach(S=>{S.addEventListener("change",()=>{const D=S.dataset.id;S.checked?q.add(D):q.delete(D);const K=S.closest(".exemplaire-row");K&&(K.style.borderColor=S.checked?"#1A6B3C":"#eee"),Q()})}),document.querySelectorAll(".exemplaire-row").forEach(S=>{S.addEventListener("click",D=>{if(D.target.closest("button")||D.target.tagName==="INPUT")return;const K=S.querySelector(".expl-check");K&&!K.disabled&&(K.checked=!K.checked,K.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(S=>{S.addEventListener("click",D=>{D.stopPropagation();const K=document.querySelector(`.expl-hist[data-hist="${S.dataset.idx}"]`);K&&(K.style.display=K.style.display==="none"?"flex":"none")})}),(ge=document.getElementById("direct-sell-btn"))==null||ge.addEventListener("click",async()=>{const S=[...q];if(!S.length)return;const D=S.length*v;if(!confirm(`Vendre ${S.length} carte${S.length>1?"s":""} ${r.surname_encoded} pour ${D.toLocaleString("fr")} crédits ? Action irréversible.`))return;await $.from("market_listings").delete().in("card_id",S),await $.from("transfer_history").delete().in("card_id",S);const{error:K}=await $.from("cards").delete().in("id",S);if(K){d(K.message,"error");return}await $.from("users").update({credits:(c.profile.credits||0)+D}).eq("id",c.profile.id),await i();const ne=document.getElementById("nav-credits");ne&&(ne.textContent=`💰 ${(c.profile.credits||0).toLocaleString("fr")}`),d(`+${D.toLocaleString("fr")} crédits ! ${S.length} carte${S.length>1?"s":""} vendue${S.length>1?"s":""}.`,"success"),l(),p("collection")}),(O=document.getElementById("market-sell-btn"))==null||O.addEventListener("click",async()=>{var _e;const S=[...q];if(!S.length){d("Sélectionne au moins un exemplaire","warning");return}const D=parseInt((_e=document.getElementById("sell-market-price"))==null?void 0:_e.value);if(!D||D<1){d("Prix invalide","error");return}const{error:K}=await $.from("cards").update({is_for_sale:!0,sale_price:D}).in("id",S);if(K){d(K.message,"error");return}const ne=S.map($e=>({seller_id:c.profile.id,card_id:$e,price:D,status:"active"})),{error:te}=await $.from("market_listings").insert(ne);te&&console.warn("[Market] insert listings:",te.message),d(`${S.length} carte${S.length>1?"s":""} mise${S.length>1?"s":""} en vente à ${D.toLocaleString("fr")} cr. chacune !`,"success"),l(),p("collection")}),(U=document.getElementById("cancel-sell-btn"))==null||U.addEventListener("click",async()=>{await $.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await $.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),d("Annonce retirée","success"),l(),p("collection")})}const Pt={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},rt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Yt(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const n=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${n}.png`}function Yi(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function Wi(e){var n;const t="https://fcnwclxlkytdfjotqkta.supabase.co";return(n=e==null?void 0:e.clubs)!=null&&n.logo_url?e.clubs.logo_url.startsWith("http")?e.clubs.logo_url:`${t}/storage/v1/object/public/assets/clubs/${e.clubs.logo_url}`:null}function Gn(e,t=44,n=58){var y;const a=e?Yt(e):null,c=e?Wi(e):null,d=Yi(e==null?void 0:e.country_code),s=(e==null?void 0:e.job)||"MIL",l=rt[s]||"#555",p={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",i=Number(s==="GK"?e==null?void 0:e.note_g:s==="DEF"?e==null?void 0:e.note_d:s==="MIL"?e==null?void 0:e.note_m:e==null?void 0:e.note_a)||0,r=Math.round(n*.19),f=Math.round(n*.25),m=n-r-f;return e?`<div style="width:${t}px;height:${n}px;border-radius:5px;border:2px solid ${p};background:${l};position:relative;overflow:hidden;flex-shrink:0">
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const s=prompt("Nom du deck :",`Deck ${((d==null?void 0:d.length)||0)+1}`);if(!s)return;const{data:l,error:p}=await $.from("decks").insert({owner_id:n.profile.id,name:s}).select().single();if(p){c(p.message,"error");return}c("Deck créé !","success"),Ti(l.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(s=>{s.addEventListener("click",()=>Ti(s.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(s=>{s.addEventListener("click",async()=>{const l=prompt("Nouveau nom :",s.dataset.name);if(!l||l===s.dataset.name)return;const{error:p}=await $.from("decks").update({name:l}).eq("id",s.dataset.rename);if(p){c(p.message,"error");return}c("Deck renommé !","success"),Zt(e,t)})}),e.querySelectorAll("[data-delete]").forEach(s=>{s.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${s.dataset.name}" ? Cette action est irréversible.`))return;await $.from("deck_cards").delete().eq("deck_id",s.dataset.delete);const{error:l}=await $.from("decks").delete().eq("id",s.dataset.delete);if(l){c(l.message,"error");return}c("Deck supprimé.","success"),Zt(e,t)})})}async function Ti(e,t,n){const{state:a,toast:c}=n;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await $.from("decks").select("*").eq("id",e).single(),{data:s}=await $.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
  ...
  <div style="height:100%;overflow-y:auto;padding-bottom:80px;background:var(--page-bg)">
  ...
  </div>`,Nn(e,t,c,n),document.getElementById("builder-back").addEventListener("click",()=>a("decks")),document.getElementById("formation-select").addEventListener("change",i=>{t.formation=i.target.value;const r=Ii(t.formation),f={};r.forEach(m=>{t.slots[m]&&(f[m]=t.slots[m])}),t.slots=f,ut(e,t,n)}),document.getElementById("save-deck").addEventListener("click",()=>On(t,n)),e.querySelectorAll("[data-remove-sub]").forEach(i=>{i.addEventListener("click",()=>{t.subs=t.subs.filter(r=>r!==i.dataset.removeSub),ut(e,t,n)})}),(p=document.getElementById("add-sub-btn"))==null||p.addEventListener("click",()=>{Rn(t,e,n)})}function Nn(e,t,n,a){const c=e.querySelector("#deck-field");if(!c)return;const d=Mt[t.formation]||{},s=Bi(t.formation)||[],l={};for(const L of n){const T=t.slots[L],C=T?t.playerCards.find(R=>R.id===T):null;l[L]=C?C.player:null}const p=300,i=300,r=48,f=64,m=13,y=16,z=38;function o(L){const T=d[L];return T?{x:T.x*p,y:T.y*i}:null}let v="";for(const[L,T]of s){const C=o(L),R=o(T);if(!C||!R)continue;const F=l[L]?{...l[L],club_id:l[L].club_id,country_code:l[L].country_code,rarity:l[L].rarity}:null,j=l[T]?{...l[T],club_id:l[T].club_id,country_code:l[T].country_code,rarity:l[T].rarity}:null,N=Ze(F,j);N==="#ff3333"||N==="#cc2222"?v+=`<line x1="${C.x.toFixed(1)}" y1="${C.y.toFixed(1)}" x2="${R.x.toFixed(1)}" y2="${R.y.toFixed(1)}" stroke="${N}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(v+=`<line x1="${C.x.toFixed(1)}" y1="${C.y.toFixed(1)}" x2="${R.x.toFixed(1)}" y2="${R.y.toFixed(1)}" stroke="${N}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,v+=`<line x1="${C.x.toFixed(1)}" y1="${C.y.toFixed(1)}" x2="${R.x.toFixed(1)}" y2="${R.y.toFixed(1)}" stroke="${N}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const L of n){const T=o(L);if(!T)continue;const C=l[L],R=L.replace(/\d+/,""),F=rt[R]||"#555",j=(T.x-r/2).toFixed(1),N=(T.y-f/2).toFixed(1),ee={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[C==null?void 0:C.rarity]||"#aaa";if(C){const le=Yt(C),re=Wi(C),M=Yi(C.country_code),Y=Number(R==="GK"?C.note_g:R==="DEF"?C.note_d:R==="MIL"?C.note_m:C.note_a)||0,ie=f-m-y;v+=`<defs><clipPath id="dcp-${L}"><rect x="${j}" y="${(T.y-f/2+m).toFixed(1)}" width="${r}" height="${ie}"/></clipPath></defs>`,v+=`<rect x="${j}" y="${N}" width="${r}" height="${f}" rx="5" fill="${F}"/>`,le&&(v+=`<image href="${le}" x="${j}" y="${(T.y-f/2+m).toFixed(1)}" width="${r}" height="${ie}" clip-path="url(#dcp-${L})" preserveAspectRatio="xMidYMin slice"/>`),v+=`<rect x="${j}" y="${N}" width="${r}" height="${m}" fill="rgba(255,255,255,0.93)"/>`,v+=`<text x="${T.x.toFixed(1)}" y="${(T.y-f/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(C.surname_encoded||"").slice(0,9)}</text>`;const q=(T.y+f/2-y).toFixed(1);v+=`<rect x="${j}" y="${q}" width="${r}" height="${y}" fill="rgba(255,255,255,0.93)"/>`,M&&(v+=`<image href="${M}" x="${(T.x-21).toFixed(1)}" y="${(T.y+f/2-y+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),v+=`<text x="${T.x.toFixed(1)}" y="${(T.y+f/2-y/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${Y}</text>`,re&&(v+=`<image href="${re}" x="${(T.x+r/2-14).toFixed(1)}" y="${(T.y+f/2-y+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),v+=`<rect x="${j}" y="${N}" width="${r}" height="${f}" rx="5" fill="none" stroke="${ee}" stroke-width="2"/>`}else v+=`<rect x="${j}" y="${N}" width="${r}" height="${f}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,v+=`<text x="${T.x.toFixed(1)}" y="${T.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,v+=`<text x="${T.x.toFixed(1)}" y="${(T.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${R}</text>`;v+=`<rect x="${j}" y="${N}" width="${r}" height="${f}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${L}" style="cursor:pointer"/>`}c.innerHTML=`<svg viewBox="${-z} ${-z} ${p+z*2} ${i+z*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${v}</svg>`,c.querySelectorAll(".deck-slot-hit").forEach(L=>{L.addEventListener("click",()=>Pn(L.dataset.pos,t,e,a))})}function Pn(e,t,n,a){var m,y,z;const{openModal:c,closeModal:d}=a,s=e.replace(/\d+/,""),l=t.slots[e],p=l?t.playerCards.find(o=>o.id===l):null;(m=p==null?void 0:p.player)==null||m.id;const i=new Set;Object.entries(t.slots).forEach(([o,v])=>{var T;if(o===e||!v)return;const L=t.playerCards.find(C=>C.id===v);(T=L==null?void 0:L.player)!=null&&T.id&&i.add(L.player.id)}),t.subs.forEach(o=>{var L;const v=t.playerCards.find(T=>T.id===o);(L=v==null?void 0:v.player)!=null&&L.id&&i.add(v.player.id)});const r=new Set,f=t.playerCards.filter(o=>{const v=o.player;return!(v.job===s||v.job2===s)||i.has(v.id)||r.has(v.id)?!1:(r.add(v.id),!0)});f.sort((o,v)=>{const L=s==="GK"?o.player.note_g:s==="DEF"?o.player.note_d:s==="MIL"?o.player.note_m:o.player.note_a;return(s==="GK"?v.player.note_g:s==="DEF"?v.player.note_d:s==="MIL"?v.player.note_m:v.player.note_a)-L}),c(`Choisir ${s} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${f.length>0?f.map(o=>{var R,F;const v=o.player,L=s==="GK"?v.note_g:s==="DEF"?v.note_d:s==="MIL"?v.note_m:v.note_a,T=Yt(v),C={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[v.rarity];return`<div class="player-option" data-card-id="${o.id}"
          ...
          <div style="width:44px;height:44px;border-radius:8px;overflow:hidden;flex-shrink:0;background:#dde;border:2px solid ${rt[s]}">
            ${T?`<img src="${T}" style="width:100%;height:100%;object-fit:cover">`:`<div style="width:100%;height:100%;background:${rt[s]};display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:700">${s}</div>`}
          </div>
          <!-- Infos -->
          <div style="flex:1;min-width:0">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-sub-selector">Fermer</button>'),(p=document.getElementById("close-sub-selector"))==null||p.addEventListener("click",c),document.querySelectorAll(".player-option").forEach(i=>{i.addEventListener("click",()=>{e.subs.push(i.dataset.cardId),c(),ut(t,e,n)})})}async function On(e,t){const{state:n,toast:a,navigate:c}=t,d=e.formationCards.find(p=>p.formation===e.formation),s=(d==null?void 0:d.id)||e.formationCardId;await $.from("decks").update({formation:e.formation,formation_card_id:s||null}).eq("id",e.deckId),await $.from("deck_cards").delete().eq("deck_id",e.deckId);const l=[];if(Object.entries(e.slots).forEach(([p,i],r)=>{l.push({deck_id:e.deckId,card_id:i,position:p,is_starter:!0,slot_order:r})}),e.subs.forEach((p,i)=>{l.push({deck_id:e.deckId,card_id:p,position:`SUB${i+1}`,is_starter:!1,slot_order:100+i})}),l.length>0){const{error:p}=await $.from("deck_cards").insert(l);if(p){a(p.message,"error");return}}a("Deck enregistré ✅","success"),c("decks")}function Ii(e){const t=Pt[e]||Pt["4-4-2"],n=["GK1"];for(let a=1;a<=t.DEF;a++)n.push(`DEF${a}`);for(let a=1;a<=t.MIL;a++)n.push(`MIL${a}`);for(let a=1;a<=t.ATT;a++)n.push(`ATT${a}`);return n}async function Xi(){const{data:e}=await $.from("booster_configs").select("*").eq("is_active",!0).order("sort_order");if(!(e!=null&&e.length))return[];const{data:t}=await $.from("booster_drop_rates").select("*").in("booster_id",e.map(n=>n.id)).order("sort_order");return e.map(n=>({...n,rates:(t||[]).filter(a=>a.booster_id===n.id)}))}function Hn(e){if(!(e!=null&&e.length))return null;const t=e.reduce((a,c)=>a+Number(c.percentage),0);let n=Math.random()*t;for(const a of e)if(n-=Number(a.percentage),n<=0)return a;return e[e.length-1]}const Ji=()=>Object.keys(Mt),Un=[{id:"players_std",img:"/manager-wars/icons/booster-players.png",name:"Players",sub:"5 cartes joueurs",cost:5e3,costLabel:"5 000 crédits",cardCount:5,type:"player"},{id:"players_pub",img:"/manager-wars/icons/booster-silver.png",name:"Players (pub)",sub:"3 cartes joueurs",cost:0,costLabel:"1 pub",cardCount:3,type:"player"},{id:"game_changer",img:"/manager-wars/icons/booster-gamechanger.png",name:"Game Changer",sub:"3 cartes spéciales",cost:1e4,costLabel:"10 000 crédits",cardCount:3,type:"game_changer"},{id:"formation",img:"/manager-wars/icons/booster-formation.png",name:"Formation",sub:"1 carte formation",cost:1e4,costLabel:"10 000 crédits",cardCount:1,type:"formation"}],ei={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé."},"Double attaque":{icon:"⚡",desc:"La prochaine attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la prochaine action IA."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function Kn(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";if(!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const n=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${n}.png`}const Li={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},Vn={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},Yn={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function zi(e,t){return e&&Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}function Wn(e){var r,f;const t=e.player;if(!t)return"";const n=t.job||"ATT",a=Li[n],c=Vn[t.rarity]||"#ccc",d=zi(t,n),s=t.job2?zi(t,t.job2):null,l=t.job2?Li[t.job2]:null,p=Kn(t),i=Yn[t.country_code]||t.country_code||"";return`
  ...
  </div>`,e.querySelectorAll(".booster-card:not(.disabled)").forEach(l=>{l.addEventListener("click",async()=>{const p=d.find(i=>i.id===l.dataset.booster);if(p){l.style.opacity="0.5",l.style.pointerEvents="none";try{await Jn(p,{state:t,toast:a,navigate:n,container:e})}catch(i){a(i.message,"error"),l.style.opacity="",l.style.pointerEvents=""}}})}),e.querySelectorAll(".booster-info-btn").forEach(l=>{l.addEventListener("click",p=>{p.stopPropagation();const i=d.find(r=>r.id===l.dataset.boosterId);to(i)})})}async function Jn(e,{state:t,toast:n,navigate:a,container:c}){var r;if(e.cost>0&&t.profile.credits<e.cost){n("Crédits insuffisants","error");return}if(e.isPub)try{await oo()}catch(f){n(f.message||"Regardez la pub entièrement pour ouvrir le booster","error");return}const{data:d}=await $.from("cards").select("card_type, player_id, formation").eq("owner_id",t.profile.id),s=new Set((d||[]).filter(f=>f.card_type==="player").map(f=>f.player_id)),l=new Set((d||[]).filter(f=>f.card_type==="formation").map(f=>f.formation));let p=[];if((r=e.rates)!=null&&r.length)p=await ti(t.profile,e);else{const f=e.type||"player";f==="player"?p=await Zi(t.profile,e.cardCount,e.cost):f==="game_changer"?p=await en(t.profile,e.cardCount,e.cost):f==="formation"?p=await tn(t.profile,e.cost):p=await ti(t.profile,e)}p.forEach(f=>{f.card_type==="player"&&f.player?f.isDuplicate=s.has(f.player.id):f.card_type==="formation"&&(f.isDuplicate=l.has(f.formation))});const{data:i}=await $.from("users").select("*").eq("id",t.profile.id).single();i&&(t.profile=i),nn(p,e,a)}function Qn(){const e=Math.random()*100;return e<.5?"legende":e<2?"special":e<10?"normal_high":"normal_low"}function tt(e){return Math.max(Number(e.note_g)||0,Number(e.note_d)||0,Number(e.note_m)||0,Number(e.note_a)||0)}function Zn(e,t){let n;switch(t){case"legende":n=e.filter(a=>a.rarity==="legende"),n.length||(n=e.filter(a=>a.rarity==="pepite"||a.rarity==="papyte")),n.length||(n=e.filter(a=>tt(a)>=6));break;case"special":n=e.filter(a=>a.rarity==="pepite"||a.rarity==="papyte"),n.length||(n=e.filter(a=>tt(a)>=6));break;case"normal_high":n=e.filter(a=>a.rarity==="normal"&&tt(a)>=6),n.length||(n=e.filter(a=>tt(a)>=6));break;default:n=e.filter(a=>a.rarity==="normal"&&tt(a)>=1&&tt(a)<=5),n.length||(n=e.filter(a=>a.rarity==="normal"));break}return n.length||(n=e),n[Math.floor(Math.random()*n.length)]}async function ti(e,t){if(t.cost>0){const{error:l}=await $.from("users").update({credits:e.credits-t.cost}).eq("id",e.id);if(l)throw l}const{data:n}=await $.from("cards").select("player_id, card_type, formation").eq("owner_id",e.id),a=new Set((n||[]).filter(l=>l.card_type==="player").map(l=>l.player_id)),c=new Set((n||[]).filter(l=>l.card_type==="formation").map(l=>l.formation)),d=new Set,s=[];for(let l=0;l<(t.cardCount||5);l++){const p=Hn(t.rates);if(p){if(p.card_type==="player"){const i=T=>({légende:"legende",pépite:"pepite",pépites:"pepite"})[T]||T,r=p.rarity?i(p.rarity):null;let f=$.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);r&&(f=f.eq("rarity",r));const{data:m}=await f;let y=m||[];if((p.note_min||p.note_max)&&(y=y.filter(T=>{const C=Math.max(Number(T.note_g)||0,Number(T.note_d)||0,Number(T.note_m)||0,Number(T.note_a)||0);return(!p.note_min||C>=p.note_min)&&(!p.note_max||C<=p.note_max)})),y.length||(p.note_min||p.note_max?(y=m||[],console.warn("[Booster] Aucun joueur avec note",p.note_min,"-",p.note_max,"— fallback rareté uniquement")):y=m||[]),!y.length)continue;let z=y.filter(T=>!d.has(T.id));z.length||(z=y);const o=z[Math.floor(Math.random()*z.length)];d.add(o.id);const v=a.has(o.id),{data:L}=await $.from("cards").insert({owner_id:e.id,player_id:o.id,card_type:"player"}).select().single();L&&(s.push({...L,player:o,isDuplicate:v}),$.rpc("record_transfer",{p_card_id:L.id,p_player_id:o.id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{}))}else if(p.card_type==="game_changer"){const{data:i}=await $.from("gc_definitions").select("id,name").eq("is_active",!0).eq("gc_type","game_changer"),r=i!=null&&i.length?i:[{name:"Ressusciter"},{name:"Double attaque"},{name:"Bouclier"},{name:"Vol de note"},{name:"Gel"}],m=r[Math.floor(Math.random()*r.length)].name,{data:y}=await $.from("cards").insert({owner_id:e.id,card_type:"game_changer",gc_type:m}).select().single();y&&s.push(y)}else if(p.card_type==="formation"){const i=Ji(),r=i[Math.floor(Math.random()*i.length)],f=c.has(r),{data:m}=await $.from("cards").insert({owner_id:e.id,card_type:"formation",formation:r}).select();m!=null&&m[0]&&s.push({...m[0],isDuplicate:f})}}}return s}async function Zi(e,t,n){if(n>0){const{error:i}=await $.from("users").update({credits:e.credits-n}).eq("id",e.id);if(i)throw i}const{data:a}=await $.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(a!=null&&a.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const c=a.filter(i=>i.job==="GK"),d=a.filter(i=>i.job!=="GK"),s=!e.first_booster_opened&&c.length>0,l=[];for(let i=0;i<t;i++){const r=i===0&&s?c:i===0?d:a,f=Qn(),m=Zn(r,f);m&&l.push(m)}s&&await $.from("users").update({first_booster_opened:!0}).eq("id",e.id);const{data:p}=await $.from("cards").insert(l.map(i=>({owner_id:e.id,player_id:i.id,card_type:"player"}))).select();return(p||[]).forEach((i,r)=>{$.rpc("record_transfer",{p_card_id:i.id,p_player_id:l[r].id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{})}),l.map((i,r)=>({...p[r],player:i}))}async function en(e,t,n){const{error:a}=await $.from("users").update({credits:e.credits-n}).eq("id",e.id);if(a)throw a;const{data:c}=await $.from("gc_definitions").select("id,name,gc_type,color,effect,image_url").eq("is_active",!0),d=c!=null&&c.length?c:Object.keys(ei).map(r=>({name:r,gc_type:"game_changer"})),s=Array.from({length:t},()=>{const r=d[Math.floor(Math.random()*d.length)];return{owner_id:e.id,card_type:"game_changer",gc_type:r.name,gc_definition_id:r.id||null}}),{data:l,error:p}=await $.from("cards").insert(s).select();return p&&console.error("[Booster GC] Erreur insert:",p.message,p),(l||[]).map(r=>{const f=c==null?void 0:c.find(m=>m.name===r.gc_type||m.id===r.gc_definition_id);return{...r,_gcDef:f||null}})}async function tn(e,t){const{error:n}=await $.from("users").update({credits:e.credits-t}).eq("id",e.id);if(n)throw n;const{data:a}=await $.from("cards").select("formation").eq("owner_id",e.id).eq("card_type","formation"),c=new Set((a||[]).map(r=>r.formation)),d=Ji(),s=d[Math.floor(Math.random()*d.length)],l=c.has(s),{data:p,error:i}=await $.from("cards").insert({owner_id:e.id,card_type:"formation",formation:s}).select();return i&&console.error("[Booster Formation] Erreur insert:",i.message,i),(p||[]).map(r=>({...r,isDuplicate:l}))}function nn(e,t,n,a=null){var Y,ie;if(!e||e.length===0){const q=document.createElement("div");q.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",q.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
      <div style="font-size:13px;color:rgba(255,255,255,0.5)">Erreur lors du tirage (permissions DB ou colonne manquante)</div>
      <button style="margin-top:10px;padding:12px 28px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer" id="anim-close-err">Fermer</button>`,document.body.appendChild(q),(Y=q.querySelector("#anim-close-err"))==null||Y.addEventListener("click",()=>q.remove());return}e=[...e].sort((q,Q)=>{const ae=q.player?tt(q.player):-1;return(Q.player?tt(Q.player):-1)-ae});const c=document.createElement("div");c.id="booster-anim-overlay",c.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(c);let d=!1;const s=document.getElementById("pack-cut-zone"),l=document.getElementById("pack-blade");let p=!1;const i=q=>q.touches&&q.touches[0]?q.touches[0].clientX:q.clientX;function r(q){d||(p=!0,l.style.opacity="1",f(q))}function f(q){if(!p||d)return;const Q=s.getBoundingClientRect(),ae=i(q)-Q.left,pe=Math.max(0,Math.min(1,ae/Q.width));l.style.width=pe*Q.width+"px",pe>=.82&&y()}function m(){d||(p=!1,l.style.transition="width .2s ease, opacity .2s ease",l.style.width="0",l.style.opacity="0",setTimeout(()=>{d||(l.style.transition="")},220))}function y(){var Q;if(d)return;d=!0,p=!1,l.style.width="100%",l.style.opacity="1",(Q=document.getElementById("cut-flash"))==null||Q.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const q=document.getElementById("cut-hint");q&&(q.style.opacity="0"),s.classList.add("pack-cut"),setTimeout(()=>{l.style.opacity="0",document.getElementById("pack-phase").style.display="none",L(0)},620)}s.addEventListener("pointerdown",r),window.addEventListener("pointermove",f),window.addEventListener("pointerup",m),s.addEventListener("touchstart",r,{passive:!0}),window.addEventListener("touchmove",f,{passive:!0}),window.addEventListener("touchend",m);let z=0,o=!1;const v=new Set;function L(q){z=q,document.getElementById("reveal-phase").style.display="flex",T(),C(q,0),ee()}function T(){const q=document.getElementById("card-dots");q&&(q.innerHTML=e.map((Q,ae)=>`<div class="card-dot" data-i="${ae}" style="width:8px;height:8px;border-radius:50%;background:${ae===z?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),q.querySelectorAll(".card-dot").forEach(Q=>Q.addEventListener("click",()=>F(parseInt(Q.dataset.i)))))}function C(q,Q){var D;const ae=e[q],pe=document.getElementById("card-counter"),he=document.getElementById("card-track");pe&&(pe.textContent=`Carte ${q+1} / ${e.length}`);const xe=document.getElementById("reveal-btns");xe&&(xe.style.display=q===e.length-1?"flex":"none");const ge=ae.card_type==="player"&&((D=ae.player)==null?void 0:D.rarity)==="legende",O=!v.has(q);v.add(q);let U=0;if(ae.card_type==="player"&&ae.player){const K=ae.player,ne=K.job||"ATT";U=Number(ne==="GK"?K.note_g:ne==="DEF"?K.note_d:ne==="MIL"?K.note_m:K.note_a)||0}const S=K=>{he.innerHTML=`
        <div id="current-card-wrap" style="position:relative;display:flex;flex-direction:column;align-items:center;gap:8px;${ge?"filter:drop-shadow(0 0 20px #7a28b8)":""}">
          <div style="transform:scale(1.25);transform-origin:center">${eo(ae)}</div>
        ...
        </div>`;const ne=document.getElementById("current-card-wrap");Q!==0?(ne.style.transition="none",ne.style.transform=`translateX(${Q>0?100:-100}%)`,requestAnimationFrame(()=>{ne.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",ne.style.transform="translateX(0)"})):ne.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),K||ge?re():M(),T()};O&&U>6&&ae.card_type==="player"&&ae.player?R(ae,()=>S(!0)):S(!1)}function R(q,Q){var U;o=!0;const ae=q.player,pe=`https://flagsapi.com/${ae.country_code}/flat/64.png`,he=(U=ae.clubs)==null?void 0:U.logo_url,xe=document.getElementById("walkout-overlay"),ge=document.getElementById("walkout-stage");if(!xe||!ge){o=!1,Q();return}xe.style.display="flex";const O=()=>{const S=ge.firstElementChild;S&&(S.classList.remove("wo-in"),S.classList.add("wo-out"))};ge.innerHTML=`<img class="wo-in" src="${pe}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(O,2e3),setTimeout(()=>{var S;ge.innerHTML=he?`<img class="wo-in" src="${he}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((S=ae.clubs)==null?void 0:S.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},2450),setTimeout(O,4450),setTimeout(()=>{xe.style.display="none",ge.innerHTML="",o=!1,navigator.vibrate&&navigator.vibrate([40,30,80]),Q()},4900)}function F(q){if(o||q<0||q>=e.length||q===z)return;const Q=q>z?1:-1;z=q,C(q,Q)}function j(){F(z+1)}function N(){F(z-1)}function ee(){const q=document.getElementById("card-viewport");if(!q||q._swipeBound)return;q._swipeBound=!0;let Q=0,ae=0,pe=0,he=!1;const xe=D=>D.touches?D.touches[0].clientX:D.clientX,ge=D=>D.touches?D.touches[0].clientY:D.clientY,O=D=>{he=!0,Q=xe(D),ae=ge(D),pe=0},U=D=>{if(!he)return;pe=xe(D)-Q;const K=ge(D)-ae;if(Math.abs(pe)<Math.abs(K))return;const ne=document.getElementById("current-card-wrap");ne&&(ne.style.transition="none",ne.style.transform=`translateX(${pe*.6}px) rotate(${pe*.02}deg)`)},S=()=>{if(!he)return;he=!1;const D=document.getElementById("current-card-wrap"),K=55;pe<=-K&&z<e.length-1?j():pe>=K&&z>0?N():D&&(D.style.transition="transform .2s ease",D.style.transform="translateX(0)")};q.addEventListener("pointerdown",O),q.addEventListener("pointermove",U),q.addEventListener("pointerup",S),q.addEventListener("pointercancel",S),q.addEventListener("touchstart",O,{passive:!0}),q.addEventListener("touchmove",U,{passive:!0}),q.addEventListener("touchend",S),q.addEventListener("click",D=>{if(Math.abs(pe)>8)return;const K=q.getBoundingClientRect();D.clientX-K.left>K.width/2?j():N()})}let le=null;function re(){const q=document.getElementById("fireworks-canvas");if(!q)return;q.width=window.innerWidth,q.height=window.innerHeight;const Q=q.getContext("2d"),ae=[];function pe(){const xe=Math.random()*q.width,ge=Math.random()*q.height*.6,O=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],U=O[Math.floor(Math.random()*O.length)];for(let S=0;S<60;S++){const D=Math.PI*2/60*S,K=2+Math.random()*5;ae.push({x:xe,y:ge,vx:Math.cos(D)*K,vy:Math.sin(D)*K,alpha:1,color:U,size:2+Math.random()*3})}}pe(),le=setInterval(pe,600);function he(){if(document.getElementById("fireworks-canvas")){Q.clearRect(0,0,q.width,q.height);for(let xe=ae.length-1;xe>=0;xe--){const ge=ae[xe];if(ge.x+=ge.vx,ge.y+=ge.vy+.08,ge.vy*=.98,ge.alpha-=.018,ge.alpha<=0){ae.splice(xe,1);continue}Q.globalAlpha=ge.alpha,Q.fillStyle=ge.color,Q.beginPath(),Q.arc(ge.x,ge.y,ge.size,0,Math.PI*2),Q.fill()}Q.globalAlpha=1,(le!==null||ae.length>0)&&requestAnimationFrame(he)}}he()}function M(){le!==null&&(clearInterval(le),le=null);const q=document.getElementById("fireworks-canvas");q&&q.getContext("2d").clearRect(0,0,q.width,q.height)}if(a){const q=document.getElementById("reveal-btns");q&&(q.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(ie=document.getElementById("reveal-next"))==null||ie.addEventListener("click",()=>{M(),c.remove(),a()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{M(),c.remove(),n("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{M(),c.remove(),n("boosters")})}function eo(e){var t,n;if(e.card_type==="player"&&e.player)return Wn(e);if(e.card_type==="game_changer"){const a=e._gcDef,c=(a==null?void 0:a.gc_type)==="ultra_game_changer",d={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},s={purple:"#b06ce0",light_blue:"#00d4ef"},l=d[a==null?void 0:a.color]||d.purple,p=s[a==null?void 0:a.color]||s.purple,i=(a==null?void 0:a.name)||e.gc_type||"Game Changer",r=(a==null?void 0:a.effect)||((t=ei[e.gc_type])==null?void 0:t.desc)||"",f=a!=null&&a.image_url?`/manager-wars/icons/${a.image_url}`:null,m=((n=ei[e.gc_type])==null?void 0:n.icon)||"⚡";return`<div style="width:170px;height:240px;background:${l};border-radius:14px;border:3px solid ${p};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${p}88;flex-shrink:0">
      ...
      </div>`,document.body.appendChild(a),a.addEventListener("click",s=>{s.target===a&&a.remove()}),(n=document.getElementById("odds-close"))==null||n.addEventListener("click",()=>a.remove());return}io()}function io(){const e=document.createElement("div");e.style.cssText=`position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const no="simulation",on="VOTRE_ZONE_ID";function oo(){switch(no){case"propellerads":return ro();case"adsense":return ao();default:return ci()}}function ci(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let n=5;t.innerHTML=`
      ...
      <button id="ad-skip" disabled style="margin-top:8px;padding:10px 24px;border-radius:20px;border:none;background:rgba(255,255,255,0.15);color:rgba(255,255,255,0.4);font-size:13px;cursor:default">
        Passer (5)
      </button>`,document.body.appendChild(t);const a=setInterval(()=>{n--;const c=document.getElementById("ad-cd"),d=document.getElementById("ad-skip");c&&(c.textContent=n),d&&(d.textContent=n>0?`Passer (${n})`:"✓ Continuer"),n<=0&&(clearInterval(a),d&&(d.disabled=!1,d.style.cssText=d.style.cssText.replace("rgba(255,255,255,0.15)","#1A6B3C").replace("rgba(255,255,255,0.4)","#fff").replace("default","pointer")),d==null||d.addEventListener("click",()=>{t.remove(),e(!0)},{once:!0}))},1e3)})}function ro(){return new Promise((e,t)=>{if(typeof window.propeller>"u")return console.warn("[Ad] PropellerAds non chargé → simulation"),e(ci());window.propeller.push({zone_id:on,onComplete:()=>e(!0),onSkip:()=>t(new Error("Publicité ignorée")),onError:()=>t(new Error("Erreur publicité"))})})}function ao(){return new Promise((e,t)=>{if(typeof window.adsbygoogle>"u")return console.warn("[Ad] AdSense non chargé → simulation"),e(ci());try{(window.adsbygoogle=window.adsbygoogle||[]).push({google_ad_client:"ca-pub-VOTRE_PUBLISHER_ID",google_ad_slot:on,google_ad_format:"rewarded",on_reward:()=>e(!0),on_dismiss:()=>t(new Error("Publicité fermée"))})}catch(n){t(n)}})}async function so(e,{state:t,navigate:n,toast:a,refreshProfile:c}){var m,y;const{data:d}=await $.from("users").select("*").eq("id",t.user.id).single();d&&(t.profile=d);let s=Array.isArray((m=t.profile)==null?void 0:m.pending_boosters)?[...t.profile.pending_boosters]:[];if(!s.length){await $.from("users").update({onboarding_done:!0}).eq("id",t.user.id),n("home");return}let l=null;try{const o=(await Xi()).find(v=>(v.name||"").toLowerCase().includes("new player"));o&&(l=Qi(o))}catch(z){console.warn('[Onboarding] Config "Booster (new player)" introuvable, fallback taux par défaut',z)}const p=s.length;let i=0;e.innerHTML=`
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(z=>{z.addEventListener("click",()=>{const o=z.dataset.id,v=d.find(T=>T.id===o);if(!v)return;const L=s.findIndex(T=>T.gc_type===v.gc_type);L>-1?s.splice(L,1):s.length<3&&s.push(v),i()})}),(f=e.querySelector("#gc-launch"))==null||f.addEventListener("click",()=>{r&&p(s)}),(m=e.querySelector("#gc-no-gc"))==null||m.addEventListener("click",()=>p([])),(y=e.querySelector("#gc-reset"))==null||y.addEventListener("click",()=>{s.length&&(s=[],i())})}i()}function co(e){var a;const t=((a=e==null?void 0:e.state)==null?void 0:a.params)||{},n=t.matchMode||"vs_ai_easy";return n==="friend"?`Match vs ${t.friendName||"Ami"}`:n==="random"?"Match vs Random":`Match vs IA — ${n.replace("vs_ai_","").toUpperCase()}`}async function po(e,t,n){const{state:a,navigate:c}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await $.from("decks").select("id,name,is_active,formation").eq("owner_id",a.profile.id).order("created_at",{ascending:!1});if(!d||d.length===0){at(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>c("decks"));return}const s=d.map(r=>r.id),{data:l}=await $.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",s).order("slot_order");let p=0;function i(){var T,C,R,F,j;const r=d[p],f=(l||[]).filter(N=>N.deck_id===r.id),m=f.filter(N=>{var ee;return N.is_starter&&((ee=N.card)==null?void 0:ee.player)}).map(N=>ii(N.card,N.position)),y=f.find(N=>{var ee;return((ee=N.card)==null?void 0:ee.card_type)==="formation"}),z=r.formation||((T=y==null?void 0:y.card)==null?void 0:T.formation)||"4-4-2",o=m.length>=11?mt(m,z):null,v=m.length>=11;Xe(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${d.map((N,ee)=>`<div style="width:7px;height:7px;border-radius:50%;background:${ee===p?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
    ...
    </div>`,function(){const ee=e.querySelector(".deck-preview-wrap svg");ee&&(ee.removeAttribute("width"),ee.removeAttribute("height"),ee.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ee.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(C=document.getElementById("prev-deck"))==null||C.addEventListener("click",()=>{p>0&&(p--,i())}),(R=document.getElementById("next-deck"))==null||R.addEventListener("click",()=>{p<d.length-1&&(p++,i())}),(F=document.getElementById("validate-deck"))==null||F.addEventListener("click",()=>{if(!v)return;const N=t.state.params||{};t.navigate("match",{...N,matchMode:N.matchMode||n,deckId:r.id})}),(j=document.getElementById("cancel-deck-select"))==null||j.addEventListener("click",()=>{He(e),c("home")});const L=document.getElementById("deck-swipe-zone");if(L){let N=0,ee=0;L.addEventListener("touchstart",le=>{N=le.touches[0].clientX,ee=le.touches[0].clientY},{passive:!0}),L.addEventListener("touchend",le=>{const re=le.changedTouches[0].clientX-N,M=le.changedTouches[0].clientY-ee;Math.abs(re)<40||Math.abs(re)<Math.abs(M)||(re<0&&p<d.length-1?(p++,i()):re>0&&p>0&&(p--,i()))},{passive:!0})}}i()}function rn(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function Ve(e,t=44,n=58){if(!e)return`<div style="width:${t}px;height:${n}px;border:1.5px dashed rgba(255,255,255,0.2);border-radius:5px"></div>`;const a=typeof e.portrait=="string"&&e.portrait.startsWith("http")?e.portrait:ze(e),c=rn(e),d=e._line||e.job||"MIL",s=gt[d]||"#555",l={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",p=e.note!==void 0?Number(e.note)||0:(Number(we(e,d))||0)+(e.boost||0),i=an(e==null?void 0:e.country_code),r=Math.round(n*.19),f=Math.round(n*.25),m=n-Math.round(n*.19)-Math.round(n*.25),y=e!=null&&e.used?.28:1;return`<div style="width:${t}px;height:${n}px;border-radius:5px;border:2px solid ${l};background:${s};position:relative;overflow:hidden;flex-shrink:0;opacity:${y}">
    <div style="position:absolute;top:0;left:0;right:0;height:${r}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:center;z-index:2">
  ...
  </div>`}function it(e,t,n){if(!(e!=null&&e.length))return"";const a=e.slice(0,5);let c='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return a.forEach((d,s)=>{if(c+=Ve(d,40,52),s<a.length-1){const l=Ze(d,a[s+1]);c+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),n!==void 0&&(c+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${n}</div>`),c+="</div>",c}function Rt(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function an(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function st(e,t,n,a,c=310,d=310,s=[]){var C;const l=Mt[t]||{},p=Bi(t)||ai[t]||[],i={},r=["ATT","MIL","DEF","GK"];for(const R of r)(e[R]||[]).forEach((j,N)=>{i[`${R}${N+1}`]=j});function f(R){const F=l[R];return F?{x:F.x*c,y:F.y*d}:null}let m="";for(const[R,F]of p){const j=f(R),N=f(F);if(!j||!N)continue;const ee=i[R],le=i[F],re=Ze(ee,le);re==="#00ff88"||re==="#FFD700"?(m+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${N.x.toFixed(1)}" y2="${N.y.toFixed(1)}"
        stroke="${re}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,m+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${N.x.toFixed(1)}" y2="${N.y.toFixed(1)}"
        stroke="${re}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):m+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${N.x.toFixed(1)}" y2="${N.y.toFixed(1)}"
        stroke="${re}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const y=48,z=64,o=13,v=16,L={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[R,F]of Object.entries(i)){const j=f(R);if(!j||!F)continue;const N=R.replace(/[0-9]/g,""),ee=gt[N]||"#555",le=s.includes(F.cardId),re=n==="attack"&&(["MIL","ATT"].includes(N)||le)&&!F.used||n==="defense"&&["GK","DEF","MIL"].includes(N)&&!F.used,M=a.includes(F.cardId);let Y;n==="attack"?Y=le?Math.max(1,Number(F.note_a)||0):N==="MIL"?Number(F.note_m)||0:Number(F.note_a)||0:n==="defense"?Y=N==="GK"?Number(F.note_g)||0:N==="MIL"?Number(F.note_m)||0:Number(F.note_d)||0:Y=Number(N==="GK"?F.note_g:N==="DEF"?F.note_d:N==="MIL"?F.note_m:F.note_a)||0,Y=Y+(F.boost||0);const ie=(j.x-y/2).toFixed(1),q=(j.y-z/2).toFixed(1),Q=L[F==null?void 0:F.rarity]||L.normal;if(F.used){const U=typeof import.meta<"u"&&"/manager-wars/"||"/",D=`${typeof window<"u"&&((C=window.location)==null?void 0:C.origin)||""}${U}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");m+=`<rect x="${ie}" y="${q}" width="${y}" height="${z}" rx="5" fill="#161616"/>`,m+=`<image href="${D}" xlink:href="${D}" x="${ie}" y="${q}" width="${y}" height="${z}" preserveAspectRatio="xMidYMid slice"/>`,m+=`<rect x="${ie}" y="${q}" width="${y}" height="${z}" rx="5" fill="none" stroke="${Q}" stroke-width="2" opacity="0.7"/>`,m+=`<rect x="${ie}" y="${q}" width="${y}" height="${z}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${F.cardId}" data-role="${N}" style="cursor:pointer"/>`;continue}const ae=M?.45:1,pe=M?"#FFD700":Q,he=M?3:(F==null?void 0:F.rarity)==="legende"||(F==null?void 0:F.rarity)==="légende"||(F==null?void 0:F.rarity)==="pepite"||(F==null?void 0:F.rarity)==="pépite"?2.5:2,xe=z-o-v;m+=`<defs><clipPath id="cp-${R}"><rect x="${ie}" y="${(j.y-z/2+o).toFixed(1)}" width="${y}" height="${xe}"/></clipPath></defs>`,m+=`<rect x="${ie}" y="${q}" width="${y}" height="${z}" rx="5" fill="${ee}" opacity="${ae}"/>`;const ge=ze(F);ge&&(m+=`<image href="${ge}" xlink:href="${ge}" x="${ie}" y="${(j.y-z/2+o).toFixed(1)}" width="${y}" height="${xe}" clip-path="url(#cp-${R})" preserveAspectRatio="xMidYMin slice"/>`),m+=`<rect x="${ie}" y="${q}" width="${y}" height="${o}" rx="4" fill="rgba(255,255,255,0.92)"/>`,m+=`<text x="${j.x.toFixed(1)}" y="${(j.y-z/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(F.name||"").slice(0,9)}</text>`;const O=(j.y+z/2-v).toFixed(1);m+=`<rect x="${ie}" y="${O}" width="${y}" height="${v}" fill="rgba(255,255,255,0.92)"/>`;{const U=Rt(F.country_code);U?m+=`<image href="${U}" xlink:href="${U}" x="${(j.x-20).toFixed(1)}" y="${(j.y+z/2-v+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:m+=`<text x="${(j.x-13).toFixed(1)}" y="${(j.y+z/2-v/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${an(F.country_code)}</text>`,m+=`<text x="${j.x.toFixed(1)}" y="${(j.y+z/2-v/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${Y}</text>`;const S=rn(F);S?m+=`<image href="${S}" xlink:href="${S}" x="${(j.x+y/2-14).toFixed(1)}" y="${(j.y+z/2-v+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:F.clubName&&(m+=`<text x="${(j.x+14).toFixed(1)}" y="${(j.y+z/2-v/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(F.clubName||"").slice(0,3).toUpperCase()}</text>`),F.boost&&(m+=`<rect x="${(j.x+y/2-12).toFixed(1)}" y="${(j.y-z/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,m+=`<text x="${(j.x+y/2-5).toFixed(1)}" y="${(j.y-z/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${F.boost}</text>`)}m+=`<rect x="${ie}" y="${q}" width="${y}" height="${z}" rx="5" fill="${M?"rgba(255,255,255,0.12)":"none"}" stroke="${pe}" stroke-width="${he}" opacity="${ae}"/>`,re&&(m+=`<rect x="${ie}" y="${q}" width="${y}" height="${z}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${M?"selected":""}" data-card-id="${F.cardId}" data-role="${N}" style="cursor:pointer"/>`)}const T=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-T} ${-T} ${c+T*2} ${d+T*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${m}
  ...
  </div>`}async function fi(e,t,n,a){var F;const{state:c,navigate:d,toast:s}=t;Xe(e);const l=c.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!l.deckId)return po(e,t,n);const p=l.deckId;let i,r,f,m;try{const j=await Promise.all([$.from("decks").select("formation,name").eq("id",p).single(),$.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",p).order("slot_order")]);i=j[0].data,f=j[0].error,r=j[1].data,m=j[1].error}catch(j){console.error("[Match] Exception chargement deck:",j),at(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>d("home"));return}if(f||m){console.error("[Match] Erreur Supabase:",f||m),at(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>d("home"));return}const y=(r||[]).filter(j=>{var N;return j.is_starter&&((N=j.card)==null?void 0:N.player)}).map(j=>ii(j.card,j.position)),z=(r||[]).filter(j=>{var N;return!j.is_starter&&((N=j.card)==null?void 0:N.player)}).map(j=>ii(j.card,j.position));if(y.length<11){at(e,"⚠️",`Deck incomplet (${y.length}/11).`,"Compléter",()=>d("decks"));return}const o=(r||[]).find(j=>{var N;return((N=j.card)==null?void 0:N.card_type)==="formation"}),v=(i==null?void 0:i.formation)||((F=o==null?void 0:o.card)==null?void 0:F.formation)||"4-4-2",{data:L,error:T}=await $.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",c.profile.id).eq("card_type","game_changer"),{data:C}=await $.from("gc_definitions").select("*").eq("is_active",!0),R=(L||[]).map(j=>({...j,_gcDef:(C==null?void 0:C.find(N=>N.name===j.gc_type||N.id===j.gc_definition_id))||null}));a({deckId:p,formation:v,starters:y,subsRaw:z,gcCardsEnriched:R,gcDefs:C||[]})}async function uo(e,t){const{state:n}=t,c=(n.params||{}).matchMode||"vs_ai_easy",d=c.replace("vs_ai_",""),s=c;await fi(e,t,c,async({deckId:l,formation:p,starters:i,subsRaw:r,gcCardsEnriched:f,gcDefs:m})=>{try{const y=mt(i,p),z=await fo(p,d),o=async v=>{try{const{data:L,error:T}=await $.from("matches").insert({home_id:n.profile.id,away_id:null,mode:s,home_deck_id:l,status:"in_progress"}).select().single();if(T){console.error("[MatchIA] Erreur création match:",T),at(e,"⚠️","Impossible de créer le match ("+T.message+").","Retour",()=>t.navigate("home"));return}const C={gcDefs:m||[],matchId:L==null?void 0:L.id,mode:s,difficulty:d,formation:p,homeTeam:y,aiTeam:z,homeSubs:r,subsUsed:0,maxSubs:Math.min(r.length,3),homeScore:0,aiScore:0,gcCards:v,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:n.profile.club_name||"Vous"};mo(e,C,t)}catch(L){console.error("[MatchIA] Exception launchMatch:",L),at(e,"⚠️","Erreur au lancement du match : "+L.message,"Retour",()=>t.navigate("home"))}};if(!f.length){o([]);return}ui(e,f,o)}catch(y){console.error("[MatchIA] Exception setup:",y),at(e,"⚠️","Erreur de préparation du match : "+y.message,"Retour",()=>t.navigate("home"))}})}async function fo(e,t){var s,l;const{data:n}=await $.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!n||n.length<11)return go(e);const a=ft[e]||ft["4-4-2"],c={GK:[],DEF:[],MIL:[],ATT:[]},d=[...n];for(const p of["GK","DEF","MIL","ATT"]){const i=d.filter(z=>z.job===p),r=d.filter(z=>z.job!==p),f=[...i,...r],m=[];for(let z=0;z<a[p];z++){const o=f[z]||d[z];o&&m.push({cardId:"ai-"+o.id+"-"+z,id:o.id,firstname:o.firstname,name:o.surname_encoded,country_code:o.country_code,club_id:o.club_id,job:o.job,job2:o.job2,note_g:Number(o.note_g)||0,note_d:Number(o.note_d)||0,note_m:Number(o.note_m)||0,note_a:Number(o.note_a)||0,rarity:o.rarity,skin:o.skin,hair:o.hair,hair_length:o.hair_length,clubName:((s=o.clubs)==null?void 0:s.encoded_name)||null,clubLogo:((l=o.clubs)==null?void 0:l.logo_url)||null,boost:0,used:!1,_line:p})}const y=ot(m.length);m.forEach((z,o)=>{z._col=y[o]}),c[p]=m}return c}function go(e){const t=ft[e]||ft["4-4-2"],n={GK:[],DEF:[],MIL:[],ATT:[]},a=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let c=0;for(const d of["GK","DEF","MIL","ATT"]){const s=[];for(let p=0;p<t[d];p++){const i=3+Math.floor(Math.random()*5);s.push({cardId:"fake-"+c,id:"fake-"+c,firstname:"IA",name:a[c%a.length],country_code:"XX",club_id:null,job:d,job2:null,note_g:d==="GK"?i:2,note_d:d==="DEF"?i:2,note_m:d==="MIL"?i:2,note_a:d==="ATT"?i:2,rarity:"normal",boost:0,used:!1,_line:d}),c++}const l=ot(s.length);s.forEach((p,i)=>{p._col=l[i]}),n[d]=s}return n}function mo(e,t,n){e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>xo(e,t,n),5e3)}function xo(e,t,n){const a=t.homeTeam.MIL||[],c=t.aiTeam.MIL||[];function d(o){return o.reduce((v,L)=>v+we(L,"MIL"),0)}function s(o){let v=0;for(let L=0;L<o.length-1;L++){const T=Ze(o[L],o[L+1]);T==="#00ff88"?v+=2:T==="#FFD700"&&(v+=1)}return v}const l=d(a)+s(a),p=d(c)+s(c),i=l>=p;function r(o,v,L,T){return`<div id="duel-row-${T}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${o.map((C,R)=>{const F=R<o.length-1?Ze(C,o[R+1]):null,j=!F||F==="#ff3333"||F==="#cc2222",N=F==="#00ff88"?"+2":F==="#FFD700"?"+1":"";return`
          <div class="duel-card duel-card-${T}" data-idx="${R}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease, transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
            ${Ve({...C,note:we(C,"MIL"),_line:"MIL"},58,78)}
          </div>
          ${R<o.length-1?`<div class="duel-link duel-link-${T}" data-idx="${R}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${j?"rgba(255,255,255,0.12)":F};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${j?"none":`0 0 8px ${F}`}">
            ${N?`<span style="position:absolute;top:-13px;left:50%;transform:translateX(-50%);font-size:8px;font-weight:900;color:${F}">${N}</span>`:""}
          </div>`:""}
          `}).join("")}
      </div>
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
  </div>`;const f=()=>{const o=(v,L)=>e.querySelectorAll(v).forEach((T,C)=>{setTimeout(()=>{T.style.opacity="1",T.style.transform="translateY(0) scale(1)"},L+C*90)});o(".duel-card-home",150),o(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((v,L)=>{setTimeout(()=>{v.style.opacity="1"},L*70)}),900),setTimeout(()=>{const v=e.querySelector("#vs-label");v&&(v.style.opacity="1",v.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(L=>L.style.opacity="1")},1250),setTimeout(()=>{m("score-home",l,800),m("score-ai",p,800)},1500)};function m(o,v,L){const T=document.getElementById(o);if(!T)return;const C=performance.now(),R=F=>{const j=Math.min(1,(F-C)/L);T.textContent=Math.round(v*(1-Math.pow(1-j,3))),j<1?requestAnimationFrame(R):T.textContent=v};requestAnimationFrame(R)}requestAnimationFrame(f),t.attacker=i?"home":"ai";const y=i?pi():null;i&&(t.boostCard={value:y}),t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:a.map(o=>({name:o.name,note:we(o,"MIL"),portrait:ze(o),job:o.job,country_code:o.country_code,rarity:o.rarity,clubName:o.clubName,clubLogo:o.clubLogo})),aiPlayers:c.map(o=>({name:o.name,note:we(o,"MIL"),portrait:ze(o),job:o.job,country_code:o.country_code,rarity:o.rarity,clubName:o.clubName,clubLogo:o.clubLogo})),homeTotal:l,aiTotal:p,text:`Duel milieu : ${t.clubName} ${l} – ${p} IA → ${i?t.clubName+" attaque":"IA attaque"}`});const z=()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",Se(e,t,n),t.attacker==="ai"&&setTimeout(()=>ni(e,t,n),800)};setTimeout(()=>{const o=document.getElementById("score-home"),v=document.getElementById("score-ai"),L=document.getElementById(i?"duel-row-home":"duel-row-ai"),T=document.getElementById(i?"duel-row-ai":"duel-row-home"),C=i?o:v,R=i?v:o;C&&(C.style.fontSize="80px",C.style.color=i?"#FFD700":"#ff6b6b",C.style.animation="duelPulse .5s ease"+(i?", duelGlow 1.5s ease infinite .5s":"")),R&&(R.style.opacity="0.25"),setTimeout(()=>{L&&(L.style.transformOrigin="center",L.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",L.style.zIndex="5"),setTimeout(()=>{var j;const F=document.getElementById("duel-shock");if(F){const N=(j=T||L)==null?void 0:j.getBoundingClientRect(),ee=e.querySelector(".match-screen").getBoundingClientRect();N&&(F.style.top=N.top-ee.top+N.height/2+"px"),F.style.animation="shockwave .5s ease-out forwards"}T&&(T.style.transformOrigin="center",T.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var j;const F=document.getElementById("duel-finale");F&&(F.innerHTML=`
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,F.style.transition="opacity .45s ease",F.style.opacity="1",F.style.pointerEvents="auto",(j=document.getElementById("start-match-btn"))==null||j.addEventListener("click",z))},600)},700)},2800)}function yo(e){if(e.type==="duel"&&(e.homeTotal!=null||e.aiTotal!=null)){const t=(e.homeTotal||0)>=(e.aiTotal||0);return`
    ...
    <div style="display:flex;align-items:center;gap:4px;${t?"flex-direction:row-reverse":""};background:rgba(255,255,255,0.04);border-radius:8px;padding:5px 8px;border:1px solid rgba(255,255,255,0.07)">
      ...
      ${nt(e.inPlayer||{})}
    </div>`}return`<div style="font-size:11px;color:${e.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${e.type==="goal"?700:400};padding:3px 2px">${e.text||""}</div>`}function Se(e,t,n){var C,R,F,j,N,ee,le,re;const a=t.selected.map(M=>M.cardId),c=t.usedSubIds||[],d=t.homeSubs.filter(M=>!c.includes(M.cardId)),l=Object.values(t.homeTeam).flat().filter(M=>M.used).length>0&&d.length>0&&t.subsUsed<t.maxSubs,p=!["GK","DEF","MIL","ATT"].some(M=>(t.aiTeam[M]||[]).some(Y=>!Y.used)),i=[...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(M=>!M.used),r=t.phase==="attack"&&p&&i.length===0?[...t.homeTeam.GK||[],...t.homeTeam.DEF||[]].filter(M=>!M.used).map(M=>M.cardId):[];t.log[t.log.length-1];const f=t.phase==="ai-attack"||t.phase==="ai-defense",m=t.phase==="attack",y=t.phase==="defense",z=t.phase==="finished",o=t.gcCards.filter(M=>!t.usedGc.includes(M.id)),v=t.boostCard&&!t.boostUsed;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column",e.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const Y=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
          ...
          </div>`}const M=t.log[t.log.length-1];return M?'<div style="padding:2px 4px">'+yo(M)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    ${(()=>{const M=window.innerWidth>=700,Y=(O,U,S)=>{var qe,Re;const D=(t.gcDefs||[]).find(k=>k.name===O.gc_type),K={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[D==null?void 0:D.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",ne={purple:"#b06ce0",light_blue:"#00d4ef"}[D==null?void 0:D.color]||"#b06ce0",te=(D==null?void 0:D.name)||O.gc_type,_e=(D==null?void 0:D.effect)||((qe=Ne[O.gc_type])==null?void 0:qe.desc)||"",$e=D!=null&&D.image_url?`/manager-wars/icons/${D.image_url}`:null,Ee=((Re=Ne[O.gc_type])==null?void 0:Re.icon)||"⚡",Me=Math.round(S*.22),Ae=Math.round(S*.22),Te=S-Me-Ae,ke=te.length>12?7:9;return`<div class="gc-mini" data-gc-id="${O.id}" data-gc-type="${O.gc_type}"
          style="box-sizing:border-box;width:${U}px;height:${S}px;border-radius:10px;border:2px solid ${ne};background:${K};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
        ...
        </div>`},ie=(O,U)=>{var S;return`<div id="boost-card"
  ...
  </div>`;function L(){const M=e.querySelector(".match-screen");if(!M)return;const Y=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);M.style.bottom="auto",M.style.height=Y+"px",M.style.minHeight=Y+"px",M.style.maxHeight=Y+"px",M.style.overflow="hidden";const ie=e.querySelector("#mobile-action-bar"),q=e.querySelector("#mobile-play-area");ie&&q&&(q.style.paddingBottom=ie.offsetHeight+"px")}if(L(),setTimeout(L,120),setTimeout(L,400),setTimeout(L,1e3),t._vvBound||(t._vvBound=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",L),window.visualViewport.addEventListener("scroll",L)),window.addEventListener("resize",L)),function(){const Y=e.querySelector(".terrain-wrapper svg");Y&&(Y.removeAttribute("width"),Y.removeAttribute("height"),Y.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",Y.setAttribute("viewBox","-26 -26 352 352"),Y.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),t._resizeBound||(t._resizeBound=!0,window.addEventListener("resize",()=>{const M=e.querySelector(".terrain-wrapper svg");M&&(M.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0")})),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase==="attack"||t.phase==="defense"){let M=!1,Y=30;const ie=()=>document.getElementById("match-timer"),q=()=>{const Q=ie();if(!Q)return;const ae=String(Math.floor(Y/60)).padStart(2,"0"),pe=String(Y%60).padStart(2,"0");Q.textContent=` ${ae}:${pe}`,Q.style.color=M?"#ff2222":"#ff9500",Q.style.fontWeight="900"};q(),t._timerInt=setInterval(()=>{if(Y--,Y<0)if(!M)M=!0,Y=15,q();else{clearInterval(t._timerInt),t._timerInt=null,t.homeScore=0,t.aiScore=3;const Q=document.createElement("div");Q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",Q.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(Q),setTimeout(()=>{Q.remove(),Tt(e,t,n)},2500)}else q()},1e3)}(C=document.getElementById("match-quit"))==null||C.addEventListener("click",()=>{He(e),confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,Tt(e,t,n))}),(R=document.getElementById("view-ai"))==null||R.addEventListener("click",()=>To(t,n)),(F=document.getElementById("toggle-history"))==null||F.addEventListener("click",()=>{var M;(M=document.getElementById("match-history-panel"))==null||M.classList.add("open")}),(j=document.getElementById("close-history"))==null||j.addEventListener("click",()=>{var M;(M=document.getElementById("match-history-panel"))==null||M.classList.remove("open")}),(N=document.getElementById("btn-action"))==null||N.addEventListener("click",()=>{t.selected.length!==0&&(m?bo(e,t,n):y&&vo(e,t,n))}),(ee=document.getElementById("btn-results"))==null||ee.addEventListener("click",()=>Tt(e,t,n)),e.querySelectorAll(".match-slot-hit").forEach(M=>{M.addEventListener("click",()=>ho(M,t,e,n))}),e.querySelectorAll(".match-used-hit").forEach(M=>{M.addEventListener("click",()=>Jt(e,t,n,null,M.dataset.cardId))}),e.querySelectorAll(".gc-mini").forEach(M=>{M.addEventListener("click",()=>$o(M.dataset.gcId,M.dataset.gcType,e,t,n))}),(le=document.getElementById("boost-card"))==null||le.addEventListener("click",()=>Ao(e,t,n)),e.querySelectorAll(".sub-btn-col").forEach(M=>{M.addEventListener("click",()=>Jt(e,t,n,M.dataset.subId))}),(re=document.getElementById("sub-btn-main"))==null||re.addEventListener("click",()=>Jt(e,t,n))}function ho(e,t,n,a){const c=e.dataset.cardId,d=e.dataset.role,s=t.selected.findIndex(l=>l.cardId===c);if(s!==-1)t.selected.splice(s,1);else{if(t.selected.length>=3){a.toast("Maximum 3 joueurs","error");return}const l=(t.homeTeam[d]||[]).find(p=>p.cardId===c);l&&t.selected.push({...l,_role:d,_line:d})}Se(n,t,a)}function gi(e,t,n){e.matchId&&$.from("matches").update({last_player_id:n}).eq("id",e.matchId).then(()=>{})}function bo(e,t,n){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),gi(t,n,n.state.profile.id);const a=!["GK","DEF","MIL","ATT"].some(s=>(t.aiTeam[s]||[]).some(l=>!l.used)),c=t.selected.map(s=>{const l=(t.homeTeam[s._role]||[]).find(i=>i.cardId===s.cardId)||s,p=a&&["GK","DEF"].includes(s._role);return{...l,_line:s._role,...p?{note_a:Math.max(1,Number(l.note_a)||0)}:{}}}),d=Ut(c,t.modifiers.home);t.pendingAttack={...d,players:[...c],side:"home"},t.selected.forEach(s=>{const l=(t.homeTeam[s._role]||[]).find(p=>p.cardId===s.cardId);l&&(l.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${d.total} (base ${d.base}${d.links?` +${d.links} liens`:""}) — ${t.selected.map(s=>s.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",Se(e,t,n),setTimeout(()=>wo(e,t,n),1200)}function vo(e,t,n){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),gi(t,n,n.state.profile.id);const a=t.selected.map(l=>({...(t.homeTeam[l._role]||[]).find(i=>i.cardId===l.cardId)||l,_line:l._role})),c=Kt(a,t.modifiers.home);t.selected.forEach(l=>{const p=(t.homeTeam[l._role]||[]).find(i=>i.cardId===l.cardId);p&&(p.used=!0)});const d=Vt(t.pendingAttack.total,c.total,t.modifiers.home),s={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(l=>({name:l.name,note:l._line==="MIL"?l.note_m:l.note_a,portrait:ze(l),job:l.job,country_code:l.country_code,rarity:l.rarity,clubName:l.clubName,clubLogo:l.clubLogo})),homePlayers:t.selected.map(l=>{const p=(t.homeTeam[l._role]||[]).find(i=>i.cardId===l.cardId)||l;return{name:p.name,note:(p._line==="GK"?Number(p.note_g)||0:p._line==="MIL"?Number(p.note_m)||0:Number(p.note_d)||0)+(p.boost||0),portrait:ze(p),job:p.job,country_code:p.country_code,rarity:p.rarity,clubName:p.clubName,clubLogo:p.clubLogo}}),homeTotal:c.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(d.shielded)s.text="🛡️ Bouclier ! But annulé.",t.log.push(s);else if(d.goal){t.aiScore++,s.isGoal=!0,s.homeScored=!1,s.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${c.total})`,t.log.push(s),t.selected=[],t.modifiers.home={},t.pendingAttack=null,Se(e,t,n),Ot(s.aiPlayers,t.homeScore,t.aiScore,!1,()=>{pt(e,t,n,"home-attack")});return}else s.text=`🧤 Défense réussie ! (${c.total} ≥ ${t.pendingAttack.total})`,t.log.push(s);t.selected=[],t.modifiers.home={},t.pendingAttack=null,pt(e,t,n,"home-attack")}function ni(e,t,n){gi(t,n,null);const a=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]],c=Pi(a,"attack",t.difficulty);if(!c.length){Gt(e,t,n);return}const d=Ut(c,t.modifiers.ai);t.pendingAttack={...d,players:c,side:"ai"},c.forEach(i=>{i.used=!0}),t.log.push({text:`🤖 IA attaque : ${d.total} (${c.map(i=>i.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const s=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(i=>!i.used),p=(t.homeSubs||[]).filter(i=>!(t.usedSubIds||[]).includes(i.cardId)).length>0&&t.subsUsed<t.maxSubs;if(s.length===0&&!p){t.aiScore++;const i={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:c.map(r=>({name:r.name,note:r._line==="MIL"?r.note_m:r.note_a,portrait:ze(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo})),aiTotal:d.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(i),t.pendingAttack=null,Se(e,t,n),Ot(i.aiPlayers,t.homeScore,t.aiScore,!1,()=>{pt(e,t,n,"home-attack")});return}t.phase="defense",Se(e,t,n)}function wo(e,t,n){var i,r;const a=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],c=Pi(a,"defense",t.difficulty);if(!["GK","DEF","MIL","ATT"].flatMap(f=>(t.aiTeam[f]||[]).filter(m=>!m.used)).length){t.homeScore++;const m={type:"duel",isGoal:!0,homeScored:!0,homePlayers:(((i=t.pendingAttack)==null?void 0:i.players)||[]).map(y=>({name:y.name,note:we(y,y._line||y.job),portrait:ze(y),job:y.job,country_code:y.country_code,rarity:y.rarity,clubName:y.clubName,clubLogo:y.clubLogo})),homeTotal:((r=t.pendingAttack)==null?void 0:r.total)||0,aiTotal:0,text:"⚽ BUT ! L'IA n'a plus de joueurs — but automatique !"};t.log.push(m),t.modifiers.ai={},t.pendingAttack=null,Se(e,t,n),Ot(m.homePlayers,t.homeScore,t.aiScore,!0,()=>{pt(e,t,n,"ai-attack")});return}const s=c.length>0?Kt(c,t.modifiers.ai).total:0;c.forEach(f=>{f.used=!0});const l=Vt(t.pendingAttack.total,s,t.modifiers.ai),p={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(f=>({name:f.name,note:f._line==="MIL"?f.note_m:f.note_a,portrait:ze(f),job:f.job,country_code:f.country_code,rarity:f.rarity,clubName:f.clubName,clubLogo:f.clubLogo})),aiPlayers:c.map(f=>({name:f.name,note:f._line==="GK"?f.note_g:f._line==="MIL"?f.note_m:f.note_d,portrait:ze(f),job:f.job,country_code:f.country_code,rarity:f.rarity,clubName:f.clubName,clubLogo:f.clubLogo})),homeTotal:t.pendingAttack.total,aiTotal:s,isGoal:!1,homeScored:!1,text:""};if(l.shielded)p.text="🛡️ Bouclier IA !",t.log.push(p);else if(l.goal){t.homeScore++,p.isGoal=!0,p.homeScored=!0,p.text=`⚽ BUT ! (${t.pendingAttack.total} > ${s})`,t.log.push(p),t.modifiers.ai={},t.pendingAttack=null,Se(e,t,n),Ot(p.homePlayers,t.homeScore,t.aiScore,!0,()=>{pt(e,t,n,"ai-attack")});return}else p.text=`🧤 IA défend (${s} ≥ ${t.pendingAttack.total})`,t.log.push(p);t.modifiers.ai={},t.pendingAttack=null,pt(e,t,n,"ai-attack")}function pt(e,t,n,a){if(t.round++,sn(t)){Tt(e,t,n);return}if(a==="home-attack"){if(![...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(d=>!d.used).length){if(![...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(l=>!l.used).length){Gt(e,t,n);return}if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(l=>!l.used).length){Gt(e,t,n);return}setTimeout(()=>ni(e,t,n),100);return}t.phase="attack",Se(e,t,n)}else{if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(d=>!d.used).length){Gt(e,t,n);return}t.phase="ai-attack",Se(e,t,n),setTimeout(()=>ni(e,t,n),800)}}function sn(e){const t=["MIL","ATT","GK","DEF"].some(a=>(e.homeTeam[a]||[]).some(c=>!c.used)),n=["MIL","ATT","GK","DEF"].some(a=>(e.aiTeam[a]||[]).some(c=>!c.used));return!t&&!n}function Gt(e,t,n){sn(t)?Tt(e,t,n):(t.phase="attack",Se(e,t,n))}function _o(e,t,n){const a=document.createElement("div");a.style.cssText=`
  ...
  `,document.body.appendChild(a);let r=!1;const f=()=>{r||(r=!0,a.remove(),n())};a.addEventListener("click",f),setTimeout(f,2e3)}function $t(e,t="rgba(0,0,0,0.8)"){const n=document.createElement("div");n.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,n.textContent=e,document.body.appendChild(n),setTimeout(()=>n.remove(),2200)}function Jt(e,t,n,a=null,c=null){var y,z;if(t.phase!=="attack"){$t("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){$t(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const d=Object.entries(t.homeTeam).flatMap(([o,v])=>(v||[]).filter(L=>L.used).map(L=>({...L,_line:L._line||o}))),s=t.homeSubs.filter(o=>!t.usedSubIds.includes(o.cardId));if(!d.length){$t("Aucun joueur utilisé à remplacer");return}if(!s.length){$t("Aucun remplaçant disponible");return}let l=Math.max(0,d.findIndex(o=>o.cardId===c));const p=((y=d[l])==null?void 0:y._line)||((z=d[l])==null?void 0:z.job);let i=a?Math.max(0,s.findIndex(o=>o.cardId===a)):Math.max(0,s.findIndex(o=>o.job===p)),r=!1;const f=document.createElement("div");f.id="sub-overlay",f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function m(){var F,j,N,ee,le,re;const o=d[l],v=s[i],L=Math.min(130,Math.round((window.innerWidth-90)/2)),T=Math.round(L*1.35),C=M=>`background:rgba(255,255,255,0.12);border:none;color:${M?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${M?"default":"pointer"};flex-shrink:0`;f.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <div>${v?Ve({...v,used:!1,boost:0},L,T):"<div>—</div>"}</div>
        <button id="in-down" style="${C(i>=s.length-1)}" ${i>=s.length-1?"disabled":""}>▼</button>
      ...
      <div id="out-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
        ...
        <div>${o?Ve({...o,used:!1,boost:0},L,T):"<div>—</div>"}</div>
    ...
    </div>`,(F=f.querySelector("#sub-close"))==null||F.addEventListener("click",()=>f.remove()),(j=f.querySelector("#out-up"))==null||j.addEventListener("click",()=>{l>0&&(l--,m())}),(N=f.querySelector("#out-down"))==null||N.addEventListener("click",()=>{l<d.length-1&&(l++,m())}),(ee=f.querySelector("#in-up"))==null||ee.addEventListener("click",()=>{i>0&&(i--,m())}),(le=f.querySelector("#in-down"))==null||le.addEventListener("click",()=>{i<s.length-1&&(i++,m())});const R=(M,Y,ie,q)=>{const Q=f.querySelector("#"+M);if(!Q)return;let ae=0;Q.addEventListener("touchstart",pe=>{ae=pe.touches[0].clientY},{passive:!0}),Q.addEventListener("touchend",pe=>{const he=pe.changedTouches[0].clientY-ae;if(Math.abs(he)<30)return;const xe=Y();he<0&&xe<q-1?(ie(xe+1),m()):he>0&&xe>0&&(ie(xe-1),m())},{passive:!0})};R("in-panel",()=>i,M=>i=M,s.length),R("out-panel",()=>l,M=>l=M,d.length),(re=f.querySelector("#sub-confirm"))==null||re.addEventListener("click",M=>{if(M.preventDefault(),M.stopPropagation(),r)return;r=!0;const Y=d[l],ie=s[i];if(!Y||!ie)return;let q=null,Q=-1;for(const[pe,he]of Object.entries(t.homeTeam)){const xe=(he||[]).findIndex(ge=>ge.cardId===Y.cardId);if(xe!==-1){q=pe,Q=xe;break}}if(Q===-1||!q){$t("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),f.remove();return}const ae={...ie,_line:q,_col:Y._col||0,used:!1,boost:0};t.homeTeam[q].splice(Q,1,ae),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(ie.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:Y.name,firstname:Y.firstname,note:we(Y,q),portrait:ze(Y),job:Y.job,country_code:Y.country_code,rarity:Y.rarity,clubName:Y.clubName,clubLogo:Y.clubLogo},inPlayer:{name:ie.name,firstname:ie.firstname,note:we(ie,q),portrait:ze(ie),job:ie.job,country_code:ie.country_code,rarity:ie.rarity,clubName:ie.clubName,clubLogo:ie.clubLogo},text:`🔄 ${ie.firstname} ${ie.name} remplace ${Y.firstname} ${Y.name}`}),f.remove(),_o(Y,ie,()=>Se(e,t,n))})}document.body.appendChild(f),m()}function $o(e,t,n,a,c){var z,o;const d=(a.gcDefs||[]).find(v=>v.name===t),s=Ne[t]||{icon:"⚡",desc:"Carte spéciale."},l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[d==null?void 0:d.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",p={purple:"#b06ce0",light_blue:"#00d4ef"}[d==null?void 0:d.color]||"#b06ce0",i=(d==null?void 0:d.name)||t,r=(d==null?void 0:d.effect)||s.desc,f=d!=null&&d.image_url?`/manager-wars/icons/${d.image_url}`:null,m=s.icon||"⚡",y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",y.innerHTML=`
    ...
    </div>`,(i=s.querySelector("#gc-picker-close"))==null||i.addEventListener("click",()=>s.remove()),s.querySelectorAll(".gc-pick-item").forEach(f=>{f.addEventListener("click",()=>{const m=f.dataset.cid,y=e.find(o=>o.cardId===m);if(!y)return;const z=l.findIndex(o=>o.cardId===m);z>-1?l.splice(z,1):l.length<t&&l.push(y),p()})}),(r=s.querySelector("#gc-picker-confirm"))==null||r.addEventListener("click",()=>{s.remove(),d(l)})}p(),document.body.appendChild(s)}const ko={BOOST_STAT:({value:e=1,count:t=1,roles:n=[]},a,c,d)=>{const s=Object.entries(a.homeTeam).filter(([l])=>!n.length||n.includes(l)).flatMap(([l,p])=>p.filter(i=>!i.used).map(i=>({...i,_line:l})));return s.length?(Dt(s,t,`Choisir ${t} joueur(s) à booster (+${e})`,c,a,l=>{l.forEach(p=>{const i=(a.homeTeam[p._line]||[]).find(r=>r.cardId===p.cardId);i&&(i.boost=(i.boost||0)+e,a.log.push({text:`⚡ +${e} sur ${i.name}`,type:"info"}))}),Se(c,a,d)}),!0):(a.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),Se(c,a,d),!0)},DEBUFF_STAT:({value:e=1,count:t=1,roles:n=[],target:a="ai"},c,d,s)=>{const l=a==="home"?c.homeTeam:c.aiTeam,p=a==="ai"?"adverse":"allié",i=Object.entries(l).filter(([r])=>!n.length||n.includes(r)).flatMap(([r,f])=>f.filter(m=>!m.used).map(m=>({...m,_line:r})));return i.length?(Dt(i,t,`Choisir ${t} joueur(s) ${p}(s) à débuffer (-${e})`,d,c,r=>{r.forEach(f=>{const y=((a==="home"?c.homeTeam:c.aiTeam)[f._line]||[]).find(z=>z.cardId===f.cardId);y&&(y.boost=(y.boost||0)-e,c.log.push({text:`🎯 -${e} sur ${y.name}${a==="ai"?" (IA)":""}`,type:"info"}))}),Se(d,c,s)}),!0):(c.log.push({text:`🎯 Aucun joueur ${p} disponible`,type:"info"}),Se(d,c,s),!0)},GRAY_PLAYER:({count:e=1,roles:t=[],target:n="ai"},a,c,d)=>{const s=n==="home"?a.homeTeam:a.aiTeam,l=n==="ai"?"adverse":"allié",p=Object.entries(s).filter(([i])=>!t.length||t.includes(i)).flatMap(([i,r])=>r.filter(f=>!f.used).map(f=>({...f,_line:i})));return p.length?(Dt(p,e,`Choisir ${e} joueur(s) ${l}(s) à exclure`,c,a,i=>{i.forEach(r=>{const m=((n==="home"?a.homeTeam:a.aiTeam)[r._line]||[]).find(y=>y.cardId===r.cardId);m&&(m.used=!0,a.log.push({text:`❌ ${m.name}${n==="ai"?" (IA)":""} exclu !`,type:"info"}))}),Se(c,a,d)}),!0):(a.log.push({text:`❌ Aucun joueur ${l} à exclure`,type:"info"}),Se(c,a,d),!0)},REVIVE_PLAYER:({count:e=1,roles:t=[]},n,a,c)=>{const d=Object.entries(n.homeTeam).filter(([s])=>!t.length||t.includes(s)).flatMap(([s,l])=>l.filter(p=>p.used).map(p=>({...p,_line:s})));return d.length?(Dt(d,e,`Choisir ${e} joueur(s) à ressusciter`,a,n,s=>{s.forEach(l=>{const p=(n.homeTeam[l._line]||[]).find(i=>i.cardId===l.cardId);p&&(p.used=!1,n.log.push({text:`💫 ${p.name} ressuscité !`,type:"info"}))}),Se(a,n,c)}),!0):(n.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),Se(a,n,c),!0)},REMOVE_GOAL:({},e)=>(e.aiScore>0?(e.aiScore--,e.log.push({text:"🚫 Dernier but IA annulé !",type:"info"})):e.log.push({text:"🚫 Aucun but à annuler",type:"info"}),!1),ADD_GOAL_DRAW:({},e)=>(e.homeScore===e.aiScore?(e.homeScore++,e.log.push({text:"🎯 But bonus (match nul) !",type:"info"})):e.log.push({text:"🎯 But bonus : non applicable (pas de match nul)",type:"info"}),!1),ADD_SUB:({value:e=1},t)=>(t.maxSubs=(t.maxSubs||3)+e,t.log.push({text:`🔄 +${e} remplacement(s) débloqué(s)`,type:"info"}),!1),CUSTOM:()=>!1};function Eo(e,t,n,a,c){a.usedGc.push(e);const d=a.gcDefs||[],s=d.find(p=>p.name===t)||d.find(p=>{var i;return((i=p.name)==null?void 0:i.toLowerCase().trim())===(t==null?void 0:t.toLowerCase().trim())});let l=!1;if(s!=null&&s.effect_type&&s.effect_type!=="CUSTOM"){const p=ko[s.effect_type];p?p(s.effect_params||{},a,n,c)||(l=!0):(c.toast(`Effet "${s.effect_type}" non implémenté`,"error"),l=!0)}else{switch(t){case"Double attaque":a.modifiers.home.doubleAttack=!0,a.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":a.modifiers.home.shield=!0,a.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const p=Object.entries(a.homeTeam).flatMap(([i,r])=>(r||[]).filter(f=>f.used).map(f=>({...f,_line:i})));p.length?(p[0].used=!1,a.log.push({text:`💫 ${p[0].name} ressuscité !`,type:"info"})):a.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":a.modifiers.ai.stolenNote=(a.modifiers.ai.stolenNote||0)+1,a.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const p=[...a.aiTeam.ATT||[],...a.aiTeam.MIL||[]].filter(i=>!i.used);if(p.length){const i=p.sort((r,f)=>we(f,"ATT")-we(r,"ATT"))[0];i.used=!0,a.log.push({text:`❄️ ${i.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":a.maxSubs++,a.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}l=!0}$.from("cards").delete().eq("id",e).then(()=>{}),l&&Se(n,a,c)}function Ao(e,t,n){const a=Object.values(t.homeTeam).flat().filter(c=>!c.used);if(!a.length){n.toast("Aucun joueur actif à booster","error");return}n.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(c=>{c.addEventListener("click",()=>{const d=c.dataset.cardId;for(const s of["GK","DEF","MIL","ATT"]){const l=(t.homeTeam[s]||[]).find(p=>p.cardId===d);if(l){l.boost=(l.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${l.name}`,type:"info"});break}}t.boostUsed=!0,n.closeModal(),Se(e,t,n)})})}function Ot(e,t,n,a,c){const d=document.createElement("div");d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const s=Array.from({length:10},(i,r)=>`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(d);let l=!1;const p=()=>{l||(l=!0,d.remove(),setTimeout(()=>c(),50))};d.addEventListener("click",p),setTimeout(p,3500)}async function Tt(e,t,n){var r,f;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:a}=n,c=t.homeScore>t.aiScore,d=t.homeScore===t.aiScore,s=c?"victoire":d?"nul":"defaite",l=Mn(t.mode,s);t.matchId&&await $.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?a.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const p={credits:(a.profile.credits||0)+l,matches_played:(a.profile.matches_played||0)+1};c?p.wins=(a.profile.wins||0)+1:d?p.draws=(a.profile.draws||0)+1:p.losses=(a.profile.losses||0)+1,await $.from("users").update(p).eq("id",a.profile.id),await n.refreshProfile();const i=document.createElement("div");i.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",i.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const y=async(C=!0)=>{f=!0,m&&($.removeChannel(m),m=null),C&&await $.rpc("cancel_matchmaking",{p_user_id:p.profile.id}).catch(()=>{})};(T=document.getElementById("mm-cancel"))==null||T.addEventListener("click",async()=>{try{await y(!0)}catch{}He(e),i("home")});const z=async(C,R,F)=>{if(f)return;f=!0,m&&($.removeChannel(m),m=null);const j=document.getElementById("mm-status");j&&(j.textContent="Adversaire trouvé !"),await new Promise(N=>setTimeout(N,600)),e.isConnected&&ln(e,t,C,F,s,l)},{data:o,error:v}=await $.rpc("try_matchmake",{p_user_id:p.profile.id,p_deck_id:n});if(v||!(o!=null&&o.success)){r("Erreur de matchmaking","error"),He(e),i("home");return}if(o.matched){z(o.match_id,o.opponent_id,!1);return}const L=document.getElementById("mm-status");L&&(L.textContent="En attente d'un autre joueur..."),m=$.channel("matchmaking-"+p.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${p.profile.id}`},C=>{const R=C.new;R.status==="matched"&&R.match_id&&z(R.match_id,R.matched_with,!0)}).subscribe()}async function Lo(e,t,n){const{state:a,navigate:c,toast:d}=t;try{($.getChannels?$.getChannels():[]).forEach(i=>{const r=i.topic||"";(r.includes("matchmaking")||r.includes("pvp-match")||r.includes("friend-invite"))&&$.removeChannel(i)})}catch{}const{data:s}=await $.from("matches").select("home_id, away_id, status").eq("id",n).single();if(!s){d("Match introuvable","error"),c("home");return}if(s.status==="finished"){d("Ce match est terminé","info"),c("home");return}const l=s.home_id===a.profile.id;ln(e,t,n,l,[],[])}async function ln(e,t,n,a,c=[],d=[]){const{state:s,navigate:l,toast:p}=t,i=a?"p1":"p2",r=a?"p2":"p1",f=(c||[]).map(k=>k.id),m=(c||[]).map(k=>({id:k.id,gc_type:k.gc_type,_gcDef:k._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:y}=await $.from("matches").select("*").eq("id",n).single();if(!y){p("Match introuvable","error"),l("home");return}async function z(){const[{data:k},{data:u},{data:g},{data:h}]=await Promise.all([$.rpc("get_deck_for_match",{p_deck_id:y.home_deck_id}),$.rpc("get_deck_for_match",{p_deck_id:y.away_deck_id}),$.from("users").select("id,pseudo,club_name").eq("id",y.home_id).single(),$.from("users").select("id,pseudo,club_name").eq("id",y.away_id).single()]),x=E=>({cardId:E.card_id,position:E.position,id:E.id,firstname:E.firstname,name:E.surname_encoded,country_code:E.country_code,club_id:E.club_id,job:E.job,job2:E.job2,note_g:Number(E.note_g)||0,note_d:Number(E.note_d)||0,note_m:Number(E.note_m)||0,note_a:Number(E.note_a)||0,rarity:E.rarity,skin:E.skin,hair:E.hair,hair_length:E.hair_length,clubName:E.club_encoded_name||null,clubLogo:E.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),b=((k==null?void 0:k.starters)||[]).map(E=>x(E)),w=((u==null?void 0:u.starters)||[]).map(E=>x(E)),_=(k==null?void 0:k.formation)||"4-4-2",I=(u==null?void 0:u.formation)||"4-4-2";return{p1Team:mt(b,_),p2Team:mt(w,I),p1Subs:((k==null?void 0:k.subs)||[]).map(E=>x(E)),p2Subs:((u==null?void 0:u.subs)||[]).map(E=>x(E)),p1Formation:_,p2Formation:I,p1Name:(g==null?void 0:g.club_name)||(g==null?void 0:g.pseudo)||"Joueur 1",p2Name:(h==null?void 0:h.club_name)||(h==null?void 0:h.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:a?f:[],gc_p2:a?[]:f,gcCardsFull_p1:a?m:[],gcCardsFull_p2:a?[]:m,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:d||[],lastActionAt:new Date().toISOString()}}let o=y.game_state&&Object.keys(y.game_state).length?y.game_state:null;if(!o)if(a){o=await z();const{data:k}=await $.from("matches").select("game_state").eq("id",n).single();!(k!=null&&k.game_state)||!Object.keys(k.game_state).length?await $.from("matches").update({game_state:o,turn_user_id:y.home_id}).eq("id",n):o=k.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let k=0;k<30&&!o;k++){await new Promise(g=>setTimeout(g,400));const{data:u}=await $.from("matches").select("game_state").eq("id",n).single();u!=null&&u.game_state&&Object.keys(u.game_state).length&&(o=u.game_state)}if(!o){p("Erreur de synchronisation","error"),l("home");return}o.gc_p2=f,o.gcCardsFull_p2=m,await $.from("matches").update({game_state:o}).eq("id",n)}let v=!1,L=null,T=!1;const C=new Set,R=new Set;function F(k){var E,A;try{$.removeChannel(j)}catch{}const u=o[i+"Score"]||0,g=o[r+"Score"]||0;let h,x;k.winner_id?(h=k.winner_id===s.profile.id,x=!1):k.forfeit?(h=u>g,x=!1):(x=u===g,h=u>g),(E=document.getElementById("pvp-end-overlay"))==null||E.remove();const b=document.createElement("div");b.id="pvp-end-overlay",b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const w=x?"🤝":h?"🏆":"😞",_=x?"MATCH NUL":h?"VICTOIRE !":"DÉFAITE",I=x?"#fff":h?"#FFD700":"#ff6b6b";b.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(b),(A=b.querySelector("#pvp-end-home"))==null||A.addEventListener("click",()=>{b.remove(),He(e),l("home")})}const j=$.channel("pvp-match-"+n).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${n}`},k=>{const u=k.new;try{if(u.status==="finished"||u.forfeit){if(v)return;v=!0,L&&(clearInterval(L),L=null),u.game_state&&(o=u.game_state),F(u);return}if(u.game_state){const g=o;o=u.game_state;const h=o._lastGC;if(h&&h.seq&&!R.has(h.seq)&&(R.add(h.seq),h.by!==i)){xe(h.type,h.by,()=>q());return}const x=g[i+"Score"]||0,b=g[r+"Score"]||0,w=o[i+"Score"]||0,_=o[r+"Score"]||0,I=w>x,E=_>b;if((I||E)&&!C.has(o.round)){C.add(o.round);const A=[...o.log||[]].reverse().find(P=>P.isGoal),B=((A==null?void 0:A.homePlayers)||[]).map(P=>({name:P.name,note:P.note,portrait:P.portrait,job:P.job}));he(B,w,_,I,()=>q());return}q()}}catch(g){console.error("[PvP] crash:",g)}}).subscribe();async function N(k){Object.assign(o,k),o.lastActionAt=new Date().toISOString();const{error:u}=await $.from("matches").update({game_state:o}).eq("id",n);u&&p("Erreur de synchronisation","error");try{q()}catch(g){console.error("[PvP] renderPvpScreen crash:",g)}}async function ee(){if(v)return;v=!0,L&&(clearInterval(L),L=null);const k=a?y.away_id:y.home_id,u=a?"p2":"p1",g=a?"p1":"p2",h={...o,[u+"Score"]:3,[g+"Score"]:0,phase:"finished"};try{await $.from("matches").update({status:"finished",forfeit:!0,winner_id:k,home_score:h.p1Score||0,away_score:h.p2Score||0,game_state:h}).eq("id",n)}catch{}try{$.removeChannel(j)}catch{}setTimeout(()=>{He(e),l("home")},800)}const le={BOOST_STAT:({value:k=1,count:u=1,roles:g=[]},h,x)=>{const b=h[i+"Team"],w=Object.entries(b).filter(([_])=>!g.length||g.includes(_)).flatMap(([_,I])=>I.filter(E=>!E.used).map(E=>({...E,_line:_})));if(!w.length){h.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),x(h);return}re(w,u,`Choisir ${u} joueur(s) à booster (+${k})`,_=>{_.forEach(I=>{const E=(b[I._line]||[]).find(A=>A.cardId===I.cardId);E&&(E.boost=(E.boost||0)+k,h.log.push({text:`⚡ +${k} sur ${E.name}`,type:"info"}))}),h[i+"Team"]=b,x(h)})},DEBUFF_STAT:({value:k=1,count:u=1,roles:g=[],target:h="ai"},x,b)=>{const w=h==="home"?i:r,_=x[w+"Team"],I=h==="home"?"allié":"adverse",E=Object.entries(_).filter(([A])=>!g.length||g.includes(A)).flatMap(([A,B])=>B.filter(P=>!P.used).map(P=>({...P,_line:A})));if(!E.length){x.log.push({text:`🎯 Aucun joueur ${I}`,type:"info"}),b(x);return}re(E,u,`Choisir ${u} joueur(s) ${I}(s) (-${k})`,A=>{A.forEach(B=>{const P=(_[B._line]||[]).find(W=>W.cardId===B.cardId);P&&(P.boost=(P.boost||0)-k,x.log.push({text:`🎯 -${k} sur ${P.name}`,type:"info"}))}),x[w+"Team"]=_,b(x)})},GRAY_PLAYER:({count:k=1,roles:u=[],target:g="ai"},h,x)=>{const b=g==="home"?i:r,w=h[b+"Team"],_=g==="home"?"allié":"adverse",I=Object.entries(w).filter(([E])=>!u.length||u.includes(E)).flatMap(([E,A])=>A.filter(B=>!B.used).map(B=>({...B,_line:E})));if(!I.length){h.log.push({text:`❌ Aucun joueur ${_}`,type:"info"}),x(h);return}re(I,k,`Choisir ${k} joueur(s) ${_}(s) à exclure`,E=>{E.forEach(A=>{const B=(w[A._line]||[]).find(P=>P.cardId===A.cardId);B&&(B.used=!0,h.log.push({text:`❌ ${B.name} exclu !`,type:"info"}))}),h[b+"Team"]=w,x(h)})},REVIVE_PLAYER:({count:k=1,roles:u=[]},g,h)=>{const x=g[i+"Team"],b=Object.entries(x).filter(([w])=>!u.length||u.includes(w)).flatMap(([w,_])=>_.filter(I=>I.used).map(I=>({...I,_line:w})));if(!b.length){g.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),h(g);return}re(b,k,`Choisir ${k} joueur(s) à ressusciter`,w=>{w.forEach(_=>{const I=(x[_._line]||[]).find(E=>E.cardId===_.cardId);I&&(I.used=!1,g.log.push({text:`💫 ${I.name} ressuscité !`,type:"info"}))}),g[i+"Team"]=x,h(g)})},REMOVE_GOAL:({},k,u)=>{const g=r+"Score";k[g]>0?(k[g]--,k.log.push({text:"🚫 Dernier but annulé !",type:"info"})):k.log.push({text:"🚫 Aucun but à annuler",type:"info"}),u(k)},ADD_GOAL_DRAW:({},k,u)=>{k[i+"Score"]===k[r+"Score"]?(k[i+"Score"]++,k.log.push({text:"🎯 But bonus !",type:"info"})):k.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),u(k)},ADD_SUB:({value:k=1},u,g)=>{u.maxSubs=(u.maxSubs||3)+k,u.log.push({text:`🔄 +${k} remplacement(s)`,type:"info"}),g(u)},CUSTOM:({},k,u)=>u(k)};function re(k,u,g,h){const x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let b=[];function w(){var I,E;const _=k.map(A=>{const B={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[A._line]||"#555",P=we(A,A._line)+(A.boost||0),ce=b.find(H=>H.cardId===A.cardId)?"#FFD700":"rgba(255,255,255,0.25)",J=A.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${A.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${ce};background:${B};overflow:hidden;cursor:pointer;${J}">
        ...
        </div>`,(I=x.querySelector("#pp-close"))==null||I.addEventListener("click",()=>x.remove()),x.querySelectorAll(".pp-item").forEach(A=>{A.addEventListener("click",()=>{const B=A.dataset.cid,P=k.find(ce=>ce.cardId===B),W=b.findIndex(ce=>ce.cardId===B);P&&(W>-1?b.splice(W,1):b.length<u&&b.push(P),w())})}),(E=x.querySelector("#pp-confirm"))==null||E.addEventListener("click",()=>{x.remove(),h(b)})}w(),document.body.appendChild(x)}async function M(k,u){const h=(o["gcCardsFull_"+i]||[]).find(_=>_.id===k),x=(h==null?void 0:h._gcDef)||(o.gcDefs||[]).find(_=>{var I;return _.name===u||((I=_.name)==null?void 0:I.toLowerCase().trim())===(u==null?void 0:u.toLowerCase().trim())}),b=[...o["usedGc_"+i]||[],k],w={type:u,by:i,seq:(o._gcAnimSeq||0)+1};R.add(w.seq),xe(u,i,async()=>{if(x!=null&&x.effect_type&&x.effect_type!=="CUSTOM"){const I=le[x.effect_type];if(I){const E={...o};I(x.effect_params||{},E,async A=>{A["usedGc_"+i]=b,A._lastGC=w,A._gcAnimSeq=w.seq,await N(A)});return}}const _={...o};switch(u){case"Remplacement+":_.maxSubs=(_.maxSubs||3)+1,_.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const I=r+"Score";_[I]>0&&(_[I]--,_.log.push({text:"🚫 But annulé",type:"info"}));break}}_["usedGc_"+i]=b,_._lastGC=w,_._gcAnimSeq=w.seq,await N(_)})}function Y(k,u){const g="usedCardIds_"+k,h=new Set(o[g]||[]);u.forEach(x=>h.add(x)),o[g]=[...h]}function ie(){for(const k of["p1","p2"]){const u=new Set(o["usedCardIds_"+k]||[]),g=o[k+"Team"];if(g)for(const h of["GK","DEF","MIL","ATT"])(g[h]||[]).forEach(x=>{x.used=u.has(x.cardId)})}}function q(){var Fe,Qe,xt,yt,ht,bt;if(o.phase==="reveal")return Q();if(o.phase==="midfield")return pe();if(o.phase==="finished")return Re();const k=o[i+"Team"],u=o[r+"Team"];ie();const g=o[i+"Score"],h=o[r+"Score"],x=o[i+"Name"],b=o[r+"Name"],w=o.phase===i+"-attack",_=o.phase===i+"-defense",I=Array.isArray(o["selected_"+i])?o["selected_"+i]:[],E=I.map(X=>X.cardId),A=window.innerWidth>=700,B=o[i+"Subs"]||[],P=o["usedSubIds_"+i]||[],W=B.filter(X=>!P.includes(X.cardId)),ce=o["gcCardsFull_"+i]||[],J=o["usedGc_"+i]||[],H=ce.filter(X=>!J.includes(X.id)),V=o.boostOwner===i&&!o.boostUsed,Z=!["GK","DEF","MIL","ATT"].some(X=>(u[X]||[]).some(G=>!G.used)),de=[...k.MIL||[],...k.ATT||[]].filter(X=>!X.used),ue=w&&Z&&de.length===0?[...k.GK||[],...k.DEF||[]].filter(X=>!X.used).map(X=>X.cardId):[];function fe(X,G,oe){var jt,_t;const se=X._gcDef,ye={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[se==null?void 0:se.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Ye={purple:"#b06ce0",light_blue:"#00d4ef"}[se==null?void 0:se.color]||"#b06ce0",We=(se==null?void 0:se.name)||X.gc_type,Je=(se==null?void 0:se.effect)||((jt=Ne[X.gc_type])==null?void 0:jt.desc)||"",et=se!=null&&se.image_url?`/manager-wars/icons/${se.image_url}`:null,St=((_t=Ne[X.gc_type])==null?void 0:_t.icon)||"⚡",Ct=Math.round(oe*.22),vt=Math.round(oe*.22),lt=oe-Ct-vt,wt=We.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${X.id}" data-gc-type="${X.gc_type}"
        ...
        <div style="height:${lt}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${et?`<img src="${et}" style="max-width:${G-10}px;max-height:${lt-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(lt*.55)}px">${St}</span>`}
        </div>
    ...
    </div>`,i==="p1"&&setTimeout(async()=>{await N({phase:"midfield"})},5e3)}let ae=!1;function pe(){if(ae)return;const k=o[i+"Team"].MIL||[],u=o[r+"Team"].MIL||[];function g(H){return H.reduce((V,Z)=>V+we(Z,"MIL"),0)}function h(H){let V=0;for(let Z=0;Z<H.length-1;Z++){const de=Ze(H[Z],H[Z+1]);de==="#00ff88"?V+=2:de==="#FFD700"&&(V+=1)}return V}const x=g(k)+h(k),b=g(u)+h(u),w=x>=b;function _(H,V,Z){return`<div id="duel-row-${Z}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        <div style="font-size:11px;color:rgba(255,255,255,0.55);letter-spacing:2px;margin-bottom:10px;text-transform:uppercase;font-weight:700">${V}</div>
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(b);let I=!1;const E=()=>{I||(I=!0,b.remove(),setTimeout(()=>x(),50))};b.addEventListener("click",E),setTimeout(E,3500)}function xe(k,u,g){var J,H;const h=(o.gcDefs||[]).find(V=>{var Z;return V.name===k||((Z=V.name)==null?void 0:Z.toLowerCase().trim())===(k==null?void 0:k.toLowerCase().trim())}),x={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[h==null?void 0:h.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",b={purple:"#b06ce0",light_blue:"#00d4ef"}[h==null?void 0:h.color]||"#b06ce0",w=(h==null?void 0:h.name)||k,_=(h==null?void 0:h.effect)||((J=Ne[k])==null?void 0:J.desc)||"",I=h!=null&&h.image_url?`/manager-wars/icons/${h.image_url}`:null,E=((H=Ne[k])==null?void 0:H.icon)||"⚡",B=u===i?"Vous":o[u+"Name"]||"Adversaire",P=document.createElement("div");P.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",P.innerHTML=`
      <style>
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(P);let W=!1;const ce=()=>{W||(W=!0,P.remove(),setTimeout(()=>g&&g(),50))};P.addEventListener("click",ce),setTimeout(ce,3e3)}function ge(k,u){var P,W,ce,J;const h=(o["gcCardsFull_"+i]||[]).find(H=>H.id===k),x=h==null?void 0:h._gcDef,b={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[x==null?void 0:x.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",w={purple:"#b06ce0",light_blue:"#00d4ef"}[x==null?void 0:x.color]||"#b06ce0",_=(x==null?void 0:x.name)||u,I=(x==null?void 0:x.effect)||((P=Ne[u])==null?void 0:P.desc)||"Carte spéciale.",E=x!=null&&x.image_url?`/manager-wars/icons/${x.image_url}`:null,A=((W=Ne[u])==null?void 0:W.icon)||"⚡",B=document.createElement("div");B.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",B.innerHTML=`
      ...
      </div>`,(fe=B.querySelector("#psub-close"))==null||fe.addEventListener("click",()=>B.remove()),(me=B.querySelector("#pout-up"))==null||me.addEventListener("click",()=>{_>0&&(_--,P())}),(be=B.querySelector("#pout-down"))==null||be.addEventListener("click",()=>{_<b.length-1&&(_++,P())}),(Ce=B.querySelector("#pin-up"))==null||Ce.addEventListener("click",()=>{E>0&&(E--,P())}),(Pe=B.querySelector("#pin-down"))==null||Pe.addEventListener("click",()=>{E<w.length-1&&(E++,P())});const ue=(Ie,Le,Be,Ge)=>{const Oe=B.querySelector("#"+Ie);if(!Oe)return;let Ue=0;Oe.addEventListener("touchstart",Ke=>{Ue=Ke.touches[0].clientY},{passive:!0}),Oe.addEventListener("touchend",Ke=>{const je=Ke.changedTouches[0].clientY-Ue;if(Math.abs(je)<30)return;const Fe=Le();je<0&&Fe<Ge-1?(Be(Fe+1),P()):je>0&&Fe>0&&(Be(Fe-1),P())},{passive:!0})};ue("pin-panel",()=>E,Ie=>E=Ie,w.length),ue("pout-panel",()=>_,Ie=>_=Ie,b.length),(De=B.querySelector("#psub-confirm"))==null||De.addEventListener("click",async Ie=>{if(Ie.preventDefault(),Ie.stopPropagation(),A)return;A=!0;const Le=b[_],Be=w[E];if(!Le||!Be)return;const Ge=Le._line,Oe=(g[Ge]||[]).findIndex(je=>je.cardId===Le.cardId);if(Oe===-1){p("Erreur : joueur introuvable","error"),B.remove();return}const Ue={...Be,_line:Ge,position:Le.position,used:!1,boost:0};g[Ge].splice(Oe,1,Ue);const Ke=[...h,Be.cardId];B.remove(),S(Le,Be,async()=>{await N({[i+"Team"]:g,[r+"Team"]:o[r+"Team"],["usedSubIds_"+i]:Ke})})})}document.body.appendChild(B),P()}function S(k,u,g){const h={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const b=(I,E,A)=>`<div style="text-align:center">
      ...
      </div>`,document.body.appendChild(u),(g=u.querySelector("#pvp-hist-close"))==null||g.addEventListener("click",()=>u.remove())}async function ne(){if(o.phase!==i+"-attack")return;const k=i==="p1"?"p2":"p1",u=(o.round||0)+1,g=[...o.log||[]];g.push({type:"info",text:`⏭️ ${o[i+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const h=Te(o),x=Ae(k),b=h||!x?"finished":k+"-attack";await N({["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:b,attacker:k,round:u,log:g}),b==="finished"&&await qe(o)}async function te(){const k=o[i+"Team"],u=!["GK","DEF","MIL","ATT"].some(b=>(o[r+"Team"][b]||[]).some(w=>!w.used)),g=(o["selected_"+i]||[]).map(b=>{const w=(k[b._role]||[]).find(P=>P.cardId===b.cardId)||b,_=u&&["GK","DEF"].includes(b._role),I=k[b._role]||[],E=I.findIndex(P=>P.cardId===b.cardId),A=ot(I.length),B=E>=0?A[E]:w._col??1;return{...w,_line:b._role,_col:B,..._?{note_a:Math.max(1,Number(w.note_a)||0)}:{}}});if(!g.length)return;const h=Ut(g,o.modifiers[i]||{});Y(i,g.map(b=>b.cardId)),g.forEach(b=>{const w=(k[b._role]||[]).find(_=>_.cardId===b.cardId);w&&(w.used=!0)}),o["selected_"+i]=[],q();const x=[...o.log||[]];if(u){const b=(o[i+"Score"]||0)+1,w=g.map(B=>({name:B.name,note:we(B,B._line||"ATT"),portrait:ze(B),job:B.job}));x.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:w,homeTotal:h.total,aiTotal:0});const _=(o.round||0)+1,I=i==="p1"?"p2":"p1",E={...o,[i+"Team"]:k,[i+"Score"]:b},A=Te(E);C.add(_),he(w,b,o[r+"Score"]||0,!0,async()=>{await N({[i+"Team"]:k,[i+"Score"]:b,["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:A?"finished":I+"-attack",attacker:I,round:_,log:x}),A&&await qe({...o,[i+"Score"]:b})});return}x.push({type:"pending",text:`⚔️ ${o[i+"Name"]} attaque (${h.total})`}),await N({[i+"Team"]:k,[r+"Team"]:o[r+"Team"],pendingAttack:{...h,players:g,side:i},["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},phase:r+"-defense",log:x})}async function _e(){const k=o[i+"Team"],u=(o["selected_"+i]||[]).map(J=>{const H=(k[J._role]||[]).find(fe=>fe.cardId===J.cardId)||J,V=k[J._role]||[],Z=V.findIndex(fe=>fe.cardId===J.cardId),de=ot(V.length),ue=Z>=0?de[Z]:H._col??1;return{...H,_line:J._role,_col:ue}}),g=Kt(u,o.modifiers[i]||{});Y(i,u.map(J=>J.cardId)),u.forEach(J=>{const H=(k[J._role]||[]).find(V=>V.cardId===J.cardId);H&&(H.used=!0)}),o["selected_"+i]=[],q();const h=Vt(o.pendingAttack.total,g.total,o.modifiers[i]||{}),x=o.pendingAttack.side,b=h==="attack"||(h==null?void 0:h.goal),w=x==="p1"?"p2":"p1",_=(o.round||0)+1,I=(o.pendingAttack.players||[]).map(J=>({name:J.name,note:we(J,J._line||"ATT"),portrait:ze(J),job:J.job})),E=[...o.log||[]];E.push({type:"duel",isGoal:b,homeScored:b&&x===i,text:b?`⚽ BUT de ${o[x+"Name"]} ! (${o.pendingAttack.total} vs ${g.total})`:`✋ Attaque stoppée (${o.pendingAttack.total} vs ${g.total})`,homePlayers:I,aiPlayers:u.map(J=>({name:J.name,note:we(J,J._line||"DEF"),portrait:ze(J),job:J.job})),homeTotal:o.pendingAttack.total,aiTotal:g.total});const A=b?(o[x+"Score"]||0)+1:o[x+"Score"]||0,B={...o,[i+"Team"]:k,[x+"Score"]:A},P=Te(B),W=P?"finished":w+"-attack",ce=async()=>{await N({[i+"Team"]:k,[r+"Team"]:o[r+"Team"],[x+"Score"]:A,["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:W,attacker:w,round:_,log:E}),(W==="finished"||P)&&await qe({...o,[x+"Score"]:A})};if(b){const J=x===i,H=J?A:o[i+"Score"]||0,V=J?o[r+"Score"]||0:A;C.add(_),he(I,H,V,J,ce)}else await ce()}function $e(k){return["MIL","ATT"].some(u=>(k[u]||[]).some(g=>!g.used))}function Ee(k){return["GK","DEF","MIL","ATT"].some(u=>(k[u]||[]).some(g=>!g.used))}function Me(k){return Ee(k)&&!$e(k)}function Ae(k){const u=o[k+"Team"],g=o[(k==="p1"?"p2":"p1")+"Team"];return!!($e(u)||!Ee(g)&&Me(u))}function Te(k){const u=["MIL","ATT"].some(E=>(k.p1Team[E]||[]).some(A=>!A.used)),g=["MIL","ATT"].some(E=>(k.p2Team[E]||[]).some(A=>!A.used)),h=Ee(k.p1Team),x=Ee(k.p2Team);return!u&&!(!x&&h)&&(!g&&!(!h&&x))}function ke(k){const u=k.p1Score||0,g=k.p2Score||0;return u===g?null:u>g?y.home_id:y.away_id}async function qe(k){try{const u=ke(k),g=u?y.home_id===u?y.away_id:y.home_id:null;await $.from("matches").update({status:"finished",winner_id:u,home_score:k.p1Score||0,away_score:k.p2Score||0}).eq("id",n),u&&g&&Hi(u,g).catch(()=>{})}catch(u){console.error("[PvP] finishMatch:",u)}}function Re(){var x;const k=o[i+"Score"],u=o[r+"Score"],g=k>u,h=k===u;Xe(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(w),(B=w.querySelector("#pvp-end-home"))==null||B.addEventListener("click",()=>{w.remove(),He(e),l("home")})}const N=$.channel("pvp-match-"+n).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${n}`},u=>{const g=u.new;try{if(g.status==="finished"||g.forfeit){if(v)return;v=!0,T&&(clearInterval(T),T=null),g.game_state&&(o=g.game_state),j(g);return}if(g.game_state){const h=o;o=g.game_state;const x=o._lastGC;if(x&&x.seq&&!F.has(x.seq)&&(F.add(x.seq),x.by!==i)){ge(x.type,x.by,()=>Q());return}const b=h[i+"Score"]||0,w=h[r+"Score"]||0,_=o[i+"Score"]||0,I=o[r+"Score"]||0,E=_>b,A=I>w;if((E||A)&&!R.has(o.round)){R.add(o.round);const B=[...o.log||[]].reverse().find(W=>W.isGoal),P=((B==null?void 0:B.homePlayers)||[]).map(W=>({name:W.name,note:W.note,portrait:W.portrait,job:W.job}));xe(P,_,I,E,()=>Q());return}Q()}}catch(h){console.error("[PvP] crash:",h)}}).subscribe();async function ee(u){Object.assign(o,u),o.lastActionAt=new Date().toISOString();const{error:g}=await $.from("matches").update({game_state:o}).eq("id",n);g&&p("Erreur de synchronisation","error");try{Q()}catch(h){console.error("[PvP] renderPvpScreen crash:",h)}}async function le(){if(v)return;v=!0,T&&(clearInterval(T),T=null);const u=a?y.away_id:y.home_id,g=a?"p2":"p1",h=a?"p1":"p2",x={...o,[g+"Score"]:3,[h+"Score"]:0,phase:"finished"};try{await $.from("matches").update({status:"finished",forfeit:!0,winner_id:u,home_score:x.p1Score||0,away_score:x.p2Score||0,game_state:x}).eq("id",n)}catch{}try{$.removeChannel(N)}catch{}setTimeout(()=>{He(e),l("home")},800)}const re={BOOST_STAT:({value:u=1,count:g=1,roles:h=[]},x,b)=>{const w=x[i+"Team"],_=Object.entries(w).filter(([I])=>!h.length||h.includes(I)).flatMap(([I,E])=>E.filter(A=>!A.used).map(A=>({...A,_line:I})));if(!_.length){x.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),b(x);return}M(_,g,`Choisir ${g} joueur(s) à booster (+${u})`,I=>{I.forEach(E=>{const A=(w[E._line]||[]).find(B=>B.cardId===E.cardId);A&&(A.boost=(A.boost||0)+u,x.log.push({text:`⚡ +${u} sur ${A.name}`,type:"info"}))}),x[i+"Team"]=w,b(x)})},DEBUFF_STAT:({value:u=1,count:g=1,roles:h=[],target:x="ai"},b,w)=>{const _=x==="home"?i:r,I=b[_+"Team"],E=x==="home"?"allié":"adverse",A=Object.entries(I).filter(([B])=>!h.length||h.includes(B)).flatMap(([B,P])=>P.filter(W=>!W.used).map(W=>({...W,_line:B})));if(!A.length){b.log.push({text:`🎯 Aucun joueur ${E}`,type:"info"}),w(b);return}M(A,g,`Choisir ${g} joueur(s) ${E}(s) (-${u})`,B=>{B.forEach(P=>{const W=(I[P._line]||[]).find(ce=>ce.cardId===P.cardId);W&&(W.boost=(W.boost||0)-u,b.log.push({text:`🎯 -${u} sur ${W.name}`,type:"info"}))}),b[_+"Team"]=I,w(b)})},GRAY_PLAYER:({count:u=1,roles:g=[],target:h="ai"},x,b)=>{const w=h==="home"?i:r,_=x[w+"Team"],I=h==="home"?"allié":"adverse",E=Object.entries(_).filter(([A])=>!g.length||g.includes(A)).flatMap(([A,B])=>B.filter(P=>!P.used).map(P=>({...P,_line:A})));if(!E.length){x.log.push({text:`❌ Aucun joueur ${I}`,type:"info"}),b(x);return}M(E,u,`Choisir ${u} joueur(s) ${I}(s) à exclure`,A=>{A.forEach(B=>{const P=(_[B._line]||[]).find(W=>W.cardId===B.cardId);P&&(P.used=!0,x.log.push({text:`❌ ${P.name} exclu !`,type:"info"}))}),x[w+"Team"]=_,b(x)})},REVIVE_PLAYER:({count:u=1,roles:g=[]},h,x)=>{const b=h[i+"Team"],w=Object.entries(b).filter(([_])=>!g.length||g.includes(_)).flatMap(([_,I])=>I.filter(E=>E.used).map(E=>({...E,_line:_})));if(!w.length){h.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),x(h);return}M(w,u,`Choisir ${u} joueur(s) à ressusciter`,_=>{_.forEach(I=>{const E=(b[I._line]||[]).find(A=>A.cardId===I.cardId);E&&(E.used=!1,h.log.push({text:`💫 ${E.name} ressuscité !`,type:"info"}))}),h[i+"Team"]=b,x(h)})},REMOVE_GOAL:({},u,g)=>{const h=r+"Score";u[h]>0?(u[h]--,u.log.push({text:"🚫 Dernier but annulé !",type:"info"})):u.log.push({text:"🚫 Aucun but à annuler",type:"info"}),g(u)},ADD_GOAL_DRAW:({},u,g)=>{u[i+"Score"]===u[r+"Score"]?(u[i+"Score"]++,u.log.push({text:"🎯 But bonus !",type:"info"})):u.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),g(u)},ADD_SUB:({value:u=1},g,h)=>{g.maxSubs=(g.maxSubs||3)+u,g.log.push({text:`🔄 +${u} remplacement(s)`,type:"info"}),h(g)},CUSTOM:({},u,g)=>g(u)};function M(u,g,h,x){const b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let w=[];function _(){var E,A;const I=u.map(B=>{const P={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[B._line]||"#555",W=we(B,B._line)+(B.boost||0),J=w.find(V=>V.cardId===B.cardId)?"#FFD700":"rgba(255,255,255,0.25)",H=B.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${B.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${J};background:${P};overflow:hidden;cursor:pointer;${H}">
        ...
        <div style="height:${wt}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${St?`<img src="${St}" style="max-width:${oe-10}px;max-height:${wt-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(wt*.55)}px">${Ct}</span>`}
        </div>
      ...
      </div>`;function Fe(){const G=e.querySelector(".match-screen");if(!G)return;const oe=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);G.style.height=oe+"px",G.style.minHeight=oe+"px",G.style.maxHeight=oe+"px",G.style.overflow="hidden";const se=e.querySelector("#mobile-action-bar"),ye=e.querySelector("#mobile-play-area");se&&ye&&(ye.style.paddingBottom=se.offsetHeight+"px")}if(Fe(),setTimeout(Fe,120),setTimeout(Fe,400),C||(C=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Fe),window.visualViewport.addEventListener("scroll",Fe)),window.addEventListener("resize",Fe)),function(){const oe=e.querySelector(".terrain-wrapper svg");oe&&(oe.removeAttribute("width"),oe.removeAttribute("height"),oe.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",oe.setAttribute("viewBox","-26 -26 352 352"),oe.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(G=>{G.addEventListener("click",()=>{if(!_&&!I)return;const oe=G.dataset.cardId,se=G.dataset.role,ye=(u[se]||[]).find(et=>et.cardId===oe);if(!ye||ye.used)return;const Ye=fe.includes(oe);if(_&&!["MIL","ATT"].includes(se)&&!Ye)return;Array.isArray(o["selected_"+i])||(o["selected_"+i]=[]);const We=o["selected_"+i],Je=We.findIndex(et=>et.cardId===oe);Je>-1?We.splice(Je,1):We.length<3&&We.push({...ye,_role:se}),Q()})}),e.querySelectorAll(".match-used-hit").forEach(G=>{G.addEventListener("click",()=>S(G.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(G=>{G.addEventListener("click",()=>S())}),(Qe=e.querySelector("#pvp-sub-open"))==null||Qe.addEventListener("click",()=>S()),e.querySelectorAll(".pvp-gc-mini").forEach(G=>{G.addEventListener("click",()=>O(G.dataset.gcId,G.dataset.gcType))}),(xt=e.querySelector("#pvp-boost-card"))==null||xt.addEventListener("click",()=>U()),(yt=e.querySelector("#pvp-action"))==null||yt.addEventListener("click",G=>{_?G.currentTarget.dataset.pass==="1"||!Te(i)?te():_e():I&&$e()}),(ht=e.querySelector("#pvp-quit"))==null||ht.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&le()}),(bt=e.querySelector("#pvp-view-opp"))==null||bt.addEventListener("click",()=>K()),(X=e.querySelector("#pvp-toggle-history"))==null||X.addEventListener("click",()=>ne()),T&&(clearInterval(T),T=null),(_||I)&&!v){let G=30,oe=!1;const se=()=>document.getElementById("pvp-timer"),ye=()=>{se()&&(se().textContent=G+"s",se().style.color=oe?"#ff4444":"#fff")};ye(),T=setInterval(()=>{G--,G<0?oe?(clearInterval(T),T=null,_&&!Te(i)?te():le()):(oe=!0,G=15,ye()):ye()},1e3)}}function ae(){Xe(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      <div style="font-size:20px;font-weight:900;color:#ff6b6b">${o[r+"Name"]||"Adversaire"}</div>
      <div style="width:min(90vw,420px)">${st(o[r+"Team"],o[r+"Formation"],null,[],300,300)}</div>
    </div>`,i==="p1"&&setTimeout(async()=>{await ee({phase:"midfield"})},5e3)}let pe=!1;function he(){if(pe)return;const u=o[i+"Team"].MIL||[],g=o[r+"Team"].MIL||[];function h(V){return V.reduce((Z,de)=>Z+we(de,"MIL"),0)}function x(V){let Z=0;for(let de=0;de<V.length-1;de++){const ue=Ze(V[de],V[de+1]);ue==="#00ff88"?Z+=2:ue==="#FFD700"&&(Z+=1)}return Z}const b=h(u)+x(u),w=h(g)+x(g),_=b>=w;function I(V,Z,de){return`<div id="duel-row-${de}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${V.map((ue,fe)=>{const me=fe<V.length-1?Ze(ue,V[fe+1]):null,be=!me||me==="#ff3333"||me==="#cc2222",Ce=me==="#00ff88"?"+2":me==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${de}" data-idx="${fe}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(w);let E=!1;const A=()=>{E||(E=!0,w.remove(),setTimeout(()=>b(),50))};w.addEventListener("click",A),setTimeout(A,3500)}function ge(u,g,h){var H,V;const x=(o.gcDefs||[]).find(Z=>{var de;return Z.name===u||((de=Z.name)==null?void 0:de.toLowerCase().trim())===(u==null?void 0:u.toLowerCase().trim())}),b={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[x==null?void 0:x.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",w={purple:"#b06ce0",light_blue:"#00d4ef"}[x==null?void 0:x.color]||"#b06ce0",_=(x==null?void 0:x.name)||u,I=(x==null?void 0:x.effect)||((H=Ne[u])==null?void 0:H.desc)||"",E=x!=null&&x.image_url?`/manager-wars/icons/${x.image_url}`:null,A=((V=Ne[u])==null?void 0:V.icon)||"⚡",P=g===i?"Vous":o[g+"Name"]||"Adversaire",W=document.createElement("div");W.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",W.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
        @keyframes gcGlow{0%,100%{box-shadow:0 0 30px ${w}66}50%{box-shadow:0 0 60px ${w}cc}}
        @keyframes gcLabel{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}
      </style>
      <div style="font-size:11px;color:${w};letter-spacing:3px;text-transform:uppercase;font-weight:700;animation:gcLabel .4s ease both">${P} joue une carte</div>
      ...
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(P),(J=P.querySelector("#pvp-gc-back"))==null||J.addEventListener("click",()=>P.remove()),(H=P.querySelector("#pvp-gc-use"))==null||H.addEventListener("click",()=>{P.remove(),Y(u,g)})}function U(){var x;const u=o[i+"Team"],g=Object.entries(u).flatMap(([b,w])=>(w||[]).filter(_=>!_.used).map(_=>({..._,_line:b})));if(!g.length)return;const h=document.createElement("div");h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",h.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">⚡ Choisir un joueur pour +${o.boostValue}</div>
        <button id="bp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
      </div>
      ...
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(me=P.querySelector("#psub-close"))==null||me.addEventListener("click",()=>P.remove()),(be=P.querySelector("#pout-up"))==null||be.addEventListener("click",()=>{I>0&&(I--,W())}),(Ce=P.querySelector("#pout-down"))==null||Ce.addEventListener("click",()=>{I<w.length-1&&(I++,W())}),(Pe=P.querySelector("#pin-up"))==null||Pe.addEventListener("click",()=>{A>0&&(A--,W())}),(De=P.querySelector("#pin-down"))==null||De.addEventListener("click",()=>{A<_.length-1&&(A++,W())});const fe=(Le,Be,Ge,Oe)=>{const Ue=P.querySelector("#"+Le);if(!Ue)return;let Ke=0;Ue.addEventListener("touchstart",je=>{Ke=je.touches[0].clientY},{passive:!0}),Ue.addEventListener("touchend",je=>{const Fe=je.changedTouches[0].clientY-Ke;if(Math.abs(Fe)<30)return;const Qe=Be();Fe<0&&Qe<Oe-1?(Ge(Qe+1),W()):Fe>0&&Qe>0&&(Ge(Qe-1),W())},{passive:!0})};fe("pin-panel",()=>A,Le=>A=Le,_.length),fe("pout-panel",()=>I,Le=>I=Le,w.length),(Ie=P.querySelector("#psub-confirm"))==null||Ie.addEventListener("click",async Le=>{if(Le.preventDefault(),Le.stopPropagation(),B)return;B=!0;const Be=w[I],Ge=_[A];if(!Be||!Ge)return;const Oe=Be._line,Ue=(h[Oe]||[]).findIndex(Fe=>Fe.cardId===Be.cardId);if(Ue===-1){p("Erreur : joueur introuvable","error"),P.remove();return}const Ke={...Ge,_line:Oe,position:Be.position,used:!1,boost:0};h[Oe].splice(Ue,1,Ke);const je=[...x,Ge.cardId];P.remove(),D(Be,Ge,async()=>{await ee({[i+"Team"]:h,[r+"Team"]:o[r+"Team"],["usedSubIds_"+i]:je})})})}document.body.appendChild(P),W()}function D(u,g,h){const x={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const w=(E,A,B)=>`<div style="text-align:center">
      ...
      </div>`,document.body.appendChild(g),(h=g.querySelector("#pvp-hist-close"))==null||h.addEventListener("click",()=>g.remove())}async function te(){if(o.phase!==i+"-attack")return;const u=i==="p1"?"p2":"p1",g=(o.round||0)+1,h=[...o.log||[]];h.push({type:"info",text:`⏭️ ${o[i+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const x=ke(o),b=Te(u),w=x||!b?"finished":u+"-attack";await ee({["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:w,attacker:u,round:g,log:h}),w==="finished"&&await Re(o)}async function _e(){const u=o[i+"Team"],g=!["GK","DEF","MIL","ATT"].some(w=>(o[r+"Team"][w]||[]).some(_=>!_.used)),h=(o["selected_"+i]||[]).map(w=>{const _=(u[w._role]||[]).find(W=>W.cardId===w.cardId)||w,I=g&&["GK","DEF"].includes(w._role),E=u[w._role]||[],A=E.findIndex(W=>W.cardId===w.cardId),B=ot(E.length),P=A>=0?B[A]:_._col??1;return{..._,_line:w._role,_col:P,...I?{note_a:Math.max(1,Number(_.note_a)||0)}:{}}});if(!h.length)return;const x=Ut(h,o.modifiers[i]||{});ie(i,h.map(w=>w.cardId)),h.forEach(w=>{const _=(u[w._role]||[]).find(I=>I.cardId===w.cardId);_&&(_.used=!0)}),o["selected_"+i]=[],Q();const b=[...o.log||[]];if(g){const w=(o[i+"Score"]||0)+1,_=h.map(P=>({name:P.name,note:we(P,P._line||"ATT"),portrait:ze(P),job:P.job}));b.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:_,homeTotal:x.total,aiTotal:0});const I=(o.round||0)+1,E=i==="p1"?"p2":"p1",A={...o,[i+"Team"]:u,[i+"Score"]:w},B=ke(A);R.add(I),xe(_,w,o[r+"Score"]||0,!0,async()=>{await ee({[i+"Team"]:u,[i+"Score"]:w,["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:B?"finished":E+"-attack",attacker:E,round:I,log:b}),B&&await Re({...o,[i+"Score"]:w})});return}b.push({type:"pending",text:`⚔️ ${o[i+"Name"]} attaque (${x.total})`}),await ee({[i+"Team"]:u,[r+"Team"]:o[r+"Team"],pendingAttack:{...x,players:h,side:i},["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},phase:r+"-defense",log:b})}async function $e(){const u=o[i+"Team"],g=(o["selected_"+i]||[]).map(H=>{const V=(u[H._role]||[]).find(me=>me.cardId===H.cardId)||H,Z=u[H._role]||[],de=Z.findIndex(me=>me.cardId===H.cardId),ue=ot(Z.length),fe=de>=0?ue[de]:V._col??1;return{...V,_line:H._role,_col:fe}}),h=Kt(g,o.modifiers[i]||{});ie(i,g.map(H=>H.cardId)),g.forEach(H=>{const V=(u[H._role]||[]).find(Z=>Z.cardId===H.cardId);V&&(V.used=!0)}),o["selected_"+i]=[],Q();const x=Vt(o.pendingAttack.total,h.total,o.modifiers[i]||{}),b=o.pendingAttack.side,w=x==="attack"||(x==null?void 0:x.goal),_=b==="p1"?"p2":"p1",I=(o.round||0)+1,E=(o.pendingAttack.players||[]).map(H=>({name:H.name,note:we(H,H._line||"ATT"),portrait:ze(H),job:H.job})),A=[...o.log||[]];A.push({type:"duel",isGoal:w,homeScored:w&&b===i,text:w?`⚽ BUT de ${o[b+"Name"]} ! (${o.pendingAttack.total} vs ${h.total})`:`✋ Attaque stoppée (${o.pendingAttack.total} vs ${h.total})`,homePlayers:E,aiPlayers:g.map(H=>({name:H.name,note:we(H,H._line||"DEF"),portrait:ze(H),job:H.job})),homeTotal:o.pendingAttack.total,aiTotal:h.total});const B=w?(o[b+"Score"]||0)+1:o[b+"Score"]||0,P={...o,[i+"Team"]:u,[b+"Score"]:B},W=ke(P),ce=W?"finished":_+"-attack",J=async()=>{await ee({[i+"Team"]:u,[r+"Team"]:o[r+"Team"],[b+"Score"]:B,["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:ce,attacker:_,round:I,log:A}),(ce==="finished"||W)&&await Re({...o,[b+"Score"]:B})};if(w){const H=b===i,V=H?B:o[i+"Score"]||0,Z=H?o[r+"Score"]||0:B;R.add(I),xe(E,V,Z,H,J)}else await J()}function Ee(u){return["MIL","ATT"].some(g=>(u[g]||[]).some(h=>!h.used))}function Me(u){return["GK","DEF","MIL","ATT"].some(g=>(u[g]||[]).some(h=>!h.used))}function Ae(u){return Me(u)&&!Ee(u)}function Te(u){const g=o[u+"Team"],h=o[(u==="p1"?"p2":"p1")+"Team"];return!!(Ee(g)||!Me(h)&&Ae(g))}function ke(u){const g=["MIL","ATT"].some(A=>(u.p1Team[A]||[]).some(B=>!B.used)),h=["MIL","ATT"].some(A=>(u.p2Team[A]||[]).some(B=>!B.used)),x=Me(u.p1Team),b=Me(u.p2Team);return!g&&!(!b&&x)&&(!h&&!(!x&&b))}function qe(u){const g=u.p1Score||0,h=u.p2Score||0;return g===h?null:g>h?y.home_id:y.away_id}async function Re(u){try{const g=qe(u),h=g?y.home_id===g?y.away_id:y.home_id:null;await $.from("matches").update({status:"finished",winner_id:g,home_score:u.p1Score||0,away_score:u.p2Score||0}).eq("id",n),g&&h&&Hi(g,h).catch(()=>{})}catch(g){console.error("[PvP] finishMatch:",g)}}function k(){var b;const u=o[i+"Score"],g=o[r+"Score"],h=u>g,x=u===g;Xe(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      <button id="pvp-home" style="padding:16px 40px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏠 Retour</button>
    </div>`,(b=document.getElementById("pvp-home"))==null||b.addEventListener("click",()=>{try{$.removeChannel(N)}catch{}He(e),l("home")})}Q()}const jo={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},Bo={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};function ct(e,t){return t&&Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}function qt(e,t,n=28){const a=t?jo[t]||"#aaa":"#bbb",c=e>0?e:"",d=n*.44;return`<svg width="${n}" height="${n}" viewBox="0 0 28 28" style="flex-shrink:0">
    <polygon points="14,2 17.5,10 27,11 20.5,17 22.5,26 14,21.5 5.5,26 7.5,17 1,11 10.5,10"
      fill="${a}" stroke="white" stroke-width="${e>0?1.2:0}"/>
    ${c?`<text x="14" y="17.5" text-anchor="middle" font-size="${d}" font-weight="900" fill="white" font-family="Arial Black,Arial">${c}</text>`:""}
  </svg>`}function Mi(e,t=20){const n=Rt(e);return n?`<img src="${n}" style="width:${t}px;height:${Math.round(t*.67)}px;object-fit:cover;border-radius:2px;flex-shrink:0">`:""}function Si(e,t=20){return e?`<img src="${e}" style="width:${t}px;height:${t}px;object-fit:contain;flex-shrink:0" onerror="this.style.display='none'">`:`<span style="font-size:${t}px;flex-shrink:0">🏟️</span>`}async function Fo(e,t){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',await mi(e,t)}async function mi(e,t){const{state:n,toast:a}=t,{data:c}=await $.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  </div>`;let p="buy";const i=()=>{var o,v,L,T,C,R;return{name:(((o=document.getElementById("flt-name"))==null?void 0:o.value)||"").toLowerCase().trim(),club:(((v=document.getElementById("flt-club"))==null?void 0:v.value)||"").toLowerCase().trim(),country:(((L=document.getElementById("flt-country"))==null?void 0:L.value)||"").toLowerCase().trim(),job:((T=document.getElementById("flt-job"))==null?void 0:T.value)||"",note1:parseInt((C=document.getElementById("flt-note1"))==null?void 0:C.value)||0,note2:parseInt((R=document.getElementById("flt-note2"))==null?void 0:R.value)||0}};function r(o){const v=i();return o.filter(L=>{var ee,le;const T=(ee=L.card)==null?void 0:ee.player;if(!T)return!1;const C=`${T.firstname} ${T.surname_encoded}`.toLowerCase(),R=(((le=T.clubs)==null?void 0:le.encoded_name)||"").toLowerCase(),F=(T.country_code||"").toLowerCase(),j=ct(T,T.job),N=T.job2?ct(T,T.job2):0;return!(v.name&&!C.includes(v.name)||v.club&&!R.includes(v.club)||v.country&&!F.includes(v.country)||v.job&&T.job!==v.job||v.note1&&j<v.note1||v.note2&&N<v.note2)})}function f(o){var F,j,N;const v=(F=o.card)==null?void 0:F.player;if(!v)return"";const L=ct(v,v.job),T=v.job2?ct(v,v.job2):0,C=(j=v.clubs)==null?void 0:j.logo_url,R=(n.profile.credits||0)>=o.price;return Bo[v.rarity],`<div class="card-panel" style="display:flex;align-items:center;gap:8px;padding:10px 12px">
      <!-- Étoile poste 1 -->
      ${qt(L,v.job,30)}
      <!-- Étoile poste 2 (grisée si pas de job2) -->
      ${qt(T,v.job2||null,26)}
      <!-- Drapeau pays -->
      ${Mi(v.country_code,22)}
      <!-- Logo club -->
      ${C?Si(C,22):'<span style="width:22px;text-align:center;flex-shrink:0">🏟️</span>'}
      <!-- Nom + vendeur -->
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:10px;color:#999;margin-top:1px">Vendeur : ${((N=o.seller)==null?void 0:N.pseudo)||"—"}</div>
      </div>
      <!-- Prix + bouton -->
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:14px;font-weight:900;color:#D4A017">${o.price.toLocaleString("fr")}</div>
        <button class="btn btn-primary btn-sm" data-buy="${o.id}" ${R?"":"disabled"} style="margin-top:4px;font-size:11px;padding:4px 10px">
          ${R?"Acheter":"Trop cher"}
        </button>
      </div>
    </div>`}function m(o){var R,F,j;const v=(R=o.card)==null?void 0:R.player;if(!v)return"";const L=ct(v,v.job),T=v.job2?ct(v,v.job2):0,C=o.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:8px;padding:10px 12px;${C?"opacity:0.65":""}">
      ${qt(L,v.job,30)}
      ${qt(T,v.job2||null,26)}
      ${Mi(v.country_code,22)}
      ${(F=v.clubs)!=null&&F.logo_url?Si(v.clubs.logo_url,22):'<span style="width:22px;text-align:center;flex-shrink:0">🏟️</span>'}
      <div style="flex:1;min-width:0">
        <div style="font-size:13px;font-weight:900;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${v.firstname} ${v.surname_encoded}</div>
        <div style="font-size:10px;color:${C?"#22c55e":"#999"};margin-top:1px">
          ${C?`✅ Vendu à ${((j=o.buyer)==null?void 0:j.pseudo)||"—"} · ${o.sold_at?new Date(o.sold_at).toLocaleDateString("fr"):""}`:`🟢 En vente depuis le ${new Date(o.listed_at).toLocaleDateString("fr")}`}
        </div>
      </div>
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:14px;font-weight:900;color:#D4A017">${o.price.toLocaleString("fr")}</div>
        ${C?'<span style="font-size:10px;font-weight:700;color:#fff;background:#22c55e;padding:3px 8px;border-radius:10px;display:inline-block;margin-top:4px">VENDU</span>':`<button class="btn btn-danger btn-sm" data-cancel="${o.id}" style="margin-top:4px;font-size:11px;padding:4px 10px">Retirer</button>`}
      </div>
    </div>`}function y(){const o=document.getElementById("mkt-content"),v=document.getElementById("mkt-filters");if(!o)return;v.style.display=p==="buy"?"flex":"none";let L=p==="buy"?r(s):[...l].sort((T,C)=>T.status!==C.status?T.status==="active"?-1:1:new Date(C.listed_at)-new Date(T.listed_at));if(!L.length){o.innerHTML=`<div style="text-align:center;color:#aaa;padding:40px">${p==="buy"?"Aucune carte trouvée.":"Aucune vente."}</div>`;return}o.innerHTML=p==="buy"?L.map(f).join(""):L.map(m).join(""),o.querySelectorAll("[data-buy]").forEach(T=>T.addEventListener("click",()=>Do(T.dataset.buy,s,e,t))),o.querySelectorAll("[data-cancel]").forEach(T=>T.addEventListener("click",()=>Go(T.dataset.cancel,e,t)))}e.querySelectorAll(".mkt-tab").forEach(o=>{o.addEventListener("click",()=>{p=o.dataset.tab,e.querySelectorAll(".mkt-tab").forEach(v=>{const L=v===o;v.style.background=L?"var(--green)":"#fff",v.style.color=L?"#fff":"var(--gray-600)",v.style.borderColor=L?"var(--green)":"var(--gray-200)"}),y()})});let z;["flt-name","flt-club","flt-country","flt-job","flt-note1","flt-note2"].forEach(o=>{var v;(v=document.getElementById(o))==null||v.addEventListener("input",()=>{clearTimeout(z),z=setTimeout(y,250)})}),y()}async function Do(e,t,n,a){const{state:c,toast:d,refreshProfile:s}=a,l=t.find(r=>r.id===e);if(!l)return;const p=l.price;if((c.profile.credits||0)<p){d("Crédits insuffisants","error");return}qo(l,async()=>{const{error:r}=await $.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:c.profile.id});if(r){d("Erreur achat : "+r.message,"error");return}await s();const f=document.getElementById("nav-credits");f&&(f.textContent=`💰 ${(c.profile.credits||0).toLocaleString("fr")}`),d("✅ Carte achetée !","success"),await mi(n,a)})}function qo(e,t){var s;const n=(s=e.card)==null?void 0:s.player,a=n?`${n.firstname} ${n.surname_encoded}`:"cette carte",c=document.createElement("div");c.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",c.innerHTML=`
  ...
  </div>`}un(gn);const ve={user:null,profile:null,page:"home",params:{}};function kt(e,t="info",n=3e3){const a=document.getElementById("toast");a&&(a.textContent=e,a.className=`show ${t}`,clearTimeout(a._t),a._t=setTimeout(()=>{a.className=""},n))}function Ro(e,t,n=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=n,document.getElementById("modal-overlay").classList.remove("hidden")}function oi(){document.getElementById("modal-overlay").classList.add("hidden")}async function Et(){if(!ve.user)return;const{data:e}=await $.from("users").select("*").eq("id",ve.user.id).single();e&&(ve.profile=e)}const xi="mw_theme";function Ht(){return localStorage.getItem(xi)||"light"}function Oo(e){var t;localStorage.setItem(xi,e),ri(e),(t=ve.profile)!=null&&t.id&&$.from("users").update({theme:e}).eq("id",ve.profile.id).then(()=>{})}function ri(e){document.documentElement.setAttribute("data-theme",e)}function Nt(e,t={}){ve.page=e,ve.params=t,cn()}async function cn(){var a,c;const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(d=>{d.classList.toggle("active",d.dataset.page===ve.page)});const t=document.getElementById("nav-credits");t&&ve.profile&&(t.textContent=`💰 ${(ve.profile.credits||0).toLocaleString("fr")}`);const n={state:ve,navigate:Nt,toast:kt,openModal:Ro,closeModal:oi,refreshProfile:Et};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',ve.page){case"home":await wi(e,n);break;case"collection":await Fn(e,n);break;case"decks":await Zt(e,n);break;case"boosters":await Xn(e,n);break;case"match":{const d=ve.params&&ve.params.matchMode||"vs_ai_easy";d==="random"?await dn(e,n):d==="friend"?await Mo(e,n,(a=ve.params)==null?void 0:a.friendId,(c=ve.params)==null?void 0:c.friendName):await uo(e,n);break}case"market":await Fo(e,n);break;case"rankings":await No(e,n);break;case"matches":await Po(e,n);break;case"friends":await wn(e,n);break;default:await wi(e,n)}}function Ho(){const e=document.getElementById("app"),t=ve.profile;if(!t)return;const n="/manager-wars/icons/";e.innerHTML=`
    <nav class="top-nav">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(a=>{a.addEventListener("click",c=>{c.preventDefault(),Nt(a.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>Nt("home")),document.getElementById("nav-credits").addEventListener("click",()=>Nt("boosters")),document.getElementById("theme-toggle").addEventListener("click",()=>{const c=Ht()==="dark"?"light":"dark";Oo(c),Ci(c)}),Ci(Ht())}function Ci(e){const t=document.getElementById("theme-icon");t&&(t.textContent=e==="dark"?"☀️":"🌙")}async function Uo(){ri(Ht()),document.getElementById("modal-overlay").addEventListener("click",n=>{n.target===n.currentTarget&&oi()}),document.getElementById("modal-close").addEventListener("click",oi);const{data:{session:e}}=await $.auth.getSession();if(!e){ji(),hi(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:kt});return}ve.user=e.user,await Et(),ji();try{const{data:n}=await $.from("formation_links_overrides").select("formation, links"),a={};(n||[]).forEach(c=>{a[c.formation]=c.links}),fn(a)}catch(n){console.warn("Impossible de charger les overrides de formation:",n)}if(!ve.profile){mn(document.getElementById("app"),{state:ve,navigate:async()=>{await Et(),Qt()},toast:kt,refreshProfile:Et});return}const t=Array.isArray(ve.profile.pending_boosters)?ve.profile.pending_boosters:[];if(!ve.profile.onboarding_done&&t.length>0){so(document.getElementById("app"),{state:ve,navigate:()=>Qt(),toast:kt,refreshProfile:Et});return}ve.profile.theme&&ve.profile.theme!==Ht()&&(localStorage.setItem(xi,ve.profile.theme),ri(ve.profile.theme)),Qt(),$.auth.onAuthStateChange(async(n,a)=>{n==="SIGNED_OUT"&&(ve.user=null,ve.profile=null,document.getElementById("app").innerHTML="",hi(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:kt}))})}function Ko(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function Wt(){const e=document.getElementById("app");e&&(e.style.height=Ko()+"px")}window.addEventListener("resize",Wt);window.addEventListener("orientationchange",()=>setTimeout(Wt,150));window.visualViewport&&window.visualViewport.addEventListener("resize",Wt);function Qt(){const e=()=>{var n;(n=ve.user)!=null&&n.id&&$.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",ve.user.id).then(()=>{})};e(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(e,6e4);const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",Wt(),Ho(),cn()}function ji(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function pn(e){var a;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const n=document.createElement("div");n.id="boot-error",n.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",n.innerHTML=`
    <div style="font-size:42px">📡</div>
    <div style="font-size:18px;font-weight:900">Connexion impossible</div>
    <div style="font-size:13px;color:rgba(255,255,255,0.6);max-width:280px">${e||"Le chargement a échoué. Vérifie ta connexion et réessaie."}</div>
    <button id="boot-retry" style="margin-top:8px;padding:12px 30px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer">Réessayer</button>`,document.body.appendChild(n),(a=document.getElementById("boot-retry"))==null||a.addEventListener("click",()=>window.location.reload())}Uo().catch(e=>{console.error("Échec du démarrage:",e),pn()});setTimeout(()=>{const e=document.getElementById("app-loader");e&&e.style.display!=="none"&&!e.classList.contains("zoom-out")&&!document.getElementById("boot-error")&&pn("Le serveur met trop de temps à répondre.")},12e3);
