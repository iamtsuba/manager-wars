import{s as E,F as xi,h as Pt,j as Yi,l as at,i as zn,k as Sn,b as Ln}from"./formation-links-DJC25wD7.js";const In="/manager-wars/";function Li(e,{navigate:t,toast:i}){let s="login";const c=()=>{var a,l,u,o,n,g;const d=s==="login";e.innerHTML=`
    <div style="
    ...
    </style>`,(a=document.getElementById("tab-login-btn"))==null||a.addEventListener("click",()=>{s="login",c()}),(l=document.getElementById("tab-reg-btn"))==null||l.addEventListener("click",()=>{s="register",c()}),d?((u=document.getElementById("login-password"))==null||u.addEventListener("keydown",v=>{var y;v.key==="Enter"&&((y=document.getElementById("login-btn"))==null||y.click())}),(o=document.getElementById("login-btn"))==null||o.addEventListener("click",async()=>{const v=document.getElementById("login-email").value.trim(),y=document.getElementById("login-password").value,T=document.getElementById("login-error");if(T.textContent="",!v||!y){T.textContent="Remplissez tous les champs.";return}const r=document.getElementById("login-btn");r.textContent="⏳ Connexion…",r.disabled=!0;const{error:p}=await E.auth.signInWithPassword({email:v,password:y});if(r.textContent="⚽ Se connecter",r.disabled=!1,p){T.textContent=p.message.includes("Invalid")?"Email ou mot de passe incorrect.":p.message;return}window.location.reload()})):((n=document.getElementById("reg-confirm"))==null||n.addEventListener("keydown",v=>{var y;v.key==="Enter"&&((y=document.getElementById("reg-btn"))==null||y.click())}),(g=document.getElementById("reg-btn"))==null||g.addEventListener("click",async()=>{const v=document.getElementById("reg-email").value.trim(),y=document.getElementById("reg-password").value,T=document.getElementById("reg-confirm").value,r=document.getElementById("reg-error");if(r.textContent="",!v||!y||!T){r.textContent="Remplissez tous les champs.";return}if(y.length<6){r.textContent="Mot de passe trop court (min. 6 caractères).";return}if(y!==T){r.textContent="Les mots de passe ne correspondent pas.";return}const p=document.getElementById("reg-btn");p.textContent="⏳ Création…",p.disabled=!0;const{error:I}=await E.auth.signUp({email:v,password:y});if(p.textContent="🚀 Créer mon compte",p.disabled=!1,I){r.textContent=I.message;return}i("Compte créé ! Connecte-toi pour commencer.","success",4e3),s="login",c(),setTimeout(()=>{const z=document.getElementById("login-email");z&&(z.value=v)},50)}))};c()}function Mn(e,{state:t,navigate:i,toast:s,refreshProfile:c}){let d="#1A6B3C",a="#D4A017";e.innerHTML=`
  ...
  `;function l(){var T;const o=document.getElementById("logo-preview"),n=document.getElementById("logo-initials"),g=((T=document.getElementById("setup-club"))==null?void 0:T.value)||"MW",v=g.trim().split(" ").filter(Boolean),y=v.length>=2?(v[0][0]+v[1][0]).toUpperCase():g.slice(0,2).toUpperCase();o&&(o.style.background=a,o.style.borderColor=d),n&&(n.textContent=y,n.style.color=d)}document.getElementById("color1").addEventListener("input",o=>{d=o.target.value,document.getElementById("swatch1").style.background=d,l()}),document.getElementById("color2").addEventListener("input",o=>{a=o.target.value,document.getElementById("swatch2").style.background=a,l()});function u(o){document.querySelectorAll(".setup-step").forEach(n=>n.classList.remove("active")),document.getElementById(`step-${o}`).classList.add("active"),document.getElementById("step-num").textContent=o,document.getElementById("progress-fill").style.width=`${Math.round(o/3*100)}%`,o===3&&l()}document.getElementById("step1-next").addEventListener("click",async()=>{const o=document.getElementById("setup-pseudo").value.trim(),n=document.getElementById("step1-error");if(n.textContent="",o.length<3){n.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:g}=await E.from("users").select("id").eq("pseudo",o).maybeSingle();if(g){n.textContent="Ce pseudo est déjà pris.";return}u(2)}),document.getElementById("step2-back").addEventListener("click",()=>u(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const o=document.getElementById("setup-club").value.trim(),n=document.getElementById("step2-error");if(n.textContent="",o.length<2){n.textContent="Nom trop court (min. 2 caractères).";return}const{data:g}=await E.from("users").select("id").eq("club_name",o).maybeSingle();if(g){n.textContent="Ce nom de club est déjà pris.";return}u(3)}),document.getElementById("step3-back").addEventListener("click",()=>u(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const o=document.getElementById("setup-pseudo").value.trim(),n=document.getElementById("setup-club").value.trim(),g=document.getElementById("step3-error"),v=document.getElementById("step3-finish");g.textContent="",v.disabled=!0,v.textContent="Création en cours…";try{const{error:y}=await E.from("users").insert({id:t.user.id,pseudo:o,club_name:n,club_color1:d,club_color2:a,credits:1e4});if(y)throw y;await Cn(t.user.id),await c(),s(`Bienvenue ${o} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(y){g.textContent=y.message,v.disabled=!1,v.textContent="🚀 Créer mon profil !"}})}async function Cn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await E.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const jn="modulepreload",qn=function(e){return"/manager-wars/"+e},Ii={},yi=function(t,i,s){let c=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),l=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));c=Promise.allSettled(i.map(u=>{if(u=qn(u),u in Ii)return;Ii[u]=!0;const o=u.endsWith(".css"),n=o?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${u}"]${n}`))return;const g=document.createElement("link");if(g.rel=o?"stylesheet":jn,o||(g.as="script"),g.crossOrigin="",g.href=u,l&&g.setAttribute("nonce",l),document.head.appendChild(g),o)return new Promise((v,y)=>{g.addEventListener("load",v),g.addEventListener("error",()=>y(new Error(`Unable to preload CSS for ${u}`)))})}))}function d(a){const l=new Event("vite:preloadError",{cancelable:!0});if(l.payload=a,window.dispatchEvent(l),!l.defaultPrevented)throw a}return c.then(a=>{for(const l of a||[])l.status==="rejected"&&d(l.reason);return t().catch(d)})},Ft="#1A6B3C",Dt="#cc2222",Bn="#D4A017",Mi="#888";async function Fn(e,t){const{state:i,toast:s}=t;e.innerHTML=`
    ...
    <div style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:8px;padding:10px 0;border-bottom:1px solid #f0f0f0">
      ...
      <div style="text-align:left;font-size:18px;font-weight:900;color:${ie}">${R}</div>
    ...
    </style>`}async function Xi({player1Id:e,player2Id:t,score1:i,score2:s,gc1:c,gc2:d}){const[a,l]=[e,t].sort(),u=e!==a,o=u?s:i,n=u?i:s,g=u?d:c,v=u?c:d,y=o>n?1:0,T=n>o?1:0,r=o===n?1:0,{data:p}=await E.from("friend_match_stats").select("*").eq("player1_id",a).eq("player2_id",l).single();p?await E.from("friend_match_stats").update({wins_p1:p.wins_p1+y,wins_p2:p.wins_p2+T,draws:p.draws+r,goals_p1:p.goals_p1+o,goals_p2:p.goals_p2+n,gc_used_p1:p.gc_used_p1+g,gc_used_p2:p.gc_used_p2+v,matches_total:p.matches_total+1}).eq("player1_id",a).eq("player2_id",l):await E.from("friend_match_stats").insert({player1_id:a,player2_id:l,wins_p1:y,wins_p2:T,draws:r,goals_p1:o,goals_p2:n,gc_used_p1:g,gc_used_p2:v,matches_total:1})}const Nn="2026.07.03-1722";async function Ci(e,{state:t,navigate:i,toast:s}){var d,a;const c=t.profile;c&&(e.innerHTML=`
  <style>
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),o=(l||[]).filter(N=>N.card_type==="player"&&N.player),n=(l||[]).filter(N=>N.card_type==="game_changer"),g=(l||[]).filter(N=>N.card_type==="formation"),v=(l||[]).filter(N=>N.card_type==="stadium"),{data:y}=await E.from("gc_definitions").select("name,gc_type,color,effect,image_url"),T={};(y||[]).forEach(N=>{T[N.name]=N});const{data:r}=await E.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),p={};(r||[]).forEach(N=>{p[N.id]=N}),v.forEach(N=>{N.stadium_def&&(p[N.stadium_id]=N.stadium_def)});const I=Object.keys(xi),z=Object.keys(Ke),C={};o.forEach(N=>{const Z=N.player.id;C[Z]=(C[Z]||0)+1}),new Set(Object.keys(C).map(N=>String(N)));const U=new Set(g.map(N=>N.formation)),D=new Set(n.map(N=>N.gc_type));let R="player",G="Tous",ie="",se=!1;function de(){return[...o].sort((N,Z)=>{const ue=Ut.indexOf(N.player.job),me=Ut.indexOf(Z.player.job);return ue!==me?ue-me:(N.player.surname_encoded||"").localeCompare(Z.player.surname_encoded||"")})}function P(){return[...u||[]].sort((N,Z)=>{const ue=Ut.indexOf(N.job),me=Ut.indexOf(Z.job);return ue!==me?ue-me:(N.surname_encoded||"").localeCompare(Z.surname_encoded||"")})}function Y(){return de().filter(N=>{const Z=N.player,ue=G==="Tous"||Z.job===G,me=!ie||`${Z.firstname} ${Z.surname_encoded}`.toLowerCase().includes(ie);return ue&&me})}function V(){return P().filter(N=>{const Z=G==="Tous"||N.job===G,ue=!ie||`${N.firstname} ${N.surname_encoded}`.toLowerCase().includes(ie);return Z&&ue})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function F(){const N=document.getElementById("col-filters");N&&(R==="player"?(N.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",Z=>{ie=Z.target.value.toLowerCase(),oe()}),e.querySelectorAll(".filter-btn").forEach(Z=>{Z.addEventListener("click",()=>{G=Z.dataset.job,F(),oe()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{se=!se,F(),oe()})):(N.innerHTML=`
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Vt=1e3;function Qn(e,t,i,s){var T,r,p;const{state:c,toast:d,closeModal:a,navigate:l,refreshProfile:u}=i,o=e.formation,n={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function g(){const I=Pt[o]||{},z=xi[o]||[],C=290,U=360,D=20;function R(ie){const se=I[ie];return se?{x:se.x*C,y:se.y*U}:null}let G=`<svg width="${C}" height="${U}" viewBox="0 0 ${C} ${U}" xmlns="http://www.w3.org/2000/svg">`;for(const[ie,se]of z){const de=R(ie),P=R(se);!de||!P||(G+=`<line x1="${de.x}" y1="${de.y}" x2="${P.x}" y2="${P.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const ie of Object.keys(I)){const se=R(ie);if(!se)continue;const de=ie.replace(/\d+/,""),P=n[de]||"#555";G+=`<circle cx="${se.x}" cy="${se.y}" r="${D}" fill="${P}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,G+=`<text x="${se.x}" y="${se.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${de}</text>`}return G+="</svg>",G}const v=t.filter(I=>I.formation===o);v.length;const y=!e.is_for_sale;s(`Formation ${o}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;background:#f9f9f9;border-radius:8px;padding:8px 12px;gap:8px">
      ...
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:13px;font-weight:700;color:${te.source==="booster"?"var(--gray-600)":"var(--yellow)"}">${N}</div>
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${g.map((te,re)=>{const N=P[te.id]||[],Z=te.is_for_sale,ue=N.length?N[N.length-1]:null,X=(n.rarity==="pepite"||n.rarity==="papyte")&&te.current_note!=null?` (☆${te.current_note})`:"";return`
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",n.note_g],["DEF",n.note_d],["MIL",n.note_m],["ATT",n.note_a]].map(([te,re])=>{const N=jt[te],Z=Number(re)||0;return`<div style="display:flex;flex-direction:column;align-items:center;gap:2px">
                <svg width="38" height="37" viewBox="0 0 38 37" style="display:block">
                  <polygon points="19,2 24,13 36,13 26,21 30,33 19,26 8,33 12,21 2,13 14,13" fill="${N}" stroke="white" stroke-width="1.5"/>
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(Ae=document.getElementById("close-detail"))==null||Ae.addEventListener("click",l);let F=new Set;const oe=()=>{const te=F.size,re=document.getElementById("sell-action-panel");re&&(re.style.display=te>0?"block":"none",document.getElementById("sell-selected-count").textContent=te,document.getElementById("sell-direct-total").textContent=(te*p).toLocaleString("fr")+" cr.")};document.querySelectorAll(".expl-check").forEach(te=>{te.addEventListener("change",()=>{const re=te.dataset.id;te.checked?F.add(re):F.delete(re);const N=te.closest(".exemplaire-row");N&&(N.style.borderColor=te.checked?"#1A6B3C":"#eee"),oe()})}),document.querySelectorAll(".exemplaire-row").forEach(te=>{te.addEventListener("click",re=>{if(re.target.closest("button")||re.target.tagName==="INPUT")return;const N=te.querySelector(".expl-check");N&&!N.disabled&&(N.checked=!N.checked,N.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(te=>{te.addEventListener("click",re=>{re.stopPropagation();const N=document.querySelector(`.expl-hist[data-hist="${te.dataset.idx}"]`);N&&(N.style.display=N.style.display==="none"?"flex":"none")})}),(we=document.getElementById("direct-sell-btn"))==null||we.addEventListener("click",async()=>{const te=[...F];if(!te.length)return;const re=te.length*p;if(!confirm(`Vendre ${te.length} carte${te.length>1?"s":""} ${n.surname_encoded} pour ${re.toLocaleString("fr")} crédits ? Action irréversible.`))return;await E.from("market_listings").delete().in("card_id",te),await E.from("transfer_history").delete().in("card_id",te);const{error:N}=await E.from("cards").delete().in("id",te);if(N){d(N.message,"error");return}await E.from("users").update({credits:(c.profile.credits||0)+re}).eq("id",c.profile.id),await o();const Z=document.getElementById("nav-credits");Z&&(Z.textContent=`💰 ${(c.profile.credits||0).toLocaleString("fr")}`),d(`+${re.toLocaleString("fr")} crédits ! ${te.length} carte${te.length>1?"s":""} vendue${te.length>1?"s":""}.`,"success"),l(),u("collection")}),($e=document.getElementById("market-sell-btn"))==null||$e.addEventListener("click",async()=>{var me;const te=[...F];if(!te.length){d("Sélectionne au moins un exemplaire","warning");return}const re=parseInt((me=document.getElementById("sell-market-price"))==null?void 0:me.value);if(!re||re<1){d("Prix invalide","error");return}const{error:N}=await E.from("cards").update({is_for_sale:!0,sale_price:re}).in("id",te);if(N){d(N.message,"error");return}const Z=te.map(X=>({seller_id:c.profile.id,card_id:X,price:re,status:"active"})),{error:ue}=await E.from("market_listings").insert(Z);ue&&console.warn("[Market] insert listings:",ue.message),d(`${te.length} carte${te.length>1?"s":""} mise${te.length>1?"s":""} en vente à ${re.toLocaleString("fr")} cr. chacune !`,"success"),l(),u("collection")}),(Ee=document.getElementById("cancel-sell-btn"))==null||Ee.addEventListener("click",async()=>{await E.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await E.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),d("Annonce retirée","success"),l(),u("collection")})}const Jt={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},vt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function ei(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function dn(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function ln(e){var i;const t="https://fcnwclxlkytdfjotqkta.supabase.co";return(i=e==null?void 0:e.clubs)!=null&&i.logo_url?e.clubs.logo_url.startsWith("http")?e.clubs.logo_url:`${t}/storage/v1/object/public/assets/clubs/${e.clubs.logo_url}`:null}function eo(e,t=44,i=58){var y;const s=e?ei(e):null,c=e?ln(e):null,d=dn(e==null?void 0:e.country_code),a=(e==null?void 0:e.job)||"MIL",l=vt[a]||"#555",u={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",o=Number(a==="GK"?e==null?void 0:e.note_g:a==="DEF"?e==null?void 0:e.note_d:a==="MIL"?e==null?void 0:e.note_m:e==null?void 0:e.note_a)||0,n=Math.round(i*.19),g=Math.round(i*.25),v=i-n-g;return e?`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${u};background:${l};position:relative;overflow:hidden;flex-shrink:0">
  ...
  </div>`,to(e,t,c,i),document.getElementById("builder-back").addEventListener("click",()=>s("decks")),document.getElementById("formation-select").addEventListener("change",n=>{t.formation=n.target.value;const g=Gi(t.formation),v={};g.forEach(y=>{t.slots[y]&&(v[y]=t.slots[y])}),t.slots=v,St(e,t,i)}),(u=document.getElementById("stadium-select"))==null||u.addEventListener("change",n=>{t.stadiumCardId=n.target.value||null}),document.getElementById("save-deck").addEventListener("click",()=>oo(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(n=>{n.addEventListener("click",()=>{t.subs=t.subs.filter(g=>g!==n.dataset.removeSub),St(e,t,i)})}),(o=document.getElementById("add-sub-btn"))==null||o.addEventListener("click",()=>{no(t,e,i)})}function to(e,t,i,s){const c=e.querySelector("#deck-field");if(!c)return;const d=Pt[t.formation]||{},a=Yi(t.formation)||[],l={};for(const I of i){const z=t.slots[I],C=z?t.playerCards.find(U=>U.id===z):null;l[I]=C?C.player:null}const u=300,o=300,n=48,g=64,v=13,y=16,T=38;function r(I){const z=d[I];return z?{x:z.x*u,y:z.y*o}:null}let p="";for(const[I,z]of a){const C=r(I),U=r(z);if(!C||!U)continue;const D=l[I]?{...l[I],club_id:l[I].club_id,country_code:l[I].country_code,rarity:l[I].rarity}:null,R=l[z]?{...l[z],club_id:l[z].club_id,country_code:l[z].country_code,rarity:l[z].rarity}:null,G=at(D,R);G==="#ff3333"||G==="#cc2222"?p+=`<line x1="${C.x.toFixed(1)}" y1="${C.y.toFixed(1)}" x2="${U.x.toFixed(1)}" y2="${U.y.toFixed(1)}" stroke="${G}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(p+=`<line x1="${C.x.toFixed(1)}" y1="${C.y.toFixed(1)}" x2="${U.x.toFixed(1)}" y2="${U.y.toFixed(1)}" stroke="${G}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,p+=`<line x1="${C.x.toFixed(1)}" y1="${C.y.toFixed(1)}" x2="${U.x.toFixed(1)}" y2="${U.y.toFixed(1)}" stroke="${G}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const I of i){const z=r(I);if(!z)continue;const C=l[I],U=I.replace(/\d+/,""),D=vt[U]||"#555",R=(z.x-n/2).toFixed(1),G=(z.y-g/2).toFixed(1),ie={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[C==null?void 0:C.rarity]||"#aaa";if(C){const se=ei(C),de=ln(C),P=dn(C.country_code),Y=Number(U==="GK"?C.note_g:U==="DEF"?C.note_d:U==="MIL"?C.note_m:C.note_a)||0,V=g-v-y;p+=`<defs><clipPath id="dcp-${I}"><rect x="${R}" y="${(z.y-g/2+v).toFixed(1)}" width="${n}" height="${V}"/></clipPath></defs>`,p+=`<rect x="${R}" y="${G}" width="${n}" height="${g}" rx="5" fill="${D}"/>`,se&&(p+=`<image href="${se}" x="${R}" y="${(z.y-g/2+v).toFixed(1)}" width="${n}" height="${V}" clip-path="url(#dcp-${I})" preserveAspectRatio="xMidYMin slice"/>`),p+=`<rect x="${R}" y="${G}" width="${n}" height="${v}" fill="rgba(255,255,255,0.93)"/>`,p+=`<text x="${z.x.toFixed(1)}" y="${(z.y-g/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(C.surname_encoded||"").slice(0,9)}</text>`;const F=(z.y+g/2-y).toFixed(1);p+=`<rect x="${R}" y="${F}" width="${n}" height="${y}" fill="rgba(255,255,255,0.93)"/>`,P&&(p+=`<image href="${P}" x="${(z.x-21).toFixed(1)}" y="${(z.y+g/2-y+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),p+=`<text x="${z.x.toFixed(1)}" y="${(z.y+g/2-y/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${Y}</text>`,de&&(p+=`<image href="${de}" x="${(z.x+n/2-14).toFixed(1)}" y="${(z.y+g/2-y+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),p+=`<rect x="${R}" y="${G}" width="${n}" height="${g}" rx="5" fill="none" stroke="${ie}" stroke-width="2"/>`}else p+=`<rect x="${R}" y="${G}" width="${n}" height="${g}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,p+=`<text x="${z.x.toFixed(1)}" y="${z.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,p+=`<text x="${z.x.toFixed(1)}" y="${(z.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${U}</text>`;p+=`<rect x="${R}" y="${G}" width="${n}" height="${g}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${I}" style="cursor:pointer"/>`}c.innerHTML=`<svg viewBox="${-T} ${-T} ${u+T*2} ${o+T*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${p}</svg>`,c.querySelectorAll(".deck-slot-hit").forEach(I=>{I.addEventListener("click",()=>io(I.dataset.pos,t,e,s))})}function io(e,t,i,s){var v,y,T;const{openModal:c,closeModal:d}=s,a=e.replace(/\d+/,""),l=t.slots[e],u=l?t.playerCards.find(r=>r.id===l):null;(v=u==null?void 0:u.player)==null||v.id;const o=new Set;Object.entries(t.slots).forEach(([r,p])=>{var z;if(r===e||!p)return;const I=t.playerCards.find(C=>C.id===p);(z=I==null?void 0:I.player)!=null&&z.id&&o.add(I.player.id)}),t.subs.forEach(r=>{var I;const p=t.playerCards.find(z=>z.id===r);(I=p==null?void 0:p.player)!=null&&I.id&&o.add(p.player.id)});const n=new Set,g=t.playerCards.filter(r=>{const p=r.player;return!(p.job===a||p.job2===a)||o.has(p.id)||n.has(p.id)?!1:(n.add(p.id),!0)});g.sort((r,p)=>{const I=a==="GK"?r.player.note_g:a==="DEF"?r.player.note_d:a==="MIL"?r.player.note_m:r.player.note_a;return(a==="GK"?p.player.note_g:a==="DEF"?p.player.note_d:a==="MIL"?p.player.note_m:p.player.note_a)-I}),c(`Choisir ${a} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${g.length>0?g.map(r=>{var U,D;const p=r.player,I=a==="GK"?p.note_g:a==="DEF"?p.note_d:a==="MIL"?p.note_m:p.note_a,z=ei(p),C={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[p.rarity];return`<div class="player-option" data-card-id="${r.id}"
  ...
  `,document.body.appendChild(c);let d=!1;const a=document.getElementById("pack-cut-zone"),l=document.getElementById("pack-blade");let u=!1;const o=F=>F.touches&&F.touches[0]?F.touches[0].clientX:F.clientX;function n(F){d||(u=!0,l.style.opacity="1",g(F))}function g(F){if(!u||d)return;const oe=a.getBoundingClientRect(),pe=o(F)-oe.left,ye=Math.max(0,Math.min(1,pe/oe.width));l.style.width=ye*oe.width+"px",ye>=.82&&y()}function v(){d||(u=!1,l.style.transition="width .2s ease, opacity .2s ease",l.style.width="0",l.style.opacity="0",setTimeout(()=>{d||(l.style.transition="")},220))}function y(){var oe;if(d)return;d=!0,u=!1,l.style.width="100%",l.style.opacity="1",(oe=document.getElementById("cut-flash"))==null||oe.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const F=document.getElementById("cut-hint");F&&(F.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{l.style.opacity="0",document.getElementById("pack-phase").style.display="none",I(0)},620)}a.addEventListener("pointerdown",n),window.addEventListener("pointermove",g),window.addEventListener("pointerup",v),a.addEventListener("touchstart",n,{passive:!0}),window.addEventListener("touchmove",g,{passive:!0}),window.addEventListener("touchend",v);let T=0,r=!1;const p=new Set;function I(F){T=F,document.getElementById("reveal-phase").style.display="flex",z(),C(F,0),ie()}function z(){const F=document.getElementById("card-dots");F&&(F.innerHTML=e.map((oe,pe)=>`<div class="card-dot" data-i="${pe}" style="width:8px;height:8px;border-radius:50%;background:${pe===T?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),F.querySelectorAll(".card-dot").forEach(oe=>oe.addEventListener("click",()=>D(parseInt(oe.dataset.i)))))}function C(F,oe){var re;const pe=e[F],ye=document.getElementById("card-counter"),ze=document.getElementById("card-track");ye&&(ye.textContent=`Carte ${F+1} / ${e.length}`);const Ae=document.getElementById("reveal-btns");Ae&&(Ae.style.display=F===e.length-1?"flex":"none");const we=pe.card_type==="player"&&((re=pe.player)==null?void 0:re.rarity)==="legende",$e=!p.has(F);p.add(F);let Ee=0;if(pe.card_type==="player"&&pe.player){const N=pe.player,Z=N.job||"ATT";Ee=Number(Z==="GK"?N.note_g:Z==="DEF"?N.note_d:Z==="MIL"?N.note_m:N.note_a)||0}const te=N=>{ze.innerHTML=`
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`}const u=n=>{e.style.overflow="",e.style.height="",e.style.display="",e.style.flexDirection="",i(n)};function o(){var g,v,y;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const n=a.length>0;e.innerHTML=`
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(T=>{T.addEventListener("click",()=>{const r=T.dataset.id,p=d.find(z=>z.id===r);if(!p)return;const I=a.findIndex(z=>z.gc_type===p.gc_type);I>-1?a.splice(I,1):a.length<3&&a.push(p),o()})}),(g=e.querySelector("#gc-launch"))==null||g.addEventListener("click",()=>{n&&u(a)}),(v=e.querySelector("#gc-no-gc"))==null||v.addEventListener("click",()=>u([])),(y=e.querySelector("#gc-reset"))==null||y.addEventListener("click",()=>{a.length&&(a=[],o())})}o()}function Eo(e){var s;const t=((s=e==null?void 0:e.state)==null?void 0:s.params)||{},i=t.matchMode||"vs_ai_easy";return i==="friend"?`Match vs ${t.friendName||"Ami"}`:i==="random"?"Match vs Random":`Match vs IA — ${i.replace("vs_ai_","").toUpperCase()}`}async function Ao(e,t,i){const{state:s,navigate:c}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await E.from("decks").select("id,name,is_active,formation").eq("owner_id",s.profile.id).order("created_at",{ascending:!1});if(!d||d.length===0){wt(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>c("decks"));return}const a=d.map(v=>v.id),{data:l}=await E.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,stadium_id,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),u=[...new Set((l||[]).filter(v=>{var y,T;return((y=v.card)==null?void 0:y.card_type)==="stadium"&&((T=v.card)==null?void 0:T.stadium_id)}).map(v=>v.card.stadium_id))],o={};if(u.length){const{data:v}=await E.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",u);(v||[]).forEach(y=>{o[y.id]=y}),(l||[]).forEach(y=>{var T,r;((T=y.card)==null?void 0:T.card_type)==="stadium"&&((r=y.card)!=null&&r.stadium_id)&&(y.card._stadiumDef=o[y.card.stadium_id]||null)})}let n=0;function g(){var D,R,G,ie,se,de,P;const v=d[n],y=(l||[]).filter(Y=>Y.deck_id===v.id),T=y.filter(Y=>{var V;return Y.is_starter&&((V=Y.card)==null?void 0:V.player)}).map(Y=>ui(Y.card,Y.position)),r=y.find(Y=>{var V;return((V=Y.card)==null?void 0:V.card_type)==="formation"}),p=v.formation||((D=r==null?void 0:r.card)==null?void 0:D.formation)||"4-4-2";let I=T.length>=11?yt(T,p):null,z=null;if(v.stadium_card_id&&I){const Y=y.find(V=>{var F;return((F=V.card)==null?void 0:F.id)===v.stadium_card_id});(R=Y==null?void 0:Y.card)!=null&&R.stadium_id&&(z=Y.card._stadiumDef||null),z&&(I=Ei(I,z))}const C=T.length>=11;et(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
  ...
  </div>`}function nt(e,t,i){if(!(e!=null&&e.length))return"";const s=e.slice(0,5);let c='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return s.forEach((d,a)=>{if(c+=Ye(d,40,52),a<s.length-1){const l=at(d,s[a+1]);c+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(c+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),c+="</div>",c}function Xt(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function hn(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function ut(e,t,i,s,c=310,d=310,a=[]){var C;const l=Pt[t]||{},u=Yi(t)||xi[t]||[],o={},n=["ATT","MIL","DEF","GK"];for(const U of n)(e[U]||[]).forEach((R,G)=>{o[`${U}${G+1}`]=R});function g(U){const D=l[U];return D?{x:D.x*c,y:D.y*d}:null}let v="";for(const[U,D]of u){const R=g(U),G=g(D);if(!R||!G)continue;const ie=o[U],se=o[D],de=at(ie,se);de==="#00ff88"||de==="#FFD700"?(v+=`<line x1="${R.x.toFixed(1)}" y1="${R.y.toFixed(1)}" x2="${G.x.toFixed(1)}" y2="${G.y.toFixed(1)}"
        stroke="${de}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,v+=`<line x1="${R.x.toFixed(1)}" y1="${R.y.toFixed(1)}" x2="${G.x.toFixed(1)}" y2="${G.y.toFixed(1)}"
        stroke="${de}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):v+=`<line x1="${R.x.toFixed(1)}" y1="${R.y.toFixed(1)}" x2="${G.x.toFixed(1)}" y2="${G.y.toFixed(1)}"
        stroke="${de}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const y=48,T=64,r=13,p=16,I={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[U,D]of Object.entries(o)){const R=g(U);if(!R||!D)continue;const G=U.replace(/[0-9]/g,""),ie=It[G]||"#555",se=a.includes(D.cardId),de=i==="attack"&&(["MIL","ATT"].includes(G)||se)&&!D.used||i==="defense"&&["GK","DEF","MIL"].includes(G)&&!D.used,P=s.includes(D.cardId);let Y;i==="attack"?Y=se?Math.max(1,Number(D.note_a)||0):G==="MIL"?Number(D.note_m)||0:Number(D.note_a)||0:i==="defense"?Y=G==="GK"?Number(D.note_g)||0:G==="MIL"?Number(D.note_m)||0:Number(D.note_d)||0:Y=Number(G==="GK"?D.note_g:G==="DEF"?D.note_d:G==="MIL"?D.note_m:D.note_a)||0,Y=Y+(D.boost||0);const V=(R.x-y/2).toFixed(1),F=(R.y-T/2).toFixed(1),oe=I[D==null?void 0:D.rarity]||I.normal;if(D.used){const Ee=typeof import.meta<"u"&&"/manager-wars/"||"/",re=`${typeof window<"u"&&((C=window.location)==null?void 0:C.origin)||""}${Ee}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");v+=`<rect x="${V}" y="${F}" width="${y}" height="${T}" rx="5" fill="#161616"/>`,v+=`<image href="${re}" xlink:href="${re}" x="${V}" y="${F}" width="${y}" height="${T}" preserveAspectRatio="xMidYMid slice"/>`,v+=`<rect x="${V}" y="${F}" width="${y}" height="${T}" rx="5" fill="none" stroke="${oe}" stroke-width="2" opacity="0.7"/>`,v+=`<rect x="${V}" y="${F}" width="${y}" height="${T}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${D.cardId}" data-role="${G}" style="cursor:pointer"/>`;continue}const pe=P?.45:1,ye=P?"#FFD700":oe,ze=P?3:(D==null?void 0:D.rarity)==="legende"||(D==null?void 0:D.rarity)==="légende"||(D==null?void 0:D.rarity)==="pepite"||(D==null?void 0:D.rarity)==="pépite"?2.5:2,Ae=T-r-p;v+=`<defs><clipPath id="cp-${U}"><rect x="${V}" y="${(R.y-T/2+r).toFixed(1)}" width="${y}" height="${Ae}"/></clipPath></defs>`,v+=`<rect x="${V}" y="${F}" width="${y}" height="${T}" rx="5" fill="${ie}" opacity="${pe}"/>`;const we=Pe(D);we&&(v+=`<image href="${we}" xlink:href="${we}" x="${V}" y="${(R.y-T/2+r).toFixed(1)}" width="${y}" height="${Ae}" clip-path="url(#cp-${U})" preserveAspectRatio="xMidYMin slice"/>`),v+=`<rect x="${V}" y="${F}" width="${y}" height="${r}" rx="4" fill="rgba(255,255,255,0.92)"/>`,v+=`<text x="${R.x.toFixed(1)}" y="${(R.y-T/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(D.name||"").slice(0,9)}</text>`;const $e=(R.y+T/2-p).toFixed(1);v+=`<rect x="${V}" y="${$e}" width="${y}" height="${p}" fill="rgba(255,255,255,0.92)"/>`;{const Ee=Xt(D.country_code);Ee?v+=`<image href="${Ee}" xlink:href="${Ee}" x="${(R.x-20).toFixed(1)}" y="${(R.y+T/2-p+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:v+=`<text x="${(R.x-13).toFixed(1)}" y="${(R.y+T/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${hn(D.country_code)}</text>`,v+=`<text x="${R.x.toFixed(1)}" y="${(R.y+T/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${Y}</text>`;const te=bn(D);te?v+=`<image href="${te}" xlink:href="${te}" x="${(R.x+y/2-14).toFixed(1)}" y="${(R.y+T/2-p+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:D.clubName&&(v+=`<text x="${(R.x+14).toFixed(1)}" y="${(R.y+T/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(D.clubName||"").slice(0,3).toUpperCase()}</text>`),D.boost&&(v+=`<rect x="${(R.x+y/2-12).toFixed(1)}" y="${(R.y-T/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,v+=`<text x="${(R.x+y/2-5).toFixed(1)}" y="${(R.y-T/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${D.boost}</text>`)}v+=`<rect x="${V}" y="${F}" width="${y}" height="${T}" rx="5" fill="${P?"rgba(255,255,255,0.12)":"none"}" stroke="${ye}" stroke-width="${ze}" opacity="${pe}"/>`,de&&(v+=`<rect x="${V}" y="${F}" width="${y}" height="${T}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${P?"selected":""}" data-card-id="${D.cardId}" data-role="${G}" style="cursor:pointer"/>`)}const z=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-z} ${-z} ${c+z*2} ${d+z*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${v}
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${r.map((C,U)=>{const D=U<r.length-1?at(C,r[U+1]):null,R=!D||D==="#ff3333"||D==="#cc2222",G=D==="#00ff88"?"+2":D==="#FFD700"?"+1":"";return`
          ...
          ${U<r.length-1?`<div class="duel-link duel-link-${z}" data-idx="${U}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${R?"rgba(255,255,255,0.12)":D};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${R?"none":`0 0 8px ${D}`}">
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,D.style.transition="opacity .45s ease",D.style.opacity="1",D.style.pointerEvents="auto",(R=document.getElementById("start-match-btn"))==null||R.addEventListener("click",T))},600)},700)},2800)}function Mo(e){if(e.type==="duel"&&(e.homeTotal!=null||e.aiTotal!=null)){const t=(e.homeTotal||0)>=(e.aiTotal||0);return`
  ...
  `,document.body.appendChild(s);let n=!1;const g=()=>{n||(n=!0,s.remove(),i())};s.addEventListener("click",g),setTimeout(g,2e3)}function Mt(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function ai(e,t,i,s=null,c=null){var y,T;if(t.phase!=="attack"){Mt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){Mt(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const d=Object.entries(t.homeTeam).flatMap(([r,p])=>(p||[]).filter(I=>I.used).map(I=>({...I,_line:I._line||r}))),a=t.homeSubs.filter(r=>!t.usedSubIds.includes(r.cardId));if(!d.length){Mt("Aucun joueur utilisé à remplacer");return}if(!a.length){Mt("Aucun remplaçant disponible");return}let l=Math.max(0,d.findIndex(r=>r.cardId===c));const u=((y=d[l])==null?void 0:y._line)||((T=d[l])==null?void 0:T.job);let o=s?Math.max(0,a.findIndex(r=>r.cardId===s)):Math.max(0,a.findIndex(r=>r.job===u)),n=!1;const g=document.createElement("div");g.id="sub-overlay",g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function v(){var D,R,G,ie,se,de;const r=d[l],p=a[o],I=Math.min(130,Math.round((window.innerWidth-90)/2)),z=Math.round(I*1.35),C=P=>`background:rgba(255,255,255,0.12);border:none;color:${P?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${P?"default":"pointer"};flex-shrink:0`;g.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
      ${e.map(g=>{const v=g._line||g.job||"MIL",y={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[v]||"#555",T=Te(g,v)+(g.boost||0),r=l.find(p=>p.cardId===g.cardId);return`<div class="gc-pick-item" data-cid="${g.cardId}"
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(d);let l=!1;const u=()=>{l||(l=!0,d.remove(),setTimeout(()=>c(),50))};d.addEventListener("click",u),setTimeout(u,3500)}async function qt(e,t,i){var n,g;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:s}=i,c=t.homeScore>t.aiScore,d=t.homeScore===t.aiScore,a=c?"victoire":d?"nul":"defaite",l=Vn(t.mode,a);t.matchId&&await E.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?s.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const u={credits:(s.profile.credits||0)+l,matches_played:(s.profile.matches_played||0)+1};c?u.wins=(s.profile.wins||0)+1:d?u.draws=(s.profile.draws||0)+1:u.losses=(s.profile.losses||0)+1,await E.from("users").update(u).eq("id",s.profile.id),await i.refreshProfile();const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",o.innerHTML=`
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="width:min(90vw,420px)">${ut(r[n+"Team"],r[n+"Formation"],null,[],300,300)}</div>
    </div>`,o==="p1"&&setTimeout(async()=>{await G({phase:"midfield"})},5e3)}let pe=!1;function ye(){if(pe)return;const M=r[o+"Team"].MIL||[],q=r[n+"Team"].MIL||[];function w(O){return O.reduce((H,W)=>H+Te(W,"MIL"),0)}function k(O){let H=0;for(let W=0;W<O.length-1;W++){const K=at(O[W],O[W+1]);K==="#00ff88"?H+=2:K==="#FFD700"&&(H+=1)}return H}const f=w(M)+k(M),m=w(q)+k(q),x=f>=m;function b(O,H,W){return`<div id="duel-row-${W}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${O.map((K,ee)=>{const ne=ee<O.length-1?at(K,O[ee+1]):null,fe=!ne||ne==="#ff3333"||ne==="#cc2222",xe=ne==="#00ff88"?"+2":ne==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${W}" data-idx="${ee}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(m);let h=!1;const $=()=>{h||(h=!0,m.remove(),setTimeout(()=>f(),50))};m.addEventListener("click",$),setTimeout($,3500)}function Ae(M,q,w){var B,O;const k=(r.gcDefs||[]).find(H=>{var W;return H.name===M||((W=H.name)==null?void 0:W.toLowerCase().trim())===(M==null?void 0:M.toLowerCase().trim())}),f={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[k==null?void 0:k.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",m={purple:"#b06ce0",light_blue:"#00d4ef"}[k==null?void 0:k.color]||"#b06ce0",x=(k==null?void 0:k.name)||M,b=(k==null?void 0:k.effect)||((B=Ke[M])==null?void 0:B.desc)||"",h=k!=null&&k.image_url?`/manager-wars/icons/${k.image_url}`:null,$=((O=Ke[M])==null?void 0:O.icon)||"⚡",_=q===o?"Vous":r[q+"Name"]||"Adversaire",S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",S.innerHTML=`
      <style>
      ...
      </div>`,document.body.appendChild(w),(k=w.querySelector("#bp-close"))==null||k.addEventListener("click",()=>w.remove()),w.querySelectorAll(".bp-item").forEach(f=>{f.addEventListener("click",async()=>{const m=f.dataset.cid,x=q.find(h=>h.cardId===m);if(!x)return;const b=(M[x._line]||[]).find(h=>h.cardId===m);b&&(b.boost=(b.boost||0)+r.boostValue),w.remove(),await G({[o+"Team"]:M,boostUsed:!0})})})}function Ee(M=null){var L,j;if(!(r.phase===o+"-attack")){u("Remplacement uniquement avant votre attaque","warning");return}const w=r[o+"Team"],k=r["usedSubIds_"+o]||[],f=r.maxSubs||3;if(k.length>=f){u(`Maximum ${f} remplacements atteint`,"warning");return}const m=Object.entries(w).flatMap(([B,O])=>(O||[]).filter(H=>H.used).map(H=>({...H,_line:B}))),x=(r[o+"Subs"]||[]).filter(B=>!k.includes(B.cardId));if(!m.length){u("Aucun joueur utilisé à remplacer","warning");return}if(!x.length){u("Aucun remplaçant disponible","warning");return}let b=Math.max(0,m.findIndex(B=>B.cardId===M));const h=((L=m[b])==null?void 0:L._line)||((j=m[b])==null?void 0:j.job);let $=Math.max(0,x.findIndex(B=>B.job===h)),A=!1;const _=document.createElement("div");_.id="pvp-sub-overlay",_.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function S(){var ne,fe,xe,he,ve,Ie;const B=m[b],O=x[$],H=Math.min(130,Math.round((window.innerWidth-90)/2)),W=Math.round(H*1.35),K=Le=>`background:rgba(255,255,255,0.12);border:none;color:${Le?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Le?"default":"pointer"};flex-shrink:0`;_.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
    ...
    </div>`,(f=document.getElementById("pvp-home"))==null||f.addEventListener("click",()=>{try{E.removeChannel(R)}catch{}We(e),l("home")})}F()}const Uo=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:wn,resumePvpMatch:Ho},Symbol.toStringTag,{value:"Module"}));async function Vo(e,t,i,s){var c,d;try{const a=(d=(c=t==null?void 0:t.state)==null?void 0:c.profile)==null?void 0:d.id;try{(E.getChannels?E.getChannels():[]).forEach(u=>{const o=u.topic||"";(o.includes("matchmaking")||o.includes("pvp-match"))&&E.removeChannel(u)})}catch(l){console.warn("[FriendMatch] cleanup canaux:",l)}a&&(await E.rpc("cancel_matchmaking",{p_user_id:a}).catch(()=>{}),await E.from("matchmaking_queue").delete().eq("user_id",a).then(()=>{},()=>{}))}catch{}await ni(e,t,"random",({deckId:a,formation:l,starters:u,subsRaw:o,gcCardsEnriched:n,gcDefs:g,stadiumDef:v})=>{const y=T=>Ko(e,t,a,l,u,o,T,g||[],i,s);if(!n.length){y([]);return}ii(e,n,y)})}async function Ko(e,t,i,s,c,d,a=[],l=[],u,o){var se;const{state:n,navigate:g,toast:v}=t,y=n.profile.id;let T=!1,r=null;et(e);try{await E.rpc("cancel_matchmaking",{p_user_id:y})}catch{}try{await E.from("matchmaking_queue").delete().eq("user_id",y)}catch{}const p=(de,P,Y,V)=>{var F;e.innerHTML=`
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      </div>`,document.body.appendChild(f),(m=f.querySelector("#bp-close"))==null||m.addEventListener("click",()=>f.remove()),f.querySelectorAll(".bp-item").forEach(x=>{x.addEventListener("click",async()=>{const b=x.dataset.cid,h=k.find(A=>A.cardId===b);if(!h)return;const $=(w[h._line]||[]).find(A=>A.cardId===b);$&&($.boost=($.boost||0)+r.boostValue),f.remove(),await se({[o+"Team"]:w,boostUsed:!0})})})}function re(w=null){var B,O;if(!(r.phase===o+"-attack")){u("Remplacement uniquement avant votre attaque","warning");return}const f=r[o+"Team"],m=r["usedSubIds_"+o]||[],x=r.maxSubs||3;if(m.length>=x){u(`Maximum ${x} remplacements atteint`,"warning");return}const b=Object.entries(f).flatMap(([H,W])=>(W||[]).filter(K=>K.used).map(K=>({...K,_line:H}))),h=(r[o+"Subs"]||[]).filter(H=>!m.includes(H.cardId));if(!b.length){u("Aucun joueur utilisé à remplacer","warning");return}if(!h.length){u("Aucun remplaçant disponible","warning");return}let $=Math.max(0,b.findIndex(H=>H.cardId===w));const A=((B=b[$])==null?void 0:B._line)||((O=b[$])==null?void 0:O.job);let _=Math.max(0,h.findIndex(H=>H.job===A)),S=!1;const L=document.createElement("div");L.id="pvp-sub-overlay",L.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function j(){var xe,he,ve,Ie,Le,Re;const H=b[$],W=h[_],K=Math.min(130,Math.round((window.innerWidth-90)/2)),ee=Math.round(K*1.35),ne=je=>`background:rgba(255,255,255,0.12);border:none;color:${je?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${je?"default":"pointer"};flex-shrink:0`;L.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(V,F)=>F+1).reverse().map(V=>{const F=(u||[]).filter(oe=>oe.matchday===V);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${V}</div>${F.map(oe=>Ri(oe,g,d,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
    ...
    <div style="background:linear-gradient(160deg,#1a0a2e,#3b1a6e);border-radius:20px;padding:32px 24px;max-width:320px;width:100%;text-align:center;color:#fff;box-shadow:0 8px 40px rgba(0,0,0,0.6)">
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏆 Retour à la Mini League</button>`,document.body.appendChild($),(j=$.querySelector("#pvp-end-home"))==null||j.addEventListener("click",()=>{$.remove(),We(e),o?l("mini-league",{openLeagueId:o}):l("mini-league")})}const se=E.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},f=>{const m=f.new;try{if(m.status==="finished"||m.forfeit){if(I||(I=!0,U&&(clearInterval(U),U=null),m.game_state&&(p=m.game_state),p.phase==="finished"&&!m.forfeit&&document.getElementById("pvp-home")))return;ie(m);return}if(m.game_state){const x=p;p=m.game_state;const b=p._lastGC;if(b&&b.seq&&!G.has(b.seq)&&(G.add(b.seq),b.by!==n)){Ee(b.type,b.by,()=>ye());return}const h=x[n+"Score"]||0,$=x[g+"Score"]||0,A=p[n+"Score"]||0,_=p[g+"Score"]||0,S=A>h,L=_>$;if((S||L)&&!R.has(p.round)){R.add(p.round);const j=[...p.log||[]].reverse().find(O=>O.isGoal),B=((j==null?void 0:j.homePlayers)||[]).map(O=>({name:O.name,note:O.note,portrait:O.portrait,job:O.job}));$e(B,A,_,S,()=>ye());return}ye()}}catch(x){console.error("[PvP] crash:",x)}}).subscribe();async function de(f){Object.assign(p,f),p.lastActionAt=new Date().toISOString();const{error:m}=await E.from("matches").update({game_state:p}).eq("id",i);m&&u("Erreur de synchronisation","error");try{ye()}catch(x){console.error("[PvP] renderPvpScreen crash:",x)}}async function P(){if(I)return;I=!0,U&&(clearInterval(U),U=null);const f=s?T.away_id:T.home_id,m=s?"p2":"p1",x=s?"p1":"p2",b={...p,[m+"Score"]:3,[x+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:f,home_score:b.p1Score||0,away_score:b.p2Score||0,game_state:b}).eq("id",i)}catch{}try{E.removeChannel(se)}catch{}setTimeout(()=>{We(e),l("mini-league")},800)}const Y={BOOST_STAT:({value:f=1,count:m=1,roles:x=[]},b,h)=>{const $=b[n+"Team"],A=Object.entries($).filter(([_])=>!x.length||x.includes(_)).flatMap(([_,S])=>S.filter(L=>!L.used).map(L=>({...L,_line:_})));if(!A.length){b.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),h(b);return}V(A,m,`Choisir ${m} joueur(s) à booster (+${f})`,_=>{_.forEach(S=>{const L=($[S._line]||[]).find(j=>j.cardId===S.cardId);L&&(L.boost=(L.boost||0)+f,b.log.push({text:`⚡ +${f} sur ${L.name}`,type:"info"}))}),b[n+"Team"]=$,h(b)})},DEBUFF_STAT:({value:f=1,count:m=1,roles:x=[],target:b="ai"},h,$)=>{const A=b==="home"?n:g,_=h[A+"Team"],S=b==="home"?"allié":"adverse",L=Object.entries(_).filter(([j])=>!x.length||x.includes(j)).flatMap(([j,B])=>B.map(O=>({...O,_line:j})));if(!L.length){h.log.push({text:`🎯 Aucun joueur ${S}`,type:"info"}),$(h);return}V(L,m,`Choisir ${m} joueur(s) ${S}(s) (-${f})`,j=>{j.forEach(B=>{const O=(_[B._line]||[]).find(H=>H.cardId===B.cardId);O&&(O.boost=(O.boost||0)-f,h.log.push({text:`🎯 -${f} sur ${O.name}`,type:"info"}))}),h[A+"Team"]=_,$(h)})},GRAY_PLAYER:({count:f=1,roles:m=[],target:x="ai"},b,h)=>{const $=x==="home"?n:g,A=b[$+"Team"],_=x==="home"?"allié":"adverse",S=Object.entries(A).filter(([L])=>!m.length||m.includes(L)).flatMap(([L,j])=>j.filter(B=>!B.used).map(B=>({...B,_line:L})));if(!S.length){b.log.push({text:`❌ Aucun joueur ${_}`,type:"info"}),h(b);return}V(S,f,`Choisir ${f} joueur(s) ${_}(s) à exclure`,L=>{const j="usedCardIds_"+$,B=new Set(b[j]||[]);L.forEach(O=>{const H=(A[O._line]||[]).find(W=>W.cardId===O.cardId);H&&(H.used=!0,B.add(O.cardId),b.log.push({text:`❌ ${H.name} exclu !`,type:"info"}))}),b[j]=[...B],b[$+"Team"]=A,h(b)})},REVIVE_PLAYER:({count:f=1,roles:m=[]},x,b)=>{const h=x[n+"Team"],$=Object.entries(h).filter(([A])=>!m.length||m.includes(A)).flatMap(([A,_])=>_.filter(S=>S.used).map(S=>({...S,_line:A})));if(!$.length){x.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),b(x);return}V($,f,`Choisir ${f} joueur(s) à ressusciter`,A=>{A.forEach(_=>{const S=(h[_._line]||[]).find(L=>L.cardId===_.cardId);S&&(S.used=!1,x.log.push({text:`💫 ${S.name} ressuscité !`,type:"info"}))}),x[n+"Team"]=h,b(x)})},REMOVE_GOAL:({},f,m)=>{const x=g+"Score";f[x]>0?(f[x]--,f.log.push({text:"🚫 Dernier but annulé !",type:"info"})):f.log.push({text:"🚫 Aucun but à annuler",type:"info"}),m(f)},ADD_GOAL_DRAW:({},f,m)=>{f[n+"Score"]===f[g+"Score"]?(f[n+"Score"]++,f.log.push({text:"🎯 But bonus !",type:"info"})):f.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),m(f)},ADD_SUB:({value:f=1},m,x)=>{m.maxSubs=(m.maxSubs||3)+f,m.log.push({text:`🔄 +${f} remplacement(s)`,type:"info"}),x(m)},CUSTOM:({},f,m)=>m(f)};function V(f,m,x,b){const h=document.createElement("div");h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let $=[];function A(){var S,L;const _=f.map(j=>{const B={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[j._line]||"#555",O=Te(j,j._line)+(j.boost||0),W=$.find(ee=>ee.cardId===j.cardId)?"#FFD700":"rgba(255,255,255,0.25)",K=j.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${j.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${W};background:${B};overflow:hidden;cursor:pointer;${K}">
        ...
        </div>`,(S=h.querySelector("#pp-close"))==null||S.addEventListener("click",()=>h.remove()),h.querySelectorAll(".pp-item").forEach(j=>{j.addEventListener("click",()=>{const B=j.dataset.cid,O=f.find(W=>W.cardId===B),H=$.findIndex(W=>W.cardId===B);O&&(H>-1?$.splice(H,1):$.length<m&&$.push(O),A())})}),(L=h.querySelector("#pp-confirm"))==null||L.addEventListener("click",()=>{h.remove(),b($)})}A(),document.body.appendChild(h)}async function F(f,m){const b=(p["gcCardsFull_"+n]||[]).find(_=>_.id===f),h=(b==null?void 0:b._gcDef)||(p.gcDefs||[]).find(_=>{var S;return _.name===m||((S=_.name)==null?void 0:S.toLowerCase().trim())===(m==null?void 0:m.toLowerCase().trim())}),$=[...p["usedGc_"+n]||[],f],A={type:m,by:n,seq:(p._gcAnimSeq||0)+1};G.add(A.seq),Ee(m,n,async()=>{if(h!=null&&h.effect_type&&h.effect_type!=="CUSTOM"){const S=Y[h.effect_type];if(S){const L={...p};S(h.effect_params||{},L,async j=>{j["usedGc_"+n]=$,j._lastGC=A,j._gcAnimSeq=A.seq,await de(j)});return}}const _={...p};switch(m){case"Remplacement+":_.maxSubs=(_.maxSubs||3)+1,_.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const S=g+"Score";_[S]>0&&(_[S]--,_.log.push({text:"🚫 But annulé",type:"info"}));break}}_["usedGc_"+n]=$,_._lastGC=A,_._gcAnimSeq=A.seq,await de(_)})}function oe(f,m){const x="usedCardIds_"+f,b=new Set(p[x]||[]);m.forEach(h=>b.add(h)),p[x]=[...b]}function pe(){for(const f of["p1","p2"]){const m=new Set(p["usedCardIds_"+f]||[]),x=p[f+"Team"];if(x)for(const b of["GK","DEF","MIL","ATT"])(x[b]||[]).forEach(h=>{h.used=m.has(h.cardId)})}}function ye(){var ot,gt,ae,_e,Q,ce;if(p.phase==="reveal")return ze();if(p.phase==="midfield")return we();if(p.phase==="finished")return w();const f=p[n+"Team"],m=p[g+"Team"];pe();const x=p[n+"Score"],b=p[g+"Score"],h=p[n+"Name"],$=p[g+"Name"],A=p.phase===n+"-attack",_=p.phase===n+"-defense",S=Array.isArray(p["selected_"+n])?p["selected_"+n]:[],L=S.map(J=>J.cardId),j=window.innerWidth>=700,B=p[n+"Subs"]||[],O=p["usedSubIds_"+n]||[],H=B.filter(J=>!O.includes(J.cardId)),W=p["gcCardsFull_"+n]||[],K=p["usedGc_"+n]||[],ee=W.filter(J=>!K.includes(J.id)),ne=p.boostOwner===n&&!p.boostUsed,fe=!["GK","DEF","MIL","ATT"].some(J=>(m[J]||[]).some(le=>!le.used)),xe=[...f.MIL||[],...f.ATT||[]].filter(J=>!J.used),he=A&&fe&&xe.length===0?[...f.GK||[],...f.DEF||[]].filter(J=>!J.used).map(J=>J.cardId):[];function ve(J,le,Me){var zi,Si;const ke=J._gcDef,tt={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ke==null?void 0:ke.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",it={purple:"#b06ce0",light_blue:"#00d4ef"}[ke==null?void 0:ke.color]||"#b06ce0",rt=(ke==null?void 0:ke.name)||J.gc_type,st=(ke==null?void 0:ke.effect)||((zi=Ke[J.gc_type])==null?void 0:zi.desc)||"",ct=ke!=null&&ke.image_url?`/manager-wars/icons/${ke.image_url}`:null,$t=((Si=Ke[J.gc_type])==null?void 0:Si.icon)||"⚡",pt=Math.round(Me*.22),ht=Math.round(Me*.22),kt=Me-pt-ht,Ht=rt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${J.id}" data-gc-type="${J.gc_type}"
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ft(f,p[n+"Formation"],Ne,L,300,300,he)}
      </div>
    ...
    </div>`,n==="p1"&&setTimeout(async()=>{await de({phase:"midfield"})},5e3)}let Ae=!1;function we(){if(Ae)return;const f=p[n+"Team"].MIL||[],m=p[g+"Team"].MIL||[];function x(ee){return ee.reduce((ne,fe)=>ne+Te(fe,"MIL"),0)}function b(ee){let ne=0;for(let fe=0;fe<ee.length-1;fe++){const xe=at(ee[fe],ee[fe+1]);xe==="#00ff88"?ne+=2:xe==="#FFD700"&&(ne+=1)}return ne}const h=x(f)+b(f),$=x(m)+b(m),A=h>=$;function _(ee,ne,fe){return`<div id="duel-row-${fe}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(O);let H=!1;const W=()=>{H||(H=!0,O.remove(),setTimeout(()=>x&&x(),50))};O.addEventListener("click",W),setTimeout(W,3e3)}function te(f,m){var O,H,W,K;const b=(p["gcCardsFull_"+n]||[]).find(ee=>ee.id===f),h=b==null?void 0:b._gcDef,$={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[h==null?void 0:h.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",A={purple:"#b06ce0",light_blue:"#00d4ef"}[h==null?void 0:h.color]||"#b06ce0",_=(h==null?void 0:h.name)||m,S=(h==null?void 0:h.effect)||((O=Ke[m])==null?void 0:O.desc)||"Carte spéciale.",L=h!=null&&h.image_url?`/manager-wars/icons/${h.image_url}`:null,j=((H=Ke[m])==null?void 0:H.icon)||"⚡",B=document.createElement("div");B.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",B.innerHTML=`
      ...
      </div>`,document.body.appendChild(x),(b=x.querySelector("#bp-close"))==null||b.addEventListener("click",()=>x.remove()),x.querySelectorAll(".bp-item").forEach(h=>{h.addEventListener("click",async()=>{const $=h.dataset.cid,A=m.find(S=>S.cardId===$);if(!A)return;const _=(f[A._line]||[]).find(S=>S.cardId===$);_&&(_.boost=(_.boost||0)+p.boostValue),x.remove(),await de({[n+"Team"]:f,boostUsed:!0})})})}function N(f=null){var H,W;if(!(p.phase===n+"-attack")){u("Remplacement uniquement avant votre attaque","warning");return}const x=p[n+"Team"],b=p["usedSubIds_"+n]||[],h=p.maxSubs||3;if(b.length>=h){u(`Maximum ${h} remplacements atteint`,"warning");return}const $=Object.entries(x).flatMap(([K,ee])=>(ee||[]).filter(ne=>ne.used).map(ne=>({...ne,_line:K}))),A=(p[n+"Subs"]||[]).filter(K=>!b.includes(K.cardId));if(!$.length){u("Aucun joueur utilisé à remplacer","warning");return}if(!A.length){u("Aucun remplaçant disponible","warning");return}let _=Math.max(0,$.findIndex(K=>K.cardId===f));const S=((H=$[_])==null?void 0:H._line)||((W=$[_])==null?void 0:W.job);let L=Math.max(0,A.findIndex(K=>K.job===S)),j=!1;const B=document.createElement("div");B.id="pvp-sub-overlay",B.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function O(){var ve,Ie,Le,Re,je,Oe;const K=$[_],ee=A[L],ne=Math.min(130,Math.round((window.innerWidth-90)/2)),fe=Math.round(ne*1.35),xe=Be=>`background:rgba(255,255,255,0.12);border:none;color:${Be?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Be?"default":"pointer"};flex-shrink:0`;B.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(ve=B.querySelector("#psub-close"))==null||ve.addEventListener("click",()=>B.remove()),(Ie=B.querySelector("#pout-up"))==null||Ie.addEventListener("click",()=>{_>0&&(_--,O())}),(Le=B.querySelector("#pout-down"))==null||Le.addEventListener("click",()=>{_<$.length-1&&(_++,O())}),(Re=B.querySelector("#pin-up"))==null||Re.addEventListener("click",()=>{L>0&&(L--,O())}),(je=B.querySelector("#pin-down"))==null||je.addEventListener("click",()=>{L<A.length-1&&(L++,O())});const he=(Be,Ve,Ne,He)=>{const Ue=B.querySelector("#"+Be);if(!Ue)return;let Je=0;Ue.addEventListener("touchstart",Ze=>{Je=Ze.touches[0].clientY},{passive:!0}),Ue.addEventListener("touchend",Ze=>{const Qe=Ze.changedTouches[0].clientY-Je;if(Math.abs(Qe)<30)return;const ot=Ve();Qe<0&&ot<He-1?(Ne(ot+1),O()):Qe>0&&ot>0&&(Ne(ot-1),O())},{passive:!0})};he("pin-panel",()=>L,Be=>L=Be,A.length),he("pout-panel",()=>_,Be=>_=Be,$.length),(Oe=B.querySelector("#psub-confirm"))==null||Oe.addEventListener("click",async Be=>{if(Be.preventDefault(),Be.stopPropagation(),j)return;j=!0;const Ve=$[_],Ne=A[L];if(!Ve||!Ne)return;const He=Ve._line,Ue=(x[He]||[]).findIndex(Qe=>Qe.cardId===Ve.cardId);if(Ue===-1){u("Erreur : joueur introuvable","error"),B.remove();return}const Je={...Ne,_line:He,position:Ve.position,used:!1,boost:0};x[He].splice(Ue,1,Je);const Ze=[...b,Ne.cardId];B.remove(),Z(Ve,Ne,async()=>{await de({[n+"Team"]:x,[g+"Team"]:p[g+"Team"],["usedSubIds_"+n]:Ze})})})}document.body.appendChild(B),O()}function Z(f,m,x){const b={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},h=document.createElement("div");h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const $=(S,L,j)=>`<div style="text-align:center">
      ...
      <div style="width:min(90vw,420px)">${ut(p[g+"Team"],p[g+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(f),(m=f.querySelector("#pvp-opp-close"))==null||m.addEventListener("click",()=>f.remove())}function me(){var b;const f=p.log||[],m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column";const x=h=>{var A,_,S;if(h.type==="duel"){const L=h.isGoal,j=h.homeScored&&n==="p1"||!h.homeScored&&L&&n==="p2",B=h.homeScored?"#FFD700":L?"#ff6b6b":"rgba(255,255,255,0.3)",O=L?j?"⚽ BUT !":"⚽ BUT adversaire !":(A=h.homePlayers)!=null&&A.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${L?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${B};margin-bottom:4px">
      ...
      </div>`,document.body.appendChild(m),(b=m.querySelector("#pvp-hist-close"))==null||b.addEventListener("click",()=>m.remove())}async function X(){if(p.phase!==n+"-attack")return;const f=n==="p1"?"p2":"p1",m=(p.round||0)+1,x=[...p.log||[]];x.push({type:"info",text:`⏭️ ${p[n+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const b=De(p),h=qe(f),$=b||!h?"finished":f+"-attack";await de({["selected_"+n]:[],modifiers:{...p.modifiers,[n]:{}},pendingAttack:null,phase:$,attacker:f,round:m,log:x}),$==="finished"&&await q(p)}async function be(){const f=p[n+"Team"],m=!["GK","DEF","MIL","ATT"].some($=>(p[g+"Team"][$]||[]).some(A=>!A.used)),x=(p["selected_"+n]||[]).map($=>{const A=(f[$._role]||[]).find(O=>O.cardId===$.cardId)||$,_=m&&["GK","DEF"].includes($._role),S=f[$._role]||[],L=S.findIndex(O=>O.cardId===$.cardId),j=lt(S.length),B=L>=0?j[L]:A._col??1;return{...A,_line:$._role,_col:B,..._?{note_a:Math.max(1,Number(A.note_a)||0)}:{}}});if(!x.length)return;const b=Nt(x,p.modifiers[n]||{});oe(n,x.map($=>$.cardId)),x.forEach($=>{const A=(f[$._role]||[]).find(_=>_.cardId===$.cardId);A&&(A.used=!0)}),p["selected_"+n]=[],ye();const h=[...p.log||[]];if(m){const $=(p[n+"Score"]||0)+1,A=x.map(B=>({name:B.name,note:Te(B,B._line||"ATT"),portrait:Pe(B),job:B.job}));h.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:A,homeTotal:b.total,aiTotal:0});const _=(p.round||0)+1,S=n==="p1"?"p2":"p1",L={...p,[n+"Team"]:f,[n+"Score"]:$},j=De(L);R.add(_),$e(A,$,p[g+"Score"]||0,!0,async()=>{await de({[n+"Team"]:f,[n+"Score"]:$,["selected_"+n]:[],modifiers:{...p.modifiers,[n]:{}},pendingAttack:null,phase:j?"finished":S+"-attack",attacker:S,round:_,log:h}),j&&await q({...p,[n+"Score"]:$})});return}h.push({type:"pending",text:`⚔️ ${p[n+"Name"]} attaque (${b.total})`}),await de({[n+"Team"]:f,[g+"Team"]:p[g+"Team"],pendingAttack:{...b,players:x,side:n},["selected_"+n]:[],modifiers:{...p.modifiers,[n]:{}},phase:g+"-defense",log:h})}async function ge(){const f=p[n+"Team"],m=(p["selected_"+n]||[]).map(K=>{const ee=(f[K._role]||[]).find(ve=>ve.cardId===K.cardId)||K,ne=f[K._role]||[],fe=ne.findIndex(ve=>ve.cardId===K.cardId),xe=lt(ne.length),he=fe>=0?xe[fe]:ee._col??1;return{...ee,_line:K._role,_col:he}}),x=Rt(m,p.modifiers[n]||{});oe(n,m.map(K=>K.cardId)),m.forEach(K=>{const ee=(f[K._role]||[]).find(ne=>ne.cardId===K.cardId);ee&&(ee.used=!0)}),p["selected_"+n]=[],ye();const b=Ot(p.pendingAttack.total,x.total,p.modifiers[n]||{}),h=p.pendingAttack.side,$=b==="attack"||(b==null?void 0:b.goal),A=h==="p1"?"p2":"p1",_=(p.round||0)+1,S=(p.pendingAttack.players||[]).map(K=>({name:K.name,note:Te(K,K._line||"ATT"),portrait:Pe(K),job:K.job})),L=[...p.log||[]];L.push({type:"duel",isGoal:$,homeScored:$&&h===n,text:$?`⚽ BUT de ${p[h+"Name"]} ! (${p.pendingAttack.total} vs ${x.total})`:`✋ Attaque stoppée (${p.pendingAttack.total} vs ${x.total})`,homePlayers:S,aiPlayers:m.map(K=>({name:K.name,note:Te(K,K._line||"DEF"),portrait:Pe(K),job:K.job})),homeTotal:p.pendingAttack.total,aiTotal:x.total});const j=$?(p[h+"Score"]||0)+1:p[h+"Score"]||0,B={...p,[n+"Team"]:f,[h+"Score"]:j},O=De(B),H=O?"finished":A+"-attack",W=async()=>{await de({[n+"Team"]:f,[g+"Team"]:p[g+"Team"],[h+"Score"]:j,["selected_"+n]:[],modifiers:{...p.modifiers,[n]:{}},pendingAttack:null,phase:H,attacker:A,round:_,log:L}),(H==="finished"||O)&&await q({...p,[h+"Score"]:j})};if($){const K=h===n,ee=K?j:p[n+"Score"]||0,ne=K?p[g+"Score"]||0:j;R.add(_),$e(S,ee,ne,K,W)}else await W()}function Se(f){return["MIL","ATT"].some(m=>(f[m]||[]).some(x=>!x.used))}function Ce(f){return["GK","DEF","MIL","ATT"].some(m=>(f[m]||[]).some(x=>!x.used))}function Fe(f){return Ce(f)&&!Se(f)}function qe(f){const m=p[f+"Team"],x=p[(f==="p1"?"p2":"p1")+"Team"];return!!(Se(m)||!Ce(x)&&Fe(m))}function De(f){const m=["MIL","ATT"].some(L=>(f.p1Team[L]||[]).some(j=>!j.used)),x=["MIL","ATT"].some(L=>(f.p2Team[L]||[]).some(j=>!j.used)),b=Ce(f.p1Team),h=Ce(f.p2Team);return!m&&!(!h&&b)&&(!x&&!(!b&&h))}function M(f){const m=f.p1Score||0,x=f.p2Score||0;return m===x?null:m>x?T.home_id:T.away_id}async function q(f){try{const m=M(f),x=m?T.home_id===m?T.away_id:T.home_id:null,b=f.p1Score||0,h=f.p2Score||0;await E.from("matches").update({status:"finished",winner_id:m,home_score:b,away_score:h}).eq("id",i);const{data:$}=await E.from("mini_league_matches").select("id, league_id, matchday").eq("match_id",i).single();if($){await E.from("mini_league_matches").update({home_score:b,away_score:h,status:"finished"}).eq("id",$.id);const{data:A}=await E.from("mini_league_matches").select("id, status").eq("league_id",$.league_id).eq("matchday",$.matchday);if((A||[]).every(S=>S.status==="finished"||S.status==="bye")){const{data:S}=await E.from("mini_leagues").select("current_day,total_days").eq("id",$.league_id).single();if(S){const L=(S.current_day||0)+1,j=L>(S.total_days||0);await E.from("mini_leagues").update({current_day:j?S.total_days:L,status:j?"finished":"active"}).eq("id",$.league_id)}}}m&&x&&_i(m,x).catch(()=>{})}catch(m){console.error("[ML] finishMatch:",m)}}function w(){var h;if(I&&document.getElementById("pvp-end-overlay"))return;I=!0;const f=p[n+"Score"],m=p[g+"Score"],x=f>m,b=f===m;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div id="mkt-content" style="padding:10px 16px;display:flex;flex-direction:column;gap:8px"></div>
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:10px;color:#999;margin-top:1px">Vendeur : ${((R=r.seller)==null?void 0:R.pseudo)||"—"}</div>
      </div>
      <div style="text-align:right;flex-shrink:0">
    ...
    </div>`}function v(r){var U,D,R;const p=(U=r.card)==null?void 0:U.player;if(!p)return"";const I=Et(p,p.job),z=p.job2?Et(p,p.job2):0,C=r.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:8px;padding:10px 12px;overflow:hidden;padding-left:6px;${C?"opacity:0.7":""}">
      ${Hi(p.rarity)}
      ${Yt(I,p.job)}
      ${Yt(z,p.job2||null)}
      ${Ui(p.country_code)}
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:10px;color:${C?"#22c55e":"#999"};margin-top:1px">
          ${C?`✅ Vendu à ${((R=r.buyer)==null?void 0:R.pseudo)||"—"} · ${r.sold_at?new Date(r.sold_at).toLocaleDateString("fr"):""}`:`🟢 En vente depuis le ${new Date(r.listed_at).toLocaleDateString("fr")}`}
        </div>
      </div>
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:14px;font-weight:900;color:#D4A017">${r.price.toLocaleString("fr")}</div>
  ...
  `}async function hr(e,{state:t,navigate:i,toast:s}){const c=t.profile;if(!c)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await E.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
      ...
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${c.id},away_id.eq.${c.id}`).order("created_at",{ascending:!1}).limit(50),a={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},l=(d||[]).filter(n=>n.status==="finished"),u=(d||[]).filter(n=>n.status==="in_progress");function o(n){const g=n.home_id===c.id;g?n.home_score:n.away_score,g?n.away_score:n.home_score;const v=n.winner_id===c.id,y=n.home_score===n.away_score&&n.status==="finished",T=n.status!=="finished"?"…":y?"N":v?"V":"D",r=n.status!=="finished"||y?"#888":v?"#1A6B3C":"#c0392b";let p=a[n.mode]||n.mode;n.away_id===null&&!p.startsWith("IA")&&(p="IA");const z=n.home_id===c.id?n.away:n.home;let C;n.away_id===null?C=p:z?C=`${z.club_name||z.pseudo} (${z.pseudo})`:C="Adversaire";let U="";n.status==="in_progress"&&Date.now()-new Date(n.created_at).getTime()>3600*1e3&&(U=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const D=new Date(n.created_at),R=D.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+D.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),G=n.status==="finished"?`${n.home_score} - ${n.away_score}`:`${n.home_score||0} - ${n.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
        <div style="font-size:13px;font-weight:600">${C}${U}</div>
        <div style="font-size:11px;color:var(--gray-600)">${p} · ${R}${n.status==="in_progress"?" · en cours":""}</div>
      </div>
      <div style="display:flex;align-items:center;gap:8px">
