...
import{s as w,l as Gt,m as Ii,F as Si,r as Se,j as Jt,h as Ai,n as An,o as Mn,T as gi,k as jn,p as Cn,b as Bn}from"./formation-links-yJiMLlPw.js";const qn="/";function Ni(e,{navigate:t,toast:i}){let n="login";const o=()=>{var a,s,l,c,d,f;const r=n==="login";e.innerHTML=`
    <div style="
  ...
  `;function s(){var g;const c=document.getElementById("logo-preview"),d=document.getElementById("logo-initials"),f=((g=document.getElementById("setup-club"))==null?void 0:g.value)||"MW",x=f.trim().split(" ").filter(Boolean),m=x.length>=2?(x[0][0]+x[1][0]).toUpperCase():f.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=r),d&&(d.textContent=m,d.style.color=r)}document.getElementById("color1").addEventListener("input",c=>{r=c.target.value,document.getElementById("swatch1").style.background=r,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function l(c){document.querySelectorAll(".setup-step").forEach(d=>d.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("step1-error");if(d.textContent="",c.length<3){d.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await w.from("users").select("id").eq("pseudo",c).maybeSingle();if(f){d.textContent="Ce pseudo est déjà pris.";return}l(2)}),document.getElementById("step2-back").addEventListener("click",()=>l(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),d=document.getElementById("step2-error");if(d.textContent="",c.length<2){d.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await w.from("users").select("id").eq("club_name",c).maybeSingle();if(f){d.textContent="Ce nom de club est déjà pris.";return}l(3)}),document.getElementById("step3-back").addEventListener("click",()=>l(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),x=document.getElementById("step3-finish");f.textContent="",x.disabled=!0,x.textContent="Création en cours…";try{const{error:m}=await w.from("users").insert({id:t.user.id,pseudo:c,club_name:d,club_color1:r,club_color2:a,credits:1e4});if(m)throw m;await Fn(t.user.id),await o(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(m){f.textContent=m.message,x.disabled=!1,x.textContent="🚀 Créer mon profil !"}})}async function Fn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await w.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const Pn="modulepreload",Gn=function(e){return"/"+e},Oi={},rn=function(t,i,n){let o=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));o=Promise.allSettled(i.map(l=>{if(l=Gn(l),l in Oi)return;Oi[l]=!0;const c=l.endsWith(".css"),d=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${d}`))return;const f=document.createElement("link");if(f.rel=c?"stylesheet":Pn,c||(f.as="script"),f.crossOrigin="",f.href=l,s&&f.setAttribute("nonce",s),document.head.appendChild(f),c)return new Promise((x,m)=>{f.addEventListener("load",x),f.addEventListener("error",()=>m(new Error(`Unable to preload CSS for ${l}`)))})}))}function r(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return o.then(a=>{for(const s of a||[])s.status==="rejected"&&r(s.reason);return t().catch(r)})},Ut="#1A6B3C",Kt="#cc2222",Rn="#D4A017",Hi="#888";async function Nn(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </div>`,await an(i,n,t),document.getElementById("btn-add-friend").addEventListener("click",()=>Hn(i,n)),document.getElementById("btn-accept-friend").addEventListener("click",()=>sn(i,n,null,t))}async function an(e,t,i={}){const{navigate:n}=i,o=e.user.id,{data:r,error:a}=await w.from("friendships").select("id, requester_id, addressee_id").eq("status","accepted").or(`requester_id.eq.${o},addressee_id.eq.${o}`),{count:s}=await w.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",o).eq("status","pending"),l=document.getElementById("pending-badge");l&&(s>0?(l.style.display="flex",l.textContent=s):l.style.display="none");const c=document.getElementById("friends-list");if(!c)return;if(a){console.error("[Friends] Erreur:",a),c.innerHTML=`<div style="color:${Kt};text-align:center;padding:16px">Erreur chargement : ${a.message}</div>`;return}const d=(r||[]).map(m=>m.requester_id===o?m.addressee_id:m.requester_id);let f={};if(d.length){const{data:m}=await w.from("users").select("id, pseudo, club_name, last_seen_at, club_color1, club_color2").in("id",d);(m||[]).forEach(g=>{f[g.id]=g})}const x=(r||[]).map(m=>({friendshipId:m.id,friend:f[m.requester_id===o?m.addressee_id:m.requester_id]||{pseudo:"?"}}));if(!x.length){c.innerHTML=`
    ...
    </style>`}const Kn="2026.07.19-0034";async function Ui(e,{state:t,navigate:i,toast:n}){var r,a;const o=t.profile;o&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",q=>{G=q.target.value.toLowerCase(),j()}),e.querySelectorAll(".filter-btn").forEach(q=>{q.addEventListener("click",()=>{E=q.dataset.job,_(),j()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{N=!N,_(),j()})):(S.innerHTML=`
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(S=>{S.addEventListener("click",()=>{C=S.dataset.tab,E="Tous",G="",N=!1,e.querySelectorAll(".col-tab-btn").forEach(q=>{const P=q.dataset.tab===C;q.style.borderBottomColor=P?"var(--green)":"transparent",q.style.color=P?"var(--green)":"var(--gray-600)"}),_(),j()})}),_(),j()}function co(e,t,i){const n=Me[e]||{icon:"⚡",desc:"Effet spécial."},o=(t==null?void 0:t.gc_type)==="ultra_game_changer",r={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},a={purple:"#b06ce0",light_blue:"#00d4ef"},s=r[t==null?void 0:t.color]||r.purple,l=a[t==null?void 0:t.color]||a.purple,c=(t==null?void 0:t.name)||e,d=(t==null?void 0:t.effect)||n.desc,f=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(g=document.getElementById("direct-sell-form-btn"))==null||g.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${c} pour ${ei.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const b=x.find(h=>!h.is_for_sale)||x[0];if(!b){r("Aucune carte à vendre","error");return}await w.from("market_listings").delete().eq("card_id",b.id),await w.from("transfer_history").delete().eq("card_id",b.id);const{error:v}=await w.from("cards").delete().eq("id",b.id);if(v){r(v.message,"error");return}await w.from("users").update({credits:(o.profile.credits||0)+ei}).eq("id",o.profile.id),await l(),r(`+${ei.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),s("collection")}),(p=document.getElementById("market-sell-form-btn"))==null||p.addEventListener("click",async()=>{const b=parseInt(document.getElementById("sell-price-form").value);if(!b||b<1){r("Prix invalide","error");return}await w.from("cards").update({is_for_sale:!0,sale_price:b}).eq("id",e.id),await w.from("market_listings").insert({seller_id:o.profile.id,card_id:e.id,price:b}),r("Carte mise en vente sur le marché !","success"),a(),s("collection")}),(u=document.getElementById("cancel-sell-form-btn"))==null||u.addEventListener("click",async()=>{await w.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await w.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),r("Annonce retirée","success"),a(),s("collection")})}async function Xi(e,t,i,n){var re,ae,se,de,ke;const{state:o,toast:r,openModal:a,closeModal:s,navigate:l,refreshProfile:c}=n,d=e.player,f=t.filter(W=>W.player.id===d.id),x=f.length,m=e.evolution_bonus||0,g=Math.max((Number(d.note_g)||0)+(d.job==="GK"||d.job2==="GK"?m:0),(Number(d.note_d)||0)+(d.job==="DEF"||d.job2==="DEF"?m:0),(Number(d.note_m)||0)+(d.job==="MIL"||d.job2==="MIL"?m:0),(Number(d.note_a)||0)+(d.job==="ATT"||d.job2==="ATT"?m:0)),p=d.rarity||"normal",{data:u}=await w.from("sell_price_configs").select("*").eq("rarity",p).lte("note_min",g).gte("note_max",g).order("note_min",{ascending:!1}).limit(1);((re=u==null?void 0:u[0])==null?void 0:re.price)??no[p];const b=d.rarity!=="legende";so(d);const v=((d.rarity==="pepite"||d.rarity==="papyte")&&e.current_note!=null?e.current_note:xi(d,d.job))+m,h=d.rarity==="pepite"||d.rarity==="papyte",k=d.job2?(h?Zn(e,ro(d.job2)):xi(d,d.job2))+(xi(d,d.job2)>0?m:0):null;mi[d.job],d.job2&&mi[d.job2];const y=to[d.rarity]||"#ccc";ao[d.country_code]||d.country_code;const C=e.evolution_bonus||0,G=v+C,N=k||0,ne=N>0?N+C:0,ie=f.map(W=>W.id);let Q={};if(ie.length){const{data:W}=await w.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",ie).order("transferred_at",{ascending:!0});(W||[]).forEach(J=>{Q[J.card_id]||(Q[J.card_id]=[]),Q[J.card_id].push(J)})}const M=W=>{const J=W.transferred_at?new Date(W.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",R=W.source==="booster"?"Booster":W.price?W.price.toLocaleString("fr"):"—";return`
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(ae=document.getElementById("close-detail"))==null||ae.addEventListener("click",s);let j=new Set;const U=()=>{const W=j.size,J=document.getElementById("sell-action-panel");if(!J)return;J.style.display=W>0?"block":"none",document.getElementById("sell-selected-count").textContent=W;const R=document.getElementById("evolve-btn");R&&(R.textContent=`⬆️ Évoluer ${W>1?"(+"+W+")":""}`)};document.querySelectorAll(".expl-check").forEach(W=>{W.addEventListener("change",()=>{const J=W.dataset.id;W.checked?j.add(J):j.delete(J);const R=W.closest(".exemplaire-row");R&&(R.style.borderColor=W.checked?"#1A6B3C":"#eee"),U()})}),document.querySelectorAll(".exemplaire-row").forEach(W=>{W.addEventListener("click",J=>{if(J.target.closest("button")||J.target.tagName==="INPUT")return;const R=W.querySelector(".expl-check");R&&!R.disabled&&(R.checked=!R.checked,R.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(W=>{W.addEventListener("click",J=>{J.stopPropagation();const R=document.querySelector(`.expl-hist[data-hist="${W.dataset.idx}"]`);R&&(R.style.display=R.style.display==="none"?"flex":"none")})}),(se=document.getElementById("evolve-btn"))==null||se.addEventListener("click",async()=>{if(x<=1)return;const W=[...j];if(!W.length)return;if(j.has(e.id)){const V=document.createElement("div");V.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",V.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
  ...
  </div>`;const d=window.innerWidth>=900,f=e.querySelector(".deck-pc-layout"),x=e.querySelector(".deck-mobile-layout");f&&(f.style.display=d?"block":"none"),x&&(x.style.display=d?"none":"block"),fo(e,t,o,i),e.querySelectorAll("#builder-back").forEach(u=>u.addEventListener("click",()=>n("decks")));const m=()=>{const{openModal:u,closeModal:b}=i,v=`<div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px">
    ...
    </div>`;u("⚽ Choisir une formation",v),document.querySelectorAll("#modal-body [data-forma]").forEach(h=>{h.addEventListener("click",()=>{t.formation=h.dataset.forma;const k=Zi(t.formation),y={};k.forEach(C=>{t.slots[C]&&(y[C]=t.slots[C])}),t.slots=y,b(),xt(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",m)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>go(t,e,i))),e.querySelectorAll("#save-deck").forEach(u=>u.addEventListener("click",()=>yo(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{t.subs=t.subs.filter(b=>b!==u.dataset.removeSub),xt(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>xo(t,e,i)))}function fo(e,t,i,n){var h,k;const o=window.innerWidth>=900,r=e.querySelector(o?"#deck-field-pc":"#deck-field-mobile");if(!r)return;const a=(h=t.stadiumCards)==null?void 0:h.find(y=>y.id===t.stadiumCardId),s=a&&((k=t.stadDefMap)==null?void 0:k[a.stadium_id])||null,l=Jt[t.formation]||{},c=Ii(t.formation)||[],d={};for(const y of i){const C=t.slots[y],E=C?t.playerCards.find(G=>G.id===C):null;E!=null&&E.player?d[y]={...E.player,_evolution_bonus:E.evolution_bonus||0,face:E.player.face||null}:d[y]=null}const f=window.innerWidth>=900,x=f?window.innerWidth-280:window.innerWidth-20,m=f?Math.min(x,860):x,g=Math.round(f?m*.82:m*.85),p=f?84:44;let u="";for(const[y,C]of c){const E=l[y],G=l[C];if(!E||!G)continue;const N=E.x*m,ne=Math.round(.03*g+E.y*.85*g),ie=G.x*m,Q=Math.round(.03*g+G.y*.85*g),M=d[y],_=d[C],j=Gt(M,_);j==="#ff3333"||j==="#cc2222"?u+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${j}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(u+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${j}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,u+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${j}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let b="";const v=Math.round(p*657/507);for(const y of i){const C=l[y];if(!C)continue;const E=d[y],G=C.x*m;C.y*g;const N=Math.round(.03*g+C.y*(.85*g)),ne=Math.round(G-p/2),ie=Math.round(N-v/2);if(E){const Q=y.replace(/\d+/,""),M=s&&(s.club_id&&String(E.club_id)===String(s.club_id)||s.country_code&&E.country_code===s.country_code),_=Se({...E,_evolution_bonus:E._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:Q});b+=`<div style="position:absolute;left:${ne}px;top:${ie}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${y}">
        <div style="position:relative">${M?'<div style="position:absolute;top:-18px;left:0;right:0;text-align:center;font-size:14px;z-index:5;line-height:1">🏟️</div>':""}${_}</div>
    ...
    <div style="position:relative;width:${m}px;height:${g}px;margin:0 auto">
      ...
      ${r.map(s=>{var b,v;const l=e.stadDefMap[s.stadium_id],c=((b=l==null?void 0:l.club)==null?void 0:b.logo_url)||null,d=l!=null&&l.image_url?"/icons/"+l.image_url:null,f=e.stadiumCardId===s.id,x=l!=null&&l.country_code&&!c?`https://flagsapi.com/${l.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,m=(l==null?void 0:l.name)||"Stade",g=m.match(/^(Stade\s+(?:de\s+)?(?:\w+)?)(.*)?$/i),p=g?g[1].trim():m.slice(0,8),u=((v=g==null?void 0:g[2])==null?void 0:v.trim())||"";return`<div class="stad-choice" data-stad-id="${s.id}" style="cursor:pointer;width:80px;text-align:center">
  ...
  `,document.body.appendChild(o);let r=!1;const a=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let l=!1;const c=_=>_.touches&&_.touches[0]?_.touches[0].clientX:_.clientX;function d(_){r||(l=!0,s.style.opacity="1",f(_))}function f(_){if(!l||r)return;const j=a.getBoundingClientRect(),U=c(_)-j.left,re=Math.max(0,Math.min(1,U/j.width));s.style.width=re*j.width+"px",re>=.82&&m()}function x(){r||(l=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{r||(s.style.transition="")},220))}function m(){var j;if(r)return;r=!0,l=!1,s.style.width="100%",s.style.opacity="1",(j=document.getElementById("cut-flash"))==null||j.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const _=document.getElementById("cut-hint");_&&(_.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",b(0)},620)}a.addEventListener("pointerdown",d),window.addEventListener("pointermove",f),window.addEventListener("pointerup",x),a.addEventListener("touchstart",d,{passive:!0}),window.addEventListener("touchmove",f,{passive:!0}),window.addEventListener("touchend",x);let g=0,p=!1;const u=new Set;function b(_){g=_,document.getElementById("reveal-phase").style.display="flex",v(),h(_,0),G()}function v(){const _=document.getElementById("card-dots");_&&(_.innerHTML=e.map((j,U)=>`<div class="card-dot" data-i="${U}" style="width:8px;height:8px;border-radius:50%;background:${U===g?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),_.querySelectorAll(".card-dot").forEach(j=>j.addEventListener("click",()=>y(parseInt(j.dataset.i)))))}function h(_,j){var R,S;const U=e[_],re=document.getElementById("card-counter"),ae=document.getElementById("card-track");re&&(re.textContent=`Carte ${_+1} / ${e.length}`);const se=document.getElementById("reveal-btns");se&&(se.style.display=_===e.length-1?"flex":"none");const de=U.card_type==="player"&&((R=U.player)==null?void 0:R.rarity)==="legende",ke=!u.has(_);u.add(_);let W=0;if(U.card_type==="player"&&U.player){const q=U.player,P=q.job||"ATT";W=Number(P==="GK"?q.note_g:P==="DEF"?q.note_d:P==="MIL"?q.note_m:q.note_a)||0}const J=q=>{ae.innerHTML=`
        ...
        ${J}
      </div>`,S&&navigator.vibrate&&navigator.vibrate([50,30,100,30,200])},(D+O)*3),setTimeout(V,(D+O)*3+D),setTimeout(()=>{q.style.display="none",P.innerHTML="",p=!1,X&&!R&&X.pause(),navigator.vibrate&&navigator.vibrate([40,30,80]),j()},(D+O)*4)}function y(_){if(p||_<0||_>=e.length||_===g)return;const j=_>g?1:-1;g=_,h(_,j)}function C(){y(g+1)}function E(){y(g-1)}function G(){const _=document.getElementById("card-viewport");if(!_||_._swipeBound)return;_._swipeBound=!0;let j=0,U=0,re=0,ae=!1;const se=R=>R.touches?R.touches[0].clientX:R.clientX,de=R=>R.touches?R.touches[0].clientY:R.clientY,ke=R=>{ae=!0,j=se(R),U=de(R),re=0},W=R=>{if(!ae)return;re=se(R)-j;const S=de(R)-U;if(Math.abs(re)<Math.abs(S))return;const q=document.getElementById("current-card-wrap");q&&(q.style.transition="none",q.style.transform=`translateX(${re*.6}px) rotate(${re*.02}deg)`)},J=()=>{if(!ae)return;ae=!1;const R=document.getElementById("current-card-wrap"),S=55;re<=-S&&g<e.length-1?C():re>=S&&g>0?E():R&&(R.style.transition="transform .2s ease",R.style.transform="translateX(0)")};_.addEventListener("pointerdown",ke),_.addEventListener("pointermove",W),_.addEventListener("pointerup",J),_.addEventListener("pointercancel",J),_.addEventListener("touchstart",ke,{passive:!0}),_.addEventListener("touchmove",W,{passive:!0}),_.addEventListener("touchend",J),_.addEventListener("click",R=>{if(Math.abs(re)>8)return;const S=_.getBoundingClientRect();R.clientX-S.left>S.width/2?C():E()})}let N=null;function ne(){const _=document.getElementById("fireworks-canvas");if(!_)return;_.width=window.innerWidth,_.height=window.innerHeight;const j=_.getContext("2d"),U=[];function re(){const se=Math.random()*_.width,de=Math.random()*_.height*.6,ke=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],W=ke[Math.floor(Math.random()*ke.length)];for(let J=0;J<60;J++){const R=Math.PI*2/60*J,S=2+Math.random()*5;U.push({x:se,y:de,vx:Math.cos(R)*S,vy:Math.sin(R)*S,alpha:1,color:W,size:2+Math.random()*3})}}re(),N=setInterval(re,600);function ae(){if(document.getElementById("fireworks-canvas")){j.clearRect(0,0,_.width,_.height);for(let se=U.length-1;se>=0;se--){const de=U[se];if(de.x+=de.vx,de.y+=de.vy+.08,de.vy*=.98,de.alpha-=.018,de.alpha<=0){U.splice(se,1);continue}j.globalAlpha=de.alpha,j.fillStyle=de.color,j.beginPath(),j.arc(de.x,de.y,de.size,0,Math.PI*2),j.fill()}j.globalAlpha=1,(N!==null||U.length>0)&&requestAnimationFrame(ae)}}ae()}function ie(){N!==null&&(clearInterval(N),N=null);const _=document.getElementById("fireworks-canvas");_&&_.getContext("2d").clearRect(0,0,_.width,_.height)}if(n){const _=document.getElementById("reveal-btns");_&&(_.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(M=document.getElementById("reveal-next"))==null||M.addEventListener("click",()=>{ie(),o.remove(),n()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{ie(),o.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{ie(),o.remove(),i("boosters")})}function Lo(e){var t,i,n,o;if(e.card_type==="player"&&e.player)return wo(e);if(e.card_type==="game_changer"){const r=e._gcDef,a=(r==null?void 0:r.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},l={purple:"#b06ce0",light_blue:"#00d4ef"},c=s[r==null?void 0:r.color]||s.purple,d=l[r==null?void 0:r.color]||l.purple,f=(r==null?void 0:r.name)||e.gc_type||"Game Changer",x=(r==null?void 0:r.effect)||((t=wi[e.gc_type])==null?void 0:t.desc)||"",m=r!=null&&r.image_url?`/icons/${r.image_url}`:null,g=((i=wi[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${c};border-radius:14px;border:3px solid ${d};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${d}88;flex-shrink:0">
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}function Io(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText=`
    ...
    `,document.body.appendChild(t);let i=5;const n=setInterval(()=>{i--;const o=document.getElementById("mw-ad-cd");o&&(o.textContent=i),i<=0&&(clearInterval(n),t.remove(),e(!0))},1e3)})}async function So(e,{state:t,navigate:i,toast:n,refreshProfile:o}){var x,m;const{data:r}=await w.from("users").select("*").eq("id",t.user.id).single();r&&(t.profile=r);let a=Array.isArray((x=t.profile)==null?void 0:x.pending_boosters)?[...t.profile.pending_boosters]:[];if(!a.length){await w.from("users").update({onboarding_done:!0}).eq("id",t.user.id),i("home");return}let s=null;try{const p=(await gn()).find(u=>(u.name||"").toLowerCase().includes("new player"));p&&(s=xn(p))}catch(g){console.warn('[Onboarding] Config "Booster (new player)" introuvable, fallback taux par défaut',g)}const l=a.length;let c=0;e.innerHTML=`
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),l=[...new Set((s||[]).filter(x=>{var m,g;return((m=x.card)==null?void 0:m.card_type)==="stadium"&&((g=x.card)==null?void 0:g.stadium_id)}).map(x=>x.card.stadium_id))],c={};if(l.length){const{data:x}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",l);(x||[]).forEach(m=>{c[m.id]=m}),(s||[]).forEach(m=>{var g,p;((g=m.card)==null?void 0:g.card_type)==="stadium"&&((p=m.card)!=null&&p.stadium_id)&&(m.card._stadiumDef=c[m.card.stadium_id]||null)})}let d=0;function f(){var y,C,E,G,N,ne,ie;const x=r[d],m=(s||[]).filter(Q=>Q.deck_id===x.id),g=m.filter(Q=>{var M;return Q.is_starter&&((M=Q.card)==null?void 0:M.player)}).map(Q=>ki(Q.card,Q.position)),p=m.find(Q=>{var M;return((M=Q.card)==null?void 0:M.card_type)==="formation"}),u=x.formation||((y=p==null?void 0:p.card)==null?void 0:y.formation)||"4-4-2";let b=g.length>=11?ri(g,u):null,v=((C=x.stadium_card)==null?void 0:C.stadium_def)||null;v&&b&&(b=Xt(b,v));const h=g.length>=11;nt(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const _=window.innerWidth>=700,j=(R,S,q)=>{var ge,Be;const P=(t.gcDefs||[]).find(je=>je.name===R.gc_type),X={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[P==null?void 0:P.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",V={purple:"#b06ce0",light_blue:"#00d4ef"}[P==null?void 0:P.color]||"#b06ce0",D=(P==null?void 0:P.name)||R.gc_type,O=(P==null?void 0:P.effect)||((ge=Me[R.gc_type])==null?void 0:ge.desc)||"",we=P!=null&&P.image_url?`/icons/${P.image_url}`:null,_e=((Be=Me[R.gc_type])==null?void 0:Be.icon)||"⚡",ye=Math.round(q*.22),be=Math.round(q*.22),$e=q-ye-be,Le=D.length>12?7:9;return`<div class="gc-mini" data-gc-id="${R.id}" data-gc-type="${R.gc_type}"
          ...
          </div>`},re=(R,S)=>S?U(130,175):j(R,130,175),ae=(R,S)=>S?U(68,95):j(R,68,95),se=_?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",de=m?`<button id="btn-results" style="${se};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:d?`<div style="${se};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:u?`<button id="btn-pass" style="${se};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER (plus d'attaquants)</button>`:f?`<button id="btn-action" style="${se};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:x?`<button id="btn-action" style="${se};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${se};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,ke=f||x?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",W=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${_?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
        ...
        <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
          ${Pt(t.homeTeam,t.formation,t.phase,n,_?1300:Lt(),_?600:qt(),c)}
        </div>
      </div>`;return _?`
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(_=>{var j,U,re;if(_.type==="duel"){const ae=_.isGoal,se=_.homeScored?"#FFD700":ae?"#ff6b6b":"rgba(255,255,255,0.3)",de=_.homeScored?"⚽ BUT !":ae?"⚽ BUT IA !":(j=_.homePlayers)!=null&&j.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${ae?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${se};margin-bottom:4px">
                <div style="font-size:9px;color:${se};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${de}</div>
                ${(U=_.homePlayers)!=null&&U.length?`<div style="margin-bottom:3px">${Ye(_.homePlayers,"rgba(255,255,255,0.7)",_.homeTotal,void 0,t.formation)}</div>`:""}
                ${(re=_.aiPlayers)!=null&&re.length?`<div style="opacity:0.7">${Ye(_.aiPlayers,"#ff6b6b",_.aiTotal,void 0,t.formation)}</div>`:""}
                ...
                <div style="display:flex;align-items:center;gap:8px">
                  ${_.outPlayer?mt({..._.outPlayer,used:!0,_line:_.outPlayer.job,rarity:"normal"},38,50):""}
                  <span style="color:rgba(255,255,255,0.4);font-size:18px">→</span>
                  ${_.inPlayer?mt({..._.inPlayer,_line:_.inPlayer.job,rarity:"normal"},38,50):""}
                </div>
    ...
    </div>`,(y=f.querySelector("#sub-close"))==null||y.addEventListener("click",()=>f.remove()),(C=f.querySelector("#out-up"))==null||C.addEventListener("click",()=>{s>0&&(s--,x())}),(E=f.querySelector("#out-down"))==null||E.addEventListener("click",()=>{s<r.length-1&&(s++,x())}),(G=f.querySelector("#in-up"))==null||G.addEventListener("click",()=>{c>0&&(c--,x())}),(N=f.querySelector("#in-down"))==null||N.addEventListener("click",()=>{c<a.length-1&&(c++,x())});const k=(ie,Q,M,_)=>{const j=f.querySelector("#"+ie);if(!j)return;let U=0;j.addEventListener("touchstart",re=>{U=re.touches[0].clientY},{passive:!0}),j.addEventListener("touchend",re=>{const ae=re.changedTouches[0].clientY-U;if(Math.abs(ae)<30)return;const se=Q();ae<0&&se<_-1?(M(se+1),x()):ae>0&&se>0&&(M(se-1),x())},{passive:!0})};k("in-panel",()=>c,ie=>c=ie,a.length),k("out-panel",()=>s,ie=>s=ie,r.length),(ne=f.querySelector("#sub-confirm"))==null||ne.addEventListener("click",ie=>{if(ie.preventDefault(),ie.stopPropagation(),d)return;d=!0;const Q=r[s],M=a[c];if(!Q||!M)return;let _=null,j=-1;for(const[re,ae]of Object.entries(t.homeTeam)){const se=(ae||[]).findIndex(de=>de.cardId===Q.cardId);if(se!==-1){_=re,j=se;break}}if(j===-1||!_){_t("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),f.remove();return}const U={...M,_line:_,_col:Q._col||0,used:!1,boost:0};t.homeTeam[_].splice(j,1,U),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(M.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:Q.name,firstname:Q.firstname,note:Ge(Q,_),portrait:Wt(Q),job:Q.job,country_code:Q.country_code,rarity:Q.rarity,clubName:Q.clubName,clubLogo:Q.clubLogo},inPlayer:{name:M.name,firstname:M.firstname,note:Ge(M,_),portrait:Wt(M),job:M.job,country_code:M.country_code,rarity:M.rarity,clubName:M.clubName,clubLogo:M.clubLogo},text:`🔄 ${M.firstname} ${M.name} remplace ${Q.firstname} ${Q.name}`}),f.remove(),qo(Q,M,()=>Ee(e,t,i))})}document.body.appendChild(f),x()}function Yo(e,t,i,n,o){var g,p;const r=(n.gcDefs||[]).find(u=>u.name===t),a=Me[t]||{icon:"⚡",desc:"Carte spéciale."},s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[r==null?void 0:r.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",l={purple:"#b06ce0",light_blue:"#00d4ef"}[r==null?void 0:r.color]||"#b06ce0",c=(r==null?void 0:r.name)||t,d=(r==null?void 0:r.effect)||a.desc,f=r!=null&&r.image_url?`/icons/${r.image_url}`:null,x=a.icon||"⚡",m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",m.innerHTML=`
    ...
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const jt=e=>_n(e);async function zt(e,t,i,n,o={}){return Ln(e,t,i,n,o.myGC||[],o.gcDefs||[],o.isRanked||!1,o.rankedData||null,o.stadiumDef||null,o.onMatchEnd||null,o.mlLeagueId||null,o.mlMatchId||null)}async function Zo(e,t,i){const{data:n}=await w.from("matches").select("home_id,away_id,mode,is_ranked").eq("id",i).single();if(!n){t.toast("Match introuvable","error"),t.navigate("home");return}const o=n.home_id===t.state.user.id;let r=null,a=null;if(n.mode==="mini_league"){const{data:s}=await w.from("mini_league_matches").select("id, league_id").eq("match_id",i).maybeSingle();s&&(r=s.league_id,a=s.id)}return Ln(e,t,i,o,[],[],n.is_ranked||!1,null,null,null,r,a)}async function Ln(e,t,i,n,o=[],r=[],a=!1,s=null,l=null,c=null,d=null,f=null){const{state:x,navigate:m,toast:g}=t,p=n?"p1":"p2",u=n?"p2":"p1",b=(o||[]).map($=>$.id),v=(o||[]).map($=>({id:$.id,gc_type:$.gc_type,_gcDef:$._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:h}=await w.from("matches").select("*").eq("id",i).single();if(!h){g("Match introuvable","error"),m("home");return}async function k(){var me,xe;console.log("[PvP] buildGameState match:",{id:h.id,home_deck_id:h.home_deck_id,away_deck_id:h.away_deck_id,mode:h.mode,is_ranked:h.is_ranked});const[{data:$,error:L},{data:z,error:I},{data:T},{data:A}]=await Promise.all([w.rpc("get_deck_for_match",{p_deck_id:h.home_deck_id}),w.rpc("get_deck_for_match",{p_deck_id:h.away_deck_id}),w.from("users").select("id,pseudo,club_name").eq("id",h.home_id).single(),w.from("users").select("id,pseudo,club_name").eq("id",h.away_id).single()]);console.log("[PvP] get_deck_for_match p1:",L==null?void 0:L.message,"p2:",I==null?void 0:I.message,"p1D:",(me=$==null?void 0:$.starters)==null?void 0:me.length,"p2D:",(xe=z==null?void 0:z.starters)==null?void 0:xe.length);const F=te=>{const Te=Number(te.evolution_bonus)||0;return{cardId:te.card_id,position:te.position,id:te.id,firstname:te.firstname,name:te.surname_real,country_code:te.country_code,club_id:te.club_id,job:te.job,job2:te.job2,note_g:(Number(te.note_g)||0)+(te.job==="GK"||te.job2==="GK"&&Number(te.note_g)>0?Te:0),note_d:(Number(te.note_d)||0)+(te.job==="DEF"||te.job2==="DEF"&&Number(te.note_d)>0?Te:0),note_m:(Number(te.note_m)||0)+(te.job==="MIL"||te.job2==="MIL"&&Number(te.note_m)>0?Te:0),note_a:(Number(te.note_a)||0)+(te.job==="ATT"||te.job2==="ATT"&&Number(te.note_a)>0?Te:0),evolution_bonus:Te,rarity:te.rarity,skin:te.skin,hair:te.hair,hair_length:te.hair_length,face:te.face||null,clubName:te.club_encoded_name||null,clubLogo:te.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},B=(($==null?void 0:$.starters)||[]).map(te=>F(te)),K=((z==null?void 0:z.starters)||[]).map(te=>F(te)),Y=($==null?void 0:$.formation)||"4-4-2",oe=(z==null?void 0:z.formation)||"4-4-2";let Z=ri(B,Y),H=ri(K,oe);const le=(($==null?void 0:$.subs)||[]).map(te=>F(te)),pe=((z==null?void 0:z.subs)||[]).map(te=>F(te)),ue=($==null?void 0:$.stadium_def)||(n?l:null),ce=(z==null?void 0:z.stadium_def)||(n?null:l);return ue&&(Z=Xt(Z,ue),oi(le,ue)),ce&&(H=Xt(H,ce),oi(pe,ce)),{p1Team:Z,p2Team:H,p1Subs:le,p2Subs:pe,p1Formation:Y,p2Formation:oe,p1Name:(T==null?void 0:T.club_name)||(T==null?void 0:T.pseudo)||"Joueur 1",p2Name:(A==null?void 0:A.club_name)||(A==null?void 0:A.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?b:[],gc_p2:n?[]:b,gcCardsFull_p1:n?v:[],gcCardsFull_p2:n?[]:v,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:r||[],lastActionAt:new Date().toISOString()}}let y=h.game_state&&Object.keys(h.game_state).length?h.game_state:null;if(console.log("[PvP] init - amIHome:",n,"gameState exists:",!!y,"match.status:",h.status,"home_id:",h.home_id,"away_id:",h.away_id,"myId:",x.profile.id),!y)if(n){y=await k(),console.log("[PvP] home - gameState built, p1Team keys:",Object.keys((y==null?void 0:y.p1Team)||{}));const{data:$}=await w.from("matches").select("game_state").eq("id",i).single();!($!=null&&$.game_state)||!Object.keys($.game_state).length?await w.from("matches").update({game_state:y,turn_user_id:h.home_id}).eq("id",i):y=$.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>',console.log("[PvP] away - waiting for home to write game_state...");for(let $=0;$<30&&!y;$++){await new Promise(z=>setTimeout(z,400));const{data:L}=await w.from("matches").select("game_state").eq("id",i).single();L!=null&&L.game_state&&Object.keys(L.game_state).length&&(y=L.game_state),$%5===0&&console.log("[PvP] away - poll",$,"game_state:",!!(L!=null&&L.game_state))}if(!y){g("Erreur de synchronisation","error"),m("home");return}y.gc_p2=b,y.gcCardsFull_p2=v,await w.from("matches").update({game_state:y}).eq("id",i)}let C=!1,E=null,G=!1;const N=new Set,ne=new Set;async function ie($){var oe,Z;try{w.removeChannel(Q)}catch{}const L=y[p+"Score"]||0,z=y[u+"Score"]||0;let I,T;$.winner_id?(I=$.winner_id===x.profile.id,T=!1):$.forfeit?(I=L>z,T=!1):(T=L===z,I=L>z);let A="";if(a&&n)try{const{data:H}=await w.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",h.home_id).single(),{data:le}=await w.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",h.away_id).single();if(H&&le){const pe=I?1:T?.5:0,ue=1-pe,ce=H.placement_matches<10,me=le.placement_matches<10,xe=computeGlicko2(H.mmr,H.mmr_deviation,H.mmr_volatility,le.mmr,le.mmr_deviation,pe===1?1:pe===0?0:.5,ce),te=computeGlicko2(le.mmr,le.mmr_deviation,le.mmr_volatility,H.mmr,H.mmr_deviation,ue===1?1:ue===0?0:.5,me);await w.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:T?null:I?h.home_id:h.away_id,p_home_id:h.home_id,p_away_id:h.away_id,p_home_delta:xe.delta,p_away_delta:te.delta,p_home_new_rd:xe.newRd,p_away_new_rd:te.newRd,p_home_new_vol:xe.newSigma,p_away_new_vol:te.newSigma});const Te=xe.delta,Fe=xe.newMmr,Ie=getTier(Fe),He=Te>=0?"+":"",It=Te>=0?"#4caf50":"#ff6b6b",Je=getTier(H.mmr);A=`
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      </div>`,(Fe=Z.querySelector("#psub-close"))==null||Fe.addEventListener("click",()=>Z.remove()),(Ie=Z.querySelector("#pout-up"))==null||Ie.addEventListener("click",()=>{B>0&&(B--,H())}),(He=Z.querySelector("#pout-down"))==null||He.addEventListener("click",()=>{B<A.length-1&&(B++,H())}),(It=Z.querySelector("#pin-up"))==null||It.addEventListener("click",()=>{Y>0&&(Y--,H())}),(Je=Z.querySelector("#pin-down"))==null||Je.addEventListener("click",()=>{Y<F.length-1&&(Y++,H())});const Te=(qe,Qe,Re,ht)=>{const Ze=Z.querySelector("#"+qe);if(!Ze)return;let Ue=0;Ze.addEventListener("touchstart",at=>{Ue=at.touches[0].clientY},{passive:!0}),Ze.addEventListener("touchend",at=>{const st=at.changedTouches[0].clientY-Ue;if(Math.abs(st)<30)return;const vt=Qe();st<0&&vt<ht-1?(Re(vt+1),H()):st>0&&vt>0&&(Re(vt-1),H())},{passive:!0})};Te("pin-panel",()=>Y,qe=>Y=qe,F.length),Te("pout-panel",()=>B,qe=>B=qe,A.length),(Qt=Z.querySelector("#psub-confirm"))==null||Qt.addEventListener("click",async qe=>{if(qe.preventDefault(),qe.stopPropagation(),oe)return;oe=!0;const Qe=A[B],Re=F[Y];if(!Qe||!Re)return;const ht=Qe._line,Ze=(z[ht]||[]).findIndex(st=>st.cardId===Qe.cardId);if(Ze===-1){g("Erreur : joueur introuvable","error"),Z.remove();return}const Ue={...Re,_line:ht,position:Qe.position,used:!1,boost:0};z[ht].splice(Ze,1,Ue);const at=[...I,Re.cardId];Z.remove(),X(Qe,Re,async()=>{await M({[p+"Team"]:z,[u+"Team"]:y[u+"Team"],["usedSubIds_"+p]:at})})})}document.body.appendChild(Z),H()}function X($,L,z){const I={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const A=(K,Y,oe)=>`<div style="text-align:center">
    ...
    </div>`,(T=document.getElementById("pvp-home"))==null||T.addEventListener("click",()=>{try{w.removeChannel(Q)}catch{}We(e),d?m("mini-league",{openLeagueId:d}):m("home")})}de()}async function Li(e,t,i=!1){var s,l;const{state:n,navigate:o,toast:r}=t,a=i&&((l=(s=t==null?void 0:t.state)==null?void 0:s.params)==null?void 0:l.rankedData)||null;await pi(e,t,i?"ranked":"random",async({deckId:c,formation:d,starters:f,subsRaw:x,gcCardsEnriched:m,gcDefs:g,stadiumDef:p})=>{nt(e);const u=async b=>{await tr(e,t,c,d,f,x,b||[],g,p,i,a)};if(!(m!=null&&m.length)){await u([]);return}li(e,m,u)})}async function er(e,t,i){return Zo(e,t,i)}async function tr(e,t,i,n,o,r,a=[],s=[],l=null,c=!1,d=null){var ie,Q;const{state:f,navigate:x,toast:m}=t,g=(d==null?void 0:d.mmr)??((ie=f.profile)==null?void 0:ie.mmr)??1e3,p=c?Ai(g):null,u=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",b=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let v=!1,h=null,k=null;e.innerHTML=`
    ...
    </div>`;const y=async()=>{v=!0,h&&(h.unsubscribe(),h=null),k&&(clearInterval(k),k=null);try{await w.rpc("cancel_matchmaking",{p_user_id:f.profile.id})}catch(M){console.error("[Matchmaking] cancel error:",M)}};(Q=document.getElementById("cancel-mm"))==null||Q.addEventListener("click",async()=>{await y(),We(e),x("home")});const C=async(M,_)=>{v||(v=!0,h&&(h.unsubscribe(),h=null),k&&(clearInterval(k),k=null),await zt(e,t,M,_,{myGC:a,gcDefs:s,stadiumDef:l,isRanked:c,rankedData:d,onMatchEnd:c?ir:null}))},E=f.user.id,{data:G,error:N}=c?await w.rpc("try_matchmake_ranked",{p_user_id:E,p_deck_id:i,p_mmr:g}):await w.rpc("try_matchmake",{p_user_id:E,p_deck_id:i});if(N||!(G!=null&&G.success)){console.error("[Matchmaking] try_matchmake error:",N||G),m("Erreur de matchmaking","error"),We(e),x("home");return}if(G.matched){await C(G.match_id,!1);return}const ne=async()=>{if(v)return;const{data:M}=await w.from("matchmaking_queue").select("status, match_id").eq("user_id",E).maybeSingle();(M==null?void 0:M.status)==="matched"&&M.match_id&&await C(M.match_id,!0)};h=w.channel(`mm_${E}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${E}`},M=>{const _=M.new;_.status==="matched"&&_.match_id&&C(_.match_id,!0)}).subscribe(),k=setInterval(ne,3e3),setTimeout(()=>{v||y()},12e4)}async function ir(e,t){const{state:i,toast:n}=t,{winnerId:o,homeId:r,awayId:a,homeScore:s,awayScore:l,matchId:c}=e,d=i.user.id,f=r===d,x=o===d,{data:m}=await w.from("users").select("mmr, mmr_rd, mmr_v").eq("id",d).single(),{data:g}=await w.from("users").select("mmr, mmr_rd, mmr_v").eq("id",f?a:r).single();if(m&&g){const p=An(m.mmr,m.mmr_rd,m.mmr_v,g.mmr,g.mmr_rd,x?1:0);await w.from("users").update({mmr:p.mmr,mmr_rd:p.rd,mmr_v:p.v,mmr_wins:x?w.sql`mmr_wins + 1`:void 0,mmr_losses:x?void 0:w.sql`mmr_losses + 1`}).eq("id",d),n(x?`+MMR ↑ ${Math.round(p.mmr-m.mmr)}`:`-MMR ↓ ${Math.round(m.mmr-p.mmr)}`,x?"success":"error",4e3)}}const nr=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:Li,resumePvpMatch:er},Symbol.toStringTag,{value:"Module"}));async function or(e,t){const{state:i,navigate:n,toast:o}=t,r=i.params||{},a=r.friendId||null,s=r.friendName||"Ami",l=!!r.isAccepting;await pi(e,t,"friend",async({deckId:c,formation:d,starters:f,subsRaw:x,gcCardsEnriched:m,gcDefs:g,stadiumDef:p})=>{nt(e);const u=async b=>{const v=b||[];l?await ar(e,t,c,v,g,p):await rr(e,t,c,v,g,p,a,s)};if(!(m!=null&&m.length)){await u([]);return}li(e,m,u)})}async function rr(e,t,i,n,o,r,a,s){var v;const{state:l,navigate:c,toast:d}=t,f=l.user.id,{data:x,error:m}=await w.from("friend_match_invites").insert({inviter_id:f,invitee_id:a,friend_id:a,status:"pending",inviter_deck_id:i,expires_at:new Date(Date.now()+2*60*1e3).toISOString()}).select("id").single();if(m||!x){console.error("[Friend] Erreur création invitation:",m),d("Impossible de créer l'invitation","error"),c("home");return}e.innerHTML=`
    ...
    </div>`;let g=!1,p=null;const u=async()=>{p&&(p.unsubscribe(),p=null)};(v=document.getElementById("cancel-friend"))==null||v.addEventListener("click",async()=>{await u(),await w.from("friend_match_invites").update({status:"declined"}).eq("id",x.id),We(e),c("home")}),p=w.channel(`friend_invite_${x.id}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${x.id}`},async({new:h})=>{g||h.status!=="accepted"||!h.match_id||(g=!0,await u(),await zt(e,t,h.match_id,!0,{myGC:n,gcDefs:o,stadiumDef:r}))}).subscribe();const b=setInterval(async()=>{if(g)return;const{data:h}=await w.from("friend_match_invites").select("status, match_id").eq("id",x.id).single();(h==null?void 0:h.status)==="accepted"&&h.match_id&&(g=!0,clearInterval(b),await u(),await zt(e,t,h.match_id,!0,{myGC:n,gcDefs:o,stadiumDef:r}))},3e3);setTimeout(async()=>{g||(clearInterval(b),await u(),d("Invitation expirée","info"),c("home"))},12e4)}async function ar(e,t,i,n,o,r){const{state:a,navigate:s,toast:l}=t,c=a.user.id,{data:d}=await w.from("friend_match_invites").select("id, inviter_id").eq("invitee_id",c).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle();if(!d){l("Aucune invitation en attente","error"),s("home");return}const{data:f,error:x}=await w.rpc("accept_friend_invite",{p_invite_id:d.id,p_invitee_deck_id:i});if(x||!(f!=null&&f.success)){console.error("[Friend] Erreur accept_friend_invite:",x||f),l((f==null?void 0:f.error)||"Impossible de rejoindre le match","error"),s("home");return}await zt(e,t,f.match_id,!1,{myGC:n,gcDefs:o,stadiumDef:r})}const Tn="#1A6B3C",gt="#D4A017",Nt="rgba(255,255,255,0.045)",kt="rgba(255,255,255,0.09)",$t="rgba(255,255,255,0.08)",Ce="#f3f5f2",it="rgba(243,245,242,0.62)",Ke="rgba(243,245,242,0.4)";async function sr(e,t){var n;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const i=(n=t.state.params)==null?void 0:n.openLeagueId;if(i){t.state.params.openLeagueId=null,await rt(e,t,i);return}await yt(e,t)}async function yt(e,t,i="waiting"){var v,h;const{state:n}=t,o=n.profile.id,{data:r}=await w.from("mini_league_members").select("league_id").eq("user_id",o),a=(r||[]).map(k=>k.league_id),{data:s,error:l}=await w.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").eq("is_archived",!1).order("created_at",{ascending:!1}).limit(30),c=l?(await w.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").order("created_at",{ascending:!1}).limit(30)).data||[]:s||[],f=(a.length?await w.from("mini_leagues").select("*, mini_league_members(count)").in("id",a).order("created_at",{ascending:!1}):{data:[]}).data||[],x=f.filter(k=>k.status==="waiting"&&!k.is_archived),m=f.filter(k=>k.status==="active"&&!k.is_archived),g=f.filter(k=>k.is_archived||k.status==="finished");let p=g;if(g.length){const{data:k}=await w.from("mini_league_matches").select("league_id").in("league_id",g.map(C=>C.id)).or(`home_id.eq.${o},away_id.eq.${o}`).not("status","eq","bye"),y=new Set((k||[]).map(C=>C.league_id));p=g.filter(C=>y.has(C.id))}const u=c.filter(k=>!a.includes(k.id)),b=[{key:"waiting",label:"🟡 En attente",count:x.length+u.length},{key:"active",label:"🟢 En cours",count:m.length},{key:"archived",label:"📁 Archivées",count:p.length}];e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?dr(x,u,o):i==="active"?lr(m,o):cr(p,o)}
    </div>
  </div>`,(v=document.getElementById("ml-create-btn"))==null||v.addEventListener("click",()=>pr(e,t)),(h=document.getElementById("ml-refresh-list"))==null||h.addEventListener("click",()=>yt(e,t,i)),e.querySelectorAll(".ml-tab").forEach(k=>k.addEventListener("click",()=>yt(e,t,k.dataset.tab))),e.querySelectorAll("[data-league-id]").forEach(k=>k.addEventListener("click",()=>rt(e,t,k.dataset.leagueId))),e.querySelectorAll("[data-join]").forEach(k=>k.addEventListener("click",y=>{y.stopPropagation(),zn(e,t,k.dataset.join,k.dataset.type)})),e.querySelectorAll("[data-delete]").forEach(k=>k.addEventListener("click",y=>{y.stopPropagation(),Ti(e,t,k.dataset.delete,k.dataset.name,i)}))}function di(e,t,i=!1){var a,s;const n=e.creator_id===t,o=e.pot||0,r=((s=(a=e.mini_league_members)==null?void 0:a[0])==null?void 0:s.count)||0;return`<div data-league-id="${e.id}" style="background:${Nt};border:1px solid ${kt};border-radius:12px;padding:14px 16px;cursor:pointer;margin-bottom:8px">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Nt};border:1px solid ${kt};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${x.map((M,_)=>{const j=g>0&&a.status==="finished"?_===0?Math.floor(g*.7):_===1?Math.floor(g*.2):_===2?Math.floor(g*.1):0:0;return`<tr style="border-bottom:1px solid ${$t};${M.userId===r?"background:rgba(74,222,128,0.08);":""}">
              <td style="padding:7px 3px 7px 0;font-weight:700;color:${_===0?gt:_<3?"#4ade80":it}">${["🥇","🥈","🥉"][_]||_+1}</td>
      ...
      <div style="background:${Nt};border:1px solid ${kt};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(M,_)=>_+1).reverse().map(M=>{const _=(l||[]).filter(j=>j.matchday===M);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${Ke};margin-bottom:6px">Journée ${M}</div>${_.map(j=>tn(j,f,r,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
    ...
    </div>`;let d=!1;const f=async u=>{if(!d){if(d=!0,clearInterval(g),m)try{m.unsubscribe()}catch{}u&&await zt(e,t,u,o,{mlLeagueId:n,mlMatchId:i,myGC:r||[],gcDefs:a,stadiumDef:s}),l()}};(p=document.getElementById("wait-back"))==null||p.addEventListener("click",()=>{if(d=!0,clearInterval(g),m)try{m.unsubscribe()}catch{}c("mini-league",{openLeagueId:n}),l()});const x=async()=>{if(d)return;const{data:u}=await w.from("mini_league_matches").select("match_id").eq("id",i).single();u!=null&&u.match_id&&f(u.match_id)},m=w.channel(`ml_match_${i}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"mini_league_matches",filter:`id=eq.${i}`},u=>{var b;(b=u.new)!=null&&b.match_id&&f(u.new.match_id)}).subscribe(),g=setInterval(x,3e3);x()})}const wr="/",_r=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
    ...
    </div>`,document.body.appendChild(o);const r=s=>{o.remove(),s&&t()};o.querySelector("#buy-cancel").addEventListener("click",()=>r(!1)),o.querySelector("#buy-ok").addEventListener("click",()=>r(!0)),o.addEventListener("click",s=>{s.target===o&&r(!1)})}async function zr(e,t,i){const{toast:n}=i,{data:o}=await w.from("market_listings").select("card_id").eq("id",e).single();if(await w.from("market_listings").update({status:"cancelled"}).eq("id",e),o!=null&&o.card_id){const{count:r}=await w.from("market_listings").select("id",{count:"exact",head:!0}).eq("card_id",o.card_id).eq("status","active");r||await w.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",o.card_id)}n("Annonce retirée","success"),Fi(t,i)}async function Ir(e,t){var d,f,x,m;const{state:i,navigate:n}=t,o=((f=(d=t==null?void 0:t.state)==null?void 0:d.params)==null?void 0:f.tab)||"global";e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const[{data:r},{data:a}]=await Promise.all([w.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100),w.from("users").select("id,pseudo,club_name,mmr,rank_tier,ranked_wins,ranked_losses,ranked_draws,placement_matches").gte("placement_matches",1).order("mmr",{ascending:!1}).limit(100)]);let s=o;function l(){var p,u;const g=document.getElementById("rankings-list");if(g){if(s==="global"){const b=r||[];g.innerHTML=b.length>0?b.map((v,h)=>`
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const b=a||[];g.innerHTML=b.length>0?b.map((v,h)=>{const k=Ai(v.mmr??1e3),y=(v.ranked_wins||0)+(v.ranked_losses||0)+(v.ranked_draws||0),C=y>0?Math.round((v.ranked_wins||0)/y*100):0,E=v.id===i.profile.id,G=(v.placement_matches||0)<10;return`
  ...
  </div>`,(n=e.querySelector("#pl-play-btn"))==null||n.addEventListener("click",t)}async function qr(){document.documentElement.setAttribute("data-theme","light"),document.getElementById("modal-overlay").addEventListener("click",i=>{i.target===i.currentTarget&&zi()}),document.getElementById("modal-close").addEventListener("click",zi);const{data:{session:e}}=await w.auth.getSession();if(!e){on(),Br(document.getElementById("app"),{onPlay:()=>Ni(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Bt})});return}ve.user=e.user,await Ot(),on();try{const{data:i}=await w.from("formation_links_overrides").select("formation, links"),n={};(i||[]).forEach(o=>{n[o.formation]=o.links}),Cn(n)}catch(i){console.warn("Impossible de charger les overrides de formation:",i)}if(!ve.profile){Dn(document.getElementById("app"),{state:ve,navigate:async()=>{await Ot(),hi()},toast:Bt,refreshProfile:Ot});return}const t=Array.isArray(ve.profile.pending_boosters)?ve.profile.pending_boosters:[];if(!ve.profile.onboarding_done&&t.length>0){So(document.getElementById("app"),{state:ve,navigate:()=>hi(),toast:Bt,refreshProfile:Ot});return}hi(),setTimeout(()=>$r(ve.profile,Ht,Bt),800),w.auth.onAuthStateChange(async(i,n)=>{i==="SIGNED_OUT"&&(ve.user=null,ve.profile=null,document.getElementById("app").innerHTML="",Ni(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Bt}))})}function Dr(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function ui(){const e=document.getElementById("app");e&&(e.style.height=Dr()+"px")}window.addEventListener("resize",ui);window.addEventListener("orientationchange",()=>setTimeout(ui,150));window.visualViewport&&window.visualViewport.addEventListener("resize",ui);function hi(){const e=()=>{var i;(i=ve.user)!=null&&i.id&&w.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",ve.user.id).then(()=>{})};e(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(e,6e4);const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",ui(),jr(),In()}function on(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function Sn(e){var n;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
