import{s as w,F as W,b as ye,c as ce,l as J,d as Xe}from"./formation-links-BD9zicgB.js";function we(t,{navigate:e,toast:i}){t.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),r=(c||[]).filter(E=>E.card_type==="player"&&E.player),d=(c||[]).filter(E=>E.card_type==="game_changer"),u=(c||[]).filter(E=>E.card_type==="formation"),p=Object.keys(W),f=Object.keys(re),g={};r.forEach(E=>{const I=E.player.id;g[I]=(g[I]||0)+1});const m=new Set(Object.keys(g).map(E=>String(E))),y=new Set(u.map(E=>E.formation)),k=new Set(d.map(E=>E.gc_type));let h="player",v="Tous",x="",$=!1;function _(){return[...r].sort((E,I)=>{const A=te.indexOf(E.player.job),z=te.indexOf(I.player.job);return A!==z?A-z:(E.player.surname_encoded||"").localeCompare(I.player.surname_encoded||"")})}function b(){return[...l||[]].sort((E,I)=>{const A=te.indexOf(E.job),z=te.indexOf(I.job);return A!==z?A-z:(E.surname_encoded||"").localeCompare(I.surname_encoded||"")})}function T(){return _().filter(E=>{const I=E.player,A=v==="Tous"||I.job===v,z=!x||`${I.firstname} ${I.surname_encoded}`.toLowerCase().includes(x);return A&&z})}function M(){return b().filter(E=>{const I=v==="Tous"||E.job===v,A=!x||`${E.firstname} ${E.surname_encoded}`.toLowerCase().includes(x);return I&&A})}t.innerHTML=`
  <div class="page">
    ...
    <div class="page-body">
      <div style="display:flex;flex-wrap:wrap;gap:10px;justify-content:flex-start" id="col-grid"></div>
  ...
  </div>`;function C(){const E=document.getElementById("col-filters");E&&(h==="player"?(E.innerHTML=`
          ...
          ${it.map(I=>`
            <button class="filter-btn" data-job="${I}"
              style="flex-shrink:0;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;cursor:pointer;
                border:1.5px solid ${I===v?"var(--green)":"var(--gray-200)"};
                background:${I===v?"var(--green)":"#fff"};
                color:${I===v?"#fff":"var(--gray-600)"}">
              ${I}
            </button>`).join("")}
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",I=>{x=I.target.value.toLowerCase(),L()}),t.querySelectorAll(".filter-btn").forEach(I=>{I.addEventListener("click",()=>{v=I.dataset.job,C(),L()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{$=!$,C(),L()})):(E.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{$=!$,C(),L()})))}function L(){const E=document.getElementById("col-grid");E&&(h==="player"?j(E):h==="formation"?ee(E):le(E))}function j(E){if($){const I=M();if(!I.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}E.innerHTML=I.map(A=>{if(m.has(String(A.id))){const S=r.find(O=>O.player.id===A.id),D=g[A.id]||1,R=D>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${D}</div>`:"",q=r.filter(O=>O.player.id===A.id&&O.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Ee(S,R+q)}return nt(A)}).join("")}else{const I=T();if(!I.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const A={},z=[];I.forEach(S=>{const D=S.player.id;A[D]||(A[D]=!0,z.push(S))}),E.innerHTML=z.map(S=>{const D=g[S.player.id]||1,R=D>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${D}</div>`:"",q=r.filter(O=>O.player.id===S.player.id&&O.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Ee(S,R+q)}).join("")}E.querySelectorAll("[data-card-id]").forEach(I=>{I.addEventListener("click",()=>{const A=r.find(z=>z.id===I.dataset.cardId);A&&dt(A,r,g,e)})})}function ee(E){const I=$?p:[...y];if(!I.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}E.innerHTML=I.map(A=>{if(y.has(A)){const S=u.find(N=>N.formation===A),D=u.filter(N=>N.formation===A).length,R=D>1?`<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${D}</div>`:"";return`
        ...
        <div style="
          ...
          <div style="font-weight:900;font-size:18px">${A}</div>
        </div>`}).join(""),E.querySelectorAll("[data-form-id]").forEach(A=>{A.addEventListener("click",()=>{const z=u.find(S=>S.id===A.dataset.formId);z&&st(z,u,e,n)})})}function le(E){const I=$?f:[...k];if(!I.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}E.innerHTML=I.map(A=>{const z=k.has(A),S=re[A]||{icon:"⚡"};if(z){const D=d.find(q=>q.gc_type===A),R=d.filter(q=>q.gc_type===A).length,N=R>1?`<div style="position:absolute;top:4px;right:4px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${R}</div>`:"";return`
        ...
        <div style="
          ...
          <div style="font-weight:700;font-size:13px">${A}</div>
        </div>`}).join(""),E.querySelectorAll("[data-gc-id]").forEach(A=>{A.addEventListener("click",()=>at(A.dataset.gcType,n))})}t.querySelectorAll(".col-tab-btn").forEach(E=>{E.addEventListener("click",()=>{h=E.dataset.tab,v="Tous",x="",$=!1,t.querySelectorAll(".col-tab-btn").forEach(I=>{const A=I.dataset.tab===h;I.style.borderBottomColor=A?"var(--green)":"transparent",I.style.color=A?"var(--green)":"var(--gray-600)"}),C(),L()})}),C(),L()}function at(t,e){const i=re[t]||{icon:"⚡",desc:"Effet spécial."};e("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Y=1e3;function st(t,e,i,o){var m,y,k;const{state:s,toast:n,closeModal:a,navigate:c,refreshProfile:l}=i,r=t.formation,d={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const h=ye[r]||{},v=W[r]||[],x=290,$=360,_=20;function b(M){const C=h[M];return C?{x:C.x*x,y:C.y*$}:null}let T=`<svg width="${x}" height="${$}" viewBox="0 0 ${x} ${$}" xmlns="http://www.w3.org/2000/svg">`;for(const[M,C]of v){const L=b(M),j=b(C);!L||!j||(T+=`<line x1="${L.x}" y1="${L.y}" x2="${j.x}" y2="${j.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const M of Object.keys(h)){const C=b(M);if(!C)continue;const L=M.replace(/\d+/,""),j=d[L]||"#555";T+=`<circle cx="${C.x}" cy="${C.y}" r="${_}" fill="${j}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,T+=`<text x="${C.x}" y="${C.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${L}</text>`}return T+="</svg>",T}const p=e.filter(h=>h.formation===r),f=p.length,g=!t.is_for_sale;o(`Formation ${r}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
      ...
      <div style="width:140px;border-radius:12px;padding:6px;background:${x};flex-shrink:0">
        <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
          ...
          <div style="position:relative;height:80px;background:#f2e8d2;display:flex;flex-direction:column;align-items:center">
            ...
            ${k!==null?`
            <svg width="32" height="31" viewBox="0 0 32 31" style="position:absolute;top:50px;z-index:2;display:block">
              <polygon points="16,2 19.5,11 30,11 22,17.5 25,27 16,21.5 7,27 10,17.5 2,11 12.5,11" fill="${v}" stroke="white" stroke-width="1.5"/>
          ...
          <div style="background:#f2e8d2;padding:3px 6px;display:flex;align-items:center;justify-content:space-between;min-height:24px">
            ...
            ${(_=d.clubs)!=null&&_.logo_url?`<img src="${d.clubs.logo_url}" style="width:22px;height:16px;object-fit:contain">`:`<div style="background:#1a3a7a;color:#fff;border-radius:2px;padding:1px 3px;font-size:6px;font-weight:800">${(((b=d.clubs)==null?void 0:b.encoded_name)||"").slice(0,6)}</div>`}
          </div>
        </div>
      </div>
    ...
    </div>`:""}`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(T=document.getElementById("close-detail"))==null||T.addEventListener("click",c),(M=document.getElementById("direct-sell-btn"))==null||M.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte ${d.surname_encoded} pour ${f.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const j=u.find(le=>!le.is_for_sale)||u[0];if(!j){n("Aucune carte à vendre","error");return}const{error:ee}=await w.from("cards").delete().eq("id",j.id);if(ee){n(ee.message,"error");return}await w.from("users").update({credits:(s.profile.credits||0)+f}).eq("id",s.profile.id),await r(),n(`+${f.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),c(),l("collection")}),(C=document.getElementById("market-sell-btn"))==null||C.addEventListener("click",async()=>{const j=parseInt(document.getElementById("sell-price").value);if(!j||j<1){n("Prix invalide","error");return}await w.from("cards").update({is_for_sale:!0,sale_price:j}).eq("id",t.id),await w.from("market_listings").insert({seller_id:s.profile.id,card_id:t.id,price:j}),n("Carte mise en vente sur le marché !","success"),c(),l("collection")}),(L=document.getElementById("cancel-sell-btn"))==null||L.addEventListener("click",async()=>{await w.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",t.id),await w.from("market_listings").update({status:"cancelled"}).eq("card_id",t.id).eq("status","active"),n("Annonce retirée","success"),c(),l("collection")})}const se={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},H={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function xe(t){const e=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!e||!(t!=null&&t.skin)||!(t!=null&&t.hair))return null;const i=t.hair==="chauve"?`${t.skin}-chauve-rase`:`${t.skin}-${t.hair}-${t.hair_length}`;return`${e}/storage/v1/object/public/assets/tetes/${i}.png`}async function qe(t,e){const{state:i,navigate:o,toast:s}=e;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await w.from("decks").select("id,name,formation_card_id,is_active").eq("owner_id",i.profile.id).order("created_at",{ascending:!1});t.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>Mes decks</h2>
  ...
  <div class="page">
    ...
    <div class="page-body">
      <button class="btn btn-primary" id="save-deck" style="width:100%" ${n<11?"disabled":""}>
  ...
  </div>`,lt(t,e,s,i),document.getElementById("builder-back").addEventListener("click",()=>o("decks")),document.getElementById("formation-select").addEventListener("change",r=>{e.formation=r.target.value;const d=Ae(e.formation),u={};d.forEach(p=>{e.slots[p]&&(u[p]=e.slots[p])}),e.slots=u,U(t,e,i)}),document.getElementById("save-deck").addEventListener("click",()=>ft(e,i)),t.querySelectorAll("[data-remove-sub]").forEach(r=>{r.addEventListener("click",()=>{e.subs=e.subs.filter(d=>d!==r.dataset.removeSub),U(t,e,i)})}),(l=document.getElementById("add-sub-btn"))==null||l.addEventListener("click",()=>{pt(e,t,i)})}function lt(t,e,i,o){const s=document.getElementById("deck-field");if(!s)return;const n=e.formation,a=ye[n]||{},c=ce?ce(n)||W[n]||[]:W[n]||[],l={};i.forEach(v=>{const x=e.slots[v],$=x?e.playerCards.find(_=>_.id===x):null;l[v]=($==null?void 0:$.player)||null});const r=i.filter(v=>v.startsWith("MIL"));let d=0,u=0;r.forEach(v=>{const x=l[v];x&&(d+=Number(x.note_m)||0)}),c.forEach(([v,x])=>{if(!v.startsWith("MIL")||!x.startsWith("MIL"))return;J(l[v],l[x])!=="#ff3333"&&u++});const p=320,f=320,g=28,m=4;function y(v){const x=a[v];return x?{x:x.x*p,y:x.y*f}:null}let k="";for(const[v,x]of c){const $=y(v),_=y(x);if(!$||!_)continue;const b=l[v],T=l[x],M=J(b,T);if(M==="#00ff88"||M==="#FFD700")k+=`<line x1="${$.x}" y1="${$.y}" x2="${_.x}" y2="${_.y}"
        stroke="${M}" stroke-width="${m*3}" stroke-linecap="round" opacity="0.25"/>`,k+=`<line x1="${$.x}" y1="${$.y}" x2="${_.x}" y2="${_.y}"
        stroke="${M}" stroke-width="${m}" stroke-linecap="round" opacity="0.95"/>`;else{const L=M==="#ff3333"||M==="#cc2222"?.75:.9;k+=`<line x1="${$.x}" y1="${$.y}" x2="${_.x}" y2="${_.y}"
        stroke="${M}" stroke-width="${m}" stroke-linecap="round" opacity="${L}"/>`}}for(const v of i){const x=y(v);if(!x)continue;const $=l[v],_=v.replace(/\d+/,""),T={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[_]||"#555",M=$?Number(_==="GK"?$.note_g:_==="DEF"?$.note_d:_==="MIL"?$.note_m:$.note_a)||0:null,C=$?($.surname_encoded||"").slice(0,8).toUpperCase():"";if($){const L=xe($);L&&(k+=`
          <defs>
            <clipPath id="clip-${v}">
              <circle cx="${x.x}" cy="${x.y}" r="${g}"/>
            </clipPath>
          </defs>
          <image href="${L}" x="${x.x-g}" y="${x.y-g}" width="${g*2}" height="${g*2}"
            clip-path="url(#clip-${v})" preserveAspectRatio="xMidYMid slice" opacity="0.85"/>`),k+=`
        <circle cx="${x.x}" cy="${x.y}" r="${g}" fill="${L?"transparent":T}"
          stroke="${T}" stroke-width="2" />
        <circle cx="${x.x}" cy="${x.y}" r="${g}" fill="${T}" opacity="${L?"0.5":"1"}"
          stroke="rgba(255,255,255,0.6)" stroke-width="2"/>
        ${L?`<image href="${L}" x="${x.x-g}" y="${x.y-g}" width="${g*2}" height="${g*2}"
          clip-path="url(#clip-${v})" preserveAspectRatio="xMidYMid slice" opacity="0.9"/>`:""}
        ...
        <rect x="${x.x-g}" y="${x.y-g}" width="${g*2}" height="${g*2}"
          fill="transparent" class="deck-slot-hit" data-pos="${v}" style="cursor:pointer"/>`}else k+=`
        <circle cx="${x.x}" cy="${x.y}" r="${g}" fill="rgba(255,255,255,0.08)"
        ...
        <rect x="${x.x-g}" y="${x.y-g}" width="${g*2}" height="${g*2}"
          fill="transparent" class="deck-slot-hit" data-pos="${v}" style="cursor:pointer"/>`}const h="";s.innerHTML=`
    <!-- Badge MIL -->
    ${r.length>0?`
    ...
    <svg viewBox="0 0 ${p} ${f}" width="100%" style="display:block;max-width:380px;margin:0 auto">
      ...
      ${u.length>0?u.map(m=>{var x,$;const y=m.player,k=a==="GK"?y.note_g:a==="DEF"?y.note_d:a==="MIL"?y.note_m:y.note_a,h=xe(y),v={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[y.rarity];return`<div class="player-option" data-card-id="${m.id}"
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;align-items:center;gap:6px;margin-top:3px">
              ...
              ${(x=y.clubs)!=null&&x.logo_url?`<img src="${y.clubs.logo_url}" style="width:18px;height:18px;object-fit:contain">`:`<span style="font-size:10px;color:var(--gray-600)">${(($=y.clubs)==null?void 0:$.encoded_name)||"—"}</span>`}
              <span style="font-size:10px;color:var(--gray-600)">${y.country_code}</span>
            </div>
          </div>
          <!-- Note -->
          <div style="width:36px;height:36px;border-radius:8px;background:${H[a]};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:16px;border:2px solid ${v};flex-shrink:0">
            ${k}
          </div>
      ...
      ${c.length>0?c.map(r=>{var f;const d=r.player,u=xe(d),p=d.job==="GK"?d.note_g:d.job==="DEF"?d.note_d:d.job==="MIL"?d.note_m:d.note_a;return`<div class="player-option" data-card-id="${r.id}"
          style="display:flex;align-items:center;gap:10px;padding:8px;border:1.5px solid var(--gray-200);border-radius:10px;cursor:pointer">
  ...
  `,document.body.appendChild(o);let s=!1;document.getElementById("pack-visual").addEventListener("click",()=>{if(s)return;s=!0;const p=document.getElementById("pack-visual");p.classList.add("shaking"),setTimeout(()=>{p.classList.add("pack-open"),setTimeout(()=>{document.getElementById("pack-phase").style.display="none",a(0)},600)},500)});let n=0;function a(p){n=p;const f=document.getElementById("reveal-phase");f.style.display="flex",c(p)}function c(p){var $;const f=t[p],g=document.getElementById("card-counter"),m=document.getElementById("single-card-slot"),y=document.getElementById("card-tap-hint");g&&(g.textContent=`Carte ${p+1} / ${t.length}`),y&&(y.textContent=p<t.length-1?"Appuie pour continuer →":"Appuie pour voir toutes tes cartes");const k=Le(f),h=f.card_type==="player"&&(($=f.player)==null?void 0:$.rarity)==="legende";m.innerHTML=`
      ...
      </div>`,h&&d();const v=document.getElementById("current-card-wrap");let x=!1;v.addEventListener("click",()=>{if(x)return;x=!0;const _=p+1;_<t.length?(v.style.transition="all 0.25s ease",v.style.transform="translateX(-120%) rotate(-15deg)",v.style.opacity="0",setTimeout(()=>c(_),250)):l()})}function l(){u(),document.getElementById("reveal-phase").style.display="none";const p=document.getElementById("recap-phase");p.style.display="flex";const f=document.getElementById("recap-grid");f.innerHTML=t.map((g,m)=>`
  ...
  `,document.body.appendChild(t),t.addEventListener("click",e=>{e.target===t&&t.remove()}),document.getElementById("odds-close").addEventListener("click",()=>t.remove())}function Et(){return new Promise(t=>{const e=document.createElement("div");e.style.cssText="position:fixed;inset:0;background:#000;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:16px";let i=5;e.innerHTML=`<div style="font-size:48px">📺</div>
  ...
  </div>`,(n=document.getElementById("msg-btn"))==null||n.addEventListener("click",s)}function Q(t){const e=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!e||!(t!=null&&t.skin)||!(t!=null&&t.hair))return null;const i=t.hair==="chauve"?`${t.skin}-chauve-rase`:`${t.skin}-${t.hair}-${t.hair_length}`;return`${e}/storage/v1/object/public/assets/tetes/${i}.png`}function pe(t){const e=t.player;return{cardId:t.id,id:e.id,firstname:e.firstname,name:e.surname_encoded,country_code:e.country_code,club_id:e.club_id,job:e.job,job2:e.job2,note_g:Number(e.note_g)||0,note_d:Number(e.note_d)||0,note_m:Number(e.note_m)||0,note_a:Number(e.note_a)||0,rarity:e.rarity,skin:e.skin,hair:e.hair,hair_length:e.hair_length,boost:0,used:!1,_line:null,_col:null}}function ve(t){return t===1?[1]:t===2?[0,2]:t===3?[0,1,2]:t===4?[0,1,1,2]:t===5?[0,1,1,1,2]:[1]}function It(){const t=Math.random()*100;return t<.1?4:t<5?3:t<20?2:1}function Pe(t,e){const i=V[e]||V["4-4-2"],o={GK:[],DEF:[],MIL:[],ATT:[]},s=[...t];for(const n of["GK","DEF","MIL","ATT"]){const a=[];for(let l=0;l<i[n];l++){let r=s.findIndex(d=>d.job===n);if(r===-1&&(r=s.findIndex(d=>d.job2===n)),r===-1&&(r=0),s[r]){const d={...s[r],_line:n};a.push(d),s.splice(r,1)}}const c=ve(a.length);a.forEach((l,r)=>{l._col=c[r]}),o[n]=a}return o}async function At(t,e){const{data:i}=await w.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length").eq("is_active",!0).limit(60);if(!i||i.length<11)return Tt(t);const o=V[t]||V["4-4-2"],s={GK:[],DEF:[],MIL:[],ATT:[]},n=[...i];for(const a of["GK","DEF","MIL","ATT"]){const c=n.filter(p=>p.job===a),l=n.filter(p=>p.job!==a),r=[...c,...l],d=[];for(let p=0;p<o[a];p++){const f=r[p]||n[p];f&&d.push({cardId:"ai-"+f.id+"-"+p,id:f.id,firstname:f.firstname,name:f.surname_encoded,country_code:f.country_code,club_id:f.club_id,job:f.job,job2:f.job2,note_g:Number(f.note_g)||0,note_d:Number(f.note_d)||0,note_m:Number(f.note_m)||0,note_a:Number(f.note_a)||0,rarity:f.rarity,skin:f.skin,hair:f.hair,hair_length:f.hair_length,boost:0,used:!1,_line:a})}const u=ve(d.length);d.forEach((p,f)=>{p._col=u[f]}),s[a]=d}return s}function Tt(t){const e=V[t]||V["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},o=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let s=0;for(const n of["GK","DEF","MIL","ATT"]){const a=[];for(let l=0;l<e[n];l++){const r=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+s,id:"fake-"+s,firstname:"IA",name:o[s%o.length],country_code:"XX",club_id:null,job:n,job2:null,note_g:n==="GK"?r:2,note_d:n==="DEF"?r:2,note_m:n==="MIL"?r:2,note_a:n==="ATT"?r:2,rarity:"normal",boost:0,used:!1,_line:n}),s++}const c=ve(a.length);a.forEach((l,r)=>{l._col=c[r]}),i[n]=a}return i}async function Lt(t,e){var $;const{state:i,navigate:o,toast:s}=e,n=i.params||{};t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const a=n.matchMode||"vs_ai_easy",c=a.replace("vs_ai_",""),l=a;if(!n.deckId)return Mt(t,e,a);const r=n.deckId,[{data:d},{data:u}]=await Promise.all([w.from("decks").select("formation,name").eq("id",r).single(),w.from("deck_cards").select(`position, is_starter, slot_order,
        card:cards(id, card_type, formation,
          player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
            note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
            clubs(encoded_name,logo_url)))`).eq("deck_id",r).order("slot_order")]),p=(u||[]).filter(_=>{var b;return _.is_starter&&((b=_.card)==null?void 0:b.player)}).map(_=>pe(_.card)),f=(u||[]).filter(_=>{var b;return!_.is_starter&&((b=_.card)==null?void 0:b.player)}).map(_=>pe(_.card));if(p.length<11){Ke(t,"⚠️",`Deck incomplet (${p.length}/11).`,"Compléter",()=>o("decks"));return}const g=(u||[]).find(_=>{var b;return((b=_.card)==null?void 0:b.card_type)==="formation"}),m=(d==null?void 0:d.formation)||(($=g==null?void 0:g.card)==null?void 0:$.formation)||"4-4-2",{data:y}=await w.from("cards").select("id,gc_type").eq("owner_id",i.profile.id).eq("card_type","game_changer"),k=Pe(p,m),h=await At(m),{data:v}=await w.from("matches").insert({home_id:i.profile.id,away_id:null,mode:l,home_deck_id:r,status:"in_progress"}).select().single(),x={matchId:v==null?void 0:v.id,mode:l,difficulty:c,formation:m,homeTeam:k,aiTeam:h,homeSubs:f,subsUsed:0,maxSubs:Math.min(f.length,3),homeScore:0,aiScore:0,gcCards:y||[],usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Ct(t,x,e)}async function Mt(t,e,i){const{state:o,navigate:s}=e;t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await w.from("decks").select("id,name,is_active,formation").eq("owner_id",o.profile.id).order("created_at",{ascending:!1});if(!n||n.length===0){Ke(t,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>s("decks"));return}const a=n.map(d=>d.id),{data:c}=await w.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length))`).in("deck_id",a).order("slot_order");let l=0;function r(){var k,h,v,x,$;const d=n[l],u=(c||[]).filter(_=>_.deck_id===d.id),p=u.filter(_=>{var b;return _.is_starter&&((b=_.card)==null?void 0:b.player)}).map(_=>pe(_.card)),f=u.find(_=>{var b;return((b=_.card)==null?void 0:b.card_type)==="formation"}),g=d.formation||((k=f==null?void 0:f.card)==null?void 0:k.formation)||"4-4-2",m=p.length>=11?Pe(p,g):null,y=p.length>=11;t.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;min-height:100vh;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;align-items:center;gap:8px;padding:10px 8px;flex-shrink:0">
        <button id="prev-deck" style="width:48px;height:48px;border-radius:50%;background:rgba(255,255,255,${l===0?"0.05":"0.15"});border:2px solid rgba(255,255,255,${l===0?"0.1":"0.3"});color:${l===0?"rgba(255,255,255,0.2)":"#fff"};font-size:22px;cursor:${l===0?"default":"pointer"};flex-shrink:0">◀</button>
        ...
        <button id="next-deck" style="width:48px;height:48px;border-radius:50%;background:rgba(255,255,255,${l===n.length-1?"0.05":"0.15"});border:2px solid rgba(255,255,255,${l===n.length-1?"0.1":"0.3"});color:${l===n.length-1?"rgba(255,255,255,0.2)":"#fff"};font-size:22px;cursor:${l===n.length-1?"default":"pointer"};flex-shrink:0">▶</button>
      </div>
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:6px;flex-shrink:0">
        ${n.map((_,b)=>`<div style="width:7px;height:7px;border-radius:50%;background:${b===l?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
      ...
      <div style="padding:12px 16px 20px;flex-shrink:0;display:flex;flex-direction:column;gap:8px">
        <button id="validate-deck" style="width:100%;padding:16px;border-radius:12px;border:none;
          background:${y?"var(--green, #1A6B3C)":"rgba(255,255,255,0.1)"};
    ...
    </div>`,(h=document.getElementById("prev-deck"))==null||h.addEventListener("click",()=>{l>0&&(l--,r())}),(v=document.getElementById("next-deck"))==null||v.addEventListener("click",()=>{l<n.length-1&&(l++,r())}),(x=document.getElementById("validate-deck"))==null||x.addEventListener("click",()=>{y&&e.navigate("match",{matchMode:i,deckId:d.id})}),($=document.getElementById("cancel-deck-select"))==null||$.addEventListener("click",()=>s("home"))}r()}function Ct(t,e,i){const o=e.homeTeam.MIL||[],s=e.aiTeam.MIL||[];function n(u){return u.reduce((p,f)=>p+G(f,"MIL"),0)}function a(u){let p=0;for(let f=0;f<u.length-1;f++){const g=J(u[f],u[f+1]);g!=="#ff3333"&&g!=="#cc2222"&&p++}return p}const c=n(o)+a(o),l=n(s)+a(s),r=c>=l;function d(u,p,f){return`<div style="text-align:center">
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;gap:24px;padding:24px;background:#0a3d1e">
    ...
    <div id="midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff;max-width:320px"></div>
      ...
      `,r&&(e.boostCard={value:g})}e.attacker=r?"home":"ai",e.log.push({text:`Duel milieu : ${e.clubName} ${c} – ${l} IA → ${r?e.clubName+" attaque":"IA attaque"}`,type:"info"}),setTimeout(()=>{e.phase=e.attacker==="home"?"attack":"ai-attack",F(t,e,i),e.attacker==="ai"&&setTimeout(()=>fe(t,e,i),1e3)},2800)},1200)}function He(t,e,i,o,s=310,n=310){const a=ye[e]||{},c=ce(e)||W[e]||[],l=26,r={},d=["ATT","MIL","DEF","GK"];for(const f of d)(t[f]||[]).forEach((m,y)=>{r[`${f}${y+1}`]=m});function u(f){const g=a[f];return g?{x:g.x*s,y:g.y*n}:null}let p="";for(const[f,g]of c){const m=u(f),y=u(g);if(!m||!y)continue;const k=r[f],h=r[g],v=J(k,h);v==="#00ff88"||v==="#FFD700"?(p+=`<line x1="${m.x.toFixed(1)}" y1="${m.y.toFixed(1)}" x2="${y.x.toFixed(1)}" y2="${y.y.toFixed(1)}"
        stroke="${v}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,p+=`<line x1="${m.x.toFixed(1)}" y1="${m.y.toFixed(1)}" x2="${y.x.toFixed(1)}" y2="${y.y.toFixed(1)}"
        stroke="${v}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):p+=`<line x1="${m.x.toFixed(1)}" y1="${m.y.toFixed(1)}" x2="${y.x.toFixed(1)}" y2="${y.y.toFixed(1)}"
        stroke="${v}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}for(const[f,g]of Object.entries(r)){const m=u(f);if(!m)continue;const y=f.replace(/[0-9]/g,""),k=K[y]||"#555",h=i==="attack"&&["MIL","ATT"].includes(y)&&!g.used||i==="defense"&&["GK","DEF","MIL"].includes(y)&&!g.used,v=o.includes(g.cardId);let x;i==="attack"?x=y==="MIL"?Number(g.note_m)||0:Number(g.note_a)||0:i==="defense"?x=y==="GK"?Number(g.note_g)||0:y==="MIL"?Number(g.note_m)||0:Number(g.note_d)||0:x=Number(y==="GK"?g.note_g:y==="DEF"?g.note_d:y==="MIL"?g.note_m:g.note_a)||0,x=x+(g.boost||0);const $=v?"#FFD700":h?"rgba(255,255,255,0.7)":"rgba(255,255,255,0.25)",_=v?3:2,b=g.used?.2:1,T=Q(g);T&&(p+=`<defs><clipPath id="mc-${f}"><circle cx="${m.x}" cy="${m.y}" r="${l}"/></clipPath></defs>`),p+=`<circle cx="${m.x}" cy="${m.y}" r="${l}" fill="${k}" opacity="${b}"
      stroke="${$}" stroke-width="${_}"/>`,T&&!g.used&&(p+=`<image href="${T}" x="${m.x-l}" y="${m.y-l}" width="${l*2}" height="${l*2}"
        clip-path="url(#mc-${f})" preserveAspectRatio="xMidYMid slice" opacity="0.8"/>`,p+=`<circle cx="${m.x}" cy="${m.y}" r="${l}" fill="${k}" opacity="0.3"
        stroke="${$}" stroke-width="${_}"/>`),g.boost&&(p+=`<rect x="${m.x+l-10}" y="${m.y-l}" width="14" height="10" rx="3" fill="#87CEEB"/>
      ...
      fill="${g.used?"rgba(255,255,255,0.2)":"white"}" font-family="Arial Black,Arial">${g.used?"–":x}</text>
    <text x="${m.x}" y="${m.y+11}" text-anchor="middle" font-size="6" fill="rgba(255,255,255,${g.used?.2:.8})"
      font-family="Arial">${(g.name||"").slice(0,8)}</text>`,h&&(p+=`<circle cx="${m.x}" cy="${m.y}" r="${l}" fill="rgba(255,255,255,0.08)"
        class="match-slot-hit ${v?"selected":""}" data-card-id="${g.cardId}" data-role="${y}"
    ...
    ${He(t,e,i,o)}
  </div>`}function F(t,e,i){var m,y,k,h,v,x,$,_;const o=e.selected.map(b=>b.cardId),s=e.usedSubIds||[],n=e.homeSubs.filter(b=>!s.includes(b.cardId)),c=Object.values(e.homeTeam).flat().filter(b=>b.used).length>0&&n.length>0&&e.subsUsed<e.maxSubs,l=e.log[e.log.length-1],r=e.phase==="ai-attack"||e.phase==="ai-defense",d=e.phase==="attack",u=e.phase==="defense",p=e.phase==="finished",f=e.gcCards.filter(b=>!e.usedGc.includes(b.id)),g=e.boostCard&&!e.boostUsed;t.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;height:100vh;overflow:hidden;background:#0a3d1e;position:relative">
    ...
    <div style="display:flex;flex:1;min-height:0;overflow:hidden">
      ...
      <div style="display:flex;flex-direction:column;gap:5px;padding:6px 3px;width:42px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
        ${n.length===0?'<div style="font-size:8px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':n.map(b=>{const T=Q(b),M=K[b.job]||"#555";return`
              <div class="sub-btn-col" data-sub-id="${b.cardId}" title="${b.firstname} ${b.name}"
                style="width:34px;height:34px;border-radius:50%;background:${M};border:2px solid rgba(255,255,255,0.4);cursor:pointer;position:relative;overflow:hidden;flex-shrink:0">
                ${T?`<img src="${T}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:0.85">`:""}
                <div style="position:absolute;bottom:0;left:0;right:0;font-size:6px;text-align:center;color:#fff;background:rgba(0,0,0,0.5);font-weight:700">${b.job}</div>
              </div>`}).join("")}
        ${c?`
      ...
      <div style="flex:1;display:grid;grid-template-columns:repeat(4,1fr);gap:3px;align-content:start">
        ${f.map(b=>{var T;return`
          <div class="gc-mini" data-gc-id="${b.id}" data-gc-type="${b.gc_type}"
            ...
            <div style="font-size:16px">${((T=re[b.gc_type])==null?void 0:T.icon)||"⚡"}</div>
