...
import{s as T,F as $i,i as Ut,k as Vi,l as nt,m as Ii,f as Nt,n as vo,T as li,j as wo,o as _o,b as $o}from"./formation-links-BZe0pGr4.js";const ko="/";function Mi(e,{navigate:t,toast:i}){let n="login";const d=()=>{var r,l,u,a,o,x;const s=n==="login";e.innerHTML=`
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
  ...
  </div>`;function N(){const H=document.getElementById("col-filters");H&&(F==="player"?(H.innerHTML=`
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(_=document.getElementById("direct-sell-form-btn"))==null||_.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${a} pour ${Jt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const A=p.find(I=>!I.is_for_sale)||p[0];if(!A){s("Aucune carte à vendre","error");return}await T.from("market_listings").delete().eq("card_id",A.id),await T.from("transfer_history").delete().eq("card_id",A.id);const{error:g}=await T.from("cards").delete().eq("id",A.id);if(g){s(g.message,"error");return}await T.from("users").update({credits:(d.profile.credits||0)+Jt}).eq("id",d.profile.id),await u(),s(`+${Jt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),r(),l("collection")}),(m=document.getElementById("market-sell-form-btn"))==null||m.addEventListener("click",async()=>{const A=parseInt(document.getElementById("sell-price-form").value);if(!A||A<1){s("Prix invalide","error");return}await T.from("cards").update({is_for_sale:!0,sale_price:A}).eq("id",e.id),await T.from("market_listings").insert({seller_id:d.profile.id,card_id:e.id,price:A}),s("Carte mise en vente sur le marché !","success"),r(),l("collection")}),(c=document.getElementById("cancel-sell-form-btn"))==null||c.addEventListener("click",async()=>{await T.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await T.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),s("Annonce retirée","success"),r(),l("collection")})}async function Ni(e,t,i,n){var xe,_e,we,ve,Ee;const{state:d,toast:s,openModal:r,closeModal:l,navigate:u,refreshProfile:a}=n,o=e.player,x=t.filter(se=>se.player.id===o.id),p=x.length,h=e.evolution_bonus||0,_=Math.max((Number(o.note_g)||0)+(o.job==="GK"||o.job2==="GK"?h:0),(Number(o.note_d)||0)+(o.job==="DEF"||o.job2==="DEF"?h:0),(Number(o.note_m)||0)+(o.job==="MIL"||o.job2==="MIL"?h:0),(Number(o.note_a)||0)+(o.job==="ATT"||o.job2==="ATT"?h:0)),m=o.rarity||"normal",{data:c}=await T.from("sell_price_configs").select("*").eq("rarity",m).lte("note_min",_).gte("note_max",_).order("note_min",{ascending:!1}).limit(1);((xe=c==null?void 0:c[0])==null?void 0:xe.price)??Yo[m];const A=o.rarity!=="legende";Qo(o);const g=((o.rarity==="pepite"||o.rarity==="papyte")&&e.current_note!=null?e.current_note:pi(o,o.job))+h,I=o.rarity==="pepite"||o.rarity==="papyte",j=o.job2?(I?io(e,Jo(o.job2)):pi(o,o.job2))+(pi(o,o.job2)>0?h:0):null;ci[o.job],o.job2&&ci[o.job2];const C=Ko[o.rarity]||"#ccc";Xo[o.country_code]||o.country_code;const F=e.evolution_bonus||0,R=g+F,V=j||0,K=V>0?V+F:0,ne=x.map(se=>se.id);let ie={};if(ne.length){const{data:se}=await T.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",ne).order("transferred_at",{ascending:!0});(se||[]).forEach(pe=>{ie[pe.card_id]||(ie[pe.card_id]=[]),ie[pe.card_id].push(pe)})}const Q=se=>{const pe=se.transferred_at?new Date(se.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",ce=se.source==="booster"?"Booster":se.price?se.price.toLocaleString("fr"):"—";return`
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",o.note_g],["DEF",o.note_d],["MIL",o.note_m],["ATT",o.note_a]].map(([se,pe])=>{const ce=ci[se],H=se==="GK"?"#fff":ce,Z=se===o.job||se===o.job2,ge=(Number(pe)||0)+(Z&&F>0?F:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(_e=document.getElementById("close-detail"))==null||_e.addEventListener("click",l);let P=new Set;const re=()=>{const se=P.size,pe=document.getElementById("sell-action-panel");if(!pe)return;pe.style.display=se>0?"block":"none",document.getElementById("sell-selected-count").textContent=se;const ce=document.getElementById("evolve-btn");ce&&(ce.textContent=`⬆️ Évoluer ${se>1?"(+"+se+")":""}`)};document.querySelectorAll(".expl-check").forEach(se=>{se.addEventListener("change",()=>{const pe=se.dataset.id;se.checked?P.add(pe):P.delete(pe);const ce=se.closest(".exemplaire-row");ce&&(ce.style.borderColor=se.checked?"#1A6B3C":"#eee"),re()})}),document.querySelectorAll(".exemplaire-row").forEach(se=>{se.addEventListener("click",pe=>{if(pe.target.closest("button")||pe.target.tagName==="INPUT")return;const ce=se.querySelector(".expl-check");ce&&!ce.disabled&&(ce.checked=!ce.checked,ce.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(se=>{se.addEventListener("click",pe=>{pe.stopPropagation();const ce=document.querySelector(`.expl-hist[data-hist="${se.dataset.idx}"]`);ce&&(ce.style.display=ce.style.display==="none"?"flex":"none")})}),(we=document.getElementById("evolve-btn"))==null||we.addEventListener("click",async()=>{if(p<=1)return;const se=[...P];if(!se.length)return;if(P.has(e.id)){const me=document.createElement("div");me.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",me.innerHTML=`
        ...
        <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.3)">
          ...
          <div style="background:#f0fdf4;border-radius:10px;padding:12px;margin-bottom:16px;font-size:13px;color:#333">
            ...
            ${j&&j>0?`<br>📈 Note 2 : <strong>${j+(e.evolution_bonus||0)}</strong> → <strong style="color:#1A6B3C">${j+(e.evolution_bonus||0)+pe.length*ce}</strong>`:""}
      ...
      <div style="display:flex;gap:0;min-height:600px">
        ...
        <div style="width:130px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 8px;gap:10px;border-right:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="display:flex;flex-direction:column;gap:6px;align-items:center" id="subs-list">
            ${a.map(j=>{const C={...j.player,_evolution_bonus:j.evolution_bonus||0};return`<div style="position:relative;flex-shrink:0;overflow:visible">
                ${ht({...C,_evolution_bonus:C._evolution_bonus||0},{width:60,showStad:!0,stadDef:u})}
                <button data-remove-sub="${j.id}"
        ...
        <div style="width:130px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 8px;gap:12px;border-left:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="width:100%;text-align:center">
            ...
            <div id="add-stad-btn" style="cursor:pointer;margin:0 auto;width:fit-content">
              ${l?(()=>{var F;const j=t.stadDefMap[l.stadium_id],C=((F=j==null?void 0:j.club)==null?void 0:F.logo_url)||null;return`<div style="width:100px;height:130px;border-radius:8px;background:linear-gradient(135deg,#1a3a6b,#0a1a3a);border:2px solid #4fc3f7;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px">
                  ${C?`<img src="${C}" style="width:48px;height:48px;object-fit:contain">`:'<span style="font-size:36px">🏟️</span>'}
                  <span style="font-size:10px;font-weight:700;color:#E87722;text-align:center;padding:0 4px">${((j==null?void 0:j.name)||"Stade").slice(0,14)}</span>
  ...
  </div>`;const o=window.innerWidth>=768,x=e.querySelector(".deck-pc-layout"),p=e.querySelector(".deck-mobile-layout");x&&(x.style.display=o?"block":"none"),p&&(p.style.display=o?"none":"block"),on(e,t,d,i),document.getElementById("builder-back").addEventListener("click",()=>n("decks")),document.getElementById("formation-select").addEventListener("change",j=>{t.formation=j.target.value;const C=Oi(t.formation),F={};C.forEach(q=>{t.slots[q]&&(F[q]=t.slots[q])}),t.slots=F,lt(e,t,i)});const h=()=>{const{openModal:j,closeModal:C}=i,F=`<div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px">
      ${r.map(q=>`<div data-forma="${q}" style="cursor:pointer;padding:10px 16px;border-radius:8px;background:${q===t.formation?"#1A6B3C":"#f0f0f0"};color:${q===t.formation?"#fff":"#111"};font-weight:900;font-size:16px;border:2px solid ${q===t.formation?"#1A6B3C":"#ddd"}">${q}</div>`).join("")}
    </div>`;j("⚽ Choisir une formation",F),document.querySelectorAll("#modal-body [data-forma]").forEach(q=>{q.addEventListener("click",()=>{t.formation=q.dataset.forma,C(),lt(e,t,i)})})};(c=document.getElementById("formation-mobile-btn"))==null||c.addEventListener("click",h),(A=document.getElementById("formation-pc-btn"))==null||A.addEventListener("click",h),(g=document.getElementById("add-stad-btn"))==null||g.addEventListener("click",()=>{nn(t,e,i)}),document.getElementById("save-deck").addEventListener("click",()=>sn(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(j=>{j.addEventListener("click",()=>{t.subs=t.subs.filter(C=>C!==j.dataset.removeSub),lt(e,t,i)})}),(I=document.getElementById("add-sub-btn"))==null||I.addEventListener("click",()=>{an(t,e,i)})}function on(e,t,i,n){var I,j;const d=window.innerWidth>=768,s=e.querySelector(d?"#deck-field-pc":"#deck-field-mobile");if(!s)return;const r=(I=t.stadiumCards)==null?void 0:I.find(C=>C.id===t.stadiumCardId),l=r&&((j=t.stadDefMap)==null?void 0:j[r.stadium_id])||null,u=Ut[t.formation]||{},a=Vi(t.formation)||[],o={};for(const C of i){const F=t.slots[C],q=F?t.playerCards.find(R=>R.id===F):null;q!=null&&q.player?o[C]={...q.player,_evolution_bonus:q.evolution_bonus||0,face:q.player.face||null}:o[C]=null}const x=window.innerWidth>=768,p=x?window.innerWidth-280:window.innerWidth-20,h=x?Math.min(p,860):p,_=Math.round(x?h*.82:h*1.2),m=x?84:44;let c="";for(const[C,F]of a){const q=u[C],R=u[F];if(!q||!R)continue;const V=q.x*h,K=Math.round(.03*_+q.y*.85*_),ne=R.x*h,ie=Math.round(.03*_+R.y*.85*_),Q=o[C],N=o[F],P=nt(Q,N);P==="#ff3333"||P==="#cc2222"?c+=`<line x1="${V.toFixed(1)}" y1="${K.toFixed(1)}" x2="${ne.toFixed(1)}" y2="${ie.toFixed(1)}" stroke="${P}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(c+=`<line x1="${V.toFixed(1)}" y1="${K.toFixed(1)}" x2="${ne.toFixed(1)}" y2="${ie.toFixed(1)}" stroke="${P}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,c+=`<line x1="${V.toFixed(1)}" y1="${K.toFixed(1)}" x2="${ne.toFixed(1)}" y2="${ie.toFixed(1)}" stroke="${P}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let A="";const g=Math.round(m*657/507);for(const C of i){const F=u[C];if(!F)continue;const q=o[C],R=F.x*h;F.y*_;const V=Math.round(.03*_+F.y*(.85*_)),K=Math.round(R-m/2),ne=Math.round(V-g/2);if(q){const ie=C.replace(/\d+/,""),Q=l&&(l.club_id&&String(q.club_id)===String(l.club_id)||l.country_code&&q.country_code===l.country_code),N=ht({...q,_evolution_bonus:q._evolution_bonus||0},{width:m,showStad:!1,stadDef:l,role:ie});A+=`<div style="position:absolute;left:${K}px;top:${ne}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${C}">
      ...
      ${x.length>0?x.map(m=>{var C,F;const c=m.player,A=m.evolution_bonus||0,g=(r==="GK"?c.note_g:r==="DEF"?c.note_d:r==="MIL"?c.note_m:c.note_a)+(r===c.job||r===c.job2?A:0),I=no(c),j={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[c.rarity];return`<div class="player-option" data-card-id="${m.id}"
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;align-items:center;gap:6px;margin-top:3px">
              ...
              ${(C=c.clubs)!=null&&C.logo_url?`<img src="${c.clubs.logo_url}" style="width:18px;height:18px;object-fit:contain">`:`<span style="font-size:10px;color:var(--gray-600)">${((F=c.clubs)==null?void 0:F.encoded_name)||"—"}</span>`}
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",r).order("slot_order"),u=[...new Set((l||[]).filter(p=>{var h,_;return((h=p.card)==null?void 0:h.card_type)==="stadium"&&((_=p.card)==null?void 0:_.stadium_id)}).map(p=>p.card.stadium_id))],a={};if(u.length){const{data:p}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",u);(p||[]).forEach(h=>{a[h.id]=h}),(l||[]).forEach(h=>{var _,m;((_=h.card)==null?void 0:_.card_type)==="stadium"&&((m=h.card)!=null&&m.stadium_id)&&(h.card._stadiumDef=a[h.card.stadium_id]||null)})}let o=0;function x(){var C,F,q,R,V,K,ne;const p=s[o],h=(l||[]).filter(ie=>ie.deck_id===p.id),_=h.filter(ie=>{var Q;return ie.is_starter&&((Q=ie.card)==null?void 0:Q.player)}).map(ie=>hi(ie.card,ie.position)),m=h.find(ie=>{var Q;return((Q=ie.card)==null?void 0:Q.card_type)==="formation"}),c=p.formation||((C=m==null?void 0:m.card)==null?void 0:C.formation)||"4-4-2";let A=_.length>=11?vt(_,c):null,g=((F=p.stadium_card)==null?void 0:F.stadium_def)||null;g&&A&&(A=Tt(A,g));const I=_.length>=11;et(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    ...
    </div>`,function(){const Q=e.querySelector(".deck-preview-wrap svg");Q&&(Q.removeAttribute("width"),Q.removeAttribute("height"),Q.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",Q.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(R=document.getElementById("prev-deck"))==null||R.addEventListener("click",()=>{o>0&&(o--,x())}),(V=document.getElementById("next-deck"))==null||V.addEventListener("click",()=>{o<s.length-1&&(o++,x())}),(K=document.getElementById("validate-deck"))==null||K.addEventListener("click",()=>{if(!I)return;const ie=t.state.params||{};t.navigate("match",{...ie,matchMode:ie.matchMode||i,deckId:p.id})}),(ne=document.getElementById("cancel-deck-select"))==null||ne.addEventListener("click",()=>{Ve(e),d("home")});const j=document.getElementById("deck-swipe-zone");if(j){let ie=0,Q=0;j.addEventListener("touchstart",N=>{ie=N.touches[0].clientX,Q=N.touches[0].clientY},{passive:!0}),j.addEventListener("touchend",N=>{const P=N.changedTouches[0].clientX-ie,re=N.changedTouches[0].clientY-Q;Math.abs(P)<40||Math.abs(P)<Math.abs(re)||(P<0&&o<s.length-1?(o++,x()):P>0&&o>0&&(o--,x()))},{passive:!0})}}x()}function kn(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function Xe(e,t=44,i=58,n=null){return ht(e,{width:t,showStad:!!n,stadDef:n,used:e==null?void 0:e.used})}function it(e,t,i){if(!(e!=null&&e.length))return"";const n=e.slice(0,5);let d='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return n.forEach((s,r)=>{if(d+=Xe(s,40,52),r<n.length-1){const l=nt(s,n[r+1]);d+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(d+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),d+="</div>",d}function En(e){return!e||e.length<2?null:`https://flagsapi.com/${e.slice(0,2).toUpperCase()}/flat/64.png`}function Tn(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function ct(e,t,i,n,d=310,s=310,r=[]){var I;const l=Ut[t]||{},u=Vi(t)||$i[t]||[],a={},o=["ATT","MIL","DEF","GK"];for(const j of o)(e[j]||[]).forEach((F,q)=>{a[`${j}${q+1}`]=F});function x(j){const C=l[j];return C?{x:C.x*d,y:C.y*s}:null}let p="";for(const[j,C]of u){const F=x(j),q=x(C);if(!F||!q)continue;const R=a[j],V=a[C],K=nt(R,V);K==="#00ff88"||K==="#FFD700"?(p+=`<line x1="${F.x.toFixed(1)}" y1="${F.y.toFixed(1)}" x2="${q.x.toFixed(1)}" y2="${q.y.toFixed(1)}"
        stroke="${K}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,p+=`<line x1="${F.x.toFixed(1)}" y1="${F.y.toFixed(1)}" x2="${q.x.toFixed(1)}" y2="${q.y.toFixed(1)}"
        stroke="${K}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):p+=`<line x1="${F.x.toFixed(1)}" y1="${F.y.toFixed(1)}" x2="${q.x.toFixed(1)}" y2="${q.y.toFixed(1)}"
        stroke="${K}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const h=48,_=64,m=13,c=16,A={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[j,C]of Object.entries(a)){const F=x(j);if(!F||!C)continue;const q=j.replace(/[0-9]/g,""),R=Ht[q]||"#555",V=r.includes(C.cardId),K=i==="attack"&&(["MIL","ATT"].includes(q)||V)&&!C.used||i==="defense"&&["GK","DEF","MIL"].includes(q)&&!C.used,ne=n.includes(C.cardId);let ie;i==="attack"?ie=V?Math.max(1,Number(C.note_a)||0):q==="MIL"?Number(C.note_m)||0:Number(C.note_a)||0:i==="defense"?ie=q==="GK"?Number(C.note_g)||0:q==="MIL"?Number(C.note_m)||0:Number(C.note_d)||0:ie=Number(q==="GK"?C.note_g:q==="DEF"?C.note_d:q==="MIL"?C.note_m:C.note_a)||0,ie=ie+(C.boost||0),C.stadiumBonus&&(i==="attack"&&(q==="ATT"||q==="MIL")||i==="defense"&&(q==="GK"||q==="DEF"||q==="MIL")?ie+=10:i||(ie+=10));const Q=(F.x-h/2).toFixed(1),N=(F.y-_/2).toFixed(1),P=A[C==null?void 0:C.rarity]||A.normal;if(C.used){const se=typeof import.meta<"u"&&"/"||"/",ce=`${typeof window<"u"&&((I=window.location)==null?void 0:I.origin)||""}${se}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");p+=`<rect x="${Q}" y="${N}" width="${h}" height="${_}" rx="5" fill="#161616"/>`,p+=`<image href="${ce}" xlink:href="${ce}" x="${Q}" y="${N}" width="${h}" height="${_}" preserveAspectRatio="xMidYMid slice"/>`,p+=`<rect x="${Q}" y="${N}" width="${h}" height="${_}" rx="5" fill="none" stroke="${P}" stroke-width="2" opacity="0.7"/>`,p+=`<rect x="${Q}" y="${N}" width="${h}" height="${_}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${C.cardId}" data-role="${q}" style="cursor:pointer"/>`;continue}const re=ne?.45:1,xe=ne?"#FFD700":P,_e=ne?3:(C==null?void 0:C.rarity)==="legende"||(C==null?void 0:C.rarity)==="légende"||(C==null?void 0:C.rarity)==="pepite"||(C==null?void 0:C.rarity)==="pépite"?2.5:2,we=_-m-c;p+=`<defs><clipPath id="cp-${j}"><rect x="${Q}" y="${(F.y-_/2+m).toFixed(1)}" width="${h}" height="${we}"/></clipPath></defs>`,p+=`<rect x="${Q}" y="${N}" width="${h}" height="${_}" rx="5" fill="${R}" opacity="${re}"/>`;const ve=He(C);ve&&(p+=`<image href="${ve}" xlink:href="${ve}" x="${Q}" y="${(F.y-_/2+m).toFixed(1)}" width="${h}" height="${we}" clip-path="url(#cp-${j})" preserveAspectRatio="xMidYMin slice"/>`),p+=`<rect x="${Q}" y="${N}" width="${h}" height="${m}" rx="4" fill="rgba(255,255,255,0.92)"/>`,p+=`<text x="${F.x.toFixed(1)}" y="${(F.y-_/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(C.name||"").slice(0,9)}</text>`;const Ee=(F.y+_/2-c).toFixed(1);p+=`<rect x="${Q}" y="${Ee}" width="${h}" height="${c}" fill="rgba(255,255,255,0.92)"/>`;{const se=En(C.country_code);se?p+=`<image href="${se}" xlink:href="${se}" x="${(F.x-20).toFixed(1)}" y="${(F.y+_/2-c+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:p+=`<text x="${(F.x-13).toFixed(1)}" y="${(F.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${Tn(C.country_code)}</text>`;const pe=C.stadiumBonus?"#E87722":"#111";p+=`<text x="${F.x.toFixed(1)}" y="${(F.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="${pe}" font-family="Arial Black">${ie}</text>`;const ce=kn(C);ce?p+=`<image href="${ce}" xlink:href="${ce}" x="${(F.x+h/2-14).toFixed(1)}" y="${(F.y+_/2-c+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:C.clubName&&(p+=`<text x="${(F.x+14).toFixed(1)}" y="${(F.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(C.clubName||"").slice(0,3).toUpperCase()}</text>`),C.stadiumBonus&&(p+=`<rect x="${(F.x-h/2).toFixed(1)}" y="${(F.y-_/2).toFixed(1)}" width="${h}" height="${_}" rx="5" fill="none" stroke="#E87722" stroke-width="2" opacity="0.8"/>`)}p+=`<rect x="${Q}" y="${N}" width="${h}" height="${_}" rx="5" fill="${ne?"rgba(255,255,255,0.12)":"none"}" stroke="${xe}" stroke-width="${_e}" opacity="${re}"/>`,K&&(p+=`<rect x="${Q}" y="${N}" width="${h}" height="${_}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${ne?"selected":""}" data-card-id="${C.cardId}" data-role="${q}" style="cursor:pointer"/>`)}const g=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-g} ${-g} ${d+g*2} ${s+g*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${p}
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>In(e,t,i),5e3)}function In(e,t,i){const n=t.homeTeam.MIL||[],d=t.aiTeam.MIL||[],s=t.stadiumDef||null,r=t.aiStadiumDef||null;function l(g,I){const j=Se(g,"MIL"),C=g.evolution_bonus||g._evolution_bonus||0,F=g.job==="MIL"||g.job2==="MIL"?C:0,q=g.stadiumBonus||I&&(I.club_id&&String(g.club_id)===String(I.club_id)||I.country_code&&g.country_code===I.country_code)?10:0;return j+F+q}function u(g,I){return g.reduce((j,C)=>j+l(C,I),0)}function a(g){let I=0;for(let j=0;j<g.length-1;j++){const C=nt(g[j],g[j+1]);C==="#00ff88"?I+=2:C==="#FFD700"&&(I+=1)}return I}const o=u(n,s)+a(n),x=u(d,r)+a(d),p=o>=x;function h(g,I,j,C,F){return`<div id="duel-row-${C}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${g.map((q,R)=>{const V=R<g.length-1?nt(q,g[R+1]):null,K=!V||V==="#ff3333"||V==="#cc2222",ne=V==="#00ff88"?"+2":V==="#FFD700"?"+1":"";l(q,F),q.stadiumBonus||F&&(F.club_id&&String(q.club_id)===String(F.club_id)||F.country_code&&(q.country_code,F.country_code));const ie=q.evolution_bonus||q._evolution_bonus||0;return`
      ...
      <div class="duel-score-line duel-score-line-${C}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
        Score: ${u(g,F)} + ${a(g)} liens = <b style="color:#fff">${u(g,F)+a(g)}</b>
      </div>
    </div>`}e.innerHTML=`
  ...
  </div>`;const _=()=>{const g=(I,j)=>e.querySelectorAll(I).forEach((C,F)=>{setTimeout(()=>{C.style.opacity="1",C.style.transform="translateY(0) scale(1)"},j+F*90)});g(".duel-card-home",150),g(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((I,j)=>{setTimeout(()=>{I.style.opacity="1"},j*70)}),900),setTimeout(()=>{const I=e.querySelector("#vs-label");I&&(I.style.opacity="1",I.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(j=>j.style.opacity="1")},1250),setTimeout(()=>{m("score-home",o,800),m("score-ai",x,800)},1500)};function m(g,I,j){const C=document.getElementById(g);if(!C)return;const F=performance.now(),q=R=>{const V=Math.min(1,(R-F)/j);C.textContent=Math.round(I*(1-Math.pow(1-V,3))),V<1?requestAnimationFrame(q):C.textContent=I};requestAnimationFrame(q)}requestAnimationFrame(_),t.attacker=p?"home":"ai";const c=p?ri():null;p&&(t.boostCard={value:c}),t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:n.map(g=>({name:g.name,note:Se(g,"MIL"),portrait:He(g),job:g.job,country_code:g.country_code,rarity:g.rarity,clubName:g.clubName,clubLogo:g.clubLogo})),aiPlayers:d.map(g=>({name:g.name,note:Se(g,"MIL"),portrait:He(g),job:g.job,country_code:g.country_code,rarity:g.rarity,clubName:g.clubName,clubLogo:g.clubLogo})),homeTotal:o,aiTotal:x,text:`Duel milieu : ${t.clubName} ${o} – ${x} IA → ${p?t.clubName+" attaque":"IA attaque"}`});const A=()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",Je(e,t,i),t.attacker==="ai"&&setTimeout(()=>ti(e,t,i),800)};setTimeout(()=>{const g=document.getElementById("score-home"),I=document.getElementById("score-ai"),j=document.getElementById(p?"duel-row-home":"duel-row-ai"),C=document.getElementById(p?"duel-row-ai":"duel-row-home"),F=p?g:I,q=p?I:g;F&&(F.style.fontSize="80px",F.style.color=p?"#FFD700":"#ff6b6b",F.style.animation="duelPulse .5s ease"+(p?", duelGlow 1.5s ease infinite .5s":"")),q&&(q.style.opacity="0.25"),setTimeout(()=>{j&&(j.style.transformOrigin="center",j.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",j.style.zIndex="5"),setTimeout(()=>{var V;const R=document.getElementById("duel-shock");if(R){const K=(V=C||j)==null?void 0:V.getBoundingClientRect(),ne=e.querySelector(".match-screen").getBoundingClientRect();K&&(R.style.top=K.top-ne.top+K.height/2+"px"),R.style.animation="shockwave .5s ease-out forwards"}C&&(C.style.transformOrigin="center",C.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var V;const R=document.getElementById("duel-finale");R&&(R.innerHTML=`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(s);let l=!1;const u=()=>{l||(l=!0,s.remove(),setTimeout(()=>d(),50))};s.addEventListener("click",u),setTimeout(u,3500)}async function Pt(e,t,i){var o,x;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,d=t.homeScore>t.aiScore,s=t.homeScore===t.aiScore,r=d?"victoire":s?"nul":"defaite",l=Ho(t.mode,r);t.matchId&&await T.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:d?n.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const u={credits:(n.profile.credits||0)+l,matches_played:(n.profile.matches_played||0)+1};d?u.wins=(n.profile.wins||0)+1:s?u.draws=(n.profile.draws||0)+1:u.losses=(n.profile.losses||0)+1,await T.from("users").update(u).eq("id",n.profile.id),await i.refreshProfile();const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",a.innerHTML=`
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(w);let E=!1;const L=()=>{E||(E=!0,w.remove(),setTimeout(()=>v(),50))};w.addEventListener("click",L),setTimeout(L,3500)}function se($,f,y){var U,G;const b=(g.gcDefs||[]).find(W=>{var te;return W.name===$||((te=W.name)==null?void 0:te.toLowerCase().trim())===($==null?void 0:$.toLowerCase().trim())}),v={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[b==null?void 0:b.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",w={purple:"#b06ce0",light_blue:"#00d4ef"}[b==null?void 0:b.color]||"#b06ce0",k=(b==null?void 0:b.name)||$,z=(b==null?void 0:b.effect)||((U=Oe[$])==null?void 0:U.desc)||"",E=b!=null&&b.image_url?`/icons/${b.image_url}`:null,L=((G=Oe[$])==null?void 0:G.icon)||"⚡",M=f===p?"Vous":g[f+"Name"]||"Adversaire",D=document.createElement("div");D.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",D.innerHTML=`
      <style>
      ...
      </div>`,(ye=M.querySelector("#psub-close"))==null||ye.addEventListener("click",()=>M.remove()),(be=M.querySelector("#pout-up"))==null||be.addEventListener("click",()=>{z>0&&(z--,D())}),(ke=M.querySelector("#pout-down"))==null||ke.addEventListener("click",()=>{z<w.length-1&&(z++,D())}),(Be=M.querySelector("#pin-up"))==null||Be.addEventListener("click",()=>{L>0&&(L--,D())}),(Fe=M.querySelector("#pin-down"))==null||Fe.addEventListener("click",()=>{L<k.length-1&&(L++,D())});const fe=(je,Ae,Ne,Ue)=>{const Ye=M.querySelector("#"+je);if(!Ye)return;let Ke=0;Ye.addEventListener("touchstart",Re=>{Ke=Re.touches[0].clientY},{passive:!0}),Ye.addEventListener("touchend",Re=>{const Ge=Re.changedTouches[0].clientY-Ke;if(Math.abs(Ge)<30)return;const We=Ae();Ge<0&&We<Ue-1?(Ne(We+1),D()):Ge>0&&We>0&&(Ne(We-1),D())},{passive:!0})};fe("pin-panel",()=>L,je=>L=je,k.length),fe("pout-panel",()=>z,je=>z=je,w.length),(Me=M.querySelector("#psub-confirm"))==null||Me.addEventListener("click",async je=>{if(je.preventDefault(),je.stopPropagation(),S)return;S=!0;const Ae=w[z],Ne=k[L];if(!Ae||!Ne)return;const Ue=Ae._line,Ye=(y[Ue]||[]).findIndex(Ge=>Ge.cardId===Ae.cardId);if(Ye===-1){x("Erreur : joueur introuvable","error"),M.remove();return}const Ke={...Ne,_line:Ue,position:Ae.position,used:!1,boost:0};y[Ue].splice(Ye,1,Ke);const Re=[...b,Ne.cardId];M.remove(),Z(Ae,Ne,async()=>{await K({[p+"Team"]:y,[h+"Team"]:g[h+"Team"],["usedSubIds_"+p]:Re})})})}document.body.appendChild(M),D()}function Z($,f,y){const b={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},v=document.createElement("div");v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const w=(E,L,S)=>`<div style="text-align:center">
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(v),(L=v.querySelector("#pvp-end-home"))==null||L.addEventListener("click",()=>{v.remove(),Ve(e),l("home")})}const R=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},B=>{const $=B.new;try{if($.status==="finished"||$.forfeit){if(c)return;c=!0,I&&(clearInterval(I),I=null),$.game_state&&(m=$.game_state),q($);return}if($.game_state){const f=m;m=$.game_state;const y=m._lastGC;if(y&&y.seq&&!F.has(y.seq)&&(F.add(y.seq),y.by!==a)){Ee(y.type,y.by,()=>re());return}const b=f[a+"Score"]||0,v=f[o+"Score"]||0,w=m[a+"Score"]||0,k=m[o+"Score"]||0,z=w>b,E=k>v;if((z||E)&&!C.has(m.round)){C.add(m.round);const L=[...m.log||[]].reverse().find(M=>M.isGoal),S=((L==null?void 0:L.homePlayers)||[]).map(M=>({name:M.name,note:M.note,portrait:M.portrait,job:M.job}));ve(S,w,k,z,()=>re());return}re()}}catch(f){console.error("[PvP] crash:",f)}}).subscribe();async function V(B){Object.assign(m,B),m.lastActionAt=new Date().toISOString();const{error:$}=await T.from("matches").update({game_state:m}).eq("id",i);$&&u("Erreur de synchronisation","error");try{re()}catch(f){console.error("[PvP] renderPvpScreen crash:",f)}}async function K(){if(c)return;c=!0,I&&(clearInterval(I),I=null);const B=n?h.away_id:h.home_id,$=n?"p2":"p1",f=n?"p1":"p2",y={...m,[$+"Score"]:3,[f+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:B,home_score:y.p1Score||0,away_score:y.p2Score||0,game_state:y}).eq("id",i)}catch{}try{T.removeChannel(R)}catch{}setTimeout(()=>{Ve(e),l("home")},800)}const ne={BOOST_STAT:({value:B=1,count:$=1,roles:f=[]},y,b)=>{const v=y[a+"Team"],w=Object.entries(v).filter(([k])=>!f.length||f.includes(k)).flatMap(([k,z])=>z.filter(E=>!E.used).map(E=>({...E,_line:k})));if(!w.length){y.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),b(y);return}ie(w,$,`Choisir ${$} joueur(s) à booster (+${B})`,k=>{k.forEach(z=>{const E=(v[z._line]||[]).find(L=>L.cardId===z.cardId);E&&(E.boost=(E.boost||0)+B,y.log.push({text:`⚡ +${B} sur ${E.name}`,type:"info"}))}),y[a+"Team"]=v,b(y)})},DEBUFF_STAT:({value:B=1,count:$=1,roles:f=[],target:y="ai"},b,v)=>{const w=y==="home"?a:o,k=b[w+"Team"],z=y==="home"?"allié":"adverse",E=Object.entries(k).filter(([L])=>!f.length||f.includes(L)).flatMap(([L,S])=>S.map(M=>({...M,_line:L})));if(!E.length){b.log.push({text:`🎯 Aucun joueur ${z}`,type:"info"}),v(b);return}ie(E,$,`Choisir ${$} joueur(s) ${z}(s) (-${B})`,L=>{L.forEach(S=>{const M=(k[S._line]||[]).find(D=>D.cardId===S.cardId);M&&(M.boost=(M.boost||0)-B,b.log.push({text:`🎯 -${B} sur ${M.name}`,type:"info"}))}),b[w+"Team"]=k,v(b)})},GRAY_PLAYER:({count:B=1,roles:$=[],target:f="ai"},y,b)=>{const v=f==="home"?a:o,w=y[v+"Team"],k=f==="home"?"allié":"adverse",z=Object.entries(w).filter(([E])=>!$.length||$.includes(E)).flatMap(([E,L])=>L.filter(S=>!S.used).map(S=>({...S,_line:E})));if(!z.length){y.log.push({text:`❌ Aucun joueur ${k}`,type:"info"}),b(y);return}ie(z,B,`Choisir ${B} joueur(s) ${k}(s) à exclure`,E=>{const L="usedCardIds_"+v,S=new Set(y[L]||[]);E.forEach(M=>{const D=(w[M._line]||[]).find(Y=>Y.cardId===M.cardId);D&&(D.used=!0,S.add(M.cardId),y.log.push({text:`❌ ${D.name} exclu !`,type:"info"}))}),y[L]=[...S],y[v+"Team"]=w,b(y)})},REVIVE_PLAYER:({count:B=1,roles:$=[]},f,y)=>{const b=f[a+"Team"],v=Object.entries(b).filter(([w])=>!$.length||$.includes(w)).flatMap(([w,k])=>k.filter(z=>z.used).map(z=>({...z,_line:w})));if(!v.length){f.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),y(f);return}ie(v,B,`Choisir ${B} joueur(s) à ressusciter`,w=>{w.forEach(k=>{const z=(b[k._line]||[]).find(E=>E.cardId===k.cardId);z&&(z.used=!1,f.log.push({text:`💫 ${z.name} ressuscité !`,type:"info"}))}),f[a+"Team"]=b,y(f)})},REMOVE_GOAL:({},B,$)=>{const f=o+"Score";B[f]>0?(B[f]--,B.log.push({text:"🚫 Dernier but annulé !",type:"info"})):B.log.push({text:"🚫 Aucun but à annuler",type:"info"}),$(B)},ADD_GOAL_DRAW:({},B,$)=>{B[a+"Score"]===B[o+"Score"]?(B[a+"Score"]++,B.log.push({text:"🎯 But bonus !",type:"info"})):B.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),$(B)},ADD_SUB:({value:B=1},$,f)=>{$.maxSubs=($.maxSubs||3)+B,$.log.push({text:`🔄 +${B} remplacement(s)`,type:"info"}),f($)},CUSTOM:({},B,$)=>$(B)};function ie(B,$,f,y){const b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let v=[];function w(){var z,E;const k=B.map(L=>{const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[L._line]||"#555",M=Se(L,L._line)+(L.boost||0),Y=v.find(U=>U.cardId===L.cardId)?"#FFD700":"rgba(255,255,255,0.25)",J=L.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${L.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${Y};background:${S};overflow:hidden;cursor:pointer;${J}">
        ...
        </div>`,(z=b.querySelector("#pp-close"))==null||z.addEventListener("click",()=>b.remove()),b.querySelectorAll(".pp-item").forEach(L=>{L.addEventListener("click",()=>{const S=L.dataset.cid,M=B.find(Y=>Y.cardId===S),D=v.findIndex(Y=>Y.cardId===S);M&&(D>-1?v.splice(D,1):v.length<$&&v.push(M),w())})}),(E=b.querySelector("#pp-confirm"))==null||E.addEventListener("click",()=>{b.remove(),y(v)})}w(),document.body.appendChild(b)}async function Q(B,$){const y=(m["gcCardsFull_"+a]||[]).find(k=>k.id===B),b=(y==null?void 0:y._gcDef)||(m.gcDefs||[]).find(k=>{var z;return k.name===$||((z=k.name)==null?void 0:z.toLowerCase().trim())===($==null?void 0:$.toLowerCase().trim())}),v=[...m["usedGc_"+a]||[],B],w={type:$,by:a,seq:(m._gcAnimSeq||0)+1};F.add(w.seq),Ee($,a,async()=>{if(b!=null&&b.effect_type&&b.effect_type!=="CUSTOM"){const z=ne[b.effect_type];if(z){const E={...m};z(b.effect_params||{},E,async L=>{L["usedGc_"+a]=v,L._lastGC=w,L._gcAnimSeq=w.seq,await V(L)});return}}const k={...m};switch($){case"Remplacement+":k.maxSubs=(k.maxSubs||3)+1,k.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const z=o+"Score";k[z]>0&&(k[z]--,k.log.push({text:"🚫 But annulé",type:"info"}));break}}k["usedGc_"+a]=v,k._lastGC=w,k._gcAnimSeq=w.seq,await V(k)})}function N(B,$){const f="usedCardIds_"+B,y=new Set(m[f]||[]);$.forEach(b=>y.add(b)),m[f]=[...y]}function P(){for(const B of["p1","p2"]){const $=new Set(m["usedCardIds_"+B]||[]),f=m[B+"Team"];if(f)for(const y of["GK","DEF","MIL","ATT"])(f[y]||[]).forEach(b=>{b.used=$.has(b.cardId)})}}function re(){var Ge,We,ot,ut,ft,gt;if(m.phase==="reveal")return xe();if(m.phase==="midfield")return we();if(m.phase==="finished")return ze();const B=m[a+"Team"],$=m[o+"Team"];P();const f=m[a+"Score"],y=m[o+"Score"],b=m[a+"Name"],v=m[o+"Name"],w=m.phase===a+"-attack",k=m.phase===a+"-defense",z=Array.isArray(m["selected_"+a])?m["selected_"+a]:[],E=z.map(le=>le.cardId),L=window.innerWidth>=700,S=m[a+"Subs"]||[],M=m["usedSubIds_"+a]||[],D=S.filter(le=>!M.includes(le.cardId)),Y=m["gcCardsFull_"+a]||[],J=m["usedGc_"+a]||[],U=Y.filter(le=>!J.includes(le.id)),G=m.boostOwner===a&&!m.boostUsed,W=!["GK","DEF","MIL","ATT"].some(le=>($[le]||[]).some(X=>!X.used)),te=[...B.MIL||[],...B.ATT||[]].filter(le=>!le.used),ee=w&&W&&te.length===0?[...B.GK||[],...B.DEF||[]].filter(le=>!le.used).map(le=>le.cardId):[];function fe(le,X,O){var xt,kt;const ae=le._gcDef,he={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ae==null?void 0:ae.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",$e={purple:"#b06ce0",light_blue:"#00d4ef"}[ae==null?void 0:ae.color]||"#b06ce0",Qe=(ae==null?void 0:ae.name)||le.gc_type,Ze=(ae==null?void 0:ae.effect)||((xt=Oe[le.gc_type])==null?void 0:xt.desc)||"",tt=ae!=null&&ae.image_url?`/icons/${ae.image_url}`:null,rt=((kt=Oe[le.gc_type])==null?void 0:kt.icon)||"⚡",_t=Math.round(O*.22),$t=Math.round(O*.22),dt=O-_t-$t,mt=Qe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${le.id}" data-gc-type="${le.gc_type}"
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(fe=S.querySelector("#psub-close"))==null||fe.addEventListener("click",()=>S.remove()),(ye=S.querySelector("#pout-up"))==null||ye.addEventListener("click",()=>{k>0&&(k--,M())}),(be=S.querySelector("#pout-down"))==null||be.addEventListener("click",()=>{k<v.length-1&&(k++,M())}),(ke=S.querySelector("#pin-up"))==null||ke.addEventListener("click",()=>{E>0&&(E--,M())}),(Be=S.querySelector("#pin-down"))==null||Be.addEventListener("click",()=>{E<w.length-1&&(E++,M())});const ee=(Me,je,Ae,Ne)=>{const Ue=S.querySelector("#"+Me);if(!Ue)return;let Ye=0;Ue.addEventListener("touchstart",Ke=>{Ye=Ke.touches[0].clientY},{passive:!0}),Ue.addEventListener("touchend",Ke=>{const Re=Ke.changedTouches[0].clientY-Ye;if(Math.abs(Re)<30)return;const Ge=je();Re<0&&Ge<Ne-1?(Ae(Ge+1),M()):Re>0&&Ge>0&&(Ae(Ge-1),M())},{passive:!0})};ee("pin-panel",()=>E,Me=>E=Me,w.length),ee("pout-panel",()=>k,Me=>k=Me,v.length),(Fe=S.querySelector("#psub-confirm"))==null||Fe.addEventListener("click",async Me=>{if(Me.preventDefault(),Me.stopPropagation(),L)return;L=!0;const je=v[k],Ae=w[E];if(!je||!Ae)return;const Ne=je._line,Ue=(f[Ne]||[]).findIndex(Re=>Re.cardId===je.cardId);if(Ue===-1){u("Erreur : joueur introuvable","error"),S.remove();return}const Ye={...Ae,_line:Ne,position:je.position,used:!1,boost:0};f[Ne].splice(Ue,1,Ye);const Ke=[...y,Ae.cardId];S.remove(),H(je,Ae,async()=>{await V({[a+"Team"]:f,[o+"Team"]:m[o+"Team"],["usedSubIds_"+a]:Ke})})})}document.body.appendChild(S),M()}function H(B,$,f){const y={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const v=(z,E,L)=>`<div style="text-align:center">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div id="mkt-content" style="padding:10px 16px;display:flex;flex-direction:column;gap:8px"></div>
  </div>`;let u="buy";const a=()=>{var m,c,A,g,I,j,C;return{name:(((m=document.getElementById("flt-name"))==null?void 0:m.value)||"").toLowerCase().trim(),club:(((c=document.getElementById("flt-club"))==null?void 0:c.value)||"").toLowerCase().trim(),country:(((A=document.getElementById("flt-country"))==null?void 0:A.value)||"").toLowerCase().trim(),job:((g=document.getElementById("flt-job"))==null?void 0:g.value)||"",rarity:((I=document.getElementById("flt-rarity"))==null?void 0:I.value)||"",note1:parseInt((j=document.getElementById("flt-note1"))==null?void 0:j.value)||0,note2:parseInt((C=document.getElementById("flt-note2"))==null?void 0:C.value)||0}};function o(m){const c=a();return m.filter(A=>{var V,K,ne;const g=(V=A.card)==null?void 0:V.player;if(!g)return!1;const I=`${g.firstname} ${g.surname_encoded}`.toLowerCase(),j=(((K=g.clubs)==null?void 0:K.encoded_name)||"").toLowerCase(),C=(g.country_code||"").toLowerCase(),F=((ne=A.card)==null?void 0:ne.evolution_bonus)||0,q=St(g,g.job,F),R=g.job2?St(g,g.job2,F):0;return!(c.name&&!I.includes(c.name)||c.club&&!j.includes(c.club)||c.country&&!C.includes(c.country)||c.job&&g.job!==c.job||c.rarity&&g.rarity!==c.rarity||c.note1&&q<c.note1||c.note2&&R<c.note2)})}function x(m){var R,V,K,ne;const c=(R=m.card)==null?void 0:R.player;if(!c)return"";const A=((V=m.card)==null?void 0:V.evolution_bonus)||0,g=St(c,c.job,A),I=c.job2?St(c,c.job2,A):0,j=(i.profile.credits||0)>=m.price,C=c.country_code?`https://flagsapi.com/${c.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,F=Qt[c.job]||"#bbb",q=c.job2?Qt[c.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
      ...
      <div style="text-align:right;flex-shrink:0">
        ...
        <button class="btn btn-primary btn-sm" data-buy="${m.id}" ${j?"":"disabled"} style="margin-top:4px;font-size:11px;padding:4px 10px">${j?"Acheter":"Trop cher"}</button>
      </div>
    </div>`}function p(m){var R,V,K,ne;const c=(R=m.card)==null?void 0:R.player;if(!c)return"";const A=((V=m.card)==null?void 0:V.evolution_bonus)||0,g=St(c,c.job,A),I=c.job2?St(c,c.job2,A):0,j=m.status==="sold",C=c.country_code?`https://flagsapi.com/${c.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,F=Qt[c.job]||"#bbb",q=c.job2?Qt[c.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${j?"opacity:0.7":""}">
      ${C?`<img src="${C}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(K=c.clubs)!=null&&K.logo_url?`<img src="${c.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${F};display:flex;align-items:center;justify-content:center">
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const A=r||[];_.innerHTML=A.length>0?A.map((g,I)=>{const j=Nt(g.mmr??1e3),C=(g.ranked_wins||0)+(g.ranked_losses||0)+(g.ranked_draws||0),F=C>0?Math.round((g.ranked_wins||0)/C*100):0,q=g.id===i.profile.id,R=(g.placement_matches||0)<10;return`
          <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${q?"border:2px solid var(--yellow)":""}">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:11px;color:var(--gray-600)">${g.club_name} · ${j.label}</div>
            </div>
            <div style="text-align:right;flex-shrink:0">
