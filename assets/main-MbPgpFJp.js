const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/formation-links-CUL_inpB.js","assets/formation-links-DCm4yyf6.css"])))=>i.map(i=>d[i]);
import{s as w,F as si,r as Ae,j as St,l as Pi,m as yt,h as di,n as gn,o as mn,T as Yt,k as xn,p as yn,b as bn}from"./formation-links-CUL_inpB.js";const hn="/";function Li(e,{navigate:t,toast:i}){let n="login";const a=()=>{var r,s,d,l,c,u;const o=n==="login";e.innerHTML=`
    <div style="
  ...
  `;function s(){var f;const l=document.getElementById("logo-preview"),c=document.getElementById("logo-initials"),u=((f=document.getElementById("setup-club"))==null?void 0:f.value)||"MW",g=u.trim().split(" ").filter(Boolean),m=g.length>=2?(g[0][0]+g[1][0]).toUpperCase():u.slice(0,2).toUpperCase();l&&(l.style.background=r,l.style.borderColor=o),c&&(c.textContent=m,c.style.color=o)}document.getElementById("color1").addEventListener("input",l=>{o=l.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",l=>{r=l.target.value,document.getElementById("swatch2").style.background=r,s()});function d(l){document.querySelectorAll(".setup-step").forEach(c=>c.classList.remove("active")),document.getElementById(`step-${l}`).classList.add("active"),document.getElementById("step-num").textContent=l,document.getElementById("progress-fill").style.width=`${Math.round(l/3*100)}%`,l===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("step1-error");if(c.textContent="",l.length<3){c.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await w.from("users").select("id").eq("pseudo",l).maybeSingle();if(u){c.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const l=document.getElementById("setup-club").value.trim(),c=document.getElementById("step2-error");if(c.textContent="",l.length<2){c.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await w.from("users").select("id").eq("club_name",l).maybeSingle();if(u){c.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),g=document.getElementById("step3-finish");u.textContent="",g.disabled=!0,g.textContent="Création en cours…";try{const{error:m}=await w.from("users").insert({id:t.user.id,pseudo:l,club_name:c,club_color1:o,club_color2:r,credits:1e4});if(m)throw m;await wn(t.user.id),await a(),n(`Bienvenue ${l} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(m){u.textContent=m.message,g.disabled=!1,g.textContent="🚀 Créer mon profil !"}})}async function wn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await w.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const _n="modulepreload",kn=function(e){return"/"+e},Ti={},Ri=function(t,i,n){let a=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),s=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));a=Promise.allSettled(i.map(d=>{if(d=kn(d),d in Ti)return;Ti[d]=!0;const l=d.endsWith(".css"),c=l?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${c}`))return;const u=document.createElement("link");if(u.rel=l?"stylesheet":_n,l||(u.as="script"),u.crossOrigin="",u.href=d,s&&u.setAttribute("nonce",s),document.head.appendChild(u),l)return new Promise((g,m)=>{u.addEventListener("load",g),u.addEventListener("error",()=>m(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(r){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=r,window.dispatchEvent(s),!s.defaultPrevented)throw r}return a.then(r=>{for(const s of r||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},Et="#1A6B3C",Lt="#cc2222",$n="#D4A017",zi="#888";async function En(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </style>`}const In="2026.07.14-1446";async function Ii(e,{state:t,navigate:i,toast:n}){var o,r;const a=t.profile;a&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="home-dark" id="home-dark">
    ...
    ${Ae(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),l=(s||[]).filter(z=>z.card_type==="player"&&z.player),c=(s||[]).filter(z=>z.card_type==="game_changer"),u=(s||[]).filter(z=>z.card_type==="formation"),g=(s||[]).filter(z=>z.card_type==="stadium"),{data:m}=await w.from("gc_definitions").select("name,gc_type,color,effect,image_url"),f={};(m||[]).forEach(z=>{f[z.name]=z});const{data:p}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),x={};(p||[]).forEach(z=>{x[z.id]=z}),g.forEach(z=>{z.stadium_def&&(x[z.stadium_id]=z.stadium_def)});const b=Object.keys(si),h=Object.keys(Se),v={};l.forEach(z=>{const S=z.player.id;v[S]=(v[S]||0)+1}),new Set(Object.keys(v).map(z=>String(z)));const k=new Set(u.map(z=>z.formation)),y=new Set(c.map(z=>z.gc_type));let C="player",E="Tous",G="",N=!1;function ee(){return[...l].sort((z,S)=>{const R=jt.indexOf(z.player.job),H=jt.indexOf(S.player.job);return R!==H?R-H:(z.player.surname_real||"").localeCompare(S.player.surname_real||"")})}function ie(){return[...d||[]].sort((z,S)=>{const R=jt.indexOf(z.job),H=jt.indexOf(S.job);return R!==H?R-H:(z.surname_real||"").localeCompare(S.surname_real||"")})}function le(){return ee().filter(z=>{const S=z.player,R=E==="Tous"||S.job===E,H=!G||`${S.firstname} ${S.surname_real}`.toLowerCase().includes(G);return R&&H})}function B(){return ie().filter(z=>{const S=E==="Tous"||z.job===E,R=!G||`${z.firstname} ${z.surname_real}`.toLowerCase().includes(G);return S&&R})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function T(){const z=document.getElementById("col-filters");z&&(C==="player"?(z.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",S=>{G=S.target.value.toLowerCase(),$()}),e.querySelectorAll(".filter-btn").forEach(S=>{S.addEventListener("click",()=>{E=S.dataset.job,T(),$()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{N=!N,T(),$()})):(z.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{N=!N,T(),$()})))}function $(){const z=document.getElementById("col-grid");z&&(C==="player"?Ee(z):C==="formation"?U(z):C==="stadium"?V(z):Q(z))}function F(z,S,R,H,K){const j=document.getElementById("col-grid"),O=document.getElementById("col-big");if(!j||!O)return;var $e=0;function _e(){const we=window.innerWidth>=768,me=document.getElementById("col-big"),Le=document.getElementById("col-grid");we&&me&&(me.style.minHeight="420px",me.style.flex="1 1 auto"),we&&Le&&(Le.style.height=Math.round(310*.76+16)+"px",Le.style.flexShrink="0",Le.style.overflowX="auto",Le.style.overflowY="hidden",Le.style.alignItems="center",Le.style.padding="8px 16px"),O.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+S(z[$e])+"</div>";var ke=O.querySelector("[data-card-id],[data-form-id],[data-gc-id]");ke&&ke.addEventListener("click",function(){H(z[$e])}),requestAnimationFrame(function(){var he=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!he||!O)){var Ce=O.clientHeight-8,je=O.clientWidth-24,Fe=he.offsetHeight,_=he.offsetWidth;if(Fe>0&&_>0&&Ce>40){var L=we?2.2:1.6,A=Math.min(Ce/Fe,je/_,L);he.style.transform="scale("+A.toFixed(3)+")",he.style.transformOrigin="top center"}}}),j.innerHTML=z.map(function(he,Ce){var je=Ce===$e,Fe="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(je?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+Ce+'" style="'+Fe+'">'+R(he,je)+"</div>"}).join(""),j.querySelectorAll(".col-mini-item").forEach(function(he){he.addEventListener("click",function(){$e=Number(he.dataset.idx),_e(),he.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}_e()}function re(z){var S=window.innerWidth>=768?.76:.54,R;if(!z||z._fake){var H=z?z.player:null;if(!H)return"";R=Mi(H)}else R=Jt(z,"");return'<div style="display:inline-block;zoom:'+S+';pointer-events:none;line-height:0">'+R+"</div>"}function pe(z,S,R){S=S||100,R=R||140;var H=St[z]||{},K={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},j=Math.max(3,Math.round(S*.06)),O=Object.entries(H).map(function(_e){var we=_e[0],me=_e[1],Le=we.replace(/\d+$/,""),ke=K[Le]||"#888",he=Math.round(me.x*S),Ce=Math.round(me.y*R);return'<circle cx="'+he+'" cy="'+Ce+'" r="'+j+'" fill="'+ke+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),$e=Math.max(1,Math.round(S/50));return'<svg viewBox="0 0 '+S+" "+R+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+S+'" height="'+R+'" fill="#1A6B3C"/><rect x="'+Math.round(S*.2)+'" y="'+Math.round(R*.02)+'" width="'+Math.round(S*.6)+'" height="'+Math.round(R*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+$e+'"/><line x1="0" y1="'+Math.round(R*.5)+'" x2="'+S+'" y2="'+Math.round(R*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+$e+'"/><ellipse cx="'+Math.round(S*.5)+'" cy="'+Math.round(R*.5)+'" rx="'+Math.round(S*.18)+'" ry="'+Math.round(R*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+$e+'"/><rect x="'+Math.round(S*.2)+'" y="'+Math.round(R*.82)+'" width="'+Math.round(S*.6)+'" height="'+Math.round(R*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+$e+'"/>'+O+"</svg>"}function de(z,S,R){var H=R>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+R+"</div>":"",K=S?'data-form-id="'+S.id+'"':"",j=z.length>7?14:z.length>5?16:19,O=!!S;return"<div "+K+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(O?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(O?"":"filter:grayscale(1);opacity:0.5")+'">'+H+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(O?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+j+"px;font-weight:900;color:"+(O?"#1A6B3C":"#aaa")+';line-height:1">'+z+'</div></div><div style="flex:1;overflow:hidden;background:'+(O?"#1A6B3C":"#ccc")+'">'+pe(z,140,220)+"</div></div>"}function ae(z,S){var R=window.innerWidth>=768?.76:.54,H=140,K=305,j=Math.round(K*.22),O=K-j,$e=z.length>7?10:13,_e=pe(z,H,O),we=S?"1.5px solid #2a7a40":"1px solid #ddd",me=S?"":"filter:grayscale(1);opacity:0.45;",Le=S?"#1A6B3C":"#bbb",ke="#fff";return'<div style="display:inline-block;zoom:'+R+';line-height:0;pointer-events:none"><div style="width:'+H+"px;height:"+K+"px;border-radius:6px;border:"+we+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+me+'"><div style="height:'+j+"px;background:"+Le+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+$e+"px;font-weight:900;color:"+ke+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(H-4)+'px">'+z+'</span></div><div style="height:'+O+'px;overflow:hidden;flex-shrink:0">'+_e+"</div></div></div>"}function Ee(z){if(N){const S=B();if(!S.length){z.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const R=S.map(H=>l.find(K=>K.player.id===H.id)||{_fake:!0,player:H,id:"fake-"+H.id});F(R,H=>H._fake?Mi(H.player):Jt(H,""),H=>H._fake?re({player:H.player,id:"x",_fake:!0}):re(H),H=>{H._fake||Ci(H,l,v,t)})}else{const S=le();if(!S.length){z.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const R={};S.forEach(K=>{const j=K.player.id;R[j]||(R[j]=[]),R[j].push(K)});const H=Object.values(R).map(K=>Pn(K));F(H,K=>{const j=v[K.player.id]||1,O=j>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${j}</div>`:"",_e=l.filter(we=>we.player.id===K.player.id&&we.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Jt(K,O+_e)},K=>re(K),K=>Ci(K,l,v,t))}}function U(z){const S=N?b:[...k];if(!S.length){z.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const R=S.map(H=>({formation:H,card:u.find(K=>K.formation===H)||null,owned:k.has(H)}));F(R,({formation:H,card:K,owned:j})=>de(H,j?K:null,j?u.filter(O=>O.formation===H).length:0),({formation:H,owned:K})=>ae(H,K),({card:H,owned:K})=>{K&&H&&Kn(H,u,t,o)})}function Q(z){const S=Object.keys(f),R=N?S.length?S:h:[...y];if(!R.length){z.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const H=R.map(K=>({type:K,gc:Se[K]||{icon:"⚡",desc:""},def:f[K]||null,owned:y.has(K),card:c.find(j=>j.gc_type===K)||null}));F(H,({type:K,gc:j,def:O,owned:$e,card:_e})=>{var L;const we=$e?c.filter(A=>A.gc_type===K).length:0,me=we>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${we}</div>`:"",Le=(O==null?void 0:O.gc_type)==="ultra_game_changer",ke={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},he={purple:"#b06ce0",light_blue:"#00d4ef"},Ce=ke[O==null?void 0:O.color]||ke.purple,je=he[O==null?void 0:O.color]||he.purple,Fe=(O==null?void 0:O.effect)||j.desc||"",_=O!=null&&O.image_url?`/icons/${O.image_url}`:((L=O==null?void 0:O.club)==null?void 0:L.logo_url)||(O!=null&&O.country_code?`https://flagsapi.com/${O.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);return $e&&_e?`<div data-gc-id="${_e.id}" data-gc-type="${K}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${je};background:${Ce};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${je}66;cursor:pointer">
          ${me}
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(z=>{z.addEventListener("click",()=>{C=z.dataset.tab,E="Tous",G="",N=!1,e.querySelectorAll(".col-tab-btn").forEach(S=>{const R=S.dataset.tab===C;S.style.borderBottomColor=R?"var(--green)":"transparent",S.style.color=R?"var(--green)":"var(--gray-600)"}),T(),$()})}),T(),$()}function Un(e,t,i){const n=Se[e]||{icon:"⚡",desc:"Effet spécial."},a=(t==null?void 0:t.gc_type)==="ultra_game_changer",o={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},r={purple:"#b06ce0",light_blue:"#00d4ef"},s=o[t==null?void 0:t.color]||o.purple,d=r[t==null?void 0:t.color]||r.purple,l=(t==null?void 0:t.name)||e,c=(t==null?void 0:t.effect)||n.desc,u=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Bt=1e3;function Kn(e,t,i,n){var f,p,x;const{state:a,toast:o,closeModal:r,navigate:s,refreshProfile:d}=i,l=e.formation,c={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const b=St[l]||{},h=si[l]||[],v=290,k=360,y=20;function C(G){const N=b[G];return N?{x:N.x*v,y:N.y*k}:null}let E=`<svg width="${v}" height="${k}" viewBox="0 0 ${v} ${k}" xmlns="http://www.w3.org/2000/svg">`;for(const[G,N]of h){const ee=C(G),ie=C(N);!ee||!ie||(E+=`<line x1="${ee.x}" y1="${ee.y}" x2="${ie.x}" y2="${ie.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const G of Object.keys(b)){const N=C(G);if(!N)continue;const ee=G.replace(/\d+/,""),ie=c[ee]||"#555";E+=`<circle cx="${N.x}" cy="${N.y}" r="${y}" fill="${ie}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,E+=`<text x="${N.x}" y="${N.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${ee}</text>`}return E+="</svg>",E}const g=t.filter(b=>b.formation===l);g.length;const m=!e.is_for_sale;n(`Formation ${l}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${u.map((U,Q)=>{const V=le[U.id]||[],z=U.is_for_sale,S=V.length?V[V.length-1]:null,H=(c.rarity==="pepite"||c.rarity==="papyte")&&U.current_note!=null?` (☆${U.current_note})`:"";return`
            <div data-card-id="${U.id}" data-card-idx="${Q}" class="exemplaire-row"
              ...
              <div style="display:flex;align-items:center;gap:10px">
                <input type="checkbox" class="expl-check" data-id="${U.id}" ${z?"disabled":""} style="width:18px;height:18px;cursor:pointer;accent-color:#1A6B3C;flex-shrink:0">
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:700">Exemplaire ${Q+1}${H}${z?" 🏷️ En vente":""}</div>
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;background:#f0fdf4;border-radius:10px;padding:10px 14px">
          <div>
            ...
            <div style="font-size:11px;color:#555;margin-top:2px">
              ${(e.evolution_bonus||0)>0?`+${e.evolution_bonus} appliqué${e.evolution_bonus>1?"s":""} · Note : ${G}${c.job2&&N>0?` / ${ee}`:""}`:`Note actuelle : ${G}${c.job2&&N>0?` / ${ee}`:""}`}
            </div>
          </div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",c.note_g],["DEF",c.note_d],["MIL",c.note_m],["ATT",c.note_a]].map(([U,Q])=>{const V=Wt[U],z=U==="GK"?"#fff":V,S=U===c.job||U===c.job2,R=(Number(Q)||0)+(S&&C>0?C:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
                <div style="width:48px;height:48px;border-radius:8px;background:#111;border:2.5px solid ${V};
                  display:flex;align-items:center;justify-content:center">
                  <span style="font-size:20px;font-weight:900;color:${z};font-family:Arial Black,Arial;line-height:1">${R}</span>
                </div>
                <span style="font-size:10px;font-weight:700;color:${z}">${U}</span>
              </div>`}).join("")}
          </div>
        </div>
        <div>
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(pe=document.getElementById("close-detail"))==null||pe.addEventListener("click",s);let $=new Set;const F=()=>{const U=$.size,Q=document.getElementById("sell-action-panel");if(!Q)return;Q.style.display=U>0?"block":"none",document.getElementById("sell-selected-count").textContent=U;const V=document.getElementById("evolve-btn");V&&(V.textContent=`⬆️ Évoluer ${U>1?"(+"+U+")":""}`)};document.querySelectorAll(".expl-check").forEach(U=>{U.addEventListener("change",()=>{const Q=U.dataset.id;U.checked?$.add(Q):$.delete(Q);const V=U.closest(".exemplaire-row");V&&(V.style.borderColor=U.checked?"#1A6B3C":"#eee"),F()})}),document.querySelectorAll(".exemplaire-row").forEach(U=>{U.addEventListener("click",Q=>{if(Q.target.closest("button")||Q.target.tagName==="INPUT")return;const V=U.querySelector(".expl-check");V&&!V.disabled&&(V.checked=!V.checked,V.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(U=>{U.addEventListener("click",Q=>{Q.stopPropagation();const V=document.querySelector(`.expl-hist[data-hist="${U.dataset.idx}"]`);V&&(V.style.display=V.style.display==="none"?"flex":"none")})}),(de=document.getElementById("evolve-btn"))==null||de.addEventListener("click",async()=>{if(g<=1)return;const U=[...$];if(!U.length)return;if($.has(e.id)){const K=document.createElement("div");K.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",K.innerHTML=`
        ...
        </div>`,document.body.appendChild(K),K.querySelector("#err-close").addEventListener("click",()=>K.remove()),K.addEventListener("click",j=>{j.target===K&&K.remove()});return}const Q=U.filter(K=>K!==e.id),V=c.rarity==="legende"?2:1;if((Q.length||1)*V,!(!Q.length&&U.length===1&&U[0]===e.id)){if(!Q.length){o("Sélectionne des copies à sacrifier","warning");return}}if(!await new Promise(K=>{const j=document.createElement("div");j.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",j.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    </div>`;x("⚽ Choisir une formation",h),document.querySelectorAll("#modal-body [data-forma]").forEach(v=>{v.addEventListener("click",()=>{t.formation=v.dataset.forma;const k=Bi(t.formation),y={};k.forEach(C=>{t.slots[C]&&(y[C]=t.slots[C])}),t.slots=y,b(),Ze(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(x=>x.addEventListener("click",m)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(x=>x.addEventListener("click",()=>Yn(t,e,i))),e.querySelectorAll("#save-deck").forEach(x=>x.addEventListener("click",()=>Jn(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(x=>{x.addEventListener("click",()=>{t.subs=t.subs.filter(b=>b!==x.dataset.removeSub),Ze(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(x=>x.addEventListener("click",()=>Xn(t,e,i)))}function Vn(e,t,i,n){var v,k;const a=window.innerWidth>=900,o=e.querySelector(a?"#deck-field-pc":"#deck-field-mobile");if(!o)return;const r=(v=t.stadiumCards)==null?void 0:v.find(y=>y.id===t.stadiumCardId),s=r&&((k=t.stadDefMap)==null?void 0:k[r.stadium_id])||null,d=St[t.formation]||{},l=Pi(t.formation)||[],c={};for(const y of i){const C=t.slots[y],E=C?t.playerCards.find(G=>G.id===C):null;E!=null&&E.player?c[y]={...E.player,_evolution_bonus:E.evolution_bonus||0,face:E.player.face||null}:c[y]=null}const u=window.innerWidth>=900,g=u?window.innerWidth-280:window.innerWidth-20,m=u?Math.min(g,860):g,f=Math.round(u?m*.82:m*.85),p=u?84:44;let x="";for(const[y,C]of l){const E=d[y],G=d[C];if(!E||!G)continue;const N=E.x*m,ee=Math.round(.03*f+E.y*.85*f),ie=G.x*m,le=Math.round(.03*f+G.y*.85*f),B=c[y],T=c[C],$=yt(B,T);$==="#ff3333"||$==="#cc2222"?x+=`<line x1="${N.toFixed(1)}" y1="${ee.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${le.toFixed(1)}" stroke="${$}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(x+=`<line x1="${N.toFixed(1)}" y1="${ee.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${le.toFixed(1)}" stroke="${$}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,x+=`<line x1="${N.toFixed(1)}" y1="${ee.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${le.toFixed(1)}" stroke="${$}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let b="";const h=Math.round(p*657/507);for(const y of i){const C=d[y];if(!C)continue;const E=c[y],G=C.x*m;C.y*f;const N=Math.round(.03*f+C.y*(.85*f)),ee=Math.round(G-p/2),ie=Math.round(N-h/2);if(E){const le=y.replace(/\d+/,""),B=s&&(s.club_id&&String(E.club_id)===String(s.club_id)||s.country_code&&E.country_code===s.country_code),T=Ae({...E,_evolution_bonus:E._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:le});b+=`<div style="position:absolute;left:${ee}px;top:${ie}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${y}">
        <div style="position:relative">${B?'<div style="position:absolute;top:-18px;left:0;right:0;text-align:center;font-size:14px;z-index:5;line-height:1">🏟️</div>':""}${T}</div>
      </div>`}else{const le=y.replace(/\d+/,"");b+=`<div style="position:absolute;left:${ee}px;top:${ie}px;width:${p}px;height:${h}px;
  ...
  `,document.body.appendChild(a);let o=!1;const r=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let d=!1;const l=T=>T.touches&&T.touches[0]?T.touches[0].clientX:T.clientX;function c(T){o||(d=!0,s.style.opacity="1",u(T))}function u(T){if(!d||o)return;const $=r.getBoundingClientRect(),F=l(T)-$.left,re=Math.max(0,Math.min(1,F/$.width));s.style.width=re*$.width+"px",re>=.82&&m()}function g(){o||(d=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function m(){var $;if(o)return;o=!0,d=!1,s.style.width="100%",s.style.opacity="1",($=document.getElementById("cut-flash"))==null||$.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const T=document.getElementById("cut-hint");T&&(T.style.opacity="0"),r.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",b(0)},620)}r.addEventListener("pointerdown",c),window.addEventListener("pointermove",u),window.addEventListener("pointerup",g),r.addEventListener("touchstart",c,{passive:!0}),window.addEventListener("touchmove",u,{passive:!0}),window.addEventListener("touchend",g);let f=0,p=!1;const x=new Set;function b(T){f=T,document.getElementById("reveal-phase").style.display="flex",h(),v(T,0),G()}function h(){const T=document.getElementById("card-dots");T&&(T.innerHTML=e.map(($,F)=>`<div class="card-dot" data-i="${F}" style="width:8px;height:8px;border-radius:50%;background:${F===f?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),T.querySelectorAll(".card-dot").forEach($=>$.addEventListener("click",()=>y(parseInt($.dataset.i)))))}function v(T,$){var V,z;const F=e[T],re=document.getElementById("card-counter"),pe=document.getElementById("card-track");re&&(re.textContent=`Carte ${T+1} / ${e.length}`);const de=document.getElementById("reveal-btns");de&&(de.style.display=T===e.length-1?"flex":"none");const ae=F.card_type==="player"&&((V=F.player)==null?void 0:V.rarity)==="legende",Ee=!x.has(T);x.add(T);let U=0;if(F.card_type==="player"&&F.player){const S=F.player,R=S.job||"ATT";U=Number(R==="GK"?S.note_g:R==="DEF"?S.note_d:R==="MIL"?S.note_m:S.note_a)||0}const Q=S=>{pe.innerHTML=`
        ...
        </div>`;const R=document.getElementById("current-card-wrap");$!==0?(R.style.transition="none",R.style.transform=`translateX(${$>0?100:-100}%)`,requestAnimationFrame(()=>{R.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",R.style.transform="translateX(0)"})):R.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),S||ae?ee():ie(),h()};Ee&&(((z=F.player)==null?void 0:z.rarity)==="legende"||U>=8)&&F.card_type==="player"&&F.player?k(F,()=>Q(!0)):Q(!1)}function k(T,$){var we;p=!0;const F=T.player,re=`https://flagsapi.com/${F.country_code}/flat/64.png`,pe=(we=F.clubs)==null?void 0:we.logo_url,de=F.face?"/"+F.face.replace(/^public\//,"").replace(/^\//,""):null,ae=F.job||"ATT",Ee=Number(ae==="GK"?F.note_g:ae==="DEF"?F.note_d:ae==="MIL"?F.note_m:F.note_a)||0,U=T.evolution_bonus||0,Q=Ee+U,V=F.rarity==="legende",z=Q>=18,S=document.getElementById("walkout-overlay"),R=document.getElementById("walkout-stage");if(!S||!R){p=!1,$();return}let H=null;V&&(H=new Audio("/sounds/Legendary.mp3"),H.volume=.8,H.play().catch(()=>{})),S.style.display="flex";const K=()=>{const me=R.firstElementChild;me&&(me.classList.remove("wo-in"),me.classList.add("wo-out"))},j=1800,O=400;R.innerHTML=`<img class="wo-in" src="${re}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(K,j),setTimeout(()=>{var me;R.innerHTML=pe?`<img class="wo-in" src="${pe}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((me=F.clubs)==null?void 0:me.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},j+O),setTimeout(K,j*2+O),setTimeout(()=>{R.innerHTML=de?`<img class="wo-in" src="${de}" style="height:200px;border-radius:50%;box-shadow:0 0 40px rgba(255,255,255,0.3);object-fit:cover;object-position:top">`:'<div class="wo-in" style="font-size:80px">👤</div>',navigator.vibrate&&navigator.vibrate(30)},(j+O)*2),setTimeout(K,(j+O)*2+j);const _e=z?"#FFD700":{GK:"#c0c0c0",DEF:"#e03030",MIL:"#D4A017",ATT:"#3fbf5f"}[ae]||"#fff";setTimeout(()=>{R.innerHTML=`<div class="wo-in" style="
        ...
        ${Q}
      </div>`,z&&navigator.vibrate&&navigator.vibrate([50,30,100,30,200])},(j+O)*3),setTimeout(K,(j+O)*3+j),setTimeout(()=>{S.style.display="none",R.innerHTML="",p=!1,H&&!V&&H.pause(),navigator.vibrate&&navigator.vibrate([40,30,80]),$()},(j+O)*4)}function y(T){if(p||T<0||T>=e.length||T===f)return;const $=T>f?1:-1;f=T,v(T,$)}function C(){y(f+1)}function E(){y(f-1)}function G(){const T=document.getElementById("card-viewport");if(!T||T._swipeBound)return;T._swipeBound=!0;let $=0,F=0,re=0,pe=!1;const de=V=>V.touches?V.touches[0].clientX:V.clientX,ae=V=>V.touches?V.touches[0].clientY:V.clientY,Ee=V=>{pe=!0,$=de(V),F=ae(V),re=0},U=V=>{if(!pe)return;re=de(V)-$;const z=ae(V)-F;if(Math.abs(re)<Math.abs(z))return;const S=document.getElementById("current-card-wrap");S&&(S.style.transition="none",S.style.transform=`translateX(${re*.6}px) rotate(${re*.02}deg)`)},Q=()=>{if(!pe)return;pe=!1;const V=document.getElementById("current-card-wrap"),z=55;re<=-z&&f<e.length-1?C():re>=z&&f>0?E():V&&(V.style.transition="transform .2s ease",V.style.transform="translateX(0)")};T.addEventListener("pointerdown",Ee),T.addEventListener("pointermove",U),T.addEventListener("pointerup",Q),T.addEventListener("pointercancel",Q),T.addEventListener("touchstart",Ee,{passive:!0}),T.addEventListener("touchmove",U,{passive:!0}),T.addEventListener("touchend",Q),T.addEventListener("click",V=>{if(Math.abs(re)>8)return;const z=T.getBoundingClientRect();V.clientX-z.left>z.width/2?C():E()})}let N=null;function ee(){const T=document.getElementById("fireworks-canvas");if(!T)return;T.width=window.innerWidth,T.height=window.innerHeight;const $=T.getContext("2d"),F=[];function re(){const de=Math.random()*T.width,ae=Math.random()*T.height*.6,Ee=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],U=Ee[Math.floor(Math.random()*Ee.length)];for(let Q=0;Q<60;Q++){const V=Math.PI*2/60*Q,z=2+Math.random()*5;F.push({x:de,y:ae,vx:Math.cos(V)*z,vy:Math.sin(V)*z,alpha:1,color:U,size:2+Math.random()*3})}}re(),N=setInterval(re,600);function pe(){if(document.getElementById("fireworks-canvas")){$.clearRect(0,0,T.width,T.height);for(let de=F.length-1;de>=0;de--){const ae=F[de];if(ae.x+=ae.vx,ae.y+=ae.vy+.08,ae.vy*=.98,ae.alpha-=.018,ae.alpha<=0){F.splice(de,1);continue}$.globalAlpha=ae.alpha,$.fillStyle=ae.color,$.beginPath(),$.arc(ae.x,ae.y,ae.size,0,Math.PI*2),$.fill()}$.globalAlpha=1,(N!==null||F.length>0)&&requestAnimationFrame(pe)}}pe()}function ie(){N!==null&&(clearInterval(N),N=null);const T=document.getElementById("fireworks-canvas");T&&T.getContext("2d").clearRect(0,0,T.width,T.height)}if(n){const T=document.getElementById("reveal-btns");T&&(T.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(B=document.getElementById("reveal-next"))==null||B.addEventListener("click",()=>{ie(),a.remove(),n()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{ie(),a.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{ie(),a.remove(),i("boosters")})}function ao(e){var t,i,n,a;if(e.card_type==="player"&&e.player)return to(e);if(e.card_type==="game_changer"){const o=e._gcDef,r=(o==null?void 0:o.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},d={purple:"#b06ce0",light_blue:"#00d4ef"},l=s[o==null?void 0:o.color]||s.purple,c=d[o==null?void 0:o.color]||d.purple,u=(o==null?void 0:o.name)||e.gc_type||"Game Changer",g=(o==null?void 0:o.effect)||((t=ti[e.gc_type])==null?void 0:t.desc)||"",m=o!=null&&o.image_url?`/icons/${o.image_url}`:null,f=((i=ti[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${l};border-radius:14px;border:3px solid ${c};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${c}88;flex-shrink:0">
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",r).order("slot_order"),d=[...new Set((s||[]).filter(g=>{var m,f;return((m=g.card)==null?void 0:m.card_type)==="stadium"&&((f=g.card)==null?void 0:f.stadium_id)}).map(g=>g.card.stadium_id))],l={};if(d.length){const{data:g}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",d);(g||[]).forEach(m=>{l[m.id]=m}),(s||[]).forEach(m=>{var f,p;((f=m.card)==null?void 0:f.card_type)==="stadium"&&((p=m.card)!=null&&p.stadium_id)&&(m.card._stadiumDef=l[m.card.stadium_id]||null)})}let c=0;function u(){var C,E,G,N,ee,ie,le;const g=o[c],m=(s||[]).filter(B=>B.deck_id===g.id),f=m.filter(B=>{var T;return B.is_starter&&((T=B.card)==null?void 0:T.player)}).map(B=>ni(B.card,B.position)),p=m.find(B=>{var T;return((T=B.card)==null?void 0:T.card_type)==="formation"}),x=g.formation||((C=p==null?void 0:p.card)==null?void 0:C.formation)||"4-4-2";let b=f.length>=11?Rt(f,x):null,h=((E=g.stadium_card)==null?void 0:E.stadium_def)||null;const v=Object.values(b||{}).flat()[0];console.log("[deck-select] stadiumDef club_id:",h==null?void 0:h.club_id,typeof(h==null?void 0:h.club_id)),console.log("[deck-select] player club_id:",v==null?void 0:v.club_id,typeof(v==null?void 0:v.club_id)),console.log("[deck-select] match?",String(h==null?void 0:h.club_id)===String(v==null?void 0:v.club_id)),h&&b&&(b=zt(b,h));const k=f.length>=11;He(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div id="deck-top-bar" style="flex-shrink:0">
        ...
        <div style="display:flex;align-items:center;gap:8px;padding:5px 14px;background:linear-gradient(90deg,rgba(232,119,34,0.3),transparent);border-top:1px solid rgba(232,119,34,0.3)">
          ...
          <span style="font-size:11px;color:#FFD700;margin-left:auto">+10 aux joueurs ${((G=h.club)==null?void 0:G.encoded_name)||h.country_code||""}</span>
        </div>`:""}
      </div>
      ...
      <!-- Pagination -->
      ${o.length>1?`<div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">${o.map((B,T)=>`<div style="width:6px;height:6px;border-radius:50%;background:${T===c?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}</div>`:""}
      ...
      <div id="deck-bottom-bar" style="flex-shrink:0;padding:10px 14px 14px;display:flex;flex-direction:column;gap:8px;background:rgba(0,0,0,0.2)">
        ...
        stroke="${E}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const f=Math.max(52,Math.round(a*.18)),p=Math.round(f*657/507);for(const[b,h]of Object.entries(c)){const v=g(b);if(!v||!h)continue;const k=b.replace(/[0-9]/g,"");h.stadiumBonus&&console.log("[buildTeamSVG] stadiumBonus=true sur",h.name,b);const y=r.includes(h.cardId),C=i==="attack"&&(["MIL","ATT"].includes(k)||y)&&!h.used||i==="defense"&&["GK","DEF","MIL"].includes(k)&&!h.used,E=n.includes(h.cardId);let G=h.boost||0;h.stadiumBonus&&(i==="attack"&&(k==="ATT"||k==="MIL")||i==="defense"&&(k==="GK"||k==="DEF"||k==="MIL")?G+=10:i||(G+=10));const N=Math.round(v.x-f/2),ee=Math.round(v.y-p/2);if(h.used){m+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${N}" y="${ee}" width="${f}" height="${p}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${h.cardId}" data-role="${k}" style="cursor:pointer"/>`;continue}const ie=Ae({...h,_evolution_bonus:0},{width:f,showStad:!0,stadDef:null,role:k,extraNote:G,_cardOffset:30}),le=E?"outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;opacity:0.75;":"";m+=`<foreignObject x="${N-2}" y="${ee-30}" width="${f+8}" height="${p+60}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="${le}position:relative">
        ${ie}
      </div>
    </foreignObject>`,C&&(m+=`<rect x="${N}" y="${ee}" width="${f}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${E?"selected":""}" data-card-id="${h.cardId}" data-role="${k}" style="cursor:pointer"/>`)}const x=s!==null?s:Math.round(Math.max(f*.7,80));return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-x} ${-x} ${a+x*2} ${o+x*2}" width="100%" style="display:block;width:100%;margin:0 auto">
    ${m}
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const $=window.innerWidth>=700,F=(z,S,R)=>{var Ce,je;const H=(t.gcDefs||[]).find(Fe=>Fe.name===z.gc_type),K={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[H==null?void 0:H.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",j={purple:"#b06ce0",light_blue:"#00d4ef"}[H==null?void 0:H.color]||"#b06ce0",O=(H==null?void 0:H.name)||z.gc_type,$e=(H==null?void 0:H.effect)||((Ce=Se[z.gc_type])==null?void 0:Ce.desc)||"",_e=H!=null&&H.image_url?`/icons/${H.image_url}`:null,we=((je=Se[z.gc_type])==null?void 0:je.icon)||"⚡",me=Math.round(R*.22),Le=Math.round(R*.22),ke=R-me-Le,he=O.length>12?7:9;return`<div class="gc-mini" data-gc-id="${z.id}" data-gc-type="${z.gc_type}"
          style="box-sizing:border-box;width:${S}px;height:${R}px;border-radius:10px;border:2px solid ${j};background:${K};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
        ...
        </div>`},re=(z,S)=>{var R;return`<div id="boost-card"
          ...
          </div>`},pe=(z,S)=>S?re(130,175):F(z,130,175),de=(z,S)=>S?re(68,95):F(z,68,95),ae=$?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Ee=f?`<button id="btn-results" style="${ae};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:u?`<div style="${ae};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:b?`<button id="btn-pass" style="${ae};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER (plus d'attaquants)</button>`:g?`<button id="btn-action" style="${ae};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:m?`<button id="btn-action" style="${ae};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${ae};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,U=g||m?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",Q=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${$?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
        ${o.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':o.map(z=>`<div class="sub-btn-col" data-sub-id="${z.cardId}" style="cursor:pointer;flex-shrink:0">${Je(z,76,100)}</div>`).join("")}
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map($=>{var F,re,pe;if($.type==="duel"){const de=$.isGoal,ae=$.homeScored?"#FFD700":de?"#ff6b6b":"rgba(255,255,255,0.3)",Ee=$.homeScored?"⚽ BUT !":de?"⚽ BUT IA !":(F=$.homePlayers)!=null&&F.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${de?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${ae};margin-bottom:4px">
                <div style="font-size:9px;color:${ae};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${Ee}</div>
                ${(re=$.homePlayers)!=null&&re.length?`<div style="margin-bottom:3px">${ut($.homePlayers,"rgba(255,255,255,0.7)",$.homeTotal)}</div>`:""}
    ...
    </div>`,(y=u.querySelector("#sub-close"))==null||y.addEventListener("click",()=>u.remove()),(C=u.querySelector("#out-up"))==null||C.addEventListener("click",()=>{s>0&&(s--,g())}),(E=u.querySelector("#out-down"))==null||E.addEventListener("click",()=>{s<o.length-1&&(s++,g())}),(G=u.querySelector("#in-up"))==null||G.addEventListener("click",()=>{l>0&&(l--,g())}),(N=u.querySelector("#in-down"))==null||N.addEventListener("click",()=>{l<r.length-1&&(l++,g())});const k=(ie,le,B,T)=>{const $=u.querySelector("#"+ie);if(!$)return;let F=0;$.addEventListener("touchstart",re=>{F=re.touches[0].clientY},{passive:!0}),$.addEventListener("touchend",re=>{const pe=re.changedTouches[0].clientY-F;if(Math.abs(pe)<30)return;const de=le();pe<0&&de<T-1?(B(de+1),g()):pe>0&&de>0&&(B(de-1),g())},{passive:!0})};k("in-panel",()=>l,ie=>l=ie,r.length),k("out-panel",()=>s,ie=>s=ie,o.length),(ee=u.querySelector("#sub-confirm"))==null||ee.addEventListener("click",ie=>{if(ie.preventDefault(),ie.stopPropagation(),c)return;c=!0;const le=o[s],B=r[l];if(!le||!B)return;let T=null,$=-1;for(const[re,pe]of Object.entries(t.homeTeam)){const de=(pe||[]).findIndex(ae=>ae.cardId===le.cardId);if(de!==-1){T=re,$=de;break}}if($===-1||!T){wt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),u.remove();return}const F={...B,_line:T,_col:le._col||0,used:!1,boost:0};t.homeTeam[T].splice($,1,F),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(B.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:le.name,firstname:le.firstname,note:qe(le,T),portrait:Tt(le),job:le.job,country_code:le.country_code,rarity:le.rarity,clubName:le.clubName,clubLogo:le.clubLogo},inPlayer:{name:B.name,firstname:B.firstname,note:qe(B,T),portrait:Tt(B),job:B.job,country_code:B.country_code,rarity:B.rarity,clubName:B.clubName,clubLogo:B.clubLogo},text:`🔄 ${B.firstname} ${B.name} remplace ${le.firstname} ${le.name}`}),u.remove(),bo(le,B,()=>Te(e,t,i))})}document.body.appendChild(u),g()}function Ao(e,t,i,n,a){var f,p;const o=(n.gcDefs||[]).find(x=>x.name===t),r=Se[t]||{icon:"⚡",desc:"Carte spéciale."},s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[o==null?void 0:o.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",d={purple:"#b06ce0",light_blue:"#00d4ef"}[o==null?void 0:o.color]||"#b06ce0",l=(o==null?void 0:o.name)||t,c=(o==null?void 0:o.effect)||r.desc,u=o!=null&&o.image_url?`/icons/${o.image_url}`:null,g=r.icon||"⚡",m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",m.innerHTML=`
    ...
    </div>`,document.body.appendChild(m),(f=m.querySelector("#gc-back"))==null||f.addEventListener("click",()=>m.remove()),(p=m.querySelector("#gc-use"))==null||p.addEventListener("click",()=>{m.remove(),Mo(e,t,i,n,a)})}function qt(e,t,i,n,a,o){const r=document.createElement("div");r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let s=[];function d(){var l,c;r.innerHTML=`
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${It(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            </div>`}}catch(J){console.error("[Ranked] MMR update error:",J)}(X=document.getElementById("pvp-end-overlay"))==null||X.remove();const q=document.createElement("div");q.id="pvp-end-overlay",q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const P=I?"🤝":M?"🏆":"😞",Y=I?"MATCH NUL":M?"VICTOIRE !":"DÉFAITE",W=I?"#fff":M?"#FFD700":"#ff6b6b";q.innerHTML=`
      ...
      ${D}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${r?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(q),(oe=q.querySelector("#pvp-end-home"))==null||oe.addEventListener("click",()=>{q.remove(),Ue(e),m(r?"ranked":"home")})}const le=w.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},_=>{const L=_.new;try{if(L.status==="finished"||L.forfeit){if(C)return;C=!0,E&&(clearInterval(E),E=null),L.game_state&&(y=L.game_state),ie(L);return}if(L.game_state){const A=y;y=L.game_state;const M=y._lastGC;if(M&&M.seq&&!ee.has(M.seq)&&(ee.add(M.seq),M.by!==p)){V(M.type,M.by,()=>ae());return}const I=A[p+"Score"]||0,D=A[x+"Score"]||0,q=y[p+"Score"]||0,P=y[x+"Score"]||0,Y=q>I,W=P>D;if((Y||W)&&!N.has(y.round)){N.add(y.round);const X=[...y.log||[]].reverse().find(J=>J.isGoal),oe=((X==null?void 0:X.homePlayers)||[]).map(J=>({name:J.name,note:J.note,portrait:J.portrait,job:J.job}));st(oe,q,P,Y,()=>ae());return}ae()}}catch(A){console.error("[PvP] crash:",A)}}).subscribe();async function B(_){Object.assign(y,_),y.lastActionAt=new Date().toISOString();const{error:L}=await w.from("matches").update({game_state:y}).eq("id",i);L&&f("Erreur de synchronisation","error");try{ae()}catch(A){console.error("[PvP] renderPvpScreen crash:",A)}}async function T(){if(C)return;C=!0,E&&(clearInterval(E),E=null);const _=n?v.away_id:v.home_id,L=n?"p2":"p1",A=n?"p1":"p2",M={...y,[L+"Score"]:3,[A+"Score"]:0,phase:"finished"};try{await w.from("matches").update({status:"finished",forfeit:!0,winner_id:_,home_score:M.p1Score||0,away_score:M.p2Score||0,game_state:M}).eq("id",i)}catch{}try{w.removeChannel(le)}catch{}setTimeout(()=>{Ue(e),m("home")},800)}const $={BOOST_STAT:({value:_=1,count:L=1,roles:A=[]},M,I)=>{const D=M[p+"Team"],q=Object.entries(D).filter(([P])=>!A.length||A.includes(P)).flatMap(([P,Y])=>Y.filter(W=>!W.used).map(W=>({...W,_line:P})));if(!q.length){M.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),I(M);return}F(q,L,`Choisir ${L} joueur(s) à booster (+${_})`,P=>{P.forEach(Y=>{const W=(D[Y._line]||[]).find(X=>X.cardId===Y.cardId);W&&(W.boost=(W.boost||0)+_,M.log.push({text:`⚡ +${_} sur ${W.name}`,type:"info"}))}),M[p+"Team"]=D,I(M)})},DEBUFF_STAT:({value:_=1,count:L=1,roles:A=[],target:M="ai"},I,D)=>{const q=M==="home"?p:x,P=I[q+"Team"],Y=M==="home"?"allié":"adverse",W=Object.entries(P).filter(([X])=>!A.length||A.includes(X)).flatMap(([X,oe])=>oe.map(J=>({...J,_line:X})));if(!W.length){I.log.push({text:`🎯 Aucun joueur ${Y}`,type:"info"}),D(I);return}F(W,L,`Choisir ${L} joueur(s) ${Y}(s) (-${_})`,X=>{X.forEach(oe=>{const J=(P[oe._line]||[]).find(ce=>ce.cardId===oe.cardId);J&&(J.boost=(J.boost||0)-_,I.log.push({text:`🎯 -${_} sur ${J.name}`,type:"info"}))}),I[q+"Team"]=P,D(I)})},GRAY_PLAYER:({count:_=1,roles:L=[],target:A="ai"},M,I)=>{const D=A==="home"?p:x,q=M[D+"Team"],P=A==="home"?"allié":"adverse",Y=Object.entries(q).filter(([W])=>!L.length||L.includes(W)).flatMap(([W,X])=>X.filter(oe=>!oe.used).map(oe=>({...oe,_line:W})));if(!Y.length){M.log.push({text:`❌ Aucun joueur ${P}`,type:"info"}),I(M);return}F(Y,_,`Choisir ${_} joueur(s) ${P}(s) à exclure`,W=>{const X="usedCardIds_"+D,oe=new Set(M[X]||[]);W.forEach(J=>{const ce=(q[J._line]||[]).find(se=>se.cardId===J.cardId);ce&&(ce.used=!0,oe.add(J.cardId),M.log.push({text:`❌ ${ce.name} exclu !`,type:"info"}))}),M[X]=[...oe],M[D+"Team"]=q,I(M)})},REVIVE_PLAYER:({count:_=1,roles:L=[]},A,M)=>{const I=A[p+"Team"],D=Object.entries(I).filter(([q])=>!L.length||L.includes(q)).flatMap(([q,P])=>P.filter(Y=>Y.used).map(Y=>({...Y,_line:q})));if(!D.length){A.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),M(A);return}F(D,_,`Choisir ${_} joueur(s) à ressusciter`,q=>{q.forEach(P=>{const Y=(I[P._line]||[]).find(W=>W.cardId===P.cardId);Y&&(Y.used=!1,A.log.push({text:`💫 ${Y.name} ressuscité !`,type:"info"}))}),A[p+"Team"]=I,M(A)})},REMOVE_GOAL:({},_,L)=>{const A=x+"Score";_[A]>0?(_[A]--,_.log.push({text:"🚫 Dernier but annulé !",type:"info"})):_.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L(_)},ADD_GOAL_DRAW:({},_,L)=>{_[p+"Score"]===_[x+"Score"]?(_[p+"Score"]++,_.log.push({text:"🎯 But bonus !",type:"info"})):_.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L(_)},ADD_SUB:({value:_=1},L,A)=>{L.maxSubs=(L.maxSubs||3)+_,L.log.push({text:`🔄 +${_} remplacement(s)`,type:"info"}),A(L)},CUSTOM:({},_,L)=>L(_)};function F(_,L,A,M){const I=document.createElement("div");I.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let D=[];function q(){var Y,W;const P=_.map(X=>{const oe={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[X._line]||"#555",J=qe(X,X._line)+(X.boost||0),se=D.find(ue=>ue.cardId===X.cardId)?"#FFD700":"rgba(255,255,255,0.25)",ne=X.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${X.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${se};background:${oe};overflow:hidden;cursor:pointer;${ne}">
        ...
        </div>`,(Y=I.querySelector("#pp-close"))==null||Y.addEventListener("click",()=>I.remove()),I.querySelectorAll(".pp-item").forEach(X=>{X.addEventListener("click",()=>{const oe=X.dataset.cid,J=_.find(se=>se.cardId===oe),ce=D.findIndex(se=>se.cardId===oe);J&&(ce>-1?D.splice(ce,1):D.length<L&&D.push(J),q())})}),(W=I.querySelector("#pp-confirm"))==null||W.addEventListener("click",()=>{I.remove(),M(D)})}q(),document.body.appendChild(I)}async function re(_,L){const M=(y["gcCardsFull_"+p]||[]).find(P=>P.id===_),I=(M==null?void 0:M._gcDef)||(y.gcDefs||[]).find(P=>{var Y;return P.name===L||((Y=P.name)==null?void 0:Y.toLowerCase().trim())===(L==null?void 0:L.toLowerCase().trim())}),D=[...y["usedGc_"+p]||[],_],q={type:L,by:p,seq:(y._gcAnimSeq||0)+1};ee.add(q.seq),V(L,p,async()=>{if(I!=null&&I.effect_type&&I.effect_type!=="CUSTOM"){const Y=$[I.effect_type];if(Y){const W={...y};Y(I.effect_params||{},W,async X=>{X["usedGc_"+p]=D,X._lastGC=q,X._gcAnimSeq=q.seq,await B(X)});return}}const P={...y};switch(L){case"Remplacement+":P.maxSubs=(P.maxSubs||3)+1,P.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const Y=x+"Score";P[Y]>0&&(P[Y]--,P.log.push({text:"🚫 But annulé",type:"info"}));break}}P["usedGc_"+p]=D,P._lastGC=q,P._gcAnimSeq=q.seq,await B(P)})}function pe(_,L){const A="usedCardIds_"+_,M=new Set(y[A]||[]);L.forEach(I=>M.add(I)),y[A]=[...M]}function de(){for(const _ of["p1","p2"]){const L=new Set(y["usedCardIds_"+_]||[]),A=y[_+"Team"];if(A)for(const M of["GK","DEF","MIL","ATT"])(A[M]||[]).forEach(I=>{I.used=L.has(I.cardId)})}}function ae(){var Ve,nt,yi,bi,hi,vi;if(y.phase==="reveal")return Ee();if(y.phase==="midfield")return Q();if(y.phase==="finished")return Fe();const _=y[p+"Team"],L=y[x+"Team"];de();const A=y[p+"Score"],M=y[x+"Score"],I=y[p+"Name"],D=y[x+"Name"],q=y.phase===p+"-attack",P=y.phase===p+"-defense",Y=Array.isArray(y["selected_"+p])?y["selected_"+p]:[],W=Y.map(te=>te.cardId),X=window.innerWidth>=700,oe=y[p+"Subs"]||[],J=y["usedSubIds_"+p]||[],ce=oe.filter(te=>!J.includes(te.cardId)),se=y["gcCardsFull_"+p]||[],ne=y["usedGc_"+p]||[],ue=se.filter(te=>!ne.includes(te.id)),fe=y.boostOwner===p&&!y.boostUsed,xe=!["GK","DEF","MIL","ATT"].some(te=>(L[te]||[]).some(ge=>!ge.used)),Z=[..._.MIL||[],..._.ATT||[]].filter(te=>!te.used),be=q&&xe&&Z.length===0?[..._.GK||[],..._.DEF||[]].filter(te=>!te.used).map(te=>te.cardId):[];function ze(te,ge,Ie){var $i,Ei;const ye=te._gcDef,ot={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ye==null?void 0:ye.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",lt={purple:"#b06ce0",light_blue:"#00d4ef"}[ye==null?void 0:ye.color]||"#b06ce0",ht=(ye==null?void 0:ye.name)||te.gc_type,vt=(ye==null?void 0:ye.effect)||(($i=Se[te.gc_type])==null?void 0:$i.desc)||"",wi=ye!=null&&ye.image_url?`/icons/${ye.image_url}`:null,un=((Ei=Se[te.gc_type])==null?void 0:Ei.icon)||"⚡",_i=Math.round(Ie*.22),ki=Math.round(Ie*.22),Vt=Ie-_i-ki,fn=ht.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${te.id}" data-gc-type="${te.gc_type}"
      ...
      </div>`;function De(){const te=e.querySelector(".match-screen");if(!te)return;const ge=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);te.style.height=ge+"px",te.style.minHeight=ge+"px",te.style.maxHeight=ge+"px",te.style.overflow="hidden";const Ie=e.querySelector("#mobile-action-bar"),ye=e.querySelector("#mobile-play-area");Ie&&ye&&(ye.style.paddingBottom=Ie.offsetHeight+"px")}if(De(),setTimeout(De,120),setTimeout(De,400),G||(G=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",De),window.visualViewport.addEventListener("scroll",De)),window.addEventListener("resize",De)),function(){const ge=e.querySelector(".terrain-wrapper svg");ge&&(ge.removeAttribute("width"),ge.removeAttribute("height"),ge.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ge.setAttribute("viewBox","-26 -26 352 352"),ge.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(te=>{te.addEventListener("click",()=>{if(!q&&!P)return;const ge=te.dataset.cardId,Ie=te.dataset.role,ye=(_[Ie]||[]).find(vt=>vt.cardId===ge);if(!ye||ye.used)return;const ot=be.includes(ge);if(q&&!["MIL","ATT"].includes(Ie)&&!ot)return;Array.isArray(y["selected_"+p])||(y["selected_"+p]=[]);const lt=y["selected_"+p],ht=lt.findIndex(vt=>vt.cardId===ge);ht>-1?lt.splice(ht,1):lt.length<3&&lt.push({...ye,_role:Ie}),ae()})}),e.querySelectorAll(".match-used-hit").forEach(te=>{te.addEventListener("click",()=>R(te.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(te=>{te.addEventListener("click",()=>R())}),(Ve=e.querySelector("#pvp-sub-open"))==null||Ve.addEventListener("click",()=>R()),e.querySelectorAll(".pvp-gc-mini").forEach(te=>{te.addEventListener("click",()=>z(te.dataset.gcId,te.dataset.gcType))}),(nt=e.querySelector("#pvp-boost-card"))==null||nt.addEventListener("click",()=>S()),(yi=e.querySelector("#pvp-action"))==null||yi.addEventListener("click",te=>{q?te.currentTarget.dataset.pass==="1"||!ke(p)?O():$e():P&&_e()}),(bi=e.querySelector("#pvp-quit"))==null||bi.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&T()}),(hi=e.querySelector("#pvp-view-opp"))==null||hi.addEventListener("click",()=>K()),(vi=e.querySelector("#pvp-toggle-history"))==null||vi.addEventListener("click",()=>j()),E&&(clearInterval(E),E=null),(q||P)&&!C){let te=30,ge=!1;const Ie=()=>document.getElementById("pvp-timer"),ye=()=>{Ie()&&(Ie().textContent=te+"s",Ie().style.color=ge?"#ff4444":"#fff")};ye(),E=setInterval(()=>{te--,te<0?ge?(clearInterval(E),E=null,q&&!ke(p)?O():T()):(ge=!0,te=15,ye()):ye()},1e3)}}function Ee(){He(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
    ...
    </div>`,p==="p1"&&setTimeout(async()=>{await B({phase:"midfield"})},5e3)}let U=!1;function Q(){if(U)return;const _=y[p+"Team"].MIL||[],L=y[x+"Team"].MIL||[],A=milScore(_)+milLinks(_),M=milScore(L)+milLinks(L),I=A>=M;function D(se,ne,ue){return`<div id="duel-row-${ue}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    </div>`;const q=(se,ne)=>e.querySelectorAll(se).forEach((ue,fe)=>{setTimeout(()=>{ue.style.opacity="1",ue.style.transform="translateY(0) scale(1)"},ne+fe*90)});q(".duel-card-me",150),q(".duel-card-opp",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((se,ne)=>setTimeout(()=>{se.style.opacity="1"},ne*70)),900),setTimeout(()=>{const se=e.querySelector("#pvp-vs");se&&(se.style.opacity="1",se.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(ne=>ne.style.opacity="1")},1250);function P(se,ne,ue){const fe=document.getElementById(se);if(!fe)return;const xe=performance.now(),Z=be=>{const ze=Math.min(1,(be-xe)/ue);fe.textContent=Math.round(ne*(1-Math.pow(1-ze,3))),ze<1?requestAnimationFrame(Z):fe.textContent=ne};requestAnimationFrame(Z)}setTimeout(()=>{P("pvp-score-me",A,800),P("pvp-score-opp",M,800)},1500);const Y=y.p1Team.MIL||[],W=y.p2Team.MIL||[],X=milScore(Y)+milLinks(Y),oe=milScore(W)+milLinks(W),J=X>=oe?"p1":"p2";let ce=y.boostValue;ce==null&&(ce=tn(),y.boostValue=ce,y.boostOwner=J,y.boostUsed=!1),U=!0,setTimeout(()=>{const se=e.querySelector("#duel-row-"+(I?"me":"opp")),ne=e.querySelector("#duel-row-"+(I?"opp":"me")),ue=document.getElementById("pvp-score-me"),fe=document.getElementById("pvp-score-opp"),xe=I?ue:fe,Z=I?fe:ue;xe&&(xe.style.fontSize="80px",xe.style.color=I?"#FFD700":"#ff6b6b",xe.style.animation="duelPulse .5s ease"+(I?",duelGlow 1.5s ease infinite .5s":"")),Z&&(Z.style.opacity="0.25"),setTimeout(()=>{se&&(se.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",se.style.zIndex="5"),setTimeout(()=>{const be=document.getElementById("duel-shock");be&&(be.style.animation="shockwave .5s ease-out forwards"),ne&&(ne.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var Ne;const be=document.getElementById("pvp-duel-finale");if(!be)return;const ze=y.boostOwner===p?'<div style="background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:3px solid #cdeffd;border-radius:18px;padding:20px 34px;text-align:center;animation:boostFlipIn .7s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 10px 36px rgba(135,206,235,.5)"><div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;font-weight:700">Carte Boost obtenue</div><div style="font-size:46px;line-height:1">⚡</div><div style="font-size:50px;font-weight:900;color:#063;line-height:1.1">+'+ce+`</div><div style="font-size:10px;color:rgba(0,0,0,.55);margin-top:4px">Applicable sur n'importe quel joueur</div></div>`:"",Me=p==="p1"?'<button id="pvp-start-match" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">▶ Commencer le match</button>':`<div style="font-size:14px;color:rgba(255,255,255,0.5);text-align:center;margin-top:8px;animation:fadeUp .4s ease both">⏳ En attente de l'adversaire...</div>`;be.innerHTML='<div style="font-size:22px;font-weight:900;color:#fff;text-align:center;animation:fadeUp .4s ease both;text-shadow:0 2px 12px rgba(0,0,0,.5)">'+(I?"⚽ "+y[p+"Name"]+"<br>gagne le milieu et attaque !":"😔 "+y[x+"Name"]+"<br>gagne l'engagement et attaque !")+"</div>"+ze+Me,be.style.transition="opacity .45s ease",be.style.opacity="1",be.style.pointerEvents="auto",(Ne=document.getElementById("pvp-start-match"))==null||Ne.addEventListener("click",async()=>{const Ge=J;await B({phase:Ge+"-attack",attacker:Ge,round:1,boostValue:ce,boostUsed:!1,boostOwner:Ge})})},600)},700)},2800)}function V(_,L,A){var ne,ue;const M=(y.gcDefs||[]).find(fe=>{var xe;return fe.name===_||((xe=fe.name)==null?void 0:xe.toLowerCase().trim())===(_==null?void 0:_.toLowerCase().trim())}),I={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[M==null?void 0:M.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",D={purple:"#b06ce0",light_blue:"#00d4ef"}[M==null?void 0:M.color]||"#b06ce0",q=(M==null?void 0:M.name)||_,P=(M==null?void 0:M.effect)||((ne=Se[_])==null?void 0:ne.desc)||"",Y=M!=null&&M.image_url?`/icons/${M.image_url}`:null,W=((ue=Se[_])==null?void 0:ue.icon)||"⚡",oe=L===p?"Vous":y[L+"Name"]||"Adversaire",J=document.createElement("div");J.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",J.innerHTML=`
      <style>
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(J);let ce=!1;const se=()=>{ce||(ce=!0,J.remove(),setTimeout(()=>A&&A(),50))};J.addEventListener("click",se),setTimeout(se,3e3)}function z(_,L){var J,ce,se,ne;const M=(y["gcCardsFull_"+p]||[]).find(ue=>ue.id===_),I=M==null?void 0:M._gcDef,D={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[I==null?void 0:I.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",q={purple:"#b06ce0",light_blue:"#00d4ef"}[I==null?void 0:I.color]||"#b06ce0",P=(I==null?void 0:I.name)||L,Y=(I==null?void 0:I.effect)||((J=Se[L])==null?void 0:J.desc)||"Carte spéciale.",W=I!=null&&I.image_url?`/icons/${I.image_url}`:null,X=((ce=Se[L])==null?void 0:ce.icon)||"⚡",oe=document.createElement("div");oe.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",oe.innerHTML=`
      ...
      </div>`,document.body.appendChild(A),(M=A.querySelector("#bp-close"))==null||M.addEventListener("click",()=>A.remove()),A.querySelectorAll(".bp-item").forEach(I=>{I.addEventListener("click",async()=>{const D=I.dataset.cid,q=L.find(Y=>Y.cardId===D);if(!q)return;const P=(_[q._line]||[]).find(Y=>Y.cardId===D);P&&(P.boost=(P.boost||0)+y.boostValue),A.remove(),await B({[p+"Team"]:_,boostUsed:!0})})})}function R(_=null){var ce,se;if(!(y.phase===p+"-attack")){f("Remplacement uniquement avant votre attaque","warning");return}const A=y[p+"Team"],M=y["usedSubIds_"+p]||[],I=y.maxSubs||3;if(M.length>=I){f(`Maximum ${I} remplacements atteint`,"warning");return}const D=Object.entries(A).flatMap(([ne,ue])=>(ue||[]).filter(fe=>fe.used).map(fe=>({...fe,_line:ne}))),q=(y[p+"Subs"]||[]).filter(ne=>!M.includes(ne.cardId));if(!D.length){f("Aucun joueur utilisé à remplacer","warning");return}if(!q.length){f("Aucun remplaçant disponible","warning");return}let P=Math.max(0,D.findIndex(ne=>ne.cardId===_));const Y=((ce=D[P])==null?void 0:ce._line)||((se=D[P])==null?void 0:se.job);let W=Math.max(0,q.findIndex(ne=>ne.job===Y)),X=!1;const oe=document.createElement("div");oe.id="pvp-sub-overlay",oe.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function J(){var ze,Me,Ne,Ge,Pe,bt;const ne=D[P],ue=q[W],fe=Math.min(130,Math.round((window.innerWidth-90)/2)),xe=Math.round(fe*1.35),Z=Be=>`background:rgba(255,255,255,0.12);border:none;color:${Be?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Be?"default":"pointer"};flex-shrink:0`;oe.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      </div>`,(ze=oe.querySelector("#psub-close"))==null||ze.addEventListener("click",()=>oe.remove()),(Me=oe.querySelector("#pout-up"))==null||Me.addEventListener("click",()=>{P>0&&(P--,J())}),(Ne=oe.querySelector("#pout-down"))==null||Ne.addEventListener("click",()=>{P<D.length-1&&(P++,J())}),(Ge=oe.querySelector("#pin-up"))==null||Ge.addEventListener("click",()=>{W>0&&(W--,J())}),(Pe=oe.querySelector("#pin-down"))==null||Pe.addEventListener("click",()=>{W<q.length-1&&(W++,J())});const be=(Be,Oe,Re,tt)=>{const Ke=oe.querySelector("#"+Be);if(!Ke)return;let it=0;Ke.addEventListener("touchstart",De=>{it=De.touches[0].clientY},{passive:!0}),Ke.addEventListener("touchend",De=>{const Ve=De.changedTouches[0].clientY-it;if(Math.abs(Ve)<30)return;const nt=Oe();Ve<0&&nt<tt-1?(Re(nt+1),J()):Ve>0&&nt>0&&(Re(nt-1),J())},{passive:!0})};be("pin-panel",()=>W,Be=>W=Be,q.length),be("pout-panel",()=>P,Be=>P=Be,D.length),(bt=oe.querySelector("#psub-confirm"))==null||bt.addEventListener("click",async Be=>{if(Be.preventDefault(),Be.stopPropagation(),X)return;X=!0;const Oe=D[P],Re=q[W];if(!Oe||!Re)return;const tt=Oe._line,Ke=(A[tt]||[]).findIndex(Ve=>Ve.cardId===Oe.cardId);if(Ke===-1){f("Erreur : joueur introuvable","error"),oe.remove();return}const it={...Re,_line:tt,position:Oe.position,used:!1,boost:0};A[tt].splice(Ke,1,it);const De=[...M,Re.cardId];oe.remove(),H(Oe,Re,async()=>{await B({[p+"Team"]:A,[x+"Team"]:y[x+"Team"],["usedSubIds_"+p]:De})})})}document.body.appendChild(oe),J()}function H(_,L,A){const M={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},I=document.createElement("div");I.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const D=(Y,W,X)=>`<div style="text-align:center">
      ...
      </div>`,document.body.appendChild(L),(A=L.querySelector("#pvp-hist-close"))==null||A.addEventListener("click",()=>L.remove())}async function O(){if(y.phase!==p+"-attack")return;const _=p==="p1"?"p2":"p1",L=(y.round||0)+1,A=[...y.log||[]];A.push({type:"info",text:`⏭️ ${y[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const M=he(y),I=ke(_),D=M||!I?"finished":_+"-attack";await B({["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:D,attacker:_,round:L,log:A}),D==="finished"&&await je(y)}async function $e(){const _=y[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(D=>(y[x+"Team"][D]||[]).some(q=>!q.used)),A=(y["selected_"+p]||[]).map(D=>{const q=(_[D._role]||[]).find(se=>se.cardId===D.cardId)||D,P=L&&["GK","DEF"].includes(D._role),Y=_[D._role]||[],W=Y.findIndex(se=>se.cardId===D.cardId),X=xt(Y.length),oe=W>=0?X[W]:q._col??1,J=y.stadiumDef||y.homeStadiumDef||null,ce=q.stadiumBonus||J&&(J.club_id&&String(q.club_id)===String(J.club_id)||J.country_code&&q.country_code===J.country_code)||!1;return{...q,_line:D._role,_col:oe,stadiumBonus:ce,...P?{note_a:Math.max(1,Number(q.note_a)||0)}:{}}});if(!A.length)return;const M=pi(A,y.modifiers[p]||{});pe(p,A.map(D=>D.cardId)),A.forEach(D=>{const q=(_[D._role]||[]).find(P=>P.cardId===D.cardId);q&&(q.used=!0)}),y["selected_"+p]=[],ae();const I=[...y.log||[]];if(L){const D=(y[p+"Score"]||0)+1,q=A.map(oe=>histPlayer(oe));I.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:q,homeTotal:M.total,aiTotal:0});const P=(y.round||0)+1,Y=p==="p1"?"p2":"p1",W={...y,[p+"Team"]:_,[p+"Score"]:D},X=he(W);N.add(P),st(q,D,y[x+"Score"]||0,!0,async()=>{await B({[p+"Team"]:_,[p+"Score"]:D,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:X?"finished":Y+"-attack",attacker:Y,round:P,log:I}),X&&await je({...y,[p+"Score"]:D})});return}I.push({type:"pending",text:`⚔️ ${y[p+"Name"]} attaque (${M.total})`}),await B({[p+"Team"]:_,[x+"Team"]:y[x+"Team"],pendingAttack:{...M,players:A,side:p},["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},phase:x+"-defense",log:I})}async function _e(){const _=y[p+"Team"],L=(y["selected_"+p]||[]).map(ne=>{const ue=(_[ne._role]||[]).find(ze=>ze.cardId===ne.cardId)||ne,fe=_[ne._role]||[],xe=fe.findIndex(ze=>ze.cardId===ne.cardId),Z=xt(fe.length),be=xe>=0?Z[xe]:ue._col??1;return{...ue,_line:ne._role,_col:be}}),A=ui(L,y.modifiers[p]||{});pe(p,L.map(ne=>ne.cardId)),L.forEach(ne=>{const ue=(_[ne._role]||[]).find(fe=>fe.cardId===ne.cardId);ue&&(ue.used=!0)}),y["selected_"+p]=[],ae();const M=fi(y.pendingAttack.total,A.total,y.modifiers[p]||{}),I=y.pendingAttack.side,D=M==="attack"||(M==null?void 0:M.goal),q=I==="p1"?"p2":"p1",P=(y.round||0)+1,Y=(y.pendingAttack.players||[]).map(ne=>histPlayer(ne)),W=[...y.log||[]];W.push({type:"duel",isGoal:D,homeScored:D&&I===p,text:D?`⚽ BUT de ${y[I+"Name"]} ! (${y.pendingAttack.total} vs ${A.total})`:`✋ Attaque stoppée (${y.pendingAttack.total} vs ${A.total})`,homePlayers:Y,aiPlayers:L.map(ne=>histPlayer(ne)),homeTotal:y.pendingAttack.total,aiTotal:A.total});const X=D?(y[I+"Score"]||0)+1:y[I+"Score"]||0,oe={...y,[p+"Team"]:_,[I+"Score"]:X},J=he(oe),ce=J?"finished":q+"-attack",se=async()=>{await B({[p+"Team"]:_,[x+"Team"]:y[x+"Team"],[I+"Score"]:X,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:ce,attacker:q,round:P,log:W}),(ce==="finished"||J)&&await je({...y,[I+"Score"]:X})};if(D){const ne=I===p,ue=ne?X:y[p+"Score"]||0,fe=ne?y[x+"Score"]||0:X;N.add(P),st(Y,ue,fe,ne,se)}else await se()}function we(_){return["MIL","ATT"].some(L=>(_[L]||[]).some(A=>!A.used))}function me(_){return["GK","DEF","MIL","ATT"].some(L=>(_[L]||[]).some(A=>!A.used))}function Le(_){return me(_)&&!we(_)}function ke(_){const L=y[_+"Team"],A=y[(_==="p1"?"p2":"p1")+"Team"];return!!(we(L)||!me(A)&&Le(L))}function he(_){const L=["MIL","ATT"].some(W=>(_.p1Team[W]||[]).some(X=>!X.used)),A=["MIL","ATT"].some(W=>(_.p2Team[W]||[]).some(X=>!X.used)),M=me(_.p1Team),I=me(_.p2Team);return!L&&!(!I&&M)&&(!A&&!(!M&&I))}function Ce(_){const L=_.p1Score||0,A=_.p2Score||0;return L===A?null:L>A?v.home_id:v.away_id}async function je(_){try{const L=Ce(_),A=L?v.home_id===L?v.away_id:v.home_id:null;await w.from("matches").update({status:"finished",winner_id:L,home_score:_.p1Score||0,away_score:_.p2Score||0}).eq("id",i),L&&A&&updateEvolutiveCards(L,A).catch(()=>{})}catch(L){console.error("[PvP] finishMatch:",L)}}function Fe(){var I;const _=y[p+"Score"],L=y[x+"Score"],A=_>L,M=_===L;He(e),e.innerHTML=`
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${g.map((B,T)=>{const $=f>0&&r.status==="finished"?T===0?Math.floor(f*.7):T===1?Math.floor(f*.2):T===2?Math.floor(f*.1):0:0;return`<tr style="border-bottom:1px solid #f5f5f5;${B.userId===o?"background:#f0fdf4;":""}">
              <td style="padding:7px 3px 7px 0;font-weight:700;color:${T===0?Xe:T<3?gt:"#555"}">${["🥇","🥈","🥉"][T]||T+1}</td>
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,r.status==="active"?r.current_day-1:r.current_day)},(B,T)=>T+1).reverse().map(B=>{const T=(d||[]).filter($=>$.matchday===B);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${B}</div>${T.map($=>Di($,u,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`}function g(p){var G,N,ee,ie;const x=(G=p.card)==null?void 0:G.player;if(!x)return"";const b=((N=p.card)==null?void 0:N.evolution_bonus)||0,h=ct(x,x.job,b),v=x.job2?ct(x,x.job2,b):0,k=p.status==="sold",y=x.country_code?`https://flagsapi.com/${x.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,C=Dt[x.job]||"#bbb",E=x.job2?Dt[x.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${k?"opacity:0.7":""}">
      ${y?`<img src="${y}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(ee=x.clubs)!=null&&ee.logo_url?`<img src="${x.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${C};display:flex;align-items:center;justify-content:center">
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const b=r||[];f.innerHTML=b.length>0?b.map((h,v)=>{const k=di(h.mmr??1e3),y=(h.ranked_wins||0)+(h.ranked_losses||0)+(h.ranked_draws||0),C=y>0?Math.round((h.ranked_wins||0)/y*100):0,E=h.id===i.profile.id,G=(h.placement_matches||0)<10;return`
