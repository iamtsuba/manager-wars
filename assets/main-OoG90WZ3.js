...
import{s as v,j as vi,q as In,t as Ut,u as Oi,F as Hi,o as tt,n as ti,g as At,r as Te,m as Mt,v as Mi,w as fo,T as zi,p as mo,x as go,d as xo}from"./special-cards-q1oKjNQl.js";const bo="/";function pn(e,{navigate:t,toast:i}){let n="login";const o=()=>{var a,s,d,c,l,f,b;const r=n==="login";if(e.innerHTML=`
    <div style="
  ...
  `;function s(){var g;const c=document.getElementById("logo-preview"),l=document.getElementById("logo-initials"),f=((g=document.getElementById("setup-club"))==null?void 0:g.value)||"MW",b=f.trim().split(" ").filter(Boolean),x=b.length>=2?(b[0][0]+b[1][0]).toUpperCase():f.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=r),l&&(l.textContent=x,l.style.color=r)}document.getElementById("color1").addEventListener("input",c=>{r=c.target.value,document.getElementById("swatch1").style.background=r,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function d(c){document.querySelectorAll(".setup-step").forEach(l=>l.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("step1-error");if(l.textContent="",c.length<3){l.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await v.from("users").select("id").eq("pseudo",c).maybeSingle();if(f){l.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),l=document.getElementById("step2-error");if(l.textContent="",c.length<2){l.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await v.from("users").select("id").eq("club_name",c).maybeSingle();if(f){l.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),b=document.getElementById("step3-finish");f.textContent="",b.disabled=!0,b.textContent="Création en cours…";try{const{error:x}=await v.from("users").insert({id:t.user.id,pseudo:c,club_name:l,club_color1:r,club_color2:a,credits:1e4});if(x)throw x;await ho(t.user.id),await o(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(x){f.textContent=x.message,b.disabled=!1,b.textContent="🚀 Créer mon profil !"}})}async function ho(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await v.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const vo="modulepreload",wo=function(e){return"/"+e},un={},Ui=function(t,i,n){let o=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));o=Promise.allSettled(i.map(d=>{if(d=wo(d),d in un)return;un[d]=!0;const c=d.endsWith(".css"),l=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${l}`))return;const f=document.createElement("link");if(f.rel=c?"stylesheet":vo,c||(f.as="script"),f.crossOrigin="",f.href=d,s&&f.setAttribute("nonce",s),document.head.appendChild(f),c)return new Promise((b,x)=>{f.addEventListener("load",b),f.addEventListener("error",()=>x(new Error(`Unable to preload CSS for ${d}`)))})}))}function r(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return o.then(a=>{for(const s of a||[])s.status==="rejected"&&r(s.reason);return t().catch(r)})},ii="#1A6B3C",ni="#cc2222",_o="#D4A017",fn="#888";async function ko(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </div>`,await Sn(i,n,t),document.getElementById("btn-add-friend").addEventListener("click",()=>Eo(i,n)),document.getElementById("btn-accept-friend").addEventListener("click",()=>Ki(i,n,null,t))}async function Sn(e,t,i={}){const{navigate:n}=i,o=e.user.id,{data:r,error:a}=await v.from("friendships").select("id, requester_id, addressee_id").eq("status","accepted").or(`requester_id.eq.${o},addressee_id.eq.${o}`),{count:s}=await v.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",o).eq("status","pending"),d=document.getElementById("pending-badge");d&&(s>0?(d.style.display="flex",d.textContent=s):d.style.display="none");const c=document.getElementById("friends-list");if(!c)return;if(a){console.error("[Friends] Erreur:",a),c.innerHTML=`<div style="color:${ni};text-align:center;padding:16px">Erreur chargement : ${a.message}</div>`;return}const l=(r||[]).map(x=>x.requester_id===o?x.addressee_id:x.requester_id);let f={};if(l.length){const{data:x}=await v.from("users").select("id, pseudo, club_name, last_seen_at, club_color1, club_color2").in("id",l);(x||[]).forEach(g=>{f[g.id]=g})}const b=(r||[]).map(x=>({friendshipId:x.id,friend:f[x.requester_id===o?x.addressee_id:x.requester_id]||{pseudo:"?"}}));if(!b.length){c.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${b.map(({friendshipId:x,friend:g})=>$o(g,x)).join("")}
    </div>`,c.querySelectorAll("[data-stats]").forEach(x=>{x.addEventListener("click",()=>Lo(e,x.dataset.stats,x.dataset.friendName))}),c.querySelectorAll("[data-match]").forEach(x=>{x.addEventListener("click",async()=>{const g=x.dataset.friendId,p=x.dataset.friendName;if(typeof n!="function"){t("Erreur navigation","error");return}const u=e.user.id,{data:y}=await v.from("friend_match_invites").select("id").eq("inviter_id",g).eq("invitee_id",u).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle(),w=!!y;console.log("[Friends] clic match",{fid:g,fname:p,isAccepting:w}),n("match",{matchMode:"friend",friendId:g,friendName:p,isAccepting:w})})})}function $o(e,t){const i=e.club_name||e.pseudo||"?",n=e.pseudo||"",o=(n||i).slice(0,2).toUpperCase(),r=e.club_color2||ii,a=e.club_color1||"#ffffff",s=e.last_seen_at?new Date(e.last_seen_at):null,d=s&&Date.now()-s.getTime()<3*60*1e3;return`
    <div style="display:flex;align-items:center;gap:12px;background:var(--tile-bg);border-radius:12px;padding:12px 14px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
      ...
      <div style="display:flex;gap:8px;flex-shrink:0">
        ...
        <button data-stats="${e.id}" data-friend-name="${i}" title="Voir les stats"
          style="width:38px;height:38px;border-radius:50%;border:2px solid ${_o};background:var(--tile-bg);font-size:16px;cursor:pointer;display:flex;align-items:center;justify-content:center">📊</button>
      </div>
    </div>`}function Eo(e,t){const i=Vi();i.innerHTML=`
    <div class="popup-box">
    ...
    ${Yi()}`,document.body.appendChild(c);const f=()=>c.remove();c.querySelector("#pending-close").addEventListener("click",f),c.addEventListener("click",b=>{b.target===c&&f()}),c.querySelectorAll("[data-accept]").forEach(b=>{b.addEventListener("click",async()=>{const{error:x}=await v.from("friendships").update({status:"accepted"}).eq("id",b.dataset.accept);if(x){t("Erreur : "+x.message,"error");return}b.closest("div[style]").remove(),t("✅ Ami accepté !","success"),Sn(e,t,n),i&&i()})}),c.querySelectorAll("[data-decline]").forEach(b=>{b.addEventListener("click",async()=>{await v.from("friendships").delete().eq("id",b.dataset.decline),b.closest("div[style]").remove(),t("Demande refusée","info"),i&&i()})})}async function Lo(e,t,i){const n=e.user.id,[o,r]=[n,t].sort(),a=n===o,{data:s}=await v.from("friend_match_stats").select("*").eq("player1_id",o).eq("player2_id",r).single(),d=e.profile.club_name||e.profile.pseudo||"Moi",c=s||{},l=a?c.wins_p1||0:c.wins_p2||0,f=a?c.wins_p2||0:c.wins_p1||0,b=c.draws||0,x=a?c.goals_p1||0:c.goals_p2||0,g=a?c.goals_p2||0:c.goals_p1||0,p=a?c.gc_used_p1||0:c.gc_used_p2||0,u=a?c.gc_used_p2||0:c.gc_used_p1||0,y=c.matches_total||0,w=($,m,M,k=ii,I=ni)=>`
    ...
    </style>`}const An="mw_sound_volume";function Wi(){const e=localStorage.getItem(An);if(e===null)return 100;const t=parseInt(e,10);return Number.isFinite(t)?Math.max(0,Math.min(100,t)):100}function zo(e){localStorage.setItem(An,String(Math.max(0,Math.min(100,Math.round(e)))))}function Xi(){return Wi()===0}function fi(e){return Math.max(0,Math.min(1,e*(Wi()/100)))}function Ji(e,t=1){if(Xi())return null;try{const i=new Audio(e);return i.volume=fi(t),i.play().catch(()=>{}),i}catch{return null}}let gt=null,Ci=null,mn=.3;function Mn(e,t=.3){if(mn=t,gt&&Ci===e&&!gt.paused){gt.volume=fi(mn);return}if(zt(),!Xi())try{const i=new Audio(e);i.loop=!0,i.volume=fi(t),i.play().catch(()=>{}),gt=i,Ci=e}catch{}}function zt(){if(gt)try{gt.pause(),gt.currentTime=0}catch{}gt=null,Ci=null}let Xt=null;function Cn(e,t=.6){if(Ue(),!Xi())try{const i=new Audio(e);i.volume=fi(t),i.play().catch(()=>{}),Xt=i}catch{}}function Ue(){if(Xt)try{Xt.pause(),Xt.currentTime=0}catch{}Xt=null}const To="2026.07.23-1550";async function gn(e,{state:t,navigate:i,toast:n}){var r,a,s;const o=t.profile;o&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="home-dark" id="home-dark">
  ...
  </div>`,requestAnimationFrame(()=>{var b,x,g,p,u;const d=((b=window.visualViewport)==null?void 0:b.height)||window.innerHeight,c=((x=document.querySelector(".top-nav"))==null?void 0:x.offsetHeight)||56,l=((g=document.querySelector(".bottom-nav"))==null?void 0:g.offsetHeight)||60;if(e.querySelector(".home-inner")){const y=d-c-l;e.querySelector(".home-dark").style.minHeight=y+"px"}if(window.innerWidth<768){const y=((p=e.querySelector(".home-hero"))==null?void 0:p.offsetHeight)||60,w=e.querySelector(".ranked-tile");e.querySelector(".play-grid");const h=((u=e.querySelector(".home-footer"))==null?void 0:u.offsetHeight)||44,$=["friend-requests-banner","match-invite-banner","ongoing-match-banner"].reduce((X,te)=>{var Z;return X+(((Z=document.getElementById(te))==null?void 0:Z.offsetHeight)||0)},0),m=12*5,M=d-c-l-y-h-$-m-32,k=Math.max(80,Math.round(M*.28)),I=Math.max(160,Math.round(M*.72)),B=Math.floor((I-10)/2);w&&(w.style.minHeight=w.style.maxHeight=k+"px"),e.querySelectorAll(".play-tile").forEach(X=>{X.style.minHeight=X.style.height=B+"px"}),e.querySelectorAll(".play-tile .play-icon").forEach(X=>{X.style.height=Math.round(B*.55)+"px"})}}),(r=document.getElementById("nav-rankings"))==null||r.addEventListener("click",()=>i("rankings")),(a=document.getElementById("nav-matches"))==null||a.addEventListener("click",()=>i("matches")),e.querySelectorAll("[data-action]").forEach(d=>{d.addEventListener("click",()=>{d.style.transform="scale(.96)",setTimeout(()=>d.style.transform="",180);const c=d.dataset.action;if(c==="match-ai"){Co(i);return}if(c==="match-random"){i("match",{matchMode:"random"});return}if(c==="match-friend"){i("friends");return}if(c==="mini-league"){i("mini-league");return}if(c==="ranked"){i("ranked");return}n("Bientôt disponible","info")})}),document.getElementById("logout-btn").addEventListener("click",async()=>{await v.auth.signOut(),window.location.reload()}),(s=document.getElementById("try-home2-btn"))==null||s.addEventListener("click",()=>i("home2")),Bn(t,n),Mo(t,n,i),jn(t,n,i),Io(t,n))}async function Io(e,t){const i=e.profile.id,{data:n}=await v.from("mini_league_members").select("league_id, prize_amount, mini_leagues(name)").eq("user_id",i).eq("prize_claimed",!1).gt("prize_amount",0);if(!(n!=null&&n.length))return;const o=n.reduce((a,s)=>a+(s.prize_amount||0),0),r=document.createElement("div");r.className="modal-overlay",r.style.zIndex="2200",r.innerHTML=`<div class="modal" style="max-width:400px">
    ...
    </div>`,document.body.appendChild(t),t.querySelector("#journal-close-v2").addEventListener("click",()=>t.remove()),t.addEventListener("click",n=>{n.target===t&&t.remove()})}async function No(e,{state:t,navigate:i,toast:n}){var m,M,k,I,B,X,te,Z;const o=t.profile;if(!o)return;const r="/icons/";qn(i,o,"home2",r);const a=o.mmr??1e3,s=vi(a),d=Do(a,s),c=In(a),l=isFinite(s.max)?s.max+1:null,[{list:f,iAmInTop:b,myPosition:x},g,p]=await Promise.all([Fo(o.id),Po(5),Go()]),u=(E,L)=>`
    <div class="rk-row ${E.id===o.id?"rk-row-me":""}">
      <div class="rk-pos ${L<3?"rk-pos-top"+(L+1):""}">${L<3?["🥇","🥈","🥉"][L]:L+1}</div>
      <div class="rk-name ${E.id===o.id?"rk-name-me":""}">${E.pseudo}</div>
      <div class="rk-rp">${(E.mmr??0).toLocaleString("fr")} RP</div>
    </div>`,y=E=>`
    <div class="news-item">
      ${E.image_url?`<img src="${E.image_url}" class="news-thumb" onerror="this.style.display='none'">`:'<div class="news-thumb news-thumb-fallback">📰</div>'}
      <div class="news-body">
        <div class="news-title">${E.title}</div>
        <div class="news-desc">${E.description||""}</div>
        <div class="news-time">${qo(E.published_at)}</div>
      </div>
    </div>`;let w=0;const h=E=>`
    <div class="promo-icon-wrap">
      <img src="${r}${E.icon||"nav-boosters.png"}" class="promo-icon" onerror="this.style.display='none'">
    </div>
    <div class="promo-info">
      <div class="promo-kicker">NOUVEAU BOOSTER</div>
      <div class="promo-title">${E.name}</div>
      <div class="promo-desc">${E.card_count||5} cartes · ${E.price_type==="pub"?"Pub gratuite":(E.price_credits||0).toLocaleString("fr")+" cr."}</div>
    </div>`;e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
      ...
      <div class="home2-dash">
        ...
        <div class="home2-col-center">
          ...
          <div class="promo-banner" id="promo-banner">
            ...
            ${p.length>1?`<div class="promo-dots">${p.map((E,L)=>`<div class="promo-dot ${L===0?"active":""}" data-dot="${L}"></div>`).join("")}</div>`:""}
          </div>`:""}
        </div>
  ...
  </div>`,requestAnimationFrame(()=>{var fe;const E=((fe=window.visualViewport)==null?void 0:fe.height)||window.innerHeight,L=document.getElementById("home2-chrome-header"),K=L?L.offsetHeight:0,Y=E-K,pe=e.querySelector(".home-dark");pe&&(pe.style.minHeight=Y+"px")});const $=()=>{const E=document.getElementById("mode-indicator");E&&(E.textContent=window.innerWidth>=1024?"🖥️ Mode PC":"📱 Mode Mobile")};if($(),window.addEventListener("resize",$),(m=document.getElementById("nav-edit-btn"))==null||m.addEventListener("click",()=>i("settings")),(M=document.getElementById("nav-profile-btn"))==null||M.addEventListener("click",()=>i("settings")),(k=document.getElementById("nav-rankings-link"))==null||k.addEventListener("click",()=>i("rankings")),(I=document.getElementById("nav-rankings-cta"))==null||I.addEventListener("click",()=>i("rankings")),(B=document.getElementById("rank-inline-link-btn"))==null||B.addEventListener("click",()=>i("rankings")),(X=document.getElementById("nav-journal-link"))==null||X.addEventListener("click",()=>Ro()),e.querySelectorAll("[data-action]").forEach(E=>{E.addEventListener("click",()=>{E.style.transform="scale(.96)",setTimeout(()=>E.style.transform="",180);const L=E.dataset.action;if(L==="match-ai"){Pn(i);return}if(L==="match-random"){i("match",{matchMode:"random"});return}if(L==="match-friend"){i("friends");return}if(L==="mini-league"){i("mini-league");return}if(L==="ranked"){i("ranked");return}n("Bientôt disponible","info")})}),p.length>1){const E=document.getElementById("promo-slide-content"),L=()=>Array.from(document.querySelectorAll(".promo-dot")),K=Y=>{w=Y,E&&(E.innerHTML=h(p[w])),L().forEach((pe,fe)=>pe.classList.toggle("active",fe===w))};setInterval(()=>K((w+1)%p.length),5e3),document.querySelectorAll(".promo-dot").forEach(Y=>{Y.addEventListener("click",()=>K(Number(Y.dataset.dot)))})}(te=document.getElementById("promo-cta-btn"))==null||te.addEventListener("click",()=>i("boosters")),document.getElementById("logout-btn").addEventListener("click",async()=>{xn(),await v.auth.signOut(),window.location.reload()}),(Z=document.getElementById("back-to-v1-btn"))==null||Z.addEventListener("click",()=>{xn(),i("home")}),Fn(t,n),Ko(t,n,i),Dn(t,n,i),Oo(t,n)}async function Oo(e,t){const i=e.profile.id,{data:n}=await v.from("mini_league_members").select("league_id, prize_amount, mini_leagues(name)").eq("user_id",i).eq("prize_claimed",!1).gt("prize_amount",0);if(!(n!=null&&n.length))return;const o=n.reduce((a,s)=>a+(s.prize_amount||0),0),r=document.createElement("div");r.className="modal-overlay",r.style.zIndex="2200",r.innerHTML=`<div class="modal" style="max-width:400px">
    <div class="modal-header"><h2 style="color:#1A6B3C">🏆 Gains à récupérer</h2></div>
    <div class="modal-body" style="padding:18px 20px">
      <p style="font-size:14px;color:#555;margin:0 0 14px">Tu as terminé sur le podium de ${n.length>1?"plusieurs mini leagues":"une mini league"} !</p>
      <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:16px">
  ...
  </div>`,document.body.appendChild(r),r.querySelector("#claim-later-btn").addEventListener("click",()=>r.remove()),r.querySelector("#claim-all-btn").addEventListener("click",async a=>{const s=a.currentTarget;s.disabled=!0,s.textContent="...";let d=0;for(const c of n){const{data:l}=await v.rpc("claim_mini_league_prize",{p_league_id:c.league_id,p_user_id:i});l!=null&&l.success&&!l.already_claimed&&(d+=l.prize||0)}if(d>0){e.profile.credits=(e.profile.credits||0)+d;const c=document.getElementById("nav-credits");c&&(c.textContent=`💰 ${e.profile.credits.toLocaleString("fr")}`),t(`💰 +${d.toLocaleString("fr")} cr. récupérés !`,"success")}r.remove()})}async function Dn(e,t,i){const n=document.getElementById("ongoing-match-banner");if(!n)return;const o=e.profile.id,{data:r}=await v.from("matches").select("id, home_id, away_id, status, mode").eq("status","active").or(`home_id.eq.${o},away_id.eq.${o}`).order("created_at",{ascending:!1});if(!(r!=null&&r.length)){n.innerHTML="";return}const a=r.map(d=>d.home_id===o?d.away_id:d.home_id).filter(Boolean);let s={};if(a.length){const{data:d}=await v.from("users").select("id, pseudo, club_name").in("id",a);(d||[]).forEach(c=>{s[c.id]=c.club_name||c.pseudo})}n.innerHTML=r.map(d=>{const c=d.home_id===o?d.away_id:d.home_id,l=s[c]||"Adversaire",f=d.mode==="mini_league";return`<div style="display:flex;align-items:center;gap:10px;background:linear-gradient(135deg,rgba(10,61,30,0.8),rgba(26,107,60,0.6));color:#fff;border-radius:12px;padding:12px 16px;border:1px solid rgba(26,107,60,0.4);box-shadow:0 3px 12px rgba(26,107,60,0.3)">
      <div style="background:rgba(255,255,255,0.15);border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0">⚽</div>
      <div style="flex:1;min-width:0">
        <div style="font-size:13px;font-weight:900">${f?"🏆 Mini League":d.mode==="friend"?"Match ami":"Match"} en cours</div>
        <div style="font-size:11px;opacity:0.8;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">vs ${l}</div>
      ...
      <button data-resume="${d.id}" data-mini="${f?"1":""}" style="width:38px;height:38px;border-radius:50%;border:none;background:#22c55e;color:#fff;font-size:18px;cursor:pointer;flex-shrink:0">⚽</button>
      <button data-abandon="${d.id}" data-opp="${c}" style="width:38px;height:38px;border-radius:50%;border:none;background:#cc2222;color:#fff;font-size:18px;cursor:pointer;flex-shrink:0">✕</button>
    </div>`}).join(""),n.querySelectorAll("[data-resume]").forEach(d=>{d.addEventListener("click",async()=>{const c=document.getElementById("page-content")||document.getElementById("app");if(d.dataset.mini==="1"){const{data:l}=await v.from("mini_league_matches").select("id, league_id").eq("match_id",d.dataset.resume).single();l?i("match-mini-league",{mlMatchId:l.id,leagueId:l.league_id}):i("mini-league")}else{const{resumePvpMatch:l}=await Ui(async()=>{const{resumePvpMatch:f}=await Promise.resolve().then(()=>no);return{resumePvpMatch:f}},void 0);l(c,{state:e,navigate:i,toast:t,openModal:null,closeModal:null,refreshProfile:null},d.dataset.resume)}})}),n.querySelectorAll("[data-abandon]").forEach(d=>{d.addEventListener("click",()=>{Uo(async()=>{await Ho(d.dataset.abandon,d.dataset.opp,o),t("Match abandonné (défaite 3-0)","info"),Dn(e,t,i)})})})}async function Ho(e,t,i){zt();const{data:n}=await v.from("matches").select("home_id, away_id, game_state, mode").eq("id",e).single();if(!n)return;const o=n.home_id===i,r=o?0:3,a=o?3:0,s=n.game_state||{};s.p1Score=r,s.p2Score=a,s.phase="finished",s.forfeit=!0,await v.from("matches").update({status:"finished",forfeit:!0,winner_id:t,home_score:r,away_score:a,game_state:s}).eq("id",e),n.mode==="mini_league"&&await v.from("mini_league_matches").update({status:"finished",home_score:r,away_score:a}).eq("match_id",e)}function Uo(e){const t=document.createElement("div");t.className="modal-overlay",t.innerHTML=`<div class="modal" style="max-width:340px">
    <div class="modal-body" style="padding:26px 22px 20px;text-align:center">
      <div style="font-size:40px;margin-bottom:8px">⚠️</div>
      <div style="font-size:17px;font-weight:900;margin-bottom:6px;color:#1a1a1a">Abandonner le match ?</div>
      <div style="font-size:13px;color:#666;margin-bottom:18px">Tu perdras par forfait <b>3-0</b>.</div>
  ...
  </div>`,document.body.appendChild(t),t.querySelector("#ab-cancel").addEventListener("click",()=>t.remove()),t.querySelector("#ab-ok").addEventListener("click",()=>{t.remove(),e()}),t.addEventListener("click",i=>{i.target===t&&t.remove()})}async function Ko(e,t,i){var s,d,c,l;const n=document.getElementById("match-invite-banner");if(!n)return;const{data:o}=await v.from("friend_match_invites").select("id, inviter_id, inviter:users!inviter_id(pseudo, club_name)").eq("invitee_id",e.user.id).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle();if(!o){n.innerHTML="";return}const r=((s=o.inviter)==null?void 0:s.club_name)||((d=o.inviter)==null?void 0:d.pseudo)||"?",a=o.inviter_id;n.innerHTML=`<div id="match-invite-btn" style="display:flex;align-items:center;gap:10px;background:linear-gradient(135deg,rgba(26,10,46,0.8),rgba(74,26,138,0.6));color:#fff;border-radius:12px;padding:12px 16px;border:1px solid rgba(122,40,184,0.4);cursor:pointer;box-shadow:0 3px 12px rgba(74,10,138,0.3)">
    <div style="background:rgba(255,255,255,0.15);border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0">⚽</div>
    <div style="flex:1"><div style="font-size:13px;font-weight:900">${r} t'invite à jouer !</div><div style="font-size:11px;opacity:0.75">Accepter la partie ?</div></div>
    <div style="display:flex;gap:8px">
      <button id="match-inv-accept" style="padding:6px 12px;border-radius:8px;border:none;background:#22c55e;color:#fff;font-size:12px;font-weight:900;cursor:pointer">Jouer !</button>
  ...
  </div>`,(c=document.getElementById("match-inv-accept"))==null||c.addEventListener("click",()=>{n.innerHTML="",i("match",{matchMode:"friend",friendId:a,friendName:r,isAccepting:!0})}),(l=document.getElementById("match-inv-decline"))==null||l.addEventListener("click",async()=>{await v.from("friend_match_invites").update({status:"declined"}).eq("id",o.id),n.innerHTML="",t("Invitation refusée","info")})}async function Fn(e,t){const i=document.getElementById("friend-requests-banner");if(!i)return;const{data:n,error:o}=await v.from("friendships").select("id, requester:users!requester_id(pseudo, club_name)").eq("addressee_id",e.user.id).eq("status","pending");if(o||!(n!=null&&n.length)){i.innerHTML="";return}const r=n.length,a=n.slice(0,2).map(d=>{var c;return((c=d.requester)==null?void 0:c.pseudo)||"?"}).join(", "),s=r>2?` +${r-2}`:"";i.innerHTML=`<div id="friend-req-btn" style="display:flex;align-items:center;gap:10px;background:linear-gradient(135deg,rgba(26,107,60,0.6),rgba(42,157,92,0.4));color:#fff;border-radius:12px;padding:12px 16px;border:1px solid rgba(26,107,60,0.4);cursor:pointer;box-shadow:0 3px 12px rgba(26,107,60,0.25)">
    <div style="background:rgba(255,255,255,0.15);border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0">👥</div>
    <div style="flex:1;min-width:0"><div style="font-size:13px;font-weight:900">${r} demande${r>1?"s":""} d'ami${r>1?"s":""}</div><div style="font-size:11px;opacity:0.85;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${a}${s}</div></div>
    <div style="font-size:20px;flex-shrink:0">›</div>
  </div>`,document.getElementById("friend-req-btn").addEventListener("click",()=>Ki(e,t,()=>Fn(e,t)))}function Pn(e){const t=[{mode:"vs_ai_easy",label:"Facile",desc:"Pour découvrir le jeu",credits:"500",icon:"🟢",bg:"#eefaf2",border:"#bfe8cf",iconBg:"#1A6B3C",text:"#12401f"},{mode:"vs_ai_medium",label:"Moyen",desc:"Un défi équilibré",credits:"1 000",icon:"🟡",bg:"#fdf7e6",border:"#f0dd9e",iconBg:"#D4A017",text:"#5c4408"},{mode:"vs_ai_hard",label:"Difficile",desc:"Réservé aux experts",credits:"1 500",icon:"🔴",bg:"#fdecec",border:"#f3bcbc",iconBg:"#bb2020",text:"#5c1010"}],i=document.createElement("div");i.className="modal-overlay",i.style.zIndex="2000",i.innerHTML=`<div class="modal" style="max-width:400px;border-radius:18px;overflow:hidden">
    <div class="modal-header"><h2>Choisir la difficulté</h2><button class="btn-icon" id="diff-cancel">✕</button></div>
    <div class="modal-body" style="padding:16px">
      <div style="display:flex;flex-direction:column;gap:10px">
        ${t.map(o=>`
  ...
  </div>`,document.body.appendChild(i),i.querySelectorAll(".diff-card").forEach(o=>{o.addEventListener("mouseenter",()=>{o.style.transform="translateY(-1px)",o.style.boxShadow="0 6px 16px -6px rgba(0,0,0,0.18)"}),o.addEventListener("mouseleave",()=>{o.style.transform="",o.style.boxShadow=""})});const n=()=>i.remove();document.getElementById("diff-cancel").addEventListener("click",n),i.addEventListener("click",o=>{o.target===i&&n()}),i.querySelectorAll("[data-mode]").forEach(o=>{o.addEventListener("click",()=>{n(),e("match",{matchMode:o.dataset.mode})})})}async function Vo(e,{state:t,navigate:i,toast:n}){const o=t.profile;if(!o)return;const r="/icons/";qn(i,o,"game",r),e.innerHTML=`
  <style>
    .game-page {
      min-height: 100%;
      background: var(--page-bg);
  ...
  </div>`,e.querySelector("#settings-back").addEventListener("click",()=>i("home")),e.querySelectorAll("[data-theme-choice]").forEach(d=>{d.addEventListener("click",()=>{za(d.dataset.themeChoice),Gn(e,t)})});const r=e.querySelector("#volume-slider"),a=e.querySelector("#volume-label");let s=null;r.addEventListener("input",()=>{zo(Number(r.value)),a.textContent=`${r.value}%`,s&&(s.volume=Math.max(0,Math.min(1,Number(r.value)/100)))}),e.querySelector("#volume-test").addEventListener("click",()=>{s=Ji("/sounds/match-opening.mp3",1)}),e.querySelector("#settings-logout").addEventListener("click",async()=>{await v.auth.signOut(),window.location.reload()})}const je={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé pour ce match."},"Double attaque":{icon:"⚡",desc:"La note d'attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la note d'un joueur adverse."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function Pe(e,t){if(!e)return 0;let i=0;switch(t){case"GK":i=Number(e.note_g)||0;break;case"DEF":i=Number(e.note_d)||0;break;case"MIL":i=Number(e.note_m)||0;break;case"ATT":i=Number(e.note_a)||0;break;default:i=0}return i+(e.boost||0)}const bn=["ATT","MIL","DEF","GK"];function Rn(e,t,i){if(!e||!t)return!1;if(i&&e.position&&t.position)return Oi(i).some(([l,f])=>l===e.position&&f===t.position||l===t.position&&f===e.position);const n=e._col!=null&&t._col!=null&&e._col===t._col,o=e._col!=null&&t._col!=null&&Math.abs(e._col-t._col)===1,r=bn.indexOf(e._line||e.job),a=bn.indexOf(t._line||t.job),s=Math.abs(r-a)===1;return(e._line||e.job)===(t._line||t.job)&&o||n&&s}function Nn(e,t){let i=0;const n=e.length;for(let o=0;o<n;o++)for(let r=o+1;r<n;r++){const a=e[o],s=e[r];if(!a||!s||!Rn(a,s,t))continue;const d=Ut(a,s);d==="#00ff88"?i+=10:d==="#FFD700"&&(i+=5)}return i}function oi(e,t={},i){let n=0,o=0;e.forEach(d=>{const c=d._line||d.job,l=Number(c==="MIL"?d.note_m:d.note_a)||0,f=d.stadiumBonus&&(c==="MIL"||c==="ATT")?10:0;n+=t.doubleAttack?l*2:l,o+=(d.boost||0)+f});const r=n+o,a=Nn(e,i);let s=r+a;return t.stolenNote&&(s-=t.stolenNote),{base:r,links:a,total:Math.max(0,s)}}function ri(e,t={},i){const n=e.reduce((a,s)=>{const d=s._line||s.job;let c=0;d==="GK"?c=Number(s.note_g)||0:d==="MIL"?c=Number(s.note_m)||0:c=Number(s.note_d)||0;const l=s.stadiumBonus&&(d==="GK"||d==="DEF"||d==="MIL")?10:0;return a+c+(s.boost||0)+l},0),o=Nn(e,i);let r=n+o;return t.stolenNote&&(r-=t.stolenNote),{base:n,links:o,total:Math.max(0,r)}}function Qi(e,t,i={}){return i.shield?{goal:!1,shielded:!0}:{goal:e>t,shielded:!1}}function On(e,t,i="easy"){const n=e.filter(a=>!a.used);if(!n.length)return[];const o=[...n].sort((a,s)=>{const d=t==="attack"?Pe(a,"ATT"):a._line==="GK"?Pe(a,"GK"):Pe(a,"DEF");return(t==="attack"?Pe(s,"ATT"):s._line==="GK"?Pe(s,"GK"):Pe(s,"DEF"))-d});let r=i==="easy"?1+Math.floor(Math.random()*2):i==="medium"?2+Math.floor(Math.random()*2):3;return o.slice(0,Math.min(r,o.length,3))}function Yo(e,t){const i={vs_ai_easy:{victoire:500,nul:250,defaite:50},vs_ai_medium:{victoire:1e3,nul:500,defaite:50},vs_ai_hard:{victoire:1500,nul:750,defaite:100},vs_ai_club:{victoire:2500,nul:1250,defaite:100}};return(i[e]||i.vs_ai_easy)[t]||0}const Wo={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},Ti={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},di=["GK","DEF","MIL","ATT"],Xo=["Tous","GK","DEF","MIL","ATT"],Jo={normal:1e3,pepite:5e3,papyte:5e3,legende:1e4};function yn(e){const t=e.player;return t?(t.rarity,Math.max(Number(t.note_g)||0,Number(t.note_d)||0,Number(t.note_m)||0,Number(t.note_a)||0)):null}function Qo(e){return e.length?e.reduce((t,i)=>yn(i)>yn(t)?i:t,e[0]):e[0]}const Zo={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function li(e,t){return e&&Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}function Ii(e,t=""){const i=e.player;if(!i)return"";const n=e.evolution_bonus||0,o={...i,_evolution_bonus:n},r=t?`<div style="position:absolute;top:6px;right:6px;z-index:10;background:#0a3d1e;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 7px">${t}</div>`:"";return`<div style="position:relative;display:inline-block;cursor:pointer" data-card-id="${e.id}">
    ${r}
    ${Te(o,{width:140})}
  </div>`}function hn(e){return`<div style="display:inline-block;filter:grayscale(1);opacity:0.4">
    ${Te(e,{width:140})}
  </div>`}async function er(e,t){const{state:i,navigate:n,toast:o,openModal:r,closeModal:a}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await v.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price, stadium_id, evolution_bonus,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter(z=>z.card_type==="player"&&z.player),l=(s||[]).filter(z=>z.card_type==="game_changer"),f=(s||[]).filter(z=>z.card_type==="formation"),b=(s||[]).filter(z=>z.card_type==="stadium"),{data:x}=await v.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(x||[]).forEach(z=>{g[z.name]=z});const{data:p}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(p||[]).forEach(z=>{u[z.id]=z}),b.forEach(z=>{z.stadium_def&&(u[z.stadium_id]=z.stadium_def)});const y=Object.keys(Hi),w=Object.keys(je),h={};c.forEach(z=>{const F=z.player.id;h[F]=(h[F]||0)+1}),new Set(Object.keys(h).map(z=>String(z)));const $=new Set(f.map(z=>z.formation)),m=new Set(l.map(z=>z.gc_type));let M="player",k="Tous",I="",B=!1;function X(){return[...c].sort((z,F)=>{const H=di.indexOf(z.player.job),N=di.indexOf(F.player.job);return H!==N?H-N:(z.player.surname_real||"").localeCompare(F.player.surname_real||"")})}function te(){return[...d||[]].sort((z,F)=>{const H=di.indexOf(z.job),N=di.indexOf(F.job);return H!==N?H-N:(z.surname_real||"").localeCompare(F.surname_real||"")})}function Z(){return X().filter(z=>{const F=z.player,H=k==="Tous"||F.job===k,N=!I||`${F.firstname} ${F.surname_real}`.toLowerCase().includes(I);return H&&N})}function E(){return te().filter(z=>{const F=k==="Tous"||z.job===k,H=!I||`${z.firstname} ${z.surname_real}`.toLowerCase().includes(I);return F&&H})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:0 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;align-items:center"></div>
  ...
  </div>`,function(F){const H=document.getElementById(F);if(!H)return;H.addEventListener("wheel",de=>{Math.abs(de.deltaY)<=Math.abs(de.deltaX)||(de.preventDefault(),H.scrollLeft+=de.deltaY)},{passive:!1});let N=!1,re=0,q=0,G=!1,se=!1;H.addEventListener("mousedown",de=>{N=!0,G=!1,re=de.pageX,q=H.scrollLeft}),window.addEventListener("mouseup",()=>{N=!1,G&&(H.style.cursor="",se=!0),G=!1}),window.addEventListener("mousemove",de=>{if(!N)return;const ve=de.pageX-re;!G&&Math.abs(ve)<6||(G=!0,H.style.cursor="grabbing",de.preventDefault(),H.scrollLeft=q-ve)}),H.addEventListener("click",de=>{se&&(de.stopPropagation(),de.preventDefault(),se=!1)},!0)}("col-grid");function L(){const z=document.getElementById("col-filters");z&&(M==="player"?(z.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",F=>{I=F.target.value.toLowerCase(),K()}),e.querySelectorAll(".filter-btn").forEach(F=>{F.addEventListener("click",()=>{k=F.dataset.job,L(),K()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{B=!B,L(),K()})):(z.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{B=!B,L(),K()})))}function K(){const z=document.getElementById("col-grid");z&&(M==="player"?W(z):M==="formation"?ue(z):M==="stadium"?V(z):U(z))}function Y(z,F,H,N,re,q=0){const G=document.getElementById("col-grid"),se=document.getElementById("col-big");if(!G||!se)return;var de=0;function ve(){var ct;const ze=window.innerWidth>=768,he=document.getElementById("col-big"),Ie=document.getElementById("col-gap"),Se=((ct=document.getElementById("col-grid"))==null?void 0:ct.parentElement)||null,Be=he?he.closest(".page"):null;if(he&&Se&&Be){let $e=0;Array.from(Be.children).forEach(function(j){j!==he&&j!==Se&&j!==Ie&&($e+=j.offsetHeight)});const Ce=Math.max(0,Be.clientHeight-$e),qe=Math.round(Ce*(q/100)),Ge=Math.max(0,Ce-qe),_=ze?50/71:45/63,T=Math.round(Ge*_),A=Math.max(0,Ge-T);Ie&&(Ie.style.height=qe+"px"),he.style.flex="none",he.style.height=T+"px",he.style.minHeight="0",Se.style.flex="none",Se.style.height=A+"px",Se.style.minHeight="0",Se.style.display="flex",Se.style.overflow="hidden"}const ke=document.getElementById("col-grid");ke&&(ke.style.height="100%",ke.style.flexShrink="0",ke.style.overflowX="auto",ke.style.overflowY="hidden",ke.style.alignItems="center",ke.style.width="100%"),se.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:center center">'+F(z[de])+"</div>";var _t=se.querySelector("[data-card-id],[data-form-id],[data-gc-id]");_t&&_t.addEventListener("click",function(){N(z[de])}),requestAnimationFrame(function(){var $e=document.getElementById("big-card-inner");if(!(!$e||!se)){var Ce=se.clientHeight,qe=se.clientWidth-16,Ge=$e.offsetHeight,_=$e.offsetWidth;if(Ge>0&&_>0&&Ce>40){var T=Math.min(Ce/Ge,qe/_);$e.style.transform="scale("+T.toFixed(3)+")",$e.style.transformOrigin="center center"}}}),G.innerHTML=z.map(function($e,Ce){var qe=Ce===de,Ge="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(qe?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+Ce+'" style="'+Ge+'">'+H($e,qe)+"</div>"}).join(""),G.querySelectorAll(".col-mini-item").forEach(function($e){$e.addEventListener("click",function(){de=Number($e.dataset.idx),ve(),$e.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})}),requestAnimationFrame(function(){var $e=ke?ke.clientHeight:0,Ce=G.querySelector(".col-mini-item > div");if(!(!$e||!Ce)){var qe=Ce.style.zoom;Ce.style.zoom="1";var Ge=Ce.offsetHeight;if(Ce.style.zoom=qe,!(Ge<=0)){var _=$e/Ge;G.querySelectorAll(".col-mini-item > div").forEach(function(T){T.style.zoom=_.toFixed(4)})}}})}ve()}function pe(z,F){var H=window.innerWidth>=768?.76:.54,N;if(!z||z._fake){var re=z?z.player:null;if(!re)return"";N=hn(re)}else N=Ii(z,"");var q=z&&!z._fake?h[z.player&&z.player.id]||1:0,G=q>0?'<div style="position:absolute;top:6px;right:6px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:11px;font-weight:900;padding:2px 7px;z-index:3;box-shadow:0 1px 4px rgba(0,0,0,0.4)">×'+q+"</div>":"";return'<div style="display:inline-block;position:relative;zoom:'+H+';pointer-events:none;line-height:0">'+G+N+"</div>"}function fe(z,F,H){var N=H>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+H+"</div>":"",re=!!F,q=Mi(z,Mt[z],{width:160});return re||(q='<div style="filter:grayscale(1);opacity:0.5">'+q+"</div>"),"<div "+(F?'data-form-id="'+F.id+'"':"")+' style="position:relative;cursor:pointer">'+N+q+"</div>"}function C(z,F){var H=window.innerWidth>=768?.76:.54,N=Mi(z,Mt[z],{width:140});return F||(N='<div style="filter:grayscale(1);opacity:0.45">'+N+"</div>"),'<div style="display:inline-block;zoom:'+H+';line-height:0;pointer-events:none">'+N+"</div>"}function W(z){if(B){const F=E();if(!F.length){z.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucun joueur.</div>';return}const H=F.map(N=>c.find(re=>re.player.id===N.id)||{_fake:!0,player:N,id:"fake-"+N.id});Y(H,N=>N._fake?hn(N.player):Ii(N,""),N=>N._fake?pe({player:N.player,id:"x",_fake:!0}):pe(N),N=>{N._fake||vn(N,c,h,t)})}else{const F=Z();if(!F.length){z.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const H={};F.forEach(re=>{const q=re.player.id;H[q]||(H[q]=[]),H[q].push(re)});const N=Object.values(H).map(re=>Qo(re));Y(N,re=>{const q=h[re.player.id]||1,G=q>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${q}</div>`:"",de=c.filter(ve=>ve.player.id===re.player.id&&ve.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Ii(re,G+de)},re=>pe(re),re=>vn(re,c,h,t))}}function ue(z){const F=B?y:[...$];if(!F.length){z.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const H=F.map(N=>({formation:N,card:f.find(re=>re.formation===N)||null,owned:$.has(N)}));Y(H,({formation:N,card:re,owned:q})=>fe(N,q?re:null,q?f.filter(G=>G.formation===N).length:0),({formation:N,owned:re})=>C(N,re),({card:N,owned:re})=>{re&&N&&ir(N,f,t,r)},"#1A6B3C",5)}function U(z){const F=Object.keys(g),H=B?F.length?F:w:[...m];if(!H.length){z.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const N=H.map(re=>({type:re,gc:je[re]||{icon:"⚡",desc:""},def:g[re]||null,owned:m.has(re),card:l.find(q=>q.gc_type===re)||null}));Y(N,({type:re,gc:q,def:G,owned:se,card:de})=>{var ke;const ve=(G==null?void 0:G.name)||re,ze=se?l.filter(_t=>_t.gc_type===re).length:0,he=ze>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${ze}</div>`:"",Ie=(G==null?void 0:G.effect)||q.desc||"",Se=G!=null&&G.image_url?`/icons/${G.image_url}`:((ke=G==null?void 0:G.club)==null?void 0:ke.logo_url)||(G!=null&&G.country_code?`https://flagsapi.com/${G.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);let Be=tt(ve,Se,q.icon,Ie,{width:160,onClick:se});return se||(Be=`<div style="filter:grayscale(1);opacity:0.5">${Be}</div>`),`<div ${se&&de?`data-gc-id="${de.id}" data-gc-type="${re}"`:""} style="position:relative">${he}${Be}</div>`},({type:re,gc:q,def:G,owned:se})=>{const de=window.innerWidth>=768?.76:.54,ve=(G==null?void 0:G.name)||re,ze=(G==null?void 0:G.effect)||q.desc||"",he=G!=null&&G.image_url?`/icons/${G.image_url}`:null;let Ie=tt(ve,he,q.icon,ze,{width:140});return se||(Ie=`<div style="filter:grayscale(1);opacity:0.45">${Ie}</div>`),`<div style="display:inline-block;zoom:${de};line-height:0;pointer-events:none">${Ie}</div>`},({type:re,owned:q,def:G})=>{q&&tr(re,G,r)},"#7a28b8",5)}function V(z){const F="#4FC3F7",H="/";if(!b.length){z.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const N={};b.forEach(q=>{const G=q.stadium_id||"?";(N[G]=N[G]||[]).push(q)});const re=Object.entries(N).map(([q,G])=>({sid:q,def:u[q]||null,count:G.length,card:G[0]}));Y(re,({def:q,count:G})=>{var Ie,Se;const se=(q==null?void 0:q.name)||"?",de=((Ie=q==null?void 0:q.club)==null?void 0:Ie.encoded_name)||(q==null?void 0:q.country_code)||"—",ve=q!=null&&q.image_url?`${H}icons/${q.image_url}`:((Se=q==null?void 0:q.club)==null?void 0:Se.logo_url)||(q!=null&&q.country_code?`https://flagsapi.com/${q.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),ze=G>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${G}</div>`:"",he=`${de}<br>+10 ⭐ joueurs alliés`;return`<div style="position:relative">${ze}${ti(se,ve,he,{width:160})}</div>`},({def:q})=>{var he,Ie;const G=window.innerWidth>=768?.76:.54,se=(q==null?void 0:q.name)||"?",de=((he=q==null?void 0:q.club)==null?void 0:he.encoded_name)||(q==null?void 0:q.country_code)||"—",ve=q!=null&&q.image_url?`${H}icons/${q.image_url}`:((Ie=q==null?void 0:q.club)==null?void 0:Ie.logo_url)||(q!=null&&q.country_code?`https://flagsapi.com/${q.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),ze=`${de}<br>+10 ⭐`;return`<div style="display:inline-block;zoom:${G};line-height:0;pointer-events:none">${ti(se,ve,ze,{width:140})}</div>`},null,F,5)}e.querySelectorAll(".col-tab-btn").forEach(z=>{z.addEventListener("click",()=>{M=z.dataset.tab,k="Tous",I="",B=!1,e.querySelectorAll(".col-tab-btn").forEach(F=>{const H=F.dataset.tab===M;F.style.borderBottomColor=H?"var(--green)":"transparent",F.style.color=H?"var(--green)":"var(--tile-fg-dim)"}),L(),K()})}),L(),K()}function tr(e,t,i){const n=je[e]||{icon:"⚡",desc:"Effet spécial."},o=(t==null?void 0:t.name)||e,r=(t==null?void 0:t.effect)||n.desc,a=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
      ${tt(o,a,n.icon,r,{width:200})}
      <div style="background:#fff3cd;border-radius:10px;padding:10px 14px;width:100%">
        <div style="font-size:12px;color:#856404">⚠️ Cette carte est à <b>usage unique</b>. Une fois jouée en match, elle est définitivement supprimée de ta collection.</div>
      </div>
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const ci=1e3;function ir(e,t,i,n){var g,p,u;const{state:o,toast:r,closeModal:a,navigate:s,refreshProfile:d}=i,c=e.formation,l={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function f(){const y=Mt[c]||{},w=Hi[c]||[],h=290,$=360,m=20;function M(I){const B=y[I];return B?{x:B.x*h,y:B.y*$}:null}let k=`<svg width="${h}" height="${$}" viewBox="0 0 ${h} ${$}" xmlns="http://www.w3.org/2000/svg">`;for(const[I,B]of w){const X=M(I),te=M(B);!X||!te||(k+=`<line x1="${X.x}" y1="${X.y}" x2="${te.x}" y2="${te.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const I of Object.keys(y)){const B=M(I);if(!B)continue;const X=I.replace(/\d+/,""),te=l[X]||"#555";k+=`<circle cx="${B.x}" cy="${B.y}" r="${m}" fill="${te}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,k+=`<text x="${B.x}" y="${B.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${X}</text>`}return k+="</svg>",k}const b=t.filter(y=>y.formation===c);b.length;const x=!e.is_for_sale;n(`Formation ${c}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(g=document.getElementById("direct-sell-form-btn"))==null||g.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${c} pour ${ci.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const y=b.find(h=>!h.is_for_sale)||b[0];if(!y){r("Aucune carte à vendre","error");return}await v.from("market_listings").delete().eq("card_id",y.id),await v.from("transfer_history").delete().eq("card_id",y.id);const{error:w}=await v.from("cards").delete().eq("id",y.id);if(w){r(w.message,"error");return}await v.from("users").update({credits:(o.profile.credits||0)+ci}).eq("id",o.profile.id),await d(),r(`+${ci.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),s("collection")}),(p=document.getElementById("market-sell-form-btn"))==null||p.addEventListener("click",async()=>{const y=parseInt(document.getElementById("sell-price-form").value);if(!y||y<1){r("Prix invalide","error");return}await v.from("cards").update({is_for_sale:!0,sale_price:y}).eq("id",e.id),await v.from("market_listings").insert({seller_id:o.profile.id,card_id:e.id,price:y}),r("Carte mise en vente sur le marché !","success"),a(),s("collection")}),(u=document.getElementById("cancel-sell-form-btn"))==null||u.addEventListener("click",async()=>{await v.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await v.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),r("Annonce retirée","success"),a(),s("collection")})}async function vn(e,t,i,n){var pe,fe,C,W,ue;const{state:o,toast:r,openModal:a,closeModal:s,navigate:d,refreshProfile:c}=n,l=e.player,f=t.filter(U=>U.player.id===l.id),b=f.length,x=e.evolution_bonus||0,g=Math.max((Number(l.note_g)||0)+(l.job==="GK"||l.job2==="GK"?x:0),(Number(l.note_d)||0)+(l.job==="DEF"||l.job2==="DEF"?x:0),(Number(l.note_m)||0)+(l.job==="MIL"||l.job2==="MIL"?x:0),(Number(l.note_a)||0)+(l.job==="ATT"||l.job2==="ATT"?x:0)),p=l.rarity||"normal",{data:u}=await v.from("sell_price_configs").select("*").eq("rarity",p).lte("note_min",g).gte("note_max",g).order("note_min",{ascending:!1}).limit(1);((pe=u==null?void 0:u[0])==null?void 0:pe.price)??Jo[p];const y=l.rarity!=="legende";At(l);const w=li(l,l.job)+x,h=l.job2?li(l,l.job2)+(li(l,l.job2)>0?x:0):null;Ti[l.job],l.job2&&Ti[l.job2];const $=Wo[l.rarity]||"#ccc";Zo[l.country_code]||l.country_code;const m=e.evolution_bonus||0,k=w+m,I=h||0,B=I>0?I+m:0,X=f.map(U=>U.id);let te={};if(X.length){const{data:U}=await v.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",X).order("transferred_at",{ascending:!0});(U||[]).forEach(V=>{te[V.card_id]||(te[V.card_id]=[]),te[V.card_id].push(V)})}const Z=X.length?`
    <div style="margin-top:16px;border-top:1px solid var(--tile-border);padding-top:14px">
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px">
        ${f.filter(U=>U.id!==e.id).map((U,V)=>{const z=te[U.id]||[],F=U.is_for_sale,H=z.length?z[z.length-1]:null,N=U.evolution_bonus||0,re=H?H.source==="booster"?"Booster":H.price?H.price.toLocaleString("fr")+" cr.":"—":"—",q=H?new Date(H.transferred_at).toLocaleDateString("fr",{day:"2-digit",month:"2-digit",year:"numeric"}):"",G=80,se=Math.round(G*657/507),de=Te({...l,_evolution_bonus:N},{width:G});return`
            <div class="exemplaire-row" data-card-id="${U.id}" data-card-idx="${V}"
              ...
              <input type="checkbox" class="expl-check"
                data-id="${U.id}" data-evo="${N}" data-note="${li(l,l.job)}"
                ${F?"disabled":""}
                style="display:none">
      ...
      <!-- Carte visuelle -->
      ${Te({...l,_evolution_bonus:x},{width:160})}
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        <div>
          ...
          <div style="font-weight:700;color:${l.rarity==="normal"?"#555":l.rarity==="papyte"?"#707070":$}">${l.rarity.toUpperCase()}</div>
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",l.note_g],["DEF",l.note_d],["MIL",l.note_m],["ATT",l.note_a]].map(([U,V])=>{const z=Ti[U],F=U==="GK"?"#fff":z,H=U===l.job||U===l.job2,N=(Number(V)||0)+(H&&m>0?m:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
                <div style="width:48px;height:48px;border-radius:8px;background:#111;border:2.5px solid ${z};
                  display:flex;align-items:center;justify-content:center">
                  <span style="font-size:20px;font-weight:900;color:${F};font-family:Arial Black,Arial;line-height:1">${N}</span>
                </div>
    ...
    ${Z}
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(fe=document.getElementById("close-detail"))==null||fe.addEventListener("click",s);let E=new Set;const L=l.rarity==="pepite"?1.3:l.rarity==="papyte"?.7:1;function K(){let U=0;return document.querySelectorAll(".expl-check:checked").forEach(V=>{if(V.dataset.id===e.id)return;const F=Number(V.dataset.evo)||0,H=Number(V.dataset.note)||0;U+=H+F}),Math.round(U*L)}const Y=()=>{const U=E.size,V=document.getElementById("sell-action-panel");if(!V)return;V.style.display=U>0?"block":"none",document.getElementById("sell-selected-count").textContent=U;const z=document.getElementById("evolve-btn");if(z){const F=K();z.textContent=F>0?`⬆️ Évoluer (+${F})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach(U=>{U.addEventListener("change",()=>{const V=U.dataset.id;U.checked?E.add(V):E.delete(V);const z=U.closest(".exemplaire-row");if(z){const F=z.querySelector(".expl-sel-overlay"),H=z.querySelector(".expl-sel-check");F&&(F.style.display=U.checked?"block":"none"),H&&(H.style.display=U.checked?"flex":"none"),z.style.transform=U.checked?"scale(1.05)":"scale(1)"}Y()})}),document.querySelectorAll(".exemplaire-row").forEach(U=>{U.addEventListener("click",V=>{if(V.target.tagName==="INPUT")return;const z=U.querySelector(".expl-check");z&&!z.disabled&&(z.checked=!z.checked,z.dispatchEvent(new Event("change")))})}),(C=document.getElementById("evolve-btn"))==null||C.addEventListener("click",async()=>{if(b<=1)return;const U=[...E];if(!U.length)return;if(E.has(e.id)){const G=document.createElement("div");G.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",G.innerHTML=`
        ...
        <div style="background:var(--tile-bg);border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.3)">
          ...
          <div style="background:#f0fdf4;border-radius:10px;padding:12px;margin-bottom:16px;font-size:13px;color:#333">
            ...
            ➕ Brut : <strong>+${z}</strong>${H?`<span style="font-size:11px;color:#888"> ${H}</span>`:""}<br>
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
        ...
        <div style="width:105px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 6px;gap:8px;border-right:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="display:flex;flex-direction:column;gap:6px;align-items:center" id="subs-list">
            ${c.map(u=>{const y={...u.player,_evolution_bonus:u.evolution_bonus||0};return`<div style="position:relative;flex-shrink:0;overflow:visible;padding-bottom:24px">
                ${Te({...y,_evolution_bonus:y._evolution_bonus||0},{width:90,showStad:!0,stadDef:d})}
    ...
    <div class="page-body" style="padding-bottom:20px">
  ...
  </div>`;const l=window.innerWidth>=900,f=e.querySelector(".deck-pc-layout"),b=e.querySelector(".deck-mobile-layout");f&&(f.style.display=l?"block":"none"),b&&(b.style.display=l?"none":"block"),or(e,t,o,i),e.querySelectorAll("#builder-back").forEach(u=>u.addEventListener("click",()=>n("decks")));const x=()=>{const{openModal:u,closeModal:y}=i,w=`<div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px">
      ${a.map(h=>`<div data-forma="${h}" style="cursor:pointer;padding:10px 16px;border-radius:8px;background:${h===t.formation?"#1A6B3C":"#f0f0f0"};color:${h===t.formation?"#fff":"#111"};font-weight:900;font-size:16px;border:2px solid ${h===t.formation?"#1A6B3C":"#ddd"}">${h}</div>`).join("")}
    </div>`;u("⚽ Choisir une formation",w),document.querySelectorAll("#modal-body [data-forma]").forEach(h=>{h.addEventListener("click",()=>{t.formation=h.dataset.forma;const $=kn(t.formation),m={};$.forEach(M=>{t.slots[M]&&(m[M]=t.slots[M])}),t.slots=m,y(),wt(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",x)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>rr(t,e,i))),e.querySelectorAll("#save-deck").forEach(u=>u.addEventListener("click",()=>dr(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{t.subs=t.subs.filter(y=>y!==u.dataset.removeSub),wt(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>sr(t,e,i)))}function or(e,t,i,n){var h,$,m;const o=window.innerWidth>=900,r=e.querySelector(o?"#deck-field-pc":"#deck-field-mobile");if(!r)return;const a=(h=t.stadiumCards)==null?void 0:h.find(M=>M.id===t.stadiumCardId),s=a&&(($=t.stadDefMap)==null?void 0:$[a.stadium_id])||null,d=Mt[t.formation]||{},c=Oi(t.formation)||[],l={};for(const M of i){const k=t.slots[M],I=k?t.playerCards.find(B=>B.id===k):null;I!=null&&I.player?l[M]={...I.player,_evolution_bonus:I.evolution_bonus||0,face:I.player.face||null}:l[M]=null}const f=window.innerWidth>=900,b=f?window.innerWidth-280:window.innerWidth-20,x=f?Math.min(b,860):b,g=Math.round(f?x*.82:x*.85),p=f?84:44;let u="";for(const[M,k]of c){const I=d[M],B=d[k];if(!I||!B)continue;const X=I.x*x,te=Math.round(.03*g+I.y*.85*g),Z=B.x*x,E=Math.round(.03*g+B.y*.85*g),L=l[M],K=l[k],Y=Ut(L,K);Y==="#ff3333"||Y==="#cc2222"?u+=`<line x1="${X.toFixed(1)}" y1="${te.toFixed(1)}" x2="${Z.toFixed(1)}" y2="${E.toFixed(1)}" stroke="${Y}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(u+=`<line x1="${X.toFixed(1)}" y1="${te.toFixed(1)}" x2="${Z.toFixed(1)}" y2="${E.toFixed(1)}" stroke="${Y}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,u+=`<line x1="${X.toFixed(1)}" y1="${te.toFixed(1)}" x2="${Z.toFixed(1)}" y2="${E.toFixed(1)}" stroke="${Y}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let y="";const w=Math.round(p*657/507);for(const M of i){const k=d[M];if(!k)continue;const I=l[M],B=k.x*x;k.y*g;const X=Math.round(.03*g+k.y*(.85*g)),te=Math.round(B-p/2),Z=Math.round(X-w/2);if(I){const E=M.replace(/\d+/,""),L=s&&(s.club_id&&String(I.club_id)===String(s.club_id)||s.country_code&&I.country_code===s.country_code),K=Te({...I,_evolution_bonus:I._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:E});L&&((m=s.club)!=null&&m.logo_url||s.image_url),y+=`<div style="position:absolute;left:${te}px;top:${Z}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${M}">
        <div style="position:relative">${K}</div>
      </div>`}else{const E=M.replace(/\d+/,"");y+=`<div style="position:absolute;left:${te}px;top:${Z}px;width:${p}px;height:${w}px;
        ...
        <span style="font-size:8px;color:rgba(255,255,255,0.3);margin-top:2px">${E}</span>
      </div>`}}r.innerHTML=`
    <div style="position:relative;width:${x}px;height:${g}px;margin:0 auto">
      ...
      ${y}
    </div>`,r.querySelectorAll(".deck-slot-hit").forEach(M=>{M.addEventListener("click",()=>ar(M.dataset.pos,t,e,n))})}function rr(e,t,i){var a;const{openModal:n,closeModal:o}=i,r=e.stadiumCards||[];n("🏟️ Choisir un stade",`<div style="display:flex;flex-wrap:wrap;gap:10px;padding:8px">
  ...
  </div>`,e.querySelectorAll(".booster-card:not(.disabled)").forEach(g=>{g.addEventListener("click",async()=>{const p=r.find(u=>u.id===g.dataset.booster);if(p){g.style.opacity="0.5",g.style.pointerEvents="none";try{await mr(p,{state:t,toast:n,navigate:i,container:e})}catch(u){n(u.message,"error"),g.style.opacity="",g.style.pointerEvents=""}}})}),e.querySelectorAll(".booster-info-btn").forEach(g=>{g.addEventListener("click",p=>{p.stopPropagation();const u=r.find(y=>y.id===g.dataset.boosterId);yr(u)})})}async function mr(e,{state:t,toast:i,navigate:n,container:o}){var f,b;if(e.cost>0&&t.profile.credits<e.cost){i("Crédits insuffisants","error");return}e.isPub&&await vr();const{data:r}=await v.from("cards").select("card_type, player_id, formation").eq("owner_id",t.profile.id),a=new Set((r||[]).filter(x=>x.card_type==="player").map(x=>x.player_id)),s=new Set((r||[]).filter(x=>x.card_type==="formation").map(x=>x.formation));let d=[],c=null;try{if((f=e.rates)!=null&&f.length)d=await qi(t.profile,e);else{const x=e.type||"player";x==="player"?d=await Vn(t.profile,e.cardCount,e.cost):x==="game_changer"?d=await Yn(t.profile,e.cardCount,e.cost):x==="formation"?d=await Wn(t.profile,e.cost):d=await qi(t.profile,e)}}catch(x){c=x.message||String(x),console.error("[Booster] Erreur:",x)}if(d!=null&&d.length&&e._boosterId&&await lr(t.user.id,e._boosterId),!(d!=null&&d.length)){const x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",x.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
      <div style="font-size:12px;color:rgba(255,255,255,0.6);max-width:320px;word-break:break-all;background:rgba(255,255,255,0.05);padding:10px;border-radius:8px;margin-top:6px">
        ${c||"Vérifie la console (F12) pour plus de détails"}
      </div>
      <button style="margin-top:10px;padding:12px 28px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer" id="anim-close-err">Fermer</button>`,document.body.appendChild(x),(b=x.querySelector("#anim-close-err"))==null||b.addEventListener("click",()=>x.remove());return}d.forEach(x=>{x.card_type==="player"&&x.player?x.isDuplicate=a.has(x.player.id):x.card_type==="formation"&&(x.isDuplicate=s.has(x.formation))});const{data:l}=await v.from("users").select("*").eq("id",t.profile.id).single();l&&(t.profile=l),Xn(d,e,n)}function gr(){const e=Math.random()*100;return e<.5?"legende":e<2?"special":e<10?"normal_high":"normal_low"}function mt(e){return Math.max(Number(e.note_g)||0,Number(e.note_d)||0,Number(e.note_m)||0,Number(e.note_a)||0)}function xr(e,t){let i;switch(t){case"legende":i=e.filter(n=>n.rarity==="legende"),i.length||(i=e.filter(n=>n.rarity==="pepite"||n.rarity==="papyte")),i.length||(i=e.filter(n=>mt(n)>=6));break;case"special":i=e.filter(n=>n.rarity==="pepite"||n.rarity==="papyte"),i.length||(i=e.filter(n=>mt(n)>=6));break;case"normal_high":i=e.filter(n=>n.rarity==="normal"&&mt(n)>=6),i.length||(i=e.filter(n=>mt(n)>=6));break;default:i=e.filter(n=>n.rarity==="normal"&&mt(n)>=1&&mt(n)<=5),i.length||(i=e.filter(n=>n.rarity==="normal"));break}return i.length||(i=e),i[Math.floor(Math.random()*i.length)]}async function qi(e,t){if(t.cost>0){const{error:b}=await v.from("users").update({credits:e.credits-t.cost}).eq("id",e.id);if(b)throw b}const i=t.allow_duplicates!==!1;let n=[];const{data:o,error:r}=await v.from("cards").select("player_id, card_type, formation, stadium_id, gc_type").eq("owner_id",e.id);if(r){const{data:b}=await v.from("cards").select("player_id, card_type, formation, gc_type").eq("owner_id",e.id);n=b||[]}else n=o||[];const a=new Set(n.filter(b=>b.card_type==="player").map(b=>b.player_id)),s=new Set(n.filter(b=>b.card_type==="formation").map(b=>b.formation)),d=new Set(n.filter(b=>b.card_type==="game_changer").map(b=>b.gc_type)),c=new Set(n.filter(b=>b.card_type==="stadium").map(b=>b.stadium_id).filter(Boolean)),l=new Set,f=[];for(let b=0;b<(t.cardCount||5);b++){const x=cr(t.rates);if(x){if(x.card_type==="player"){const g=k=>({légende:"legende",pépite:"pepite",pépites:"pepite"})[k]||k,p=x.rarity?g(x.rarity):null;let u=v.from("players").select("id,job,firstname,surname_real,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);p&&(u=u.eq("rarity",p));const{data:y}=await u;let w=y||[];if((x.note_min||x.note_max)&&(w=w.filter(k=>{const I=Math.max(Number(k.note_g)||0,Number(k.note_d)||0,Number(k.note_m)||0,Number(k.note_a)||0);return(!x.note_min||I>=x.note_min)&&(!x.note_max||I<=x.note_max)})),w.length||(x.note_min||x.note_max?(w=y||[],console.warn("[Booster] Aucun joueur avec note",x.note_min,"-",x.note_max,"— fallback rareté uniquement")):w=y||[]),!w.length)continue;let h=w.filter(k=>!l.has(k.id));if(i)h.length||(h=w);else if(h=h.filter(k=>!a.has(k.id)),!h.length)continue;const $=h[Math.floor(Math.random()*h.length)];l.add($.id);const m=a.has($.id),{data:M}=await v.from("cards").insert({owner_id:e.id,player_id:$.id,card_type:"player"}).select().single();M&&(f.push({...M,player:$,isDuplicate:m}),v.rpc("record_transfer",{p_card_id:M.id,p_player_id:$.id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{}))}else if(x.card_type==="game_changer"){const{data:g}=await v.from("gc_definitions").select("id,name,color,effect,image_url,gc_type").eq("is_active",!0).eq("gc_type","game_changer"),p=g!=null&&g.length?g:[{name:"Ressusciter"},{name:"Double attaque"},{name:"Bouclier"},{name:"Vol de note"},{name:"Gel"}],u=i?p:p.filter($=>!d.has($.name));if(!i&&!u.length)continue;const y=u[Math.floor(Math.random()*u.length)],w=y.name,{data:h}=await v.from("cards").insert({owner_id:e.id,card_type:"game_changer",gc_type:w,gc_definition_id:y.id||null}).select().single();h&&f.push({...h,_gcDef:y})}else if(x.card_type==="formation"){const g=Un(),p=i?g:g.filter(h=>!s.has(h));if(!i&&!p.length)continue;const u=p[Math.floor(Math.random()*p.length)],y=s.has(u),{data:w}=await v.from("cards").insert({owner_id:e.id,card_type:"formation",formation:u}).select();w!=null&&w[0]&&f.push({...w[0],isDuplicate:y})}else if(x.card_type==="stadium"){const{data:g,error:p}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)");if(p){console.error("[Booster] stadium_definitions:",p.message);continue}if(!(g!=null&&g.length)){console.warn("[Booster] Aucun stade en DB");continue}const u=i?g:g.filter($=>!c.has($.id));if(!i&&!u.length)continue;const y=u[Math.floor(Math.random()*u.length)],{data:w,error:h}=await v.from("cards").insert({owner_id:e.id,card_type:"stadium",stadium_id:y.id}).select("id,card_type,stadium_id").single();if(h){console.error("[Booster] insert stadium card:",h.message);continue}w&&f.push({...w,rarity:"normal",_stadiumDef:y})}}}return f}async function Vn(e,t,i){if(i>0){const{error:c}=await v.from("users").update({credits:e.credits-i}).eq("id",e.id);if(c)throw c}const{data:n}=await v.from("players").select("id,job,firstname,surname_real,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(n!=null&&n.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const o=n.filter(c=>c.job==="GK"),r=n.filter(c=>c.job!=="GK"),a=!e.first_booster_opened&&o.length>0,s=[];for(let c=0;c<t;c++){const l=c===0&&a?o:c===0?r:n,f=gr(),b=xr(l,f);b&&s.push(b)}a&&await v.from("users").update({first_booster_opened:!0}).eq("id",e.id);const{data:d}=await v.from("cards").insert(s.map(c=>({owner_id:e.id,player_id:c.id,card_type:"player"}))).select();return(d||[]).forEach((c,l)=>{v.rpc("record_transfer",{p_card_id:c.id,p_player_id:s[l].id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{})}),s.map((c,l)=>({...d[l],player:c}))}async function Yn(e,t,i){const{error:n}=await v.from("users").update({credits:e.credits-i}).eq("id",e.id);if(n)throw n;const{data:o}=await v.from("gc_definitions").select("id,name,gc_type,color,effect,image_url").eq("is_active",!0),r=o!=null&&o.length?o:Object.keys(Bi).map(f=>({name:f,gc_type:"game_changer"})),a=Array.from({length:t},()=>r[Math.floor(Math.random()*r.length)]),s=a.map(f=>({owner_id:e.id,card_type:"game_changer",gc_type:f.name,gc_definition_id:f.id||null})),{data:d,error:c}=await v.from("cards").insert(s).select();return c&&console.error("[Booster GC] Erreur insert:",c.message,c),(d||[]).map((f,b)=>({...f,_gcDef:a[b]||null}))}async function Wn(e,t){const{error:i}=await v.from("users").update({credits:e.credits-t}).eq("id",e.id);if(i)throw i;const{data:n}=await v.from("cards").select("formation").eq("owner_id",e.id).eq("card_type","formation"),o=new Set((n||[]).map(l=>l.formation)),r=Un(),a=r[Math.floor(Math.random()*r.length)],s=o.has(a),{data:d,error:c}=await v.from("cards").insert({owner_id:e.id,card_type:"formation",formation:a}).select();return c&&console.error("[Booster Formation] Erreur insert:",c.message,c),(d||[]).map(l=>({...l,isDuplicate:s}))}function Xn(e,t,i,n=null){var Z,E;if(!e||e.length===0){const L=document.createElement("div");L.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",L.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
      <div style="font-size:13px;color:rgba(255,255,255,0.5)">Erreur lors du tirage (permissions DB ou colonne manquante)</div>
      <button style="margin-top:10px;padding:12px 28px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer" id="anim-close-err">Fermer</button>`,document.body.appendChild(L),(Z=L.querySelector("#anim-close-err"))==null||Z.addEventListener("click",()=>L.remove());return}e=[...e].sort((L,K)=>{const Y=L.player?mt(L.player):-1;return(K.player?mt(K.player):-1)-Y});const o=document.createElement("div");o.id="booster-anim-overlay",o.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(o);let r=!1;const a=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let d=!1;const c=L=>L.touches&&L.touches[0]?L.touches[0].clientX:L.clientX;function l(L){r||(d=!0,s.style.opacity="1",f(L))}function f(L){if(!d||r)return;const K=a.getBoundingClientRect(),Y=c(L)-K.left,pe=Math.max(0,Math.min(1,Y/K.width));s.style.width=pe*K.width+"px",pe>=.82&&x()}function b(){r||(d=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{r||(s.style.transition="")},220))}function x(){var K;if(r)return;r=!0,d=!1,s.style.width="100%",s.style.opacity="1",(K=document.getElementById("cut-flash"))==null||K.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const L=document.getElementById("cut-hint");L&&(L.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",y(0)},620)}a.addEventListener("pointerdown",l),window.addEventListener("pointermove",f),window.addEventListener("pointerup",b),a.addEventListener("touchstart",l,{passive:!0}),window.addEventListener("touchmove",f,{passive:!0}),window.addEventListener("touchend",b);let g=0,p=!1;const u=new Set;function y(L){g=L,document.getElementById("reveal-phase").style.display="flex",w(),h(L,0),I()}function w(){const L=document.getElementById("card-dots");L&&(L.innerHTML=e.map((K,Y)=>`<div class="card-dot" data-i="${Y}" style="width:8px;height:8px;border-radius:50%;background:${Y===g?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),L.querySelectorAll(".card-dot").forEach(K=>K.addEventListener("click",()=>m(parseInt(K.dataset.i)))))}function h(L,K){var z,F;const Y=e[L],pe=document.getElementById("card-counter"),fe=document.getElementById("card-track");pe&&(pe.textContent=`Carte ${L+1} / ${e.length}`);const C=document.getElementById("reveal-btns");C&&(C.style.display=L===e.length-1?"flex":"none");const W=Y.card_type==="player"&&((z=Y.player)==null?void 0:z.rarity)==="legende",ue=!u.has(L);u.add(L);let U=0;if(Y.card_type==="player"&&Y.player){const H=Y.player,N=H.job||"ATT";U=(Number(N==="GK"?H.note_g:N==="DEF"?H.note_d:N==="MIL"?H.note_m:H.note_a)||0)+(Y.evolution_bonus||0)}const V=H=>{fe.innerHTML=`
        <div id="current-card-wrap" style="position:relative;display:flex;flex-direction:column;align-items:center;gap:8px;${W?"filter:drop-shadow(0 0 20px #7a28b8)":""}">
          <div style="transform:scale(1.25);transform-origin:center">${br(Y)}</div>
        ...
        </div>`;const N=document.getElementById("current-card-wrap");K!==0?(N.style.transition="none",N.style.transform=`translateX(${K>0?100:-100}%)`,requestAnimationFrame(()=>{N.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",N.style.transform="translateX(0)"})):N.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),H||W?X():te(),w()};ue&&(((F=Y.player)==null?void 0:F.rarity)==="legende"||U>=18)&&Y.card_type==="player"&&Y.player?$(Y,()=>V(!0)):V(!1)}function $(L,K){var ze;p=!0;const Y=L.player,pe=`https://flagsapi.com/${Y.country_code}/flat/64.png`,fe=(ze=Y.clubs)==null?void 0:ze.logo_url,C=At(Y),W=Y.job||"ATT",ue=Number(W==="GK"?Y.note_g:W==="DEF"?Y.note_d:W==="MIL"?Y.note_m:Y.note_a)||0,U=L.evolution_bonus||0,V=ue+U,z=V>=18&&V<=20,F=V>=18,H=document.getElementById("walkout-overlay"),N=document.getElementById("walkout-stage");if(!H||!N){p=!1,K();return}let re=null;z&&(re=Ji("/sounds/Legendary.mp3",.8)),H.style.display="flex";const q=()=>{const he=N.firstElementChild;he&&(he.classList.remove("wo-in"),he.classList.add("wo-out"))},G=1800,se=400;N.innerHTML=`<img class="wo-in" src="${pe}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(q,G),setTimeout(()=>{var he;N.innerHTML=fe?`<img class="wo-in" src="${fe}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((he=Y.clubs)==null?void 0:he.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},G+se),setTimeout(q,G*2+se),setTimeout(()=>{N.innerHTML=C?`<img class="wo-in" src="${C}" style="height:200px;border-radius:50%;box-shadow:0 0 40px rgba(255,255,255,0.3);object-fit:cover;object-position:top">`:'<div class="wo-in" style="font-size:80px">👤</div>',navigator.vibrate&&navigator.vibrate(30)},(G+se)*2),setTimeout(q,(G+se)*2+G);const ve=F?"#FFD700":{GK:"#c0c0c0",DEF:"#e03030",MIL:"#D4A017",ATT:"#3fbf5f"}[W]||"#fff";setTimeout(()=>{N.innerHTML=`<div class="wo-in" style="
        ...
        ${V}
      </div>`,F&&navigator.vibrate&&navigator.vibrate([50,30,100,30,200])},(G+se)*3),setTimeout(q,(G+se)*3+G),setTimeout(()=>{H.style.display="none",N.innerHTML="",p=!1,re&&!z&&re.pause(),navigator.vibrate&&navigator.vibrate([40,30,80]),K()},(G+se)*4)}function m(L){if(p||L<0||L>=e.length||L===g)return;const K=L>g?1:-1;g=L,h(L,K)}function M(){m(g+1)}function k(){m(g-1)}function I(){const L=document.getElementById("card-viewport");if(!L||L._swipeBound)return;L._swipeBound=!0;let K=0,Y=0,pe=0,fe=!1;const C=z=>z.touches?z.touches[0].clientX:z.clientX,W=z=>z.touches?z.touches[0].clientY:z.clientY,ue=z=>{fe=!0,K=C(z),Y=W(z),pe=0},U=z=>{if(!fe)return;pe=C(z)-K;const F=W(z)-Y;if(Math.abs(pe)<Math.abs(F))return;const H=document.getElementById("current-card-wrap");H&&(H.style.transition="none",H.style.transform=`translateX(${pe*.6}px) rotate(${pe*.02}deg)`)},V=()=>{if(!fe)return;fe=!1;const z=document.getElementById("current-card-wrap"),F=55;pe<=-F&&g<e.length-1?M():pe>=F&&g>0?k():z&&(z.style.transition="transform .2s ease",z.style.transform="translateX(0)")};L.addEventListener("pointerdown",ue),L.addEventListener("pointermove",U),L.addEventListener("pointerup",V),L.addEventListener("pointercancel",V),L.addEventListener("touchstart",ue,{passive:!0}),L.addEventListener("touchmove",U,{passive:!0}),L.addEventListener("touchend",V),L.addEventListener("click",z=>{if(Math.abs(pe)>8)return;const F=L.getBoundingClientRect();z.clientX-F.left>F.width/2?M():k()})}let B=null;function X(){const L=document.getElementById("fireworks-canvas");if(!L)return;L.width=window.innerWidth,L.height=window.innerHeight;const K=L.getContext("2d"),Y=[];function pe(){const C=Math.random()*L.width,W=Math.random()*L.height*.6,ue=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],U=ue[Math.floor(Math.random()*ue.length)];for(let V=0;V<60;V++){const z=Math.PI*2/60*V,F=2+Math.random()*5;Y.push({x:C,y:W,vx:Math.cos(z)*F,vy:Math.sin(z)*F,alpha:1,color:U,size:2+Math.random()*3})}}pe(),B=setInterval(pe,600);function fe(){if(document.getElementById("fireworks-canvas")){K.clearRect(0,0,L.width,L.height);for(let C=Y.length-1;C>=0;C--){const W=Y[C];if(W.x+=W.vx,W.y+=W.vy+.08,W.vy*=.98,W.alpha-=.018,W.alpha<=0){Y.splice(C,1);continue}K.globalAlpha=W.alpha,K.fillStyle=W.color,K.beginPath(),K.arc(W.x,W.y,W.size,0,Math.PI*2),K.fill()}K.globalAlpha=1,(B!==null||Y.length>0)&&requestAnimationFrame(fe)}}fe()}function te(){B!==null&&(clearInterval(B),B=null);const L=document.getElementById("fireworks-canvas");L&&L.getContext("2d").clearRect(0,0,L.width,L.height)}if(n){const L=document.getElementById("reveal-btns");L&&(L.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(E=document.getElementById("reveal-next"))==null||E.addEventListener("click",()=>{te(),o.remove(),n()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{te(),o.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{te(),o.remove(),i("boosters")})}function br(e){var t,i,n,o;if(e.card_type==="player"&&e.player)return ur(e);if(e.card_type==="game_changer"){const r=e._gcDef,a=(r==null?void 0:r.name)||e.gc_type||"Game Changer",s=(r==null?void 0:r.effect)||((t=Bi[e.gc_type])==null?void 0:t.desc)||"",d=r!=null&&r.image_url?`/icons/${r.image_url}`:null,c=((i=Bi[e.gc_type])==null?void 0:i.icon)||"⚡";return tt(a,d,c,s,{width:170})}if(e.card_type==="formation")return Mi(e.formation,Mt[e.formation],{width:160});if(e.card_type==="stadium"){const r=e._stadiumDef,a=(r==null?void 0:r.name)||"Stade",s=((n=r==null?void 0:r.club)==null?void 0:n.encoded_name)||(r==null?void 0:r.country_code)||"—",d=r!=null&&r.image_url?`/icons/${r.image_url}`:((o=r==null?void 0:r.club)==null?void 0:o.logo_url)||(r!=null&&r.country_code?`https://flagsapi.com/${r.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),c=`${s}<br>+10 ⭐ joueurs alliés`;return ti(a,d,c,{width:170})}return'<div style="width:140px;height:200px;background:#333;border-radius:12px"></div>'}function yr(e){var t,i;if((t=e==null?void 0:e.rates)!=null&&t.length){const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;align-items:center;justify-content:center;z-index:4000;padding:16px";const o={normal:"#ccc",pepite:"#D4A017",pépite:"#D4A017",papyte:"#909090",legende:"#7a28b8",légende:"#7a28b8"},r={player:"Joueur",formation:"Formation",game_changer:"Game Changer",game_helper:"Game Helper"};n.innerHTML=`
      ...
      </div>`,document.body.appendChild(n),n.addEventListener("click",a=>{a.target===n&&n.remove()}),(i=document.getElementById("odds-close"))==null||i.addEventListener("click",()=>n.remove());return}hr()}function hr(){const e=document.createElement("div");e.style.cssText=`position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}function vr(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText=`
    ...
    `,document.body.appendChild(t);let i=5;const n=setInterval(()=>{i--;const o=document.getElementById("mw-ad-cd");o&&(o.textContent=i),i<=0&&(clearInterval(n),t.remove(),e(!0))},1e3)})}async function wr(e,{state:t,navigate:i,toast:n,refreshProfile:o}){var b,x;const{data:r}=await v.from("users").select("*").eq("id",t.user.id).single();r&&(t.profile=r);let a=Array.isArray((b=t.profile)==null?void 0:b.pending_boosters)?[...t.profile.pending_boosters]:[];if(!a.length){await v.from("users").update({onboarding_done:!0}).eq("id",t.user.id),i("home");return}let s=null;try{const p=(await Hn()).find(u=>(u.name||"").toLowerCase().includes("new player"));p&&(s=Kn(p))}catch(g){console.warn('[Onboarding] Config "Booster (new player)" introuvable, fallback taux par défaut',g)}const d=a.length;let c=0;e.innerHTML=`
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(g=>{g.addEventListener("click",()=>{const p=g.dataset.id,u=r.find(w=>w.id===p);if(!u)return;const y=a.findIndex(w=>w.gc_type===u.gc_type);y>-1?a.splice(y,1):a.length<3&&a.push(u),c()})}),(f=e.querySelector("#gc-launch"))==null||f.addEventListener("click",()=>{l&&d(a)}),(b=e.querySelector("#gc-no-gc"))==null||b.addEventListener("click",()=>d([])),(x=e.querySelector("#gc-reset"))==null||x.addEventListener("click",()=>{a.length&&(a=[],c())})}c()}function $r(e,t){var o;const i=((o=t==null?void 0:t.state)==null?void 0:o.params)||{},n=e||i.matchMode||"vs_ai_easy";return n==="friend"?`Match vs ${i.friendName||"Ami"}`:n==="random"?"Match vs Random":n==="ranked"?"Match Classé":n==="mini_league"||n==="mini-league"?"🏆 Match de Mini League":`Match vs IA — ${n.replace("vs_ai_","").toUpperCase()}`}async function Er(e,t,i){const{state:n,navigate:o}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:r}=await v.from("decks").select(`id,name,is_active,formation,stadium_card_id,
      stadium_card:cards!stadium_card_id(id,stadium_id,
        stadium_def:stadium_definitions(id,name,club_id,country_code,image_url,
          club:clubs(encoded_name,logo_url)))`).eq("owner_id",n.profile.id).order("created_at",{ascending:!1});if(!r||r.length===0){Tt(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>o("decks"));return}const a=r.map(b=>b.id),{data:s}=await v.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),d=[...new Set((s||[]).filter(b=>{var x,g;return((x=b.card)==null?void 0:x.card_type)==="stadium"&&((g=b.card)==null?void 0:g.stadium_id)}).map(b=>b.card.stadium_id))],c={};if(d.length){const{data:b}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",d);(b||[]).forEach(x=>{c[x.id]=x}),(s||[]).forEach(x=>{var g,p;((g=x.card)==null?void 0:g.card_type)==="stadium"&&((p=x.card)!=null&&p.stadium_id)&&(x.card._stadiumDef=c[x.card.stadium_id]||null)})}let l=0;function f(){var m,M,k,I,B,X,te;const b=r[l],x=(s||[]).filter(Z=>Z.deck_id===b.id),g=x.filter(Z=>{var E;return Z.is_starter&&((E=Z.card)==null?void 0:E.player)}).map(Z=>Di(Z.card,Z.position)),p=x.find(Z=>{var E;return((E=Z.card)==null?void 0:E.card_type)==="formation"}),u=b.formation||((m=p==null?void 0:p.card)==null?void 0:m.formation)||"4-4-2";let y=g.length>=11?xi(g,u):null,w=((M=b.stadium_card)==null?void 0:M.stadium_def)||null;w&&y&&(y=ai(y,w));const h=g.length>=11;st(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <div id="deck-top-bar" style="flex-shrink:0">
        <div style="padding:8px 16px;background:rgba(0,0,0,0.4);text-align:center">
          <div style="font-size:10px;opacity:.6;letter-spacing:2px;text-transform:uppercase">${$r(i,t)}</div>
      ...
      <!-- Pagination -->
      ${r.length>1?`<div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">${r.map((Z,E)=>`<div style="width:6px;height:6px;border-radius:50%;background:${E===l?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}</div>`:""}
    ...
    </div>`,requestAnimationFrame(()=>requestAnimationFrame(function Z(){const E=e.querySelector(".deck-preview-wrap"),L=e.querySelector("#deck-swipe-zone");if(!E||!L||!y)return;const K=L.clientWidth>=900,Y=Math.max(200,L.clientHeight-(K?60:40)),fe=Math.max(200,L.clientWidth-16),C=K?Math.min(117,Math.max(52,Math.round(fe*.22))):Math.max(44,Math.round(fe*.168));if(Y<220||fe<220){requestAnimationFrame(Z);return}const W=K?null:Math.round(C*.55);E.innerHTML=Ot(y,u,null,[],fe,Y,[],W),E.style.cssText=`width:${fe}px;height:${Y}px;overflow:hidden;margin:${K?0:60}px auto 0`;const ue=E.querySelector("svg");ue&&(ue.style.cssText="display:block;width:100%;height:100%",ue.setAttribute("preserveAspectRatio",K?"xMidYMid meet":"none"))})),(I=document.getElementById("prev-deck"))==null||I.addEventListener("click",()=>{l>0&&(l--,f())}),(B=document.getElementById("next-deck"))==null||B.addEventListener("click",()=>{l<r.length-1&&(l++,f())}),(X=document.getElementById("validate-deck"))==null||X.addEventListener("click",()=>{if(!h)return;const Z=t.state.params||{};t.navigate("match",{...Z,matchMode:Z.matchMode||i,deckId:b.id})}),(te=document.getElementById("cancel-deck-select"))==null||te.addEventListener("click",()=>{it(e),o("home")});const $=document.getElementById("deck-swipe-zone");if($){let Z=0,E=0;$.addEventListener("touchstart",L=>{Z=L.touches[0].clientX,E=L.touches[0].clientY},{passive:!0}),$.addEventListener("touchend",L=>{const K=L.changedTouches[0].clientX-Z,Y=L.changedTouches[0].clientY-E;Math.abs(K)<40||Math.abs(K)<Math.abs(Y)||(K<0&&l<r.length-1?(l++,f()):K>0&&l>0&&(l--,f()))},{passive:!0})}}f()}function vt(e,t=44,i=58,n=null){return Te(e,{width:t,showStad:!!n,stadDef:n,used:e==null?void 0:e.used})}function Je(e,t,i,n,o){if(!(e!=null&&e.length))return"";const r=e.slice(0,5);let a='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return r.forEach((s,d)=>{const c=s._line||s.job||"MIL";let l=s.boost||0;if(s.stadiumBonus&&(n==="attack"&&(c==="ATT"||c==="MIL")||n==="defense"&&(c==="GK"||c==="DEF"||c==="MIL")?l+=10:n||(l+=10)),a+=Te(s,{width:40,role:c,extraNote:l}),d<r.length-1){const f=r[d+1],b=Rn(s,f,o)?Ut(s,f):null;a+=`<div style="width:7px;height:3px;background:${!b||b==="#ff3333"||b==="#cc2222"?"rgba(255,255,255,0.12)":b};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(a+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),a+="</div>",a}function _i(e,t,i,n,o=310,r=310,a=[],s=null){const d=Mt[t]||{},c=Oi(t)||Hi[t]||[],l={},f=["ATT","MIL","DEF","GK"];for(const y of f)(e[y]||[]).forEach((h,$)=>{l[`${y}${$+1}`]=h});function b(y){const w=d[y];return w?{x:w.x*o,y:w.y*r}:null}let x="";for(const[y,w]of c){const h=b(y),$=b(w);if(!h||!$)continue;const m=l[y],M=l[w],k=Ut(m,M);k==="#00ff88"||k==="#FFD700"?(x+=`<line x1="${h.x.toFixed(1)}" y1="${h.y.toFixed(1)}" x2="${$.x.toFixed(1)}" y2="${$.y.toFixed(1)}"
        stroke="${k}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,x+=`<line x1="${h.x.toFixed(1)}" y1="${h.y.toFixed(1)}" x2="${$.x.toFixed(1)}" y2="${$.y.toFixed(1)}"
        stroke="${k}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):x+=`<line x1="${h.x.toFixed(1)}" y1="${h.y.toFixed(1)}" x2="${$.x.toFixed(1)}" y2="${$.y.toFixed(1)}"
        stroke="${k}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=typeof window<"u"&&window.innerWidth>=900?Math.min(Math.max(81,Math.round(o*.225)),117):Math.max(44,Math.round(o*.168)),p=Math.round(g*657/507);for(const[y,w]of Object.entries(l)){const h=b(y);if(!h||!w)continue;const $=y.replace(/[0-9]/g,""),m=a.includes(w.cardId),M=i==="attack"&&(["MIL","ATT"].includes($)||m)&&!w.used||i==="defense"&&["GK","DEF","MIL"].includes($)&&!w.used,k=n.includes(w.cardId);let I=w.boost||0,B=!1;w.stadiumBonus&&(i==="attack"&&($==="ATT"||$==="MIL")||i==="defense"&&($==="GK"||$==="DEF"||$==="MIL")?(I+=10,B=!0):i||(I+=10,B=!0));const X=Math.round(h.x-g/2),te=Math.round(h.y-p/2);if(w.used){x+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${X}" y="${te}" width="${g}" height="${p}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${w.cardId}" data-role="${$}" style="cursor:pointer"/>`;continue}const Z=Te({...w,_evolution_bonus:0,stadiumBonus:!1},{width:g,showStad:!1,stadDef:null,role:$,extraNote:I,_cardOffset:30,_forceStadColor:B}),E=k?`position:absolute;top:30px;left:0;width:${g}px;height:${p}px;outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;pointer-events:none;`:"";x+=`<foreignObject x="${X-2}" y="${te-30}" width="${g+8}" height="${p+60}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="position:relative">
        ${Z}
        ${k?`<div style="${E}"></div>`:""}
      </div>
    </foreignObject>`,M&&(x+=`<rect x="${X}" y="${te}" width="${g}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${k?"selected":""}" data-card-id="${w.cardId}" data-role="${$}" style="cursor:pointer"/>`)}const u=s!==null?s:Math.round(Math.max(g*.7,80));return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-u} ${-u} ${o+u*2} ${r+u*2}" width="100%" style="display:block;width:100%;margin:0 auto">
    ${x}
  </svg>`}function Ot(e,t,i,n,o=300,r=300,a=[],s=null){return`<div id="match-terrain-wrap" style="position:relative;padding:0 4px">
    ${_i(e,t,i,n,o,r,a,s)}
  </div>`}async function ki(e,t,i,n){var M;const{state:o,navigate:r,toast:a}=t;st(e);const s=o.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!s.deckId)return Er(e,t,i);const d=s.deckId;let c,l,f,b;try{const k=await Promise.all([v.from("decks").select("formation,name,stadium_card_id").eq("id",d).single(),v.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_real,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",d).order("slot_order")]);c=k[0].data,f=k[0].error,l=k[1].data,b=k[1].error}catch(k){console.error("[Match] Exception chargement deck:",k),Tt(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>r("home"));return}if(f||b){console.error("[Match] Erreur Supabase:",f||b),Tt(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>r("home"));return}const x=(l||[]).filter(k=>{var I;return k.is_starter&&((I=k.card)==null?void 0:I.player)}).map(k=>Di(k.card,k.position)),g=(l||[]).filter(k=>{var I;return!k.is_starter&&((I=k.card)==null?void 0:I.player)}).map(k=>Di(k.card,k.position));if(x.length<11){Tt(e,"⚠️",`Deck incomplet (${x.length}/11).`,"Compléter",()=>r("decks"));return}const p=(l||[]).find(k=>{var I;return((I=k.card)==null?void 0:I.card_type)==="formation"}),u=(c==null?void 0:c.formation)||((M=p==null?void 0:p.card)==null?void 0:M.formation)||"4-4-2",{data:y,error:w}=await v.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",o.profile.id).eq("card_type","game_changer"),{data:h}=await v.from("gc_definitions").select("*").eq("is_active",!0),$=(y||[]).map(k=>({...k,_gcDef:(h==null?void 0:h.find(I=>I.name===k.gc_type||I.id===k.gc_definition_id))||null}));let m=null;if(c!=null&&c.stadium_card_id){const{data:k}=await v.from("cards").select("stadium_id").eq("id",c.stadium_card_id).single();if(k!=null&&k.stadium_id){const{data:I}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",k.stadium_id).single();m=I||null}}n({deckId:d,formation:u,starters:x,subsRaw:g,gcCardsEnriched:$,gcDefs:h||[],stadiumDef:m})}function It(){return Math.min(window.innerWidth-40,860)}function Rt(){return Math.round(It()*1.1)}function Qn(e){var o,r;if(!e)return null;const t=e._line||e.job||"MIL",i=t==="GK"?e.note_g||0:t==="DEF"?e.note_d||0:t==="MIL"?e.note_m||0:e.note_a||0,n=e.stadiumBonus?10:0;return{name:e.name,firstname:e.firstname||"",note:i+(e.boost||0)+n,note_g:e.note_g||0,note_d:e.note_d||0,note_m:e.note_m||0,note_a:e.note_a||0,_evolution_bonus:0,stadiumBonus:e.stadiumBonus||!1,boost:e.boost||0,job:e.job,job2:e.job2||null,_line:e._line||e.job,_col:e._col,country_code:e.country_code,club_id:e.club_id,rarity:e.rarity,clubName:e.clubName||((o=e.clubs)==null?void 0:o.encoded_name)||null,clubLogo:e.clubLogo||((r=e.clubs)==null?void 0:r.logo_url)||null,face:e.face||null,portrait:At(e)}}function Lr(e,t,i,n="Adversaire"){Mn("/sounds/match-opening.mp3",.3);const o=It(),r=Rt();return`
    ...
    </div>`}function Zn(e){var r,a,s;if(!e)return"";const t=d=>d?Te({...d,_evolution_bonus:0},{width:52,role:d._line||d.job,showStad:!!d.stadiumBonus,extraNote:d.boost||0}):"",n={goal:"⚽","goal-home":"⚽","goal-ai":"⚽",duel:"⚔️",midfield:"🎯",sub:"🔄",gc:"⚡",boost:"💥","defense-won":"🛡️","attack-won":"⚔️","defense-lost":"😓","attack-lost":"😓"}[e.type]||"📋";return`
    ...
    <div style="animation:goalPop 0.4s ease-out;text-align:center">
      ...
      }`,document.head.appendChild(d)}setTimeout(()=>{a.style.opacity="0",a.style.transition="opacity 0.4s",setTimeout(()=>{a.remove(),o==null||o()},400)},1800)}function zr(e,t,i){const n=document.getElementById("sub-anim-overlay");n&&n.remove();const o=document.createElement("div");o.id="sub-anim-overlay",o.style.cssText=`
    position:fixed;inset:0;z-index:3000;
    display:flex;flex-direction:column;align-items:center;justify-content:center;
    background:rgba(0,0,0,0.8);pointer-events:none`;const r=Math.min(120,Math.round(window.innerWidth/4)),a=e?Te({...e,_evolution_bonus:0},{width:r,role:e._line||e.job}):"",s=t?Te({...t,_evolution_bonus:0},{width:r,role:t._line||t.job}):"";o.innerHTML=`
    ...
    pointer-events:none;white-space:nowrap;max-width:90vw;text-align:center`,o.textContent=e,!document.getElementById("game-toast-style")){const r=document.createElement("style");r.id="game-toast-style",r.textContent=`
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${e.map((r,a)=>{const s=a<e.length-1?Ut(r,e[a+1]):null,d=!s||s==="#ff3333"||s==="#cc2222",c=s==="#00ff88"?"+10":s==="#FFD700"?"+5":"";return eo(r,o),r.stadiumBonus||o&&(o.club_id&&String(r.club_id)===String(o.club_id)||o.country_code&&(r.country_code,o.country_code)),`
          <div class="duel-card duel-card-${n}" data-idx="${a}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease, transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
            ${Te({...r,_evolution_bonus:0},{width:window.innerWidth>=900?Math.min(130,Math.max(80,Math.round(window.innerWidth*.08))):58,showStad:!0,stadDef:o,role:"MIL",extraNote:r.boost||0})}
          </div>
    ...
    </div>`}async function Tr(e,t){const{state:i}=t,o=(i.params||{}).matchMode||"vs_ai_easy",r=o.replace("vs_ai_",""),a=o;await ki(e,t,o,async({deckId:s,formation:d,starters:c,subsRaw:l,gcCardsEnriched:f,gcDefs:b,stadiumDef:x})=>{try{let g=xi(c,d);x&&(g=ai(g,x),gi(l,x));const p=await Ir(d,r),u=p.lines||p,y=async w=>{try{const h=a==="vs_ai_club"?"club":a,{data:$,error:m}=await v.from("matches").insert({home_id:i.profile.id,away_id:null,mode:h,home_deck_id:s,status:"in_progress"}).select().single();if(m){console.error("[MatchIA] Erreur création match:",m),Tt(e,"⚠️","Impossible de créer le match ("+m.message+").","Retour",()=>t.navigate("home"));return}const M=p.stadiumDef||null;M&&u&&(ai(u,M),gi(p.subs||[],M));const k={gcDefs:b||[],matchId:$==null?void 0:$.id,mode:a,difficulty:r,formation:d,homeTeam:g,aiTeam:u,homeSubs:l,subsUsed:0,maxSubs:Math.min(l.length,3),aiSubs:p.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((p.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:p.gcCards||[],aiUsedGc:[],aiStadiumDef:M,homeScore:0,aiScore:0,gcCards:w,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Ar(e,k,t)}catch(h){console.error("[MatchIA] Exception launchMatch:",h),Tt(e,"⚠️","Erreur au lancement du match : "+h.message,"Retour",()=>t.navigate("home"))}};if(!f.length){y([]);return}wi(e,f,y)}catch(g){console.error("[MatchIA] Exception setup:",g),Tt(e,"⚠️","Erreur de préparation du match : "+g.message,"Retour",()=>t.navigate("home"))}})}async function Ir(e,t){var u;const{data:i}=await v.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:Sr(e),subs:[],gcCards:[],stadiumDef:null};const n=Nt[e]||Nt["4-4-2"],o={GK:[],DEF:[],MIL:[],ATT:[]},r=new Set;function a(y,w,h){var $,m;return r.add(y.id),{cardId:"ai-"+y.id+"-"+h,id:y.id,firstname:y.firstname,name:y.surname_real,country_code:y.country_code,club_id:y.club_id,job:y.job,job2:y.job2,note_g:Number(y.note_g)||0,note_d:Number(y.note_d)||0,note_m:Number(y.note_m)||0,note_a:Number(y.note_a)||0,rarity:y.rarity,skin:y.skin,hair:y.hair,hair_length:y.hair_length,clubName:(($=y.clubs)==null?void 0:$.encoded_name)||null,clubLogo:((m=y.clubs)==null?void 0:m.logo_url)||null,boost:0,used:!1,_line:w}}for(const y of["GK","DEF","MIL","ATT"]){const w=i.filter(k=>k.job===y&&!r.has(k.id)),h=i.filter(k=>k.job!==y&&!r.has(k.id)),$=[...w,...h],m=[];for(let k=0;k<n[y];k++){const I=$[k];I&&m.push(a(I,y,k))}const M=xt(m.length);m.forEach((k,I)=>{k._col=M[I]}),o[y]=m}const d=i.filter(y=>!r.has(y.id)).slice(0,5).map((y,w)=>a(y,y.job,100+w)),f=Object.keys(je).sort(()=>Math.random()-.5).slice(0,3).map((y,w)=>{var h,$;return{id:"ai-gc-"+w,gc_type:y,name:((h=je[y])==null?void 0:h.name)||y,icon:(($=je[y])==null?void 0:$.icon)||"⚡"}}),b=Object.values(o).flat(),x={};b.forEach(y=>{y.club_id&&(x[y.club_id]=(x[y.club_id]||0)+1)});const g=(u=Object.entries(x).sort((y,w)=>w[1]-y[1])[0])==null?void 0:u[0];let p=null;if(g){const{data:y}=await v.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",g).single();y&&(p={club_id:y.id,country_code:null,name:y.encoded_name+" Stadium",club:{encoded_name:y.encoded_name,logo_url:y.logo_url}})}return{lines:o,subs:d,gcCards:f,stadiumDef:p}}function Sr(e){const t=Nt[e]||Nt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let o=0;for(const r of["GK","DEF","MIL","ATT"]){const a=[];for(let d=0;d<t[r];d++){const c=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+o,id:"fake-"+o,firstname:"IA",name:n[o%n.length],country_code:"XX",club_id:null,job:r,job2:null,note_g:r==="GK"?c:2,note_d:r==="DEF"?c:2,note_m:r==="MIL"?c:2,note_a:r==="ATT"?c:2,rarity:"normal",boost:0,used:!1,_line:r}),o++}const s=xt(a.length);a.forEach((d,c)=>{d._col=s[c]}),i[r]=a}return i}function Ar(e,t,i){var o;Mn("/sounds/match-opening.mp3",.3);const n=`<div style="position:relative;width:22px;height:22px;flex-shrink:0;display:flex;align-items:center;justify-content:center">
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const W=t.pendingAttack;let ue="";if(t.selected.length>0){const U=t.selected.map(z=>({...(t.homeTeam[z._role]||[]).find(H=>H.cardId===z.cardId)||z,_line:z._role})),V=ri(U,t.modifiers.home,t.formation);ue=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
              ...
              <div style="display:flex;justify-content:center">${Je(U.map(z=>({...z,used:!1})),"#3a7bd5",V.total,"defense",t.formation)}</div>
            ...
            <div style="display:flex;justify-content:center">${Je((W.players||[]).map(U=>({...U,used:!1})),"#ff6b6b",W.total,"attack",t.formation)}</div>
            ${ue}
            ...
            <div style="display:flex;justify-content:center">${Je((W.players||[]).map(ue=>({...ue,used:!1})),"#00ff88",W.total,"attack",t.formation)}</div>
          </div>`}if(t.phase==="attack"&&t.selected.length>0){const W=t.selected.map(U=>{const V=(t.homeTeam[U._role]||[]).find(F=>F.cardId===U.cardId)||U,z=["GK","DEF"].includes(U._role);return{...V,_line:U._role,...z?{note_a:Math.max(1,Number(V.note_a)||0)}:{}}}),ue=oi(W,t.modifiers.home,t.formation);return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #FFD700;text-align:center">
            ...
            <div style="display:flex;justify-content:center">${Je(W.map(U=>({...U,used:!1})),"#FFD700",ue.total,"attack",t.formation)}</div>
          </div>`}const C=t.log[t.log.length-1];return C?'<div style="padding:2px 4px">'+Zn(C)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    ${(()=>{const C=window.innerWidth>=700,W=(q,G,se)=>{var Se,Be;const de=(t.gcDefs||[]).find(ke=>ke.name===q.gc_type),ve=(de==null?void 0:de.name)||q.gc_type,ze=(de==null?void 0:de.effect)||((Se=je[q.gc_type])==null?void 0:Se.desc)||"",he=de!=null&&de.image_url?`/icons/${de.image_url}`:null,Ie=((Be=je[q.gc_type])==null?void 0:Be.icon)||"⚡";return`<div class="gc-mini" data-gc-id="${q.id}" data-gc-type="${q.gc_type}" style="flex-shrink:0;cursor:pointer">
          ${tt(ve,he,Ie,ze,{width:G})}
          ...
          </div>`},U=(q,G)=>G?ue(95,162):W(q,95),V=(q,G)=>G?ue(68,116):W(q,68),z=C?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",F=w?`<button id="btn-results" style="${z};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:p?`<div style="${z};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:m?`<button id="btn-pass" style="${z};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER (plus d'attaquants)</button>`:u?`<button id="btn-action" style="${z};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:y?`<button id="btn-action" style="${z};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${z};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,H=u||y?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",N=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${C?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
        ...
        <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
          ${Ot(t.homeTeam,t.formation,t.phase,n,C?1300:It(),C?600:Rt(),g)}
        </div>
      </div>`;return C?`
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(C=>{var W,ue,U;if(C.type==="duel"){const V=C.isGoal,z=C.homeScored?"#FFD700":V?"#ff6b6b":"rgba(255,255,255,0.3)",F=C.homeScored?"⚽ BUT !":V?"⚽ BUT IA !":(W=C.homePlayers)!=null&&W.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${V?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${z};margin-bottom:4px">
                <div style="font-size:9px;color:${z};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${F}</div>
  ...
  </div>`;function I(){const C=e.querySelector(".match-screen");if(!C)return;const W=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);C.style.bottom="auto",C.style.height=W+"px",C.style.minHeight=W+"px",C.style.maxHeight=W+"px",C.style.overflow="hidden";const ue=e.querySelector("#mobile-action-bar"),U=e.querySelector("#mobile-play-area");ue&&U&&(U.style.paddingBottom=ue.offsetHeight+"px")}if(I(),setTimeout(I,120),setTimeout(I,400),setTimeout(I,1e3),t._vvBound||(t._vvBound=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",I),window.visualViewport.addEventListener("scroll",I)),window.addEventListener("resize",I)),function(){const W=e.querySelector("#match-field .terrain-wrapper svg")||e.querySelector(".terrain-wrapper svg");if(!W)return;const ue=W.closest("#match-terrain-wrap");ue&&(ue.style.cssText="position:relative;width:100%;height:100%;padding:0"),W.removeAttribute("width"),W.removeAttribute("height"),W.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",W.setAttribute("preserveAspectRatio","xMidYMid meet")}(),t._resizeBound||(t._resizeBound=!0,window.addEventListener("resize",()=>{const C=e.querySelector(".terrain-wrapper svg");if(C){const W=C.closest("#match-terrain-wrap");W&&(W.style.cssText="position:relative;width:100%;height:100%;padding:0"),C.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0"}})),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),Ue(),t.phase==="attack"||t.phase==="defense"){let C=!1,W=30;const ue=()=>document.getElementById("match-timer"),U=()=>{const V=ue();if(!V)return;const z=String(Math.floor(W/60)).padStart(2,"0"),F=String(W%60).padStart(2,"0");V.textContent=` ${z}:${F}`,V.style.color=C?"#ff2222":"#ff9500",V.style.fontWeight="900"};U(),t._timerInt=setInterval(()=>{if(W--,W<0)if(!C)C=!0,W=15,U(),Cn("/sounds/timer-urgent.mp3",.6);else{clearInterval(t._timerInt),t._timerInt=null,Ue(),t.homeScore=0,t.aiScore=3;const V=document.createElement("div");V.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",V.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(V),setTimeout(()=>{V.remove(),ht(e,t,i)},2500)}else U()},1e3)}(X=document.getElementById("match-quit"))==null||X.addEventListener("click",()=>{it(e),confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,ht(e,t,i))}),(te=document.getElementById("view-ai"))==null||te.addEventListener("click",()=>Or(t,i)),(Z=document.getElementById("toggle-history"))==null||Z.addEventListener("click",()=>{var C;(C=document.getElementById("match-history-panel"))==null||C.classList.add("open")}),(E=document.getElementById("close-history"))==null||E.addEventListener("click",()=>{var C;(C=document.getElementById("match-history-panel"))==null||C.classList.remove("open")}),(L=document.getElementById("btn-action"))==null||L.addEventListener("click",()=>{t.selected.length!==0&&(u?jr(e,t,i):y&&Br(e,t,i))}),(K=document.getElementById("btn-results"))==null||K.addEventListener("click",()=>ht(e,t,i)),(Y=document.getElementById("btn-pass"))==null||Y.addEventListener("click",()=>{if(t.log.push({text:"⏭️ Vous passez votre tour (plus d'attaquants)",type:"info"}),Fi(t.aiTeam,t.homeTeam)){t.log.push({text:"🏁 Plus personne ne peut attaquer — match terminé.",type:"info"}),ht(e,t,i);return}t.phase="ai-attack",_e(e,t,i),setTimeout(()=>en(e,t,i),800)}),e.querySelectorAll(".match-slot-hit").forEach(C=>{C.addEventListener("click",()=>Cr(C,t,e,i))}),e.querySelectorAll(".match-used-hit").forEach(C=>{C.addEventListener("click",()=>Si(e,t,i,null,C.dataset.cardId))}),e.querySelectorAll(".gc-mini").forEach(C=>{u?C.addEventListener("click",()=>Pr(C.dataset.gcId,C.dataset.gcType,e,t,i)):(C.style.opacity="0.35",C.style.cursor="default",C.addEventListener("click",()=>$t("⚡ Les Game Changers ne sont utilisables qu'en attaque","rgba(180,100,0,0.9)")))}),(pe=document.getElementById("boost-card"))==null||pe.addEventListener("click",()=>Nr(e,t,i)),e.querySelectorAll(".sub-btn-col").forEach(C=>{C.addEventListener("click",()=>Si(e,t,i,C.dataset.subId))}),(fe=document.getElementById("sub-btn-main"))==null||fe.addEventListener("click",()=>Si(e,t,i))}function Cr(e,t,i,n){const o=e.dataset.cardId,r=e.dataset.role,a=t.selected.findIndex(s=>s.cardId===o);if(a!==-1)t.selected.splice(a,1);else{if(t.selected.length>=3){n.toast("Maximum 3 joueurs","error");return}const s=(t.homeTeam[r]||[]).find(d=>d.cardId===o);s&&t.selected.push({...s,_role:r,_line:r})}_e(i,t,n)}function Zi(e,t,i){e.matchId&&v.from("matches").update({last_player_id:i}).eq("id",e.matchId).then(()=>{})}function jr(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),Ue(),Zi(t,i,i.state.profile.id);const n=t.selected.map(r=>{const a=(t.homeTeam[r._role]||[]).find(d=>d.cardId===r.cardId)||r,s=["GK","DEF"].includes(r._role);return{...a,_line:r._role,...s?{note_a:Math.max(1,Number(a.note_a)||0)}:{}}}),o=oi(n,t.modifiers.home,t.formation);t.pendingAttack={...o,players:[...n],side:"home"},t.selected.forEach(r=>{const a=(t.homeTeam[r._role]||[]).find(s=>s.cardId===r.cardId);a&&(a.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${o.total} (base ${o.base}${o.links?` +${o.links} liens`:""}) — ${t.selected.map(r=>r.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",_e(e,t,i),setTimeout(()=>Fr(e,t,i),1200)}function Br(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),Ue(),Zi(t,i,i.state.profile.id);const n=t.stadiumDef||null,o=t.selected.map(d=>{const c=(t.homeTeam[d._role]||[]).find(f=>f.cardId===d.cardId)||d,l=c.stadiumBonus||n&&(n.club_id&&String(c.club_id)===String(n.club_id)||n.country_code&&c.country_code===n.country_code)||!1;return{...c,_line:d._role,stadiumBonus:l}}),r=ri(o,t.modifiers.home,t.formation);t.selected.forEach(d=>{const c=(t.homeTeam[d._role]||[]).find(l=>l.cardId===d.cardId);c&&(c.used=!0)});const a=Qi(t.pendingAttack.total,r.total,t.modifiers.home),s={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(d=>Ke(d)),homePlayers:t.selected.map(d=>{const c=(t.homeTeam[d._role]||[]).find(l=>l.cardId===d.cardId)||d;return Ke(c)}),homeTotal:r.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(a.shielded)s.text="🛡️ Bouclier ! But annulé.",t.log.push(s);else if(a.goal){t.aiScore++,s.isGoal=!0,s.homeScored=!1,s.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${r.total})`,t.log.push(s),t.selected=[],t.modifiers.home={},t.pendingAttack=null,_e(e,t,i),Qe(s.aiPlayers,t.homeScore,t.aiScore,!1,()=>{St(e,t,i,"home-attack")});return}else s.text=`🧤 Défense réussie ! (${r.total} ≥ ${t.pendingAttack.total})`,t.log.push(s);t.selected=[],t.modifiers.home={},t.pendingAttack=null,St(e,t,i,"home-attack")}function qr(e){if(e.aiSubsUsed>=e.aiMaxSubs)return;const t=Object.values(e.aiTeam).flat().filter(d=>d.used);if(!t.length)return;const i=(e.aiSubs||[]).filter(d=>!e.aiUsedSubIds.includes(d.cardId));if(!i.length)return;const n=t[Math.floor(Math.random()*t.length)],o=i.find(d=>d.job===n.job)||i[0],r={...o,used:!1,_line:n._line,_col:n._col},a=e.aiTeam[n._line],s=a.findIndex(d=>d.cardId===n.cardId);s!==-1&&(a[s]=r),e.aiUsedSubIds.push(o.cardId),e.aiSubsUsed++,e.log.push({text:`🔄 IA : ${o.firstname} ${o.name} remplace ${n.firstname} ${n.name}`,type:"info"})}function Dr(e){var n;if(!((n=e.aiGcCards)!=null&&n.length))return;const t=e.aiGcCards.filter(o=>!e.aiUsedGc.includes(o.id));if(!t.length||Math.random()>.3)return;const i=t[Math.floor(Math.random()*t.length)];switch(e.aiUsedGc.push(i.id),je[i.gc_type],i.gc_type){case"Boost+2":{const o=Object.values(e.aiTeam).flat().filter(r=>!r.used);if(o.length){const r=o[Math.floor(Math.random()*o.length)];r.boost=(r.boost||0)+2}break}case"Boost+3":{const o=Object.values(e.aiTeam).flat().filter(r=>!r.used);if(o.length){const r=o[Math.floor(Math.random()*o.length)];r.boost=(r.boost||0)+3}break}case"Remplacement+":e.aiMaxSubs=(e.aiMaxSubs||3)+1;break;case"Bouclier":e.modifiers.ai.shield=!0;break;case"Nul+1":e.modifiers.ai.drawBonus=(e.modifiers.ai.drawBonus||0)+1;break}e.log.push({text:`⚡ IA joue ${i.icon||"⚡"} ${i.name}`,type:"gc"})}function en(e,t,i){Zi(t,i,null),qr(t),Dr(t);let n=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(l=>!l.used),o=!1;if(!n.length){const l=[...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(b=>!b.used),f=Ht(t.homeTeam);if(f||l.length===0){const b=(t.aiTeam.DEF||[]).filter(g=>!g.used),x=(t.aiTeam.GK||[]).filter(g=>!g.used);n=f?b.length?b:x:[...b,...x],o=!0}}const r=On(n,"attack",t.difficulty);if(!r.length){Pi(e,t,i);return}o&&r.forEach(l=>{l._line=l._line||l.job,l.note_a=Math.max(1,Number(l.note_a)||0)});const a=oi(r,t.modifiers.ai,t.formation);t.pendingAttack={...a,players:r,side:"ai"},r.forEach(l=>{l.used=!0}),t.log.push({text:`🤖 IA attaque : ${a.total} (${r.map(l=>l.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const s=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(l=>!l.used),c=(t.homeSubs||[]).filter(l=>!(t.usedSubIds||[]).includes(l.cardId)).length>0&&t.subsUsed<t.maxSubs;if(s.length===0&&!c){if(r.length===1&&(r[0]._line==="GK"||r[0].job==="GK")&&Ht(t.homeTeam)&&t.homeScore===t.aiScore){t.aiScore++,yi(e,t,i,"ai",r[0]);return}t.aiScore++;const f={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:r.map(b=>Ke(b)),aiTotal:a.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(f),t.pendingAttack=null,_e(e,t,i),Qe(f.aiPlayers,t.homeScore,t.aiScore,!1,()=>{St(e,t,i,"home-attack")});return}t.phase="defense",_e(e,t,i)}function Fr(e,t,i){var c,l;const n=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],o=On(n,"defense",t.difficulty);if(!["GK","DEF","MIL","ATT"].flatMap(f=>(t.aiTeam[f]||[]).filter(b=>!b.used)).length){const f=((c=t.pendingAttack)==null?void 0:c.players)||[];if(f.length===1&&(f[0]._line==="GK"||f[0].job==="GK")&&t.homeScore===t.aiScore){t.homeScore++,yi(e,t,i,"home",f[0]);return}t.homeScore++;const x={type:"duel",isGoal:!0,homeScored:!0,homePlayers:f.map(g=>Ke(g)),homeTotal:((l=t.pendingAttack)==null?void 0:l.total)||0,aiTotal:0,text:"⚽ BUT ! L'IA n'a plus de joueurs — but automatique !"};t.log.push(x),t.modifiers.ai={},t.pendingAttack=null,_e(e,t,i),Qe(x.homePlayers,t.homeScore,t.aiScore,!0,()=>{St(e,t,i,"ai-attack")});return}const a=o.length>0?ri(o,t.modifiers.ai,t.formation).total:0;o.forEach(f=>{f.used=!0});const s=Qi(t.pendingAttack.total,a,t.modifiers.ai),d={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(f=>Ke(f)),aiPlayers:o.map(f=>Ke(f)),homeTotal:t.pendingAttack.total,aiTotal:a,isGoal:!1,homeScored:!1,text:""};if(s.shielded)d.text="🛡️ Bouclier IA !",t.log.push(d);else if(s.goal){t.homeScore++,d.isGoal=!0,d.homeScored=!0,d.text=`⚽ BUT ! (${t.pendingAttack.total} > ${a})`,t.log.push(d),t.modifiers.ai={},t.pendingAttack=null,_e(e,t,i),Qe(d.homePlayers,t.homeScore,t.aiScore,!0,()=>{St(e,t,i,"ai-attack")});return}else d.text=`🧤 IA défend (${a} ≥ ${t.pendingAttack.total})`,t.log.push(d);t.modifiers.ai={},t.pendingAttack=null,St(e,t,i,"ai-attack")}function St(e,t,i,n){if(t.round++,!to(e,t,i)){if(tn(t)){ht(e,t,i);return}if(n==="home-attack"){if(!["MIL","ATT","GK","DEF"].some(r=>(t.homeTeam[r]||[]).some(a=>!a.used))){Pi(e,t,i);return}t.phase="attack",_e(e,t,i)}else{if(!["MIL","ATT","GK","DEF"].some(r=>(t.aiTeam[r]||[]).some(a=>!a.used))){Pi(e,t,i);return}t.phase="ai-attack",_e(e,t,i),setTimeout(()=>en(e,t,i),800)}}}function Ht(e){return!["GK","DEF","MIL","ATT"].some(t=>(e[t]||[]).some(i=>!i.used))}function $n(e){const t=(e.GK||[]).some(n=>!n.used),i=["DEF","MIL","ATT"].some(n=>(e[n]||[]).some(o=>!o.used));return t&&!i}function to(e,t,i){if(t.homeScore!==t.aiScore)return!1;if($n(t.homeTeam)&&Ht(t.aiTeam)){const n=(t.homeTeam.GK||[]).find(o=>!o.used);return n?(n.used=!0,t.homeScore++,yi(e,t,i,"home",n),!0):!1}if($n(t.aiTeam)&&Ht(t.homeTeam)){const n=(t.aiTeam.GK||[]).find(o=>!o.used);return n?(n.used=!0,t.aiScore++,yi(e,t,i,"ai",n),!0):!1}return!1}function yi(e,t,i,n,o){t.log.push({type:"duel",isGoal:!0,homeScored:n==="home",homePlayers:n==="home"?[Ke(o)]:[],aiPlayers:n==="ai"?[Ke(o)]:[],text:`⚽ DERNIER CORNER — Le gardien ${n==="home"?"":"adverse "}marque !`});const r=document.createElement("div");r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(r);const a=(s,d)=>new Promise(c=>{r.innerHTML=`<div style="font-size:32px;font-weight:900;color:${d};letter-spacing:2px;animation:lcFade 1.4s ease both">${s}</div>
    <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(c,1400)});(async()=>(await a("⚽ DERNIER CORNER","#FFD700"),await a("🧤 LE GARDIEN MONTE !","#4fc3f7"),r.remove(),t.pendingAttack=null,_e(e,t,i),Qe([Ke(o)],t.homeScore,t.aiScore,n==="home",()=>{if(tn(t)){ht(e,t,i);return}St(e,t,i,n==="home"?"ai-attack":"home-attack")})))()}function tn(e){const t=["MIL","ATT","GK","DEF"].some(n=>(e.homeTeam[n]||[]).some(o=>!o.used)),i=["MIL","ATT","GK","DEF"].some(n=>(e.aiTeam[n]||[]).some(o=>!o.used));return!t&&!i}function Fi(e,t){return!(["MIL","ATT"].some(o=>(e[o]||[]).some(r=>!r.used))||Ht(t)||!["MIL","ATT"].some(o=>(t[o]||[]).some(r=>!r.used)))}function Pi(e,t,i){if(!to(e,t,i)){if(tn(t)){ht(e,t,i);return}if(Fi(t.homeTeam,t.aiTeam)&&Fi(t.aiTeam,t.homeTeam)){t.log.push({text:"🏁 Plus personne ne peut attaquer — match terminé.",type:"info"}),ht(e,t,i);return}t.phase="attack",_e(e,t,i)}}function Si(e,t,i,n=null,o=null){var x,g;if(t.phase!=="attack"){$t("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){$t(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const r=Object.entries(t.homeTeam).flatMap(([p,u])=>(u||[]).filter(y=>y.used).map(y=>({...y,_line:y._line||p}))),a=t.homeSubs.filter(p=>!t.usedSubIds.includes(p.cardId));if(!r.length){$t("Aucun joueur utilisé à remplacer");return}if(!a.length){$t("Aucun remplaçant disponible");return}let s=Math.max(0,r.findIndex(p=>p.cardId===o));const d=((x=r[s])==null?void 0:x._line)||((g=r[s])==null?void 0:g.job);let c=n?Math.max(0,a.findIndex(p=>p.cardId===n)):Math.max(0,a.findIndex(p=>p.job===d)),l=!1;const f=document.createElement("div");f.id="sub-overlay",f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function b(){var m,M,k,I,B,X;const p=r[s],u=a[c],y=Math.min(130,Math.round((window.innerWidth-90)/2)),w=Math.round(y*1.35),h=te=>`background:rgba(255,255,255,0.12);border:none;color:${te?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${te?"default":"pointer"};flex-shrink:0`;f.innerHTML=`
    ...
    </div>`,(m=f.querySelector("#sub-close"))==null||m.addEventListener("click",()=>f.remove()),(M=f.querySelector("#out-up"))==null||M.addEventListener("click",()=>{s>0&&(s--,b())}),(k=f.querySelector("#out-down"))==null||k.addEventListener("click",()=>{s<r.length-1&&(s++,b())}),(I=f.querySelector("#in-up"))==null||I.addEventListener("click",()=>{c>0&&(c--,b())}),(B=f.querySelector("#in-down"))==null||B.addEventListener("click",()=>{c<a.length-1&&(c++,b())});const $=(te,Z,E,L)=>{const K=f.querySelector("#"+te);if(!K)return;let Y=0;K.addEventListener("touchstart",pe=>{Y=pe.touches[0].clientY},{passive:!0}),K.addEventListener("touchend",pe=>{const fe=pe.changedTouches[0].clientY-Y;if(Math.abs(fe)<30)return;const C=Z();fe<0&&C<L-1?(E(C+1),b()):fe>0&&C>0&&(E(C-1),b())},{passive:!0})};$("in-panel",()=>c,te=>c=te,a.length),$("out-panel",()=>s,te=>s=te,r.length),(X=f.querySelector("#sub-confirm"))==null||X.addEventListener("click",te=>{if(te.preventDefault(),te.stopPropagation(),l)return;l=!0;const Z=r[s],E=a[c];if(!Z||!E)return;let L=null,K=-1;for(const[pe,fe]of Object.entries(t.homeTeam)){const C=(fe||[]).findIndex(W=>W.cardId===Z.cardId);if(C!==-1){L=pe,K=C;break}}if(K===-1||!L){$t("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),f.remove();return}const Y={...E,_line:L,_col:Z._col||0,used:!1,boost:0};t.homeTeam[L].splice(K,1,Y),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(E.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:Z.name,firstname:Z.firstname,note:Pe(Z,L),portrait:At(Z),job:Z.job,country_code:Z.country_code,rarity:Z.rarity,clubName:Z.clubName,clubLogo:Z.clubLogo},inPlayer:{name:E.name,firstname:E.firstname,note:Pe(E,L),portrait:At(E),job:E.job,country_code:E.country_code,rarity:E.rarity,clubName:E.clubName,clubLogo:E.clubLogo},text:`🔄 ${E.firstname} ${E.name} remplace ${Z.firstname} ${Z.name}`}),f.remove(),zr(Z,E,()=>_e(e,t,i))})}document.body.appendChild(f),b()}function Pr(e,t,i,n,o){var b,x;const r=(n.gcDefs||[]).find(g=>g.name===t),a=je[t]||{icon:"⚡",desc:"Carte spéciale."},s=(r==null?void 0:r.name)||t,d=(r==null?void 0:r.effect)||a.desc,c=r!=null&&r.image_url?`/icons/${r.image_url}`:null,l=a.icon||"⚡",f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",f.innerHTML=`
    ${tt(s,c,l,d,{width:190})}
    <!-- Boutons -->
    <div style="display:flex;gap:12px;width:190px">
      <button id="gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
      <button id="gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
    </div>`,document.body.appendChild(f),(b=f.querySelector("#gc-back"))==null||b.addEventListener("click",()=>f.remove()),(x=f.querySelector("#gc-use"))==null||x.addEventListener("click",()=>{f.remove(),Rr(e,t,i,n,o)})}function pi(e,t,i,n,o,r){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let s=[];function d(){var c,l;a.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start;justify-content:center">
      ${e.map(f=>{const b=f._line||f.job||"MIL",x=s.find(p=>p.cardId===f.cardId),g=Te({...f,_evolution_bonus:0},{width:90,showStad:!0,role:b,extraNote:f.boost||0});return`<div class="gc-pick-item" data-cid="${f.cardId}"
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ${n.map(o=>`
        <div class="player-boost-opt" data-card-id="${o.cardId}"
          ...
          <div style="width:32px;height:32px;background:${kr[o.job]||"#888"};border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:13px">${Pe(o,o._line||o.job)}</div>
          <div style="flex:1"><b>${o.firstname} ${o.name}</b><div style="font-size:11px;color:#888">${o._line||o.job}</div></div>
          <div style="color:#87CEEB;font-weight:700">+${t.boostCard.value}</div>
        </div>`).join("")}
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(o=>{o.addEventListener("click",()=>{const r=o.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const s=(t.homeTeam[a]||[]).find(d=>d.cardId===r);if(s){s.boost=(s.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${s.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),_e(e,t,i)})})}async function ht(e,t,i){var l,f;zt(),Ue(),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,o=t.homeScore>t.aiScore,r=t.homeScore===t.aiScore,a=o?"victoire":r?"nul":"defaite",s=Yo(t.mode,a);t.matchId&&await v.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:o?n.profile.id:null,home_credits_reward:s,played_at:new Date().toISOString()}).eq("id",t.matchId);const d={credits:(n.profile.credits||0)+s,matches_played:(n.profile.matches_played||0)+1};o?d.wins=(n.profile.wins||0)+1:r?d.draws=(n.profile.draws||0)+1:d.losses=(n.profile.losses||0)+1,await v.from("users").update(d).eq("id",n.profile.id),await i.refreshProfile();const c=document.createElement("div");c.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",c.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
    ...
    </div>`,document.body.appendChild(c),(l=document.getElementById("res-home"))==null||l.addEventListener("click",()=>{c.remove(),it(e),i.navigate("home")}),(f=document.getElementById("res-replay"))==null||f.addEventListener("click",()=>{c.remove(),it(e),i.navigate("match",{matchMode:t.mode})})}function Or(e,t){t.openModal("Équipe adverse (IA)",`<div style="background:#0a3d1e;padding:12px;border-radius:8px">
      ${_i(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Hr={pepite:{win:2,loss:-1,boosterStart:"min"},papyte:{win:1,loss:-2,boosterStart:"max"}};function Ur(e){const t=e.job||"ATT";return Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}async function Kr(e,t){!e||!t||await Promise.all([En(e,"win"),En(t,"loss")])}async function En(e,t){const{data:i}=await v.from("cards").select("id, current_note, player:players(rarity, job, note_g, note_d, note_m, note_a, note_min, note_max)").eq("owner_id",e).eq("card_type","player");if(!(i!=null&&i.length))return;const n=i.filter(o=>{var r,a;return((r=o.player)==null?void 0:r.rarity)==="pepite"||((a=o.player)==null?void 0:a.rarity)==="papyte"});n.length&&await Promise.all(n.map(o=>{const r=Hr[o.player.rarity],a=t==="win"?r.win:r.loss,s=o.player.note_min??1,d=o.player.note_max??10,c=Ur(o.player),l=o.current_note??c,f=Math.min(d,Math.max(s,l+a));return v.from("cards").update({current_note:f}).eq("id",o.id)}))}const Dt=e=>Qn(e);async function Ct(e,t,i,n,o={}){return io(e,t,i,n,o.myGC||[],o.gcDefs||[],o.isRanked||!1,o.rankedData||null,o.stadiumDef||null,o.onMatchEnd||null,o.mlLeagueId||null,o.mlMatchId||null)}async function Vr(e,t,i){const{data:n}=await v.from("matches").select("home_id,away_id,mode,is_ranked").eq("id",i).single();if(!n){t.toast("Match introuvable","error"),t.navigate("home");return}const o=n.home_id===t.state.user.id;let r=null,a=null;if(n.mode==="mini_league"){const{data:s}=await v.from("mini_league_matches").select("id, league_id").eq("match_id",i).maybeSingle();s&&(r=s.league_id,a=s.id)}return io(e,t,i,o,[],[],n.is_ranked||!1,null,null,null,r,a)}async function io(e,t,i,n,o=[],r=[],a=!1,s=null,d=null,c=null,l=null,f=null){const{state:b,navigate:x,toast:g}=t,p=n?"p1":"p2",u=n?"p2":"p1",y=(o||[]).map(_=>_.id),w=(o||[]).map(_=>({id:_.id,gc_type:_.gc_type,_gcDef:_._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:h}=await v.from("matches").select("*").eq("id",i).single();if(!h){g("Match introuvable","error"),x("home");return}async function $(){var xe,ye;console.log("[PvP] buildGameState match:",{id:h.id,home_deck_id:h.home_deck_id,away_deck_id:h.away_deck_id,mode:h.mode,is_ranked:h.is_ranked});const[{data:_,error:T},{data:A,error:j},{data:S},{data:R}]=await Promise.all([v.rpc("get_deck_for_match",{p_deck_id:h.home_deck_id}),v.rpc("get_deck_for_match",{p_deck_id:h.away_deck_id}),v.from("users").select("id,pseudo,club_name").eq("id",h.home_id).single(),v.from("users").select("id,pseudo,club_name").eq("id",h.away_id).single()]);console.log("[PvP] get_deck_for_match p1:",T==null?void 0:T.message,"p2:",j==null?void 0:j.message,"p1D:",(xe=_==null?void 0:_.starters)==null?void 0:xe.length,"p2D:",(ye=A==null?void 0:A.starters)==null?void 0:ye.length);const D=oe=>{const Ee=Number(oe.evolution_bonus)||0;return{cardId:oe.card_id,position:oe.position,id:oe.id,firstname:oe.firstname,name:oe.surname_real,country_code:oe.country_code,club_id:oe.club_id,job:oe.job,job2:oe.job2,note_g:(Number(oe.note_g)||0)+(oe.job==="GK"||oe.job2==="GK"&&Number(oe.note_g)>0?Ee:0),note_d:(Number(oe.note_d)||0)+(oe.job==="DEF"||oe.job2==="DEF"&&Number(oe.note_d)>0?Ee:0),note_m:(Number(oe.note_m)||0)+(oe.job==="MIL"||oe.job2==="MIL"&&Number(oe.note_m)>0?Ee:0),note_a:(Number(oe.note_a)||0)+(oe.job==="ATT"||oe.job2==="ATT"&&Number(oe.note_a)>0?Ee:0),evolution_bonus:Ee,rarity:oe.rarity,skin:oe.skin,hair:oe.hair,hair_length:oe.hair_length,face:oe.face||null,clubName:oe.club_encoded_name||null,clubLogo:oe.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},P=((_==null?void 0:_.starters)||[]).map(oe=>D(oe)),O=((A==null?void 0:A.starters)||[]).map(oe=>D(oe)),Q=(_==null?void 0:_.formation)||"4-4-2",ne=(A==null?void 0:A.formation)||"4-4-2";let ie=xi(P,Q),ee=xi(O,ne);const ae=((_==null?void 0:_.subs)||[]).map(oe=>D(oe)),le=((A==null?void 0:A.subs)||[]).map(oe=>D(oe)),ge=(_==null?void 0:_.stadium_def)||(n?d:null),ce=(A==null?void 0:A.stadium_def)||(n?null:d);return ge&&(ie=ai(ie,ge),gi(ae,ge)),ce&&(ee=ai(ee,ce),gi(le,ce)),{p1Team:ie,p2Team:ee,p1Subs:ae,p2Subs:le,p1Formation:Q,p2Formation:ne,p1Name:(S==null?void 0:S.club_name)||(S==null?void 0:S.pseudo)||"Joueur 1",p2Name:(R==null?void 0:R.club_name)||(R==null?void 0:R.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?y:[],gc_p2:n?[]:y,gcCardsFull_p1:n?w:[],gcCardsFull_p2:n?[]:w,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:r||[],lastActionAt:new Date().toISOString()}}let m=h.game_state&&Object.keys(h.game_state).length?h.game_state:null;if(console.log("[PvP] init - amIHome:",n,"gameState exists:",!!m,"match.status:",h.status,"home_id:",h.home_id,"away_id:",h.away_id,"myId:",b.profile.id),!m)if(n){m=await $(),console.log("[PvP] home - gameState built, p1Team keys:",Object.keys((m==null?void 0:m.p1Team)||{}));const{data:_}=await v.from("matches").select("game_state").eq("id",i).single();!(_!=null&&_.game_state)||!Object.keys(_.game_state).length?await v.from("matches").update({game_state:m,turn_user_id:h.home_id}).eq("id",i):m=_.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>',console.log("[PvP] away - waiting for home to write game_state...");for(let _=0;_<30&&!m;_++){await new Promise(A=>setTimeout(A,400));const{data:T}=await v.from("matches").select("game_state").eq("id",i).single();T!=null&&T.game_state&&Object.keys(T.game_state).length&&(m=T.game_state),_%5===0&&console.log("[PvP] away - poll",_,"game_state:",!!(T!=null&&T.game_state))}if(!m){g("Erreur de synchronisation","error"),x("home");return}m.gc_p2=y,m.gcCardsFull_p2=w,await v.from("matches").update({game_state:m}).eq("id",i)}let M=!1,k=null,I=!1;const B=new Set,X=new Set;async function te(_){var ne,ie;zt();try{v.removeChannel(Z)}catch{}const T=m[p+"Score"]||0,A=m[u+"Score"]||0;let j,S;_.winner_id?(j=_.winner_id===b.profile.id,S=!1):_.forfeit?(j=T>A,S=!1):(S=T===A,j=T>A);let R="";if(a&&n)try{const{data:ee}=await v.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",h.home_id).single(),{data:ae}=await v.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",h.away_id).single();if(ee&&ae){const le=j?1:S?.5:0,ge=1-le,ce=ee.placement_matches<10,xe=ae.placement_matches<10,ye=computeGlicko2(ee.mmr,ee.mmr_deviation,ee.mmr_volatility,ae.mmr,ae.mmr_deviation,le===1?1:le===0?0:.5,ce),oe=computeGlicko2(ae.mmr,ae.mmr_deviation,ae.mmr_volatility,ee.mmr,ee.mmr_deviation,ge===1?1:ge===0?0:.5,xe);await v.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:S?null:j?h.home_id:h.away_id,p_home_id:h.home_id,p_away_id:h.away_id,p_home_delta:ye.delta,p_away_delta:oe.delta,p_home_new_rd:ye.newRd,p_away_new_rd:oe.newRd,p_home_new_vol:ye.newSigma,p_away_new_vol:oe.newSigma});const Ee=ye.delta,Re=ye.newMmr,Le=getTier(Re),Ve=Ee>=0?"+":"",Oe=Ee>=0?"#4caf50":"#ff6b6b",nt=getTier(ee.mmr);R=`
            ...
            </div>`}}catch(ee){console.error("[Ranked] MMR update error:",ee)}(ne=document.getElementById("pvp-end-overlay"))==null||ne.remove();const D=document.createElement("div");D.id="pvp-end-overlay",D.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const P=S?"🤝":j?"🏆":"😞",O=S?"MATCH NUL":j?"VICTOIRE !":"DÉFAITE",Q=S?"#fff":j?"#FFD700":"#ff6b6b";D.innerHTML=`
      ...
      ${R}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${l?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(D),(ie=D.querySelector("#pvp-end-home"))==null||ie.addEventListener("click",()=>{D.remove(),it(e),l?x("mini-league",{openLeagueId:l}):x(a?"ranked":"home")})}const Z=v.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},_=>{var A;const T=_.new;try{if(T.status==="finished"||T.forfeit){if(M)return;M=!0,k&&(clearInterval(k),k=null),Ue(),T.game_state&&(m=T.game_state),te(T);return}if(T.game_state){const j=m;m=T.game_state;const S=m._lastGC;if(S&&S.seq&&!X.has(S.seq)&&(X.add(S.seq),S.by!==p)){z(S.type,S.by,()=>W());return}const R=j[p+"Score"]||0,D=j[u+"Score"]||0,P=m[p+"Score"]||0,O=m[u+"Score"]||0,Q=P>R,ne=O>D;if((Q||ne)&&!B.has(m.round)){B.add(m.round);const ie=[...m.log||[]].reverse().find(le=>le.isGoal),ee=((ie==null?void 0:ie.homePlayers)||[]).map(le=>({name:le.name,note:le.note,portrait:le.portrait,job:le.job}));!!((A=ie==null?void 0:ie.text)!=null&&A.includes("DERNIER CORNER"))?ze().then(()=>{Qe(ee,P,O,Q,()=>W())}):Qe(ee,P,O,Q,()=>W());return}W()}}catch(j){console.error("[PvP] crash:",j)}}).subscribe();async function E(_){Object.assign(m,_),m.lastActionAt=new Date().toISOString();const{error:T}=await v.from("matches").update({game_state:m}).eq("id",i);T&&g("Erreur de synchronisation","error");try{W()}catch(A){console.error("[PvP] renderPvpScreen crash:",A)}}async function L(){if(M)return;M=!0,zt(),Ue(),k&&(clearInterval(k),k=null);const _=n?h.away_id:h.home_id,T=n?"p2":"p1",A=n?"p1":"p2",j={...m,[T+"Score"]:3,[A+"Score"]:0,phase:"finished"},S=j.p1Score||0,R=j.p2Score||0;try{await v.from("matches").update({game_state:j}).eq("id",i);const{data:D,error:P}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:_,p_home_score:S,p_away_score:R,p_forfeit:!0});if(P&&console.error("[PvP] finish_pvp_match (forfeit):",P),l)try{await v.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(O){console.error("[PvP] check_and_finish_mini_league (forfeit):",O)}try{const{data:O}=await v.rpc("apply_match_rewards",{p_match_id:i});O!=null&&O.success&&!(O!=null&&O.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(O){console.error("[PvP] apply_match_rewards (forfeit):",O)}}catch{}try{v.removeChannel(Z)}catch{}setTimeout(()=>{it(e),l?x("mini-league",{openLeagueId:l}):x("home")},800)}const K={BOOST_STAT:({value:_=1,count:T=1,roles:A=[]},j,S)=>{const R=j[p+"Team"],D=Object.entries(R).filter(([P])=>!A.length||A.includes(P)).flatMap(([P,O])=>O.filter(Q=>!Q.used).map(Q=>({...Q,_line:P})));if(!D.length){j.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),S(j);return}Y(D,T,`Choisir ${T} joueur(s) à booster (+${_})`,P=>{P.forEach(O=>{const Q=(R[O._line]||[]).find(ne=>ne.cardId===O.cardId);Q&&(Q.boost=(Q.boost||0)+_,j.log.push({text:`⚡ +${_} sur ${Q.name}`,type:"info"}))}),j[p+"Team"]=R,S(j)})},DEBUFF_STAT:({value:_=1,count:T=1,roles:A=[],target:j="ai"},S,R)=>{const D=j==="home"?p:u,P=S[D+"Team"],O=j==="home"?"allié":"adverse",Q=Object.entries(P).filter(([ne])=>!A.length||A.includes(ne)).flatMap(([ne,ie])=>ie.map(ee=>({...ee,_line:ne})));if(!Q.length){S.log.push({text:`🎯 Aucun joueur ${O}`,type:"info"}),R(S);return}Y(Q,T,`Choisir ${T} joueur(s) ${O}(s) (-${_})`,ne=>{ne.forEach(ie=>{const ee=(P[ie._line]||[]).find(ae=>ae.cardId===ie.cardId);ee&&(ee.boost=(ee.boost||0)-_,S.log.push({text:`🎯 -${_} sur ${ee.name}`,type:"info"}))}),S[D+"Team"]=P,R(S)})},GRAY_PLAYER:({count:_=1,roles:T=[],target:A="ai"},j,S)=>{const R=A==="home"?p:u,D=j[R+"Team"],P=A==="home"?"allié":"adverse",O=Object.entries(D).filter(([Q])=>!T.length||T.includes(Q)).flatMap(([Q,ne])=>ne.filter(ie=>!ie.used).map(ie=>({...ie,_line:Q})));if(!O.length){j.log.push({text:`❌ Aucun joueur ${P}`,type:"info"}),S(j);return}Y(O,_,`Choisir ${_} joueur(s) ${P}(s) à exclure`,Q=>{const ne="usedCardIds_"+R,ie=new Set(j[ne]||[]);Q.forEach(ee=>{const ae=(D[ee._line]||[]).find(le=>le.cardId===ee.cardId);ae&&(ae.used=!0,ie.add(ee.cardId),j.log.push({text:`❌ ${ae.name} exclu !`,type:"info"}))}),j[ne]=[...ie],j[R+"Team"]=D,S(j)})},REVIVE_PLAYER:({count:_=1,roles:T=[]},A,j)=>{const S=A[p+"Team"],R=Object.entries(S).filter(([D])=>!T.length||T.includes(D)).flatMap(([D,P])=>P.filter(O=>O.used).map(O=>({...O,_line:D})));if(!R.length){A.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),j(A);return}Y(R,_,`Choisir ${_} joueur(s) à ressusciter`,D=>{D.forEach(P=>{const O=(S[P._line]||[]).find(Q=>Q.cardId===P.cardId);O&&(O.used=!1,A.log.push({text:`💫 ${O.name} ressuscité !`,type:"info"}))}),A[p+"Team"]=S,j(A)})},REMOVE_GOAL:({},_,T)=>{const A=u+"Score";_[A]>0?(_[A]--,_.log.push({text:"🚫 Dernier but annulé !",type:"info"})):_.log.push({text:"🚫 Aucun but à annuler",type:"info"}),T(_)},ADD_GOAL_DRAW:({},_,T)=>{_[p+"Score"]===_[u+"Score"]?(_[p+"Score"]++,_.log.push({text:"🎯 But bonus !",type:"info"})):_.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),T(_)},ADD_SUB:({value:_=1},T,A)=>{T.maxSubs=(T.maxSubs||3)+_,T.log.push({text:`🔄 +${_} remplacement(s)`,type:"info"}),A(T)},CUSTOM:({},_,T)=>T(_)};function Y(_,T,A,j){const S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let R=[];function D(){var O,Q;const P=_.map(ne=>{const ie=ne._line||ne.job||"MIL",ee=R.find(le=>le.cardId===ne.cardId),ae=Te({...ne,_evolution_bonus:0},{width:90,showStad:!0,role:ie,extraNote:ne.boost||0});return`<div class="pp-item" data-cid="${ne.cardId}"
        ...
        </div>`,(O=S.querySelector("#pp-close"))==null||O.addEventListener("click",()=>S.remove()),S.querySelectorAll(".pp-item").forEach(ne=>{ne.addEventListener("click",()=>{const ie=ne.dataset.cid,ee=_.find(le=>le.cardId===ie),ae=R.findIndex(le=>le.cardId===ie);ee&&(ae>-1?R.splice(ae,1):R.length<T&&R.push(ee),D())})}),(Q=S.querySelector("#pp-confirm"))==null||Q.addEventListener("click",()=>{S.remove(),j(R)})}D(),document.body.appendChild(S)}async function pe(_,T){const j=(m["gcCardsFull_"+p]||[]).find(P=>P.id===_),S=(j==null?void 0:j._gcDef)||(m.gcDefs||[]).find(P=>{var O;return P.name===T||((O=P.name)==null?void 0:O.toLowerCase().trim())===(T==null?void 0:T.toLowerCase().trim())}),R=[...m["usedGc_"+p]||[],_],D={type:T,by:p,seq:(m._gcAnimSeq||0)+1};X.add(D.seq),z(T,p,async()=>{if(S!=null&&S.effect_type&&S.effect_type!=="CUSTOM"){const O=K[S.effect_type];if(O){const Q={...m};O(S.effect_params||{},Q,async ne=>{ne["usedGc_"+p]=R,ne._lastGC=D,ne._gcAnimSeq=D.seq,await E(ne)});return}}const P={...m};switch(T){case"Remplacement+":P.maxSubs=(P.maxSubs||3)+1,P.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const O=u+"Score";P[O]>0&&(P[O]--,P.log.push({text:"🚫 But annulé",type:"info"}));break}}P["usedGc_"+p]=R,P._lastGC=D,P._gcAnimSeq=D.seq,await E(P)})}function fe(_,T){const A="usedCardIds_"+_,j=new Set(m[A]||[]);T.forEach(S=>j.add(S)),m[A]=[...j]}function C(){for(const _ of["p1","p2"]){const T=new Set(m["usedCardIds_"+_]||[]),A=m[_+"Team"];if(A)for(const j of["GK","DEF","MIL","ATT"])(A[j]||[]).forEach(S=>{S.used=T.has(S.cardId)})}}function W(){var rn,an,sn,dn,ln,cn;if(m.phase==="reveal")return ue();if(m.phase==="midfield")return V();if(m.phase==="finished")return Ge();const _=m[p+"Team"],T=m[u+"Team"];C();const A=m[p+"Score"],j=m[u+"Score"],S=m[p+"Name"],R=m[u+"Name"],D=m.phase===p+"-attack",P=m.phase===p+"-defense",O=Array.isArray(m["selected_"+p])?m["selected_"+p]:[],Q=O.map(J=>J.cardId),ne=window.innerWidth>=700,ie=m[p+"Subs"]||[],ee=m["usedSubIds_"+p]||[],ae=ie.filter(J=>!ee.includes(J.cardId)),le=m["gcCardsFull_"+p]||[],ge=m["usedGc_"+p]||[],ce=le.filter(J=>!ge.includes(J.id)),xe=m.boostOwner===p&&!m.boostUsed,ye=[..._.MIL||[],..._.ATT||[]].filter(J=>!J.used),oe=[...T.MIL||[],...T.ATT||[]].filter(J=>!J.used),Ee=!ke(T),Re=ye.length===0&&oe.length===0&&!Ee,Le=(_.DEF||[]).filter(J=>!J.used),Ve=(_.GK||[]).filter(J=>!J.used);let Oe=[];D&&ye.length===0&&(Ee?(Oe=Le.map(J=>J.cardId),Le.length===0&&(Oe=Oe.concat(Ve.map(J=>J.cardId)))):Re&&(Oe=[...Le,...Ve].map(J=>J.cardId)));function nt(J,me,Ae){var Me,ft;const we=J._gcDef,qt=(we==null?void 0:we.name)||J.gc_type,at=(we==null?void 0:we.effect)||((Me=je[J.gc_type])==null?void 0:Me.desc)||"",Fe=we!=null&&we.image_url?`/icons/${we.image_url}`:null,Ne=((ft=je[J.gc_type])==null?void 0:ft.icon)||"⚡";return`<div class="pvp-gc-mini" data-gc-id="${J.id}" data-gc-type="${J.gc_type}" style="flex-shrink:0;cursor:pointer">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${Ot(_,m[p+"Formation"],ut,Q,ne?1300:It(),ne?600:Rt(),Oe)}
      </div>
    </div>`,rt=m[p+"Team"],uo=(()=>{var me,Ae,we,qt,at;if(P&&((me=m.pendingAttack)!=null&&me.players)){const Fe=m.pendingAttack;let Ne="";if(O.length>0){const Me=O.map(We=>{const Vt=(rt[We._role]||[]).find(Li=>Li.cardId===We.cardId)||We,Yt=rt[We._role]||[],si=Yt.findIndex(Li=>Li.cardId===We.cardId),Ei=xt(Yt.length),Wt=si>=0?Ei[si]:Vt._col??1;return{...Vt,_line:We._role,_col:Wt}}),ft=ri(Me,((Ae=m.modifiers)==null?void 0:Ae[p])||{},m[p+"Formation"]);Ne=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
        ...
        </div>`}const J=(m.log||[]).slice(-1)[0];return J?'<div style="padding:2px 4px">'+Zn(J)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3);text-align:center">⏳ Match en cours...</div>'})(),on=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
      ...
      <div style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">${uo}</div>
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-play-area" style="flex:1;min-height:0;display:flex;overflow:hidden">
          <div id="match-field" style="flex:1;min-width:0;min-height:0;overflow:hidden">
            <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
              ${Ot(_,m[p+"Formation"],ut,Q,It(),Rt(),Oe)}
            </div>
          </div>
        </div>
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
      ...
      </div>`;function Bt(){const J=e.querySelector(".match-screen");if(!J)return;const me=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);J.style.height=me+"px",J.style.minHeight=me+"px",J.style.maxHeight=me+"px",J.style.overflow="hidden";const Ae=e.querySelector("#mobile-action-bar"),we=e.querySelector("#mobile-play-area");Ae&&we&&(we.style.paddingBottom=Ae.offsetHeight+"px")}if(Bt(),setTimeout(Bt,120),setTimeout(Bt,400),I||(I=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Bt),window.visualViewport.addEventListener("scroll",Bt)),window.addEventListener("resize",Bt)),function(){const me=e.querySelector("#match-field .terrain-wrapper svg")||e.querySelector(".terrain-wrapper svg");if(!me)return;const Ae=me.closest("#match-terrain-wrap");Ae&&(Ae.style.cssText="position:relative;width:100%;height:100%;padding:0"),me.removeAttribute("width"),me.removeAttribute("height"),me.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",me.setAttribute("preserveAspectRatio","xMidYMid meet")}(),m._pvpResizeBound||(m._pvpResizeBound=!0,window.addEventListener("resize",()=>{const J=e.querySelector(".terrain-wrapper svg");if(J){const me=J.closest("#match-terrain-wrap");me&&(me.style.cssText="position:relative;width:100%;height:100%;padding:0"),J.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0"}})),e.querySelectorAll(".match-slot-hit").forEach(J=>{J.addEventListener("click",()=>{if(!D&&!P)return;const me=J.dataset.cardId,Ae=J.dataset.role,we=(_[Ae]||[]).find(Ne=>Ne.cardId===me);if(!we||we.used)return;const qt=Oe.includes(me);if(D&&!["MIL","ATT"].includes(Ae)&&!qt)return;Array.isArray(m["selected_"+p])||(m["selected_"+p]=[]);const at=m["selected_"+p],Fe=at.findIndex(Ne=>Ne.cardId===me);Fe>-1?at.splice(Fe,1):at.length<3&&at.push({...we,_role:Ae}),W()})}),e.querySelectorAll(".match-used-hit").forEach(J=>{J.addEventListener("click",()=>N(J.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(J=>{J.addEventListener("click",()=>N())}),(rn=e.querySelector("#pvp-sub-open"))==null||rn.addEventListener("click",()=>N()),e.querySelectorAll(".pvp-gc-mini").forEach(J=>{D?J.addEventListener("click",()=>F(J.dataset.gcId,J.dataset.gcType)):(J.style.opacity="0.35",J.style.cursor="default",J.addEventListener("click",()=>$t("⚡ Les Game Changers ne sont utilisables qu'en attaque","rgba(180,100,0,0.9)")))}),(an=e.querySelector("#pvp-boost-card"))==null||an.addEventListener("click",()=>H()),(sn=e.querySelector("#pvp-action"))==null||sn.addEventListener("click",J=>{D?J.currentTarget.dataset.pass==="1"||!ct(p)?se():Ie():P&&Se()}),(dn=e.querySelector("#pvp-quit"))==null||dn.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&L()}),(ln=e.querySelector("#pvp-view-opp"))==null||ln.addEventListener("click",()=>q()),(cn=e.querySelector("#pvp-toggle-history"))==null||cn.addEventListener("click",()=>G()),k&&(clearInterval(k),k=null),Ue(),(D||P)&&!M){let J=30,me=!1;const Ae=()=>document.getElementById("pvp-timer"),we=()=>{Ae()&&(Ae().textContent=J+"s",Ae().style.color=me?"#ff4444":"#fff")};we(),k=setInterval(()=>{J--,J<0?me?(clearInterval(k),k=null,Ue(),D&&!ct(p)?se():L()):(me=!0,J=15,we(),Cn("/sounds/timer-urgent.mp3",.6)):we()},1e3)}}function ue(){st(e),e.innerHTML=`
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      ${bi(T,m[u+"Name"]||"Adversaire","#e03030","opp",null)}
      <div id="duel-shock" style="position:absolute;left:50%;top:50%;width:120px;height:120px;border-radius:50%;border:6px solid #FFD700;opacity:0;pointer-events:none"></div>
      <div id="pvp-duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center,rgba(10,61,30,.4),rgba(10,61,30,.92))"></div>
    </div>`;const R=(ae,le)=>e.querySelectorAll(ae).forEach((ge,ce)=>{setTimeout(()=>{ge.style.opacity="1",ge.style.transform="translateY(0) scale(1)"},le+ce*90)});R(".duel-card-me",150),R(".duel-card-opp",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((ae,le)=>setTimeout(()=>{ae.style.opacity="1"},le*70)),900),setTimeout(()=>{const ae=e.querySelector("#pvp-vs");ae&&(ae.style.opacity="1",ae.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(le=>le.style.opacity="1")},1250);function D(ae,le,ge){const ce=document.getElementById(ae);if(!ce)return;const xe=performance.now(),ye=oe=>{const Ee=Math.min(1,(oe-xe)/ge);ce.textContent=Math.round(le*(1-Math.pow(1-Ee,3))),Ee<1?requestAnimationFrame(ye):ce.textContent=le};requestAnimationFrame(ye)}setTimeout(()=>{D("pvp-score-me",A,800),D("pvp-score-opp",j,800)},1500);const P=m.p1Team.MIL||[],O=m.p2Team.MIL||[],Q=bt(P)+yt(P),ne=bt(O)+yt(O),ie=Q>=ne?"p1":"p2";let ee=m.boostValue;ee==null&&(ee=Jn(),m.boostValue=ee,m.boostOwner=ie,m.boostUsed=!1),U=!0,setTimeout(()=>{const ae=e.querySelector("#duel-row-"+(S?"me":"opp")),le=e.querySelector("#duel-row-"+(S?"opp":"me")),ge=document.getElementById("pvp-score-me"),ce=document.getElementById("pvp-score-opp"),xe=S?ge:ce,ye=S?ce:ge;xe&&(xe.style.fontSize="80px",xe.style.color=S?"#FFD700":"#ff6b6b",xe.style.animation="duelPulse .5s ease"+(S?",duelGlow 1.5s ease infinite .5s":"")),ye&&(ye.style.opacity="0.25"),setTimeout(()=>{ae&&(ae.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",ae.style.zIndex="5"),setTimeout(()=>{const oe=document.getElementById("duel-shock");oe&&(oe.style.animation="shockwave .5s ease-out forwards"),le&&(le.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var Le;const oe=document.getElementById("pvp-duel-finale");if(!oe)return;const Ee=m.boostOwner===p?'<div style="background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:3px solid #cdeffd;border-radius:18px;padding:20px 34px;text-align:center;animation:boostFlipIn .7s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 10px 36px rgba(135,206,235,.5)"><div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;font-weight:700">Carte Boost obtenue</div><div style="font-size:46px;line-height:1">⚡</div><div style="font-size:50px;font-weight:900;color:#063;line-height:1.1">+'+ee+`</div><div style="font-size:10px;color:rgba(0,0,0,.55);margin-top:4px">Applicable sur n'importe quel joueur</div></div>`:"",Re=p==="p1"?'<button id="pvp-start-match" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">▶ Commencer le match</button>':`<div style="font-size:14px;color:rgba(255,255,255,0.5);text-align:center;margin-top:8px;animation:fadeUp .4s ease both">⏳ En attente de l'adversaire...</div>`;oe.innerHTML='<div style="font-size:22px;font-weight:900;color:#fff;text-align:center;animation:fadeUp .4s ease both;text-shadow:0 2px 12px rgba(0,0,0,.5)">'+(S?"⚽ "+m[p+"Name"]+"<br>gagne le milieu et attaque !":"😔 "+m[u+"Name"]+"<br>gagne l'engagement et attaque !")+"</div>"+Ee+Re,oe.style.transition="opacity .45s ease",oe.style.opacity="1",oe.style.pointerEvents="auto",(Le=document.getElementById("pvp-start-match"))==null||Le.addEventListener("click",async()=>{const Ve=ie;await E({phase:Ve+"-attack",attacker:Ve,round:1,boostValue:ee,boostUsed:!1,boostOwner:Ve})})},600)},700)},2800)}function z(_,T,A){var le,ge;const j=(m.gcDefs||[]).find(ce=>{var xe;return ce.name===_||((xe=ce.name)==null?void 0:xe.toLowerCase().trim())===(_==null?void 0:_.toLowerCase().trim())}),S={purple:"#b06ce0",light_blue:"#00d4ef"}[j==null?void 0:j.color]||"#b06ce0",R=(j==null?void 0:j.name)||_,D=(j==null?void 0:j.effect)||((le=je[_])==null?void 0:le.desc)||"",P=j!=null&&j.image_url?`/icons/${j.image_url}`:null,O=((ge=je[_])==null?void 0:ge.icon)||"⚡",ne=T===p?"Vous":m[T+"Name"]||"Adversaire",ie=document.createElement("div");ie.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",ie.innerHTML=`
      <style>
      ...
      <div style="animation:gcFlipIn .7s cubic-bezier(.34,1.56,.64,1) both,gcGlow 1.8s ease infinite .7s">
        ${tt(R,P,O,D,{width:200})}
      </div>
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(ie);let ee=!1;const ae=()=>{ee||(ee=!0,ie.remove(),setTimeout(()=>A&&A(),50))};ie.addEventListener("click",ae),setTimeout(ae,3e3)}function F(_,T){var ne,ie,ee,ae;const j=(m["gcCardsFull_"+p]||[]).find(le=>le.id===_),S=j==null?void 0:j._gcDef,R=(S==null?void 0:S.name)||T,D=(S==null?void 0:S.effect)||((ne=je[T])==null?void 0:ne.desc)||"Carte spéciale.",P=S!=null&&S.image_url?`/icons/${S.image_url}`:null,O=((ie=je[T])==null?void 0:ie.icon)||"⚡",Q=document.createElement("div");Q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",Q.innerHTML=`
      ${tt(R,P,O,D,{width:190})}
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(Q),(ee=Q.querySelector("#pvp-gc-back"))==null||ee.addEventListener("click",()=>Q.remove()),(ae=Q.querySelector("#pvp-gc-use"))==null||ae.addEventListener("click",()=>{Q.remove(),pe(_,T)})}function H(){var j;const _=m[p+"Team"],T=Object.entries(_).flatMap(([S,R])=>(R||[]).filter(D=>!D.used).map(D=>({...D,_line:S})));if(!T.length)return;const A=document.createElement("div");A.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",A.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${T.map(S=>{const R={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[S._line]||"#555",D=Pe(S,S._line)+(S.boost||0);return`<div class="bp-item" data-cid="${S.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${R};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(A),(j=A.querySelector("#bp-close"))==null||j.addEventListener("click",()=>A.remove()),A.querySelectorAll(".bp-item").forEach(S=>{S.addEventListener("click",async()=>{const R=S.dataset.cid,D=T.find(O=>O.cardId===R);if(!D)return;const P=(_[D._line]||[]).find(O=>O.cardId===R);P&&(P.boost=(P.boost||0)+m.boostValue),A.remove(),await E({[p+"Team"]:_,boostUsed:!0})})})}function N(_=null){var ae,le;if(!(m.phase===p+"-attack")){g("Remplacement uniquement avant votre attaque","warning");return}const A=m[p+"Team"],j=m["usedSubIds_"+p]||[],S=m.maxSubs||3;if(j.length>=S){g(`Maximum ${S} remplacements atteint`,"warning");return}const R=Object.entries(A).flatMap(([ge,ce])=>(ce||[]).filter(xe=>xe.used).map(xe=>({...xe,_line:ge}))),D=(m[p+"Subs"]||[]).filter(ge=>!j.includes(ge.cardId));if(!R.length){g("Aucun joueur utilisé à remplacer","warning");return}if(!D.length){g("Aucun remplaçant disponible","warning");return}let P=Math.max(0,R.findIndex(ge=>ge.cardId===_));const O=((ae=R[P])==null?void 0:ae._line)||((le=R[P])==null?void 0:le.job);let Q=Math.max(0,D.findIndex(ge=>ge.job===O)),ne=!1;const ie=document.createElement("div");ie.id="pvp-sub-overlay",ie.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function ee(){var Re,Le,Ve,Oe,nt,Kt;const ge=R[P],ce=D[Q],xe=Math.min(130,Math.round((window.innerWidth-90)/2)),ye=Math.round(xe*1.35),oe=De=>`background:rgba(255,255,255,0.12);border:none;color:${De?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${De?"default":"pointer"};flex-shrink:0`;ie.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          ...
          <div>${ce?vt({...ce,used:!1,boost:0},xe,ye):"<div>—</div>"}</div>
      ...
      </div>`,document.body.appendChild(T),(A=T.querySelector("#pvp-hist-close"))==null||A.addEventListener("click",()=>T.remove())}async function se(){if(m.phase!==p+"-attack")return;const _=p==="p1"?"p2":"p1",T=(m.round||0)+1,A=[...m.log||[]];A.push({type:"info",text:`⏭️ ${m[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const j=$e(m),S=ct(_),R=j||!S?"finished":_+"-attack";await E({["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:R,attacker:_,round:T,log:A}),R==="finished"&&await qe(m)}function de(_){const T=(_.GK||[]).some(j=>!j.used),A=["DEF","MIL","ATT"].some(j=>(_[j]||[]).some(S=>!S.used));return T&&!A}function ve(_){return(_.p1Score||0)!==(_.p2Score||0)?null:de(_.p1Team)&&!ke(_.p2Team)?{side:"p1",gk:(_.p1Team.GK||[]).find(T=>!T.used)}:de(_.p2Team)&&!ke(_.p1Team)?{side:"p2",gk:(_.p2Team.GK||[]).find(T=>!T.used)}:null}async function ze(){const _=document.createElement("div");_.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(_);const T=(A,j)=>new Promise(S=>{_.innerHTML=`<div style="font-size:32px;font-weight:900;color:${j};letter-spacing:2px;animation:lcFade 1.4s ease both">${A}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(S,1400)});await T("⚽ DERNIER CORNER","#FFD700"),await T("🧤 LE GARDIEN MONTE !","#4fc3f7"),_.remove()}async function he(_,T){const{side:A,gk:j}=_;if(!j)return;const S=A+"Score",R=(T[S]||0)+1;j.used=!0;const D=[...T.log||[]];D.push({type:"duel",isGoal:!0,homeScored:A===p,homePlayers:A==="p1"?[Dt(j)]:[],aiPlayers:A==="p2"?[Dt(j)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${T[A+"Name"]} marque !`});const P=(T.round||0)+1,O=A==="p1"?"p2":"p1",Q={...T,[S]:R},ne=$e(Q);B.add(P),await ze();const ie=A===p?R:T[p+"Score"]||0,ee=A===p?T[u+"Score"]||0:R;Qe([Dt(j)],ie,ee,A===p,async()=>{await E({[S]:R,log:D,round:P,pendingAttack:null,phase:ne?"finished":O+"-attack",attacker:O,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}}}),ne&&await qe({...m,[S]:R})})}async function Ie(){const _=m[p+"Team"],T=!["GK","DEF","MIL","ATT"].some(R=>(m[u+"Team"][R]||[]).some(D=>!D.used)),A=(m["selected_"+p]||[]).map(R=>{const D=(_[R._role]||[]).find(le=>le.cardId===R.cardId)||R,P=["GK","DEF"].includes(R._role),O=_[R._role]||[],Q=O.findIndex(le=>le.cardId===R.cardId),ne=xt(O.length),ie=Q>=0?ne[Q]:D._col??1,ee=m.stadiumDef||m.homeStadiumDef||null,ae=D.stadiumBonus||ee&&(ee.club_id&&String(D.club_id)===String(ee.club_id)||ee.country_code&&D.country_code===ee.country_code)||!1;return{...D,_line:R._role,_col:ie,stadiumBonus:ae,...P?{note_a:Math.max(1,Number(D.note_a)||0)}:{}}});if(!A.length)return;const j=oi(A,m.modifiers[p]||{},m[p+"Formation"]);fe(p,A.map(R=>R.cardId)),A.forEach(R=>{const D=(_[R._role]||[]).find(P=>P.cardId===R.cardId);D&&(D.used=!0)}),m["selected_"+p]=[],W();const S=[...m.log||[]];if(T){if(A.length===1&&(A[0]._line==="GK"||A[0].job==="GK")&&(m.p1Score||0)===(m.p2Score||0)){await he({side:p,gk:A[0]},{...m,[p+"Team"]:_,log:S});return}const D=(m[p+"Score"]||0)+1,P=A.map(ee=>Dt(ee));S.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:P,homeTotal:j.total,aiTotal:0});const O=(m.round||0)+1,Q=p==="p1"?"p2":"p1",ne={...m,[p+"Team"]:_,[p+"Score"]:D},ie=$e(ne);B.add(O),Qe(P,D,m[u+"Score"]||0,!0,async()=>{await E({[p+"Team"]:_,[p+"Score"]:D,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:ie?"finished":Q+"-attack",attacker:Q,round:O,log:S}),ie&&await qe({...m,[p+"Score"]:D})});return}S.push({type:"pending",text:`⚔️ ${m[p+"Name"]} attaque (${j.total})`}),await E({[p+"Team"]:_,[u+"Team"]:m[u+"Team"],pendingAttack:{...j,players:A,side:p},["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},phase:u+"-defense",log:S})}async function Se(){const _=m[p+"Team"],T=(m["selected_"+p]||[]).map(ce=>{const xe=(_[ce._role]||[]).find(Le=>Le.cardId===ce.cardId)||ce,ye=_[ce._role]||[],oe=ye.findIndex(Le=>Le.cardId===ce.cardId),Ee=xt(ye.length),Re=oe>=0?Ee[oe]:xe._col??1;return{...xe,_line:ce._role,_col:Re}}),A=ri(T,m.modifiers[p]||{},m[p+"Formation"]);fe(p,T.map(ce=>ce.cardId)),T.forEach(ce=>{const xe=(_[ce._role]||[]).find(ye=>ye.cardId===ce.cardId);xe&&(xe.used=!0)}),m["selected_"+p]=[],W();const j=Qi(m.pendingAttack.total,A.total,m.modifiers[p]||{}),S=m.pendingAttack.side,R=j==="attack"||(j==null?void 0:j.goal),D=S==="p1"?"p2":"p1",P=(m.round||0)+1,O=(m.pendingAttack.players||[]).map(ce=>Dt(ce)),Q=[...m.log||[]];Q.push({type:"duel",isGoal:R,homeScored:R&&S===p,text:R?`⚽ BUT de ${m[S+"Name"]} ! (${m.pendingAttack.total} vs ${A.total})`:`✋ Attaque stoppée (${m.pendingAttack.total} vs ${A.total})`,homePlayers:O,aiPlayers:T.map(ce=>Dt(ce)),homeTotal:m.pendingAttack.total,aiTotal:A.total});const ne=R?(m[S+"Score"]||0)+1:m[S+"Score"]||0,ie={...m,[p+"Team"]:_,[S+"Score"]:ne},ee=$e(ie),ae=ee?"finished":D+"-attack",le=!ee&&!R?ve(ie):null,ge=async()=>{if(le){await he(le,ie);return}await E({[p+"Team"]:_,[u+"Team"]:m[u+"Team"],[S+"Score"]:ne,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:ae,attacker:D,round:P,log:Q}),(ae==="finished"||ee)&&await qe({...m,[S+"Score"]:ne})};if(R){const ce=S===p,xe=ce?ne:m[p+"Score"]||0,ye=ce?m[u+"Score"]||0:ne;B.add(P),Qe(O,xe,ye,ce,ge)}else await ge()}function Be(_){return["MIL","ATT"].some(T=>(_[T]||[]).some(A=>!A.used))}function ke(_){return["GK","DEF","MIL","ATT"].some(T=>(_[T]||[]).some(A=>!A.used))}function _t(_){return ke(_)&&!Be(_)}function ct(_){const T=m[_+"Team"],A=m[(_==="p1"?"p2":"p1")+"Team"];return!!(Be(T)||_t(T)&&!ke(A)||_t(T)&&!Be(A)&&ke(A))}function $e(_){return!ke(_.p1Team)&&!ke(_.p2Team)}function Ce(_){const T=_.p1Score||0,A=_.p2Score||0;return T===A?null:T>A?h.home_id:h.away_id}async function qe(_){zt(),Ue();try{const T=Ce(_),A=T?h.home_id===T?h.away_id:h.home_id:null,j=_.p1Score||0,S=_.p2Score||0,{data:R,error:D}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:T,p_home_score:j,p_away_score:S,p_forfeit:!1});if(D&&console.error("[PvP] finish_pvp_match:",D),l)try{await v.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(P){console.error("[PvP] check_and_finish_mini_league:",P)}T&&A&&Kr(T,A).catch(()=>{});try{const{data:P}=await v.rpc("apply_match_rewards",{p_match_id:i});P!=null&&P.success&&!(P!=null&&P.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(P){console.error("[PvP] apply_match_rewards:",P)}if(typeof c=="function")try{await c({homeScore:j,awayScore:S,winnerId:T,homeId:h.home_id,awayId:h.away_id,matchId:i})}catch(P){console.error("[PvP] onMatchEnd:",P)}}catch(T){console.error("[PvP] finishMatch:",T)}}function Ge(){var S;const _=m[p+"Score"],T=m[u+"Score"],A=_>T,j=_===T;st(e),e.innerHTML=`
    ...
    </div>`,(S=document.getElementById("pvp-home"))==null||S.addEventListener("click",()=>{try{v.removeChannel(Z)}catch{}it(e),l?x("mini-league",{openLeagueId:l}):x("home")})}W()}async function Gi(e,t,i=!1){var s,d;const{state:n,navigate:o,toast:r}=t,a=i&&((d=(s=t==null?void 0:t.state)==null?void 0:s.params)==null?void 0:d.rankedData)||null;await ki(e,t,i?"ranked":"random",async({deckId:c,formation:l,starters:f,subsRaw:b,gcCardsEnriched:x,gcDefs:g,stadiumDef:p})=>{st(e);const u=async y=>{await Wr(e,t,c,l,f,b,y||[],g,p,i,a)};if(!(x!=null&&x.length)){await u([]);return}wi(e,x,u)})}async function Yr(e,t,i){return Vr(e,t,i)}async function Wr(e,t,i,n,o,r,a=[],s=[],d=null,c=!1,l=null){var te,Z;const{state:f,navigate:b,toast:x}=t,g=(l==null?void 0:l.mmr)??((te=f.profile)==null?void 0:te.mmr)??1e3,p=c?vi(g):null,u=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",y=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let w=!1,h=null,$=null;e.innerHTML=`
    ...
    </div>`;const m=async()=>{w=!0,h&&(h.unsubscribe(),h=null),$&&(clearInterval($),$=null);try{await v.rpc("cancel_matchmaking",{p_user_id:f.profile.id})}catch(E){console.error("[Matchmaking] cancel error:",E)}};(Z=document.getElementById("cancel-mm"))==null||Z.addEventListener("click",async()=>{await m(),it(e),b("home")});const M=async(E,L)=>{w||(w=!0,h&&(h.unsubscribe(),h=null),$&&(clearInterval($),$=null),await Ct(e,t,E,L,{myGC:a,gcDefs:s,stadiumDef:d,isRanked:c,rankedData:l,onMatchEnd:c?Xr:null}))},k=f.user.id,{data:I,error:B}=c?await v.rpc("try_matchmake_ranked",{p_user_id:k,p_deck_id:i,p_mmr:g}):await v.rpc("try_matchmake",{p_user_id:k,p_deck_id:i});if(B||!(I!=null&&I.success)){console.error("[Matchmaking] try_matchmake error:",B||I),x("Erreur de matchmaking","error"),it(e),b("home");return}if(I.matched){await M(I.match_id,!1);return}const X=async()=>{if(w)return;const{data:E}=await v.from("matchmaking_queue").select("status, match_id").eq("user_id",k).maybeSingle();(E==null?void 0:E.status)==="matched"&&E.match_id&&await M(E.match_id,!0)};h=v.channel(`mm_${k}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${k}`},E=>{const L=E.new;L.status==="matched"&&L.match_id&&M(L.match_id,!0)}).subscribe(),$=setInterval(X,3e3),setTimeout(()=>{w||m()},12e4)}async function Xr(e,t){const{state:i,toast:n}=t,{winnerId:o,homeId:r,awayId:a,homeScore:s,awayScore:d,matchId:c}=e,l=i.user.id,f=r===l,b=o===l,{data:x}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",l).single(),{data:g}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",f?a:r).single();if(x&&g){const p=fo(x.mmr,x.mmr_rd,x.mmr_v,g.mmr,g.mmr_rd,b?1:0);await v.from("users").update({mmr:p.mmr,mmr_rd:p.rd,mmr_v:p.v,mmr_wins:b?v.sql`mmr_wins + 1`:void 0,mmr_losses:b?void 0:v.sql`mmr_losses + 1`}).eq("id",l),n(b?`+MMR ↑ ${Math.round(p.mmr-x.mmr)}`:`-MMR ↓ ${Math.round(x.mmr-p.mmr)}`,b?"success":"error",4e3)}}const no=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:Gi,resumePvpMatch:Yr},Symbol.toStringTag,{value:"Module"}));async function Jr(e,t){const{state:i,navigate:n,toast:o}=t,r=i.params||{},a=r.friendId||null,s=r.friendName||"Ami",d=!!r.isAccepting;await ki(e,t,"friend",async({deckId:c,formation:l,starters:f,subsRaw:b,gcCardsEnriched:x,gcDefs:g,stadiumDef:p})=>{st(e);const u=async y=>{const w=y||[];d?await Zr(e,t,c,w,g,p):await Qr(e,t,c,w,g,p,a,s)};if(!(x!=null&&x.length)){await u([]);return}wi(e,x,u)})}async function Qr(e,t,i,n,o,r,a,s){var w;const{state:d,navigate:c,toast:l}=t,f=d.user.id,{data:b,error:x}=await v.from("friend_match_invites").insert({inviter_id:f,invitee_id:a,friend_id:a,status:"pending",inviter_deck_id:i,expires_at:new Date(Date.now()+2*60*1e3).toISOString()}).select("id").single();if(x||!b){console.error("[Friend] Erreur création invitation:",x),l("Impossible de créer l'invitation","error"),c("home");return}e.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <div style="padding:14px 16px;background:var(--nav-bg,#0d1a0f);border-bottom:1px solid ${Lt};display:flex;align-items:center;justify-content:space-between">
      <div>
        ...
        <div style="font-size:12px;color:${ro}">Championnats 3 à 8 joueurs</div>
      </div>
      <div style="display:flex;align-items:center;gap:8px">
    ...
    <div style="padding:20px;display:flex;flex-direction:column;gap:14px">
      ...
      </div>`,document.body.appendChild(t);const i=t.querySelector("#ml-pwd-input"),n=o=>{t.remove(),e(o)};setTimeout(()=>i==null?void 0:i.focus(),80),t.querySelector("#pwd-cancel").addEventListener("click",()=>n(null)),t.querySelector("#pwd-ok").addEventListener("click",()=>n(i.value.trim())),i.addEventListener("keydown",o=>{o.key==="Enter"&&n(i.value.trim())})})}async function ao(e,t,i,n){var f;const{toast:o,state:r}=t,a=r.profile.id;let s=null;if(n==="private"&&(s=await aa(),s===null))return;const{data:d,error:c}=await v.rpc("join_mini_league",{p_league_id:i,p_user_id:a,p_password:s});if(c||!(d!=null&&d.success)){console.error("[MiniLeague] join_mini_league:",c||d),o((d==null?void 0:d.error)||"Erreur lors de l'inscription","error");return}if(typeof t.refreshProfile=="function")await t.refreshProfile();else{const{data:b}=await v.from("users").select("credits").eq("id",a).single();r.profile&&b&&(r.profile.credits=b.credits)}const l=document.getElementById("nav-credits");l&&(l.textContent=`💰 ${(((f=r.profile)==null?void 0:f.credits)||0).toLocaleString("fr")}`),o("✅ Inscrit ! Pot mis à jour.","success"),lt(e,t,i)}async function sa(e,t,i,n){const{toast:o,state:r}=t,a=r.profile.id;if(!await new Promise(b=>{const x=document.createElement("div");x.className="modal-overlay",x.style.zIndex="2100",x.innerHTML=`<div class="modal" style="max-width:360px">
    ...
    </div>`,document.body.appendChild(x),x.querySelector("#lv-ok").onclick=()=>{x.remove(),b(!0)},x.querySelector("#lv-cancel").onclick=()=>{x.remove(),b(!1)}}))return;const{data:d,error:c}=await v.rpc("leave_mini_league",{p_league_id:i,p_user_id:a});if(c||!(d!=null&&d.success)){o((d==null?void 0:d.error)||"Erreur lors de la désinscription","error");return}const l=d.refund||n;r.profile&&(r.profile.credits=(r.profile.credits||0)+l);const f=document.getElementById("nav-credits");f&&(f.textContent=`💰 ${(r.profile.credits||0).toLocaleString("fr")}`),o(`↩️ Désinscrit · +${l.toLocaleString("fr")} cr. remboursés`,"success"),et(e,t,"waiting")}async function Ri(e,t,i,n,o){const{toast:r}=t;if(!confirm(`Supprimer définitivement "${n}" et tous ses matchs/membres ? Action irréversible.`))return;await v.from("mini_league_matches").delete().eq("league_id",i),await v.from("mini_league_members").delete().eq("league_id",i);const{error:a}=await v.from("mini_leagues").delete().eq("id",i);if(a){r("Erreur suppression ("+a.message+")","error");return}r("Mini League supprimée avec succès","success"),et(e,t,o)}async function da(e,t,i){await v.from("mini_leagues").update({is_archived:!0}).eq("id",i),t.toast("Mini League archivée","success"),et(e,t,"archived")}async function lt(e,t,i){var $,m,M,k,I,B,X,te,Z;const{state:n,toast:o}=t,r=n.profile.id,[{data:a},{data:s},{data:d}]=await Promise.all([v.from("mini_leagues").select("*").eq("id",i).single(),v.from("mini_league_members").select("*, user:users(id,pseudo,club_name,club_color1,club_color2)").eq("league_id",i),v.from("mini_league_matches").select("*").eq("league_id",i).order("matchday").order("created_at")]);if(!a){o("Introuvable","error"),await et(e,t);return}const c=(s||[]).some(E=>E.user_id===r),l=a.creator_id===r,f=(s||[]).map(E=>E.user).filter(Boolean),b=fa(f,d||[]),x=(d||[]).filter(E=>E.matchday===a.current_day),g=a.pot||0,p=a.entry_fee||100,u=x.length>0&&x.every(E=>E.status==="finished"||E.status==="bye"),y=a.current_day>=a.total_days,w=(s||[]).find(E=>E.user_id===r);e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <div style="padding:14px 16px;background:var(--nav-bg,#0d1a0f);border-bottom:1px solid ${Lt};display:flex;align-items:center;gap:10px">
      ...
      <div style="flex:1">
        ...
        <div style="font-size:11px;color:${ro}">${a.mode==="aller-retour"?"Aller-Retour":"Aller"} · max ${a.max_players} · 💰 ${p} cr./joueur</div>
      </div>
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        ${f.map(E=>`
          <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid ${Lt}">
            <div style="width:36px;height:36px;border-radius:50%;background:${E.club_color2||oo};display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:${E.club_color1||"#fff"}">${(E.pseudo||"?").slice(0,2).toUpperCase()}</div>
            <div style="flex:1"><div style="font-size:13px;font-weight:700;color:${Xe}">${E.club_name||E.pseudo}</div><div style="font-size:11px;color:${Ze}">@${E.pseudo}</div></div>
            ${E.id===a.creator_id?'<span style="font-size:10px;color:#D4A017;font-weight:700">👑</span>':""}
          </div>`).join("")}
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${b.map((E,L)=>{const K=g>0&&a.status==="finished"?L===0?Math.floor(g*.7):L===1?Math.floor(g*.2):L===2?Math.floor(g*.1):0:0;return`<tr style="border-bottom:1px solid ${Lt};${E.userId===r?"background:rgba(74,222,128,0.08);":""}">
              <td style="padding:7px 3px 7px 0;font-weight:700;color:${L===0?dt:L<3?"#4ade80":Et}">${["🥇","🥈","🥉"][L]||L+1}</td>
              <td style="padding:7px 3px"><div style="font-weight:700;color:${Xe}">${E.clubName}</div><div style="font-size:10px;color:${Ze}">@${E.pseudo}</div></td>
              <td style="text-align:center;color:${Et}">${E.played}</td><td style="text-align:center;color:${Et}">${E.won}-${E.drawn}-${E.lost}</td>
              <td style="text-align:center;color:${E.goalDiff>=0?"#4ade80":"#ff6b6b"}">${E.goalDiff>=0?"+":""}${E.goalDiff}</td>
              <td style="text-align:center;font-weight:900;font-size:14px;color:${Xe}">${E.points}</td>
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(E,L)=>L+1).reverse().map(E=>{const L=(d||[]).filter(K=>K.matchday===E);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${Ze};margin-bottom:6px">Journée ${E}</div>${L.map(K=>Ln(K,f,r,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  </div>`;const h=a.status==="waiting"?"waiting":a.status==="active"?"active":"archived";if(($=document.getElementById("ml-back"))==null||$.addEventListener("click",()=>et(e,t,h)),(m=document.getElementById("ml-refresh"))==null||m.addEventListener("click",async E=>{const L=E.currentTarget;L.style.opacity="0.5",await lt(e,t,i)}),(M=document.getElementById("ml-start-btn"))==null||M.addEventListener("click",()=>la(e,t,a,f)),(k=document.getElementById("ml-next-day"))==null||k.addEventListener("click",()=>ca(e,t,i)),(I=document.getElementById("ml-join-now"))==null||I.addEventListener("click",()=>ao(e,t,i,a.type)),(B=document.getElementById("ml-leave-btn"))==null||B.addEventListener("click",()=>sa(e,t,i,p)),(X=document.getElementById("ml-delete-btn"))==null||X.addEventListener("click",()=>Ri(e,t,i,a.name,"waiting")),(te=document.getElementById("ml-delete-now"))==null||te.addEventListener("click",()=>Ri(e,t,i,a.name,h)),(Z=document.getElementById("ml-archive-btn"))==null||Z.addEventListener("click",()=>da(e,t,i)),e.querySelectorAll("[data-play-match]").forEach(E=>{E.addEventListener("click",()=>{const L=x.find(K=>K.id===E.dataset.playMatch);L&&t.navigate("match-mini-league",{mlMatchId:L.id,leagueId:i})})}),a.status==="finished"&&w){const E=b.findIndex(L=>L.userId===r);E>=0&&E<3&&w.prize_amount>0&&!w.prize_claimed&&setTimeout(()=>pa(e,t,a,w,E),400)}}function Ln(e,t,i,n,o=!1){const r=f=>t.find(b=>b.id===f);if(e.is_bye){const f=r(e.home_id);return`<div style="padding:8px;border-radius:8px;background:rgba(255,255,255,0.03);margin-bottom:6px;font-size:12px;color:${Et};text-align:center">🔵 ${(f==null?void 0:f.club_name)||(f==null?void 0:f.pseudo)||"?"} exempté(e)</div>`}const a=r(e.home_id),s=r(e.away_id),d=e.home_id===i||e.away_id===i,c=d&&e.status==="pending"&&n&&!o,l=e.status==="finished"?`${e.home_score} - ${e.away_score}`:"vs";return`<div style="display:flex;align-items:center;gap:8px;padding:10px;border-radius:8px;background:${d?"rgba(26,107,60,0.16)":"rgba(255,255,255,0.03)"};margin-bottom:6px;border:1px solid ${d?"rgba(74,222,128,0.35)":Lt}">
  ...
  </div>`}async function la(e,t,i,n){const{toast:o,state:r}=t,a=ua(n.map(c=>c.id),i.mode),s=[];a.forEach((c,l)=>c.forEach(f=>s.push({league_id:i.id,matchday:l+1,home_id:f.home||f.bye,away_id:f.away||null,is_bye:!!f.bye,status:f.bye?"bye":"pending"})));const{error:d}=await v.from("mini_league_matches").insert(s);if(d){o("Erreur calendrier : "+d.message,"error");return}await v.from("mini_leagues").update({status:"active",current_day:1,total_days:a.length}).eq("id",i.id),o(`🚀 Lancée ! Pot : ${(i.pot||0).toLocaleString("fr")} cr.`,"success"),lt(e,t,i.id)}async function ca(e,t,i){const{data:n}=await v.from("mini_leagues").select("current_day,total_days,pot").eq("id",i).single(),o=(n.current_day||0)+1;if(o>(n.total_days||0)){lt(e,t,i);return}await v.from("mini_leagues").update({current_day:o}).eq("id",i),t.toast(`Journée ${o} commencée !`,"success"),lt(e,t,i)}async function pa(e,t,i,n,o){var b,x;const{state:r,toast:a}=t,s=[Math.floor((i.pot||0)*.7),Math.floor((i.pot||0)*.2),Math.floor((i.pot||0)*.1)],d=["🥇","🥈","🥉"][o],c=n.prize_amount||s[o]||0,l=n.prize_claimed,f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",f.innerHTML=`
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function ba(e,t,i){let n=0;const o=document.createElement("div");o.id="tutorial-overlay",o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const r=()=>{var l,f,b;const s=t[n],d=n===t.length-1,c=Math.round((n+1)/t.length*100);o.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let d="buy";const c=()=>{var p,u,y,w,h,$,m;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((u=document.getElementById("flt-club"))==null?void 0:u.value)||"").toLowerCase().trim(),country:(((y=document.getElementById("flt-country"))==null?void 0:y.value)||"").toLowerCase().trim(),job:((w=document.getElementById("flt-job"))==null?void 0:w.value)||"",rarity:((h=document.getElementById("flt-rarity"))==null?void 0:h.value)||"",note1:parseInt(($=document.getElementById("flt-note1"))==null?void 0:$.value)||0,note2:parseInt((m=document.getElementById("flt-note2"))==null?void 0:m.value)||0}};function l(p){const u=c();return p.filter(y=>{var B,X,te;const w=(B=y.card)==null?void 0:B.player;if(!w)return!1;const h=`${w.firstname} ${w.surname_real}`.toLowerCase(),$=(((X=w.clubs)==null?void 0:X.encoded_name)||"").toLowerCase(),m=(w.country_code||"").toLowerCase(),M=((te=y.card)==null?void 0:te.evolution_bonus)||0,k=Ft(w,w.job,M),I=w.job2?Ft(w,w.job2,M):0;return!(u.name&&!h.includes(u.name)||u.club&&!$.includes(u.club)||u.country&&!m.includes(u.country)||u.job&&w.job!==u.job||u.rarity&&w.rarity!==u.rarity||u.note1&&k<u.note1||u.note2&&I<u.note2)})}function f(p){var I,B,X,te;const u=(I=p.card)==null?void 0:I.player;if(!u)return"";const y=((B=p.card)==null?void 0:B.evolution_bonus)||0,w=Ft(u,u.job,y),h=u.job2?Ft(u,u.job2,y):0,$=(i.profile.credits||0)>=p.price,m=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,M=ui[u.job]||"#bbb",k=u.job2?ui[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
      ${m?`<img src="${m}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      <!-- Logo club -->
      ${(X=u.clubs)!=null&&X.logo_url?`<img src="${u.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <!-- Notes -->
    ...
    </div>`}function b(p){var I,B,X,te;const u=(I=p.card)==null?void 0:I.player;if(!u)return"";const y=((B=p.card)==null?void 0:B.evolution_bonus)||0,w=Ft(u,u.job,y),h=u.job2?Ft(u,u.job2,y):0,$=p.status==="sold",m=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,M=ui[u.job]||"#bbb",k=u.job2?ui[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${$?"opacity:0.7":""}">
      ${m?`<img src="${m}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(X=u.clubs)!=null&&X.logo_url?`<img src="${u.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${M};display:flex;align-items:center;justify-content:center">
    ...
    </div>`}function x(){const p=document.getElementById("mkt-content"),u=document.getElementById("mkt-filters");if(p){if(u.style.display=d==="buy"?"flex":"none",d==="buy"){const y=l(a);p.innerHTML=y.length?y.map(f).join(""):'<div style="text-align:center;color:#aaa;padding:40px">Aucune carte trouvée.</div>'}else{const y=s.filter(h=>h.status==="active").sort((h,$)=>new Date($.listed_at)-new Date(h.listed_at)),w=s.filter(h=>h.status==="sold").sort((h,$)=>new Date($.sold_at||$.listed_at)-new Date(h.sold_at||h.listed_at));p.innerHTML=(y.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:4px 0 6px;text-transform:uppercase;letter-spacing:1px">🟢 En vente (${y.length})</div>`+y.map(b).join(""):"")+(w.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:12px 0 6px;text-transform:uppercase;letter-spacing:1px">✅ Ventes réussies (${w.length})</div>`+w.map(b).join(""):"")+(!y.length&&!w.length?'<div style="text-align:center;color:#aaa;padding:40px">Aucune vente pour le moment.</div>':"")}p.querySelectorAll("[data-buy]").forEach(y=>y.addEventListener("click",()=>va(y.dataset.buy,a,e,t))),p.querySelectorAll("[data-cancel]").forEach(y=>y.addEventListener("click",()=>_a(y.dataset.cancel,e,t)))}}e.querySelectorAll(".mkt-tab").forEach(p=>{p.addEventListener("click",()=>{d=p.dataset.tab,e.querySelectorAll(".mkt-tab").forEach(u=>{const y=u===p;u.style.background=y?"var(--green)":"var(--tile-bg)",u.style.color=y?"#fff":"var(--tile-fg-dim)",u.style.borderColor=y?"var(--green)":"var(--tile-border)"}),x()})});let g;["flt-name","flt-club","flt-country","flt-job","flt-rarity","flt-note1","flt-note2"].forEach(p=>{var u;(u=document.getElementById(p))==null||u.addEventListener("input",()=>{clearTimeout(g),g=setTimeout(x,250)})}),x()}async function va(e,t,i,n){const{state:o,toast:r,refreshProfile:a}=n,s=t.find(l=>l.id===e);if(!s)return;const d=s.price;if((o.profile.credits||0)<d){r("Crédits insuffisants","error");return}wa(s,async()=>{const{error:l}=await v.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:o.profile.id});if(l){r("Erreur achat : "+l.message,"error");return}await a();const f=document.getElementById("nav-credits");f&&(f.textContent=`💰 ${(o.profile.credits||0).toLocaleString("fr")}`),r("✅ Carte achetée !","success"),await nn(i,n)})}function wa(e,t){var a;const i=(a=e.card)==null?void 0:a.player,n=i?`${i.firstname} ${i.surname_real}`:"cette carte",o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",o.innerHTML=`
        ...
        <button id="buy-ok" style="flex:1;padding:12px;border-radius:10px;border:none;background:var(--green);color:#fff;font-size:14px;font-weight:900;cursor:pointer">Confirmer</button>
    ...
    </div>`,document.body.appendChild(o);const r=s=>{o.remove(),s&&t()};o.querySelector("#buy-cancel").addEventListener("click",()=>r(!1)),o.querySelector("#buy-ok").addEventListener("click",()=>r(!0)),o.addEventListener("click",s=>{s.target===o&&r(!1)})}async function _a(e,t,i){const{toast:n}=i,{data:o}=await v.from("market_listings").select("card_id").eq("id",e).single();if(await v.from("market_listings").update({status:"cancelled"}).eq("id",e),o!=null&&o.card_id){const{count:r}=await v.from("market_listings").select("id",{count:"exact",head:!0}).eq("card_id",o.card_id).eq("status","active");r||await v.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",o.card_id)}n("Annonce retirée","success"),nn(t,i)}async function ka(e,t){var x,g,p,u,y;const{state:i,navigate:n}=t,o=((g=(x=t==null?void 0:t.state)==null?void 0:x.params)==null?void 0:g.tab)||"ranked";e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:var(--tile-fg-dim)">⚽ Chargement...</div>';const[{data:r},{data:a},{data:s}]=await Promise.all([v.from("users").select("id,pseudo,club_name,mmr,rank_tier,ranked_wins,ranked_losses,ranked_draws,placement_matches").gte("placement_matches",1).order("mmr",{ascending:!1}).limit(100),v.rpc("get_mini_league_leaderboard"),v.rpc("get_random_leaderboard")]);let d=o;const c=w=>w<3?["#D4A017","#a0a0a0","#cd7f32"][w]:"var(--green)",l=w=>w<3?"#000":"#fff";function f(){var $,m,M;const w=document.getElementById("rankings-legend");w&&(w.innerHTML=d==="random"?'<div style="font-size:11px;color:var(--tile-fg-dim);text-align:center;margin-bottom:10px">📊 = score de classement fiable (tient compte du nombre de matchs joués, pas seulement du %)</div>':"");const h=document.getElementById("rankings-list");if(h){if(d==="ranked"){const k=r||[];h.innerHTML=k.length>0?k.map((I,B)=>{const X=vi(I.mmr??1e3),te=(I.ranked_wins||0)+(I.ranked_losses||0)+(I.ranked_draws||0),Z=te>0?Math.round((I.ranked_wins||0)/te*100):0,E=I.id===i.profile.id,L=(I.placement_matches||0)<10;return`
          <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${E?"border:2px solid var(--yellow)":""}">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:11px;color:var(--tile-fg-dim)">${I.club_name} · ${X.label}</div>
            </div>
            <div style="text-align:right;flex-shrink:0">
              <div style="font-size:20px">${L?"❓":X.emoji}</div>
              <div style="font-size:11px;font-weight:700;color:${X.color}">${L?"Placement":X.label}</div>
              ${L?"":`<div style="font-size:10px;color:var(--tile-fg-dim)">${Z}% WR</div>`}
            </div>
  ...
  <div class="page">
    ...
    </div>`,(M=document.getElementById("ranked-back"))==null||M.addEventListener("click",()=>n("home"));return}const d=s.mmr??1e3,c=s.mmr_deviation??350,l=s.mmr_volatility??.06,f=s.placement_matches??0,b=f<10,x=Math.max(0,10-f),g=vi(d),p=In(d),u=zi.findIndex(X=>X.id===g.id),y=zi[u+1]||null,w={bronze:"linear-gradient(160deg,#3d1c00,#7a3e00)",silver:"linear-gradient(160deg,#1a1a2e,#3a3a5e)",gold:"linear-gradient(160deg,#1a1200,#4a3500)",platinum:"linear-gradient(160deg,#001a20,#003040)",diamond:"linear-gradient(160deg,#001030,#1a2860)",master:"linear-gradient(160deg,#1a0030,#3d0070)"},h=(s.ranked_wins||0)+(s.ranked_losses||0)+(s.ranked_draws||0),$=h>0?Math.round((s.ranked_wins||0)/h*100):0,m=zi.map(X=>`
    <div style="display:flex;flex-direction:column;align-items:center;gap:2px;opacity:${g.id===X.id?1:.35};
      transform:${g.id===X.id?"scale(1.15)":"scale(1)"};transition:all 0.3s">
      <div style="font-size:${g.id===X.id?"28px":"20px"}">${X.emoji}</div>
      <div style="font-size:9px;color:${X.color};font-weight:${g.id===X.id?"900":"400"};letter-spacing:0.5px">${X.label.toUpperCase()}</div>
    </div>
  `).join("");e.innerHTML=`
  ...
  </div>`}mo(xo);const be={user:null,profile:null,page:"home",params:{}};function Pt(e,t="info",i=3e3){const n=document.getElementById("toast");n&&(n.textContent=e,n.className=`show ${t}`,clearTimeout(n._t),n._t=setTimeout(()=>{n.className=""},i))}function La(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function Ni(){document.getElementById("modal-overlay").classList.add("hidden")}async function Zt(){if(!be.user)return;const{data:e}=await v.from("users").select("*").eq("id",be.user.id).single();e&&(be.profile=e)}const so="mw_theme";function lo(){return localStorage.getItem(so)||"dark"}function za(e){var t;localStorage.setItem(so,e),Ta(e),(t=be.profile)!=null&&t.id&&v.from("users").update({theme:e}).eq("id",be.profile.id).then(()=>{})}function Ta(e){document.documentElement.setAttribute("data-theme",e)}function Gt(e,t={}){be.page=e,be.params=t,co()}async function co(){var n,o,r,a;const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(s=>{s.classList.toggle("active",s.dataset.page===be.page)});const t=document.getElementById("nav-credits");t&&be.profile&&(t.textContent=`💰 ${(be.profile.credits||0).toLocaleString("fr")}`);const i={state:be,navigate:Gt,toast:Pt,openModal:La,closeModal:Ni,refreshProfile:Zt};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',be.page){case"home":await gn(e,i);break;case"home2":await No(e,i);break;case"game":await Vo(e,i);break;case"settings":await Gn(e,i);break;case"collection":await er(e,i);break;case"decks":await ji(e,i);break;case"boosters":await fr(e,i);break;case"ranked":await $a(e,i);break;case"match":{const s=be.params&&be.params.matchMode||"vs_ai_easy";s==="random"?await Gi(e,i,!1):s==="ranked"?await Gi(e,i,!0):s==="friend"?await Jr(e,i,(n=be.params)==null?void 0:n.friendId,(o=be.params)==null?void 0:o.friendName):s==="mini_league"||s==="mini-league"?await zn(e,i,(r=be.params)==null?void 0:r.mlMatchId,(a=be.params)==null?void 0:a.leagueId):await Tr(e,i);break}case"market":await ha(e,i);break;case"rankings":await ka(e,i);break;case"matches":await Ea(e,i);break;case"friends":await ko(e,i);break;case"mini-league":await ta(e,i);break;case"match-mini-league":{const s=be.params||{};await zn(e,i,s.mlMatchId,s.leagueId);break}default:await gn(e,i)}}function Ia(){var n,o;const e=document.getElementById("app"),t=be.profile;if(!t)return;const i="/icons/";e.innerHTML=`
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.23-1550" style="cursor:pointer">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(r=>{r.addEventListener("click",a=>{a.preventDefault(),Gt(r.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>Gt("home")),document.getElementById("nav-credits").addEventListener("click",()=>Gt("boosters")),(n=document.getElementById("journal-btn"))==null||n.addEventListener("click",()=>Sa()),(o=document.getElementById("settings-btn"))==null||o.addEventListener("click",()=>Gt("settings"))}async function Sa(){const{data:e}=await v.from("patch_notes").select("*").eq("is_published",!0).order("published_at",{ascending:!1}).limit(20),t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px";const i=(e||[]).map(n=>{const o=new Date(n.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"long",year:"numeric"});return`<div style="padding:14px 0;border-bottom:1px solid #f0f0f0">
    ...
    </div>`,document.body.appendChild(t),t.querySelector("#journal-close").addEventListener("click",()=>t.remove()),t.addEventListener("click",n=>{n.target===t&&t.remove()})}function Aa(e,{onPlay:t}){var n;const i="/icons/";e.style.cssText="",e.innerHTML=`
  <style>
    #public-landing{
  ...
  </div>`,(n=e.querySelector("#pl-play-btn"))==null||n.addEventListener("click",t)}async function Ma(){document.documentElement.setAttribute("data-theme",lo()),document.getElementById("modal-overlay").addEventListener("click",i=>{i.target===i.currentTarget&&Ni()}),document.getElementById("modal-close").addEventListener("click",Ni);const{data:{session:e}}=await v.auth.getSession();if(!e){Tn(),Aa(document.getElementById("app"),{onPlay:()=>pn(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Pt})});return}be.user=e.user,await Zt(),Tn();try{const{data:i}=await v.from("formation_links_overrides").select("formation, links"),n={};(i||[]).forEach(o=>{n[o.formation]=o.links}),go(n)}catch(i){console.warn("Impossible de charger les overrides de formation:",i)}if(!be.profile){yo(document.getElementById("app"),{state:be,navigate:async()=>{await Zt(),Ai()},toast:Pt,refreshProfile:Zt});return}const t=Array.isArray(be.profile.pending_boosters)?be.profile.pending_boosters:[];if(!be.profile.onboarding_done&&t.length>0){wr(document.getElementById("app"),{state:be,navigate:()=>Ai(),toast:Pt,refreshProfile:Zt});return}Ai(),setTimeout(()=>ya(be.profile,Gt,Pt),800),v.auth.onAuthStateChange(async(i,n)=>{i==="SIGNED_OUT"&&(be.user=null,be.profile=null,document.getElementById("app").innerHTML="",pn(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Pt}))})}function Ca(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function $i(){const e=document.getElementById("app");e&&(e.style.height=Ca()+"px")}window.addEventListener("resize",$i);window.addEventListener("orientationchange",()=>setTimeout($i,150));window.visualViewport&&window.visualViewport.addEventListener("resize",$i);function Ai(){const e=()=>{var i;(i=be.user)!=null&&i.id&&v.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",be.user.id).then(()=>{})};e(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(e,6e4);const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",$i(),Ia(),co()}function Tn(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function po(e){var n;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
    <div style="font-size:42px">📡</div>
    <div style="font-size:18px;font-weight:900">Connexion impossible</div>
    <div style="font-size:13px;color:rgba(255,255,255,0.6);max-width:280px">${e||"Le chargement a échoué. Vérifie ta connexion et réessaie."}</div>
    <button id="boot-retry" style="margin-top:8px;padding:12px 30px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer">Réessayer</button>`,document.body.appendChild(i),(n=document.getElementById("boot-retry"))==null||n.addEventListener("click",()=>window.location.reload())}Ma().catch(e=>{console.error("Échec du démarrage:",e),po()});setTimeout(()=>{const e=document.getElementById("app-loader");e&&e.style.display!=="none"&&!e.classList.contains("zoom-out")&&!document.getElementById("boot-error")&&po("Le serveur met trop de temps à répondre.")},12e3);
