import{s as k,F as Je,b as Ie,c as It,l as fe,d as Yt}from"./formation-links-CDBKB_z4.js";function ft(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),d=(o||[]).filter(w=>w.card_type==="player"&&w.player),s=(o||[]).filter(w=>w.card_type==="game_changer"),r=(o||[]).filter(w=>w.card_type==="formation"),{data:f}=await k.from("gc_definitions").select("name,gc_type,color,effect,image_url"),u={};(f||[]).forEach(w=>{u[w.name]=w});const g=Object.keys(Je),m=Object.keys(de),b={};d.forEach(w=>{const y=w.player.id;b[y]=(b[y]||0)+1}),new Set(Object.keys(b).map(w=>String(w)));const T=new Set(r.map(w=>w.formation)),_=new Set(s.map(w=>w.gc_type));let x="player",A="Tous",D="",v=!1;function E(){return[...d].sort((w,y)=>{const j=Le.indexOf(w.player.job),F=Le.indexOf(y.player.job);return j!==F?j-F:(w.player.surname_encoded||"").localeCompare(y.player.surname_encoded||"")})}function L(){return[...c||[]].sort((w,y)=>{const j=Le.indexOf(w.job),F=Le.indexOf(y.job);return j!==F?j-F:(w.surname_encoded||"").localeCompare(y.surname_encoded||"")})}function h(){return E().filter(w=>{const y=w.player,j=A==="Tous"||y.job===A,F=!D||`${y.firstname} ${y.surname_encoded}`.toLowerCase().includes(D);return j&&F})}function $(){return L().filter(w=>{const y=A==="Tous"||w.job===A,j=!D||`${w.firstname} ${w.surname_encoded}`.toLowerCase().includes(D);return y&&j})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function I(){const w=document.getElementById("col-filters");w&&(x==="player"?(w.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",y=>{D=y.target.value.toLowerCase(),S()}),e.querySelectorAll(".filter-btn").forEach(y=>{y.addEventListener("click",()=>{A=y.dataset.job,I(),S()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{v=!v,I(),S()})):(w.innerHTML=`
        <div style="display:flex;justify-content:flex-end">
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{v=!v,I(),S()})))}function S(){const w=document.getElementById("col-grid");w&&(x==="player"?M(w):x==="formation"?O(w):H(w))}function z(w,y,j,F,q){q=q||"#7a28b8";const P=document.getElementById("col-grid"),R=document.getElementById("col-big");if(!P||!R)return;var W=0;function V(){R.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+y(w[W])+"</div>";var Y=R.querySelector("[data-card-id],[data-form-id],[data-gc-id]");Y&&Y.addEventListener("click",function(){F(w[W])}),requestAnimationFrame(function(){var U=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!U||!R)){var Z=R.clientHeight-8,ie=R.clientWidth-24,ne=U.offsetHeight,re=U.offsetWidth;if(ne>0&&re>0&&Z>40){var se=Math.min(Z/ne,ie/re,1);U.style.transform="scale("+se.toFixed(3)+")",U.style.transformOrigin="top center"}}}),P.innerHTML=w.map(function(U,Z){return'<div class="col-mini-item" data-idx="'+Z+'" style="flex-shrink:0;cursor:pointer;border-radius:8px;border:1.5px solid '+(Z===W?q:"transparent")+';transition:border-color .15s;overflow:hidden">'+j(U,Z===W)+"</div>"}).join(""),P.querySelectorAll(".col-mini-item").forEach(function(U){U.addEventListener("click",function(){W=Number(U.dataset.idx),V(),U.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}V()}function B(w){var y=.54,j=Math.round(140*y),F=Math.round(310*y),q;if(!w||w._fake){var P=w?w.player:null;if(!P)return"";q=yt(P)}else q=Oe(w,"");return'<div style="width:'+j+"px;height:"+F+'px;overflow:hidden;position:relative;flex-shrink:0"><div style="transform:scale('+y+');transform-origin:top left;position:absolute;top:0;left:0;pointer-events:none">'+q+"</div></div>"}function C(w,y,j){y=y||100,j=j||140;var F=Ie[w]||{},q={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},P=Math.max(3,Math.round(y*.06)),R=Object.entries(F).map(function(V){var Y=V[0],U=V[1],Z=Y.replace(/\d+$/,""),ie=q[Z]||"#888",ne=Math.round(U.x*y),re=Math.round(U.y*j);return'<circle cx="'+ne+'" cy="'+re+'" r="'+P+'" fill="'+ie+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),W=Math.max(1,Math.round(y/50));return'<svg viewBox="0 0 '+y+" "+j+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+y+'" height="'+j+'" fill="#1A6B3C"/><rect x="'+Math.round(y*.2)+'" y="'+Math.round(j*.02)+'" width="'+Math.round(y*.6)+'" height="'+Math.round(j*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+W+'"/><line x1="0" y1="'+Math.round(j*.5)+'" x2="'+y+'" y2="'+Math.round(j*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+W+'"/><ellipse cx="'+Math.round(y*.5)+'" cy="'+Math.round(j*.5)+'" rx="'+Math.round(y*.18)+'" ry="'+Math.round(j*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+W+'"/><rect x="'+Math.round(y*.2)+'" y="'+Math.round(j*.82)+'" width="'+Math.round(y*.6)+'" height="'+Math.round(j*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+W+'"/>'+R+"</svg>"}function N(w,y,j){var F=j>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+j+"</div>":"",q=y?'data-form-id="'+y.id+'"':"",P=w.length>7?14:w.length>5?16:19,R=!!y;return"<div "+q+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(R?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(R?"":"filter:grayscale(1);opacity:0.5")+'">'+F+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(R?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+P+"px;font-weight:900;color:"+(R?"#1A6B3C":"#aaa")+';line-height:1">'+w+'</div></div><div style="flex:1;overflow:hidden;background:'+(R?"#1A6B3C":"#ccc")+'">'+C(w,140,220)+"</div></div>"}function G(w,y){var j=.54,F=Math.round(140*j),q=Math.round(305*j),P=Math.round(q*.22),R=q-P,W=w.length>7?5:7,V=C(w,F,R),Y=y?"1.5px solid #2a7a40":"1px solid #ddd",U=y?"":"filter:grayscale(1);opacity:0.45;",Z=y?"#1A6B3C":"#bbb",ie="#fff";return'<div style="width:'+F+"px;height:"+q+"px;border-radius:6px;border:"+Y+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+U+'"><div style="height:'+P+"px;background:"+Z+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+W+"px;font-weight:900;color:"+ie+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(F-4)+'px">'+w+'</span></div><div style="height:'+R+'px;overflow:hidden;flex-shrink:0">'+V+"</div></div>"}function M(w){if(v){const y=$();if(!y.length){w.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const j=y.map(F=>d.find(q=>q.player.id===F.id)||{_fake:!0,player:F,id:"fake-"+F.id});z(j,F=>F._fake?yt(F.player):Oe(F,""),F=>F._fake?B({player:F.player,id:"x",_fake:!0}):B(F),F=>{F._fake||ht(F,d,b,t)},"#1A6B3C")}else{const y=h();if(!y.length){w.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const j={},F=[];y.forEach(q=>{j[q.player.id]||(j[q.player.id]=!0,F.push(q))}),z(F,q=>{const P=b[q.player.id]||1,R=P>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${P}</div>`:"",V=d.filter(Y=>Y.player.id===q.player.id&&Y.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Oe(q,R+V)},q=>B(q),q=>ht(q,d,b,t),"#1A6B3C")}}function O(w){const y=v?g:[...T];if(!y.length){w.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const j=y.map(F=>({formation:F,card:r.find(q=>q.formation===F)||null,owned:T.has(F)}));z(j,({formation:F,card:q,owned:P})=>N(F,P?q:null,P?r.filter(R=>R.formation===F).length:0),({formation:F,owned:q})=>G(F,q),({card:F,owned:q})=>{q&&F&&ni(F,r,t,n)},"#1A6B3C")}function H(w){const y=Object.keys(u),j=v?y.length?y:m:[..._];if(!j.length){w.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const F=j.map(q=>({type:q,gc:de[q]||{icon:"⚡",desc:""},def:u[q]||null,owned:_.has(q),card:s.find(P=>P.gc_type===q)||null}));z(F,({type:q,gc:P,def:R,owned:W,card:V})=>{const Y=W?s.filter(K=>K.gc_type===q).length:0,U=Y>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${Y}</div>`:"",Z=(R==null?void 0:R.gc_type)==="ultra_game_changer",ie={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},ne={purple:"#b06ce0",light_blue:"#00d4ef"},re=ie[R==null?void 0:R.color]||ie.purple,se=ne[R==null?void 0:R.color]||ne.purple,ce=(R==null?void 0:R.effect)||P.desc||"",_e=R!=null&&R.image_url?`/manager-wars/icons/${R.image_url}`:null;return W&&V?`<div data-gc-id="${V.id}" data-gc-type="${q}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${se};background:${re};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${se}66;cursor:pointer">
          ${U}
          ...
          <div style="height:150px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${_e?`<img src="${_e}" style="max-width:120px;max-height:120px;object-fit:contain;border-radius:6px">`:`<span style="font-size:64px">${P.icon}</span>`}
          </div>
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const we=1e3;function ni(e,t,i,l){var m,b,T;const{state:p,toast:n,closeModal:a,navigate:o,refreshProfile:c}=i,d=e.formation,s={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function r(){const _=Ie[d]||{},x=Je[d]||[],A=290,D=360,v=20;function E(h){const $=_[h];return $?{x:$.x*A,y:$.y*D}:null}let L=`<svg width="${A}" height="${D}" viewBox="0 0 ${A} ${D}" xmlns="http://www.w3.org/2000/svg">`;for(const[h,$]of x){const I=E(h),S=E($);!I||!S||(L+=`<line x1="${I.x}" y1="${I.y}" x2="${S.x}" y2="${S.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const h of Object.keys(_)){const $=E(h);if(!$)continue;const I=h.replace(/\d+/,""),S=s[I]||"#555";L+=`<circle cx="${$.x}" cy="${$.y}" r="${v}" fill="${S}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,L+=`<text x="${$.x}" y="${$.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${I}</text>`}return L+="</svg>",L}const f=t.filter(_=>_.formation===d),u=f.length,g=!e.is_for_sale;l(`Formation ${d}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(m=document.getElementById("direct-sell-form-btn"))==null||m.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${d} pour ${we.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const _=f.find(A=>!A.is_for_sale)||f[0];if(!_){n("Aucune carte à vendre","error");return}const{error:x}=await k.from("cards").delete().eq("id",_.id);if(x){n(x.message,"error");return}await k.from("users").update({credits:(p.profile.credits||0)+we}).eq("id",p.profile.id),await c(),n(`+${we.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),o("collection")}),(b=document.getElementById("market-sell-form-btn"))==null||b.addEventListener("click",async()=>{const _=parseInt(document.getElementById("sell-price-form").value);if(!_||_<1){n("Prix invalide","error");return}await k.from("cards").update({is_for_sale:!0,sale_price:_}).eq("id",e.id),await k.from("market_listings").insert({seller_id:p.profile.id,card_id:e.id,price:_}),n("Carte mise en vente sur le marché !","success"),a(),o("collection")}),(T=document.getElementById("cancel-sell-form-btn"))==null||T.addEventListener("click",async()=>{await k.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await k.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),n("Annonce retirée","success"),a(),o("collection")})}async function ht(e,t,i,l){var L,h,$,I,S,z;const{state:p,toast:n,openModal:a,closeModal:o,navigate:c,refreshProfile:d}=l,s=e.player,r=t.filter(B=>B.player.id===s.id),f=r.length,u=ei[s.rarity]||1e3,g=s.rarity!=="legende",m=zt(s),b=ke(s,s.job),T=s.job2?ke(s,s.job2):null,_=Be[s.job]||"#1A6B3C",x=s.job2?Be[s.job2]:null,A=Mt[s.rarity]||"#ccc",D=tt[s.country_code]||s.country_code||"",{data:v}=await k.from("transfer_history").select("club_name, manager_name, source, price, transferred_at").eq("player_id",s.id).order("transferred_at",{ascending:!0}),E=v&&v.length?`
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
  ...
  </div>`,ri(e,t,p,i),document.getElementById("builder-back").addEventListener("click",()=>l("decks")),document.getElementById("formation-select").addEventListener("change",d=>{t.formation=d.target.value;const s=bt(t.formation),r={};s.forEach(f=>{t.slots[f]&&(r[f]=t.slots[f])}),t.slots=r,he(e,t,i)}),document.getElementById("save-deck").addEventListener("click",()=>di(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(d=>{d.addEventListener("click",()=>{t.subs=t.subs.filter(s=>s!==d.dataset.removeSub),he(e,t,i)})}),(c=document.getElementById("add-sub-btn"))==null||c.addEventListener("click",()=>{si(t,e,i)})}function ri(e,t,i,l){const p=e.querySelector("#deck-field");if(!p)return;const n=Ie[t.formation]||{},a=It(t.formation)||[],o={};for(const T of i){const _=t.slots[T],x=_?t.playerCards.find(A=>A.id===_):null;o[T]=x?x.player:null}const c=300,d=300,s=48,r=64,f=13,u=16,g=38;function m(T){const _=n[T];return _?{x:_.x*c,y:_.y*d}:null}let b="";for(const[T,_]of a){const x=m(T),A=m(_);if(!x||!A)continue;const D=o[T]?{...o[T],club_id:o[T].club_id,country_code:o[T].country_code,rarity:o[T].rarity}:null,v=o[_]?{...o[_],club_id:o[_].club_id,country_code:o[_].country_code,rarity:o[_].rarity}:null,E=fe(D,v);E==="#ff3333"||E==="#cc2222"?b+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${A.x.toFixed(1)}" y2="${A.y.toFixed(1)}" stroke="${E}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(b+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${A.x.toFixed(1)}" y2="${A.y.toFixed(1)}" stroke="${E}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,b+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${A.x.toFixed(1)}" y2="${A.y.toFixed(1)}" stroke="${E}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const T of i){const _=m(T);if(!_)continue;const x=o[T],A=T.replace(/\d+/,""),D=pe[A]||"#555",v=(_.x-s/2).toFixed(1),E=(_.y-r/2).toFixed(1),L={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[x==null?void 0:x.rarity]||"#aaa";if(x){const h=Re(x),$=jt(x),I=Ct(x.country_code),S=Number(A==="GK"?x.note_g:A==="DEF"?x.note_d:A==="MIL"?x.note_m:x.note_a)||0,z=r-f-u;b+=`<defs><clipPath id="dcp-${T}"><rect x="${v}" y="${(_.y-r/2+f).toFixed(1)}" width="${s}" height="${z}"/></clipPath></defs>`,b+=`<rect x="${v}" y="${E}" width="${s}" height="${r}" rx="5" fill="${D}"/>`,h&&(b+=`<image href="${h}" x="${v}" y="${(_.y-r/2+f).toFixed(1)}" width="${s}" height="${z}" clip-path="url(#dcp-${T})" preserveAspectRatio="xMidYMin slice"/>`),b+=`<rect x="${v}" y="${E}" width="${s}" height="${f}" fill="rgba(255,255,255,0.93)"/>`,b+=`<text x="${_.x.toFixed(1)}" y="${(_.y-r/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(x.surname_encoded||"").slice(0,9)}</text>`;const B=(_.y+r/2-u).toFixed(1);b+=`<rect x="${v}" y="${B}" width="${s}" height="${u}" fill="rgba(255,255,255,0.93)"/>`,I&&(b+=`<image href="${I}" x="${(_.x-21).toFixed(1)}" y="${(_.y+r/2-u+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),b+=`<text x="${_.x.toFixed(1)}" y="${(_.y+r/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${S}</text>`,$&&(b+=`<image href="${$}" x="${(_.x+s/2-14).toFixed(1)}" y="${(_.y+r/2-u+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),b+=`<rect x="${v}" y="${E}" width="${s}" height="${r}" rx="5" fill="none" stroke="${L}" stroke-width="2"/>`}else b+=`<rect x="${v}" y="${E}" width="${s}" height="${r}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,b+=`<text x="${_.x.toFixed(1)}" y="${_.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,b+=`<text x="${_.x.toFixed(1)}" y="${(_.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${A}</text>`;b+=`<rect x="${v}" y="${E}" width="${s}" height="${r}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${T}" style="cursor:pointer"/>`}p.innerHTML=`<svg viewBox="${-g} ${-g} ${c+g*2} ${d+g*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${b}</svg>`,p.querySelectorAll(".deck-slot-hit").forEach(T=>{T.addEventListener("click",()=>ai(T.dataset.pos,t,e,l))})}function ai(e,t,i,l){var f,u,g;const{openModal:p,closeModal:n}=l,a=e.replace(/\d+/,""),o=t.slots[e],c=o?t.playerCards.find(m=>m.id===o):null;(f=c==null?void 0:c.player)==null||f.id;const d=new Set;Object.entries(t.slots).forEach(([m,b])=>{var _;if(m===e||!b)return;const T=t.playerCards.find(x=>x.id===b);(_=T==null?void 0:T.player)!=null&&_.id&&d.add(T.player.id)}),t.subs.forEach(m=>{var T;const b=t.playerCards.find(_=>_.id===m);(T=b==null?void 0:b.player)!=null&&T.id&&d.add(b.player.id)});const s=new Set,r=t.playerCards.filter(m=>{const b=m.player;return!(b.job===a||b.job2===a)||d.has(b.id)||s.has(b.id)?!1:(s.add(b.id),!0)});r.sort((m,b)=>{const T=a==="GK"?m.player.note_g:a==="DEF"?m.player.note_d:a==="MIL"?m.player.note_m:m.player.note_a;return(a==="GK"?b.player.note_g:a==="DEF"?b.player.note_d:a==="MIL"?b.player.note_m:b.player.note_a)-T}),p(`Choisir ${a} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${r.length>0?r.map(m=>{var A,D;const b=m.player,T=a==="GK"?b.note_g:a==="DEF"?b.note_d:a==="MIL"?b.note_m:b.note_a,_=Re(b),x={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[b.rarity];return`<div class="player-option" data-card-id="${m.id}"
          ...
          <div style="width:44px;height:44px;border-radius:8px;overflow:hidden;flex-shrink:0;background:#dde;border:2px solid ${pe[a]}">
            ${_?`<img src="${_}" style="width:100%;height:100%;object-fit:cover">`:`<div style="width:100%;height:100%;background:${pe[a]};display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:700">${a}</div>`}
          </div>
          <!-- Infos -->
  ...
  `,document.body.appendChild(l);let p=!1;document.getElementById("pack-visual").addEventListener("click",()=>{if(p)return;p=!0;const u=document.getElementById("pack-visual");u.classList.add("shaking"),setTimeout(()=>{u.classList.add("pack-open"),setTimeout(()=>{document.getElementById("pack-phase").style.display="none",a(0)},600)},500)});let n=0;function a(u){n=u;const g=document.getElementById("reveal-phase");g.style.display="flex",o(u)}function o(u){var v;const g=e[u],m=document.getElementById("card-counter"),b=document.getElementById("single-card-slot"),T=document.getElementById("card-tap-hint");m&&(m.textContent=`Carte ${u+1} / ${e.length}`),T&&(T.textContent=u<e.length-1?"Appuie pour continuer →":"Appuie pour voir toutes tes cartes");const _=wt(g),x=g.card_type==="player"&&((v=g.player)==null?void 0:v.rarity)==="legende";b.innerHTML=`
      ...
      </div>`).join("")}let d=null;function s(){const u=document.getElementById("fireworks-canvas");if(!u)return;u.width=window.innerWidth,u.height=window.innerHeight;const g=u.getContext("2d"),m=[];function b(){const _=Math.random()*u.width,x=Math.random()*u.height*.6,A=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],D=A[Math.floor(Math.random()*A.length)];for(let v=0;v<60;v++){const E=Math.PI*2/60*v,L=2+Math.random()*5;m.push({x:_,y:x,vx:Math.cos(E)*L,vy:Math.sin(E)*L,alpha:1,color:D,size:2+Math.random()*3})}}b(),d=setInterval(b,600);function T(){if(document.getElementById("fireworks-canvas")){g.clearRect(0,0,u.width,u.height);for(let _=m.length-1;_>=0;_--){const x=m[_];if(x.x+=x.vx,x.y+=x.vy+.08,x.vy*=.98,x.alpha-=.018,x.alpha<=0){m.splice(_,1);continue}g.globalAlpha=x.alpha,g.fillStyle=x.color,g.beginPath(),g.arc(x.x,x.y,x.size,0,Math.PI*2),g.fill()}g.globalAlpha=1,(d!==null||m.length>0)&&requestAnimationFrame(T)}}T()}function r(){d!==null&&(clearInterval(d),d=null);const u=document.getElementById("fireworks-canvas");u&&u.getContext("2d").clearRect(0,0,u.width,u.height)}document.getElementById("reveal-collection").addEventListener("click",()=>{r(),l.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{r(),l.remove(),i("boosters")})}function wt(e){var t,i,l,p;if(e.card_type==="player"&&e.player){const n=e.player,a=n.job||"ATT",o=mi[a]||"#1A6B3C",c=yi[n.rarity]||"#ccc",d=a==="GK"?n.note_g:a==="DEF"?n.note_d:a==="MIL"?n.note_m:n.note_a,s=xi(n),r={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL"}[n.country_code]||n.country_code;return`<div style="width:140px;height:200px;background:#f2e8d2;border-radius:12px;border:3px solid ${c};overflow:hidden;display:flex;flex-direction:column">
      <!-- Nom -->
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const Mi="simulation",Ft="VOTRE_ZONE_ID";function zi(){switch(Mi){case"propellerads":return Ci();case"adsense":return ji();default:return it()}}function it(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const r=()=>document.getElementById("mm-status"),f=async(T=!0)=>{d=!0,s&&(k.removeChannel(s),s=null),T&&await k.rpc("cancel_matchmaking",{p_user_id:a.profile.id}).catch(()=>{})};(b=document.getElementById("mm-cancel"))==null||b.addEventListener("click",async()=>{try{await f(!0)}catch(T){console.warn("[Matchmaking] cleanup error:",T)}le(e),o("home")});const u=async(T,_,x)=>{d||(d=!0,s&&(k.removeChannel(s),s=null),r()&&(r().textContent="Adversaire trouvé !"),await new Promise(A=>setTimeout(A,600)),e.isConnected!==!1&&Gi(e,t,T,x))},{data:g,error:m}=await k.rpc("try_matchmake",{p_user_id:a.profile.id,p_deck_id:i});if(m||!(g!=null&&g.success)){c("Erreur de matchmaking","error"),le(e),o("home");return}if(g.matched){u(g.match_id,g.opponent_id,!1);return}r()&&(r().textContent="En attente d'un autre joueur..."),s=k.channel("matchmaking-"+a.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${a.profile.id}`},T=>{const _=T.new;_.status==="matched"&&_.match_id&&u(_.match_id,_.matched_with,!0)}).subscribe()}async function Gi(e,t,i,l){const{state:p,navigate:n,toast:a}=t,o=l?"p1":"p2",c=l?"p2":"p1";e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation du match...</div>';const{data:d}=await k.from("matches").select("*").eq("id",i).single();if(!d){a("Match introuvable","error"),n("home");return}async function s(){const[{data:h,error:$},{data:I,error:S},{data:z},{data:B}]=await Promise.all([k.rpc("get_deck_for_match",{p_deck_id:d.home_deck_id}),k.rpc("get_deck_for_match",{p_deck_id:d.away_deck_id}),k.from("users").select("id,pseudo,club_name").eq("id",d.home_id).single(),k.from("users").select("id,pseudo,club_name").eq("id",d.away_id).single()]);$&&console.error("[PvP] get_deck_for_match p1 error:",$),S&&console.error("[PvP] get_deck_for_match p2 error:",S);function C(y,j){return{cardId:y.card_id,position:j||y.position,id:y.id,firstname:y.firstname,name:y.surname_encoded,country_code:y.country_code,club_id:y.club_id,job:y.job,job2:y.job2,note_g:Number(y.note_g)||0,note_d:Number(y.note_d)||0,note_m:Number(y.note_m)||0,note_a:Number(y.note_a)||0,rarity:y.rarity,skin:y.skin,hair:y.hair,hair_length:y.hair_length,clubName:y.club_encoded_name||null,clubLogo:y.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}}const N=((h==null?void 0:h.starters)||[]).map(y=>C(y,y.position)),G=((I==null?void 0:I.starters)||[]).map(y=>C(y,y.position)),M=((h==null?void 0:h.subs)||[]).map(y=>C(y,y.position)),O=((I==null?void 0:I.subs)||[]).map(y=>C(y,y.position)),H=(h==null?void 0:h.formation)||"4-4-2",w=(I==null?void 0:I.formation)||"4-4-2";return console.log("[PvP] p1Formation:",H,"p1 starters:",N.length,"positions:",N.map(y=>y.position)),console.log("[PvP] p2Formation:",w,"p2 starters:",G.length,"positions:",G.map(y=>y.position)),{p1Team:Ge(N,H),p2Team:Ge(G,w),p1Subs:M,p2Subs:O,p1Formation:H,p2Formation:w,p1Name:(z==null?void 0:z.club_name)||(z==null?void 0:z.pseudo)||"Joueur 1",p2Name:(B==null?void 0:B.club_name)||(B==null?void 0:B.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"gc-select",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:[],gc_p2:[],gcReady_p1:!1,gcReady_p2:!1,usedGc_p1:[],usedGc_p2:[],lastActionAt:new Date().toISOString()}}let r=d.game_state&&Object.keys(d.game_state).length?d.game_state:null;if(!r)if(l){r=await s();const{data:h}=await k.from("matches").select("game_state").eq("id",i).single();!(h!=null&&h.game_state)||!Object.keys(h.game_state).length?await k.from("matches").update({game_state:r,turn_user_id:d.home_id}).eq("id",i):r=h.game_state}else{e.innerHTML=`<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation avec l'adversaire...</div>`;for(let h=0;h<30&&!r;h++){await new Promise(I=>setTimeout(I,400));const{data:$}=await k.from("matches").select("game_state").eq("id",i).single();$!=null&&$.game_state&&Object.keys($.game_state).length&&(r=$.game_state)}if(!r){a("Erreur de synchronisation","error"),n("home");return}}const f=k.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},h=>{const $=h.new;try{$.game_state&&(r=$.game_state,m()),($.status==="finished"||$.forfeit)&&m()}catch(I){console.error("[PvP] Realtime render crash:",I,"gameState:",r),a("Erreur de synchro temps réel : "+I.message,"error")}}).subscribe();async function u(h){Object.assign(r,h),r.lastActionAt=new Date().toISOString();const{error:$}=await k.from("matches").update({game_state:r}).eq("id",i);$&&(console.error("[PvP] pushState DB error:",$.message,$),a("Erreur de synchronisation : "+$.message,"error"));try{m()}catch(I){console.error("[PvP] renderPvpScreen crash après pushState:",I),a("Erreur d'affichage : "+I.message,"error")}}async function g(){const h=l?d.away_id:d.home_id;await k.from("matches").update({status:"finished",forfeit:!0,winner_id:h}).eq("id",i),k.removeChannel(f),le(e),n("home")}function m(){var re,se,ce,_e;const h=r[o+"Team"];r[c+"Team"];const $=r[o+"Score"],I=r[c+"Score"],S=r[o+"Name"],z=r[c+"Name"];if(r.phase==="gc-select")return b();if(r.phase==="reveal")return T();if(r.phase==="midfield")return _();if(r.phase==="finished")return v();const B=r.phase===o+"-attack",C=r.phase===o+"-defense",N=r.phase===c+"-attack"||r.phase===c+"-defense",G=Array.isArray(r["selected_"+o])?r["selected_"+o]:[],M=G.map(K=>K.cardId);e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const O=window.innerWidth>=700,H=r[o+"Subs"]||[],w=r["usedSubIds_"+o]||[],y=H.filter(K=>!w.includes(K.cardId)),j=r["gcCardsFull_"+o]||[],F=r["usedGc_"+o]||[],q=j.filter(K=>!F.includes(K.id)),P=r.boostOwner===o&&!r.boostUsed,R=(K,oe,ae)=>{var pt,ut;const X=K._gcDef,ge={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[X==null?void 0:X.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Te={purple:"#b06ce0",light_blue:"#00d4ef"}[X==null?void 0:X.color]||"#b06ce0",xe=(X==null?void 0:X.name)||K.gc_type,Kt=(X==null?void 0:X.effect)||((pt=de[K.gc_type])==null?void 0:pt.desc)||"",dt=X!=null&&X.image_url?`/manager-wars/icons/${X.image_url}`:null,Ut=((ut=de[K.gc_type])==null?void 0:ut.icon)||"⚡",lt=Math.round(ae*.22),ct=Math.round(ae*.22),Ne=ae-lt-ct,Vt=xe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${K.id}" data-gc-type="${K.gc_type}"
      ...
      </div>`,e.querySelectorAll(".match-slot-hit").forEach(K=>{K.addEventListener("click",()=>{if(!B&&!C)return;const oe=K.dataset.cardId,ae=K.dataset.role,X=(h[ae]||[]).find(xe=>xe.cardId===oe);if(!X||X.used)return;Array.isArray(r["selected_"+o])||(r["selected_"+o]=[]);const ge=r["selected_"+o],Te=ge.findIndex(xe=>xe.cardId===oe);Te>-1?ge.splice(Te,1):ge.length<3&&ge.push({...X,_role:ae}),m()})}),(re=e.querySelector("#pvp-boost-card"))==null||re.addEventListener("click",()=>{x()}),e.querySelectorAll(".pvp-gc-mini").forEach(K=>{K.addEventListener("click",()=>{D(K.dataset.gcId,K.dataset.gcType)})}),(se=e.querySelector("#pvp-action"))==null||se.addEventListener("click",()=>{B?E():C&&L()}),(ce=e.querySelector("#pvp-quit"))==null||ce.addEventListener("click",()=>{confirm("Quitter le match ? Vous perdrez par forfait.")&&g()}),(_e=e.querySelector("#pvp-view-opp"))==null||_e.addEventListener("click",()=>{A()}),r._timerInt&&(clearInterval(r._timerInt),r._timerInt=null),B||C){let K=30,oe=!1;const ae=()=>document.getElementById("pvp-timer"),X=()=>{ae()&&(ae().textContent=K+"s",ae().style.color=oe?"#ff4444":"#fff")};X(),r._timerInt=setInterval(()=>{K--,K<=15&&!oe&&(oe=!0),X(),K<=0&&(clearInterval(r._timerInt),r._timerInt=null,g())},1e3)}}function b(){var C;const h=r["gcReady_"+o],$=r["gcReady_"+c];Array.isArray(r["gc_"+o])||(r["gc_"+o]=[]);let I=r["gc_"+o];const S=3;function z(N,G){const M=N._gcDef,O={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},H={purple:"#9b59b6",light_blue:"#00bcd4"},w=O[M==null?void 0:M.color]||O.purple,y=H[M==null?void 0:M.color]||H.purple;return`<div class="pvp-gc-card" data-id="${N.id}"
        style="width:100px;border-radius:10px;border:3px solid ${G?"#FFD700":y};background:${w};
    ...
    </div>`,h)return;const B=o==="p1"?d.home_id:d.away_id;k.from("cards").select("id,gc_type").eq("owner_id",B).eq("card_type","game_changer").then(async({data:N})=>{const{data:G}=await k.from("gc_definitions").select("*").eq("is_active",!0),M=new Set,O=(N||[]).filter(w=>M.has(w.gc_type)?!1:(M.add(w.gc_type),!0)).map(w=>({...w,_gcDef:(G==null?void 0:G.find(y=>y.name===w.gc_type))||null})),H=document.getElementById("pvp-gc-grid");if(H){if(!O.length){H.innerHTML='<div style="color:rgba(255,255,255,0.4);font-size:13px;text-align:center;margin-top:30px">Aucune carte Game Changer disponible.</div>';return}H.innerHTML=O.map(w=>z(w,I.includes(w.id))).join(""),H.querySelectorAll(".pvp-gc-card").forEach(w=>{w.addEventListener("click",()=>{const y=w.dataset.id,j=I.indexOf(y);j>-1?I.splice(j,1):I.length<S&&I.push(y),m()})})}}),(C=e.querySelector("#pvp-gc-validate"))==null||C.addEventListener("click",async()=>{const N=o==="p1"?d.home_id:d.away_id,{data:G}=await k.from("cards").select("id,gc_type").eq("owner_id",N).eq("card_type","game_changer").in("id",I),{data:M}=await k.from("gc_definitions").select("*").eq("is_active",!0),O=(G||[]).map(H=>({...H,_gcDef:(M==null?void 0:M.find(w=>w.name===H.gc_type))||null}));await u({["gcReady_"+o]:!0,["gcCardsFull_"+o]:O}),r["gcReady_"+c]&&await u({phase:"reveal"})})}function T(){e.style.overflow="hidden",e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      ...
      <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${G.map((H,w)=>{const y=te(H),j=w<G.length-1?fe(H,G[w+1]):null,F=j&&j!=="#ff3333"&&j!=="#cc2222";return`
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    </div>`,setTimeout(()=>{const G=document.getElementById("pvp-score-me"),M=document.getElementById("pvp-score-opp"),O=document.getElementById("pvp-midfield-result");G&&M&&(C?(G.style.fontSize="80px",G.style.color="#FFD700",M.style.opacity="0.25"):(M.style.fontSize="80px",M.style.color="#ff6b6b",G.style.opacity="0.25"));const H=r.p1Team.MIL||[],w=r.p2Team.MIL||[],y=I(H)+S(H),j=I(w)+S(w),F=y>=j?"p1":"p2";r[F+"Name"],O&&(O.style.opacity="1",O.innerHTML=`<div style="font-size:20px;font-weight:900;margin-bottom:10px">
      ...
      </div>`,document.body.appendChild(w),(y=w.querySelector("#pvp-gc-back"))==null||y.addEventListener("click",()=>w.remove()),(j=w.querySelector("#pvp-gc-use"))==null||j.addEventListener("click",async()=>{w.remove();const F=r["usedGc_"+o]||[];F.push(h),await u({["usedGc_"+o]:F})})}function v(){var z;const h=r[o+"Score"],$=r[c+"Score"],I=h>$,S=h===$;e.style.overflow="hidden",e.innerHTML=`
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order");let c=0;function d(){var _,x,A,D,v;const s=n[c],r=(o||[]).filter(E=>E.deck_id===s.id),f=r.filter(E=>{var L;return E.is_starter&&((L=E.card)==null?void 0:L.player)}).map(E=>Ue(E.card,E.position)),u=r.find(E=>{var L;return((L=E.card)==null?void 0:L.card_type)==="formation"}),g=s.formation||((_=u==null?void 0:u.card)==null?void 0:_.formation)||"4-4-2",m=f.length>=11?Ge(f,g):null,b=f.length>=11;e.innerHTML=`
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${r.map((g,m)=>{const b=te(g),T=m<r.length-1?fe(g,r[m+1]):null,_=T&&T!=="#ff3333"&&T!=="#cc2222";return`
          ...
          ${T?`<div style="width:14px;height:4px;border-radius:2px;background:${T};flex-shrink:0;opacity:${_?.9:.3}"></div>`:""}
          `}).join("")}
      </div>
      <div style="margin-top:6px;font-size:11px;color:rgba(255,255,255,0.5)">
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${r}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${p?`<img src="${p}" style="width:${r-4}px;height:${r-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,r-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
        ...
        stroke="${L}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const f=48,u=64,g=13,m=16,b={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[_,x]of Object.entries(c)){const A=s(_);if(!A||!x)continue;const D=_.replace(/[0-9]/g,""),v=be[D]||"#555",E=i==="attack"&&["MIL","ATT"].includes(D)&&!x.used||i==="defense"&&["GK","DEF","MIL"].includes(D)&&!x.used,L=l.includes(x.cardId);let h;i==="attack"?h=D==="MIL"?Number(x.note_m)||0:Number(x.note_a)||0:i==="defense"?h=D==="GK"?Number(x.note_g)||0:D==="MIL"?Number(x.note_m)||0:Number(x.note_d)||0:h=Number(D==="GK"?x.note_g:D==="DEF"?x.note_d:D==="MIL"?x.note_m:x.note_a)||0,h=h+(x.boost||0);const $=(A.x-f/2).toFixed(1),I=(A.y-u/2).toFixed(1),S=x.used?.25:1,z=b[x==null?void 0:x.rarity]||b.normal,B=L?"#ff3030":z,C=L?3:(x==null?void 0:x.rarity)==="légende"||(x==null?void 0:x.rarity)==="pépite"?2.5:2,N=u-g-m;r+=`<defs><clipPath id="cp-${_}"><rect x="${$}" y="${(A.y-u/2+g).toFixed(1)}" width="${f}" height="${N}"/></clipPath></defs>`,r+=`<rect x="${$}" y="${I}" width="${f}" height="${u}" rx="5" fill="${v}" opacity="${S}"/>`;const G=te(x);G&&!x.used&&(r+=`<image href="${G}" x="${$}" y="${(A.y-u/2+g).toFixed(1)}" width="${f}" height="${N}" clip-path="url(#cp-${_})" preserveAspectRatio="xMidYMin slice"/>`),r+=`<rect x="${$}" y="${I}" width="${f}" height="${g}" rx="4" fill="rgba(255,255,255,0.92)"/>`,r+=`<text x="${A.x.toFixed(1)}" y="${(A.y-u/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(x.name||"").slice(0,9)}</text>`;const M=(A.y+u/2-m).toFixed(1);if(r+=`<rect x="${$}" y="${M}" width="${f}" height="${m}" fill="rgba(255,255,255,0.92)"/>`,x.used)r+=`<text x="${A.x.toFixed(1)}" y="${(A.y+u/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="13" font-weight="900" fill="rgba(0,0,0,0.4)" font-family="Arial Black">–</text>`;else{const O=Ve(x.country_code);O?r+=`<image href="${O}" x="${(A.x-20).toFixed(1)}" y="${(A.y+u/2-m+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:r+=`<text x="${(A.x-13).toFixed(1)}" y="${(A.y+u/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${Rt(x.country_code)}</text>`,r+=`<text x="${A.x.toFixed(1)}" y="${(A.y+u/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${h}</text>`;const H=qt(x);H?r+=`<image href="${H}" x="${(A.x+f/2-14).toFixed(1)}" y="${(A.y+u/2-m+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:x.clubName&&(r+=`<text x="${(A.x+14).toFixed(1)}" y="${(A.y+u/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(x.clubName||"").slice(0,3).toUpperCase()}</text>`),x.boost&&(r+=`<rect x="${(A.x+f/2-12).toFixed(1)}" y="${(A.y-u/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,r+=`<text x="${(A.x+f/2-5).toFixed(1)}" y="${(A.y-u/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${x.boost}</text>`)}r+=`<rect x="${$}" y="${I}" width="${f}" height="${u}" rx="5" fill="${L?"rgba(255,255,255,0.12)":"none"}" stroke="${B}" stroke-width="${C}" opacity="${S}"/>`,E&&(r+=`<rect x="${$}" y="${I}" width="${f}" height="${u}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${L?"selected":""}" data-card-id="${x.cardId}" data-role="${D}" style="cursor:pointer"/>`)}const T=38;return`<svg viewBox="${-T} ${-T} ${p+T*2} ${n+T*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${r}
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;height:100%;width:100%">
    ...
    ${(()=>{const v=window.innerWidth>=700,E=(C,N,G)=>{var V,Y;const M=(t.gcDefs||[]).find(U=>U.name===C.gc_type),O={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[M==null?void 0:M.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",H={purple:"#b06ce0",light_blue:"#00d4ef"}[M==null?void 0:M.color]||"#b06ce0",w=(M==null?void 0:M.name)||C.gc_type,y=(M==null?void 0:M.effect)||((V=de[C.gc_type])==null?void 0:V.desc)||"",j=M!=null&&M.image_url?`/manager-wars/icons/${M.image_url}`:null,F=((Y=de[C.gc_type])==null?void 0:Y.icon)||"⚡",q=Math.round(G*.22),P=Math.round(G*.22),R=G-q-P,W=w.length>12?7:9;return`<div class="gc-mini" data-gc-id="${C.id}" data-gc-type="${C.gc_type}"
          ...
          <div style="height:${q}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
            <span style="font-size:${W}px;font-weight:900;color:#fff;text-align:center;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${N-6}px">${w}</span>
            <span style="font-size:6px;color:rgba(255,255,255,0.45)">⚡ GC</span>
          </div>
  ...
  `;function c(d){const s=document.getElementById("mkt-content"),r=d==="buy"?a:o;if(r.length===0){s.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${f.map(u=>{var x,A,D,v;const g=(x=u.card)==null?void 0:x.player;if(!g)return"";const m=g.job==="GK"?g.note_g:g.job==="DEF"?g.note_d:g.job==="MIL"?g.note_m:g.note_a,b=en[g.rarity],T=(i.profile.credits||0)>=u.price,_=u.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${_?"opacity:0.65":""}">
          ...
          <div style="flex:1;min-width:0">
            ...
            ${d==="buy"?`<div style="font-size:11px;color:var(--gray-600)">Vendeur : ${((D=u.seller)==null?void 0:D.pseudo)||"—"}</div>`:_?`<div style="font-size:11px;color:var(--green)">✅ Vendu à ${((v=u.buyer)==null?void 0:v.pseudo)||"—"} · ${u.sold_at?new Date(u.sold_at).toLocaleDateString("fr"):""}</div>`:`<div style="font-size:11px;color:var(--gray-600)">🟢 En vente depuis le ${new Date(u.listed_at).toLocaleDateString("fr")}</div>`}
          </div>
