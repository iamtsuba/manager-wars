import{s as $,F as tt,b as Ce,c as Dt,l as ge,d as si}from"./formation-links-Bt44rMnM.js";function $t(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),n=(d||[]).filter(_=>_.card_type==="player"&&_.player),r=(d||[]).filter(_=>_.card_type==="game_changer"),p=(d||[]).filter(_=>_.card_type==="formation"),{data:g}=await $.from("gc_definitions").select("name,gc_type,color,effect,image_url"),f={};(g||[]).forEach(_=>{f[_.name]=_});const y=Object.keys(tt),u=Object.keys(de),h={};n.forEach(_=>{const b=_.player.id;h[b]=(h[b]||0)+1}),new Set(Object.keys(h).map(_=>String(_)));const L=new Set(p.map(_=>_.formation)),v=new Set(r.map(_=>_.gc_type));let x="player",k="Tous",B="",m=!1;function I(){return[...n].sort((_,b)=>{const C=Be.indexOf(_.player.job),E=Be.indexOf(b.player.job);return C!==E?C-E:(_.player.surname_encoded||"").localeCompare(b.player.surname_encoded||"")})}function M(){return[...l||[]].sort((_,b)=>{const C=Be.indexOf(_.job),E=Be.indexOf(b.job);return C!==E?C-E:(_.surname_encoded||"").localeCompare(b.surname_encoded||"")})}function R(){return I().filter(_=>{const b=_.player,C=k==="Tous"||b.job===k,E=!B||`${b.firstname} ${b.surname_encoded}`.toLowerCase().includes(B);return C&&E})}function q(){return M().filter(_=>{const b=k==="Tous"||_.job===k,C=!B||`${_.firstname} ${_.surname_encoded}`.toLowerCase().includes(B);return b&&C})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Ee=1e3;function yi(e,t,i,s){var u,h,L;const{state:c,toast:o,closeModal:a,navigate:d,refreshProfile:l}=i,n=e.formation,r={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function p(){const v=Ce[n]||{},x=tt[n]||[],k=290,B=360,m=20;function I(R){const q=v[R];return q?{x:q.x*k,y:q.y*B}:null}let M=`<svg width="${k}" height="${B}" viewBox="0 0 ${k} ${B}" xmlns="http://www.w3.org/2000/svg">`;for(const[R,q]of x){const P=I(R),V=I(q);!P||!V||(M+=`<line x1="${P.x}" y1="${P.y}" x2="${V.x}" y2="${V.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const R of Object.keys(v)){const q=I(R);if(!q)continue;const P=R.replace(/\d+/,""),V=r[P]||"#555";M+=`<circle cx="${q.x}" cy="${q.y}" r="${m}" fill="${V}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,M+=`<text x="${q.x}" y="${q.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${P}</text>`}return M+="</svg>",M}const g=t.filter(v=>v.formation===n),f=g.length,y=!e.is_for_sale;s(`Formation ${n}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const Hi="simulation",Kt="VOTRE_ZONE_ID";function Ui(){switch(Hi){case"propellerads":return Ki();case"adsense":return Vi();default:return at()}}function at(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order");let l=0;function n(){var v,x,k,B,m;const r=o[l],p=(d||[]).filter(I=>I.deck_id===r.id),g=p.filter(I=>{var M;return I.is_starter&&((M=I.card)==null?void 0:M.player)}).map(I=>Je(I.card,I.position)),f=p.find(I=>{var M;return((M=I.card)==null?void 0:M.card_type)==="formation"}),y=r.formation||((v=f==null?void 0:f.card)==null?void 0:v.formation)||"4-4-2",u=g.length>=11?Oe(g,y):null,h=g.length>=11;Yt(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${p}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${c?`<img src="${c}" style="width:${p-4}px;height:${p-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,p-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
        ...
        stroke="${M}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=48,f=64,y=13,u=16,h={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[v,x]of Object.entries(l)){const k=r(v);if(!k||!x)continue;const B=v.replace(/[0-9]/g,""),m=ke[B]||"#555",I=i==="attack"&&["MIL","ATT"].includes(B)&&!x.used||i==="defense"&&["GK","DEF","MIL"].includes(B)&&!x.used,M=s.includes(x.cardId);let R;i==="attack"?R=B==="MIL"?Number(x.note_m)||0:Number(x.note_a)||0:i==="defense"?R=B==="GK"?Number(x.note_g)||0:B==="MIL"?Number(x.note_m)||0:Number(x.note_d)||0:R=Number(B==="GK"?x.note_g:B==="DEF"?x.note_d:B==="MIL"?x.note_m:x.note_a)||0,R=R+(x.boost||0);const q=(k.x-g/2).toFixed(1),P=(k.y-f/2).toFixed(1),V=x.used?.25:1,Z=h[x==null?void 0:x.rarity]||h.normal,K=M?"#ff3030":Z,ee=M?3:(x==null?void 0:x.rarity)==="légende"||(x==null?void 0:x.rarity)==="pépite"?2.5:2,A=f-y-u;p+=`<defs><clipPath id="cp-${v}"><rect x="${q}" y="${(k.y-f/2+y).toFixed(1)}" width="${g}" height="${A}"/></clipPath></defs>`,p+=`<rect x="${q}" y="${P}" width="${g}" height="${f}" rx="5" fill="${m}" opacity="${V}"/>`;const z=oe(x);z&&!x.used&&(p+=`<image href="${z}" x="${q}" y="${(k.y-f/2+y).toFixed(1)}" width="${g}" height="${A}" clip-path="url(#cp-${v})" preserveAspectRatio="xMidYMin slice"/>`),p+=`<rect x="${q}" y="${P}" width="${g}" height="${y}" rx="4" fill="rgba(255,255,255,0.92)"/>`,p+=`<text x="${k.x.toFixed(1)}" y="${(k.y-f/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(x.name||"").slice(0,9)}</text>`;const F=(k.y+f/2-u).toFixed(1);if(p+=`<rect x="${q}" y="${F}" width="${g}" height="${u}" fill="rgba(255,255,255,0.92)"/>`,x.used)p+=`<text x="${k.x.toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="13" font-weight="900" fill="rgba(0,0,0,0.4)" font-family="Arial Black">–</text>`;else{const G=Xe(x.country_code);G?p+=`<image href="${G}" x="${(k.x-20).toFixed(1)}" y="${(k.y+f/2-u+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:p+=`<text x="${(k.x-13).toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${Xt(x.country_code)}</text>`,p+=`<text x="${k.x.toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${R}</text>`;const D=Jt(x);D?p+=`<image href="${D}" x="${(k.x+g/2-14).toFixed(1)}" y="${(k.y+f/2-u+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:x.clubName&&(p+=`<text x="${(k.x+14).toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(x.clubName||"").slice(0,3).toUpperCase()}</text>`),x.boost&&(p+=`<rect x="${(k.x+g/2-12).toFixed(1)}" y="${(k.y-f/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,p+=`<text x="${(k.x+g/2-5).toFixed(1)}" y="${(k.y-f/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${x.boost}</text>`)}p+=`<rect x="${q}" y="${P}" width="${g}" height="${f}" rx="5" fill="${M?"rgba(255,255,255,0.12)":"none"}" stroke="${K}" stroke-width="${ee}" opacity="${V}"/>`,I&&(p+=`<rect x="${q}" y="${P}" width="${g}" height="${f}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${M?"selected":""}" data-card-id="${x.cardId}" data-role="${B}" style="cursor:pointer"/>`)}const L=38;return`<svg viewBox="${-L} ${-L} ${c+L*2} ${o+L*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${p}
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;height:100%;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:100px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const I=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
            ...
            ${Ie((I.players||[]).map(M=>({...M,used:!1})),"#ff6b6b",I.total)}
            ...
            ${Ie((I.players||[]).map(M=>({...M,used:!1})),"#00ff88",I.total)}
    ...
    ${(()=>{const m=window.innerWidth>=700,I=(A,z,F)=>{var H,U;const G=(t.gcDefs||[]).find(W=>W.name===A.gc_type),D={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[G==null?void 0:G.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",_={purple:"#b06ce0",light_blue:"#00d4ef"}[G==null?void 0:G.color]||"#b06ce0",b=(G==null?void 0:G.name)||A.gc_type,C=(G==null?void 0:G.effect)||((H=de[A.gc_type])==null?void 0:H.desc)||"",E=G!=null&&G.image_url?`/manager-wars/icons/${G.image_url}`:null,w=((U=de[A.gc_type])==null?void 0:U.icon)||"⚡",S=Math.round(F*.22),T=Math.round(F*.22),N=F-S-T,j=b.length>12?7:9;return`<div class="gc-mini" data-gc-id="${A.id}" data-gc-type="${A.gc_type}"
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(m=>{var I,M,R;if(m.type==="duel"){const q=m.isGoal,P=m.homeScored?"#FFD700":q?"#ff6b6b":"rgba(255,255,255,0.3)",V=m.homeScored?"⚽ BUT !":q?"⚽ BUT IA !":(I=m.homePlayers)!=null&&I.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${q?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${P};margin-bottom:4px">
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(o);let d=!1;const l=()=>{d||(d=!0,o.remove(),setTimeout(()=>c(),50))};o.addEventListener("click",l),setTimeout(l,3500)}async function we(e,t,i){var r,p;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:s}=i,c=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,a=c?"victoire":o?"nul":"defaite",d=ui(t.mode,a);t.matchId&&await $.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?s.profile.id:null,home_credits_reward:d,played_at:new Date().toISOString()}).eq("id",t.matchId);const l={credits:(s.profile.credits||0)+d,matches_played:(s.profile.matches_played||0)+1};c?l.wins=(s.profile.wins||0)+1:o?l.draws=(s.profile.draws||0)+1:l.losses=(s.profile.losses||0)+1,await $.from("users").update(l).eq("id",s.profile.id),await i.refreshProfile();const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",n.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const f=()=>document.getElementById("mm-status"),y=async(x=!0)=>{p=!0,g&&($.removeChannel(g),g=null),x&&await $.rpc("cancel_matchmaking",{p_user_id:l.profile.id}).catch(()=>{})};(v=document.getElementById("mm-cancel"))==null||v.addEventListener("click",async()=>{try{await y(!0)}catch(x){console.warn("[Matchmaking] cleanup error:",x)}le(e),n("home")});const u=async(x,k,B)=>{p||(p=!0,g&&($.removeChannel(g),g=null),f()&&(f().textContent="Adversaire trouvé !"),await new Promise(m=>setTimeout(m,600)),e.isConnected!==!1&&fo(e,t,x,B,a,d))},{data:h,error:L}=await $.rpc("try_matchmake",{p_user_id:l.profile.id,p_deck_id:i});if(L||!(h!=null&&h.success)){r("Erreur de matchmaking","error"),le(e),n("home");return}if(h.matched){u(h.match_id,h.opponent_id,!1);return}f()&&(f().textContent="En attente d'un autre joueur..."),g=$.channel("matchmaking-"+l.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${l.profile.id}`},x=>{const k=x.new;k.status==="matched"&&k.match_id&&u(k.match_id,k.matched_with,!0)}).subscribe()}async function fo(e,t,i,s,c=[],o=[]){const{state:a,navigate:d,toast:l}=t,n=s?"p1":"p2",r=s?"p2":"p1",p=(c||[]).map(A=>A.id),g=(c||[]).map(A=>({id:A.id,gc_type:A.gc_type,_gcDef:A._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation du match...</div>';const{data:f}=await $.from("matches").select("*").eq("id",i).single();if(!f){l("Match introuvable","error"),d("home");return}async function y(){const[{data:A,error:z},{data:F,error:G},{data:D},{data:_}]=await Promise.all([$.rpc("get_deck_for_match",{p_deck_id:f.home_deck_id}),$.rpc("get_deck_for_match",{p_deck_id:f.away_deck_id}),$.from("users").select("id,pseudo,club_name").eq("id",f.home_id).single(),$.from("users").select("id,pseudo,club_name").eq("id",f.away_id).single()]);z&&console.error("[PvP] get_deck_for_match p1 error:",z),G&&console.error("[PvP] get_deck_for_match p2 error:",G);function b(j,H){return{cardId:j.card_id,position:H||j.position,id:j.id,firstname:j.firstname,name:j.surname_encoded,country_code:j.country_code,club_id:j.club_id,job:j.job,job2:j.job2,note_g:Number(j.note_g)||0,note_d:Number(j.note_d)||0,note_m:Number(j.note_m)||0,note_a:Number(j.note_a)||0,rarity:j.rarity,skin:j.skin,hair:j.hair,hair_length:j.hair_length,clubName:j.club_encoded_name||null,clubLogo:j.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}}const C=((A==null?void 0:A.starters)||[]).map(j=>b(j,j.position)),E=((F==null?void 0:F.starters)||[]).map(j=>b(j,j.position)),w=((A==null?void 0:A.subs)||[]).map(j=>b(j,j.position)),S=((F==null?void 0:F.subs)||[]).map(j=>b(j,j.position)),T=(A==null?void 0:A.formation)||"4-4-2",N=(F==null?void 0:F.formation)||"4-4-2";return console.log("[PvP] p1Formation:",T,"p1 starters:",C.length,"positions:",C.map(j=>j.position)),console.log("[PvP] p2Formation:",N,"p2 starters:",E.length,"positions:",E.map(j=>j.position)),{p1Team:Oe(C,T),p2Team:Oe(E,N),p1Subs:w,p2Subs:S,p1Formation:T,p2Formation:N,p1Name:(D==null?void 0:D.club_name)||(D==null?void 0:D.pseudo)||"Joueur 1",p2Name:(_==null?void 0:_.club_name)||(_==null?void 0:_.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:s?p:[],gc_p2:s?[]:p,gcCardsFull_p1:s?g:[],gcCardsFull_p2:s?[]:g,gcReady_p1:!0,gcReady_p2:!0,usedGc_p1:[],usedGc_p2:[],lastActionAt:new Date().toISOString()}}let u=f.game_state&&Object.keys(f.game_state).length?f.game_state:null;if(!u)if(s){u=await y();const{data:A}=await $.from("matches").select("game_state").eq("id",i).single();!(A!=null&&A.game_state)||!Object.keys(A.game_state).length?await $.from("matches").update({game_state:u,turn_user_id:f.home_id}).eq("id",i):u=A.game_state}else{e.innerHTML=`<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation avec l'adversaire...</div>`;for(let A=0;A<30&&!u;A++){await new Promise(F=>setTimeout(F,400));const{data:z}=await $.from("matches").select("game_state").eq("id",i).single();z!=null&&z.game_state&&Object.keys(z.game_state).length&&(u=z.game_state)}if(!u){l("Erreur de synchronisation","error"),d("home");return}u.gc_p2=p,u.gcCardsFull_p2=g,await $.from("matches").update({game_state:u}).eq("id",i)}let h=!1;function L(A){var C;try{$.removeChannel(v)}catch{}const z=a.profile.id,F=A.winner_id===z,G=!!A.forfeit,D=u[n+"Score"]??0,_=u[r+"Score"]??0,b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center",b.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(b),(C=b.querySelector("#pvp-end-home"))==null||C.addEventListener("click",()=>{b.remove(),le(e),d("home")})}const v=$.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},A=>{const z=A.new;try{if(z.status==="finished"||z.forfeit){if(h)return;h=!0,u._timerInt&&(clearInterval(u._timerInt),u._timerInt=null),z.game_state&&(u=z.game_state),L(z);return}z.game_state&&(u=z.game_state,B())}catch(F){console.error("[PvP] Realtime render crash:",F,"gameState:",u)}}).subscribe();async function x(A){Object.assign(u,A),u.lastActionAt=new Date().toISOString();const{error:z}=await $.from("matches").update({game_state:u}).eq("id",i);z&&(console.error("[PvP] pushState DB error:",z.message,z),l("Erreur de synchronisation : "+z.message,"error"));try{B()}catch(F){console.error("[PvP] renderPvpScreen crash après pushState:",F),l("Erreur d'affichage : "+F.message,"error")}}async function k(){if(h)return;h=!0,u._timerInt&&(clearInterval(u._timerInt),u._timerInt=null);const A=s?f.away_id:f.home_id;try{await $.from("matches").update({status:"finished",forfeit:!0,winner_id:A}).eq("id",i)}catch(z){console.warn("[PvP] forfeit DB error:",z)}try{$.removeChannel(v)}catch{}le(e),d("home")}function B(){var ft,gt,xt,mt,yt;const A=u[n+"Team"];u[r+"Team"];const z=u[n+"Score"],F=u[r+"Score"],G=u[n+"Name"],D=u[r+"Name"];if(u.phase==="gc-select")return m();if(u.phase==="reveal")return I();if(u.phase==="midfield")return M();if(u.phase==="finished")return Z();const _=u.phase===n+"-attack",b=u.phase===n+"-defense",C=u.phase===r+"-attack"||u.phase===r+"-defense",E=Array.isArray(u["selected_"+n])?u["selected_"+n]:[],w=E.map(O=>O.cardId);e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const S=window.innerWidth>=700,T=u[n+"Subs"]||[],N=u["usedSubIds_"+n]||[],j=T.filter(O=>!N.includes(O.cardId)),H=u["gcCardsFull_"+n]||[],U=u["usedGc_"+n]||[],W=H.filter(O=>!U.includes(O.id)),ie=u.boostOwner===n&&!u.boostUsed,re=(O,J,Q)=>{var wt,_t;const Y=O._gcDef,ce={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[Y==null?void 0:Y.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",je={purple:"#b06ce0",light_blue:"#00d4ef"}[Y==null?void 0:Y.color]||"#b06ce0",ye=(Y==null?void 0:Y.name)||O.gc_type,ni=(Y==null?void 0:Y.effect)||((wt=de[O.gc_type])==null?void 0:wt.desc)||"",ht=Y!=null&&Y.image_url?`/manager-wars/icons/${Y.image_url}`:null,ri=((_t=de[O.gc_type])==null?void 0:_t.icon)||"⚡",vt=Math.round(Q*.22),bt=Math.round(Q*.22),Ke=Q-vt-bt,ai=ye.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${O.id}" data-gc-type="${O.gc_type}"
    ...
    </div>`,A)return;const _=n==="p1"?f.home_id:f.away_id;$.from("cards").select("id,gc_type").eq("owner_id",_).eq("card_type","game_changer").then(async({data:C})=>{const{data:E}=await $.from("gc_definitions").select("*").eq("is_active",!0),w=new Set,S=(C||[]).filter(N=>w.has(N.gc_type)?!1:(w.add(N.gc_type),!0)).map(N=>({...N,_gcDef:(E==null?void 0:E.find(j=>j.name===N.gc_type))||null})),T=document.getElementById("pvp-gc-grid");if(T){if(!S.length){T.innerHTML='<div style="color:rgba(255,255,255,0.4);font-size:13px;text-align:center;margin-top:30px">Aucune carte Game Changer disponible.</div>';return}T.innerHTML=S.map(N=>D(N,F.includes(N.id))).join(""),T.querySelectorAll(".pvp-gc-card").forEach(N=>{N.addEventListener("click",()=>{const j=N.dataset.id,H=F.indexOf(j);H>-1?F.splice(H,1):F.length<G&&F.push(j),B()})})}}),(b=e.querySelector("#pvp-gc-validate"))==null||b.addEventListener("click",async()=>{const C=n==="p1"?f.home_id:f.away_id,{data:E}=await $.from("cards").select("id,gc_type").eq("owner_id",C).eq("card_type","game_changer").in("id",F),{data:w}=await $.from("gc_definitions").select("*").eq("is_active",!0),S=(E||[]).map(T=>({...T,_gcDef:(w==null?void 0:w.find(N=>N.name===T.gc_type))||null}));await x({["gcReady_"+n]:!0,["gcCardsFull_"+n]:S}),u["gcReady_"+r]&&await x({phase:"reveal"})})}function I(){e.style.overflow="hidden",e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    </div>`,setTimeout(()=>{const E=document.getElementById("pvp-score-me"),w=document.getElementById("pvp-score-opp"),S=document.getElementById("pvp-midfield-result");E&&w&&(b?(E.style.fontSize="80px",E.style.color="#FFD700",w.style.opacity="0.25"):(w.style.fontSize="80px",w.style.color="#ff6b6b",E.style.opacity="0.25"));const T=u.p1Team.MIL||[],N=u.p2Team.MIL||[],j=F(T)+G(T),H=F(N)+G(N),U=j>=H?"p1":"p2";u[U+"Name"];let W=u.boostValue;W==null&&(W=Vt(),u.boostValue=W,u.boostOwner=U,u.boostUsed=!1),S&&(S.style.opacity="1",S.innerHTML=`<div style="font-size:20px;font-weight:900;margin-bottom:10px">
      ...
      </div>`,document.body.appendChild(F),(G=F.querySelector("#boost-picker-close"))==null||G.addEventListener("click",()=>F.remove()),F.querySelectorAll(".boost-pick-item").forEach(D=>{D.addEventListener("click",async()=>{const _=D.dataset.cid,b=z.find(w=>w.cardId===_);if(!b)return;const C=u[n+"Team"],E=(C[b._line]||[]).find(w=>w.cardId===_);E&&(E.boost=(E.boost||0)+u.boostValue),F.remove(),await x({[n+"Team"]:C,boostUsed:!0})})})}function q(){var F;const A=Array.isArray(u.log)?u.log:[],z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",z.innerHTML=`
      ...
      </div>`,document.body.appendChild(N),(j=N.querySelector("#pvp-gc-back"))==null||j.addEventListener("click",()=>N.remove()),(H=N.querySelector("#pvp-gc-use"))==null||H.addEventListener("click",async()=>{N.remove();const U=u["usedGc_"+n]||[];U.push(A),await x({["usedGc_"+n]:U})})}function Z(){var D;const A=u[n+"Score"],z=u[r+"Score"],F=A>z,G=A===z;e.style.overflow="hidden",e.innerHTML=`
