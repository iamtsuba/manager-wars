...
import{s as T,F as _i,i as Ht,k as Ki,l as ot,m as Li,f as Gt,n as bn,T as di,j as hn,o as vn,b as wn}from"./formation-links-BZe0pGr4.js";const _n="/";function Ii(e,{navigate:t,toast:i}){let o="login";const d=()=>{var r,l,p,s,n,x;const a=o==="login";e.innerHTML=`
  ...
  <div style="position:absolute;left:${_e}px;top:${re}px;width:${we}px;height:${xe}px;
    ...
    <span style="font-size:${x(80)};font-weight:900;color:${L};font-family:Arial Black,Arial;line-height:1;text-shadow:0 2px 8px rgba(0,0,0,0.9)">${A}</span>
  </div>
  ...
  <div style="position:absolute;left:${ce}px;top:${se}px;width:${Ee}px;height:${ve}px;z-index:3;
    ...
    ${st(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),s=(l||[]).filter(H=>H.card_type==="player"&&H.player),n=(l||[]).filter(H=>H.card_type==="game_changer"),x=(l||[]).filter(H=>H.card_type==="formation"),u=(l||[]).filter(H=>H.card_type==="stadium"),{data:h}=await T.from("gc_definitions").select("name,gc_type,color,effect,image_url"),_={};(h||[]).forEach(H=>{_[H.name]=H});const{data:m}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),c={};(m||[]).forEach(H=>{c[H.id]=H}),u.forEach(H=>{H.stadium_def&&(c[H.stadium_id]=H.stadium_def)});const A=Object.keys(_i),g=Object.keys(Oe),L={};s.forEach(H=>{const Z=H.player.id;L[Z]=(L[Z]||0)+1}),new Set(Object.keys(L).map(H=>String(H)));const C=new Set(x.map(H=>H.formation)),q=new Set(n.map(H=>H.gc_type));let D="player",j="Tous",N="",V=!1;function K(){return[...s].sort((H,Z)=>{const ge=Yt.indexOf(H.player.job),de=Yt.indexOf(Z.player.job);return ge!==de?ge-de:(H.player.surname_encoded||"").localeCompare(Z.player.surname_encoded||"")})}function oe(){return[...p||[]].sort((H,Z)=>{const ge=Yt.indexOf(H.job),de=Yt.indexOf(Z.job);return ge!==de?ge-de:(H.surname_encoded||"").localeCompare(Z.surname_encoded||"")})}function ie(){return K().filter(H=>{const Z=H.player,ge=j==="Tous"||Z.job===j,de=!N||`${Z.firstname} ${Z.surname_encoded}`.toLowerCase().includes(N);return ge&&de})}function Q(){return oe().filter(H=>{const Z=j==="Tous"||H.job===j,ge=!N||`${H.firstname} ${H.surname_encoded}`.toLowerCase().includes(N);return Z&&ge})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function R(){const H=document.getElementById("col-filters");H&&(D==="player"?(H.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",Z=>{N=Z.target.value.toLowerCase(),P()}),e.querySelectorAll(".filter-btn").forEach(Z=>{Z.addEventListener("click",()=>{j=Z.dataset.job,R(),P()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{V=!V,R(),P()})):(H.innerHTML=`
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Wt=1e3;function Zn(e,t,i,o){var _,m,c;const{state:d,toast:a,closeModal:r,navigate:l,refreshProfile:p}=i,s=e.formation,n={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function x(){const A=Ht[s]||{},g=_i[s]||[],L=290,C=360,q=20;function D(N){const V=A[N];return V?{x:V.x*L,y:V.y*C}:null}let j=`<svg width="${L}" height="${C}" viewBox="0 0 ${L} ${C}" xmlns="http://www.w3.org/2000/svg">`;for(const[N,V]of g){const K=D(N),oe=D(V);!K||!oe||(j+=`<line x1="${K.x}" y1="${K.y}" x2="${oe.x}" y2="${oe.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const N of Object.keys(A)){const V=D(N);if(!V)continue;const K=N.replace(/\d+/,""),oe=n[K]||"#555";j+=`<circle cx="${V.x}" cy="${V.y}" r="${q}" fill="${oe}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,j+=`<text x="${V.x}" y="${V.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${K}</text>`}return j+="</svg>",j}const u=t.filter(A=>A.formation===s);u.length;const h=!e.is_for_sale;o(`Formation ${s}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",n.note_g],["DEF",n.note_d],["MIL",n.note_m],["ATT",n.note_a]].map(([se,pe])=>{const ce=li[se],H=se==="GK"?"#fff":ce,Z=se===n.job||se===n.job2,ge=(Number(pe)||0)+(Z&&D>0?D:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(_e=document.getElementById("close-detail"))==null||_e.addEventListener("click",l);let P=new Set;const re=()=>{const se=P.size,pe=document.getElementById("sell-action-panel");if(!pe)return;pe.style.display=se>0?"block":"none",document.getElementById("sell-selected-count").textContent=se;const ce=document.getElementById("evolve-btn");ce&&(ce.textContent=`⬆️ Évoluer ${se>1?"(+"+se+")":""}`)};document.querySelectorAll(".expl-check").forEach(se=>{se.addEventListener("change",()=>{const pe=se.dataset.id;se.checked?P.add(pe):P.delete(pe);const ce=se.closest(".exemplaire-row");ce&&(ce.style.borderColor=se.checked?"#1A6B3C":"#eee"),re()})}),document.querySelectorAll(".exemplaire-row").forEach(se=>{se.addEventListener("click",pe=>{if(pe.target.closest("button")||pe.target.tagName==="INPUT")return;const ce=se.querySelector(".expl-check");ce&&!ce.disabled&&(ce.checked=!ce.checked,ce.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(se=>{se.addEventListener("click",pe=>{pe.stopPropagation();const ce=document.querySelector(`.expl-hist[data-hist="${se.dataset.idx}"]`);ce&&(ce.style.display=ce.style.display==="none"?"flex":"none")})}),(we=document.getElementById("evolve-btn"))==null||we.addEventListener("click",async()=>{if(u<=1)return;const se=[...P];if(!se.length)return;if(P.has(e.id)){const me=document.createElement("div");me.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",me.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    <div class="page-body" style="padding-bottom:20px">
    ...
    </div>`;c("⚽ Choisir une formation",g),document.querySelectorAll("#modal-body [data-forma]").forEach(L=>{L.addEventListener("click",()=>{t.formation=L.dataset.forma,A(),ct(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(c=>c.addEventListener("click",h)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(c=>c.addEventListener("click",()=>to(t,e,i))),e.querySelectorAll("#save-deck").forEach(c=>c.addEventListener("click",()=>oo(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(c=>{c.addEventListener("click",()=>{t.subs=t.subs.filter(A=>A!==c.dataset.removeSub),ct(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(c=>c.addEventListener("click",()=>no(t,e,i)))}function eo(e,t,i,o){var L,C;const d=window.innerWidth>=900,a=e.querySelector(d?"#deck-field-pc":"#deck-field-mobile");if(!a)return;const r=(L=t.stadiumCards)==null?void 0:L.find(q=>q.id===t.stadiumCardId),l=r&&((C=t.stadDefMap)==null?void 0:C[r.stadium_id])||null,p=Ht[t.formation]||{},s=Ki(t.formation)||[],n={};for(const q of i){const D=t.slots[q],j=D?t.playerCards.find(N=>N.id===D):null;j!=null&&j.player?n[q]={...j.player,_evolution_bonus:j.evolution_bonus||0,face:j.player.face||null}:n[q]=null}const x=window.innerWidth>=900,u=x?window.innerWidth-280:window.innerWidth-20,h=x?Math.min(u,860):u,_=Math.round(x?h*.82:h*1.05),m=x?84:44;let c="";for(const[q,D]of s){const j=p[q],N=p[D];if(!j||!N)continue;const V=j.x*h,K=Math.round(.03*_+j.y*.85*_),oe=N.x*h,ie=Math.round(.03*_+N.y*.85*_),Q=n[q],R=n[D],P=ot(Q,R);P==="#ff3333"||P==="#cc2222"?c+=`<line x1="${V.toFixed(1)}" y1="${K.toFixed(1)}" x2="${oe.toFixed(1)}" y2="${ie.toFixed(1)}" stroke="${P}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(c+=`<line x1="${V.toFixed(1)}" y1="${K.toFixed(1)}" x2="${oe.toFixed(1)}" y2="${ie.toFixed(1)}" stroke="${P}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,c+=`<line x1="${V.toFixed(1)}" y1="${K.toFixed(1)}" x2="${oe.toFixed(1)}" y2="${ie.toFixed(1)}" stroke="${P}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let A="";const g=Math.round(m*657/507);for(const q of i){const D=p[q];if(!D)continue;const j=n[q],N=D.x*h;D.y*_;const V=Math.round(.03*_+D.y*(.85*_)),K=Math.round(N-m/2),oe=Math.round(V-g/2);if(j){const ie=q.replace(/\d+/,""),Q=l&&(l.club_id&&String(j.club_id)===String(l.club_id)||l.country_code&&j.country_code===l.country_code),R=st({...j,_evolution_bonus:j._evolution_bonus||0},{width:m,showStad:!0,stadDef:l,role:ie});A+=`<div style="position:absolute;left:${K}px;top:${oe}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${q}">
      ...
      </div>`}else{const ie=q.replace(/\d+/,"");A+=`<div style="position:absolute;left:${K}px;top:${oe}px;width:${m}px;height:${g}px;
    ...
    <div style="position:relative;width:${h}px;height:${_}px;margin:0 auto">
      ...
      ${A}
    </div>`,a.querySelectorAll(".deck-slot-hit").forEach(q=>{q.addEventListener("click",()=>io(q.dataset.pos,t,e,o))})}function to(e,t,i){var r;const{openModal:o,closeModal:d}=i,a=e.stadiumCards||[];o("🏟️ Choisir un stade",`<div style="display:flex;flex-wrap:wrap;gap:10px;padding:8px">
    ...
    </div>`),(r=document.getElementById("stad-none"))==null||r.addEventListener("click",()=>{e.stadiumCardId=null,d(),ct(t,e,i)}),document.querySelectorAll(".stad-choice").forEach(l=>{l.addEventListener("click",()=>{e.stadiumCardId=l.dataset.stadId,d(),ct(t,e,i)})})}function io(e,t,i,o){var _,m,c,A,g;const{openModal:d,closeModal:a}=o,r=e.replace(/\d+/,""),l=(_=t.stadiumCards)==null?void 0:_.find(L=>L.id===t.stadiumCardId),p=l&&((m=t.stadDefMap)==null?void 0:m[l.stadium_id])||null,s=t.slots[e],n=s?t.playerCards.find(L=>L.id===s):null;(c=n==null?void 0:n.player)==null||c.id;const x=new Set;Object.entries(t.slots).forEach(([L,C])=>{var D;if(L===e||!C)return;const q=t.playerCards.find(j=>j.id===C);(D=q==null?void 0:q.player)!=null&&D.id&&x.add(q.player.id)}),t.subs.forEach(L=>{var q;const C=t.playerCards.find(D=>D.id===L);(q=C==null?void 0:C.player)!=null&&q.id&&x.add(C.player.id)});const u=new Set,h=t.playerCards.filter(L=>{const C=L.player;return!(C.job===r||C.job2===r)||x.has(C.id)||u.has(C.id)?!1:(u.add(C.id),!0)});h.sort((L,C)=>{const q=L.evolution_bonus||0,D=C.evolution_bonus||0,j=(r==="GK"?L.player.note_g:r==="DEF"?L.player.note_d:r==="MIL"?L.player.note_m:L.player.note_a)+(r===L.player.job||r===L.player.job2?q:0);return(r==="GK"?C.player.note_g:r==="DEF"?C.player.note_d:r==="MIL"?C.player.note_m:C.player.note_a)+(r===C.player.job||r===C.player.job2?D:0)-j}),d(`Choisir ${r} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${h.length>0?'<div style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center">'+h.map(L=>{const C={...L.player,_evolution_bonus:L.evolution_bonus||0};return`<div class="player-option" data-card-id="${L.id}" style="cursor:pointer">
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",r).order("slot_order"),p=[...new Set((l||[]).filter(u=>{var h,_;return((h=u.card)==null?void 0:h.card_type)==="stadium"&&((_=u.card)==null?void 0:_.stadium_id)}).map(u=>u.card.stadium_id))],s={};if(p.length){const{data:u}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",p);(u||[]).forEach(h=>{s[h.id]=h}),(l||[]).forEach(h=>{var _,m;((_=h.card)==null?void 0:_.card_type)==="stadium"&&((m=h.card)!=null&&m.stadium_id)&&(h.card._stadiumDef=s[h.card.stadium_id]||null)})}let n=0;function x(){var q,D,j,N,V,K,oe;const u=a[n],h=(l||[]).filter(ie=>ie.deck_id===u.id),_=h.filter(ie=>{var Q;return ie.is_starter&&((Q=ie.card)==null?void 0:Q.player)}).map(ie=>bi(ie.card,ie.position)),m=h.find(ie=>{var Q;return((Q=ie.card)==null?void 0:Q.card_type)==="formation"}),c=u.formation||((q=m==null?void 0:m.card)==null?void 0:q.formation)||"4-4-2";let A=_.length>=11?vt(_,c):null,g=((D=u.stadium_card)==null?void 0:D.stadium_def)||null;g&&A&&(A=Tt(A,g));const L=_.length>=11;et(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;align-items:center;gap:8px;padding:6px 14px;background:linear-gradient(90deg,rgba(232,119,34,0.3),rgba(196,90,0,0.2));border-top:1px solid rgba(232,119,34,0.4);flex-shrink:0">
        ...
        <div style="font-size:11px;font-weight:900;color:#FFD700">+10 aux joueurs ${((j=g.club)==null?void 0:j.encoded_name)||g.country_code||""}</div>
      </div>`:""}
    ...
    </div>`,function(){const Q=e.querySelector(".deck-preview-wrap svg");Q&&(Q.removeAttribute("width"),Q.removeAttribute("height"),Q.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",Q.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(N=document.getElementById("prev-deck"))==null||N.addEventListener("click",()=>{n>0&&(n--,x())}),(V=document.getElementById("next-deck"))==null||V.addEventListener("click",()=>{n<a.length-1&&(n++,x())}),(K=document.getElementById("validate-deck"))==null||K.addEventListener("click",()=>{if(!L)return;const ie=t.state.params||{};t.navigate("match",{...ie,matchMode:ie.matchMode||i,deckId:u.id})}),(oe=document.getElementById("cancel-deck-select"))==null||oe.addEventListener("click",()=>{Ve(e),d("home")});const C=document.getElementById("deck-swipe-zone");if(C){let ie=0,Q=0;C.addEventListener("touchstart",R=>{ie=R.touches[0].clientX,Q=R.touches[0].clientY},{passive:!0}),C.addEventListener("touchend",R=>{const P=R.changedTouches[0].clientX-ie,re=R.changedTouches[0].clientY-Q;Math.abs(P)<40||Math.abs(P)<Math.abs(re)||(P<0&&n<a.length-1?(n++,x()):P>0&&n>0&&(n--,x()))},{passive:!0})}}x()}function _o(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function Xe(e,t=44,i=58,o=null){return st(e,{width:t,showStad:!!o,stadDef:o,used:e==null?void 0:e.used})}function it(e,t,i){if(!(e!=null&&e.length))return"";const o=e.slice(0,5);let d='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return o.forEach((a,r)=>{if(d+=Xe(a,40,52),r<o.length-1){const l=ot(a,o[r+1]);d+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(d+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),d+="</div>",d}function $o(e){return!e||e.length<2?null:`https://flagsapi.com/${e.slice(0,2).toUpperCase()}/flat/64.png`}function ko(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function pt(e,t,i,o,d=310,a=310,r=[]){var L;const l=Ht[t]||{},p=Ki(t)||_i[t]||[],s={},n=["ATT","MIL","DEF","GK"];for(const C of n)(e[C]||[]).forEach((D,j)=>{s[`${C}${j+1}`]=D});function x(C){const q=l[C];return q?{x:q.x*d,y:q.y*a}:null}let u="";for(const[C,q]of p){const D=x(C),j=x(q);if(!D||!j)continue;const N=s[C],V=s[q],K=ot(N,V);K==="#00ff88"||K==="#FFD700"?(u+=`<line x1="${D.x.toFixed(1)}" y1="${D.y.toFixed(1)}" x2="${j.x.toFixed(1)}" y2="${j.y.toFixed(1)}"
        stroke="${K}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,u+=`<line x1="${D.x.toFixed(1)}" y1="${D.y.toFixed(1)}" x2="${j.x.toFixed(1)}" y2="${j.y.toFixed(1)}"
        stroke="${K}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):u+=`<line x1="${D.x.toFixed(1)}" y1="${D.y.toFixed(1)}" x2="${j.x.toFixed(1)}" y2="${j.y.toFixed(1)}"
        stroke="${K}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const h=48,_=64,m=13,c=16,A={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[C,q]of Object.entries(s)){const D=x(C);if(!D||!q)continue;const j=C.replace(/[0-9]/g,""),N=Ot[j]||"#555",V=r.includes(q.cardId),K=i==="attack"&&(["MIL","ATT"].includes(j)||V)&&!q.used||i==="defense"&&["GK","DEF","MIL"].includes(j)&&!q.used,oe=o.includes(q.cardId);let ie;i==="attack"?ie=V?Math.max(1,Number(q.note_a)||0):j==="MIL"?Number(q.note_m)||0:Number(q.note_a)||0:i==="defense"?ie=j==="GK"?Number(q.note_g)||0:j==="MIL"?Number(q.note_m)||0:Number(q.note_d)||0:ie=Number(j==="GK"?q.note_g:j==="DEF"?q.note_d:j==="MIL"?q.note_m:q.note_a)||0,ie=ie+(q.boost||0),q.stadiumBonus&&(i==="attack"&&(j==="ATT"||j==="MIL")||i==="defense"&&(j==="GK"||j==="DEF"||j==="MIL")?ie+=10:i||(ie+=10));const Q=(D.x-h/2).toFixed(1),R=(D.y-_/2).toFixed(1),P=A[q==null?void 0:q.rarity]||A.normal;if(q.used){const se=typeof import.meta<"u"&&"/"||"/",ce=`${typeof window<"u"&&((L=window.location)==null?void 0:L.origin)||""}${se}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");u+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="#161616"/>`,u+=`<image href="${ce}" xlink:href="${ce}" x="${Q}" y="${R}" width="${h}" height="${_}" preserveAspectRatio="xMidYMid slice"/>`,u+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="none" stroke="${P}" stroke-width="2" opacity="0.7"/>`,u+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${q.cardId}" data-role="${j}" style="cursor:pointer"/>`;continue}const re=oe?.45:1,xe=oe?"#FFD700":P,_e=oe?3:(q==null?void 0:q.rarity)==="legende"||(q==null?void 0:q.rarity)==="légende"||(q==null?void 0:q.rarity)==="pepite"||(q==null?void 0:q.rarity)==="pépite"?2.5:2,we=_-m-c;u+=`<defs><clipPath id="cp-${C}"><rect x="${Q}" y="${(D.y-_/2+m).toFixed(1)}" width="${h}" height="${we}"/></clipPath></defs>`,u+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="${N}" opacity="${re}"/>`;const ve=He(q);ve&&(u+=`<image href="${ve}" xlink:href="${ve}" x="${Q}" y="${(D.y-_/2+m).toFixed(1)}" width="${h}" height="${we}" clip-path="url(#cp-${C})" preserveAspectRatio="xMidYMin slice"/>`),u+=`<rect x="${Q}" y="${R}" width="${h}" height="${m}" rx="4" fill="rgba(255,255,255,0.92)"/>`,u+=`<text x="${D.x.toFixed(1)}" y="${(D.y-_/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(q.name||"").slice(0,9)}</text>`;const Ee=(D.y+_/2-c).toFixed(1);u+=`<rect x="${Q}" y="${Ee}" width="${h}" height="${c}" fill="rgba(255,255,255,0.92)"/>`;{const se=$o(q.country_code);se?u+=`<image href="${se}" xlink:href="${se}" x="${(D.x-20).toFixed(1)}" y="${(D.y+_/2-c+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:u+=`<text x="${(D.x-13).toFixed(1)}" y="${(D.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${ko(q.country_code)}</text>`;const pe=q.stadiumBonus?"#E87722":"#111";u+=`<text x="${D.x.toFixed(1)}" y="${(D.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="${pe}" font-family="Arial Black">${ie}</text>`;const ce=_o(q);ce?u+=`<image href="${ce}" xlink:href="${ce}" x="${(D.x+h/2-14).toFixed(1)}" y="${(D.y+_/2-c+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:q.clubName&&(u+=`<text x="${(D.x+14).toFixed(1)}" y="${(D.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(q.clubName||"").slice(0,3).toUpperCase()}</text>`),q.stadiumBonus&&(u+=`<rect x="${(D.x-h/2).toFixed(1)}" y="${(D.y-_/2).toFixed(1)}" width="${h}" height="${_}" rx="5" fill="none" stroke="#E87722" stroke-width="2" opacity="0.8"/>`)}u+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="${oe?"rgba(255,255,255,0.12)":"none"}" stroke="${xe}" stroke-width="${_e}" opacity="${re}"/>`,K&&(u+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${oe?"selected":""}" data-card-id="${q.cardId}" data-role="${j}" style="cursor:pointer"/>`)}const g=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-g} ${-g} ${d+g*2} ${a+g*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${u}
  ...
  </div>`}async function ai(e,t,i,o){var D;const{state:d,navigate:a,toast:r}=t;et(e);const l=d.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!l.deckId)return wo(e,t,i);const p=l.deckId;let s,n,x,u;try{const j=await Promise.all([T.from("decks").select("formation,name,stadium_card_id").eq("id",p).single(),T.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",p).order("slot_order")]);s=j[0].data,x=j[0].error,n=j[1].data,u=j[1].error}catch(j){console.error("[Match] Exception chargement deck:",j),Et(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>a("home"));return}if(x||u){console.error("[Match] Erreur Supabase:",x||u),Et(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>a("home"));return}const h=(n||[]).filter(j=>{var N;return j.is_starter&&((N=j.card)==null?void 0:N.player)}).map(j=>bi(j.card,j.position)),_=(n||[]).filter(j=>{var N;return!j.is_starter&&((N=j.card)==null?void 0:N.player)}).map(j=>bi(j.card,j.position));if(h.length<11){Et(e,"⚠️",`Deck incomplet (${h.length}/11).`,"Compléter",()=>a("decks"));return}const m=(n||[]).find(j=>{var N;return((N=j.card)==null?void 0:N.card_type)==="formation"}),c=(s==null?void 0:s.formation)||((D=m==null?void 0:m.card)==null?void 0:D.formation)||"4-4-2",{data:A,error:g}=await T.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",d.profile.id).eq("card_type","game_changer"),{data:L}=await T.from("gc_definitions").select("*").eq("is_active",!0),C=(A||[]).map(j=>({...j,_gcDef:(L==null?void 0:L.find(N=>N.name===j.gc_type||N.id===j.gc_definition_id))||null}));let q=null;if(s!=null&&s.stadium_card_id){const{data:j}=await T.from("cards").select("stadium_id").eq("id",s.stadium_card_id).single();if(j!=null&&j.stadium_id){const{data:N}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",j.stadium_id).single();q=N||null}}o({deckId:p,formation:c,starters:h,subsRaw:_,gcCardsEnriched:C,gcDefs:L||[],stadiumDef:q})}async function Eo(e,t){const{state:i}=t,d=(i.params||{}).matchMode||"vs_ai_easy",a=d.replace("vs_ai_",""),r=d;await ai(e,t,d,async({deckId:l,formation:p,starters:s,subsRaw:n,gcCardsEnriched:x,gcDefs:u,stadiumDef:h})=>{try{let _=vt(s,p);h&&(_=Tt(_,h),jt(n,h));const m=await To(p,a),c=m.lines||m,A=async g=>{try{const{data:L,error:C}=await T.from("matches").insert({home_id:i.profile.id,away_id:null,mode:r,home_deck_id:l,status:"in_progress"}).select().single();if(C){console.error("[MatchIA] Erreur création match:",C),Et(e,"⚠️","Impossible de créer le match ("+C.message+").","Retour",()=>t.navigate("home"));return}const q=m.stadiumDef||null;q&&c&&(Tt(c,q),jt(m.subs||[],q));const D={gcDefs:u||[],matchId:L==null?void 0:L.id,mode:r,difficulty:a,formation:p,homeTeam:_,aiTeam:c,homeSubs:n,subsUsed:0,maxSubs:Math.min(n.length,3),aiSubs:m.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((m.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:m.gcCards||[],aiUsedGc:[],aiStadiumDef:q,homeScore:0,aiScore:0,gcCards:g,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};zo(e,D,t)}catch(L){console.error("[MatchIA] Exception launchMatch:",L),Et(e,"⚠️","Erreur au lancement du match : "+L.message,"Retour",()=>t.navigate("home"))}};if(!x.length){A([]);return}ri(e,x,A)}catch(_){console.error("[MatchIA] Exception setup:",_),Et(e,"⚠️","Erreur de préparation du match : "+_.message,"Retour",()=>t.navigate("home"))}})}async function To(e,t){var c;const{data:i}=await T.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:So(e),subs:[],gcCards:[],stadiumDef:null};const o=Mt[e]||Mt["4-4-2"],d={GK:[],DEF:[],MIL:[],ATT:[]},a=new Set;function r(A,g,L){var C,q;return a.add(A.id),{cardId:"ai-"+A.id+"-"+L,id:A.id,firstname:A.firstname,name:A.surname_encoded,country_code:A.country_code,club_id:A.club_id,job:A.job,job2:A.job2,note_g:Number(A.note_g)||0,note_d:Number(A.note_d)||0,note_m:Number(A.note_m)||0,note_a:Number(A.note_a)||0,rarity:A.rarity,skin:A.skin,hair:A.hair,hair_length:A.hair_length,clubName:((C=A.clubs)==null?void 0:C.encoded_name)||null,clubLogo:((q=A.clubs)==null?void 0:q.logo_url)||null,boost:0,used:!1,_line:g}}for(const A of["GK","DEF","MIL","ATT"]){const g=i.filter(j=>j.job===A&&!a.has(j.id)),L=i.filter(j=>j.job!==A&&!a.has(j.id)),C=[...g,...L],q=[];for(let j=0;j<o[A];j++){const N=C[j];N&&q.push(r(N,A,j))}const D=dt(q.length);q.forEach((j,N)=>{j._col=D[N]}),d[A]=q}const p=i.filter(A=>!a.has(A.id)).slice(0,5).map((A,g)=>r(A,A.job,100+g)),x=Object.keys(Oe).sort(()=>Math.random()-.5).slice(0,3).map((A,g)=>{var L,C;return{id:"ai-gc-"+g,gc_type:A,name:((L=Oe[A])==null?void 0:L.name)||A,icon:((C=Oe[A])==null?void 0:C.icon)||"⚡"}}),u=Object.values(d).flat(),h={};u.forEach(A=>{A.club_id&&(h[A.club_id]=(h[A.club_id]||0)+1)});const _=(c=Object.entries(h).sort((A,g)=>g[1]-A[1])[0])==null?void 0:c[0];let m=null;if(_){const{data:A}=await T.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",_).single();A&&(m={club_id:A.id,country_code:null,name:A.encoded_name+" Stadium",club:{encoded_name:A.encoded_name,logo_url:A.logo_url}})}return{lines:d,subs:p,gcCards:x,stadiumDef:m}}function So(e){const t=Mt[e]||Mt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},o=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let d=0;for(const a of["GK","DEF","MIL","ATT"]){const r=[];for(let p=0;p<t[a];p++){const s=3+Math.floor(Math.random()*5);r.push({cardId:"fake-"+d,id:"fake-"+d,firstname:"IA",name:o[d%o.length],country_code:"XX",club_id:null,job:a,job2:null,note_g:a==="GK"?s:2,note_d:a==="DEF"?s:2,note_m:a==="MIL"?s:2,note_a:a==="ATT"?s:2,rarity:"normal",boost:0,used:!1,_line:a}),d++}const l=dt(r.length);r.forEach((p,s)=>{p._col=l[s]}),i[a]=r}return i}function zo(e,t,i){var o;e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>Ao(e,t,i),5e3)}function Ao(e,t,i){const o=t.homeTeam.MIL||[],d=t.aiTeam.MIL||[],a=t.stadiumDef||null,r=t.aiStadiumDef||null;function l(g,L){const C=ze(g,"MIL"),q=g.evolution_bonus||g._evolution_bonus||0,D=g.job==="MIL"||g.job2==="MIL"?q:0,j=g.stadiumBonus||L&&(L.club_id&&String(g.club_id)===String(L.club_id)||L.country_code&&g.country_code===L.country_code)?10:0;return C+D+j}function p(g,L){return g.reduce((C,q)=>C+l(q,L),0)}function s(g){let L=0;for(let C=0;C<g.length-1;C++){const q=ot(g[C],g[C+1]);q==="#00ff88"?L+=2:q==="#FFD700"&&(L+=1)}return L}const n=p(o,a)+s(o),x=p(d,r)+s(d),u=n>=x;function h(g,L,C,q,D){return`<div id="duel-row-${q}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${g.map((j,N)=>{const V=N<g.length-1?ot(j,g[N+1]):null,K=!V||V==="#ff3333"||V==="#cc2222",oe=V==="#00ff88"?"+2":V==="#FFD700"?"+1":"";l(j,D),j.stadiumBonus||D&&(D.club_id&&String(j.club_id)===String(D.club_id)||D.country_code&&(j.country_code,D.country_code));const ie=j.evolution_bonus||j._evolution_bonus||0;return`
          <div class="duel-card duel-card-${q}" data-idx="${N}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease, transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
            ${Xe({...j,_evolution_bonus:ie},58,78,D)}
          </div>
          ${N<g.length-1?`<div class="duel-link duel-link-${q}" data-idx="${N}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${K?"rgba(255,255,255,0.12)":V};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${K?"none":`0 0 8px ${V}`}">
  ...
  </div>`;const _=()=>{const g=(L,C)=>e.querySelectorAll(L).forEach((q,D)=>{setTimeout(()=>{q.style.opacity="1",q.style.transform="translateY(0) scale(1)"},C+D*90)});g(".duel-card-home",150),g(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((L,C)=>{setTimeout(()=>{L.style.opacity="1"},C*70)}),900),setTimeout(()=>{const L=e.querySelector("#vs-label");L&&(L.style.opacity="1",L.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(C=>C.style.opacity="1")},1250),setTimeout(()=>{m("score-home",n,800),m("score-ai",x,800)},1500)};function m(g,L,C){const q=document.getElementById(g);if(!q)return;const D=performance.now(),j=N=>{const V=Math.min(1,(N-D)/C);q.textContent=Math.round(L*(1-Math.pow(1-V,3))),V<1?requestAnimationFrame(j):q.textContent=L};requestAnimationFrame(j)}requestAnimationFrame(_),t.attacker=u?"home":"ai";const c=u?oi():null;u&&(t.boostCard={value:c}),t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:o.map(g=>({name:g.name,note:ze(g,"MIL"),portrait:He(g),job:g.job,country_code:g.country_code,rarity:g.rarity,clubName:g.clubName,clubLogo:g.clubLogo})),aiPlayers:d.map(g=>({name:g.name,note:ze(g,"MIL"),portrait:He(g),job:g.job,country_code:g.country_code,rarity:g.rarity,clubName:g.clubName,clubLogo:g.clubLogo})),homeTotal:n,aiTotal:x,text:`Duel milieu : ${t.clubName} ${n} – ${x} IA → ${u?t.clubName+" attaque":"IA attaque"}`});const A=()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",Je(e,t,i),t.attacker==="ai"&&setTimeout(()=>ei(e,t,i),800)};setTimeout(()=>{const g=document.getElementById("score-home"),L=document.getElementById("score-ai"),C=document.getElementById(u?"duel-row-home":"duel-row-ai"),q=document.getElementById(u?"duel-row-ai":"duel-row-home"),D=u?g:L,j=u?L:g;D&&(D.style.fontSize="80px",D.style.color=u?"#FFD700":"#ff6b6b",D.style.animation="duelPulse .5s ease"+(u?", duelGlow 1.5s ease infinite .5s":"")),j&&(j.style.opacity="0.25"),setTimeout(()=>{C&&(C.style.transformOrigin="center",C.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",C.style.zIndex="5"),setTimeout(()=>{var V;const N=document.getElementById("duel-shock");if(N){const K=(V=q||C)==null?void 0:V.getBoundingClientRect(),oe=e.querySelector(".match-screen").getBoundingClientRect();K&&(N.style.top=K.top-oe.top+K.height/2+"px"),N.style.animation="shockwave .5s ease-out forwards"}q&&(q.style.transformOrigin="center",q.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var V;const N=document.getElementById("duel-finale");N&&(N.innerHTML=`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(a);let l=!1;const p=()=>{l||(l=!0,a.remove(),setTimeout(()=>d(),50))};a.addEventListener("click",p),setTimeout(p,3500)}async function Dt(e,t,i){var n,x;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:o}=i,d=t.homeScore>t.aiScore,a=t.homeScore===t.aiScore,r=d?"victoire":a?"nul":"defaite",l=Rn(t.mode,r);t.matchId&&await T.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:d?o.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const p={credits:(o.profile.credits||0)+l,matches_played:(o.profile.matches_played||0)+1};d?p.wins=(o.profile.wins||0)+1:a?p.draws=(o.profile.draws||0)+1:p.losses=(o.profile.losses||0)+1,await T.from("users").update(p).eq("id",o.profile.id),await i.refreshProfile();const s=document.createElement("div");s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",s.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const C=async(N=!0)=>{_=!0,m&&(T.removeChannel(m),m=null),N&&await T.rpc("cancel_matchmaking",{p_user_id:x.profile.id}).catch(()=>{})};(j=document.getElementById("mm-cancel"))==null||j.addEventListener("click",async()=>{try{await C(!0)}catch{}Ve(e),u("home")});const q=async(N,V,K)=>{if(_)return;_=!0,m&&(T.removeChannel(m),m=null);const oe=document.getElementById("mm-status");oe&&(oe.textContent="Adversaire trouvé !"),await new Promise(ie=>setTimeout(ie,600)),e.isConnected&&un(e,t,N,K,r,l,s,n,p)};if(s){const N=[{range:50,delay:0},{range:100,delay:15e3},{range:200,delay:3e4},{range:400,delay:45e3},{range:800,delay:6e4}];let V=0,K=!1;const oe=async()=>{if(_||K)return;const{range:ie}=N[V],Q=document.getElementById("mm-range");Q&&(Q.textContent=`Plage MMR : ±${ie}`);const{data:R,error:P}=await T.rpc("try_matchmake_ranked",{p_user_id:x.profile.id,p_deck_id:i,p_mmr:c,p_range:ie});if(console.log("[Ranked] try_matchmake_ranked result:",R,"error:",JSON.stringify(P)),_)return;if(P||!(R!=null&&R.success)){h("Erreur matchmaking ranked","error"),Ve(e),u("home");return}if(R.matched){K=!0,q(R.match_id,R.opponent_id,!1);return}const re=document.getElementById("mm-status");if(re&&(re.textContent="En attente d'un adversaire..."),m||(m=T.channel("matchmaking-"+x.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${x.profile.id}`},xe=>{const _e=xe.new;_e.status==="matched"&&_e.match_id&&!K&&(K=!0,q(_e.match_id,_e.matched_with,!0))}).subscribe()),V<N.length-1){const xe=N[V+1].delay-N[V].delay;V++,setTimeout(()=>{!_&&!K&&oe()},xe)}};await oe()}else{const{data:N,error:V}=await T.rpc("try_matchmake",{p_user_id:x.profile.id,p_deck_id:i});if(V||!(N!=null&&N.success)){h("Erreur de matchmaking","error"),Ve(e),u("home");return}if(N.matched){q(N.match_id,N.opponent_id,!1);return}const K=document.getElementById("mm-status");K&&(K.textContent="En attente d'un autre joueur..."),m=T.channel("matchmaking-"+x.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${x.profile.id}`},oe=>{const ie=oe.new;ie.status==="matched"&&ie.match_id&&q(ie.match_id,ie.matched_with,!0)}).subscribe()}}async function Ho(e,t,i){const{state:o,navigate:d,toast:a}=t;try{(T.getChannels?T.getChannels():[]).forEach(s=>{const n=s.topic||"";(n.includes("matchmaking")||n.includes("pvp-match")||n.includes("friend-invite"))&&T.removeChannel(s)})}catch{}const{data:r}=await T.from("matches").select("home_id, away_id, status").eq("id",i).single();if(!r){a("Match introuvable","error"),d("home");return}if(r.status==="finished"){a("Ce match est terminé","info"),d("home");return}const l=r.home_id===o.profile.id;un(e,t,i,l,[],[])}async function un(e,t,i,o,d=[],a=[],r=!1,l=null,p=null){const{state:s,navigate:n,toast:x}=t,u=o?"p1":"p2",h=o?"p2":"p1",_=(d||[]).map($=>$.id),m=(d||[]).map($=>({id:$.id,gc_type:$.gc_type,_gcDef:$._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:c}=await T.from("matches").select("*").eq("id",i).single();if(!c){x("Match introuvable","error"),n("home");return}async function A(){var W,te;console.log("[PvP] buildGameState match:",{id:c.id,home_deck_id:c.home_deck_id,away_deck_id:c.away_deck_id,mode:c.mode,is_ranked:c.is_ranked});const[{data:$,error:f},{data:y,error:b},{data:v},{data:w}]=await Promise.all([T.rpc("get_deck_for_match",{p_deck_id:c.home_deck_id}),T.rpc("get_deck_for_match",{p_deck_id:c.away_deck_id}),T.from("users").select("id,pseudo,club_name").eq("id",c.home_id).single(),T.from("users").select("id,pseudo,club_name").eq("id",c.away_id).single()]);console.log("[PvP] get_deck_for_match p1:",f==null?void 0:f.message,"p2:",b==null?void 0:b.message,"p1D:",(W=$==null?void 0:$.starters)==null?void 0:W.length,"p2D:",(te=y==null?void 0:y.starters)==null?void 0:te.length);const k=ee=>{const fe=Number(ee.evolution_bonus)||0;return{cardId:ee.card_id,position:ee.position,id:ee.id,firstname:ee.firstname,name:ee.surname_encoded,country_code:ee.country_code,club_id:ee.club_id,job:ee.job,job2:ee.job2,note_g:(Number(ee.note_g)||0)+(ee.job==="GK"||ee.job2==="GK"&&Number(ee.note_g)>0?fe:0),note_d:(Number(ee.note_d)||0)+(ee.job==="DEF"||ee.job2==="DEF"&&Number(ee.note_d)>0?fe:0),note_m:(Number(ee.note_m)||0)+(ee.job==="MIL"||ee.job2==="MIL"&&Number(ee.note_m)>0?fe:0),note_a:(Number(ee.note_a)||0)+(ee.job==="ATT"||ee.job2==="ATT"&&Number(ee.note_a)>0?fe:0),evolution_bonus:fe,rarity:ee.rarity,skin:ee.skin,hair:ee.hair,hair_length:ee.hair_length,clubName:ee.club_encoded_name||null,clubLogo:ee.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},S=(($==null?void 0:$.starters)||[]).map(ee=>k(ee)),E=((y==null?void 0:y.starters)||[]).map(ee=>k(ee)),I=($==null?void 0:$.formation)||"4-4-2",z=(y==null?void 0:y.formation)||"4-4-2";let M=vt(S,I),F=vt(E,z);const Y=(($==null?void 0:$.subs)||[]).map(ee=>k(ee)),J=((y==null?void 0:y.subs)||[]).map(ee=>k(ee)),U=($==null?void 0:$.stadium_def)||(o?p:null),G=(y==null?void 0:y.stadium_def)||(o?null:p);return U&&(M=Tt(M,U),jt(Y,U)),G&&(F=Tt(F,G),jt(J,G)),{p1Team:M,p2Team:F,p1Subs:Y,p2Subs:J,p1Formation:I,p2Formation:z,p1Name:(v==null?void 0:v.club_name)||(v==null?void 0:v.pseudo)||"Joueur 1",p2Name:(w==null?void 0:w.club_name)||(w==null?void 0:w.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:o?_:[],gc_p2:o?[]:_,gcCardsFull_p1:o?m:[],gcCardsFull_p2:o?[]:m,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:a||[],lastActionAt:new Date().toISOString()}}let g=c.game_state&&Object.keys(c.game_state).length?c.game_state:null;if(console.log("[PvP] init - amIHome:",o,"gameState exists:",!!g,"match.status:",c.status,"home_id:",c.home_id,"away_id:",c.away_id,"myId:",s.profile.id),!g)if(o){g=await A(),console.log("[PvP] home - gameState built, p1Team keys:",Object.keys((g==null?void 0:g.p1Team)||{}));const{data:$}=await T.from("matches").select("game_state").eq("id",i).single();!($!=null&&$.game_state)||!Object.keys($.game_state).length?await T.from("matches").update({game_state:g,turn_user_id:c.home_id}).eq("id",i):g=$.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>',console.log("[PvP] away - waiting for home to write game_state...");for(let $=0;$<30&&!g;$++){await new Promise(y=>setTimeout(y,400));const{data:f}=await T.from("matches").select("game_state").eq("id",i).single();f!=null&&f.game_state&&Object.keys(f.game_state).length&&(g=f.game_state),$%5===0&&console.log("[PvP] away - poll",$,"game_state:",!!(f!=null&&f.game_state))}if(!g){x("Erreur de synchronisation","error"),n("home");return}g.gc_p2=_,g.gcCardsFull_p2=m,await T.from("matches").update({game_state:g}).eq("id",i)}let L=!1,C=null,q=!1;const D=new Set,j=new Set;async function N($){var z,M;try{T.removeChannel(V)}catch{}const f=g[u+"Score"]||0,y=g[h+"Score"]||0;let b,v;$.winner_id?(b=$.winner_id===s.profile.id,v=!1):$.forfeit?(b=f>y,v=!1):(v=f===y,b=f>y);let w="";if(r&&o)try{const{data:F}=await T.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",c.home_id).single(),{data:Y}=await T.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",c.away_id).single();if(F&&Y){const J=b?1:v?.5:0,U=1-J,G=F.placement_matches<10,W=Y.placement_matches<10,te=Li(F.mmr,F.mmr_deviation,F.mmr_volatility,Y.mmr,Y.mmr_deviation,J===1?1:J===0?0:.5,G),ee=Li(Y.mmr,Y.mmr_deviation,Y.mmr_volatility,F.mmr,F.mmr_deviation,U===1?1:U===0?0:.5,W);await T.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:v?null:b?c.home_id:c.away_id,p_home_id:c.home_id,p_away_id:c.away_id,p_home_delta:te.delta,p_away_delta:ee.delta,p_home_new_rd:te.newRd,p_away_new_rd:ee.newRd,p_home_new_vol:te.newSigma,p_away_new_vol:ee.newSigma});const fe=te.delta,ye=te.newMmr,be=Gt(ye),ke=fe>=0?"+":"",Be=fe>=0?"#4caf50":"#ff6b6b",Fe=Gt(F.mmr);w=`
            ...
            </div>`}}catch(F){console.error("[Ranked] MMR update error:",F)}(z=document.getElementById("pvp-end-overlay"))==null||z.remove();const k=document.createElement("div");k.id="pvp-end-overlay",k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const S=v?"🤝":b?"🏆":"😞",E=v?"MATCH NUL":b?"VICTOIRE !":"DÉFAITE",I=v?"#fff":b?"#FFD700":"#ff6b6b";k.innerHTML=`
      ...
      ${w}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${r?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(k),(M=k.querySelector("#pvp-end-home"))==null||M.addEventListener("click",()=>{k.remove(),Ve(e),n(r?"ranked":"home")})}const V=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},$=>{const f=$.new;try{if(f.status==="finished"||f.forfeit){if(L)return;L=!0,C&&(clearInterval(C),C=null),f.game_state&&(g=f.game_state),N(f);return}if(f.game_state){const y=g;g=f.game_state;const b=g._lastGC;if(b&&b.seq&&!j.has(b.seq)&&(j.add(b.seq),b.by!==u)){se(b.type,b.by,()=>xe());return}const v=y[u+"Score"]||0,w=y[h+"Score"]||0,k=g[u+"Score"]||0,S=g[h+"Score"]||0,E=k>v,I=S>w;if((E||I)&&!D.has(g.round)){D.add(g.round);const z=[...g.log||[]].reverse().find(F=>F.isGoal),M=((z==null?void 0:z.homePlayers)||[]).map(F=>({name:F.name,note:F.note,portrait:F.portrait,job:F.job}));Ee(M,k,S,E,()=>xe());return}xe()}}catch(y){console.error("[PvP] crash:",y)}}).subscribe();async function K($){Object.assign(g,$),g.lastActionAt=new Date().toISOString();const{error:f}=await T.from("matches").update({game_state:g}).eq("id",i);f&&x("Erreur de synchronisation","error");try{xe()}catch(y){console.error("[PvP] renderPvpScreen crash:",y)}}async function oe(){if(L)return;L=!0,C&&(clearInterval(C),C=null);const $=o?c.away_id:c.home_id,f=o?"p2":"p1",y=o?"p1":"p2",b={...g,[f+"Score"]:3,[y+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:$,home_score:b.p1Score||0,away_score:b.p2Score||0,game_state:b}).eq("id",i)}catch{}try{T.removeChannel(V)}catch{}setTimeout(()=>{Ve(e),n("home")},800)}const ie={BOOST_STAT:({value:$=1,count:f=1,roles:y=[]},b,v)=>{const w=b[u+"Team"],k=Object.entries(w).filter(([S])=>!y.length||y.includes(S)).flatMap(([S,E])=>E.filter(I=>!I.used).map(I=>({...I,_line:S})));if(!k.length){b.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),v(b);return}Q(k,f,`Choisir ${f} joueur(s) à booster (+${$})`,S=>{S.forEach(E=>{const I=(w[E._line]||[]).find(z=>z.cardId===E.cardId);I&&(I.boost=(I.boost||0)+$,b.log.push({text:`⚡ +${$} sur ${I.name}`,type:"info"}))}),b[u+"Team"]=w,v(b)})},DEBUFF_STAT:({value:$=1,count:f=1,roles:y=[],target:b="ai"},v,w)=>{const k=b==="home"?u:h,S=v[k+"Team"],E=b==="home"?"allié":"adverse",I=Object.entries(S).filter(([z])=>!y.length||y.includes(z)).flatMap(([z,M])=>M.map(F=>({...F,_line:z})));if(!I.length){v.log.push({text:`🎯 Aucun joueur ${E}`,type:"info"}),w(v);return}Q(I,f,`Choisir ${f} joueur(s) ${E}(s) (-${$})`,z=>{z.forEach(M=>{const F=(S[M._line]||[]).find(Y=>Y.cardId===M.cardId);F&&(F.boost=(F.boost||0)-$,v.log.push({text:`🎯 -${$} sur ${F.name}`,type:"info"}))}),v[k+"Team"]=S,w(v)})},GRAY_PLAYER:({count:$=1,roles:f=[],target:y="ai"},b,v)=>{const w=y==="home"?u:h,k=b[w+"Team"],S=y==="home"?"allié":"adverse",E=Object.entries(k).filter(([I])=>!f.length||f.includes(I)).flatMap(([I,z])=>z.filter(M=>!M.used).map(M=>({...M,_line:I})));if(!E.length){b.log.push({text:`❌ Aucun joueur ${S}`,type:"info"}),v(b);return}Q(E,$,`Choisir ${$} joueur(s) ${S}(s) à exclure`,I=>{const z="usedCardIds_"+w,M=new Set(b[z]||[]);I.forEach(F=>{const Y=(k[F._line]||[]).find(J=>J.cardId===F.cardId);Y&&(Y.used=!0,M.add(F.cardId),b.log.push({text:`❌ ${Y.name} exclu !`,type:"info"}))}),b[z]=[...M],b[w+"Team"]=k,v(b)})},REVIVE_PLAYER:({count:$=1,roles:f=[]},y,b)=>{const v=y[u+"Team"],w=Object.entries(v).filter(([k])=>!f.length||f.includes(k)).flatMap(([k,S])=>S.filter(E=>E.used).map(E=>({...E,_line:k})));if(!w.length){y.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),b(y);return}Q(w,$,`Choisir ${$} joueur(s) à ressusciter`,k=>{k.forEach(S=>{const E=(v[S._line]||[]).find(I=>I.cardId===S.cardId);E&&(E.used=!1,y.log.push({text:`💫 ${E.name} ressuscité !`,type:"info"}))}),y[u+"Team"]=v,b(y)})},REMOVE_GOAL:({},$,f)=>{const y=h+"Score";$[y]>0?($[y]--,$.log.push({text:"🚫 Dernier but annulé !",type:"info"})):$.log.push({text:"🚫 Aucun but à annuler",type:"info"}),f($)},ADD_GOAL_DRAW:({},$,f)=>{$[u+"Score"]===$[h+"Score"]?($[u+"Score"]++,$.log.push({text:"🎯 But bonus !",type:"info"})):$.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),f($)},ADD_SUB:({value:$=1},f,y)=>{f.maxSubs=(f.maxSubs||3)+$,f.log.push({text:`🔄 +${$} remplacement(s)`,type:"info"}),y(f)},CUSTOM:({},$,f)=>f($)};function Q($,f,y,b){const v=document.createElement("div");v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let w=[];function k(){var E,I;const S=$.map(z=>{const M={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[z._line]||"#555",F=ze(z,z._line)+(z.boost||0),J=w.find(G=>G.cardId===z.cardId)?"#FFD700":"rgba(255,255,255,0.25)",U=z.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${z.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${J};background:${M};overflow:hidden;cursor:pointer;${U}">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ut($,g[u+"Formation"],Ne,I,300,300,fe)}
      </div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(w);let E=!1;const I=()=>{E||(E=!0,w.remove(),setTimeout(()=>v(),50))};w.addEventListener("click",I),setTimeout(I,3500)}function se($,f,y){var U,G;const b=(g.gcDefs||[]).find(W=>{var te;return W.name===$||((te=W.name)==null?void 0:te.toLowerCase().trim())===($==null?void 0:$.toLowerCase().trim())}),v={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[b==null?void 0:b.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",w={purple:"#b06ce0",light_blue:"#00d4ef"}[b==null?void 0:b.color]||"#b06ce0",k=(b==null?void 0:b.name)||$,S=(b==null?void 0:b.effect)||((U=Oe[$])==null?void 0:U.desc)||"",E=b!=null&&b.image_url?`/icons/${b.image_url}`:null,I=((G=Oe[$])==null?void 0:G.icon)||"⚡",M=f===u?"Vous":g[f+"Name"]||"Adversaire",F=document.createElement("div");F.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",F.innerHTML=`
      <style>
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(F);let Y=!1;const J=()=>{Y||(Y=!0,F.remove(),setTimeout(()=>y&&y(),50))};F.addEventListener("click",J),setTimeout(J,3e3)}function pe($,f){var F,Y,J,U;const b=(g["gcCardsFull_"+u]||[]).find(G=>G.id===$),v=b==null?void 0:b._gcDef,w={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[v==null?void 0:v.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",k={purple:"#b06ce0",light_blue:"#00d4ef"}[v==null?void 0:v.color]||"#b06ce0",S=(v==null?void 0:v.name)||f,E=(v==null?void 0:v.effect)||((F=Oe[f])==null?void 0:F.desc)||"Carte spéciale.",I=v!=null&&v.image_url?`/icons/${v.image_url}`:null,z=((Y=Oe[f])==null?void 0:Y.icon)||"⚡",M=document.createElement("div");M.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",M.innerHTML=`
      <div style="width:190px;border-radius:16px;border:3px solid ${k};background:${w};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 40px ${k}66">
        ...
        <div style="height:160px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${I?`<img src="${I}" style="max-width:150px;max-height:150px;object-fit:contain">`:`<span style="font-size:72px">${z}</span>`}
        </div>
        <div style="padding:10px;background:rgba(0,0,0,0.38);text-align:center">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          <div style="font-size:9px;color:#00ff88;letter-spacing:2px;text-transform:uppercase;font-weight:700">Joueur qui entre</div>
          <button id="pin-up" style="${ee(I===0)}" ${I===0?"disabled":""}>▲</button>
          <div>${G?Xe({...G,used:!1,boost:0},W,te):"<div>—</div>"}</div>
          <button id="pin-down" style="${ee(I>=k.length-1)}" ${I>=k.length-1?"disabled":""}>▼</button>
      ...
      </div>`,(ye=M.querySelector("#psub-close"))==null||ye.addEventListener("click",()=>M.remove()),(be=M.querySelector("#pout-up"))==null||be.addEventListener("click",()=>{S>0&&(S--,F())}),(ke=M.querySelector("#pout-down"))==null||ke.addEventListener("click",()=>{S<w.length-1&&(S++,F())}),(Be=M.querySelector("#pin-up"))==null||Be.addEventListener("click",()=>{I>0&&(I--,F())}),(Fe=M.querySelector("#pin-down"))==null||Fe.addEventListener("click",()=>{I<k.length-1&&(I++,F())});const fe=(je,Ae,Ne,Ue)=>{const Ye=M.querySelector("#"+je);if(!Ye)return;let Ke=0;Ye.addEventListener("touchstart",Re=>{Ke=Re.touches[0].clientY},{passive:!0}),Ye.addEventListener("touchend",Re=>{const Ge=Re.changedTouches[0].clientY-Ke;if(Math.abs(Ge)<30)return;const We=Ae();Ge<0&&We<Ue-1?(Ne(We+1),F()):Ge>0&&We>0&&(Ne(We-1),F())},{passive:!0})};fe("pin-panel",()=>I,je=>I=je,k.length),fe("pout-panel",()=>S,je=>S=je,w.length),(Me=M.querySelector("#psub-confirm"))==null||Me.addEventListener("click",async je=>{if(je.preventDefault(),je.stopPropagation(),z)return;z=!0;const Ae=w[S],Ne=k[I];if(!Ae||!Ne)return;const Ue=Ae._line,Ye=(y[Ue]||[]).findIndex(Ge=>Ge.cardId===Ae.cardId);if(Ye===-1){x("Erreur : joueur introuvable","error"),M.remove();return}const Ke={...Ne,_line:Ue,position:Ae.position,used:!1,boost:0};y[Ue].splice(Ye,1,Ke);const Re=[...b,Ne.cardId];M.remove(),Z(Ae,Ne,async()=>{await K({[u+"Team"]:y,[h+"Team"]:g[h+"Team"],["usedSubIds_"+u]:Re})})})}document.body.appendChild(M),F()}function Z($,f,y){const b={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},v=document.createElement("div");v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const w=(E,I,z)=>`<div style="text-align:center">
      <div style="font-size:9px;color:${I};letter-spacing:2px;text-transform:uppercase;font-weight:700;margin-bottom:6px">${z}</div>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      </div>`,(R=document.getElementById("lobby-cancel"))==null||R.addEventListener("click",async()=>{_=!0,m&&(T.removeChannel(m),m=null),ie&&await T.from("friend_match_invites").update({status:"declined"}).eq("id",ie),Ve(e),x("home")})},A=async(K,oe)=>{_=!0,m&&(T.removeChannel(m),m=null),await new Promise(ie=>setTimeout(ie,600)),e.isConnected&&Yo(e,t,K,oe,r,l)},{data:g}=await T.from("friend_match_invites").select("*").eq("status","pending").or(`and(inviter_id.eq.${h},invitee_id.eq.${p}),and(inviter_id.eq.${p},invitee_id.eq.${h})`).order("created_at",{ascending:!1}).limit(1).maybeSingle();let L,C;if(g&&g.inviter_id===p)C=!1,L=g.id,await T.from("friend_match_invites").update({invitee_deck_id:i,invitee_ready:!0,status:"accepted"}).eq("id",L);else{C=!0;const{data:K,error:oe}=await T.from("friend_match_invites").insert({inviter_id:h,invitee_id:p,friend_id:p,inviter_deck_id:i,inviter_ready:!0,status:"pending"}).select().single();if(oe||!K){u("Erreur création invitation","error"),Ve(e),x("home");return}L=K.id}if(c(!0,!C,L),!C){const K={home_id:p,away_id:h,home_deck_id:g.inviter_deck_id,away_deck_id:i,status:"active",mode:"friend"},oe=await T.from("matches").insert(K).select().single(),ie=oe.data;if(!ie){u("Erreur création match: "+(((V=oe.error)==null?void 0:V.message)||""),"error"),Ve(e),x("home");return}await T.from("friend_match_invites").update({match_id:ie.id,status:"matched"}).eq("id",L),A(ie.id,!1);return}let q=!1;const D=K=>{_||q||(K.status==="matched"&&K.match_id?(q=!0,clearInterval(j),clearInterval(N),A(K.match_id,!0)):K.status==="declined"?(clearInterval(j),clearInterval(N),u(`${s} a décliné l'invitation`,"warning"),Ve(e),x("home")):K.invitee_ready&&c(!0,!0,L))},j=setInterval(()=>{if(_){clearInterval(j);return}T.from("matchmaking_queue").delete().eq("user_id",h).then(()=>{},()=>{})},3e3),N=setInterval(async()=>{if(_||q){clearInterval(N);return}const{data:K}=await T.from("friend_match_invites").select("*").eq("id",L).maybeSingle();K&&D(K)},1200);m=T.channel("friend-invite-"+L).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${L}`},K=>D(K.new)).subscribe()}async function Yo(e,t,i,o,d=[],a=[]){const{state:r,navigate:l,toast:p}=t,s=o?"p1":"p2",n=o?"p2":"p1",x=(d||[]).map(B=>B.id),u=(d||[]).map(B=>({id:B.id,gc_type:B.gc_type,_gcDef:B._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:h}=await T.from("matches").select("*").eq("id",i).single();if(!h){p("Match introuvable","error"),l("home");return}async function _(){const[{data:B},{data:$},{data:f},{data:y}]=await Promise.all([T.rpc("get_deck_for_match",{p_deck_id:h.home_deck_id}),T.rpc("get_deck_for_match",{p_deck_id:h.away_deck_id}),T.from("users").select("id,pseudo,club_name").eq("id",h.home_id).single(),T.from("users").select("id,pseudo,club_name").eq("id",h.away_id).single()]),b=E=>{const I=Number(E.evolution_bonus)||0;return{cardId:E.card_id,position:E.position,id:E.id,firstname:E.firstname,name:E.surname_encoded,country_code:E.country_code,club_id:E.club_id,job:E.job,job2:E.job2,note_g:(Number(E.note_g)||0)+(E.job==="GK"||E.job2==="GK"&&Number(E.note_g)>0?I:0),note_d:(Number(E.note_d)||0)+(E.job==="DEF"||E.job2==="DEF"&&Number(E.note_d)>0?I:0),note_m:(Number(E.note_m)||0)+(E.job==="MIL"||E.job2==="MIL"&&Number(E.note_m)>0?I:0),note_a:(Number(E.note_a)||0)+(E.job==="ATT"||E.job2==="ATT"&&Number(E.note_a)>0?I:0),evolution_bonus:I,rarity:E.rarity,skin:E.skin,hair:E.hair,hair_length:E.hair_length,clubName:E.club_encoded_name||null,clubLogo:E.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},v=((B==null?void 0:B.starters)||[]).map(E=>b(E)),w=(($==null?void 0:$.starters)||[]).map(E=>b(E)),k=(B==null?void 0:B.formation)||"4-4-2",S=($==null?void 0:$.formation)||"4-4-2";return{p1Team:(()=>{const E=vt(v,k);return stadiumDef?Tt(E,stadiumDef):E})(),p2Team:vt(w,S),p1Subs:(()=>{const E=((B==null?void 0:B.subs)||[]).map(I=>b(I));return stadiumDef&&jt(E,stadiumDef),E})(),p2Subs:(($==null?void 0:$.subs)||[]).map(E=>b(E)),p1Formation:k,p2Formation:S,p1Name:(f==null?void 0:f.club_name)||(f==null?void 0:f.pseudo)||"Joueur 1",p2Name:(y==null?void 0:y.club_name)||(y==null?void 0:y.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:o?x:[],gc_p2:o?[]:x,gcCardsFull_p1:o?u:[],gcCardsFull_p2:o?[]:u,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:a||[],lastActionAt:new Date().toISOString()}}let m=h.game_state&&Object.keys(h.game_state).length?h.game_state:null;if(!m)if(o){m=await _();const{data:B}=await T.from("matches").select("game_state").eq("id",i).single();!(B!=null&&B.game_state)||!Object.keys(B.game_state).length?await T.from("matches").update({game_state:m,turn_user_id:h.home_id}).eq("id",i):m=B.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let B=0;B<60&&!m;B++){await new Promise(f=>setTimeout(f,400));const{data:$}=await T.from("matches").select("game_state").eq("id",i).single();$!=null&&$.game_state&&Object.keys($.game_state).length&&(m=$.game_state)}if(!m){p("Erreur de synchronisation","error"),l("home");return}m.gc_p2=x,m.gcCardsFull_p2=u,await T.from("matches").update({game_state:m}).eq("id",i)}let c=!1,A=!1,g=!1,L=null,C=!1;const q=new Set,D=new Set;function j(B){var E,I;try{T.removeChannel(N)}catch{}const $=m[s+"Score"]||0,f=m[n+"Score"]||0;if(!g){g=!0;const z=B.winner_id||($>f?r.profile.id:f>$?"opp":null),M=z===r.profile.id?"win":z?"loss":null;M&&ni(()=>Promise.resolve().then(()=>nn),void 0).then(F=>F.applyOwnEvolution(r.profile.id,M)).catch(()=>{})}if(!A){const z=m.p1Score||0,M=m.p2Score||0,F=(m.usedGc_p1||[]).length,Y=(m.usedGc_p2||[]).length,J=B.winner_id||(z>M?h.home_id:M>z?h.away_id:null);(J?r.profile.id===J:r.profile.id<(o?h.away_id:h.home_id))&&(A=!0,Wi({player1Id:h.home_id,player2Id:h.away_id,score1:z,score2:M,gc1:F,gc2:Y}).catch(G=>console.warn("[FriendMatch] updateStats:",G)))}let y,b;B.winner_id?(y=B.winner_id===r.profile.id,b=!1):B.forfeit?(y=$>f,b=!1):(b=$===f,y=$>f),(E=document.getElementById("pvp-end-overlay"))==null||E.remove();const v=document.createElement("div");v.id="pvp-end-overlay",v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const w=b?"🤝":y?"🏆":"😞",k=b?"MATCH NUL":y?"VICTOIRE !":"DÉFAITE",S=b?"#fff":y?"#FFD700":"#ff6b6b";v.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(v),(I=v.querySelector("#pvp-end-home"))==null||I.addEventListener("click",()=>{v.remove(),Ve(e),l("home")})}const N=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},B=>{const $=B.new;try{if($.status==="finished"||$.forfeit){if(c)return;c=!0,L&&(clearInterval(L),L=null),$.game_state&&(m=$.game_state),j($);return}if($.game_state){const f=m;m=$.game_state;const y=m._lastGC;if(y&&y.seq&&!D.has(y.seq)&&(D.add(y.seq),y.by!==s)){Ee(y.type,y.by,()=>re());return}const b=f[s+"Score"]||0,v=f[n+"Score"]||0,w=m[s+"Score"]||0,k=m[n+"Score"]||0,S=w>b,E=k>v;if((S||E)&&!q.has(m.round)){q.add(m.round);const I=[...m.log||[]].reverse().find(M=>M.isGoal),z=((I==null?void 0:I.homePlayers)||[]).map(M=>({name:M.name,note:M.note,portrait:M.portrait,job:M.job}));ve(z,w,k,S,()=>re());return}re()}}catch(f){console.error("[PvP] crash:",f)}}).subscribe();async function V(B){Object.assign(m,B),m.lastActionAt=new Date().toISOString();const{error:$}=await T.from("matches").update({game_state:m}).eq("id",i);$&&p("Erreur de synchronisation","error");try{re()}catch(f){console.error("[PvP] renderPvpScreen crash:",f)}}async function K(){if(c)return;c=!0,L&&(clearInterval(L),L=null);const B=o?h.away_id:h.home_id,$=o?"p2":"p1",f=o?"p1":"p2",y={...m,[$+"Score"]:3,[f+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:B,home_score:y.p1Score||0,away_score:y.p2Score||0,game_state:y}).eq("id",i)}catch{}try{T.removeChannel(N)}catch{}setTimeout(()=>{Ve(e),l("home")},800)}const oe={BOOST_STAT:({value:B=1,count:$=1,roles:f=[]},y,b)=>{const v=y[s+"Team"],w=Object.entries(v).filter(([k])=>!f.length||f.includes(k)).flatMap(([k,S])=>S.filter(E=>!E.used).map(E=>({...E,_line:k})));if(!w.length){y.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),b(y);return}ie(w,$,`Choisir ${$} joueur(s) à booster (+${B})`,k=>{k.forEach(S=>{const E=(v[S._line]||[]).find(I=>I.cardId===S.cardId);E&&(E.boost=(E.boost||0)+B,y.log.push({text:`⚡ +${B} sur ${E.name}`,type:"info"}))}),y[s+"Team"]=v,b(y)})},DEBUFF_STAT:({value:B=1,count:$=1,roles:f=[],target:y="ai"},b,v)=>{const w=y==="home"?s:n,k=b[w+"Team"],S=y==="home"?"allié":"adverse",E=Object.entries(k).filter(([I])=>!f.length||f.includes(I)).flatMap(([I,z])=>z.map(M=>({...M,_line:I})));if(!E.length){b.log.push({text:`🎯 Aucun joueur ${S}`,type:"info"}),v(b);return}ie(E,$,`Choisir ${$} joueur(s) ${S}(s) (-${B})`,I=>{I.forEach(z=>{const M=(k[z._line]||[]).find(F=>F.cardId===z.cardId);M&&(M.boost=(M.boost||0)-B,b.log.push({text:`🎯 -${B} sur ${M.name}`,type:"info"}))}),b[w+"Team"]=k,v(b)})},GRAY_PLAYER:({count:B=1,roles:$=[],target:f="ai"},y,b)=>{const v=f==="home"?s:n,w=y[v+"Team"],k=f==="home"?"allié":"adverse",S=Object.entries(w).filter(([E])=>!$.length||$.includes(E)).flatMap(([E,I])=>I.filter(z=>!z.used).map(z=>({...z,_line:E})));if(!S.length){y.log.push({text:`❌ Aucun joueur ${k}`,type:"info"}),b(y);return}ie(S,B,`Choisir ${B} joueur(s) ${k}(s) à exclure`,E=>{const I="usedCardIds_"+v,z=new Set(y[I]||[]);E.forEach(M=>{const F=(w[M._line]||[]).find(Y=>Y.cardId===M.cardId);F&&(F.used=!0,z.add(M.cardId),y.log.push({text:`❌ ${F.name} exclu !`,type:"info"}))}),y[I]=[...z],y[v+"Team"]=w,b(y)})},REVIVE_PLAYER:({count:B=1,roles:$=[]},f,y)=>{const b=f[s+"Team"],v=Object.entries(b).filter(([w])=>!$.length||$.includes(w)).flatMap(([w,k])=>k.filter(S=>S.used).map(S=>({...S,_line:w})));if(!v.length){f.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),y(f);return}ie(v,B,`Choisir ${B} joueur(s) à ressusciter`,w=>{w.forEach(k=>{const S=(b[k._line]||[]).find(E=>E.cardId===k.cardId);S&&(S.used=!1,f.log.push({text:`💫 ${S.name} ressuscité !`,type:"info"}))}),f[s+"Team"]=b,y(f)})},REMOVE_GOAL:({},B,$)=>{const f=n+"Score";B[f]>0?(B[f]--,B.log.push({text:"🚫 Dernier but annulé !",type:"info"})):B.log.push({text:"🚫 Aucun but à annuler",type:"info"}),$(B)},ADD_GOAL_DRAW:({},B,$)=>{B[s+"Score"]===B[n+"Score"]?(B[s+"Score"]++,B.log.push({text:"🎯 But bonus !",type:"info"})):B.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),$(B)},ADD_SUB:({value:B=1},$,f)=>{$.maxSubs=($.maxSubs||3)+B,$.log.push({text:`🔄 +${B} remplacement(s)`,type:"info"}),f($)},CUSTOM:({},B,$)=>$(B)};function ie(B,$,f,y){const b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let v=[];function w(){var S,E;const k=B.map(I=>{const z={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[I._line]||"#555",M=ze(I,I._line)+(I.boost||0),Y=v.find(U=>U.cardId===I.cardId)?"#FFD700":"rgba(255,255,255,0.25)",J=I.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${I.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${Y};background:${z};overflow:hidden;cursor:pointer;${J}">
        ...
        </div>`,(S=b.querySelector("#pp-close"))==null||S.addEventListener("click",()=>b.remove()),b.querySelectorAll(".pp-item").forEach(I=>{I.addEventListener("click",()=>{const z=I.dataset.cid,M=B.find(Y=>Y.cardId===z),F=v.findIndex(Y=>Y.cardId===z);M&&(F>-1?v.splice(F,1):v.length<$&&v.push(M),w())})}),(E=b.querySelector("#pp-confirm"))==null||E.addEventListener("click",()=>{b.remove(),y(v)})}w(),document.body.appendChild(b)}async function Q(B,$){const y=(m["gcCardsFull_"+s]||[]).find(k=>k.id===B),b=(y==null?void 0:y._gcDef)||(m.gcDefs||[]).find(k=>{var S;return k.name===$||((S=k.name)==null?void 0:S.toLowerCase().trim())===($==null?void 0:$.toLowerCase().trim())}),v=[...m["usedGc_"+s]||[],B],w={type:$,by:s,seq:(m._gcAnimSeq||0)+1};D.add(w.seq),Ee($,s,async()=>{if(b!=null&&b.effect_type&&b.effect_type!=="CUSTOM"){const S=oe[b.effect_type];if(S){const E={...m};S(b.effect_params||{},E,async I=>{I["usedGc_"+s]=v,I._lastGC=w,I._gcAnimSeq=w.seq,await V(I)});return}}const k={...m};switch($){case"Remplacement+":k.maxSubs=(k.maxSubs||3)+1,k.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const S=n+"Score";k[S]>0&&(k[S]--,k.log.push({text:"🚫 But annulé",type:"info"}));break}}k["usedGc_"+s]=v,k._lastGC=w,k._gcAnimSeq=w.seq,await V(k)})}function R(B,$){const f="usedCardIds_"+B,y=new Set(m[f]||[]);$.forEach(b=>y.add(b)),m[f]=[...y]}function P(){for(const B of["p1","p2"]){const $=new Set(m["usedCardIds_"+B]||[]),f=m[B+"Team"];if(f)for(const y of["GK","DEF","MIL","ATT"])(f[y]||[]).forEach(b=>{b.used=$.has(b.cardId)})}}function re(){var Ge,We,nt,ft,gt,mt;if(m.phase==="reveal")return xe();if(m.phase==="midfield")return we();if(m.phase==="finished")return Se();const B=m[s+"Team"],$=m[n+"Team"];P();const f=m[s+"Score"],y=m[n+"Score"],b=m[s+"Name"],v=m[n+"Name"],w=m.phase===s+"-attack",k=m.phase===s+"-defense",S=Array.isArray(m["selected_"+s])?m["selected_"+s]:[],E=S.map(le=>le.cardId),I=window.innerWidth>=700,z=m[s+"Subs"]||[],M=m["usedSubIds_"+s]||[],F=z.filter(le=>!M.includes(le.cardId)),Y=m["gcCardsFull_"+s]||[],J=m["usedGc_"+s]||[],U=Y.filter(le=>!J.includes(le.id)),G=m.boostOwner===s&&!m.boostUsed,W=!["GK","DEF","MIL","ATT"].some(le=>($[le]||[]).some(X=>!X.used)),te=[...B.MIL||[],...B.ATT||[]].filter(le=>!le.used),ee=w&&W&&te.length===0?[...B.GK||[],...B.DEF||[]].filter(le=>!le.used).map(le=>le.cardId):[];function fe(le,X,O){var yt,kt;const ae=le._gcDef,he={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ae==null?void 0:ae.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",$e={purple:"#b06ce0",light_blue:"#00d4ef"}[ae==null?void 0:ae.color]||"#b06ce0",Qe=(ae==null?void 0:ae.name)||le.gc_type,Ze=(ae==null?void 0:ae.effect)||((yt=Oe[le.gc_type])==null?void 0:yt.desc)||"",tt=ae!=null&&ae.image_url?`/icons/${ae.image_url}`:null,rt=((kt=Oe[le.gc_type])==null?void 0:kt.icon)||"⚡",_t=Math.round(O*.22),$t=Math.round(O*.22),lt=O-_t-$t,xt=Qe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${le.id}" data-gc-type="${le.gc_type}"
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ut(B,m[s+"Formation"],Ae,E,300,300,ee)}
      </div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      ${B!=null&&B.length?`<div style="display:flex;gap:10px;animation:scoreIn .4s ease 1s both;position:relative;z-index:1">
        ${B.map(I=>`<div style="text-align:center">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(M);let F=!1;const Y=()=>{F||(F=!0,M.remove(),setTimeout(()=>f&&f(),50))};M.addEventListener("click",Y),setTimeout(Y,3e3)}function se(B,$){var M,F,Y,J;const y=(m["gcCardsFull_"+s]||[]).find(U=>U.id===B),b=y==null?void 0:y._gcDef,v={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[b==null?void 0:b.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",w={purple:"#b06ce0",light_blue:"#00d4ef"}[b==null?void 0:b.color]||"#b06ce0",k=(b==null?void 0:b.name)||$,S=(b==null?void 0:b.effect)||((M=Oe[$])==null?void 0:M.desc)||"Carte spéciale.",E=b!=null&&b.image_url?`/icons/${b.image_url}`:null,I=((F=Oe[$])==null?void 0:F.icon)||"⚡",z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",z.innerHTML=`
      ...
      </div>`,document.body.appendChild(f),(y=f.querySelector("#bp-close"))==null||y.addEventListener("click",()=>f.remove()),f.querySelectorAll(".bp-item").forEach(b=>{b.addEventListener("click",async()=>{const v=b.dataset.cid,w=$.find(S=>S.cardId===v);if(!w)return;const k=(B[w._line]||[]).find(S=>S.cardId===v);k&&(k.boost=(k.boost||0)+m.boostValue),f.remove(),await V({[s+"Team"]:B,boostUsed:!0})})})}function ce(B=null){var F,Y;if(!(m.phase===s+"-attack")){p("Remplacement uniquement avant votre attaque","warning");return}const f=m[s+"Team"],y=m["usedSubIds_"+s]||[],b=m.maxSubs||3;if(y.length>=b){p(`Maximum ${b} remplacements atteint`,"warning");return}const v=Object.entries(f).flatMap(([J,U])=>(U||[]).filter(G=>G.used).map(G=>({...G,_line:J}))),w=(m[s+"Subs"]||[]).filter(J=>!y.includes(J.cardId));if(!v.length){p("Aucun joueur utilisé à remplacer","warning");return}if(!w.length){p("Aucun remplaçant disponible","warning");return}let k=Math.max(0,v.findIndex(J=>J.cardId===B));const S=((F=v[k])==null?void 0:F._line)||((Y=v[k])==null?void 0:Y.job);let E=Math.max(0,w.findIndex(J=>J.job===S)),I=!1;const z=document.createElement("div");z.id="pvp-sub-overlay",z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function M(){var fe,ye,be,ke,Be,Fe;const J=v[k],U=w[E],G=Math.min(130,Math.round((window.innerWidth-90)/2)),W=Math.round(G*1.35),te=Me=>`background:rgba(255,255,255,0.12);border:none;color:${Me?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Me?"default":"pointer"};flex-shrink:0`;z.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="width:min(90vw,420px)">${pt(m[n+"Team"],m[n+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(B),($=B.querySelector("#pvp-opp-close"))==null||$.addEventListener("click",()=>B.remove())}function ge(){var y;const B=m.log||[],$=document.createElement("div");$.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column";const f=b=>{var w,k,S;if(b.type==="duel"){const E=b.isGoal,I=b.homeScored&&s==="p1"||!b.homeScored&&E&&s==="p2",z=b.homeScored?"#FFD700":E?"#ff6b6b":"rgba(255,255,255,0.3)",M=E?I?"⚽ BUT !":"⚽ BUT adversaire !":(w=b.homePlayers)!=null&&w.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${E?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${z};margin-bottom:4px">
  ...
  </div>`,(A=document.getElementById("ml-create-btn"))==null||A.addEventListener("click",()=>Zo(e,t)),e.querySelectorAll(".ml-tab").forEach(g=>g.addEventListener("click",()=>St(e,t,g.dataset.tab))),e.querySelectorAll("[data-league-id]").forEach(g=>g.addEventListener("click",()=>wt(e,t,g.dataset.leagueId))),e.querySelectorAll("[data-join]").forEach(g=>g.addEventListener("click",L=>{L.stopPropagation(),fn(e,t,g.dataset.join,g.dataset.type)})),e.querySelectorAll("[data-delete]").forEach(g=>g.addEventListener("click",L=>{L.stopPropagation(),vi(e,t,g.dataset.delete,g.dataset.name,i)}))}function ii(e,t,i=!1){var r,l;const o=e.creator_id===t,d=e.pot||0,a=((l=(r=e.mini_league_members)==null?void 0:r[0])==null?void 0:l.count)||0;return`<div data-league-id="${e.id}" style="background:#fff;border-radius:12px;padding:14px 16px;box-shadow:0 1px 6px rgba(0,0,0,0.08);cursor:pointer;margin-bottom:8px">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="background:linear-gradient(160deg,#1a0a2e,#3b1a6e);border-radius:20px;padding:32px 24px;max-width:320px;width:100%;text-align:center;color:#fff;box-shadow:0 8px 40px rgba(0,0,0,0.6)">
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏆 Retour à la Mini League</button>`,document.body.appendChild(k),(M=k.querySelector("#pvp-end-home"))==null||M.addEventListener("click",()=>{k.remove(),Ve(e),s?l("mini-league",{openLeagueId:s}):l("mini-league")})}const V=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},f=>{const y=f.new;try{if(y.status==="finished"||y.forfeit){if(A||(A=!0,C&&(clearInterval(C),C=null),y.game_state&&(c=y.game_state),c.phase==="finished"&&!y.forfeit&&document.getElementById("pvp-home")))return;N(y);return}if(y.game_state){const b=c;c=y.game_state;const v=c._lastGC;if(v&&v.seq&&!j.has(v.seq)&&(j.add(v.seq),v.by!==n)){se(v.type,v.by,()=>xe());return}const w=b[n+"Score"]||0,k=b[x+"Score"]||0,S=c[n+"Score"]||0,E=c[x+"Score"]||0,I=S>w,z=E>k;if((I||z)&&!D.has(c.round)){D.add(c.round);const M=[...c.log||[]].reverse().find(Y=>Y.isGoal),F=((M==null?void 0:M.homePlayers)||[]).map(Y=>({name:Y.name,note:Y.note,portrait:Y.portrait,job:Y.job}));Ee(F,S,E,I,()=>xe());return}xe()}}catch(b){console.error("[PvP] crash:",b)}}).subscribe();async function K(f){Object.assign(c,f),c.lastActionAt=new Date().toISOString();const{error:y}=await T.from("matches").update({game_state:c}).eq("id",i);y&&p("Erreur de synchronisation","error");try{xe()}catch(b){console.error("[PvP] renderPvpScreen crash:",b)}}async function oe(){if(A)return;A=!0,C&&(clearInterval(C),C=null);const f=o?_.away_id:_.home_id,y=o?"p2":"p1",b=o?"p1":"p2",v={...c,[y+"Score"]:3,[b+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:f,home_score:v.p1Score||0,away_score:v.p2Score||0,game_state:v}).eq("id",i)}catch{}try{T.removeChannel(V)}catch{}setTimeout(()=>{Ve(e),l("mini-league")},800)}const ie={BOOST_STAT:({value:f=1,count:y=1,roles:b=[]},v,w)=>{const k=v[n+"Team"],S=Object.entries(k).filter(([E])=>!b.length||b.includes(E)).flatMap(([E,I])=>I.filter(z=>!z.used).map(z=>({...z,_line:E})));if(!S.length){v.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),w(v);return}Q(S,y,`Choisir ${y} joueur(s) à booster (+${f})`,E=>{E.forEach(I=>{const z=(k[I._line]||[]).find(M=>M.cardId===I.cardId);z&&(z.boost=(z.boost||0)+f,v.log.push({text:`⚡ +${f} sur ${z.name}`,type:"info"}))}),v[n+"Team"]=k,w(v)})},DEBUFF_STAT:({value:f=1,count:y=1,roles:b=[],target:v="ai"},w,k)=>{const S=v==="home"?n:x,E=w[S+"Team"],I=v==="home"?"allié":"adverse",z=Object.entries(E).filter(([M])=>!b.length||b.includes(M)).flatMap(([M,F])=>F.map(Y=>({...Y,_line:M})));if(!z.length){w.log.push({text:`🎯 Aucun joueur ${I}`,type:"info"}),k(w);return}Q(z,y,`Choisir ${y} joueur(s) ${I}(s) (-${f})`,M=>{M.forEach(F=>{const Y=(E[F._line]||[]).find(J=>J.cardId===F.cardId);Y&&(Y.boost=(Y.boost||0)-f,w.log.push({text:`🎯 -${f} sur ${Y.name}`,type:"info"}))}),w[S+"Team"]=E,k(w)})},GRAY_PLAYER:({count:f=1,roles:y=[],target:b="ai"},v,w)=>{const k=b==="home"?n:x,S=v[k+"Team"],E=b==="home"?"allié":"adverse",I=Object.entries(S).filter(([z])=>!y.length||y.includes(z)).flatMap(([z,M])=>M.filter(F=>!F.used).map(F=>({...F,_line:z})));if(!I.length){v.log.push({text:`❌ Aucun joueur ${E}`,type:"info"}),w(v);return}Q(I,f,`Choisir ${f} joueur(s) ${E}(s) à exclure`,z=>{const M="usedCardIds_"+k,F=new Set(v[M]||[]);z.forEach(Y=>{const J=(S[Y._line]||[]).find(U=>U.cardId===Y.cardId);J&&(J.used=!0,F.add(Y.cardId),v.log.push({text:`❌ ${J.name} exclu !`,type:"info"}))}),v[M]=[...F],v[k+"Team"]=S,w(v)})},REVIVE_PLAYER:({count:f=1,roles:y=[]},b,v)=>{const w=b[n+"Team"],k=Object.entries(w).filter(([S])=>!y.length||y.includes(S)).flatMap(([S,E])=>E.filter(I=>I.used).map(I=>({...I,_line:S})));if(!k.length){b.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),v(b);return}Q(k,f,`Choisir ${f} joueur(s) à ressusciter`,S=>{S.forEach(E=>{const I=(w[E._line]||[]).find(z=>z.cardId===E.cardId);I&&(I.used=!1,b.log.push({text:`💫 ${I.name} ressuscité !`,type:"info"}))}),b[n+"Team"]=w,v(b)})},REMOVE_GOAL:({},f,y)=>{const b=x+"Score";f[b]>0?(f[b]--,f.log.push({text:"🚫 Dernier but annulé !",type:"info"})):f.log.push({text:"🚫 Aucun but à annuler",type:"info"}),y(f)},ADD_GOAL_DRAW:({},f,y)=>{f[n+"Score"]===f[x+"Score"]?(f[n+"Score"]++,f.log.push({text:"🎯 But bonus !",type:"info"})):f.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),y(f)},ADD_SUB:({value:f=1},y,b)=>{y.maxSubs=(y.maxSubs||3)+f,y.log.push({text:`🔄 +${f} remplacement(s)`,type:"info"}),b(y)},CUSTOM:({},f,y)=>y(f)};function Q(f,y,b,v){const w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let k=[];function S(){var I,z;const E=f.map(M=>{const F={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[M._line]||"#555",Y=ze(M,M._line)+(M.boost||0),U=k.find(W=>W.cardId===M.cardId)?"#FFD700":"rgba(255,255,255,0.25)",G=M.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${M.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${U};background:${F};overflow:hidden;cursor:pointer;${G}">
        ...
        </div>`,(I=w.querySelector("#pp-close"))==null||I.addEventListener("click",()=>w.remove()),w.querySelectorAll(".pp-item").forEach(M=>{M.addEventListener("click",()=>{const F=M.dataset.cid,Y=f.find(U=>U.cardId===F),J=k.findIndex(U=>U.cardId===F);Y&&(J>-1?k.splice(J,1):k.length<y&&k.push(Y),S())})}),(z=w.querySelector("#pp-confirm"))==null||z.addEventListener("click",()=>{w.remove(),v(k)})}S(),document.body.appendChild(w)}async function R(f,y){const v=(c["gcCardsFull_"+n]||[]).find(E=>E.id===f),w=(v==null?void 0:v._gcDef)||(c.gcDefs||[]).find(E=>{var I;return E.name===y||((I=E.name)==null?void 0:I.toLowerCase().trim())===(y==null?void 0:y.toLowerCase().trim())}),k=[...c["usedGc_"+n]||[],f],S={type:y,by:n,seq:(c._gcAnimSeq||0)+1};j.add(S.seq),se(y,n,async()=>{if(w!=null&&w.effect_type&&w.effect_type!=="CUSTOM"){const I=ie[w.effect_type];if(I){const z={...c};I(w.effect_params||{},z,async M=>{M["usedGc_"+n]=k,M._lastGC=S,M._gcAnimSeq=S.seq,await K(M)});return}}const E={...c};switch(y){case"Remplacement+":E.maxSubs=(E.maxSubs||3)+1,E.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const I=x+"Score";E[I]>0&&(E[I]--,E.log.push({text:"🚫 But annulé",type:"info"}));break}}E["usedGc_"+n]=k,E._lastGC=S,E._gcAnimSeq=S.seq,await K(E)})}function P(f,y){const b="usedCardIds_"+f,v=new Set(c[b]||[]);y.forEach(w=>v.add(w)),c[b]=[...v]}function re(){for(const f of["p1","p2"]){const y=new Set(c["usedCardIds_"+f]||[]),b=c[f+"Team"];if(b)for(const v of["GK","DEF","MIL","ATT"])(b[v]||[]).forEach(w=>{w.used=y.has(w.cardId)})}}function xe(){var nt,ft,gt,mt,le,X;if(c.phase==="reveal")return _e();if(c.phase==="midfield")return ve();if(c.phase==="finished")return B();const f=c[n+"Team"],y=c[x+"Team"];re();const b=c[n+"Score"],v=c[x+"Score"],w=c[n+"Name"],k=c[x+"Name"],S=c.phase===n+"-attack",E=c.phase===n+"-defense",I=Array.isArray(c["selected_"+n])?c["selected_"+n]:[],z=I.map(O=>O.cardId),M=window.innerWidth>=700,F=c[n+"Subs"]||[],Y=c["usedSubIds_"+n]||[],J=F.filter(O=>!Y.includes(O.cardId)),U=c["gcCardsFull_"+n]||[],G=c["usedGc_"+n]||[],W=U.filter(O=>!G.includes(O.id)),te=c.boostOwner===n&&!c.boostUsed,ee=!["GK","DEF","MIL","ATT"].some(O=>(y[O]||[]).some(ae=>!ae.used)),fe=[...f.MIL||[],...f.ATT||[]].filter(O=>!O.used),ye=S&&ee&&fe.length===0?[...f.GK||[],...f.DEF||[]].filter(O=>!O.used).map(O=>O.cardId):[];function be(O,ae,he){var Ct,Ai;const $e=O._gcDef,Qe={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[$e==null?void 0:$e.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Ze={purple:"#b06ce0",light_blue:"#00d4ef"}[$e==null?void 0:$e.color]||"#b06ce0",tt=($e==null?void 0:$e.name)||O.gc_type,rt=($e==null?void 0:$e.effect)||((Ct=Oe[O.gc_type])==null?void 0:Ct.desc)||"",_t=$e!=null&&$e.image_url?`/icons/${$e.image_url}`:null,$t=((Ai=Oe[O.gc_type])==null?void 0:Ai.icon)||"⚡",lt=Math.round(he*.22),xt=Math.round(he*.22),yt=he-lt-xt,kt=tt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${O.id}" data-gc-type="${O.gc_type}"
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      </div>`,document.body.appendChild(b),(v=b.querySelector("#bp-close"))==null||v.addEventListener("click",()=>b.remove()),b.querySelectorAll(".bp-item").forEach(w=>{w.addEventListener("click",async()=>{const k=w.dataset.cid,S=y.find(I=>I.cardId===k);if(!S)return;const E=(f[S._line]||[]).find(I=>I.cardId===k);E&&(E.boost=(E.boost||0)+c.boostValue),b.remove(),await K({[n+"Team"]:f,boostUsed:!0})})})}function H(f=null){var J,U;if(!(c.phase===n+"-attack")){p("Remplacement uniquement avant votre attaque","warning");return}const b=c[n+"Team"],v=c["usedSubIds_"+n]||[],w=c.maxSubs||3;if(v.length>=w){p(`Maximum ${w} remplacements atteint`,"warning");return}const k=Object.entries(b).flatMap(([G,W])=>(W||[]).filter(te=>te.used).map(te=>({...te,_line:G}))),S=(c[n+"Subs"]||[]).filter(G=>!v.includes(G.cardId));if(!k.length){p("Aucun joueur utilisé à remplacer","warning");return}if(!S.length){p("Aucun remplaçant disponible","warning");return}let E=Math.max(0,k.findIndex(G=>G.cardId===f));const I=((J=k[E])==null?void 0:J._line)||((U=k[E])==null?void 0:U.job);let z=Math.max(0,S.findIndex(G=>G.job===I)),M=!1;const F=document.createElement("div");F.id="pvp-sub-overlay",F.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function Y(){var be,ke,Be,Fe,Me,je;const G=k[E],W=S[z],te=Math.min(130,Math.round((window.innerWidth-90)/2)),ee=Math.round(te*1.35),fe=Ae=>`background:rgba(255,255,255,0.12);border:none;color:${Ae?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Ae?"default":"pointer"};flex-shrink:0`;F.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(be=F.querySelector("#psub-close"))==null||be.addEventListener("click",()=>F.remove()),(ke=F.querySelector("#pout-up"))==null||ke.addEventListener("click",()=>{E>0&&(E--,Y())}),(Be=F.querySelector("#pout-down"))==null||Be.addEventListener("click",()=>{E<k.length-1&&(E++,Y())}),(Fe=F.querySelector("#pin-up"))==null||Fe.addEventListener("click",()=>{z>0&&(z--,Y())}),(Me=F.querySelector("#pin-down"))==null||Me.addEventListener("click",()=>{z<S.length-1&&(z++,Y())});const ye=(Ae,Ne,Ue,Ye)=>{const Ke=F.querySelector("#"+Ae);if(!Ke)return;let Re=0;Ke.addEventListener("touchstart",Ge=>{Re=Ge.touches[0].clientY},{passive:!0}),Ke.addEventListener("touchend",Ge=>{const We=Ge.changedTouches[0].clientY-Re;if(Math.abs(We)<30)return;const nt=Ne();We<0&&nt<Ye-1?(Ue(nt+1),Y()):We>0&&nt>0&&(Ue(nt-1),Y())},{passive:!0})};ye("pin-panel",()=>z,Ae=>z=Ae,S.length),ye("pout-panel",()=>E,Ae=>E=Ae,k.length),(je=F.querySelector("#psub-confirm"))==null||je.addEventListener("click",async Ae=>{if(Ae.preventDefault(),Ae.stopPropagation(),M)return;M=!0;const Ne=k[E],Ue=S[z];if(!Ne||!Ue)return;const Ye=Ne._line,Ke=(b[Ye]||[]).findIndex(We=>We.cardId===Ne.cardId);if(Ke===-1){p("Erreur : joueur introuvable","error"),F.remove();return}const Re={...Ue,_line:Ye,position:Ne.position,used:!1,boost:0};b[Ye].splice(Ke,1,Re);const Ge=[...v,Ue.cardId];F.remove(),Z(Ne,Ue,async()=>{await K({[n+"Team"]:b,[x+"Team"]:c[x+"Team"],["usedSubIds_"+n]:Ge})})})}document.body.appendChild(F),Y()}function Z(f,y,b){const v={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const k=(I,z,M)=>`<div style="text-align:center">
      <div style="font-size:9px;color:${z};letter-spacing:2px;text-transform:uppercase;font-weight:700;margin-bottom:6px">${M}</div>
      <div style="width:70px;height:70px;border-radius:50%;background:${v[I.job]||"#555"};border:3px solid ${z};position:relative;overflow:hidden;margin:0 auto">
        ${He(I)?`<img src="${He(I)}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover">`:""}
      </div>
      ...
      <div style="width:min(90vw,420px)">${pt(c[x+"Team"],c[x+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(f),(y=f.querySelector("#pvp-opp-close"))==null||y.addEventListener("click",()=>f.remove())}function de(){var v;const f=c.log||[],y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column";const b=w=>{var S,E,I;if(w.type==="duel"){const z=w.isGoal,M=w.homeScored&&n==="p1"||!w.homeScored&&z&&n==="p2",F=w.homeScored?"#FFD700":z?"#ff6b6b":"rgba(255,255,255,0.3)",Y=z?M?"⚽ BUT !":"⚽ BUT adversaire !":(S=w.homePlayers)!=null&&S.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${z?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${F};margin-bottom:4px">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div id="mkt-content" style="padding:10px 16px;display:flex;flex-direction:column;gap:8px"></div>
  </div>`;let p="buy";const s=()=>{var m,c,A,g,L,C,q;return{name:(((m=document.getElementById("flt-name"))==null?void 0:m.value)||"").toLowerCase().trim(),club:(((c=document.getElementById("flt-club"))==null?void 0:c.value)||"").toLowerCase().trim(),country:(((A=document.getElementById("flt-country"))==null?void 0:A.value)||"").toLowerCase().trim(),job:((g=document.getElementById("flt-job"))==null?void 0:g.value)||"",rarity:((L=document.getElementById("flt-rarity"))==null?void 0:L.value)||"",note1:parseInt((C=document.getElementById("flt-note1"))==null?void 0:C.value)||0,note2:parseInt((q=document.getElementById("flt-note2"))==null?void 0:q.value)||0}};function n(m){const c=s();return m.filter(A=>{var V,K,oe;const g=(V=A.card)==null?void 0:V.player;if(!g)return!1;const L=`${g.firstname} ${g.surname_encoded}`.toLowerCase(),C=(((K=g.clubs)==null?void 0:K.encoded_name)||"").toLowerCase(),q=(g.country_code||"").toLowerCase(),D=((oe=A.card)==null?void 0:oe.evolution_bonus)||0,j=zt(g,g.job,D),N=g.job2?zt(g,g.job2,D):0;return!(c.name&&!L.includes(c.name)||c.club&&!C.includes(c.club)||c.country&&!q.includes(c.country)||c.job&&g.job!==c.job||c.rarity&&g.rarity!==c.rarity||c.note1&&j<c.note1||c.note2&&N<c.note2)})}function x(m){var N,V,K,oe;const c=(N=m.card)==null?void 0:N.player;if(!c)return"";const A=((V=m.card)==null?void 0:V.evolution_bonus)||0,g=zt(c,c.job,A),L=c.job2?zt(c,c.job2,A):0,C=(i.profile.credits||0)>=m.price,q=c.country_code?`https://flagsapi.com/${c.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,D=Xt[c.job]||"#bbb",j=c.job2?Xt[c.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
      ...
      <div style="display:flex;gap:4px;flex-shrink:0">
        ...
        ${L?`<div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${j};display:flex;align-items:center;justify-content:center">
          <span style="font-size:14px;font-weight:900;color:${j};font-family:Arial Black,Arial">${L}</span>
        </div>`:""}
      </div>
      <!-- Nom -->
      <div style="flex:1;min-width:0">
    ...
    </div>`}function u(m){var N,V,K,oe;const c=(N=m.card)==null?void 0:N.player;if(!c)return"";const A=((V=m.card)==null?void 0:V.evolution_bonus)||0,g=zt(c,c.job,A),L=c.job2?zt(c,c.job2,A):0,C=m.status==="sold",q=c.country_code?`https://flagsapi.com/${c.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,D=Xt[c.job]||"#bbb",j=c.job2?Xt[c.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${C?"opacity:0.7":""}">
      ${q?`<img src="${q}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(K=c.clubs)!=null&&K.logo_url?`<img src="${c.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${D};display:flex;align-items:center;justify-content:center">
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const A=r||[];_.innerHTML=A.length>0?A.map((g,L)=>{const C=Gt(g.mmr??1e3),q=(g.ranked_wins||0)+(g.ranked_losses||0)+(g.ranked_draws||0),D=q>0?Math.round((g.ranked_wins||0)/q*100):0,j=g.id===i.profile.id,N=(g.placement_matches||0)<10;return`
