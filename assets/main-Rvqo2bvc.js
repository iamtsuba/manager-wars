...
import{s as v,j as Ei,q as qo,t as Dt,u as Li,F as Ii,o as nt,n as Ht,g as Bt,r as ze,m as ut,v as ni,w as Do,T as Bi,p as wn,x as _n,d as kn}from"./special-cards-YynorolH.js";const $n="/";function go(t,{navigate:e,toast:i}){let o="login";const r=()=>{var a,d,c,l,s,f,y;const n=o==="login";if(t.innerHTML=`
    <div style="
    ...
    </style>`,(a=document.getElementById("tab-login-btn"))==null||a.addEventListener("click",()=>{o="login",r()}),(d=document.getElementById("tab-reg-btn"))==null||d.addEventListener("click",()=>{o="register",r()}),n)(c=document.getElementById("login-password"))==null||c.addEventListener("keydown",x=>{var g;x.key==="Enter"&&((g=document.getElementById("login-btn"))==null||g.click())}),(l=document.getElementById("login-btn"))==null||l.addEventListener("click",async()=>{const x=document.getElementById("login-email").value.trim(),g=document.getElementById("login-password").value,p=document.getElementById("login-error");if(p.textContent="",!x||!g){p.textContent="Remplissez tous les champs.";return}const b=document.getElementById("login-btn");b.textContent="⏳ Connexion…",b.disabled=!0;const{error:u}=await v.auth.signInWithPassword({email:x,password:g});if(b.textContent="⚽ Se connecter",b.disabled=!1,u){p.textContent=u.message.includes("Invalid")?"Email ou mot de passe incorrect.":u.message;return}window.location.reload()});else{(s=document.getElementById("reg-confirm"))==null||s.addEventListener("keydown",p=>{var b;p.key==="Enter"&&((b=document.getElementById("reg-btn"))==null||b.click())});let x=null,g=!1;(f=document.getElementById("reg-access-code"))==null||f.addEventListener("input",p=>{const b=p.target.value,u=document.getElementById("access-code-status"),w=document.getElementById("reg-btn");if(clearTimeout(x),g=!1,w&&(w.disabled=!0,w.style.opacity="0.45",w.style.cursor="not-allowed"),!b){u&&(u.textContent="");return}u&&(u.textContent="⏳ Vérification…",u.style.color="rgba(255,255,255,0.4)"),x=setTimeout(async()=>{const{data:h,error:E}=await v.rpc("check_signup_password",{input_password:b});if(E){u&&(u.textContent="Erreur de vérification.",u.style.color="#f87171");return}g=!!h,u&&(u.textContent=g?"✅ Code valide":"❌ Code incorrect",u.style.color=g?"#4ade80":"#f87171"),w&&(w.disabled=!g,w.style.opacity=g?"1":"0.45",w.style.cursor=g?"pointer":"not-allowed")},400)}),(y=document.getElementById("reg-btn"))==null||y.addEventListener("click",async()=>{var $;const p=document.getElementById("reg-email").value.trim(),b=document.getElementById("reg-password").value,u=document.getElementById("reg-confirm").value,w=(($=document.getElementById("reg-access-code"))==null?void 0:$.value)||"",h=document.getElementById("reg-error");if(h.textContent="",!p||!b||!u){h.textContent="Remplissez tous les champs.";return}if(b.length<6){h.textContent="Mot de passe trop court (min. 6 caractères).";return}if(b!==u){h.textContent="Les mots de passe ne correspondent pas.";return}const{data:E}=await v.rpc("check_signup_password",{input_password:w});if(!E){h.textContent="Code d'accès incorrect.";return}const m=document.getElementById("reg-btn");m.textContent="⏳ Création…",m.disabled=!0;const{error:S}=await v.auth.signUp({email:p,password:b});if(m.textContent="🚀 Créer mon compte",m.disabled=!1,S){h.textContent=S.message;return}i("Compte créé ! Connecte-toi pour commencer.","success",4e3),o="login",r(),setTimeout(()=>{const T=document.getElementById("login-email");T&&(T.value=p)},50)})}};r()}function En(t,{state:e,navigate:i,toast:o,refreshProfile:r}){let n="#1A6B3C",a="#D4A017";t.innerHTML=`
  ...
  `;function d(){var g;const l=document.getElementById("logo-preview"),s=document.getElementById("logo-initials"),f=((g=document.getElementById("setup-club"))==null?void 0:g.value)||"MW",y=f.trim().split(" ").filter(Boolean),x=y.length>=2?(y[0][0]+y[1][0]).toUpperCase():f.slice(0,2).toUpperCase();l&&(l.style.background=a,l.style.borderColor=n),s&&(s.textContent=x,s.style.color=n)}document.getElementById("color1").addEventListener("input",l=>{n=l.target.value,document.getElementById("swatch1").style.background=n,d()}),document.getElementById("color2").addEventListener("input",l=>{a=l.target.value,document.getElementById("swatch2").style.background=a,d()});function c(l){document.querySelectorAll(".setup-step").forEach(s=>s.classList.remove("active")),document.getElementById(`step-${l}`).classList.add("active"),document.getElementById("step-num").textContent=l,document.getElementById("progress-fill").style.width=`${Math.round(l/3*100)}%`,l===3&&d()}document.getElementById("step1-next").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),s=document.getElementById("step1-error");if(s.textContent="",l.length<3){s.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await v.from("users").select("id").eq("pseudo",l).maybeSingle();if(f){s.textContent="Ce pseudo est déjà pris.";return}c(2)}),document.getElementById("step2-back").addEventListener("click",()=>c(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const l=document.getElementById("setup-club").value.trim(),s=document.getElementById("step2-error");if(s.textContent="",l.length<2){s.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await v.from("users").select("id").eq("club_name",l).maybeSingle();if(f){s.textContent="Ce nom de club est déjà pris.";return}c(3)}),document.getElementById("step3-back").addEventListener("click",()=>c(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),s=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),y=document.getElementById("step3-finish");f.textContent="",y.disabled=!0,y.textContent="Création en cours…";try{const{error:x}=await v.from("users").insert({id:e.user.id,pseudo:l,club_name:s,club_color1:n,club_color2:a,credits:15e3});if(x)throw x;await Ln(e.user.id),await r(),o(`Bienvenue ${l} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(x){f.textContent=x.message,y.disabled=!1,y.textContent="🚀 Créer mon profil !"}})}async function Ln(t){const e=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await v.from("users").update({pending_boosters:e,onboarding_done:!1,first_booster_opened:!1}).eq("id",t)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const In="modulepreload",Tn=function(t){return"/"+t},bo={},Fo=function(e,i,o){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),d=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(c=>{if(c=Tn(c),c in bo)return;bo[c]=!0;const l=c.endsWith(".css"),s=l?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${c}"]${s}`))return;const f=document.createElement("link");if(f.rel=l?"stylesheet":In,l||(f.as="script"),f.crossOrigin="",f.href=c,d&&f.setAttribute("nonce",d),document.head.appendChild(f),l)return new Promise((y,x)=>{f.addEventListener("load",y),f.addEventListener("error",()=>x(new Error(`Unable to preload CSS for ${c}`)))})}))}function n(a){const d=new Event("vite:preloadError",{cancelable:!0});if(d.payload=a,window.dispatchEvent(d),!d.defaultPrevented)throw a}return r.then(a=>{for(const d of a||[])d.status==="rejected"&&n(d.reason);return e().catch(n)})},Po="mw_sound_volume";function Ji(){const t=localStorage.getItem(Po);if(t===null)return 100;const e=parseInt(t,10);return Number.isFinite(e)?Math.max(0,Math.min(100,e)):100}function Sn(t){localStorage.setItem(Po,String(Math.max(0,Math.min(100,Math.round(t)))))}function Qi(){return Ji()===0}function yi(t){return Math.max(0,Math.min(1,t*(Ji()/100)))}function Zi(t,e=1){if(Qi())return null;try{const i=new Audio(t);return i.volume=yi(e),i.play().catch(()=>{}),i}catch{return null}}let ht=null,Ni=null,xo=.3;function No(t,e=.3){if(xo=e,ht&&Ni===t&&!ht.paused){ht.volume=yi(xo);return}if(Rt(),!Qi())try{const i=new Audio(t);i.loop=!0,i.volume=yi(e),i.play().catch(()=>{}),ht=i,Ni=t}catch{}}function Rt(){if(ht)try{ht.pause(),ht.currentTime=0}catch{}ht=null,Ni=null}let Qt=null;function Go(t,e=.6){if(Ke(),!Qi())try{const i=new Audio(t);i.volume=yi(e),i.play().catch(()=>{}),Qt=i}catch{}}function Ke(){if(Qt)try{Qt.pause(),Qt.currentTime=0}catch{}Qt=null}const An=[{key:"home2",route:"home2",label:"HOME",icon:"nav-home.png"},{key:"cards",route:"collection",label:"CARDS",icon:"nav-collection.png"},{key:"decks",route:"decks",label:"DECKS",icon:"nav-decks.png"},{key:"boosters",route:"boosters",label:"BOOSTERS",icon:"nav-boosters.png"},{key:"market",route:"market",label:"MERCATO",icon:"nav-market.png"}];function Ro(t,e,i,o){if(!document.getElementById("home2-chrome-style")){const s=document.createElement("style");s.id="home2-chrome-style",s.textContent=`
    ...
    `,document.head.appendChild(s)}const r=An.map(s=>`
    ...
    <div class="news-item">
      ...
      <div class="news-body">
        ...
        <div class="news-time">${zn(G.published_at)}</div>
      </div>
  ...
  <div class="home-dark" id="home-dark">
    ...
    </div>`}).join(""),o.querySelectorAll("[data-resume]").forEach(c=>{c.addEventListener("click",async()=>{mt();const l=document.getElementById("page-content")||document.getElementById("app");if(c.dataset.mini==="1"){const{data:s}=await v.from("mini_league_matches").select("id, league_id").eq("match_id",c.dataset.resume).single();s?i("match-mini-league",{mlMatchId:s.id,leagueId:s.league_id}):i("mini-league")}else{const{resumePvpMatch:s}=await Fo(async()=>{const{resumePvpMatch:f}=await Promise.resolve().then(()=>Zr);return{resumePvpMatch:f}},void 0);s(l,{state:t,navigate:i,toast:e,openModal:null,closeModal:null,refreshProfile:null},c.dataset.resume)}})}),o.querySelectorAll("[data-abandon]").forEach(c=>{c.addEventListener("click",()=>{Pn(async()=>{await Fn(c.dataset.abandon,c.dataset.opp,r),e("Match abandonné (défaite 3-0)","info"),Oo(t,e,i)})})})}async function Fn(t,e,i){Rt();const{data:o}=await v.from("matches").select("home_id, away_id, game_state, mode").eq("id",t).single();if(!o)return;const r=o.home_id===i,n=r?0:3,a=r?3:0,d=o.game_state||{};d.p1Score=n,d.p2Score=a,d.phase="finished",d.forfeit=!0,await v.from("matches").update({status:"finished",forfeit:!0,winner_id:e,home_score:n,away_score:a,game_state:d}).eq("id",t),o.mode==="mini_league"&&await v.from("mini_league_matches").update({status:"finished",home_score:n,away_score:a}).eq("match_id",t)}function Pn(t){const e=document.createElement("div");e.className="modal-overlay",e.innerHTML=`<div class="modal" style="max-width:340px">
  ...
  </div>`,document.getElementById("friend-req-btn").addEventListener("click",()=>Vo(t,e,()=>Ho(t,e)))}async function Uo(t,e){const i=document.createElement("div");i.className="modal-overlay",i.style.zIndex="2000",i.innerHTML=`<div class="modal" style="max-width:420px;border-radius:18px;overflow:hidden">
  ...
  </div>`,document.body.appendChild(i);const o=()=>i.remove();document.getElementById("solo-cancel").addEventListener("click",o),i.addEventListener("click",l=>{l.target===i&&o()});const[{data:r},{data:n}]=await Promise.all([v.from("solo_levels").select("*").eq("is_active",!0).order("level_number"),v.from("user_solo_progress").select("unlocked_level").eq("user_id",e.profile.id).maybeSingle()]),a=(n==null?void 0:n.unlocked_level)||1,d=r||[],c=document.getElementById("solo-levels-list");if(c){if(!d.length){c.innerHTML='<div style="text-align:center;color:#999;padding:20px">Aucun niveau configuré.</div>';return}c.innerHTML=d.map(l=>{const s=l.level_number>a,f=l.level_number===a,y=s?"#f0f0f0":f?"#eefaf2":"#f7f7f7",x=s?"#ddd":f?"#bfe8cf":"#e0e0e0",g=s?"#999":"#12401f";return`
      <div class="solo-level-card" data-level="${l.level_number}" data-locked="${s}"
        style="cursor:${s?"not-allowed":"pointer"};display:flex;align-items:center;gap:14px;padding:14px 16px;border-radius:14px;background:${y};border:1px solid ${x};opacity:${s?.6:1};transition:transform .12s ease">
    ...
    </div>`,await Ko(i,o,e),document.getElementById("btn-add-friend").addEventListener("click",()=>Hn(i,o)),document.getElementById("btn-accept-friend").addEventListener("click",()=>Vo(i,o,null,e))}async function Ko(t,e,i={}){const{navigate:o}=i,r=t.user.id,{data:n,error:a}=await v.from("friendships").select("id, requester_id, addressee_id").eq("status","accepted").or(`requester_id.eq.${r},addressee_id.eq.${r}`),{count:d}=await v.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",r).eq("status","pending"),c=document.getElementById("pending-badge");c&&(d>0?(c.style.display="flex",c.textContent=d):c.style.display="none");const l=document.getElementById("friends-list");if(!l)return;if(a){console.error("[Friends] Erreur:",a),l.innerHTML=`<div style="color:${ai};text-align:center;padding:16px">Erreur chargement : ${a.message}</div>`;return}const s=(n||[]).map(x=>x.requester_id===r?x.addressee_id:x.requester_id);let f={};if(s.length){const{data:x}=await v.from("users").select("id, pseudo, club_name, last_seen_at, club_color1, club_color2").in("id",s);(x||[]).forEach(g=>{f[g.id]=g})}const y=(n||[]).map(x=>({friendshipId:x.id,friend:f[x.requester_id===r?x.addressee_id:x.requester_id]||{pseudo:"?"}}));if(!y.length){l.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${y.map(({friendshipId:x,friend:g})=>On(g,x)).join("")}
    </div>`,l.querySelectorAll("[data-stats]").forEach(x=>{x.addEventListener("click",()=>Un(t,x.dataset.stats,x.dataset.friendName))}),l.querySelectorAll("[data-match]").forEach(x=>{x.addEventListener("click",async()=>{const g=x.dataset.friendId,p=x.dataset.friendName;if(typeof o!="function"){e("Erreur navigation","error");return}const b=t.user.id,{data:u}=await v.from("friend_match_invites").select("id").eq("inviter_id",g).eq("invitee_id",b).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle(),w=!!u;console.log("[Friends] clic match",{fid:g,fname:p,isAccepting:w}),mt(),o("match",{matchMode:"friend",friendId:g,friendName:p,isAccepting:w})})})}function On(t,e){const i=t.club_name||t.pseudo||"?",o=t.pseudo||"",r=(o||i).slice(0,2).toUpperCase(),n=t.club_color2||ri,a=t.club_color1||"#ffffff",d=t.last_seen_at?new Date(t.last_seen_at):null,c=d&&Date.now()-d.getTime()<3*60*1e3;return`
    <div style="display:flex;align-items:center;gap:12px;background:var(--tile-bg);border-radius:12px;padding:12px 14px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
      ...
      <div style="display:flex;gap:8px;flex-shrink:0">
        ...
        <button data-stats="${t.id}" data-friend-name="${i}" title="Voir les stats"
          style="width:38px;height:38px;border-radius:50%;border:2px solid ${Gn};background:var(--tile-bg);font-size:16px;cursor:pointer;display:flex;align-items:center;justify-content:center">📊</button>
      </div>
    </div>`}function Hn(t,e){const i=eo();i.innerHTML=`
    <div class="popup-box">
    ...
    <div class="popup-box" style="max-width:380px">
      ...
      ${u===0?`
        ...
        ${w("Défaites",f,s)}
        ${w("Buts marqués",x,g)}
        ${w("Buts encaissés",g,x,ai,ri)}
        ${w("GC utilisés ⚡",p,b,"#7a28b8","#7a28b8")}
    ...
    </style>`}async function Kn(t,{state:e,navigate:i,toast:o}){const r=e.profile;if(!r)return;const n="/icons/";Ro(i,r,"game",n),t.innerHTML=`
  <style>
    .game-page {
      min-height: 100%;
      background: var(--page-bg);
  ...
  </div>`,t.querySelectorAll("[data-action]").forEach(a=>{a.addEventListener("click",()=>{a.style.transform="scale(.96)",setTimeout(()=>a.style.transform="",180);const d=a.dataset.action;if(d==="match-ai"){Uo(i,e);return}if(d==="match-random"){mt(),i("match",{matchMode:"random"});return}if(d==="match-friend"){i("friends");return}if(d==="mini-league"){i("mini-league");return}if(d==="ranked"){i("ranked");return}o("Bientôt disponible","info")})})}async function Yo(t,e){var p,b;const{state:i,navigate:o}=e,r=oi(),n=Ji(),a=(p=i==null?void 0:i.profile)==null?void 0:p.is_admin,d="2026.07.29-1012";t.innerHTML=`
  ...
  </div>`,t.querySelectorAll("[data-theme-choice]").forEach(u=>{u.addEventListener("click",()=>{jo(u.dataset.themeChoice),Yo(t,e)})});const c=t.querySelector("#team-color1"),l=t.querySelector("#team-color2"),s=t.querySelector("#team-preview"),f=()=>{s&&(s.style.background=`linear-gradient(135deg, ${c.value} 50%, ${l.value} 50%)`)};f(),c==null||c.addEventListener("input",f),l==null||l.addEventListener("input",f),(b=t.querySelector("#team-save"))==null||b.addEventListener("click",async()=>{const u=t.querySelector("#team-error"),w=t.querySelector("#team-save"),h=t.querySelector("#team-club-name").value.trim();if(h.length<2){u.textContent="Nom de club trop court.";return}w.disabled=!0,w.textContent="⏳ Enregistrement...",u.textContent="";const{error:E}=await v.from("users").update({club_name:h,club_color1:c.value,club_color2:l.value}).eq("id",i.profile.id);if(w.disabled=!1,w.textContent="💾 Enregistrer",E){u.textContent=E.message;return}i.profile.club_name=h,i.profile.club_color1=c.value,i.profile.club_color2=l.value,u.style.color="#2ecc71",u.textContent="✅ Modifications enregistrées.",oi()==="club"&&jo("club")});const y=t.querySelector("#volume-slider"),x=t.querySelector("#volume-label");let g=null;y.addEventListener("input",()=>{Sn(Number(y.value)),x.textContent=`${y.value}%`,g&&(g.volume=Math.max(0,Math.min(1,Number(y.value)/100)))}),t.querySelector("#volume-test").addEventListener("click",()=>{g=Zi("/sounds/match-opening.mp3",1)}),t.querySelector("#settings-logout").addEventListener("click",async()=>{await v.auth.signOut(),window.location.reload()})}const Oe={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé pour ce match."},"Double attaque":{icon:"⚡",desc:"La note d'attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la note d'un joueur adverse."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function Pe(t,e){if(!t)return 0;let i=0;switch(e){case"GK":i=Number(t.note_g)||0;break;case"DEF":i=Number(t.note_d)||0;break;case"MIL":i=Number(t.note_m)||0;break;case"ATT":i=Number(t.note_a)||0;break;default:i=0}return i+(t.boost||0)}const ho=["ATT","MIL","DEF","GK"];function Wo(t,e,i){if(!t||!e)return!1;if(i&&t.position&&e.position)return Li(i).some(([s,f])=>s===t.position&&f===e.position||s===e.position&&f===t.position);const o=t._col!=null&&e._col!=null&&t._col===e._col,r=t._col!=null&&e._col!=null&&Math.abs(t._col-e._col)===1,n=ho.indexOf(t._line||t.job),a=ho.indexOf(e._line||e.job),d=Math.abs(n-a)===1;return(t._line||t.job)===(e._line||e.job)&&r||o&&d}function Xo(t,e){let i=0;const o=t.length;for(let r=0;r<o;r++)for(let n=r+1;n<o;n++){const a=t[r],d=t[n];if(!a||!d||!Wo(a,d,e))continue;const c=Dt(a,d);c==="#00ff88"?i+=10:c==="#FFD700"&&(i+=5)}return i}function si(t,e={},i){let o=0,r=0;t.forEach(c=>{const l=c._line||c.job,s=Number(l==="MIL"?c.note_m:c.note_a)||0,f=c.stadiumBonus&&(l==="MIL"||l==="ATT")?10:0;o+=e.doubleAttack?s*2:s,r+=(c.boost||0)+f});const n=o+r,a=Xo(t,i);let d=n+a;return e.stolenNote&&(d-=e.stolenNote),{base:n,links:a,total:Math.max(0,d)}}function li(t,e={},i){const o=t.reduce((a,d)=>{const c=d._line||d.job;let l=0;c==="GK"?l=Number(d.note_g)||0:c==="MIL"?l=Number(d.note_m)||0:l=Number(d.note_d)||0;const s=d.stadiumBonus&&(c==="GK"||c==="DEF"||c==="MIL")?10:0;return a+l+(d.boost||0)+s},0),r=Xo(t,i);let n=o+r;return e.stolenNote&&(n-=e.stolenNote),{base:o,links:r,total:Math.max(0,n)}}function io(t,e,i={}){return i.shield?{goal:!1,shielded:!0}:{goal:t>e,shielded:!1}}function Jo(t,e,i="easy"){const o=t.filter(a=>!a.used);if(!o.length)return[];const r=[...o].sort((a,d)=>{const c=e==="attack"?Pe(a,"ATT"):a._line==="GK"?Pe(a,"GK"):Pe(a,"DEF");return(e==="attack"?Pe(d,"ATT"):d._line==="GK"?Pe(d,"GK"):Pe(d,"DEF"))-c});let n=i==="easy"?1+Math.floor(Math.random()*2):i==="medium"?2+Math.floor(Math.random()*2):3;return r.slice(0,Math.min(n,r.length,3))}function Vn(t,e){const i={vs_ai_easy:{victoire:500,nul:250,defaite:50},vs_ai_medium:{victoire:1e3,nul:500,defaite:50},vs_ai_hard:{victoire:1500,nul:750,defaite:100},vs_ai_club:{victoire:2500,nul:1250,defaite:100}};return(i[t]||i.vs_ai_easy)[e]||0}const Yn={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},Jt={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},pi=["GK","DEF","MIL","ATT"],Wn=["Tous","GK","DEF","MIL","ATT"],Xn={normal:1e3,pepite:5e3,papyte:5e3,legende:1e4};function vo(t){const e=t.player;return e?(e.rarity,Math.max(Number(e.note_g)||0,Number(e.note_d)||0,Number(e.note_m)||0,Number(e.note_a)||0)):null}function Jn(t){return t.length?t.reduce((e,i)=>vo(i)>vo(e)?i:e,t[0]):t[0]}const Qn={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function ui(t,e){return t&&Number(e==="GK"?t.note_g:e==="DEF"?t.note_d:e==="MIL"?t.note_m:t.note_a)||0}function Di(t,e=""){const i=t.player;if(!i)return"";const o=t.evolution_bonus||0,r={...i,_evolution_bonus:o},n=e?`<div style="position:absolute;top:6px;right:6px;z-index:10;background:#0a3d1e;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 7px">${e}</div>`:"";return`<div style="position:relative;display:inline-block;cursor:pointer" data-card-id="${t.id}">
    ${n}
    ${ze(r,{width:140})}
  </div>`}function wo(t){return`<div style="display:inline-block;filter:grayscale(1);opacity:0.4">
    ${ze(t,{width:140})}
  </div>`}async function Zn(t,e){const{state:i,navigate:o,toast:r,openModal:n,closeModal:a}=e;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await v.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price, stadium_id, evolution_bonus,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),l=(d||[]).filter(C=>C.card_type==="player"&&C.player),s=(d||[]).filter(C=>C.card_type==="game_changer"),f=(d||[]).filter(C=>C.card_type==="formation"),y=(d||[]).filter(C=>C.card_type==="stadium"),{data:x}=await v.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(x||[]).forEach(C=>{g[C.name]=C});const{data:p}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),b={};(p||[]).forEach(C=>{b[C.id]=C}),y.forEach(C=>{C.stadium_def&&(b[C.stadium_id]=C.stadium_def)});const u=Object.keys(Ii),w=Object.keys(Oe),h={};l.forEach(C=>{const K=C.player.id;h[K]=(h[K]||0)+1}),new Set(Object.keys(h).map(C=>String(C)));const E=new Set(f.map(C=>C.formation)),m=new Set(s.map(C=>C.gc_type));let S="player",$="Tous",T="",P=!1;function G(){return[...l].sort((C,K)=>{const Q=pi.indexOf(C.player.job),X=pi.indexOf(K.player.job);return Q!==X?Q-X:(C.player.surname_real||"").localeCompare(K.player.surname_real||"")})}function O(){return[...c||[]].sort((C,K)=>{const Q=pi.indexOf(C.job),X=pi.indexOf(K.job);return Q!==X?Q-X:(C.surname_real||"").localeCompare(K.surname_real||"")})}function V(){return G().filter(C=>{const K=C.player,Q=$==="Tous"||K.job===$,X=!T||`${K.firstname} ${K.surname_real}`.toLowerCase().includes(T);return Q&&X})}function I(){return O().filter(C=>{const K=$==="Tous"||C.job===$,Q=!T||`${C.firstname} ${C.surname_real}`.toLowerCase().includes(T);return K&&Q})}t.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(g=document.getElementById("direct-sell-form-btn"))==null||g.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${l} pour ${fi.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const u=y.find(h=>!h.is_for_sale)||y[0];if(!u){n("Aucune carte à vendre","error");return}await v.from("market_listings").delete().eq("card_id",u.id),await v.from("transfer_history").delete().eq("card_id",u.id);const{error:w}=await v.from("cards").delete().eq("id",u.id);if(w){n(w.message,"error");return}await v.from("users").update({credits:(r.profile.credits||0)+fi}).eq("id",r.profile.id),await c(),n(`+${fi.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),d("collection")}),(p=document.getElementById("market-sell-form-btn"))==null||p.addEventListener("click",async()=>{const u=parseInt(document.getElementById("sell-price-form").value);if(!u||u<1){n("Prix invalide","error");return}await v.from("cards").update({is_for_sale:!0,sale_price:u}).eq("id",t.id),await v.from("market_listings").insert({seller_id:r.profile.id,card_id:t.id,price:u}),n("Carte mise en vente sur le marché !","success"),a(),d("collection")}),(b=document.getElementById("cancel-sell-form-btn"))==null||b.addEventListener("click",async()=>{await v.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",t.id),await v.from("market_listings").update({status:"cancelled"}).eq("card_id",t.id).eq("status","active"),n("Annonce retirée","success"),a(),d("collection")})}async function _o(t,e,i,o){var pe,me,fe,C,K,Q,X,re,F;const{state:r,toast:n,openModal:a,closeModal:d,navigate:c,refreshProfile:l}=o,s=t.player,f=e.filter(z=>z.player.id===s.id),y=f.length,x=t.evolution_bonus||0,g=Math.max((Number(s.note_g)||0)+(s.job==="GK"||s.job2==="GK"?x:0),(Number(s.note_d)||0)+(s.job==="DEF"||s.job2==="DEF"?x:0),(Number(s.note_m)||0)+(s.job==="MIL"||s.job2==="MIL"?x:0),(Number(s.note_a)||0)+(s.job==="ATT"||s.job2==="ATT"?x:0)),p=s.rarity||"normal",{data:b}=await v.from("sell_price_configs").select("*").eq("rarity",p).lte("note_min",g).gte("note_max",g).order("note_min",{ascending:!1}).limit(1);((pe=b==null?void 0:b[0])==null?void 0:pe.price)??Xn[p];const u=((me=b==null?void 0:b[0])==null?void 0:me.price_min)??null,w=((fe=b==null?void 0:b[0])==null?void 0:fe.price_max)??null,h=s.rarity!=="legende";Bt(s);const E=ui(s,s.job)+x,m=s.job2?ui(s,s.job2)+(ui(s,s.job2)>0?x:0):null;Jt[s.job],s.job2&&Jt[s.job2];const S=Yn[s.rarity]||"#ccc",$=Qn[s.country_code]||s.country_code||"",T=t.evolution_bonus||0,G=E+T,O=m||0,V=O>0?O+T:0,I=f.map(z=>z.id);let _={};if(I.length){const{data:z}=await v.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",I).order("transferred_at",{ascending:!0});(z||[]).forEach(Y=>{_[Y.card_id]||(_[Y.card_id]=[]),_[Y.card_id].push(Y)})}const R=I.length?`
    <div style="margin-top:16px;border-top:1px solid var(--tile-border);padding-top:14px">
      ...
      <!-- Carte visuelle -->
      ${ze({...s,_evolution_bonus:x},{width:160})}
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(K=document.getElementById("close-detail"))==null||K.addEventListener("click",d);let W=new Set;const de=s.rarity==="pepite"?1.3:s.rarity==="papyte"?.7:1;function ce(){let z=0;return document.querySelectorAll(".expl-check:checked").forEach(Y=>{if(Y.dataset.id===t.id)return;const se=Number(Y.dataset.evo)||0,ge=Number(Y.dataset.note)||0;z+=ge+se}),Math.round(z*de)}const j=()=>{const z=W.size,Y=document.getElementById("sell-action-panel");if(!Y)return;Y.style.display=z>0?"block":"none",document.getElementById("sell-selected-count").textContent=z;const Z=document.getElementById("evolve-btn");if(Z){const se=ce();Z.textContent=se>0?`⬆️ Évoluer (+${se})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach(z=>{z.addEventListener("change",()=>{const Y=z.dataset.id;z.checked?W.add(Y):W.delete(Y);const Z=z.closest(".exemplaire-row");if(Z){const se=Z.querySelector(".expl-sel-overlay"),ge=Z.querySelector(".expl-sel-check");se&&(se.style.display=z.checked?"block":"none"),ge&&(ge.style.display=z.checked?"flex":"none"),Z.style.transform=z.checked?"scale(1.05)":"scale(1)"}j()})}),document.querySelectorAll(".exemplaire-row").forEach(z=>{z.addEventListener("click",Y=>{if(Y.target.tagName==="INPUT")return;const Z=z.querySelector(".expl-check");Z&&!Z.disabled&&(Z.checked=!Z.checked,Z.dispatchEvent(new Event("change")))})}),(Q=document.getElementById("evolve-btn"))==null||Q.addEventListener("click",async()=>{if(y<=1)return;const z=[...W];if(!z.length)return;if(W.has(t.id)){const xe=document.createElement("div");xe.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",xe.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    <div class="page-body">
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const c=await ko("Nom du deck",`Deck ${((n==null?void 0:n.length)||0)+1}`);if(!c)return;const{data:l,error:s}=await v.from("decks").insert({owner_id:i.profile.id,name:c}).select().single();if(s){r(s.message,"error");return}r("Deck créé !","success"),$o(l.id,t,e)}),t.querySelectorAll("[data-open-deck]").forEach(c=>{c.addEventListener("click",()=>$o(c.dataset.openDeck,t,e))}),t.querySelectorAll("[data-rename]").forEach(c=>{c.addEventListener("click",async()=>{const l=await ko("Nouveau nom",c.dataset.name);if(!l||l===c.dataset.name)return;const{error:s}=await v.from("decks").update({name:l}).eq("id",c.dataset.rename);if(s){r(s.message,"error");return}r("Deck renommé !","success"),Gi(t,e)})}),t.querySelectorAll("[data-delete]").forEach(c=>{c.addEventListener("click",async()=>{if(!await ir(`Supprimer le deck "${c.dataset.name}" ? Cette action est irréversible.`,!0))return;await v.from("deck_cards").delete().eq("deck_id",c.dataset.delete);const{error:l}=await v.from("decks").delete().eq("id",c.dataset.delete);if(l){r(l.message,"error");return}r("Deck supprimé.","success"),Gi(t,e)})})}async function $o(t,e,i){const{state:o,toast:r}=i;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await v.from("decks").select("*").eq("id",t).single(),{data:a}=await v.from("cards").select(`id, card_type, formation, stadium_id, evolution_bonus,
      player:players(id, firstname, surname_real, country_code, club_id, job, job2,
      ...
      stadium_def:stadium_definitions(id, name, club_id, country_code, image_url,
        club:clubs(encoded_name, logo_url))`).eq("owner_id",o.profile.id).eq("card_type","stadium"),c=(a||[]).filter(u=>u.card_type==="player"&&u.player),l=(a||[]).filter(u=>u.card_type==="formation"),s=(d||[]).filter(u=>u.card_type==="stadium"),f=[...new Set(s.map(u=>u.stadium_id).filter(Boolean))];let y={};if(s.forEach(u=>{u.stadium_def&&u.stadium_id&&(y[u.stadium_id]=u.stadium_def)}),f.length&&Object.keys(y).length<f.length){const{data:u}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",f);(u||[]).forEach(w=>{y[w.id]=w})}const x=l.map(u=>u.formation).filter(Boolean),{data:g}=await v.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",t);let p=n.formation||"4-4-2";x.length>0&&!x.includes(p)&&(p=x[0]);const b={deckId:t,name:n.name,formation:p,formationCardId:n.formation_card_id,stadiumCardId:n.stadium_card_id||null,slots:{},subs:[],playerCards:c,formationCards:l,stadiumCards:s,stadDefMap:y,availableFormations:x};(g||[]).forEach(u=>{u.is_starter?b.slots[u.position]=u.card_id:b.subs.includes(u.card_id)||b.subs.push(u.card_id)}),$t(e,b,i,!0)}function $t(t,e,i,o=!1){var p,b;const{navigate:r}=i;o||sr(e,i),hi[e.formation];const n=Eo(e.formation),a=n.filter(u=>e.slots[u]).length,d=e.availableFormations.length>0?e.availableFormations:Object.keys(hi),c=(p=e.stadiumCards)==null?void 0:p.find(u=>u.id===e.stadiumCardId),l=c&&((b=e.stadDefMap)==null?void 0:b[c.stadium_id])||null,s=e.subs.map(u=>e.playerCards.find(w=>w.id===u)).filter(Boolean);s.length!==e.subs.length&&(e.subs=s.map(u=>u.id)),[...Object.values(e.slots),...e.subs],t.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;const f=window.innerWidth>=900,y=t.querySelector(".deck-pc-layout"),x=t.querySelector(".deck-mobile-layout");y&&(y.style.display=f?"block":"none"),x&&(x.style.display=f?"none":"block"),t.querySelectorAll("#save-deck").forEach(u=>{u.closest(".page-body").style.display=f?"none":"block"}),or(t,e,n,i),t.querySelectorAll("#builder-back").forEach(u=>u.addEventListener("click",()=>r("decks")));const g=()=>{const{openModal:u,closeModal:w}=i,E=`<div style="display:flex;flex-wrap:wrap;gap:12px;padding:8px;justify-content:center">
    ...
    </div>`;u("⚽ Choisir une formation",E),document.querySelectorAll("#modal-body [data-forma]").forEach(m=>{m.addEventListener("click",()=>{e.formation=m.dataset.forma;const S=Eo(e.formation),$={};S.forEach(T=>{e.slots[T]&&($[T]=e.slots[T])}),e.slots=$,w(),$t(t,e,i)})})};t.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",g)),t.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>nr(e,t,i))),t.querySelectorAll("#save-deck, #save-deck-pc").forEach(u=>u.addEventListener("click",()=>lr(e,i))),t.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{e.subs=e.subs.filter(w=>w!==u.dataset.removeSub),$t(t,e,i)})}),t.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>ar(e,t,i)))}function or(t,e,i,o){var h,E,m;const r=window.innerWidth>=900,n=t.querySelector(r?"#deck-field-pc":"#deck-field-mobile");if(!n)return;const a=(h=e.stadiumCards)==null?void 0:h.find(S=>S.id===e.stadiumCardId),d=a&&((E=e.stadDefMap)==null?void 0:E[a.stadium_id])||null,c=ut[e.formation]||{},l=Li(e.formation)||[],s={};for(const S of i){const $=e.slots[S],T=$?e.playerCards.find(P=>P.id===$):null;T!=null&&T.player?s[S]={...T.player,_evolution_bonus:T.evolution_bonus||0,face:T.player.face||null}:s[S]=null}const f=window.innerWidth>=900,y=f?window.innerWidth-280:window.innerWidth-20,x=f?Math.min(y,860):y,g=Math.round(f?x*.82:x*.85),p=f?84:44;let b="";for(const[S,$]of l){const T=c[S],P=c[$];if(!T||!P)continue;const G=T.x*x,O=Math.round(.03*g+T.y*.85*g),V=P.x*x,I=Math.round(.03*g+P.y*.85*g),_=s[S],R=s[$],W=Dt(_,R);W==="#ff3333"||W==="#cc2222"?b+=`<line x1="${G.toFixed(1)}" y1="${O.toFixed(1)}" x2="${V.toFixed(1)}" y2="${I.toFixed(1)}" stroke="${W}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(b+=`<line x1="${G.toFixed(1)}" y1="${O.toFixed(1)}" x2="${V.toFixed(1)}" y2="${I.toFixed(1)}" stroke="${W}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,b+=`<line x1="${G.toFixed(1)}" y1="${O.toFixed(1)}" x2="${V.toFixed(1)}" y2="${I.toFixed(1)}" stroke="${W}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let u="";const w=Math.round(p*657/507);for(const S of i){const $=c[S];if(!$)continue;const T=s[S],P=$.x*x;$.y*g;const G=Math.round(.03*g+$.y*(.85*g)),O=Math.round(P-p/2),V=Math.round(G-w/2);if(T){const I=S.replace(/\d+/,""),_=d&&(d.club_id&&String(T.club_id)===String(d.club_id)||d.country_code&&T.country_code===d.country_code),R=ze({...T,_evolution_bonus:T._evolution_bonus||0},{width:p,showStad:!0,stadDef:d,role:I});_&&((m=d.club)!=null&&m.logo_url||d.image_url),u+=`<div style="position:absolute;left:${O}px;top:${V}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${S}">
    ...
    <div style="position:relative;width:${x}px;height:${g}px;margin:0 auto">
      ...
      ${u}
    </div>`,n.querySelectorAll(".deck-slot-hit").forEach(S=>{S.addEventListener("click",()=>rr(S.dataset.pos,e,t,o))})}function nr(t,e,i){var d;const{openModal:o,closeModal:r}=i,n=new Set,a=(t.stadiumCards||[]).filter(c=>{const l=c.stadium_id||c.id;return n.has(l)?!1:(n.add(l),!0)});o("🏟️ Choisir un stade",`<div style="display:flex;flex-wrap:wrap;gap:10px;padding:8px">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(u=document.getElementById("close-selector"))==null||u.addEventListener("click",n),(w=document.getElementById("remove-player"))==null||w.addEventListener("click",()=>{delete e.slots[t],n(),$t(i,e,o)}),document.querySelectorAll(".player-option").forEach(h=>{h.addEventListener("click",()=>{e.slots[t]=h.dataset.cardId,n(),$t(i,e,o)})})}function ar(t,e,i){var s,f,y;const{openModal:o,closeModal:r}=i,n=(s=t.stadiumCards)==null?void 0:s.find(x=>x.id===t.stadiumCardId),a=n&&((f=t.stadDefMap)==null?void 0:f[n.stadium_id])||null,d=new Set;Object.keys(t.slots).forEach(x=>{var b;const g=t.slots[x];if(!g)return;const p=t.playerCards.find(u=>u.id===g);(b=p==null?void 0:p.player)!=null&&b.id&&d.add(p.player.id)}),t.subs.forEach(x=>{var p;const g=t.playerCards.find(b=>b.id===x);(p=g==null?void 0:g.player)!=null&&p.id&&d.add(g.player.id)});const c=new Set,l=t.playerCards.filter(x=>{var g,p,b;return d.has((g=x.player)==null?void 0:g.id)||c.has((p=x.player)==null?void 0:p.id)?!1:(c.add((b=x.player)==null?void 0:b.id),!0)});o("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-sub-selector">Fermer</button>'),(y=document.getElementById("close-sub-selector"))==null||y.addEventListener("click",r),document.querySelectorAll(".player-option").forEach(x=>{x.addEventListener("click",()=>{t.subs.push(x.dataset.cardId),r(),$t(e,t,i)})})}async function Qo(t){const e=t.formationCards.find(r=>r.formation===t.formation),i=(e==null?void 0:e.id)||t.formationCardId;await v.from("decks").update({formation:t.formation,formation_card_id:i||null,stadium_card_id:t.stadiumCardId||null}).eq("id",t.deckId),await v.from("deck_cards").delete().eq("deck_id",t.deckId);const o=[];return Object.entries(t.slots).forEach(([r,n],a)=>{o.push({deck_id:t.deckId,card_id:n,position:r,is_starter:!0,slot_order:a})}),t.subs.forEach((r,n)=>{o.push({deck_id:t.deckId,card_id:r,position:`SUB${n+1}`,is_starter:!1,slot_order:100+n})}),o.length>0?await v.from("deck_cards").insert(o):{error:null}}let Ri=null;function sr(t,e){const{toast:i}=e;clearTimeout(Ri),document.querySelectorAll(".autosave-indicator").forEach(r=>{r.textContent="⏳ Enregistrement...",r.style.opacity="1",r.style.color=""}),Ri=setTimeout(async()=>{const{error:r}=await Qo(t),n=document.querySelectorAll(".autosave-indicator");if(r){n.forEach(a=>{a.textContent="⚠️ Erreur d'enregistrement",a.style.color="#ff6b6b"}),i(r.message,"error");return}n.forEach(a=>{a.textContent="✅ Enregistré",a.style.color="",setTimeout(()=>{a.style.opacity="0"},1500)})},600)}async function lr(t,e){const{toast:i,navigate:o}=e;clearTimeout(Ri);const{error:r}=await Qo(t);if(r){i(r.message,"error");return}i("Deck enregistré ✅","success"),o("decks")}function Eo(t){const e=hi[t]||hi["4-4-2"],i=["GK1"];for(let o=1;o<=e.DEF;o++)i.push(`DEF${o}`);for(let o=1;o<=e.MIL;o++)i.push(`MIL${o}`);for(let o=1;o<=e.ATT;o++)i.push(`ATT${o}`);return i}async function Zo(t=null){const e=new Date().toISOString().slice(0,10),{data:i}=await v.from("booster_configs").select("*").eq("is_active",!0).order("sort_order");if(!(i!=null&&i.length))return[];const o=i.filter(a=>!(a.available_from&&e<a.available_from||a.available_until&&e>a.available_until));if(!o.length)return[];let r=o;if(t){const a=o.filter(d=>d.max_per_user!=null);if(a.length){const{data:d}=await v.from("booster_claims").select("booster_id").eq("user_id",t).in("booster_id",a.map(l=>l.id)),c={};(d||[]).forEach(l=>{c[l.booster_id]=(c[l.booster_id]||0)+1}),r=o.filter(l=>l.max_per_user==null?!0:(c[l.id]||0)<l.max_per_user)}}if(!r.length)return[];const{data:n}=await v.from("booster_drop_rates").select("*").in("booster_id",r.map(a=>a.id)).order("sort_order");return r.map(a=>({...a,rates:(n||[]).filter(d=>d.booster_id===a.id)}))}async function dr(t,e){const{data:i}=await v.from("booster_configs").select("max_per_user").eq("id",e).single();i!=null&&i.max_per_user&&await v.from("booster_claims").insert({user_id:t,booster_id:e})}function cr(t){if(!(t!=null&&t.length))return null;const e=t.reduce((o,r)=>o+Number(r.percentage),0);let i=Math.random()*e;for(const o of t)if(i-=Number(o.percentage),i<=0)return o;return t[t.length-1]}const en=()=>Object.keys(ut),pr=[{id:"players_std",img:"/icons/booster-players.png",name:"Players",sub:"5 cartes joueurs",cost:5e3,costLabel:"5 000 crédits",cardCount:5,type:"player"},{id:"players_pub",img:"/icons/booster-silver.png",name:"Players (pub)",sub:"3 cartes joueurs",cost:0,costLabel:"1 pub",cardCount:3,type:"player"},{id:"game_changer",img:"/icons/booster-gamechanger.png",name:"Game Changer",sub:"3 cartes spéciales",cost:1e4,costLabel:"10 000 crédits",cardCount:3,type:"game_changer"},{id:"formation",img:"/icons/booster-formation.png",name:"Formation",sub:"1 carte formation",cost:1e4,costLabel:"10 000 crédits",cardCount:1,type:"formation"}],Oi={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé."},"Double attaque":{icon:"⚡",desc:"La prochaine attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la prochaine action IA."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function ur(t){const e=t.player;if(!e)return"";const i=t.evolution_bonus||0;return ze({...e,_evolution_bonus:i},{width:140})}function tn(t){var r;const e={};(t.rates||[]).forEach(n=>{e[n.card_type]=(e[n.card_type]||0)+Number(n.percentage||0)});const i=((r=Object.entries(e).sort((n,a)=>a[1]-n[1])[0])==null?void 0:r[0])||"player",o=t.image_url||"booster-players.png";return{id:t.id,img:"/icons/"+o,name:t.name,sub:`${t.card_count} carte(s)`,cost:t.price_type==="credits"&&t.price_credits||0,costLabel:t.price_type==="credits"?`${(t.price_credits||0).toLocaleString("fr")} crédits`:t.price_type==="pub"?"1 pub":"Gratuit",cardCount:t.card_count||5,type:i,isPub:t.price_type==="pub",rates:t.rates||[],allow_duplicates:t.allow_duplicates!==!1,_boosterId:t.id,_raw:t}}async function fr(t,{state:e,navigate:i,toast:o}){var f,y,x;const r=((f=e.profile)==null?void 0:f.credits)||0;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⏳ Chargement...</div>';let n=[];try{n=(await Zo((y=e.user)==null?void 0:y.id)).map(tn)}catch(g){console.warn("Erreur chargement boosters DB, fallback hardcodé",g)}n.length||(n=pr.map(g=>({...g,rates:[],isPub:g.id==="players_pub"})));const a=await v.from("cards").select("player_id, card_type, formation, stadium_id, gc_type").eq("owner_id",e.profile.id).then(g=>g.data||[]),d=new Set(a.filter(g=>g.card_type==="stadium").map(g=>g.stadium_id)),c=new Set(a.filter(g=>g.card_type==="formation").map(g=>g.formation)),l=new Set(a.filter(g=>g.card_type==="game_changer").map(g=>g.gc_type)),s={};for(const g of n){if(g.allow_duplicates!==!1||!((x=g.rates)!=null&&x.length))continue;const p=[...new Set((g.rates||[]).map(u=>u.card_type))];let b=!1;for(const u of p)if(u==="stadium"){const{data:w}=await v.from("stadium_definitions").select("id");if((w||[]).some(h=>!d.has(h.id))){b=!0;break}}else if(u==="game_changer"){const{data:w}=await v.from("gc_definitions").select("name").eq("is_active",!0);if((w||[]).some(h=>!l.has(h.name))){b=!0;break}}else if(u==="formation"){const{FORMATION_LINKS:w}=await Fo(async()=>{const{FORMATION_LINKS:h}=await import("./special-cards-YynorolH.js").then(E=>E.y);return{FORMATION_LINKS:h}},__vite__mapDeps([0,1]));if(Object.keys(w).some(h=>!c.has(h))){b=!0;break}}else{b=!0;break}b||(s[g.id]=!0)}t.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📦 Boosters</h2>
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${n.map(g=>{const p=g.cost===0||r>=g.cost,b=s[g.id]===!0;return`<div class="booster-card ${!p||b?"disabled":""}" data-booster="${g.id}" style="position:relative">
  ...
  </div>`,t.querySelectorAll(".booster-card:not(.disabled)").forEach(g=>{g.addEventListener("click",async()=>{const p=n.find(b=>b.id===g.dataset.booster);if(p){g.style.opacity="0.5",g.style.pointerEvents="none";try{await mr(p,{state:e,toast:o,navigate:i,container:t})}catch(b){o(b.message,"error"),g.style.opacity="",g.style.pointerEvents=""}}})}),t.querySelectorAll(".booster-info-btn").forEach(g=>{g.addEventListener("click",p=>{p.stopPropagation();const b=n.find(u=>u.id===g.dataset.boosterId);yr(b)})})}async function mr(t,{state:e,toast:i,navigate:o,container:r}){var f,y;if(t.cost>0&&e.profile.credits<t.cost){i("Crédits insuffisants","error");return}t.isPub&&await vr();const{data:n}=await v.from("cards").select("card_type, player_id, formation").eq("owner_id",e.profile.id),a=new Set((n||[]).filter(x=>x.card_type==="player").map(x=>x.player_id)),d=new Set((n||[]).filter(x=>x.card_type==="formation").map(x=>x.formation));let c=[],l=null;try{if((f=t.rates)!=null&&f.length)c=await Hi(e.profile,t);else{const x=t.type||"player";x==="player"?c=await on(e.profile,t.cardCount,t.cost):x==="game_changer"?c=await nn(e.profile,t.cardCount,t.cost):x==="formation"?c=await rn(e.profile,t.cost):c=await Hi(e.profile,t)}}catch(x){l=x.message||String(x),console.error("[Booster] Erreur:",x)}if(c!=null&&c.length&&t._boosterId&&await dr(e.user.id,t._boosterId),!(c!=null&&c.length)){const x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",x.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
      <div style="font-size:12px;color:rgba(255,255,255,0.6);max-width:320px;word-break:break-all;background:rgba(255,255,255,0.05);padding:10px;border-radius:8px;margin-top:6px">
        ${l||"Vérifie la console (F12) pour plus de détails"}
      </div>
      <button style="margin-top:10px;padding:12px 28px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer" id="anim-close-err">Fermer</button>`,document.body.appendChild(x),(y=x.querySelector("#anim-close-err"))==null||y.addEventListener("click",()=>x.remove());return}c.forEach(x=>{x.card_type==="player"&&x.player?x.isDuplicate=a.has(x.player.id):x.card_type==="formation"&&(x.isDuplicate=d.has(x.formation))});const{data:s}=await v.from("users").select("*").eq("id",e.profile.id).single();s&&(e.profile=s),an(c,t,o)}function gr(){const t=Math.random()*100;return t<.5?"legende":t<2?"special":t<10?"normal_high":"normal_low"}function yt(t){return Math.max(Number(t.note_g)||0,Number(t.note_d)||0,Number(t.note_m)||0,Number(t.note_a)||0)}function br(t,e){let i;switch(e){case"legende":i=t.filter(o=>o.rarity==="legende"),i.length||(i=t.filter(o=>o.rarity==="pepite"||o.rarity==="papyte")),i.length||(i=t.filter(o=>yt(o)>=6));break;case"special":i=t.filter(o=>o.rarity==="pepite"||o.rarity==="papyte"),i.length||(i=t.filter(o=>yt(o)>=6));break;case"normal_high":i=t.filter(o=>o.rarity==="normal"&&yt(o)>=6),i.length||(i=t.filter(o=>yt(o)>=6));break;default:i=t.filter(o=>o.rarity==="normal"&&yt(o)>=1&&yt(o)<=5),i.length||(i=t.filter(o=>o.rarity==="normal"));break}return i.length||(i=t),i[Math.floor(Math.random()*i.length)]}async function Hi(t,e){if(e.cost>0){const{error:y}=await v.from("users").update({credits:t.credits-e.cost}).eq("id",t.id);if(y)throw y;t.credits-=e.cost,Ti(t.credits)}const i=e.allow_duplicates!==!1;let o=[];const{data:r,error:n}=await v.from("cards").select("player_id, card_type, formation, stadium_id, gc_type").eq("owner_id",t.id);if(n){const{data:y}=await v.from("cards").select("player_id, card_type, formation, gc_type").eq("owner_id",t.id);o=y||[]}else o=r||[];const a=new Set(o.filter(y=>y.card_type==="player").map(y=>y.player_id)),d=new Set(o.filter(y=>y.card_type==="formation").map(y=>y.formation)),c=new Set(o.filter(y=>y.card_type==="game_changer").map(y=>y.gc_type)),l=new Set(o.filter(y=>y.card_type==="stadium").map(y=>y.stadium_id).filter(Boolean)),s=new Set,f=[];for(let y=0;y<(e.cardCount||5);y++){const x=cr(e.rates);if(x){if(x.card_type==="player"){const g=$=>({légende:"legende",pépite:"pepite",pépites:"pepite"})[$]||$,p=x.rarity?g(x.rarity):null;let b=v.from("players").select("id,job,firstname,surname_real,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);p&&(b=b.eq("rarity",p));const{data:u}=await b;let w=u||[];if((x.note_min||x.note_max)&&(w=w.filter($=>{const T=Math.max(Number($.note_g)||0,Number($.note_d)||0,Number($.note_m)||0,Number($.note_a)||0);return(!x.note_min||T>=x.note_min)&&(!x.note_max||T<=x.note_max)})),w.length||(x.note_min||x.note_max?(w=u||[],console.warn("[Booster] Aucun joueur avec note",x.note_min,"-",x.note_max,"— fallback rareté uniquement")):w=u||[]),!w.length)continue;let h=w.filter($=>!s.has($.id));if(i)h.length||(h=w);else if(h=h.filter($=>!a.has($.id)),!h.length)continue;const E=h[Math.floor(Math.random()*h.length)];s.add(E.id);const m=a.has(E.id),{data:S}=await v.from("cards").insert({owner_id:t.id,player_id:E.id,card_type:"player"}).select().single();S&&(f.push({...S,player:E,isDuplicate:m}),v.rpc("record_transfer",{p_card_id:S.id,p_player_id:E.id,p_club_name:t.club_name||t.pseudo,p_manager_name:t.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{}))}else if(x.card_type==="game_changer"){const{data:g}=await v.from("gc_definitions").select("id,name,color,effect,image_url,gc_type").eq("is_active",!0).eq("gc_type","game_changer"),p=g!=null&&g.length?g:[{name:"Ressusciter"},{name:"Double attaque"},{name:"Bouclier"},{name:"Vol de note"},{name:"Gel"}],b=i?p:p.filter(E=>!c.has(E.name));if(!i&&!b.length)continue;const u=b[Math.floor(Math.random()*b.length)],w=u.name,{data:h}=await v.from("cards").insert({owner_id:t.id,card_type:"game_changer",gc_type:w,gc_definition_id:u.id||null}).select().single();h&&f.push({...h,_gcDef:u})}else if(x.card_type==="formation"){const g=en(),p=i?g:g.filter(h=>!d.has(h));if(!i&&!p.length)continue;const b=p[Math.floor(Math.random()*p.length)],u=d.has(b),{data:w}=await v.from("cards").insert({owner_id:t.id,card_type:"formation",formation:b}).select();w!=null&&w[0]&&f.push({...w[0],isDuplicate:u})}else if(x.card_type==="stadium"){const{data:g,error:p}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)");if(p){console.error("[Booster] stadium_definitions:",p.message);continue}if(!(g!=null&&g.length)){console.warn("[Booster] Aucun stade en DB");continue}const b=i?g:g.filter(E=>!l.has(E.id));if(!i&&!b.length)continue;const u=b[Math.floor(Math.random()*b.length)],{data:w,error:h}=await v.from("cards").insert({owner_id:t.id,card_type:"stadium",stadium_id:u.id}).select("id,card_type,stadium_id").single();if(h){console.error("[Booster] insert stadium card:",h.message);continue}w&&f.push({...w,rarity:"normal",_stadiumDef:u})}}}return f}async function on(t,e,i){if(i>0){const{error:l}=await v.from("users").update({credits:t.credits-i}).eq("id",t.id);if(l)throw l;t.credits-=i,Ti(t.credits)}const{data:o}=await v.from("players").select("id,job,firstname,surname_real,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(o!=null&&o.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const r=o.filter(l=>l.job==="GK"),n=o.filter(l=>l.job!=="GK"),a=!t.first_booster_opened&&r.length>0,d=[];for(let l=0;l<e;l++){const s=l===0&&a?r:l===0?n:o,f=gr(),y=br(s,f);y&&d.push(y)}a&&await v.from("users").update({first_booster_opened:!0}).eq("id",t.id);const{data:c}=await v.from("cards").insert(d.map(l=>({owner_id:t.id,player_id:l.id,card_type:"player"}))).select();return(c||[]).forEach((l,s)=>{v.rpc("record_transfer",{p_card_id:l.id,p_player_id:d[s].id,p_club_name:t.club_name||t.pseudo,p_manager_name:t.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{})}),d.map((l,s)=>({...c[s],player:l}))}async function nn(t,e,i){const{error:o}=await v.from("users").update({credits:t.credits-i}).eq("id",t.id);if(o)throw o;t.credits-=i,Ti(t.credits);const{data:r}=await v.from("gc_definitions").select("id,name,gc_type,color,effect,image_url").eq("is_active",!0),n=r!=null&&r.length?r:Object.keys(Oi).map(f=>({name:f,gc_type:"game_changer"})),a=Array.from({length:e},()=>n[Math.floor(Math.random()*n.length)]),d=a.map(f=>({owner_id:t.id,card_type:"game_changer",gc_type:f.name,gc_definition_id:f.id||null})),{data:c,error:l}=await v.from("cards").insert(d).select();return l&&console.error("[Booster GC] Erreur insert:",l.message,l),(c||[]).map((f,y)=>({...f,_gcDef:a[y]||null}))}async function rn(t,e){const{error:i}=await v.from("users").update({credits:t.credits-e}).eq("id",t.id);if(i)throw i;t.credits-=e,Ti(t.credits);const{data:o}=await v.from("cards").select("formation").eq("owner_id",t.id).eq("card_type","formation"),r=new Set((o||[]).map(s=>s.formation)),n=en(),a=n[Math.floor(Math.random()*n.length)],d=r.has(a),{data:c,error:l}=await v.from("cards").insert({owner_id:t.id,card_type:"formation",formation:a}).select();return l&&console.error("[Booster Formation] Erreur insert:",l.message,l),(c||[]).map(s=>({...s,isDuplicate:d}))}function an(t,e,i,o=null){var V,I;if(!t||t.length===0){const _=document.createElement("div");_.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",_.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
  ...
  `,document.body.appendChild(r);let n=!1;const a=document.getElementById("pack-cut-zone"),d=document.getElementById("pack-blade");let c=!1;const l=_=>_.touches&&_.touches[0]?_.touches[0].clientX:_.clientX;function s(_){n||(c=!0,d.style.opacity="1",f(_))}function f(_){if(!c||n)return;const R=a.getBoundingClientRect(),W=l(_)-R.left,de=Math.max(0,Math.min(1,W/R.width));d.style.width=de*R.width+"px",de>=.82&&x()}function y(){n||(c=!1,d.style.transition="width .2s ease, opacity .2s ease",d.style.width="0",d.style.opacity="0",setTimeout(()=>{n||(d.style.transition="")},220))}function x(){var R;if(n)return;n=!0,c=!1,d.style.width="100%",d.style.opacity="1",(R=document.getElementById("cut-flash"))==null||R.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const _=document.getElementById("cut-hint");_&&(_.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{d.style.opacity="0",document.getElementById("pack-phase").style.display="none",u(0)},620)}a.addEventListener("pointerdown",s),window.addEventListener("pointermove",f),window.addEventListener("pointerup",y),a.addEventListener("touchstart",s,{passive:!0}),window.addEventListener("touchmove",f,{passive:!0}),window.addEventListener("touchend",y);let g=0,p=!1;const b=new Set;function u(_){g=_,document.getElementById("reveal-phase").style.display="flex",w(),h(_,0),T()}function w(){const _=document.getElementById("card-dots");_&&(_.innerHTML=t.map((R,W)=>`<div class="card-dot" data-i="${W}" style="width:8px;height:8px;border-radius:50%;background:${W===g?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),_.querySelectorAll(".card-dot").forEach(R=>R.addEventListener("click",()=>m(parseInt(R.dataset.i)))))}function h(_,R){var C,K;const W=t[_],de=document.getElementById("card-counter"),ce=document.getElementById("card-track");de&&(de.textContent=`Carte ${_+1} / ${t.length}`);const j=document.getElementById("reveal-btns");j&&(j.style.display=_===t.length-1?"flex":"none");const H=W.card_type==="player"&&((C=W.player)==null?void 0:C.rarity)==="legende",pe=!b.has(_);b.add(_);let me=0;if(W.card_type==="player"&&W.player){const Q=W.player,X=Q.job||"ATT";me=(Number(X==="GK"?Q.note_g:X==="DEF"?Q.note_d:X==="MIL"?Q.note_m:Q.note_a)||0)+(W.evolution_bonus||0)}const fe=Q=>{ce.innerHTML=`
      ...
      </div>`,document.body.appendChild(o),o.addEventListener("click",a=>{a.target===o&&o.remove()}),(i=document.getElementById("odds-close"))==null||i.addEventListener("click",()=>o.remove());return}hr()}function hr(){const t=document.createElement("div");t.style.cssText=`position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;
  ...
  `,document.body.appendChild(t),t.addEventListener("click",e=>{e.target===t&&t.remove()}),document.getElementById("odds-close").addEventListener("click",()=>t.remove())}function vr(){return new Promise(t=>{const e=document.createElement("div");e.style.cssText=`
    ...
    `,document.body.appendChild(e);try{(window.adsbygoogle=window.adsbygoogle||[]).push({})}catch(r){console.warn("[AdSense]",r)}let i=5;const o=setInterval(()=>{i--;const r=document.getElementById("mw-ad-cd");r&&(r.textContent=i),i<=0&&(clearInterval(o),e.remove(),t(!0))},1e3)})}async function wr(t,{state:e,navigate:i,toast:o,refreshProfile:r}){var y,x;const{data:n}=await v.from("users").select("*").eq("id",e.user.id).single();n&&(e.profile=n);let a=Array.isArray((y=e.profile)==null?void 0:y.pending_boosters)?[...e.profile.pending_boosters]:[];if(!a.length){await v.from("users").update({onboarding_done:!0}).eq("id",e.user.id),i("home");return}let d=null;try{const p=(await Zo()).find(b=>(b.name||"").toLowerCase().includes("new player"));p&&(d=tn(p))}catch(g){console.warn('[Onboarding] Config "Booster (new player)" introuvable, fallback taux par défaut',g)}const c=a.length;let l=0;t.innerHTML=`
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
  ...
  </div>`;const s=async()=>{await v.from("users").update({pending_boosters:a}).eq("id",e.user.id)};async function f(){var w;if(l>=c||!a.length){await v.from("users").update({pending_boosters:[],onboarding_done:!0}).eq("id",e.user.id),r&&await r(),o("Tous tes boosters sont ouverts ! Bon jeu 🎮","success",4e3),i("home");return}const g=a[0],{data:p}=await v.from("users").select("*").eq("id",e.user.id).single();p&&(e.profile=p);let b=[];try{if(g.type==="formation")b=await rn(e.profile,0);else if(g.type==="game_changer")b=await nn(e.profile,g.count||3,0);else if(d&&((w=d.rates)!=null&&w.length)){const h={...d,cost:0,cardCount:g.count||d.cardCount||5};b=await Hi(e.profile,h),g.guaranteeGK&&!e.profile.first_booster_opened&&(b.some(m=>m.player&&m.player.job==="GK")||await _r(e.profile,b),await v.from("users").update({first_booster_opened:!0}).eq("id",e.profile.id))}else b=await on(e.profile,g.count||5,0)}catch(h){o(h.message||"Erreur ouverture booster","error");return}a.shift(),l++,await s();const u=g.type==="formation"?{name:"Booster Formation",img:"/icons/booster-formation.png"}:g.type==="game_changer"?{name:"Booster Game Changer",img:"/icons/booster-gamechanger.png"}:{name:`Booster Joueurs (${l}/${c})`,img:(d==null?void 0:d.img)||"/icons/booster-players.png"};an(b,u,i,()=>{f()})}(x=document.getElementById("onboard-start"))==null||x.addEventListener("click",()=>f())}async function _r(t,e){try{const{data:i}=await v.from("players").select("id,job,firstname,surname_real,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0).eq("job","GK");if(!(i!=null&&i.length))return;const o=i[Math.floor(Math.random()*i.length)],r=e.findIndex(a=>a.player);if(r===-1)return;const n=e[r];await v.from("cards").update({player_id:o.id}).eq("id",n.id),e[r]={...n,player_id:o.id,player:o}}catch(i){console.warn("[Onboarding] ensureGK échec",i)}}const Et={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},kr={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Mt(t,e,i,o,r){var n;t.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
    <div style="text-align:center;padding:40px;color:#fff">
      <div style="font-size:48px;margin-bottom:16px">${e}</div>
      <p style="margin-bottom:16px">${i}</p>
      <button class="btn btn-primary" id="msg-btn">${o}</button>
    </div>
  </div>`,(n=document.getElementById("msg-btn"))==null||n.addEventListener("click",r)}function Ui(t,e){var n,a;const i=t.player,o=t.evolution_bonus||0,r=i.job2&&Number(i[`note_${i.job2.toLowerCase()}`])||0;return{cardId:t.id,position:e||null,id:i.id,firstname:i.firstname,name:i.surname_real,country_code:i.country_code,club_id:i.club_id,job:i.job,job2:i.job2,note_g:(Number(i.note_g)||0)+(i.job==="GK"?o:0)+(i.job2==="GK"&&r>0?o:0),note_d:(Number(i.note_d)||0)+(i.job==="DEF"?o:0)+(i.job2==="DEF"&&r>0?o:0),note_m:(Number(i.note_m)||0)+(i.job==="MIL"?o:0)+(i.job2==="MIL"&&r>0?o:0),note_a:(Number(i.note_a)||0)+(i.job==="ATT"?o:0)+(i.job2==="ATT"&&r>0?o:0),evolution_bonus:o,rarity:i.rarity,skin:i.skin,hair:i.hair,hair_length:i.hair_length,face:i.face||null,clubName:((n=i.clubs)==null?void 0:n.encoded_name)||null,clubLogo:((a=i.clubs)==null?void 0:a.logo_url)||null,boost:0,used:!1,_line:null,_col:null}}function di(t,e){if(!e||!t)return t;const{club_id:i,country_code:o}=e;return Object.values(t).forEach(r=>{Array.isArray(r)&&r.forEach(n=>{const a=i&&String(n.club_id)===String(i),d=o&&String(n.country_code)===String(o);(a||d)&&(n.stadiumBonus=!0)})}),t}function vi(t,e){if(!e||!(t!=null&&t.length))return t;const{club_id:i,country_code:o}=e;return t.forEach(r=>{if(!r)return;const n=i&&String(r.club_id)===String(i),a=o&&String(r.country_code)===String(o);(n||a)&&(r.stadiumBonus=!0)}),t}function dt(t){return t===1?[1]:t===2?[0,2]:t===3?[0,1,2]:t===4?[0,1,1,2]:t===5?[0,1,1,1,2]:[1]}function sn(){const t=Math.random()*100;return t<10?10:t<30?5:3}function wi(t,e){const i=Et[e]||Et["4-4-2"],o={GK:[],DEF:[],MIL:[],ATT:[]};if(t.length&&t.every(a=>a.position)){for(const a of["GK","DEF","MIL","ATT"]){const d=t.filter(l=>l.position&&l.position.replace(/\d+$/,"")===a).sort((l,s)=>parseInt(l.position.replace(/\D+/g,""),10)-parseInt(s.position.replace(/\D+/g,""),10)).map(l=>({...l,_line:a})),c=dt(d.length);d.forEach((l,s)=>{l._col=c[s]}),o[a]=d}return o}const n=[...t];for(const a of["GK","DEF","MIL","ATT"]){const d=[];for(let l=0;l<i[a];l++){let s=n.findIndex(f=>f.job===a);if(s===-1&&(s=n.findIndex(f=>f.job2===a)),s===-1&&(s=0),n[s]){const f={...n[s],_line:a};d.push(f),n.splice(s,1)}}const c=dt(d.length);d.forEach((l,s)=>{l._col=c[s]}),o[a]=d}return o}function ct(t){document.querySelectorAll(".top-nav, .bottom-nav").forEach(e=>{e.style.setProperty("display","none","important"),e.dataset.matchHidden="1"}),t&&t.style.setProperty("padding-bottom","0","important")}function Ve(t){document.querySelectorAll(".top-nav, .bottom-nav").forEach(e=>{e.style.removeProperty("display"),delete e.dataset.matchHidden}),t&&t.style.removeProperty("padding-bottom")}function Si(t,e,i){const r=new Set,n=e.filter(s=>{var y;const f=((y=s._gcDef)==null?void 0:y.name)||s.gc_type||s.id;return r.has(f)?!1:(r.add(f),!0)});let a=[];function d(s,f){const y=s._gcDef,x=(y==null?void 0:y.name)||s.gc_type,g=y!=null&&y.image_url?`/icons/${y.image_url}`:null,p=nt(x,g,"⚡",(y==null?void 0:y.effect)||"",{width:100});return`<div class="gc-select-card" data-id="${s.id}"
    ...
    </div>`}const c=s=>{t.style.overflow="",t.style.height="",t.style.display="",t.style.flexDirection="",i(s)};function l(){var f,y,x;t.style.overflow="hidden",t.style.height="100%",t.style.display="flex",t.style.flexDirection="column";const s=a.length>0;t.innerHTML=`
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px 16px">
        ${n.map(g=>{const p=a.find(b=>b.gc_type===g.gc_type);return d(g,!!p)}).join("")}
      </div>
    ...
    </div>`,t.querySelectorAll(".gc-select-card").forEach(g=>{g.addEventListener("click",()=>{const p=g.dataset.id,b=n.find(w=>w.id===p);if(!b)return;const u=a.findIndex(w=>w.gc_type===b.gc_type);u>-1?a.splice(u,1):a.length<3&&a.push(b),l()})}),(f=t.querySelector("#gc-launch"))==null||f.addEventListener("click",()=>{s&&c(a)}),(y=t.querySelector("#gc-no-gc"))==null||y.addEventListener("click",()=>c([])),(x=t.querySelector("#gc-reset"))==null||x.addEventListener("click",()=>{a.length&&(a=[],l())})}l()}function $r(t,e){var r;const i=((r=e==null?void 0:e.state)==null?void 0:r.params)||{},o=t||i.matchMode||"vs_ai_easy";return o==="friend"?`Match vs ${i.friendName||"Ami"}`:o==="random"?"Match vs Random":o==="ranked"?"Match Classé":o==="mini_league"||o==="mini-league"?"🏆 Match de Mini League":`Match vs IA — ${o.replace("vs_ai_","").toUpperCase()}`}async function Er(t,e,i){const{state:o,navigate:r}=e;t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await v.from("decks").select(`id,name,is_active,formation,stadium_card_id,
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),c=[...new Set((d||[]).filter(y=>{var x,g;return((x=y.card)==null?void 0:x.card_type)==="stadium"&&((g=y.card)==null?void 0:g.stadium_id)}).map(y=>y.card.stadium_id))],l={};if(c.length){const{data:y}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",c);(y||[]).forEach(x=>{l[x.id]=x}),(d||[]).forEach(x=>{var g,p;((g=x.card)==null?void 0:g.card_type)==="stadium"&&((p=x.card)!=null&&p.stadium_id)&&(x.card._stadiumDef=l[x.card.stadium_id]||null)})}let s=0;function f(){var m,S,$,T,P,G,O;const y=n[s],x=(d||[]).filter(V=>V.deck_id===y.id),g=x.filter(V=>{var I;return V.is_starter&&((I=V.card)==null?void 0:I.player)}).map(V=>Ui(V.card,V.position)),p=x.find(V=>{var I;return((I=V.card)==null?void 0:I.card_type)==="formation"}),b=y.formation||((m=p==null?void 0:p.card)==null?void 0:m.formation)||"4-4-2";let u=g.length>=11?wi(g,b):null,w=((S=y.stadium_card)==null?void 0:S.stadium_def)||null;w&&u&&(u=di(u,w));const h=g.length>=11;ct(t),t.style.height="100%",t.style.overflow="hidden",t.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <div id="deck-top-bar" style="flex-shrink:0">
        <div style="padding:8px 16px;background:rgba(0,0,0,0.4);text-align:center">
          <div style="font-size:10px;opacity:.6;letter-spacing:2px;text-transform:uppercase">${$r(i,e)}</div>
        ...
        <div style="display:flex;align-items:center;gap:8px;padding:6px 8px">
          ...
          <div style="flex:1;text-align:center">
            ...
            <div style="font-size:11px;opacity:.6">${b} · ${g.length}/11${y.is_active?" · ⭐":""}</div>
          </div>
      ...
      <div id="deck-bottom-bar" style="flex-shrink:0;padding:10px 14px 14px;display:flex;flex-direction:column;gap:8px;background:rgba(0,0,0,0.2)">
        ...
        stroke="${$}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=typeof window<"u"&&window.innerWidth>=900?Math.min(Math.max(81,Math.round(r*.225)),117):Math.max(44,Math.round(r*.168)),p=Math.round(g*657/507);for(const[u,w]of Object.entries(s)){const h=y(u);if(!h||!w)continue;const E=u.replace(/[0-9]/g,""),m=a.includes(w.cardId),S=i==="attack"&&(["MIL","ATT"].includes(E)||m)&&!w.used||i==="defense"&&["GK","DEF","MIL"].includes(E)&&!w.used,$=o.includes(w.cardId);let T=w.boost||0,P=!1;w.stadiumBonus&&(i==="attack"&&(E==="ATT"||E==="MIL")||i==="defense"&&(E==="GK"||E==="DEF"||E==="MIL")?(T+=10,P=!0):i||(T+=10,P=!0));const G=Math.round(h.x-g/2),O=Math.round(h.y-p/2);if(w.used){x+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${G}" y="${O}" width="${g}" height="${p}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${w.cardId}" data-role="${E}" style="cursor:pointer"/>`;continue}const V=ze({...w,_evolution_bonus:0,stadiumBonus:!1},{width:g,showStad:!1,stadDef:null,role:E,extraNote:T,_cardOffset:30,_forceStadColor:P}),I=$?`position:absolute;top:30px;left:0;width:${g}px;height:${p}px;outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;pointer-events:none;`:"";x+=`<foreignObject x="${G-2}" y="${O-30}" width="${g+8}" height="${p+60}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="position:relative">
        ${V}
        ${$?`<div style="${I}"></div>`:""}
      </div>
    </foreignObject>`,S&&(x+=`<rect x="${G}" y="${O}" width="${g}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${$?"selected":""}" data-card-id="${w.cardId}" data-role="${E}" style="cursor:pointer"/>`)}const b=d!==null?d:Math.round(Math.max(g*.7,80));return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-b} ${-b} ${r+b*2} ${n+b*2}" width="100%" style="display:block;width:100%;margin:0 auto">
    ${x}
  </svg>`}function Ut(t,e,i,o,r=300,n=300,a=[],d=null){return`<div id="match-terrain-wrap" style="position:relative;padding:0 4px">
    ${Ai(t,e,i,o,r,n,a,d)}
  </div>`}async function zi(t,e,i,o){var S;const{state:r,navigate:n,toast:a}=e;ct(t);const d=r.params||{};if(t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!d.deckId)return Er(t,e,i);const c=d.deckId;let l,s,f,y;try{const $=await Promise.all([v.from("decks").select("formation,name,stadium_card_id").eq("id",c).single(),v.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_real,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",c).order("slot_order")]);l=$[0].data,f=$[0].error,s=$[1].data,y=$[1].error}catch($){console.error("[Match] Exception chargement deck:",$),Mt(t,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>n("home"));return}if(f||y){console.error("[Match] Erreur Supabase:",f||y),Mt(t,"⚠️","Erreur lors du chargement du deck.","Retour",()=>n("home"));return}const x=(s||[]).filter($=>{var T;return $.is_starter&&((T=$.card)==null?void 0:T.player)}).map($=>Ui($.card,$.position)),g=(s||[]).filter($=>{var T;return!$.is_starter&&((T=$.card)==null?void 0:T.player)}).map($=>Ui($.card,$.position));if(x.length<11){Mt(t,"⚠️",`Deck incomplet (${x.length}/11).`,"Compléter",()=>n("decks"));return}const p=(s||[]).find($=>{var T;return((T=$.card)==null?void 0:T.card_type)==="formation"}),b=(l==null?void 0:l.formation)||((S=p==null?void 0:p.card)==null?void 0:S.formation)||"4-4-2",{data:u,error:w}=await v.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",r.profile.id).eq("card_type","game_changer"),{data:h}=await v.from("gc_definitions").select("*").eq("is_active",!0),E=(u||[]).map($=>({...$,_gcDef:(h==null?void 0:h.find(T=>T.name===$.gc_type||T.id===$.gc_definition_id))||null}));let m=null;if(l!=null&&l.stadium_card_id){const{data:$}=await v.from("cards").select("stadium_id").eq("id",l.stadium_card_id).single();if($!=null&&$.stadium_id){const{data:T}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",$.stadium_id).single();m=T||null}}o({deckId:c,formation:b,starters:x,subsRaw:g,gcCardsEnriched:E,gcDefs:h||[],stadiumDef:m})}function Ct(){return Math.min(window.innerWidth-40,860)}function Ot(){return Math.round(Ct()*1.1)}function ln(t){var r,n;if(!t)return null;const e=t._line||t.job||"MIL",i=e==="GK"?t.note_g||0:e==="DEF"?t.note_d||0:e==="MIL"?t.note_m||0:t.note_a||0,o=t.stadiumBonus?10:0;return{name:t.name,firstname:t.firstname||"",note:i+(t.boost||0)+o,note_g:t.note_g||0,note_d:t.note_d||0,note_m:t.note_m||0,note_a:t.note_a||0,_evolution_bonus:0,stadiumBonus:t.stadiumBonus||!1,boost:t.boost||0,job:t.job,job2:t.job2||null,_line:t._line||t.job,_col:t._col,country_code:t.country_code,club_id:t.club_id,rarity:t.rarity,clubName:t.clubName||((r=t.clubs)==null?void 0:r.encoded_name)||null,clubLogo:t.clubLogo||((n=t.clubs)==null?void 0:n.logo_url)||null,face:t.face||null,portrait:Bt(t)}}function Lr(t,e,i,o="Adversaire"){No("/sounds/match-opening.mp3",.3);const r=Ct(),n=Ot();return`
    ...
    </div>`}function dn(t){var n,a,d;if(!t)return"";const e=c=>c?ze({...c,_evolution_bonus:0},{width:52,role:c._line||c.job,showStad:!!c.stadiumBonus,extraNote:c.boost||0}):"",o={goal:"⚽","goal-home":"⚽","goal-ai":"⚽",duel:"⚔️",midfield:"🎯",sub:"🔄",gc:"⚡",boost:"💥","defense-won":"🛡️","attack-won":"⚔️","defense-lost":"😓","attack-lost":"😓"}[t.type]||"📋";return`
    ...
    <div style="animation:goalPop 0.4s ease-out;text-align:center">
      ...
      }`,document.head.appendChild(c)}setTimeout(()=>{a.style.opacity="0",a.style.transition="opacity 0.4s",setTimeout(()=>{a.remove(),r==null||r()},400)},1800)}function Ir(t,e,i){const o=document.getElementById("sub-anim-overlay");o&&o.remove();const r=document.createElement("div");r.id="sub-anim-overlay",r.style.cssText=`
    position:fixed;inset:0;z-index:3000;
    display:flex;flex-direction:column;align-items:center;justify-content:center;
    ...
    pointer-events:none;white-space:nowrap;max-width:90vw;text-align:center`,r.textContent=t,!document.getElementById("game-toast-style")){const n=document.createElement("style");n.id="game-toast-style",n.textContent=`
      ...
      }`,document.head.appendChild(n)}document.body.appendChild(r),setTimeout(()=>{r.style.opacity="0",r.style.transition="opacity 0.3s",setTimeout(()=>r.remove(),300)},i)}function cn(t,e){const i=Pe(t,"MIL"),o=t.stadiumBonus||e&&(e.club_id&&String(t.club_id)===String(e.club_id)||e.country_code&&t.country_code===e.country_code)?10:0;return i+o}function vt(t,e){return t.reduce((i,o)=>i+cn(o,e),0)}function wt(t){let e=0;for(let i=0;i<t.length-1;i++){const o=Dt(t[i],t[i+1]);o==="#00ff88"?e+=10:o==="#FFD700"&&(e+=5)}return e}function _i(t,e,i,o,r){return`<div id="duel-row-${o}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${t.map((n,a)=>{const d=a<t.length-1?Dt(n,t[a+1]):null,c=!d||d==="#ff3333"||d==="#cc2222",l=d==="#00ff88"?"+10":d==="#FFD700"?"+5":"";return cn(n,r),n.stadiumBonus||r&&(r.club_id&&String(n.club_id)===String(r.club_id)||r.country_code&&(n.country_code,r.country_code)),`
    ...
    </div>`}async function Tr(t,e){const{state:i}=e,o=i.params||{},r=o.matchMode||"vs_ai_easy",n=r==="solo",a=r==="ranked_ai",d=o.soloLevel||1,c=r.replace("vs_ai_",""),l=r;let s=null;if(n){const{data:g}=await v.from("solo_levels").select("*").eq("level_number",d).maybeSingle();s=g||{level_number:d,target_note_avg:10,nb_liens_jaune:2,nb_liens_vert:1,nb_joueurs_stade:2,reward_credits:500}}const f=o.rankedData||null,y=o.presetSetup||null;async function x({deckId:g,formation:p,starters:b,subsRaw:u,gcCardsEnriched:w,gcDefs:h,stadiumDef:E}){try{let m=wi(b,p);E&&(m=di(m,E),vi(u,E));let S=null;if(a){const G=Ar(m),{jaune:O,vert:V}=zr(m,p),I=.9+Math.random()*.2;S={target_note_avg:Math.max(1,Math.round(G*I/16)),nb_liens_jaune:O,nb_liens_vert:V,nb_joueurs_stade:E?3:0}}const $=n?await Io(p,s):a?await Io(p,S):await Sr(p,c),T=$.lines||$,P=async G=>{try{const O=l==="vs_ai_club"||l==="solo"||l==="ranked_ai"?"club":l,{data:V,error:I}=await v.from("matches").insert({home_id:i.profile.id,away_id:null,mode:O,home_deck_id:g,status:"in_progress"}).select().single();if(I){console.error("[MatchIA] Erreur création match:",I),Mt(t,"⚠️","Impossible de créer le match ("+I.message+").","Retour",()=>e.navigate("home"));return}const _=$.stadiumDef||null;_&&T&&(di(T,_),vi($.subs||[],_));const R={gcDefs:h||[],matchId:V==null?void 0:V.id,mode:l,difficulty:c,formation:p,isSolo:n,soloLevel:d,soloLevelConfig:s,isRankedAI:a,rankedAIData:f,homeTeam:m,aiTeam:T,homeSubs:u,subsUsed:0,maxSubs:Math.min(u.length,3),aiSubs:$.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min(($.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:$.gcCards||[],aiUsedGc:[],aiStadiumDef:_,homeScore:0,aiScore:0,gcCards:G,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Mr(t,R,e)}catch(O){console.error("[MatchIA] Exception launchMatch:",O),Mt(t,"⚠️","Erreur au lancement du match : "+O.message,"Retour",()=>e.navigate("home"))}};if(a&&y){P(y.gcCardsEnriched||[]);return}if(!w.length){P([]);return}Si(t,w,P)}catch(m){console.error("[MatchIA] Exception setup:",m),Mt(t,"⚠️","Erreur de préparation du match : "+m.message,"Retour",()=>e.navigate("home"))}}y?await x(y):await zi(t,e,r,x)}async function Sr(t,e){var b;const{data:i}=await v.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:pn(t),subs:[],gcCards:[],stadiumDef:null};const o=Et[t]||Et["4-4-2"],r={GK:[],DEF:[],MIL:[],ATT:[]},n=new Set;function a(u,w,h){var E,m;return n.add(u.id),{cardId:"ai-"+u.id+"-"+h,id:u.id,firstname:u.firstname,name:u.surname_real,country_code:u.country_code,club_id:u.club_id,job:u.job,job2:u.job2,note_g:Number(u.note_g)||0,note_d:Number(u.note_d)||0,note_m:Number(u.note_m)||0,note_a:Number(u.note_a)||0,rarity:u.rarity,skin:u.skin,hair:u.hair,hair_length:u.hair_length,face:u.face,clubName:((E=u.clubs)==null?void 0:E.encoded_name)||null,clubLogo:((m=u.clubs)==null?void 0:m.logo_url)||null,boost:0,used:!1,_line:w}}for(const u of["GK","DEF","MIL","ATT"]){const w=i.filter($=>$.job===u&&!n.has($.id)),h=i.filter($=>$.job!==u&&!n.has($.id)),E=[...w,...h],m=[];for(let $=0;$<o[u];$++){const T=E[$];T&&m.push(a(T,u,$))}const S=dt(m.length);m.forEach(($,T)=>{$._col=S[T]}),r[u]=m}const c=i.filter(u=>!n.has(u.id)).slice(0,5).map((u,w)=>a(u,u.job,100+w)),{data:l}=await v.from("gc_definitions").select("*").eq("is_active",!0),f=[...l||[]].sort(()=>Math.random()-.5).slice(0,3).map((u,w)=>({id:"ai-gc-"+w,gc_type:u.name,name:u.name,icon:"⚡",effect_type:u.effect_type,effect_params:u.effect_params||{}})),y=Object.values(r).flat(),x={};y.forEach(u=>{u.club_id&&(x[u.club_id]=(x[u.club_id]||0)+1)});const g=(b=Object.entries(x).sort((u,w)=>w[1]-u[1])[0])==null?void 0:b[0];let p=null;if(g){const{data:u}=await v.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",g).single();u&&(p={club_id:u.id,country_code:null,name:u.encoded_name+" Stadium",club:{encoded_name:u.encoded_name,logo_url:u.logo_url}})}return{lines:r,subs:c,gcCards:f,stadiumDef:p}}function Ar(t){let e=0;for(const i of["GK","DEF","MIL","ATT"])(t[i]||[]).forEach(o=>{const r=o._line||i,n=Number(r==="GK"?o.note_g:r==="DEF"?o.note_d:r==="MIL"?o.note_m:o.note_a)||0;e+=n+(o.boost||0)});return e}function zr(t,e){const i={};for(const a of["GK","DEF","MIL","ATT"])(t[a]||[]).forEach((d,c)=>{i[`${a}${c+1}`]=d});const o=Li(e)||Ii[e]||[];let r=0,n=0;for(const[a,d]of o){const c=i[a],l=i[d];if(!c||!l)continue;const s=Dt(c,l);s==="#00ff88"?n++:s==="#FFD700"&&r++}return{jaune:r,vert:n}}function Lo(t){const e=t.job||"ATT";return Number(e==="GK"?t.note_g:e==="DEF"?t.note_d:e==="MIL"?t.note_m:t.note_a)||0}function mi(t,e,i){if(i<=0||!t.length)return[];const o=[...t].sort((r,n)=>Math.abs(Lo(r)-e)-Math.abs(Lo(n)-e));return o.slice(0,Math.min(i,o.length))}async function Io(t,e){var V,I;const i=Number(e==null?void 0:e.target_note_avg)||10,o=Number(e==null?void 0:e.nb_liens_jaune)||0,r=Number(e==null?void 0:e.nb_liens_vert)||0,n=Number(e==null?void 0:e.nb_joueurs_stade)||0,{data:a}=await v.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url,country_code)").eq("is_active",!0).limit(300);if(!a||a.length<16)return{lines:pn(t),subs:[],gcCards:[],stadiumDef:null};const d={};a.forEach(_=>{_.club_id&&(d[_.club_id]=(d[_.club_id]||0)+1)});const c=((V=Object.entries(d).sort((_,R)=>R[1]-_[1])[0])==null?void 0:V[0])||null,l=c?a.filter(_=>_.club_id===c):[],s=((I=l[0])==null?void 0:I.country_code)||null,f=s?a.filter(_=>_.country_code===s&&_.club_id!==c):[],y=Math.min(l.length,Math.max(2,r+1),16),x=Math.min(f.length,Math.max(0,o),16-y),g=[],p=new Set;mi(l,i,y).forEach(_=>{g.push(_),p.add(_.id)}),mi(f,i,x).forEach(_=>{g.push(_),p.add(_.id)});const b=Math.max(0,16-g.length),u=a.filter(_=>!p.has(_.id));mi(u,i,b).forEach(_=>{g.push(_),p.add(_.id)});const w=Et[t]||Et["4-4-2"],h={GK:[],DEF:[],MIL:[],ATT:[]},E=new Set;function m(_,R,W){var de,ce;return E.add(_.id),{cardId:"ai-"+_.id+"-"+W,id:_.id,firstname:_.firstname,name:_.surname_real,country_code:_.country_code,club_id:_.club_id,job:_.job,job2:_.job2,note_g:Number(_.note_g)||0,note_d:Number(_.note_d)||0,note_m:Number(_.note_m)||0,note_a:Number(_.note_a)||0,rarity:_.rarity,skin:_.skin,hair:_.hair,hair_length:_.hair_length,face:_.face,clubName:((de=_.clubs)==null?void 0:de.encoded_name)||null,clubLogo:((ce=_.clubs)==null?void 0:ce.logo_url)||null,boost:0,used:!1,_line:R}}for(const _ of["GK","DEF","MIL","ATT"]){const R=g.filter(H=>H.job===_&&!E.has(H.id)),W=g.filter(H=>H.job!==_&&!E.has(H.id)),de=[...R,...W],ce=[];for(let H=0;H<w[_];H++){const pe=de[H];pe&&ce.push(m(pe,_,H))}const j=dt(ce.length);ce.forEach((H,pe)=>{H._col=j[pe]}),h[_]=ce}let S=g.filter(_=>!E.has(_.id));if(S.length<5){const _=a.filter(R=>!E.has(R.id)&&!S.some(W=>W.id===R.id));S=S.concat(mi(_,i,5-S.length))}const $=S.slice(0,5).map((_,R)=>m(_,_.job,100+R)),{data:T}=await v.from("gc_definitions").select("*").eq("is_active",!0),G=[...T||[]].sort(()=>Math.random()-.5).slice(0,3).map((_,R)=>({id:"ai-gc-"+R,gc_type:_.name,name:_.name,icon:"⚡",effect_type:_.effect_type,effect_params:_.effect_params||{}}));let O=null;if(n>0&&c){const{data:_}=await v.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",c).single();_&&(O={club_id:_.id,country_code:null,name:_.encoded_name+" Stadium",club:{encoded_name:_.encoded_name,logo_url:_.logo_url}})}return{lines:h,subs:$,gcCards:G,stadiumDef:O}}function pn(t){const e=Et[t]||Et["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},o=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let r=0;for(const n of["GK","DEF","MIL","ATT"]){const a=[];for(let c=0;c<e[n];c++){const l=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+r,id:"fake-"+r,firstname:"IA",name:o[r%o.length],country_code:"XX",club_id:null,job:n,job2:null,note_g:n==="GK"?l:2,note_d:n==="DEF"?l:2,note_m:n==="MIL"?l:2,note_a:n==="ATT"?l:2,rarity:"normal",boost:0,used:!1,_line:n}),r++}const d=dt(a.length);a.forEach((c,l)=>{c._col=d[l]}),i[n]=a}return i}function Mr(t,e,i){var r;No("/sounds/match-opening.mp3",.3);const o=`<div style="position:relative;width:22px;height:22px;flex-shrink:0;display:flex;align-items:center;justify-content:center">
  ...
  </div>`,requestAnimationFrame(()=>requestAnimationFrame(()=>{const n=t.querySelector(".opponent-preview-wrap"),a=t.querySelector("#opponent-swipe-zone");if(!n||!a)return;const d=a.clientWidth>=900,c=Math.max(200,a.clientHeight-(d?8:40)),l=Math.max(200,a.clientWidth-(d?8:16)),s=d?null:Math.round(Math.max(44,Math.round(l*.168))*.55);n.innerHTML=Ut(e.aiTeam,e.formation,null,[],l,c,[],s),n.style.cssText=`width:${l}px;height:${c}px;overflow:hidden;flex-shrink:0`;const f=n.querySelector("svg");f&&(f.style.cssText="display:block;width:100%;height:100%",f.setAttribute("preserveAspectRatio",d?"xMidYMid meet":"none"))})),setTimeout(()=>Cr(t,e,i),5e3)}const Ye=t=>ln(t);function Cr(t,e,i){const o=e.homeTeam.MIL||[],r=e.aiTeam.MIL||[],n=e.stadiumDef||null,a=e.aiStadiumDef||null,d=vt(o,n)+wt(o),c=vt(r,a)+wt(r),l=d>=c;t.innerHTML=`
  ...
  </div>`;const s=()=>{const g=(p,b)=>t.querySelectorAll(p).forEach((u,w)=>{setTimeout(()=>{u.style.opacity="1",u.style.transform="translateY(0) scale(1)"},b+w*90)});g(".duel-card-home",150),g(".duel-card-ai",500),setTimeout(()=>t.querySelectorAll(".duel-link").forEach((p,b)=>{setTimeout(()=>{p.style.opacity="1"},b*70)}),900),setTimeout(()=>{const p=t.querySelector("#vs-label");p&&(p.style.opacity="1",p.style.animation="vsFlash .5s ease"),t.querySelectorAll(".duel-score-line").forEach(b=>b.style.opacity="1")},1250),setTimeout(()=>{f("score-home",d,800),f("score-ai",c,800)},1500)};function f(g,p,b){const u=document.getElementById(g);if(!u)return;const w=performance.now(),h=E=>{const m=Math.min(1,(E-w)/b);u.textContent=Math.round(p*(1-Math.pow(1-m,3))),m<1?requestAnimationFrame(h):u.textContent=p};requestAnimationFrame(h)}requestAnimationFrame(s),e.attacker=l?"home":"ai";const y=l?sn():null;l&&(e.boostCard={value:y}),e.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:o.map(g=>Ye(g)),aiPlayers:r.map(g=>Ye(g)),homeTotal:d,aiTotal:c,text:`Duel milieu : ${e.clubName} ${d} – ${c} IA → ${l?e.clubName+" attaque":"IA attaque"}`});const x=()=>{e.phase=e.attacker==="home"?"attack":"ai-attack",Te(t,e,i),e.attacker==="ai"&&setTimeout(()=>no(t,e,i),800)};setTimeout(()=>{const g=document.getElementById("score-home"),p=document.getElementById("score-ai"),b=document.getElementById(l?"duel-row-home":"duel-row-ai"),u=document.getElementById(l?"duel-row-ai":"duel-row-home"),w=l?g:p,h=l?p:g;w&&(w.style.fontSize="80px",w.style.color=l?"#FFD700":"#ff6b6b",w.style.animation="duelPulse .5s ease"+(l?", duelGlow 1.5s ease infinite .5s":"")),h&&(h.style.opacity="0.25"),setTimeout(()=>{b&&(b.style.transformOrigin="center",b.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",b.style.zIndex="5"),setTimeout(()=>{var m;const E=document.getElementById("duel-shock");if(E){const S=(m=u||b)==null?void 0:m.getBoundingClientRect(),$=t.querySelector(".match-screen").getBoundingClientRect();S&&(E.style.top=S.top-$.top+S.height/2+"px"),E.style.animation="shockwave .5s ease-out forwards"}u&&(u.style.transformOrigin="center",u.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var m;const E=document.getElementById("duel-finale");E&&(E.innerHTML=`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(e.phase==="defense"&&e.pendingAttack){const H=e.pendingAttack;let pe="";if(e.selected.length>0){const me=e.selected.map(C=>({...(e.homeTeam[C._role]||[]).find(Q=>Q.cardId===C.cardId)||C,_line:C._role})),fe=li(me,e.modifiers.home,e.formation);pe=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
          ...
          </div>`}const j=e.log[e.log.length-1];return j?'<div style="padding:2px 4px">'+dn(j)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <div>${b?kt({...b,used:!1,boost:0},u,w):"<div>—</div>"}</div>
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start;justify-content:center">
      ${t.map(f=>{const y=f._line||f.job||"MIL",x=d.find(p=>p.cardId===f.cardId),g=ze({...f,_evolution_bonus:0},{width:90,showStad:!0,role:y,extraNote:f.boost||0});return`<div class="gc-pick-item" data-cid="${f.cardId}"
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ${o.map(r=>`
        <div class="player-boost-opt" data-card-id="${r.cardId}"
          ...
          <div style="width:32px;height:32px;background:${kr[r.job]||"#888"};border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:13px">${Pe(r,r._line||r.job)}</div>
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(r=>{r.addEventListener("click",()=>{const n=r.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const d=(e.homeTeam[a]||[]).find(c=>c.cardId===n);if(d){d.boost=(d.boost||0)+e.boostCard.value,e.log.push({text:`⚡ Boost +${e.boostCard.value} appliqué à ${d.name}`,type:"info"});break}}e.boostUsed=!0,i.closeModal(),Te(t,e,i)})})}async function _t(t,e,i){var y,x,g,p;Rt(),Ke(),e._timerInt&&(clearInterval(e._timerInt),e._timerInt=null),e.phase="finished";const{state:o}=i,r=e.homeScore>e.aiScore,n=e.homeScore===e.aiScore,a=r?"victoire":n?"nul":"defaite",d=e.isSolo&&((y=e.soloLevelConfig)!=null&&y.reward_credits)&&r?Number(e.soloLevelConfig.reward_credits):Vn(e.mode,a);if(e.isSolo&&r)try{const{data:b}=await v.from("user_solo_progress").select("unlocked_level").eq("user_id",o.profile.id).maybeSingle(),u=(b==null?void 0:b.unlocked_level)||1;e.soloLevel>=u&&await v.from("user_solo_progress").upsert({user_id:o.profile.id,unlocked_level:e.soloLevel+1,updated_at:new Date().toISOString()})}catch(b){console.warn("[Solo] Erreur mise à jour progression:",b.message)}const c="00000000-0000-0000-0000-000000000000";let l=null;if(e.isRankedAI)try{const{data:b}=await v.from("users").select("mmr, mmr_deviation, mmr_volatility, placement_matches").eq("id",o.profile.id).single();if(b){const u=b.mmr??1e3,w=b.mmr_deviation??350,h=b.mmr_volatility??.06,E=r?1:n?.5:0,m=(b.placement_matches??0)<10,S=Do(u,w,h,u,350,E,m),$=n?null:r?o.profile.id:c;await v.rpc("update_mmr_after_ranked",{p_match_id:e.matchId,p_winner_id:$,p_home_id:o.profile.id,p_away_id:c,p_home_delta:S.delta,p_away_delta:0,p_home_new_rd:S.newRd,p_away_new_rd:350,p_home_new_vol:S.newSigma,p_away_new_vol:.06}),l=S.delta}}catch(b){console.warn("[RankedAI] Erreur mise à jour MMR:",b.message)}e.matchId&&await v.from("matches").update({status:"finished",home_score:e.homeScore,away_score:e.aiScore,winner_id:r?o.profile.id:null,home_credits_reward:d,played_at:new Date().toISOString()}).eq("id",e.matchId);const s={credits:(o.profile.credits||0)+d,matches_played:(o.profile.matches_played||0)+1};r?s.wins=(o.profile.wins||0)+1:n?s.draws=(o.profile.draws||0)+1:s.losses=(o.profile.losses||0)+1,await v.from("users").update(s).eq("id",o.profile.id),await i.refreshProfile();const f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",f.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:380px;width:100%">
      ...
      ${Ai(t.aiTeam,t.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            </div>`}}catch(te){console.error("[Ranked] MMR update error:",te)}(oe=document.getElementById("pvp-end-overlay"))==null||oe.remove();const q=document.createElement("div");q.id="pvp-end-overlay",q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const D=A?"🤝":B?"🏆":"😞",U=A?"MATCH NUL":B?"VICTOIRE !":"DÉFAITE",ee=A?"#fff":B?"#FFD700":"#ff6b6b";q.innerHTML=`
      ...
      <div style="font-size:18px">${m[p+"Name"]} ${L} – ${M} ${m[b+"Name"]}</div>
      ${k.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${B?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      ${N}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${s?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(q),(ie=q.querySelector("#pvp-end-home"))==null||ie.addEventListener("click",()=>{q.remove(),Ve(t),s?x("mini-league",{openLeagueId:s}):x(a?"ranked":"home")})}const V=v.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},k=>{var M;const L=k.new;try{if(L.status==="finished"||L.forfeit){if(S)return;S=!0,$&&(clearInterval($),$=null),Ke(),L.game_state&&(m=L.game_state),O(L);return}if(L.game_state){const B=m;m=L.game_state;const A=m._lastGC;if(A&&A.seq&&!G.has(A.seq)&&(G.add(A.seq),A.by!==p)){C(A.type,A.by,()=>H());return}const N=B[p+"Score"]||0,q=B[b+"Score"]||0,D=m[p+"Score"]||0,U=m[b+"Score"]||0,ee=D>N,oe=U>q;if((ee||oe)&&!P.has(m.round)){P.add(m.round);const ie=[...m.log||[]].reverse().find(le=>le.isGoal),te=((ie==null?void 0:ie.homePlayers)||[]).map(le=>({name:le.name,note:le.note,portrait:le.portrait,job:le.job}));!!((M=ie==null?void 0:ie.text)!=null&&M.includes("DERNIER CORNER"))?ge().then(()=>{tt(te,D,U,ee,()=>H())}):tt(te,D,U,ee,()=>H());return}H()}}catch(B){console.error("[PvP] crash:",B)}}).subscribe();async function I(k){Object.assign(m,k),m.lastActionAt=new Date().toISOString();const{error:L}=await v.from("matches").update({game_state:m}).eq("id",i);L&&g("Erreur de synchronisation","error");try{H()}catch(M){console.error("[PvP] renderPvpScreen crash:",M)}}async function _(){if(S)return;S=!0,Rt(),Ke(),$&&(clearInterval($),$=null);const k=o?h.away_id:h.home_id,L=o?"p2":"p1",M=o?"p1":"p2",B={...m,[L+"Score"]:3,[M+"Score"]:0,phase:"finished"},A=B.p1Score||0,N=B.p2Score||0;try{await v.from("matches").update({game_state:B}).eq("id",i);const{data:q,error:D}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:k,p_home_score:A,p_away_score:N,p_forfeit:!0});if(D&&console.error("[PvP] finish_pvp_match (forfeit):",D),s)try{await v.rpc("check_and_finish_mini_league",{p_league_id:s})}catch(U){console.error("[PvP] check_and_finish_mini_league (forfeit):",U)}try{const{data:U}=await v.rpc("apply_match_rewards",{p_match_id:i});U!=null&&U.success&&!(U!=null&&U.skipped)&&typeof e.refreshProfile=="function"&&await e.refreshProfile()}catch(U){console.error("[PvP] apply_match_rewards (forfeit):",U)}}catch{}try{v.removeChannel(V)}catch{}setTimeout(()=>{Ve(t),s?x("mini-league",{openLeagueId:s}):x("home")},800)}const R={BOOST_STAT:({value:k=1,count:L=1,roles:M=[]},B,A)=>{const N=B[p+"Team"],q=Object.entries(N).filter(([D])=>!M.length||M.includes(D)).flatMap(([D,U])=>U.filter(ee=>!ee.used).map(ee=>({...ee,_line:D})));if(!q.length){B.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),A(B);return}W(q,L,`Choisir ${L} joueur(s) à booster (+${k})`,D=>{D.forEach(U=>{const ee=(N[U._line]||[]).find(oe=>oe.cardId===U.cardId);ee&&(ee.boost=(ee.boost||0)+k,B.log.push({text:`⚡ +${k} sur ${ee.name}`,type:"info"}))}),B[p+"Team"]=N,A(B)})},DEBUFF_STAT:({value:k=1,count:L=1,roles:M=[],target:B="ai"},A,N)=>{const q=B==="home"?p:b,D=A[q+"Team"],U=B==="home"?"allié":"adverse",ee=Object.entries(D).filter(([oe])=>!M.length||M.includes(oe)).flatMap(([oe,ie])=>ie.map(te=>({...te,_line:oe})));if(!ee.length){A.log.push({text:`🎯 Aucun joueur ${U}`,type:"info"}),N(A);return}W(ee,L,`Choisir ${L} joueur(s) ${U}(s) (-${k})`,oe=>{oe.forEach(ie=>{const te=(D[ie._line]||[]).find(ae=>ae.cardId===ie.cardId);te&&(te.boost=(te.boost||0)-k,A.log.push({text:`🎯 -${k} sur ${te.name}`,type:"info"}))}),A[q+"Team"]=D,N(A)})},GRAY_PLAYER:({count:k=1,roles:L=[],target:M="ai"},B,A)=>{const N=M==="home"?p:b,q=B[N+"Team"],D=M==="home"?"allié":"adverse",U=Object.entries(q).filter(([ee])=>!L.length||L.includes(ee)).flatMap(([ee,oe])=>oe.filter(ie=>!ie.used).map(ie=>({...ie,_line:ee})));if(!U.length){B.log.push({text:`❌ Aucun joueur ${D}`,type:"info"}),A(B);return}W(U,k,`Choisir ${k} joueur(s) ${D}(s) à exclure`,ee=>{const oe="usedCardIds_"+N,ie=new Set(B[oe]||[]);ee.forEach(te=>{const ae=(q[te._line]||[]).find(le=>le.cardId===te.cardId);ae&&(ae.used=!0,ie.add(te.cardId),B.log.push({text:`❌ ${ae.name} exclu !`,type:"info"}))}),B[oe]=[...ie],B[N+"Team"]=q,A(B)})},REVIVE_PLAYER:({count:k=1,roles:L=[]},M,B)=>{const A=M[p+"Team"],N=Object.entries(A).filter(([q])=>!L.length||L.includes(q)).flatMap(([q,D])=>D.filter(U=>U.used).map(U=>({...U,_line:q})));if(!N.length){M.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),B(M);return}W(N,k,`Choisir ${k} joueur(s) à ressusciter`,q=>{q.forEach(D=>{const U=(A[D._line]||[]).find(ee=>ee.cardId===D.cardId);U&&(U.used=!1,M.log.push({text:`💫 ${U.name} ressuscité !`,type:"info"}))}),M[p+"Team"]=A,B(M)})},REMOVE_GOAL:({},k,L)=>{const M=b+"Score";k[M]>0?(k[M]--,k.log.push({text:"🚫 Dernier but annulé !",type:"info"})):k.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L(k)},ADD_GOAL_DRAW:({},k,L)=>{k[p+"Score"]===k[b+"Score"]?(k[p+"Score"]++,k.log.push({text:"🎯 But bonus !",type:"info"})):k.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L(k)},ADD_SUB:({value:k=1},L,M)=>{L.maxSubs=(L.maxSubs||3)+k,L.log.push({text:`🔄 +${k} remplacement(s)`,type:"info"}),M(L)},CUSTOM:({},k,L)=>L(k)};function W(k,L,M,B){const A=document.createElement("div");A.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let N=[];function q(){var U,ee;const D=k.map(oe=>{const ie=oe._line||oe.job||"MIL",te=N.find(le=>le.cardId===oe.cardId),ae=ze({...oe,_evolution_bonus:0},{width:90,showStad:!0,role:ie,extraNote:oe.boost||0});return`<div class="pp-item" data-cid="${oe.cardId}"
        ...
        </div>`,(U=A.querySelector("#pp-close"))==null||U.addEventListener("click",()=>A.remove()),A.querySelectorAll(".pp-item").forEach(oe=>{oe.addEventListener("click",()=>{const ie=oe.dataset.cid,te=k.find(le=>le.cardId===ie),ae=N.findIndex(le=>le.cardId===ie);te&&(ae>-1?N.splice(ae,1):N.length<L&&N.push(te),q())})}),(ee=A.querySelector("#pp-confirm"))==null||ee.addEventListener("click",()=>{A.remove(),B(N)})}q(),document.body.appendChild(A)}async function de(k,L){const B=(m["gcCardsFull_"+p]||[]).find(D=>D.id===k),A=(B==null?void 0:B._gcDef)||(m.gcDefs||[]).find(D=>{var U;return D.name===L||((U=D.name)==null?void 0:U.toLowerCase().trim())===(L==null?void 0:L.toLowerCase().trim())}),N=[...m["usedGc_"+p]||[],k],q={type:L,by:p,seq:(m._gcAnimSeq||0)+1};G.add(q.seq),C(L,p,async()=>{if(A!=null&&A.effect_type&&A.effect_type!=="CUSTOM"){const U=R[A.effect_type];if(U){const ee={...m};U(A.effect_params||{},ee,async oe=>{oe["usedGc_"+p]=N,oe._lastGC=q,oe._gcAnimSeq=q.seq,await I(oe)});return}}const D={...m};switch(L){case"Remplacement+":D.maxSubs=(D.maxSubs||3)+1,D.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const U=b+"Score";D[U]>0&&(D[U]--,D.log.push({text:"🚫 But annulé",type:"info"}));break}}D["usedGc_"+p]=N,D._lastGC=q,D._gcAnimSeq=q.seq,await I(D)})}function ce(k,L){const M="usedCardIds_"+k,B=new Set(m[M]||[]);L.forEach(A=>B.add(A)),m[M]=[...B]}function j(){for(const k of["p1","p2"]){const L=new Set(m["usedCardIds_"+k]||[]),M=m[k+"Team"];if(M)for(const B of["GK","DEF","MIL","ATT"])(M[B]||[]).forEach(A=>{A.used=L.has(A.cardId)})}}function H(){var lo,co,po,uo,fo,mo;if(m.phase==="reveal")return pe();if(m.phase==="midfield")return fe();if(m.phase==="finished")return Ne();const k=m[p+"Team"],L=m[b+"Team"];j();const M=m[p+"Score"],B=m[b+"Score"],A=m[p+"Name"],N=m[b+"Name"],q=m.phase===p+"-attack",D=m.phase===p+"-defense",U=Array.isArray(m["selected_"+p])?m["selected_"+p]:[],ee=U.map(J=>J.cardId),oe=window.innerWidth>=700,ie=m[p+"Subs"]||[],te=m["usedSubIds_"+p]||[],ae=ie.filter(J=>!te.includes(J.cardId)),le=m["gcCardsFull_"+p]||[],we=m["usedGc_"+p]||[],ue=le.filter(J=>!we.includes(J.id)),_e=m.boostOwner===p&&!m.boostUsed,ke=[...k.MIL||[],...k.ATT||[]].filter(J=>!J.used),ne=[...L.MIL||[],...L.ATT||[]].filter(J=>!J.used),Se=!be(L),Ge=ke.length===0&&ne.length===0&&!Se,Ae=(k.DEF||[]).filter(J=>!J.used),Xe=(k.GK||[]).filter(J=>!J.used);let He=[];q&&ke.length===0&&(Se?(He=Ae.map(J=>J.cardId),Ae.length===0&&(He=He.concat(Xe.map(J=>J.cardId)))):Ge&&(He=[...Ae,...Xe].map(J=>J.cardId)));function rt(J,he,Me){var Ce,xt;const Ie=J._gcDef,Nt=(Ie==null?void 0:Ie.name)||J.gc_type,lt=(Ie==null?void 0:Ie.effect)||((Ce=Oe[J.gc_type])==null?void 0:Ce.desc)||"",Fe=Ie!=null&&Ie.image_url?`/icons/${Ie.image_url}`:null,Re=((xt=Oe[J.gc_type])==null?void 0:xt.icon)||"⚡";return`<div class="pvp-gc-mini" data-gc-id="${J.id}" data-gc-type="${J.gc_type}" style="flex-shrink:0;cursor:pointer">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${Ut(k,m[p+"Formation"],bt,ee,oe?1300:Ct(),oe?600:Ot(),He)}
          ...
          <div style="display:flex;justify-content:center">${et((Fe.players||[]).map(Ce=>({...Ce,used:!1})),"#ff6b6b",Fe.total,"attack",m[b+"Formation"])}</div>
          ${Re}
        ...
        </div>`}const J=(m.log||[]).slice(-1)[0];return J?'<div style="padding:2px 4px">'+dn(J)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3);text-align:center">⏳ Match en cours...</div>'})(),so=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
      ...
      <div style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">${vn}</div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${L.map(A=>{const N={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[A._line]||"#555",q=Pe(A,A._line)+(A.boost||0);return`<div class="bp-item" data-cid="${A.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${N};overflow:hidden;cursor:pointer">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(A);let q=!1;const D=()=>{q||(q=!0,A.remove(),setTimeout(()=>M(),50))};A.addEventListener("click",D),setTimeout(D,2200)}function F(){var L;const k=document.createElement("div");k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",k.innerHTML=`
      <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-transform:uppercase">Équipe adverse</div>
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${m[b+"Name"]}</div>
      <div style="width:min(90vw,420px)">${Ai(m[b+"Team"],m[b+"Formation"],null,[],Ct(),Ot())}</div>
      ...
      </div>`,document.body.appendChild(L),(M=L.querySelector("#pvp-hist-close"))==null||M.addEventListener("click",()=>L.remove())}async function Y(){if(m.phase!==p+"-attack")return;const k=p==="p1"?"p2":"p1",L=(m.round||0)+1,M=[...m.log||[]];M.push({type:"info",text:`⏭️ ${m[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const B=Le(m),A=We(k),N=B||!A?"finished":k+"-attack";await I({["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:N,attacker:k,round:L,log:M}),N==="finished"&&await qe(m)}function Z(k){const L=(k.GK||[]).some(B=>!B.used),M=["DEF","MIL","ATT"].some(B=>(k[B]||[]).some(A=>!A.used));return L&&!M}function se(k){return(k.p1Score||0)!==(k.p2Score||0)?null:Z(k.p1Team)&&!be(k.p2Team)?{side:"p1",gk:(k.p1Team.GK||[]).find(L=>!L.used)}:Z(k.p2Team)&&!be(k.p1Team)?{side:"p2",gk:(k.p2Team.GK||[]).find(L=>!L.used)}:null}async function ge(){const k=document.createElement("div");k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(k);const L=(M,B)=>new Promise(A=>{k.innerHTML=`<div style="font-size:32px;font-weight:900;color:${B};letter-spacing:2px;animation:lcFade 1.4s ease both">${M}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(A,1400)});await L("⚽ DERNIER CORNER","#FFD700"),await L("🧤 LE GARDIEN MONTE !","#4fc3f7"),k.remove()}async function ye(k,L){const{side:M,gk:B}=k;if(!B)return;const A=M+"Score",N=(L[A]||0)+1;B.used=!0;const q=[...L.log||[]];q.push({type:"duel",isGoal:!0,homeScored:M===p,homePlayers:M==="p1"?[Gt(B)]:[],aiPlayers:M==="p2"?[Gt(B)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${L[M+"Name"]} marque !`});const D=(L.round||0)+1,U=M==="p1"?"p2":"p1",ee={...L,[A]:N},oe=Le(ee);P.add(D),await ge();const ie=M===p?N:L[p+"Score"]||0,te=M===p?L[b+"Score"]||0:N;tt([Gt(B)],ie,te,M===p,async()=>{await I({[A]:N,log:q,round:D,pendingAttack:null,phase:oe?"finished":U+"-attack",attacker:U,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}}}),oe&&await qe({...m,[A]:N})})}async function $e(){const k=m[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(N=>(m[b+"Team"][N]||[]).some(q=>!q.used)),M=(m["selected_"+p]||[]).map(N=>{const q=(k[N._role]||[]).find(le=>le.cardId===N.cardId)||N,D=["GK","DEF"].includes(N._role),U=k[N._role]||[],ee=U.findIndex(le=>le.cardId===N.cardId),oe=dt(U.length),ie=ee>=0?oe[ee]:q._col??1,te=m.stadiumDef||m.homeStadiumDef||null,ae=q.stadiumBonus||te&&(te.club_id&&String(q.club_id)===String(te.club_id)||te.country_code&&q.country_code===te.country_code)||!1;return{...q,_line:N._role,_col:ie,stadiumBonus:ae,...D?{note_a:Math.max(1,Number(q.note_a)||0)}:{}}});if(!M.length)return;const B=si(M,m.modifiers[p]||{},m[p+"Formation"]);ce(p,M.map(N=>N.cardId)),M.forEach(N=>{const q=(k[N._role]||[]).find(D=>D.cardId===N.cardId);q&&(q.used=!0)}),m["selected_"+p]=[],H();const A=[...m.log||[]];if(L){if(M.length===1&&(M[0]._line==="GK"||M[0].job==="GK")&&(m.p1Score||0)===(m.p2Score||0)){await ye({side:p,gk:M[0]},{...m,[p+"Team"]:k,log:A});return}const q=(m[p+"Score"]||0)+1,D=M.map(te=>Gt(te));A.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:D,homeTotal:B.total,aiTotal:0});const U=(m.round||0)+1,ee=p==="p1"?"p2":"p1",oe={...m,[p+"Team"]:k,[p+"Score"]:q},ie=Le(oe);P.add(U),tt(D,q,m[b+"Score"]||0,!0,async()=>{await I({[p+"Team"]:k,[p+"Score"]:q,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:ie?"finished":ee+"-attack",attacker:ee,round:U,log:A}),ie&&await qe({...m,[p+"Score"]:q})});return}A.push({type:"pending",text:`⚔️ ${m[p+"Name"]} attaque (${B.total})`}),await I({[p+"Team"]:k,[b+"Team"]:m[b+"Team"],pendingAttack:{...B,players:M,side:p},["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},phase:b+"-defense",log:A})}async function Ee(){const k=m[p+"Team"],L=(m["selected_"+p]||[]).map(ue=>{const _e=(k[ue._role]||[]).find(Ae=>Ae.cardId===ue.cardId)||ue,ke=k[ue._role]||[],ne=ke.findIndex(Ae=>Ae.cardId===ue.cardId),Se=dt(ke.length),Ge=ne>=0?Se[ne]:_e._col??1;return{..._e,_line:ue._role,_col:Ge}}),M=li(L,m.modifiers[p]||{},m[p+"Formation"]);ce(p,L.map(ue=>ue.cardId)),L.forEach(ue=>{const _e=(k[ue._role]||[]).find(ke=>ke.cardId===ue.cardId);_e&&(_e.used=!0)}),m["selected_"+p]=[],H();const B=io(m.pendingAttack.total,M.total,m.modifiers[p]||{}),A=m.pendingAttack.side,N=B==="attack"||(B==null?void 0:B.goal),q=A==="p1"?"p2":"p1",D=(m.round||0)+1,U=(m.pendingAttack.players||[]).map(ue=>Gt(ue)),ee=[...m.log||[]];ee.push({type:"duel",isGoal:N,homeScored:N&&A===p,text:N?`⚽ BUT de ${m[A+"Name"]} ! (${m.pendingAttack.total} vs ${M.total})`:`✋ Attaque stoppée (${m.pendingAttack.total} vs ${M.total})`,homePlayers:U,aiPlayers:L.map(ue=>Gt(ue)),homeTotal:m.pendingAttack.total,aiTotal:M.total});const oe=N?(m[A+"Score"]||0)+1:m[A+"Score"]||0,ie={...m,[p+"Team"]:k,[A+"Score"]:oe},te=Le(ie),ae=te?"finished":q+"-attack",le=!te&&!N?se(ie):null,we=async()=>{if(le){await ye(le,ie);return}await I({[p+"Team"]:k,[b+"Team"]:m[b+"Team"],[A+"Score"]:oe,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:ae,attacker:q,round:D,log:ee}),(ae==="finished"||te)&&await qe({...m,[A+"Score"]:oe})};if(N){const ue=A===p,_e=ue?oe:m[p+"Score"]||0,ke=ue?m[b+"Score"]||0:oe;P.add(D),tt(U,_e,ke,ue,we)}else await we()}function xe(k){return["MIL","ATT"].some(L=>(k[L]||[]).some(M=>!M.used))}function be(k){return["GK","DEF","MIL","ATT"].some(L=>(k[L]||[]).some(M=>!M.used))}function je(k){return be(k)&&!xe(k)}function We(k){const L=m[k+"Team"],M=m[(k==="p1"?"p2":"p1")+"Team"];return!!(xe(L)||je(L)&&!be(M)||je(L)&&!xe(M)&&be(M))}function Le(k){return!be(k.p1Team)&&!be(k.p2Team)}function Be(k){const L=k.p1Score||0,M=k.p2Score||0;return L===M?null:L>M?h.home_id:h.away_id}async function qe(k){Rt(),Ke();try{const L=Be(k),M=L?h.home_id===L?h.away_id:h.home_id:null,B=k.p1Score||0,A=k.p2Score||0,{data:N,error:q}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:L,p_home_score:B,p_away_score:A,p_forfeit:!1});if(q&&console.error("[PvP] finish_pvp_match:",q),s)try{await v.rpc("check_and_finish_mini_league",{p_league_id:s})}catch(D){console.error("[PvP] check_and_finish_mini_league:",D)}L&&M&&Yr(L,M).catch(()=>{});try{const{data:D}=await v.rpc("apply_match_rewards",{p_match_id:i});D!=null&&D.success&&!(D!=null&&D.skipped)&&typeof e.refreshProfile=="function"&&await e.refreshProfile()}catch(D){console.error("[PvP] apply_match_rewards:",D)}if(typeof l=="function")try{await l({homeScore:B,awayScore:A,winnerId:L,homeId:h.home_id,awayId:h.away_id,matchId:i})}catch(D){console.error("[PvP] onMatchEnd:",D)}}catch(L){console.error("[PvP] finishMatch:",L)}}function Ne(){var A;const k=m[p+"Score"],L=m[b+"Score"],M=k>L,B=k===L;ct(t),t.innerHTML=`
    ...
    </div>`,(A=document.getElementById("pvp-home"))==null||A.addEventListener("click",()=>{try{v.removeChannel(V)}catch{}Ve(t),s?x("mini-league",{openLeagueId:s}):x("home")})}H()}async function Yi(t,e,i=!1){var d,c;const{state:o,navigate:r,toast:n}=e,a=i&&((c=(d=e==null?void 0:e.state)==null?void 0:d.params)==null?void 0:c.rankedData)||null;await zi(t,e,i?"ranked":"random",async({deckId:l,formation:s,starters:f,subsRaw:y,gcCardsEnriched:x,gcDefs:g,stadiumDef:p})=>{ct(t);const b=async u=>{await Jr(t,e,l,s,f,y,u||[],g,p,i,a)};if(!(x!=null&&x.length)){await b([]);return}Si(t,x,b)})}async function Xr(t,e,i){return Wr(t,e,i)}async function Jr(t,e,i,o,r,n,a=[],d=[],c=null,l=!1,s=null){var O,V;const{state:f,navigate:y,toast:x}=e,g=(s==null?void 0:s.mmr)??((O=f.profile)==null?void 0:O.mmr)??1e3,p=l?Ei(g):null,b=l?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",u=l?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let w=!1,h=null,E=null;t.innerHTML=`
    ...
    </div>`;const m=async()=>{w=!0,h&&(h.unsubscribe(),h=null),E&&(clearInterval(E),E=null);try{await v.rpc("cancel_matchmaking",{p_user_id:f.profile.id})}catch(I){console.error("[Matchmaking] cancel error:",I)}};(V=document.getElementById("cancel-mm"))==null||V.addEventListener("click",async()=>{await m(),Ve(t),y("home")});const S=async(I,_)=>{w||(w=!0,h&&(h.unsubscribe(),h=null),E&&(clearInterval(E),E=null),await qt(t,e,I,_,{myGC:a,gcDefs:d,stadiumDef:c,isRanked:l,rankedData:s,onMatchEnd:l?Qr:null}))},$=f.user.id,{data:T,error:P}=l?await v.rpc("try_matchmake_ranked",{p_user_id:$,p_deck_id:i,p_mmr:g,p_range:200}):await v.rpc("try_matchmake",{p_user_id:$,p_deck_id:i});if(P||!(T!=null&&T.success)){console.error("[Matchmaking] try_matchmake error:",P||T),x("Erreur de matchmaking","error"),Ve(t),y("home");return}if(T.matched){await S(T.match_id,!1);return}const G=async()=>{if(w)return;const{data:I}=await v.from("matchmaking_queue").select("status, match_id").eq("user_id",$).maybeSingle();(I==null?void 0:I.status)==="matched"&&I.match_id&&await S(I.match_id,!0)};h=v.channel(`mm_${$}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${$}`},I=>{const _=I.new;_.status==="matched"&&_.match_id&&S(_.match_id,!0)}).subscribe(),E=setInterval(G,3e3),l?setTimeout(async()=>{if(!w){w=!0,h&&(h.unsubscribe(),h=null),E&&(clearInterval(E),E=null);try{await v.rpc("cancel_matchmaking",{p_user_id:f.profile.id})}catch(I){console.error("[Matchmaking] cancel error:",I)}x("Aucun adversaire trouvé — match contre une IA calibrée sur votre niveau","info",4e3),y("match",{matchMode:"ranked_ai",rankedData:s,presetSetup:{deckId:i,formation:o,starters:r,subsRaw:n,gcCardsEnriched:a,gcDefs:d,stadiumDef:c}})}},2e4):setTimeout(()=>{w||m()},12e4)}async function Qr(t,e){const{state:i,toast:o}=e,{winnerId:r,homeId:n,awayId:a,homeScore:d,awayScore:c,matchId:l}=t,s=i.user.id,f=n===s,y=r===s,{data:x}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",s).single(),{data:g}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",f?a:n).single();if(x&&g){const p=Do(x.mmr,x.mmr_rd,x.mmr_v,g.mmr,g.mmr_rd,y?1:0);await v.from("users").update({mmr:p.mmr,mmr_rd:p.rd,mmr_v:p.v,mmr_wins:y?v.sql`mmr_wins + 1`:void 0,mmr_losses:y?void 0:v.sql`mmr_losses + 1`}).eq("id",s),o(y?`+MMR ↑ ${Math.round(p.mmr-x.mmr)}`:`-MMR ↓ ${Math.round(x.mmr-p.mmr)}`,y?"success":"error",4e3)}}const Zr=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:Yi,resumePvpMatch:Xr},Symbol.toStringTag,{value:"Module"}));async function ea(t,e){const{state:i,navigate:o,toast:r}=e,n=i.params||{},a=n.friendId||null,d=n.friendName||"Ami",c=!!n.isAccepting;await zi(t,e,"friend",async({deckId:l,formation:s,starters:f,subsRaw:y,gcCardsEnriched:x,gcDefs:g,stadiumDef:p})=>{ct(t);const b=async u=>{const w=u||[];c?await ia(t,e,l,w,g,p):await ta(t,e,l,w,g,p,a,d)};if(!(x!=null&&x.length)){await b([]);return}Si(t,x,b)})}async function ta(t,e,i,o,r,n,a,d){var w;const{state:c,navigate:l,toast:s}=e,f=c.user.id,{data:y,error:x}=await v.from("friend_match_invites").insert({inviter_id:f,invitee_id:a,friend_id:a,status:"pending",inviter_deck_id:i,expires_at:new Date(Date.now()+2*60*1e3).toISOString()}).select("id").single();if(x||!y){console.error("[Friend] Erreur création invitation:",x),s("Impossible de créer l'invitation","error"),l("home");return}t.innerHTML=`
    ...
    </div>`;let g=!1,p=null;const b=async()=>{p&&(p.unsubscribe(),p=null)};(w=document.getElementById("cancel-friend"))==null||w.addEventListener("click",async()=>{await b(),await v.from("friend_match_invites").update({status:"declined"}).eq("id",y.id),Ve(t),l("home")}),p=v.channel(`friend_invite_${y.id}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${y.id}`},async({new:h})=>{g||h.status!=="accepted"||!h.match_id||(g=!0,await b(),await qt(t,e,h.match_id,!0,{myGC:o,gcDefs:r,stadiumDef:n}))}).subscribe();const u=setInterval(async()=>{if(g)return;const{data:h}=await v.from("friend_match_invites").select("status, match_id").eq("id",y.id).single();(h==null?void 0:h.status)==="accepted"&&h.match_id&&(g=!0,clearInterval(u),await b(),await qt(t,e,h.match_id,!0,{myGC:o,gcDefs:r,stadiumDef:n}))},3e3);setTimeout(async()=>{g||(clearInterval(u),await b(),s("Invitation expirée","info"),l("home"))},12e4)}async function ia(t,e,i,o,r,n){const{state:a,navigate:d,toast:c}=e,l=a.user.id,{data:s}=await v.from("friend_match_invites").select("id, inviter_id").eq("invitee_id",l).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle();if(!s){c("Aucune invitation en attente","error"),d("home");return}const{data:f,error:y}=await v.rpc("accept_friend_invite",{p_invite_id:s.id,p_invitee_deck_id:i});if(y||!(f!=null&&f.success)){console.error("[Friend] Erreur accept_friend_invite:",y||f),c((f==null?void 0:f.error)||"Impossible de rejoindre le match","error"),d("home");return}await qt(t,e,f.match_id,!1,{myGC:o,gcDefs:r,stadiumDef:n})}const mn="#1A6B3C",pt="#D4A017",Zt="var(--tile-bg)",ei="var(--tile-border)",At="var(--divider)",Ze="var(--tile-fg-on-page)",St="var(--tile-fg-dim)",it="var(--tile-fg-faint)",ii="var(--nav-fg,#fff)",gn="rgba(255,255,255,0.62)",oa="rgba(255,255,255,0.4)";async function na(t,e){var o;t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const i=(o=e.state.params)==null?void 0:o.openLeagueId;if(i){e.state.params.openLeagueId=null,await ft(t,e,i);return}await ot(t,e)}async function ot(t,e,i="waiting"){var h,E;const{state:o}=e,r=o.profile.id,{data:n}=await v.from("mini_league_members").select("league_id, prize_amount, prize_claimed").eq("user_id",r),a=(n||[]).map(m=>m.league_id),d={};(n||[]).forEach(m=>{d[m.league_id]={amount:m.prize_amount,claimed:m.prize_claimed}});const{data:c,error:l}=await v.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").eq("is_archived",!1).order("created_at",{ascending:!1}).limit(30),s=l?(await v.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").order("created_at",{ascending:!1}).limit(30)).data||[]:c||[],y=(a.length?await v.from("mini_leagues").select("*, mini_league_members(count)").in("id",a).order("created_at",{ascending:!1}):{data:[]}).data||[],x=y.filter(m=>m.status==="waiting"&&!m.is_archived),g=y.filter(m=>m.status==="active"&&!m.is_archived),p=y.filter(m=>m.is_archived||m.status==="finished");let b=p;if(p.length){const{data:m}=await v.from("mini_league_matches").select("league_id").in("league_id",p.map($=>$.id)).or(`home_id.eq.${r},away_id.eq.${r}`).not("status","eq","bye"),S=new Set((m||[]).map($=>$.league_id));b=p.filter($=>S.has($.id))}const u=s.filter(m=>!a.includes(m.id)),w=[{key:"waiting",label:"🟡 En attente",count:x.length+u.length},{key:"active",label:"🟢 En cours",count:g.length},{key:"archived",label:"📁 Archivées",count:b.length}];t.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <div style="padding:14px 16px;background:var(--nav-bg,#0d1a0f);border-bottom:1px solid ${At};display:flex;align-items:center;justify-content:space-between">
      <div>
        ...
        <div style="font-size:12px;color:${gn}">Championnats 3 à 8 joueurs</div>
      </div>
      <div style="display:flex;align-items:center;gap:8px">
  ...
  </div>`}function ra(t,e,i){const o=[];return t.length&&(o.push(`<div style="font-size:11px;font-weight:700;color:${it};text-transform:uppercase;letter-spacing:1px">Mes leagues en attente</div>`),o.push(...t.map(r=>$i(r,i,!1)))),e.length&&(o.push(`<div style="font-size:11px;font-weight:700;color:${it};text-transform:uppercase;letter-spacing:1px;margin-top:4px">Rejoindre</div>`),o.push(...e.map(r=>$i(r,i,!0)))),o.length?o.join(""):`<div style="text-align:center;padding:40px;color:${it}">🏆<br>Aucune mini league.<br>Crée la première !</div>`}function aa(t,e){return t.length?t.map(i=>$i(i,e)).join(""):`<div style="text-align:center;padding:40px;color:${it}">Aucune mini league en cours.</div>`}function sa(t,e,i={}){return t.length?t.map(o=>$i(o,e,!1,i[o.id])).join(""):`<div style="text-align:center;padding:40px;color:${it}">Aucune mini league archivée.</div>`}function la(t,e){const i=document.createElement("div");i.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px",i.innerHTML=`<div style="background:#fff;border-radius:16px;width:100%;max-width:400px;max-height:90vh;overflow-y:auto">
    ...
    <div style="padding:20px;display:flex;flex-direction:column;gap:14px">
    ...
    </div></div>`,document.body.appendChild(i),i.querySelector("#ml-form-close").addEventListener("click",()=>i.remove()),i.querySelectorAll('input[name="ml-type"]').forEach(o=>o.addEventListener("change",()=>{document.getElementById("ml-pwd-block").style.display=o.value==="private"?"block":"none"})),i.querySelector("#ml-create-confirm").addEventListener("click",async()=>{var g,p,b,u;const{toast:o}=e,r=document.getElementById("ml-name").value.trim(),n=parseInt(document.getElementById("ml-max").value)||6,a=parseInt(document.getElementById("ml-fee").value)||500,d=((g=i.querySelector('input[name="ml-type"]:checked'))==null?void 0:g.value)||"public",c=((p=i.querySelector('input[name="ml-mode"]:checked'))==null?void 0:p.value)||"aller",l=((u=(b=document.getElementById("ml-pwd"))==null?void 0:b.value)==null?void 0:u.trim())||null;if(!r){o("Le nom est obligatoire","error");return}if(a<100){o("Mise minimum : 100 crédits","error");return}if(d==="private"&&!l){o("Mot de passe requis","error");return}const{data:s,error:f}=await v.from("mini_leagues").insert({name:r,creator_id:e.state.profile.id,type:d,password:l,mode:c,max_players:n,entry_fee:a}).select().single();if(f){o("Erreur : "+f.message,"error");return}const{data:y}=await v.from("users").select("credits").eq("id",e.state.profile.id).single();if(((y==null?void 0:y.credits)||0)<a){await v.from("mini_leagues").delete().eq("id",s.id),o(`Crédits insuffisants pour la mise (${a.toLocaleString("fr")} cr.)`,"error");return}await v.from("users").update({credits:y.credits-a}).eq("id",e.state.profile.id),await v.from("mini_leagues").update({pot:a}).eq("id",s.id),await v.from("mini_league_members").insert({league_id:s.id,user_id:e.state.profile.id}),e.state.profile&&(e.state.profile.credits=y.credits-a);const x=document.getElementById("nav-credits");x&&(x.textContent=`💰 ${(e.state.profile.credits||0).toLocaleString("fr")}`),i.remove(),o(`Mini League créée ! ${a.toLocaleString("fr")} cr. déduits.`,"success"),ft(t,e,s.id)})}function da(){return new Promise(t=>{const e=document.createElement("div");e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9500;display:flex;align-items:center;justify-content:center;padding:20px",e.innerHTML=`
      ...
      </div>`,document.body.appendChild(e);const i=e.querySelector("#ml-pwd-input"),o=r=>{e.remove(),t(r)};setTimeout(()=>i==null?void 0:i.focus(),80),e.querySelector("#pwd-cancel").addEventListener("click",()=>o(null)),e.querySelector("#pwd-ok").addEventListener("click",()=>o(i.value.trim())),i.addEventListener("keydown",r=>{r.key==="Enter"&&o(i.value.trim())})})}async function bn(t,e,i,o){var f;const{toast:r,state:n}=e,a=n.profile.id;let d=null;if(o==="private"&&(d=await da(),d===null))return;const{data:c,error:l}=await v.rpc("join_mini_league",{p_league_id:i,p_user_id:a,p_password:d});if(l||!(c!=null&&c.success)){console.error("[MiniLeague] join_mini_league:",l||c),r((c==null?void 0:c.error)||"Erreur lors de l'inscription","error");return}if(typeof e.refreshProfile=="function")await e.refreshProfile();else{const{data:y}=await v.from("users").select("credits").eq("id",a).single();n.profile&&y&&(n.profile.credits=y.credits)}const s=document.getElementById("nav-credits");s&&(s.textContent=`💰 ${(((f=n.profile)==null?void 0:f.credits)||0).toLocaleString("fr")}`),r("✅ Inscrit ! Pot mis à jour.","success"),ft(t,e,i)}async function ca(t,e,i,o){const{toast:r,state:n}=e,a=n.profile.id;if(!await new Promise(y=>{const x=document.createElement("div");x.className="modal-overlay",x.style.zIndex="2100",x.innerHTML=`<div class="modal" style="max-width:360px">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <div style="padding:14px 16px;background:var(--nav-bg,#0d1a0f);border-bottom:1px solid ${At};display:flex;align-items:center;gap:10px">
      ...
      <div style="flex:1">
        ...
        <div style="font-size:11px;color:${gn}">${a.mode==="aller-retour"?"Aller-Retour":"Aller"} · max ${a.max_players} · 💰 ${p} cr./joueur</div>
      </div>
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Zt};border:1px solid ${ei};border-radius:12px;padding:16px">
        ...
        ${f.map(I=>`
          <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid ${At}">
            <div style="width:36px;height:36px;border-radius:50%;background:${I.club_color2||mn};display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:${I.club_color1||"#fff"}">${(I.pseudo||"?").slice(0,2).toUpperCase()}</div>
      ...
      <div style="background:${Zt};border:1px solid ${ei};border-radius:12px;padding:16px">
        ...
        ${x.map(I=>Ao(I,f,n,l)).join("")}
        ${s&&b&&!u?'<button id="ml-next-day" class="btn btn-primary" style="width:100%;margin-top:12px">➡️ Journée suivante</button>':""}
      </div>`:""}
      ...
      <div style="background:${Zt};border:1px solid ${ei};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(I,_)=>_+1).reverse().map(I=>{const _=(c||[]).filter(R=>R.matchday===I);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${it};margin-bottom:6px">Journée ${I}</div>${_.map(R=>Ao(R,f,n,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  </div>`}async function ua(t,e,i,o){const{toast:r,state:n}=e,a=ga(o.map(l=>l.id),i.mode),d=[];a.forEach((l,s)=>l.forEach(f=>d.push({league_id:i.id,matchday:s+1,home_id:f.home||f.bye,away_id:f.away||null,is_bye:!!f.bye,status:f.bye?"bye":"pending"})));const{error:c}=await v.from("mini_league_matches").insert(d);if(c){r("Erreur calendrier : "+c.message,"error");return}await v.from("mini_leagues").update({status:"active",current_day:1,total_days:a.length}).eq("id",i.id),r(`🚀 Lancée ! Pot : ${(i.pot||0).toLocaleString("fr")} cr.`,"success"),ft(t,e,i.id)}async function fa(t,e,i){const{data:o}=await v.from("mini_leagues").select("current_day,total_days,pot").eq("id",i).single(),r=(o.current_day||0)+1;if(r>(o.total_days||0)){ft(t,e,i);return}await v.from("mini_leagues").update({current_day:r}).eq("id",i),e.toast(`Journée ${r} commencée !`,"success"),ft(t,e,i)}async function ma(t,e,i,o,r){var y,x;const{state:n,toast:a}=e,d=[Math.floor((i.pot||0)*.7),Math.floor((i.pot||0)*.2),Math.floor((i.pot||0)*.1)],c=["🥇","🥈","🥉"][r],l=o.prize_amount||d[r]||0,s=o.prize_claimed,f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",f.innerHTML=`
    ...
    </div>`,document.body.appendChild(f),(y=f.querySelector("#prize-close"))==null||y.addEventListener("click",()=>f.remove()),(x=f.querySelector("#claim-prize-btn"))==null||x.addEventListener("click",async g=>{const p=g.currentTarget;p.disabled=!0,p.textContent="...";const{data:b,error:u}=await v.rpc("claim_mini_league_prize",{p_league_id:i.id,p_user_id:n.profile.id});if(u||!(b!=null&&b.success)){console.error("[MiniLeague] claim_mini_league_prize:",u||b),a((b==null?void 0:b.error)||"Erreur lors de la récupération","error"),p.disabled=!1,p.textContent=`💰 Récupérer ${l.toLocaleString("fr")} cr.`;return}if(b.already_claimed)a("Déjà récupéré précédemment","info");else{const w=(n.profile.credits||0)+b.prize;n.profile&&(n.profile.credits=w);const h=document.getElementById("nav-credits");h&&(h.textContent=`💰 ${w.toLocaleString("fr")}`),a(`💰 +${b.prize.toLocaleString("fr")} cr. ajoutés à ton solde !`,"success")}f.remove(),ft(t,e,i.id)})}function ga(t,e){const o=t.length%2===0?[...t]:[...t,null],r=o.length;let n=o.slice(1);const a=[];for(let d=0;d<r-1;d++){const c=[],l=[o[0],...n];for(let s=0;s<r/2;s++){const f=l[s],y=l[r-1-s];f===null?c.push({bye:y}):y===null?c.push({bye:f}):c.push({home:f,away:y})}a.push(c),n=[n[n.length-1],...n.slice(0,-1)]}return e==="aller-retour"?[...a,...a.map(d=>d.map(c=>c.bye?c:{home:c.away,away:c.home}))]:a}function ba(t,e){const i={};return t.forEach(o=>{i[o.id]={userId:o.id,pseudo:o.pseudo,clubName:o.club_name||o.pseudo,played:0,won:0,drawn:0,lost:0,goalsFor:0,goalsAgainst:0,goalDiff:0,points:0}}),e.filter(o=>o.status==="finished"&&!o.is_bye&&o.home_score!=null).forEach(o=>{const r=i[o.home_id],n=i[o.away_id];!r||!n||(r.played++,n.played++,r.goalsFor+=o.home_score,r.goalsAgainst+=o.away_score,n.goalsFor+=o.away_score,n.goalsAgainst+=o.home_score,o.home_score>o.away_score?(r.won++,r.points+=3,n.lost++):o.home_score<o.away_score?(n.won++,n.points+=3,r.lost++):(r.drawn++,r.points++,n.drawn++,n.points++),r.goalDiff=r.goalsFor-r.goalsAgainst,n.goalDiff=n.goalsFor-n.goalsAgainst)}),Object.values(i).sort((o,r)=>r.points-o.points||r.goalDiff-o.goalDiff||r.goalsFor-o.goalsFor)}async function zo(t,e){const{state:i,navigate:o,toast:r}=e,n=i.params||{},a=n.leagueId||null,d=n.mlMatchId||null,c=i.user.id;if(!d||!a){r("Match introuvable","error"),o("mini-league");return}const{data:l,error:s}=await v.from("mini_league_matches").select("id, league_id, home_id, away_id, status, match_id").eq("id",d).single();if(s||!l){r("Match introuvable","error"),o("mini-league",{openLeagueId:a});return}if(l.home_id!==c&&l.away_id!==c){r("Vous ne faites pas partie de ce match","error"),o("mini-league",{openLeagueId:a});return}const f=l.home_id===c;if(l.match_id){await qt(t,e,l.match_id,f,{mlLeagueId:a,mlMatchId:d});return}await zi(t,e,"mini_league",async({deckId:y,gcCardsEnriched:x,gcDefs:g,stadiumDef:p})=>{ct(t);const b=async u=>{const{data:w,error:h}=await v.rpc("start_mini_league_match",{p_ml_match_id:d,p_user_id:c,p_deck_id:y});if(h||!(w!=null&&w.success)){console.error("[MiniLeague] start_mini_league_match error:",h||w),r((w==null?void 0:w.error)||"Impossible de lancer le match","error"),o("mini-league",{openLeagueId:a});return}if(w.matched){await qt(t,e,w.match_id,f,{mlLeagueId:a,mlMatchId:d,myGC:u||[],gcDefs:g,stadiumDef:p});return}await xa(t,e,{mlMatchId:d,leagueId:a,amIHome:f,chosenGC:u,gcDefs:g,stadiumDef:p})};if(!(x!=null&&x.length)){await b([]);return}Si(t,x,b)})}function xa(t,e,{mlMatchId:i,leagueId:o,amIHome:r,chosenGC:n,gcDefs:a,stadiumDef:d}){return new Promise(c=>{var p;const{navigate:l}=e;t.innerHTML=`
    <div style="min-height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;background:linear-gradient(135deg,#0a1a2e,#0d3d1e);color:#fff;padding:32px;text-align:center;gap:20px">
      <div style="font-size:36px">🏆</div>
    ...
    </div>`;let s=!1;const f=async b=>{if(!s){if(s=!0,clearInterval(g),x)try{x.unsubscribe()}catch{}b&&await qt(t,e,b,r,{mlLeagueId:o,mlMatchId:i,myGC:n||[],gcDefs:a,stadiumDef:d}),c()}};(p=document.getElementById("wait-back"))==null||p.addEventListener("click",()=>{if(s=!0,clearInterval(g),x)try{x.unsubscribe()}catch{}l("mini-league",{openLeagueId:o}),c()});const y=async()=>{if(s)return;const{data:b}=await v.from("mini_league_matches").select("match_id").eq("id",i).single();b!=null&&b.match_id&&f(b.match_id)},x=v.channel(`ml_match_${i}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"mini_league_matches",filter:`id=eq.${i}`},b=>{var u;(u=b.new)!=null&&u.match_id&&f(b.new.match_id)}).subscribe(),g=setInterval(y,3e3);y()})}const ya="/",ha=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function va(t,e,i){let o=0;const r=document.createElement("div");r.id="tutorial-overlay",r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const n=()=>{var s,f,y;const d=e[o],c=o===e.length-1,l=Math.round((o+1)/e.length*100);r.innerHTML=`
      <div style="background:#fff;border-radius:20px;width:100%;max-width:420px;max-height:90vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 16px 64px rgba(0,0,0,0.4)">
        ...
        <!-- Image optionnelle -->
        ${d.image_url?`<div style="padding:0 24px 8px;text-align:center"><img src="${ya}icons/${d.image_url}" style="max-height:160px;max-width:100%;border-radius:12px;object-fit:contain"></div>`:""}
        <!-- Contenu -->
    ...
    `,r.querySelectorAll("ul,ol").forEach(x=>{x.style.paddingLeft="20px",x.style.marginTop="6px",x.style.marginBottom="6px"}),r.querySelectorAll("li").forEach(x=>{x.style.marginBottom="4px"}),r.querySelectorAll("p").forEach(x=>{x.style.marginBottom="8px"}),(s=r.querySelector("#tuto-prev"))==null||s.addEventListener("click",()=>{o--,n()}),(f=r.querySelector("#tuto-next"))==null||f.addEventListener("click",()=>{c?a():(o++,n())}),(y=r.querySelector("#tuto-skip"))==null||y.addEventListener("click",()=>{confirm("Passer le tutoriel ? Tu pourras le revoir plus tard depuis les paramètres.")&&a()})},a=async()=>{r.remove(),t!=null&&t.id&&await v.from("users").update({tutorial_done:!0}).eq("id",t.id),i==null||i()};document.body.appendChild(r),n()}async function Mo(t,e,i){if(!t||t.tutorial_done)return;let o=[];const{data:r,error:n}=await v.rpc("get_tutorial_steps");if(!n&&(r==null?void 0:r.length)>0)o=r,console.log(`[Tutorial] RPC OK → ${o.length} étapes`);else{const{data:d,error:c}=await v.from("tutorial_steps").select("*").eq("is_active",!0).order("step_order");!c&&(d==null?void 0:d.length)>0?(o=d,console.log(`[Tutorial] Direct OK → ${o.length} étapes`)):(console.warn(`[Tutorial] Aucune étape DB (RPC: ${n==null?void 0:n.message}, Direct: ${c==null?void 0:c.message})`),i&&i("[Tutorial] DB vide ou inaccessible — tuto local utilisé","warning",5e3))}const a=o.length>0?o.map(d=>({emoji:d.emoji,title:d.title,color:d.color,content:d.content,image_url:d.image_url||null})):ha;va(t,a,()=>e("boosters"))}const Co={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function bi(t,e,i=0){return e?(Number(e==="GK"?t.note_g:e==="DEF"?t.note_d:e==="MIL"?t.note_m:t.note_a)||0)+(e===t.job||e===t.job2?i:0):0}async function wa(t,e){t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',await ao(t,e)}async function ao(t,e){const{state:i,toast:o}=e,{data:r}=await v.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let c="buy";const l=()=>{var p,b,u,w,h,E,m;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((b=document.getElementById("flt-club"))==null?void 0:b.value)||"").toLowerCase().trim(),country:(((u=document.getElementById("flt-country"))==null?void 0:u.value)||"").toLowerCase().trim(),job:((w=document.getElementById("flt-job"))==null?void 0:w.value)||"",rarity:((h=document.getElementById("flt-rarity"))==null?void 0:h.value)||"",note1:parseInt((E=document.getElementById("flt-note1"))==null?void 0:E.value)||0,note2:parseInt((m=document.getElementById("flt-note2"))==null?void 0:m.value)||0}};function s(p){const b=l();return p.filter(u=>{var P,G,O;const w=(P=u.card)==null?void 0:P.player;if(!w)return!1;const h=`${w.firstname} ${w.surname_real}`.toLowerCase(),E=(((G=w.clubs)==null?void 0:G.encoded_name)||"").toLowerCase(),m=(w.country_code||"").toLowerCase(),S=((O=u.card)==null?void 0:O.evolution_bonus)||0,$=bi(w,w.job,S),T=w.job2?bi(w,w.job2,S):0;return!(b.name&&!h.includes(b.name)||b.club&&!E.includes(b.club)||b.country&&!m.includes(b.country)||b.job&&w.job!==b.job||b.rarity&&w.rarity!==b.rarity||b.note1&&$<b.note1||b.note2&&T<b.note2)})}function f(p){var E,m,S;const b=(E=p.card)==null?void 0:E.player;if(!b)return"";const u=((m=p.card)==null?void 0:m.evolution_bonus)||0,w=(i.profile.credits||0)>=p.price;return`<div class="mkt-buy-tile">
      ${ze({...b,_evolution_bonus:u},{width:140})}
    ...
    </div>`}function y(p){var G,O,V,I;const b=(G=p.card)==null?void 0:G.player;if(!b)return"";const u=((O=p.card)==null?void 0:O.evolution_bonus)||0,w=bi(b,b.job,u),h=b.job2?bi(b,b.job2,u):0,E=p.status==="sold",m=b.country_code?`https://flagsapi.com/${b.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,S=Co[b.job]||"#bbb",$=b.job2?Co[b.job2]||"#bbb":null,T=b.job==="GK"?"#fff":S,P=b.job2==="GK"?"#fff":$;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${E?"opacity:0.7":""}">
      ${m?`<img src="${m}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(V=b.clubs)!=null&&V.logo_url?`<img src="${b.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
  ...
  <div class="page">
  ...
  </div>`,f(),(p=document.getElementById("tab-ranked"))==null||p.addEventListener("click",()=>{c="ranked",f()}),(b=document.getElementById("tab-mini-league"))==null||b.addEventListener("click",()=>{c="mini-league",f()}),(u=document.getElementById("tab-random"))==null||u.addEventListener("click",()=>{c="random",f()})}async function La(t,e){var S,$,T,P;const{state:i,navigate:o,toast:r}=e,n=i.profile;t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const[{data:a},{data:d}]=await Promise.all([v.from("ranked_seasons").select("*").eq("is_active",!0).maybeSingle(),v.from("users").select("id,pseudo,club_name,mmr,mmr_deviation,mmr_volatility,rank_tier,placement_matches,ranked_wins,ranked_losses,ranked_draws").eq("id",n.id).single()]);if(!d){r("Erreur chargement profil","error"),o("home");return}if(!a){t.innerHTML=`
    ...
    </div>`,(S=document.getElementById("ranked-back"))==null||S.addEventListener("click",()=>o("home"));return}const c=d.mmr??1e3,l=d.mmr_deviation??350,s=d.mmr_volatility??.06,f=d.placement_matches??0,y=f<10,x=Math.max(0,10-f),g=Ei(c),p=qo(c),b=Bi.findIndex(G=>G.id===g.id),u=Bi[b+1]||null,w={bronze:"linear-gradient(160deg,#3d1c00,#7a3e00)",silver:"linear-gradient(160deg,#1a1a2e,#3a3a5e)",gold:"linear-gradient(160deg,#1a1200,#4a3500)",platinum:"linear-gradient(160deg,#001a20,#003040)",diamond:"linear-gradient(160deg,#001030,#1a2860)",master:"linear-gradient(160deg,#1a0030,#3d0070)"},h=(d.ranked_wins||0)+(d.ranked_losses||0)+(d.ranked_draws||0),E=h>0?Math.round((d.ranked_wins||0)/h*100):0,m=Bi.map(G=>`
  ...
  </div>`}wn(kn);const ve={user:null,profile:null,page:"home",params:{}};function It(t,e="info",i=3e3){const o=document.getElementById("toast");o&&(o.textContent=t,o.className=`show ${e}`,clearTimeout(o._t),o._t=setTimeout(()=>{o.className=""},i))}function Ta(t,e,i=""){document.getElementById("modal-title").textContent=t,document.getElementById("modal-body").innerHTML=e,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function Xi(){document.getElementById("modal-overlay").classList.add("hidden")}async function ti(){if(!ve.user)return;const{data:t}=await v.from("users").select("*").eq("id",ve.user.id).single();t&&(ve.profile=t)}const xn="mw_theme";function oi(){return localStorage.getItem(xn)||"club"}function jo(t){var e;localStorage.setItem(xn,t),xi(t),(e=ve.profile)!=null&&e.id&&v.from("users").update({theme:t}).eq("id",ve.profile.id).then(()=>{})}function xi(t){var e,i;document.documentElement.setAttribute("data-theme",t),t==="club"&&(document.documentElement.style.setProperty("--club-color1",((e=ve.profile)==null?void 0:e.club_color1)||"#0a0f0a"),document.documentElement.style.setProperty("--club-color2",((i=ve.profile)==null?void 0:i.club_color2)||"#080d08"))}function zt(t,e={}){ve.page=t,ve.params=e,yn()}async function yn(){var o,r,n,a;const t=document.getElementById("page-content");if(!t)return;document.querySelectorAll(".bottom-nav a").forEach(d=>{d.classList.toggle("active",d.dataset.page===ve.page)});const e=document.getElementById("nav-credits");e&&ve.profile&&(e.textContent=`💰 ${(ve.profile.credits||0).toLocaleString("fr")}`);const i={state:ve,navigate:zt,toast:It,openModal:Ta,closeModal:Xi,refreshProfile:ti};switch(t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',ve.page){case"home":await qi(t,i);break;case"home2":await qi(t,i);break;case"game":await Kn(t,i);break;case"settings":await Yo(t,i);break;case"collection":await Zn(t,i);break;case"decks":await Gi(t,i);break;case"boosters":await fr(t,i);break;case"ranked":await La(t,i);break;case"match":{const d=ve.params&&ve.params.matchMode||"vs_ai_easy";d==="random"?await Yi(t,i,!1):d==="ranked"?await Yi(t,i,!0):d==="friend"?await ea(t,i,(o=ve.params)==null?void 0:o.friendId,(r=ve.params)==null?void 0:r.friendName):d==="mini_league"||d==="mini-league"?await zo(t,i,(n=ve.params)==null?void 0:n.mlMatchId,(a=ve.params)==null?void 0:a.leagueId):await Tr(t,i);break}case"market":await wa(t,i);break;case"rankings":await Ea(t,i);break;case"matches":await Ia(t,i);break;case"friends":await Rn(t,i);break;case"mini-league":await na(t,i);break;case"match-mini-league":{const d=ve.params||{};await zo(t,i,d.mlMatchId,d.leagueId);break}default:await qi(t,i)}}function Sa(){var o,r;const t=document.getElementById("app"),e=ve.profile;if(!e)return;const i="/icons/";t.innerHTML=`
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.29-1012" style="cursor:pointer">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(n=>{n.addEventListener("click",a=>{a.preventDefault(),zt(n.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>zt("home")),document.getElementById("nav-credits").addEventListener("click",()=>zt("boosters")),(o=document.getElementById("journal-btn"))==null||o.addEventListener("click",()=>Aa()),(r=document.getElementById("settings-btn"))==null||r.addEventListener("click",()=>zt("settings"))}async function Aa(){const{data:t}=await v.from("patch_notes").select("*").eq("is_published",!0).order("published_at",{ascending:!1}).limit(20),e=document.createElement("div");e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px";const i=(t||[]).map(o=>{const r=new Date(o.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"long",year:"numeric"});return`<div style="padding:14px 0;border-bottom:1px solid #f0f0f0">
      ${o.image_url?`<img src="${o.image_url}" style="width:100%;max-height:160px;object-fit:cover;border-radius:8px;margin-bottom:10px">`:""}
      <div style="font-size:12px;color:#999;margin-bottom:4px">${r}</div>
      <div style="font-size:15px;font-weight:900;margin-bottom:6px">${o.title}</div>
      <div style="font-size:13px;color:#444;line-height:1.6;white-space:pre-line">${o.description}</div>
    ...
    </div>`,document.body.appendChild(e),e.querySelector("#journal-close").addEventListener("click",()=>e.remove()),e.addEventListener("click",o=>{o.target===e&&e.remove()})}function za(t,{onPlay:e}){var o;const i="/icons/";t.style.cssText="",t.innerHTML=`
  <style>
    #public-landing{
  ...
  <div id="public-landing">
  ...
  </div>`,(o=t.querySelector("#pl-play-btn"))==null||o.addEventListener("click",e)}async function Ma(){xi(oi()),document.getElementById("modal-overlay").addEventListener("click",i=>{i.target===i.currentTarget&&Xi()}),document.getElementById("modal-close").addEventListener("click",Xi);const{data:{session:t}}=await v.auth.getSession();if(!t){Bo(),za(document.getElementById("app"),{onPlay:()=>go(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:It})});return}ve.user=t.user,await ti(),xi(oi()),Bo();try{const{data:i}=await v.from("formation_links_overrides").select("formation, links"),o={};(i||[]).forEach(r=>{o[r.formation]=r.links}),_n(o)}catch(i){console.warn("Impossible de charger les overrides de formation:",i)}if(!ve.profile){En(document.getElementById("app"),{state:ve,navigate:async()=>{await ti(),xi(oi()),Pi()},toast:It,refreshProfile:ti});return}const e=Array.isArray(ve.profile.pending_boosters)?ve.profile.pending_boosters:[];if(!ve.profile.onboarding_done&&e.length>0){wr(document.getElementById("app"),{state:ve,toast:It,refreshProfile:ti,navigate:()=>{Pi(),setTimeout(()=>Mo(ve.profile,zt,It),800)}});return}Pi(),setTimeout(()=>Mo(ve.profile,zt,It),800),v.auth.onAuthStateChange(async(i,o)=>{i==="SIGNED_OUT"&&(ve.user=null,ve.profile=null,document.getElementById("app").innerHTML="",go(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:It}))})}function Ca(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function Mi(){const t=document.getElementById("app");t&&(t.style.height=Ca()+"px")}window.addEventListener("resize",Mi);window.addEventListener("orientationchange",()=>setTimeout(Mi,150));window.visualViewport&&window.visualViewport.addEventListener("resize",Mi);function Pi(){const t=()=>{var i;(i=ve.user)!=null&&i.id&&v.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",ve.user.id).then(()=>{})};t(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(t,6e4);const e=document.getElementById("app");e.style.display="flex",e.style.flexDirection="column",Mi(),Sa(),yn()}function Bo(){const t=document.getElementById("app-loader"),e=document.getElementById("app");e&&(e.style.display=""),t&&(t.classList.add("zoom-out"),setTimeout(()=>t.style.display="none",500))}function hn(t){var o;const e=document.getElementById("app-loader");if(e&&(e.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
    ...
    <button id="boot-retry" style="margin-top:8px;padding:12px 30px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer">Réessayer</button>`,document.body.appendChild(i),(o=document.getElementById("boot-retry"))==null||o.addEventListener("click",()=>window.location.reload())}Ma().catch(t=>{console.error("Échec du démarrage:",t),hn()});setTimeout(()=>{const t=document.getElementById("app-loader");t&&t.style.display!=="none"&&!t.classList.contains("zoom-out")&&!document.getElementById("boot-error")&&hn("Le serveur met trop de temps à répondre.")},12e3);
