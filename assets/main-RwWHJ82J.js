import{s as k,l as J,F as ke,b as ge,c as He,d as ut}from"./formation-links-BrrCMBql.js";function je(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),n=(l||[]).filter(A=>A.card_type==="player"&&A.player),c=(l||[]).filter(A=>A.card_type==="game_changer"),p=(l||[]).filter(A=>A.card_type==="formation"),f=Object.keys(ke),u=Object.keys(ie),x={};n.forEach(A=>{const L=A.player.id;x[L]=(x[L]||0)+1});const m=new Set(Object.keys(x).map(A=>String(A))),b=new Set(p.map(A=>A.formation)),$=new Set(c.map(A=>A.gc_type));let y="player",g="Tous",w="",E=!1;function h(){return[...n].sort((A,L)=>{const M=re.indexOf(A.player.job),B=re.indexOf(L.player.job);return M!==B?M-B:(A.player.surname_encoded||"").localeCompare(L.player.surname_encoded||"")})}function v(){return[...r||[]].sort((A,L)=>{const M=re.indexOf(A.job),B=re.indexOf(L.job);return M!==B?M-B:(A.surname_encoded||"").localeCompare(L.surname_encoded||"")})}function _(){return h().filter(A=>{const L=A.player,M=g==="Tous"||L.job===g,B=!w||`${L.firstname} ${L.surname_encoded}`.toLowerCase().includes(w);return M&&B})}function T(){return v().filter(A=>{const L=g==="Tous"||A.job===g,M=!w||`${A.firstname} ${A.surname_encoded}`.toLowerCase().includes(w);return L&&M})}e.innerHTML=`
  <div class="page">
    ...
    <div class="page-body">
      <div style="display:flex;flex-wrap:wrap;gap:10px;justify-content:flex-start" id="col-grid"></div>
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{E=!E,I(),z()})))}function z(){const A=document.getElementById("col-grid");A&&(y==="player"?C(A):y==="formation"?N(A):q(A))}function C(A){if(E){const L=T();if(!L.length){A.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}A.innerHTML=L.map(M=>{if(m.has(String(M.id))){const S=n.find(H=>H.player.id===M.id),G=x[M.id]||1,O=G>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${G}</div>`:"",K=n.filter(H=>H.player.id===M.id&&H.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Fe(S,O+K)}return vt(M)}).join("")}else{const L=_();if(!L.length){A.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const M={},B=[];L.forEach(S=>{const G=S.player.id;M[G]||(M[G]=!0,B.push(S))}),A.innerHTML=B.map(S=>{const G=x[S.player.id]||1,O=G>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${G}</div>`:"",K=n.filter(H=>H.player.id===S.player.id&&H.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Fe(S,O+K)}).join("")}A.querySelectorAll("[data-card-id]").forEach(L=>{L.addEventListener("click",()=>{const M=n.find(B=>B.id===L.dataset.cardId);M&&kt(M,n,x,t)})})}function N(A){const L=E?f:[...b];if(!L.length){A.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}A.innerHTML=L.map(M=>{if(b.has(M)){const S=p.find(P=>P.formation===M),G=p.filter(P=>P.formation===M).length,O=G>1?`<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${G}</div>`:"";return`
        ...
        <div style="
          ...
          <div style="font-weight:900;font-size:18px">${M}</div>
        </div>`}).join(""),A.querySelectorAll("[data-form-id]").forEach(M=>{M.addEventListener("click",()=>{const B=p.find(S=>S.id===M.dataset.formId);B&&_t(B,p,t,o)})})}function q(A){const L=E?u:[...$];if(!L.length){A.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}A.innerHTML=L.map(M=>{const B=$.has(M),S=ie[M]||{icon:"⚡"};if(B){const G=c.find(K=>K.gc_type===M),O=c.filter(K=>K.gc_type===M).length,P=O>1?`<div style="position:absolute;top:4px;right:4px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${O}</div>`:"";return`
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const ee=1e3;function _t(e,t,i,a){var m,b,$;const{state:d,toast:o,closeModal:s,navigate:l,refreshProfile:r}=i,n=e.formation,c={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function p(){const y=ge[n]||{},g=ke[n]||[],w=290,E=360,h=20;function v(T){const I=y[T];return I?{x:I.x*w,y:I.y*E}:null}let _=`<svg width="${w}" height="${E}" viewBox="0 0 ${w} ${E}" xmlns="http://www.w3.org/2000/svg">`;for(const[T,I]of g){const z=v(T),C=v(I);!z||!C||(_+=`<line x1="${z.x}" y1="${z.y}" x2="${C.x}" y2="${C.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const T of Object.keys(y)){const I=v(T);if(!I)continue;const z=T.replace(/\d+/,""),C=c[z]||"#555";_+=`<circle cx="${I.x}" cy="${I.y}" r="${h}" fill="${C}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,_+=`<text x="${I.x}" y="${I.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${z}</text>`}return _+="</svg>",_}const f=t.filter(y=>y.formation===n),u=f.length,x=!e.is_for_sale;a(`Formation ${n}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;min-height:100%;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${p}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${d?`<img src="${d}" style="width:${p-4}px;height:${p-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,p-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
        ...
        stroke="${_}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const f=48,u=64,x=13,m=16,b={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[y,g]of Object.entries(r)){const w=c(y);if(!w||!g)continue;const E=y.replace(/[0-9]/g,""),h=Z[E]||"#555",v=i==="attack"&&["MIL","ATT"].includes(E)&&!g.used||i==="defense"&&["GK","DEF","MIL"].includes(E)&&!g.used,_=a.includes(g.cardId);let T;i==="attack"?T=E==="MIL"?Number(g.note_m)||0:Number(g.note_a)||0:i==="defense"?T=E==="GK"?Number(g.note_g)||0:E==="MIL"?Number(g.note_m)||0:Number(g.note_d)||0:T=Number(E==="GK"?g.note_g:E==="DEF"?g.note_d:E==="MIL"?g.note_m:g.note_a)||0,T=T+(g.boost||0);const I=(w.x-f/2).toFixed(1),z=(w.y-u/2).toFixed(1),C=g.used?.25:1,N=b[g==null?void 0:g.rarity]||b.normal,q=_?"#ff3030":N,A=_?3:(g==null?void 0:g.rarity)==="légende"||(g==null?void 0:g.rarity)==="pépite"?2.5:2,L=u-x-m;p+=`<defs><clipPath id="cp-${y}"><rect x="${I}" y="${(w.y-u/2+x).toFixed(1)}" width="${f}" height="${L}"/></clipPath></defs>`,p+=`<rect x="${I}" y="${z}" width="${f}" height="${u}" rx="5" fill="${h}" opacity="${C}"/>`;const M=R(g);M&&!g.used&&(p+=`<image href="${M}" x="${I}" y="${(w.y-u/2+x).toFixed(1)}" width="${f}" height="${L}" clip-path="url(#cp-${y})" preserveAspectRatio="xMidYMin slice"/>`),p+=`<rect x="${I}" y="${z}" width="${f}" height="${x}" rx="4" fill="rgba(255,255,255,0.92)"/>`,p+=`<text x="${w.x.toFixed(1)}" y="${(w.y-u/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(g.name||"").slice(0,9)}</text>`;const B=(w.y+u/2-m).toFixed(1);if(p+=`<rect x="${I}" y="${B}" width="${f}" height="${m}" fill="rgba(255,255,255,0.92)"/>`,g.used)p+=`<text x="${w.x.toFixed(1)}" y="${(w.y+u/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="13" font-weight="900" fill="rgba(0,0,0,0.4)" font-family="Arial Black">–</text>`;else{const S=ve(g.country_code);S?p+=`<image href="${S}" x="${(w.x-20).toFixed(1)}" y="${(w.y+u/2-m+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:p+=`<text x="${(w.x-13).toFixed(1)}" y="${(w.y+u/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${st(g.country_code)}</text>`,p+=`<text x="${w.x.toFixed(1)}" y="${(w.y+u/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${T}</text>`;const G=at(g);G?p+=`<image href="${G}" x="${(w.x+f/2-14).toFixed(1)}" y="${(w.y+u/2-m+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:g.clubName&&(p+=`<text x="${(w.x+14).toFixed(1)}" y="${(w.y+u/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(g.clubName||"").slice(0,3).toUpperCase()}</text>`),g.boost&&(p+=`<rect x="${(w.x+f/2-12).toFixed(1)}" y="${(w.y-u/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,p+=`<text x="${(w.x+f/2-5).toFixed(1)}" y="${(w.y-u/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${g.boost}</text>`)}p+=`<rect x="${I}" y="${z}" width="${f}" height="${u}" rx="5" fill="${_?"rgba(255,255,255,0.12)":"none"}" stroke="${q}" stroke-width="${A}" opacity="${C}"/>`,v&&(p+=`<rect x="${I}" y="${z}" width="${f}" height="${u}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${_?"selected":""}" data-card-id="${g.cardId}" data-role="${E}" style="cursor:pointer"/>`)}const $=38;return`<svg viewBox="${-$} ${-$} ${d+$*2} ${o+$*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${p}
  ...
  `,document.body.appendChild(a);let c=!1;const p=()=>{c||(c=!0,a.remove(),i())};a.addEventListener("click",p),setTimeout(p,2e3)}function te(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function qe(e,t,i,a=null){if(t.phase!=="attack"){te("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){te(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const d=Object.values(t.homeTeam).flat().filter(p=>p.used),o=t.homeSubs.filter(p=>!t.usedSubIds.includes(p.cardId));if(!d.length){te("Aucun joueur utilisé à remplacer");return}if(!o.length){te("Aucun remplaçant disponible");return}let s=0,l=Math.max(0,o.findIndex(p=>p.cardId===a)),r=!1;const n=document.createElement("div");n.id="sub-overlay",n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function c(){var $,y,g,w,E,h;const p=d[s],f=o[l],u=Math.min(130,Math.round((window.innerWidth-90)/2)),x=Math.round(u*1.35),m=v=>`background:rgba(255,255,255,0.12);border:none;color:${v?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${v?"default":"pointer"};flex-shrink:0`;n.innerHTML=`
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(d=>{d.addEventListener("click",()=>{const o=d.dataset.cardId;for(const s of["GK","DEF","MIL","ATT"]){const l=(t.homeTeam[s]||[]).find(r=>r.cardId===o);if(l){l.boost=(l.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${l.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),D(e,t,i)})})}function Me(e,t,i,a,d){const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const s=Array.from({length:10},(n,c)=>`
