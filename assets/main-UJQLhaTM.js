...
import{s as v,j as Li,q as Do,t as Dt,u as Ii,F as Ti,o as nt,n as Ht,g as qt,r as ze,m as ut,v as ni,w as Fo,T as qi,p as _n,x as kn,d as $n}from"./special-cards-YynorolH.js";const En="/";function go(t,{navigate:e,toast:i}){let o="login";const r=()=>{var a,d,c,s,l,u,y;const n=o==="login";if(t.innerHTML=`
    <div style="
  ...
  <div class="home-dark" id="home-dark">
    ...
    </style>`}async function Wn(t,{state:e,navigate:i,toast:o}){const r=e.profile;if(!r)return;const n="/icons/";Oo(i,r,"game",n,o),t.innerHTML=`
  <style>
    .game-page {
      min-height: 100%;
      background: var(--page-bg);
  ...
  </div>`,t.querySelectorAll("[data-action]").forEach(a=>{a.addEventListener("click",()=>{a.style.transform="scale(.96)",setTimeout(()=>a.style.transform="",180);const d=a.dataset.action;if(d==="match-ai"){Ko(i,e);return}if(d==="match-random"){mt(),i("match",{matchMode:"random"});return}if(d==="match-friend"){i("friends");return}if(d==="mini-league"){i("mini-league");return}if(d==="ranked"){i("ranked");return}o("Bientôt disponible","info")})})}async function Wo(t,e){var p,x;const{state:i,navigate:o}=e,r=oi(),n=Ji(),a=(p=i==null?void 0:i.profile)==null?void 0:p.is_admin,d="2026.07.29-1057";t.innerHTML=`
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(K=document.getElementById("close-detail"))==null||K.addEventListener("click",d);let W=new Set;const de=l.rarity==="pepite"?1.3:l.rarity==="papyte"?.7:1;function ce(){let z=0;return document.querySelectorAll(".expl-check:checked").forEach(Y=>{if(Y.dataset.id===t.id)return;const se=Number(Y.dataset.evo)||0,ge=Number(Y.dataset.note)||0;z+=ge+se}),Math.round(z*de)}const j=()=>{const z=W.size,Y=document.getElementById("sell-action-panel");if(!Y)return;Y.style.display=z>0?"block":"none",document.getElementById("sell-selected-count").textContent=z;const Z=document.getElementById("evolve-btn");if(Z){const se=ce();Z.textContent=se>0?`⬆️ Évoluer (+${se})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach(z=>{z.addEventListener("change",()=>{const Y=z.dataset.id;z.checked?W.add(Y):W.delete(Y);const Z=z.closest(".exemplaire-row");if(Z){const se=Z.querySelector(".expl-sel-overlay"),ge=Z.querySelector(".expl-sel-check");se&&(se.style.display=z.checked?"block":"none"),ge&&(ge.style.display=z.checked?"flex":"none"),Z.style.transform=z.checked?"scale(1.05)":"scale(1)"}j()})}),document.querySelectorAll(".exemplaire-row").forEach(z=>{z.addEventListener("click",Y=>{if(Y.target.tagName==="INPUT")return;const Z=z.querySelector(".expl-check");Z&&!Z.disabled&&(Z.checked=!Z.checked,Z.dispatchEvent(new Event("change")))})}),(Q=document.getElementById("evolve-btn"))==null||Q.addEventListener("click",async()=>{if(y<=1)return;const z=[...W];if(!z.length)return;if(W.has(t.id)){const xe=document.createElement("div");xe.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",xe.innerHTML=`
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const c=await $o("Nom du deck",`Deck ${((n==null?void 0:n.length)||0)+1}`);if(!c)return;const{data:s,error:l}=await v.from("decks").insert({owner_id:i.profile.id,name:c}).select().single();if(l){r(l.message,"error");return}r("Deck créé !","success"),Eo(s.id,t,e)}),t.querySelectorAll("[data-open-deck]").forEach(c=>{c.addEventListener("click",()=>Eo(c.dataset.openDeck,t,e))}),t.querySelectorAll("[data-rename]").forEach(c=>{c.addEventListener("click",async()=>{const s=await $o("Nouveau nom",c.dataset.name);if(!s||s===c.dataset.name)return;const{error:l}=await v.from("decks").update({name:s}).eq("id",c.dataset.rename);if(l){r(l.message,"error");return}r("Deck renommé !","success"),Ri(t,e)})}),t.querySelectorAll("[data-delete]").forEach(c=>{c.addEventListener("click",async()=>{if(!await rr(`Supprimer le deck "${c.dataset.name}" ? Cette action est irréversible.`,!0))return;await v.from("deck_cards").delete().eq("deck_id",c.dataset.delete);const{error:s}=await v.from("decks").delete().eq("id",c.dataset.delete);if(s){r(s.message,"error");return}r("Deck supprimé.","success"),Ri(t,e)})})}async function Eo(t,e,i){const{state:o,toast:r}=i;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await v.from("decks").select("*").eq("id",t).single(),{data:a}=await v.from("cards").select(`id, card_type, formation, stadium_id, evolution_bonus,
      player:players(id, firstname, surname_real, country_code, club_id, job, job2,
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Zt};border:1px solid ${ei};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(I,_)=>_+1).reverse().map(I=>{const _=(c||[]).filter(R=>R.matchday===I);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${it};margin-bottom:6px">Journée ${I}</div>${_.map(R=>zo(R,u,n,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  <div class="page">
    ...
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.29-1057" style="cursor:pointer">
