...
import{s as T,F as _i,i as Ht,k as Vi,l as ot,m as Li,f as Gt,n as hn,T as di,j as vn,o as wn,b as _n}from"./formation-links-BZe0pGr4.js";const $n="/";function Ii(e,{navigate:t,toast:i}){let o="login";const d=()=>{var r,l,p,s,n,x;const a=o==="login";e.innerHTML=`
    <div style="
    ...
    </style>`,(r=document.getElementById("tab-login-btn"))==null||r.addEventListener("click",()=>{o="login",d()}),(l=document.getElementById("tab-reg-btn"))==null||l.addEventListener("click",()=>{o="register",d()}),a?((p=document.getElementById("login-password"))==null||p.addEventListener("keydown",f=>{var h;f.key==="Enter"&&((h=document.getElementById("login-btn"))==null||h.click())}),(s=document.getElementById("login-btn"))==null||s.addEventListener("click",async()=>{const f=document.getElementById("login-email").value.trim(),h=document.getElementById("login-password").value,_=document.getElementById("login-error");if(_.textContent="",!f||!h){_.textContent="Remplissez tous les champs.";return}const u=document.getElementById("login-btn");u.textContent="⏳ Connexion…",u.disabled=!0;const{error:c}=await T.auth.signInWithPassword({email:f,password:h});if(u.textContent="⚽ Se connecter",u.disabled=!1,c){_.textContent=c.message.includes("Invalid")?"Email ou mot de passe incorrect.":c.message;return}window.location.reload()})):((n=document.getElementById("reg-confirm"))==null||n.addEventListener("keydown",f=>{var h;f.key==="Enter"&&((h=document.getElementById("reg-btn"))==null||h.click())}),(x=document.getElementById("reg-btn"))==null||x.addEventListener("click",async()=>{const f=document.getElementById("reg-email").value.trim(),h=document.getElementById("reg-password").value,_=document.getElementById("reg-confirm").value,u=document.getElementById("reg-error");if(u.textContent="",!f||!h||!_){u.textContent="Remplissez tous les champs.";return}if(h.length<6){u.textContent="Mot de passe trop court (min. 6 caractères).";return}if(h!==_){u.textContent="Les mots de passe ne correspondent pas.";return}const c=document.getElementById("reg-btn");c.textContent="⏳ Création…",c.disabled=!0;const{error:A}=await T.auth.signUp({email:f,password:h});if(c.textContent="🚀 Créer mon compte",c.disabled=!1,A){u.textContent=A.message;return}i("Compte créé ! Connecte-toi pour commencer.","success",4e3),o="login",d(),setTimeout(()=>{const m=document.getElementById("login-email");m&&(m.value=f)},50)}))};d()}function kn(e,{state:t,navigate:i,toast:o,refreshProfile:d}){let a="#1A6B3C",r="#D4A017";e.innerHTML=`
  ...
  `;function l(){var _;const s=document.getElementById("logo-preview"),n=document.getElementById("logo-initials"),x=((_=document.getElementById("setup-club"))==null?void 0:_.value)||"MW",f=x.trim().split(" ").filter(Boolean),h=f.length>=2?(f[0][0]+f[1][0]).toUpperCase():x.slice(0,2).toUpperCase();s&&(s.style.background=r,s.style.borderColor=a),n&&(n.textContent=h,n.style.color=a)}document.getElementById("color1").addEventListener("input",s=>{a=s.target.value,document.getElementById("swatch1").style.background=a,l()}),document.getElementById("color2").addEventListener("input",s=>{r=s.target.value,document.getElementById("swatch2").style.background=r,l()});function p(s){document.querySelectorAll(".setup-step").forEach(n=>n.classList.remove("active")),document.getElementById(`step-${s}`).classList.add("active"),document.getElementById("step-num").textContent=s,document.getElementById("progress-fill").style.width=`${Math.round(s/3*100)}%`,s===3&&l()}document.getElementById("step1-next").addEventListener("click",async()=>{const s=document.getElementById("setup-pseudo").value.trim(),n=document.getElementById("step1-error");if(n.textContent="",s.length<3){n.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:x}=await T.from("users").select("id").eq("pseudo",s).maybeSingle();if(x){n.textContent="Ce pseudo est déjà pris.";return}p(2)}),document.getElementById("step2-back").addEventListener("click",()=>p(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const s=document.getElementById("setup-club").value.trim(),n=document.getElementById("step2-error");if(n.textContent="",s.length<2){n.textContent="Nom trop court (min. 2 caractères).";return}const{data:x}=await T.from("users").select("id").eq("club_name",s).maybeSingle();if(x){n.textContent="Ce nom de club est déjà pris.";return}p(3)}),document.getElementById("step3-back").addEventListener("click",()=>p(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const s=document.getElementById("setup-pseudo").value.trim(),n=document.getElementById("setup-club").value.trim(),x=document.getElementById("step3-error"),f=document.getElementById("step3-finish");x.textContent="",f.disabled=!0,f.textContent="Création en cours…";try{const{error:h}=await T.from("users").insert({id:t.user.id,pseudo:s,club_name:n,club_color1:a,club_color2:r,credits:1e4});if(h)throw h;await En(t.user.id),await d(),o(`Bienvenue ${s} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(h){x.textContent=h.message,f.disabled=!1,f.textContent="🚀 Créer mon profil !"}})}async function En(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await T.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const Tn="modulepreload",zn=function(e){return"/"+e},Mi={},ni=function(t,i,o){let d=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),l=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));d=Promise.allSettled(i.map(p=>{if(p=zn(p),p in Mi)return;Mi[p]=!0;const s=p.endsWith(".css"),n=s?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${p}"]${n}`))return;const x=document.createElement("link");if(x.rel=s?"stylesheet":Tn,s||(x.as="script"),x.crossOrigin="",x.href=p,l&&x.setAttribute("nonce",l),document.head.appendChild(x),s)return new Promise((f,h)=>{x.addEventListener("load",f),x.addEventListener("error",()=>h(new Error(`Unable to preload CSS for ${p}`)))})}))}function a(r){const l=new Event("vite:preloadError",{cancelable:!0});if(l.payload=r,window.dispatchEvent(l),!l.defaultPrevented)throw r}return d.then(r=>{for(const l of r||[])l.status==="rejected"&&a(l.reason);return t().catch(a)})},Nt="#1A6B3C",Rt="#cc2222",Sn="#D4A017",ji="#888";async function An(e,t){const{state:i,toast:o}=t;e.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${f.map(({friendshipId:h,friend:_})=>Ln(_,h)).join("")}
    </div>`,s.querySelectorAll("[data-stats]").forEach(h=>{h.addEventListener("click",()=>Mn(e,h.dataset.stats,h.dataset.friendName))}),s.querySelectorAll("[data-match]").forEach(h=>{h.addEventListener("click",()=>{const _=h.dataset.friendId,u=h.dataset.friendName;console.log("[Friends] clic match",{fid:_,fname:u,hasNavigate:typeof o}),typeof o=="function"?o("match",{matchMode:"friend",friendId:_,friendName:u}):t("Erreur navigation","error")})})}function Ln(e,t){const i=e.club_name||e.pseudo||"?",o=e.pseudo||"",d=(o||i).slice(0,2).toUpperCase(),a=e.club_color2||Nt,r=e.club_color1||"#ffffff",l=e.last_seen_at?new Date(e.last_seen_at):null,p=l&&Date.now()-l.getTime()<3*60*1e3;return`
    ...
    </div>`}function In(e,t){const i=$i();i.innerHTML=`
    <div class="popup-box">
            ...
            ${n.map(f=>{var h,_,u;return`
              <div style="display:flex;align-items:center;gap:10px;background:#f9f9f9;border-radius:10px;padding:10px 12px">
                <div style="flex:1;font-size:14px;font-weight:700">${((h=f.requester)==null?void 0:h.club_name)||((_=f.requester)==null?void 0:_.pseudo)||"?"}
                  <span style="font-size:11px;color:#999;font-weight:400">(${((u=f.requester)==null?void 0:u.pseudo)||""})</span>
                </div>
    ...
    ${ki()}`,document.body.appendChild(s);const x=()=>s.remove();s.querySelector("#pending-close").addEventListener("click",x),s.addEventListener("click",f=>{f.target===s&&x()}),s.querySelectorAll("[data-accept]").forEach(f=>{f.addEventListener("click",async()=>{const{error:h}=await T.from("friendships").update({status:"accepted"}).eq("id",f.dataset.accept);if(h){t("Erreur : "+h.message,"error");return}f.closest("div[style]").remove(),t("✅ Ami accepté !","success"),Yi(e,t,o),i&&i()})}),s.querySelectorAll("[data-decline]").forEach(f=>{f.addEventListener("click",async()=>{await T.from("friendships").delete().eq("id",f.dataset.decline),f.closest("div[style]").remove(),t("Demande refusée","info"),i&&i()})})}async function Mn(e,t,i){const o=e.user.id,[d,a]=[o,t].sort(),r=o===d,{data:l}=await T.from("friend_match_stats").select("*").eq("player1_id",d).eq("player2_id",a).single(),p=e.profile.club_name||e.profile.pseudo||"Moi",s=l||{},n=r?s.wins_p1||0:s.wins_p2||0,x=r?s.wins_p2||0:s.wins_p1||0,f=s.draws||0,h=r?s.goals_p1||0:s.goals_p2||0,_=r?s.goals_p2||0:s.goals_p1||0,u=r?s.gc_used_p1||0:s.gc_used_p2||0,c=r?s.gc_used_p2||0:s.gc_used_p1||0,A=s.matches_total||0,m=(F,I,q,C=Nt,G=Rt)=>`
    <div style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:8px;padding:10px 0;border-bottom:1px solid #f0f0f0">
      ...
      <div style="text-align:left;font-size:18px;font-weight:900;color:${G}">${q}</div>
    </div>`,M=$i();M.innerHTML=`
    ...
    </style>`}async function Ji({player1Id:e,player2Id:t,score1:i,score2:o,gc1:d,gc2:a}){const[r,l]=[e,t].sort(),p=e!==r,s=p?o:i,n=p?i:o,x=p?a:d,f=p?d:a,h=s>n?1:0,_=n>s?1:0,u=s===n?1:0,{data:c}=await T.from("friend_match_stats").select("*").eq("player1_id",r).eq("player2_id",l).single();c?await T.from("friend_match_stats").update({wins_p1:c.wins_p1+h,wins_p2:c.wins_p2+_,draws:c.draws+u,goals_p1:c.goals_p1+s,goals_p2:c.goals_p2+n,gc_used_p1:c.gc_used_p1+x,gc_used_p2:c.gc_used_p2+f,matches_total:c.matches_total+1}).eq("player1_id",r).eq("player2_id",l):await T.from("friend_match_stats").insert({player1_id:r,player2_id:l,wins_p1:h,wins_p2:_,draws:u,goals_p1:s,goals_p2:n,gc_used_p1:x,gc_used_p2:f,matches_total:1})}const jn="2026.07.08-0005";async function Ci(e,{state:t,navigate:i,toast:o}){var a,r;const d=t.profile;d&&(e.innerHTML=`
  <style>
  ...
  <div class="page" style="height:100%;overflow-y:auto">
    <div class="page-body home-page-body">
      ...
      <div style="text-align:center;padding:4px 0 8px;display:flex;flex-direction:column;gap:6px;align-items:center">
        ...
        <div style="font-size:10px;color:var(--gray-400);font-family:monospace;letter-spacing:0.5px">build ${jn}</div>`:""}
      </div>

    </div>
  </div>
  `,e.querySelectorAll("[data-nav]").forEach(l=>{l.addEventListener("click",p=>{p.preventDefault(),i(l.dataset.nav)})}),(a=document.getElementById("nav-rankings"))==null||a.addEventListener("click",()=>i("rankings")),(r=document.getElementById("nav-matches"))==null||r.addEventListener("click",()=>i("matches")),e.querySelectorAll("[data-action]").forEach(l=>{l.addEventListener("click",()=>{l.classList.add("tapped"),setTimeout(()=>l.classList.remove("tapped"),200);const p=l.dataset.action;if(p==="match-ai"){Fn(i);return}if(p==="match-random"){i("match",{matchMode:"random"});return}if(p==="match-friend"){i("friends");return}if(p==="mini-league"){i("mini-league");return}if(p==="ranked"){i("ranked");return}o("Bientôt disponible","info")})}),window.innerWidth<768&&requestAnimationFrame(()=>{var F,I,q,C,G,V;const l=((F=window.visualViewport)==null?void 0:F.height)||window.innerHeight,p=((I=document.querySelector(".top-nav"))==null?void 0:I.offsetHeight)||56,s=((q=document.querySelector(".bottom-nav"))==null?void 0:q.offsetHeight)||60,n=((C=e.querySelector(".hero-compact"))==null?void 0:C.offsetHeight)||52,x=e.querySelector(".ranked-banner");e.querySelector(".play-grid");const f=((V=(G=e.querySelector("#logout-btn"))==null?void 0:G.closest("div"))==null?void 0:V.offsetHeight)||44,h=["friend-requests-banner","match-invite-banner","ongoing-match-banner"].reduce((K,oe)=>{var ie;return K+(((ie=document.getElementById(oe))==null?void 0:ie.offsetHeight)||0)},0),_=14*5,u=l-p-s-n-f-h-_,c=Math.max(80,Math.round(u*.28)),A=Math.max(160,Math.round(u*.72)),m=Math.floor((A-10)/2);x&&(x.style.minHeight=x.style.maxHeight=c+"px"),e.querySelectorAll(".play-grid .play-card").forEach(K=>{K.style.minHeight=K.style.height=m+"px"});const M=Math.round(m*.55);e.querySelectorAll(".play-card .play-icon").forEach(K=>{K.style.height=M+"px"})}),document.getElementById("logout-btn").addEventListener("click",async()=>{await T.auth.signOut(),window.location.reload()}),Qi(t,o),Bn(t,o,i),Xi(t,o,i))}async function Xi(e,t,i){const o=document.getElementById("ongoing-match-banner");if(!o)return;const d=e.profile.id,{data:a}=await T.from("matches").select("id, home_id, away_id, status, mode").eq("status","active").or(`home_id.eq.${d},away_id.eq.${d}`).order("created_at",{ascending:!1});if(!(a!=null&&a.length)){o.innerHTML="";return}const r=a.map(p=>p.home_id===d?p.away_id:p.home_id).filter(Boolean);let l={};if(r.length){const{data:p}=await T.from("users").select("id, pseudo, club_name").in("id",r);(p||[]).forEach(s=>{l[s.id]=s.club_name||s.pseudo})}o.innerHTML=a.map(p=>{const s=p.home_id===d?p.away_id:p.home_id,n=l[s]||"Adversaire",x=p.mode==="mini_league";return`
    ...
    </div>`,document.getElementById("friend-req-btn").addEventListener("click",()=>{Wi(e,t,()=>Qi(e,t))})}function Fn(e){const t=[{mode:"vs_ai_easy",label:"Facile",sub:"Gain 500 cr.",icon:"🟢"},{mode:"vs_ai_medium",label:"Moyen",sub:"Gain 1 000 cr.",icon:"🟡"},{mode:"vs_ai_hard",label:"Difficile",sub:"Gain 1 500 cr.",icon:"🟠"},{mode:"vs_ai_club",label:"Club",sub:"Gain 2 500 cr.",icon:"🔴"}],i=document.createElement("div");i.className="modal-overlay",i.style.zIndex="2000",i.innerHTML=`
  ...
  `,document.body.appendChild(i);const o=()=>i.remove();document.getElementById("diff-cancel").addEventListener("click",o),i.addEventListener("click",d=>{d.target===i&&o()}),i.querySelectorAll("[data-mode]").forEach(d=>{d.addEventListener("click",()=>{o(),e("match",{matchMode:d.dataset.mode})})})}const Bt="/",qi={GK:Bt+"cards/card-GK.png",DEF:Bt+"cards/card-DEF.png",MIL:Bt+"cards/card-MIL.png",ATT:Bt+"cards/card-ATT.png"},Dn={GK:"#c0c0c0",DEF:"#e03030",MIL:"#D4A017",ATT:"#3fbf5f"};function Pn(e){if(!(e!=null&&e.face))return null;const t=e.face.replace(/^public\//,"").replace(/^\//,"");return Bt+t}function Gn(e){return!e||e.length<2?null:`https://flagsapi.com/${e.slice(0,2).toUpperCase()}/flat/64.png`}function Nn(e){var o;const t="https://fcnwclxlkytdfjotqkta.supabase.co",i=((o=e==null?void 0:e.clubs)==null?void 0:o.logo_url)||(e==null?void 0:e.clubLogo);return i?i.startsWith("http")?i:`${t}/storage/v1/object/public/assets/clubs/${i}`:null}function Bi(e,t,i=0){if(!t)return 0;const d="note_"+({GK:"g",DEF:"d",MIL:"m",ATT:"a"}[t]||t.toLowerCase());return(Number(e[d])||0)+(t===e.job||t===e.job2?i:0)}function Rn(e,t){return t?t.club_id&&String(e.club_id)===String(t.club_id)||t.country_code&&e.country_code===t.country_code:!1}function st(e,t={}){var Z;const{width:i=160,showStad:o=!1,stadDef:d=null,used:a=!1,extraNote:r=0,role:l=null}=t;if(!e)return`<div style="width:${i}px;height:${Math.round(i*657/507)}px;border-radius:8px;background:#111;opacity:0.3"></div>`;const p=i/507,s=Math.round(i*657/507),n=ge=>Math.round(ge*p),x=ge=>n(ge)+"px",f=l||e._line||e.job||"MIL",h=Dn[f]||"#D4A017",_=qi[f]||qi.MIL,u=e._evolution_bonus??e.evolution_bonus??0,c=o&&(e.stadiumBonus||Rn(e,d))?10:0,A=Bi(e,f,u)+r+c,m=l&&l!==e.job,M=c>0?"#4fc3f7":m?"#E87722":h,F=!l&&e.job2&&e.job2!==f?e.job2:null,I=F?Bi(e,F,u)+r+c:null,q=Pn(e),C=Gn(e.country_code),G=Nn(e),V=(e.firstname||"").toUpperCase(),K=(e.surname_encoded||e.name||"").toUpperCase(),oe=a?"opacity:0.35;":"",ie=K.length>11?34:K.length>8?42:50,Q=n(155),R=n(260),P=n(235),re=n(390),xe=n(180),_e=n(100),we=n(309),ve=n(85),Ee=n(95),se=n(435),pe=n(38),ce=n(374),H=n(6);return`<div style="position:relative;width:${i}px;height:${s+(c>0?n(22):0)}px;flex-shrink:0;${oe}user-select:none">
  ...
  <div style="position:absolute;left:${ce}px;top:${se}px;width:${Ee}px;height:${ve}px;z-index:3;
    display:flex;align-items:center;justify-content:center;overflow:hidden">
    ${G?`<img src="${G}" style="width:100%;height:100%;object-fit:contain">`:`<span style="font-size:${x(14)};font-weight:900;color:#fff">${(((Z=e.clubs)==null?void 0:Z.encoded_name)||e.clubName||"").slice(0,3).toUpperCase()}</span>`}
  </div>

</div></div>`}const Oe={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé pour ce match."},"Double attaque":{icon:"⚡",desc:"La note d'attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la note d'un joueur adverse."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function Se(e,t){if(!e)return 0;switch(t){case"GK":return Number(e.note_g)||0;case"DEF":return Number(e.note_d)||0;case"MIL":return Number(e.note_m)||0;case"ATT":return Number(e.note_a)||0;default:return 0}}const Fi=["ATT","MIL","DEF","GK"];function Zi(e){let t=0;const i=e.length;for(let o=0;o<i;o++)for(let d=o+1;d<i;d++){const a=e[o],r=e[d];if(!a||!r)continue;const l=a._col!=null&&r._col!=null&&a._col===r._col,p=a._col!=null&&r._col!=null&&Math.abs(a._col-r._col)===1,s=Fi.indexOf(a._line||a.job),n=Fi.indexOf(r._line||r.job),x=Math.abs(s-n)===1;if(!((a._line||a.job)===(r._line||r.job)&&p||l&&x))continue;const _=a.country_code&&r.country_code&&a.country_code===r.country_code,u=a.club_id&&r.club_id&&a.club_id===r.club_id;_&&u?t+=2:(_||u)&&(t+=1)}return t}function Ut(e,t={}){const i=e.reduce((a,r)=>{const l=r._line||r.job,p=r.stadiumBonus&&(l==="MIL"||l==="ATT")?10:0;return a+(Number(l==="MIL"?r.note_m:r.note_a)||0)+(r.boost||0)+p},0),o=Zi(e);let d=i+o;return t.doubleAttack&&(d*=2),t.stolenNote&&(d-=t.stolenNote),{base:i,links:o,total:Math.max(0,d)}}function Kt(e,t={}){const i=e.reduce((a,r)=>{const l=r._line||r.job;let p=0;l==="GK"?p=Number(r.note_g)||0:l==="MIL"?p=Number(r.note_m)||0:p=Number(r.note_d)||0;const s=r.stadiumBonus&&(l==="GK"||l==="DEF"||l==="MIL")?10:0;return a+p+(r.boost||0)+s},0),o=Zi(e);let d=i+o;return t.stolenNote&&(d-=t.stolenNote),{base:i,links:o,total:Math.max(0,d)}}function Vt(e,t,i={}){return i.shield?{goal:!1,shielded:!0}:{goal:e>t,shielded:!1}}function en(e,t,i="easy"){const o=e.filter(r=>!r.used);if(!o.length)return[];const d=[...o].sort((r,l)=>{const p=t==="attack"?Se(r,"ATT"):r._line==="GK"?Se(r,"GK"):Se(r,"DEF");return(t==="attack"?Se(l,"ATT"):l._line==="GK"?Se(l,"GK"):Se(l,"DEF"))-p});let a=i==="easy"?1+Math.floor(Math.random()*2):i==="medium"?2+Math.floor(Math.random()*2):3;return d.slice(0,Math.min(a,d.length,3))}function On(e,t){const i={vs_ai_easy:{victoire:500,nul:250,defaite:50},vs_ai_medium:{victoire:1e3,nul:500,defaite:50},vs_ai_hard:{victoire:1500,nul:750,defaite:100},vs_ai_club:{victoire:2500,nul:1250,defaite:100}};return(i[e]||i.vs_ai_easy)[t]||0}const tn={pepite:{win:2,loss:-1,boosterStart:"min"},papyte:{win:1,loss:-2,boosterStart:"max"}};function nn(e,t){const i=e.player;if(!i)return 0;const o=i.rarity;if(o!=="pepite"&&o!=="papyte")return Number(i[t])||0;const d=Ei(i),a=Number(i[t])||0;if(a<=0)return 0;const r=i.note_min??1,l=i.note_max??10,s=(e.current_note??d)-d;return Math.min(l,Math.max(r,a+s))}function Ei(e){const t=e.job||"ATT";return Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}async function Ti(e,t){!e||!t||await Promise.all([gi(e,"win"),gi(t,"loss")])}async function gi(e,t){const{data:i}=await T.from("cards").select("id, current_note, player:players(rarity, job, note_g, note_d, note_m, note_a, note_min, note_max)").eq("owner_id",e).eq("card_type","player");if(!(i!=null&&i.length))return;const o=i.filter(d=>{var a,r;return((a=d.player)==null?void 0:a.rarity)==="pepite"||((r=d.player)==null?void 0:r.rarity)==="papyte"});o.length&&await Promise.all(o.map(d=>{const a=tn[d.player.rarity],r=t==="win"?a.win:a.loss,l=d.player.note_min??1,p=d.player.note_max??10,s=Ei(d.player),n=d.current_note??s,x=Math.min(p,Math.max(l,n+r));return T.from("cards").update({current_note:x}).eq("id",d.id)}))}async function Hn(e,t){return gi(e,t)}const on=Object.freeze(Object.defineProperty({__proto__:null,EVOLUTIVE_RULES:tn,applyOwnEvolution:Hn,currentSecondaryNote:nn,getBaseMainNote:Ei,updateEvolutiveCards:Ti},Symbol.toStringTag,{value:"Module"})),Un={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},li={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},Yt=["GK","DEF","MIL","ATT"],Kn=["Tous","GK","DEF","MIL","ATT"],Vn={normal:1e3,pepite:5e3,papyte:5e3,legende:1e4};function Di(e){const t=e.player;if(!t)return null;const i=t.rarity;return(i==="pepite"||i==="papyte")&&e.current_note!=null?e.current_note:Math.max(Number(t.note_g)||0,Number(t.note_d)||0,Number(t.note_m)||0,Number(t.note_a)||0)}function Yn(e){return e.length?e.reduce((t,i)=>Di(i)>Di(t)?i:t,e[0]):e[0]}function Wn(e){return e==="GK"?"note_g":e==="DEF"?"note_d":e==="MIL"?"note_m":"note_a"}const Jn={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function Xn(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";if(!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function ci(e,t){return e&&Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}function pi(e,t=""){const i=e.player;if(!i)return"";const o=e.evolution_bonus||0,d={...i,_evolution_bonus:o},a=t?`<div style="position:absolute;top:6px;right:6px;z-index:10;background:#0a3d1e;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 7px">${t}</div>`:"";return`<div style="position:relative;display:inline-block;cursor:pointer" data-card-id="${e.id}">
    ${a}
    ${st(d,{width:140})}
  </div>`}function Pi(e){return`<div style="display:inline-block;filter:grayscale(1);opacity:0.4">
    ${st(e,{width:140})}
  </div>`}async function Qn(e,t){const{state:i,navigate:o,toast:d,openModal:a,closeModal:r}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await T.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price, stadium_id, evolution_bonus,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),s=(l||[]).filter(H=>H.card_type==="player"&&H.player),n=(l||[]).filter(H=>H.card_type==="game_changer"),x=(l||[]).filter(H=>H.card_type==="formation"),f=(l||[]).filter(H=>H.card_type==="stadium"),{data:h}=await T.from("gc_definitions").select("name,gc_type,color,effect,image_url"),_={};(h||[]).forEach(H=>{_[H.name]=H});const{data:u}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),c={};(u||[]).forEach(H=>{c[H.id]=H}),f.forEach(H=>{H.stadium_def&&(c[H.stadium_id]=H.stadium_def)});const A=Object.keys(_i),m=Object.keys(Oe),M={};s.forEach(H=>{const Z=H.player.id;M[Z]=(M[Z]||0)+1}),new Set(Object.keys(M).map(H=>String(H)));const F=new Set(x.map(H=>H.formation)),I=new Set(n.map(H=>H.gc_type));let q="player",C="Tous",G="",V=!1;function K(){return[...s].sort((H,Z)=>{const ge=Yt.indexOf(H.player.job),de=Yt.indexOf(Z.player.job);return ge!==de?ge-de:(H.player.surname_encoded||"").localeCompare(Z.player.surname_encoded||"")})}function oe(){return[...p||[]].sort((H,Z)=>{const ge=Yt.indexOf(H.job),de=Yt.indexOf(Z.job);return ge!==de?ge-de:(H.surname_encoded||"").localeCompare(Z.surname_encoded||"")})}function ie(){return K().filter(H=>{const Z=H.player,ge=C==="Tous"||Z.job===C,de=!G||`${Z.firstname} ${Z.surname_encoded}`.toLowerCase().includes(G);return ge&&de})}function Q(){return oe().filter(H=>{const Z=C==="Tous"||H.job===C,ge=!G||`${H.firstname} ${H.surname_encoded}`.toLowerCase().includes(G);return Z&&ge})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",Z=>{G=Z.target.value.toLowerCase(),P()}),e.querySelectorAll(".filter-btn").forEach(Z=>{Z.addEventListener("click",()=>{C=Z.dataset.job,R(),P()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{V=!V,R(),P()})):(H.innerHTML=`
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(H=>{H.addEventListener("click",()=>{q=H.dataset.tab,C="Tous",G="",V=!1,e.querySelectorAll(".col-tab-btn").forEach(Z=>{const ge=Z.dataset.tab===q;Z.style.borderBottomColor=ge?"var(--green)":"transparent",Z.style.color=ge?"var(--green)":"var(--gray-600)"}),R(),P()})}),R(),P()}function Zn(e,t,i){const o=Oe[e]||{icon:"⚡",desc:"Effet spécial."},d=(t==null?void 0:t.gc_type)==="ultra_game_changer",a={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},r={purple:"#b06ce0",light_blue:"#00d4ef"},l=a[t==null?void 0:t.color]||a.purple,p=r[t==null?void 0:t.color]||r.purple,s=(t==null?void 0:t.name)||e,n=(t==null?void 0:t.effect)||o.desc,x=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Wt=1e3;function eo(e,t,i,o){var _,u,c;const{state:d,toast:a,closeModal:r,navigate:l,refreshProfile:p}=i,s=e.formation,n={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function x(){const A=Ht[s]||{},m=_i[s]||[],M=290,F=360,I=20;function q(G){const V=A[G];return V?{x:V.x*M,y:V.y*F}:null}let C=`<svg width="${M}" height="${F}" viewBox="0 0 ${M} ${F}" xmlns="http://www.w3.org/2000/svg">`;for(const[G,V]of m){const K=q(G),oe=q(V);!K||!oe||(C+=`<line x1="${K.x}" y1="${K.y}" x2="${oe.x}" y2="${oe.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const G of Object.keys(A)){const V=q(G);if(!V)continue;const K=G.replace(/\d+/,""),oe=n[K]||"#555";C+=`<circle cx="${V.x}" cy="${V.y}" r="${I}" fill="${oe}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,C+=`<text x="${V.x}" y="${V.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${K}</text>`}return C+="</svg>",C}const f=t.filter(A=>A.formation===s);f.length;const h=!e.is_for_sale;o(`Formation ${s}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(_=document.getElementById("direct-sell-form-btn"))==null||_.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${s} pour ${Wt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const A=f.find(M=>!M.is_for_sale)||f[0];if(!A){a("Aucune carte à vendre","error");return}await T.from("market_listings").delete().eq("card_id",A.id),await T.from("transfer_history").delete().eq("card_id",A.id);const{error:m}=await T.from("cards").delete().eq("id",A.id);if(m){a(m.message,"error");return}await T.from("users").update({credits:(d.profile.credits||0)+Wt}).eq("id",d.profile.id),await p(),a(`+${Wt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),r(),l("collection")}),(u=document.getElementById("market-sell-form-btn"))==null||u.addEventListener("click",async()=>{const A=parseInt(document.getElementById("sell-price-form").value);if(!A||A<1){a("Prix invalide","error");return}await T.from("cards").update({is_for_sale:!0,sale_price:A}).eq("id",e.id),await T.from("market_listings").insert({seller_id:d.profile.id,card_id:e.id,price:A}),a("Carte mise en vente sur le marché !","success"),r(),l("collection")}),(c=document.getElementById("cancel-sell-form-btn"))==null||c.addEventListener("click",async()=>{await T.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await T.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),a("Annonce retirée","success"),r(),l("collection")})}async function Gi(e,t,i,o){var xe,_e,we,ve,Ee;const{state:d,toast:a,openModal:r,closeModal:l,navigate:p,refreshProfile:s}=o,n=e.player,x=t.filter(se=>se.player.id===n.id),f=x.length,h=e.evolution_bonus||0,_=Math.max((Number(n.note_g)||0)+(n.job==="GK"||n.job2==="GK"?h:0),(Number(n.note_d)||0)+(n.job==="DEF"||n.job2==="DEF"?h:0),(Number(n.note_m)||0)+(n.job==="MIL"||n.job2==="MIL"?h:0),(Number(n.note_a)||0)+(n.job==="ATT"||n.job2==="ATT"?h:0)),u=n.rarity||"normal",{data:c}=await T.from("sell_price_configs").select("*").eq("rarity",u).lte("note_min",_).gte("note_max",_).order("note_min",{ascending:!1}).limit(1);((xe=c==null?void 0:c[0])==null?void 0:xe.price)??Vn[u];const A=n.rarity!=="legende";Xn(n);const m=((n.rarity==="pepite"||n.rarity==="papyte")&&e.current_note!=null?e.current_note:ci(n,n.job))+h,M=n.rarity==="pepite"||n.rarity==="papyte",F=n.job2?(M?nn(e,Wn(n.job2)):ci(n,n.job2))+(ci(n,n.job2)>0?h:0):null;li[n.job],n.job2&&li[n.job2];const I=Un[n.rarity]||"#ccc";Jn[n.country_code]||n.country_code;const q=e.evolution_bonus||0,G=m+q,V=F||0,K=V>0?V+q:0,oe=x.map(se=>se.id);let ie={};if(oe.length){const{data:se}=await T.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",oe).order("transferred_at",{ascending:!0});(se||[]).forEach(pe=>{ie[pe.card_id]||(ie[pe.card_id]=[]),ie[pe.card_id].push(pe)})}const Q=se=>{const pe=se.transferred_at?new Date(se.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",ce=se.source==="booster"?"Booster":se.price?se.price.toLocaleString("fr"):"—";return`
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;background:#f0fdf4;border-radius:10px;padding:10px 14px">
          <div>
            ...
            <div style="font-size:11px;color:#555;margin-top:2px">
              ${(e.evolution_bonus||0)>0?`+${e.evolution_bonus} appliqué${e.evolution_bonus>1?"s":""} · Note : ${G}${n.job2&&V>0?` / ${K}`:""}`:`Note actuelle : ${G}${n.job2&&V>0?` / ${K}`:""}`}
            </div>
          </div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",n.note_g],["DEF",n.note_d],["MIL",n.note_m],["ATT",n.note_a]].map(([se,pe])=>{const ce=li[se],H=se==="GK"?"#fff":ce,Z=se===n.job||se===n.job2,ge=(Number(pe)||0)+(Z&&q>0?q:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(_e=document.getElementById("close-detail"))==null||_e.addEventListener("click",l);let P=new Set;const re=()=>{const se=P.size,pe=document.getElementById("sell-action-panel");if(!pe)return;pe.style.display=se>0?"block":"none",document.getElementById("sell-selected-count").textContent=se;const ce=document.getElementById("evolve-btn");ce&&(ce.textContent=`⬆️ Évoluer ${se>1?"(+"+se+")":""}`)};document.querySelectorAll(".expl-check").forEach(se=>{se.addEventListener("change",()=>{const pe=se.dataset.id;se.checked?P.add(pe):P.delete(pe);const ce=se.closest(".exemplaire-row");ce&&(ce.style.borderColor=se.checked?"#1A6B3C":"#eee"),re()})}),document.querySelectorAll(".exemplaire-row").forEach(se=>{se.addEventListener("click",pe=>{if(pe.target.closest("button")||pe.target.tagName==="INPUT")return;const ce=se.querySelector(".expl-check");ce&&!ce.disabled&&(ce.checked=!ce.checked,ce.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(se=>{se.addEventListener("click",pe=>{pe.stopPropagation();const ce=document.querySelector(`.expl-hist[data-hist="${se.dataset.idx}"]`);ce&&(ce.style.display=ce.style.display==="none"?"flex":"none")})}),(we=document.getElementById("evolve-btn"))==null||we.addEventListener("click",async()=>{if(f<=1)return;const se=[...P];if(!se.length)return;if(P.has(e.id)){const me=document.createElement("div");me.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",me.innerHTML=`
        ...
        <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.3)">
          ...
          <div style="background:#f0fdf4;border-radius:10px;padding:12px;margin-bottom:16px;font-size:13px;color:#333">
            ...
            📈 Note : <strong>${m+(e.evolution_bonus||0)}</strong> → <strong style="color:#1A6B3C">${m+(e.evolution_bonus||0)+pe.length*ce}</strong>
  ...
  <div style="height:100%;overflow-y:auto;padding-bottom:80px;background:var(--page-bg)">
    ...
    <div class="page-body">
      ...
      stadium_def:stadium_definitions(id, name, club_id, country_code, image_url,
        club:clubs(encoded_name, logo_url))`).eq("owner_id",o.profile.id).eq("card_type","stadium"),p=(r||[]).filter(A=>A.card_type==="player"&&A.player),s=(r||[]).filter(A=>A.card_type==="formation"),n=(l||[]).filter(A=>A.card_type==="stadium"),x=[...new Set(n.map(A=>A.stadium_id).filter(Boolean))];let f={};if(n.forEach(A=>{A.stadium_def&&A.stadium_id&&(f[A.stadium_id]=A.stadium_def)}),x.length&&Object.keys(f).length<x.length){const{data:A}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",x);(A||[]).forEach(m=>{f[m.id]=m})}const h=s.map(A=>A.formation).filter(Boolean),{data:_}=await T.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let u=a.formation||"4-4-2";h.length>0&&!h.includes(u)&&(u=h[0]);const c={deckId:e,name:a.name,formation:u,formationCardId:a.formation_card_id,stadiumCardId:a.stadium_card_id||null,slots:{},subs:[],playerCards:p,formationCards:s,stadiumCards:n,stadDefMap:f,availableFormations:h};(_||[]).forEach(A=>{A.is_starter?c.slots[A.position]=A.card_id:c.subs.includes(A.card_id)||c.subs.push(A.card_id)}),ct(t,c,i)}function ct(e,t,i){var _,u,c,A,m,M,F;const{navigate:o}=i;Zt[t.formation];const d=Oi(t.formation),a=d.filter(I=>t.slots[I]).length,r=t.availableFormations.length>0?t.availableFormations:Object.keys(Zt),l=(_=t.stadiumCards)==null?void 0:_.find(I=>I.id===t.stadiumCardId),p=l&&((u=t.stadDefMap)==null?void 0:u[l.stadium_id])||null,s=t.subs.map(I=>t.playerCards.find(q=>q.id===I)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div style="height:100%;overflow-y:auto;padding-bottom:80px;background:var(--page-bg)">
      ...
      <div style="display:flex;gap:0;min-height:600px">
        ...
        <div style="width:130px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 8px;gap:10px;border-right:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="display:flex;flex-direction:column;gap:6px;align-items:center" id="subs-list">
            ${s.map(I=>{const q={...I.player,_evolution_bonus:I.evolution_bonus||0};return`<div style="position:relative;flex-shrink:0;overflow:visible">
                ${st({...q,_evolution_bonus:q._evolution_bonus||0},{width:60,showStad:!0,stadDef:p})}
        ...
        <div style="display:flex;gap:10px;align-items:flex-start">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;gap:4px;align-items:flex-end;flex-wrap:nowrap;overflow-x:auto" id="subs-list">
              ${s.map(I=>{const q={...I.player,_evolution_bonus:I.evolution_bonus||0};return`<div style="position:relative;flex-shrink:0;overflow:visible">
                  ${st({...q,_evolution_bonus:q._evolution_bonus||0},{width:44,showStad:!0,stadDef:p})}
                  <button data-remove-sub="${I.id}"
    ...
    <div class="page-body" style="padding-bottom:20px">
  ...
  </div>`;const n=window.innerWidth>=900,x=e.querySelector(".deck-pc-layout"),f=e.querySelector(".deck-mobile-layout");x&&(x.style.display=n?"block":"none"),f&&(f.style.display=n?"none":"block"),to(e,t,d,i),document.getElementById("builder-back").addEventListener("click",()=>o("decks")),document.getElementById("formation-select").addEventListener("change",I=>{t.formation=I.target.value;const q=Oi(t.formation),C={};q.forEach(G=>{t.slots[G]&&(C[G]=t.slots[G])}),t.slots=C,ct(e,t,i)});const h=()=>{const{openModal:I,closeModal:q}=i,C=`<div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px">
      ${r.map(G=>`<div data-forma="${G}" style="cursor:pointer;padding:10px 16px;border-radius:8px;background:${G===t.formation?"#1A6B3C":"#f0f0f0"};color:${G===t.formation?"#fff":"#111"};font-weight:900;font-size:16px;border:2px solid ${G===t.formation?"#1A6B3C":"#ddd"}">${G}</div>`).join("")}
    </div>`;I("⚽ Choisir une formation",C),document.querySelectorAll("#modal-body [data-forma]").forEach(G=>{G.addEventListener("click",()=>{t.formation=G.dataset.forma,q(),ct(e,t,i)})})};(c=document.getElementById("formation-mobile-btn"))==null||c.addEventListener("click",h),(A=document.getElementById("formation-pc-btn"))==null||A.addEventListener("click",h),(m=document.getElementById("add-stad-btn-pc"))==null||m.addEventListener("click",()=>Ri(t,e,i)),(M=document.getElementById("add-stad-btn"))==null||M.addEventListener("click",()=>Ri(t,e,i)),document.getElementById("save-deck").addEventListener("click",()=>oo(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(I=>{I.addEventListener("click",()=>{t.subs=t.subs.filter(q=>q!==I.dataset.removeSub),ct(e,t,i)})}),(F=document.getElementById("add-sub-btn"))==null||F.addEventListener("click",()=>{no(t,e,i)})}function to(e,t,i,o){var M,F;const d=window.innerWidth>=900,a=e.querySelector(d?"#deck-field-pc":"#deck-field-mobile");if(!a)return;const r=(M=t.stadiumCards)==null?void 0:M.find(I=>I.id===t.stadiumCardId),l=r&&((F=t.stadDefMap)==null?void 0:F[r.stadium_id])||null,p=Ht[t.formation]||{},s=Vi(t.formation)||[],n={};for(const I of i){const q=t.slots[I],C=q?t.playerCards.find(G=>G.id===q):null;C!=null&&C.player?n[I]={...C.player,_evolution_bonus:C.evolution_bonus||0,face:C.player.face||null}:n[I]=null}const x=window.innerWidth>=900,f=x?window.innerWidth-280:window.innerWidth-20,h=x?Math.min(f,860):f,_=Math.round(x?h*.82:h*1.05),u=x?84:44;let c="";for(const[I,q]of s){const C=p[I],G=p[q];if(!C||!G)continue;const V=C.x*h,K=Math.round(.03*_+C.y*.85*_),oe=G.x*h,ie=Math.round(.03*_+G.y*.85*_),Q=n[I],R=n[q],P=ot(Q,R);P==="#ff3333"||P==="#cc2222"?c+=`<line x1="${V.toFixed(1)}" y1="${K.toFixed(1)}" x2="${oe.toFixed(1)}" y2="${ie.toFixed(1)}" stroke="${P}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(c+=`<line x1="${V.toFixed(1)}" y1="${K.toFixed(1)}" x2="${oe.toFixed(1)}" y2="${ie.toFixed(1)}" stroke="${P}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,c+=`<line x1="${V.toFixed(1)}" y1="${K.toFixed(1)}" x2="${oe.toFixed(1)}" y2="${ie.toFixed(1)}" stroke="${P}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let A="";const m=Math.round(u*657/507);for(const I of i){const q=p[I];if(!q)continue;const C=n[I],G=q.x*h;q.y*_;const V=Math.round(.03*_+q.y*(.85*_)),K=Math.round(G-u/2),oe=Math.round(V-m/2);if(C){const ie=I.replace(/\d+/,""),Q=l&&(l.club_id&&String(C.club_id)===String(l.club_id)||l.country_code&&C.country_code===l.country_code),R=st({...C,_evolution_bonus:C._evolution_bonus||0},{width:u,showStad:!1,stadDef:l,role:ie});A+=`<div style="position:absolute;left:${K}px;top:${oe}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${I}">
      ...
      </div>`}else{const ie=I.replace(/\d+/,"");A+=`<div style="position:absolute;left:${K}px;top:${oe}px;width:${u}px;height:${m}px;
    ...
    <div style="position:relative;width:${h}px;height:${_}px;margin:0 auto">
      ...
      ${A}
    </div>`,a.querySelectorAll(".deck-slot-hit").forEach(I=>{I.addEventListener("click",()=>io(I.dataset.pos,t,e,o))})}function Ri(e,t,i){var r;const{openModal:o,closeModal:d}=i,a=e.stadiumCards||[];o("🏟️ Choisir un stade",`<div style="display:flex;flex-wrap:wrap;gap:10px;padding:8px">
      ...
      ${a.map(l=>{var f;const p=e.stadDefMap[l.stadium_id],s=((f=p==null?void 0:p.club)==null?void 0:f.logo_url)||null,n=p!=null&&p.image_url?"/icons/"+p.image_url:null,x=e.stadiumCardId===l.id;return`<div class="stad-choice" data-stad-id="${l.id}" style="cursor:pointer;width:70px;text-align:center">
    ...
    </div>`),(r=document.getElementById("stad-none"))==null||r.addEventListener("click",()=>{e.stadiumCardId=null,d(),ct(t,e,i)}),document.querySelectorAll(".stad-choice").forEach(l=>{l.addEventListener("click",()=>{e.stadiumCardId=l.dataset.stadId,d(),ct(t,e,i)})})}function io(e,t,i,o){var f,h,_;const{openModal:d,closeModal:a}=o,r=e.replace(/\d+/,""),l=t.slots[e],p=l?t.playerCards.find(u=>u.id===l):null;(f=p==null?void 0:p.player)==null||f.id;const s=new Set;Object.entries(t.slots).forEach(([u,c])=>{var m;if(u===e||!c)return;const A=t.playerCards.find(M=>M.id===c);(m=A==null?void 0:A.player)!=null&&m.id&&s.add(A.player.id)}),t.subs.forEach(u=>{var A;const c=t.playerCards.find(m=>m.id===u);(A=c==null?void 0:c.player)!=null&&A.id&&s.add(c.player.id)});const n=new Set,x=t.playerCards.filter(u=>{const c=u.player;return!(c.job===r||c.job2===r)||s.has(c.id)||n.has(c.id)?!1:(n.add(c.id),!0)});x.sort((u,c)=>{const A=u.evolution_bonus||0,m=c.evolution_bonus||0,M=(r==="GK"?u.player.note_g:r==="DEF"?u.player.note_d:r==="MIL"?u.player.note_m:u.player.note_a)+(r===u.player.job||r===u.player.job2?A:0);return(r==="GK"?c.player.note_g:r==="DEF"?c.player.note_d:r==="MIL"?c.player.note_m:c.player.note_a)+(r===c.player.job||r===c.player.job2?m:0)-M}),d(`Choisir ${r} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${x.length>0?'<div style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center">'+x.map(u=>{const c={...u.player,_evolution_bonus:u.evolution_bonus||0};return`<div class="player-option" data-card-id="${u.id}" style="cursor:pointer">
          ${st(c,{width:100,showStad:!0,stadDef:_stadDef,role:r})}
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(h=document.getElementById("close-selector"))==null||h.addEventListener("click",a),(_=document.getElementById("remove-player"))==null||_.addEventListener("click",()=>{delete t.slots[e],a(),ct(i,t,o)}),document.querySelectorAll(".player-option").forEach(u=>{u.addEventListener("click",()=>{t.slots[e]=u.dataset.cardId,a(),ct(i,t,o)})})}function no(e,t,i){var n,x,f;const{openModal:o,closeModal:d}=i,a=(n=e.stadiumCards)==null?void 0:n.find(h=>h.id===e.stadiumCardId),r=a&&((x=e.stadDefMap)==null?void 0:x[a.stadium_id])||null,l=new Set;Object.keys(e.slots).forEach(h=>{var c;const _=e.slots[h];if(!_)return;const u=e.playerCards.find(A=>A.id===_);(c=u==null?void 0:u.player)!=null&&c.id&&l.add(u.player.id)}),e.subs.forEach(h=>{var u;const _=e.playerCards.find(c=>c.id===h);(u=_==null?void 0:_.player)!=null&&u.id&&l.add(_.player.id)});const p=new Set,s=e.playerCards.filter(h=>{var _,u,c;return l.has((_=h.player)==null?void 0:_.id)||p.has((u=h.player)==null?void 0:u.id)?!1:(p.add((c=h.player)==null?void 0:c.id),!0)});o("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-sub-selector">Fermer</button>'),(f=document.getElementById("close-sub-selector"))==null||f.addEventListener("click",d),document.querySelectorAll(".player-option").forEach(h=>{h.addEventListener("click",()=>{e.subs.push(h.dataset.cardId),d(),ct(t,e,i)})})}async function oo(e,t){const{state:i,toast:o,navigate:d}=t,a=e.formationCards.find(p=>p.formation===e.formation),r=(a==null?void 0:a.id)||e.formationCardId;await T.from("decks").update({formation:e.formation,formation_card_id:r||null,stadium_card_id:e.stadiumCardId||null}).eq("id",e.deckId),await T.from("deck_cards").delete().eq("deck_id",e.deckId);const l=[];if(Object.entries(e.slots).forEach(([p,s],n)=>{l.push({deck_id:e.deckId,card_id:s,position:p,is_starter:!0,slot_order:n})}),e.subs.forEach((p,s)=>{l.push({deck_id:e.deckId,card_id:p,position:`SUB${s+1}`,is_starter:!1,slot_order:100+s})}),l.length>0){const{error:p}=await T.from("deck_cards").insert(l);if(p){o(p.message,"error");return}}o("Deck enregistré ✅","success"),d("decks")}function Oi(e){const t=Zt[e]||Zt["4-4-2"],i=["GK1"];for(let o=1;o<=t.DEF;o++)i.push(`DEF${o}`);for(let o=1;o<=t.MIL;o++)i.push(`MIL${o}`);for(let o=1;o<=t.ATT;o++)i.push(`ATT${o}`);return i}async function rn(e=null){const t=new Date().toISOString().slice(0,10),{data:i}=await T.from("booster_configs").select("*").eq("is_active",!0).order("sort_order");if(!(i!=null&&i.length))return[];const o=i.filter(r=>!(r.available_from&&t<r.available_from||r.available_until&&t>r.available_until));if(!o.length)return[];let d=o;if(e){const r=o.filter(l=>l.max_per_user!=null);if(r.length){const{data:l}=await T.from("booster_claims").select("booster_id").eq("user_id",e).in("booster_id",r.map(s=>s.id)),p={};(l||[]).forEach(s=>{p[s.booster_id]=(p[s.booster_id]||0)+1}),d=o.filter(s=>s.max_per_user==null?!0:(p[s.id]||0)<s.max_per_user)}}if(!d.length)return[];const{data:a}=await T.from("booster_drop_rates").select("*").in("booster_id",d.map(r=>r.id)).order("sort_order");return d.map(r=>({...r,rates:(a||[]).filter(l=>l.booster_id===r.id)}))}async function ro(e,t){const{data:i}=await T.from("booster_configs").select("max_per_user").eq("id",t).single();i!=null&&i.max_per_user&&await T.from("booster_claims").insert({user_id:e,booster_id:t})}function ao(e){if(!(e!=null&&e.length))return null;const t=e.reduce((o,d)=>o+Number(d.percentage),0);let i=Math.random()*t;for(const o of e)if(i-=Number(o.percentage),i<=0)return o;return e[e.length-1]}const an=()=>Object.keys(Ht),so=[{id:"players_std",img:"/icons/booster-players.png",name:"Players",sub:"5 cartes joueurs",cost:5e3,costLabel:"5 000 crédits",cardCount:5,type:"player"},{id:"players_pub",img:"/icons/booster-silver.png",name:"Players (pub)",sub:"3 cartes joueurs",cost:0,costLabel:"1 pub",cardCount:3,type:"player"},{id:"game_changer",img:"/icons/booster-gamechanger.png",name:"Game Changer",sub:"3 cartes spéciales",cost:1e4,costLabel:"10 000 crédits",cardCount:3,type:"game_changer"},{id:"formation",img:"/icons/booster-formation.png",name:"Formation",sub:"1 carte formation",cost:1e4,costLabel:"10 000 crédits",cardCount:1,type:"formation"}],xi={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé."},"Double attaque":{icon:"⚡",desc:"La prochaine attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la prochaine action IA."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function lo(e){const t=e.player;if(!t)return"";const i=e.evolution_bonus||0;return st({...t,_evolution_bonus:i},{width:140})}function sn(e){var d;const t={};(e.rates||[]).forEach(a=>{t[a.card_type]=(t[a.card_type]||0)+Number(a.percentage||0)});const i=((d=Object.entries(t).sort((a,r)=>r[1]-a[1])[0])==null?void 0:d[0])||"player",o=e.image_url||"booster-players.png";return{id:e.id,img:"/icons/"+o,name:e.name,sub:`${e.card_count} carte(s)`,cost:e.price_type==="credits"&&e.price_credits||0,costLabel:e.price_type==="credits"?`${(e.price_credits||0).toLocaleString("fr")} crédits`:e.price_type==="pub"?"1 pub":"Gratuit",cardCount:e.card_count||5,type:i,isPub:e.price_type==="pub",rates:e.rates||[],allow_duplicates:e.allow_duplicates!==!1,_boosterId:e.id,_raw:e}}async function co(e,{state:t,navigate:i,toast:o}){var x,f,h;const d=((x=t.profile)==null?void 0:x.credits)||0;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⏳ Chargement...</div>';let a=[];try{a=(await rn((f=t.user)==null?void 0:f.id)).map(sn)}catch(_){console.warn("Erreur chargement boosters DB, fallback hardcodé",_)}a.length||(a=so.map(_=>({..._,rates:[],isPub:_.id==="players_pub"})));const r=await T.from("cards").select("player_id, card_type, formation, stadium_id, gc_type").eq("owner_id",t.profile.id).then(_=>_.data||[]),l=new Set(r.filter(_=>_.card_type==="stadium").map(_=>_.stadium_id)),p=new Set(r.filter(_=>_.card_type==="formation").map(_=>_.formation)),s=new Set(r.filter(_=>_.card_type==="game_changer").map(_=>_.gc_type)),n={};for(const _ of a){if(_.allow_duplicates!==!1||!((h=_.rates)!=null&&h.length))continue;const u=[...new Set((_.rates||[]).map(A=>A.card_type))];let c=!1;for(const A of u)if(A==="stadium"){const{data:m}=await T.from("stadium_definitions").select("id");if((m||[]).some(M=>!l.has(M.id))){c=!0;break}}else if(A==="game_changer"){const{data:m}=await T.from("gc_definitions").select("name").eq("is_active",!0);if((m||[]).some(M=>!s.has(M.name))){c=!0;break}}else if(A==="formation"){const{FORMATION_LINKS:m}=await ni(async()=>{const{FORMATION_LINKS:M}=await import("./formation-links-BZe0pGr4.js").then(F=>F.p);return{FORMATION_LINKS:M}},__vite__mapDeps([0,1]));if(Object.keys(m).some(M=>!p.has(M))){c=!0;break}}else{c=!0;break}c||(n[_.id]=!0)}e.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📦 Boosters</h2>
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${a.map(_=>{const u=_.cost===0||d>=_.cost,c=n[_.id]===!0;return`<div class="booster-card ${!u||c?"disabled":""}" data-booster="${_.id}" style="position:relative">
  ...
  </div>`,e.querySelectorAll(".booster-card:not(.disabled)").forEach(_=>{_.addEventListener("click",async()=>{const u=a.find(c=>c.id===_.dataset.booster);if(u){_.style.opacity="0.5",_.style.pointerEvents="none";try{await po(u,{state:t,toast:o,navigate:i,container:e})}catch(c){o(c.message,"error"),_.style.opacity="",_.style.pointerEvents=""}}})}),e.querySelectorAll(".booster-info-btn").forEach(_=>{_.addEventListener("click",u=>{u.stopPropagation();const c=a.find(A=>A.id===_.dataset.boosterId);mo(c)})})}async function po(e,{state:t,toast:i,navigate:o,container:d}){var x,f;if(e.cost>0&&t.profile.credits<e.cost){i("Crédits insuffisants","error");return}e.isPub&&await yo();const{data:a}=await T.from("cards").select("card_type, player_id, formation").eq("owner_id",t.profile.id),r=new Set((a||[]).filter(h=>h.card_type==="player").map(h=>h.player_id)),l=new Set((a||[]).filter(h=>h.card_type==="formation").map(h=>h.formation));let p=[],s=null;try{if((x=e.rates)!=null&&x.length)p=await yi(t.profile,e);else{const h=e.type||"player";h==="player"?p=await dn(t.profile,e.cardCount,e.cost):h==="game_changer"?p=await ln(t.profile,e.cardCount,e.cost):h==="formation"?p=await cn(t.profile,e.cost):p=await yi(t.profile,e)}}catch(h){s=h.message||String(h),console.error("[Booster] Erreur:",h)}if(p!=null&&p.length&&e._boosterId&&await ro(t.user.id,e._boosterId),!(p!=null&&p.length)){const h=document.createElement("div");h.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",h.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
      <div style="font-size:12px;color:rgba(255,255,255,0.6);max-width:320px;word-break:break-all;background:rgba(255,255,255,0.05);padding:10px;border-radius:8px;margin-top:6px">
        ${s||"Vérifie la console (F12) pour plus de détails"}
      </div>
      <button style="margin-top:10px;padding:12px 28px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer" id="anim-close-err">Fermer</button>`,document.body.appendChild(h),(f=h.querySelector("#anim-close-err"))==null||f.addEventListener("click",()=>h.remove());return}p.forEach(h=>{h.card_type==="player"&&h.player?h.isDuplicate=r.has(h.player.id):h.card_type==="formation"&&(h.isDuplicate=l.has(h.formation))});const{data:n}=await T.from("users").select("*").eq("id",t.profile.id).single();n&&(t.profile=n),pn(p,e,o)}function uo(){const e=Math.random()*100;return e<.5?"legende":e<2?"special":e<10?"normal_high":"normal_low"}function bt(e){return Math.max(Number(e.note_g)||0,Number(e.note_d)||0,Number(e.note_m)||0,Number(e.note_a)||0)}function fo(e,t){let i;switch(t){case"legende":i=e.filter(o=>o.rarity==="legende"),i.length||(i=e.filter(o=>o.rarity==="pepite"||o.rarity==="papyte")),i.length||(i=e.filter(o=>bt(o)>=6));break;case"special":i=e.filter(o=>o.rarity==="pepite"||o.rarity==="papyte"),i.length||(i=e.filter(o=>bt(o)>=6));break;case"normal_high":i=e.filter(o=>o.rarity==="normal"&&bt(o)>=6),i.length||(i=e.filter(o=>bt(o)>=6));break;default:i=e.filter(o=>o.rarity==="normal"&&bt(o)>=1&&bt(o)<=5),i.length||(i=e.filter(o=>o.rarity==="normal"));break}return i.length||(i=e),i[Math.floor(Math.random()*i.length)]}async function yi(e,t){if(t.cost>0){const{error:f}=await T.from("users").update({credits:e.credits-t.cost}).eq("id",e.id);if(f)throw f}const i=t.allow_duplicates!==!1;let o=[];const{data:d,error:a}=await T.from("cards").select("player_id, card_type, formation, stadium_id, gc_type").eq("owner_id",e.id);if(a){const{data:f}=await T.from("cards").select("player_id, card_type, formation, gc_type").eq("owner_id",e.id);o=f||[]}else o=d||[];const r=new Set(o.filter(f=>f.card_type==="player").map(f=>f.player_id)),l=new Set(o.filter(f=>f.card_type==="formation").map(f=>f.formation)),p=new Set(o.filter(f=>f.card_type==="game_changer").map(f=>f.gc_type)),s=new Set(o.filter(f=>f.card_type==="stadium").map(f=>f.stadium_id).filter(Boolean)),n=new Set,x=[];for(let f=0;f<(t.cardCount||5);f++){const h=ao(t.rates);if(h){if(h.card_type==="player"){const _=C=>({légende:"legende",pépite:"pepite",pépites:"pepite"})[C]||C,u=h.rarity?_(h.rarity):null;let c=T.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);u&&(c=c.eq("rarity",u));const{data:A}=await c;let m=A||[];if((h.note_min||h.note_max)&&(m=m.filter(C=>{const G=Math.max(Number(C.note_g)||0,Number(C.note_d)||0,Number(C.note_m)||0,Number(C.note_a)||0);return(!h.note_min||G>=h.note_min)&&(!h.note_max||G<=h.note_max)})),m.length||(h.note_min||h.note_max?(m=A||[],console.warn("[Booster] Aucun joueur avec note",h.note_min,"-",h.note_max,"— fallback rareté uniquement")):m=A||[]),!m.length)continue;let M=m.filter(C=>!n.has(C.id));if(i)M.length||(M=m);else if(M=M.filter(C=>!r.has(C.id)),!M.length)continue;const F=M[Math.floor(Math.random()*M.length)];n.add(F.id);const I=r.has(F.id),{data:q}=await T.from("cards").insert({owner_id:e.id,player_id:F.id,card_type:"player"}).select().single();q&&(x.push({...q,player:F,isDuplicate:I}),T.rpc("record_transfer",{p_card_id:q.id,p_player_id:F.id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{}))}else if(h.card_type==="game_changer"){const{data:_}=await T.from("gc_definitions").select("id,name").eq("is_active",!0).eq("gc_type","game_changer"),u=_!=null&&_.length?_:[{name:"Ressusciter"},{name:"Double attaque"},{name:"Bouclier"},{name:"Vol de note"},{name:"Gel"}],c=i?u:u.filter(F=>!p.has(F.name));if(!i&&!c.length)continue;const m=c[Math.floor(Math.random()*c.length)].name,{data:M}=await T.from("cards").insert({owner_id:e.id,card_type:"game_changer",gc_type:m}).select().single();M&&x.push(M)}else if(h.card_type==="formation"){const _=an(),u=i?_:_.filter(M=>!l.has(M));if(!i&&!u.length)continue;const c=u[Math.floor(Math.random()*u.length)],A=l.has(c),{data:m}=await T.from("cards").insert({owner_id:e.id,card_type:"formation",formation:c}).select();m!=null&&m[0]&&x.push({...m[0],isDuplicate:A})}else if(h.card_type==="stadium"){const{data:_,error:u}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)");if(u){console.error("[Booster] stadium_definitions:",u.message);continue}if(!(_!=null&&_.length)){console.warn("[Booster] Aucun stade en DB");continue}const c=i?_:_.filter(F=>!s.has(F.id));if(!i&&!c.length)continue;const A=c[Math.floor(Math.random()*c.length)],{data:m,error:M}=await T.from("cards").insert({owner_id:e.id,card_type:"stadium",stadium_id:A.id}).select("id,card_type,stadium_id").single();if(M){console.error("[Booster] insert stadium card:",M.message);continue}m&&x.push({...m,rarity:"normal",_stadiumDef:A})}}}return x}async function dn(e,t,i){if(i>0){const{error:s}=await T.from("users").update({credits:e.credits-i}).eq("id",e.id);if(s)throw s}const{data:o}=await T.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(o!=null&&o.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const d=o.filter(s=>s.job==="GK"),a=o.filter(s=>s.job!=="GK"),r=!e.first_booster_opened&&d.length>0,l=[];for(let s=0;s<t;s++){const n=s===0&&r?d:s===0?a:o,x=uo(),f=fo(n,x);f&&l.push(f)}r&&await T.from("users").update({first_booster_opened:!0}).eq("id",e.id);const{data:p}=await T.from("cards").insert(l.map(s=>({owner_id:e.id,player_id:s.id,card_type:"player"}))).select();return(p||[]).forEach((s,n)=>{T.rpc("record_transfer",{p_card_id:s.id,p_player_id:l[n].id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{})}),l.map((s,n)=>({...p[n],player:s}))}async function ln(e,t,i){const{error:o}=await T.from("users").update({credits:e.credits-i}).eq("id",e.id);if(o)throw o;const{data:d}=await T.from("gc_definitions").select("id,name,gc_type,color,effect,image_url").eq("is_active",!0),a=d!=null&&d.length?d:Object.keys(xi).map(n=>({name:n,gc_type:"game_changer"})),r=Array.from({length:t},()=>{const n=a[Math.floor(Math.random()*a.length)];return{owner_id:e.id,card_type:"game_changer",gc_type:n.name,gc_definition_id:n.id||null}}),{data:l,error:p}=await T.from("cards").insert(r).select();return p&&console.error("[Booster GC] Erreur insert:",p.message,p),(l||[]).map(n=>{const x=d==null?void 0:d.find(f=>f.name===n.gc_type||f.id===n.gc_definition_id);return{...n,_gcDef:x||null}})}async function cn(e,t){const{error:i}=await T.from("users").update({credits:e.credits-t}).eq("id",e.id);if(i)throw i;const{data:o}=await T.from("cards").select("formation").eq("owner_id",e.id).eq("card_type","formation"),d=new Set((o||[]).map(n=>n.formation)),a=an(),r=a[Math.floor(Math.random()*a.length)],l=d.has(r),{data:p,error:s}=await T.from("cards").insert({owner_id:e.id,card_type:"formation",formation:r}).select();return s&&console.error("[Booster Formation] Erreur insert:",s.message,s),(p||[]).map(n=>({...n,isDuplicate:l}))}function pn(e,t,i,o=null){var ie,Q;if(!e||e.length===0){const R=document.createElement("div");R.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",R.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
      <div style="font-size:13px;color:rgba(255,255,255,0.5)">Erreur lors du tirage (permissions DB ou colonne manquante)</div>
      <button style="margin-top:10px;padding:12px 28px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer" id="anim-close-err">Fermer</button>`,document.body.appendChild(R),(ie=R.querySelector("#anim-close-err"))==null||ie.addEventListener("click",()=>R.remove());return}e=[...e].sort((R,P)=>{const re=R.player?bt(R.player):-1;return(P.player?bt(P.player):-1)-re});const d=document.createElement("div");d.id="booster-anim-overlay",d.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}function yo(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText=`
    ...
    `,document.body.appendChild(t);let i=5;const o=setInterval(()=>{i--;const d=document.getElementById("mw-ad-cd");d&&(d.textContent=i),i<=0&&(clearInterval(o),t.remove(),e(!0))},1e3)})}async function bo(e,{state:t,navigate:i,toast:o,refreshProfile:d}){var f,h;const{data:a}=await T.from("users").select("*").eq("id",t.user.id).single();a&&(t.profile=a);let r=Array.isArray((f=t.profile)==null?void 0:f.pending_boosters)?[...t.profile.pending_boosters]:[];if(!r.length){await T.from("users").update({onboarding_done:!0}).eq("id",t.user.id),i("home");return}let l=null;try{const u=(await rn()).find(c=>(c.name||"").toLowerCase().includes("new player"));u&&(l=sn(u))}catch(_){console.warn('[Onboarding] Config "Booster (new player)" introuvable, fallback taux par défaut',_)}const p=r.length;let s=0;e.innerHTML=`
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`}const p=n=>{e.style.overflow="",e.style.height="",e.style.display="",e.style.flexDirection="",i(n)};function s(){var x,f,h;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const n=r.length>0;e.innerHTML=`
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px 16px">
        ${a.map(_=>{const u=r.find(c=>c.gc_type===_.gc_type);return l(_,!!u)}).join("")}
      </div>
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(_=>{_.addEventListener("click",()=>{const u=_.dataset.id,c=a.find(m=>m.id===u);if(!c)return;const A=r.findIndex(m=>m.gc_type===c.gc_type);A>-1?r.splice(A,1):r.length<3&&r.push(c),s()})}),(x=e.querySelector("#gc-launch"))==null||x.addEventListener("click",()=>{n&&p(r)}),(f=e.querySelector("#gc-no-gc"))==null||f.addEventListener("click",()=>p([])),(h=e.querySelector("#gc-reset"))==null||h.addEventListener("click",()=>{r.length&&(r=[],s())})}s()}function vo(e){var o;const t=((o=e==null?void 0:e.state)==null?void 0:o.params)||{},i=t.matchMode||"vs_ai_easy";return i==="friend"?`Match vs ${t.friendName||"Ami"}`:i==="random"?"Match vs Random":`Match vs IA — ${i.replace("vs_ai_","").toUpperCase()}`}async function wo(e,t,i){const{state:o,navigate:d}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:a}=await T.from("decks").select(`id,name,is_active,formation,stadium_card_id,
      stadium_card:cards!stadium_card_id(id,stadium_id,
        stadium_def:stadium_definitions(id,name,club_id,country_code,image_url,
          club:clubs(encoded_name,logo_url)))`).eq("owner_id",o.profile.id).order("created_at",{ascending:!1});if(!a||a.length===0){Et(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>d("decks"));return}const r=a.map(f=>f.id),{data:l}=await T.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",r).order("slot_order"),p=[...new Set((l||[]).filter(f=>{var h,_;return((h=f.card)==null?void 0:h.card_type)==="stadium"&&((_=f.card)==null?void 0:_.stadium_id)}).map(f=>f.card.stadium_id))],s={};if(p.length){const{data:f}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",p);(f||[]).forEach(h=>{s[h.id]=h}),(l||[]).forEach(h=>{var _,u;((_=h.card)==null?void 0:_.card_type)==="stadium"&&((u=h.card)!=null&&u.stadium_id)&&(h.card._stadiumDef=s[h.card.stadium_id]||null)})}let n=0;function x(){var I,q,C,G,V,K,oe;const f=a[n],h=(l||[]).filter(ie=>ie.deck_id===f.id),_=h.filter(ie=>{var Q;return ie.is_starter&&((Q=ie.card)==null?void 0:Q.player)}).map(ie=>bi(ie.card,ie.position)),u=h.find(ie=>{var Q;return((Q=ie.card)==null?void 0:Q.card_type)==="formation"}),c=f.formation||((I=u==null?void 0:u.card)==null?void 0:I.formation)||"4-4-2";let A=_.length>=11?vt(_,c):null,m=((q=f.stadium_card)==null?void 0:q.stadium_def)||null;m&&A&&(A=Tt(A,m));const M=_.length>=11;et(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:hidden;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center;padding:4px">
        ${A?`<div class="deck-preview-wrap" style="aspect-ratio:1/1;max-width:100%;max-height:100%;width:auto;height:100%;overflow:hidden">${ut(A,c,null,[],285,285)}</div>`:`<div style="display:flex;align-items:center;justify-content:center;height:100%;opacity:.4;flex-direction:column;gap:8px">
    ...
    </div>`,function(){const Q=e.querySelector(".deck-preview-wrap svg");Q&&(Q.removeAttribute("width"),Q.removeAttribute("height"),Q.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",Q.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(G=document.getElementById("prev-deck"))==null||G.addEventListener("click",()=>{n>0&&(n--,x())}),(V=document.getElementById("next-deck"))==null||V.addEventListener("click",()=>{n<a.length-1&&(n++,x())}),(K=document.getElementById("validate-deck"))==null||K.addEventListener("click",()=>{if(!M)return;const ie=t.state.params||{};t.navigate("match",{...ie,matchMode:ie.matchMode||i,deckId:f.id})}),(oe=document.getElementById("cancel-deck-select"))==null||oe.addEventListener("click",()=>{Ve(e),d("home")});const F=document.getElementById("deck-swipe-zone");if(F){let ie=0,Q=0;F.addEventListener("touchstart",R=>{ie=R.touches[0].clientX,Q=R.touches[0].clientY},{passive:!0}),F.addEventListener("touchend",R=>{const P=R.changedTouches[0].clientX-ie,re=R.changedTouches[0].clientY-Q;Math.abs(P)<40||Math.abs(P)<Math.abs(re)||(P<0&&n<a.length-1?(n++,x()):P>0&&n>0&&(n--,x()))},{passive:!0})}}x()}function _o(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function Xe(e,t=44,i=58,o=null){return st(e,{width:t,showStad:!!o,stadDef:o,used:e==null?void 0:e.used})}function it(e,t,i){if(!(e!=null&&e.length))return"";const o=e.slice(0,5);let d='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return o.forEach((a,r)=>{if(d+=Xe(a,40,52),r<o.length-1){const l=ot(a,o[r+1]);d+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(d+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),d+="</div>",d}function $o(e){return!e||e.length<2?null:`https://flagsapi.com/${e.slice(0,2).toUpperCase()}/flat/64.png`}function ko(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function pt(e,t,i,o,d=310,a=310,r=[]){var M;const l=Ht[t]||{},p=Vi(t)||_i[t]||[],s={},n=["ATT","MIL","DEF","GK"];for(const F of n)(e[F]||[]).forEach((q,C)=>{s[`${F}${C+1}`]=q});function x(F){const I=l[F];return I?{x:I.x*d,y:I.y*a}:null}let f="";for(const[F,I]of p){const q=x(F),C=x(I);if(!q||!C)continue;const G=s[F],V=s[I],K=ot(G,V);K==="#00ff88"||K==="#FFD700"?(f+=`<line x1="${q.x.toFixed(1)}" y1="${q.y.toFixed(1)}" x2="${C.x.toFixed(1)}" y2="${C.y.toFixed(1)}"
        stroke="${K}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,f+=`<line x1="${q.x.toFixed(1)}" y1="${q.y.toFixed(1)}" x2="${C.x.toFixed(1)}" y2="${C.y.toFixed(1)}"
        stroke="${K}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):f+=`<line x1="${q.x.toFixed(1)}" y1="${q.y.toFixed(1)}" x2="${C.x.toFixed(1)}" y2="${C.y.toFixed(1)}"
        stroke="${K}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const h=48,_=64,u=13,c=16,A={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[F,I]of Object.entries(s)){const q=x(F);if(!q||!I)continue;const C=F.replace(/[0-9]/g,""),G=Ot[C]||"#555",V=r.includes(I.cardId),K=i==="attack"&&(["MIL","ATT"].includes(C)||V)&&!I.used||i==="defense"&&["GK","DEF","MIL"].includes(C)&&!I.used,oe=o.includes(I.cardId);let ie;i==="attack"?ie=V?Math.max(1,Number(I.note_a)||0):C==="MIL"?Number(I.note_m)||0:Number(I.note_a)||0:i==="defense"?ie=C==="GK"?Number(I.note_g)||0:C==="MIL"?Number(I.note_m)||0:Number(I.note_d)||0:ie=Number(C==="GK"?I.note_g:C==="DEF"?I.note_d:C==="MIL"?I.note_m:I.note_a)||0,ie=ie+(I.boost||0),I.stadiumBonus&&(i==="attack"&&(C==="ATT"||C==="MIL")||i==="defense"&&(C==="GK"||C==="DEF"||C==="MIL")?ie+=10:i||(ie+=10));const Q=(q.x-h/2).toFixed(1),R=(q.y-_/2).toFixed(1),P=A[I==null?void 0:I.rarity]||A.normal;if(I.used){const se=typeof import.meta<"u"&&"/"||"/",ce=`${typeof window<"u"&&((M=window.location)==null?void 0:M.origin)||""}${se}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");f+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="#161616"/>`,f+=`<image href="${ce}" xlink:href="${ce}" x="${Q}" y="${R}" width="${h}" height="${_}" preserveAspectRatio="xMidYMid slice"/>`,f+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="none" stroke="${P}" stroke-width="2" opacity="0.7"/>`,f+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${I.cardId}" data-role="${C}" style="cursor:pointer"/>`;continue}const re=oe?.45:1,xe=oe?"#FFD700":P,_e=oe?3:(I==null?void 0:I.rarity)==="legende"||(I==null?void 0:I.rarity)==="légende"||(I==null?void 0:I.rarity)==="pepite"||(I==null?void 0:I.rarity)==="pépite"?2.5:2,we=_-u-c;f+=`<defs><clipPath id="cp-${F}"><rect x="${Q}" y="${(q.y-_/2+u).toFixed(1)}" width="${h}" height="${we}"/></clipPath></defs>`,f+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="${G}" opacity="${re}"/>`;const ve=He(I);ve&&(f+=`<image href="${ve}" xlink:href="${ve}" x="${Q}" y="${(q.y-_/2+u).toFixed(1)}" width="${h}" height="${we}" clip-path="url(#cp-${F})" preserveAspectRatio="xMidYMin slice"/>`),f+=`<rect x="${Q}" y="${R}" width="${h}" height="${u}" rx="4" fill="rgba(255,255,255,0.92)"/>`,f+=`<text x="${q.x.toFixed(1)}" y="${(q.y-_/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(I.name||"").slice(0,9)}</text>`;const Ee=(q.y+_/2-c).toFixed(1);f+=`<rect x="${Q}" y="${Ee}" width="${h}" height="${c}" fill="rgba(255,255,255,0.92)"/>`;{const se=$o(I.country_code);se?f+=`<image href="${se}" xlink:href="${se}" x="${(q.x-20).toFixed(1)}" y="${(q.y+_/2-c+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:f+=`<text x="${(q.x-13).toFixed(1)}" y="${(q.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${ko(I.country_code)}</text>`;const pe=I.stadiumBonus?"#E87722":"#111";f+=`<text x="${q.x.toFixed(1)}" y="${(q.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="${pe}" font-family="Arial Black">${ie}</text>`;const ce=_o(I);ce?f+=`<image href="${ce}" xlink:href="${ce}" x="${(q.x+h/2-14).toFixed(1)}" y="${(q.y+_/2-c+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:I.clubName&&(f+=`<text x="${(q.x+14).toFixed(1)}" y="${(q.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(I.clubName||"").slice(0,3).toUpperCase()}</text>`),I.stadiumBonus&&(f+=`<rect x="${(q.x-h/2).toFixed(1)}" y="${(q.y-_/2).toFixed(1)}" width="${h}" height="${_}" rx="5" fill="none" stroke="#E87722" stroke-width="2" opacity="0.8"/>`)}f+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="${oe?"rgba(255,255,255,0.12)":"none"}" stroke="${xe}" stroke-width="${_e}" opacity="${re}"/>`,K&&(f+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${oe?"selected":""}" data-card-id="${I.cardId}" data-role="${C}" style="cursor:pointer"/>`)}const m=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-m} ${-m} ${d+m*2} ${a+m*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${f}
  </svg>`}function ut(e,t,i,o,d=300,a=300,r=[]){return`<div id="match-terrain-wrap" style="position:relative;padding:0 4px">
    ${pt(e,t,i,o,d,a,r)}
  ...
  </div>`}async function ai(e,t,i,o){var q;const{state:d,navigate:a,toast:r}=t;et(e);const l=d.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!l.deckId)return wo(e,t,i);const p=l.deckId;let s,n,x,f;try{const C=await Promise.all([T.from("decks").select("formation,name,stadium_card_id").eq("id",p).single(),T.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",p).order("slot_order")]);s=C[0].data,x=C[0].error,n=C[1].data,f=C[1].error}catch(C){console.error("[Match] Exception chargement deck:",C),Et(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>a("home"));return}if(x||f){console.error("[Match] Erreur Supabase:",x||f),Et(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>a("home"));return}const h=(n||[]).filter(C=>{var G;return C.is_starter&&((G=C.card)==null?void 0:G.player)}).map(C=>bi(C.card,C.position)),_=(n||[]).filter(C=>{var G;return!C.is_starter&&((G=C.card)==null?void 0:G.player)}).map(C=>bi(C.card,C.position));if(h.length<11){Et(e,"⚠️",`Deck incomplet (${h.length}/11).`,"Compléter",()=>a("decks"));return}const u=(n||[]).find(C=>{var G;return((G=C.card)==null?void 0:G.card_type)==="formation"}),c=(s==null?void 0:s.formation)||((q=u==null?void 0:u.card)==null?void 0:q.formation)||"4-4-2",{data:A,error:m}=await T.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",d.profile.id).eq("card_type","game_changer"),{data:M}=await T.from("gc_definitions").select("*").eq("is_active",!0),F=(A||[]).map(C=>({...C,_gcDef:(M==null?void 0:M.find(G=>G.name===C.gc_type||G.id===C.gc_definition_id))||null}));let I=null;if(s!=null&&s.stadium_card_id){const{data:C}=await T.from("cards").select("stadium_id").eq("id",s.stadium_card_id).single();if(C!=null&&C.stadium_id){const{data:G}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",C.stadium_id).single();I=G||null}}o({deckId:p,formation:c,starters:h,subsRaw:_,gcCardsEnriched:F,gcDefs:M||[],stadiumDef:I})}async function Eo(e,t){const{state:i}=t,d=(i.params||{}).matchMode||"vs_ai_easy",a=d.replace("vs_ai_",""),r=d;await ai(e,t,d,async({deckId:l,formation:p,starters:s,subsRaw:n,gcCardsEnriched:x,gcDefs:f,stadiumDef:h})=>{try{let _=vt(s,p);h&&(_=Tt(_,h),jt(n,h));const u=await To(p,a),c=u.lines||u,A=async m=>{try{const{data:M,error:F}=await T.from("matches").insert({home_id:i.profile.id,away_id:null,mode:r,home_deck_id:l,status:"in_progress"}).select().single();if(F){console.error("[MatchIA] Erreur création match:",F),Et(e,"⚠️","Impossible de créer le match ("+F.message+").","Retour",()=>t.navigate("home"));return}const I=u.stadiumDef||null;I&&c&&(Tt(c,I),jt(u.subs||[],I));const q={gcDefs:f||[],matchId:M==null?void 0:M.id,mode:r,difficulty:a,formation:p,homeTeam:_,aiTeam:c,homeSubs:n,subsUsed:0,maxSubs:Math.min(n.length,3),aiSubs:u.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((u.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:u.gcCards||[],aiUsedGc:[],aiStadiumDef:I,homeScore:0,aiScore:0,gcCards:m,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};So(e,q,t)}catch(M){console.error("[MatchIA] Exception launchMatch:",M),Et(e,"⚠️","Erreur au lancement du match : "+M.message,"Retour",()=>t.navigate("home"))}};if(!x.length){A([]);return}ri(e,x,A)}catch(_){console.error("[MatchIA] Exception setup:",_),Et(e,"⚠️","Erreur de préparation du match : "+_.message,"Retour",()=>t.navigate("home"))}})}async function To(e,t){var c;const{data:i}=await T.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:zo(e),subs:[],gcCards:[],stadiumDef:null};const o=Mt[e]||Mt["4-4-2"],d={GK:[],DEF:[],MIL:[],ATT:[]},a=new Set;function r(A,m,M){var F,I;return a.add(A.id),{cardId:"ai-"+A.id+"-"+M,id:A.id,firstname:A.firstname,name:A.surname_encoded,country_code:A.country_code,club_id:A.club_id,job:A.job,job2:A.job2,note_g:Number(A.note_g)||0,note_d:Number(A.note_d)||0,note_m:Number(A.note_m)||0,note_a:Number(A.note_a)||0,rarity:A.rarity,skin:A.skin,hair:A.hair,hair_length:A.hair_length,clubName:((F=A.clubs)==null?void 0:F.encoded_name)||null,clubLogo:((I=A.clubs)==null?void 0:I.logo_url)||null,boost:0,used:!1,_line:m}}for(const A of["GK","DEF","MIL","ATT"]){const m=i.filter(C=>C.job===A&&!a.has(C.id)),M=i.filter(C=>C.job!==A&&!a.has(C.id)),F=[...m,...M],I=[];for(let C=0;C<o[A];C++){const G=F[C];G&&I.push(r(G,A,C))}const q=dt(I.length);I.forEach((C,G)=>{C._col=q[G]}),d[A]=I}const p=i.filter(A=>!a.has(A.id)).slice(0,5).map((A,m)=>r(A,A.job,100+m)),x=Object.keys(Oe).sort(()=>Math.random()-.5).slice(0,3).map((A,m)=>{var M,F;return{id:"ai-gc-"+m,gc_type:A,name:((M=Oe[A])==null?void 0:M.name)||A,icon:((F=Oe[A])==null?void 0:F.icon)||"⚡"}}),f=Object.values(d).flat(),h={};f.forEach(A=>{A.club_id&&(h[A.club_id]=(h[A.club_id]||0)+1)});const _=(c=Object.entries(h).sort((A,m)=>m[1]-A[1])[0])==null?void 0:c[0];let u=null;if(_){const{data:A}=await T.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",_).single();A&&(u={club_id:A.id,country_code:null,name:A.encoded_name+" Stadium",club:{encoded_name:A.encoded_name,logo_url:A.logo_url}})}return{lines:d,subs:p,gcCards:x,stadiumDef:u}}function zo(e){const t=Mt[e]||Mt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},o=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let d=0;for(const a of["GK","DEF","MIL","ATT"]){const r=[];for(let p=0;p<t[a];p++){const s=3+Math.floor(Math.random()*5);r.push({cardId:"fake-"+d,id:"fake-"+d,firstname:"IA",name:o[d%o.length],country_code:"XX",club_id:null,job:a,job2:null,note_g:a==="GK"?s:2,note_d:a==="DEF"?s:2,note_m:a==="MIL"?s:2,note_a:a==="ATT"?s:2,rarity:"normal",boost:0,used:!1,_line:a}),d++}const l=dt(r.length);r.forEach((p,s)=>{p._col=l[s]}),i[a]=r}return i}function So(e,t,i){var o;e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>Ao(e,t,i),5e3)}function Ao(e,t,i){const o=t.homeTeam.MIL||[],d=t.aiTeam.MIL||[],a=t.stadiumDef||null,r=t.aiStadiumDef||null;function l(m,M){const F=Se(m,"MIL"),I=m.evolution_bonus||m._evolution_bonus||0,q=m.job==="MIL"||m.job2==="MIL"?I:0,C=m.stadiumBonus||M&&(M.club_id&&String(m.club_id)===String(M.club_id)||M.country_code&&m.country_code===M.country_code)?10:0;return F+q+C}function p(m,M){return m.reduce((F,I)=>F+l(I,M),0)}function s(m){let M=0;for(let F=0;F<m.length-1;F++){const I=ot(m[F],m[F+1]);I==="#00ff88"?M+=2:I==="#FFD700"&&(M+=1)}return M}const n=p(o,a)+s(o),x=p(d,r)+s(d),f=n>=x;function h(m,M,F,I,q){return`<div id="duel-row-${I}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${m.map((C,G)=>{const V=G<m.length-1?ot(C,m[G+1]):null,K=!V||V==="#ff3333"||V==="#cc2222",oe=V==="#00ff88"?"+2":V==="#FFD700"?"+1":"";l(C,q),C.stadiumBonus||q&&(q.club_id&&String(C.club_id)===String(q.club_id)||q.country_code&&(C.country_code,q.country_code));const ie=C.evolution_bonus||C._evolution_bonus||0;return`
          <div class="duel-card duel-card-${I}" data-idx="${G}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease, transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
            ${Xe({...C,_evolution_bonus:ie},58,78,q)}
          </div>
          ${G<m.length-1?`<div class="duel-link duel-link-${I}" data-idx="${G}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${K?"rgba(255,255,255,0.12)":V};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${K?"none":`0 0 8px ${V}`}">
      ...
      <div class="duel-score-line duel-score-line-${I}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
        Score: ${p(m,q)} + ${s(m)} liens = <b style="color:#fff">${p(m,q)+s(m)}</b>
      </div>
    </div>`}e.innerHTML=`
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
  </div>`;const _=()=>{const m=(M,F)=>e.querySelectorAll(M).forEach((I,q)=>{setTimeout(()=>{I.style.opacity="1",I.style.transform="translateY(0) scale(1)"},F+q*90)});m(".duel-card-home",150),m(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((M,F)=>{setTimeout(()=>{M.style.opacity="1"},F*70)}),900),setTimeout(()=>{const M=e.querySelector("#vs-label");M&&(M.style.opacity="1",M.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(F=>F.style.opacity="1")},1250),setTimeout(()=>{u("score-home",n,800),u("score-ai",x,800)},1500)};function u(m,M,F){const I=document.getElementById(m);if(!I)return;const q=performance.now(),C=G=>{const V=Math.min(1,(G-q)/F);I.textContent=Math.round(M*(1-Math.pow(1-V,3))),V<1?requestAnimationFrame(C):I.textContent=M};requestAnimationFrame(C)}requestAnimationFrame(_),t.attacker=f?"home":"ai";const c=f?oi():null;f&&(t.boostCard={value:c}),t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:o.map(m=>({name:m.name,note:Se(m,"MIL"),portrait:He(m),job:m.job,country_code:m.country_code,rarity:m.rarity,clubName:m.clubName,clubLogo:m.clubLogo})),aiPlayers:d.map(m=>({name:m.name,note:Se(m,"MIL"),portrait:He(m),job:m.job,country_code:m.country_code,rarity:m.rarity,clubName:m.clubName,clubLogo:m.clubLogo})),homeTotal:n,aiTotal:x,text:`Duel milieu : ${t.clubName} ${n} – ${x} IA → ${f?t.clubName+" attaque":"IA attaque"}`});const A=()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",Je(e,t,i),t.attacker==="ai"&&setTimeout(()=>ei(e,t,i),800)};setTimeout(()=>{const m=document.getElementById("score-home"),M=document.getElementById("score-ai"),F=document.getElementById(f?"duel-row-home":"duel-row-ai"),I=document.getElementById(f?"duel-row-ai":"duel-row-home"),q=f?m:M,C=f?M:m;q&&(q.style.fontSize="80px",q.style.color=f?"#FFD700":"#ff6b6b",q.style.animation="duelPulse .5s ease"+(f?", duelGlow 1.5s ease infinite .5s":"")),C&&(C.style.opacity="0.25"),setTimeout(()=>{F&&(F.style.transformOrigin="center",F.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",F.style.zIndex="5"),setTimeout(()=>{var V;const G=document.getElementById("duel-shock");if(G){const K=(V=I||F)==null?void 0:V.getBoundingClientRect(),oe=e.querySelector(".match-screen").getBoundingClientRect();K&&(G.style.top=K.top-oe.top+K.height/2+"px"),G.style.animation="shockwave .5s ease-out forwards"}I&&(I.style.transformOrigin="center",I.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var V;const G=document.getElementById("duel-finale");G&&(G.innerHTML=`
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,G.style.transition="opacity .45s ease",G.style.opacity="1",G.style.pointerEvents="auto",(V=document.getElementById("start-match-btn"))==null||V.addEventListener("click",A))},600)},700)},2800)}function Lo(e){if(e.type==="duel"&&(e.homeTotal!=null||e.aiTotal!=null)){const t=(e.homeTotal||0)>=(e.aiTotal||0);return`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const P=window.innerWidth>=700,re=(H,Z,ge)=>{var B,$;const de=(t.gcDefs||[]).find(g=>g.name===H.gc_type),me={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[de==null?void 0:de.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",ne={purple:"#b06ce0",light_blue:"#00d4ef"}[de==null?void 0:de.color]||"#b06ce0",ue=(de==null?void 0:de.name)||H.gc_type,Le=(de==null?void 0:de.effect)||((B=Oe[H.gc_type])==null?void 0:B.desc)||"",Ie=de!=null&&de.image_url?`/icons/${de.image_url}`:null,Ce=(($=Oe[H.gc_type])==null?void 0:$.icon)||"⚡",Te=Math.round(ge*.22),qe=Math.round(ge*.22),Pe=ge-Te-qe,ze=ue.length>12?7:9;return`<div class="gc-mini" data-gc-id="${H.id}" data-gc-type="${H.gc_type}"
        ...
        <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
          ${ut(t.homeTeam,t.formation,t.phase,o,300,300,n)}
        </div>
      </div>`;return P?`
  ...
  `,document.body.appendChild(o);let n=!1;const x=()=>{n||(n=!0,o.remove(),i())};o.addEventListener("click",x),setTimeout(x,2e3)}function qt(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function ui(e,t,i,o=null,d=null){var h,_;if(t.phase!=="attack"){qt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){qt(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const a=Object.entries(t.homeTeam).flatMap(([u,c])=>(c||[]).filter(A=>A.used).map(A=>({...A,_line:A._line||u}))),r=t.homeSubs.filter(u=>!t.usedSubIds.includes(u.cardId));if(!a.length){qt("Aucun joueur utilisé à remplacer");return}if(!r.length){qt("Aucun remplaçant disponible");return}let l=Math.max(0,a.findIndex(u=>u.cardId===d));const p=((h=a[l])==null?void 0:h._line)||((_=a[l])==null?void 0:_.job);let s=o?Math.max(0,r.findIndex(u=>u.cardId===o)):Math.max(0,r.findIndex(u=>u.job===p)),n=!1;const x=document.createElement("div");x.id="sub-overlay",x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function f(){var I,q,C,G,V,K;const u=a[l],c=r[s],A=Math.min(130,Math.round((window.innerWidth-90)/2)),m=Math.round(A*1.35),M=oe=>`background:rgba(255,255,255,0.12);border:none;color:${oe?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${oe?"default":"pointer"};flex-shrink:0`;x.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <button id="in-up" style="${M(s===0)}" ${s===0?"disabled":""}>▲</button>
        <div>${c?Xe({...c,used:!1,boost:0},A,m):"<div>—</div>"}</div>
        <button id="in-down" style="${M(s>=r.length-1)}" ${s>=r.length-1?"disabled":""}>▼</button>
      ...
      <div id="out-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
        ...
        <button id="out-down" style="${M(l>=a.length-1)}" ${l>=a.length-1?"disabled":""}>▼</button>
        <div style="font-size:10px;color:rgba(255,255,255,0.35)">${l+1}/${a.length}</div>
      </div>
    </div>
    <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
      <button id="sub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
    </div>`,(I=x.querySelector("#sub-close"))==null||I.addEventListener("click",()=>x.remove()),(q=x.querySelector("#out-up"))==null||q.addEventListener("click",()=>{l>0&&(l--,f())}),(C=x.querySelector("#out-down"))==null||C.addEventListener("click",()=>{l<a.length-1&&(l++,f())}),(G=x.querySelector("#in-up"))==null||G.addEventListener("click",()=>{s>0&&(s--,f())}),(V=x.querySelector("#in-down"))==null||V.addEventListener("click",()=>{s<r.length-1&&(s++,f())});const F=(oe,ie,Q,R)=>{const P=x.querySelector("#"+oe);if(!P)return;let re=0;P.addEventListener("touchstart",xe=>{re=xe.touches[0].clientY},{passive:!0}),P.addEventListener("touchend",xe=>{const _e=xe.changedTouches[0].clientY-re;if(Math.abs(_e)<30)return;const we=ie();_e<0&&we<R-1?(Q(we+1),f()):_e>0&&we>0&&(Q(we-1),f())},{passive:!0})};F("in-panel",()=>s,oe=>s=oe,r.length),F("out-panel",()=>l,oe=>l=oe,a.length),(K=x.querySelector("#sub-confirm"))==null||K.addEventListener("click",oe=>{if(oe.preventDefault(),oe.stopPropagation(),n)return;n=!0;const ie=a[l],Q=r[s];if(!ie||!Q)return;let R=null,P=-1;for(const[xe,_e]of Object.entries(t.homeTeam)){const we=(_e||[]).findIndex(ve=>ve.cardId===ie.cardId);if(we!==-1){R=xe,P=we;break}}if(P===-1||!R){qt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),x.remove();return}const re={...Q,_line:R,_col:ie._col||0,used:!1,boost:0};t.homeTeam[R].splice(P,1,re),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(Q.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:ie.name,firstname:ie.firstname,note:Se(ie,R),portrait:He(ie),job:ie.job,country_code:ie.country_code,rarity:ie.rarity,clubName:ie.clubName,clubLogo:ie.clubLogo},inPlayer:{name:Q.name,firstname:Q.firstname,note:Se(Q,R),portrait:He(Q),job:Q.job,country_code:Q.country_code,rarity:Q.rarity,clubName:Q.clubName,clubLogo:Q.clubLogo},text:`🔄 ${Q.firstname} ${Q.name} remplace ${ie.firstname} ${ie.name}`}),x.remove(),Fo(ie,Q,()=>Je(e,t,i))})}document.body.appendChild(x),f()}function Do(e,t,i,o,d){var _,u;const a=(o.gcDefs||[]).find(c=>c.name===t),r=Oe[t]||{icon:"⚡",desc:"Carte spéciale."},l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[a==null?void 0:a.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",p={purple:"#b06ce0",light_blue:"#00d4ef"}[a==null?void 0:a.color]||"#b06ce0",s=(a==null?void 0:a.name)||t,n=(a==null?void 0:a.effect)||r.desc,x=a!=null&&a.image_url?`/icons/${a.image_url}`:null,f=r.icon||"⚡",h=document.createElement("div");h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",h.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
      ${e.map(x=>{const f=x._line||x.job||"MIL",h={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[f]||"#555",_=Se(x,f)+(x.boost||0),u=l.find(c=>c.cardId===x.cardId);return`<div class="gc-pick-item" data-cid="${x.cardId}"
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ${o.map(d=>`
        <div class="player-boost-opt" data-card-id="${d.cardId}"
          ...
          <div style="width:32px;height:32px;background:${Ot[d.job]||"#888"};border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:13px">${Se(d,d._line||d.job)}</div>
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(a);let l=!1;const p=()=>{l||(l=!0,a.remove(),setTimeout(()=>d(),50))};a.addEventListener("click",p),setTimeout(p,3500)}async function Dt(e,t,i){var n,x;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:o}=i,d=t.homeScore>t.aiScore,a=t.homeScore===t.aiScore,r=d?"victoire":a?"nul":"defaite",l=On(t.mode,r);t.matchId&&await T.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:d?o.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const p={credits:(o.profile.credits||0)+l,matches_played:(o.profile.matches_played||0)+1};d?p.wins=(o.profile.wins||0)+1:a?p.draws=(o.profile.draws||0)+1:p.losses=(o.profile.losses||0)+1,await T.from("users").update(p).eq("id",o.profile.id),await i.refreshProfile();const s=document.createElement("div");s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",s.innerHTML=`
    ...
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}async function hi(e,t,i=!1){var d,a,r,l;try{const p=(a=(d=t==null?void 0:t.state)==null?void 0:d.profile)==null?void 0:a.id;try{(T.getChannels?T.getChannels():[]).forEach(n=>{const x=n.topic||"";(x.includes("matchmaking")||x.includes("pvp-match")||x.includes("friend-invite"))&&T.removeChannel(n)})}catch(s){console.warn("[Random] cleanup canaux:",s)}p&&await T.rpc("cancel_matchmaking",{p_user_id:p}).catch(()=>{})}catch{}const o=i&&((l=(r=t==null?void 0:t.state)==null?void 0:r.params)==null?void 0:l.rankedData)||null;await ai(e,t,"random",({deckId:p,formation:s,starters:n,subsRaw:x,gcCardsEnriched:f,gcDefs:h,stadiumDef:_})=>{const u=c=>Oo(e,t,p,s,n,x,c,h||[],_,i,o);if(!f.length){u([]);return}ri(e,f,u)})}async function Oo(e,t,i,o,d,a,r=[],l=[],p=null,s=!1,n=null){var q,C;const{state:x,navigate:f,toast:h}=t;let _=!1,u=null;et(e);const c=(n==null?void 0:n.mmr)??((q=x.profile)==null?void 0:q.mmr)??1e3,A=s?Gt(c):null,m=s?"linear-gradient(180deg,#1a0030,#0a1628)":"linear-gradient(180deg,#0a1628,#1a0a2e)",M=s?(A==null?void 0:A.color)||"#D4A017":"#FFD700";e.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const F=async(G=!0)=>{_=!0,u&&(T.removeChannel(u),u=null),G&&await T.rpc("cancel_matchmaking",{p_user_id:x.profile.id}).catch(()=>{})};(C=document.getElementById("mm-cancel"))==null||C.addEventListener("click",async()=>{try{await F(!0)}catch{}Ve(e),f("home")});const I=async(G,V,K)=>{if(_)return;_=!0,u&&(T.removeChannel(u),u=null);const oe=document.getElementById("mm-status");oe&&(oe.textContent="Adversaire trouvé !"),await new Promise(ie=>setTimeout(ie,600)),e.isConnected&&fn(e,t,G,K,r,l,s,n,p)};if(s){const G=[{range:50,delay:0},{range:100,delay:15e3},{range:200,delay:3e4},{range:400,delay:45e3},{range:800,delay:6e4}];let V=0,K=!1;const oe=async()=>{if(_||K)return;const{range:ie}=G[V],Q=document.getElementById("mm-range");Q&&(Q.textContent=`Plage MMR : ±${ie}`);const{data:R,error:P}=await T.rpc("try_matchmake_ranked",{p_user_id:x.profile.id,p_deck_id:i,p_mmr:c,p_range:ie});if(console.log("[Ranked] try_matchmake_ranked result:",R,"error:",JSON.stringify(P)),_)return;if(P||!(R!=null&&R.success)){h("Erreur matchmaking ranked","error"),Ve(e),f("home");return}if(R.matched){K=!0,I(R.match_id,R.opponent_id,!1);return}const re=document.getElementById("mm-status");if(re&&(re.textContent="En attente d'un adversaire..."),u||(u=T.channel("matchmaking-"+x.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${x.profile.id}`},xe=>{const _e=xe.new;_e.status==="matched"&&_e.match_id&&!K&&(K=!0,I(_e.match_id,_e.matched_with,!0))}).subscribe()),V<G.length-1){const xe=G[V+1].delay-G[V].delay;V++,setTimeout(()=>{!_&&!K&&oe()},xe)}};await oe()}else{const{data:G,error:V}=await T.rpc("try_matchmake",{p_user_id:x.profile.id,p_deck_id:i});if(V||!(G!=null&&G.success)){h("Erreur de matchmaking","error"),Ve(e),f("home");return}if(G.matched){I(G.match_id,G.opponent_id,!1);return}const K=document.getElementById("mm-status");K&&(K.textContent="En attente d'un autre joueur..."),u=T.channel("matchmaking-"+x.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${x.profile.id}`},oe=>{const ie=oe.new;ie.status==="matched"&&ie.match_id&&I(ie.match_id,ie.matched_with,!0)}).subscribe()}}async function Ho(e,t,i){const{state:o,navigate:d,toast:a}=t;try{(T.getChannels?T.getChannels():[]).forEach(s=>{const n=s.topic||"";(n.includes("matchmaking")||n.includes("pvp-match")||n.includes("friend-invite"))&&T.removeChannel(s)})}catch{}const{data:r}=await T.from("matches").select("home_id, away_id, status").eq("id",i).single();if(!r){a("Match introuvable","error"),d("home");return}if(r.status==="finished"){a("Ce match est terminé","info"),d("home");return}const l=r.home_id===o.profile.id;fn(e,t,i,l,[],[])}async function fn(e,t,i,o,d=[],a=[],r=!1,l=null,p=null){const{state:s,navigate:n,toast:x}=t,f=o?"p1":"p2",h=o?"p2":"p1",_=(d||[]).map($=>$.id),u=(d||[]).map($=>({id:$.id,gc_type:$.gc_type,_gcDef:$._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:c}=await T.from("matches").select("*").eq("id",i).single();if(!c){x("Match introuvable","error"),n("home");return}async function A(){var W,te;console.log("[PvP] buildGameState match:",{id:c.id,home_deck_id:c.home_deck_id,away_deck_id:c.away_deck_id,mode:c.mode,is_ranked:c.is_ranked});const[{data:$,error:g},{data:y,error:b},{data:v},{data:w}]=await Promise.all([T.rpc("get_deck_for_match",{p_deck_id:c.home_deck_id}),T.rpc("get_deck_for_match",{p_deck_id:c.away_deck_id}),T.from("users").select("id,pseudo,club_name").eq("id",c.home_id).single(),T.from("users").select("id,pseudo,club_name").eq("id",c.away_id).single()]);console.log("[PvP] get_deck_for_match p1:",g==null?void 0:g.message,"p2:",b==null?void 0:b.message,"p1D:",(W=$==null?void 0:$.starters)==null?void 0:W.length,"p2D:",(te=y==null?void 0:y.starters)==null?void 0:te.length);const k=ee=>{const fe=Number(ee.evolution_bonus)||0;return{cardId:ee.card_id,position:ee.position,id:ee.id,firstname:ee.firstname,name:ee.surname_encoded,country_code:ee.country_code,club_id:ee.club_id,job:ee.job,job2:ee.job2,note_g:(Number(ee.note_g)||0)+(ee.job==="GK"||ee.job2==="GK"&&Number(ee.note_g)>0?fe:0),note_d:(Number(ee.note_d)||0)+(ee.job==="DEF"||ee.job2==="DEF"&&Number(ee.note_d)>0?fe:0),note_m:(Number(ee.note_m)||0)+(ee.job==="MIL"||ee.job2==="MIL"&&Number(ee.note_m)>0?fe:0),note_a:(Number(ee.note_a)||0)+(ee.job==="ATT"||ee.job2==="ATT"&&Number(ee.note_a)>0?fe:0),evolution_bonus:fe,rarity:ee.rarity,skin:ee.skin,hair:ee.hair,hair_length:ee.hair_length,clubName:ee.club_encoded_name||null,clubLogo:ee.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},z=(($==null?void 0:$.starters)||[]).map(ee=>k(ee)),E=((y==null?void 0:y.starters)||[]).map(ee=>k(ee)),L=($==null?void 0:$.formation)||"4-4-2",S=(y==null?void 0:y.formation)||"4-4-2";let j=vt(z,L),D=vt(E,S);const Y=(($==null?void 0:$.subs)||[]).map(ee=>k(ee)),J=((y==null?void 0:y.subs)||[]).map(ee=>k(ee)),U=($==null?void 0:$.stadium_def)||(o?p:null),N=(y==null?void 0:y.stadium_def)||(o?null:p);return U&&(j=Tt(j,U),jt(Y,U)),N&&(D=Tt(D,N),jt(J,N)),{p1Team:j,p2Team:D,p1Subs:Y,p2Subs:J,p1Formation:L,p2Formation:S,p1Name:(v==null?void 0:v.club_name)||(v==null?void 0:v.pseudo)||"Joueur 1",p2Name:(w==null?void 0:w.club_name)||(w==null?void 0:w.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:o?_:[],gc_p2:o?[]:_,gcCardsFull_p1:o?u:[],gcCardsFull_p2:o?[]:u,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:a||[],lastActionAt:new Date().toISOString()}}let m=c.game_state&&Object.keys(c.game_state).length?c.game_state:null;if(console.log("[PvP] init - amIHome:",o,"gameState exists:",!!m,"match.status:",c.status,"home_id:",c.home_id,"away_id:",c.away_id,"myId:",s.profile.id),!m)if(o){m=await A(),console.log("[PvP] home - gameState built, p1Team keys:",Object.keys((m==null?void 0:m.p1Team)||{}));const{data:$}=await T.from("matches").select("game_state").eq("id",i).single();!($!=null&&$.game_state)||!Object.keys($.game_state).length?await T.from("matches").update({game_state:m,turn_user_id:c.home_id}).eq("id",i):m=$.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>',console.log("[PvP] away - waiting for home to write game_state...");for(let $=0;$<30&&!m;$++){await new Promise(y=>setTimeout(y,400));const{data:g}=await T.from("matches").select("game_state").eq("id",i).single();g!=null&&g.game_state&&Object.keys(g.game_state).length&&(m=g.game_state),$%5===0&&console.log("[PvP] away - poll",$,"game_state:",!!(g!=null&&g.game_state))}if(!m){x("Erreur de synchronisation","error"),n("home");return}m.gc_p2=_,m.gcCardsFull_p2=u,await T.from("matches").update({game_state:m}).eq("id",i)}let M=!1,F=null,I=!1;const q=new Set,C=new Set;async function G($){var S,j;try{T.removeChannel(V)}catch{}const g=m[f+"Score"]||0,y=m[h+"Score"]||0;let b,v;$.winner_id?(b=$.winner_id===s.profile.id,v=!1):$.forfeit?(b=g>y,v=!1):(v=g===y,b=g>y);let w="";if(r&&o)try{const{data:D}=await T.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",c.home_id).single(),{data:Y}=await T.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",c.away_id).single();if(D&&Y){const J=b?1:v?.5:0,U=1-J,N=D.placement_matches<10,W=Y.placement_matches<10,te=Li(D.mmr,D.mmr_deviation,D.mmr_volatility,Y.mmr,Y.mmr_deviation,J===1?1:J===0?0:.5,N),ee=Li(Y.mmr,Y.mmr_deviation,Y.mmr_volatility,D.mmr,D.mmr_deviation,U===1?1:U===0?0:.5,W);await T.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:v?null:b?c.home_id:c.away_id,p_home_id:c.home_id,p_away_id:c.away_id,p_home_delta:te.delta,p_away_delta:ee.delta,p_home_new_rd:te.newRd,p_away_new_rd:ee.newRd,p_home_new_vol:te.newSigma,p_away_new_vol:ee.newSigma});const fe=te.delta,ye=te.newMmr,be=Gt(ye),ke=fe>=0?"+":"",Be=fe>=0?"#4caf50":"#ff6b6b",Fe=Gt(D.mmr);w=`
            <div style="background:rgba(255,255,255,0.08);border-radius:14px;padding:14px 20px;text-align:center;min-width:220px">
              ...
              ${N?`<div style="font-size:11px;color:rgba(255,255,255,0.4);margin-top:6px">Match de placement (${D.placement_matches+1}/10)</div>`:""}
      ...
      <div style="font-size:18px">${m[f+"Name"]} ${g} – ${y} ${m[h+"Name"]}</div>
      ${$.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${b?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      ${w}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${r?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(k),(j=k.querySelector("#pvp-end-home"))==null||j.addEventListener("click",()=>{k.remove(),Ve(e),n(r?"ranked":"home")})}const V=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},$=>{const g=$.new;try{if(g.status==="finished"||g.forfeit){if(M)return;M=!0,F&&(clearInterval(F),F=null),g.game_state&&(m=g.game_state),G(g);return}if(g.game_state){const y=m;m=g.game_state;const b=m._lastGC;if(b&&b.seq&&!C.has(b.seq)&&(C.add(b.seq),b.by!==f)){se(b.type,b.by,()=>xe());return}const v=y[f+"Score"]||0,w=y[h+"Score"]||0,k=m[f+"Score"]||0,z=m[h+"Score"]||0,E=k>v,L=z>w;if((E||L)&&!q.has(m.round)){q.add(m.round);const S=[...m.log||[]].reverse().find(D=>D.isGoal),j=((S==null?void 0:S.homePlayers)||[]).map(D=>({name:D.name,note:D.note,portrait:D.portrait,job:D.job}));Ee(j,k,z,E,()=>xe());return}xe()}}catch(y){console.error("[PvP] crash:",y)}}).subscribe();async function K($){Object.assign(m,$),m.lastActionAt=new Date().toISOString();const{error:g}=await T.from("matches").update({game_state:m}).eq("id",i);g&&x("Erreur de synchronisation","error");try{xe()}catch(y){console.error("[PvP] renderPvpScreen crash:",y)}}async function oe(){if(M)return;M=!0,F&&(clearInterval(F),F=null);const $=o?c.away_id:c.home_id,g=o?"p2":"p1",y=o?"p1":"p2",b={...m,[g+"Score"]:3,[y+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:$,home_score:b.p1Score||0,away_score:b.p2Score||0,game_state:b}).eq("id",i)}catch{}try{T.removeChannel(V)}catch{}setTimeout(()=>{Ve(e),n("home")},800)}const ie={BOOST_STAT:({value:$=1,count:g=1,roles:y=[]},b,v)=>{const w=b[f+"Team"],k=Object.entries(w).filter(([z])=>!y.length||y.includes(z)).flatMap(([z,E])=>E.filter(L=>!L.used).map(L=>({...L,_line:z})));if(!k.length){b.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),v(b);return}Q(k,g,`Choisir ${g} joueur(s) à booster (+${$})`,z=>{z.forEach(E=>{const L=(w[E._line]||[]).find(S=>S.cardId===E.cardId);L&&(L.boost=(L.boost||0)+$,b.log.push({text:`⚡ +${$} sur ${L.name}`,type:"info"}))}),b[f+"Team"]=w,v(b)})},DEBUFF_STAT:({value:$=1,count:g=1,roles:y=[],target:b="ai"},v,w)=>{const k=b==="home"?f:h,z=v[k+"Team"],E=b==="home"?"allié":"adverse",L=Object.entries(z).filter(([S])=>!y.length||y.includes(S)).flatMap(([S,j])=>j.map(D=>({...D,_line:S})));if(!L.length){v.log.push({text:`🎯 Aucun joueur ${E}`,type:"info"}),w(v);return}Q(L,g,`Choisir ${g} joueur(s) ${E}(s) (-${$})`,S=>{S.forEach(j=>{const D=(z[j._line]||[]).find(Y=>Y.cardId===j.cardId);D&&(D.boost=(D.boost||0)-$,v.log.push({text:`🎯 -${$} sur ${D.name}`,type:"info"}))}),v[k+"Team"]=z,w(v)})},GRAY_PLAYER:({count:$=1,roles:g=[],target:y="ai"},b,v)=>{const w=y==="home"?f:h,k=b[w+"Team"],z=y==="home"?"allié":"adverse",E=Object.entries(k).filter(([L])=>!g.length||g.includes(L)).flatMap(([L,S])=>S.filter(j=>!j.used).map(j=>({...j,_line:L})));if(!E.length){b.log.push({text:`❌ Aucun joueur ${z}`,type:"info"}),v(b);return}Q(E,$,`Choisir ${$} joueur(s) ${z}(s) à exclure`,L=>{const S="usedCardIds_"+w,j=new Set(b[S]||[]);L.forEach(D=>{const Y=(k[D._line]||[]).find(J=>J.cardId===D.cardId);Y&&(Y.used=!0,j.add(D.cardId),b.log.push({text:`❌ ${Y.name} exclu !`,type:"info"}))}),b[S]=[...j],b[w+"Team"]=k,v(b)})},REVIVE_PLAYER:({count:$=1,roles:g=[]},y,b)=>{const v=y[f+"Team"],w=Object.entries(v).filter(([k])=>!g.length||g.includes(k)).flatMap(([k,z])=>z.filter(E=>E.used).map(E=>({...E,_line:k})));if(!w.length){y.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),b(y);return}Q(w,$,`Choisir ${$} joueur(s) à ressusciter`,k=>{k.forEach(z=>{const E=(v[z._line]||[]).find(L=>L.cardId===z.cardId);E&&(E.used=!1,y.log.push({text:`💫 ${E.name} ressuscité !`,type:"info"}))}),y[f+"Team"]=v,b(y)})},REMOVE_GOAL:({},$,g)=>{const y=h+"Score";$[y]>0?($[y]--,$.log.push({text:"🚫 Dernier but annulé !",type:"info"})):$.log.push({text:"🚫 Aucun but à annuler",type:"info"}),g($)},ADD_GOAL_DRAW:({},$,g)=>{$[f+"Score"]===$[h+"Score"]?($[f+"Score"]++,$.log.push({text:"🎯 But bonus !",type:"info"})):$.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),g($)},ADD_SUB:({value:$=1},g,y)=>{g.maxSubs=(g.maxSubs||3)+$,g.log.push({text:`🔄 +${$} remplacement(s)`,type:"info"}),y(g)},CUSTOM:({},$,g)=>g($)};function Q($,g,y,b){const v=document.createElement("div");v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let w=[];function k(){var E,L;const z=$.map(S=>{const j={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[S._line]||"#555",D=Se(S,S._line)+(S.boost||0),J=w.find(N=>N.cardId===S.cardId)?"#FFD700":"rgba(255,255,255,0.25)",U=S.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${S.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${J};background:${j};overflow:hidden;cursor:pointer;${U}">
        ...
        </div>`,(E=v.querySelector("#pp-close"))==null||E.addEventListener("click",()=>v.remove()),v.querySelectorAll(".pp-item").forEach(S=>{S.addEventListener("click",()=>{const j=S.dataset.cid,D=$.find(J=>J.cardId===j),Y=w.findIndex(J=>J.cardId===j);D&&(Y>-1?w.splice(Y,1):w.length<g&&w.push(D),k())})}),(L=v.querySelector("#pp-confirm"))==null||L.addEventListener("click",()=>{v.remove(),b(w)})}k(),document.body.appendChild(v)}async function R($,g){const b=(m["gcCardsFull_"+f]||[]).find(z=>z.id===$),v=(b==null?void 0:b._gcDef)||(m.gcDefs||[]).find(z=>{var E;return z.name===g||((E=z.name)==null?void 0:E.toLowerCase().trim())===(g==null?void 0:g.toLowerCase().trim())}),w=[...m["usedGc_"+f]||[],$],k={type:g,by:f,seq:(m._gcAnimSeq||0)+1};C.add(k.seq),se(g,f,async()=>{if(v!=null&&v.effect_type&&v.effect_type!=="CUSTOM"){const E=ie[v.effect_type];if(E){const L={...m};E(v.effect_params||{},L,async S=>{S["usedGc_"+f]=w,S._lastGC=k,S._gcAnimSeq=k.seq,await K(S)});return}}const z={...m};switch(g){case"Remplacement+":z.maxSubs=(z.maxSubs||3)+1,z.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const E=h+"Score";z[E]>0&&(z[E]--,z.log.push({text:"🚫 But annulé",type:"info"}));break}}z["usedGc_"+f]=w,z._lastGC=k,z._gcAnimSeq=k.seq,await K(z)})}function P($,g){const y="usedCardIds_"+$,b=new Set(m[y]||[]);g.forEach(v=>b.add(v)),m[y]=[...b]}function re(){for(const $ of["p1","p2"]){const g=new Set(m["usedCardIds_"+$]||[]),y=m[$+"Team"];if(y)for(const b of["GK","DEF","MIL","ATT"])(y[b]||[]).forEach(v=>{v.used=g.has(v.cardId)})}}function xe(){var We,nt,ft,gt,mt,le;if(m.phase==="reveal")return _e();if(m.phase==="midfield")return ve();if(m.phase==="finished")return B();const $=m[f+"Team"],g=m[h+"Team"];re();const y=m[f+"Score"],b=m[h+"Score"],v=m[f+"Name"],w=m[h+"Name"],k=m.phase===f+"-attack",z=m.phase===f+"-defense",E=Array.isArray(m["selected_"+f])?m["selected_"+f]:[],L=E.map(X=>X.cardId),S=window.innerWidth>=700,j=m[f+"Subs"]||[],D=m["usedSubIds_"+f]||[],Y=j.filter(X=>!D.includes(X.cardId)),J=m["gcCardsFull_"+f]||[],U=m["usedGc_"+f]||[],N=J.filter(X=>!U.includes(X.id)),W=m.boostOwner===f&&!m.boostUsed,te=!["GK","DEF","MIL","ATT"].some(X=>(g[X]||[]).some(O=>!O.used)),ee=[...$.MIL||[],...$.ATT||[]].filter(X=>!X.used),fe=k&&te&&ee.length===0?[...$.GK||[],...$.DEF||[]].filter(X=>!X.used).map(X=>X.cardId):[];function ye(X,O,ae){var kt,Ct;const he=X._gcDef,$e={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[he==null?void 0:he.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Qe={purple:"#b06ce0",light_blue:"#00d4ef"}[he==null?void 0:he.color]||"#b06ce0",Ze=(he==null?void 0:he.name)||X.gc_type,tt=(he==null?void 0:he.effect)||((kt=Oe[X.gc_type])==null?void 0:kt.desc)||"",rt=he!=null&&he.image_url?`/icons/${he.image_url}`:null,_t=((Ct=Oe[X.gc_type])==null?void 0:Ct.icon)||"⚡",$t=Math.round(ae*.22),lt=Math.round(ae*.22),xt=ae-$t-lt,yt=Ze.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${X.id}" data-gc-type="${X.gc_type}"
        ...
        <div style="height:${$t}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
          <span style="font-size:${yt}px;font-weight:900;color:#fff;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${O-6}px">${Ze}</span>
          <span style="font-size:6px;color:rgba(255,255,255,0.45)">⚡ GC</span>
        </div>
        <div style="height:${xt}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${rt?`<img src="${rt}" style="max-width:${O-10}px;max-height:${xt-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(xt*.55)}px">${_t}</span>`}
        </div>
        ...
        <div style="font-size:${Math.round(O*.09)}px;color:#000;font-weight:900">+${m.boostValue}</div>
      </div>`}const ke=(X,O)=>O?be(130,175):ye(X,130,175),Be=(X,O)=>O?be(68,95):ye(X,68,95),Fe=S?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Me=k?Te(f)?`<button id="pvp-action" style="${Fe};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${E.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${Fe};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:z?`<button id="pvp-action" style="${Fe};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${E.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${w}</div>`,je=k&&!Te(f)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':k||z?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${E.length}/3 sélectionné(s)</div>`:"",Ae=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${S?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ut($,m[f+"Formation"],Ne,L,300,300,fe)}
      </div>
        ...
        </div>`}return`<div style="font-size:11px;color:${X.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${X.type==="goal"?700:400};padding:3px 2px">${X.text||""}</div>`}const Ke=(()=>{var O,ae;if(z&&((O=m.pendingAttack)!=null&&O.players)){const he=m.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
        ...
        </div>`}if(k&&((ae=m.pendingAttack)!=null&&ae.players)){const he=m.pendingAttack;return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #00ff88">
          <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="width:min(90vw,420px)">${pt(m[h+"Team"],m[h+"Formation"],null,[],300,300)}</div>
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${N.map((ee,fe)=>{const ye=fe<N.length-1?ot(ee,N[fe+1]):null,be=!ye||ye==="#ff3333"||ye==="#cc2222",ke=ye==="#00ff88"?"+2":ye==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${te}" data-idx="${fe}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${Xe({...ee,note:Se(ee,"MIL"),_line:"MIL"},58,78)}
            </div>
            ${fe<N.length-1?`<div class="duel-link duel-link-${te}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${be?"rgba(255,255,255,0.12)":ye};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${be?"none":`0 0 8px ${ye}`}">
        ...
        <div class="duel-score-line duel-score-line-${te}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
          Score: ${y(N)} + ${b(N)} liens = <b style="color:#fff">${y(N)+b(N)}</b>
        </div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(w);let E=!1;const L=()=>{E||(E=!0,w.remove(),setTimeout(()=>v(),50))};w.addEventListener("click",L),setTimeout(L,3500)}function se($,g,y){var U,N;const b=(m.gcDefs||[]).find(W=>{var te;return W.name===$||((te=W.name)==null?void 0:te.toLowerCase().trim())===($==null?void 0:$.toLowerCase().trim())}),v={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[b==null?void 0:b.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",w={purple:"#b06ce0",light_blue:"#00d4ef"}[b==null?void 0:b.color]||"#b06ce0",k=(b==null?void 0:b.name)||$,z=(b==null?void 0:b.effect)||((U=Oe[$])==null?void 0:U.desc)||"",E=b!=null&&b.image_url?`/icons/${b.image_url}`:null,L=((N=Oe[$])==null?void 0:N.icon)||"⚡",j=g===f?"Vous":m[g+"Name"]||"Adversaire",D=document.createElement("div");D.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",D.innerHTML=`
      <style>
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(D);let Y=!1;const J=()=>{Y||(Y=!0,D.remove(),setTimeout(()=>y&&y(),50))};D.addEventListener("click",J),setTimeout(J,3e3)}function pe($,g){var D,Y,J,U;const b=(m["gcCardsFull_"+f]||[]).find(N=>N.id===$),v=b==null?void 0:b._gcDef,w={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[v==null?void 0:v.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",k={purple:"#b06ce0",light_blue:"#00d4ef"}[v==null?void 0:v.color]||"#b06ce0",z=(v==null?void 0:v.name)||g,E=(v==null?void 0:v.effect)||((D=Oe[g])==null?void 0:D.desc)||"Carte spéciale.",L=v!=null&&v.image_url?`/icons/${v.image_url}`:null,S=((Y=Oe[g])==null?void 0:Y.icon)||"⚡",j=document.createElement("div");j.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",j.innerHTML=`
      ...
      </div>`,document.body.appendChild(y),(b=y.querySelector("#bp-close"))==null||b.addEventListener("click",()=>y.remove()),y.querySelectorAll(".bp-item").forEach(v=>{v.addEventListener("click",async()=>{const w=v.dataset.cid,k=g.find(E=>E.cardId===w);if(!k)return;const z=($[k._line]||[]).find(E=>E.cardId===w);z&&(z.boost=(z.boost||0)+m.boostValue),y.remove(),await K({[f+"Team"]:$,boostUsed:!0})})})}function H($=null){var Y,J;if(!(m.phase===f+"-attack")){x("Remplacement uniquement avant votre attaque","warning");return}const y=m[f+"Team"],b=m["usedSubIds_"+f]||[],v=m.maxSubs||3;if(b.length>=v){x(`Maximum ${v} remplacements atteint`,"warning");return}const w=Object.entries(y).flatMap(([U,N])=>(N||[]).filter(W=>W.used).map(W=>({...W,_line:U}))),k=(m[f+"Subs"]||[]).filter(U=>!b.includes(U.cardId));if(!w.length){x("Aucun joueur utilisé à remplacer","warning");return}if(!k.length){x("Aucun remplaçant disponible","warning");return}let z=Math.max(0,w.findIndex(U=>U.cardId===$));const E=((Y=w[z])==null?void 0:Y._line)||((J=w[z])==null?void 0:J.job);let L=Math.max(0,k.findIndex(U=>U.job===E)),S=!1;const j=document.createElement("div");j.id="pvp-sub-overlay",j.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function D(){var ye,be,ke,Be,Fe,Me;const U=w[z],N=k[L],W=Math.min(130,Math.round((window.innerWidth-90)/2)),te=Math.round(W*1.35),ee=je=>`background:rgba(255,255,255,0.12);border:none;color:${je?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${je?"default":"pointer"};flex-shrink:0`;j.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          ...
          <div>${N?Xe({...N,used:!1,boost:0},W,te):"<div>—</div>"}</div>
      ...
      <div style="width:min(90vw,420px)">${pt(m[h+"Team"],m[h+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild($),(g=$.querySelector("#pvp-opp-close"))==null||g.addEventListener("click",()=>$.remove())}function de(){var y;const $=m.log||[],g=document.createElement("div");g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",g.innerHTML=`
      ...
      </div>`,document.body.appendChild(g),(y=g.querySelector("#pvp-hist-close"))==null||y.addEventListener("click",()=>g.remove())}async function me(){if(m.phase!==f+"-attack")return;const $=f==="p1"?"p2":"p1",g=(m.round||0)+1,y=[...m.log||[]];y.push({type:"info",text:`⏭️ ${m[f+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const b=qe(m),v=Te($),w=b||!v?"finished":$+"-attack";await K({["selected_"+f]:[],modifiers:{...m.modifiers,[f]:{}},pendingAttack:null,phase:w,attacker:$,round:g,log:y}),w==="finished"&&await ze(m)}async function ne(){const $=m[f+"Team"],g=!["GK","DEF","MIL","ATT"].some(w=>(m[h+"Team"][w]||[]).some(k=>!k.used)),y=(m["selected_"+f]||[]).map(w=>{const k=($[w._role]||[]).find(D=>D.cardId===w.cardId)||w,z=g&&["GK","DEF"].includes(w._role),E=$[w._role]||[],L=E.findIndex(D=>D.cardId===w.cardId),S=dt(E.length),j=L>=0?S[L]:k._col??1;return{...k,_line:w._role,_col:j,...z?{note_a:Math.max(1,Number(k.note_a)||0)}:{}}});if(!y.length)return;const b=Ut(y,m.modifiers[f]||{});P(f,y.map(w=>w.cardId)),y.forEach(w=>{const k=($[w._role]||[]).find(z=>z.cardId===w.cardId);k&&(k.used=!0)}),m["selected_"+f]=[],xe();const v=[...m.log||[]];if(g){const w=(m[f+"Score"]||0)+1,k=y.map(j=>({name:j.name,note:Se(j,j._line||"ATT"),portrait:He(j),job:j.job}));v.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:k,homeTotal:b.total,aiTotal:0});const z=(m.round||0)+1,E=f==="p1"?"p2":"p1",L={...m,[f+"Team"]:$,[f+"Score"]:w},S=qe(L);q.add(z),Ee(k,w,m[h+"Score"]||0,!0,async()=>{await K({[f+"Team"]:$,[f+"Score"]:w,["selected_"+f]:[],modifiers:{...m.modifiers,[f]:{}},pendingAttack:null,phase:S?"finished":E+"-attack",attacker:E,round:z,log:v}),S&&await ze({...m,[f+"Score"]:w})});return}v.push({type:"pending",text:`⚔️ ${m[f+"Name"]} attaque (${b.total})`}),await K({[f+"Team"]:$,[h+"Team"]:m[h+"Team"],pendingAttack:{...b,players:y,side:f},["selected_"+f]:[],modifiers:{...m.modifiers,[f]:{}},phase:h+"-defense",log:v})}async function ue(){const $=m[f+"Team"],g=(m["selected_"+f]||[]).map(U=>{const N=($[U._role]||[]).find(ye=>ye.cardId===U.cardId)||U,W=$[U._role]||[],te=W.findIndex(ye=>ye.cardId===U.cardId),ee=dt(W.length),fe=te>=0?ee[te]:N._col??1;return{...N,_line:U._role,_col:fe}}),y=Kt(g,m.modifiers[f]||{});P(f,g.map(U=>U.cardId)),g.forEach(U=>{const N=($[U._role]||[]).find(W=>W.cardId===U.cardId);N&&(N.used=!0)}),m["selected_"+f]=[],xe();const b=Vt(m.pendingAttack.total,y.total,m.modifiers[f]||{}),v=m.pendingAttack.side,w=b==="attack"||(b==null?void 0:b.goal),k=v==="p1"?"p2":"p1",z=(m.round||0)+1,E=(m.pendingAttack.players||[]).map(U=>({name:U.name,note:Se(U,U._line||"ATT"),portrait:He(U),job:U.job})),L=[...m.log||[]];L.push({type:"duel",isGoal:w,homeScored:w&&v===f,text:w?`⚽ BUT de ${m[v+"Name"]} ! (${m.pendingAttack.total} vs ${y.total})`:`✋ Attaque stoppée (${m.pendingAttack.total} vs ${y.total})`,homePlayers:E,aiPlayers:g.map(U=>({name:U.name,note:Se(U,U._line||"DEF"),portrait:He(U),job:U.job})),homeTotal:m.pendingAttack.total,aiTotal:y.total});const S=w?(m[v+"Score"]||0)+1:m[v+"Score"]||0,j={...m,[f+"Team"]:$,[v+"Score"]:S},D=qe(j),Y=D?"finished":k+"-attack",J=async()=>{await K({[f+"Team"]:$,[h+"Team"]:m[h+"Team"],[v+"Score"]:S,["selected_"+f]:[],modifiers:{...m.modifiers,[f]:{}},pendingAttack:null,phase:Y,attacker:k,round:z,log:L}),(Y==="finished"||D)&&await ze({...m,[v+"Score"]:S})};if(w){const U=v===f,N=U?S:m[f+"Score"]||0,W=U?m[h+"Score"]||0:S;q.add(z),Ee(E,N,W,U,J)}else await J()}function Le($){return["MIL","ATT"].some(g=>($[g]||[]).some(y=>!y.used))}function Ie($){return["GK","DEF","MIL","ATT"].some(g=>($[g]||[]).some(y=>!y.used))}function Ce($){return Ie($)&&!Le($)}function Te($){const g=m[$+"Team"],y=m[($==="p1"?"p2":"p1")+"Team"];return!!(Le(g)||!Ie(y)&&Ce(g))}function qe($){const g=["MIL","ATT"].some(L=>($.p1Team[L]||[]).some(S=>!S.used)),y=["MIL","ATT"].some(L=>($.p2Team[L]||[]).some(S=>!S.used)),b=Ie($.p1Team),v=Ie($.p2Team);return!g&&!(!v&&b)&&(!y&&!(!b&&v))}function Pe($){const g=$.p1Score||0,y=$.p2Score||0;return g===y?null:g>y?c.home_id:c.away_id}async function ze($){try{const g=Pe($),y=g?c.home_id===g?c.away_id:c.home_id:null;await T.from("matches").update({status:"finished",winner_id:g,home_score:$.p1Score||0,away_score:$.p2Score||0}).eq("id",i),g&&y&&updateEvolutiveCards(g,y).catch(()=>{})}catch(g){console.error("[PvP] finishMatch:",g)}}function B(){var v;const $=m[f+"Score"],g=m[h+"Score"],y=$>g,b=$===g;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      </div>`,(R=document.getElementById("lobby-cancel"))==null||R.addEventListener("click",async()=>{_=!0,u&&(T.removeChannel(u),u=null),ie&&await T.from("friend_match_invites").update({status:"declined"}).eq("id",ie),Ve(e),x("home")})},A=async(K,oe)=>{_=!0,u&&(T.removeChannel(u),u=null),await new Promise(ie=>setTimeout(ie,600)),e.isConnected&&Yo(e,t,K,oe,r,l)},{data:m}=await T.from("friend_match_invites").select("*").eq("status","pending").or(`and(inviter_id.eq.${h},invitee_id.eq.${p}),and(inviter_id.eq.${p},invitee_id.eq.${h})`).order("created_at",{ascending:!1}).limit(1).maybeSingle();let M,F;if(m&&m.inviter_id===p)F=!1,M=m.id,await T.from("friend_match_invites").update({invitee_deck_id:i,invitee_ready:!0,status:"accepted"}).eq("id",M);else{F=!0;const{data:K,error:oe}=await T.from("friend_match_invites").insert({inviter_id:h,invitee_id:p,friend_id:p,inviter_deck_id:i,inviter_ready:!0,status:"pending"}).select().single();if(oe||!K){f("Erreur création invitation","error"),Ve(e),x("home");return}M=K.id}if(c(!0,!F,M),!F){const K={home_id:p,away_id:h,home_deck_id:m.inviter_deck_id,away_deck_id:i,status:"active",mode:"friend"},oe=await T.from("matches").insert(K).select().single(),ie=oe.data;if(!ie){f("Erreur création match: "+(((V=oe.error)==null?void 0:V.message)||""),"error"),Ve(e),x("home");return}await T.from("friend_match_invites").update({match_id:ie.id,status:"matched"}).eq("id",M),A(ie.id,!1);return}let I=!1;const q=K=>{_||I||(K.status==="matched"&&K.match_id?(I=!0,clearInterval(C),clearInterval(G),A(K.match_id,!0)):K.status==="declined"?(clearInterval(C),clearInterval(G),f(`${s} a décliné l'invitation`,"warning"),Ve(e),x("home")):K.invitee_ready&&c(!0,!0,M))},C=setInterval(()=>{if(_){clearInterval(C);return}T.from("matchmaking_queue").delete().eq("user_id",h).then(()=>{},()=>{})},3e3),G=setInterval(async()=>{if(_||I){clearInterval(G);return}const{data:K}=await T.from("friend_match_invites").select("*").eq("id",M).maybeSingle();K&&q(K)},1200);u=T.channel("friend-invite-"+M).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${M}`},K=>q(K.new)).subscribe()}async function Yo(e,t,i,o,d=[],a=[]){const{state:r,navigate:l,toast:p}=t,s=o?"p1":"p2",n=o?"p2":"p1",x=(d||[]).map(B=>B.id),f=(d||[]).map(B=>({id:B.id,gc_type:B.gc_type,_gcDef:B._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:h}=await T.from("matches").select("*").eq("id",i).single();if(!h){p("Match introuvable","error"),l("home");return}async function _(){const[{data:B},{data:$},{data:g},{data:y}]=await Promise.all([T.rpc("get_deck_for_match",{p_deck_id:h.home_deck_id}),T.rpc("get_deck_for_match",{p_deck_id:h.away_deck_id}),T.from("users").select("id,pseudo,club_name").eq("id",h.home_id).single(),T.from("users").select("id,pseudo,club_name").eq("id",h.away_id).single()]),b=E=>{const L=Number(E.evolution_bonus)||0;return{cardId:E.card_id,position:E.position,id:E.id,firstname:E.firstname,name:E.surname_encoded,country_code:E.country_code,club_id:E.club_id,job:E.job,job2:E.job2,note_g:(Number(E.note_g)||0)+(E.job==="GK"||E.job2==="GK"&&Number(E.note_g)>0?L:0),note_d:(Number(E.note_d)||0)+(E.job==="DEF"||E.job2==="DEF"&&Number(E.note_d)>0?L:0),note_m:(Number(E.note_m)||0)+(E.job==="MIL"||E.job2==="MIL"&&Number(E.note_m)>0?L:0),note_a:(Number(E.note_a)||0)+(E.job==="ATT"||E.job2==="ATT"&&Number(E.note_a)>0?L:0),evolution_bonus:L,rarity:E.rarity,skin:E.skin,hair:E.hair,hair_length:E.hair_length,clubName:E.club_encoded_name||null,clubLogo:E.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},v=((B==null?void 0:B.starters)||[]).map(E=>b(E)),w=(($==null?void 0:$.starters)||[]).map(E=>b(E)),k=(B==null?void 0:B.formation)||"4-4-2",z=($==null?void 0:$.formation)||"4-4-2";return{p1Team:(()=>{const E=vt(v,k);return stadiumDef?Tt(E,stadiumDef):E})(),p2Team:vt(w,z),p1Subs:(()=>{const E=((B==null?void 0:B.subs)||[]).map(L=>b(L));return stadiumDef&&jt(E,stadiumDef),E})(),p2Subs:(($==null?void 0:$.subs)||[]).map(E=>b(E)),p1Formation:k,p2Formation:z,p1Name:(g==null?void 0:g.club_name)||(g==null?void 0:g.pseudo)||"Joueur 1",p2Name:(y==null?void 0:y.club_name)||(y==null?void 0:y.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:o?x:[],gc_p2:o?[]:x,gcCardsFull_p1:o?f:[],gcCardsFull_p2:o?[]:f,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:a||[],lastActionAt:new Date().toISOString()}}let u=h.game_state&&Object.keys(h.game_state).length?h.game_state:null;if(!u)if(o){u=await _();const{data:B}=await T.from("matches").select("game_state").eq("id",i).single();!(B!=null&&B.game_state)||!Object.keys(B.game_state).length?await T.from("matches").update({game_state:u,turn_user_id:h.home_id}).eq("id",i):u=B.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let B=0;B<60&&!u;B++){await new Promise(g=>setTimeout(g,400));const{data:$}=await T.from("matches").select("game_state").eq("id",i).single();$!=null&&$.game_state&&Object.keys($.game_state).length&&(u=$.game_state)}if(!u){p("Erreur de synchronisation","error"),l("home");return}u.gc_p2=x,u.gcCardsFull_p2=f,await T.from("matches").update({game_state:u}).eq("id",i)}let c=!1,A=!1,m=!1,M=null,F=!1;const I=new Set,q=new Set;function C(B){var E,L;try{T.removeChannel(G)}catch{}const $=u[s+"Score"]||0,g=u[n+"Score"]||0;if(!m){m=!0;const S=B.winner_id||($>g?r.profile.id:g>$?"opp":null),j=S===r.profile.id?"win":S?"loss":null;j&&ni(()=>Promise.resolve().then(()=>on),void 0).then(D=>D.applyOwnEvolution(r.profile.id,j)).catch(()=>{})}if(!A){const S=u.p1Score||0,j=u.p2Score||0,D=(u.usedGc_p1||[]).length,Y=(u.usedGc_p2||[]).length,J=B.winner_id||(S>j?h.home_id:j>S?h.away_id:null);(J?r.profile.id===J:r.profile.id<(o?h.away_id:h.home_id))&&(A=!0,Ji({player1Id:h.home_id,player2Id:h.away_id,score1:S,score2:j,gc1:D,gc2:Y}).catch(N=>console.warn("[FriendMatch] updateStats:",N)))}let y,b;B.winner_id?(y=B.winner_id===r.profile.id,b=!1):B.forfeit?(y=$>g,b=!1):(b=$===g,y=$>g),(E=document.getElementById("pvp-end-overlay"))==null||E.remove();const v=document.createElement("div");v.id="pvp-end-overlay",v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const w=b?"🤝":y?"🏆":"😞",k=b?"MATCH NUL":y?"VICTOIRE !":"DÉFAITE",z=b?"#fff":y?"#FFD700":"#ff6b6b";v.innerHTML=`
      <div style="font-size:64px">${w}</div>
      <div style="font-size:26px;font-weight:900;color:${z}">${k}</div>
      <div style="font-size:18px">${u[s+"Name"]} ${$} – ${g} ${u[n+"Name"]}</div>
      ${B.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${y?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(v),(L=v.querySelector("#pvp-end-home"))==null||L.addEventListener("click",()=>{v.remove(),Ve(e),l("home")})}const G=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},B=>{const $=B.new;try{if($.status==="finished"||$.forfeit){if(c)return;c=!0,M&&(clearInterval(M),M=null),$.game_state&&(u=$.game_state),C($);return}if($.game_state){const g=u;u=$.game_state;const y=u._lastGC;if(y&&y.seq&&!q.has(y.seq)&&(q.add(y.seq),y.by!==s)){Ee(y.type,y.by,()=>re());return}const b=g[s+"Score"]||0,v=g[n+"Score"]||0,w=u[s+"Score"]||0,k=u[n+"Score"]||0,z=w>b,E=k>v;if((z||E)&&!I.has(u.round)){I.add(u.round);const L=[...u.log||[]].reverse().find(j=>j.isGoal),S=((L==null?void 0:L.homePlayers)||[]).map(j=>({name:j.name,note:j.note,portrait:j.portrait,job:j.job}));ve(S,w,k,z,()=>re());return}re()}}catch(g){console.error("[PvP] crash:",g)}}).subscribe();async function V(B){Object.assign(u,B),u.lastActionAt=new Date().toISOString();const{error:$}=await T.from("matches").update({game_state:u}).eq("id",i);$&&p("Erreur de synchronisation","error");try{re()}catch(g){console.error("[PvP] renderPvpScreen crash:",g)}}async function K(){if(c)return;c=!0,M&&(clearInterval(M),M=null);const B=o?h.away_id:h.home_id,$=o?"p2":"p1",g=o?"p1":"p2",y={...u,[$+"Score"]:3,[g+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:B,home_score:y.p1Score||0,away_score:y.p2Score||0,game_state:y}).eq("id",i)}catch{}try{T.removeChannel(G)}catch{}setTimeout(()=>{Ve(e),l("home")},800)}const oe={BOOST_STAT:({value:B=1,count:$=1,roles:g=[]},y,b)=>{const v=y[s+"Team"],w=Object.entries(v).filter(([k])=>!g.length||g.includes(k)).flatMap(([k,z])=>z.filter(E=>!E.used).map(E=>({...E,_line:k})));if(!w.length){y.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),b(y);return}ie(w,$,`Choisir ${$} joueur(s) à booster (+${B})`,k=>{k.forEach(z=>{const E=(v[z._line]||[]).find(L=>L.cardId===z.cardId);E&&(E.boost=(E.boost||0)+B,y.log.push({text:`⚡ +${B} sur ${E.name}`,type:"info"}))}),y[s+"Team"]=v,b(y)})},DEBUFF_STAT:({value:B=1,count:$=1,roles:g=[],target:y="ai"},b,v)=>{const w=y==="home"?s:n,k=b[w+"Team"],z=y==="home"?"allié":"adverse",E=Object.entries(k).filter(([L])=>!g.length||g.includes(L)).flatMap(([L,S])=>S.map(j=>({...j,_line:L})));if(!E.length){b.log.push({text:`🎯 Aucun joueur ${z}`,type:"info"}),v(b);return}ie(E,$,`Choisir ${$} joueur(s) ${z}(s) (-${B})`,L=>{L.forEach(S=>{const j=(k[S._line]||[]).find(D=>D.cardId===S.cardId);j&&(j.boost=(j.boost||0)-B,b.log.push({text:`🎯 -${B} sur ${j.name}`,type:"info"}))}),b[w+"Team"]=k,v(b)})},GRAY_PLAYER:({count:B=1,roles:$=[],target:g="ai"},y,b)=>{const v=g==="home"?s:n,w=y[v+"Team"],k=g==="home"?"allié":"adverse",z=Object.entries(w).filter(([E])=>!$.length||$.includes(E)).flatMap(([E,L])=>L.filter(S=>!S.used).map(S=>({...S,_line:E})));if(!z.length){y.log.push({text:`❌ Aucun joueur ${k}`,type:"info"}),b(y);return}ie(z,B,`Choisir ${B} joueur(s) ${k}(s) à exclure`,E=>{const L="usedCardIds_"+v,S=new Set(y[L]||[]);E.forEach(j=>{const D=(w[j._line]||[]).find(Y=>Y.cardId===j.cardId);D&&(D.used=!0,S.add(j.cardId),y.log.push({text:`❌ ${D.name} exclu !`,type:"info"}))}),y[L]=[...S],y[v+"Team"]=w,b(y)})},REVIVE_PLAYER:({count:B=1,roles:$=[]},g,y)=>{const b=g[s+"Team"],v=Object.entries(b).filter(([w])=>!$.length||$.includes(w)).flatMap(([w,k])=>k.filter(z=>z.used).map(z=>({...z,_line:w})));if(!v.length){g.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),y(g);return}ie(v,B,`Choisir ${B} joueur(s) à ressusciter`,w=>{w.forEach(k=>{const z=(b[k._line]||[]).find(E=>E.cardId===k.cardId);z&&(z.used=!1,g.log.push({text:`💫 ${z.name} ressuscité !`,type:"info"}))}),g[s+"Team"]=b,y(g)})},REMOVE_GOAL:({},B,$)=>{const g=n+"Score";B[g]>0?(B[g]--,B.log.push({text:"🚫 Dernier but annulé !",type:"info"})):B.log.push({text:"🚫 Aucun but à annuler",type:"info"}),$(B)},ADD_GOAL_DRAW:({},B,$)=>{B[s+"Score"]===B[n+"Score"]?(B[s+"Score"]++,B.log.push({text:"🎯 But bonus !",type:"info"})):B.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),$(B)},ADD_SUB:({value:B=1},$,g)=>{$.maxSubs=($.maxSubs||3)+B,$.log.push({text:`🔄 +${B} remplacement(s)`,type:"info"}),g($)},CUSTOM:({},B,$)=>$(B)};function ie(B,$,g,y){const b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let v=[];function w(){var z,E;const k=B.map(L=>{const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[L._line]||"#555",j=Se(L,L._line)+(L.boost||0),Y=v.find(U=>U.cardId===L.cardId)?"#FFD700":"rgba(255,255,255,0.25)",J=L.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${L.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${Y};background:${S};overflow:hidden;cursor:pointer;${J}">
        ...
        </div>`,(z=b.querySelector("#pp-close"))==null||z.addEventListener("click",()=>b.remove()),b.querySelectorAll(".pp-item").forEach(L=>{L.addEventListener("click",()=>{const S=L.dataset.cid,j=B.find(Y=>Y.cardId===S),D=v.findIndex(Y=>Y.cardId===S);j&&(D>-1?v.splice(D,1):v.length<$&&v.push(j),w())})}),(E=b.querySelector("#pp-confirm"))==null||E.addEventListener("click",()=>{b.remove(),y(v)})}w(),document.body.appendChild(b)}async function Q(B,$){const y=(u["gcCardsFull_"+s]||[]).find(k=>k.id===B),b=(y==null?void 0:y._gcDef)||(u.gcDefs||[]).find(k=>{var z;return k.name===$||((z=k.name)==null?void 0:z.toLowerCase().trim())===($==null?void 0:$.toLowerCase().trim())}),v=[...u["usedGc_"+s]||[],B],w={type:$,by:s,seq:(u._gcAnimSeq||0)+1};q.add(w.seq),Ee($,s,async()=>{if(b!=null&&b.effect_type&&b.effect_type!=="CUSTOM"){const z=oe[b.effect_type];if(z){const E={...u};z(b.effect_params||{},E,async L=>{L["usedGc_"+s]=v,L._lastGC=w,L._gcAnimSeq=w.seq,await V(L)});return}}const k={...u};switch($){case"Remplacement+":k.maxSubs=(k.maxSubs||3)+1,k.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const z=n+"Score";k[z]>0&&(k[z]--,k.log.push({text:"🚫 But annulé",type:"info"}));break}}k["usedGc_"+s]=v,k._lastGC=w,k._gcAnimSeq=w.seq,await V(k)})}function R(B,$){const g="usedCardIds_"+B,y=new Set(u[g]||[]);$.forEach(b=>y.add(b)),u[g]=[...y]}function P(){for(const B of["p1","p2"]){const $=new Set(u["usedCardIds_"+B]||[]),g=u[B+"Team"];if(g)for(const y of["GK","DEF","MIL","ATT"])(g[y]||[]).forEach(b=>{b.used=$.has(b.cardId)})}}function re(){var Ge,We,nt,ft,gt,mt;if(u.phase==="reveal")return xe();if(u.phase==="midfield")return we();if(u.phase==="finished")return ze();const B=u[s+"Team"],$=u[n+"Team"];P();const g=u[s+"Score"],y=u[n+"Score"],b=u[s+"Name"],v=u[n+"Name"],w=u.phase===s+"-attack",k=u.phase===s+"-defense",z=Array.isArray(u["selected_"+s])?u["selected_"+s]:[],E=z.map(le=>le.cardId),L=window.innerWidth>=700,S=u[s+"Subs"]||[],j=u["usedSubIds_"+s]||[],D=S.filter(le=>!j.includes(le.cardId)),Y=u["gcCardsFull_"+s]||[],J=u["usedGc_"+s]||[],U=Y.filter(le=>!J.includes(le.id)),N=u.boostOwner===s&&!u.boostUsed,W=!["GK","DEF","MIL","ATT"].some(le=>($[le]||[]).some(X=>!X.used)),te=[...B.MIL||[],...B.ATT||[]].filter(le=>!le.used),ee=w&&W&&te.length===0?[...B.GK||[],...B.DEF||[]].filter(le=>!le.used).map(le=>le.cardId):[];function fe(le,X,O){var yt,kt;const ae=le._gcDef,he={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ae==null?void 0:ae.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",$e={purple:"#b06ce0",light_blue:"#00d4ef"}[ae==null?void 0:ae.color]||"#b06ce0",Qe=(ae==null?void 0:ae.name)||le.gc_type,Ze=(ae==null?void 0:ae.effect)||((yt=Oe[le.gc_type])==null?void 0:yt.desc)||"",tt=ae!=null&&ae.image_url?`/icons/${ae.image_url}`:null,rt=((kt=Oe[le.gc_type])==null?void 0:kt.icon)||"⚡",_t=Math.round(O*.22),$t=Math.round(O*.22),lt=O-_t-$t,xt=Qe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${le.id}" data-gc-type="${le.gc_type}"
        ...
        <div style="height:${_t}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
          <span style="font-size:${xt}px;font-weight:900;color:#fff;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${X-6}px">${Qe}</span>
          <span style="font-size:6px;color:rgba(255,255,255,0.45)">⚡ GC</span>
        </div>
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ut(B,u[s+"Formation"],Ae,E,300,300,ee)}
      </div>
        ...
        </div>`}if(w&&((O=u.pendingAttack)!=null&&O.players)){const ae=u.pendingAttack;return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #00ff88">
          <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
          ${it((ae.players||[]).map(he=>({...he,used:!1})),"#00ff88",ae.total)}
        </div>`}const le=(u.log||[]).slice(-1)[0];return le?'<div style="padding:2px 4px">'+Ue(le)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})(),Ke=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
        <button id="pvp-quit" style="width:34px;height:34px;border-radius:50%;background:rgba(220,50,50,0.7);border:none;color:#fff;font-size:16px;cursor:pointer;flex-shrink:0">✕</button>
        <div style="flex:1;display:flex;align-items:center;justify-content:center;gap:8px">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(v);let z=!1;const E=()=>{z||(z=!0,v.remove(),setTimeout(()=>b(),50))};v.addEventListener("click",E),setTimeout(E,3500)}function Ee(B,$,g){var J,U;const y=(u.gcDefs||[]).find(N=>{var W;return N.name===B||((W=N.name)==null?void 0:W.toLowerCase().trim())===(B==null?void 0:B.toLowerCase().trim())}),b={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[y==null?void 0:y.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",v={purple:"#b06ce0",light_blue:"#00d4ef"}[y==null?void 0:y.color]||"#b06ce0",w=(y==null?void 0:y.name)||B,k=(y==null?void 0:y.effect)||((J=Oe[B])==null?void 0:J.desc)||"",z=y!=null&&y.image_url?`/icons/${y.image_url}`:null,E=((U=Oe[B])==null?void 0:U.icon)||"⚡",S=$===s?"Vous":u[$+"Name"]||"Adversaire",j=document.createElement("div");j.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",j.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
      ...
      </div>`,document.body.appendChild(g),(y=g.querySelector("#bp-close"))==null||y.addEventListener("click",()=>g.remove()),g.querySelectorAll(".bp-item").forEach(b=>{b.addEventListener("click",async()=>{const v=b.dataset.cid,w=$.find(z=>z.cardId===v);if(!w)return;const k=(B[w._line]||[]).find(z=>z.cardId===v);k&&(k.boost=(k.boost||0)+u.boostValue),g.remove(),await V({[s+"Team"]:B,boostUsed:!0})})})}function ce(B=null){var D,Y;if(!(u.phase===s+"-attack")){p("Remplacement uniquement avant votre attaque","warning");return}const g=u[s+"Team"],y=u["usedSubIds_"+s]||[],b=u.maxSubs||3;if(y.length>=b){p(`Maximum ${b} remplacements atteint`,"warning");return}const v=Object.entries(g).flatMap(([J,U])=>(U||[]).filter(N=>N.used).map(N=>({...N,_line:J}))),w=(u[s+"Subs"]||[]).filter(J=>!y.includes(J.cardId));if(!v.length){p("Aucun joueur utilisé à remplacer","warning");return}if(!w.length){p("Aucun remplaçant disponible","warning");return}let k=Math.max(0,v.findIndex(J=>J.cardId===B));const z=((D=v[k])==null?void 0:D._line)||((Y=v[k])==null?void 0:Y.job);let E=Math.max(0,w.findIndex(J=>J.job===z)),L=!1;const S=document.createElement("div");S.id="pvp-sub-overlay",S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function j(){var fe,ye,be,ke,Be,Fe;const J=v[k],U=w[E],N=Math.min(130,Math.round((window.innerWidth-90)/2)),W=Math.round(N*1.35),te=Me=>`background:rgba(255,255,255,0.12);border:none;color:${Me?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Me?"default":"pointer"};flex-shrink:0`;S.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          ...
          <div>${U?Xe({...U,used:!1,boost:0},N,W):"<div>—</div>"}</div>
        ...
        <div id="pout-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
          ...
          <div>${J?Xe({...J,used:!1,boost:0},N,W):"<div>—</div>"}</div>
      ...
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(fe=S.querySelector("#psub-close"))==null||fe.addEventListener("click",()=>S.remove()),(ye=S.querySelector("#pout-up"))==null||ye.addEventListener("click",()=>{k>0&&(k--,j())}),(be=S.querySelector("#pout-down"))==null||be.addEventListener("click",()=>{k<v.length-1&&(k++,j())}),(ke=S.querySelector("#pin-up"))==null||ke.addEventListener("click",()=>{E>0&&(E--,j())}),(Be=S.querySelector("#pin-down"))==null||Be.addEventListener("click",()=>{E<w.length-1&&(E++,j())});const ee=(Me,je,Ae,Ne)=>{const Ue=S.querySelector("#"+Me);if(!Ue)return;let Ye=0;Ue.addEventListener("touchstart",Ke=>{Ye=Ke.touches[0].clientY},{passive:!0}),Ue.addEventListener("touchend",Ke=>{const Re=Ke.changedTouches[0].clientY-Ye;if(Math.abs(Re)<30)return;const Ge=je();Re<0&&Ge<Ne-1?(Ae(Ge+1),j()):Re>0&&Ge>0&&(Ae(Ge-1),j())},{passive:!0})};ee("pin-panel",()=>E,Me=>E=Me,w.length),ee("pout-panel",()=>k,Me=>k=Me,v.length),(Fe=S.querySelector("#psub-confirm"))==null||Fe.addEventListener("click",async Me=>{if(Me.preventDefault(),Me.stopPropagation(),L)return;L=!0;const je=v[k],Ae=w[E];if(!je||!Ae)return;const Ne=je._line,Ue=(g[Ne]||[]).findIndex(Re=>Re.cardId===je.cardId);if(Ue===-1){p("Erreur : joueur introuvable","error"),S.remove();return}const Ye={...Ae,_line:Ne,position:je.position,used:!1,boost:0};g[Ne].splice(Ue,1,Ye);const Ke=[...y,Ae.cardId];S.remove(),H(je,Ae,async()=>{await V({[s+"Team"]:g,[n+"Team"]:u[n+"Team"],["usedSubIds_"+s]:Ke})})})}document.body.appendChild(S),j()}function H(B,$,g){const y={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const v=(z,E,L)=>`<div style="text-align:center">
      ...
      </div>`,document.body.appendChild($),(y=$.querySelector("#pvp-hist-close"))==null||y.addEventListener("click",()=>$.remove())}async function de(){if(u.phase!==s+"-attack")return;const B=s==="p1"?"p2":"p1",$=(u.round||0)+1,g=[...u.log||[]];g.push({type:"info",text:`⏭️ ${u[s+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const y=Te(u),b=Ce(B),v=y||!b?"finished":B+"-attack";await V({["selected_"+s]:[],modifiers:{...u.modifiers,[s]:{}},pendingAttack:null,phase:v,attacker:B,round:$,log:g}),v==="finished"&&await Pe(u)}async function me(){const B=u[s+"Team"],$=!["GK","DEF","MIL","ATT"].some(v=>(u[n+"Team"][v]||[]).some(w=>!w.used)),g=(u["selected_"+s]||[]).map(v=>{const w=(B[v._role]||[]).find(j=>j.cardId===v.cardId)||v,k=$&&["GK","DEF"].includes(v._role),z=B[v._role]||[],E=z.findIndex(j=>j.cardId===v.cardId),L=dt(z.length),S=E>=0?L[E]:w._col??1;return{...w,_line:v._role,_col:S,...k?{note_a:Math.max(1,Number(w.note_a)||0)}:{}}});if(!g.length)return;const y=Ut(g,u.modifiers[s]||{});R(s,g.map(v=>v.cardId)),g.forEach(v=>{const w=(B[v._role]||[]).find(k=>k.cardId===v.cardId);w&&(w.used=!0)}),u["selected_"+s]=[],re();const b=[...u.log||[]];if($){const v=(u[s+"Score"]||0)+1,w=g.map(S=>({name:S.name,note:Se(S,S._line||"ATT"),portrait:He(S),job:S.job}));b.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:w,homeTotal:y.total,aiTotal:0});const k=(u.round||0)+1,z=s==="p1"?"p2":"p1",E={...u,[s+"Team"]:B,[s+"Score"]:v},L=Te(E);I.add(k),ve(w,v,u[n+"Score"]||0,!0,async()=>{await V({[s+"Team"]:B,[s+"Score"]:v,["selected_"+s]:[],modifiers:{...u.modifiers,[s]:{}},pendingAttack:null,phase:L?"finished":z+"-attack",attacker:z,round:k,log:b}),L&&await Pe({...u,[s+"Score"]:v})});return}b.push({type:"pending",text:`⚔️ ${u[s+"Name"]} attaque (${y.total})`}),await V({[s+"Team"]:B,[n+"Team"]:u[n+"Team"],pendingAttack:{...y,players:g,side:s},["selected_"+s]:[],modifiers:{...u.modifiers,[s]:{}},phase:n+"-defense",log:b})}async function ne(){const B=u[s+"Team"],$=(u["selected_"+s]||[]).map(J=>{const U=(B[J._role]||[]).find(fe=>fe.cardId===J.cardId)||J,N=B[J._role]||[],W=N.findIndex(fe=>fe.cardId===J.cardId),te=dt(N.length),ee=W>=0?te[W]:U._col??1;return{...U,_line:J._role,_col:ee}}),g=Kt($,u.modifiers[s]||{});R(s,$.map(J=>J.cardId)),$.forEach(J=>{const U=(B[J._role]||[]).find(N=>N.cardId===J.cardId);U&&(U.used=!0)}),u["selected_"+s]=[],re();const y=Vt(u.pendingAttack.total,g.total,u.modifiers[s]||{}),b=u.pendingAttack.side,v=y==="attack"||(y==null?void 0:y.goal),w=b==="p1"?"p2":"p1",k=(u.round||0)+1,z=(u.pendingAttack.players||[]).map(J=>({name:J.name,note:Se(J,J._line||"ATT"),portrait:He(J),job:J.job})),E=[...u.log||[]];E.push({type:"duel",isGoal:v,homeScored:v&&b===s,text:v?`⚽ BUT de ${u[b+"Name"]} ! (${u.pendingAttack.total} vs ${g.total})`:`✋ Attaque stoppée (${u.pendingAttack.total} vs ${g.total})`,homePlayers:z,aiPlayers:$.map(J=>({name:J.name,note:Se(J,J._line||"DEF"),portrait:He(J),job:J.job})),homeTotal:u.pendingAttack.total,aiTotal:g.total});const L=v?(u[b+"Score"]||0)+1:u[b+"Score"]||0,S={...u,[s+"Team"]:B,[b+"Score"]:L},j=Te(S),D=j?"finished":w+"-attack",Y=async()=>{await V({[s+"Team"]:B,[n+"Team"]:u[n+"Team"],[b+"Score"]:L,["selected_"+s]:[],modifiers:{...u.modifiers,[s]:{}},pendingAttack:null,phase:D,attacker:w,round:k,log:E}),(D==="finished"||j)&&await Pe({...u,[b+"Score"]:L})};if(v){const J=b===s,U=J?L:u[s+"Score"]||0,N=J?u[n+"Score"]||0:L;I.add(k),ve(z,U,N,J,Y)}else await Y()}function ue(B){return["MIL","ATT"].some($=>(B[$]||[]).some(g=>!g.used))}function Le(B){return["GK","DEF","MIL","ATT"].some($=>(B[$]||[]).some(g=>!g.used))}function Ie(B){return Le(B)&&!ue(B)}function Ce(B){const $=u[B+"Team"],g=u[(B==="p1"?"p2":"p1")+"Team"];return!!(ue($)||!Le(g)&&Ie($))}function Te(B){const $=["MIL","ATT"].some(E=>(B.p1Team[E]||[]).some(L=>!L.used)),g=["MIL","ATT"].some(E=>(B.p2Team[E]||[]).some(L=>!L.used)),y=Le(B.p1Team),b=Le(B.p2Team);return!$&&!(!b&&y)&&(!g&&!(!y&&b))}function qe(B){const $=B.p1Score||0,g=B.p2Score||0;return $===g?null:$>g?h.home_id:h.away_id}async function Pe(B){try{const $=qe(B),g=$?h.home_id===$?h.away_id:h.home_id:null;await T.from("matches").update({status:"finished",winner_id:$,home_score:B.p1Score||0,away_score:B.p2Score||0}).eq("id",i),$&&g&&Ti($,g).catch(()=>{})}catch($){console.error("[PvP] finishMatch:",$)}}function ze(){var b;const B=u[s+"Score"],$=u[n+"Score"],g=B>$,y=B===$;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="display:flex;flex-wrap:wrap;gap:8px;font-size:11px;color:#666;margin-bottom:${i?"10px":"0"}">
      ...
      ${d>0?`<span style="color:${ht};font-weight:700">🏆 ${d.toLocaleString("fr")} cr.</span>`:""}
    ...
    <div style="padding:20px;display:flex;flex-direction:column;gap:14px">
      ...
      </div>`,document.body.appendChild(t);const i=t.querySelector("#ml-pwd-input"),o=d=>{t.remove(),e(d)};setTimeout(()=>i==null?void 0:i.focus(),80),t.querySelector("#pwd-cancel").addEventListener("click",()=>o(null)),t.querySelector("#pwd-ok").addEventListener("click",()=>o(i.value.trim())),i.addEventListener("keydown",d=>{d.key==="Enter"&&o(i.value.trim())}),t.addEventListener("click",d=>{d.target===t&&o(null)})})}async function gn(e,t,i,o){const{toast:d,state:a}=t,r=a.profile.id,{data:l}=await T.from("mini_leagues").select("password,status,entry_fee,pot,max_players").eq("id",i).single();if(!l){d("Mini League introuvable","error");return}if(l.status!=="waiting"){d("Cette Mini League a déjà démarré","warning");return}const{count:p}=await T.from("mini_league_members").select("id",{count:"exact",head:!0}).eq("league_id",i);if(p>=l.max_players){d("La Mini League est complète","warning");return}if(o==="private"){const u=await er();if(u===null)return;if(l.password!==u){d("Mot de passe incorrect","error");return}}const s=l.entry_fee||100,{data:n}=await T.from("users").select("credits").eq("id",r).single();if(((n==null?void 0:n.credits)||0)<s){d(`Crédits insuffisants — il te faut ${s.toLocaleString("fr")} cr. (solde : ${((n==null?void 0:n.credits)||0).toLocaleString("fr")} cr.)`,"error");return}const{error:x}=await T.from("mini_league_members").insert({league_id:i,user_id:r});if(x){d("Erreur inscription : "+x.message,"error");return}const{error:f}=await T.from("users").update({credits:n.credits-s}).eq("id",r),{error:h}=await T.from("mini_leagues").update({pot:(l.pot||0)+s}).eq("id",i);(f||h)&&console.error("Erreur post-inscription:",f,h),a.profile&&(a.profile.credits=n.credits-s);const _=document.getElementById("nav-credits");_&&(_.textContent=`💰 ${(a.profile.credits||0).toLocaleString("fr")}`),d(`✅ Inscrit ! −${s.toLocaleString("fr")} cr. · Pot : ${((l.pot||0)+s).toLocaleString("fr")} cr.`,"success"),wt(e,t,i)}async function tr(e,t,i,o){const{toast:d,state:a}=t,r=a.profile.id;if(!confirm(`Te désinscrire et récupérer ${o.toLocaleString("fr")} cr. ?`))return;const{data:l}=await T.from("mini_leagues").select("pot,status").eq("id",i).single();if(!l||l.status!=="waiting"){d("Impossible — la league a déjà démarré","error");return}const{data:p}=await T.from("users").select("credits").eq("id",r).single();await T.from("mini_leagues").update({pot:Math.max(0,(l.pot||0)-o)}).eq("id",i),await T.from("users").update({credits:((p==null?void 0:p.credits)||0)+o}).eq("id",r),await T.from("mini_league_members").delete().eq("league_id",i).eq("user_id",r),a.profile&&(a.profile.credits=((p==null?void 0:p.credits)||0)+o);const s=document.getElementById("nav-credits");s&&(s.textContent=`💰 ${(a.profile.credits||0).toLocaleString("fr")}`),d(`↩️ Désinscrit · +${o.toLocaleString("fr")} cr. remboursés`,"success"),zt(e,t,"waiting")}async function vi(e,t,i,o,d){const{toast:a}=t;if(!confirm(`Supprimer définitivement "${o}" et tous ses matchs/membres ? Action irréversible.`))return;await T.from("mini_league_matches").delete().eq("league_id",i),await T.from("mini_league_members").delete().eq("league_id",i);const{error:r}=await T.from("mini_leagues").delete().eq("id",i);if(r){a("Erreur suppression ("+r.message+")","error");return}a("Mini League supprimée avec succès","success"),zt(e,t,d)}async function ir(e,t,i){await T.from("mini_leagues").update({is_archived:!0}).eq("id",i),t.toast("Mini League archivée","success"),zt(e,t,"archived")}async function wt(e,t,i){var F,I,q,C,G,V,K,oe,ie;const{state:o,toast:d}=t,a=o.profile.id,[{data:r},{data:l},{data:p}]=await Promise.all([T.from("mini_leagues").select("*").eq("id",i).single(),T.from("mini_league_members").select("*, user:users(id,pseudo,club_name,club_color1,club_color2)").eq("league_id",i),T.from("mini_league_matches").select("*").eq("league_id",i).order("matchday").order("created_at")]);if(!r){d("Introuvable","error"),await zt(e,t);return}const s=(l||[]).some(Q=>Q.user_id===a),n=r.creator_id===a,x=(l||[]).map(Q=>Q.user).filter(Boolean),f=xn(x,p||[]),h=(p||[]).filter(Q=>Q.matchday===r.current_day),_=r.pot||0,u=r.entry_fee||100,c=h.length>0&&h.every(Q=>Q.status==="finished"||Q.status==="bye"),A=r.current_day>=r.total_days,m=(l||[]).find(Q=>Q.user_id===a);e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${f.map((Q,R)=>{const P=_>0&&r.status==="finished"?R===0?Math.floor(_*.7):R===1?Math.floor(_*.2):R===2?Math.floor(_*.1):0:0;return`<tr style="border-bottom:1px solid #f5f5f5;${Q.userId===a?"background:#f0fdf4;":""}">
              <td style="padding:7px 3px 7px 0;font-weight:700;color:${R===0?ht:R<3?It:"#555"}">${["🥇","🥈","🥉"][R]||R+1}</td>
              ...
              ${_>0&&r.status==="finished"?`<td style="text-align:right;font-weight:700;color:${ht}">${P?P.toLocaleString("fr")+" cr.":"—"}</td>`:""}
            </tr>`}).join("")}</tbody>
        </table>
      </div>`:""}
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,r.status==="active"?r.current_day-1:r.current_day)},(Q,R)=>R+1).reverse().map(Q=>{const R=(p||[]).filter(P=>P.matchday===Q);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${Q}</div>${R.map(P=>Hi(P,x,a,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
    ...
    <div style="background:linear-gradient(160deg,#1a0a2e,#3b1a6e);border-radius:20px;padding:32px 24px;max-width:320px;width:100%;text-align:center;color:#fff;box-shadow:0 8px 40px rgba(0,0,0,0.6)">
      ...
      <div style="background:rgba(212,160,23,0.2);border:2px solid ${ht};border-radius:14px;padding:16px;margin-bottom:24px">
        ...
        <div style="font-size:32px;font-weight:900;color:${ht}">${s.toLocaleString("fr")} cr.</div>
      ...
      ${n?'<div style="font-size:13px;color:#86efac;margin-bottom:16px">✅ Crédits déjà récupérés</div>':`<button id="claim-prize-btn" style="width:100%;padding:14px;border-radius:12px;border:none;background:${ht};color:#111;font-size:16px;font-weight:900;cursor:pointer;margin-bottom:12px">💰 Récupérer ${s.toLocaleString("fr")} cr.</button>`}
      <button id="prize-close" style="background:rgba(255,255,255,0.1);border:none;color:rgba(255,255,255,0.7);padding:10px 24px;border-radius:10px;font-size:14px;cursor:pointer">Fermer</button>
    </div>`,document.body.appendChild(x),(f=x.querySelector("#prize-close"))==null||f.addEventListener("click",()=>x.remove()),x.addEventListener("click",_=>{_.target===x&&x.remove()}),(h=x.querySelector("#claim-prize-btn"))==null||h.addEventListener("click",async()=>{const{data:_}=await T.from("users").select("credits").eq("id",a.profile.id).single();await T.from("users").update({credits:((_==null?void 0:_.credits)||0)+s}).eq("id",a.profile.id),await T.from("mini_league_members").update({prize_claimed:!0}).eq("league_id",i.id).eq("user_id",a.profile.id),a.profile&&(a.profile.credits=((_==null?void 0:_.credits)||0)+s);const u=document.getElementById("nav-credits");u&&(u.textContent=`💰 ${a.profile.credits.toLocaleString("fr")}`),r(`💰 +${s.toLocaleString("fr")} cr. ajoutés à ton solde !`,"success"),x.remove(),wt(e,t,i.id)})}function ar(e,t){const o=e.length%2===0?[...e]:[...e,null],d=o.length;let a=o.slice(1);const r=[];for(let l=0;l<d-1;l++){const p=[],s=[o[0],...a];for(let n=0;n<d/2;n++){const x=s[n],f=s[d-1-n];x===null?p.push({bye:f}):f===null?p.push({bye:x}):p.push({home:x,away:f})}r.push(p),a=[a[a.length-1],...a.slice(0,-1)]}return t==="aller-retour"?[...r,...r.map(l=>l.map(p=>p.bye?p:{home:p.away,away:p.home}))]:r}function xn(e,t){const i={};return e.forEach(o=>{i[o.id]={userId:o.id,pseudo:o.pseudo,clubName:o.club_name||o.pseudo,played:0,won:0,drawn:0,lost:0,goalsFor:0,goalsAgainst:0,goalDiff:0,points:0}}),t.filter(o=>o.status==="finished"&&!o.is_bye&&o.home_score!=null).forEach(o=>{const d=i[o.home_id],a=i[o.away_id];!d||!a||(d.played++,a.played++,d.goalsFor+=o.home_score,d.goalsAgainst+=o.away_score,a.goalsFor+=o.away_score,a.goalsAgainst+=o.home_score,o.home_score>o.away_score?(d.won++,d.points+=3,a.lost++):o.home_score<o.away_score?(a.won++,a.points+=3,d.lost++):(d.drawn++,d.points++,a.drawn++,a.points++),d.goalDiff=d.goalsFor-d.goalsAgainst,a.goalDiff=a.goalsFor-a.goalsAgainst)}),Object.values(i).sort((o,d)=>d.points-o.points||d.goalDiff-o.goalDiff||d.goalsFor-o.goalsFor)}async function Ui(e,t,i,o){var d,a;t.state.params={...t.state.params,matchMode:"mini-league",mlMatchId:i,leagueId:o};try{const r=(a=(d=t==null?void 0:t.state)==null?void 0:d.profile)==null?void 0:a.id;try{(T.getChannels?T.getChannels():[]).forEach(p=>{const s=p.topic||"";(s.includes("matchmaking")||s.includes("pvp-match"))&&T.removeChannel(p)})}catch(l){console.warn("[FriendMatch] cleanup canaux:",l)}r&&(await T.rpc("cancel_matchmaking",{p_user_id:r}).catch(()=>{}),await T.from("matchmaking_queue").delete().eq("user_id",r).then(()=>{},()=>{}))}catch{}await ai(e,t,"random",({deckId:r,formation:l,starters:p,subsRaw:s,gcCardsEnriched:n,gcDefs:x,stadiumDef:f})=>{const h=_=>sr(e,t,r,l,p,s,_,x||[],i);if(!n.length){h([]);return}ri(e,n,h)})}async function sr(e,t,i,o,d,a,r=[],l=[],p,s){const{state:n,navigate:x,toast:f}=t,h=n.profile.id;let _=!1,u=null;et(e);try{await T.rpc("cancel_matchmaking",{p_user_id:h})}catch{}try{await T.from("matchmaking_queue").delete().eq("user_id",h)}catch{}const{data:c}=await T.from("mini_league_matches").select("*, home:users!home_id(id,pseudo,club_name), away:users!away_id(id,pseudo,club_name)").eq("id",p).single();if(!c){f("Match introuvable","error"),Ve(e),x("mini-league");return}if(c.home_id!==h&&c.away_id!==h){f("Tu ne fais pas partie de ce match","error"),Ve(e),x("mini-league");return}const A=c.home_id===h,m=A?c.away:c.home,M=A?c.away_id:c.home_id,F=(q,C)=>{var G;e.innerHTML=`
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:20px;background:linear-gradient(180deg,#0a1628,#1a0a2e);padding:24px;text-align:center;color:#fff">
        ...
        <button id="lobby-cancel" style="padding:10px 24px;border-radius:10px;border:1.5px solid rgba(255,255,255,0.2);background:transparent;color:rgba(255,255,255,0.6);font-size:13px;cursor:pointer">Annuler</button>
        <style>@keyframes lobbyPulse{0%,100%{opacity:1}50%{opacity:0.4}}</style>
      </div>`,(G=document.getElementById("lobby-cancel"))==null||G.addEventListener("click",()=>{_=!0,clearInterval(u),Ve(e),x("mini-league")})},I=async(q,C)=>{_=!0,clearInterval(u),await new Promise(G=>setTimeout(G,600)),e.isConnected&&dr(e,t,q,C,r,l)};if(F(),A){let q=c.match_id;if(!q){const{data:G,error:V}=await T.from("matches").insert({home_id:h,away_id:M,home_deck_id:i,away_deck_id:null,status:"active",mode:"mini_league"}).select().single();if(V||!G){f("Erreur création match","error"),Ve(e),x("mini-league");return}q=G.id,await T.from("mini_league_matches").update({match_id:q,status:"playing"}).eq("id",p)}const C=q;u=setInterval(async()=>{if(_){clearInterval(u);return}const{data:G}=await T.from("matches").select("away_deck_id").eq("id",C).single();G!=null&&G.away_deck_id&&(clearInterval(u),I(C,!0))},1500)}else{let q=c.match_id;u=setInterval(async()=>{if(_){clearInterval(u);return}const{data:C}=await T.from("mini_league_matches").select("match_id").eq("id",p).single();C!=null&&C.match_id&&(clearInterval(u),q=C.match_id,await T.from("matches").update({away_deck_id:i}).eq("id",q),I(q,!1))},1500)}}async function dr(e,t,i,o,d=[],a=[]){var $;const{state:r,navigate:l,toast:p}=t,s=(($=r.params)==null?void 0:$.leagueId)||null,n=o?"p1":"p2",x=o?"p2":"p1",f=(d||[]).map(g=>g.id),h=(d||[]).map(g=>({id:g.id,gc_type:g.gc_type,_gcDef:g._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:_}=await T.from("matches").select("*").eq("id",i).single();if(!_){p("Match introuvable","error"),l("mini-league");return}async function u(){const[{data:g},{data:y},{data:b},{data:v}]=await Promise.all([T.rpc("get_deck_for_match",{p_deck_id:_.home_deck_id}),T.rpc("get_deck_for_match",{p_deck_id:_.away_deck_id}),T.from("users").select("id,pseudo,club_name").eq("id",_.home_id).single(),T.from("users").select("id,pseudo,club_name").eq("id",_.away_id).single()]),w=S=>{const j=Number(S.evolution_bonus)||0;return{cardId:S.card_id,position:S.position,id:S.id,firstname:S.firstname,name:S.surname_encoded,country_code:S.country_code,club_id:S.club_id,job:S.job,job2:S.job2,note_g:(Number(S.note_g)||0)+(S.job==="GK"||S.job2==="GK"&&Number(S.note_g)>0?j:0),note_d:(Number(S.note_d)||0)+(S.job==="DEF"||S.job2==="DEF"&&Number(S.note_d)>0?j:0),note_m:(Number(S.note_m)||0)+(S.job==="MIL"||S.job2==="MIL"&&Number(S.note_m)>0?j:0),note_a:(Number(S.note_a)||0)+(S.job==="ATT"||S.job2==="ATT"&&Number(S.note_a)>0?j:0),evolution_bonus:j,rarity:S.rarity,skin:S.skin,hair:S.hair,hair_length:S.hair_length,clubName:S.club_encoded_name||null,clubLogo:S.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},k=((g==null?void 0:g.starters)||[]).map(S=>w(S)),z=((y==null?void 0:y.starters)||[]).map(S=>w(S)),E=(g==null?void 0:g.formation)||"4-4-2",L=(y==null?void 0:y.formation)||"4-4-2";return{p1Team:(()=>{const S=vt(k,E);return stadiumDef?Tt(S,stadiumDef):S})(),p2Team:vt(z,L),p1Subs:(()=>{const S=((g==null?void 0:g.subs)||[]).map(j=>w(j));return stadiumDef&&jt(S,stadiumDef),S})(),p2Subs:((y==null?void 0:y.subs)||[]).map(S=>w(S)),p1Formation:E,p2Formation:L,p1Name:(b==null?void 0:b.club_name)||(b==null?void 0:b.pseudo)||"Joueur 1",p2Name:(v==null?void 0:v.club_name)||(v==null?void 0:v.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:o?f:[],gc_p2:o?[]:f,gcCardsFull_p1:o?h:[],gcCardsFull_p2:o?[]:h,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:a||[],lastActionAt:new Date().toISOString()}}let c=_.game_state&&Object.keys(_.game_state).length?_.game_state:null;if(!c)if(o){c=await u();const{data:g}=await T.from("matches").select("game_state").eq("id",i).single();!(g!=null&&g.game_state)||!Object.keys(g.game_state).length?await T.from("matches").update({game_state:c,turn_user_id:_.home_id}).eq("id",i):c=g.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let g=0;g<60&&!c;g++){await new Promise(b=>setTimeout(b,400));const{data:y}=await T.from("matches").select("game_state").eq("id",i).single();y!=null&&y.game_state&&Object.keys(y.game_state).length&&(c=y.game_state)}if(!c){p("Erreur de synchronisation","error"),l("mini-league");return}c.gc_p2=f,c.gcCardsFull_p2=h,await T.from("matches").update({game_state:c}).eq("id",i)}let A=!1,m=!1,M=!1,F=null,I=!1;const q=new Set,C=new Set;function G(g){var S,j;try{T.removeChannel(V)}catch{}const y=c[n+"Score"]||0,b=c[x+"Score"]||0;if(!M){M=!0;const D=g.winner_id||(y>b?r.profile.id:b>y?"opp":null),Y=D===r.profile.id?"win":D?"loss":null;Y&&ni(()=>Promise.resolve().then(()=>on),void 0).then(J=>J.applyOwnEvolution(r.profile.id,Y)).catch(()=>{})}if(!m){const D=c.p1Score||0,Y=c.p2Score||0,J=(c.usedGc_p1||[]).length,U=(c.usedGc_p2||[]).length,N=g.winner_id||(D>Y?_.home_id:Y>D?_.away_id:null);(N?r.profile.id===N:r.profile.id<(o?_.away_id:_.home_id))&&(m=!0,Ji({player1Id:_.home_id,player2Id:_.away_id,score1:D,score2:Y,gc1:J,gc2:U}).catch(te=>console.warn("[FriendMatch] updateStats:",te)))}let v,w;g.winner_id?(v=g.winner_id===r.profile.id,w=!1):g.forfeit?(v=y>b,w=!1):(w=y===b,v=y>b),(S=document.getElementById("pvp-end-overlay"))==null||S.remove();const k=document.createElement("div");k.id="pvp-end-overlay",k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const z=w?"🤝":v?"🏆":"😞",E=w?"MATCH NUL":v?"VICTOIRE !":"DÉFAITE",L=w?"#fff":v?"#FFD700":"#ff6b6b";k.innerHTML=`
      <div style="font-size:64px">${z}</div>
      <div style="font-size:26px;font-weight:900;color:${L}">${E}</div>
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏆 Retour à la Mini League</button>`,document.body.appendChild(k),(j=k.querySelector("#pvp-end-home"))==null||j.addEventListener("click",()=>{k.remove(),Ve(e),s?l("mini-league",{openLeagueId:s}):l("mini-league")})}const V=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},g=>{const y=g.new;try{if(y.status==="finished"||y.forfeit){if(A||(A=!0,F&&(clearInterval(F),F=null),y.game_state&&(c=y.game_state),c.phase==="finished"&&!y.forfeit&&document.getElementById("pvp-home")))return;G(y);return}if(y.game_state){const b=c;c=y.game_state;const v=c._lastGC;if(v&&v.seq&&!C.has(v.seq)&&(C.add(v.seq),v.by!==n)){se(v.type,v.by,()=>xe());return}const w=b[n+"Score"]||0,k=b[x+"Score"]||0,z=c[n+"Score"]||0,E=c[x+"Score"]||0,L=z>w,S=E>k;if((L||S)&&!q.has(c.round)){q.add(c.round);const j=[...c.log||[]].reverse().find(Y=>Y.isGoal),D=((j==null?void 0:j.homePlayers)||[]).map(Y=>({name:Y.name,note:Y.note,portrait:Y.portrait,job:Y.job}));Ee(D,z,E,L,()=>xe());return}xe()}}catch(b){console.error("[PvP] crash:",b)}}).subscribe();async function K(g){Object.assign(c,g),c.lastActionAt=new Date().toISOString();const{error:y}=await T.from("matches").update({game_state:c}).eq("id",i);y&&p("Erreur de synchronisation","error");try{xe()}catch(b){console.error("[PvP] renderPvpScreen crash:",b)}}async function oe(){if(A)return;A=!0,F&&(clearInterval(F),F=null);const g=o?_.away_id:_.home_id,y=o?"p2":"p1",b=o?"p1":"p2",v={...c,[y+"Score"]:3,[b+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:g,home_score:v.p1Score||0,away_score:v.p2Score||0,game_state:v}).eq("id",i)}catch{}try{T.removeChannel(V)}catch{}setTimeout(()=>{Ve(e),l("mini-league")},800)}const ie={BOOST_STAT:({value:g=1,count:y=1,roles:b=[]},v,w)=>{const k=v[n+"Team"],z=Object.entries(k).filter(([E])=>!b.length||b.includes(E)).flatMap(([E,L])=>L.filter(S=>!S.used).map(S=>({...S,_line:E})));if(!z.length){v.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),w(v);return}Q(z,y,`Choisir ${y} joueur(s) à booster (+${g})`,E=>{E.forEach(L=>{const S=(k[L._line]||[]).find(j=>j.cardId===L.cardId);S&&(S.boost=(S.boost||0)+g,v.log.push({text:`⚡ +${g} sur ${S.name}`,type:"info"}))}),v[n+"Team"]=k,w(v)})},DEBUFF_STAT:({value:g=1,count:y=1,roles:b=[],target:v="ai"},w,k)=>{const z=v==="home"?n:x,E=w[z+"Team"],L=v==="home"?"allié":"adverse",S=Object.entries(E).filter(([j])=>!b.length||b.includes(j)).flatMap(([j,D])=>D.map(Y=>({...Y,_line:j})));if(!S.length){w.log.push({text:`🎯 Aucun joueur ${L}`,type:"info"}),k(w);return}Q(S,y,`Choisir ${y} joueur(s) ${L}(s) (-${g})`,j=>{j.forEach(D=>{const Y=(E[D._line]||[]).find(J=>J.cardId===D.cardId);Y&&(Y.boost=(Y.boost||0)-g,w.log.push({text:`🎯 -${g} sur ${Y.name}`,type:"info"}))}),w[z+"Team"]=E,k(w)})},GRAY_PLAYER:({count:g=1,roles:y=[],target:b="ai"},v,w)=>{const k=b==="home"?n:x,z=v[k+"Team"],E=b==="home"?"allié":"adverse",L=Object.entries(z).filter(([S])=>!y.length||y.includes(S)).flatMap(([S,j])=>j.filter(D=>!D.used).map(D=>({...D,_line:S})));if(!L.length){v.log.push({text:`❌ Aucun joueur ${E}`,type:"info"}),w(v);return}Q(L,g,`Choisir ${g} joueur(s) ${E}(s) à exclure`,S=>{const j="usedCardIds_"+k,D=new Set(v[j]||[]);S.forEach(Y=>{const J=(z[Y._line]||[]).find(U=>U.cardId===Y.cardId);J&&(J.used=!0,D.add(Y.cardId),v.log.push({text:`❌ ${J.name} exclu !`,type:"info"}))}),v[j]=[...D],v[k+"Team"]=z,w(v)})},REVIVE_PLAYER:({count:g=1,roles:y=[]},b,v)=>{const w=b[n+"Team"],k=Object.entries(w).filter(([z])=>!y.length||y.includes(z)).flatMap(([z,E])=>E.filter(L=>L.used).map(L=>({...L,_line:z})));if(!k.length){b.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),v(b);return}Q(k,g,`Choisir ${g} joueur(s) à ressusciter`,z=>{z.forEach(E=>{const L=(w[E._line]||[]).find(S=>S.cardId===E.cardId);L&&(L.used=!1,b.log.push({text:`💫 ${L.name} ressuscité !`,type:"info"}))}),b[n+"Team"]=w,v(b)})},REMOVE_GOAL:({},g,y)=>{const b=x+"Score";g[b]>0?(g[b]--,g.log.push({text:"🚫 Dernier but annulé !",type:"info"})):g.log.push({text:"🚫 Aucun but à annuler",type:"info"}),y(g)},ADD_GOAL_DRAW:({},g,y)=>{g[n+"Score"]===g[x+"Score"]?(g[n+"Score"]++,g.log.push({text:"🎯 But bonus !",type:"info"})):g.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),y(g)},ADD_SUB:({value:g=1},y,b)=>{y.maxSubs=(y.maxSubs||3)+g,y.log.push({text:`🔄 +${g} remplacement(s)`,type:"info"}),b(y)},CUSTOM:({},g,y)=>y(g)};function Q(g,y,b,v){const w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let k=[];function z(){var L,S;const E=g.map(j=>{const D={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[j._line]||"#555",Y=Se(j,j._line)+(j.boost||0),U=k.find(W=>W.cardId===j.cardId)?"#FFD700":"rgba(255,255,255,0.25)",N=j.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${j.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${U};background:${D};overflow:hidden;cursor:pointer;${N}">
        ...
        </div>`,(L=w.querySelector("#pp-close"))==null||L.addEventListener("click",()=>w.remove()),w.querySelectorAll(".pp-item").forEach(j=>{j.addEventListener("click",()=>{const D=j.dataset.cid,Y=g.find(U=>U.cardId===D),J=k.findIndex(U=>U.cardId===D);Y&&(J>-1?k.splice(J,1):k.length<y&&k.push(Y),z())})}),(S=w.querySelector("#pp-confirm"))==null||S.addEventListener("click",()=>{w.remove(),v(k)})}z(),document.body.appendChild(w)}async function R(g,y){const v=(c["gcCardsFull_"+n]||[]).find(E=>E.id===g),w=(v==null?void 0:v._gcDef)||(c.gcDefs||[]).find(E=>{var L;return E.name===y||((L=E.name)==null?void 0:L.toLowerCase().trim())===(y==null?void 0:y.toLowerCase().trim())}),k=[...c["usedGc_"+n]||[],g],z={type:y,by:n,seq:(c._gcAnimSeq||0)+1};C.add(z.seq),se(y,n,async()=>{if(w!=null&&w.effect_type&&w.effect_type!=="CUSTOM"){const L=ie[w.effect_type];if(L){const S={...c};L(w.effect_params||{},S,async j=>{j["usedGc_"+n]=k,j._lastGC=z,j._gcAnimSeq=z.seq,await K(j)});return}}const E={...c};switch(y){case"Remplacement+":E.maxSubs=(E.maxSubs||3)+1,E.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const L=x+"Score";E[L]>0&&(E[L]--,E.log.push({text:"🚫 But annulé",type:"info"}));break}}E["usedGc_"+n]=k,E._lastGC=z,E._gcAnimSeq=z.seq,await K(E)})}function P(g,y){const b="usedCardIds_"+g,v=new Set(c[b]||[]);y.forEach(w=>v.add(w)),c[b]=[...v]}function re(){for(const g of["p1","p2"]){const y=new Set(c["usedCardIds_"+g]||[]),b=c[g+"Team"];if(b)for(const v of["GK","DEF","MIL","ATT"])(b[v]||[]).forEach(w=>{w.used=y.has(w.cardId)})}}function xe(){var nt,ft,gt,mt,le,X;if(c.phase==="reveal")return _e();if(c.phase==="midfield")return ve();if(c.phase==="finished")return B();const g=c[n+"Team"],y=c[x+"Team"];re();const b=c[n+"Score"],v=c[x+"Score"],w=c[n+"Name"],k=c[x+"Name"],z=c.phase===n+"-attack",E=c.phase===n+"-defense",L=Array.isArray(c["selected_"+n])?c["selected_"+n]:[],S=L.map(O=>O.cardId),j=window.innerWidth>=700,D=c[n+"Subs"]||[],Y=c["usedSubIds_"+n]||[],J=D.filter(O=>!Y.includes(O.cardId)),U=c["gcCardsFull_"+n]||[],N=c["usedGc_"+n]||[],W=U.filter(O=>!N.includes(O.id)),te=c.boostOwner===n&&!c.boostUsed,ee=!["GK","DEF","MIL","ATT"].some(O=>(y[O]||[]).some(ae=>!ae.used)),fe=[...g.MIL||[],...g.ATT||[]].filter(O=>!O.used),ye=z&&ee&&fe.length===0?[...g.GK||[],...g.DEF||[]].filter(O=>!O.used).map(O=>O.cardId):[];function be(O,ae,he){var Ct,Ai;const $e=O._gcDef,Qe={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[$e==null?void 0:$e.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Ze={purple:"#b06ce0",light_blue:"#00d4ef"}[$e==null?void 0:$e.color]||"#b06ce0",tt=($e==null?void 0:$e.name)||O.gc_type,rt=($e==null?void 0:$e.effect)||((Ct=Oe[O.gc_type])==null?void 0:Ct.desc)||"",_t=$e!=null&&$e.image_url?`/icons/${$e.image_url}`:null,$t=((Ai=Oe[O.gc_type])==null?void 0:Ai.icon)||"⚡",lt=Math.round(he*.22),xt=Math.round(he*.22),yt=he-lt-xt,kt=tt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${O.id}" data-gc-type="${O.gc_type}"
        ...
        <div style="height:${yt}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${_t?`<img src="${_t}" style="max-width:${ae-10}px;max-height:${yt-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(yt*.55)}px">${$t}</span>`}
        </div>
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ut(g,c[n+"Formation"],Ue,S,300,300,ye)}
      </div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(k);let L=!1;const S=()=>{L||(L=!0,k.remove(),setTimeout(()=>w(),50))};k.addEventListener("click",S),setTimeout(S,3500)}function se(g,y,b){var N,W;const v=(c.gcDefs||[]).find(te=>{var ee;return te.name===g||((ee=te.name)==null?void 0:ee.toLowerCase().trim())===(g==null?void 0:g.toLowerCase().trim())}),w={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[v==null?void 0:v.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",k={purple:"#b06ce0",light_blue:"#00d4ef"}[v==null?void 0:v.color]||"#b06ce0",z=(v==null?void 0:v.name)||g,E=(v==null?void 0:v.effect)||((N=Oe[g])==null?void 0:N.desc)||"",L=v!=null&&v.image_url?`/icons/${v.image_url}`:null,S=((W=Oe[g])==null?void 0:W.icon)||"⚡",D=y===n?"Vous":c[y+"Name"]||"Adversaire",Y=document.createElement("div");Y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",Y.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
      ...
      </div>`,document.body.appendChild(b),(v=b.querySelector("#bp-close"))==null||v.addEventListener("click",()=>b.remove()),b.querySelectorAll(".bp-item").forEach(w=>{w.addEventListener("click",async()=>{const k=w.dataset.cid,z=y.find(L=>L.cardId===k);if(!z)return;const E=(g[z._line]||[]).find(L=>L.cardId===k);E&&(E.boost=(E.boost||0)+c.boostValue),b.remove(),await K({[n+"Team"]:g,boostUsed:!0})})})}function H(g=null){var J,U;if(!(c.phase===n+"-attack")){p("Remplacement uniquement avant votre attaque","warning");return}const b=c[n+"Team"],v=c["usedSubIds_"+n]||[],w=c.maxSubs||3;if(v.length>=w){p(`Maximum ${w} remplacements atteint`,"warning");return}const k=Object.entries(b).flatMap(([N,W])=>(W||[]).filter(te=>te.used).map(te=>({...te,_line:N}))),z=(c[n+"Subs"]||[]).filter(N=>!v.includes(N.cardId));if(!k.length){p("Aucun joueur utilisé à remplacer","warning");return}if(!z.length){p("Aucun remplaçant disponible","warning");return}let E=Math.max(0,k.findIndex(N=>N.cardId===g));const L=((J=k[E])==null?void 0:J._line)||((U=k[E])==null?void 0:U.job);let S=Math.max(0,z.findIndex(N=>N.job===L)),j=!1;const D=document.createElement("div");D.id="pvp-sub-overlay",D.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function Y(){var be,ke,Be,Fe,Me,je;const N=k[E],W=z[S],te=Math.min(130,Math.round((window.innerWidth-90)/2)),ee=Math.round(te*1.35),fe=Ae=>`background:rgba(255,255,255,0.12);border:none;color:${Ae?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Ae?"default":"pointer"};flex-shrink:0`;D.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        ...
        <div id="pout-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
          ...
          <div>${N?Xe({...N,used:!1,boost:0},te,ee):"<div>—</div>"}</div>
      ...
      </div>`,document.body.appendChild(y),(v=y.querySelector("#pvp-hist-close"))==null||v.addEventListener("click",()=>y.remove())}async function me(){if(c.phase!==n+"-attack")return;const g=n==="p1"?"p2":"p1",y=(c.round||0)+1,b=[...c.log||[]];b.push({type:"info",text:`⏭️ ${c[n+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const v=qe(c),w=Te(g),k=v||!w?"finished":g+"-attack";await K({["selected_"+n]:[],modifiers:{...c.modifiers,[n]:{}},pendingAttack:null,phase:k,attacker:g,round:y,log:b}),k==="finished"&&await ze(c)}async function ne(){const g=c[n+"Team"],y=!["GK","DEF","MIL","ATT"].some(k=>(c[x+"Team"][k]||[]).some(z=>!z.used)),b=(c["selected_"+n]||[]).map(k=>{const z=(g[k._role]||[]).find(Y=>Y.cardId===k.cardId)||k,E=y&&["GK","DEF"].includes(k._role),L=g[k._role]||[],S=L.findIndex(Y=>Y.cardId===k.cardId),j=dt(L.length),D=S>=0?j[S]:z._col??1;return{...z,_line:k._role,_col:D,...E?{note_a:Math.max(1,Number(z.note_a)||0)}:{}}});if(!b.length)return;const v=Ut(b,c.modifiers[n]||{});P(n,b.map(k=>k.cardId)),b.forEach(k=>{const z=(g[k._role]||[]).find(E=>E.cardId===k.cardId);z&&(z.used=!0)}),c["selected_"+n]=[],xe();const w=[...c.log||[]];if(y){const k=(c[n+"Score"]||0)+1,z=b.map(D=>({name:D.name,note:Se(D,D._line||"ATT"),portrait:He(D),job:D.job}));w.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:z,homeTotal:v.total,aiTotal:0});const E=(c.round||0)+1,L=n==="p1"?"p2":"p1",S={...c,[n+"Team"]:g,[n+"Score"]:k},j=qe(S);q.add(E),Ee(z,k,c[x+"Score"]||0,!0,async()=>{await K({[n+"Team"]:g,[n+"Score"]:k,["selected_"+n]:[],modifiers:{...c.modifiers,[n]:{}},pendingAttack:null,phase:j?"finished":L+"-attack",attacker:L,round:E,log:w}),j&&await ze({...c,[n+"Score"]:k})});return}w.push({type:"pending",text:`⚔️ ${c[n+"Name"]} attaque (${v.total})`}),await K({[n+"Team"]:g,[x+"Team"]:c[x+"Team"],pendingAttack:{...v,players:b,side:n},["selected_"+n]:[],modifiers:{...c.modifiers,[n]:{}},phase:x+"-defense",log:w})}async function ue(){const g=c[n+"Team"],y=(c["selected_"+n]||[]).map(N=>{const W=(g[N._role]||[]).find(be=>be.cardId===N.cardId)||N,te=g[N._role]||[],ee=te.findIndex(be=>be.cardId===N.cardId),fe=dt(te.length),ye=ee>=0?fe[ee]:W._col??1;return{...W,_line:N._role,_col:ye}}),b=Kt(y,c.modifiers[n]||{});P(n,y.map(N=>N.cardId)),y.forEach(N=>{const W=(g[N._role]||[]).find(te=>te.cardId===N.cardId);W&&(W.used=!0)}),c["selected_"+n]=[],xe();const v=Vt(c.pendingAttack.total,b.total,c.modifiers[n]||{}),w=c.pendingAttack.side,k=v==="attack"||(v==null?void 0:v.goal),z=w==="p1"?"p2":"p1",E=(c.round||0)+1,L=(c.pendingAttack.players||[]).map(N=>({name:N.name,note:Se(N,N._line||"ATT"),portrait:He(N),job:N.job})),S=[...c.log||[]];S.push({type:"duel",isGoal:k,homeScored:k&&w===n,text:k?`⚽ BUT de ${c[w+"Name"]} ! (${c.pendingAttack.total} vs ${b.total})`:`✋ Attaque stoppée (${c.pendingAttack.total} vs ${b.total})`,homePlayers:L,aiPlayers:y.map(N=>({name:N.name,note:Se(N,N._line||"DEF"),portrait:He(N),job:N.job})),homeTotal:c.pendingAttack.total,aiTotal:b.total});const j=k?(c[w+"Score"]||0)+1:c[w+"Score"]||0,D={...c,[n+"Team"]:g,[w+"Score"]:j},Y=qe(D),J=Y?"finished":z+"-attack",U=async()=>{await K({[n+"Team"]:g,[x+"Team"]:c[x+"Team"],[w+"Score"]:j,["selected_"+n]:[],modifiers:{...c.modifiers,[n]:{}},pendingAttack:null,phase:J,attacker:z,round:E,log:S}),(J==="finished"||Y)&&await ze({...c,[w+"Score"]:j})};if(k){const N=w===n,W=N?j:c[n+"Score"]||0,te=N?c[x+"Score"]||0:j;q.add(E),Ee(L,W,te,N,U)}else await U()}function Le(g){return["MIL","ATT"].some(y=>(g[y]||[]).some(b=>!b.used))}function Ie(g){return["GK","DEF","MIL","ATT"].some(y=>(g[y]||[]).some(b=>!b.used))}function Ce(g){return Ie(g)&&!Le(g)}function Te(g){const y=c[g+"Team"],b=c[(g==="p1"?"p2":"p1")+"Team"];return!!(Le(y)||!Ie(b)&&Ce(y))}function qe(g){const y=["MIL","ATT"].some(S=>(g.p1Team[S]||[]).some(j=>!j.used)),b=["MIL","ATT"].some(S=>(g.p2Team[S]||[]).some(j=>!j.used)),v=Ie(g.p1Team),w=Ie(g.p2Team);return!y&&!(!w&&v)&&(!b&&!(!v&&w))}function Pe(g){const y=g.p1Score||0,b=g.p2Score||0;return y===b?null:y>b?_.home_id:_.away_id}async function ze(g){try{const y=Pe(g),b=y?_.home_id===y?_.away_id:_.home_id:null,v=g.p1Score||0,w=g.p2Score||0;await T.from("matches").update({status:"finished",winner_id:y,home_score:v,away_score:w}).eq("id",i);const{data:k}=await T.from("mini_league_matches").select("id, league_id, matchday").eq("match_id",i).single();if(k){await T.from("mini_league_matches").update({home_score:v,away_score:w,status:"finished"}).eq("id",k.id);const{data:z}=await T.from("mini_league_matches").select("id, status").eq("league_id",k.league_id).eq("matchday",k.matchday);if((z||[]).every(L=>L.status==="finished"||L.status==="bye")){const{data:L}=await T.from("mini_leagues").select("current_day,total_days").eq("id",k.league_id).single();if(L){const S=(L.current_day||0)+1,j=S>(L.total_days||0);await T.from("mini_leagues").update({current_day:j?L.total_days:S,status:j?"finished":"active"}).eq("id",k.league_id)}}}y&&b&&Ti(y,b).catch(()=>{})}catch(y){console.error("[ML] finishMatch:",y)}}function B(){var w;if(A&&document.getElementById("pvp-end-overlay"))return;A=!0;const g=c[n+"Score"],y=c[x+"Score"],b=g>y,v=g===y;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function pr(e,t,i){let o=0;const d=document.createElement("div");d.id="tutorial-overlay",d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const a=()=>{var n,x,f;const l=t[o],p=o===t.length-1,s=Math.round((o+1)/t.length*100);d.innerHTML=`
    ...
    `,d.querySelectorAll("ul,ol").forEach(h=>{h.style.paddingLeft="20px",h.style.marginTop="6px",h.style.marginBottom="6px"}),d.querySelectorAll("li").forEach(h=>{h.style.marginBottom="4px"}),d.querySelectorAll("p").forEach(h=>{h.style.marginBottom="8px"}),(n=d.querySelector("#tuto-prev"))==null||n.addEventListener("click",()=>{o--,a()}),(x=d.querySelector("#tuto-next"))==null||x.addEventListener("click",()=>{p?r():(o++,a())}),(f=d.querySelector("#tuto-skip"))==null||f.addEventListener("click",()=>{confirm("Passer le tutoriel ? Tu pourras le revoir plus tard depuis les paramètres.")&&r()})},r=async()=>{d.remove(),e!=null&&e.id&&await T.from("users").update({tutorial_done:!0}).eq("id",e.id),i==null||i()};document.body.appendChild(d),a()}async function ur(e,t,i){if(!e||e.tutorial_done)return;let o=[];const{data:d,error:a}=await T.rpc("get_tutorial_steps");if(!a&&(d==null?void 0:d.length)>0)o=d,console.log(`[Tutorial] RPC OK → ${o.length} étapes`);else{const{data:l,error:p}=await T.from("tutorial_steps").select("*").eq("is_active",!0).order("step_order");!p&&(l==null?void 0:l.length)>0?(o=l,console.log(`[Tutorial] Direct OK → ${o.length} étapes`)):(console.warn(`[Tutorial] Aucune étape DB (RPC: ${a==null?void 0:a.message}, Direct: ${p==null?void 0:p.message})`),i&&i("[Tutorial] DB vide ou inaccessible — tuto local utilisé","warning",5e3))}const r=o.length>0?o.map(l=>({emoji:l.emoji,title:l.title,color:l.color,content:l.content,image_url:l.image_url||null})):cr;pr(e,r,()=>t("boosters"))}const Xt={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function St(e,t,i=0){return t?(Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0)+(t===e.job||t===e.job2?i:0):0}async function fr(e,t){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',await Si(e,t)}async function Si(e,t){const{state:i,toast:o}=t,{data:d}=await T.from("market_listings").select(`id, price, status, listed_at, seller_id,
      ...
      card:cards(id, card_type, current_note, evolution_bonus,
        player:players(id, firstname, surname_encoded, country_code, job, job2,
          ...
          clubs(encoded_name, logo_url)))`).eq("seller_id",i.profile.id).in("status",["active","sold"]).order("listed_at",{ascending:!1}).limit(100),r=(d||[]).filter(u=>u.seller_id!==i.profile.id),l=a||[];e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <!-- Header -->
    ...
    <div id="mkt-content" style="padding:10px 16px;display:flex;flex-direction:column;gap:8px"></div>
  </div>`;let p="buy";const s=()=>{var u,c,A,m,M,F,I;return{name:(((u=document.getElementById("flt-name"))==null?void 0:u.value)||"").toLowerCase().trim(),club:(((c=document.getElementById("flt-club"))==null?void 0:c.value)||"").toLowerCase().trim(),country:(((A=document.getElementById("flt-country"))==null?void 0:A.value)||"").toLowerCase().trim(),job:((m=document.getElementById("flt-job"))==null?void 0:m.value)||"",rarity:((M=document.getElementById("flt-rarity"))==null?void 0:M.value)||"",note1:parseInt((F=document.getElementById("flt-note1"))==null?void 0:F.value)||0,note2:parseInt((I=document.getElementById("flt-note2"))==null?void 0:I.value)||0}};function n(u){const c=s();return u.filter(A=>{var V,K,oe;const m=(V=A.card)==null?void 0:V.player;if(!m)return!1;const M=`${m.firstname} ${m.surname_encoded}`.toLowerCase(),F=(((K=m.clubs)==null?void 0:K.encoded_name)||"").toLowerCase(),I=(m.country_code||"").toLowerCase(),q=((oe=A.card)==null?void 0:oe.evolution_bonus)||0,C=St(m,m.job,q),G=m.job2?St(m,m.job2,q):0;return!(c.name&&!M.includes(c.name)||c.club&&!F.includes(c.club)||c.country&&!I.includes(c.country)||c.job&&m.job!==c.job||c.rarity&&m.rarity!==c.rarity||c.note1&&C<c.note1||c.note2&&G<c.note2)})}function x(u){var G,V,K,oe;const c=(G=u.card)==null?void 0:G.player;if(!c)return"";const A=((V=u.card)==null?void 0:V.evolution_bonus)||0,m=St(c,c.job,A),M=c.job2?St(c,c.job2,A):0,F=(i.profile.credits||0)>=u.price,I=c.country_code?`https://flagsapi.com/${c.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,q=Xt[c.job]||"#bbb",C=c.job2?Xt[c.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
      ...
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${q};display:flex;align-items:center;justify-content:center">
          <span style="font-size:14px;font-weight:900;color:${q};font-family:Arial Black,Arial">${m}</span>
        </div>
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:10px;color:#999;margin-top:1px">Vendeur : ${((oe=u.seller)==null?void 0:oe.pseudo)||"—"}</div>
      </div>
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:14px;font-weight:900;color:#D4A017">${u.price.toLocaleString("fr")}</div>
        <button class="btn btn-primary btn-sm" data-buy="${u.id}" ${F?"":"disabled"} style="margin-top:4px;font-size:11px;padding:4px 10px">${F?"Acheter":"Trop cher"}</button>
      </div>
    </div>`}function f(u){var G,V,K,oe;const c=(G=u.card)==null?void 0:G.player;if(!c)return"";const A=((V=u.card)==null?void 0:V.evolution_bonus)||0,m=St(c,c.job,A),M=c.job2?St(c,c.job2,A):0,F=u.status==="sold",I=c.country_code?`https://flagsapi.com/${c.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,q=Xt[c.job]||"#bbb",C=c.job2?Xt[c.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${F?"opacity:0.7":""}">
      ${I?`<img src="${I}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(K=c.clubs)!=null&&K.logo_url?`<img src="${c.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${q};display:flex;align-items:center;justify-content:center">
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:10px;color:${F?"#22c55e":"#999"};margin-top:1px">
          ${F?`✅ Vendu à ${((oe=u.buyer)==null?void 0:oe.pseudo)||"—"} · ${u.sold_at?new Date(u.sold_at).toLocaleDateString("fr"):""}`:`🟢 En vente depuis le ${new Date(u.listed_at).toLocaleDateString("fr")}`}
        </div>
      </div>
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:14px;font-weight:900;color:#D4A017">${u.price.toLocaleString("fr")}</div>
        ${F?'<span style="font-size:10px;font-weight:700;color:#fff;background:#22c55e;padding:3px 8px;border-radius:10px;display:inline-block;margin-top:4px">VENDU</span>':`<button class="btn btn-danger btn-sm" data-cancel="${u.id}" style="margin-top:4px;font-size:11px;padding:4px 10px">Retirer</button>`}
      </div>
    </div>`}function h(){const u=document.getElementById("mkt-content"),c=document.getElementById("mkt-filters");if(u){if(c.style.display=p==="buy"?"flex":"none",p==="buy"){const A=n(r);u.innerHTML=A.length?A.map(x).join(""):'<div style="text-align:center;color:#aaa;padding:40px">Aucune carte trouvée.</div>'}else{const A=l.filter(M=>M.status==="active").sort((M,F)=>new Date(F.listed_at)-new Date(M.listed_at)),m=l.filter(M=>M.status==="sold").sort((M,F)=>new Date(F.sold_at||F.listed_at)-new Date(M.sold_at||M.listed_at));u.innerHTML=(A.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:4px 0 6px;text-transform:uppercase;letter-spacing:1px">🟢 En vente (${A.length})</div>`+A.map(f).join(""):"")+(m.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:12px 0 6px;text-transform:uppercase;letter-spacing:1px">✅ Ventes réussies (${m.length})</div>`+m.map(f).join(""):"")+(!A.length&&!m.length?'<div style="text-align:center;color:#aaa;padding:40px">Aucune vente pour le moment.</div>':"")}u.querySelectorAll("[data-buy]").forEach(A=>A.addEventListener("click",()=>gr(A.dataset.buy,r,e,t))),u.querySelectorAll("[data-cancel]").forEach(A=>A.addEventListener("click",()=>xr(A.dataset.cancel,e,t)))}}e.querySelectorAll(".mkt-tab").forEach(u=>{u.addEventListener("click",()=>{p=u.dataset.tab,e.querySelectorAll(".mkt-tab").forEach(c=>{const A=c===u;c.style.background=A?"var(--green)":"#fff",c.style.color=A?"#fff":"var(--gray-600)",c.style.borderColor=A?"var(--green)":"var(--gray-200)"}),h()})});let _;["flt-name","flt-club","flt-country","flt-job","flt-rarity","flt-note1","flt-note2"].forEach(u=>{var c;(c=document.getElementById(u))==null||c.addEventListener("input",()=>{clearTimeout(_),_=setTimeout(h,250)})}),h()}async function gr(e,t,i,o){const{state:d,toast:a,refreshProfile:r}=o,l=t.find(n=>n.id===e);if(!l)return;const p=l.price;if((d.profile.credits||0)<p){a("Crédits insuffisants","error");return}mr(l,async()=>{const{error:n}=await T.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:d.profile.id});if(n){a("Erreur achat : "+n.message,"error");return}await r();const x=document.getElementById("nav-credits");x&&(x.textContent=`💰 ${(d.profile.credits||0).toLocaleString("fr")}`),a("✅ Carte achetée !","success"),await Si(i,o)})}function mr(e,t){var r;const i=(r=e.card)==null?void 0:r.player,o=i?`${i.firstname} ${i.surname_encoded}`:"cette carte",d=document.createElement("div");d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",d.innerHTML=`
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const A=r||[];_.innerHTML=A.length>0?A.map((m,M)=>{const F=Gt(m.mmr??1e3),I=(m.ranked_wins||0)+(m.ranked_losses||0)+(m.ranked_draws||0),q=I>0?Math.round((m.ranked_wins||0)/I*100):0,C=m.id===i.profile.id,G=(m.placement_matches||0)<10;return`
          <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${C?"border:2px solid var(--yellow)":""}">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:11px;color:var(--gray-600)">${m.club_name} · ${F.label}</div>
            </div>
            <div style="text-align:right;flex-shrink:0">
              <div style="font-size:20px">${G?"❓":F.emoji}</div>
              <div style="font-size:11px;font-weight:700;color:${F.color}">${G?"Placement":F.label}</div>
              ${G?"":`<div style="font-size:10px;color:var(--gray-600)">${q}% WR</div>`}
            </div>
          </div>`}).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun joueur classé.</div>'}(u=document.getElementById("tab-global"))!=null&&u.style&&(document.getElementById("tab-global").style.cssText=s(l==="global")),(c=document.getElementById("tab-ranked"))!=null&&c.style&&(document.getElementById("tab-ranked").style.cssText=s(l==="ranked"))}}const s=_=>`flex:1;padding:10px;border:none;border-radius:10px;cursor:pointer;font-size:13px;font-weight:${_?"900":"400"};background:${_?"var(--green)":"rgba(255,255,255,0.06)"};color:${_?"#fff":"var(--gray-600)"};transition:all 0.2s`;e.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📊 Classements</h2>
    </div>
  ...
  </div>`,(C=document.getElementById("ranked-back"))==null||C.addEventListener("click",()=>o("home")),(G=document.getElementById("ranked-leaderboard-btn"))==null||G.addEventListener("click",()=>o("rankings",{tab:"ranked"})),(V=document.getElementById("ranked-play-btn"))==null||V.addEventListener("click",()=>{o("match",{matchMode:"ranked",rankedData:{mmr:p,rd:s,sigma:n,isPlacement:f}})})}async function hr(e,{state:t,navigate:i,toast:o}){const d=t.profile;if(!d)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:a}=await T.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
      home:users!home_id(pseudo,club_name),
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${d.id},away_id.eq.${d.id}`).order("created_at",{ascending:!1}).limit(50),r={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},l=(a||[]).filter(n=>n.status==="finished"),p=(a||[]).filter(n=>n.status==="in_progress");function s(n){const x=n.home_id===d.id;x?n.home_score:n.away_score,x?n.away_score:n.home_score;const f=n.winner_id===d.id,h=n.home_score===n.away_score&&n.status==="finished",_=n.status!=="finished"?"…":h?"N":f?"V":"D",u=n.status!=="finished"||h?"#888":f?"#1A6B3C":"#c0392b";let c=r[n.mode]||n.mode;n.away_id===null&&!c.startsWith("IA")&&(c="IA");const m=n.home_id===d.id?n.away:n.home;let M;n.away_id===null?M=c:m?M=`${m.club_name||m.pseudo} (${m.pseudo})`:M="Adversaire";let F="";n.status==="in_progress"&&Date.now()-new Date(n.created_at).getTime()>3600*1e3&&(F=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const I=new Date(n.created_at),q=I.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+I.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),C=n.status==="finished"?`${n.home_score} - ${n.away_score}`:`${n.home_score||0} - ${n.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
  ...
  </div>`}vn(_n);const De={user:null,profile:null,page:"home",params:{}};function At(e,t="info",i=3e3){const o=document.getElementById("toast");o&&(o.textContent=e,o.className=`show ${t}`,clearTimeout(o._t),o._t=setTimeout(()=>{o.className=""},i))}function vr(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function wi(){document.getElementById("modal-overlay").classList.add("hidden")}async function Ft(){if(!De.user)return;const{data:e}=await T.from("users").select("*").eq("id",De.user.id).single();e&&(De.profile=e)}function Pt(e,t={}){De.page=e,De.params=t,yn()}async function yn(){var o,d,a,r;const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(l=>{l.classList.toggle("active",l.dataset.page===De.page)});const t=document.getElementById("nav-credits");t&&De.profile&&(t.textContent=`💰 ${(De.profile.credits||0).toLocaleString("fr")}`);const i={state:De,navigate:Pt,toast:At,openModal:vr,closeModal:wi,refreshProfile:Ft};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',De.page){case"home":await Ci(e,i);break;case"collection":await Qn(e,i);break;case"decks":await mi(e,i);break;case"boosters":await co(e,i);break;case"ranked":await br(e,i);break;case"match":{const l=De.params&&De.params.matchMode||"vs_ai_easy";l==="random"?await hi(e,i,!1):l==="ranked"?await hi(e,i,!0):l==="friend"?await Ko(e,i,(o=De.params)==null?void 0:o.friendId,(d=De.params)==null?void 0:d.friendName):l==="mini-league"?await Ui(e,i,(a=De.params)==null?void 0:a.mlMatchId,(r=De.params)==null?void 0:r.leagueId):await Eo(e,i);break}case"market":await fr(e,i);break;case"rankings":await yr(e,i);break;case"matches":await hr(e,i);break;case"friends":await An(e,i);break;case"mini-league":await Wo(e,i);break;case"match-mini-league":{const l=De.params||{};await Ui(e,i,l.mlMatchId,l.leagueId);break}default:await Ci(e,i)}}function wr(){var o;const e=document.getElementById("app"),t=De.profile;if(!t)return;const i="/icons/";e.innerHTML=`
    <nav class="top-nav">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(d=>{d.addEventListener("click",a=>{a.preventDefault(),Pt(d.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>Pt("home")),document.getElementById("nav-credits").addEventListener("click",()=>Pt("boosters")),(o=document.getElementById("journal-btn"))==null||o.addEventListener("click",()=>_r())}async function _r(){const{data:e}=await T.from("patch_notes").select("*").eq("is_published",!0).order("published_at",{ascending:!1}).limit(20),t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px";const i=(e||[]).map(o=>{const d=new Date(o.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"long",year:"numeric"});return`<div style="padding:14px 0;border-bottom:1px solid #f0f0f0">
    ...
    </div>`,document.body.appendChild(t),t.querySelector("#journal-close").addEventListener("click",()=>t.remove()),t.addEventListener("click",o=>{o.target===t&&t.remove()})}async function $r(){document.documentElement.setAttribute("data-theme","light"),document.getElementById("modal-overlay").addEventListener("click",i=>{i.target===i.currentTarget&&wi()}),document.getElementById("modal-close").addEventListener("click",wi);const{data:{session:e}}=await T.auth.getSession();if(!e){Ki(),Ii(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:At});return}De.user=e.user,await Ft(),Ki();try{const{data:i}=await T.from("formation_links_overrides").select("formation, links"),o={};(i||[]).forEach(d=>{o[d.formation]=d.links}),wn(o)}catch(i){console.warn("Impossible de charger les overrides de formation:",i)}if(!De.profile){kn(document.getElementById("app"),{state:De,navigate:async()=>{await Ft(),fi()},toast:At,refreshProfile:Ft});return}const t=Array.isArray(De.profile.pending_boosters)?De.profile.pending_boosters:[];if(!De.profile.onboarding_done&&t.length>0){bo(document.getElementById("app"),{state:De,navigate:()=>fi(),toast:At,refreshProfile:Ft});return}fi(),setTimeout(()=>ur(De.profile,Pt,At),800),T.auth.onAuthStateChange(async(i,o)=>{i==="SIGNED_OUT"&&(De.user=null,De.profile=null,document.getElementById("app").innerHTML="",Ii(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:At}))})}function kr(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function si(){const e=document.getElementById("app");e&&(e.style.height=kr()+"px")}window.addEventListener("resize",si);window.addEventListener("orientationchange",()=>setTimeout(si,150));window.visualViewport&&window.visualViewport.addEventListener("resize",si);function fi(){const e=()=>{var i;(i=De.user)!=null&&i.id&&T.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",De.user.id).then(()=>{})};e(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(e,6e4);const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",si(),wr(),yn()}function Ki(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function bn(e){var o;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
    <div style="font-size:42px">📡</div>
    <div style="font-size:18px;font-weight:900">Connexion impossible</div>
    <div style="font-size:13px;color:rgba(255,255,255,0.6);max-width:280px">${e||"Le chargement a échoué. Vérifie ta connexion et réessaie."}</div>
    <button id="boot-retry" style="margin-top:8px;padding:12px 30px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer">Réessayer</button>`,document.body.appendChild(i),(o=document.getElementById("boot-retry"))==null||o.addEventListener("click",()=>window.location.reload())}$r().catch(e=>{console.error("Échec du démarrage:",e),bn()});setTimeout(()=>{const e=document.getElementById("app-loader");e&&e.style.display!=="none"&&!e.classList.contains("zoom-out")&&!document.getElementById("boot-error")&&bn("Le serveur met trop de temps à répondre.")},12e3);
