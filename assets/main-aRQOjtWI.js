import{s as A,F as oi,h as St,j as Ai,l as Qe,i as Xi,k as Ji,b as Qi}from"./formation-links-CEzN0rcz.js";function mi(e,{navigate:t,toast:o}){e.innerHTML=`
  ...
  `;function l(){var k;const i=document.getElementById("logo-preview"),r=document.getElementById("logo-initials"),f=((k=document.getElementById("setup-club"))==null?void 0:k.value)||"MW",m=f.trim().split(" ").filter(Boolean),x=m.length>=2?(m[0][0]+m[1][0]).toUpperCase():f.slice(0,2).toUpperCase();i&&(i.style.background=d,i.style.borderColor=s),r&&(r.textContent=x,r.style.color=s)}document.getElementById("color1").addEventListener("input",i=>{s=i.target.value,document.getElementById("swatch1").style.background=s,l()}),document.getElementById("color2").addEventListener("input",i=>{d=i.target.value,document.getElementById("swatch2").style.background=d,l()});function u(i){document.querySelectorAll(".setup-step").forEach(r=>r.classList.remove("active")),document.getElementById(`step-${i}`).classList.add("active"),document.getElementById("step-num").textContent=i,document.getElementById("progress-fill").style.width=`${Math.round(i/3*100)}%`,i===3&&l()}document.getElementById("step1-next").addEventListener("click",async()=>{const i=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("step1-error");if(r.textContent="",i.length<3){r.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await A.from("users").select("id").eq("pseudo",i).maybeSingle();if(f){r.textContent="Ce pseudo est déjà pris.";return}u(2)}),document.getElementById("step2-back").addEventListener("click",()=>u(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const i=document.getElementById("setup-club").value.trim(),r=document.getElementById("step2-error");if(r.textContent="",i.length<2){r.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await A.from("users").select("id").eq("club_name",i).maybeSingle();if(f){r.textContent="Ce nom de club est déjà pris.";return}u(3)}),document.getElementById("step3-back").addEventListener("click",()=>u(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const i=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),m=document.getElementById("step3-finish");f.textContent="",m.disabled=!0,m.textContent="Création en cours…";try{const{error:x}=await A.from("users").insert({id:t.user.id,pseudo:i,club_name:r,club_color1:s,club_color2:d,credits:1e4});if(x)throw x;await en(t.user.id),await p(),a(`Bienvenue ${i} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(x){f.textContent=x.message,m.disabled=!1,m.textContent="🚀 Créer mon profil !"}})}async function en(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await A.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(o){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",o)}}const Lt="#1A6B3C",zt="#cc2222",tn="#D4A017",xi="#888";async function nn(e,t){const{state:o,toast:a}=t;e.innerHTML=`
    ...
    </style>`}const sn="2026.06.27-2123";async function yi(e,{state:t,navigate:o,toast:a}){var s,d;const p=t.profile;p&&(e.innerHTML=`
  <div class="page">
    <div class="page-body">
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      ...
      <div style="background:#fff3cd;border-radius:10px;padding:10px 14px;width:100%">
        ...
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const ue of Object.keys(M)){const re=C(ue);if(!re)continue;const D=ue.replace(/\d+/,""),Y=r[D]||"#555";ee+=`<circle cx="${re.x}" cy="${re.y}" r="${S}" fill="${Y}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,ee+=`<text x="${re.x}" y="${re.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${D}</text>`}return ee+="</svg>",ee}const m=t.filter(M=>M.formation===i),x=m.length,k=!e.is_for_sale;a(`Formation ${i}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",d).order("slot_order");let u=0;function i(){var M,j,O,B,S;const r=s[u],f=(l||[]).filter(C=>C.deck_id===r.id),m=f.filter(C=>{var ee;return C.is_starter&&((ee=C.card)==null?void 0:ee.player)}).map(C=>Zt(C.card,C.position)),x=f.find(C=>{var ee;return((ee=C.card)==null?void 0:ee.card_type)==="formation"}),k=r.formation||((M=x==null?void 0:x.card)==null?void 0:M.formation)||"4-4-2",n=m.length>=11?ut(m,k):null,E=m.length>=11;We(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${f}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${p?`<img src="${p}" style="width:${f-4}px;height:${f-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,f-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
        ...
        stroke="${re}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const x=48,k=64,n=13,E=16,z={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[O,B]of Object.entries(i)){const S=f(O);if(!S||!B)continue;const C=O.replace(/[0-9]/g,""),ee=pt[C]||"#555",ue=d.includes(B.cardId),re=o==="attack"&&(["MIL","ATT"].includes(C)||ue)&&!B.used||o==="defense"&&["GK","DEF","MIL"].includes(C)&&!B.used,D=a.includes(B.cardId);let Y;o==="attack"?Y=ue?Math.max(1,Number(B.note_a)||0):C==="MIL"?Number(B.note_m)||0:Number(B.note_a)||0:o==="defense"?Y=C==="GK"?Number(B.note_g)||0:C==="MIL"?Number(B.note_m)||0:Number(B.note_d)||0:Y=Number(C==="GK"?B.note_g:C==="DEF"?B.note_d:C==="MIL"?B.note_m:B.note_a)||0,Y=Y+(B.boost||0);const te=(S.x-x/2).toFixed(1),F=(S.y-k/2).toFixed(1),Q=z[B==null?void 0:B.rarity]||z.normal;if(B.used){const N=typeof import.meta<"u"&&"/manager-wars/"||"/",P=`${typeof window<"u"&&((j=window.location)==null?void 0:j.origin)||""}${N}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");m+=`<rect x="${te}" y="${F}" width="${x}" height="${k}" rx="5" fill="#161616"/>`,m+=`<image href="${P}" xlink:href="${P}" x="${te}" y="${F}" width="${x}" height="${k}" preserveAspectRatio="xMidYMid slice"/>`,m+=`<rect x="${te}" y="${F}" width="${x}" height="${k}" rx="5" fill="none" stroke="${Q}" stroke-width="2" opacity="0.7"/>`,m+=`<rect x="${te}" y="${F}" width="${x}" height="${k}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${B.cardId}" data-role="${C}" style="cursor:pointer"/>`;continue}const K=D?.45:1,ae=D?"#FFD700":Q,ge=D?3:(B==null?void 0:B.rarity)==="legende"||(B==null?void 0:B.rarity)==="légende"||(B==null?void 0:B.rarity)==="pepite"||(B==null?void 0:B.rarity)==="pépite"?2.5:2,me=k-n-E;m+=`<defs><clipPath id="cp-${O}"><rect x="${te}" y="${(S.y-k/2+n).toFixed(1)}" width="${x}" height="${me}"/></clipPath></defs>`,m+=`<rect x="${te}" y="${F}" width="${x}" height="${k}" rx="5" fill="${ee}" opacity="${K}"/>`;const xe=Ie(B);xe&&(m+=`<image href="${xe}" xlink:href="${xe}" x="${te}" y="${(S.y-k/2+n).toFixed(1)}" width="${x}" height="${me}" clip-path="url(#cp-${O})" preserveAspectRatio="xMidYMin slice"/>`),m+=`<rect x="${te}" y="${F}" width="${x}" height="${n}" rx="4" fill="rgba(255,255,255,0.92)"/>`,m+=`<text x="${S.x.toFixed(1)}" y="${(S.y-k/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(B.name||"").slice(0,9)}</text>`;const R=(S.y+k/2-E).toFixed(1);m+=`<rect x="${te}" y="${R}" width="${x}" height="${E}" fill="rgba(255,255,255,0.92)"/>`;{const N=ei(B.country_code);N?m+=`<image href="${N}" xlink:href="${N}" x="${(S.x-20).toFixed(1)}" y="${(S.y+k/2-E+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:m+=`<text x="${(S.x-13).toFixed(1)}" y="${(S.y+k/2-E/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${Ki(B.country_code)}</text>`,m+=`<text x="${S.x.toFixed(1)}" y="${(S.y+k/2-E/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${Y}</text>`;const W=Ui(B);W?m+=`<image href="${W}" xlink:href="${W}" x="${(S.x+x/2-14).toFixed(1)}" y="${(S.y+k/2-E+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:B.clubName&&(m+=`<text x="${(S.x+14).toFixed(1)}" y="${(S.y+k/2-E/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(B.clubName||"").slice(0,3).toUpperCase()}</text>`),B.boost&&(m+=`<rect x="${(S.x+x/2-12).toFixed(1)}" y="${(S.y-k/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,m+=`<text x="${(S.x+x/2-5).toFixed(1)}" y="${(S.y-k/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${B.boost}</text>`)}m+=`<rect x="${te}" y="${F}" width="${x}" height="${k}" rx="5" fill="${D?"rgba(255,255,255,0.12)":"none"}" stroke="${ae}" stroke-width="${ge}" opacity="${K}"/>`,re&&(m+=`<rect x="${te}" y="${F}" width="${x}" height="${k}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${D?"selected":""}" data-card-id="${B.cardId}" data-role="${C}" style="cursor:pointer"/>`)}const M=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-M} ${-M} ${p+M*2} ${s+M*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${m}
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(s);let l=!1;const u=()=>{l||(l=!0,s.remove(),setTimeout(()=>p(),50))};s.addEventListener("click",u),setTimeout(u,3500)}async function It(e,t,o){var r,f;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:a}=o,p=t.homeScore>t.aiScore,s=t.homeScore===t.aiScore,d=p?"victoire":s?"nul":"defaite",l=cn(t.mode,d);t.matchId&&await A.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:p?a.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const u={credits:(a.profile.credits||0)+l,matches_played:(a.profile.matches_played||0)+1};p?u.wins=(a.profile.wins||0)+1:s?u.draws=(a.profile.draws||0)+1:u.losses=(a.profile.losses||0)+1,await A.from("users").update(u).eq("id",a.profile.id),await o.refreshProfile();const i=document.createElement("div");i.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",i.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const x=async(j=!0)=>{f=!0,m&&(A.removeChannel(m),m=null),j&&await A.rpc("cancel_matchmaking",{p_user_id:u.profile.id}).catch(()=>{})};(M=document.getElementById("mm-cancel"))==null||M.addEventListener("click",async()=>{try{await x(!0)}catch{}Ne(e),i("home")});const k=async(j,O,B)=>{if(f)return;f=!0,m&&(A.removeChannel(m),m=null);const S=document.getElementById("mm-status");S&&(S.textContent="Adversaire trouvé !"),await new Promise(C=>setTimeout(C,600)),e.isConnected&&ao(e,t,j,B,d,l)},{data:n,error:E}=await A.rpc("try_matchmake",{p_user_id:u.profile.id,p_deck_id:o});if(E||!(n!=null&&n.success)){r("Erreur de matchmaking","error"),Ne(e),i("home");return}if(n.matched){k(n.match_id,n.opponent_id,!1);return}const z=document.getElementById("mm-status");z&&(z.textContent="En attente d'un autre joueur..."),m=A.channel("matchmaking-"+u.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${u.profile.id}`},j=>{const O=j.new;O.status==="matched"&&O.match_id&&k(O.match_id,O.matched_with,!0)}).subscribe()}async function ao(e,t,o,a,p=[],s=[]){const{state:d,navigate:l,toast:u}=t,i=a?"p1":"p2",r=a?"p2":"p1",f=(p||[]).map(c=>c.id),m=(p||[]).map(c=>({id:c.id,gc_type:c.gc_type,_gcDef:c._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:x}=await A.from("matches").select("*").eq("id",o).single();if(!x){u("Match introuvable","error"),l("home");return}async function k(){const[{data:c},{data:g},{data:b},{data:h}]=await Promise.all([A.rpc("get_deck_for_match",{p_deck_id:x.home_deck_id}),A.rpc("get_deck_for_match",{p_deck_id:x.away_deck_id}),A.from("users").select("id,pseudo,club_name").eq("id",x.home_id).single(),A.from("users").select("id,pseudo,club_name").eq("id",x.away_id).single()]),y=$=>({cardId:$.card_id,position:$.position,id:$.id,firstname:$.firstname,name:$.surname_encoded,country_code:$.country_code,club_id:$.club_id,job:$.job,job2:$.job2,note_g:Number($.note_g)||0,note_d:Number($.note_d)||0,note_m:Number($.note_m)||0,note_a:Number($.note_a)||0,rarity:$.rarity,skin:$.skin,hair:$.hair,hair_length:$.hair_length,clubName:$.club_encoded_name||null,clubLogo:$.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),v=((c==null?void 0:c.starters)||[]).map($=>y($)),w=((g==null?void 0:g.starters)||[]).map($=>y($)),_=(c==null?void 0:c.formation)||"4-4-2",I=(g==null?void 0:g.formation)||"4-4-2";return{p1Team:ut(v,_),p2Team:ut(w,I),p1Subs:((c==null?void 0:c.subs)||[]).map($=>y($)),p2Subs:((g==null?void 0:g.subs)||[]).map($=>y($)),p1Formation:_,p2Formation:I,p1Name:(b==null?void 0:b.club_name)||(b==null?void 0:b.pseudo)||"Joueur 1",p2Name:(h==null?void 0:h.club_name)||(h==null?void 0:h.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:a?f:[],gc_p2:a?[]:f,gcCardsFull_p1:a?m:[],gcCardsFull_p2:a?[]:m,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let n=x.game_state&&Object.keys(x.game_state).length?x.game_state:null;if(!n)if(a){n=await k();const{data:c}=await A.from("matches").select("game_state").eq("id",o).single();!(c!=null&&c.game_state)||!Object.keys(c.game_state).length?await A.from("matches").update({game_state:n,turn_user_id:x.home_id}).eq("id",o):n=c.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let c=0;c<30&&!n;c++){await new Promise(b=>setTimeout(b,400));const{data:g}=await A.from("matches").select("game_state").eq("id",o).single();g!=null&&g.game_state&&Object.keys(g.game_state).length&&(n=g.game_state)}if(!n){u("Erreur de synchronisation","error"),l("home");return}n.gc_p2=f,n.gcCardsFull_p2=m,await A.from("matches").update({game_state:n}).eq("id",o)}let E=!1,z=null,M=!1;const j=new Set,O=new Set;function B(c){var $,L;try{A.removeChannel(S)}catch{}const g=n[i+"Score"]||0,b=n[r+"Score"]||0;let h,y;c.winner_id?(h=c.winner_id===d.profile.id,y=!1):c.forfeit?(h=g>b,y=!1):(y=g===b,h=g>b),($=document.getElementById("pvp-end-overlay"))==null||$.remove();const v=document.createElement("div");v.id="pvp-end-overlay",v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const w=y?"🤝":h?"🏆":"😞",_=y?"MATCH NUL":h?"VICTOIRE !":"DÉFAITE",I=y?"#fff":h?"#FFD700":"#ff6b6b";v.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(v),(L=v.querySelector("#pvp-end-home"))==null||L.addEventListener("click",()=>{v.remove(),Ne(e),l("home")})}const S=A.channel("pvp-match-"+o).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${o}`},c=>{const g=c.new;try{if(g.status==="finished"||g.forfeit){if(E)return;E=!0,z&&(clearInterval(z),z=null),g.game_state&&(n=g.game_state),B(g);return}if(g.game_state){const b=n;n=g.game_state;const h=n._lastGC;if(h&&h.seq&&!O.has(h.seq)&&(O.add(h.seq),h.by!==i)){me(h.type,h.by,()=>F());return}const y=b[i+"Score"]||0,v=b[r+"Score"]||0,w=n[i+"Score"]||0,_=n[r+"Score"]||0,I=w>y,$=_>v;if((I||$)&&!j.has(n.round)){j.add(n.round);const L=[...n.log||[]].reverse().find(U=>U.isGoal),q=((L==null?void 0:L.homePlayers)||[]).map(U=>({name:U.name,note:U.note,portrait:U.portrait,job:U.job}));ge(q,w,_,I,()=>F());return}F()}}catch(b){console.error("[PvP] crash:",b)}}).subscribe();async function C(c){Object.assign(n,c),n.lastActionAt=new Date().toISOString();const{error:g}=await A.from("matches").update({game_state:n}).eq("id",o);g&&u("Erreur de synchronisation","error");try{F()}catch(b){console.error("[PvP] renderPvpScreen crash:",b)}}async function ee(){if(E)return;E=!0,z&&(clearInterval(z),z=null);const c=a?x.away_id:x.home_id,g=a?"p2":"p1",b=a?"p1":"p2",h={...n,[g+"Score"]:3,[b+"Score"]:0,phase:"finished"};try{await A.from("matches").update({status:"finished",forfeit:!0,winner_id:c,home_score:h.p1Score||0,away_score:h.p2Score||0,game_state:h}).eq("id",o)}catch{}try{A.removeChannel(S)}catch{}setTimeout(()=>{Ne(e),l("home")},800)}const ue={BOOST_STAT:({value:c=1,count:g=1,roles:b=[]},h,y)=>{const v=h[i+"Team"],w=Object.entries(v).filter(([_])=>!b.length||b.includes(_)).flatMap(([_,I])=>I.filter($=>!$.used).map($=>({...$,_line:_})));if(!w.length){h.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),y(h);return}re(w,g,`Choisir ${g} joueur(s) à booster (+${c})`,_=>{_.forEach(I=>{const $=(v[I._line]||[]).find(L=>L.cardId===I.cardId);$&&($.boost=($.boost||0)+c,h.log.push({text:`⚡ +${c} sur ${$.name}`,type:"info"}))}),h[i+"Team"]=v,y(h)})},DEBUFF_STAT:({value:c=1,count:g=1,roles:b=[],target:h="ai"},y,v)=>{const w=h==="home"?i:r,_=y[w+"Team"],I=h==="home"?"allié":"adverse",$=Object.entries(_).filter(([L])=>!b.length||b.includes(L)).flatMap(([L,q])=>q.filter(U=>!U.used).map(U=>({...U,_line:L})));if(!$.length){y.log.push({text:`🎯 Aucun joueur ${I}`,type:"info"}),v(y);return}re($,g,`Choisir ${g} joueur(s) ${I}(s) (-${c})`,L=>{L.forEach(q=>{const U=(_[q._line]||[]).find(oe=>oe.cardId===q.cardId);U&&(U.boost=(U.boost||0)-c,y.log.push({text:`🎯 -${c} sur ${U.name}`,type:"info"}))}),y[w+"Team"]=_,v(y)})},GRAY_PLAYER:({count:c=1,roles:g=[],target:b="ai"},h,y)=>{const v=b==="home"?i:r,w=h[v+"Team"],_=b==="home"?"allié":"adverse",I=Object.entries(w).filter(([$])=>!g.length||g.includes($)).flatMap(([$,L])=>L.filter(q=>!q.used).map(q=>({...q,_line:$})));if(!I.length){h.log.push({text:`❌ Aucun joueur ${_}`,type:"info"}),y(h);return}re(I,c,`Choisir ${c} joueur(s) ${_}(s) à exclure`,$=>{$.forEach(L=>{const q=(w[L._line]||[]).find(U=>U.cardId===L.cardId);q&&(q.used=!0,h.log.push({text:`❌ ${q.name} exclu !`,type:"info"}))}),h[v+"Team"]=w,y(h)})},REVIVE_PLAYER:({count:c=1,roles:g=[]},b,h)=>{const y=b[i+"Team"],v=Object.entries(y).filter(([w])=>!g.length||g.includes(w)).flatMap(([w,_])=>_.filter(I=>I.used).map(I=>({...I,_line:w})));if(!v.length){b.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),h(b);return}re(v,c,`Choisir ${c} joueur(s) à ressusciter`,w=>{w.forEach(_=>{const I=(y[_._line]||[]).find($=>$.cardId===_.cardId);I&&(I.used=!1,b.log.push({text:`💫 ${I.name} ressuscité !`,type:"info"}))}),b[i+"Team"]=y,h(b)})},REMOVE_GOAL:({},c,g)=>{const b=r+"Score";c[b]>0?(c[b]--,c.log.push({text:"🚫 Dernier but annulé !",type:"info"})):c.log.push({text:"🚫 Aucun but à annuler",type:"info"}),g(c)},ADD_GOAL_DRAW:({},c,g)=>{c[i+"Score"]===c[r+"Score"]?(c[i+"Score"]++,c.log.push({text:"🎯 But bonus !",type:"info"})):c.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),g(c)},ADD_SUB:({value:c=1},g,b)=>{g.maxSubs=(g.maxSubs||3)+c,g.log.push({text:`🔄 +${c} remplacement(s)`,type:"info"}),b(g)},CUSTOM:({},c,g)=>g(c)};function re(c,g,b,h){const y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let v=[];function w(){var I,$;const _=c.map(L=>{const q={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[L._line]||"#555",U=be(L,L._line)+(L.boost||0),le=v.find(H=>H.cardId===L.cardId)?"#FFD700":"rgba(255,255,255,0.25)",G=L.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${L.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${le};background:${q};overflow:hidden;cursor:pointer;${G}">
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
    ...
    </div>`,(y=document.getElementById("pvp-home"))==null||y.addEventListener("click",()=>{try{A.removeChannel(S)}catch{}Ne(e),l("home")})}F()}async function so(e,t,o,a){var p,s;try{const d=(s=(p=t==null?void 0:t.state)==null?void 0:p.profile)==null?void 0:s.id;d&&(await A.rpc("cancel_matchmaking",{p_user_id:d}).catch(()=>{}),await A.from("matchmaking_queue").delete().eq("user_id",d).then(()=>{},()=>{}))}catch{}await pi(e,t,"random",({deckId:d,formation:l,starters:u,subsRaw:i,gcCardsEnriched:r,gcDefs:f})=>{const m=x=>lo(e,t,d,l,u,i,x,f||[],o,a);if(!r.length){m([]);return}ci(e,r,m)})}async function lo(e,t,o,a,p,s,d=[],l=[],u,i){var B;const{state:r,navigate:f,toast:m}=t,x=r.profile.id;let k=!1,n=null;We(e);try{await A.rpc("cancel_matchmaking",{p_user_id:x})}catch{}try{await A.from("matchmaking_queue").delete().eq("user_id",x)}catch{}const E=(S,C,ee,ue)=>{var re;e.innerHTML=`
      ...
      </div>`,(re=document.getElementById("lobby-cancel"))==null||re.addEventListener("click",async()=>{k=!0,n&&(A.removeChannel(n),n=null),ee&&await A.from("friend_match_invites").update({status:"declined"}).eq("id",ee),Ne(e),f("home")})},z=async(S,C)=>{k=!0,n&&(A.removeChannel(n),n=null),await new Promise(ee=>setTimeout(ee,600)),e.isConnected&&co(e,t,S,C,d,l)},{data:M}=await A.from("friend_match_invites").select("*").eq("status","pending").or(`and(inviter_id.eq.${x},invitee_id.eq.${u}),and(inviter_id.eq.${u},invitee_id.eq.${x})`).order("created_at",{ascending:!1}).limit(1).maybeSingle();let j,O;if(M&&M.inviter_id===u)O=!1,j=M.id,await A.from("friend_match_invites").update({invitee_deck_id:o,invitee_ready:!0,status:"accepted"}).eq("id",j);else{O=!0;const{data:S,error:C}=await A.from("friend_match_invites").insert({inviter_id:x,invitee_id:u,friend_id:u,inviter_deck_id:o,inviter_ready:!0,status:"pending"}).select().single();if(C||!S){m("Erreur création invitation","error"),Ne(e),f("home");return}j=S.id}if(E(!0,!O,j),!O){let S=null;const C={home_id:u,away_id:x,home_deck_id:M.inviter_deck_id,away_deck_id:o,status:"active"};let ee=await A.from("matches").insert({...C,mode:"friend"}).select().single();if(ee.error&&(console.warn("[FriendMatch] insert avec mode échoué, retry sans mode:",ee.error.message),ee=await A.from("matches").insert(C).select().single()),S=ee.data,!S){m("Erreur création match: "+(((B=ee.error)==null?void 0:B.message)||""),"error"),Ne(e),f("home");return}await A.from("friend_match_invites").update({match_id:S.id,status:"matched"}).eq("id",j),z(S.id,!1);return}n=A.channel("friend-invite-"+j).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${j}`},async S=>{if(k)return;const C=S.new;C.status==="matched"&&C.match_id?z(C.match_id,!0):C.status==="declined"?(m(`${i} a décliné l'invitation`,"warning"),Ne(e),f("home")):C.invitee_ready&&E(!0,!0,j)}).subscribe()}async function co(e,t,o,a,p=[],s=[]){const{state:d,navigate:l,toast:u}=t,i=a?"p1":"p2",r=a?"p2":"p1",f=(p||[]).map(c=>c.id),m=(p||[]).map(c=>({id:c.id,gc_type:c.gc_type,_gcDef:c._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:x}=await A.from("matches").select("*").eq("id",o).single();if(!x){u("Match introuvable","error"),l("home");return}async function k(){const[{data:c},{data:g},{data:b},{data:h}]=await Promise.all([A.rpc("get_deck_for_match",{p_deck_id:x.home_deck_id}),A.rpc("get_deck_for_match",{p_deck_id:x.away_deck_id}),A.from("users").select("id,pseudo,club_name").eq("id",x.home_id).single(),A.from("users").select("id,pseudo,club_name").eq("id",x.away_id).single()]),y=$=>({cardId:$.card_id,position:$.position,id:$.id,firstname:$.firstname,name:$.surname_encoded,country_code:$.country_code,club_id:$.club_id,job:$.job,job2:$.job2,note_g:Number($.note_g)||0,note_d:Number($.note_d)||0,note_m:Number($.note_m)||0,note_a:Number($.note_a)||0,rarity:$.rarity,skin:$.skin,hair:$.hair,hair_length:$.hair_length,clubName:$.club_encoded_name||null,clubLogo:$.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),v=((c==null?void 0:c.starters)||[]).map($=>y($)),w=((g==null?void 0:g.starters)||[]).map($=>y($)),_=(c==null?void 0:c.formation)||"4-4-2",I=(g==null?void 0:g.formation)||"4-4-2";return{p1Team:ut(v,_),p2Team:ut(w,I),p1Subs:((c==null?void 0:c.subs)||[]).map($=>y($)),p2Subs:((g==null?void 0:g.subs)||[]).map($=>y($)),p1Formation:_,p2Formation:I,p1Name:(b==null?void 0:b.club_name)||(b==null?void 0:b.pseudo)||"Joueur 1",p2Name:(h==null?void 0:h.club_name)||(h==null?void 0:h.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:a?f:[],gc_p2:a?[]:f,gcCardsFull_p1:a?m:[],gcCardsFull_p2:a?[]:m,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let n=x.game_state&&Object.keys(x.game_state).length?x.game_state:null;if(!n)if(a){n=await k();const{data:c}=await A.from("matches").select("game_state").eq("id",o).single();!(c!=null&&c.game_state)||!Object.keys(c.game_state).length?await A.from("matches").update({game_state:n,turn_user_id:x.home_id}).eq("id",o):n=c.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let c=0;c<30&&!n;c++){await new Promise(b=>setTimeout(b,400));const{data:g}=await A.from("matches").select("game_state").eq("id",o).single();g!=null&&g.game_state&&Object.keys(g.game_state).length&&(n=g.game_state)}if(!n){u("Erreur de synchronisation","error"),l("home");return}n.gc_p2=f,n.gcCardsFull_p2=m,await A.from("matches").update({game_state:n}).eq("id",o)}let E=!1,z=null,M=!1;const j=new Set,O=new Set;function B(c){var $,L;try{A.removeChannel(S)}catch{}const g=n[i+"Score"]||0,b=n[r+"Score"]||0;let h,y;c.winner_id?(h=c.winner_id===d.profile.id,y=!1):c.forfeit?(h=g>b,y=!1):(y=g===b,h=g>b),($=document.getElementById("pvp-end-overlay"))==null||$.remove();const v=document.createElement("div");v.id="pvp-end-overlay",v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const w=y?"🤝":h?"🏆":"😞",_=y?"MATCH NUL":h?"VICTOIRE !":"DÉFAITE",I=y?"#fff":h?"#FFD700":"#ff6b6b";v.innerHTML=`
      <div style="font-size:64px">${w}</div>
      <div style="font-size:26px;font-weight:900;color:${I}">${_}</div>
      <div style="font-size:18px">${n[i+"Name"]} ${g} – ${b} ${n[r+"Name"]}</div>
      ${c.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${h?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
  ...
  `;function u(i){const r=document.getElementById("mkt-content"),f=i==="buy"?d:l;if(f.length===0){r.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${m.map(x=>{var j,O,B,S;const k=(j=x.card)==null?void 0:j.player;if(!k)return"";const n=k.job==="GK"?k.note_g:k.job==="DEF"?k.note_d:k.job==="MIL"?k.note_m:k.note_a,E=po[k.rarity],z=(o.profile.credits||0)>=x.price,M=x.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${M?"opacity:0.65":""}">
          ...
          <div style="flex:1;min-width:0">
            ...
            ${i==="buy"?`<div style="font-size:11px;color:var(--gray-600)">Vendeur : ${((B=x.seller)==null?void 0:B.pseudo)||"—"}</div>`:M?`<div style="font-size:11px;color:var(--green)">✅ Vendu à ${((S=x.buyer)==null?void 0:S.pseudo)||"—"} · ${x.sold_at?new Date(x.sold_at).toLocaleDateString("fr"):""}</div>`:`<div style="font-size:11px;color:var(--gray-600)">🟢 En vente depuis le ${new Date(x.listed_at).toLocaleDateString("fr")}</div>`}
          </div>
