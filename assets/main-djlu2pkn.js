...
import{s as v,j as vi,q as zn,t as Ut,u as Oi,F as Hi,o as tt,n as Rt,g as Mt,r as Se,m as Ct,v as Mi,w as go,T as Ti,p as xo,x as bo,d as yo}from"./special-cards-lEGt-tGs.js";const ho="/";function pn(t,{navigate:e,toast:i}){let n="login";const r=()=>{var a,s,l,d,c,u,b;const o=n==="login";if(t.innerHTML=`
    <div style="
    ...
    </style>`,(a=document.getElementById("tab-login-btn"))==null||a.addEventListener("click",()=>{n="login",r()}),(s=document.getElementById("tab-reg-btn"))==null||s.addEventListener("click",()=>{n="register",r()}),o)(l=document.getElementById("login-password"))==null||l.addEventListener("keydown",x=>{var g;x.key==="Enter"&&((g=document.getElementById("login-btn"))==null||g.click())}),(d=document.getElementById("login-btn"))==null||d.addEventListener("click",async()=>{const x=document.getElementById("login-email").value.trim(),g=document.getElementById("login-password").value,p=document.getElementById("login-error");if(p.textContent="",!x||!g){p.textContent="Remplissez tous les champs.";return}const f=document.getElementById("login-btn");f.textContent="⏳ Connexion…",f.disabled=!0;const{error:y}=await v.auth.signInWithPassword({email:x,password:g});if(f.textContent="⚽ Se connecter",f.disabled=!1,y){p.textContent=y.message.includes("Invalid")?"Email ou mot de passe incorrect.":y.message;return}window.location.reload()});else{(c=document.getElementById("reg-confirm"))==null||c.addEventListener("keydown",p=>{var f;p.key==="Enter"&&((f=document.getElementById("reg-btn"))==null||f.click())});let x=null,g=!1;(u=document.getElementById("reg-access-code"))==null||u.addEventListener("input",p=>{const f=p.target.value,y=document.getElementById("access-code-status"),w=document.getElementById("reg-btn");if(clearTimeout(x),g=!1,w&&(w.disabled=!0,w.style.opacity="0.45",w.style.cursor="not-allowed"),!f){y&&(y.textContent="");return}y&&(y.textContent="⏳ Vérification…",y.style.color="rgba(255,255,255,0.4)"),x=setTimeout(async()=>{const{data:h,error:L}=await v.rpc("check_signup_password",{input_password:f});if(L){y&&(y.textContent="Erreur de vérification.",y.style.color="#f87171");return}g=!!h,y&&(y.textContent=g?"✅ Code valide":"❌ Code incorrect",y.style.color=g?"#4ade80":"#f87171"),w&&(w.disabled=!g,w.style.opacity=g?"1":"0.45",w.style.cursor=g?"pointer":"not-allowed")},400)}),(b=document.getElementById("reg-btn"))==null||b.addEventListener("click",async()=>{var E;const p=document.getElementById("reg-email").value.trim(),f=document.getElementById("reg-password").value,y=document.getElementById("reg-confirm").value,w=((E=document.getElementById("reg-access-code"))==null?void 0:E.value)||"",h=document.getElementById("reg-error");if(h.textContent="",!p||!f||!y){h.textContent="Remplissez tous les champs.";return}if(f.length<6){h.textContent="Mot de passe trop court (min. 6 caractères).";return}if(f!==y){h.textContent="Les mots de passe ne correspondent pas.";return}const{data:L}=await v.rpc("check_signup_password",{input_password:w});if(!L){h.textContent="Code d'accès incorrect.";return}const m=document.getElementById("reg-btn");m.textContent="⏳ Création…",m.disabled=!0;const{error:A}=await v.auth.signUp({email:p,password:f});if(m.textContent="🚀 Créer mon compte",m.disabled=!1,A){h.textContent=A.message;return}i("Compte créé ! Connecte-toi pour commencer.","success",4e3),n="login",r(),setTimeout(()=>{const S=document.getElementById("login-email");S&&(S.value=p)},50)})}};r()}function vo(t,{state:e,navigate:i,toast:n,refreshProfile:r}){let o="#1A6B3C",a="#D4A017";t.innerHTML=`
  ...
  `;function s(){var g;const d=document.getElementById("logo-preview"),c=document.getElementById("logo-initials"),u=((g=document.getElementById("setup-club"))==null?void 0:g.value)||"MW",b=u.trim().split(" ").filter(Boolean),x=b.length>=2?(b[0][0]+b[1][0]).toUpperCase():u.slice(0,2).toUpperCase();d&&(d.style.background=a,d.style.borderColor=o),c&&(c.textContent=x,c.style.color=o)}document.getElementById("color1").addEventListener("input",d=>{o=d.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",d=>{a=d.target.value,document.getElementById("swatch2").style.background=a,s()});function l(d){document.querySelectorAll(".setup-step").forEach(c=>c.classList.remove("active")),document.getElementById(`step-${d}`).classList.add("active"),document.getElementById("step-num").textContent=d,document.getElementById("progress-fill").style.width=`${Math.round(d/3*100)}%`,d===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const d=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("step1-error");if(c.textContent="",d.length<3){c.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await v.from("users").select("id").eq("pseudo",d).maybeSingle();if(u){c.textContent="Ce pseudo est déjà pris.";return}l(2)}),document.getElementById("step2-back").addEventListener("click",()=>l(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const d=document.getElementById("setup-club").value.trim(),c=document.getElementById("step2-error");if(c.textContent="",d.length<2){c.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await v.from("users").select("id").eq("club_name",d).maybeSingle();if(u){c.textContent="Ce nom de club est déjà pris.";return}l(3)}),document.getElementById("step3-back").addEventListener("click",()=>l(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const d=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),b=document.getElementById("step3-finish");u.textContent="",b.disabled=!0,b.textContent="Création en cours…";try{const{error:x}=await v.from("users").insert({id:e.user.id,pseudo:d,club_name:c,club_color1:o,club_color2:a,credits:1e4});if(x)throw x;await wo(e.user.id),await r(),n(`Bienvenue ${d} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(x){u.textContent=x.message,b.disabled=!1,b.textContent="🚀 Créer mon profil !"}})}async function wo(t){const e=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await v.from("users").update({pending_boosters:e,onboarding_done:!1,first_booster_opened:!1}).eq("id",t)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const _o="modulepreload",ko=function(t){return"/"+t},un={},Ui=function(e,i,n){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(l=>{if(l=ko(l),l in un)return;un[l]=!0;const d=l.endsWith(".css"),c=d?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${c}`))return;const u=document.createElement("link");if(u.rel=d?"stylesheet":_o,d||(u.as="script"),u.crossOrigin="",u.href=l,s&&u.setAttribute("nonce",s),document.head.appendChild(u),d)return new Promise((b,x)=>{u.addEventListener("load",b),u.addEventListener("error",()=>x(new Error(`Unable to preload CSS for ${l}`)))})}))}function o(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return r.then(a=>{for(const s of a||[])s.status==="rejected"&&o(s.reason);return e().catch(o)})},ti="#1A6B3C",ii="#cc2222",$o="#D4A017",fn="#888";async function Eo(t,e){const{state:i,toast:n}=e;t.innerHTML=`
    ...
    </div>`,await An(i,n,e),document.getElementById("btn-add-friend").addEventListener("click",()=>To(i,n)),document.getElementById("btn-accept-friend").addEventListener("click",()=>Ki(i,n,null,e))}async function An(t,e,i={}){const{navigate:n}=i,r=t.user.id,{data:o,error:a}=await v.from("friendships").select("id, requester_id, addressee_id").eq("status","accepted").or(`requester_id.eq.${r},addressee_id.eq.${r}`),{count:s}=await v.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",r).eq("status","pending"),l=document.getElementById("pending-badge");l&&(s>0?(l.style.display="flex",l.textContent=s):l.style.display="none");const d=document.getElementById("friends-list");if(!d)return;if(a){console.error("[Friends] Erreur:",a),d.innerHTML=`<div style="color:${ii};text-align:center;padding:16px">Erreur chargement : ${a.message}</div>`;return}const c=(o||[]).map(x=>x.requester_id===r?x.addressee_id:x.requester_id);let u={};if(c.length){const{data:x}=await v.from("users").select("id, pseudo, club_name, last_seen_at, club_color1, club_color2").in("id",c);(x||[]).forEach(g=>{u[g.id]=g})}const b=(o||[]).map(x=>({friendshipId:x.id,friend:u[x.requester_id===r?x.addressee_id:x.requester_id]||{pseudo:"?"}}));if(!b.length){d.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${b.map(({friendshipId:x,friend:g})=>Lo(g,x)).join("")}
    </div>`,d.querySelectorAll("[data-stats]").forEach(x=>{x.addEventListener("click",()=>Io(t,x.dataset.stats,x.dataset.friendName))}),d.querySelectorAll("[data-match]").forEach(x=>{x.addEventListener("click",async()=>{const g=x.dataset.friendId,p=x.dataset.friendName;if(typeof n!="function"){e("Erreur navigation","error");return}const f=t.user.id,{data:y}=await v.from("friend_match_invites").select("id").eq("inviter_id",g).eq("invitee_id",f).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle(),w=!!y;console.log("[Friends] clic match",{fid:g,fname:p,isAccepting:w}),n("match",{matchMode:"friend",friendId:g,friendName:p,isAccepting:w})})})}function Lo(t,e){const i=t.club_name||t.pseudo||"?",n=t.pseudo||"",r=(n||i).slice(0,2).toUpperCase(),o=t.club_color2||ti,a=t.club_color1||"#ffffff",s=t.last_seen_at?new Date(t.last_seen_at):null,l=s&&Date.now()-s.getTime()<3*60*1e3;return`
    <div style="display:flex;align-items:center;gap:12px;background:var(--tile-bg);border-radius:12px;padding:12px 14px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
      ...
      <div style="display:flex;gap:8px;flex-shrink:0">
        ...
        <button data-stats="${t.id}" data-friend-name="${i}" title="Voir les stats"
          style="width:38px;height:38px;border-radius:50%;border:2px solid ${$o};background:var(--tile-bg);font-size:16px;cursor:pointer;display:flex;align-items:center;justify-content:center">📊</button>
      </div>
    ...
    ${Yi()}`,document.body.appendChild(i);const n=i.querySelector("#friend-pseudo-input"),r=i.querySelector("#add-friend-error"),o=()=>i.remove();n.focus(),i.querySelector("#add-cancel").addEventListener("click",o),i.addEventListener("click",a=>{a.target===i&&o()}),i.querySelector("#add-ok").addEventListener("click",async()=>{const a=n.value.trim();if(!a){r.textContent="Entre un pseudo";return}r.textContent="";const{data:s}=await v.from("users").select("id, pseudo").ilike("pseudo",a).single();if(!s){r.textContent="Utilisateur introuvable";return}if(s.id===t.user.id){r.textContent="Tu ne peux pas t'ajouter toi-même";return}const{data:l}=await v.from("friendships").select("id, status").or(`and(requester_id.eq.${t.user.id},addressee_id.eq.${s.id}),and(requester_id.eq.${s.id},addressee_id.eq.${t.user.id})`).single();if(l){const c=l.status==="accepted"?"Vous êtes déjà amis !":l.status==="pending"?"Demande déjà envoyée":"Une demande existe déjà";r.textContent=c;return}const{error:d}=await v.from("friendships").insert({requester_id:t.user.id,addressee_id:s.id,status:"pending"});if(d){r.textContent="Erreur : "+d.message;return}o(),e(`✅ Demande envoyée à ${s.pseudo} !`,"success")})}async function Ki(t,e,i=null,n={}){const r=t.user.id,{data:o}=await v.from("friendships").select("id, requester_id").eq("addressee_id",r).eq("status","pending").order("created_at",{ascending:!1}),a=(o||[]).map(b=>b.requester_id);let s={};if(a.length){const{data:b}=await v.from("users").select("id, pseudo, club_name").in("id",a);(b||[]).forEach(x=>{s[x.id]=x})}const l=(o||[]).map(b=>({...b,requester:s[b.requester_id]||{pseudo:"?"}})),d=Vi(),c=l||[];d.innerHTML=`
    <div class="popup-box">
    ...
    ${Yi()}`,document.body.appendChild(d);const u=()=>d.remove();d.querySelector("#pending-close").addEventListener("click",u),d.addEventListener("click",b=>{b.target===d&&u()}),d.querySelectorAll("[data-accept]").forEach(b=>{b.addEventListener("click",async()=>{const{error:x}=await v.from("friendships").update({status:"accepted"}).eq("id",b.dataset.accept);if(x){e("Erreur : "+x.message,"error");return}b.closest("div[style]").remove(),e("✅ Ami accepté !","success"),An(t,e,n),i&&i()})}),d.querySelectorAll("[data-decline]").forEach(b=>{b.addEventListener("click",async()=>{await v.from("friendships").delete().eq("id",b.dataset.decline),b.closest("div[style]").remove(),e("Demande refusée","info"),i&&i()})})}async function Io(t,e,i){const n=t.user.id,[r,o]=[n,e].sort(),a=n===r,{data:s}=await v.from("friend_match_stats").select("*").eq("player1_id",r).eq("player2_id",o).single(),l=t.profile.club_name||t.profile.pseudo||"Moi",d=s||{},c=a?d.wins_p1||0:d.wins_p2||0,u=a?d.wins_p2||0:d.wins_p1||0,b=d.draws||0,x=a?d.goals_p1||0:d.goals_p2||0,g=a?d.goals_p2||0:d.goals_p1||0,p=a?d.gc_used_p1||0:d.gc_used_p2||0,f=a?d.gc_used_p2||0:d.gc_used_p1||0,y=d.matches_total||0,w=(L,m,A,E=ti,S=ii)=>`
    ...
    <div class="popup-box" style="max-width:380px">
      ...
      ${y===0?`
        ...
        ${w("GC utilisés ⚡",p,f,"#7a28b8","#7a28b8")}
  ...
  <div class="home-dark" id="home-dark">
    <div class="home-inner">
      ...
      <div class="home-footer">
        ...
        <div style="font-size:10px;color:rgba(255,255,255,0.25);font-family:monospace">build ${zo}</div>`:""}
      </div>

    </div>
  </div>`,requestAnimationFrame(()=>{var b,x,g,p,f;const l=((b=window.visualViewport)==null?void 0:b.height)||window.innerHeight,d=((x=document.querySelector(".top-nav"))==null?void 0:x.offsetHeight)||56,c=((g=document.querySelector(".bottom-nav"))==null?void 0:g.offsetHeight)||60;if(t.querySelector(".home-inner")){const y=l-d-c;t.querySelector(".home-dark").style.minHeight=y+"px"}if(window.innerWidth<768){const y=((p=t.querySelector(".home-hero"))==null?void 0:p.offsetHeight)||60,w=t.querySelector(".ranked-tile");t.querySelector(".play-grid");const h=((f=t.querySelector(".home-footer"))==null?void 0:f.offsetHeight)||44,L=["friend-requests-banner","match-invite-banner","ongoing-match-banner"].reduce((W,ee)=>{var G;return W+(((G=document.getElementById(ee))==null?void 0:G.offsetHeight)||0)},0),m=12*5,A=l-d-c-y-h-L-m-32,E=Math.max(80,Math.round(A*.28)),S=Math.max(160,Math.round(A*.72)),q=Math.floor((S-10)/2);w&&(w.style.minHeight=w.style.maxHeight=E+"px"),t.querySelectorAll(".play-tile").forEach(W=>{W.style.minHeight=W.style.height=q+"px"}),t.querySelectorAll(".play-tile .play-icon").forEach(W=>{W.style.height=Math.round(q*.55)+"px"})}}),(o=document.getElementById("nav-rankings"))==null||o.addEventListener("click",()=>i("rankings")),(a=document.getElementById("nav-matches"))==null||a.addEventListener("click",()=>i("matches")),t.querySelectorAll("[data-action]").forEach(l=>{l.addEventListener("click",()=>{l.style.transform="scale(.96)",setTimeout(()=>l.style.transform="",180);const d=l.dataset.action;if(d==="match-ai"){Bo(i);return}if(d==="match-random"){i("match",{matchMode:"random"});return}if(d==="match-friend"){i("friends");return}if(d==="mini-league"){i("mini-league");return}if(d==="ranked"){i("ranked");return}n("Bientôt disponible","info")})}),document.getElementById("logout-btn").addEventListener("click",async()=>{await v.auth.signOut(),window.location.reload()}),(s=document.getElementById("try-home2-btn"))==null||s.addEventListener("click",()=>i("home2")),qn(e,n),jo(e,n,i),Bn(e,n,i),Ao(e,n))}async function Ao(t,e){const i=t.profile.id,{data:n}=await v.from("mini_league_members").select("league_id, prize_amount, mini_leagues(name)").eq("user_id",i).eq("prize_claimed",!1).gt("prize_amount",0);if(!(n!=null&&n.length))return;const r=n.reduce((a,s)=>a+(s.prize_amount||0),0),o=document.createElement("div");o.className="modal-overlay",o.style.zIndex="2200",o.innerHTML=`<div class="modal" style="max-width:400px">
  ...
  </div>`,document.body.appendChild(o),o.querySelector("#claim-later-btn").addEventListener("click",()=>o.remove()),o.querySelector("#claim-all-btn").addEventListener("click",async a=>{const s=a.currentTarget;s.disabled=!0,s.textContent="...";let l=0;for(const d of n){const{data:c}=await v.rpc("claim_mini_league_prize",{p_league_id:d.league_id,p_user_id:i});c!=null&&c.success&&!c.already_claimed&&(l+=c.prize||0)}if(l>0){t.profile.credits=(t.profile.credits||0)+l;const d=document.getElementById("nav-credits");d&&(d.textContent=`💰 ${t.profile.credits.toLocaleString("fr")}`),e(`💰 +${l.toLocaleString("fr")} cr. récupérés !`,"success")}o.remove()})}async function Bn(t,e,i){const n=document.getElementById("ongoing-match-banner");if(!n)return;const r=t.profile.id,{data:o}=await v.from("matches").select("id, home_id, away_id, status, mode").eq("status","active").or(`home_id.eq.${r},away_id.eq.${r}`).order("created_at",{ascending:!1});if(!(o!=null&&o.length)){n.innerHTML="";return}const a=o.map(l=>l.home_id===r?l.away_id:l.home_id).filter(Boolean);let s={};if(a.length){const{data:l}=await v.from("users").select("id, pseudo, club_name").in("id",a);(l||[]).forEach(d=>{s[d.id]=d.club_name||d.pseudo})}n.innerHTML=o.map(l=>{const d=l.home_id===r?l.away_id:l.home_id,c=s[d]||"Adversaire",u=l.mode==="mini_league";return`<div style="display:flex;align-items:center;gap:10px;background:linear-gradient(135deg,rgba(10,61,30,0.8),rgba(26,107,60,0.6));color:#fff;border-radius:12px;padding:12px 16px;border:1px solid rgba(26,107,60,0.4);box-shadow:0 3px 12px rgba(26,107,60,0.3)">
    ...
    </div>`}).join(""),n.querySelectorAll("[data-resume]").forEach(l=>{l.addEventListener("click",async()=>{const d=document.getElementById("page-content")||document.getElementById("app");if(l.dataset.mini==="1"){const{data:c}=await v.from("mini_league_matches").select("id, league_id").eq("match_id",l.dataset.resume).single();c?i("match-mini-league",{mlMatchId:c.id,leagueId:c.league_id}):i("mini-league")}else{const{resumePvpMatch:c}=await Ui(async()=>{const{resumePvpMatch:u}=await Promise.resolve().then(()=>ro);return{resumePvpMatch:u}},void 0);c(d,{state:t,navigate:i,toast:e,openModal:null,closeModal:null,refreshProfile:null},l.dataset.resume)}})}),n.querySelectorAll("[data-abandon]").forEach(l=>{l.addEventListener("click",()=>{Co(async()=>{await Mo(l.dataset.abandon,l.dataset.opp,r),e("Match abandonné (défaite 3-0)","info"),Bn(t,e,i)})})})}async function Mo(t,e,i){It();const{data:n}=await v.from("matches").select("home_id, away_id, game_state, mode").eq("id",t).single();if(!n)return;const r=n.home_id===i,o=r?0:3,a=r?3:0,s=n.game_state||{};s.p1Score=o,s.p2Score=a,s.phase="finished",s.forfeit=!0,await v.from("matches").update({status:"finished",forfeit:!0,winner_id:e,home_score:o,away_score:a,game_state:s}).eq("id",t),n.mode==="mini_league"&&await v.from("mini_league_matches").update({status:"finished",home_score:o,away_score:a}).eq("match_id",t)}function Co(t){const e=document.createElement("div");e.className="modal-overlay",e.innerHTML=`<div class="modal" style="max-width:340px">
  ...
  </div>`,document.body.appendChild(e),e.querySelector("#ab-cancel").addEventListener("click",()=>e.remove()),e.querySelector("#ab-ok").addEventListener("click",()=>{e.remove(),t()}),e.addEventListener("click",i=>{i.target===e&&e.remove()})}async function jo(t,e,i){var s,l,d,c;const n=document.getElementById("match-invite-banner");if(!n)return;const{data:r}=await v.from("friend_match_invites").select("id, inviter_id, inviter:users!inviter_id(pseudo, club_name)").eq("invitee_id",t.user.id).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle();if(!r){n.innerHTML="";return}const o=((s=r.inviter)==null?void 0:s.club_name)||((l=r.inviter)==null?void 0:l.pseudo)||"?",a=r.inviter_id;n.innerHTML=`<div id="match-invite-btn" style="display:flex;align-items:center;gap:10px;background:linear-gradient(135deg,rgba(26,10,46,0.8),rgba(74,26,138,0.6));color:#fff;border-radius:12px;padding:12px 16px;border:1px solid rgba(122,40,184,0.4);cursor:pointer;box-shadow:0 3px 12px rgba(74,10,138,0.3)">
    ...
    `,document.head.appendChild(d)}const r=qo.map(d=>`
    <a class="home2-chrome-tab" data-route="${d.route}" data-key="${d.key}">
      ${d.icon?`<img src="${n}${d.icon}">`:`<span class="home2-chrome-tab-emoji">${d.emoji}</span>`}${d.label}
    ...
    `,document.body.appendChild(a),a.querySelector("#home2-mobtop-settings-btn").addEventListener("click",()=>t("settings")),a.querySelector("#home2-mobtop-credits").addEventListener("click",()=>t("boosters")));let s=document.getElementById("home2-mobile-bottom");s||(s=document.createElement("div"),s.id="home2-mobile-bottom",s.className="home2-mobile-bottom",s.innerHTML=r,document.body.appendChild(s)),document.querySelectorAll(".home2-chrome-tab").forEach(d=>{d._v2Bound||(d._v2Bound=!0,d.addEventListener("click",()=>{document.querySelectorAll(".home2-chrome-tab").forEach(c=>c.classList.remove("active")),document.querySelectorAll(`.home2-chrome-tab[data-key="${d.dataset.key}"]`).forEach(c=>c.classList.add("active")),t(d.dataset.route)}))}),document.querySelectorAll(".home2-chrome-tab").forEach(d=>d.classList.toggle("active",d.dataset.key===i));const l=`💰 ${(e.credits||0).toLocaleString("fr")}`;document.getElementById("home2-chrome-credits")&&(document.getElementById("home2-chrome-credits").textContent=l),document.getElementById("home2-mobtop-credits")&&(document.getElementById("home2-mobtop-credits").textContent=l),requestAnimationFrame(()=>{const d=window.innerWidth<1024;document.documentElement.style.setProperty("--v2-top-height",(d?a.offsetHeight:o.offsetHeight)+"px"),document.documentElement.style.setProperty("--v2-bottom-height",s.offsetHeight+"px")})}function Do(){var t,e,i,n;(t=document.getElementById("home2-chrome-header"))==null||t.remove(),(e=document.getElementById("home2-mobile-top"))==null||e.remove(),(i=document.getElementById("home2-mobile-bottom"))==null||i.remove(),(n=document.getElementById("home2-chrome-style"))==null||n.remove()}function Fo(t){if(!t)return"";const e=Date.now()-new Date(t).getTime(),i=Math.floor(e/6e4);if(i<1)return"à l'instant";if(i<60)return`il y a ${i}min`;const n=Math.floor(i/60);if(n<24)return`il y a ${n}h`;const r=Math.floor(n/24);return r<7?`il y a ${r}j`:new Date(t).toLocaleDateString("fr-FR",{day:"2-digit",month:"short"})}function Po(t,e){if(!isFinite(e.max))return"";const i=e.max-e.min+1,n=Math.floor((t-e.min)/(i/3));return["III","II","I"][Math.min(2,Math.max(0,n))]}async function No(t){const{data:e}=await v.from("users").select("id,pseudo,club_name,mmr").order("mmr",{ascending:!1}).limit(5),i=e||[],n=i.some(o=>o.id===t);let r=null;if(!n){const{data:o}=await v.from("users").select("mmr").eq("id",t).single();if(o){const{count:a}=await v.from("users").select("id",{count:"exact",head:!0}).gt("mmr",o.mmr||0);r=(a||0)+1}}return{list:i,iAmInTop:n,myPosition:r}}async function Go(t=4){const{data:e}=await v.from("patch_notes").select("id,title,description,image_url,published_at").eq("is_published",!0).order("published_at",{ascending:!1}).limit(t);return e||[]}async function Ro(){const{data:t}=await v.from("booster_configs").select("id,name,icon,price_type,price_credits,card_count").eq("is_active",!0).order("created_at",{ascending:!1}).limit(5);return t||[]}async function Oo(){const{data:t}=await v.from("patch_notes").select("*").eq("is_published",!0).order("published_at",{ascending:!1}).limit(20),e=document.createElement("div");e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px";const i=(t||[]).map(n=>{const r=new Date(n.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"long",year:"numeric"});return`<div style="padding:14px 0;border-bottom:1px solid #f0f0f0">
    ...
    </div>`,document.body.appendChild(e),e.querySelector("#journal-close-v2").addEventListener("click",()=>e.remove()),e.addEventListener("click",n=>{n.target===e&&e.remove()})}async function Ho(t,{state:e,navigate:i,toast:n}){var m,A,E,S,q,W,ee;const r=e.profile;if(!r)return;const o="/icons/";Dn(i,r,"home2",o);const a=r.mmr??1e3,s=vi(a),l=Po(a,s),d=zn(a),c=isFinite(s.max)?s.max+1:null,[{list:u,iAmInTop:b,myPosition:x},g,p]=await Promise.all([No(r.id),Go(5),Ro()]),f=(G,I)=>`
    ...
    <div class="news-item">
      ...
      <div class="news-body">
        ...
        <div class="news-time">${Fo(G.published_at)}</div>
      </div>
  ...
  <div class="home-dark" id="home-dark">
      ...
      <div class="home2-dash">
        ...
        <div class="home2-col-center">
          ...
          <div class="promo-banner" id="promo-banner">
            ...
            ${p.length>1?`<div class="promo-dots">${p.map((G,I)=>`<div class="promo-dot ${I===0?"active":""}" data-dot="${I}"></div>`).join("")}</div>`:""}
          </div>`:""}
        </div>
  ...
  </div>`,requestAnimationFrame(()=>{var ae,de;const G=((ae=window.visualViewport)==null?void 0:ae.height)||window.innerHeight,I=window.innerWidth<1024;let _=0;if(I){const j=document.getElementById("home2-mobile-top"),U=document.getElementById("home2-mobile-bottom");_=((j==null?void 0:j.offsetHeight)||0)+((U==null?void 0:U.offsetHeight)||0)}else _=((de=document.getElementById("home2-chrome-header"))==null?void 0:de.offsetHeight)||0;const H=G-_,Y=t.querySelector(".home-dark");Y&&(Y.style.minHeight=H+"px")});const L=()=>{const G=document.getElementById("mode-indicator");G&&(G.textContent=window.innerWidth>=1024?"🖥️ Mode PC":"📱 Mode Mobile")};if(L(),window.addEventListener("resize",L),(m=document.getElementById("nav-profile-btn"))==null||m.addEventListener("click",()=>i("settings")),(A=document.getElementById("nav-rankings-link"))==null||A.addEventListener("click",()=>i("rankings")),(E=document.getElementById("nav-rankings-cta"))==null||E.addEventListener("click",()=>i("rankings")),(S=document.getElementById("rank-inline-link-btn"))==null||S.addEventListener("click",()=>i("rankings")),(q=document.getElementById("nav-journal-link"))==null||q.addEventListener("click",()=>Oo()),t.querySelectorAll("[data-action]").forEach(G=>{G.addEventListener("click",()=>{G.style.transform="scale(.96)",setTimeout(()=>G.style.transform="",180);const I=G.dataset.action;if(I==="match-ai"){Nn(i,e);return}if(I==="match-random"){i("match",{matchMode:"random"});return}if(I==="match-friend"){i("friends");return}if(I==="mini-league"){i("mini-league");return}if(I==="ranked"){i("ranked");return}n("Bientôt disponible","info")})}),p.length>1){const G=document.getElementById("promo-slide-content"),I=()=>Array.from(document.querySelectorAll(".promo-dot")),_=H=>{w=H,G&&(G.innerHTML=h(p[w])),I().forEach((Y,ae)=>Y.classList.toggle("active",ae===w))};setInterval(()=>_((w+1)%p.length),5e3),document.querySelectorAll(".promo-dot").forEach(H=>{H.addEventListener("click",()=>_(Number(H.dataset.dot)))})}(W=document.getElementById("promo-cta-btn"))==null||W.addEventListener("click",()=>i("boosters")),(ee=document.getElementById("back-to-v1-btn"))==null||ee.addEventListener("click",()=>{Do(),i("home")}),Pn(e,n),Yo(e,n,i),Fn(e,n,i),Uo(e,n)}async function Uo(t,e){const i=t.profile.id,{data:n}=await v.from("mini_league_members").select("league_id, prize_amount, mini_leagues(name)").eq("user_id",i).eq("prize_claimed",!1).gt("prize_amount",0);if(!(n!=null&&n.length))return;const r=n.reduce((a,s)=>a+(s.prize_amount||0),0),o=document.createElement("div");o.className="modal-overlay",o.style.zIndex="2200",o.innerHTML=`<div class="modal" style="max-width:400px">
    <div class="modal-header"><h2 style="color:#1A6B3C">🏆 Gains à récupérer</h2></div>
    <div class="modal-body" style="padding:18px 20px">
      <p style="font-size:14px;color:#555;margin:0 0 14px">Tu as terminé sur le podium de ${n.length>1?"plusieurs mini leagues":"une mini league"} !</p>
      <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:16px">
  ...
  </div>`,document.body.appendChild(o),o.querySelector("#claim-later-btn").addEventListener("click",()=>o.remove()),o.querySelector("#claim-all-btn").addEventListener("click",async a=>{const s=a.currentTarget;s.disabled=!0,s.textContent="...";let l=0;for(const d of n){const{data:c}=await v.rpc("claim_mini_league_prize",{p_league_id:d.league_id,p_user_id:i});c!=null&&c.success&&!c.already_claimed&&(l+=c.prize||0)}if(l>0){t.profile.credits=(t.profile.credits||0)+l;const d=document.getElementById("nav-credits");d&&(d.textContent=`💰 ${t.profile.credits.toLocaleString("fr")}`),e(`💰 +${l.toLocaleString("fr")} cr. récupérés !`,"success")}o.remove()})}async function Fn(t,e,i){const n=document.getElementById("ongoing-match-banner");if(!n)return;const r=t.profile.id,{data:o}=await v.from("matches").select("id, home_id, away_id, status, mode").eq("status","active").or(`home_id.eq.${r},away_id.eq.${r}`).order("created_at",{ascending:!1});if(!(o!=null&&o.length)){n.innerHTML="";return}const a=o.map(l=>l.home_id===r?l.away_id:l.home_id).filter(Boolean);let s={};if(a.length){const{data:l}=await v.from("users").select("id, pseudo, club_name").in("id",a);(l||[]).forEach(d=>{s[d.id]=d.club_name||d.pseudo})}n.innerHTML=o.map(l=>{const d=l.home_id===r?l.away_id:l.home_id,c=s[d]||"Adversaire",u=l.mode==="mini_league";return`<div style="display:flex;align-items:center;gap:10px;background:linear-gradient(135deg,rgba(10,61,30,0.8),rgba(26,107,60,0.6));color:#fff;border-radius:12px;padding:12px 16px;border:1px solid rgba(26,107,60,0.4);box-shadow:0 3px 12px rgba(26,107,60,0.3)">
      <div style="background:rgba(255,255,255,0.15);border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0">⚽</div>
      <div style="flex:1;min-width:0">
        <div style="font-size:13px;font-weight:900">${u?"🏆 Mini League":l.mode==="friend"?"Match ami":"Match"} en cours</div>
        <div style="font-size:11px;opacity:0.8;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">vs ${c}</div>
      ...
      <button data-resume="${l.id}" data-mini="${u?"1":""}" style="width:38px;height:38px;border-radius:50%;border:none;background:#22c55e;color:#fff;font-size:18px;cursor:pointer;flex-shrink:0">⚽</button>
      <button data-abandon="${l.id}" data-opp="${d}" style="width:38px;height:38px;border-radius:50%;border:none;background:#cc2222;color:#fff;font-size:18px;cursor:pointer;flex-shrink:0">✕</button>
    </div>`}).join(""),n.querySelectorAll("[data-resume]").forEach(l=>{l.addEventListener("click",async()=>{const d=document.getElementById("page-content")||document.getElementById("app");if(l.dataset.mini==="1"){const{data:c}=await v.from("mini_league_matches").select("id, league_id").eq("match_id",l.dataset.resume).single();c?i("match-mini-league",{mlMatchId:c.id,leagueId:c.league_id}):i("mini-league")}else{const{resumePvpMatch:c}=await Ui(async()=>{const{resumePvpMatch:u}=await Promise.resolve().then(()=>ro);return{resumePvpMatch:u}},void 0);c(d,{state:t,navigate:i,toast:e,openModal:null,closeModal:null,refreshProfile:null},l.dataset.resume)}})}),n.querySelectorAll("[data-abandon]").forEach(l=>{l.addEventListener("click",()=>{Vo(async()=>{await Ko(l.dataset.abandon,l.dataset.opp,r),e("Match abandonné (défaite 3-0)","info"),Fn(t,e,i)})})})}async function Ko(t,e,i){It();const{data:n}=await v.from("matches").select("home_id, away_id, game_state, mode").eq("id",t).single();if(!n)return;const r=n.home_id===i,o=r?0:3,a=r?3:0,s=n.game_state||{};s.p1Score=o,s.p2Score=a,s.phase="finished",s.forfeit=!0,await v.from("matches").update({status:"finished",forfeit:!0,winner_id:e,home_score:o,away_score:a,game_state:s}).eq("id",t),n.mode==="mini_league"&&await v.from("mini_league_matches").update({status:"finished",home_score:o,away_score:a}).eq("match_id",t)}function Vo(t){const e=document.createElement("div");e.className="modal-overlay",e.innerHTML=`<div class="modal" style="max-width:340px">
    <div class="modal-body" style="padding:26px 22px 20px;text-align:center">
      <div style="font-size:40px;margin-bottom:8px">⚠️</div>
      <div style="font-size:17px;font-weight:900;margin-bottom:6px;color:#1a1a1a">Abandonner le match ?</div>
      <div style="font-size:13px;color:#666;margin-bottom:18px">Tu perdras par forfait <b>3-0</b>.</div>
  ...
  </div>`,document.body.appendChild(e),e.querySelector("#ab-cancel").addEventListener("click",()=>e.remove()),e.querySelector("#ab-ok").addEventListener("click",()=>{e.remove(),t()}),e.addEventListener("click",i=>{i.target===e&&e.remove()})}async function Yo(t,e,i){var s,l,d,c;const n=document.getElementById("match-invite-banner");if(!n)return;const{data:r}=await v.from("friend_match_invites").select("id, inviter_id, inviter:users!inviter_id(pseudo, club_name)").eq("invitee_id",t.user.id).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle();if(!r){n.innerHTML="";return}const o=((s=r.inviter)==null?void 0:s.club_name)||((l=r.inviter)==null?void 0:l.pseudo)||"?",a=r.inviter_id;n.innerHTML=`<div id="match-invite-btn" style="display:flex;align-items:center;gap:10px;background:linear-gradient(135deg,rgba(26,10,46,0.8),rgba(74,26,138,0.6));color:#fff;border-radius:12px;padding:12px 16px;border:1px solid rgba(122,40,184,0.4);cursor:pointer;box-shadow:0 3px 12px rgba(74,10,138,0.3)">
    <div style="background:rgba(255,255,255,0.15);border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0">⚽</div>
    <div style="flex:1"><div style="font-size:13px;font-weight:900">${o} t'invite à jouer !</div><div style="font-size:11px;opacity:0.75">Accepter la partie ?</div></div>
    <div style="display:flex;gap:8px">
      <button id="match-inv-accept" style="padding:6px 12px;border-radius:8px;border:none;background:#22c55e;color:#fff;font-size:12px;font-weight:900;cursor:pointer">Jouer !</button>
  ...
  </div>`,(d=document.getElementById("match-inv-accept"))==null||d.addEventListener("click",()=>{n.innerHTML="",i("match",{matchMode:"friend",friendId:a,friendName:o,isAccepting:!0})}),(c=document.getElementById("match-inv-decline"))==null||c.addEventListener("click",async()=>{await v.from("friend_match_invites").update({status:"declined"}).eq("id",r.id),n.innerHTML="",e("Invitation refusée","info")})}async function Pn(t,e){const i=document.getElementById("friend-requests-banner");if(!i)return;const{data:n,error:r}=await v.from("friendships").select("id, requester:users!requester_id(pseudo, club_name)").eq("addressee_id",t.user.id).eq("status","pending");if(r||!(n!=null&&n.length)){i.innerHTML="";return}const o=n.length,a=n.slice(0,2).map(l=>{var d;return((d=l.requester)==null?void 0:d.pseudo)||"?"}).join(", "),s=o>2?` +${o-2}`:"";i.innerHTML=`<div id="friend-req-btn" style="display:flex;align-items:center;gap:10px;background:linear-gradient(135deg,rgba(26,107,60,0.6),rgba(42,157,92,0.4));color:#fff;border-radius:12px;padding:12px 16px;border:1px solid rgba(26,107,60,0.4);cursor:pointer;box-shadow:0 3px 12px rgba(26,107,60,0.25)">
    <div style="background:rgba(255,255,255,0.15);border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0">👥</div>
    <div style="flex:1;min-width:0"><div style="font-size:13px;font-weight:900">${o} demande${o>1?"s":""} d'ami${o>1?"s":""}</div><div style="font-size:11px;opacity:0.85;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${a}${s}</div></div>
    <div style="font-size:20px;flex-shrink:0">›</div>
  </div>`,document.getElementById("friend-req-btn").addEventListener("click",()=>Ki(t,e,()=>Pn(t,e)))}async function Nn(t,e){const i=document.createElement("div");i.className="modal-overlay",i.style.zIndex="2000",i.innerHTML=`<div class="modal" style="max-width:420px;border-radius:18px;overflow:hidden">
    ...
    <div class="modal-body" style="padding:16px">
  ...
  </div>`,document.body.appendChild(i);const n=()=>i.remove();document.getElementById("solo-cancel").addEventListener("click",n),i.addEventListener("click",d=>{d.target===i&&n()});const[{data:r},{data:o}]=await Promise.all([v.from("solo_levels").select("*").eq("is_active",!0).order("level_number"),v.from("user_solo_progress").select("unlocked_level").eq("user_id",e.profile.id).maybeSingle()]),a=(o==null?void 0:o.unlocked_level)||1,s=r||[],l=document.getElementById("solo-levels-list");if(l){if(!s.length){l.innerHTML='<div style="text-align:center;color:#999;padding:20px">Aucun niveau configuré.</div>';return}l.innerHTML=s.map(d=>{const c=d.level_number>a,u=d.level_number===a,b=c?"#f0f0f0":u?"#eefaf2":"#f7f7f7",x=c?"#ddd":u?"#bfe8cf":"#e0e0e0",g=c?"#999":"#12401f";return`
      ...
      </div>`}).join(""),l.querySelectorAll(".solo-level-card").forEach(d=>{d.dataset.locked!=="true"&&(d.addEventListener("mouseenter",()=>{d.style.transform="translateY(-1px)"}),d.addEventListener("mouseleave",()=>{d.style.transform=""}),d.addEventListener("click",()=>{n(),t("match",{matchMode:"solo",soloLevel:Number(d.dataset.level)})}))})}}async function Wo(t,{state:e,navigate:i,toast:n}){const r=e.profile;if(!r)return;const o="/icons/";Dn(i,r,"game",o),t.innerHTML=`
  <style>
    .game-page {
      min-height: 100%;
      background: var(--page-bg);
  ...
  </div>`,t.querySelectorAll("[data-theme-choice]").forEach(b=>{b.addEventListener("click",()=>{Sa(b.dataset.themeChoice),Gn(t,e)})});const l=t.querySelector("#volume-slider"),d=t.querySelector("#volume-label");let c=null;l.addEventListener("input",()=>{So(Number(l.value)),d.textContent=`${l.value}%`,c&&(c.volume=Math.max(0,Math.min(1,Number(l.value)/100)))}),t.querySelector("#volume-test").addEventListener("click",()=>{c=Ji("/sounds/match-opening.mp3",1)}),t.querySelector("#settings-logout").addEventListener("click",async()=>{await v.auth.signOut(),window.location.reload()})}const ze={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé pour ce match."},"Double attaque":{icon:"⚡",desc:"La note d'attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la note d'un joueur adverse."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function Pe(t,e){if(!t)return 0;let i=0;switch(e){case"GK":i=Number(t.note_g)||0;break;case"DEF":i=Number(t.note_d)||0;break;case"MIL":i=Number(t.note_m)||0;break;case"ATT":i=Number(t.note_a)||0;break;default:i=0}return i+(t.boost||0)}const xn=["ATT","MIL","DEF","GK"];function Rn(t,e,i){if(!t||!e)return!1;if(i&&t.position&&e.position)return Oi(i).some(([c,u])=>c===t.position&&u===e.position||c===e.position&&u===t.position);const n=t._col!=null&&e._col!=null&&t._col===e._col,r=t._col!=null&&e._col!=null&&Math.abs(t._col-e._col)===1,o=xn.indexOf(t._line||t.job),a=xn.indexOf(e._line||e.job),s=Math.abs(o-a)===1;return(t._line||t.job)===(e._line||e.job)&&r||n&&s}function On(t,e){let i=0;const n=t.length;for(let r=0;r<n;r++)for(let o=r+1;o<n;o++){const a=t[r],s=t[o];if(!a||!s||!Rn(a,s,e))continue;const l=Ut(a,s);l==="#00ff88"?i+=10:l==="#FFD700"&&(i+=5)}return i}function ni(t,e={},i){let n=0,r=0;t.forEach(l=>{const d=l._line||l.job,c=Number(d==="MIL"?l.note_m:l.note_a)||0,u=l.stadiumBonus&&(d==="MIL"||d==="ATT")?10:0;n+=e.doubleAttack?c*2:c,r+=(l.boost||0)+u});const o=n+r,a=On(t,i);let s=o+a;return e.stolenNote&&(s-=e.stolenNote),{base:o,links:a,total:Math.max(0,s)}}function oi(t,e={},i){const n=t.reduce((a,s)=>{const l=s._line||s.job;let d=0;l==="GK"?d=Number(s.note_g)||0:l==="MIL"?d=Number(s.note_m)||0:d=Number(s.note_d)||0;const c=s.stadiumBonus&&(l==="GK"||l==="DEF"||l==="MIL")?10:0;return a+d+(s.boost||0)+c},0),r=On(t,i);let o=n+r;return e.stolenNote&&(o-=e.stolenNote),{base:n,links:r,total:Math.max(0,o)}}function Qi(t,e,i={}){return i.shield?{goal:!1,shielded:!0}:{goal:t>e,shielded:!1}}function Hn(t,e,i="easy"){const n=t.filter(a=>!a.used);if(!n.length)return[];const r=[...n].sort((a,s)=>{const l=e==="attack"?Pe(a,"ATT"):a._line==="GK"?Pe(a,"GK"):Pe(a,"DEF");return(e==="attack"?Pe(s,"ATT"):s._line==="GK"?Pe(s,"GK"):Pe(s,"DEF"))-l});let o=i==="easy"?1+Math.floor(Math.random()*2):i==="medium"?2+Math.floor(Math.random()*2):3;return r.slice(0,Math.min(o,r.length,3))}function Xo(t,e){const i={vs_ai_easy:{victoire:500,nul:250,defaite:50},vs_ai_medium:{victoire:1e3,nul:500,defaite:50},vs_ai_hard:{victoire:1500,nul:750,defaite:100},vs_ai_club:{victoire:2500,nul:1250,defaite:100}};return(i[t]||i.vs_ai_easy)[e]||0}const Jo={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},Ii={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},si=["GK","DEF","MIL","ATT"],Qo=["Tous","GK","DEF","MIL","ATT"],Zo={normal:1e3,pepite:5e3,papyte:5e3,legende:1e4};function bn(t){const e=t.player;return e?(e.rarity,Math.max(Number(e.note_g)||0,Number(e.note_d)||0,Number(e.note_m)||0,Number(e.note_a)||0)):null}function er(t){return t.length?t.reduce((e,i)=>bn(i)>bn(e)?i:e,t[0]):t[0]}const tr={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function di(t,e){return t&&Number(e==="GK"?t.note_g:e==="DEF"?t.note_d:e==="MIL"?t.note_m:t.note_a)||0}function Si(t,e=""){const i=t.player;if(!i)return"";const n=t.evolution_bonus||0,r={...i,_evolution_bonus:n},o=e?`<div style="position:absolute;top:6px;right:6px;z-index:10;background:#0a3d1e;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 7px">${e}</div>`:"";return`<div style="position:relative;display:inline-block;cursor:pointer" data-card-id="${t.id}">
    ${o}
    ${Se(r,{width:140})}
  </div>`}function yn(t){return`<div style="display:inline-block;filter:grayscale(1);opacity:0.4">
    ${Se(t,{width:140})}
  </div>`}async function ir(t,e){const{state:i,navigate:n,toast:r,openModal:o,closeModal:a}=e;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await v.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price, stadium_id, evolution_bonus,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),d=(s||[]).filter($=>$.card_type==="player"&&$.player),c=(s||[]).filter($=>$.card_type==="game_changer"),u=(s||[]).filter($=>$.card_type==="formation"),b=(s||[]).filter($=>$.card_type==="stadium"),{data:x}=await v.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(x||[]).forEach($=>{g[$.name]=$});const{data:p}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),f={};(p||[]).forEach($=>{f[$.id]=$}),b.forEach($=>{$.stadium_def&&(f[$.stadium_id]=$.stadium_def)});const y=Object.keys(Hi),w=Object.keys(ze),h={};d.forEach($=>{const C=$.player.id;h[C]=(h[C]||0)+1}),new Set(Object.keys(h).map($=>String($)));const L=new Set(u.map($=>$.formation)),m=new Set(c.map($=>$.gc_type));let A="player",E="Tous",S="",q=!1;function W(){return[...d].sort(($,C)=>{const O=si.indexOf($.player.job),P=si.indexOf(C.player.job);return O!==P?O-P:($.player.surname_real||"").localeCompare(C.player.surname_real||"")})}function ee(){return[...l||[]].sort(($,C)=>{const O=si.indexOf($.job),P=si.indexOf(C.job);return O!==P?O-P:($.surname_real||"").localeCompare(C.surname_real||"")})}function G(){return W().filter($=>{const C=$.player,O=E==="Tous"||C.job===E,P=!S||`${C.firstname} ${C.surname_real}`.toLowerCase().includes(S);return O&&P})}function I(){return ee().filter($=>{const C=E==="Tous"||$.job===E,O=!S||`${$.firstname} ${$.surname_real}`.toLowerCase().includes(S);return C&&O})}t.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:0 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;align-items:center"></div>
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{q=!q,_(),H()})))}function H(){const $=document.getElementById("col-grid");$&&(A==="player"?U($):A==="formation"?le($):A==="stadium"?ue($):fe($))}function Y($,C,O,P,X,D=0){const K=document.getElementById("col-grid"),pe=document.getElementById("col-big");if(!K||!pe)return;var ne=0;function me(){var pt;const we=window.innerWidth>=768,he=document.getElementById("col-big"),Te=document.getElementById("col-gap"),Ae=((pt=document.getElementById("col-grid"))==null?void 0:pt.parentElement)||null,Be=he?he.closest(".page"):null;if(he&&Ae&&Be){let Ee=0;Array.from(Be.children).forEach(function(B){B!==he&&B!==Ae&&B!==Te&&(Ee+=B.offsetHeight)});const je=Math.max(0,Be.clientHeight-Ee),qe=Math.round(je*(D/100)),Ne=Math.max(0,je-qe),k=we?50/71:45/63,T=Math.round(Ne*k),M=Math.max(0,Ne-T);Te&&(Te.style.height=qe+"px"),he.style.flex="none",he.style.height=T+"px",he.style.minHeight="0",Ae.style.flex="none",Ae.style.height=M+"px",Ae.style.minHeight="0",Ae.style.display="flex",Ae.style.overflow="hidden"}const $e=document.getElementById("col-grid");$e&&($e.style.height="100%",$e.style.flexShrink="0",$e.style.overflowX="auto",$e.style.overflowY="hidden",$e.style.alignItems="center",$e.style.width="100%"),pe.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:center center">'+C($[ne])+"</div>";var kt=pe.querySelector("[data-card-id],[data-form-id],[data-gc-id]");kt&&kt.addEventListener("click",function(){P($[ne])}),requestAnimationFrame(function(){var Ee=document.getElementById("big-card-inner");if(!(!Ee||!pe)){var je=pe.clientHeight,qe=pe.clientWidth-16,Ne=Ee.offsetHeight,k=Ee.offsetWidth;if(Ne>0&&k>0&&je>40){var T=Math.min(je/Ne,qe/k);Ee.style.transform="scale("+T.toFixed(3)+")",Ee.style.transformOrigin="center center"}}}),K.innerHTML=$.map(function(Ee,je){var qe=je===ne,Ne="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(qe?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+je+'" style="'+Ne+'">'+O(Ee,qe)+"</div>"}).join(""),K.querySelectorAll(".col-mini-item").forEach(function(Ee){Ee.addEventListener("click",function(){ne=Number(Ee.dataset.idx),me(),Ee.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})}),requestAnimationFrame(function(){var Ee=$e?$e.clientHeight:0,je=K.querySelector(".col-mini-item > div");if(!(!Ee||!je)){var qe=je.style.zoom;je.style.zoom="1";var Ne=je.offsetHeight;if(je.style.zoom=qe,!(Ne<=0)){var k=Ee/Ne;K.querySelectorAll(".col-mini-item > div").forEach(function(T){T.style.zoom=k.toFixed(4)})}}})}me()}function ae($,C){var O=window.innerWidth>=768?.76:.54,P;if(!$||$._fake){var X=$?$.player:null;if(!X)return"";P=yn(X)}else P=Si($,"");var D=$&&!$._fake?h[$.player&&$.player.id]||1:0,K=D>0?'<div style="position:absolute;top:6px;right:6px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:11px;font-weight:900;padding:2px 7px;z-index:3;box-shadow:0 1px 4px rgba(0,0,0,0.4)">×'+D+"</div>":"";return'<div style="display:inline-block;position:relative;zoom:'+O+';pointer-events:none;line-height:0">'+K+P+"</div>"}function de($,C,O){var P=O>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+O+"</div>":"",X=!!C,D=Mi($,Ct[$],{width:160});return X||(D='<div style="filter:grayscale(1);opacity:0.5">'+D+"</div>"),"<div "+(C?'data-form-id="'+C.id+'"':"")+' style="position:relative;cursor:pointer">'+P+D+"</div>"}function j($,C){var O=window.innerWidth>=768?.76:.54,P=Mi($,Ct[$],{width:140});return C||(P='<div style="filter:grayscale(1);opacity:0.45">'+P+"</div>"),'<div style="display:inline-block;zoom:'+O+';line-height:0;pointer-events:none">'+P+"</div>"}function U($){if(q){const C=I();if(!C.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucun joueur.</div>';return}const O=C.map(P=>d.find(X=>X.player.id===P.id)||{_fake:!0,player:P,id:"fake-"+P.id});Y(O,P=>P._fake?yn(P.player):Si(P,""),P=>P._fake?ae({player:P.player,id:"x",_fake:!0}):ae(P),P=>{P._fake||hn(P,d,h,e)})}else{const C=G();if(!C.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const O={};C.forEach(X=>{const D=X.player.id;O[D]||(O[D]=[]),O[D].push(X)});const P=Object.values(O).map(X=>er(X));Y(P,X=>{const D=h[X.player.id]||1,K=D>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${D}</div>`:"",ne=d.filter(me=>me.player.id===X.player.id&&me.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Si(X,K+ne)},X=>ae(X),X=>hn(X,d,h,e))}}function le($){const C=q?y:[...L];if(!C.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const O=C.map(P=>({formation:P,card:u.find(X=>X.formation===P)||null,owned:L.has(P)}));Y(O,({formation:P,card:X,owned:D})=>de(P,D?X:null,D?u.filter(K=>K.formation===P).length:0),({formation:P,owned:X})=>j(P,X),({card:P,owned:X})=>{X&&P&&or(P,u,e,o)},"#1A6B3C",5)}function fe($){const C=Object.keys(g),O=q?C.length?C:w:[...m];if(!O.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const P=O.map(X=>({type:X,gc:ze[X]||{icon:"⚡",desc:""},def:g[X]||null,owned:m.has(X),card:c.find(D=>D.gc_type===X)||null}));Y(P,({type:X,gc:D,def:K,owned:pe,card:ne})=>{var $e;const me=(K==null?void 0:K.name)||X,we=pe?c.filter(kt=>kt.gc_type===X).length:0,he=we>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${we}</div>`:"",Te=(K==null?void 0:K.effect)||D.desc||"",Ae=K!=null&&K.image_url?`/icons/${K.image_url}`:(($e=K==null?void 0:K.club)==null?void 0:$e.logo_url)||(K!=null&&K.country_code?`https://flagsapi.com/${K.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);let Be=tt(me,Ae,D.icon,Te,{width:160,onClick:pe});return pe||(Be=`<div style="filter:grayscale(1);opacity:0.5">${Be}</div>`),`<div ${pe&&ne?`data-gc-id="${ne.id}" data-gc-type="${X}"`:""} style="position:relative">${he}${Be}</div>`},({type:X,gc:D,def:K,owned:pe})=>{const ne=window.innerWidth>=768?.76:.54,me=(K==null?void 0:K.name)||X,we=(K==null?void 0:K.effect)||D.desc||"",he=K!=null&&K.image_url?`/icons/${K.image_url}`:null;let Te=tt(me,he,D.icon,we,{width:140});return pe||(Te=`<div style="filter:grayscale(1);opacity:0.45">${Te}</div>`),`<div style="display:inline-block;zoom:${ne};line-height:0;pointer-events:none">${Te}</div>`},({type:X,owned:D,def:K})=>{D&&nr(X,K,o)},"#7a28b8",5)}function ue($){const C="#4FC3F7",O="/";if(!b.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const P={};b.forEach(D=>{const K=D.stadium_id||"?";(P[K]=P[K]||[]).push(D)});const X=Object.entries(P).map(([D,K])=>({sid:D,def:f[D]||null,count:K.length,card:K[0]}));Y(X,({def:D,count:K})=>{var Te,Ae;const pe=(D==null?void 0:D.name)||"?",ne=((Te=D==null?void 0:D.club)==null?void 0:Te.encoded_name)||(D==null?void 0:D.country_code)||"—",me=D!=null&&D.image_url?`${O}icons/${D.image_url}`:((Ae=D==null?void 0:D.club)==null?void 0:Ae.logo_url)||(D!=null&&D.country_code?`https://flagsapi.com/${D.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),we=K>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${K}</div>`:"",he=`${ne}<br>+10 ⭐ joueurs alliés`;return`<div style="position:relative">${we}${Rt(pe,me,he,{width:160})}</div>`},({def:D})=>{var he,Te;const K=window.innerWidth>=768?.76:.54,pe=(D==null?void 0:D.name)||"?",ne=((he=D==null?void 0:D.club)==null?void 0:he.encoded_name)||(D==null?void 0:D.country_code)||"—",me=D!=null&&D.image_url?`${O}icons/${D.image_url}`:((Te=D==null?void 0:D.club)==null?void 0:Te.logo_url)||(D!=null&&D.country_code?`https://flagsapi.com/${D.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),we=`${ne}<br>+10 ⭐`;return`<div style="display:inline-block;zoom:${K};line-height:0;pointer-events:none">${Rt(pe,me,we,{width:140})}</div>`},null,C,5)}t.querySelectorAll(".col-tab-btn").forEach($=>{$.addEventListener("click",()=>{A=$.dataset.tab,E="Tous",S="",q=!1,t.querySelectorAll(".col-tab-btn").forEach(C=>{const O=C.dataset.tab===A;C.style.borderBottomColor=O?"var(--green)":"transparent",C.style.color=O?"var(--green)":"var(--tile-fg-dim)"}),_(),H()})}),_(),H()}function nr(t,e,i){const n=ze[t]||{icon:"⚡",desc:"Effet spécial."},r=(e==null?void 0:e.name)||t,o=(e==null?void 0:e.effect)||n.desc,a=e!=null&&e.image_url?`/icons/${e.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
      ${tt(r,a,n.icon,o,{width:200})}
      <div style="background:#fff3cd;border-radius:10px;padding:10px 14px;width:100%">
        <div style="font-size:12px;color:#856404">⚠️ Cette carte est à <b>usage unique</b>. Une fois jouée en match, elle est définitivement supprimée de ta collection.</div>
      </div>
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const li=1e3;function or(t,e,i,n){var g,p,f;const{state:r,toast:o,closeModal:a,navigate:s,refreshProfile:l}=i,d=t.formation,c={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const y=Ct[d]||{},w=Hi[d]||[],h=290,L=360,m=20;function A(S){const q=y[S];return q?{x:q.x*h,y:q.y*L}:null}let E=`<svg width="${h}" height="${L}" viewBox="0 0 ${h} ${L}" xmlns="http://www.w3.org/2000/svg">`;for(const[S,q]of w){const W=A(S),ee=A(q);!W||!ee||(E+=`<line x1="${W.x}" y1="${W.y}" x2="${ee.x}" y2="${ee.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const S of Object.keys(y)){const q=A(S);if(!q)continue;const W=S.replace(/\d+/,""),ee=c[W]||"#555";E+=`<circle cx="${q.x}" cy="${q.y}" r="${m}" fill="${ee}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,E+=`<text x="${q.x}" y="${q.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${W}</text>`}return E+="</svg>",E}const b=e.filter(y=>y.formation===d);b.length;const x=!t.is_for_sale;n(`Formation ${d}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(g=document.getElementById("direct-sell-form-btn"))==null||g.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${d} pour ${li.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const y=b.find(h=>!h.is_for_sale)||b[0];if(!y){o("Aucune carte à vendre","error");return}await v.from("market_listings").delete().eq("card_id",y.id),await v.from("transfer_history").delete().eq("card_id",y.id);const{error:w}=await v.from("cards").delete().eq("id",y.id);if(w){o(w.message,"error");return}await v.from("users").update({credits:(r.profile.credits||0)+li}).eq("id",r.profile.id),await l(),o(`+${li.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),s("collection")}),(p=document.getElementById("market-sell-form-btn"))==null||p.addEventListener("click",async()=>{const y=parseInt(document.getElementById("sell-price-form").value);if(!y||y<1){o("Prix invalide","error");return}await v.from("cards").update({is_for_sale:!0,sale_price:y}).eq("id",t.id),await v.from("market_listings").insert({seller_id:r.profile.id,card_id:t.id,price:y}),o("Carte mise en vente sur le marché !","success"),a(),s("collection")}),(f=document.getElementById("cancel-sell-form-btn"))==null||f.addEventListener("click",async()=>{await v.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",t.id),await v.from("market_listings").update({status:"cancelled"}).eq("card_id",t.id).eq("status","active"),o("Annonce retirée","success"),a(),s("collection")})}async function hn(t,e,i,n){var de,j,U,le,fe,ue;const{state:r,toast:o,openModal:a,closeModal:s,navigate:l,refreshProfile:d}=n,c=t.player,u=e.filter($=>$.player.id===c.id),b=u.length,x=t.evolution_bonus||0,g=Math.max((Number(c.note_g)||0)+(c.job==="GK"||c.job2==="GK"?x:0),(Number(c.note_d)||0)+(c.job==="DEF"||c.job2==="DEF"?x:0),(Number(c.note_m)||0)+(c.job==="MIL"||c.job2==="MIL"?x:0),(Number(c.note_a)||0)+(c.job==="ATT"||c.job2==="ATT"?x:0)),p=c.rarity||"normal",{data:f}=await v.from("sell_price_configs").select("*").eq("rarity",p).lte("note_min",g).gte("note_max",g).order("note_min",{ascending:!1}).limit(1);((de=f==null?void 0:f[0])==null?void 0:de.price)??Zo[p];const y=c.rarity!=="legende";Mt(c);const w=di(c,c.job)+x,h=c.job2?di(c,c.job2)+(di(c,c.job2)>0?x:0):null;Ii[c.job],c.job2&&Ii[c.job2];const L=Jo[c.rarity]||"#ccc";tr[c.country_code]||c.country_code;const m=t.evolution_bonus||0,E=w+m,S=h||0,q=S>0?S+m:0,W=u.map($=>$.id);let ee={};if(W.length){const{data:$}=await v.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",W).order("transferred_at",{ascending:!0});($||[]).forEach(C=>{ee[C.card_id]||(ee[C.card_id]=[]),ee[C.card_id].push(C)})}const G=W.length?`
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",c.note_g],["DEF",c.note_d],["MIL",c.note_m],["ATT",c.note_a]].map(([$,C])=>{const O=Ii[$],P=$==="GK"?"#fff":O,X=$===c.job||$===c.job2,D=(Number(C)||0)+(X&&m>0?m:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(j=document.getElementById("close-detail"))==null||j.addEventListener("click",s);let I=new Set;const _=c.rarity==="pepite"?1.3:c.rarity==="papyte"?.7:1;function H(){let $=0;return document.querySelectorAll(".expl-check:checked").forEach(C=>{if(C.dataset.id===t.id)return;const P=Number(C.dataset.evo)||0,X=Number(C.dataset.note)||0;$+=X+P}),Math.round($*_)}const Y=()=>{const $=I.size,C=document.getElementById("sell-action-panel");if(!C)return;C.style.display=$>0?"block":"none",document.getElementById("sell-selected-count").textContent=$;const O=document.getElementById("evolve-btn");if(O){const P=H();O.textContent=P>0?`⬆️ Évoluer (+${P})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach($=>{$.addEventListener("change",()=>{const C=$.dataset.id;$.checked?I.add(C):I.delete(C);const O=$.closest(".exemplaire-row");if(O){const P=O.querySelector(".expl-sel-overlay"),X=O.querySelector(".expl-sel-check");P&&(P.style.display=$.checked?"block":"none"),X&&(X.style.display=$.checked?"flex":"none"),O.style.transform=$.checked?"scale(1.05)":"scale(1)"}Y()})}),document.querySelectorAll(".exemplaire-row").forEach($=>{$.addEventListener("click",C=>{if(C.target.tagName==="INPUT")return;const O=$.querySelector(".expl-check");O&&!O.disabled&&(O.checked=!O.checked,O.dispatchEvent(new Event("change")))})}),(U=document.getElementById("evolve-btn"))==null||U.addEventListener("click",async()=>{if(b<=1)return;const $=[...I];if(!$.length)return;if(I.has(t.id)){const ne=document.createElement("div");ne.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",ne.innerHTML=`
    ...
    </div>`,document.body.appendChild(n);const r=n.querySelector("#pm-input");r.focus(),r.select();const o=a=>{n.remove(),i(a)};n.querySelector("#pm-ok").addEventListener("click",()=>o(r.value.trim()||null)),n.querySelector("#pm-cancel").addEventListener("click",()=>o(null)),n.querySelector("#pm-cancel2").addEventListener("click",()=>o(null)),n.addEventListener("click",a=>{a.target===n&&o(null)}),r.addEventListener("keydown",a=>{a.key==="Enter"&&o(r.value.trim()||null),a.key==="Escape"&&o(null)})})}function rr(t,e=!1){return new Promise(i=>{const n=document.createElement("div");n.className="modal-overlay",n.style.zIndex="2100",n.innerHTML=`<div class="modal" style="max-width:380px">
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    <div class="page-body">
      ...
      stadium_def:stadium_definitions(id, name, club_id, country_code, image_url,
        club:clubs(encoded_name, logo_url))`).eq("owner_id",n.profile.id).eq("card_type","stadium"),l=(a||[]).filter(y=>y.card_type==="player"&&y.player),d=(a||[]).filter(y=>y.card_type==="formation"),c=(s||[]).filter(y=>y.card_type==="stadium"),u=[...new Set(c.map(y=>y.stadium_id).filter(Boolean))];let b={};if(c.forEach(y=>{y.stadium_def&&y.stadium_id&&(b[y.stadium_id]=y.stadium_def)}),u.length&&Object.keys(b).length<u.length){const{data:y}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",u);(y||[]).forEach(w=>{b[w.id]=w})}const x=d.map(y=>y.formation).filter(Boolean),{data:g}=await v.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",t);let p=o.formation||"4-4-2";x.length>0&&!x.includes(p)&&(p=x[0]);const f={deckId:t,name:o.name,formation:p,formationCardId:o.formation_card_id,stadiumCardId:o.stadium_card_id||null,slots:{},subs:[],playerCards:l,formationCards:d,stadiumCards:c,stadDefMap:b,availableFormations:x};(g||[]).forEach(y=>{y.is_starter?f.slots[y.position]=y.card_id:f.subs.includes(y.card_id)||f.subs.push(y.card_id)}),wt(e,f,i)}function wt(t,e,i){var g,p;const{navigate:n}=i;mi[e.formation];const r=_n(e.formation),o=r.filter(f=>e.slots[f]).length,a=e.availableFormations.length>0?e.availableFormations:Object.keys(mi),s=(g=e.stadiumCards)==null?void 0:g.find(f=>f.id===e.stadiumCardId),l=s&&((p=e.stadDefMap)==null?void 0:p[s.stadium_id])||null,d=e.subs.map(f=>e.playerCards.find(y=>y.id===f)).filter(Boolean);d.length!==e.subs.length&&(e.subs=d.map(f=>f.id)),[...Object.values(e.slots),...e.subs],t.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <div class="page-header" style="display:flex;align-items:center;gap:8px;padding:6px 12px;min-height:0">
      ...
      <div style="flex:1">
        ...
        <p style="font-size:11px;margin:0">${o}/11 · ${d.length}/5 rempl.</p>
      </div>
    </div>
      ...
      <div style="display:flex;gap:0;min-height:600px">
        ...
        <div style="width:105px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 6px;gap:8px;border-right:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="display:flex;flex-direction:column;gap:6px;align-items:center" id="subs-list">
            ${d.map(f=>{const y={...f.player,_evolution_bonus:f.evolution_bonus||0};return`<div style="position:relative;flex-shrink:0;overflow:visible;padding-bottom:24px">
        ...
        <div style="width:130px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 8px;gap:12px;border-left:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="width:100%;text-align:center">
            ...
            <div id="add-stad-btn-pc" style="cursor:pointer;margin:0 auto;width:fit-content">
              ${s?(()=>{var w;const f=e.stadDefMap[s.stadium_id],y=((w=f==null?void 0:f.club)==null?void 0:w.logo_url)||(f==null?void 0:f.image_url)||null;return Rt((f==null?void 0:f.name)||"Stade",y,"+10 ⭐",{width:100})})():`<div style="width:100px;height:171px;border:2px dashed rgba(79,195,247,0.4);border-radius:8px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px">
        ...
        <div style="display:flex;gap:6px;align-items:flex-start">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;gap:2px;align-items:center;flex-wrap:nowrap;overflow-x:auto;scrollbar-width:none;-ms-overflow-style:none" id="subs-list" class="no-scrollbar">
              ${d.map(f=>{const y={...f.player,_evolution_bonus:f.evolution_bonus||0};return`<div style="position:relative;flex-shrink:0;overflow:visible;padding-bottom:20px">
          ...
          <div style="flex-shrink:0;text-align:center">
            ...
            <div id="add-stad-btn" style="cursor:pointer">
              ${s?(()=>{var w;const f=e.stadDefMap[s.stadium_id],y=((w=f==null?void 0:f.club)==null?void 0:w.logo_url)||(f==null?void 0:f.image_url)||null;return Rt((f==null?void 0:f.name)||"Stade",y,"+10⭐",{width:44})})():`<div style="width:44px;height:57px;border:2px dashed rgba(79,195,247,0.5);border-radius:6px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2px">
  ...
  </div>`;const c=window.innerWidth>=900,u=t.querySelector(".deck-pc-layout"),b=t.querySelector(".deck-mobile-layout");u&&(u.style.display=c?"block":"none"),b&&(b.style.display=c?"none":"block"),ar(t,e,r,i),t.querySelectorAll("#builder-back").forEach(f=>f.addEventListener("click",()=>n("decks")));const x=()=>{const{openModal:f,closeModal:y}=i,w=`<div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px">
    ...
    </div>`;f("⚽ Choisir une formation",w),document.querySelectorAll("#modal-body [data-forma]").forEach(h=>{h.addEventListener("click",()=>{e.formation=h.dataset.forma;const L=_n(e.formation),m={};L.forEach(A=>{e.slots[A]&&(m[A]=e.slots[A])}),e.slots=m,y(),wt(t,e,i)})})};t.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(f=>f.addEventListener("click",x)),t.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(f=>f.addEventListener("click",()=>sr(e,t,i))),t.querySelectorAll("#save-deck").forEach(f=>f.addEventListener("click",()=>cr(e,i))),t.querySelectorAll("[data-remove-sub]").forEach(f=>{f.addEventListener("click",()=>{e.subs=e.subs.filter(y=>y!==f.dataset.removeSub),wt(t,e,i)})}),t.querySelectorAll("#add-sub-btn").forEach(f=>f.addEventListener("click",()=>lr(e,t,i)))}function ar(t,e,i,n){var h,L,m;const r=window.innerWidth>=900,o=t.querySelector(r?"#deck-field-pc":"#deck-field-mobile");if(!o)return;const a=(h=e.stadiumCards)==null?void 0:h.find(A=>A.id===e.stadiumCardId),s=a&&((L=e.stadDefMap)==null?void 0:L[a.stadium_id])||null,l=Ct[e.formation]||{},d=Oi(e.formation)||[],c={};for(const A of i){const E=e.slots[A],S=E?e.playerCards.find(q=>q.id===E):null;S!=null&&S.player?c[A]={...S.player,_evolution_bonus:S.evolution_bonus||0,face:S.player.face||null}:c[A]=null}const u=window.innerWidth>=900,b=u?window.innerWidth-280:window.innerWidth-20,x=u?Math.min(b,860):b,g=Math.round(u?x*.82:x*.85),p=u?84:44;let f="";for(const[A,E]of d){const S=l[A],q=l[E];if(!S||!q)continue;const W=S.x*x,ee=Math.round(.03*g+S.y*.85*g),G=q.x*x,I=Math.round(.03*g+q.y*.85*g),_=c[A],H=c[E],Y=Ut(_,H);Y==="#ff3333"||Y==="#cc2222"?f+=`<line x1="${W.toFixed(1)}" y1="${ee.toFixed(1)}" x2="${G.toFixed(1)}" y2="${I.toFixed(1)}" stroke="${Y}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(f+=`<line x1="${W.toFixed(1)}" y1="${ee.toFixed(1)}" x2="${G.toFixed(1)}" y2="${I.toFixed(1)}" stroke="${Y}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,f+=`<line x1="${W.toFixed(1)}" y1="${ee.toFixed(1)}" x2="${G.toFixed(1)}" y2="${I.toFixed(1)}" stroke="${Y}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let y="";const w=Math.round(p*657/507);for(const A of i){const E=l[A];if(!E)continue;const S=c[A],q=E.x*x;E.y*g;const W=Math.round(.03*g+E.y*(.85*g)),ee=Math.round(q-p/2),G=Math.round(W-w/2);if(S){const I=A.replace(/\d+/,""),_=s&&(s.club_id&&String(S.club_id)===String(s.club_id)||s.country_code&&S.country_code===s.country_code),H=Se({...S,_evolution_bonus:S._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:I});_&&((m=s.club)!=null&&m.logo_url||s.image_url),y+=`<div style="position:absolute;left:${ee}px;top:${G}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${A}">
        <div style="position:relative">${H}</div>
      </div>`}else{const I=A.replace(/\d+/,"");y+=`<div style="position:absolute;left:${ee}px;top:${G}px;width:${p}px;height:${w}px;
    ...
    <div style="position:relative;width:${x}px;height:${g}px;margin:0 auto">
      <svg style="position:absolute;inset:0;width:100%;height:100%;pointer-events:none" viewBox="0 0 ${x} ${g}">${f}</svg>
      ${y}
    </div>`,o.querySelectorAll(".deck-slot-hit").forEach(A=>{A.addEventListener("click",()=>dr(A.dataset.pos,e,t,n))})}function sr(t,e,i){var a;const{openModal:n,closeModal:r}=i,o=t.stadiumCards||[];n("🏟️ Choisir un stade",`<div style="display:flex;flex-wrap:wrap;gap:10px;padding:8px">
      ...
      ${o.map(s=>{var b;const l=t.stadDefMap[s.stadium_id],d=((b=l==null?void 0:l.club)==null?void 0:b.logo_url)||(l==null?void 0:l.image_url)||null,c=t.stadiumCardId===s.id,u=Rt((l==null?void 0:l.name)||"Stade",d,"+10⭐",{width:90});return`<div class="stad-choice" data-stad-id="${s.id}" style="cursor:pointer;position:relative;border-radius:8px;${c?"box-shadow:0 0 0 3px #4fc3f7":""}">
          ${u}
        </div>`}).join("")}
    </div>`),(a=document.getElementById("stad-none"))==null||a.addEventListener("click",()=>{t.stadiumCardId=null,r(),wt(e,t,i)}),document.querySelectorAll(".stad-choice").forEach(s=>{s.addEventListener("click",()=>{t.stadiumCardId=s.dataset.stadId,r(),wt(e,t,i)})})}function dr(t,e,i,n){var g,p,f,y,w;const{openModal:r,closeModal:o}=n,a=t.replace(/\d+/,""),s=(g=e.stadiumCards)==null?void 0:g.find(h=>h.id===e.stadiumCardId),l=s&&((p=e.stadDefMap)==null?void 0:p[s.stadium_id])||null,d=e.slots[t],c=d?e.playerCards.find(h=>h.id===d):null;(f=c==null?void 0:c.player)==null||f.id;const u=new Set;Object.entries(e.slots).forEach(([h,L])=>{var A;if(h===t||!L)return;const m=e.playerCards.find(E=>E.id===L);(A=m==null?void 0:m.player)!=null&&A.id&&u.add(m.player.id)}),e.subs.forEach(h=>{var m;const L=e.playerCards.find(A=>A.id===h);(m=L==null?void 0:L.player)!=null&&m.id&&u.add(L.player.id)});const b=new Set,x=e.playerCards.filter(h=>{const L=h.player;return!(L.job===a||L.job2===a)||u.has(L.id)||b.has(L.id)?!1:(b.add(L.id),!0)});x.sort((h,L)=>{const m=h.evolution_bonus||0,A=L.evolution_bonus||0,E=(a==="GK"?h.player.note_g:a==="DEF"?h.player.note_d:a==="MIL"?h.player.note_m:h.player.note_a)+(a===h.player.job||a===h.player.job2?m:0);return(a==="GK"?L.player.note_g:a==="DEF"?L.player.note_d:a==="MIL"?L.player.note_m:L.player.note_a)+(a===L.player.job||a===L.player.job2?A:0)-E}),r(`Choisir ${a} — ${t}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${e.slots[t]?`
      ...
      ${x.length>0?'<div style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center">'+x.map(h=>{const L={...h.player,_evolution_bonus:h.evolution_bonus||0};return`<div class="player-option" data-card-id="${h.id}" style="cursor:pointer">
          ${Se(L,{width:100,showStad:!0,stadDef:l,role:a})}
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(y=document.getElementById("close-selector"))==null||y.addEventListener("click",o),(w=document.getElementById("remove-player"))==null||w.addEventListener("click",()=>{delete e.slots[t],o(),wt(i,e,n)}),document.querySelectorAll(".player-option").forEach(h=>{h.addEventListener("click",()=>{e.slots[t]=h.dataset.cardId,o(),wt(i,e,n)})})}function lr(t,e,i){var c,u,b;const{openModal:n,closeModal:r}=i,o=(c=t.stadiumCards)==null?void 0:c.find(x=>x.id===t.stadiumCardId),a=o&&((u=t.stadDefMap)==null?void 0:u[o.stadium_id])||null,s=new Set;Object.keys(t.slots).forEach(x=>{var f;const g=t.slots[x];if(!g)return;const p=t.playerCards.find(y=>y.id===g);(f=p==null?void 0:p.player)!=null&&f.id&&s.add(p.player.id)}),t.subs.forEach(x=>{var p;const g=t.playerCards.find(f=>f.id===x);(p=g==null?void 0:g.player)!=null&&p.id&&s.add(g.player.id)});const l=new Set,d=t.playerCards.filter(x=>{var g,p,f;return s.has((g=x.player)==null?void 0:g.id)||l.has((p=x.player)==null?void 0:p.id)?!1:(l.add((f=x.player)==null?void 0:f.id),!0)});n("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-sub-selector">Fermer</button>'),(b=document.getElementById("close-sub-selector"))==null||b.addEventListener("click",r),document.querySelectorAll(".player-option").forEach(x=>{x.addEventListener("click",()=>{t.subs.push(x.dataset.cardId),r(),wt(e,t,i)})})}async function cr(t,e){const{state:i,toast:n,navigate:r}=e,o=t.formationCards.find(l=>l.formation===t.formation),a=(o==null?void 0:o.id)||t.formationCardId;await v.from("decks").update({formation:t.formation,formation_card_id:a||null,stadium_card_id:t.stadiumCardId||null}).eq("id",t.deckId),await v.from("deck_cards").delete().eq("deck_id",t.deckId);const s=[];if(Object.entries(t.slots).forEach(([l,d],c)=>{s.push({deck_id:t.deckId,card_id:d,position:l,is_starter:!0,slot_order:c})}),t.subs.forEach((l,d)=>{s.push({deck_id:t.deckId,card_id:l,position:`SUB${d+1}`,is_starter:!1,slot_order:100+d})}),s.length>0){const{error:l}=await v.from("deck_cards").insert(s);if(l){n(l.message,"error");return}}n("Deck enregistré ✅","success"),r("decks")}function _n(t){const e=mi[t]||mi["4-4-2"],i=["GK1"];for(let n=1;n<=e.DEF;n++)i.push(`DEF${n}`);for(let n=1;n<=e.MIL;n++)i.push(`MIL${n}`);for(let n=1;n<=e.ATT;n++)i.push(`ATT${n}`);return i}async function Un(t=null){const e=new Date().toISOString().slice(0,10),{data:i}=await v.from("booster_configs").select("*").eq("is_active",!0).order("sort_order");if(!(i!=null&&i.length))return[];const n=i.filter(a=>!(a.available_from&&e<a.available_from||a.available_until&&e>a.available_until));if(!n.length)return[];let r=n;if(t){const a=n.filter(s=>s.max_per_user!=null);if(a.length){const{data:s}=await v.from("booster_claims").select("booster_id").eq("user_id",t).in("booster_id",a.map(d=>d.id)),l={};(s||[]).forEach(d=>{l[d.booster_id]=(l[d.booster_id]||0)+1}),r=n.filter(d=>d.max_per_user==null?!0:(l[d.id]||0)<d.max_per_user)}}if(!r.length)return[];const{data:o}=await v.from("booster_drop_rates").select("*").in("booster_id",r.map(a=>a.id)).order("sort_order");return r.map(a=>({...a,rates:(o||[]).filter(s=>s.booster_id===a.id)}))}async function pr(t,e){const{data:i}=await v.from("booster_configs").select("max_per_user").eq("id",e).single();i!=null&&i.max_per_user&&await v.from("booster_claims").insert({user_id:t,booster_id:e})}function ur(t){if(!(t!=null&&t.length))return null;const e=t.reduce((n,r)=>n+Number(r.percentage),0);let i=Math.random()*e;for(const n of t)if(i-=Number(n.percentage),i<=0)return n;return t[t.length-1]}const Kn=()=>Object.keys(Ct),fr=[{id:"players_std",img:"/icons/booster-players.png",name:"Players",sub:"5 cartes joueurs",cost:5e3,costLabel:"5 000 crédits",cardCount:5,type:"player"},{id:"players_pub",img:"/icons/booster-silver.png",name:"Players (pub)",sub:"3 cartes joueurs",cost:0,costLabel:"1 pub",cardCount:3,type:"player"},{id:"game_changer",img:"/icons/booster-gamechanger.png",name:"Game Changer",sub:"3 cartes spéciales",cost:1e4,costLabel:"10 000 crédits",cardCount:3,type:"game_changer"},{id:"formation",img:"/icons/booster-formation.png",name:"Formation",sub:"1 carte formation",cost:1e4,costLabel:"10 000 crédits",cardCount:1,type:"formation"}],Bi={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé."},"Double attaque":{icon:"⚡",desc:"La prochaine attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la prochaine action IA."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function mr(t){const e=t.player;if(!e)return"";const i=t.evolution_bonus||0;return Se({...e,_evolution_bonus:i},{width:140})}function Vn(t){var r;const e={};(t.rates||[]).forEach(o=>{e[o.card_type]=(e[o.card_type]||0)+Number(o.percentage||0)});const i=((r=Object.entries(e).sort((o,a)=>a[1]-o[1])[0])==null?void 0:r[0])||"player",n=t.image_url||"booster-players.png";return{id:t.id,img:"/icons/"+n,name:t.name,sub:`${t.card_count} carte(s)`,cost:t.price_type==="credits"&&t.price_credits||0,costLabel:t.price_type==="credits"?`${(t.price_credits||0).toLocaleString("fr")} crédits`:t.price_type==="pub"?"1 pub":"Gratuit",cardCount:t.card_count||5,type:i,isPub:t.price_type==="pub",rates:t.rates||[],allow_duplicates:t.allow_duplicates!==!1,_boosterId:t.id,_raw:t}}async function gr(t,{state:e,navigate:i,toast:n}){var u,b,x;const r=((u=e.profile)==null?void 0:u.credits)||0;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⏳ Chargement...</div>';let o=[];try{o=(await Un((b=e.user)==null?void 0:b.id)).map(Vn)}catch(g){console.warn("Erreur chargement boosters DB, fallback hardcodé",g)}o.length||(o=fr.map(g=>({...g,rates:[],isPub:g.id==="players_pub"})));const a=await v.from("cards").select("player_id, card_type, formation, stadium_id, gc_type").eq("owner_id",e.profile.id).then(g=>g.data||[]),s=new Set(a.filter(g=>g.card_type==="stadium").map(g=>g.stadium_id)),l=new Set(a.filter(g=>g.card_type==="formation").map(g=>g.formation)),d=new Set(a.filter(g=>g.card_type==="game_changer").map(g=>g.gc_type)),c={};for(const g of o){if(g.allow_duplicates!==!1||!((x=g.rates)!=null&&x.length))continue;const p=[...new Set((g.rates||[]).map(y=>y.card_type))];let f=!1;for(const y of p)if(y==="stadium"){const{data:w}=await v.from("stadium_definitions").select("id");if((w||[]).some(h=>!s.has(h.id))){f=!0;break}}else if(y==="game_changer"){const{data:w}=await v.from("gc_definitions").select("name").eq("is_active",!0);if((w||[]).some(h=>!d.has(h.name))){f=!0;break}}else if(y==="formation"){const{FORMATION_LINKS:w}=await Ui(async()=>{const{FORMATION_LINKS:h}=await import("./special-cards-lEGt-tGs.js").then(L=>L.y);return{FORMATION_LINKS:h}},__vite__mapDeps([0,1]));if(Object.keys(w).some(h=>!l.has(h))){f=!0;break}}else{f=!0;break}f||(c[g.id]=!0)}t.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📦 Boosters</h2>
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${o.map(g=>{const p=g.cost===0||r>=g.cost,f=c[g.id]===!0;return`<div class="booster-card ${!p||f?"disabled":""}" data-booster="${g.id}" style="position:relative">
  ...
  </div>`,t.querySelectorAll(".booster-card:not(.disabled)").forEach(g=>{g.addEventListener("click",async()=>{const p=o.find(f=>f.id===g.dataset.booster);if(p){g.style.opacity="0.5",g.style.pointerEvents="none";try{await xr(p,{state:e,toast:n,navigate:i,container:t})}catch(f){n(f.message,"error"),g.style.opacity="",g.style.pointerEvents=""}}})}),t.querySelectorAll(".booster-info-btn").forEach(g=>{g.addEventListener("click",p=>{p.stopPropagation();const f=o.find(y=>y.id===g.dataset.boosterId);vr(f)})})}async function xr(t,{state:e,toast:i,navigate:n,container:r}){var u,b;if(t.cost>0&&e.profile.credits<t.cost){i("Crédits insuffisants","error");return}t.isPub&&await _r();const{data:o}=await v.from("cards").select("card_type, player_id, formation").eq("owner_id",e.profile.id),a=new Set((o||[]).filter(x=>x.card_type==="player").map(x=>x.player_id)),s=new Set((o||[]).filter(x=>x.card_type==="formation").map(x=>x.formation));let l=[],d=null;try{if((u=t.rates)!=null&&u.length)l=await qi(e.profile,t);else{const x=t.type||"player";x==="player"?l=await Yn(e.profile,t.cardCount,t.cost):x==="game_changer"?l=await Wn(e.profile,t.cardCount,t.cost):x==="formation"?l=await Xn(e.profile,t.cost):l=await qi(e.profile,t)}}catch(x){d=x.message||String(x),console.error("[Booster] Erreur:",x)}if(l!=null&&l.length&&t._boosterId&&await pr(e.user.id,t._boosterId),!(l!=null&&l.length)){const x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",x.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
      <div style="font-size:12px;color:rgba(255,255,255,0.6);max-width:320px;word-break:break-all;background:rgba(255,255,255,0.05);padding:10px;border-radius:8px;margin-top:6px">
        ${d||"Vérifie la console (F12) pour plus de détails"}
      </div>
      <button style="margin-top:10px;padding:12px 28px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer" id="anim-close-err">Fermer</button>`,document.body.appendChild(x),(b=x.querySelector("#anim-close-err"))==null||b.addEventListener("click",()=>x.remove());return}l.forEach(x=>{x.card_type==="player"&&x.player?x.isDuplicate=a.has(x.player.id):x.card_type==="formation"&&(x.isDuplicate=s.has(x.formation))});const{data:c}=await v.from("users").select("*").eq("id",e.profile.id).single();c&&(e.profile=c),Jn(l,t,n)}function br(){const t=Math.random()*100;return t<.5?"legende":t<2?"special":t<10?"normal_high":"normal_low"}function gt(t){return Math.max(Number(t.note_g)||0,Number(t.note_d)||0,Number(t.note_m)||0,Number(t.note_a)||0)}function yr(t,e){let i;switch(e){case"legende":i=t.filter(n=>n.rarity==="legende"),i.length||(i=t.filter(n=>n.rarity==="pepite"||n.rarity==="papyte")),i.length||(i=t.filter(n=>gt(n)>=6));break;case"special":i=t.filter(n=>n.rarity==="pepite"||n.rarity==="papyte"),i.length||(i=t.filter(n=>gt(n)>=6));break;case"normal_high":i=t.filter(n=>n.rarity==="normal"&&gt(n)>=6),i.length||(i=t.filter(n=>gt(n)>=6));break;default:i=t.filter(n=>n.rarity==="normal"&&gt(n)>=1&&gt(n)<=5),i.length||(i=t.filter(n=>n.rarity==="normal"));break}return i.length||(i=t),i[Math.floor(Math.random()*i.length)]}async function qi(t,e){if(e.cost>0){const{error:b}=await v.from("users").update({credits:t.credits-e.cost}).eq("id",t.id);if(b)throw b}const i=e.allow_duplicates!==!1;let n=[];const{data:r,error:o}=await v.from("cards").select("player_id, card_type, formation, stadium_id, gc_type").eq("owner_id",t.id);if(o){const{data:b}=await v.from("cards").select("player_id, card_type, formation, gc_type").eq("owner_id",t.id);n=b||[]}else n=r||[];const a=new Set(n.filter(b=>b.card_type==="player").map(b=>b.player_id)),s=new Set(n.filter(b=>b.card_type==="formation").map(b=>b.formation)),l=new Set(n.filter(b=>b.card_type==="game_changer").map(b=>b.gc_type)),d=new Set(n.filter(b=>b.card_type==="stadium").map(b=>b.stadium_id).filter(Boolean)),c=new Set,u=[];for(let b=0;b<(e.cardCount||5);b++){const x=ur(e.rates);if(x){if(x.card_type==="player"){const g=E=>({légende:"legende",pépite:"pepite",pépites:"pepite"})[E]||E,p=x.rarity?g(x.rarity):null;let f=v.from("players").select("id,job,firstname,surname_real,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);p&&(f=f.eq("rarity",p));const{data:y}=await f;let w=y||[];if((x.note_min||x.note_max)&&(w=w.filter(E=>{const S=Math.max(Number(E.note_g)||0,Number(E.note_d)||0,Number(E.note_m)||0,Number(E.note_a)||0);return(!x.note_min||S>=x.note_min)&&(!x.note_max||S<=x.note_max)})),w.length||(x.note_min||x.note_max?(w=y||[],console.warn("[Booster] Aucun joueur avec note",x.note_min,"-",x.note_max,"— fallback rareté uniquement")):w=y||[]),!w.length)continue;let h=w.filter(E=>!c.has(E.id));if(i)h.length||(h=w);else if(h=h.filter(E=>!a.has(E.id)),!h.length)continue;const L=h[Math.floor(Math.random()*h.length)];c.add(L.id);const m=a.has(L.id),{data:A}=await v.from("cards").insert({owner_id:t.id,player_id:L.id,card_type:"player"}).select().single();A&&(u.push({...A,player:L,isDuplicate:m}),v.rpc("record_transfer",{p_card_id:A.id,p_player_id:L.id,p_club_name:t.club_name||t.pseudo,p_manager_name:t.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{}))}else if(x.card_type==="game_changer"){const{data:g}=await v.from("gc_definitions").select("id,name,color,effect,image_url,gc_type").eq("is_active",!0).eq("gc_type","game_changer"),p=g!=null&&g.length?g:[{name:"Ressusciter"},{name:"Double attaque"},{name:"Bouclier"},{name:"Vol de note"},{name:"Gel"}],f=i?p:p.filter(L=>!l.has(L.name));if(!i&&!f.length)continue;const y=f[Math.floor(Math.random()*f.length)],w=y.name,{data:h}=await v.from("cards").insert({owner_id:t.id,card_type:"game_changer",gc_type:w,gc_definition_id:y.id||null}).select().single();h&&u.push({...h,_gcDef:y})}else if(x.card_type==="formation"){const g=Kn(),p=i?g:g.filter(h=>!s.has(h));if(!i&&!p.length)continue;const f=p[Math.floor(Math.random()*p.length)],y=s.has(f),{data:w}=await v.from("cards").insert({owner_id:t.id,card_type:"formation",formation:f}).select();w!=null&&w[0]&&u.push({...w[0],isDuplicate:y})}else if(x.card_type==="stadium"){const{data:g,error:p}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)");if(p){console.error("[Booster] stadium_definitions:",p.message);continue}if(!(g!=null&&g.length)){console.warn("[Booster] Aucun stade en DB");continue}const f=i?g:g.filter(L=>!d.has(L.id));if(!i&&!f.length)continue;const y=f[Math.floor(Math.random()*f.length)],{data:w,error:h}=await v.from("cards").insert({owner_id:t.id,card_type:"stadium",stadium_id:y.id}).select("id,card_type,stadium_id").single();if(h){console.error("[Booster] insert stadium card:",h.message);continue}w&&u.push({...w,rarity:"normal",_stadiumDef:y})}}}return u}async function Yn(t,e,i){if(i>0){const{error:d}=await v.from("users").update({credits:t.credits-i}).eq("id",t.id);if(d)throw d}const{data:n}=await v.from("players").select("id,job,firstname,surname_real,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(n!=null&&n.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const r=n.filter(d=>d.job==="GK"),o=n.filter(d=>d.job!=="GK"),a=!t.first_booster_opened&&r.length>0,s=[];for(let d=0;d<e;d++){const c=d===0&&a?r:d===0?o:n,u=br(),b=yr(c,u);b&&s.push(b)}a&&await v.from("users").update({first_booster_opened:!0}).eq("id",t.id);const{data:l}=await v.from("cards").insert(s.map(d=>({owner_id:t.id,player_id:d.id,card_type:"player"}))).select();return(l||[]).forEach((d,c)=>{v.rpc("record_transfer",{p_card_id:d.id,p_player_id:s[c].id,p_club_name:t.club_name||t.pseudo,p_manager_name:t.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{})}),s.map((d,c)=>({...l[c],player:d}))}async function Wn(t,e,i){const{error:n}=await v.from("users").update({credits:t.credits-i}).eq("id",t.id);if(n)throw n;const{data:r}=await v.from("gc_definitions").select("id,name,gc_type,color,effect,image_url").eq("is_active",!0),o=r!=null&&r.length?r:Object.keys(Bi).map(u=>({name:u,gc_type:"game_changer"})),a=Array.from({length:e},()=>o[Math.floor(Math.random()*o.length)]),s=a.map(u=>({owner_id:t.id,card_type:"game_changer",gc_type:u.name,gc_definition_id:u.id||null})),{data:l,error:d}=await v.from("cards").insert(s).select();return d&&console.error("[Booster GC] Erreur insert:",d.message,d),(l||[]).map((u,b)=>({...u,_gcDef:a[b]||null}))}async function Xn(t,e){const{error:i}=await v.from("users").update({credits:t.credits-e}).eq("id",t.id);if(i)throw i;const{data:n}=await v.from("cards").select("formation").eq("owner_id",t.id).eq("card_type","formation"),r=new Set((n||[]).map(c=>c.formation)),o=Kn(),a=o[Math.floor(Math.random()*o.length)],s=r.has(a),{data:l,error:d}=await v.from("cards").insert({owner_id:t.id,card_type:"formation",formation:a}).select();return d&&console.error("[Booster Formation] Erreur insert:",d.message,d),(l||[]).map(c=>({...c,isDuplicate:s}))}function Jn(t,e,i,n=null){var G,I;if(!t||t.length===0){const _=document.createElement("div");_.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",_.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
  ...
  `,document.body.appendChild(r);let o=!1;const a=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let l=!1;const d=_=>_.touches&&_.touches[0]?_.touches[0].clientX:_.clientX;function c(_){o||(l=!0,s.style.opacity="1",u(_))}function u(_){if(!l||o)return;const H=a.getBoundingClientRect(),Y=d(_)-H.left,ae=Math.max(0,Math.min(1,Y/H.width));s.style.width=ae*H.width+"px",ae>=.82&&x()}function b(){o||(l=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function x(){var H;if(o)return;o=!0,l=!1,s.style.width="100%",s.style.opacity="1",(H=document.getElementById("cut-flash"))==null||H.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const _=document.getElementById("cut-hint");_&&(_.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",y(0)},620)}a.addEventListener("pointerdown",c),window.addEventListener("pointermove",u),window.addEventListener("pointerup",b),a.addEventListener("touchstart",c,{passive:!0}),window.addEventListener("touchmove",u,{passive:!0}),window.addEventListener("touchend",b);let g=0,p=!1;const f=new Set;function y(_){g=_,document.getElementById("reveal-phase").style.display="flex",w(),h(_,0),S()}function w(){const _=document.getElementById("card-dots");_&&(_.innerHTML=t.map((H,Y)=>`<div class="card-dot" data-i="${Y}" style="width:8px;height:8px;border-radius:50%;background:${Y===g?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),_.querySelectorAll(".card-dot").forEach(H=>H.addEventListener("click",()=>m(parseInt(H.dataset.i)))))}function h(_,H){var $,C;const Y=t[_],ae=document.getElementById("card-counter"),de=document.getElementById("card-track");ae&&(ae.textContent=`Carte ${_+1} / ${t.length}`);const j=document.getElementById("reveal-btns");j&&(j.style.display=_===t.length-1?"flex":"none");const U=Y.card_type==="player"&&(($=Y.player)==null?void 0:$.rarity)==="legende",le=!f.has(_);f.add(_);let fe=0;if(Y.card_type==="player"&&Y.player){const O=Y.player,P=O.job||"ATT";fe=(Number(P==="GK"?O.note_g:P==="DEF"?O.note_d:P==="MIL"?O.note_m:O.note_a)||0)+(Y.evolution_bonus||0)}const ue=O=>{de.innerHTML=`
        <div id="current-card-wrap" style="position:relative;display:flex;flex-direction:column;align-items:center;gap:8px;${U?"filter:drop-shadow(0 0 20px #7a28b8)":""}">
          <div style="transform:scale(1.25);transform-origin:center">${hr(Y)}</div>
      ...
      </div>`,document.body.appendChild(n),n.addEventListener("click",a=>{a.target===n&&n.remove()}),(i=document.getElementById("odds-close"))==null||i.addEventListener("click",()=>n.remove());return}wr()}function wr(){const t=document.createElement("div");t.style.cssText=`position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;
  ...
  `,document.body.appendChild(t),t.addEventListener("click",e=>{e.target===t&&t.remove()}),document.getElementById("odds-close").addEventListener("click",()=>t.remove())}function _r(){return new Promise(t=>{const e=document.createElement("div");e.style.cssText=`
    ...
    `,document.body.appendChild(e);let i=5;const n=setInterval(()=>{i--;const r=document.getElementById("mw-ad-cd");r&&(r.textContent=i),i<=0&&(clearInterval(n),e.remove(),t(!0))},1e3)})}async function kr(t,{state:e,navigate:i,toast:n,refreshProfile:r}){var b,x;const{data:o}=await v.from("users").select("*").eq("id",e.user.id).single();o&&(e.profile=o);let a=Array.isArray((b=e.profile)==null?void 0:b.pending_boosters)?[...e.profile.pending_boosters]:[];if(!a.length){await v.from("users").update({onboarding_done:!0}).eq("id",e.user.id),i("home");return}let s=null;try{const p=(await Un()).find(f=>(f.name||"").toLowerCase().includes("new player"));p&&(s=Vn(p))}catch(g){console.warn('[Onboarding] Config "Booster (new player)" introuvable, fallback taux par défaut',g)}const l=a.length;let d=0;t.innerHTML=`
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
  ...
  </div>`;const c=async()=>{await v.from("users").update({pending_boosters:a}).eq("id",e.user.id)};async function u(){var w;if(d>=l||!a.length){await v.from("users").update({pending_boosters:[],onboarding_done:!0}).eq("id",e.user.id),r&&await r(),n("Tous tes boosters sont ouverts ! Bon jeu 🎮","success",4e3),i("home");return}const g=a[0],{data:p}=await v.from("users").select("*").eq("id",e.user.id).single();p&&(e.profile=p);let f=[];try{if(g.type==="formation")f=await Xn(e.profile,0);else if(g.type==="game_changer")f=await Wn(e.profile,g.count||3,0);else if(s&&((w=s.rates)!=null&&w.length)){const h={...s,cost:0,cardCount:g.count||s.cardCount||5};f=await qi(e.profile,h),g.guaranteeGK&&!e.profile.first_booster_opened&&(f.some(m=>m.player&&m.player.job==="GK")||await $r(e.profile,f),await v.from("users").update({first_booster_opened:!0}).eq("id",e.profile.id))}else f=await Yn(e.profile,g.count||5,0)}catch(h){n(h.message||"Erreur ouverture booster","error");return}a.shift(),d++,await c();const y=g.type==="formation"?{name:"Booster Formation",img:"/icons/booster-formation.png"}:g.type==="game_changer"?{name:"Booster Game Changer",img:"/icons/booster-gamechanger.png"}:{name:`Booster Joueurs (${d}/${l})`,img:(s==null?void 0:s.img)||"/icons/booster-players.png"};Jn(f,y,i,()=>{u()})}(x=document.getElementById("onboard-start"))==null||x.addEventListener("click",()=>u())}async function $r(t,e){try{const{data:i}=await v.from("players").select("id,job,firstname,surname_real,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0).eq("job","GK");if(!(i!=null&&i.length))return;const n=i[Math.floor(Math.random()*i.length)],r=e.findIndex(a=>a.player);if(r===-1)return;const o=e[r];await v.from("cards").update({player_id:n.id}).eq("id",o.id),e[r]={...o,player_id:n.id,player:n}}catch(i){console.warn("[Onboarding] ensureGK échec",i)}}const _t={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},Er={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function St(t,e,i,n,r){var o;t.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
    <div style="text-align:center;padding:40px;color:#fff">
      <div style="font-size:48px;margin-bottom:16px">${e}</div>
      <p style="margin-bottom:16px">${i}</p>
      <button class="btn btn-primary" id="msg-btn">${n}</button>
    </div>
  </div>`,(o=document.getElementById("msg-btn"))==null||o.addEventListener("click",r)}function Di(t,e){var o,a;const i=t.player,n=t.evolution_bonus||0,r=i.job2&&Number(i[`note_${i.job2.toLowerCase()}`])||0;return{cardId:t.id,position:e||null,id:i.id,firstname:i.firstname,name:i.surname_real,country_code:i.country_code,club_id:i.club_id,job:i.job,job2:i.job2,note_g:(Number(i.note_g)||0)+(i.job==="GK"?n:0)+(i.job2==="GK"&&r>0?n:0),note_d:(Number(i.note_d)||0)+(i.job==="DEF"?n:0)+(i.job2==="DEF"&&r>0?n:0),note_m:(Number(i.note_m)||0)+(i.job==="MIL"?n:0)+(i.job2==="MIL"&&r>0?n:0),note_a:(Number(i.note_a)||0)+(i.job==="ATT"?n:0)+(i.job2==="ATT"&&r>0?n:0),evolution_bonus:n,rarity:i.rarity,skin:i.skin,hair:i.hair,hair_length:i.hair_length,face:i.face||null,clubName:((o=i.clubs)==null?void 0:o.encoded_name)||null,clubLogo:((a=i.clubs)==null?void 0:a.logo_url)||null,boost:0,used:!1,_line:null,_col:null}}function ri(t,e){if(!e||!t)return t;const{club_id:i,country_code:n}=e;return Object.values(t).forEach(r=>{Array.isArray(r)&&r.forEach(o=>{const a=i&&String(o.club_id)===String(i),s=n&&String(o.country_code)===String(n);(a||s)&&(o.stadiumBonus=!0)})}),t}function gi(t,e){if(!e||!(t!=null&&t.length))return t;const{club_id:i,country_code:n}=e;return t.forEach(r=>{if(!r)return;const o=i&&String(r.club_id)===String(i),a=n&&String(r.country_code)===String(n);(o||a)&&(r.stadiumBonus=!0)}),t}function st(t){return t===1?[1]:t===2?[0,2]:t===3?[0,1,2]:t===4?[0,1,1,2]:t===5?[0,1,1,1,2]:[1]}function Qn(){const t=Math.random()*100;return t<10?10:t<30?5:3}function xi(t,e){const i=_t[e]||_t["4-4-2"],n={GK:[],DEF:[],MIL:[],ATT:[]};if(t.length&&t.every(a=>a.position)){for(const a of["GK","DEF","MIL","ATT"]){const s=t.filter(d=>d.position&&d.position.replace(/\d+$/,"")===a).sort((d,c)=>parseInt(d.position.replace(/\D+/g,""),10)-parseInt(c.position.replace(/\D+/g,""),10)).map(d=>({...d,_line:a})),l=st(s.length);s.forEach((d,c)=>{d._col=l[c]}),n[a]=s}return n}const o=[...t];for(const a of["GK","DEF","MIL","ATT"]){const s=[];for(let d=0;d<i[a];d++){let c=o.findIndex(u=>u.job===a);if(c===-1&&(c=o.findIndex(u=>u.job2===a)),c===-1&&(c=0),o[c]){const u={...o[c],_line:a};s.push(u),o.splice(c,1)}}const l=st(s.length);s.forEach((d,c)=>{d._col=l[c]}),n[a]=s}return n}function dt(t){document.querySelectorAll(".top-nav, .bottom-nav").forEach(e=>{e.style.setProperty("display","none","important"),e.dataset.matchHidden="1"}),t&&t.style.setProperty("padding-bottom","0","important")}function it(t){document.querySelectorAll(".top-nav, .bottom-nav").forEach(e=>{e.style.removeProperty("display"),delete e.dataset.matchHidden}),t&&t.style.removeProperty("padding-bottom")}function wi(t,e,i){const r=new Set,o=e.filter(c=>{const u=c.gc_type||c.id;return r.has(u)?!1:(r.add(u),!0)});let a=[];function s(c,u){const b=c._gcDef,x=(b==null?void 0:b.name)||c.gc_type,g=b!=null&&b.image_url?`/icons/${b.image_url}`:null,p=tt(x,g,"⚡",(b==null?void 0:b.effect)||"",{width:100});return`<div class="gc-select-card" data-id="${c.id}"
    ...
    </div>`}const l=c=>{t.style.overflow="",t.style.height="",t.style.display="",t.style.flexDirection="",i(c)};function d(){var u,b,x;t.style.overflow="hidden",t.style.height="100%",t.style.display="flex",t.style.flexDirection="column";const c=a.length>0;t.innerHTML=`
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px 16px">
        ${o.map(g=>{const p=a.find(f=>f.gc_type===g.gc_type);return s(g,!!p)}).join("")}
      </div>
    ...
    </div>`,t.querySelectorAll(".gc-select-card").forEach(g=>{g.addEventListener("click",()=>{const p=g.dataset.id,f=o.find(w=>w.id===p);if(!f)return;const y=a.findIndex(w=>w.gc_type===f.gc_type);y>-1?a.splice(y,1):a.length<3&&a.push(f),d()})}),(u=t.querySelector("#gc-launch"))==null||u.addEventListener("click",()=>{c&&l(a)}),(b=t.querySelector("#gc-no-gc"))==null||b.addEventListener("click",()=>l([])),(x=t.querySelector("#gc-reset"))==null||x.addEventListener("click",()=>{a.length&&(a=[],d())})}d()}function Lr(t,e){var r;const i=((r=e==null?void 0:e.state)==null?void 0:r.params)||{},n=t||i.matchMode||"vs_ai_easy";return n==="friend"?`Match vs ${i.friendName||"Ami"}`:n==="random"?"Match vs Random":n==="ranked"?"Match Classé":n==="mini_league"||n==="mini-league"?"🏆 Match de Mini League":`Match vs IA — ${n.replace("vs_ai_","").toUpperCase()}`}async function Tr(t,e,i){const{state:n,navigate:r}=e;t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await v.from("decks").select(`id,name,is_active,formation,stadium_card_id,
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),l=[...new Set((s||[]).filter(b=>{var x,g;return((x=b.card)==null?void 0:x.card_type)==="stadium"&&((g=b.card)==null?void 0:g.stadium_id)}).map(b=>b.card.stadium_id))],d={};if(l.length){const{data:b}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",l);(b||[]).forEach(x=>{d[x.id]=x}),(s||[]).forEach(x=>{var g,p;((g=x.card)==null?void 0:g.card_type)==="stadium"&&((p=x.card)!=null&&p.stadium_id)&&(x.card._stadiumDef=d[x.card.stadium_id]||null)})}let c=0;function u(){var m,A,E,S,q,W,ee;const b=o[c],x=(s||[]).filter(G=>G.deck_id===b.id),g=x.filter(G=>{var I;return G.is_starter&&((I=G.card)==null?void 0:I.player)}).map(G=>Di(G.card,G.position)),p=x.find(G=>{var I;return((I=G.card)==null?void 0:I.card_type)==="formation"}),f=b.formation||((m=p==null?void 0:p.card)==null?void 0:m.formation)||"4-4-2";let y=g.length>=11?xi(g,f):null,w=((A=b.stadium_card)==null?void 0:A.stadium_def)||null;w&&y&&(y=ri(y,w));const h=g.length>=11;dt(t),t.style.height="100%",t.style.overflow="hidden",t.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <div id="deck-top-bar" style="flex-shrink:0">
        <div style="padding:8px 16px;background:rgba(0,0,0,0.4);text-align:center">
          <div style="font-size:10px;opacity:.6;letter-spacing:2px;text-transform:uppercase">${Lr(i,e)}</div>
        ...
        <div style="display:flex;align-items:center;gap:8px;padding:6px 8px">
          ...
          <div style="flex:1;text-align:center">
            ...
            <div style="font-size:11px;opacity:.6">${f} · ${g.length}/11${b.is_active?" · ⭐":""}</div>
          </div>
      ...
      <!-- Pagination -->
      ${o.length>1?`<div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">${o.map((G,I)=>`<div style="width:6px;height:6px;border-radius:50%;background:${I===c?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}</div>`:""}
      ...
      <div id="deck-bottom-bar" style="flex-shrink:0;padding:10px 14px 14px;display:flex;flex-direction:column;gap:8px;background:rgba(0,0,0,0.2)">
    ...
    </div>`,requestAnimationFrame(()=>requestAnimationFrame(function G(){const I=t.querySelector(".deck-preview-wrap"),_=t.querySelector("#deck-swipe-zone");if(!I||!_||!y)return;const H=_.clientWidth>=900,Y=Math.max(200,_.clientHeight-(H?60:40)),de=Math.max(200,_.clientWidth-16),j=H?Math.min(117,Math.max(52,Math.round(de*.22))):Math.max(44,Math.round(de*.168));if(Y<220||de<220){requestAnimationFrame(G);return}const U=H?null:Math.round(j*.55);I.innerHTML=Ot(y,f,null,[],de,Y,[],U),I.style.cssText=`width:${de}px;height:${Y}px;overflow:hidden;margin:${H?0:60}px auto 0`;const le=I.querySelector("svg");le&&(le.style.cssText="display:block;width:100%;height:100%",le.setAttribute("preserveAspectRatio",H?"xMidYMid meet":"none"))})),(S=document.getElementById("prev-deck"))==null||S.addEventListener("click",()=>{c>0&&(c--,u())}),(q=document.getElementById("next-deck"))==null||q.addEventListener("click",()=>{c<o.length-1&&(c++,u())}),(W=document.getElementById("validate-deck"))==null||W.addEventListener("click",()=>{if(!h)return;const G=e.state.params||{};e.navigate("match",{...G,matchMode:G.matchMode||i,deckId:b.id})}),(ee=document.getElementById("cancel-deck-select"))==null||ee.addEventListener("click",()=>{it(t),r("home")});const L=document.getElementById("deck-swipe-zone");if(L){let G=0,I=0;L.addEventListener("touchstart",_=>{G=_.touches[0].clientX,I=_.touches[0].clientY},{passive:!0}),L.addEventListener("touchend",_=>{const H=_.changedTouches[0].clientX-G,Y=_.changedTouches[0].clientY-I;Math.abs(H)<40||Math.abs(H)<Math.abs(Y)||(H<0&&c<o.length-1?(c++,u()):H>0&&c>0&&(c--,u()))},{passive:!0})}}u()}function vt(t,e=44,i=58,n=null){return Se(t,{width:e,showStad:!!n,stadDef:n,used:t==null?void 0:t.used})}function Je(t,e,i,n,r){if(!(t!=null&&t.length))return"";const o=t.slice(0,5);let a='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return o.forEach((s,l)=>{const d=s._line||s.job||"MIL";let c=s.boost||0;if(s.stadiumBonus&&(n==="attack"&&(d==="ATT"||d==="MIL")||n==="defense"&&(d==="GK"||d==="DEF"||d==="MIL")?c+=10:n||(c+=10)),a+=Se(s,{width:40,role:d,extraNote:c}),l<o.length-1){const u=o[l+1],b=Rn(s,u,r)?Ut(s,u):null;a+=`<div style="width:7px;height:3px;background:${!b||b==="#ff3333"||b==="#cc2222"?"rgba(255,255,255,0.12)":b};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(a+=`<div style="margin-left:6px;background:${e};color:${e==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),a+="</div>",a}function _i(t,e,i,n,r=310,o=310,a=[],s=null){const l=Ct[e]||{},d=Oi(e)||Hi[e]||[],c={},u=["ATT","MIL","DEF","GK"];for(const y of u)(t[y]||[]).forEach((h,L)=>{c[`${y}${L+1}`]=h});function b(y){const w=l[y];return w?{x:w.x*r,y:w.y*o}:null}let x="";for(const[y,w]of d){const h=b(y),L=b(w);if(!h||!L)continue;const m=c[y],A=c[w],E=Ut(m,A);E==="#00ff88"||E==="#FFD700"?(x+=`<line x1="${h.x.toFixed(1)}" y1="${h.y.toFixed(1)}" x2="${L.x.toFixed(1)}" y2="${L.y.toFixed(1)}"
        ...
        stroke="${E}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=typeof window<"u"&&window.innerWidth>=900?Math.min(Math.max(81,Math.round(r*.225)),117):Math.max(44,Math.round(r*.168)),p=Math.round(g*657/507);for(const[y,w]of Object.entries(c)){const h=b(y);if(!h||!w)continue;const L=y.replace(/[0-9]/g,""),m=a.includes(w.cardId),A=i==="attack"&&(["MIL","ATT"].includes(L)||m)&&!w.used||i==="defense"&&["GK","DEF","MIL"].includes(L)&&!w.used,E=n.includes(w.cardId);let S=w.boost||0,q=!1;w.stadiumBonus&&(i==="attack"&&(L==="ATT"||L==="MIL")||i==="defense"&&(L==="GK"||L==="DEF"||L==="MIL")?(S+=10,q=!0):i||(S+=10,q=!0));const W=Math.round(h.x-g/2),ee=Math.round(h.y-p/2);if(w.used){x+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${W}" y="${ee}" width="${g}" height="${p}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${w.cardId}" data-role="${L}" style="cursor:pointer"/>`;continue}const G=Se({...w,_evolution_bonus:0,stadiumBonus:!1},{width:g,showStad:!1,stadDef:null,role:L,extraNote:S,_cardOffset:30,_forceStadColor:q}),I=E?`position:absolute;top:30px;left:0;width:${g}px;height:${p}px;outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;pointer-events:none;`:"";x+=`<foreignObject x="${W-2}" y="${ee-30}" width="${g+8}" height="${p+60}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="position:relative">
        ${G}
        ${E?`<div style="${I}"></div>`:""}
      </div>
    </foreignObject>`,A&&(x+=`<rect x="${W}" y="${ee}" width="${g}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${E?"selected":""}" data-card-id="${w.cardId}" data-role="${L}" style="cursor:pointer"/>`)}const f=s!==null?s:Math.round(Math.max(g*.7,80));return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-f} ${-f} ${r+f*2} ${o+f*2}" width="100%" style="display:block;width:100%;margin:0 auto">
    ${x}
  </svg>`}function Ot(t,e,i,n,r=300,o=300,a=[],s=null){return`<div id="match-terrain-wrap" style="position:relative;padding:0 4px">
    ${_i(t,e,i,n,r,o,a,s)}
  </div>`}async function ki(t,e,i,n){var A;const{state:r,navigate:o,toast:a}=e;dt(t);const s=r.params||{};if(t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!s.deckId)return Tr(t,e,i);const l=s.deckId;let d,c,u,b;try{const E=await Promise.all([v.from("decks").select("formation,name,stadium_card_id").eq("id",l).single(),v.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_real,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",l).order("slot_order")]);d=E[0].data,u=E[0].error,c=E[1].data,b=E[1].error}catch(E){console.error("[Match] Exception chargement deck:",E),St(t,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>o("home"));return}if(u||b){console.error("[Match] Erreur Supabase:",u||b),St(t,"⚠️","Erreur lors du chargement du deck.","Retour",()=>o("home"));return}const x=(c||[]).filter(E=>{var S;return E.is_starter&&((S=E.card)==null?void 0:S.player)}).map(E=>Di(E.card,E.position)),g=(c||[]).filter(E=>{var S;return!E.is_starter&&((S=E.card)==null?void 0:S.player)}).map(E=>Di(E.card,E.position));if(x.length<11){St(t,"⚠️",`Deck incomplet (${x.length}/11).`,"Compléter",()=>o("decks"));return}const p=(c||[]).find(E=>{var S;return((S=E.card)==null?void 0:S.card_type)==="formation"}),f=(d==null?void 0:d.formation)||((A=p==null?void 0:p.card)==null?void 0:A.formation)||"4-4-2",{data:y,error:w}=await v.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",r.profile.id).eq("card_type","game_changer"),{data:h}=await v.from("gc_definitions").select("*").eq("is_active",!0),L=(y||[]).map(E=>({...E,_gcDef:(h==null?void 0:h.find(S=>S.name===E.gc_type||S.id===E.gc_definition_id))||null}));let m=null;if(d!=null&&d.stadium_card_id){const{data:E}=await v.from("cards").select("stadium_id").eq("id",d.stadium_card_id).single();if(E!=null&&E.stadium_id){const{data:S}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",E.stadium_id).single();m=S||null}}n({deckId:l,formation:f,starters:x,subsRaw:g,gcCardsEnriched:L,gcDefs:h||[],stadiumDef:m})}function zt(){return Math.min(window.innerWidth-40,860)}function Gt(){return Math.round(zt()*1.1)}function Zn(t){var r,o;if(!t)return null;const e=t._line||t.job||"MIL",i=e==="GK"?t.note_g||0:e==="DEF"?t.note_d||0:e==="MIL"?t.note_m||0:t.note_a||0,n=t.stadiumBonus?10:0;return{name:t.name,firstname:t.firstname||"",note:i+(t.boost||0)+n,note_g:t.note_g||0,note_d:t.note_d||0,note_m:t.note_m||0,note_a:t.note_a||0,_evolution_bonus:0,stadiumBonus:t.stadiumBonus||!1,boost:t.boost||0,job:t.job,job2:t.job2||null,_line:t._line||t.job,_col:t._col,country_code:t.country_code,club_id:t.club_id,rarity:t.rarity,clubName:t.clubName||((r=t.clubs)==null?void 0:r.encoded_name)||null,clubLogo:t.clubLogo||((o=t.clubs)==null?void 0:o.logo_url)||null,face:t.face||null,portrait:Mt(t)}}function Ir(t,e,i,n="Adversaire"){Cn("/sounds/match-opening.mp3",.3);const r=zt(),o=Gt();return`
    ...
    </div>`}function eo(t){var o,a,s;if(!t)return"";const e=l=>l?Se({...l,_evolution_bonus:0},{width:52,role:l._line||l.job,showStad:!!l.stadiumBonus,extraNote:l.boost||0}):"",n={goal:"⚽","goal-home":"⚽","goal-ai":"⚽",duel:"⚔️",midfield:"🎯",sub:"🔄",gc:"⚡",boost:"💥","defense-won":"🛡️","attack-won":"⚔️","defense-lost":"😓","attack-lost":"😓"}[t.type]||"📋";return`
    ...
    <div style="animation:goalPop 0.4s ease-out;text-align:center">
      ...
      }`,document.head.appendChild(l)}setTimeout(()=>{a.style.opacity="0",a.style.transition="opacity 0.4s",setTimeout(()=>{a.remove(),r==null||r()},400)},1800)}function Sr(t,e,i){const n=document.getElementById("sub-anim-overlay");n&&n.remove();const r=document.createElement("div");r.id="sub-anim-overlay",r.style.cssText=`
    position:fixed;inset:0;z-index:3000;
    display:flex;flex-direction:column;align-items:center;justify-content:center;
    ...
    pointer-events:none;white-space:nowrap;max-width:90vw;text-align:center`,r.textContent=t,!document.getElementById("game-toast-style")){const o=document.createElement("style");o.id="game-toast-style",o.textContent=`
      ...
      }`,document.head.appendChild(o)}document.body.appendChild(r),setTimeout(()=>{r.style.opacity="0",r.style.transition="opacity 0.3s",setTimeout(()=>r.remove(),300)},i)}function to(t,e){const i=Pe(t,"MIL"),n=t.stadiumBonus||e&&(e.club_id&&String(t.club_id)===String(e.club_id)||e.country_code&&t.country_code===e.country_code)?10:0;return i+n}function bt(t,e){return t.reduce((i,n)=>i+to(n,e),0)}function yt(t){let e=0;for(let i=0;i<t.length-1;i++){const n=Ut(t[i],t[i+1]);n==="#00ff88"?e+=10:n==="#FFD700"&&(e+=5)}return e}function bi(t,e,i,n,r){return`<div id="duel-row-${n}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${t.map((o,a)=>{const s=a<t.length-1?Ut(o,t[a+1]):null,l=!s||s==="#ff3333"||s==="#cc2222",d=s==="#00ff88"?"+10":s==="#FFD700"?"+5":"";return to(o,r),o.stadiumBonus||r&&(r.club_id&&String(o.club_id)===String(r.club_id)||r.country_code&&(o.country_code,r.country_code)),`
          ...
          ${a<t.length-1?`<div class="duel-link duel-link-${n}" data-idx="${a}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${l?"rgba(255,255,255,0.12)":s};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${l?"none":`0 0 8px ${s}`}">
    ...
    </div>`}async function zr(t,e){const{state:i}=e,n=i.params||{},r=n.matchMode||"vs_ai_easy",o=r==="solo",a=n.soloLevel||1,s=r.replace("vs_ai_",""),l=r;let d=null;if(o){const{data:c}=await v.from("solo_levels").select("*").eq("level_number",a).maybeSingle();d=c||{level_number:a,target_note_avg:10,nb_liens_jaune:2,nb_liens_vert:1,nb_joueurs_stade:2,reward_credits:500}}await ki(t,e,r,async({deckId:c,formation:u,starters:b,subsRaw:x,gcCardsEnriched:g,gcDefs:p,stadiumDef:f})=>{try{let y=xi(b,u);f&&(y=ri(y,f),gi(x,f));const w=o?await Mr(u,d):await Ar(u,s),h=w.lines||w,L=async m=>{try{const A=l==="vs_ai_club"||l==="solo"?"club":l,{data:E,error:S}=await v.from("matches").insert({home_id:i.profile.id,away_id:null,mode:A,home_deck_id:c,status:"in_progress"}).select().single();if(S){console.error("[MatchIA] Erreur création match:",S),St(t,"⚠️","Impossible de créer le match ("+S.message+").","Retour",()=>e.navigate("home"));return}const q=w.stadiumDef||null;q&&h&&(ri(h,q),gi(w.subs||[],q));const W={gcDefs:p||[],matchId:E==null?void 0:E.id,mode:l,difficulty:s,formation:u,isSolo:o,soloLevel:a,soloLevelConfig:d,homeTeam:y,aiTeam:h,homeSubs:x,subsUsed:0,maxSubs:Math.min(x.length,3),aiSubs:w.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((w.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:w.gcCards||[],aiUsedGc:[],aiStadiumDef:q,homeScore:0,aiScore:0,gcCards:m,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Cr(t,W,e)}catch(A){console.error("[MatchIA] Exception launchMatch:",A),St(t,"⚠️","Erreur au lancement du match : "+A.message,"Retour",()=>e.navigate("home"))}};if(!g.length){L([]);return}wi(t,g,L)}catch(y){console.error("[MatchIA] Exception setup:",y),St(t,"⚠️","Erreur de préparation du match : "+y.message,"Retour",()=>e.navigate("home"))}})}async function Ar(t,e){var f;const{data:i}=await v.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:io(t),subs:[],gcCards:[],stadiumDef:null};const n=_t[t]||_t["4-4-2"],r={GK:[],DEF:[],MIL:[],ATT:[]},o=new Set;function a(y,w,h){var L,m;return o.add(y.id),{cardId:"ai-"+y.id+"-"+h,id:y.id,firstname:y.firstname,name:y.surname_real,country_code:y.country_code,club_id:y.club_id,job:y.job,job2:y.job2,note_g:Number(y.note_g)||0,note_d:Number(y.note_d)||0,note_m:Number(y.note_m)||0,note_a:Number(y.note_a)||0,rarity:y.rarity,skin:y.skin,hair:y.hair,hair_length:y.hair_length,clubName:((L=y.clubs)==null?void 0:L.encoded_name)||null,clubLogo:((m=y.clubs)==null?void 0:m.logo_url)||null,boost:0,used:!1,_line:w}}for(const y of["GK","DEF","MIL","ATT"]){const w=i.filter(E=>E.job===y&&!o.has(E.id)),h=i.filter(E=>E.job!==y&&!o.has(E.id)),L=[...w,...h],m=[];for(let E=0;E<n[y];E++){const S=L[E];S&&m.push(a(S,y,E))}const A=st(m.length);m.forEach((E,S)=>{E._col=A[S]}),r[y]=m}const l=i.filter(y=>!o.has(y.id)).slice(0,5).map((y,w)=>a(y,y.job,100+w)),u=Object.keys(ze).sort(()=>Math.random()-.5).slice(0,3).map((y,w)=>{var h,L;return{id:"ai-gc-"+w,gc_type:y,name:((h=ze[y])==null?void 0:h.name)||y,icon:((L=ze[y])==null?void 0:L.icon)||"⚡"}}),b=Object.values(r).flat(),x={};b.forEach(y=>{y.club_id&&(x[y.club_id]=(x[y.club_id]||0)+1)});const g=(f=Object.entries(x).sort((y,w)=>w[1]-y[1])[0])==null?void 0:f[0];let p=null;if(g){const{data:y}=await v.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",g).single();y&&(p={club_id:y.id,country_code:null,name:y.encoded_name+" Stadium",club:{encoded_name:y.encoded_name,logo_url:y.logo_url}})}return{lines:r,subs:l,gcCards:u,stadiumDef:p}}function kn(t){const e=t.job||"ATT";return Number(e==="GK"?t.note_g:e==="DEF"?t.note_d:e==="MIL"?t.note_m:t.note_a)||0}function ci(t,e,i){if(i<=0||!t.length)return[];const n=[...t].sort((r,o)=>Math.abs(kn(r)-e)-Math.abs(kn(o)-e));return n.slice(0,Math.min(i,n.length))}async function Mr(t,e){var G,I;const i=Number(e==null?void 0:e.target_note_avg)||10,n=Number(e==null?void 0:e.nb_liens_jaune)||0,r=Number(e==null?void 0:e.nb_liens_vert)||0,o=Number(e==null?void 0:e.nb_joueurs_stade)||0,{data:a}=await v.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url,country_code)").eq("is_active",!0).limit(300);if(!a||a.length<16)return{lines:io(t),subs:[],gcCards:[],stadiumDef:null};const s={};a.forEach(_=>{_.club_id&&(s[_.club_id]=(s[_.club_id]||0)+1)});const l=((G=Object.entries(s).sort((_,H)=>H[1]-_[1])[0])==null?void 0:G[0])||null,d=l?a.filter(_=>_.club_id===l):[],c=((I=d[0])==null?void 0:I.country_code)||null,u=c?a.filter(_=>_.country_code===c&&_.club_id!==l):[],b=Math.min(d.length,Math.max(2,r+1),16),x=Math.min(u.length,Math.max(0,n),16-b),g=[],p=new Set;ci(d,i,b).forEach(_=>{g.push(_),p.add(_.id)}),ci(u,i,x).forEach(_=>{g.push(_),p.add(_.id)});const f=Math.max(0,16-g.length),y=a.filter(_=>!p.has(_.id));ci(y,i,f).forEach(_=>{g.push(_),p.add(_.id)});const w=_t[t]||_t["4-4-2"],h={GK:[],DEF:[],MIL:[],ATT:[]},L=new Set;function m(_,H,Y){var ae,de;return L.add(_.id),{cardId:"ai-"+_.id+"-"+Y,id:_.id,firstname:_.firstname,name:_.surname_real,country_code:_.country_code,club_id:_.club_id,job:_.job,job2:_.job2,note_g:Number(_.note_g)||0,note_d:Number(_.note_d)||0,note_m:Number(_.note_m)||0,note_a:Number(_.note_a)||0,rarity:_.rarity,skin:_.skin,hair:_.hair,hair_length:_.hair_length,clubName:((ae=_.clubs)==null?void 0:ae.encoded_name)||null,clubLogo:((de=_.clubs)==null?void 0:de.logo_url)||null,boost:0,used:!1,_line:H}}for(const _ of["GK","DEF","MIL","ATT"]){const H=g.filter(U=>U.job===_&&!L.has(U.id)),Y=g.filter(U=>U.job!==_&&!L.has(U.id)),ae=[...H,...Y],de=[];for(let U=0;U<w[_];U++){const le=ae[U];le&&de.push(m(le,_,U))}const j=st(de.length);de.forEach((U,le)=>{U._col=j[le]}),h[_]=de}let A=g.filter(_=>!L.has(_.id));if(A.length<5){const _=a.filter(H=>!L.has(H.id)&&!A.some(Y=>Y.id===H.id));A=A.concat(ci(_,i,5-A.length))}const E=A.slice(0,5).map((_,H)=>m(_,_.job,100+H)),W=Object.keys(ze).sort(()=>Math.random()-.5).slice(0,3).map((_,H)=>{var Y,ae;return{id:"ai-gc-"+H,gc_type:_,name:((Y=ze[_])==null?void 0:Y.name)||_,icon:((ae=ze[_])==null?void 0:ae.icon)||"⚡"}});let ee=null;if(o>0&&l){const{data:_}=await v.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",l).single();_&&(ee={club_id:_.id,country_code:null,name:_.encoded_name+" Stadium",club:{encoded_name:_.encoded_name,logo_url:_.logo_url}})}return{lines:h,subs:E,gcCards:W,stadiumDef:ee}}function io(t){const e=_t[t]||_t["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let r=0;for(const o of["GK","DEF","MIL","ATT"]){const a=[];for(let l=0;l<e[o];l++){const d=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+r,id:"fake-"+r,firstname:"IA",name:n[r%n.length],country_code:"XX",club_id:null,job:o,job2:null,note_g:o==="GK"?d:2,note_d:o==="DEF"?d:2,note_m:o==="MIL"?d:2,note_a:o==="ATT"?d:2,rarity:"normal",boost:0,used:!1,_line:o}),r++}const s=st(a.length);a.forEach((l,d)=>{l._col=s[d]}),i[o]=a}return i}function Cr(t,e,i){var r;Cn("/sounds/match-opening.mp3",.3);const n=`<div style="position:relative;width:22px;height:22px;flex-shrink:0;display:flex;align-items:center;justify-content:center">
  ...
  </div>`,requestAnimationFrame(()=>requestAnimationFrame(()=>{const o=t.querySelector(".opponent-preview-wrap"),a=t.querySelector("#opponent-swipe-zone");if(!o||!a)return;const s=a.clientWidth>=900,l=Math.max(200,a.clientHeight-(s?8:40)),d=Math.max(200,a.clientWidth-(s?8:16)),c=s?null:Math.round(Math.max(44,Math.round(d*.168))*.55);o.innerHTML=Ot(e.aiTeam,e.formation,null,[],d,l,[],c),o.style.cssText=`width:${d}px;height:${l}px;overflow:hidden;flex-shrink:0`;const u=o.querySelector("svg");u&&(u.style.cssText="display:block;width:100%;height:100%",u.setAttribute("preserveAspectRatio",s?"xMidYMid meet":"none"))})),setTimeout(()=>jr(t,e,i),5e3)}const Ke=t=>Zn(t);function jr(t,e,i){const n=e.homeTeam.MIL||[],r=e.aiTeam.MIL||[],o=e.stadiumDef||null,a=e.aiStadiumDef||null,s=bt(n,o)+yt(n),l=bt(r,a)+yt(r),d=s>=l;t.innerHTML=`
  ...
  </div>`;const c=()=>{const g=(p,f)=>t.querySelectorAll(p).forEach((y,w)=>{setTimeout(()=>{y.style.opacity="1",y.style.transform="translateY(0) scale(1)"},f+w*90)});g(".duel-card-home",150),g(".duel-card-ai",500),setTimeout(()=>t.querySelectorAll(".duel-link").forEach((p,f)=>{setTimeout(()=>{p.style.opacity="1"},f*70)}),900),setTimeout(()=>{const p=t.querySelector("#vs-label");p&&(p.style.opacity="1",p.style.animation="vsFlash .5s ease"),t.querySelectorAll(".duel-score-line").forEach(f=>f.style.opacity="1")},1250),setTimeout(()=>{u("score-home",s,800),u("score-ai",l,800)},1500)};function u(g,p,f){const y=document.getElementById(g);if(!y)return;const w=performance.now(),h=L=>{const m=Math.min(1,(L-w)/f);y.textContent=Math.round(p*(1-Math.pow(1-m,3))),m<1?requestAnimationFrame(h):y.textContent=p};requestAnimationFrame(h)}requestAnimationFrame(c),e.attacker=d?"home":"ai";const b=d?Qn():null;d&&(e.boostCard={value:b}),e.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:n.map(g=>Ke(g)),aiPlayers:r.map(g=>Ke(g)),homeTotal:s,aiTotal:l,text:`Duel milieu : ${e.clubName} ${s} – ${l} IA → ${d?e.clubName+" attaque":"IA attaque"}`});const x=()=>{e.phase=e.attacker==="home"?"attack":"ai-attack",ke(t,e,i),e.attacker==="ai"&&setTimeout(()=>en(t,e,i),800)};setTimeout(()=>{const g=document.getElementById("score-home"),p=document.getElementById("score-ai"),f=document.getElementById(d?"duel-row-home":"duel-row-ai"),y=document.getElementById(d?"duel-row-ai":"duel-row-home"),w=d?g:p,h=d?p:g;w&&(w.style.fontSize="80px",w.style.color=d?"#FFD700":"#ff6b6b",w.style.animation="duelPulse .5s ease"+(d?", duelGlow 1.5s ease infinite .5s":"")),h&&(h.style.opacity="0.25"),setTimeout(()=>{f&&(f.style.transformOrigin="center",f.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",f.style.zIndex="5"),setTimeout(()=>{var m;const L=document.getElementById("duel-shock");if(L){const A=(m=y||f)==null?void 0:m.getBoundingClientRect(),E=t.querySelector(".match-screen").getBoundingClientRect();A&&(L.style.top=A.top-E.top+A.height/2+"px"),L.style.animation="shockwave .5s ease-out forwards"}y&&(y.style.transformOrigin="center",y.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var m;const L=document.getElementById("duel-finale");L&&(L.innerHTML=`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(e.phase==="defense"&&e.pendingAttack){const U=e.pendingAttack;let le="";if(e.selected.length>0){const fe=e.selected.map($=>({...(e.homeTeam[$._role]||[]).find(O=>O.cardId===$.cardId)||$,_line:$._role})),ue=oi(fe,e.modifiers.home,e.formation);le=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
          ...
          </div>`}const j=e.log[e.log.length-1];return j?'<div style="padding:2px 4px">'+eo(j)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    ${(()=>{const j=window.innerWidth>=700,U=(D,K,pe)=>{var Ae,Be;const ne=(e.gcDefs||[]).find($e=>$e.name===D.gc_type),me=(ne==null?void 0:ne.name)||D.gc_type,we=(ne==null?void 0:ne.effect)||((Ae=ze[D.gc_type])==null?void 0:Ae.desc)||"",he=ne!=null&&ne.image_url?`/icons/${ne.image_url}`:null,Te=((Be=ze[D.gc_type])==null?void 0:Be.icon)||"⚡";return`<div class="gc-mini" data-gc-id="${D.id}" data-gc-type="${D.gc_type}" style="flex-shrink:0;cursor:pointer">
  ...
  <div id="match-history-panel">
  ...
  </div>`;function S(){const j=t.querySelector(".match-screen");if(!j)return;const U=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);j.style.bottom="auto",j.style.height=U+"px",j.style.minHeight=U+"px",j.style.maxHeight=U+"px",j.style.overflow="hidden";const le=t.querySelector("#mobile-action-bar"),fe=t.querySelector("#mobile-play-area");le&&fe&&(fe.style.paddingBottom=le.offsetHeight+"px")}if(S(),setTimeout(S,120),setTimeout(S,400),setTimeout(S,1e3),e._vvBound||(e._vvBound=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",S),window.visualViewport.addEventListener("scroll",S)),window.addEventListener("resize",S)),function(){const U=t.querySelector("#match-field .terrain-wrapper svg")||t.querySelector(".terrain-wrapper svg");if(!U)return;const le=U.closest("#match-terrain-wrap");le&&(le.style.cssText="position:relative;width:100%;height:100%;padding:0"),U.removeAttribute("width"),U.removeAttribute("height"),U.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",U.setAttribute("preserveAspectRatio","xMidYMid meet")}(),e._resizeBound||(e._resizeBound=!0,window.addEventListener("resize",()=>{const j=t.querySelector(".terrain-wrapper svg");if(j){const U=j.closest("#match-terrain-wrap");U&&(U.style.cssText="position:relative;width:100%;height:100%;padding:0"),j.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0"}})),e._timerInt&&(clearInterval(e._timerInt),e._timerInt=null),Ue(),e.phase==="attack"||e.phase==="defense"){let j=!1,U=30;const le=()=>document.getElementById("match-timer"),fe=()=>{const ue=le();if(!ue)return;const $=String(Math.floor(U/60)).padStart(2,"0"),C=String(U%60).padStart(2,"0");ue.textContent=` ${$}:${C}`,ue.style.color=j?"#ff2222":"#ff9500",ue.style.fontWeight="900"};fe(),e._timerInt=setInterval(()=>{if(U--,U<0)if(!j)j=!0,U=15,fe(),jn("/sounds/timer-urgent.mp3",.6);else{clearInterval(e._timerInt),e._timerInt=null,Ue(),e.homeScore=0,e.aiScore=3;const ue=document.createElement("div");ue.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",ue.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(ue),setTimeout(()=>{ue.remove(),ht(t,e,i)},2500)}else fe()},1e3)}(W=document.getElementById("match-quit"))==null||W.addEventListener("click",()=>{it(t),confirm("Abandonner ? Résultat : défaite 3-0")&&(e.homeScore=0,e.aiScore=3,ht(t,e,i))}),(ee=document.getElementById("view-ai"))==null||ee.addEventListener("click",()=>Ur(e,i)),(G=document.getElementById("toggle-history"))==null||G.addEventListener("click",()=>{var j;(j=document.getElementById("match-history-panel"))==null||j.classList.add("open")}),(I=document.getElementById("close-history"))==null||I.addEventListener("click",()=>{var j;(j=document.getElementById("match-history-panel"))==null||j.classList.remove("open")}),(_=document.getElementById("btn-action"))==null||_.addEventListener("click",()=>{e.selected.length!==0&&(f?qr(t,e,i):y&&Dr(t,e,i))}),(H=document.getElementById("btn-results"))==null||H.addEventListener("click",()=>ht(t,e,i)),(Y=document.getElementById("btn-pass"))==null||Y.addEventListener("click",()=>{if(e.log.push({text:"⏭️ Vous passez votre tour (plus d'attaquants)",type:"info"}),Fi(e.aiTeam,e.homeTeam)){e.log.push({text:"🏁 Plus personne ne peut attaquer — match terminé.",type:"info"}),ht(t,e,i);return}e.phase="ai-attack",ke(t,e,i),setTimeout(()=>en(t,e,i),800)}),t.querySelectorAll(".match-slot-hit").forEach(j=>{j.addEventListener("click",()=>Br(j,e,t,i))}),t.querySelectorAll(".match-used-hit").forEach(j=>{j.addEventListener("click",()=>zi(t,e,i,null,j.dataset.cardId))}),t.querySelectorAll(".gc-mini").forEach(j=>{f?j.addEventListener("click",()=>Gr(j.dataset.gcId,j.dataset.gcType,t,e,i)):(j.style.opacity="0.35",j.style.cursor="default",j.addEventListener("click",()=>Et("⚡ Les Game Changers ne sont utilisables qu'en attaque","rgba(180,100,0,0.9)")))}),(ae=document.getElementById("boost-card"))==null||ae.addEventListener("click",()=>Hr(t,e,i)),t.querySelectorAll(".sub-btn-col").forEach(j=>{j.addEventListener("click",()=>zi(t,e,i,j.dataset.subId))}),(de=document.getElementById("sub-btn-main"))==null||de.addEventListener("click",()=>zi(t,e,i))}function Br(t,e,i,n){const r=t.dataset.cardId,o=t.dataset.role,a=e.selected.findIndex(s=>s.cardId===r);if(a!==-1)e.selected.splice(a,1);else{if(e.selected.length>=3){n.toast("Maximum 3 joueurs","error");return}const s=(e.homeTeam[o]||[]).find(l=>l.cardId===r);s&&e.selected.push({...s,_role:o,_line:o})}ke(i,e,n)}function Zi(t,e,i){t.matchId&&v.from("matches").update({last_player_id:i}).eq("id",t.matchId).then(()=>{})}function qr(t,e,i){e._timerInt&&(clearInterval(e._timerInt),e._timerInt=null),Ue(),Zi(e,i,i.state.profile.id);const n=e.selected.map(o=>{const a=(e.homeTeam[o._role]||[]).find(l=>l.cardId===o.cardId)||o,s=["GK","DEF"].includes(o._role);return{...a,_line:o._role,...s?{note_a:Math.max(1,Number(a.note_a)||0)}:{}}}),r=ni(n,e.modifiers.home,e.formation);e.pendingAttack={...r,players:[...n],side:"home"},e.selected.forEach(o=>{const a=(e.homeTeam[o._role]||[]).find(s=>s.cardId===o.cardId);a&&(a.used=!0)}),e.log.push({text:`⚔️ Vous attaquez : ${r.total} (base ${r.base}${r.links?` +${r.links} liens`:""}) — ${e.selected.map(o=>o.name).join(", ")}`,type:"info"}),e.selected=[],e.modifiers.home={},e.phase="ai-defense",ke(t,e,i),setTimeout(()=>Nr(t,e,i),1200)}function Dr(t,e,i){e._timerInt&&(clearInterval(e._timerInt),e._timerInt=null),Ue(),Zi(e,i,i.state.profile.id);const n=e.stadiumDef||null,r=e.selected.map(l=>{const d=(e.homeTeam[l._role]||[]).find(u=>u.cardId===l.cardId)||l,c=d.stadiumBonus||n&&(n.club_id&&String(d.club_id)===String(n.club_id)||n.country_code&&d.country_code===n.country_code)||!1;return{...d,_line:l._role,stadiumBonus:c}}),o=oi(r,e.modifiers.home,e.formation);e.selected.forEach(l=>{const d=(e.homeTeam[l._role]||[]).find(c=>c.cardId===l.cardId);d&&(d.used=!0)});const a=Qi(e.pendingAttack.total,o.total,e.modifiers.home),s={type:"duel",title:"Défense",aiPlayers:(e.pendingAttack.players||[]).map(l=>Ke(l)),homePlayers:e.selected.map(l=>{const d=(e.homeTeam[l._role]||[]).find(c=>c.cardId===l.cardId)||l;return Ke(d)}),homeTotal:o.total,aiTotal:e.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(a.shielded)s.text="🛡️ Bouclier ! But annulé.",e.log.push(s);else if(a.goal){e.aiScore++,s.isGoal=!0,s.homeScored=!1,s.text=`⚽ BUT IA ! (${e.pendingAttack.total} > ${o.total})`,e.log.push(s),e.selected=[],e.modifiers.home={},e.pendingAttack=null,ke(t,e,i),Qe(s.aiPlayers,e.homeScore,e.aiScore,!1,()=>{At(t,e,i,"home-attack")});return}else s.text=`🧤 Défense réussie ! (${o.total} ≥ ${e.pendingAttack.total})`,e.log.push(s);e.selected=[],e.modifiers.home={},e.pendingAttack=null,At(t,e,i,"home-attack")}function Fr(t){if(t.aiSubsUsed>=t.aiMaxSubs)return;const e=Object.values(t.aiTeam).flat().filter(l=>l.used);if(!e.length)return;const i=(t.aiSubs||[]).filter(l=>!t.aiUsedSubIds.includes(l.cardId));if(!i.length)return;const n=e[Math.floor(Math.random()*e.length)],r=i.find(l=>l.job===n.job)||i[0],o={...r,used:!1,_line:n._line,_col:n._col},a=t.aiTeam[n._line],s=a.findIndex(l=>l.cardId===n.cardId);s!==-1&&(a[s]=o),t.aiUsedSubIds.push(r.cardId),t.aiSubsUsed++,t.log.push({text:`🔄 IA : ${r.firstname} ${r.name} remplace ${n.firstname} ${n.name}`,type:"info"})}function Pr(t){var n;if(!((n=t.aiGcCards)!=null&&n.length))return;const e=t.aiGcCards.filter(r=>!t.aiUsedGc.includes(r.id));if(!e.length||Math.random()>.3)return;const i=e[Math.floor(Math.random()*e.length)];switch(t.aiUsedGc.push(i.id),ze[i.gc_type],i.gc_type){case"Boost+2":{const r=Object.values(t.aiTeam).flat().filter(o=>!o.used);if(r.length){const o=r[Math.floor(Math.random()*r.length)];o.boost=(o.boost||0)+2}break}case"Boost+3":{const r=Object.values(t.aiTeam).flat().filter(o=>!o.used);if(r.length){const o=r[Math.floor(Math.random()*r.length)];o.boost=(o.boost||0)+3}break}case"Remplacement+":t.aiMaxSubs=(t.aiMaxSubs||3)+1;break;case"Bouclier":t.modifiers.ai.shield=!0;break;case"Nul+1":t.modifiers.ai.drawBonus=(t.modifiers.ai.drawBonus||0)+1;break}t.log.push({text:`⚡ IA joue ${i.icon||"⚡"} ${i.name}`,type:"gc"})}function en(t,e,i){Zi(e,i,null),Fr(e),Pr(e);let n=[...e.aiTeam.MIL||[],...e.aiTeam.ATT||[]].filter(c=>!c.used),r=!1;if(!n.length){const c=[...e.homeTeam.MIL||[],...e.homeTeam.ATT||[]].filter(b=>!b.used),u=Ht(e.homeTeam);if(u||c.length===0){const b=(e.aiTeam.DEF||[]).filter(g=>!g.used),x=(e.aiTeam.GK||[]).filter(g=>!g.used);n=u?b.length?b:x:[...b,...x],r=!0}}const o=Hn(n,"attack",e.difficulty);if(!o.length){Pi(t,e,i);return}r&&o.forEach(c=>{c._line=c._line||c.job,c.note_a=Math.max(1,Number(c.note_a)||0)});const a=ni(o,e.modifiers.ai,e.formation);e.pendingAttack={...a,players:o,side:"ai"},o.forEach(c=>{c.used=!0}),e.log.push({text:`🤖 IA attaque : ${a.total} (${o.map(c=>c.name).join(", ")})`,type:"info"}),e.modifiers.ai={};const s=[...e.homeTeam.GK||[],...e.homeTeam.DEF||[],...e.homeTeam.MIL||[]].filter(c=>!c.used),d=(e.homeSubs||[]).filter(c=>!(e.usedSubIds||[]).includes(c.cardId)).length>0&&e.subsUsed<e.maxSubs;if(s.length===0&&!d){if(o.length===1&&(o[0]._line==="GK"||o[0].job==="GK")&&Ht(e.homeTeam)&&e.homeScore===e.aiScore){e.aiScore++,yi(t,e,i,"ai",o[0]);return}e.aiScore++;const u={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:o.map(b=>Ke(b)),aiTotal:a.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};e.log.push(u),e.pendingAttack=null,ke(t,e,i),Qe(u.aiPlayers,e.homeScore,e.aiScore,!1,()=>{At(t,e,i,"home-attack")});return}e.phase="defense",ke(t,e,i)}function Nr(t,e,i){var d,c;const n=[...e.aiTeam.GK||[],...e.aiTeam.DEF||[],...e.aiTeam.MIL||[]],r=Hn(n,"defense",e.difficulty);if(!["GK","DEF","MIL","ATT"].flatMap(u=>(e.aiTeam[u]||[]).filter(b=>!b.used)).length){const u=((d=e.pendingAttack)==null?void 0:d.players)||[];if(u.length===1&&(u[0]._line==="GK"||u[0].job==="GK")&&e.homeScore===e.aiScore){e.homeScore++,yi(t,e,i,"home",u[0]);return}e.homeScore++;const x={type:"duel",isGoal:!0,homeScored:!0,homePlayers:u.map(g=>Ke(g)),homeTotal:((c=e.pendingAttack)==null?void 0:c.total)||0,aiTotal:0,text:"⚽ BUT ! L'IA n'a plus de joueurs — but automatique !"};e.log.push(x),e.modifiers.ai={},e.pendingAttack=null,ke(t,e,i),Qe(x.homePlayers,e.homeScore,e.aiScore,!0,()=>{At(t,e,i,"ai-attack")});return}const a=r.length>0?oi(r,e.modifiers.ai,e.formation).total:0;r.forEach(u=>{u.used=!0});const s=Qi(e.pendingAttack.total,a,e.modifiers.ai),l={type:"duel",title:"Attaque",homePlayers:(e.pendingAttack.players||[]).map(u=>Ke(u)),aiPlayers:r.map(u=>Ke(u)),homeTotal:e.pendingAttack.total,aiTotal:a,isGoal:!1,homeScored:!1,text:""};if(s.shielded)l.text="🛡️ Bouclier IA !",e.log.push(l);else if(s.goal){e.homeScore++,l.isGoal=!0,l.homeScored=!0,l.text=`⚽ BUT ! (${e.pendingAttack.total} > ${a})`,e.log.push(l),e.modifiers.ai={},e.pendingAttack=null,ke(t,e,i),Qe(l.homePlayers,e.homeScore,e.aiScore,!0,()=>{At(t,e,i,"ai-attack")});return}else l.text=`🧤 IA défend (${a} ≥ ${e.pendingAttack.total})`,e.log.push(l);e.modifiers.ai={},e.pendingAttack=null,At(t,e,i,"ai-attack")}function At(t,e,i,n){if(e.round++,!no(t,e,i)){if(tn(e)){ht(t,e,i);return}if(n==="home-attack"){if(!["MIL","ATT","GK","DEF"].some(o=>(e.homeTeam[o]||[]).some(a=>!a.used))){Pi(t,e,i);return}e.phase="attack",ke(t,e,i)}else{if(!["MIL","ATT","GK","DEF"].some(o=>(e.aiTeam[o]||[]).some(a=>!a.used))){Pi(t,e,i);return}e.phase="ai-attack",ke(t,e,i),setTimeout(()=>en(t,e,i),800)}}}function Ht(t){return!["GK","DEF","MIL","ATT"].some(e=>(t[e]||[]).some(i=>!i.used))}function $n(t){const e=(t.GK||[]).some(n=>!n.used),i=["DEF","MIL","ATT"].some(n=>(t[n]||[]).some(r=>!r.used));return e&&!i}function no(t,e,i){if(e.homeScore!==e.aiScore)return!1;if($n(e.homeTeam)&&Ht(e.aiTeam)){const n=(e.homeTeam.GK||[]).find(r=>!r.used);return n?(n.used=!0,e.homeScore++,yi(t,e,i,"home",n),!0):!1}if($n(e.aiTeam)&&Ht(e.homeTeam)){const n=(e.aiTeam.GK||[]).find(r=>!r.used);return n?(n.used=!0,e.aiScore++,yi(t,e,i,"ai",n),!0):!1}return!1}function yi(t,e,i,n,r){e.log.push({type:"duel",isGoal:!0,homeScored:n==="home",homePlayers:n==="home"?[Ke(r)]:[],aiPlayers:n==="ai"?[Ke(r)]:[],text:`⚽ DERNIER CORNER — Le gardien ${n==="home"?"":"adverse "}marque !`});const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(o);const a=(s,l)=>new Promise(d=>{o.innerHTML=`<div style="font-size:32px;font-weight:900;color:${l};letter-spacing:2px;animation:lcFade 1.4s ease both">${s}</div>
    <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(d,1400)});(async()=>(await a("⚽ DERNIER CORNER","#FFD700"),await a("🧤 LE GARDIEN MONTE !","#4fc3f7"),o.remove(),e.pendingAttack=null,ke(t,e,i),Qe([Ke(r)],e.homeScore,e.aiScore,n==="home",()=>{if(tn(e)){ht(t,e,i);return}At(t,e,i,n==="home"?"ai-attack":"home-attack")})))()}function tn(t){const e=["MIL","ATT","GK","DEF"].some(n=>(t.homeTeam[n]||[]).some(r=>!r.used)),i=["MIL","ATT","GK","DEF"].some(n=>(t.aiTeam[n]||[]).some(r=>!r.used));return!e&&!i}function Fi(t,e){return!(["MIL","ATT"].some(r=>(t[r]||[]).some(o=>!o.used))||Ht(e)||!["MIL","ATT"].some(r=>(e[r]||[]).some(o=>!o.used)))}function Pi(t,e,i){if(!no(t,e,i)){if(tn(e)){ht(t,e,i);return}if(Fi(e.homeTeam,e.aiTeam)&&Fi(e.aiTeam,e.homeTeam)){e.log.push({text:"🏁 Plus personne ne peut attaquer — match terminé.",type:"info"}),ht(t,e,i);return}e.phase="attack",ke(t,e,i)}}function zi(t,e,i,n=null,r=null){var x,g;if(e.phase!=="attack"){Et("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(e.usedSubIds||(e.usedSubIds=[]),e.subsUsed>=e.maxSubs){Et(`Maximum ${e.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const o=Object.entries(e.homeTeam).flatMap(([p,f])=>(f||[]).filter(y=>y.used).map(y=>({...y,_line:y._line||p}))),a=e.homeSubs.filter(p=>!e.usedSubIds.includes(p.cardId));if(!o.length){Et("Aucun joueur utilisé à remplacer");return}if(!a.length){Et("Aucun remplaçant disponible");return}let s=Math.max(0,o.findIndex(p=>p.cardId===r));const l=((x=o[s])==null?void 0:x._line)||((g=o[s])==null?void 0:g.job);let d=n?Math.max(0,a.findIndex(p=>p.cardId===n)):Math.max(0,a.findIndex(p=>p.job===l)),c=!1;const u=document.createElement("div");u.id="sub-overlay",u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function b(){var m,A,E,S,q,W;const p=o[s],f=a[d],y=Math.min(130,Math.round((window.innerWidth-90)/2)),w=Math.round(y*1.35),h=ee=>`background:rgba(255,255,255,0.12);border:none;color:${ee?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${ee?"default":"pointer"};flex-shrink:0`;u.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <div>${f?vt({...f,used:!1,boost:0},y,w):"<div>—</div>"}</div>
    ...
    </div>`,(m=u.querySelector("#sub-close"))==null||m.addEventListener("click",()=>u.remove()),(A=u.querySelector("#out-up"))==null||A.addEventListener("click",()=>{s>0&&(s--,b())}),(E=u.querySelector("#out-down"))==null||E.addEventListener("click",()=>{s<o.length-1&&(s++,b())}),(S=u.querySelector("#in-up"))==null||S.addEventListener("click",()=>{d>0&&(d--,b())}),(q=u.querySelector("#in-down"))==null||q.addEventListener("click",()=>{d<a.length-1&&(d++,b())});const L=(ee,G,I,_)=>{const H=u.querySelector("#"+ee);if(!H)return;let Y=0;H.addEventListener("touchstart",ae=>{Y=ae.touches[0].clientY},{passive:!0}),H.addEventListener("touchend",ae=>{const de=ae.changedTouches[0].clientY-Y;if(Math.abs(de)<30)return;const j=G();de<0&&j<_-1?(I(j+1),b()):de>0&&j>0&&(I(j-1),b())},{passive:!0})};L("in-panel",()=>d,ee=>d=ee,a.length),L("out-panel",()=>s,ee=>s=ee,o.length),(W=u.querySelector("#sub-confirm"))==null||W.addEventListener("click",ee=>{if(ee.preventDefault(),ee.stopPropagation(),c)return;c=!0;const G=o[s],I=a[d];if(!G||!I)return;let _=null,H=-1;for(const[ae,de]of Object.entries(e.homeTeam)){const j=(de||[]).findIndex(U=>U.cardId===G.cardId);if(j!==-1){_=ae,H=j;break}}if(H===-1||!_){Et("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),u.remove();return}const Y={...I,_line:_,_col:G._col||0,used:!1,boost:0};e.homeTeam[_].splice(H,1,Y),e.usedSubIds||(e.usedSubIds=[]),e.usedSubIds.push(I.cardId),e.subsUsed++,e.selected=[],e.log.push({type:"sub",subSide:"home",clubName:e.clubName,outPlayer:{name:G.name,firstname:G.firstname,note:Pe(G,_),portrait:Mt(G),job:G.job,country_code:G.country_code,rarity:G.rarity,clubName:G.clubName,clubLogo:G.clubLogo},inPlayer:{name:I.name,firstname:I.firstname,note:Pe(I,_),portrait:Mt(I),job:I.job,country_code:I.country_code,rarity:I.rarity,clubName:I.clubName,clubLogo:I.clubLogo},text:`🔄 ${I.firstname} ${I.name} remplace ${G.firstname} ${G.name}`}),u.remove(),Sr(G,I,()=>ke(t,e,i))})}document.body.appendChild(u),b()}function Gr(t,e,i,n,r){var b,x;const o=(n.gcDefs||[]).find(g=>g.name===e),a=ze[e]||{icon:"⚡",desc:"Carte spéciale."},s=(o==null?void 0:o.name)||e,l=(o==null?void 0:o.effect)||a.desc,d=o!=null&&o.image_url?`/icons/${o.image_url}`:null,c=a.icon||"⚡",u=document.createElement("div");u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",u.innerHTML=`
    ${tt(s,d,c,l,{width:190})}
    <!-- Boutons -->
    <div style="display:flex;gap:12px;width:190px">
      <button id="gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
      <button id="gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
    </div>`,document.body.appendChild(u),(b=u.querySelector("#gc-back"))==null||b.addEventListener("click",()=>u.remove()),(x=u.querySelector("#gc-use"))==null||x.addEventListener("click",()=>{u.remove(),Or(t,e,i,n,r)})}function pi(t,e,i,n,r,o){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let s=[];function l(){var d,c;a.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start;justify-content:center">
      ${t.map(u=>{const b=u._line||u.job||"MIL",x=s.find(p=>p.cardId===u.cardId),g=Se({...u,_evolution_bonus:0},{width:90,showStad:!0,role:b,extraNote:u.boost||0});return`<div class="gc-pick-item" data-cid="${u.cardId}"
    ...
    </div>`,(d=a.querySelector("#gc-picker-close"))==null||d.addEventListener("click",()=>a.remove()),a.querySelectorAll(".gc-pick-item").forEach(u=>{u.addEventListener("click",()=>{const b=u.dataset.cid,x=t.find(p=>p.cardId===b);if(!x)return;const g=s.findIndex(p=>p.cardId===b);g>-1?s.splice(g,1):s.length<e&&s.push(x),l()})}),(c=a.querySelector("#gc-picker-confirm"))==null||c.addEventListener("click",()=>{a.remove(),o(s)})}l(),document.body.appendChild(a)}const Rr={BOOST_STAT:({value:t=1,count:e=1,roles:i=[]},n,r,o)=>{const a=Object.entries(n.homeTeam).filter(([s])=>!i.length||i.includes(s)).flatMap(([s,l])=>l.filter(d=>!d.used).map(d=>({...d,_line:s})));return a.length?(pi(a,e,`Choisir ${e} joueur(s) à booster (+${t})`,r,n,s=>{s.forEach(l=>{const d=(n.homeTeam[l._line]||[]).find(c=>c.cardId===l.cardId);d&&(d.boost=(d.boost||0)+t,n.log.push({text:`⚡ +${t} sur ${d.name}`,type:"info"}))}),ke(r,n,o)}),!0):(n.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),ke(r,n,o),!0)},DEBUFF_STAT:({value:t=1,count:e=1,roles:i=[],target:n="ai"},r,o,a)=>{const s=n==="home"?r.homeTeam:r.aiTeam,l=n==="ai"?"adverse":"allié",d=Object.entries(s).filter(([c])=>!i.length||i.includes(c)).flatMap(([c,u])=>u.filter(b=>!b.used).map(b=>({...b,_line:c})));return d.length?(pi(d,e,`Choisir ${e} joueur(s) ${l}(s) à débuffer (-${t})`,o,r,c=>{c.forEach(u=>{const x=((n==="home"?r.homeTeam:r.aiTeam)[u._line]||[]).find(g=>g.cardId===u.cardId);x&&(x.boost=(x.boost||0)-t,r.log.push({text:`🎯 -${t} sur ${x.name}${n==="ai"?" (IA)":""}`,type:"info"}))}),ke(o,r,a)}),!0):(r.log.push({text:`🎯 Aucun joueur ${l} disponible`,type:"info"}),ke(o,r,a),!0)},GRAY_PLAYER:({count:t=1,roles:e=[],target:i="ai"},n,r,o)=>{const a=i==="home"?n.homeTeam:n.aiTeam,s=i==="ai"?"adverse":"allié",l=Object.entries(a).filter(([d])=>!e.length||e.includes(d)).flatMap(([d,c])=>c.filter(u=>!u.used).map(u=>({...u,_line:d})));return l.length?(pi(l,t,`Choisir ${t} joueur(s) ${s}(s) à exclure`,r,n,d=>{d.forEach(c=>{const b=((i==="home"?n.homeTeam:n.aiTeam)[c._line]||[]).find(x=>x.cardId===c.cardId);b&&(b.used=!0,n.log.push({text:`❌ ${b.name}${i==="ai"?" (IA)":""} exclu !`,type:"info"}))}),ke(r,n,o)}),!0):(n.log.push({text:`❌ Aucun joueur ${s} à exclure`,type:"info"}),ke(r,n,o),!0)},REVIVE_PLAYER:({count:t=1,roles:e=[]},i,n,r)=>{const o=Object.entries(i.homeTeam).filter(([a])=>!e.length||e.includes(a)).flatMap(([a,s])=>s.filter(l=>l.used).map(l=>({...l,_line:a})));return o.length?(pi(o,t,`Choisir ${t} joueur(s) à ressusciter`,n,i,a=>{a.forEach(s=>{const l=(i.homeTeam[s._line]||[]).find(d=>d.cardId===s.cardId);l&&(l.used=!1,i.log.push({text:`💫 ${l.name} ressuscité !`,type:"info"}))}),ke(n,i,r)}),!0):(i.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),ke(n,i,r),!0)},REMOVE_GOAL:({},t)=>(t.aiScore>0?(t.aiScore--,t.log.push({text:"🚫 Dernier but IA annulé !",type:"info"})):t.log.push({text:"🚫 Aucun but à annuler",type:"info"}),!1),ADD_GOAL_DRAW:({},t)=>(t.homeScore===t.aiScore?(t.homeScore++,t.log.push({text:"🎯 But bonus (match nul) !",type:"info"})):t.log.push({text:"🎯 But bonus : non applicable (pas de match nul)",type:"info"}),!1),ADD_SUB:({value:t=1},e)=>(e.maxSubs=(e.maxSubs||3)+t,e.log.push({text:`🔄 +${t} remplacement(s) débloqué(s)`,type:"info"}),!1),CUSTOM:()=>!1};function Or(t,e,i,n,r){n.usedGc.push(t);const o=n.gcDefs||[],a=o.find(l=>l.name===e)||o.find(l=>{var d;return((d=l.name)==null?void 0:d.toLowerCase().trim())===(e==null?void 0:e.toLowerCase().trim())});let s=!1;if(a!=null&&a.effect_type&&a.effect_type!=="CUSTOM"){const l=Rr[a.effect_type];l?l(a.effect_params||{},n,i,r)||(s=!0):(r.toast(`Effet "${a.effect_type}" non implémenté`,"error"),s=!0)}else{switch(e){case"Double attaque":n.modifiers.home.doubleAttack=!0,n.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":n.modifiers.home.shield=!0,n.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const l=Object.entries(n.homeTeam).flatMap(([d,c])=>(c||[]).filter(u=>u.used).map(u=>({...u,_line:d})));l.length?(l[0].used=!1,n.log.push({text:`💫 ${l[0].name} ressuscité !`,type:"info"})):n.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":n.modifiers.ai.stolenNote=(n.modifiers.ai.stolenNote||0)+1,n.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const l=[...n.aiTeam.ATT||[],...n.aiTeam.MIL||[]].filter(d=>!d.used);if(l.length){const d=l.sort((c,u)=>Pe(u,"ATT")-Pe(c,"ATT"))[0];d.used=!0,n.log.push({text:`❄️ ${d.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":n.maxSubs++,n.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}s=!0}v.from("cards").delete().eq("id",t).then(()=>{}),s&&ke(i,n,r)}function Hr(t,e,i){const n=Object.values(e.homeTeam).flat().filter(r=>!r.used);if(!n.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ${n.map(r=>`
        <div class="player-boost-opt" data-card-id="${r.cardId}"
          ...
          <div style="width:32px;height:32px;background:${Er[r.job]||"#888"};border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:13px">${Pe(r,r._line||r.job)}</div>
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(r=>{r.addEventListener("click",()=>{const o=r.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const s=(e.homeTeam[a]||[]).find(l=>l.cardId===o);if(s){s.boost=(s.boost||0)+e.boostCard.value,e.log.push({text:`⚡ Boost +${e.boostCard.value} appliqué à ${s.name}`,type:"info"});break}}e.boostUsed=!0,i.closeModal(),ke(t,e,i)})})}async function ht(t,e,i){var c,u,b;It(),Ue(),e._timerInt&&(clearInterval(e._timerInt),e._timerInt=null),e.phase="finished";const{state:n}=i,r=e.homeScore>e.aiScore,o=e.homeScore===e.aiScore,a=r?"victoire":o?"nul":"defaite",s=e.isSolo&&((c=e.soloLevelConfig)!=null&&c.reward_credits)&&r?Number(e.soloLevelConfig.reward_credits):Xo(e.mode,a);if(e.isSolo&&r)try{const{data:x}=await v.from("user_solo_progress").select("unlocked_level").eq("user_id",n.profile.id).maybeSingle(),g=(x==null?void 0:x.unlocked_level)||1;e.soloLevel>=g&&await v.from("user_solo_progress").upsert({user_id:n.profile.id,unlocked_level:e.soloLevel+1,updated_at:new Date().toISOString()})}catch(x){console.warn("[Solo] Erreur mise à jour progression:",x.message)}e.matchId&&await v.from("matches").update({status:"finished",home_score:e.homeScore,away_score:e.aiScore,winner_id:r?n.profile.id:null,home_credits_reward:s,played_at:new Date().toISOString()}).eq("id",e.matchId);const l={credits:(n.profile.credits||0)+s,matches_played:(n.profile.matches_played||0)+1};r?l.wins=(n.profile.wins||0)+1:o?l.draws=(n.profile.draws||0)+1:l.losses=(n.profile.losses||0)+1,await v.from("users").update(l).eq("id",n.profile.id),await i.refreshProfile();const d=document.createElement("div");d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",d.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
    ...
    </div>`,document.body.appendChild(d),(u=document.getElementById("res-home"))==null||u.addEventListener("click",()=>{d.remove(),it(t),i.navigate("home")}),(b=document.getElementById("res-replay"))==null||b.addEventListener("click",()=>{d.remove(),it(t),i.navigate("match",e.isSolo?{matchMode:e.mode,soloLevel:e.soloLevel}:{matchMode:e.mode})})}function Ur(t,e){e.openModal("Équipe adverse (IA)",`<div style="background:#0a3d1e;padding:12px;border-radius:8px">
      ${_i(t.aiTeam,t.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Kr={pepite:{win:2,loss:-1,boosterStart:"min"},papyte:{win:1,loss:-2,boosterStart:"max"}};function Vr(t){const e=t.job||"ATT";return Number(e==="GK"?t.note_g:e==="DEF"?t.note_d:e==="MIL"?t.note_m:t.note_a)||0}async function Yr(t,e){!t||!e||await Promise.all([En(t,"win"),En(e,"loss")])}async function En(t,e){const{data:i}=await v.from("cards").select("id, current_note, player:players(rarity, job, note_g, note_d, note_m, note_a, note_min, note_max)").eq("owner_id",t).eq("card_type","player");if(!(i!=null&&i.length))return;const n=i.filter(r=>{var o,a;return((o=r.player)==null?void 0:o.rarity)==="pepite"||((a=r.player)==null?void 0:a.rarity)==="papyte"});n.length&&await Promise.all(n.map(r=>{const o=Kr[r.player.rarity],a=e==="win"?o.win:o.loss,s=r.player.note_min??1,l=r.player.note_max??10,d=Vr(r.player),c=r.current_note??d,u=Math.min(l,Math.max(s,c+a));return v.from("cards").update({current_note:u}).eq("id",r.id)}))}const Ft=t=>Zn(t);async function jt(t,e,i,n,r={}){return oo(t,e,i,n,r.myGC||[],r.gcDefs||[],r.isRanked||!1,r.rankedData||null,r.stadiumDef||null,r.onMatchEnd||null,r.mlLeagueId||null,r.mlMatchId||null)}async function Wr(t,e,i){const{data:n}=await v.from("matches").select("home_id,away_id,mode,is_ranked").eq("id",i).single();if(!n){e.toast("Match introuvable","error"),e.navigate("home");return}const r=n.home_id===e.state.user.id;let o=null,a=null;if(n.mode==="mini_league"){const{data:s}=await v.from("mini_league_matches").select("id, league_id").eq("match_id",i).maybeSingle();s&&(o=s.league_id,a=s.id)}return oo(t,e,i,r,[],[],n.is_ranked||!1,null,null,null,o,a)}async function oo(t,e,i,n,r=[],o=[],a=!1,s=null,l=null,d=null,c=null,u=null){const{state:b,navigate:x,toast:g}=e,p=n?"p1":"p2",f=n?"p2":"p1",y=(r||[]).map(k=>k.id),w=(r||[]).map(k=>({id:k.id,gc_type:k.gc_type,_gcDef:k._gcDef||null}));t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:h}=await v.from("matches").select("*").eq("id",i).single();if(!h){g("Match introuvable","error"),x("home");return}async function L(){var be,ve;console.log("[PvP] buildGameState match:",{id:h.id,home_deck_id:h.home_deck_id,away_deck_id:h.away_deck_id,mode:h.mode,is_ranked:h.is_ranked});const[{data:k,error:T},{data:M,error:B},{data:z},{data:R}]=await Promise.all([v.rpc("get_deck_for_match",{p_deck_id:h.home_deck_id}),v.rpc("get_deck_for_match",{p_deck_id:h.away_deck_id}),v.from("users").select("id,pseudo,club_name").eq("id",h.home_id).single(),v.from("users").select("id,pseudo,club_name").eq("id",h.away_id).single()]);console.log("[PvP] get_deck_for_match p1:",T==null?void 0:T.message,"p2:",B==null?void 0:B.message,"p1D:",(be=k==null?void 0:k.starters)==null?void 0:be.length,"p2D:",(ve=M==null?void 0:M.starters)==null?void 0:ve.length);const F=oe=>{const Le=Number(oe.evolution_bonus)||0;return{cardId:oe.card_id,position:oe.position,id:oe.id,firstname:oe.firstname,name:oe.surname_real,country_code:oe.country_code,club_id:oe.club_id,job:oe.job,job2:oe.job2,note_g:(Number(oe.note_g)||0)+(oe.job==="GK"||oe.job2==="GK"&&Number(oe.note_g)>0?Le:0),note_d:(Number(oe.note_d)||0)+(oe.job==="DEF"||oe.job2==="DEF"&&Number(oe.note_d)>0?Le:0),note_m:(Number(oe.note_m)||0)+(oe.job==="MIL"||oe.job2==="MIL"&&Number(oe.note_m)>0?Le:0),note_a:(Number(oe.note_a)||0)+(oe.job==="ATT"||oe.job2==="ATT"&&Number(oe.note_a)>0?Le:0),evolution_bonus:Le,rarity:oe.rarity,skin:oe.skin,hair:oe.hair,hair_length:oe.hair_length,face:oe.face||null,clubName:oe.club_encoded_name||null,clubLogo:oe.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},N=((k==null?void 0:k.starters)||[]).map(oe=>F(oe)),V=((M==null?void 0:M.starters)||[]).map(oe=>F(oe)),Q=(k==null?void 0:k.formation)||"4-4-2",ie=(M==null?void 0:M.formation)||"4-4-2";let te=xi(N,Q),Z=xi(V,ie);const re=((k==null?void 0:k.subs)||[]).map(oe=>F(oe)),se=((M==null?void 0:M.subs)||[]).map(oe=>F(oe)),xe=(k==null?void 0:k.stadium_def)||(n?l:null),ce=(M==null?void 0:M.stadium_def)||(n?null:l);return xe&&(te=ri(te,xe),gi(re,xe)),ce&&(Z=ri(Z,ce),gi(se,ce)),{p1Team:te,p2Team:Z,p1Subs:re,p2Subs:se,p1Formation:Q,p2Formation:ie,p1Name:(z==null?void 0:z.club_name)||(z==null?void 0:z.pseudo)||"Joueur 1",p2Name:(R==null?void 0:R.club_name)||(R==null?void 0:R.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?y:[],gc_p2:n?[]:y,gcCardsFull_p1:n?w:[],gcCardsFull_p2:n?[]:w,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:o||[],lastActionAt:new Date().toISOString()}}let m=h.game_state&&Object.keys(h.game_state).length?h.game_state:null;if(console.log("[PvP] init - amIHome:",n,"gameState exists:",!!m,"match.status:",h.status,"home_id:",h.home_id,"away_id:",h.away_id,"myId:",b.profile.id),!m)if(n){m=await L(),console.log("[PvP] home - gameState built, p1Team keys:",Object.keys((m==null?void 0:m.p1Team)||{}));const{data:k}=await v.from("matches").select("game_state").eq("id",i).single();!(k!=null&&k.game_state)||!Object.keys(k.game_state).length?await v.from("matches").update({game_state:m,turn_user_id:h.home_id}).eq("id",i):m=k.game_state}else{t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>',console.log("[PvP] away - waiting for home to write game_state...");for(let k=0;k<30&&!m;k++){await new Promise(M=>setTimeout(M,400));const{data:T}=await v.from("matches").select("game_state").eq("id",i).single();T!=null&&T.game_state&&Object.keys(T.game_state).length&&(m=T.game_state),k%5===0&&console.log("[PvP] away - poll",k,"game_state:",!!(T!=null&&T.game_state))}if(!m){g("Erreur de synchronisation","error"),x("home");return}m.gc_p2=y,m.gcCardsFull_p2=w,await v.from("matches").update({game_state:m}).eq("id",i)}let A=!1,E=null,S=!1;const q=new Set,W=new Set;async function ee(k){var ie,te;It();try{v.removeChannel(G)}catch{}const T=m[p+"Score"]||0,M=m[f+"Score"]||0;let B,z;k.winner_id?(B=k.winner_id===b.profile.id,z=!1):k.forfeit?(B=T>M,z=!1):(z=T===M,B=T>M);let R="";if(a&&n)try{const{data:Z}=await v.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",h.home_id).single(),{data:re}=await v.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",h.away_id).single();if(Z&&re){const se=B?1:z?.5:0,xe=1-se,ce=Z.placement_matches<10,be=re.placement_matches<10,ve=computeGlicko2(Z.mmr,Z.mmr_deviation,Z.mmr_volatility,re.mmr,re.mmr_deviation,se===1?1:se===0?0:.5,ce),oe=computeGlicko2(re.mmr,re.mmr_deviation,re.mmr_volatility,Z.mmr,Z.mmr_deviation,xe===1?1:xe===0?0:.5,be);await v.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:z?null:B?h.home_id:h.away_id,p_home_id:h.home_id,p_away_id:h.away_id,p_home_delta:ve.delta,p_away_delta:oe.delta,p_home_new_rd:ve.newRd,p_away_new_rd:oe.newRd,p_home_new_vol:ve.newSigma,p_away_new_vol:oe.newSigma});const Le=ve.delta,Ge=ve.newMmr,Ie=getTier(Ge),Ve=Le>=0?"+":"",Oe=Le>=0?"#4caf50":"#ff6b6b",nt=getTier(Z.mmr);R=`
            <div style="background:rgba(255,255,255,0.08);border-radius:14px;padding:14px 20px;text-align:center;min-width:220px">
              ...
              ${Ie.id!==nt.id?`<div style="font-size:20px;font-weight:900;color:${Ie.color}">
                    ${Ge>Z.mmr?"📈":"📉"} ${nt.emoji} ${nt.label} → ${Ie.emoji} ${Ie.label}
                  </div>
      ...
      <div style="font-size:18px">${m[p+"Name"]} ${T} – ${M} ${m[f+"Name"]}</div>
      ${k.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${B?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      ${R}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${c?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(F),(te=F.querySelector("#pvp-end-home"))==null||te.addEventListener("click",()=>{F.remove(),it(t),c?x("mini-league",{openLeagueId:c}):x(a?"ranked":"home")})}const G=v.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},k=>{var M;const T=k.new;try{if(T.status==="finished"||T.forfeit){if(A)return;A=!0,E&&(clearInterval(E),E=null),Ue(),T.game_state&&(m=T.game_state),ee(T);return}if(T.game_state){const B=m;m=T.game_state;const z=m._lastGC;if(z&&z.seq&&!W.has(z.seq)&&(W.add(z.seq),z.by!==p)){$(z.type,z.by,()=>U());return}const R=B[p+"Score"]||0,F=B[f+"Score"]||0,N=m[p+"Score"]||0,V=m[f+"Score"]||0,Q=N>R,ie=V>F;if((Q||ie)&&!q.has(m.round)){q.add(m.round);const te=[...m.log||[]].reverse().find(se=>se.isGoal),Z=((te==null?void 0:te.homePlayers)||[]).map(se=>({name:se.name,note:se.note,portrait:se.portrait,job:se.job}));!!((M=te==null?void 0:te.text)!=null&&M.includes("DERNIER CORNER"))?we().then(()=>{Qe(Z,N,V,Q,()=>U())}):Qe(Z,N,V,Q,()=>U());return}U()}}catch(B){console.error("[PvP] crash:",B)}}).subscribe();async function I(k){Object.assign(m,k),m.lastActionAt=new Date().toISOString();const{error:T}=await v.from("matches").update({game_state:m}).eq("id",i);T&&g("Erreur de synchronisation","error");try{U()}catch(M){console.error("[PvP] renderPvpScreen crash:",M)}}async function _(){if(A)return;A=!0,It(),Ue(),E&&(clearInterval(E),E=null);const k=n?h.away_id:h.home_id,T=n?"p2":"p1",M=n?"p1":"p2",B={...m,[T+"Score"]:3,[M+"Score"]:0,phase:"finished"},z=B.p1Score||0,R=B.p2Score||0;try{await v.from("matches").update({game_state:B}).eq("id",i);const{data:F,error:N}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:k,p_home_score:z,p_away_score:R,p_forfeit:!0});if(N&&console.error("[PvP] finish_pvp_match (forfeit):",N),c)try{await v.rpc("check_and_finish_mini_league",{p_league_id:c})}catch(V){console.error("[PvP] check_and_finish_mini_league (forfeit):",V)}try{const{data:V}=await v.rpc("apply_match_rewards",{p_match_id:i});V!=null&&V.success&&!(V!=null&&V.skipped)&&typeof e.refreshProfile=="function"&&await e.refreshProfile()}catch(V){console.error("[PvP] apply_match_rewards (forfeit):",V)}}catch{}try{v.removeChannel(G)}catch{}setTimeout(()=>{it(t),c?x("mini-league",{openLeagueId:c}):x("home")},800)}const H={BOOST_STAT:({value:k=1,count:T=1,roles:M=[]},B,z)=>{const R=B[p+"Team"],F=Object.entries(R).filter(([N])=>!M.length||M.includes(N)).flatMap(([N,V])=>V.filter(Q=>!Q.used).map(Q=>({...Q,_line:N})));if(!F.length){B.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),z(B);return}Y(F,T,`Choisir ${T} joueur(s) à booster (+${k})`,N=>{N.forEach(V=>{const Q=(R[V._line]||[]).find(ie=>ie.cardId===V.cardId);Q&&(Q.boost=(Q.boost||0)+k,B.log.push({text:`⚡ +${k} sur ${Q.name}`,type:"info"}))}),B[p+"Team"]=R,z(B)})},DEBUFF_STAT:({value:k=1,count:T=1,roles:M=[],target:B="ai"},z,R)=>{const F=B==="home"?p:f,N=z[F+"Team"],V=B==="home"?"allié":"adverse",Q=Object.entries(N).filter(([ie])=>!M.length||M.includes(ie)).flatMap(([ie,te])=>te.map(Z=>({...Z,_line:ie})));if(!Q.length){z.log.push({text:`🎯 Aucun joueur ${V}`,type:"info"}),R(z);return}Y(Q,T,`Choisir ${T} joueur(s) ${V}(s) (-${k})`,ie=>{ie.forEach(te=>{const Z=(N[te._line]||[]).find(re=>re.cardId===te.cardId);Z&&(Z.boost=(Z.boost||0)-k,z.log.push({text:`🎯 -${k} sur ${Z.name}`,type:"info"}))}),z[F+"Team"]=N,R(z)})},GRAY_PLAYER:({count:k=1,roles:T=[],target:M="ai"},B,z)=>{const R=M==="home"?p:f,F=B[R+"Team"],N=M==="home"?"allié":"adverse",V=Object.entries(F).filter(([Q])=>!T.length||T.includes(Q)).flatMap(([Q,ie])=>ie.filter(te=>!te.used).map(te=>({...te,_line:Q})));if(!V.length){B.log.push({text:`❌ Aucun joueur ${N}`,type:"info"}),z(B);return}Y(V,k,`Choisir ${k} joueur(s) ${N}(s) à exclure`,Q=>{const ie="usedCardIds_"+R,te=new Set(B[ie]||[]);Q.forEach(Z=>{const re=(F[Z._line]||[]).find(se=>se.cardId===Z.cardId);re&&(re.used=!0,te.add(Z.cardId),B.log.push({text:`❌ ${re.name} exclu !`,type:"info"}))}),B[ie]=[...te],B[R+"Team"]=F,z(B)})},REVIVE_PLAYER:({count:k=1,roles:T=[]},M,B)=>{const z=M[p+"Team"],R=Object.entries(z).filter(([F])=>!T.length||T.includes(F)).flatMap(([F,N])=>N.filter(V=>V.used).map(V=>({...V,_line:F})));if(!R.length){M.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),B(M);return}Y(R,k,`Choisir ${k} joueur(s) à ressusciter`,F=>{F.forEach(N=>{const V=(z[N._line]||[]).find(Q=>Q.cardId===N.cardId);V&&(V.used=!1,M.log.push({text:`💫 ${V.name} ressuscité !`,type:"info"}))}),M[p+"Team"]=z,B(M)})},REMOVE_GOAL:({},k,T)=>{const M=f+"Score";k[M]>0?(k[M]--,k.log.push({text:"🚫 Dernier but annulé !",type:"info"})):k.log.push({text:"🚫 Aucun but à annuler",type:"info"}),T(k)},ADD_GOAL_DRAW:({},k,T)=>{k[p+"Score"]===k[f+"Score"]?(k[p+"Score"]++,k.log.push({text:"🎯 But bonus !",type:"info"})):k.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),T(k)},ADD_SUB:({value:k=1},T,M)=>{T.maxSubs=(T.maxSubs||3)+k,T.log.push({text:`🔄 +${k} remplacement(s)`,type:"info"}),M(T)},CUSTOM:({},k,T)=>T(k)};function Y(k,T,M,B){const z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let R=[];function F(){var V,Q;const N=k.map(ie=>{const te=ie._line||ie.job||"MIL",Z=R.find(se=>se.cardId===ie.cardId),re=Se({...ie,_evolution_bonus:0},{width:90,showStad:!0,role:te,extraNote:ie.boost||0});return`<div class="pp-item" data-cid="${ie.cardId}"
        ...
        </div>`,(V=z.querySelector("#pp-close"))==null||V.addEventListener("click",()=>z.remove()),z.querySelectorAll(".pp-item").forEach(ie=>{ie.addEventListener("click",()=>{const te=ie.dataset.cid,Z=k.find(se=>se.cardId===te),re=R.findIndex(se=>se.cardId===te);Z&&(re>-1?R.splice(re,1):R.length<T&&R.push(Z),F())})}),(Q=z.querySelector("#pp-confirm"))==null||Q.addEventListener("click",()=>{z.remove(),B(R)})}F(),document.body.appendChild(z)}async function ae(k,T){const B=(m["gcCardsFull_"+p]||[]).find(N=>N.id===k),z=(B==null?void 0:B._gcDef)||(m.gcDefs||[]).find(N=>{var V;return N.name===T||((V=N.name)==null?void 0:V.toLowerCase().trim())===(T==null?void 0:T.toLowerCase().trim())}),R=[...m["usedGc_"+p]||[],k],F={type:T,by:p,seq:(m._gcAnimSeq||0)+1};W.add(F.seq),$(T,p,async()=>{if(z!=null&&z.effect_type&&z.effect_type!=="CUSTOM"){const V=H[z.effect_type];if(V){const Q={...m};V(z.effect_params||{},Q,async ie=>{ie["usedGc_"+p]=R,ie._lastGC=F,ie._gcAnimSeq=F.seq,await I(ie)});return}}const N={...m};switch(T){case"Remplacement+":N.maxSubs=(N.maxSubs||3)+1,N.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const V=f+"Score";N[V]>0&&(N[V]--,N.log.push({text:"🚫 But annulé",type:"info"}));break}}N["usedGc_"+p]=R,N._lastGC=F,N._gcAnimSeq=F.seq,await I(N)})}function de(k,T){const M="usedCardIds_"+k,B=new Set(m[M]||[]);T.forEach(z=>B.add(z)),m[M]=[...B]}function j(){for(const k of["p1","p2"]){const T=new Set(m["usedCardIds_"+k]||[]),M=m[k+"Team"];if(M)for(const B of["GK","DEF","MIL","ATT"])(M[B]||[]).forEach(z=>{z.used=T.has(z.cardId)})}}function U(){var rn,an,sn,dn,ln,cn;if(m.phase==="reveal")return le();if(m.phase==="midfield")return ue();if(m.phase==="finished")return Ne();const k=m[p+"Team"],T=m[f+"Team"];j();const M=m[p+"Score"],B=m[f+"Score"],z=m[p+"Name"],R=m[f+"Name"],F=m.phase===p+"-attack",N=m.phase===p+"-defense",V=Array.isArray(m["selected_"+p])?m["selected_"+p]:[],Q=V.map(J=>J.cardId),ie=window.innerWidth>=700,te=m[p+"Subs"]||[],Z=m["usedSubIds_"+p]||[],re=te.filter(J=>!Z.includes(J.cardId)),se=m["gcCardsFull_"+p]||[],xe=m["usedGc_"+p]||[],ce=se.filter(J=>!xe.includes(J.id)),be=m.boostOwner===p&&!m.boostUsed,ve=[...k.MIL||[],...k.ATT||[]].filter(J=>!J.used),oe=[...T.MIL||[],...T.ATT||[]].filter(J=>!J.used),Le=!$e(T),Ge=ve.length===0&&oe.length===0&&!Le,Ie=(k.DEF||[]).filter(J=>!J.used),Ve=(k.GK||[]).filter(J=>!J.used);let Oe=[];F&&ve.length===0&&(Le?(Oe=Ie.map(J=>J.cardId),Ie.length===0&&(Oe=Oe.concat(Ve.map(J=>J.cardId)))):Ge&&(Oe=[...Ie,...Ve].map(J=>J.cardId)));function nt(J,ge,Me){var Ce,mt;const _e=J._gcDef,Dt=(_e==null?void 0:_e.name)||J.gc_type,at=(_e==null?void 0:_e.effect)||((Ce=ze[J.gc_type])==null?void 0:Ce.desc)||"",Fe=_e!=null&&_e.image_url?`/icons/${_e.image_url}`:null,Re=((mt=ze[J.gc_type])==null?void 0:mt.icon)||"⚡";return`<div class="pvp-gc-mini" data-gc-id="${J.id}" data-gc-type="${J.gc_type}" style="flex-shrink:0;cursor:pointer">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${Ot(k,m[p+"Formation"],ft,Q,ie?1300:zt(),ie?600:Gt(),Oe)}
          ...
          <div style="display:flex;justify-content:center">${Je((Fe.players||[]).map(Ce=>({...Ce,used:!1})),"#ff6b6b",Fe.total,"attack",m[f+"Formation"])}</div>
          ${Re}
        ...
        </div>`}const J=(m.log||[]).slice(-1)[0];return J?'<div style="padding:2px 4px">'+eo(J)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3);text-align:center">⏳ Match en cours...</div>'})(),on=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
      ...
      <div style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">${mo}</div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      ${bi(T,m[f+"Name"]||"Adversaire","#e03030","opp",null)}
      <div id="duel-shock" style="position:absolute;left:50%;top:50%;width:120px;height:120px;border-radius:50%;border:6px solid #FFD700;opacity:0;pointer-events:none"></div>
      <div id="pvp-duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center,rgba(10,61,30,.4),rgba(10,61,30,.92))"></div>
    </div>`;const R=(re,se)=>t.querySelectorAll(re).forEach((xe,ce)=>{setTimeout(()=>{xe.style.opacity="1",xe.style.transform="translateY(0) scale(1)"},se+ce*90)});R(".duel-card-me",150),R(".duel-card-opp",500),setTimeout(()=>t.querySelectorAll(".duel-link").forEach((re,se)=>setTimeout(()=>{re.style.opacity="1"},se*70)),900),setTimeout(()=>{const re=t.querySelector("#pvp-vs");re&&(re.style.opacity="1",re.style.animation="vsFlash .5s ease"),t.querySelectorAll(".duel-score-line").forEach(se=>se.style.opacity="1")},1250);function F(re,se,xe){const ce=document.getElementById(re);if(!ce)return;const be=performance.now(),ve=oe=>{const Le=Math.min(1,(oe-be)/xe);ce.textContent=Math.round(se*(1-Math.pow(1-Le,3))),Le<1?requestAnimationFrame(ve):ce.textContent=se};requestAnimationFrame(ve)}setTimeout(()=>{F("pvp-score-me",M,800),F("pvp-score-opp",B,800)},1500);const N=m.p1Team.MIL||[],V=m.p2Team.MIL||[],Q=bt(N)+yt(N),ie=bt(V)+yt(V),te=Q>=ie?"p1":"p2";let Z=m.boostValue;Z==null&&(Z=Qn(),m.boostValue=Z,m.boostOwner=te,m.boostUsed=!1),fe=!0,setTimeout(()=>{const re=t.querySelector("#duel-row-"+(z?"me":"opp")),se=t.querySelector("#duel-row-"+(z?"opp":"me")),xe=document.getElementById("pvp-score-me"),ce=document.getElementById("pvp-score-opp"),be=z?xe:ce,ve=z?ce:xe;be&&(be.style.fontSize="80px",be.style.color=z?"#FFD700":"#ff6b6b",be.style.animation="duelPulse .5s ease"+(z?",duelGlow 1.5s ease infinite .5s":"")),ve&&(ve.style.opacity="0.25"),setTimeout(()=>{re&&(re.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",re.style.zIndex="5"),setTimeout(()=>{const oe=document.getElementById("duel-shock");oe&&(oe.style.animation="shockwave .5s ease-out forwards"),se&&(se.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var Ie;const oe=document.getElementById("pvp-duel-finale");if(!oe)return;const Le=m.boostOwner===p?'<div style="background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:3px solid #cdeffd;border-radius:18px;padding:20px 34px;text-align:center;animation:boostFlipIn .7s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 10px 36px rgba(135,206,235,.5)"><div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;font-weight:700">Carte Boost obtenue</div><div style="font-size:46px;line-height:1">⚡</div><div style="font-size:50px;font-weight:900;color:#063;line-height:1.1">+'+Z+`</div><div style="font-size:10px;color:rgba(0,0,0,.55);margin-top:4px">Applicable sur n'importe quel joueur</div></div>`:"",Ge=p==="p1"?'<button id="pvp-start-match" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">▶ Commencer le match</button>':`<div style="font-size:14px;color:rgba(255,255,255,0.5);text-align:center;margin-top:8px;animation:fadeUp .4s ease both">⏳ En attente de l'adversaire...</div>`;oe.innerHTML='<div style="font-size:22px;font-weight:900;color:#fff;text-align:center;animation:fadeUp .4s ease both;text-shadow:0 2px 12px rgba(0,0,0,.5)">'+(z?"⚽ "+m[p+"Name"]+"<br>gagne le milieu et attaque !":"😔 "+m[f+"Name"]+"<br>gagne l'engagement et attaque !")+"</div>"+Le+Ge,oe.style.transition="opacity .45s ease",oe.style.opacity="1",oe.style.pointerEvents="auto",(Ie=document.getElementById("pvp-start-match"))==null||Ie.addEventListener("click",async()=>{const Ve=te;await I({phase:Ve+"-attack",attacker:Ve,round:1,boostValue:Z,boostUsed:!1,boostOwner:Ve})})},600)},700)},2800)}function $(k,T,M){var se,xe;const B=(m.gcDefs||[]).find(ce=>{var be;return ce.name===k||((be=ce.name)==null?void 0:be.toLowerCase().trim())===(k==null?void 0:k.toLowerCase().trim())}),z={purple:"#b06ce0",light_blue:"#00d4ef"}[B==null?void 0:B.color]||"#b06ce0",R=(B==null?void 0:B.name)||k,F=(B==null?void 0:B.effect)||((se=ze[k])==null?void 0:se.desc)||"",N=B!=null&&B.image_url?`/icons/${B.image_url}`:null,V=((xe=ze[k])==null?void 0:xe.icon)||"⚡",ie=T===p?"Vous":m[T+"Name"]||"Adversaire",te=document.createElement("div");te.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",te.innerHTML=`
      <style>
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(te);let Z=!1;const re=()=>{Z||(Z=!0,te.remove(),setTimeout(()=>M&&M(),50))};te.addEventListener("click",re),setTimeout(re,3e3)}function C(k,T){var ie,te,Z,re;const B=(m["gcCardsFull_"+p]||[]).find(se=>se.id===k),z=B==null?void 0:B._gcDef,R=(z==null?void 0:z.name)||T,F=(z==null?void 0:z.effect)||((ie=ze[T])==null?void 0:ie.desc)||"Carte spéciale.",N=z!=null&&z.image_url?`/icons/${z.image_url}`:null,V=((te=ze[T])==null?void 0:te.icon)||"⚡",Q=document.createElement("div");Q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",Q.innerHTML=`
      ${tt(R,N,V,F,{width:190})}
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(Q),(Z=Q.querySelector("#pvp-gc-back"))==null||Z.addEventListener("click",()=>Q.remove()),(re=Q.querySelector("#pvp-gc-use"))==null||re.addEventListener("click",()=>{Q.remove(),ae(k,T)})}function O(){var B;const k=m[p+"Team"],T=Object.entries(k).flatMap(([z,R])=>(R||[]).filter(F=>!F.used).map(F=>({...F,_line:z})));if(!T.length)return;const M=document.createElement("div");M.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",M.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${T.map(z=>{const R={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[z._line]||"#555",F=Pe(z,z._line)+(z.boost||0);return`<div class="bp-item" data-cid="${z.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${R};overflow:hidden;cursor:pointer">
      ...
      </div>`,(Ge=te.querySelector("#psub-close"))==null||Ge.addEventListener("click",()=>te.remove()),(Ie=te.querySelector("#pout-up"))==null||Ie.addEventListener("click",()=>{N>0&&(N--,Z())}),(Ve=te.querySelector("#pout-down"))==null||Ve.addEventListener("click",()=>{N<R.length-1&&(N++,Z())}),(Oe=te.querySelector("#pin-up"))==null||Oe.addEventListener("click",()=>{Q>0&&(Q--,Z())}),(nt=te.querySelector("#pin-down"))==null||nt.addEventListener("click",()=>{Q<F.length-1&&(Q++,Z())});const Le=(De,Ye,He,ut)=>{const ot=te.querySelector("#"+De);if(!ot)return;let Bt=0;ot.addEventListener("touchstart",ft=>{Bt=ft.touches[0].clientY},{passive:!0}),ot.addEventListener("touchend",ft=>{const $t=ft.changedTouches[0].clientY-Bt;if(Math.abs($t)<30)return;const rt=Ye();$t<0&&rt<ut-1?(He(rt+1),Z()):$t>0&&rt>0&&(He(rt-1),Z())},{passive:!0})};Le("pin-panel",()=>Q,De=>Q=De,F.length),Le("pout-panel",()=>N,De=>N=De,R.length),(Kt=te.querySelector("#psub-confirm"))==null||Kt.addEventListener("click",async De=>{if(De.preventDefault(),De.stopPropagation(),ie)return;ie=!0;const Ye=R[N],He=F[Q];if(!Ye||!He)return;const ut=Ye._line,ot=(M[ut]||[]).findIndex($t=>$t.cardId===Ye.cardId);if(ot===-1){g("Erreur : joueur introuvable","error"),te.remove();return}const Bt={...He,_line:ut,position:Ye.position,used:!1,boost:0};M[ut].splice(ot,1,Bt);const ft=[...B,He.cardId];te.remove(),X(Ye,He,async()=>{await I({[p+"Team"]:M,[f+"Team"]:m[f+"Team"],["usedSubIds_"+p]:ft})})})}document.body.appendChild(te),Z()}function X(k,T,M){const B={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const R=(V,Q,ie)=>`<div style="text-align:center">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(z);let F=!1;const N=()=>{F||(F=!0,z.remove(),setTimeout(()=>M(),50))};z.addEventListener("click",N),setTimeout(N,2200)}function D(){var T;const k=document.createElement("div");k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",k.innerHTML=`
      <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-transform:uppercase">Équipe adverse</div>
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${m[f+"Name"]}</div>
      <div style="width:min(90vw,420px)">${_i(m[f+"Team"],m[f+"Formation"],null,[],zt(),Gt())}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(k),(T=k.querySelector("#pvp-opp-close"))==null||T.addEventListener("click",()=>k.remove())}function K(){var M;const k=m.log||[],T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",T.innerHTML=`
      ...
      </div>`,document.body.appendChild(T),(M=T.querySelector("#pvp-hist-close"))==null||M.addEventListener("click",()=>T.remove())}async function pe(){if(m.phase!==p+"-attack")return;const k=p==="p1"?"p2":"p1",T=(m.round||0)+1,M=[...m.log||[]];M.push({type:"info",text:`⏭️ ${m[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const B=Ee(m),z=pt(k),R=B||!z?"finished":k+"-attack";await I({["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:R,attacker:k,round:T,log:M}),R==="finished"&&await qe(m)}function ne(k){const T=(k.GK||[]).some(B=>!B.used),M=["DEF","MIL","ATT"].some(B=>(k[B]||[]).some(z=>!z.used));return T&&!M}function me(k){return(k.p1Score||0)!==(k.p2Score||0)?null:ne(k.p1Team)&&!$e(k.p2Team)?{side:"p1",gk:(k.p1Team.GK||[]).find(T=>!T.used)}:ne(k.p2Team)&&!$e(k.p1Team)?{side:"p2",gk:(k.p2Team.GK||[]).find(T=>!T.used)}:null}async function we(){const k=document.createElement("div");k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(k);const T=(M,B)=>new Promise(z=>{k.innerHTML=`<div style="font-size:32px;font-weight:900;color:${B};letter-spacing:2px;animation:lcFade 1.4s ease both">${M}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(z,1400)});await T("⚽ DERNIER CORNER","#FFD700"),await T("🧤 LE GARDIEN MONTE !","#4fc3f7"),k.remove()}async function he(k,T){const{side:M,gk:B}=k;if(!B)return;const z=M+"Score",R=(T[z]||0)+1;B.used=!0;const F=[...T.log||[]];F.push({type:"duel",isGoal:!0,homeScored:M===p,homePlayers:M==="p1"?[Ft(B)]:[],aiPlayers:M==="p2"?[Ft(B)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${T[M+"Name"]} marque !`});const N=(T.round||0)+1,V=M==="p1"?"p2":"p1",Q={...T,[z]:R},ie=Ee(Q);q.add(N),await we();const te=M===p?R:T[p+"Score"]||0,Z=M===p?T[f+"Score"]||0:R;Qe([Ft(B)],te,Z,M===p,async()=>{await I({[z]:R,log:F,round:N,pendingAttack:null,phase:ie?"finished":V+"-attack",attacker:V,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}}}),ie&&await qe({...m,[z]:R})})}async function Te(){const k=m[p+"Team"],T=!["GK","DEF","MIL","ATT"].some(R=>(m[f+"Team"][R]||[]).some(F=>!F.used)),M=(m["selected_"+p]||[]).map(R=>{const F=(k[R._role]||[]).find(se=>se.cardId===R.cardId)||R,N=["GK","DEF"].includes(R._role),V=k[R._role]||[],Q=V.findIndex(se=>se.cardId===R.cardId),ie=st(V.length),te=Q>=0?ie[Q]:F._col??1,Z=m.stadiumDef||m.homeStadiumDef||null,re=F.stadiumBonus||Z&&(Z.club_id&&String(F.club_id)===String(Z.club_id)||Z.country_code&&F.country_code===Z.country_code)||!1;return{...F,_line:R._role,_col:te,stadiumBonus:re,...N?{note_a:Math.max(1,Number(F.note_a)||0)}:{}}});if(!M.length)return;const B=ni(M,m.modifiers[p]||{},m[p+"Formation"]);de(p,M.map(R=>R.cardId)),M.forEach(R=>{const F=(k[R._role]||[]).find(N=>N.cardId===R.cardId);F&&(F.used=!0)}),m["selected_"+p]=[],U();const z=[...m.log||[]];if(T){if(M.length===1&&(M[0]._line==="GK"||M[0].job==="GK")&&(m.p1Score||0)===(m.p2Score||0)){await he({side:p,gk:M[0]},{...m,[p+"Team"]:k,log:z});return}const F=(m[p+"Score"]||0)+1,N=M.map(Z=>Ft(Z));z.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:N,homeTotal:B.total,aiTotal:0});const V=(m.round||0)+1,Q=p==="p1"?"p2":"p1",ie={...m,[p+"Team"]:k,[p+"Score"]:F},te=Ee(ie);q.add(V),Qe(N,F,m[f+"Score"]||0,!0,async()=>{await I({[p+"Team"]:k,[p+"Score"]:F,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:te?"finished":Q+"-attack",attacker:Q,round:V,log:z}),te&&await qe({...m,[p+"Score"]:F})});return}z.push({type:"pending",text:`⚔️ ${m[p+"Name"]} attaque (${B.total})`}),await I({[p+"Team"]:k,[f+"Team"]:m[f+"Team"],pendingAttack:{...B,players:M,side:p},["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},phase:f+"-defense",log:z})}async function Ae(){const k=m[p+"Team"],T=(m["selected_"+p]||[]).map(ce=>{const be=(k[ce._role]||[]).find(Ie=>Ie.cardId===ce.cardId)||ce,ve=k[ce._role]||[],oe=ve.findIndex(Ie=>Ie.cardId===ce.cardId),Le=st(ve.length),Ge=oe>=0?Le[oe]:be._col??1;return{...be,_line:ce._role,_col:Ge}}),M=oi(T,m.modifiers[p]||{},m[p+"Formation"]);de(p,T.map(ce=>ce.cardId)),T.forEach(ce=>{const be=(k[ce._role]||[]).find(ve=>ve.cardId===ce.cardId);be&&(be.used=!0)}),m["selected_"+p]=[],U();const B=Qi(m.pendingAttack.total,M.total,m.modifiers[p]||{}),z=m.pendingAttack.side,R=B==="attack"||(B==null?void 0:B.goal),F=z==="p1"?"p2":"p1",N=(m.round||0)+1,V=(m.pendingAttack.players||[]).map(ce=>Ft(ce)),Q=[...m.log||[]];Q.push({type:"duel",isGoal:R,homeScored:R&&z===p,text:R?`⚽ BUT de ${m[z+"Name"]} ! (${m.pendingAttack.total} vs ${M.total})`:`✋ Attaque stoppée (${m.pendingAttack.total} vs ${M.total})`,homePlayers:V,aiPlayers:T.map(ce=>Ft(ce)),homeTotal:m.pendingAttack.total,aiTotal:M.total});const ie=R?(m[z+"Score"]||0)+1:m[z+"Score"]||0,te={...m,[p+"Team"]:k,[z+"Score"]:ie},Z=Ee(te),re=Z?"finished":F+"-attack",se=!Z&&!R?me(te):null,xe=async()=>{if(se){await he(se,te);return}await I({[p+"Team"]:k,[f+"Team"]:m[f+"Team"],[z+"Score"]:ie,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:re,attacker:F,round:N,log:Q}),(re==="finished"||Z)&&await qe({...m,[z+"Score"]:ie})};if(R){const ce=z===p,be=ce?ie:m[p+"Score"]||0,ve=ce?m[f+"Score"]||0:ie;q.add(N),Qe(V,be,ve,ce,xe)}else await xe()}function Be(k){return["MIL","ATT"].some(T=>(k[T]||[]).some(M=>!M.used))}function $e(k){return["GK","DEF","MIL","ATT"].some(T=>(k[T]||[]).some(M=>!M.used))}function kt(k){return $e(k)&&!Be(k)}function pt(k){const T=m[k+"Team"],M=m[(k==="p1"?"p2":"p1")+"Team"];return!!(Be(T)||kt(T)&&!$e(M)||kt(T)&&!Be(M)&&$e(M))}function Ee(k){return!$e(k.p1Team)&&!$e(k.p2Team)}function je(k){const T=k.p1Score||0,M=k.p2Score||0;return T===M?null:T>M?h.home_id:h.away_id}async function qe(k){It(),Ue();try{const T=je(k),M=T?h.home_id===T?h.away_id:h.home_id:null,B=k.p1Score||0,z=k.p2Score||0,{data:R,error:F}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:T,p_home_score:B,p_away_score:z,p_forfeit:!1});if(F&&console.error("[PvP] finish_pvp_match:",F),c)try{await v.rpc("check_and_finish_mini_league",{p_league_id:c})}catch(N){console.error("[PvP] check_and_finish_mini_league:",N)}T&&M&&Yr(T,M).catch(()=>{});try{const{data:N}=await v.rpc("apply_match_rewards",{p_match_id:i});N!=null&&N.success&&!(N!=null&&N.skipped)&&typeof e.refreshProfile=="function"&&await e.refreshProfile()}catch(N){console.error("[PvP] apply_match_rewards:",N)}if(typeof d=="function")try{await d({homeScore:B,awayScore:z,winnerId:T,homeId:h.home_id,awayId:h.away_id,matchId:i})}catch(N){console.error("[PvP] onMatchEnd:",N)}}catch(T){console.error("[PvP] finishMatch:",T)}}function Ne(){var z;const k=m[p+"Score"],T=m[f+"Score"],M=k>T,B=k===T;dt(t),t.innerHTML=`
    ...
    </div>`,(z=document.getElementById("pvp-home"))==null||z.addEventListener("click",()=>{try{v.removeChannel(G)}catch{}it(t),c?x("mini-league",{openLeagueId:c}):x("home")})}U()}async function Ni(t,e,i=!1){var s,l;const{state:n,navigate:r,toast:o}=e,a=i&&((l=(s=e==null?void 0:e.state)==null?void 0:s.params)==null?void 0:l.rankedData)||null;await ki(t,e,i?"ranked":"random",async({deckId:d,formation:c,starters:u,subsRaw:b,gcCardsEnriched:x,gcDefs:g,stadiumDef:p})=>{dt(t);const f=async y=>{await Jr(t,e,d,c,u,b,y||[],g,p,i,a)};if(!(x!=null&&x.length)){await f([]);return}wi(t,x,f)})}async function Xr(t,e,i){return Wr(t,e,i)}async function Jr(t,e,i,n,r,o,a=[],s=[],l=null,d=!1,c=null){var ee,G;const{state:u,navigate:b,toast:x}=e,g=(c==null?void 0:c.mmr)??((ee=u.profile)==null?void 0:ee.mmr)??1e3,p=d?vi(g):null,f=d?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",y=d?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let w=!1,h=null,L=null;t.innerHTML=`
    ...
    </div>`;const m=async()=>{w=!0,h&&(h.unsubscribe(),h=null),L&&(clearInterval(L),L=null);try{await v.rpc("cancel_matchmaking",{p_user_id:u.profile.id})}catch(I){console.error("[Matchmaking] cancel error:",I)}};(G=document.getElementById("cancel-mm"))==null||G.addEventListener("click",async()=>{await m(),it(t),b("home")});const A=async(I,_)=>{w||(w=!0,h&&(h.unsubscribe(),h=null),L&&(clearInterval(L),L=null),await jt(t,e,I,_,{myGC:a,gcDefs:s,stadiumDef:l,isRanked:d,rankedData:c,onMatchEnd:d?Qr:null}))},E=u.user.id,{data:S,error:q}=d?await v.rpc("try_matchmake_ranked",{p_user_id:E,p_deck_id:i,p_mmr:g}):await v.rpc("try_matchmake",{p_user_id:E,p_deck_id:i});if(q||!(S!=null&&S.success)){console.error("[Matchmaking] try_matchmake error:",q||S),x("Erreur de matchmaking","error"),it(t),b("home");return}if(S.matched){await A(S.match_id,!1);return}const W=async()=>{if(w)return;const{data:I}=await v.from("matchmaking_queue").select("status, match_id").eq("user_id",E).maybeSingle();(I==null?void 0:I.status)==="matched"&&I.match_id&&await A(I.match_id,!0)};h=v.channel(`mm_${E}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${E}`},I=>{const _=I.new;_.status==="matched"&&_.match_id&&A(_.match_id,!0)}).subscribe(),L=setInterval(W,3e3),setTimeout(()=>{w||m()},12e4)}async function Qr(t,e){const{state:i,toast:n}=e,{winnerId:r,homeId:o,awayId:a,homeScore:s,awayScore:l,matchId:d}=t,c=i.user.id,u=o===c,b=r===c,{data:x}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",c).single(),{data:g}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",u?a:o).single();if(x&&g){const p=go(x.mmr,x.mmr_rd,x.mmr_v,g.mmr,g.mmr_rd,b?1:0);await v.from("users").update({mmr:p.mmr,mmr_rd:p.rd,mmr_v:p.v,mmr_wins:b?v.sql`mmr_wins + 1`:void 0,mmr_losses:b?void 0:v.sql`mmr_losses + 1`}).eq("id",c),n(b?`+MMR ↑ ${Math.round(p.mmr-x.mmr)}`:`-MMR ↓ ${Math.round(x.mmr-p.mmr)}`,b?"success":"error",4e3)}}const ro=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:Ni,resumePvpMatch:Xr},Symbol.toStringTag,{value:"Module"}));async function Zr(t,e){const{state:i,navigate:n,toast:r}=e,o=i.params||{},a=o.friendId||null,s=o.friendName||"Ami",l=!!o.isAccepting;await ki(t,e,"friend",async({deckId:d,formation:c,starters:u,subsRaw:b,gcCardsEnriched:x,gcDefs:g,stadiumDef:p})=>{dt(t);const f=async y=>{const w=y||[];l?await ta(t,e,d,w,g,p):await ea(t,e,d,w,g,p,a,s)};if(!(x!=null&&x.length)){await f([]);return}wi(t,x,f)})}async function ea(t,e,i,n,r,o,a,s){var w;const{state:l,navigate:d,toast:c}=e,u=l.user.id,{data:b,error:x}=await v.from("friend_match_invites").insert({inviter_id:u,invitee_id:a,friend_id:a,status:"pending",inviter_deck_id:i,expires_at:new Date(Date.now()+2*60*1e3).toISOString()}).select("id").single();if(x||!b){console.error("[Friend] Erreur création invitation:",x),c("Impossible de créer l'invitation","error"),d("home");return}t.innerHTML=`
    ...
    </div>`;let g=!1,p=null;const f=async()=>{p&&(p.unsubscribe(),p=null)};(w=document.getElementById("cancel-friend"))==null||w.addEventListener("click",async()=>{await f(),await v.from("friend_match_invites").update({status:"declined"}).eq("id",b.id),it(t),d("home")}),p=v.channel(`friend_invite_${b.id}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${b.id}`},async({new:h})=>{g||h.status!=="accepted"||!h.match_id||(g=!0,await f(),await jt(t,e,h.match_id,!0,{myGC:n,gcDefs:r,stadiumDef:o}))}).subscribe();const y=setInterval(async()=>{if(g)return;const{data:h}=await v.from("friend_match_invites").select("status, match_id").eq("id",b.id).single();(h==null?void 0:h.status)==="accepted"&&h.match_id&&(g=!0,clearInterval(y),await f(),await jt(t,e,h.match_id,!0,{myGC:n,gcDefs:r,stadiumDef:o}))},3e3);setTimeout(async()=>{g||(clearInterval(y),await f(),c("Invitation expirée","info"),d("home"))},12e4)}async function ta(t,e,i,n,r,o){const{state:a,navigate:s,toast:l}=e,d=a.user.id,{data:c}=await v.from("friend_match_invites").select("id, inviter_id").eq("invitee_id",d).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle();if(!c){l("Aucune invitation en attente","error"),s("home");return}const{data:u,error:b}=await v.rpc("accept_friend_invite",{p_invite_id:c.id,p_invitee_deck_id:i});if(b||!(u!=null&&u.success)){console.error("[Friend] Erreur accept_friend_invite:",b||u),l((u==null?void 0:u.error)||"Impossible de rejoindre le match","error"),s("home");return}await jt(t,e,u.match_id,!1,{myGC:n,gcDefs:r,stadiumDef:o})}const ao="#1A6B3C",lt="#D4A017",Jt="var(--tile-bg)",Qt="var(--tile-border)",Tt="var(--divider)",Xe="var(--tile-fg-on-page)",Lt="var(--tile-fg-dim)",Ze="var(--tile-fg-faint)",ei="var(--nav-fg,#fff)",so="rgba(255,255,255,0.62)",ia="rgba(255,255,255,0.4)";async function na(t,e){var n;t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const i=(n=e.state.params)==null?void 0:n.openLeagueId;if(i){e.state.params.openLeagueId=null,await ct(t,e,i);return}await et(t,e)}async function et(t,e,i="waiting"){var h,L;const{state:n}=e,r=n.profile.id,{data:o}=await v.from("mini_league_members").select("league_id, prize_amount, prize_claimed").eq("user_id",r),a=(o||[]).map(m=>m.league_id),s={};(o||[]).forEach(m=>{s[m.league_id]={amount:m.prize_amount,claimed:m.prize_claimed}});const{data:l,error:d}=await v.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").eq("is_archived",!1).order("created_at",{ascending:!1}).limit(30),c=d?(await v.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").order("created_at",{ascending:!1}).limit(30)).data||[]:l||[],b=(a.length?await v.from("mini_leagues").select("*, mini_league_members(count)").in("id",a).order("created_at",{ascending:!1}):{data:[]}).data||[],x=b.filter(m=>m.status==="waiting"&&!m.is_archived),g=b.filter(m=>m.status==="active"&&!m.is_archived),p=b.filter(m=>m.is_archived||m.status==="finished");let f=p;if(p.length){const{data:m}=await v.from("mini_league_matches").select("league_id").in("league_id",p.map(E=>E.id)).or(`home_id.eq.${r},away_id.eq.${r}`).not("status","eq","bye"),A=new Set((m||[]).map(E=>E.league_id));f=p.filter(E=>A.has(E.id))}const y=c.filter(m=>!a.includes(m.id)),w=[{key:"waiting",label:"🟡 En attente",count:x.length+y.length},{key:"active",label:"🟢 En cours",count:g.length},{key:"archived",label:"📁 Archivées",count:f.length}];t.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <div style="padding:14px 16px;background:var(--nav-bg,#0d1a0f);border-bottom:1px solid ${Tt};display:flex;align-items:center;justify-content:space-between">
      <div>
        ...
        <div style="font-size:12px;color:${so}">Championnats 3 à 8 joueurs</div>
      </div>
      <div style="display:flex;align-items:center;gap:8px">
    ...
    <div style="padding:20px;display:flex;flex-direction:column;gap:14px">
    ...
    </div></div>`,document.body.appendChild(i),i.querySelector("#ml-form-close").addEventListener("click",()=>i.remove()),i.querySelectorAll('input[name="ml-type"]').forEach(n=>n.addEventListener("change",()=>{document.getElementById("ml-pwd-block").style.display=n.value==="private"?"block":"none"})),i.querySelector("#ml-create-confirm").addEventListener("click",async()=>{var g,p,f,y;const{toast:n}=e,r=document.getElementById("ml-name").value.trim(),o=parseInt(document.getElementById("ml-max").value)||6,a=parseInt(document.getElementById("ml-fee").value)||500,s=((g=i.querySelector('input[name="ml-type"]:checked'))==null?void 0:g.value)||"public",l=((p=i.querySelector('input[name="ml-mode"]:checked'))==null?void 0:p.value)||"aller",d=((y=(f=document.getElementById("ml-pwd"))==null?void 0:f.value)==null?void 0:y.trim())||null;if(!r){n("Le nom est obligatoire","error");return}if(a<100){n("Mise minimum : 100 crédits","error");return}if(s==="private"&&!d){n("Mot de passe requis","error");return}const{data:c,error:u}=await v.from("mini_leagues").insert({name:r,creator_id:e.state.profile.id,type:s,password:d,mode:l,max_players:o,entry_fee:a}).select().single();if(u){n("Erreur : "+u.message,"error");return}const{data:b}=await v.from("users").select("credits").eq("id",e.state.profile.id).single();if(((b==null?void 0:b.credits)||0)<a){await v.from("mini_leagues").delete().eq("id",c.id),n(`Crédits insuffisants pour la mise (${a.toLocaleString("fr")} cr.)`,"error");return}await v.from("users").update({credits:b.credits-a}).eq("id",e.state.profile.id),await v.from("mini_leagues").update({pot:a}).eq("id",c.id),await v.from("mini_league_members").insert({league_id:c.id,user_id:e.state.profile.id}),e.state.profile&&(e.state.profile.credits=b.credits-a);const x=document.getElementById("nav-credits");x&&(x.textContent=`💰 ${(e.state.profile.credits||0).toLocaleString("fr")}`),i.remove(),n(`Mini League créée ! ${a.toLocaleString("fr")} cr. déduits.`,"success"),ct(t,e,c.id)})}function da(){return new Promise(t=>{const e=document.createElement("div");e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9500;display:flex;align-items:center;justify-content:center;padding:20px",e.innerHTML=`
      ...
      </div>`,document.body.appendChild(e);const i=e.querySelector("#ml-pwd-input"),n=r=>{e.remove(),t(r)};setTimeout(()=>i==null?void 0:i.focus(),80),e.querySelector("#pwd-cancel").addEventListener("click",()=>n(null)),e.querySelector("#pwd-ok").addEventListener("click",()=>n(i.value.trim())),i.addEventListener("keydown",r=>{r.key==="Enter"&&n(i.value.trim())})})}async function lo(t,e,i,n){var u;const{toast:r,state:o}=e,a=o.profile.id;let s=null;if(n==="private"&&(s=await da(),s===null))return;const{data:l,error:d}=await v.rpc("join_mini_league",{p_league_id:i,p_user_id:a,p_password:s});if(d||!(l!=null&&l.success)){console.error("[MiniLeague] join_mini_league:",d||l),r((l==null?void 0:l.error)||"Erreur lors de l'inscription","error");return}if(typeof e.refreshProfile=="function")await e.refreshProfile();else{const{data:b}=await v.from("users").select("credits").eq("id",a).single();o.profile&&b&&(o.profile.credits=b.credits)}const c=document.getElementById("nav-credits");c&&(c.textContent=`💰 ${(((u=o.profile)==null?void 0:u.credits)||0).toLocaleString("fr")}`),r("✅ Inscrit ! Pot mis à jour.","success"),ct(t,e,i)}async function la(t,e,i,n){const{toast:r,state:o}=e,a=o.profile.id;if(!await new Promise(b=>{const x=document.createElement("div");x.className="modal-overlay",x.style.zIndex="2100",x.innerHTML=`<div class="modal" style="max-width:360px">
    ...
    </div>`,document.body.appendChild(x),x.querySelector("#lv-ok").onclick=()=>{x.remove(),b(!0)},x.querySelector("#lv-cancel").onclick=()=>{x.remove(),b(!1)}}))return;const{data:l,error:d}=await v.rpc("leave_mini_league",{p_league_id:i,p_user_id:a});if(d||!(l!=null&&l.success)){r((l==null?void 0:l.error)||"Erreur lors de la désinscription","error");return}const c=l.refund||n;o.profile&&(o.profile.credits=(o.profile.credits||0)+c);const u=document.getElementById("nav-credits");u&&(u.textContent=`💰 ${(o.profile.credits||0).toLocaleString("fr")}`),r(`↩️ Désinscrit · +${c.toLocaleString("fr")} cr. remboursés`,"success"),et(t,e,"waiting")}async function Gi(t,e,i,n,r){const{toast:o}=e;if(!confirm(`Supprimer définitivement "${n}" et tous ses matchs/membres ? Action irréversible.`))return;await v.from("mini_league_matches").delete().eq("league_id",i),await v.from("mini_league_members").delete().eq("league_id",i);const{error:a}=await v.from("mini_leagues").delete().eq("id",i);if(a){o("Erreur suppression ("+a.message+")","error");return}o("Mini League supprimée avec succès","success"),et(t,e,r)}async function ca(t,e,i){await v.from("mini_leagues").update({is_archived:!0}).eq("id",i),e.toast("Mini League archivée","success"),et(t,e,"archived")}async function ct(t,e,i){var L,m,A,E,S,q,W,ee,G;const{state:n,toast:r}=e,o=n.profile.id,[{data:a},{data:s},{data:l}]=await Promise.all([v.from("mini_leagues").select("*").eq("id",i).single(),v.from("mini_league_members").select("*, user:users(id,pseudo,club_name,club_color1,club_color2)").eq("league_id",i),v.from("mini_league_matches").select("*").eq("league_id",i).order("matchday").order("created_at")]);if(!a){r("Introuvable","error"),await et(t,e);return}const d=(s||[]).some(I=>I.user_id===o),c=a.creator_id===o,u=(s||[]).map(I=>I.user).filter(Boolean),b=ga(u,l||[]),x=(l||[]).filter(I=>I.matchday===a.current_day),g=a.pot||0,p=a.entry_fee||100,f=x.length>0&&x.every(I=>I.status==="finished"||I.status==="bye"),y=a.current_day>=a.total_days,w=(s||[]).find(I=>I.user_id===o);t.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <div style="padding:14px 16px;background:var(--nav-bg,#0d1a0f);border-bottom:1px solid ${Tt};display:flex;align-items:center;gap:10px">
      ...
      <div style="flex:1">
        ...
        <div style="font-size:11px;color:${so}">${a.mode==="aller-retour"?"Aller-Retour":"Aller"} · max ${a.max_players} · 💰 ${p} cr./joueur</div>
      </div>
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        <div style="font-size:14px;font-weight:900;margin-bottom:4px;color:${Xe}">👥 Joueurs (${u.length}/${a.max_players})</div>
        <div style="font-size:12px;color:${Lt};margin-bottom:10px">💰 ${p} cr./joueur → Pot estimé : ${(p*u.length).toLocaleString("fr")} cr. (🥇${Math.floor(p*u.length*.7).toLocaleString("fr")} · 🥈${Math.floor(p*u.length*.2).toLocaleString("fr")} · 🥉${Math.floor(p*u.length*.1).toLocaleString("fr")})</div>
        ${u.map(I=>`
          <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid ${Tt}">
            <div style="width:36px;height:36px;border-radius:50%;background:${I.club_color2||ao};display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:${I.club_color1||"#fff"}">${(I.pseudo||"?").slice(0,2).toUpperCase()}</div>
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        ${x.map(I=>Ln(I,u,o,d)).join("")}
        ${c&&f&&!y?'<button id="ml-next-day" class="btn btn-primary" style="width:100%;margin-top:12px">➡️ Journée suivante</button>':""}
      </div>`:""}
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          <thead><tr style="font-size:10px;color:${Ze};text-transform:uppercase;border-bottom:2px solid ${Tt}">
          ...
          <tbody>${b.map((I,_)=>{const H=g>0&&a.status==="finished"?_===0?Math.floor(g*.7):_===1?Math.floor(g*.2):_===2?Math.floor(g*.1):0:0;return`<tr style="border-bottom:1px solid ${Tt};${I.userId===o?"background:rgba(74,222,128,0.08);":""}">
              ...
              <td style="text-align:center;color:${Lt}">${I.played}</td><td style="text-align:center;color:${Lt}">${I.won}-${I.drawn}-${I.lost}</td>
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(I,_)=>_+1).reverse().map(I=>{const _=(l||[]).filter(H=>H.matchday===I);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${Ze};margin-bottom:6px">Journée ${I}</div>${_.map(H=>Ln(H,u,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  </div>`;const h=a.status==="waiting"?"waiting":a.status==="active"?"active":"archived";if((L=document.getElementById("ml-back"))==null||L.addEventListener("click",()=>et(t,e,h)),(m=document.getElementById("ml-refresh"))==null||m.addEventListener("click",async I=>{const _=I.currentTarget;_.style.opacity="0.5",await ct(t,e,i)}),(A=document.getElementById("ml-start-btn"))==null||A.addEventListener("click",()=>pa(t,e,a,u)),(E=document.getElementById("ml-next-day"))==null||E.addEventListener("click",()=>ua(t,e,i)),(S=document.getElementById("ml-join-now"))==null||S.addEventListener("click",()=>lo(t,e,i,a.type)),(q=document.getElementById("ml-leave-btn"))==null||q.addEventListener("click",()=>la(t,e,i,p)),(W=document.getElementById("ml-delete-btn"))==null||W.addEventListener("click",()=>Gi(t,e,i,a.name,"waiting")),(ee=document.getElementById("ml-delete-now"))==null||ee.addEventListener("click",()=>Gi(t,e,i,a.name,h)),(G=document.getElementById("ml-archive-btn"))==null||G.addEventListener("click",()=>ca(t,e,i)),t.querySelectorAll("[data-play-match]").forEach(I=>{I.addEventListener("click",()=>{const _=x.find(H=>H.id===I.dataset.playMatch);_&&e.navigate("match-mini-league",{mlMatchId:_.id,leagueId:i})})}),a.status==="finished"&&w){const I=b.findIndex(_=>_.userId===o);I>=0&&I<3&&w.prize_amount>0&&!w.prize_claimed&&setTimeout(()=>fa(t,e,a,w,I),400)}}function Ln(t,e,i,n,r=!1){const o=u=>e.find(b=>b.id===u);if(t.is_bye){const u=o(t.home_id);return`<div style="padding:8px;border-radius:8px;background:rgba(255,255,255,0.03);margin-bottom:6px;font-size:12px;color:${Lt};text-align:center">🔵 ${(u==null?void 0:u.club_name)||(u==null?void 0:u.pseudo)||"?"} exempté(e)</div>`}const a=o(t.home_id),s=o(t.away_id),l=t.home_id===i||t.away_id===i,d=l&&t.status==="pending"&&n&&!r,c=t.status==="finished"?`${t.home_score} - ${t.away_score}`:"vs";return`<div style="display:flex;align-items:center;gap:8px;padding:10px;border-radius:8px;background:${l?"rgba(26,107,60,0.16)":"rgba(255,255,255,0.03)"};margin-bottom:6px;border:1px solid ${l?"rgba(74,222,128,0.35)":Tt}">
  ...
  </div>`}async function pa(t,e,i,n){const{toast:r,state:o}=e,a=ma(n.map(d=>d.id),i.mode),s=[];a.forEach((d,c)=>d.forEach(u=>s.push({league_id:i.id,matchday:c+1,home_id:u.home||u.bye,away_id:u.away||null,is_bye:!!u.bye,status:u.bye?"bye":"pending"})));const{error:l}=await v.from("mini_league_matches").insert(s);if(l){r("Erreur calendrier : "+l.message,"error");return}await v.from("mini_leagues").update({status:"active",current_day:1,total_days:a.length}).eq("id",i.id),r(`🚀 Lancée ! Pot : ${(i.pot||0).toLocaleString("fr")} cr.`,"success"),ct(t,e,i.id)}async function ua(t,e,i){const{data:n}=await v.from("mini_leagues").select("current_day,total_days,pot").eq("id",i).single(),r=(n.current_day||0)+1;if(r>(n.total_days||0)){ct(t,e,i);return}await v.from("mini_leagues").update({current_day:r}).eq("id",i),e.toast(`Journée ${r} commencée !`,"success"),ct(t,e,i)}async function fa(t,e,i,n,r){var b,x;const{state:o,toast:a}=e,s=[Math.floor((i.pot||0)*.7),Math.floor((i.pot||0)*.2),Math.floor((i.pot||0)*.1)],l=["🥇","🥈","🥉"][r],d=n.prize_amount||s[r]||0,c=n.prize_claimed,u=document.createElement("div");u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",u.innerHTML=`
    ...
    </div>`,document.body.appendChild(u),(b=u.querySelector("#prize-close"))==null||b.addEventListener("click",()=>u.remove()),(x=u.querySelector("#claim-prize-btn"))==null||x.addEventListener("click",async g=>{const p=g.currentTarget;p.disabled=!0,p.textContent="...";const{data:f,error:y}=await v.rpc("claim_mini_league_prize",{p_league_id:i.id,p_user_id:o.profile.id});if(y||!(f!=null&&f.success)){console.error("[MiniLeague] claim_mini_league_prize:",y||f),a((f==null?void 0:f.error)||"Erreur lors de la récupération","error"),p.disabled=!1,p.textContent=`💰 Récupérer ${d.toLocaleString("fr")} cr.`;return}if(f.already_claimed)a("Déjà récupéré précédemment","info");else{const w=(o.profile.credits||0)+f.prize;o.profile&&(o.profile.credits=w);const h=document.getElementById("nav-credits");h&&(h.textContent=`💰 ${w.toLocaleString("fr")}`),a(`💰 +${f.prize.toLocaleString("fr")} cr. ajoutés à ton solde !`,"success")}u.remove(),ct(t,e,i.id)})}function ma(t,e){const n=t.length%2===0?[...t]:[...t,null],r=n.length;let o=n.slice(1);const a=[];for(let s=0;s<r-1;s++){const l=[],d=[n[0],...o];for(let c=0;c<r/2;c++){const u=d[c],b=d[r-1-c];u===null?l.push({bye:b}):b===null?l.push({bye:u}):l.push({home:u,away:b})}a.push(l),o=[o[o.length-1],...o.slice(0,-1)]}return e==="aller-retour"?[...a,...a.map(s=>s.map(l=>l.bye?l:{home:l.away,away:l.home}))]:a}function ga(t,e){const i={};return t.forEach(n=>{i[n.id]={userId:n.id,pseudo:n.pseudo,clubName:n.club_name||n.pseudo,played:0,won:0,drawn:0,lost:0,goalsFor:0,goalsAgainst:0,goalDiff:0,points:0}}),e.filter(n=>n.status==="finished"&&!n.is_bye&&n.home_score!=null).forEach(n=>{const r=i[n.home_id],o=i[n.away_id];!r||!o||(r.played++,o.played++,r.goalsFor+=n.home_score,r.goalsAgainst+=n.away_score,o.goalsFor+=n.away_score,o.goalsAgainst+=n.home_score,n.home_score>n.away_score?(r.won++,r.points+=3,o.lost++):n.home_score<n.away_score?(o.won++,o.points+=3,r.lost++):(r.drawn++,r.points++,o.drawn++,o.points++),r.goalDiff=r.goalsFor-r.goalsAgainst,o.goalDiff=o.goalsFor-o.goalsAgainst)}),Object.values(i).sort((n,r)=>r.points-n.points||r.goalDiff-n.goalDiff||r.goalsFor-n.goalsFor)}async function Tn(t,e){const{state:i,navigate:n,toast:r}=e,o=i.params||{},a=o.leagueId||null,s=o.mlMatchId||null,l=i.user.id;if(!s||!a){r("Match introuvable","error"),n("mini-league");return}const{data:d,error:c}=await v.from("mini_league_matches").select("id, league_id, home_id, away_id, status, match_id").eq("id",s).single();if(c||!d){r("Match introuvable","error"),n("mini-league",{openLeagueId:a});return}if(d.home_id!==l&&d.away_id!==l){r("Vous ne faites pas partie de ce match","error"),n("mini-league",{openLeagueId:a});return}const u=d.home_id===l;if(d.match_id){await jt(t,e,d.match_id,u,{mlLeagueId:a,mlMatchId:s});return}await ki(t,e,"mini_league",async({deckId:b,gcCardsEnriched:x,gcDefs:g,stadiumDef:p})=>{dt(t);const f=async y=>{const{data:w,error:h}=await v.rpc("start_mini_league_match",{p_ml_match_id:s,p_user_id:l,p_deck_id:b});if(h||!(w!=null&&w.success)){console.error("[MiniLeague] start_mini_league_match error:",h||w),r((w==null?void 0:w.error)||"Impossible de lancer le match","error"),n("mini-league",{openLeagueId:a});return}if(w.matched){await jt(t,e,w.match_id,u,{mlLeagueId:a,mlMatchId:s,myGC:y||[],gcDefs:g,stadiumDef:p});return}await xa(t,e,{mlMatchId:s,leagueId:a,amIHome:u,chosenGC:y,gcDefs:g,stadiumDef:p})};if(!(x!=null&&x.length)){await f([]);return}wi(t,x,f)})}function xa(t,e,{mlMatchId:i,leagueId:n,amIHome:r,chosenGC:o,gcDefs:a,stadiumDef:s}){return new Promise(l=>{var p;const{navigate:d}=e;t.innerHTML=`
    <div style="min-height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;background:linear-gradient(135deg,#0a1a2e,#0d3d1e);color:#fff;padding:32px;text-align:center;gap:20px">
      <div style="font-size:36px">🏆</div>
    ...
    </div>`;let c=!1;const u=async f=>{if(!c){if(c=!0,clearInterval(g),x)try{x.unsubscribe()}catch{}f&&await jt(t,e,f,r,{mlLeagueId:n,mlMatchId:i,myGC:o||[],gcDefs:a,stadiumDef:s}),l()}};(p=document.getElementById("wait-back"))==null||p.addEventListener("click",()=>{if(c=!0,clearInterval(g),x)try{x.unsubscribe()}catch{}d("mini-league",{openLeagueId:n}),l()});const b=async()=>{if(c)return;const{data:f}=await v.from("mini_league_matches").select("match_id").eq("id",i).single();f!=null&&f.match_id&&u(f.match_id)},x=v.channel(`ml_match_${i}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"mini_league_matches",filter:`id=eq.${i}`},f=>{var y;(y=f.new)!=null&&y.match_id&&u(f.new.match_id)}).subscribe(),g=setInterval(b,3e3);b()})}const ba="/",ya=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function ha(t,e,i){let n=0;const r=document.createElement("div");r.id="tutorial-overlay",r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const o=()=>{var c,u,b;const s=e[n],l=n===e.length-1,d=Math.round((n+1)/e.length*100);r.innerHTML=`
      <div style="background:#fff;border-radius:20px;width:100%;max-width:420px;max-height:90vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 16px 64px rgba(0,0,0,0.4)">
        ...
        <!-- Image optionnelle -->
        ${s.image_url?`<div style="padding:0 24px 8px;text-align:center"><img src="${ba}icons/${s.image_url}" style="max-height:160px;max-width:100%;border-radius:12px;object-fit:contain"></div>`:""}
        <!-- Contenu -->
    ...
    `,r.querySelectorAll("ul,ol").forEach(x=>{x.style.paddingLeft="20px",x.style.marginTop="6px",x.style.marginBottom="6px"}),r.querySelectorAll("li").forEach(x=>{x.style.marginBottom="4px"}),r.querySelectorAll("p").forEach(x=>{x.style.marginBottom="8px"}),(c=r.querySelector("#tuto-prev"))==null||c.addEventListener("click",()=>{n--,o()}),(u=r.querySelector("#tuto-next"))==null||u.addEventListener("click",()=>{l?a():(n++,o())}),(b=r.querySelector("#tuto-skip"))==null||b.addEventListener("click",()=>{confirm("Passer le tutoriel ? Tu pourras le revoir plus tard depuis les paramètres.")&&a()})},a=async()=>{r.remove(),t!=null&&t.id&&await v.from("users").update({tutorial_done:!0}).eq("id",t.id),i==null||i()};document.body.appendChild(r),o()}async function va(t,e,i){if(!t||t.tutorial_done)return;let n=[];const{data:r,error:o}=await v.rpc("get_tutorial_steps");if(!o&&(r==null?void 0:r.length)>0)n=r,console.log(`[Tutorial] RPC OK → ${n.length} étapes`);else{const{data:s,error:l}=await v.from("tutorial_steps").select("*").eq("is_active",!0).order("step_order");!l&&(s==null?void 0:s.length)>0?(n=s,console.log(`[Tutorial] Direct OK → ${n.length} étapes`)):(console.warn(`[Tutorial] Aucune étape DB (RPC: ${o==null?void 0:o.message}, Direct: ${l==null?void 0:l.message})`),i&&i("[Tutorial] DB vide ou inaccessible — tuto local utilisé","warning",5e3))}const a=n.length>0?n.map(s=>({emoji:s.emoji,title:s.title,color:s.color,content:s.content,image_url:s.image_url||null})):ya;ha(t,a,()=>e("boosters"))}const In={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function ui(t,e,i=0){return e?(Number(e==="GK"?t.note_g:e==="DEF"?t.note_d:e==="MIL"?t.note_m:t.note_a)||0)+(e===t.job||e===t.job2?i:0):0}async function wa(t,e){t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',await nn(t,e)}async function nn(t,e){const{state:i,toast:n}=e,{data:r}=await v.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let l="buy";const d=()=>{var p,f,y,w,h,L,m;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((f=document.getElementById("flt-club"))==null?void 0:f.value)||"").toLowerCase().trim(),country:(((y=document.getElementById("flt-country"))==null?void 0:y.value)||"").toLowerCase().trim(),job:((w=document.getElementById("flt-job"))==null?void 0:w.value)||"",rarity:((h=document.getElementById("flt-rarity"))==null?void 0:h.value)||"",note1:parseInt((L=document.getElementById("flt-note1"))==null?void 0:L.value)||0,note2:parseInt((m=document.getElementById("flt-note2"))==null?void 0:m.value)||0}};function c(p){const f=d();return p.filter(y=>{var q,W,ee;const w=(q=y.card)==null?void 0:q.player;if(!w)return!1;const h=`${w.firstname} ${w.surname_real}`.toLowerCase(),L=(((W=w.clubs)==null?void 0:W.encoded_name)||"").toLowerCase(),m=(w.country_code||"").toLowerCase(),A=((ee=y.card)==null?void 0:ee.evolution_bonus)||0,E=ui(w,w.job,A),S=w.job2?ui(w,w.job2,A):0;return!(f.name&&!h.includes(f.name)||f.club&&!L.includes(f.club)||f.country&&!m.includes(f.country)||f.job&&w.job!==f.job||f.rarity&&w.rarity!==f.rarity||f.note1&&E<f.note1||f.note2&&S<f.note2)})}function u(p){var L,m,A;const f=(L=p.card)==null?void 0:L.player;if(!f)return"";const y=((m=p.card)==null?void 0:m.evolution_bonus)||0,w=(i.profile.credits||0)>=p.price;return`<div class="mkt-buy-tile">
      ${Se({...f,_evolution_bonus:y},{width:140})}
    ...
    </div>`}function b(p){var S,q,W,ee;const f=(S=p.card)==null?void 0:S.player;if(!f)return"";const y=((q=p.card)==null?void 0:q.evolution_bonus)||0,w=ui(f,f.job,y),h=f.job2?ui(f,f.job2,y):0,L=p.status==="sold",m=f.country_code?`https://flagsapi.com/${f.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,A=In[f.job]||"#bbb",E=f.job2?In[f.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${L?"opacity:0.7":""}">
      ${m?`<img src="${m}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(W=f.clubs)!=null&&W.logo_url?`<img src="${f.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
    ...
    </div>`}function x(){const p=document.getElementById("mkt-content"),f=document.getElementById("mkt-filters");if(p){if(f.style.display=l==="buy"?"flex":"none",l==="buy"){const y=c(a);p.innerHTML=y.length?`<div class="mkt-buy-grid">${y.map(u).join("")}</div>`:'<div style="text-align:center;color:#aaa;padding:40px">Aucune carte trouvée.</div>'}else{const y=s.filter(h=>h.status==="active").sort((h,L)=>new Date(L.listed_at)-new Date(h.listed_at)),w=s.filter(h=>h.status==="sold").sort((h,L)=>new Date(L.sold_at||L.listed_at)-new Date(h.sold_at||h.listed_at));p.innerHTML=(y.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:4px 0 6px;text-transform:uppercase;letter-spacing:1px">🟢 En vente (${y.length})</div>`+y.map(b).join(""):"")+(w.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:12px 0 6px;text-transform:uppercase;letter-spacing:1px">✅ Ventes réussies (${w.length})</div>`+w.map(b).join(""):"")+(!y.length&&!w.length?'<div style="text-align:center;color:#aaa;padding:40px">Aucune vente pour le moment.</div>':"")}p.querySelectorAll("[data-buy]").forEach(y=>y.addEventListener("click",()=>_a(y.dataset.buy,a,t,e))),p.querySelectorAll("[data-cancel]").forEach(y=>y.addEventListener("click",()=>$a(y.dataset.cancel,t,e)))}}t.querySelectorAll(".mkt-tab").forEach(p=>{p.addEventListener("click",()=>{l=p.dataset.tab,t.querySelectorAll(".mkt-tab").forEach(f=>{const y=f===p;f.style.background=y?"var(--green)":"var(--tile-bg)",f.style.color=y?"#fff":"var(--tile-fg-dim)",f.style.borderColor=y?"var(--green)":"var(--tile-border)"}),x()})});let g;["flt-name","flt-club","flt-country","flt-job","flt-rarity","flt-note1","flt-note2"].forEach(p=>{var f;(f=document.getElementById(p))==null||f.addEventListener("input",()=>{clearTimeout(g),g=setTimeout(x,250)})}),x()}async function _a(t,e,i,n){const{state:r,toast:o,refreshProfile:a}=n,s=e.find(c=>c.id===t);if(!s)return;const l=s.price;if((r.profile.credits||0)<l){o("Crédits insuffisants","error");return}ka(s,async()=>{const{error:c}=await v.rpc("buy_market_card",{p_listing_id:t,p_buyer_id:r.profile.id});if(c){o("Erreur achat : "+c.message,"error");return}await a();const u=document.getElementById("nav-credits");u&&(u.textContent=`💰 ${(r.profile.credits||0).toLocaleString("fr")}`),o("✅ Carte achetée !","success"),await nn(i,n)})}function ka(t,e){var a;const i=(a=t.card)==null?void 0:a.player,n=i?`${i.firstname} ${i.surname_real}`:"cette carte",r=document.createElement("div");r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",r.innerHTML=`
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      <div style="display:flex;gap:10px">
        ...
        <button id="buy-ok" style="flex:1;padding:12px;border-radius:10px;border:none;background:var(--green);color:#fff;font-size:14px;font-weight:900;cursor:pointer">Confirmer</button>
    ...
    </div>`,document.body.appendChild(r);const o=s=>{r.remove(),s&&e()};r.querySelector("#buy-cancel").addEventListener("click",()=>o(!1)),r.querySelector("#buy-ok").addEventListener("click",()=>o(!0)),r.addEventListener("click",s=>{s.target===r&&o(!1)})}async function $a(t,e,i){const{toast:n}=i,{data:r}=await v.from("market_listings").select("card_id").eq("id",t).single();if(await v.from("market_listings").update({status:"cancelled"}).eq("id",t),r!=null&&r.card_id){const{count:o}=await v.from("market_listings").select("id",{count:"exact",head:!0}).eq("card_id",r.card_id).eq("status","active");o||await v.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",r.card_id)}n("Annonce retirée","success"),nn(e,i)}async function Ea(t,e){var x,g,p,f,y;const{state:i,navigate:n}=e,r=((g=(x=e==null?void 0:e.state)==null?void 0:x.params)==null?void 0:g.tab)||"ranked";t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:var(--tile-fg-dim)">⚽ Chargement...</div>';const[{data:o},{data:a},{data:s}]=await Promise.all([v.from("users").select("id,pseudo,club_name,mmr,rank_tier,ranked_wins,ranked_losses,ranked_draws,placement_matches").gte("placement_matches",1).order("mmr",{ascending:!1}).limit(100),v.rpc("get_mini_league_leaderboard"),v.rpc("get_random_leaderboard")]);let l=r;const d=w=>w<3?["#D4A017","#a0a0a0","#cd7f32"][w]:"var(--green)",c=w=>w<3?"#000":"#fff";function u(){var L,m,A;const w=document.getElementById("rankings-legend");w&&(w.innerHTML=l==="random"?'<div style="font-size:11px;color:var(--tile-fg-dim);text-align:center;margin-bottom:10px">📊 = score de classement fiable (tient compte du nombre de matchs joués, pas seulement du %)</div>':"");const h=document.getElementById("rankings-list");if(h){if(l==="ranked"){const E=o||[];h.innerHTML=E.length>0?E.map((S,q)=>{const W=vi(S.mmr??1e3),ee=(S.ranked_wins||0)+(S.ranked_losses||0)+(S.ranked_draws||0),G=ee>0?Math.round((S.ranked_wins||0)/ee*100):0,I=S.id===i.profile.id,_=(S.placement_matches||0)<10;return`
          ...
          </div>`}).join(""):'<div style="text-align:center;color:var(--tile-fg-dim);padding:40px">Aucun joueur classé.</div>'}else if(l==="mini-league"){const E=a||[];h.innerHTML=E.length>0?E.map((S,q)=>`
        <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${S.user_id===i.profile.id?"border:2px solid var(--yellow)":""}">
          <div style="width:32px;height:32px;border-radius:50%;background:${d(q)};color:${c(q)};display:flex;align-items:center;justify-content:center;font-weight:900;flex-shrink:0;font-size:${q<3?"16":"13"}px">${q<3?["🥇","🥈","🥉"][q]:q+1}</div>
          <div style="flex:1;min-width:0">
  ...
  <div class="page">
  ...
  </div>`,u(),(p=document.getElementById("tab-ranked"))==null||p.addEventListener("click",()=>{l="ranked",u()}),(f=document.getElementById("tab-mini-league"))==null||f.addEventListener("click",()=>{l="mini-league",u()}),(y=document.getElementById("tab-random"))==null||y.addEventListener("click",()=>{l="random",u()})}async function La(t,e){var A,E,S,q;const{state:i,navigate:n,toast:r}=e,o=i.profile;t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const[{data:a},{data:s}]=await Promise.all([v.from("ranked_seasons").select("*").eq("is_active",!0).maybeSingle(),v.from("users").select("id,pseudo,club_name,mmr,mmr_deviation,mmr_volatility,rank_tier,placement_matches,ranked_wins,ranked_losses,ranked_draws").eq("id",o.id).single()]);if(!s){r("Erreur chargement profil","error"),n("home");return}if(!a){t.innerHTML=`
    ...
    </div>`,(A=document.getElementById("ranked-back"))==null||A.addEventListener("click",()=>n("home"));return}const l=s.mmr??1e3,d=s.mmr_deviation??350,c=s.mmr_volatility??.06,u=s.placement_matches??0,b=u<10,x=Math.max(0,10-u),g=vi(l),p=zn(l),f=Ti.findIndex(W=>W.id===g.id),y=Ti[f+1]||null,w={bronze:"linear-gradient(160deg,#3d1c00,#7a3e00)",silver:"linear-gradient(160deg,#1a1a2e,#3a3a5e)",gold:"linear-gradient(160deg,#1a1200,#4a3500)",platinum:"linear-gradient(160deg,#001a20,#003040)",diamond:"linear-gradient(160deg,#001030,#1a2860)",master:"linear-gradient(160deg,#1a0030,#3d0070)"},h=(s.ranked_wins||0)+(s.ranked_losses||0)+(s.ranked_draws||0),L=h>0?Math.round((s.ranked_wins||0)/h*100):0,m=Ti.map(W=>`
  ...
  <div class="page">
    ...
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.26-1106" style="cursor:pointer">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(o=>{o.addEventListener("click",a=>{a.preventDefault(),Nt(o.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>Nt("home")),document.getElementById("nav-credits").addEventListener("click",()=>Nt("boosters")),(n=document.getElementById("journal-btn"))==null||n.addEventListener("click",()=>Ma()),(r=document.getElementById("settings-btn"))==null||r.addEventListener("click",()=>Nt("settings"))}async function Ma(){const{data:t}=await v.from("patch_notes").select("*").eq("is_published",!0).order("published_at",{ascending:!1}).limit(20),e=document.createElement("div");e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px";const i=(t||[]).map(n=>{const r=new Date(n.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"long",year:"numeric"});return`<div style="padding:14px 0;border-bottom:1px solid #f0f0f0">
      ${n.image_url?`<img src="${n.image_url}" style="width:100%;max-height:160px;object-fit:cover;border-radius:8px;margin-bottom:10px">`:""}
      <div style="font-size:12px;color:#999;margin-bottom:4px">${r}</div>
      <div style="font-size:15px;font-weight:900;margin-bottom:6px">${n.title}</div>
      <div style="font-size:13px;color:#444;line-height:1.6;white-space:pre-line">${n.description}</div>
    ...
    </div>`,document.body.appendChild(e),e.querySelector("#journal-close").addEventListener("click",()=>e.remove()),e.addEventListener("click",n=>{n.target===e&&e.remove()})}function Ca(t,{onPlay:e}){var n;const i="/icons/";t.style.cssText="",t.innerHTML=`
  <style>
    #public-landing{
  ...
  <div id="public-landing">
  ...
  </div>`,(n=t.querySelector("#pl-play-btn"))==null||n.addEventListener("click",e)}async function ja(){document.documentElement.setAttribute("data-theme",po()),document.getElementById("modal-overlay").addEventListener("click",i=>{i.target===i.currentTarget&&Ri()}),document.getElementById("modal-close").addEventListener("click",Ri);const{data:{session:t}}=await v.auth.getSession();if(!t){Sn(),Ca(document.getElementById("app"),{onPlay:()=>pn(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Pt})});return}ye.user=t.user,await Zt(),Sn();try{const{data:i}=await v.from("formation_links_overrides").select("formation, links"),n={};(i||[]).forEach(r=>{n[r.formation]=r.links}),bo(n)}catch(i){console.warn("Impossible de charger les overrides de formation:",i)}if(!ye.profile){vo(document.getElementById("app"),{state:ye,navigate:async()=>{await Zt(),Ai()},toast:Pt,refreshProfile:Zt});return}const e=Array.isArray(ye.profile.pending_boosters)?ye.profile.pending_boosters:[];if(!ye.profile.onboarding_done&&e.length>0){kr(document.getElementById("app"),{state:ye,navigate:()=>Ai(),toast:Pt,refreshProfile:Zt});return}Ai(),setTimeout(()=>va(ye.profile,Nt,Pt),800),v.auth.onAuthStateChange(async(i,n)=>{i==="SIGNED_OUT"&&(ye.user=null,ye.profile=null,document.getElementById("app").innerHTML="",pn(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Pt}))})}function Ba(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function $i(){const t=document.getElementById("app");t&&(t.style.height=Ba()+"px")}window.addEventListener("resize",$i);window.addEventListener("orientationchange",()=>setTimeout($i,150));window.visualViewport&&window.visualViewport.addEventListener("resize",$i);function Ai(){const t=()=>{var i;(i=ye.user)!=null&&i.id&&v.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",ye.user.id).then(()=>{})};t(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(t,6e4);const e=document.getElementById("app");e.style.display="flex",e.style.flexDirection="column",$i(),Aa(),uo()}function Sn(){const t=document.getElementById("app-loader"),e=document.getElementById("app");e&&(e.style.display=""),t&&(t.classList.add("zoom-out"),setTimeout(()=>t.style.display="none",500))}function fo(t){var n;const e=document.getElementById("app-loader");if(e&&(e.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
    ...
    <button id="boot-retry" style="margin-top:8px;padding:12px 30px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer">Réessayer</button>`,document.body.appendChild(i),(n=document.getElementById("boot-retry"))==null||n.addEventListener("click",()=>window.location.reload())}ja().catch(t=>{console.error("Échec du démarrage:",t),fo()});setTimeout(()=>{const t=document.getElementById("app-loader");t&&t.style.display!=="none"&&!t.classList.contains("zoom-out")&&!document.getElementById("boot-error")&&fo("Le serveur met trop de temps à répondre.")},12e3);
