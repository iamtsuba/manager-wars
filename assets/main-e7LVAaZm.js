...
import{s as v,j as wi,q as So,t as Ut,u as Hi,F as Ui,o as tt,n as Rt,g as Mt,r as Se,m as wt,v as fi,w as fn,T as Ii,p as mn,x as gn,d as xn}from"./special-cards-lEGt-tGs.js";const bn="/";function po(t,{navigate:e,toast:i}){let o="login";const a=()=>{var r,s,d,l,c,f,b;const n=o==="login";if(t.innerHTML=`
    <div style="
  ...
  `;function s(){var g;const l=document.getElementById("logo-preview"),c=document.getElementById("logo-initials"),f=((g=document.getElementById("setup-club"))==null?void 0:g.value)||"MW",b=f.trim().split(" ").filter(Boolean),x=b.length>=2?(b[0][0]+b[1][0]).toUpperCase():f.slice(0,2).toUpperCase();l&&(l.style.background=r,l.style.borderColor=n),c&&(c.textContent=x,c.style.color=n)}document.getElementById("color1").addEventListener("input",l=>{n=l.target.value,document.getElementById("swatch1").style.background=n,s()}),document.getElementById("color2").addEventListener("input",l=>{r=l.target.value,document.getElementById("swatch2").style.background=r,s()});function d(l){document.querySelectorAll(".setup-step").forEach(c=>c.classList.remove("active")),document.getElementById(`step-${l}`).classList.add("active"),document.getElementById("step-num").textContent=l,document.getElementById("progress-fill").style.width=`${Math.round(l/3*100)}%`,l===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("step1-error");if(c.textContent="",l.length<3){c.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await v.from("users").select("id").eq("pseudo",l).maybeSingle();if(f){c.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const l=document.getElementById("setup-club").value.trim(),c=document.getElementById("step2-error");if(c.textContent="",l.length<2){c.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await v.from("users").select("id").eq("club_name",l).maybeSingle();if(f){c.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),b=document.getElementById("step3-finish");f.textContent="",b.disabled=!0,b.textContent="Création en cours…";try{const{error:x}=await v.from("users").insert({id:e.user.id,pseudo:l,club_name:c,club_color1:n,club_color2:r,credits:1e4});if(x)throw x;await hn(e.user.id),await a(),o(`Bienvenue ${l} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(x){f.textContent=x.message,b.disabled=!1,b.textContent="🚀 Créer mon profil !"}})}async function hn(t){const e=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await v.from("users").update({pending_boosters:e,onboarding_done:!1,first_booster_opened:!1}).eq("id",t)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const vn="modulepreload",wn=function(t){return"/"+t},uo={},Ao=function(e,i,o){let a=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),s=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));a=Promise.allSettled(i.map(d=>{if(d=wn(d),d in uo)return;uo[d]=!0;const l=d.endsWith(".css"),c=l?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${c}`))return;const f=document.createElement("link");if(f.rel=l?"stylesheet":vn,l||(f.as="script"),f.crossOrigin="",f.href=d,s&&f.setAttribute("nonce",s),document.head.appendChild(f),l)return new Promise((b,x)=>{f.addEventListener("load",b),f.addEventListener("error",()=>x(new Error(`Unable to preload CSS for ${d}`)))})}))}function n(r){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=r,window.dispatchEvent(s),!s.defaultPrevented)throw r}return a.then(r=>{for(const s of r||[])s.status==="rejected"&&n(s.reason);return e().catch(n)})},ti="#1A6B3C",ii="#cc2222",_n="#D4A017",fo="#888";async function kn(t,e){const{state:i,toast:o}=e;t.innerHTML=`
    ...
    </div>`,await zo(i,o,e),document.getElementById("btn-add-friend").addEventListener("click",()=>En(i,o)),document.getElementById("btn-accept-friend").addEventListener("click",()=>Mo(i,o,null,e))}async function zo(t,e,i={}){const{navigate:o}=i,a=t.user.id,{data:n,error:r}=await v.from("friendships").select("id, requester_id, addressee_id").eq("status","accepted").or(`requester_id.eq.${a},addressee_id.eq.${a}`),{count:s}=await v.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",a).eq("status","pending"),d=document.getElementById("pending-badge");d&&(s>0?(d.style.display="flex",d.textContent=s):d.style.display="none");const l=document.getElementById("friends-list");if(!l)return;if(r){console.error("[Friends] Erreur:",r),l.innerHTML=`<div style="color:${ii};text-align:center;padding:16px">Erreur chargement : ${r.message}</div>`;return}const c=(n||[]).map(x=>x.requester_id===a?x.addressee_id:x.requester_id);let f={};if(c.length){const{data:x}=await v.from("users").select("id, pseudo, club_name, last_seen_at, club_color1, club_color2").in("id",c);(x||[]).forEach(g=>{f[g.id]=g})}const b=(n||[]).map(x=>({friendshipId:x.id,friend:f[x.requester_id===a?x.addressee_id:x.requester_id]||{pseudo:"?"}}));if(!b.length){l.innerHTML=`
    ...
    ${Vi()}`,document.body.appendChild(l);const f=()=>l.remove();l.querySelector("#pending-close").addEventListener("click",f),l.addEventListener("click",b=>{b.target===l&&f()}),l.querySelectorAll("[data-accept]").forEach(b=>{b.addEventListener("click",async()=>{const{error:x}=await v.from("friendships").update({status:"accepted"}).eq("id",b.dataset.accept);if(x){e("Erreur : "+x.message,"error");return}b.closest("div[style]").remove(),e("✅ Ami accepté !","success"),zo(t,e,o),i&&i()})}),l.querySelectorAll("[data-decline]").forEach(b=>{b.addEventListener("click",async()=>{await v.from("friendships").delete().eq("id",b.dataset.decline),b.closest("div[style]").remove(),e("Demande refusée","info"),i&&i()})})}async function Ln(t,e,i){const o=t.user.id,[a,n]=[o,e].sort(),r=o===a,{data:s}=await v.from("friend_match_stats").select("*").eq("player1_id",a).eq("player2_id",n).single(),d=t.profile.club_name||t.profile.pseudo||"Moi",l=s||{},c=r?l.wins_p1||0:l.wins_p2||0,f=r?l.wins_p2||0:l.wins_p1||0,b=l.draws||0,x=r?l.goals_p1||0:l.goals_p2||0,g=r?l.goals_p2||0:l.goals_p1||0,p=r?l.gc_used_p1||0:l.gc_used_p2||0,u=r?l.gc_used_p2||0:l.gc_used_p1||0,y=l.matches_total||0,w=(L,m,M,E=ti,I=ii)=>`
    ...
    `,document.head.appendChild(l)}const a=In.map(l=>`
    ...
    `,document.body.appendChild(r),r.querySelector("#home2-mobtop-settings-btn").addEventListener("click",()=>t("settings")),r.querySelector("#home2-mobtop-credits").addEventListener("click",()=>t("boosters")));let s=document.getElementById("home2-mobile-bottom");s||(s=document.createElement("div"),s.id="home2-mobile-bottom",s.className="home2-mobile-bottom",s.innerHTML=a,document.body.appendChild(s)),document.querySelectorAll(".home2-chrome-tab").forEach(l=>{l._v2Bound||(l._v2Bound=!0,l.addEventListener("click",()=>{document.querySelectorAll(".home2-chrome-tab").forEach(c=>c.classList.remove("active")),document.querySelectorAll(`.home2-chrome-tab[data-key="${l.dataset.key}"]`).forEach(c=>c.classList.add("active")),t(l.dataset.route)}))}),document.querySelectorAll(".home2-chrome-tab").forEach(l=>l.classList.toggle("active",l.dataset.key===i));const d=`💰 ${(e.credits||0).toLocaleString("fr")}`;document.getElementById("home2-chrome-credits")&&(document.getElementById("home2-chrome-credits").textContent=d),document.getElementById("home2-mobtop-credits")&&(document.getElementById("home2-mobtop-credits").textContent=d),n.style.display="",r.style.display="",s.style.display="",requestAnimationFrame(()=>{requestAnimationFrame(()=>{const l=window.innerWidth<1024;document.documentElement.style.setProperty("--v2-top-height",(l?r.offsetHeight:n.offsetHeight)+"px"),document.documentElement.style.setProperty("--v2-bottom-height",s.offsetHeight+"px")})})}function Ji(){const t=document.getElementById("home2-chrome-header");t&&(t.style.display="none");const e=document.getElementById("home2-mobile-top");e&&(e.style.display="none");const i=document.getElementById("home2-mobile-bottom");i&&(i.style.display="none")}function Sn(t){if(!t)return"";const e=Date.now()-new Date(t).getTime(),i=Math.floor(e/6e4);if(i<1)return"à l'instant";if(i<60)return`il y a ${i}min`;const o=Math.floor(i/60);if(o<24)return`il y a ${o}h`;const a=Math.floor(o/24);return a<7?`il y a ${a}j`:new Date(t).toLocaleDateString("fr-FR",{day:"2-digit",month:"short"})}function An(t,e){if(!isFinite(e.max))return"";const i=e.max-e.min+1,o=Math.floor((t-e.min)/(i/3));return["III","II","I"][Math.min(2,Math.max(0,o))]}async function zn(t){const{data:e}=await v.from("users").select("id,pseudo,club_name,mmr").order("mmr",{ascending:!1}).limit(5),i=e||[],o=i.some(n=>n.id===t);let a=null;if(!o){const{data:n}=await v.from("users").select("mmr").eq("id",t).single();if(n){const{count:r}=await v.from("users").select("id",{count:"exact",head:!0}).gt("mmr",n.mmr||0);a=(r||0)+1}}return{list:i,iAmInTop:o,myPosition:a}}async function Mn(t=4){const{data:e}=await v.from("patch_notes").select("id,title,description,image_url,published_at").eq("is_published",!0).order("published_at",{ascending:!1}).limit(t);return e||[]}async function Cn(){const{data:t}=await v.from("booster_configs").select("id,name,icon,price_type,price_credits,card_count").eq("is_active",!0).order("created_at",{ascending:!1}).limit(5);return t||[]}async function jn(){const{data:t}=await v.from("patch_notes").select("*").eq("is_published",!0).order("published_at",{ascending:!1}).limit(20),e=document.createElement("div");e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px";const i=(t||[]).map(o=>{const a=new Date(o.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"long",year:"numeric"});return`<div style="padding:14px 0;border-bottom:1px solid #f0f0f0">
    ...
    </div>`,document.body.appendChild(e),e.querySelector("#journal-close-v2").addEventListener("click",()=>e.remove()),e.addEventListener("click",o=>{o.target===e&&e.remove()})}async function Si(t,{state:e,navigate:i,toast:o}){var L,m,M,E,I,D;const a=e.profile;if(!a)return;const n="/icons/";qo(i,a,"home2",n);const r=a.mmr??1e3,s=wi(r),d=An(r,s),l=So(r),c=isFinite(s.max)?s.max+1:null,[{list:f,iAmInTop:b,myPosition:x},g,p]=await Promise.all([zn(a.id),Mn(5),Cn()]),u=(O,Y)=>`
    <div class="rk-row ${O.id===a.id?"rk-row-me":""}">
      <div class="rk-pos ${Y<3?"rk-pos-top"+(Y+1):""}">${Y<3?["🥇","🥈","🥉"][Y]:Y+1}</div>
      <div class="rk-name ${O.id===a.id?"rk-name-me":""}">${O.pseudo}</div>
    ...
    <div class="news-item">
      ...
      <div class="news-body">
        ...
        <div class="news-time">${Sn(O.published_at)}</div>
      </div>
  ...
  <div class="home-dark" id="home-dark">
      ...
      <div class="home2-dash">
        ...
        <div class="home2-col-center">
          ...
          <div class="promo-banner" id="promo-banner">
            ...
            ${p.length>1?`<div class="promo-dots">${p.map((O,Y)=>`<div class="promo-dot ${Y===0?"active":""}" data-dot="${Y}"></div>`).join("")}</div>`:""}
          </div>`:""}
        </div>
  ...
  </div>`,requestAnimationFrame(()=>{var H,W;const O=((H=window.visualViewport)==null?void 0:H.height)||window.innerHeight,Y=window.innerWidth<1024;let J=0;if(Y){const ae=document.getElementById("home2-mobile-top"),le=document.getElementById("home2-mobile-bottom");J=((ae==null?void 0:ae.offsetHeight)||0)+((le==null?void 0:le.offsetHeight)||0)}else J=((W=document.getElementById("home2-chrome-header"))==null?void 0:W.offsetHeight)||0;const A=O-J,_=t.querySelector(".home-dark");_&&(_.style.minHeight=A+"px")}),(L=document.getElementById("nav-profile-btn"))==null||L.addEventListener("click",()=>i("settings")),(m=document.getElementById("nav-rankings-link"))==null||m.addEventListener("click",()=>i("rankings")),(M=document.getElementById("nav-rankings-cta"))==null||M.addEventListener("click",()=>i("rankings")),(E=document.getElementById("rank-inline-link-btn"))==null||E.addEventListener("click",()=>i("rankings")),(I=document.getElementById("nav-journal-link"))==null||I.addEventListener("click",()=>jn()),t.querySelectorAll("[data-action]").forEach(O=>{O.addEventListener("click",()=>{O.style.transform="scale(.96)",setTimeout(()=>O.style.transform="",180);const Y=O.dataset.action;if(Y==="match-ai"){Po(i,e);return}if(Y==="match-random"){Ji(),i("match",{matchMode:"random"});return}if(Y==="match-friend"){i("friends");return}if(Y==="mini-league"){i("mini-league");return}if(Y==="ranked"){i("ranked");return}o("Bientôt disponible","info")})}),p.length>1){const O=document.getElementById("promo-slide-content"),Y=()=>Array.from(document.querySelectorAll(".promo-dot")),J=A=>{w=A,O&&(O.innerHTML=h(p[w])),Y().forEach((_,H)=>_.classList.toggle("active",H===w))};setInterval(()=>J((w+1)%p.length),5e3),document.querySelectorAll(".promo-dot").forEach(A=>{A.addEventListener("click",()=>J(Number(A.dataset.dot)))})}(D=document.getElementById("promo-cta-btn"))==null||D.addEventListener("click",()=>i("boosters")),Fo(e,o),Fn(e,o,i),Do(e,o,i),Bn(e,o)}async function Bn(t,e){const i=t.profile.id,{data:o}=await v.from("mini_league_members").select("league_id, prize_amount, mini_leagues(name)").eq("user_id",i).eq("prize_claimed",!1).gt("prize_amount",0);if(!(o!=null&&o.length))return;const a=o.reduce((r,s)=>r+(s.prize_amount||0),0),n=document.createElement("div");n.className="modal-overlay",n.style.zIndex="2200",n.innerHTML=`<div class="modal" style="max-width:400px">
    ...
    </div>`}).join(""),o.querySelectorAll("[data-resume]").forEach(d=>{d.addEventListener("click",async()=>{const l=document.getElementById("page-content")||document.getElementById("app");if(d.dataset.mini==="1"){const{data:c}=await v.from("mini_league_matches").select("id, league_id").eq("match_id",d.dataset.resume).single();c?i("match-mini-league",{mlMatchId:c.id,leagueId:c.league_id}):i("mini-league")}else{const{resumePvpMatch:c}=await Ao(async()=>{const{resumePvpMatch:f}=await Promise.resolve().then(()=>Or);return{resumePvpMatch:f}},void 0);c(l,{state:t,navigate:i,toast:e,openModal:null,closeModal:null,refreshProfile:null},d.dataset.resume)}})}),o.querySelectorAll("[data-abandon]").forEach(d=>{d.addEventListener("click",()=>{Dn(async()=>{await qn(d.dataset.abandon,d.dataset.opp,a),e("Match abandonné (défaite 3-0)","info"),Do(t,e,i)})})})}async function qn(t,e,i){Gt();const{data:o}=await v.from("matches").select("home_id, away_id, game_state, mode").eq("id",t).single();if(!o)return;const a=o.home_id===i,n=a?0:3,r=a?3:0,s=o.game_state||{};s.p1Score=n,s.p2Score=r,s.phase="finished",s.forfeit=!0,await v.from("matches").update({status:"finished",forfeit:!0,winner_id:e,home_score:n,away_score:r,game_state:s}).eq("id",t),o.mode==="mini_league"&&await v.from("mini_league_matches").update({status:"finished",home_score:n,away_score:r}).eq("match_id",t)}function Dn(t){const e=document.createElement("div");e.className="modal-overlay",e.innerHTML=`<div class="modal" style="max-width:340px">
    ...
    <div class="modal-body" style="padding:16px">
  ...
  </div>`,document.body.appendChild(i);const o=()=>i.remove();document.getElementById("solo-cancel").addEventListener("click",o),i.addEventListener("click",l=>{l.target===i&&o()});const[{data:a},{data:n}]=await Promise.all([v.from("solo_levels").select("*").eq("is_active",!0).order("level_number"),v.from("user_solo_progress").select("unlocked_level").eq("user_id",e.profile.id).maybeSingle()]),r=(n==null?void 0:n.unlocked_level)||1,s=a||[],d=document.getElementById("solo-levels-list");if(d){if(!s.length){d.innerHTML='<div style="text-align:center;color:#999;padding:20px">Aucun niveau configuré.</div>';return}d.innerHTML=s.map(l=>{const c=l.level_number>r,f=l.level_number===r,b=c?"#f0f0f0":f?"#eefaf2":"#f7f7f7",x=c?"#ddd":f?"#bfe8cf":"#e0e0e0",g=c?"#999":"#12401f";return`
      ...
      </div>`}).join(""),d.querySelectorAll(".solo-level-card").forEach(l=>{l.dataset.locked!=="true"&&(l.addEventListener("mouseenter",()=>{l.style.transform="translateY(-1px)"}),l.addEventListener("mouseleave",()=>{l.style.transform=""}),l.addEventListener("click",()=>{o(),Ji(),t("match",{matchMode:"solo",soloLevel:Number(l.dataset.level)})}))})}}async function Pn(t,{state:e,navigate:i,toast:o}){const a=e.profile;if(!a)return;const n="/icons/";qo(i,a,"game",n),t.innerHTML=`
  <style>
    .game-page {
      min-height: 100%;
      background: var(--page-bg);
  ...
  </div>`,t.querySelectorAll("[data-action]").forEach(r=>{r.addEventListener("click",()=>{r.style.transform="scale(.96)",setTimeout(()=>r.style.transform="",180);const s=r.dataset.action;if(s==="match-ai"){Po(i,e);return}if(s==="match-random"){Ji(),i("match",{matchMode:"random"});return}if(s==="match-friend"){i("friends");return}if(s==="mini-league"){i("mini-league");return}if(s==="ranked"){i("ranked");return}o("Bientôt disponible","info")})})}async function Go(t,e){var f;const{state:i,navigate:o}=e,a=dn(),n=Yi(),r=(f=i==null?void 0:i.profile)==null?void 0:f.is_admin,s="2026.07.26-1839";t.innerHTML=`
  ...
  </div>`,t.querySelectorAll("[data-theme-choice]").forEach(b=>{b.addEventListener("click",()=>{va(b.dataset.themeChoice),Go(t,e)})});const d=t.querySelector("#volume-slider"),l=t.querySelector("#volume-label");let c=null;d.addEventListener("input",()=>{Tn(Number(d.value)),l.textContent=`${d.value}%`,c&&(c.volume=Math.max(0,Math.min(1,Number(d.value)/100)))}),t.querySelector("#volume-test").addEventListener("click",()=>{c=Xi("/sounds/match-opening.mp3",1)}),t.querySelector("#settings-logout").addEventListener("click",async()=>{await v.auth.signOut(),window.location.reload()})}const Ae={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé pour ce match."},"Double attaque":{icon:"⚡",desc:"La note d'attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la note d'un joueur adverse."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function Pe(t,e){if(!t)return 0;let i=0;switch(e){case"GK":i=Number(t.note_g)||0;break;case"DEF":i=Number(t.note_d)||0;break;case"MIL":i=Number(t.note_m)||0;break;case"ATT":i=Number(t.note_a)||0;break;default:i=0}return i+(t.boost||0)}const go=["ATT","MIL","DEF","GK"];function No(t,e,i){if(!t||!e)return!1;if(i&&t.position&&e.position)return Hi(i).some(([c,f])=>c===t.position&&f===e.position||c===e.position&&f===t.position);const o=t._col!=null&&e._col!=null&&t._col===e._col,a=t._col!=null&&e._col!=null&&Math.abs(t._col-e._col)===1,n=go.indexOf(t._line||t.job),r=go.indexOf(e._line||e.job),s=Math.abs(n-r)===1;return(t._line||t.job)===(e._line||e.job)&&a||o&&s}function Ro(t,e){let i=0;const o=t.length;for(let a=0;a<o;a++)for(let n=a+1;n<o;n++){const r=t[a],s=t[n];if(!r||!s||!No(r,s,e))continue;const d=Ut(r,s);d==="#00ff88"?i+=10:d==="#FFD700"&&(i+=5)}return i}function oi(t,e={},i){let o=0,a=0;t.forEach(d=>{const l=d._line||d.job,c=Number(l==="MIL"?d.note_m:d.note_a)||0,f=d.stadiumBonus&&(l==="MIL"||l==="ATT")?10:0;o+=e.doubleAttack?c*2:c,a+=(d.boost||0)+f});const n=o+a,r=Ro(t,i);let s=n+r;return e.stolenNote&&(s-=e.stolenNote),{base:n,links:r,total:Math.max(0,s)}}function ni(t,e={},i){const o=t.reduce((r,s)=>{const d=s._line||s.job;let l=0;d==="GK"?l=Number(s.note_g)||0:d==="MIL"?l=Number(s.note_m)||0:l=Number(s.note_d)||0;const c=s.stadiumBonus&&(d==="GK"||d==="DEF"||d==="MIL")?10:0;return r+l+(s.boost||0)+c},0),a=Ro(t,i);let n=o+a;return e.stolenNote&&(n-=e.stolenNote),{base:o,links:a,total:Math.max(0,n)}}function Qi(t,e,i={}){return i.shield?{goal:!1,shielded:!0}:{goal:t>e,shielded:!1}}function Oo(t,e,i="easy"){const o=t.filter(r=>!r.used);if(!o.length)return[];const a=[...o].sort((r,s)=>{const d=e==="attack"?Pe(r,"ATT"):r._line==="GK"?Pe(r,"GK"):Pe(r,"DEF");return(e==="attack"?Pe(s,"ATT"):s._line==="GK"?Pe(s,"GK"):Pe(s,"DEF"))-d});let n=i==="easy"?1+Math.floor(Math.random()*2):i==="medium"?2+Math.floor(Math.random()*2):3;return a.slice(0,Math.min(n,a.length,3))}function Gn(t,e){const i={vs_ai_easy:{victoire:500,nul:250,defaite:50},vs_ai_medium:{victoire:1e3,nul:500,defaite:50},vs_ai_hard:{victoire:1500,nul:750,defaite:100},vs_ai_club:{victoire:2500,nul:1250,defaite:100}};return(i[t]||i.vs_ai_easy)[e]||0}const Nn={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},Ai={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},si=["GK","DEF","MIL","ATT"],Rn=["Tous","GK","DEF","MIL","ATT"],On={normal:1e3,pepite:5e3,papyte:5e3,legende:1e4};function xo(t){const e=t.player;return e?(e.rarity,Math.max(Number(e.note_g)||0,Number(e.note_d)||0,Number(e.note_m)||0,Number(e.note_a)||0)):null}function Hn(t){return t.length?t.reduce((e,i)=>xo(i)>xo(e)?i:e,t[0]):t[0]}const Un={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function li(t,e){return t&&Number(e==="GK"?t.note_g:e==="DEF"?t.note_d:e==="MIL"?t.note_m:t.note_a)||0}function zi(t,e=""){const i=t.player;if(!i)return"";const o=t.evolution_bonus||0,a={...i,_evolution_bonus:o},n=e?`<div style="position:absolute;top:6px;right:6px;z-index:10;background:#0a3d1e;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 7px">${e}</div>`:"";return`<div style="position:relative;display:inline-block;cursor:pointer" data-card-id="${t.id}">
    ${n}
    ${Se(a,{width:140})}
  </div>`}function bo(t){return`<div style="display:inline-block;filter:grayscale(1);opacity:0.4">
    ${Se(t,{width:140})}
  </div>`}async function Kn(t,e){const{state:i,navigate:o,toast:a,openModal:n,closeModal:r}=e;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await v.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price, stadium_id, evolution_bonus,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),l=(s||[]).filter($=>$.card_type==="player"&&$.player),c=(s||[]).filter($=>$.card_type==="game_changer"),f=(s||[]).filter($=>$.card_type==="formation"),b=(s||[]).filter($=>$.card_type==="stadium"),{data:x}=await v.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(x||[]).forEach($=>{g[$.name]=$});const{data:p}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(p||[]).forEach($=>{u[$.id]=$}),b.forEach($=>{$.stadium_def&&(u[$.stadium_id]=$.stadium_def)});const y=Object.keys(Ui),w=Object.keys(Ae),h={};l.forEach($=>{const C=$.player.id;h[C]=(h[C]||0)+1}),new Set(Object.keys(h).map($=>String($)));const L=new Set(f.map($=>$.formation)),m=new Set(c.map($=>$.gc_type));let M="player",E="Tous",I="",D=!1;function O(){return[...l].sort(($,C)=>{const R=si.indexOf($.player.job),P=si.indexOf(C.player.job);return R!==P?R-P:($.player.surname_real||"").localeCompare(C.player.surname_real||"")})}function Y(){return[...d||[]].sort(($,C)=>{const R=si.indexOf($.job),P=si.indexOf(C.job);return R!==P?R-P:($.surname_real||"").localeCompare(C.surname_real||"")})}function J(){return O().filter($=>{const C=$.player,R=E==="Tous"||C.job===E,P=!I||`${C.firstname} ${C.surname_real}`.toLowerCase().includes(I);return R&&P})}function A(){return Y().filter($=>{const C=E==="Tous"||$.job===E,R=!I||`${$.firstname} ${$.surname_real}`.toLowerCase().includes(I);return C&&R})}t.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:0 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;align-items:center"></div>
  ...
  </div>`,function(C){const R=document.getElementById(C);if(!R)return;R.addEventListener("wheel",oe=>{Math.abs(oe.deltaY)<=Math.abs(oe.deltaX)||(oe.preventDefault(),R.scrollLeft+=oe.deltaY)},{passive:!1});let P=!1,X=0,q=0,K=!1,pe=!1;R.addEventListener("mousedown",oe=>{P=!0,K=!1,X=oe.pageX,q=R.scrollLeft}),window.addEventListener("mouseup",()=>{P=!1,K&&(R.style.cursor="",pe=!0),K=!1}),window.addEventListener("mousemove",oe=>{if(!P)return;const me=oe.pageX-X;!K&&Math.abs(me)<6||(K=!0,R.style.cursor="grabbing",oe.preventDefault(),R.scrollLeft=q-me)}),R.addEventListener("click",oe=>{pe&&(oe.stopPropagation(),oe.preventDefault(),pe=!1)},!0)}("col-grid");function _(){const $=document.getElementById("col-filters");$&&(M==="player"?($.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",C=>{I=C.target.value.toLowerCase(),H()}),t.querySelectorAll(".filter-btn").forEach(C=>{C.addEventListener("click",()=>{E=C.dataset.job,_(),H()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{D=!D,_(),H()})):($.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{D=!D,_(),H()})))}function H(){const $=document.getElementById("col-grid");$&&(M==="player"?U($):M==="formation"?de($):M==="stadium"?ue($):fe($))}function W($,C,R,P,X,q=0){const K=document.getElementById("col-grid"),pe=document.getElementById("col-big");if(!K||!pe)return;var oe=0;function me(){var pt;const we=window.innerWidth>=768,he=document.getElementById("col-big"),Te=document.getElementById("col-gap"),ze=((pt=document.getElementById("col-grid"))==null?void 0:pt.parentElement)||null,Be=he?he.closest(".page"):null;if(he&&ze&&Be){let Ee=0;Array.from(Be.children).forEach(function(B){B!==he&&B!==ze&&B!==Te&&(Ee+=B.offsetHeight)});const je=Math.max(0,Be.clientHeight-Ee),qe=Math.round(je*(q/100)),Ge=Math.max(0,je-qe),k=we?50/71:45/63,T=Math.round(Ge*k),z=Math.max(0,Ge-T);Te&&(Te.style.height=qe+"px"),he.style.flex="none",he.style.height=T+"px",he.style.minHeight="0",ze.style.flex="none",ze.style.height=z+"px",ze.style.minHeight="0",ze.style.display="flex",ze.style.overflow="hidden"}const $e=document.getElementById("col-grid");$e&&($e.style.height="100%",$e.style.flexShrink="0",$e.style.overflowX="auto",$e.style.overflowY="hidden",$e.style.alignItems="center",$e.style.width="100%"),pe.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:center center">'+C($[oe])+"</div>";var $t=pe.querySelector("[data-card-id],[data-form-id],[data-gc-id]");$t&&$t.addEventListener("click",function(){P($[oe])}),requestAnimationFrame(function(){var Ee=document.getElementById("big-card-inner");if(!(!Ee||!pe)){var je=pe.clientHeight,qe=pe.clientWidth-16,Ge=Ee.offsetHeight,k=Ee.offsetWidth;if(Ge>0&&k>0&&je>40){var T=Math.min(je/Ge,qe/k);Ee.style.transform="scale("+T.toFixed(3)+")",Ee.style.transformOrigin="center center"}}}),K.innerHTML=$.map(function(Ee,je){var qe=je===oe,Ge="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(qe?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+je+'" style="'+Ge+'">'+R(Ee,qe)+"</div>"}).join(""),K.querySelectorAll(".col-mini-item").forEach(function(Ee){Ee.addEventListener("click",function(){oe=Number(Ee.dataset.idx),me(),Ee.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})}),requestAnimationFrame(function(){var Ee=$e?$e.clientHeight:0,je=K.querySelector(".col-mini-item > div");if(!(!Ee||!je)){var qe=je.style.zoom;je.style.zoom="1";var Ge=je.offsetHeight;if(je.style.zoom=qe,!(Ge<=0)){var k=Ee/Ge;K.querySelectorAll(".col-mini-item > div").forEach(function(T){T.style.zoom=k.toFixed(4)})}}})}me()}function ae($,C){var R=window.innerWidth>=768?.76:.54,P;if(!$||$._fake){var X=$?$.player:null;if(!X)return"";P=bo(X)}else P=zi($,"");var q=$&&!$._fake?h[$.player&&$.player.id]||1:0,K=q>0?'<div style="position:absolute;top:6px;right:6px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:11px;font-weight:900;padding:2px 7px;z-index:3;box-shadow:0 1px 4px rgba(0,0,0,0.4)">×'+q+"</div>":"";return'<div style="display:inline-block;position:relative;zoom:'+R+';pointer-events:none;line-height:0">'+K+P+"</div>"}function le($,C,R){var P=R>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+R+"</div>":"",X=!!C,q=fi($,wt[$],{width:160});return X||(q='<div style="filter:grayscale(1);opacity:0.5">'+q+"</div>"),"<div "+(C?'data-form-id="'+C.id+'"':"")+' style="position:relative;cursor:pointer">'+P+q+"</div>"}function j($,C){var R=window.innerWidth>=768?.76:.54,P=fi($,wt[$],{width:140});return C||(P='<div style="filter:grayscale(1);opacity:0.45">'+P+"</div>"),'<div style="display:inline-block;zoom:'+R+';line-height:0;pointer-events:none">'+P+"</div>"}function U($){if(D){const C=A();if(!C.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucun joueur.</div>';return}const R=C.map(P=>l.find(X=>X.player.id===P.id)||{_fake:!0,player:P,id:"fake-"+P.id});W(R,P=>P._fake?bo(P.player):zi(P,""),P=>P._fake?ae({player:P.player,id:"x",_fake:!0}):ae(P),P=>{P._fake||yo(P,l,h,e)})}else{const C=J();if(!C.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const R={};C.forEach(X=>{const q=X.player.id;R[q]||(R[q]=[]),R[q].push(X)});const P=Object.values(R).map(X=>Hn(X));W(P,X=>{const q=h[X.player.id]||1,K=q>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${q}</div>`:"",oe=l.filter(me=>me.player.id===X.player.id&&me.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return zi(X,K+oe)},X=>ae(X),X=>yo(X,l,h,e))}}function de($){const C=D?y:[...L];if(!C.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const R=C.map(P=>({formation:P,card:f.find(X=>X.formation===P)||null,owned:L.has(P)}));W(R,({formation:P,card:X,owned:q})=>le(P,q?X:null,q?f.filter(K=>K.formation===P).length:0),({formation:P,owned:X})=>j(P,X),({card:P,owned:X})=>{X&&P&&Yn(P,f,e,n)},"#1A6B3C",5)}function fe($){const C=Object.keys(g),R=D?C.length?C:w:[...m];if(!R.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const P=R.map(X=>({type:X,gc:Ae[X]||{icon:"⚡",desc:""},def:g[X]||null,owned:m.has(X),card:c.find(q=>q.gc_type===X)||null}));W(P,({type:X,gc:q,def:K,owned:pe,card:oe})=>{var $e;const me=(K==null?void 0:K.name)||X,we=pe?c.filter($t=>$t.gc_type===X).length:0,he=we>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${we}</div>`:"",Te=(K==null?void 0:K.effect)||q.desc||"",ze=K!=null&&K.image_url?`/icons/${K.image_url}`:(($e=K==null?void 0:K.club)==null?void 0:$e.logo_url)||(K!=null&&K.country_code?`https://flagsapi.com/${K.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);let Be=tt(me,ze,q.icon,Te,{width:160,onClick:pe});return pe||(Be=`<div style="filter:grayscale(1);opacity:0.5">${Be}</div>`),`<div ${pe&&oe?`data-gc-id="${oe.id}" data-gc-type="${X}"`:""} style="position:relative">${he}${Be}</div>`},({type:X,gc:q,def:K,owned:pe})=>{const oe=window.innerWidth>=768?.76:.54,me=(K==null?void 0:K.name)||X,we=(K==null?void 0:K.effect)||q.desc||"",he=K!=null&&K.image_url?`/icons/${K.image_url}`:null;let Te=tt(me,he,q.icon,we,{width:140});return pe||(Te=`<div style="filter:grayscale(1);opacity:0.45">${Te}</div>`),`<div style="display:inline-block;zoom:${oe};line-height:0;pointer-events:none">${Te}</div>`},({type:X,owned:q,def:K})=>{q&&Vn(X,K,n)},"#7a28b8",5)}function ue($){const C="#4FC3F7",R="/";if(!b.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const P={};b.forEach(q=>{const K=q.stadium_id||"?";(P[K]=P[K]||[]).push(q)});const X=Object.entries(P).map(([q,K])=>({sid:q,def:u[q]||null,count:K.length,card:K[0]}));W(X,({def:q,count:K})=>{var Te,ze;const pe=(q==null?void 0:q.name)||"?",oe=((Te=q==null?void 0:q.club)==null?void 0:Te.encoded_name)||(q==null?void 0:q.country_code)||"—",me=q!=null&&q.image_url?`${R}icons/${q.image_url}`:((ze=q==null?void 0:q.club)==null?void 0:ze.logo_url)||(q!=null&&q.country_code?`https://flagsapi.com/${q.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),we=K>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${K}</div>`:"",he=`${oe}<br>+10 ⭐ joueurs alliés`;return`<div style="position:relative">${we}${Rt(pe,me,he,{width:160})}</div>`},({def:q})=>{var he,Te;const K=window.innerWidth>=768?.76:.54,pe=(q==null?void 0:q.name)||"?",oe=((he=q==null?void 0:q.club)==null?void 0:he.encoded_name)||(q==null?void 0:q.country_code)||"—",me=q!=null&&q.image_url?`${R}icons/${q.image_url}`:((Te=q==null?void 0:q.club)==null?void 0:Te.logo_url)||(q!=null&&q.country_code?`https://flagsapi.com/${q.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),we=`${oe}<br>+10 ⭐`;return`<div style="display:inline-block;zoom:${K};line-height:0;pointer-events:none">${Rt(pe,me,we,{width:140})}</div>`},null,C,5)}t.querySelectorAll(".col-tab-btn").forEach($=>{$.addEventListener("click",()=>{M=$.dataset.tab,E="Tous",I="",D=!1,t.querySelectorAll(".col-tab-btn").forEach(C=>{const R=C.dataset.tab===M;C.style.borderBottomColor=R?"var(--green)":"transparent",C.style.color=R?"var(--green)":"var(--tile-fg-dim)"}),_(),H()})}),_(),H()}function Vn(t,e,i){const o=Ae[t]||{icon:"⚡",desc:"Effet spécial."},a=(e==null?void 0:e.name)||t,n=(e==null?void 0:e.effect)||o.desc,r=e!=null&&e.image_url?`/icons/${e.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
      ${tt(a,r,o.icon,n,{width:200})}
      <div style="background:#fff3cd;border-radius:10px;padding:10px 14px;width:100%">
        <div style="font-size:12px;color:#856404">⚠️ Cette carte est à <b>usage unique</b>. Une fois jouée en match, elle est définitivement supprimée de ta collection.</div>
      </div>
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const di=1e3;function Yn(t,e,i,o){var g,p,u;const{state:a,toast:n,closeModal:r,navigate:s,refreshProfile:d}=i,l=t.formation,c={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function f(){const y=wt[l]||{},w=Ui[l]||[],h=290,L=360,m=20;function M(I){const D=y[I];return D?{x:D.x*h,y:D.y*L}:null}let E=`<svg width="${h}" height="${L}" viewBox="0 0 ${h} ${L}" xmlns="http://www.w3.org/2000/svg">`;for(const[I,D]of w){const O=M(I),Y=M(D);!O||!Y||(E+=`<line x1="${O.x}" y1="${O.y}" x2="${Y.x}" y2="${Y.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const I of Object.keys(y)){const D=M(I);if(!D)continue;const O=I.replace(/\d+/,""),Y=c[O]||"#555";E+=`<circle cx="${D.x}" cy="${D.y}" r="${m}" fill="${Y}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,E+=`<text x="${D.x}" y="${D.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${O}</text>`}return E+="</svg>",E}const b=e.filter(y=>y.formation===l);b.length;const x=!t.is_for_sale;o(`Formation ${l}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    <div style="margin-top:12px;border-top:1px solid var(--tile-border);padding-top:12px">
      ...
      <div style="display:flex;gap:8px">
        <input type="number" id="sell-price-form" min="1" placeholder="Prix en crédits" value="${di}"
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(g=document.getElementById("direct-sell-form-btn"))==null||g.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${l} pour ${di.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const y=b.find(h=>!h.is_for_sale)||b[0];if(!y){n("Aucune carte à vendre","error");return}await v.from("market_listings").delete().eq("card_id",y.id),await v.from("transfer_history").delete().eq("card_id",y.id);const{error:w}=await v.from("cards").delete().eq("id",y.id);if(w){n(w.message,"error");return}await v.from("users").update({credits:(a.profile.credits||0)+di}).eq("id",a.profile.id),await d(),n(`+${di.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),r(),s("collection")}),(p=document.getElementById("market-sell-form-btn"))==null||p.addEventListener("click",async()=>{const y=parseInt(document.getElementById("sell-price-form").value);if(!y||y<1){n("Prix invalide","error");return}await v.from("cards").update({is_for_sale:!0,sale_price:y}).eq("id",t.id),await v.from("market_listings").insert({seller_id:a.profile.id,card_id:t.id,price:y}),n("Carte mise en vente sur le marché !","success"),r(),s("collection")}),(u=document.getElementById("cancel-sell-form-btn"))==null||u.addEventListener("click",async()=>{await v.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",t.id),await v.from("market_listings").update({status:"cancelled"}).eq("card_id",t.id).eq("status","active"),n("Annonce retirée","success"),r(),s("collection")})}async function yo(t,e,i,o){var le,j,U,de,fe,ue;const{state:a,toast:n,openModal:r,closeModal:s,navigate:d,refreshProfile:l}=o,c=t.player,f=e.filter($=>$.player.id===c.id),b=f.length,x=t.evolution_bonus||0,g=Math.max((Number(c.note_g)||0)+(c.job==="GK"||c.job2==="GK"?x:0),(Number(c.note_d)||0)+(c.job==="DEF"||c.job2==="DEF"?x:0),(Number(c.note_m)||0)+(c.job==="MIL"||c.job2==="MIL"?x:0),(Number(c.note_a)||0)+(c.job==="ATT"||c.job2==="ATT"?x:0)),p=c.rarity||"normal",{data:u}=await v.from("sell_price_configs").select("*").eq("rarity",p).lte("note_min",g).gte("note_max",g).order("note_min",{ascending:!1}).limit(1);((le=u==null?void 0:u[0])==null?void 0:le.price)??On[p];const y=c.rarity!=="legende";Mt(c);const w=li(c,c.job)+x,h=c.job2?li(c,c.job2)+(li(c,c.job2)>0?x:0):null;Ai[c.job],c.job2&&Ai[c.job2];const L=Nn[c.rarity]||"#ccc";Un[c.country_code]||c.country_code;const m=t.evolution_bonus||0,E=w+m,I=h||0,D=I>0?I+m:0,O=f.map($=>$.id);let Y={};if(O.length){const{data:$}=await v.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",O).order("transferred_at",{ascending:!0});($||[]).forEach(C=>{Y[C.card_id]||(Y[C.card_id]=[]),Y[C.card_id].push(C)})}const J=O.length?`
    <div style="margin-top:16px;border-top:1px solid var(--tile-border);padding-top:14px">
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px">
        ${f.filter($=>$.id!==t.id).map(($,C)=>{const R=Y[$.id]||[],P=$.is_for_sale,X=R.length?R[R.length-1]:null,q=$.evolution_bonus||0,K=X?X.source==="booster"?"Booster":X.price?X.price.toLocaleString("fr")+" cr.":"—":"—",pe=X?new Date(X.transferred_at).toLocaleDateString("fr",{day:"2-digit",month:"2-digit",year:"numeric"}):"",oe=80,me=Math.round(oe*657/507),we=Se({...c,_evolution_bonus:q},{width:oe});return`
            <div class="exemplaire-row" data-card-id="${$.id}" data-card-idx="${C}"
              ...
              <input type="checkbox" class="expl-check"
                data-id="${$.id}" data-evo="${q}" data-note="${li(c,c.job)}"
                ${P?"disabled":""}
                style="display:none">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;background:#f0fdf4;border-radius:10px;padding:10px 14px">
          <div>
            ...
            <div style="font-size:11px;color:#555;margin-top:2px">
              ${(t.evolution_bonus||0)>0?`+${t.evolution_bonus} appliqué${t.evolution_bonus>1?"s":""} · Note : ${E}${c.job2&&I>0?` / ${D}`:""}`:`Note actuelle : ${E}${c.job2&&I>0?` / ${D}`:""}`}
            </div>
          </div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",c.note_g],["DEF",c.note_d],["MIL",c.note_m],["ATT",c.note_a]].map(([$,C])=>{const R=Ai[$],P=$==="GK"?"#fff":R,X=$===c.job||$===c.job2,q=(Number(C)||0)+(X&&m>0?m:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
    ...
    ${J}
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(j=document.getElementById("close-detail"))==null||j.addEventListener("click",s);let A=new Set;const _=c.rarity==="pepite"?1.3:c.rarity==="papyte"?.7:1;function H(){let $=0;return document.querySelectorAll(".expl-check:checked").forEach(C=>{if(C.dataset.id===t.id)return;const P=Number(C.dataset.evo)||0,X=Number(C.dataset.note)||0;$+=X+P}),Math.round($*_)}const W=()=>{const $=A.size,C=document.getElementById("sell-action-panel");if(!C)return;C.style.display=$>0?"block":"none",document.getElementById("sell-selected-count").textContent=$;const R=document.getElementById("evolve-btn");if(R){const P=H();R.textContent=P>0?`⬆️ Évoluer (+${P})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach($=>{$.addEventListener("change",()=>{const C=$.dataset.id;$.checked?A.add(C):A.delete(C);const R=$.closest(".exemplaire-row");if(R){const P=R.querySelector(".expl-sel-overlay"),X=R.querySelector(".expl-sel-check");P&&(P.style.display=$.checked?"block":"none"),X&&(X.style.display=$.checked?"flex":"none"),R.style.transform=$.checked?"scale(1.05)":"scale(1)"}W()})}),document.querySelectorAll(".exemplaire-row").forEach($=>{$.addEventListener("click",C=>{if(C.target.tagName==="INPUT")return;const R=$.querySelector(".expl-check");R&&!R.disabled&&(R.checked=!R.checked,R.dispatchEvent(new Event("change")))})}),(U=document.getElementById("evolve-btn"))==null||U.addEventListener("click",async()=>{if(b<=1)return;const $=[...A];if(!$.length)return;if(A.has(t.id)){const oe=document.createElement("div");oe.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",oe.innerHTML=`
        ...
        <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.3)">
          ...
          <div style="background:#f0fdf4;border-radius:10px;padding:12px;margin-bottom:16px;font-size:13px;color:#333">
            ...
            ➕ Brut : <strong>+${R}</strong>${X?`<span style="font-size:11px;color:#888"> ${X}</span>`:""}<br>
    ...
    </div>`,document.body.appendChild(o);const a=o.querySelector("#pm-input");a.focus(),a.select();const n=r=>{o.remove(),i(r)};o.querySelector("#pm-ok").addEventListener("click",()=>n(a.value.trim()||null)),o.querySelector("#pm-cancel").addEventListener("click",()=>n(null)),o.querySelector("#pm-cancel2").addEventListener("click",()=>n(null)),o.addEventListener("click",r=>{r.target===o&&n(null)}),a.addEventListener("keydown",r=>{r.key==="Enter"&&n(a.value.trim()||null),r.key==="Escape"&&n(null)})})}function Wn(t,e=!1){return new Promise(i=>{const o=document.createElement("div");o.className="modal-overlay",o.style.zIndex="2100",o.innerHTML=`<div class="modal" style="max-width:380px">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <div class="page-header" style="display:flex;align-items:center;gap:8px;padding:6px 12px;min-height:0">
      ...
      <div style="flex:1">
        ...
        <p style="font-size:11px;margin:0">${n}/11 · ${l.length}/5 rempl.</p>
      </div>
    </div>
    ...
    </div>`;u("⚽ Choisir une formation",w),document.querySelectorAll("#modal-body [data-forma]").forEach(h=>{h.addEventListener("click",()=>{e.formation=h.dataset.forma;const L=wo(e.formation),m={};L.forEach(M=>{e.slots[M]&&(m[M]=e.slots[M])}),e.slots=m,y(),_t(t,e,i)})})};t.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",x)),t.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>Jn(e,t,i))),t.querySelectorAll("#save-deck, #save-deck-pc").forEach(u=>u.addEventListener("click",()=>er(e,i))),t.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{e.subs=e.subs.filter(y=>y!==u.dataset.removeSub),_t(t,e,i)})}),t.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>Zn(e,t,i)))}function Xn(t,e,i,o){var h,L,m;const a=window.innerWidth>=900,n=t.querySelector(a?"#deck-field-pc":"#deck-field-mobile");if(!n)return;const r=(h=e.stadiumCards)==null?void 0:h.find(M=>M.id===e.stadiumCardId),s=r&&((L=e.stadDefMap)==null?void 0:L[r.stadium_id])||null,d=wt[e.formation]||{},l=Hi(e.formation)||[],c={};for(const M of i){const E=e.slots[M],I=E?e.playerCards.find(D=>D.id===E):null;I!=null&&I.player?c[M]={...I.player,_evolution_bonus:I.evolution_bonus||0,face:I.player.face||null}:c[M]=null}const f=window.innerWidth>=900,b=f?window.innerWidth-280:window.innerWidth-20,x=f?Math.min(b,860):b,g=Math.round(f?x*.82:x*.85),p=f?84:44;let u="";for(const[M,E]of l){const I=d[M],D=d[E];if(!I||!D)continue;const O=I.x*x,Y=Math.round(.03*g+I.y*.85*g),J=D.x*x,A=Math.round(.03*g+D.y*.85*g),_=c[M],H=c[E],W=Ut(_,H);W==="#ff3333"||W==="#cc2222"?u+=`<line x1="${O.toFixed(1)}" y1="${Y.toFixed(1)}" x2="${J.toFixed(1)}" y2="${A.toFixed(1)}" stroke="${W}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(u+=`<line x1="${O.toFixed(1)}" y1="${Y.toFixed(1)}" x2="${J.toFixed(1)}" y2="${A.toFixed(1)}" stroke="${W}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,u+=`<line x1="${O.toFixed(1)}" y1="${Y.toFixed(1)}" x2="${J.toFixed(1)}" y2="${A.toFixed(1)}" stroke="${W}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let y="";const w=Math.round(p*657/507);for(const M of i){const E=d[M];if(!E)continue;const I=c[M],D=E.x*x;E.y*g;const O=Math.round(.03*g+E.y*(.85*g)),Y=Math.round(D-p/2),J=Math.round(O-w/2);if(I){const A=M.replace(/\d+/,""),_=s&&(s.club_id&&String(I.club_id)===String(s.club_id)||s.country_code&&I.country_code===s.country_code),H=Se({...I,_evolution_bonus:I._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:A});_&&((m=s.club)!=null&&m.logo_url||s.image_url),y+=`<div style="position:absolute;left:${Y}px;top:${J}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${M}">
        <div style="position:relative">${H}</div>
      </div>`}else{const A=M.replace(/\d+/,"");y+=`<div style="position:absolute;left:${Y}px;top:${J}px;width:${p}px;height:${w}px;
    ...
    <div style="position:relative;width:${x}px;height:${g}px;margin:0 auto">
      ...
      ${y}
    </div>`,n.querySelectorAll(".deck-slot-hit").forEach(M=>{M.addEventListener("click",()=>Qn(M.dataset.pos,e,t,o))})}function Jn(t,e,i){var r;const{openModal:o,closeModal:a}=i,n=t.stadiumCards||[];o("🏟️ Choisir un stade",`<div style="display:flex;flex-wrap:wrap;gap:10px;padding:8px">
      ...
      ${x.length>0?'<div style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center">'+x.map(h=>{const L={...h.player,_evolution_bonus:h.evolution_bonus||0};return`<div class="player-option" data-card-id="${h.id}" style="cursor:pointer">
          ${Se(L,{width:100,showStad:!0,stadDef:d,role:r})}
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(y=document.getElementById("close-selector"))==null||y.addEventListener("click",n),(w=document.getElementById("remove-player"))==null||w.addEventListener("click",()=>{delete e.slots[t],n(),_t(i,e,o)}),document.querySelectorAll(".player-option").forEach(h=>{h.addEventListener("click",()=>{e.slots[t]=h.dataset.cardId,n(),_t(i,e,o)})})}function Zn(t,e,i){var c,f,b;const{openModal:o,closeModal:a}=i,n=(c=t.stadiumCards)==null?void 0:c.find(x=>x.id===t.stadiumCardId),r=n&&((f=t.stadDefMap)==null?void 0:f[n.stadium_id])||null,s=new Set;Object.keys(t.slots).forEach(x=>{var u;const g=t.slots[x];if(!g)return;const p=t.playerCards.find(y=>y.id===g);(u=p==null?void 0:p.player)!=null&&u.id&&s.add(p.player.id)}),t.subs.forEach(x=>{var p;const g=t.playerCards.find(u=>u.id===x);(p=g==null?void 0:g.player)!=null&&p.id&&s.add(g.player.id)});const d=new Set,l=t.playerCards.filter(x=>{var g,p,u;return s.has((g=x.player)==null?void 0:g.id)||d.has((p=x.player)==null?void 0:p.id)?!1:(d.add((u=x.player)==null?void 0:u.id),!0)});o("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-sub-selector">Fermer</button>'),(b=document.getElementById("close-sub-selector"))==null||b.addEventListener("click",a),document.querySelectorAll(".player-option").forEach(x=>{x.addEventListener("click",()=>{t.subs.push(x.dataset.cardId),a(),_t(e,t,i)})})}async function er(t,e){const{state:i,toast:o,navigate:a}=e,n=t.formationCards.find(d=>d.formation===t.formation),r=(n==null?void 0:n.id)||t.formationCardId;await v.from("decks").update({formation:t.formation,formation_card_id:r||null,stadium_card_id:t.stadiumCardId||null}).eq("id",t.deckId),await v.from("deck_cards").delete().eq("deck_id",t.deckId);const s=[];if(Object.entries(t.slots).forEach(([d,l],c)=>{s.push({deck_id:t.deckId,card_id:l,position:d,is_starter:!0,slot_order:c})}),t.subs.forEach((d,l)=>{s.push({deck_id:t.deckId,card_id:d,position:`SUB${l+1}`,is_starter:!1,slot_order:100+l})}),s.length>0){const{error:d}=await v.from("deck_cards").insert(s);if(d){o(d.message,"error");return}}o("Deck enregistré ✅","success"),a("decks")}function wo(t){const e=gi[t]||gi["4-4-2"],i=["GK1"];for(let o=1;o<=e.DEF;o++)i.push(`DEF${o}`);for(let o=1;o<=e.MIL;o++)i.push(`MIL${o}`);for(let o=1;o<=e.ATT;o++)i.push(`ATT${o}`);return i}async function Ho(t=null){const e=new Date().toISOString().slice(0,10),{data:i}=await v.from("booster_configs").select("*").eq("is_active",!0).order("sort_order");if(!(i!=null&&i.length))return[];const o=i.filter(r=>!(r.available_from&&e<r.available_from||r.available_until&&e>r.available_until));if(!o.length)return[];let a=o;if(t){const r=o.filter(s=>s.max_per_user!=null);if(r.length){const{data:s}=await v.from("booster_claims").select("booster_id").eq("user_id",t).in("booster_id",r.map(l=>l.id)),d={};(s||[]).forEach(l=>{d[l.booster_id]=(d[l.booster_id]||0)+1}),a=o.filter(l=>l.max_per_user==null?!0:(d[l.id]||0)<l.max_per_user)}}if(!a.length)return[];const{data:n}=await v.from("booster_drop_rates").select("*").in("booster_id",a.map(r=>r.id)).order("sort_order");return a.map(r=>({...r,rates:(n||[]).filter(s=>s.booster_id===r.id)}))}async function tr(t,e){const{data:i}=await v.from("booster_configs").select("max_per_user").eq("id",e).single();i!=null&&i.max_per_user&&await v.from("booster_claims").insert({user_id:t,booster_id:e})}function ir(t){if(!(t!=null&&t.length))return null;const e=t.reduce((o,a)=>o+Number(a.percentage),0);let i=Math.random()*e;for(const o of t)if(i-=Number(o.percentage),i<=0)return o;return t[t.length-1]}const Uo=()=>Object.keys(wt),or=[{id:"players_std",img:"/icons/booster-players.png",name:"Players",sub:"5 cartes joueurs",cost:5e3,costLabel:"5 000 crédits",cardCount:5,type:"player"},{id:"players_pub",img:"/icons/booster-silver.png",name:"Players (pub)",sub:"3 cartes joueurs",cost:0,costLabel:"1 pub",cardCount:3,type:"player"},{id:"game_changer",img:"/icons/booster-gamechanger.png",name:"Game Changer",sub:"3 cartes spéciales",cost:1e4,costLabel:"10 000 crédits",cardCount:3,type:"game_changer"},{id:"formation",img:"/icons/booster-formation.png",name:"Formation",sub:"1 carte formation",cost:1e4,costLabel:"10 000 crédits",cardCount:1,type:"formation"}],qi={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé."},"Double attaque":{icon:"⚡",desc:"La prochaine attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la prochaine action IA."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function nr(t){const e=t.player;if(!e)return"";const i=t.evolution_bonus||0;return Se({...e,_evolution_bonus:i},{width:140})}function Ko(t){var a;const e={};(t.rates||[]).forEach(n=>{e[n.card_type]=(e[n.card_type]||0)+Number(n.percentage||0)});const i=((a=Object.entries(e).sort((n,r)=>r[1]-n[1])[0])==null?void 0:a[0])||"player",o=t.image_url||"booster-players.png";return{id:t.id,img:"/icons/"+o,name:t.name,sub:`${t.card_count} carte(s)`,cost:t.price_type==="credits"&&t.price_credits||0,costLabel:t.price_type==="credits"?`${(t.price_credits||0).toLocaleString("fr")} crédits`:t.price_type==="pub"?"1 pub":"Gratuit",cardCount:t.card_count||5,type:i,isPub:t.price_type==="pub",rates:t.rates||[],allow_duplicates:t.allow_duplicates!==!1,_boosterId:t.id,_raw:t}}async function rr(t,{state:e,navigate:i,toast:o}){var f,b,x;const a=((f=e.profile)==null?void 0:f.credits)||0;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⏳ Chargement...</div>';let n=[];try{n=(await Ho((b=e.user)==null?void 0:b.id)).map(Ko)}catch(g){console.warn("Erreur chargement boosters DB, fallback hardcodé",g)}n.length||(n=or.map(g=>({...g,rates:[],isPub:g.id==="players_pub"})));const r=await v.from("cards").select("player_id, card_type, formation, stadium_id, gc_type").eq("owner_id",e.profile.id).then(g=>g.data||[]),s=new Set(r.filter(g=>g.card_type==="stadium").map(g=>g.stadium_id)),d=new Set(r.filter(g=>g.card_type==="formation").map(g=>g.formation)),l=new Set(r.filter(g=>g.card_type==="game_changer").map(g=>g.gc_type)),c={};for(const g of n){if(g.allow_duplicates!==!1||!((x=g.rates)!=null&&x.length))continue;const p=[...new Set((g.rates||[]).map(y=>y.card_type))];let u=!1;for(const y of p)if(y==="stadium"){const{data:w}=await v.from("stadium_definitions").select("id");if((w||[]).some(h=>!s.has(h.id))){u=!0;break}}else if(y==="game_changer"){const{data:w}=await v.from("gc_definitions").select("name").eq("is_active",!0);if((w||[]).some(h=>!l.has(h.name))){u=!0;break}}else if(y==="formation"){const{FORMATION_LINKS:w}=await Ao(async()=>{const{FORMATION_LINKS:h}=await import("./special-cards-lEGt-tGs.js").then(L=>L.y);return{FORMATION_LINKS:h}},__vite__mapDeps([0,1]));if(Object.keys(w).some(h=>!d.has(h))){u=!0;break}}else{u=!0;break}u||(c[g.id]=!0)}t.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📦 Boosters</h2>
  ...
  `,document.body.appendChild(a);let n=!1;const r=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let d=!1;const l=_=>_.touches&&_.touches[0]?_.touches[0].clientX:_.clientX;function c(_){n||(d=!0,s.style.opacity="1",f(_))}function f(_){if(!d||n)return;const H=r.getBoundingClientRect(),W=l(_)-H.left,ae=Math.max(0,Math.min(1,W/H.width));s.style.width=ae*H.width+"px",ae>=.82&&x()}function b(){n||(d=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{n||(s.style.transition="")},220))}function x(){var H;if(n)return;n=!0,d=!1,s.style.width="100%",s.style.opacity="1",(H=document.getElementById("cut-flash"))==null||H.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const _=document.getElementById("cut-hint");_&&(_.style.opacity="0"),r.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",y(0)},620)}r.addEventListener("pointerdown",c),window.addEventListener("pointermove",f),window.addEventListener("pointerup",b),r.addEventListener("touchstart",c,{passive:!0}),window.addEventListener("touchmove",f,{passive:!0}),window.addEventListener("touchend",b);let g=0,p=!1;const u=new Set;function y(_){g=_,document.getElementById("reveal-phase").style.display="flex",w(),h(_,0),I()}function w(){const _=document.getElementById("card-dots");_&&(_.innerHTML=t.map((H,W)=>`<div class="card-dot" data-i="${W}" style="width:8px;height:8px;border-radius:50%;background:${W===g?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),_.querySelectorAll(".card-dot").forEach(H=>H.addEventListener("click",()=>m(parseInt(H.dataset.i)))))}function h(_,H){var $,C;const W=t[_],ae=document.getElementById("card-counter"),le=document.getElementById("card-track");ae&&(ae.textContent=`Carte ${_+1} / ${t.length}`);const j=document.getElementById("reveal-btns");j&&(j.style.display=_===t.length-1?"flex":"none");const U=W.card_type==="player"&&(($=W.player)==null?void 0:$.rarity)==="legende",de=!u.has(_);u.add(_);let fe=0;if(W.card_type==="player"&&W.player){const R=W.player,P=R.job||"ATT";fe=(Number(P==="GK"?R.note_g:P==="DEF"?R.note_d:P==="MIL"?R.note_m:R.note_a)||0)+(W.evolution_bonus||0)}const ue=R=>{le.innerHTML=`
        <div id="current-card-wrap" style="position:relative;display:flex;flex-direction:column;align-items:center;gap:8px;${U?"filter:drop-shadow(0 0 20px #7a28b8)":""}">
          <div style="transform:scale(1.25);transform-origin:center">${dr(W)}</div>
        ...
        </div>`;const P=document.getElementById("current-card-wrap");H!==0?(P.style.transition="none",P.style.transform=`translateX(${H>0?100:-100}%)`,requestAnimationFrame(()=>{P.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",P.style.transform="translateX(0)"})):P.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),R||U?O():Y(),w()};de&&(((C=W.player)==null?void 0:C.rarity)==="legende"||fe>=18)&&W.card_type==="player"&&W.player?L(W,()=>ue(!0)):ue(!1)}function L(_,H){var we;p=!0;const W=_.player,ae=`https://flagsapi.com/${W.country_code}/flat/64.png`,le=(we=W.clubs)==null?void 0:we.logo_url,j=Mt(W),U=W.job||"ATT",de=Number(U==="GK"?W.note_g:U==="DEF"?W.note_d:U==="MIL"?W.note_m:W.note_a)||0,fe=_.evolution_bonus||0,ue=de+fe,$=ue>=18&&ue<=20,C=ue>=18,R=document.getElementById("walkout-overlay"),P=document.getElementById("walkout-stage");if(!R||!P){p=!1,H();return}let X=null;$&&(X=Xi("/sounds/Legendary.mp3",.8)),R.style.display="flex";const q=()=>{const he=P.firstElementChild;he&&(he.classList.remove("wo-in"),he.classList.add("wo-out"))},K=1800,pe=400;P.innerHTML=`<img class="wo-in" src="${ae}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(q,K),setTimeout(()=>{var he;P.innerHTML=le?`<img class="wo-in" src="${le}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((he=W.clubs)==null?void 0:he.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},K+pe),setTimeout(q,K*2+pe),setTimeout(()=>{P.innerHTML=j?`<img class="wo-in" src="${j}" style="height:200px;border-radius:50%;box-shadow:0 0 40px rgba(255,255,255,0.3);object-fit:cover;object-position:top">`:'<div class="wo-in" style="font-size:80px">👤</div>',navigator.vibrate&&navigator.vibrate(30)},(K+pe)*2),setTimeout(q,(K+pe)*2+K);const me=C?"#FFD700":{GK:"#c0c0c0",DEF:"#e03030",MIL:"#D4A017",ATT:"#3fbf5f"}[U]||"#fff";setTimeout(()=>{P.innerHTML=`<div class="wo-in" style="
      ...
      </div>`,document.body.appendChild(o),o.addEventListener("click",r=>{r.target===o&&o.remove()}),(i=document.getElementById("odds-close"))==null||i.addEventListener("click",()=>o.remove());return}pr()}function pr(){const t=document.createElement("div");t.style.cssText=`position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;
  ...
  `,document.body.appendChild(t),t.addEventListener("click",e=>{e.target===t&&t.remove()}),document.getElementById("odds-close").addEventListener("click",()=>t.remove())}function ur(){return new Promise(t=>{const e=document.createElement("div");e.style.cssText=`
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
  ...
  </div>`;const c=async()=>{await v.from("users").update({pending_boosters:r}).eq("id",e.user.id)};async function f(){var w;if(l>=d||!r.length){await v.from("users").update({pending_boosters:[],onboarding_done:!0}).eq("id",e.user.id),a&&await a(),o("Tous tes boosters sont ouverts ! Bon jeu 🎮","success",4e3),i("home");return}const g=r[0],{data:p}=await v.from("users").select("*").eq("id",e.user.id).single();p&&(e.profile=p);let u=[];try{if(g.type==="formation")u=await Wo(e.profile,0);else if(g.type==="game_changer")u=await Yo(e.profile,g.count||3,0);else if(s&&((w=s.rates)!=null&&w.length)){const h={...s,cost:0,cardCount:g.count||s.cardCount||5};u=await Di(e.profile,h),g.guaranteeGK&&!e.profile.first_booster_opened&&(u.some(m=>m.player&&m.player.job==="GK")||await mr(e.profile,u),await v.from("users").update({first_booster_opened:!0}).eq("id",e.profile.id))}else u=await Vo(e.profile,g.count||5,0)}catch(h){o(h.message||"Erreur ouverture booster","error");return}r.shift(),l++,await c();const y=g.type==="formation"?{name:"Booster Formation",img:"/icons/booster-formation.png"}:g.type==="game_changer"?{name:"Booster Game Changer",img:"/icons/booster-gamechanger.png"}:{name:`Booster Joueurs (${l}/${d})`,img:(s==null?void 0:s.img)||"/icons/booster-players.png"};Xo(u,y,i,()=>{f()})}(x=document.getElementById("onboard-start"))==null||x.addEventListener("click",()=>f())}async function mr(t,e){try{const{data:i}=await v.from("players").select("id,job,firstname,surname_real,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0).eq("job","GK");if(!(i!=null&&i.length))return;const o=i[Math.floor(Math.random()*i.length)],a=e.findIndex(r=>r.player);if(a===-1)return;const n=e[a];await v.from("cards").update({player_id:o.id}).eq("id",n.id),e[a]={...n,player_id:o.id,player:o}}catch(i){console.warn("[Onboarding] ensureGK échec",i)}}const kt={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},gr={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function St(t,e,i,o,a){var n;t.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
    <div style="text-align:center;padding:40px;color:#fff">
      <div style="font-size:48px;margin-bottom:16px">${e}</div>
      <p style="margin-bottom:16px">${i}</p>
      <button class="btn btn-primary" id="msg-btn">${o}</button>
    </div>
  </div>`,(n=document.getElementById("msg-btn"))==null||n.addEventListener("click",a)}function Fi(t,e){var n,r;const i=t.player,o=t.evolution_bonus||0,a=i.job2&&Number(i[`note_${i.job2.toLowerCase()}`])||0;return{cardId:t.id,position:e||null,id:i.id,firstname:i.firstname,name:i.surname_real,country_code:i.country_code,club_id:i.club_id,job:i.job,job2:i.job2,note_g:(Number(i.note_g)||0)+(i.job==="GK"?o:0)+(i.job2==="GK"&&a>0?o:0),note_d:(Number(i.note_d)||0)+(i.job==="DEF"?o:0)+(i.job2==="DEF"&&a>0?o:0),note_m:(Number(i.note_m)||0)+(i.job==="MIL"?o:0)+(i.job2==="MIL"&&a>0?o:0),note_a:(Number(i.note_a)||0)+(i.job==="ATT"?o:0)+(i.job2==="ATT"&&a>0?o:0),evolution_bonus:o,rarity:i.rarity,skin:i.skin,hair:i.hair,hair_length:i.hair_length,face:i.face||null,clubName:((n=i.clubs)==null?void 0:n.encoded_name)||null,clubLogo:((r=i.clubs)==null?void 0:r.logo_url)||null,boost:0,used:!1,_line:null,_col:null}}function ri(t,e){if(!e||!t)return t;const{club_id:i,country_code:o}=e;return Object.values(t).forEach(a=>{Array.isArray(a)&&a.forEach(n=>{const r=i&&String(n.club_id)===String(i),s=o&&String(n.country_code)===String(o);(r||s)&&(n.stadiumBonus=!0)})}),t}function xi(t,e){if(!e||!(t!=null&&t.length))return t;const{club_id:i,country_code:o}=e;return t.forEach(a=>{if(!a)return;const n=i&&String(a.club_id)===String(i),r=o&&String(a.country_code)===String(o);(n||r)&&(a.stadiumBonus=!0)}),t}function st(t){return t===1?[1]:t===2?[0,2]:t===3?[0,1,2]:t===4?[0,1,1,2]:t===5?[0,1,1,1,2]:[1]}function Jo(){const t=Math.random()*100;return t<10?10:t<30?5:3}function bi(t,e){const i=kt[e]||kt["4-4-2"],o={GK:[],DEF:[],MIL:[],ATT:[]};if(t.length&&t.every(r=>r.position)){for(const r of["GK","DEF","MIL","ATT"]){const s=t.filter(l=>l.position&&l.position.replace(/\d+$/,"")===r).sort((l,c)=>parseInt(l.position.replace(/\D+/g,""),10)-parseInt(c.position.replace(/\D+/g,""),10)).map(l=>({...l,_line:r})),d=st(s.length);s.forEach((l,c)=>{l._col=d[c]}),o[r]=s}return o}const n=[...t];for(const r of["GK","DEF","MIL","ATT"]){const s=[];for(let l=0;l<i[r];l++){let c=n.findIndex(f=>f.job===r);if(c===-1&&(c=n.findIndex(f=>f.job2===r)),c===-1&&(c=0),n[c]){const f={...n[c],_line:r};s.push(f),n.splice(c,1)}}const d=st(s.length);s.forEach((l,c)=>{l._col=d[c]}),o[r]=s}return o}function lt(t){document.querySelectorAll(".top-nav, .bottom-nav").forEach(e=>{e.style.setProperty("display","none","important"),e.dataset.matchHidden="1"}),t&&t.style.setProperty("padding-bottom","0","important")}function it(t){document.querySelectorAll(".top-nav, .bottom-nav").forEach(e=>{e.style.removeProperty("display"),delete e.dataset.matchHidden}),t&&t.style.removeProperty("padding-bottom")}function _i(t,e,i){const a=new Set,n=e.filter(c=>{const f=c.gc_type||c.id;return a.has(f)?!1:(a.add(f),!0)});let r=[];function s(c,f){const b=c._gcDef,x=(b==null?void 0:b.name)||c.gc_type,g=b!=null&&b.image_url?`/icons/${b.image_url}`:null,p=tt(x,g,"⚡",(b==null?void 0:b.effect)||"",{width:100});return`<div class="gc-select-card" data-id="${c.id}"
    ...
    </div>`}const d=c=>{t.style.overflow="",t.style.height="",t.style.display="",t.style.flexDirection="",i(c)};function l(){var f,b,x;t.style.overflow="hidden",t.style.height="100%",t.style.display="flex",t.style.flexDirection="column";const c=r.length>0;t.innerHTML=`
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",r).order("slot_order"),d=[...new Set((s||[]).filter(b=>{var x,g;return((x=b.card)==null?void 0:x.card_type)==="stadium"&&((g=b.card)==null?void 0:g.stadium_id)}).map(b=>b.card.stadium_id))],l={};if(d.length){const{data:b}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",d);(b||[]).forEach(x=>{l[x.id]=x}),(s||[]).forEach(x=>{var g,p;((g=x.card)==null?void 0:g.card_type)==="stadium"&&((p=x.card)!=null&&p.stadium_id)&&(x.card._stadiumDef=l[x.card.stadium_id]||null)})}let c=0;function f(){var m,M,E,I,D,O,Y;const b=n[c],x=(s||[]).filter(J=>J.deck_id===b.id),g=x.filter(J=>{var A;return J.is_starter&&((A=J.card)==null?void 0:A.player)}).map(J=>Fi(J.card,J.position)),p=x.find(J=>{var A;return((A=J.card)==null?void 0:A.card_type)==="formation"}),u=b.formation||((m=p==null?void 0:p.card)==null?void 0:m.formation)||"4-4-2";let y=g.length>=11?bi(g,u):null,w=((M=b.stadium_card)==null?void 0:M.stadium_def)||null;w&&y&&(y=ri(y,w));const h=g.length>=11;lt(t),t.style.height="100%",t.style.overflow="hidden",t.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <div id="deck-top-bar" style="flex-shrink:0">
        <div style="padding:8px 16px;background:rgba(0,0,0,0.4);text-align:center">
          <div style="font-size:10px;opacity:.6;letter-spacing:2px;text-transform:uppercase">${xr(i,e)}</div>
      ...
      <!-- Pagination -->
      ${n.length>1?`<div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">${n.map((J,A)=>`<div style="width:6px;height:6px;border-radius:50%;background:${A===c?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}</div>`:""}
      ...
      <div id="deck-bottom-bar" style="flex-shrink:0;padding:10px 14px 14px;display:flex;flex-direction:column;gap:8px;background:rgba(0,0,0,0.2)">
    ...
    </div>`,requestAnimationFrame(()=>requestAnimationFrame(function J(){const A=t.querySelector(".deck-preview-wrap"),_=t.querySelector("#deck-swipe-zone");if(!A||!_||!y)return;const H=_.clientWidth>=900,W=Math.max(200,_.clientHeight-(H?60:40)),le=Math.max(200,_.clientWidth-16),j=H?Math.min(117,Math.max(52,Math.round(le*.22))):Math.max(44,Math.round(le*.168));if(W<220||le<220){requestAnimationFrame(J);return}const U=H?null:Math.round(j*.55);A.innerHTML=Ot(y,u,null,[],le,W,[],U),A.style.cssText=`width:${le}px;height:${W}px;overflow:hidden;margin:${H?0:60}px auto 0`;const de=A.querySelector("svg");de&&(de.style.cssText="display:block;width:100%;height:100%",de.setAttribute("preserveAspectRatio",H?"xMidYMid meet":"none"))})),(I=document.getElementById("prev-deck"))==null||I.addEventListener("click",()=>{c>0&&(c--,f())}),(D=document.getElementById("next-deck"))==null||D.addEventListener("click",()=>{c<n.length-1&&(c++,f())}),(O=document.getElementById("validate-deck"))==null||O.addEventListener("click",()=>{if(!h)return;const J=e.state.params||{};e.navigate("match",{...J,matchMode:J.matchMode||i,deckId:b.id})}),(Y=document.getElementById("cancel-deck-select"))==null||Y.addEventListener("click",()=>{it(t),a("home")});const L=document.getElementById("deck-swipe-zone");if(L){let J=0,A=0;L.addEventListener("touchstart",_=>{J=_.touches[0].clientX,A=_.touches[0].clientY},{passive:!0}),L.addEventListener("touchend",_=>{const H=_.changedTouches[0].clientX-J,W=_.changedTouches[0].clientY-A;Math.abs(H)<40||Math.abs(H)<Math.abs(W)||(H<0&&c<n.length-1?(c++,f()):H>0&&c>0&&(c--,f()))},{passive:!0})}}f()}function vt(t,e=44,i=58,o=null){return Se(t,{width:e,showStad:!!o,stadDef:o,used:t==null?void 0:t.used})}function Je(t,e,i,o,a){if(!(t!=null&&t.length))return"";const n=t.slice(0,5);let r='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return n.forEach((s,d)=>{const l=s._line||s.job||"MIL";let c=s.boost||0;if(s.stadiumBonus&&(o==="attack"&&(l==="ATT"||l==="MIL")||o==="defense"&&(l==="GK"||l==="DEF"||l==="MIL")?c+=10:o||(c+=10)),r+=Se(s,{width:40,role:l,extraNote:c}),d<n.length-1){const f=n[d+1],b=No(s,f,a)?Ut(s,f):null;r+=`<div style="width:7px;height:3px;background:${!b||b==="#ff3333"||b==="#cc2222"?"rgba(255,255,255,0.12)":b};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(r+=`<div style="margin-left:6px;background:${e};color:${e==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),r+="</div>",r}function ki(t,e,i,o,a=310,n=310,r=[],s=null){const d=wt[e]||{},l=Hi(e)||Ui[e]||[],c={},f=["ATT","MIL","DEF","GK"];for(const y of f)(t[y]||[]).forEach((h,L)=>{c[`${y}${L+1}`]=h});function b(y){const w=d[y];return w?{x:w.x*a,y:w.y*n}:null}let x="";for(const[y,w]of l){const h=b(y),L=b(w);if(!h||!L)continue;const m=c[y],M=c[w],E=Ut(m,M);E==="#00ff88"||E==="#FFD700"?(x+=`<line x1="${h.x.toFixed(1)}" y1="${h.y.toFixed(1)}" x2="${L.x.toFixed(1)}" y2="${L.y.toFixed(1)}"
        ...
        stroke="${E}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=typeof window<"u"&&window.innerWidth>=900?Math.min(Math.max(81,Math.round(a*.225)),117):Math.max(44,Math.round(a*.168)),p=Math.round(g*657/507);for(const[y,w]of Object.entries(c)){const h=b(y);if(!h||!w)continue;const L=y.replace(/[0-9]/g,""),m=r.includes(w.cardId),M=i==="attack"&&(["MIL","ATT"].includes(L)||m)&&!w.used||i==="defense"&&["GK","DEF","MIL"].includes(L)&&!w.used,E=o.includes(w.cardId);let I=w.boost||0,D=!1;w.stadiumBonus&&(i==="attack"&&(L==="ATT"||L==="MIL")||i==="defense"&&(L==="GK"||L==="DEF"||L==="MIL")?(I+=10,D=!0):i||(I+=10,D=!0));const O=Math.round(h.x-g/2),Y=Math.round(h.y-p/2);if(w.used){x+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${O}" y="${Y}" width="${g}" height="${p}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${w.cardId}" data-role="${L}" style="cursor:pointer"/>`;continue}const J=Se({...w,_evolution_bonus:0,stadiumBonus:!1},{width:g,showStad:!1,stadDef:null,role:L,extraNote:I,_cardOffset:30,_forceStadColor:D}),A=E?`position:absolute;top:30px;left:0;width:${g}px;height:${p}px;outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;pointer-events:none;`:"";x+=`<foreignObject x="${O-2}" y="${Y-30}" width="${g+8}" height="${p+60}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="position:relative">
        ${J}
        ${E?`<div style="${A}"></div>`:""}
      </div>
    </foreignObject>`,M&&(x+=`<rect x="${O}" y="${Y}" width="${g}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${E?"selected":""}" data-card-id="${w.cardId}" data-role="${L}" style="cursor:pointer"/>`)}const u=s!==null?s:Math.round(Math.max(g*.7,80));return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-u} ${-u} ${a+u*2} ${n+u*2}" width="100%" style="display:block;width:100%;margin:0 auto">
    ${x}
  </svg>`}function Ot(t,e,i,o,a=300,n=300,r=[],s=null){return`<div id="match-terrain-wrap" style="position:relative;padding:0 4px">
    ${ki(t,e,i,o,a,n,r,s)}
  </div>`}async function $i(t,e,i,o){var M;const{state:a,navigate:n,toast:r}=e;lt(t);const s=a.params||{};if(t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!s.deckId)return br(t,e,i);const d=s.deckId;let l,c,f,b;try{const E=await Promise.all([v.from("decks").select("formation,name,stadium_card_id").eq("id",d).single(),v.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_real,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",d).order("slot_order")]);l=E[0].data,f=E[0].error,c=E[1].data,b=E[1].error}catch(E){console.error("[Match] Exception chargement deck:",E),St(t,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>n("home"));return}if(f||b){console.error("[Match] Erreur Supabase:",f||b),St(t,"⚠️","Erreur lors du chargement du deck.","Retour",()=>n("home"));return}const x=(c||[]).filter(E=>{var I;return E.is_starter&&((I=E.card)==null?void 0:I.player)}).map(E=>Fi(E.card,E.position)),g=(c||[]).filter(E=>{var I;return!E.is_starter&&((I=E.card)==null?void 0:I.player)}).map(E=>Fi(E.card,E.position));if(x.length<11){St(t,"⚠️",`Deck incomplet (${x.length}/11).`,"Compléter",()=>n("decks"));return}const p=(c||[]).find(E=>{var I;return((I=E.card)==null?void 0:I.card_type)==="formation"}),u=(l==null?void 0:l.formation)||((M=p==null?void 0:p.card)==null?void 0:M.formation)||"4-4-2",{data:y,error:w}=await v.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",a.profile.id).eq("card_type","game_changer"),{data:h}=await v.from("gc_definitions").select("*").eq("is_active",!0),L=(y||[]).map(E=>({...E,_gcDef:(h==null?void 0:h.find(I=>I.name===E.gc_type||I.id===E.gc_definition_id))||null}));let m=null;if(l!=null&&l.stadium_card_id){const{data:E}=await v.from("cards").select("stadium_id").eq("id",l.stadium_card_id).single();if(E!=null&&E.stadium_id){const{data:I}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",E.stadium_id).single();m=I||null}}o({deckId:d,formation:u,starters:x,subsRaw:g,gcCardsEnriched:L,gcDefs:h||[],stadiumDef:m})}function At(){return Math.min(window.innerWidth-40,860)}function Nt(){return Math.round(At()*1.1)}function Qo(t){var a,n;if(!t)return null;const e=t._line||t.job||"MIL",i=e==="GK"?t.note_g||0:e==="DEF"?t.note_d||0:e==="MIL"?t.note_m||0:t.note_a||0,o=t.stadiumBonus?10:0;return{name:t.name,firstname:t.firstname||"",note:i+(t.boost||0)+o,note_g:t.note_g||0,note_d:t.note_d||0,note_m:t.note_m||0,note_a:t.note_a||0,_evolution_bonus:0,stadiumBonus:t.stadiumBonus||!1,boost:t.boost||0,job:t.job,job2:t.job2||null,_line:t._line||t.job,_col:t._col,country_code:t.country_code,club_id:t.club_id,rarity:t.rarity,clubName:t.clubName||((a=t.clubs)==null?void 0:a.encoded_name)||null,clubLogo:t.clubLogo||((n=t.clubs)==null?void 0:n.logo_url)||null,face:t.face||null,portrait:Mt(t)}}function yr(t,e,i,o="Adversaire"){jo("/sounds/match-opening.mp3",.3);const a=At(),n=Nt();return`
    ...
    </div>`}function Zo(t){var n,r,s;if(!t)return"";const e=d=>d?Se({...d,_evolution_bonus:0},{width:52,role:d._line||d.job,showStad:!!d.stadiumBonus,extraNote:d.boost||0}):"",o={goal:"⚽","goal-home":"⚽","goal-ai":"⚽",duel:"⚔️",midfield:"🎯",sub:"🔄",gc:"⚡",boost:"💥","defense-won":"🛡️","attack-won":"⚔️","defense-lost":"😓","attack-lost":"😓"}[t.type]||"📋";return`
    ...
    <div style="animation:goalPop 0.4s ease-out;text-align:center">
      ...
      }`,document.head.appendChild(d)}setTimeout(()=>{r.style.opacity="0",r.style.transition="opacity 0.4s",setTimeout(()=>{r.remove(),a==null||a()},400)},1800)}function hr(t,e,i){const o=document.getElementById("sub-anim-overlay");o&&o.remove();const a=document.createElement("div");a.id="sub-anim-overlay",a.style.cssText=`
    position:fixed;inset:0;z-index:3000;
    display:flex;flex-direction:column;align-items:center;justify-content:center;
    ...
    pointer-events:none;white-space:nowrap;max-width:90vw;text-align:center`,a.textContent=t,!document.getElementById("game-toast-style")){const n=document.createElement("style");n.id="game-toast-style",n.textContent=`
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${t.map((n,r)=>{const s=r<t.length-1?Ut(n,t[r+1]):null,d=!s||s==="#ff3333"||s==="#cc2222",l=s==="#00ff88"?"+10":s==="#FFD700"?"+5":"";return en(n,a),n.stadiumBonus||a&&(a.club_id&&String(n.club_id)===String(a.club_id)||a.country_code&&(n.country_code,a.country_code)),`
    ...
    </div>`}async function vr(t,e){const{state:i}=e,o=i.params||{},a=o.matchMode||"vs_ai_easy",n=a==="solo",r=o.soloLevel||1,s=a.replace("vs_ai_",""),d=a;let l=null;if(n){const{data:c}=await v.from("solo_levels").select("*").eq("level_number",r).maybeSingle();l=c||{level_number:r,target_note_avg:10,nb_liens_jaune:2,nb_liens_vert:1,nb_joueurs_stade:2,reward_credits:500}}await $i(t,e,a,async({deckId:c,formation:f,starters:b,subsRaw:x,gcCardsEnriched:g,gcDefs:p,stadiumDef:u})=>{try{let y=bi(b,f);u&&(y=ri(y,u),xi(x,u));const w=n?await _r(f,l):await wr(f,s),h=w.lines||w,L=async m=>{try{const M=d==="vs_ai_club"||d==="solo"?"club":d,{data:E,error:I}=await v.from("matches").insert({home_id:i.profile.id,away_id:null,mode:M,home_deck_id:c,status:"in_progress"}).select().single();if(I){console.error("[MatchIA] Erreur création match:",I),St(t,"⚠️","Impossible de créer le match ("+I.message+").","Retour",()=>e.navigate("home"));return}const D=w.stadiumDef||null;D&&h&&(ri(h,D),xi(w.subs||[],D));const O={gcDefs:p||[],matchId:E==null?void 0:E.id,mode:d,difficulty:s,formation:f,isSolo:n,soloLevel:r,soloLevelConfig:l,homeTeam:y,aiTeam:h,homeSubs:x,subsUsed:0,maxSubs:Math.min(x.length,3),aiSubs:w.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((w.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:w.gcCards||[],aiUsedGc:[],aiStadiumDef:D,homeScore:0,aiScore:0,gcCards:m,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};kr(t,O,e)}catch(M){console.error("[MatchIA] Exception launchMatch:",M),St(t,"⚠️","Erreur au lancement du match : "+M.message,"Retour",()=>e.navigate("home"))}};if(!g.length){L([]);return}_i(t,g,L)}catch(y){console.error("[MatchIA] Exception setup:",y),St(t,"⚠️","Erreur de préparation du match : "+y.message,"Retour",()=>e.navigate("home"))}})}async function wr(t,e){var u;const{data:i}=await v.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:tn(t),subs:[],gcCards:[],stadiumDef:null};const o=kt[t]||kt["4-4-2"],a={GK:[],DEF:[],MIL:[],ATT:[]},n=new Set;function r(y,w,h){var L,m;return n.add(y.id),{cardId:"ai-"+y.id+"-"+h,id:y.id,firstname:y.firstname,name:y.surname_real,country_code:y.country_code,club_id:y.club_id,job:y.job,job2:y.job2,note_g:Number(y.note_g)||0,note_d:Number(y.note_d)||0,note_m:Number(y.note_m)||0,note_a:Number(y.note_a)||0,rarity:y.rarity,skin:y.skin,hair:y.hair,hair_length:y.hair_length,face:y.face,clubName:((L=y.clubs)==null?void 0:L.encoded_name)||null,clubLogo:((m=y.clubs)==null?void 0:m.logo_url)||null,boost:0,used:!1,_line:w}}for(const y of["GK","DEF","MIL","ATT"]){const w=i.filter(E=>E.job===y&&!n.has(E.id)),h=i.filter(E=>E.job!==y&&!n.has(E.id)),L=[...w,...h],m=[];for(let E=0;E<o[y];E++){const I=L[E];I&&m.push(r(I,y,E))}const M=st(m.length);m.forEach((E,I)=>{E._col=M[I]}),a[y]=m}const d=i.filter(y=>!n.has(y.id)).slice(0,5).map((y,w)=>r(y,y.job,100+w)),f=Object.keys(Ae).sort(()=>Math.random()-.5).slice(0,3).map((y,w)=>{var h,L;return{id:"ai-gc-"+w,gc_type:y,name:((h=Ae[y])==null?void 0:h.name)||y,icon:((L=Ae[y])==null?void 0:L.icon)||"⚡"}}),b=Object.values(a).flat(),x={};b.forEach(y=>{y.club_id&&(x[y.club_id]=(x[y.club_id]||0)+1)});const g=(u=Object.entries(x).sort((y,w)=>w[1]-y[1])[0])==null?void 0:u[0];let p=null;if(g){const{data:y}=await v.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",g).single();y&&(p={club_id:y.id,country_code:null,name:y.encoded_name+" Stadium",club:{encoded_name:y.encoded_name,logo_url:y.logo_url}})}return{lines:a,subs:d,gcCards:f,stadiumDef:p}}function _o(t){const e=t.job||"ATT";return Number(e==="GK"?t.note_g:e==="DEF"?t.note_d:e==="MIL"?t.note_m:t.note_a)||0}function ci(t,e,i){if(i<=0||!t.length)return[];const o=[...t].sort((a,n)=>Math.abs(_o(a)-e)-Math.abs(_o(n)-e));return o.slice(0,Math.min(i,o.length))}async function _r(t,e){var J,A;const i=Number(e==null?void 0:e.target_note_avg)||10,o=Number(e==null?void 0:e.nb_liens_jaune)||0,a=Number(e==null?void 0:e.nb_liens_vert)||0,n=Number(e==null?void 0:e.nb_joueurs_stade)||0,{data:r}=await v.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url,country_code)").eq("is_active",!0).limit(300);if(!r||r.length<16)return{lines:tn(t),subs:[],gcCards:[],stadiumDef:null};const s={};r.forEach(_=>{_.club_id&&(s[_.club_id]=(s[_.club_id]||0)+1)});const d=((J=Object.entries(s).sort((_,H)=>H[1]-_[1])[0])==null?void 0:J[0])||null,l=d?r.filter(_=>_.club_id===d):[],c=((A=l[0])==null?void 0:A.country_code)||null,f=c?r.filter(_=>_.country_code===c&&_.club_id!==d):[],b=Math.min(l.length,Math.max(2,a+1),16),x=Math.min(f.length,Math.max(0,o),16-b),g=[],p=new Set;ci(l,i,b).forEach(_=>{g.push(_),p.add(_.id)}),ci(f,i,x).forEach(_=>{g.push(_),p.add(_.id)});const u=Math.max(0,16-g.length),y=r.filter(_=>!p.has(_.id));ci(y,i,u).forEach(_=>{g.push(_),p.add(_.id)});const w=kt[t]||kt["4-4-2"],h={GK:[],DEF:[],MIL:[],ATT:[]},L=new Set;function m(_,H,W){var ae,le;return L.add(_.id),{cardId:"ai-"+_.id+"-"+W,id:_.id,firstname:_.firstname,name:_.surname_real,country_code:_.country_code,club_id:_.club_id,job:_.job,job2:_.job2,note_g:Number(_.note_g)||0,note_d:Number(_.note_d)||0,note_m:Number(_.note_m)||0,note_a:Number(_.note_a)||0,rarity:_.rarity,skin:_.skin,hair:_.hair,hair_length:_.hair_length,face:_.face,clubName:((ae=_.clubs)==null?void 0:ae.encoded_name)||null,clubLogo:((le=_.clubs)==null?void 0:le.logo_url)||null,boost:0,used:!1,_line:H}}for(const _ of["GK","DEF","MIL","ATT"]){const H=g.filter(U=>U.job===_&&!L.has(U.id)),W=g.filter(U=>U.job!==_&&!L.has(U.id)),ae=[...H,...W],le=[];for(let U=0;U<w[_];U++){const de=ae[U];de&&le.push(m(de,_,U))}const j=st(le.length);le.forEach((U,de)=>{U._col=j[de]}),h[_]=le}let M=g.filter(_=>!L.has(_.id));if(M.length<5){const _=r.filter(H=>!L.has(H.id)&&!M.some(W=>W.id===H.id));M=M.concat(ci(_,i,5-M.length))}const E=M.slice(0,5).map((_,H)=>m(_,_.job,100+H)),O=Object.keys(Ae).sort(()=>Math.random()-.5).slice(0,3).map((_,H)=>{var W,ae;return{id:"ai-gc-"+H,gc_type:_,name:((W=Ae[_])==null?void 0:W.name)||_,icon:((ae=Ae[_])==null?void 0:ae.icon)||"⚡"}});let Y=null;if(n>0&&d){const{data:_}=await v.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",d).single();_&&(Y={club_id:_.id,country_code:null,name:_.encoded_name+" Stadium",club:{encoded_name:_.encoded_name,logo_url:_.logo_url}})}return{lines:h,subs:E,gcCards:O,stadiumDef:Y}}function tn(t){const e=kt[t]||kt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},o=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let a=0;for(const n of["GK","DEF","MIL","ATT"]){const r=[];for(let d=0;d<e[n];d++){const l=3+Math.floor(Math.random()*5);r.push({cardId:"fake-"+a,id:"fake-"+a,firstname:"IA",name:o[a%o.length],country_code:"XX",club_id:null,job:n,job2:null,note_g:n==="GK"?l:2,note_d:n==="DEF"?l:2,note_m:n==="MIL"?l:2,note_a:n==="ATT"?l:2,rarity:"normal",boost:0,used:!1,_line:n}),a++}const s=st(r.length);r.forEach((d,l)=>{d._col=s[l]}),i[n]=r}return i}function kr(t,e,i){var a;jo("/sounds/match-opening.mp3",.3);const o=`<div style="position:relative;width:22px;height:22px;flex-shrink:0;display:flex;align-items:center;justify-content:center">
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(e.phase==="defense"&&e.pendingAttack){const U=e.pendingAttack;let de="";if(e.selected.length>0){const fe=e.selected.map($=>({...(e.homeTeam[$._role]||[]).find(R=>R.cardId===$.cardId)||$,_line:$._role})),ue=ni(fe,e.modifiers.home,e.formation);de=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
            ...
            <div style="display:flex;justify-content:center">${Je(U.map(fe=>({...fe,used:!1})),"#FFD700",de.total,"attack",e.formation)}</div>
    ...
    ${(()=>{const j=window.innerWidth>=700,U=(q,K,pe)=>{var ze,Be;const oe=(e.gcDefs||[]).find($e=>$e.name===q.gc_type),me=(oe==null?void 0:oe.name)||q.gc_type,we=(oe==null?void 0:oe.effect)||((ze=Ae[q.gc_type])==null?void 0:ze.desc)||"",he=oe!=null&&oe.image_url?`/icons/${oe.image_url}`:null,Te=((Be=Ae[q.gc_type])==null?void 0:Be.icon)||"⚡";return`<div class="gc-mini" data-gc-id="${q.id}" data-gc-type="${q.gc_type}" style="flex-shrink:0;cursor:pointer">
        ...
        </div>`},de=(q,K)=>{var pe;return`<div id="boost-card"
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${e.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...e.log].reverse().map(j=>{var U,de,fe;if(j.type==="duel"){const ue=j.isGoal,$=j.homeScored?"#FFD700":ue?"#ff6b6b":"rgba(255,255,255,0.3)",C=j.homeScored?"⚽ BUT !":ue?"⚽ BUT IA !":(U=j.homePlayers)!=null&&U.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${ue?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${$};margin-bottom:4px">
                ...
                ${(de=j.homePlayers)!=null&&de.length?`<div style="margin-bottom:3px">${Je(j.homePlayers,"rgba(255,255,255,0.7)",j.homeTotal,void 0,e.formation)}</div>`:""}
  ...
  </div>`;function I(){const j=t.querySelector(".match-screen");if(!j)return;const U=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);j.style.bottom="auto",j.style.height=U+"px",j.style.minHeight=U+"px",j.style.maxHeight=U+"px",j.style.overflow="hidden";const de=t.querySelector("#mobile-action-bar"),fe=t.querySelector("#mobile-play-area");de&&fe&&(fe.style.paddingBottom=de.offsetHeight+"px")}if(I(),setTimeout(I,120),setTimeout(I,400),setTimeout(I,1e3),e._vvBound||(e._vvBound=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",I),window.visualViewport.addEventListener("scroll",I)),window.addEventListener("resize",I)),function(){const U=t.querySelector("#match-field .terrain-wrapper svg")||t.querySelector(".terrain-wrapper svg");if(!U)return;const de=U.closest("#match-terrain-wrap");de&&(de.style.cssText="position:relative;width:100%;height:100%;padding:0"),U.removeAttribute("width"),U.removeAttribute("height"),U.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",U.setAttribute("preserveAspectRatio","xMidYMid meet")}(),e._resizeBound||(e._resizeBound=!0,window.addEventListener("resize",()=>{const j=t.querySelector(".terrain-wrapper svg");if(j){const U=j.closest("#match-terrain-wrap");U&&(U.style.cssText="position:relative;width:100%;height:100%;padding:0"),j.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0"}})),e._timerInt&&(clearInterval(e._timerInt),e._timerInt=null),Ue(),e.phase==="attack"||e.phase==="defense"){let j=!1,U=30;const de=()=>document.getElementById("match-timer"),fe=()=>{const ue=de();if(!ue)return;const $=String(Math.floor(U/60)).padStart(2,"0"),C=String(U%60).padStart(2,"0");ue.textContent=` ${$}:${C}`,ue.style.color=j?"#ff2222":"#ff9500",ue.style.fontWeight="900"};fe(),e._timerInt=setInterval(()=>{if(U--,U<0)if(!j)j=!0,U=15,fe(),Bo("/sounds/timer-urgent.mp3",.6);else{clearInterval(e._timerInt),e._timerInt=null,Ue(),e.homeScore=0,e.aiScore=3;const ue=document.createElement("div");ue.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",ue.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(ue),setTimeout(()=>{ue.remove(),ht(t,e,i)},2500)}else fe()},1e3)}(O=document.getElementById("match-quit"))==null||O.addEventListener("click",()=>{it(t),confirm("Abandonner ? Résultat : défaite 3-0")&&(e.homeScore=0,e.aiScore=3,ht(t,e,i))}),(Y=document.getElementById("view-ai"))==null||Y.addEventListener("click",()=>Br(e,i)),(J=document.getElementById("toggle-history"))==null||J.addEventListener("click",()=>{var j;(j=document.getElementById("match-history-panel"))==null||j.classList.add("open")}),(A=document.getElementById("close-history"))==null||A.addEventListener("click",()=>{var j;(j=document.getElementById("match-history-panel"))==null||j.classList.remove("open")}),(_=document.getElementById("btn-action"))==null||_.addEventListener("click",()=>{e.selected.length!==0&&(u?Lr(t,e,i):y&&Tr(t,e,i))}),(H=document.getElementById("btn-results"))==null||H.addEventListener("click",()=>ht(t,e,i)),(W=document.getElementById("btn-pass"))==null||W.addEventListener("click",()=>{if(e.log.push({text:"⏭️ Vous passez votre tour (plus d'attaquants)",type:"info"}),Pi(e.aiTeam,e.homeTeam)){e.log.push({text:"🏁 Plus personne ne peut attaquer — match terminé.",type:"info"}),ht(t,e,i);return}e.phase="ai-attack",ke(t,e,i),setTimeout(()=>eo(t,e,i),800)}),t.querySelectorAll(".match-slot-hit").forEach(j=>{j.addEventListener("click",()=>Er(j,e,t,i))}),t.querySelectorAll(".match-used-hit").forEach(j=>{j.addEventListener("click",()=>Mi(t,e,i,null,j.dataset.cardId))}),t.querySelectorAll(".gc-mini").forEach(j=>{u?j.addEventListener("click",()=>zr(j.dataset.gcId,j.dataset.gcType,t,e,i)):(j.style.opacity="0.35",j.style.cursor="default",j.addEventListener("click",()=>Lt("⚡ Les Game Changers ne sont utilisables qu'en attaque","rgba(180,100,0,0.9)")))}),(ae=document.getElementById("boost-card"))==null||ae.addEventListener("click",()=>jr(t,e,i)),t.querySelectorAll(".sub-btn-col").forEach(j=>{j.addEventListener("click",()=>Mi(t,e,i,j.dataset.subId))}),(le=document.getElementById("sub-btn-main"))==null||le.addEventListener("click",()=>Mi(t,e,i))}function Er(t,e,i,o){const a=t.dataset.cardId,n=t.dataset.role,r=e.selected.findIndex(s=>s.cardId===a);if(r!==-1)e.selected.splice(r,1);else{if(e.selected.length>=3){o.toast("Maximum 3 joueurs","error");return}const s=(e.homeTeam[n]||[]).find(d=>d.cardId===a);s&&e.selected.push({...s,_role:n,_line:n})}ke(i,e,o)}function Zi(t,e,i){t.matchId&&v.from("matches").update({last_player_id:i}).eq("id",t.matchId).then(()=>{})}function Lr(t,e,i){e._timerInt&&(clearInterval(e._timerInt),e._timerInt=null),Ue(),Zi(e,i,i.state.profile.id);const o=e.selected.map(n=>{const r=(e.homeTeam[n._role]||[]).find(d=>d.cardId===n.cardId)||n,s=["GK","DEF"].includes(n._role);return{...r,_line:n._role,...s?{note_a:Math.max(1,Number(r.note_a)||0)}:{}}}),a=oi(o,e.modifiers.home,e.formation);e.pendingAttack={...a,players:[...o],side:"home"},e.selected.forEach(n=>{const r=(e.homeTeam[n._role]||[]).find(s=>s.cardId===n.cardId);r&&(r.used=!0)}),e.log.push({text:`⚔️ Vous attaquez : ${a.total} (base ${a.base}${a.links?` +${a.links} liens`:""}) — ${e.selected.map(n=>n.name).join(", ")}`,type:"info"}),e.selected=[],e.modifiers.home={},e.phase="ai-defense",ke(t,e,i),setTimeout(()=>Ar(t,e,i),1200)}function Tr(t,e,i){e._timerInt&&(clearInterval(e._timerInt),e._timerInt=null),Ue(),Zi(e,i,i.state.profile.id);const o=e.stadiumDef||null,a=e.selected.map(d=>{const l=(e.homeTeam[d._role]||[]).find(f=>f.cardId===d.cardId)||d,c=l.stadiumBonus||o&&(o.club_id&&String(l.club_id)===String(o.club_id)||o.country_code&&l.country_code===o.country_code)||!1;return{...l,_line:d._role,stadiumBonus:c}}),n=ni(a,e.modifiers.home,e.formation);e.selected.forEach(d=>{const l=(e.homeTeam[d._role]||[]).find(c=>c.cardId===d.cardId);l&&(l.used=!0)});const r=Qi(e.pendingAttack.total,n.total,e.modifiers.home),s={type:"duel",title:"Défense",aiPlayers:(e.pendingAttack.players||[]).map(d=>Ke(d)),homePlayers:e.selected.map(d=>{const l=(e.homeTeam[d._role]||[]).find(c=>c.cardId===d.cardId)||d;return Ke(l)}),homeTotal:n.total,aiTotal:e.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(r.shielded)s.text="🛡️ Bouclier ! But annulé.",e.log.push(s);else if(r.goal){e.aiScore++,s.isGoal=!0,s.homeScored=!1,s.text=`⚽ BUT IA ! (${e.pendingAttack.total} > ${n.total})`,e.log.push(s),e.selected=[],e.modifiers.home={},e.pendingAttack=null,ke(t,e,i),Qe(s.aiPlayers,e.homeScore,e.aiScore,!1,()=>{zt(t,e,i,"home-attack")});return}else s.text=`🧤 Défense réussie ! (${n.total} ≥ ${e.pendingAttack.total})`,e.log.push(s);e.selected=[],e.modifiers.home={},e.pendingAttack=null,zt(t,e,i,"home-attack")}function Ir(t){if(t.aiSubsUsed>=t.aiMaxSubs)return;const e=Object.values(t.aiTeam).flat().filter(d=>d.used);if(!e.length)return;const i=(t.aiSubs||[]).filter(d=>!t.aiUsedSubIds.includes(d.cardId));if(!i.length)return;const o=e[Math.floor(Math.random()*e.length)],a=i.find(d=>d.job===o.job)||i[0],n={...a,used:!1,_line:o._line,_col:o._col},r=t.aiTeam[o._line],s=r.findIndex(d=>d.cardId===o.cardId);s!==-1&&(r[s]=n),t.aiUsedSubIds.push(a.cardId),t.aiSubsUsed++,t.log.push({text:`🔄 IA : ${a.firstname} ${a.name} remplace ${o.firstname} ${o.name}`,type:"info"})}function Sr(t){var o;if(!((o=t.aiGcCards)!=null&&o.length))return;const e=t.aiGcCards.filter(a=>!t.aiUsedGc.includes(a.id));if(!e.length||Math.random()>.3)return;const i=e[Math.floor(Math.random()*e.length)];switch(t.aiUsedGc.push(i.id),Ae[i.gc_type],i.gc_type){case"Boost+2":{const a=Object.values(t.aiTeam).flat().filter(n=>!n.used);if(a.length){const n=a[Math.floor(Math.random()*a.length)];n.boost=(n.boost||0)+2}break}case"Boost+3":{const a=Object.values(t.aiTeam).flat().filter(n=>!n.used);if(a.length){const n=a[Math.floor(Math.random()*a.length)];n.boost=(n.boost||0)+3}break}case"Remplacement+":t.aiMaxSubs=(t.aiMaxSubs||3)+1;break;case"Bouclier":t.modifiers.ai.shield=!0;break;case"Nul+1":t.modifiers.ai.drawBonus=(t.modifiers.ai.drawBonus||0)+1;break}t.log.push({text:`⚡ IA joue ${i.icon||"⚡"} ${i.name}`,type:"gc"})}function eo(t,e,i){Zi(e,i,null),Ir(e),Sr(e);let o=[...e.aiTeam.MIL||[],...e.aiTeam.ATT||[]].filter(c=>!c.used),a=!1;if(!o.length){const c=[...e.homeTeam.MIL||[],...e.homeTeam.ATT||[]].filter(b=>!b.used),f=Ht(e.homeTeam);if(f||c.length===0){const b=(e.aiTeam.DEF||[]).filter(g=>!g.used),x=(e.aiTeam.GK||[]).filter(g=>!g.used);o=f?b.length?b:x:[...b,...x],a=!0}}const n=Oo(o,"attack",e.difficulty);if(!n.length){Gi(t,e,i);return}a&&n.forEach(c=>{c._line=c._line||c.job,c.note_a=Math.max(1,Number(c.note_a)||0)});const r=oi(n,e.modifiers.ai,e.formation);e.pendingAttack={...r,players:n,side:"ai"},n.forEach(c=>{c.used=!0}),e.log.push({text:`🤖 IA attaque : ${r.total} (${n.map(c=>c.name).join(", ")})`,type:"info"}),e.modifiers.ai={};const s=[...e.homeTeam.GK||[],...e.homeTeam.DEF||[],...e.homeTeam.MIL||[]].filter(c=>!c.used),l=(e.homeSubs||[]).filter(c=>!(e.usedSubIds||[]).includes(c.cardId)).length>0&&e.subsUsed<e.maxSubs;if(s.length===0&&!l){if(n.length===1&&(n[0]._line==="GK"||n[0].job==="GK")&&Ht(e.homeTeam)&&e.homeScore===e.aiScore){e.aiScore++,hi(t,e,i,"ai",n[0]);return}e.aiScore++;const f={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:n.map(b=>Ke(b)),aiTotal:r.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};e.log.push(f),e.pendingAttack=null,ke(t,e,i),Qe(f.aiPlayers,e.homeScore,e.aiScore,!1,()=>{zt(t,e,i,"home-attack")});return}e.phase="defense",ke(t,e,i)}function Ar(t,e,i){var l,c;const o=[...e.aiTeam.GK||[],...e.aiTeam.DEF||[],...e.aiTeam.MIL||[]],a=Oo(o,"defense",e.difficulty);if(!["GK","DEF","MIL","ATT"].flatMap(f=>(e.aiTeam[f]||[]).filter(b=>!b.used)).length){const f=((l=e.pendingAttack)==null?void 0:l.players)||[];if(f.length===1&&(f[0]._line==="GK"||f[0].job==="GK")&&e.homeScore===e.aiScore){e.homeScore++,hi(t,e,i,"home",f[0]);return}e.homeScore++;const x={type:"duel",isGoal:!0,homeScored:!0,homePlayers:f.map(g=>Ke(g)),homeTotal:((c=e.pendingAttack)==null?void 0:c.total)||0,aiTotal:0,text:"⚽ BUT ! L'IA n'a plus de joueurs — but automatique !"};e.log.push(x),e.modifiers.ai={},e.pendingAttack=null,ke(t,e,i),Qe(x.homePlayers,e.homeScore,e.aiScore,!0,()=>{zt(t,e,i,"ai-attack")});return}const r=a.length>0?ni(a,e.modifiers.ai,e.formation).total:0;a.forEach(f=>{f.used=!0});const s=Qi(e.pendingAttack.total,r,e.modifiers.ai),d={type:"duel",title:"Attaque",homePlayers:(e.pendingAttack.players||[]).map(f=>Ke(f)),aiPlayers:a.map(f=>Ke(f)),homeTotal:e.pendingAttack.total,aiTotal:r,isGoal:!1,homeScored:!1,text:""};if(s.shielded)d.text="🛡️ Bouclier IA !",e.log.push(d);else if(s.goal){e.homeScore++,d.isGoal=!0,d.homeScored=!0,d.text=`⚽ BUT ! (${e.pendingAttack.total} > ${r})`,e.log.push(d),e.modifiers.ai={},e.pendingAttack=null,ke(t,e,i),Qe(d.homePlayers,e.homeScore,e.aiScore,!0,()=>{zt(t,e,i,"ai-attack")});return}else d.text=`🧤 IA défend (${r} ≥ ${e.pendingAttack.total})`,e.log.push(d);e.modifiers.ai={},e.pendingAttack=null,zt(t,e,i,"ai-attack")}function zt(t,e,i,o){if(e.round++,!on(t,e,i)){if(to(e)){ht(t,e,i);return}if(o==="home-attack"){if(!["MIL","ATT","GK","DEF"].some(n=>(e.homeTeam[n]||[]).some(r=>!r.used))){Gi(t,e,i);return}e.phase="attack",ke(t,e,i)}else{if(!["MIL","ATT","GK","DEF"].some(n=>(e.aiTeam[n]||[]).some(r=>!r.used))){Gi(t,e,i);return}e.phase="ai-attack",ke(t,e,i),setTimeout(()=>eo(t,e,i),800)}}}function Ht(t){return!["GK","DEF","MIL","ATT"].some(e=>(t[e]||[]).some(i=>!i.used))}function ko(t){const e=(t.GK||[]).some(o=>!o.used),i=["DEF","MIL","ATT"].some(o=>(t[o]||[]).some(a=>!a.used));return e&&!i}function on(t,e,i){if(e.homeScore!==e.aiScore)return!1;if(ko(e.homeTeam)&&Ht(e.aiTeam)){const o=(e.homeTeam.GK||[]).find(a=>!a.used);return o?(o.used=!0,e.homeScore++,hi(t,e,i,"home",o),!0):!1}if(ko(e.aiTeam)&&Ht(e.homeTeam)){const o=(e.aiTeam.GK||[]).find(a=>!a.used);return o?(o.used=!0,e.aiScore++,hi(t,e,i,"ai",o),!0):!1}return!1}function hi(t,e,i,o,a){e.log.push({type:"duel",isGoal:!0,homeScored:o==="home",homePlayers:o==="home"?[Ke(a)]:[],aiPlayers:o==="ai"?[Ke(a)]:[],text:`⚽ DERNIER CORNER — Le gardien ${o==="home"?"":"adverse "}marque !`});const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(n);const r=(s,d)=>new Promise(l=>{n.innerHTML=`<div style="font-size:32px;font-weight:900;color:${d};letter-spacing:2px;animation:lcFade 1.4s ease both">${s}</div>
    <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(l,1400)});(async()=>(await r("⚽ DERNIER CORNER","#FFD700"),await r("🧤 LE GARDIEN MONTE !","#4fc3f7"),n.remove(),e.pendingAttack=null,ke(t,e,i),Qe([Ke(a)],e.homeScore,e.aiScore,o==="home",()=>{if(to(e)){ht(t,e,i);return}zt(t,e,i,o==="home"?"ai-attack":"home-attack")})))()}function to(t){const e=["MIL","ATT","GK","DEF"].some(o=>(t.homeTeam[o]||[]).some(a=>!a.used)),i=["MIL","ATT","GK","DEF"].some(o=>(t.aiTeam[o]||[]).some(a=>!a.used));return!e&&!i}function Pi(t,e){return!(["MIL","ATT"].some(a=>(t[a]||[]).some(n=>!n.used))||Ht(e)||!["MIL","ATT"].some(a=>(e[a]||[]).some(n=>!n.used)))}function Gi(t,e,i){if(!on(t,e,i)){if(to(e)){ht(t,e,i);return}if(Pi(e.homeTeam,e.aiTeam)&&Pi(e.aiTeam,e.homeTeam)){e.log.push({text:"🏁 Plus personne ne peut attaquer — match terminé.",type:"info"}),ht(t,e,i);return}e.phase="attack",ke(t,e,i)}}function Mi(t,e,i,o=null,a=null){var x,g;if(e.phase!=="attack"){Lt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(e.usedSubIds||(e.usedSubIds=[]),e.subsUsed>=e.maxSubs){Lt(`Maximum ${e.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const n=Object.entries(e.homeTeam).flatMap(([p,u])=>(u||[]).filter(y=>y.used).map(y=>({...y,_line:y._line||p}))),r=e.homeSubs.filter(p=>!e.usedSubIds.includes(p.cardId));if(!n.length){Lt("Aucun joueur utilisé à remplacer");return}if(!r.length){Lt("Aucun remplaçant disponible");return}let s=Math.max(0,n.findIndex(p=>p.cardId===a));const d=((x=n[s])==null?void 0:x._line)||((g=n[s])==null?void 0:g.job);let l=o?Math.max(0,r.findIndex(p=>p.cardId===o)):Math.max(0,r.findIndex(p=>p.job===d)),c=!1;const f=document.createElement("div");f.id="sub-overlay",f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function b(){var m,M,E,I,D,O;const p=n[s],u=r[l],y=Math.min(130,Math.round((window.innerWidth-90)/2)),w=Math.round(y*1.35),h=Y=>`background:rgba(255,255,255,0.12);border:none;color:${Y?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Y?"default":"pointer"};flex-shrink:0`;f.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <button id="in-down" style="${h(l>=r.length-1)}" ${l>=r.length-1?"disabled":""}>▼</button>
    ...
    </div>`,(m=f.querySelector("#sub-close"))==null||m.addEventListener("click",()=>f.remove()),(M=f.querySelector("#out-up"))==null||M.addEventListener("click",()=>{s>0&&(s--,b())}),(E=f.querySelector("#out-down"))==null||E.addEventListener("click",()=>{s<n.length-1&&(s++,b())}),(I=f.querySelector("#in-up"))==null||I.addEventListener("click",()=>{l>0&&(l--,b())}),(D=f.querySelector("#in-down"))==null||D.addEventListener("click",()=>{l<r.length-1&&(l++,b())});const L=(Y,J,A,_)=>{const H=f.querySelector("#"+Y);if(!H)return;let W=0;H.addEventListener("touchstart",ae=>{W=ae.touches[0].clientY},{passive:!0}),H.addEventListener("touchend",ae=>{const le=ae.changedTouches[0].clientY-W;if(Math.abs(le)<30)return;const j=J();le<0&&j<_-1?(A(j+1),b()):le>0&&j>0&&(A(j-1),b())},{passive:!0})};L("in-panel",()=>l,Y=>l=Y,r.length),L("out-panel",()=>s,Y=>s=Y,n.length),(O=f.querySelector("#sub-confirm"))==null||O.addEventListener("click",Y=>{if(Y.preventDefault(),Y.stopPropagation(),c)return;c=!0;const J=n[s],A=r[l];if(!J||!A)return;let _=null,H=-1;for(const[ae,le]of Object.entries(e.homeTeam)){const j=(le||[]).findIndex(U=>U.cardId===J.cardId);if(j!==-1){_=ae,H=j;break}}if(H===-1||!_){Lt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),f.remove();return}const W={...A,_line:_,_col:J._col||0,used:!1,boost:0};e.homeTeam[_].splice(H,1,W),e.usedSubIds||(e.usedSubIds=[]),e.usedSubIds.push(A.cardId),e.subsUsed++,e.selected=[],e.log.push({type:"sub",subSide:"home",clubName:e.clubName,outPlayer:{name:J.name,firstname:J.firstname,note:Pe(J,_),portrait:Mt(J),job:J.job,country_code:J.country_code,rarity:J.rarity,clubName:J.clubName,clubLogo:J.clubLogo},inPlayer:{name:A.name,firstname:A.firstname,note:Pe(A,_),portrait:Mt(A),job:A.job,country_code:A.country_code,rarity:A.rarity,clubName:A.clubName,clubLogo:A.clubLogo},text:`🔄 ${A.firstname} ${A.name} remplace ${J.firstname} ${J.name}`}),f.remove(),hr(J,A,()=>ke(t,e,i))})}document.body.appendChild(f),b()}function zr(t,e,i,o,a){var b,x;const n=(o.gcDefs||[]).find(g=>g.name===e),r=Ae[e]||{icon:"⚡",desc:"Carte spéciale."},s=(n==null?void 0:n.name)||e,d=(n==null?void 0:n.effect)||r.desc,l=n!=null&&n.image_url?`/icons/${n.image_url}`:null,c=r.icon||"⚡",f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",f.innerHTML=`
    ${tt(s,l,c,d,{width:190})}
    <!-- Boutons -->
    <div style="display:flex;gap:12px;width:190px">
      <button id="gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
      <button id="gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
    </div>`,document.body.appendChild(f),(b=f.querySelector("#gc-back"))==null||b.addEventListener("click",()=>f.remove()),(x=f.querySelector("#gc-use"))==null||x.addEventListener("click",()=>{f.remove(),Cr(t,e,i,o,a)})}function pi(t,e,i,o,a,n){const r=document.createElement("div");r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let s=[];function d(){var l,c;r.innerHTML=`
    ...
    </div>`,(l=r.querySelector("#gc-picker-close"))==null||l.addEventListener("click",()=>r.remove()),r.querySelectorAll(".gc-pick-item").forEach(f=>{f.addEventListener("click",()=>{const b=f.dataset.cid,x=t.find(p=>p.cardId===b);if(!x)return;const g=s.findIndex(p=>p.cardId===b);g>-1?s.splice(g,1):s.length<e&&s.push(x),d()})}),(c=r.querySelector("#gc-picker-confirm"))==null||c.addEventListener("click",()=>{r.remove(),n(s)})}d(),document.body.appendChild(r)}const Mr={BOOST_STAT:({value:t=1,count:e=1,roles:i=[]},o,a,n)=>{const r=Object.entries(o.homeTeam).filter(([s])=>!i.length||i.includes(s)).flatMap(([s,d])=>d.filter(l=>!l.used).map(l=>({...l,_line:s})));return r.length?(pi(r,e,`Choisir ${e} joueur(s) à booster (+${t})`,a,o,s=>{s.forEach(d=>{const l=(o.homeTeam[d._line]||[]).find(c=>c.cardId===d.cardId);l&&(l.boost=(l.boost||0)+t,o.log.push({text:`⚡ +${t} sur ${l.name}`,type:"info"}))}),ke(a,o,n)}),!0):(o.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),ke(a,o,n),!0)},DEBUFF_STAT:({value:t=1,count:e=1,roles:i=[],target:o="ai"},a,n,r)=>{const s=o==="home"?a.homeTeam:a.aiTeam,d=o==="ai"?"adverse":"allié",l=Object.entries(s).filter(([c])=>!i.length||i.includes(c)).flatMap(([c,f])=>f.filter(b=>!b.used).map(b=>({...b,_line:c})));return l.length?(pi(l,e,`Choisir ${e} joueur(s) ${d}(s) à débuffer (-${t})`,n,a,c=>{c.forEach(f=>{const x=((o==="home"?a.homeTeam:a.aiTeam)[f._line]||[]).find(g=>g.cardId===f.cardId);x&&(x.boost=(x.boost||0)-t,a.log.push({text:`🎯 -${t} sur ${x.name}${o==="ai"?" (IA)":""}`,type:"info"}))}),ke(n,a,r)}),!0):(a.log.push({text:`🎯 Aucun joueur ${d} disponible`,type:"info"}),ke(n,a,r),!0)},GRAY_PLAYER:({count:t=1,roles:e=[],target:i="ai"},o,a,n)=>{const r=i==="home"?o.homeTeam:o.aiTeam,s=i==="ai"?"adverse":"allié",d=Object.entries(r).filter(([l])=>!e.length||e.includes(l)).flatMap(([l,c])=>c.filter(f=>!f.used).map(f=>({...f,_line:l})));return d.length?(pi(d,t,`Choisir ${t} joueur(s) ${s}(s) à exclure`,a,o,l=>{l.forEach(c=>{const b=((i==="home"?o.homeTeam:o.aiTeam)[c._line]||[]).find(x=>x.cardId===c.cardId);b&&(b.used=!0,o.log.push({text:`❌ ${b.name}${i==="ai"?" (IA)":""} exclu !`,type:"info"}))}),ke(a,o,n)}),!0):(o.log.push({text:`❌ Aucun joueur ${s} à exclure`,type:"info"}),ke(a,o,n),!0)},REVIVE_PLAYER:({count:t=1,roles:e=[]},i,o,a)=>{const n=Object.entries(i.homeTeam).filter(([r])=>!e.length||e.includes(r)).flatMap(([r,s])=>s.filter(d=>d.used).map(d=>({...d,_line:r})));return n.length?(pi(n,t,`Choisir ${t} joueur(s) à ressusciter`,o,i,r=>{r.forEach(s=>{const d=(i.homeTeam[s._line]||[]).find(l=>l.cardId===s.cardId);d&&(d.used=!1,i.log.push({text:`💫 ${d.name} ressuscité !`,type:"info"}))}),ke(o,i,a)}),!0):(i.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),ke(o,i,a),!0)},REMOVE_GOAL:({},t)=>(t.aiScore>0?(t.aiScore--,t.log.push({text:"🚫 Dernier but IA annulé !",type:"info"})):t.log.push({text:"🚫 Aucun but à annuler",type:"info"}),!1),ADD_GOAL_DRAW:({},t)=>(t.homeScore===t.aiScore?(t.homeScore++,t.log.push({text:"🎯 But bonus (match nul) !",type:"info"})):t.log.push({text:"🎯 But bonus : non applicable (pas de match nul)",type:"info"}),!1),ADD_SUB:({value:t=1},e)=>(e.maxSubs=(e.maxSubs||3)+t,e.log.push({text:`🔄 +${t} remplacement(s) débloqué(s)`,type:"info"}),!1),CUSTOM:()=>!1};function Cr(t,e,i,o,a){o.usedGc.push(t);const n=o.gcDefs||[],r=n.find(d=>d.name===e)||n.find(d=>{var l;return((l=d.name)==null?void 0:l.toLowerCase().trim())===(e==null?void 0:e.toLowerCase().trim())});let s=!1;if(r!=null&&r.effect_type&&r.effect_type!=="CUSTOM"){const d=Mr[r.effect_type];d?d(r.effect_params||{},o,i,a)||(s=!0):(a.toast(`Effet "${r.effect_type}" non implémenté`,"error"),s=!0)}else{switch(e){case"Double attaque":o.modifiers.home.doubleAttack=!0,o.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":o.modifiers.home.shield=!0,o.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const d=Object.entries(o.homeTeam).flatMap(([l,c])=>(c||[]).filter(f=>f.used).map(f=>({...f,_line:l})));d.length?(d[0].used=!1,o.log.push({text:`💫 ${d[0].name} ressuscité !`,type:"info"})):o.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":o.modifiers.ai.stolenNote=(o.modifiers.ai.stolenNote||0)+1,o.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const d=[...o.aiTeam.ATT||[],...o.aiTeam.MIL||[]].filter(l=>!l.used);if(d.length){const l=d.sort((c,f)=>Pe(f,"ATT")-Pe(c,"ATT"))[0];l.used=!0,o.log.push({text:`❄️ ${l.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":o.maxSubs++,o.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}s=!0}v.from("cards").delete().eq("id",t).then(()=>{}),s&&ke(i,o,a)}function jr(t,e,i){const o=Object.values(e.homeTeam).flat().filter(a=>!a.used);if(!o.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ${o.map(a=>`
        <div class="player-boost-opt" data-card-id="${a.cardId}"
          ...
          <div style="width:32px;height:32px;background:${gr[a.job]||"#888"};border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:13px">${Pe(a,a._line||a.job)}</div>
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(a=>{a.addEventListener("click",()=>{const n=a.dataset.cardId;for(const r of["GK","DEF","MIL","ATT"]){const s=(e.homeTeam[r]||[]).find(d=>d.cardId===n);if(s){s.boost=(s.boost||0)+e.boostCard.value,e.log.push({text:`⚡ Boost +${e.boostCard.value} appliqué à ${s.name}`,type:"info"});break}}e.boostUsed=!0,i.closeModal(),ke(t,e,i)})})}async function ht(t,e,i){var c,f,b;Gt(),Ue(),e._timerInt&&(clearInterval(e._timerInt),e._timerInt=null),e.phase="finished";const{state:o}=i,a=e.homeScore>e.aiScore,n=e.homeScore===e.aiScore,r=a?"victoire":n?"nul":"defaite",s=e.isSolo&&((c=e.soloLevelConfig)!=null&&c.reward_credits)&&a?Number(e.soloLevelConfig.reward_credits):Gn(e.mode,r);if(e.isSolo&&a)try{const{data:x}=await v.from("user_solo_progress").select("unlocked_level").eq("user_id",o.profile.id).maybeSingle(),g=(x==null?void 0:x.unlocked_level)||1;e.soloLevel>=g&&await v.from("user_solo_progress").upsert({user_id:o.profile.id,unlocked_level:e.soloLevel+1,updated_at:new Date().toISOString()})}catch(x){console.warn("[Solo] Erreur mise à jour progression:",x.message)}e.matchId&&await v.from("matches").update({status:"finished",home_score:e.homeScore,away_score:e.aiScore,winner_id:a?o.profile.id:null,home_credits_reward:s,played_at:new Date().toISOString()}).eq("id",e.matchId);const d={credits:(o.profile.credits||0)+s,matches_played:(o.profile.matches_played||0)+1};a?d.wins=(o.profile.wins||0)+1:n?d.draws=(o.profile.draws||0)+1:d.losses=(o.profile.losses||0)+1,await v.from("users").update(d).eq("id",o.profile.id),await i.refreshProfile();const l=document.createElement("div");l.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",l.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${ki(t.aiTeam,t.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            <div style="background:rgba(255,255,255,0.08);border-radius:14px;padding:14px 20px;text-align:center;min-width:220px">
              ...
              ${Ie.id!==ot.id?`<div style="font-size:20px;font-weight:900;color:${Ie.color}">
                    ${Ne>ee.mmr?"📈":"📉"} ${ot.emoji} ${ot.label} → ${Ie.emoji} ${Ie.label}
                  </div>
                  <div style="font-size:13px;color:rgba(255,255,255,0.5);margin-top:4px">${Ne>ee.mmr?"Promotion !":"Rétrogradation"}</div>`:`<div style="font-size:36px">${Le>=0?"📈":"📉"}</div>
            ...
            </div>`}}catch(ee){console.error("[Ranked] MMR update error:",ee)}(ie=document.getElementById("pvp-end-overlay"))==null||ie.remove();const F=document.createElement("div");F.id="pvp-end-overlay",F.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const G=S?"🤝":B?"🏆":"😞",V=S?"MATCH NUL":B?"VICTOIRE !":"DÉFAITE",Z=S?"#fff":B?"#FFD700":"#ff6b6b";F.innerHTML=`
      ...
      ${N}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${c?"🏆 Retour à la Mini League":r?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(F),(te=F.querySelector("#pvp-end-home"))==null||te.addEventListener("click",()=>{F.remove(),it(t),c?x("mini-league",{openLeagueId:c}):x(r?"ranked":"home")})}const J=v.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},k=>{var z;const T=k.new;try{if(T.status==="finished"||T.forfeit){if(M)return;M=!0,E&&(clearInterval(E),E=null),Ue(),T.game_state&&(m=T.game_state),Y(T);return}if(T.game_state){const B=m;m=T.game_state;const S=m._lastGC;if(S&&S.seq&&!O.has(S.seq)&&(O.add(S.seq),S.by!==p)){$(S.type,S.by,()=>U());return}const N=B[p+"Score"]||0,F=B[u+"Score"]||0,G=m[p+"Score"]||0,V=m[u+"Score"]||0,Z=G>N,ie=V>F;if((Z||ie)&&!D.has(m.round)){D.add(m.round);const te=[...m.log||[]].reverse().find(se=>se.isGoal),ee=((te==null?void 0:te.homePlayers)||[]).map(se=>({name:se.name,note:se.note,portrait:se.portrait,job:se.job}));!!((z=te==null?void 0:te.text)!=null&&z.includes("DERNIER CORNER"))?we().then(()=>{Qe(ee,G,V,Z,()=>U())}):Qe(ee,G,V,Z,()=>U());return}U()}}catch(B){console.error("[PvP] crash:",B)}}).subscribe();async function A(k){Object.assign(m,k),m.lastActionAt=new Date().toISOString();const{error:T}=await v.from("matches").update({game_state:m}).eq("id",i);T&&g("Erreur de synchronisation","error");try{U()}catch(z){console.error("[PvP] renderPvpScreen crash:",z)}}async function _(){if(M)return;M=!0,Gt(),Ue(),E&&(clearInterval(E),E=null);const k=o?h.away_id:h.home_id,T=o?"p2":"p1",z=o?"p1":"p2",B={...m,[T+"Score"]:3,[z+"Score"]:0,phase:"finished"},S=B.p1Score||0,N=B.p2Score||0;try{await v.from("matches").update({game_state:B}).eq("id",i);const{data:F,error:G}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:k,p_home_score:S,p_away_score:N,p_forfeit:!0});if(G&&console.error("[PvP] finish_pvp_match (forfeit):",G),c)try{await v.rpc("check_and_finish_mini_league",{p_league_id:c})}catch(V){console.error("[PvP] check_and_finish_mini_league (forfeit):",V)}try{const{data:V}=await v.rpc("apply_match_rewards",{p_match_id:i});V!=null&&V.success&&!(V!=null&&V.skipped)&&typeof e.refreshProfile=="function"&&await e.refreshProfile()}catch(V){console.error("[PvP] apply_match_rewards (forfeit):",V)}}catch{}try{v.removeChannel(J)}catch{}setTimeout(()=>{it(t),c?x("mini-league",{openLeagueId:c}):x("home")},800)}const H={BOOST_STAT:({value:k=1,count:T=1,roles:z=[]},B,S)=>{const N=B[p+"Team"],F=Object.entries(N).filter(([G])=>!z.length||z.includes(G)).flatMap(([G,V])=>V.filter(Z=>!Z.used).map(Z=>({...Z,_line:G})));if(!F.length){B.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),S(B);return}W(F,T,`Choisir ${T} joueur(s) à booster (+${k})`,G=>{G.forEach(V=>{const Z=(N[V._line]||[]).find(ie=>ie.cardId===V.cardId);Z&&(Z.boost=(Z.boost||0)+k,B.log.push({text:`⚡ +${k} sur ${Z.name}`,type:"info"}))}),B[p+"Team"]=N,S(B)})},DEBUFF_STAT:({value:k=1,count:T=1,roles:z=[],target:B="ai"},S,N)=>{const F=B==="home"?p:u,G=S[F+"Team"],V=B==="home"?"allié":"adverse",Z=Object.entries(G).filter(([ie])=>!z.length||z.includes(ie)).flatMap(([ie,te])=>te.map(ee=>({...ee,_line:ie})));if(!Z.length){S.log.push({text:`🎯 Aucun joueur ${V}`,type:"info"}),N(S);return}W(Z,T,`Choisir ${T} joueur(s) ${V}(s) (-${k})`,ie=>{ie.forEach(te=>{const ee=(G[te._line]||[]).find(re=>re.cardId===te.cardId);ee&&(ee.boost=(ee.boost||0)-k,S.log.push({text:`🎯 -${k} sur ${ee.name}`,type:"info"}))}),S[F+"Team"]=G,N(S)})},GRAY_PLAYER:({count:k=1,roles:T=[],target:z="ai"},B,S)=>{const N=z==="home"?p:u,F=B[N+"Team"],G=z==="home"?"allié":"adverse",V=Object.entries(F).filter(([Z])=>!T.length||T.includes(Z)).flatMap(([Z,ie])=>ie.filter(te=>!te.used).map(te=>({...te,_line:Z})));if(!V.length){B.log.push({text:`❌ Aucun joueur ${G}`,type:"info"}),S(B);return}W(V,k,`Choisir ${k} joueur(s) ${G}(s) à exclure`,Z=>{const ie="usedCardIds_"+N,te=new Set(B[ie]||[]);Z.forEach(ee=>{const re=(F[ee._line]||[]).find(se=>se.cardId===ee.cardId);re&&(re.used=!0,te.add(ee.cardId),B.log.push({text:`❌ ${re.name} exclu !`,type:"info"}))}),B[ie]=[...te],B[N+"Team"]=F,S(B)})},REVIVE_PLAYER:({count:k=1,roles:T=[]},z,B)=>{const S=z[p+"Team"],N=Object.entries(S).filter(([F])=>!T.length||T.includes(F)).flatMap(([F,G])=>G.filter(V=>V.used).map(V=>({...V,_line:F})));if(!N.length){z.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),B(z);return}W(N,k,`Choisir ${k} joueur(s) à ressusciter`,F=>{F.forEach(G=>{const V=(S[G._line]||[]).find(Z=>Z.cardId===G.cardId);V&&(V.used=!1,z.log.push({text:`💫 ${V.name} ressuscité !`,type:"info"}))}),z[p+"Team"]=S,B(z)})},REMOVE_GOAL:({},k,T)=>{const z=u+"Score";k[z]>0?(k[z]--,k.log.push({text:"🚫 Dernier but annulé !",type:"info"})):k.log.push({text:"🚫 Aucun but à annuler",type:"info"}),T(k)},ADD_GOAL_DRAW:({},k,T)=>{k[p+"Score"]===k[u+"Score"]?(k[p+"Score"]++,k.log.push({text:"🎯 But bonus !",type:"info"})):k.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),T(k)},ADD_SUB:({value:k=1},T,z)=>{T.maxSubs=(T.maxSubs||3)+k,T.log.push({text:`🔄 +${k} remplacement(s)`,type:"info"}),z(T)},CUSTOM:({},k,T)=>T(k)};function W(k,T,z,B){const S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let N=[];function F(){var V,Z;const G=k.map(ie=>{const te=ie._line||ie.job||"MIL",ee=N.find(se=>se.cardId===ie.cardId),re=Se({...ie,_evolution_bonus:0},{width:90,showStad:!0,role:te,extraNote:ie.boost||0});return`<div class="pp-item" data-cid="${ie.cardId}"
        ...
        </div>`,(V=S.querySelector("#pp-close"))==null||V.addEventListener("click",()=>S.remove()),S.querySelectorAll(".pp-item").forEach(ie=>{ie.addEventListener("click",()=>{const te=ie.dataset.cid,ee=k.find(se=>se.cardId===te),re=N.findIndex(se=>se.cardId===te);ee&&(re>-1?N.splice(re,1):N.length<T&&N.push(ee),F())})}),(Z=S.querySelector("#pp-confirm"))==null||Z.addEventListener("click",()=>{S.remove(),B(N)})}F(),document.body.appendChild(S)}async function ae(k,T){const B=(m["gcCardsFull_"+p]||[]).find(G=>G.id===k),S=(B==null?void 0:B._gcDef)||(m.gcDefs||[]).find(G=>{var V;return G.name===T||((V=G.name)==null?void 0:V.toLowerCase().trim())===(T==null?void 0:T.toLowerCase().trim())}),N=[...m["usedGc_"+p]||[],k],F={type:T,by:p,seq:(m._gcAnimSeq||0)+1};O.add(F.seq),$(T,p,async()=>{if(S!=null&&S.effect_type&&S.effect_type!=="CUSTOM"){const V=H[S.effect_type];if(V){const Z={...m};V(S.effect_params||{},Z,async ie=>{ie["usedGc_"+p]=N,ie._lastGC=F,ie._gcAnimSeq=F.seq,await A(ie)});return}}const G={...m};switch(T){case"Remplacement+":G.maxSubs=(G.maxSubs||3)+1,G.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const V=u+"Score";G[V]>0&&(G[V]--,G.log.push({text:"🚫 But annulé",type:"info"}));break}}G["usedGc_"+p]=N,G._lastGC=F,G._gcAnimSeq=F.seq,await A(G)})}function le(k,T){const z="usedCardIds_"+k,B=new Set(m[z]||[]);T.forEach(S=>B.add(S)),m[z]=[...B]}function j(){for(const k of["p1","p2"]){const T=new Set(m["usedCardIds_"+k]||[]),z=m[k+"Team"];if(z)for(const B of["GK","DEF","MIL","ATT"])(z[B]||[]).forEach(S=>{S.used=T.has(S.cardId)})}}function U(){var no,ro,ao,so,lo,co;if(m.phase==="reveal")return de();if(m.phase==="midfield")return ue();if(m.phase==="finished")return Ge();const k=m[p+"Team"],T=m[u+"Team"];j();const z=m[p+"Score"],B=m[u+"Score"],S=m[p+"Name"],N=m[u+"Name"],F=m.phase===p+"-attack",G=m.phase===p+"-defense",V=Array.isArray(m["selected_"+p])?m["selected_"+p]:[],Z=V.map(Q=>Q.cardId),ie=window.innerWidth>=700,te=m[p+"Subs"]||[],ee=m["usedSubIds_"+p]||[],re=te.filter(Q=>!ee.includes(Q.cardId)),se=m["gcCardsFull_"+p]||[],xe=m["usedGc_"+p]||[],ce=se.filter(Q=>!xe.includes(Q.id)),be=m.boostOwner===p&&!m.boostUsed,ve=[...k.MIL||[],...k.ATT||[]].filter(Q=>!Q.used),ne=[...T.MIL||[],...T.ATT||[]].filter(Q=>!Q.used),Le=!$e(T),Ne=ve.length===0&&ne.length===0&&!Le,Ie=(k.DEF||[]).filter(Q=>!Q.used),Ve=(k.GK||[]).filter(Q=>!Q.used);let Oe=[];F&&ve.length===0&&(Le?(Oe=Ie.map(Q=>Q.cardId),Ie.length===0&&(Oe=Oe.concat(Ve.map(Q=>Q.cardId)))):Ne&&(Oe=[...Ie,...Ve].map(Q=>Q.cardId)));function ot(Q,ge,Me){var Ce,mt;const _e=Q._gcDef,qt=(_e==null?void 0:_e.name)||Q.gc_type,at=(_e==null?void 0:_e.effect)||((Ce=Ae[Q.gc_type])==null?void 0:Ce.desc)||"",Fe=_e!=null&&_e.image_url?`/icons/${_e.image_url}`:null,Re=((mt=Ae[Q.gc_type])==null?void 0:mt.icon)||"⚡";return`<div class="pvp-gc-mini" data-gc-id="${Q.id}" data-gc-type="${Q.gc_type}" style="flex-shrink:0;cursor:pointer">
        ${tt(qt,Fe,Re,at,{width:ge})}
      </div>`}function Kt(Q,ge){return`<div id="pvp-boost-card"
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${Ot(k,m[p+"Formation"],ft,Z,ie?1300:At(),ie?600:Nt(),Oe)}
      </div>
        ...
        </div>`}const Q=(m.log||[]).slice(-1)[0];return Q?'<div style="padding:2px 4px">'+Zo(Q)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3);text-align:center">⏳ Match en cours...</div>'})(),oo=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-play-area" style="flex:1;min-height:0;display:flex;overflow:hidden">
          <div id="match-field" style="flex:1;min-width:0;min-height:0;overflow:hidden">
            <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
              ${Ot(k,m[p+"Formation"],ft,Z,At(),Nt(),Oe)}
            </div>
          </div>
        </div>
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
          <div style="display:flex;gap:6px;overflow-x:auto;align-items:flex-end;min-height:96px;padding-bottom:2px">
            ${ce.map(Q=>Ye(Q,!1)).join("")}
            ${be?Ye(null,!0):""}
      ...
      </div>`;function Bt(){const Q=t.querySelector(".match-screen");if(!Q)return;const ge=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);Q.style.height=ge+"px",Q.style.minHeight=ge+"px",Q.style.maxHeight=ge+"px",Q.style.overflow="hidden";const Me=t.querySelector("#mobile-action-bar"),_e=t.querySelector("#mobile-play-area");Me&&_e&&(_e.style.paddingBottom=Me.offsetHeight+"px")}if(Bt(),setTimeout(Bt,120),setTimeout(Bt,400),I||(I=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Bt),window.visualViewport.addEventListener("scroll",Bt)),window.addEventListener("resize",Bt)),function(){const ge=t.querySelector("#match-field .terrain-wrapper svg")||t.querySelector(".terrain-wrapper svg");if(!ge)return;const Me=ge.closest("#match-terrain-wrap");Me&&(Me.style.cssText="position:relative;width:100%;height:100%;padding:0"),ge.removeAttribute("width"),ge.removeAttribute("height"),ge.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ge.setAttribute("preserveAspectRatio","xMidYMid meet")}(),m._pvpResizeBound||(m._pvpResizeBound=!0,window.addEventListener("resize",()=>{const Q=t.querySelector(".terrain-wrapper svg");if(Q){const ge=Q.closest("#match-terrain-wrap");ge&&(ge.style.cssText="position:relative;width:100%;height:100%;padding:0"),Q.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0"}})),t.querySelectorAll(".match-slot-hit").forEach(Q=>{Q.addEventListener("click",()=>{if(!F&&!G)return;const ge=Q.dataset.cardId,Me=Q.dataset.role,_e=(k[Me]||[]).find(Re=>Re.cardId===ge);if(!_e||_e.used)return;const qt=Oe.includes(ge);if(F&&!["MIL","ATT"].includes(Me)&&!qt)return;Array.isArray(m["selected_"+p])||(m["selected_"+p]=[]);const at=m["selected_"+p],Fe=at.findIndex(Re=>Re.cardId===ge);Fe>-1?at.splice(Fe,1):at.length<3&&at.push({..._e,_role:Me}),U()})}),t.querySelectorAll(".match-used-hit").forEach(Q=>{Q.addEventListener("click",()=>P(Q.dataset.cardId))}),t.querySelectorAll(".pvp-sub-btn").forEach(Q=>{Q.addEventListener("click",()=>P())}),(no=t.querySelector("#pvp-sub-open"))==null||no.addEventListener("click",()=>P()),t.querySelectorAll(".pvp-gc-mini").forEach(Q=>{F?Q.addEventListener("click",()=>C(Q.dataset.gcId,Q.dataset.gcType)):(Q.style.opacity="0.35",Q.style.cursor="default",Q.addEventListener("click",()=>Lt("⚡ Les Game Changers ne sont utilisables qu'en attaque","rgba(180,100,0,0.9)")))}),(ro=t.querySelector("#pvp-boost-card"))==null||ro.addEventListener("click",()=>R()),(ao=t.querySelector("#pvp-action"))==null||ao.addEventListener("click",Q=>{F?Q.currentTarget.dataset.pass==="1"||!pt(p)?pe():Te():G&&ze()}),(so=t.querySelector("#pvp-quit"))==null||so.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&_()}),(lo=t.querySelector("#pvp-view-opp"))==null||lo.addEventListener("click",()=>q()),(co=t.querySelector("#pvp-toggle-history"))==null||co.addEventListener("click",()=>K()),E&&(clearInterval(E),E=null),Ue(),(F||G)&&!M){let Q=30,ge=!1;const Me=()=>document.getElementById("pvp-timer"),_e=()=>{Me()&&(Me().textContent=Q+"s",Me().style.color=ge?"#ff4444":"#fff")};_e(),E=setInterval(()=>{Q--,Q<0?ge?(clearInterval(E),E=null,Ue(),F&&!pt(p)?pe():_()):(ge=!0,Q=15,_e(),Bo("/sounds/timer-urgent.mp3",.6)):_e()},1e3)}}function de(){lt(t),t.innerHTML=`
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(te);let ee=!1;const re=()=>{ee||(ee=!0,te.remove(),setTimeout(()=>z&&z(),50))};te.addEventListener("click",re),setTimeout(re,3e3)}function C(k,T){var ie,te,ee,re;const B=(m["gcCardsFull_"+p]||[]).find(se=>se.id===k),S=B==null?void 0:B._gcDef,N=(S==null?void 0:S.name)||T,F=(S==null?void 0:S.effect)||((ie=Ae[T])==null?void 0:ie.desc)||"Carte spéciale.",G=S!=null&&S.image_url?`/icons/${S.image_url}`:null,V=((te=Ae[T])==null?void 0:te.icon)||"⚡",Z=document.createElement("div");Z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",Z.innerHTML=`
      ${tt(N,G,V,F,{width:190})}
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(Z),(ee=Z.querySelector("#pvp-gc-back"))==null||ee.addEventListener("click",()=>Z.remove()),(re=Z.querySelector("#pvp-gc-use"))==null||re.addEventListener("click",()=>{Z.remove(),ae(k,T)})}function R(){var B;const k=m[p+"Team"],T=Object.entries(k).flatMap(([S,N])=>(N||[]).filter(F=>!F.used).map(F=>({...F,_line:S})));if(!T.length)return;const z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",z.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${T.map(S=>{const N={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[S._line]||"#555",F=Pe(S,S._line)+(S.boost||0);return`<div class="bp-item" data-cid="${S.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${N};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(z),(B=z.querySelector("#bp-close"))==null||B.addEventListener("click",()=>z.remove()),z.querySelectorAll(".bp-item").forEach(S=>{S.addEventListener("click",async()=>{const N=S.dataset.cid,F=T.find(V=>V.cardId===N);if(!F)return;const G=(k[F._line]||[]).find(V=>V.cardId===N);G&&(G.boost=(G.boost||0)+m.boostValue),z.remove(),await A({[p+"Team"]:k,boostUsed:!0})})})}function P(k=null){var re,se;if(!(m.phase===p+"-attack")){g("Remplacement uniquement avant votre attaque","warning");return}const z=m[p+"Team"],B=m["usedSubIds_"+p]||[],S=m.maxSubs||3;if(B.length>=S){g(`Maximum ${S} remplacements atteint`,"warning");return}const N=Object.entries(z).flatMap(([xe,ce])=>(ce||[]).filter(be=>be.used).map(be=>({...be,_line:xe}))),F=(m[p+"Subs"]||[]).filter(xe=>!B.includes(xe.cardId));if(!N.length){g("Aucun joueur utilisé à remplacer","warning");return}if(!F.length){g("Aucun remplaçant disponible","warning");return}let G=Math.max(0,N.findIndex(xe=>xe.cardId===k));const V=((re=N[G])==null?void 0:re._line)||((se=N[G])==null?void 0:se.job);let Z=Math.max(0,F.findIndex(xe=>xe.job===V)),ie=!1;const te=document.createElement("div");te.id="pvp-sub-overlay",te.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function ee(){var Ne,Ie,Ve,Oe,ot,Kt;const xe=N[G],ce=F[Z],be=Math.min(130,Math.round((window.innerWidth-90)/2)),ve=Math.round(be*1.35),ne=De=>`background:rgba(255,255,255,0.12);border:none;color:${De?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${De?"default":"pointer"};flex-shrink:0`;te.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      </div>`,(Ne=te.querySelector("#psub-close"))==null||Ne.addEventListener("click",()=>te.remove()),(Ie=te.querySelector("#pout-up"))==null||Ie.addEventListener("click",()=>{G>0&&(G--,ee())}),(Ve=te.querySelector("#pout-down"))==null||Ve.addEventListener("click",()=>{G<N.length-1&&(G++,ee())}),(Oe=te.querySelector("#pin-up"))==null||Oe.addEventListener("click",()=>{Z>0&&(Z--,ee())}),(ot=te.querySelector("#pin-down"))==null||ot.addEventListener("click",()=>{Z<F.length-1&&(Z++,ee())});const Le=(De,Ye,He,ut)=>{const nt=te.querySelector("#"+De);if(!nt)return;let jt=0;nt.addEventListener("touchstart",ft=>{jt=ft.touches[0].clientY},{passive:!0}),nt.addEventListener("touchend",ft=>{const Et=ft.changedTouches[0].clientY-jt;if(Math.abs(Et)<30)return;const rt=Ye();Et<0&&rt<ut-1?(He(rt+1),ee()):Et>0&&rt>0&&(He(rt-1),ee())},{passive:!0})};Le("pin-panel",()=>Z,De=>Z=De,F.length),Le("pout-panel",()=>G,De=>G=De,N.length),(Kt=te.querySelector("#psub-confirm"))==null||Kt.addEventListener("click",async De=>{if(De.preventDefault(),De.stopPropagation(),ie)return;ie=!0;const Ye=N[G],He=F[Z];if(!Ye||!He)return;const ut=Ye._line,nt=(z[ut]||[]).findIndex(Et=>Et.cardId===Ye.cardId);if(nt===-1){g("Erreur : joueur introuvable","error"),te.remove();return}const jt={...He,_line:ut,position:Ye.position,used:!1,boost:0};z[ut].splice(nt,1,jt);const ft=[...B,He.cardId];te.remove(),X(Ye,He,async()=>{await A({[p+"Team"]:z,[u+"Team"]:m[u+"Team"],["usedSubIds_"+p]:ft})})})}document.body.appendChild(te),ee()}function X(k,T,z){const B={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const N=(V,Z,ie)=>`<div style="text-align:center">
      <div style="font-size:9px;color:${Z};letter-spacing:2px;text-transform:uppercase;font-weight:700;margin-bottom:6px">${ie}</div>
      <div style="width:70px;height:70px;border-radius:50%;background:${B[V.job]||"#555"};border:3px solid ${Z};position:relative;overflow:hidden;margin:0 auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(S);let F=!1;const G=()=>{F||(F=!0,S.remove(),setTimeout(()=>z(),50))};S.addEventListener("click",G),setTimeout(G,2200)}function q(){var T;const k=document.createElement("div");k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",k.innerHTML=`
      ...
      </div>`,document.body.appendChild(T),(z=T.querySelector("#pvp-hist-close"))==null||z.addEventListener("click",()=>T.remove())}async function pe(){if(m.phase!==p+"-attack")return;const k=p==="p1"?"p2":"p1",T=(m.round||0)+1,z=[...m.log||[]];z.push({type:"info",text:`⏭️ ${m[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const B=Ee(m),S=pt(k),N=B||!S?"finished":k+"-attack";await A({["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:N,attacker:k,round:T,log:z}),N==="finished"&&await qe(m)}function oe(k){const T=(k.GK||[]).some(B=>!B.used),z=["DEF","MIL","ATT"].some(B=>(k[B]||[]).some(S=>!S.used));return T&&!z}function me(k){return(k.p1Score||0)!==(k.p2Score||0)?null:oe(k.p1Team)&&!$e(k.p2Team)?{side:"p1",gk:(k.p1Team.GK||[]).find(T=>!T.used)}:oe(k.p2Team)&&!$e(k.p1Team)?{side:"p2",gk:(k.p2Team.GK||[]).find(T=>!T.used)}:null}async function we(){const k=document.createElement("div");k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(k);const T=(z,B)=>new Promise(S=>{k.innerHTML=`<div style="font-size:32px;font-weight:900;color:${B};letter-spacing:2px;animation:lcFade 1.4s ease both">${z}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(S,1400)});await T("⚽ DERNIER CORNER","#FFD700"),await T("🧤 LE GARDIEN MONTE !","#4fc3f7"),k.remove()}async function he(k,T){const{side:z,gk:B}=k;if(!B)return;const S=z+"Score",N=(T[S]||0)+1;B.used=!0;const F=[...T.log||[]];F.push({type:"duel",isGoal:!0,homeScored:z===p,homePlayers:z==="p1"?[Dt(B)]:[],aiPlayers:z==="p2"?[Dt(B)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${T[z+"Name"]} marque !`});const G=(T.round||0)+1,V=z==="p1"?"p2":"p1",Z={...T,[S]:N},ie=Ee(Z);D.add(G),await we();const te=z===p?N:T[p+"Score"]||0,ee=z===p?T[u+"Score"]||0:N;Qe([Dt(B)],te,ee,z===p,async()=>{await A({[S]:N,log:F,round:G,pendingAttack:null,phase:ie?"finished":V+"-attack",attacker:V,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}}}),ie&&await qe({...m,[S]:N})})}async function Te(){const k=m[p+"Team"],T=!["GK","DEF","MIL","ATT"].some(N=>(m[u+"Team"][N]||[]).some(F=>!F.used)),z=(m["selected_"+p]||[]).map(N=>{const F=(k[N._role]||[]).find(se=>se.cardId===N.cardId)||N,G=["GK","DEF"].includes(N._role),V=k[N._role]||[],Z=V.findIndex(se=>se.cardId===N.cardId),ie=st(V.length),te=Z>=0?ie[Z]:F._col??1,ee=m.stadiumDef||m.homeStadiumDef||null,re=F.stadiumBonus||ee&&(ee.club_id&&String(F.club_id)===String(ee.club_id)||ee.country_code&&F.country_code===ee.country_code)||!1;return{...F,_line:N._role,_col:te,stadiumBonus:re,...G?{note_a:Math.max(1,Number(F.note_a)||0)}:{}}});if(!z.length)return;const B=oi(z,m.modifiers[p]||{},m[p+"Formation"]);le(p,z.map(N=>N.cardId)),z.forEach(N=>{const F=(k[N._role]||[]).find(G=>G.cardId===N.cardId);F&&(F.used=!0)}),m["selected_"+p]=[],U();const S=[...m.log||[]];if(T){if(z.length===1&&(z[0]._line==="GK"||z[0].job==="GK")&&(m.p1Score||0)===(m.p2Score||0)){await he({side:p,gk:z[0]},{...m,[p+"Team"]:k,log:S});return}const F=(m[p+"Score"]||0)+1,G=z.map(ee=>Dt(ee));S.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:G,homeTotal:B.total,aiTotal:0});const V=(m.round||0)+1,Z=p==="p1"?"p2":"p1",ie={...m,[p+"Team"]:k,[p+"Score"]:F},te=Ee(ie);D.add(V),Qe(G,F,m[u+"Score"]||0,!0,async()=>{await A({[p+"Team"]:k,[p+"Score"]:F,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:te?"finished":Z+"-attack",attacker:Z,round:V,log:S}),te&&await qe({...m,[p+"Score"]:F})});return}S.push({type:"pending",text:`⚔️ ${m[p+"Name"]} attaque (${B.total})`}),await A({[p+"Team"]:k,[u+"Team"]:m[u+"Team"],pendingAttack:{...B,players:z,side:p},["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},phase:u+"-defense",log:S})}async function ze(){const k=m[p+"Team"],T=(m["selected_"+p]||[]).map(ce=>{const be=(k[ce._role]||[]).find(Ie=>Ie.cardId===ce.cardId)||ce,ve=k[ce._role]||[],ne=ve.findIndex(Ie=>Ie.cardId===ce.cardId),Le=st(ve.length),Ne=ne>=0?Le[ne]:be._col??1;return{...be,_line:ce._role,_col:Ne}}),z=ni(T,m.modifiers[p]||{},m[p+"Formation"]);le(p,T.map(ce=>ce.cardId)),T.forEach(ce=>{const be=(k[ce._role]||[]).find(ve=>ve.cardId===ce.cardId);be&&(be.used=!0)}),m["selected_"+p]=[],U();const B=Qi(m.pendingAttack.total,z.total,m.modifiers[p]||{}),S=m.pendingAttack.side,N=B==="attack"||(B==null?void 0:B.goal),F=S==="p1"?"p2":"p1",G=(m.round||0)+1,V=(m.pendingAttack.players||[]).map(ce=>Dt(ce)),Z=[...m.log||[]];Z.push({type:"duel",isGoal:N,homeScored:N&&S===p,text:N?`⚽ BUT de ${m[S+"Name"]} ! (${m.pendingAttack.total} vs ${z.total})`:`✋ Attaque stoppée (${m.pendingAttack.total} vs ${z.total})`,homePlayers:V,aiPlayers:T.map(ce=>Dt(ce)),homeTotal:m.pendingAttack.total,aiTotal:z.total});const ie=N?(m[S+"Score"]||0)+1:m[S+"Score"]||0,te={...m,[p+"Team"]:k,[S+"Score"]:ie},ee=Ee(te),re=ee?"finished":F+"-attack",se=!ee&&!N?me(te):null,xe=async()=>{if(se){await he(se,te);return}await A({[p+"Team"]:k,[u+"Team"]:m[u+"Team"],[S+"Score"]:ie,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:re,attacker:F,round:G,log:Z}),(re==="finished"||ee)&&await qe({...m,[S+"Score"]:ie})};if(N){const ce=S===p,be=ce?ie:m[p+"Score"]||0,ve=ce?m[u+"Score"]||0:ie;D.add(G),Qe(V,be,ve,ce,xe)}else await xe()}function Be(k){return["MIL","ATT"].some(T=>(k[T]||[]).some(z=>!z.used))}function $e(k){return["GK","DEF","MIL","ATT"].some(T=>(k[T]||[]).some(z=>!z.used))}function $t(k){return $e(k)&&!Be(k)}function pt(k){const T=m[k+"Team"],z=m[(k==="p1"?"p2":"p1")+"Team"];return!!(Be(T)||$t(T)&&!$e(z)||$t(T)&&!Be(z)&&$e(z))}function Ee(k){return!$e(k.p1Team)&&!$e(k.p2Team)}function je(k){const T=k.p1Score||0,z=k.p2Score||0;return T===z?null:T>z?h.home_id:h.away_id}async function qe(k){Gt(),Ue();try{const T=je(k),z=T?h.home_id===T?h.away_id:h.home_id:null,B=k.p1Score||0,S=k.p2Score||0,{data:N,error:F}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:T,p_home_score:B,p_away_score:S,p_forfeit:!1});if(F&&console.error("[PvP] finish_pvp_match:",F),c)try{await v.rpc("check_and_finish_mini_league",{p_league_id:c})}catch(G){console.error("[PvP] check_and_finish_mini_league:",G)}T&&z&&Fr(T,z).catch(()=>{});try{const{data:G}=await v.rpc("apply_match_rewards",{p_match_id:i});G!=null&&G.success&&!(G!=null&&G.skipped)&&typeof e.refreshProfile=="function"&&await e.refreshProfile()}catch(G){console.error("[PvP] apply_match_rewards:",G)}if(typeof l=="function")try{await l({homeScore:B,awayScore:S,winnerId:T,homeId:h.home_id,awayId:h.away_id,matchId:i})}catch(G){console.error("[PvP] onMatchEnd:",G)}}catch(T){console.error("[PvP] finishMatch:",T)}}function Ge(){var S;const k=m[p+"Score"],T=m[u+"Score"],z=k>T,B=k===T;lt(t),t.innerHTML=`
    ...
    </div>`,(S=document.getElementById("pvp-home"))==null||S.addEventListener("click",()=>{try{v.removeChannel(J)}catch{}it(t),c?x("mini-league",{openLeagueId:c}):x("home")})}U()}async function Ni(t,e,i=!1){var s,d;const{state:o,navigate:a,toast:n}=e,r=i&&((d=(s=e==null?void 0:e.state)==null?void 0:s.params)==null?void 0:d.rankedData)||null;await $i(t,e,i?"ranked":"random",async({deckId:l,formation:c,starters:f,subsRaw:b,gcCardsEnriched:x,gcDefs:g,stadiumDef:p})=>{lt(t);const u=async y=>{await Nr(t,e,l,c,f,b,y||[],g,p,i,r)};if(!(x!=null&&x.length)){await u([]);return}_i(t,x,u)})}async function Gr(t,e,i){return Pr(t,e,i)}async function Nr(t,e,i,o,a,n,r=[],s=[],d=null,l=!1,c=null){var Y,J;const{state:f,navigate:b,toast:x}=e,g=(c==null?void 0:c.mmr)??((Y=f.profile)==null?void 0:Y.mmr)??1e3,p=l?wi(g):null,u=l?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",y=l?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let w=!1,h=null,L=null;t.innerHTML=`
    ...
    </div>`;const m=async()=>{w=!0,h&&(h.unsubscribe(),h=null),L&&(clearInterval(L),L=null);try{await v.rpc("cancel_matchmaking",{p_user_id:f.profile.id})}catch(A){console.error("[Matchmaking] cancel error:",A)}};(J=document.getElementById("cancel-mm"))==null||J.addEventListener("click",async()=>{await m(),it(t),b("home")});const M=async(A,_)=>{w||(w=!0,h&&(h.unsubscribe(),h=null),L&&(clearInterval(L),L=null),await Ct(t,e,A,_,{myGC:r,gcDefs:s,stadiumDef:d,isRanked:l,rankedData:c,onMatchEnd:l?Rr:null}))},E=f.user.id,{data:I,error:D}=l?await v.rpc("try_matchmake_ranked",{p_user_id:E,p_deck_id:i,p_mmr:g}):await v.rpc("try_matchmake",{p_user_id:E,p_deck_id:i});if(D||!(I!=null&&I.success)){console.error("[Matchmaking] try_matchmake error:",D||I),x("Erreur de matchmaking","error"),it(t),b("home");return}if(I.matched){await M(I.match_id,!1);return}const O=async()=>{if(w)return;const{data:A}=await v.from("matchmaking_queue").select("status, match_id").eq("user_id",E).maybeSingle();(A==null?void 0:A.status)==="matched"&&A.match_id&&await M(A.match_id,!0)};h=v.channel(`mm_${E}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${E}`},A=>{const _=A.new;_.status==="matched"&&_.match_id&&M(_.match_id,!0)}).subscribe(),L=setInterval(O,3e3),setTimeout(()=>{w||m()},12e4)}async function Rr(t,e){const{state:i,toast:o}=e,{winnerId:a,homeId:n,awayId:r,homeScore:s,awayScore:d,matchId:l}=t,c=i.user.id,f=n===c,b=a===c,{data:x}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",c).single(),{data:g}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",f?r:n).single();if(x&&g){const p=fn(x.mmr,x.mmr_rd,x.mmr_v,g.mmr,g.mmr_rd,b?1:0);await v.from("users").update({mmr:p.mmr,mmr_rd:p.rd,mmr_v:p.v,mmr_wins:b?v.sql`mmr_wins + 1`:void 0,mmr_losses:b?void 0:v.sql`mmr_losses + 1`}).eq("id",c),o(b?`+MMR ↑ ${Math.round(p.mmr-x.mmr)}`:`-MMR ↓ ${Math.round(x.mmr-p.mmr)}`,b?"success":"error",4e3)}}const Or=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:Ni,resumePvpMatch:Gr},Symbol.toStringTag,{value:"Module"}));async function Hr(t,e){const{state:i,navigate:o,toast:a}=e,n=i.params||{},r=n.friendId||null,s=n.friendName||"Ami",d=!!n.isAccepting;await $i(t,e,"friend",async({deckId:l,formation:c,starters:f,subsRaw:b,gcCardsEnriched:x,gcDefs:g,stadiumDef:p})=>{lt(t);const u=async y=>{const w=y||[];d?await Kr(t,e,l,w,g,p):await Ur(t,e,l,w,g,p,r,s)};if(!(x!=null&&x.length)){await u([]);return}_i(t,x,u)})}async function Ur(t,e,i,o,a,n,r,s){var w;const{state:d,navigate:l,toast:c}=e,f=d.user.id,{data:b,error:x}=await v.from("friend_match_invites").insert({inviter_id:f,invitee_id:r,friend_id:r,status:"pending",inviter_deck_id:i,expires_at:new Date(Date.now()+2*60*1e3).toISOString()}).select("id").single();if(x||!b){console.error("[Friend] Erreur création invitation:",x),c("Impossible de créer l'invitation","error"),l("home");return}t.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="display:flex;background:var(--nav-bg,#0d1a0f);border-bottom:1px solid ${It}">
      ${w.map(m=>`<button class="ml-tab" data-tab="${m.key}" style="flex:1;padding:11px 4px;border:none;border-bottom:2px solid ${i===m.key?rn:"transparent"};background:none;font-size:12px;font-weight:${i===m.key?"900":"600"};color:${i===m.key?"#4ade80":Vr};cursor:pointer">${m.label}${m.count?` (${m.count})`:""}</button>`).join("")}
    </div>
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?Wr(x,y,a):i==="active"?Xr(g,a):Jr(u,a,s)}
    </div>
  </div>`,(h=document.getElementById("ml-create-btn"))==null||h.addEventListener("click",()=>Qr(t,e)),(L=document.getElementById("ml-refresh-list"))==null||L.addEventListener("click",()=>et(t,e,i)),t.querySelectorAll(".ml-tab").forEach(m=>m.addEventListener("click",()=>et(t,e,m.dataset.tab))),t.querySelectorAll("[data-league-id]").forEach(m=>m.addEventListener("click",()=>ct(t,e,m.dataset.leagueId))),t.querySelectorAll("[data-join]").forEach(m=>m.addEventListener("click",M=>{M.stopPropagation(),sn(t,e,m.dataset.join,m.dataset.type)})),t.querySelectorAll("[data-delete]").forEach(m=>m.addEventListener("click",M=>{M.stopPropagation(),Ri(t,e,m.dataset.delete,m.dataset.name,i)})),t.querySelectorAll("[data-claim-league]").forEach(m=>m.addEventListener("click",async M=>{M.stopPropagation(),m.disabled=!0,m.textContent="...";const{data:E,error:I}=await v.rpc("claim_mini_league_prize",{p_league_id:m.dataset.claimLeague,p_user_id:o.profile.id});if(I||!(E!=null&&E.success)){e.toast((E==null?void 0:E.error)||"Erreur lors de la récupération","error"),et(t,e,i);return}if(!E.already_claimed){o.profile.credits=(o.profile.credits||0)+E.prize;const D=document.getElementById("nav-credits");D&&(D.textContent=`💰 ${o.profile.credits.toLocaleString("fr")}`),e.toast(`💰 +${E.prize.toLocaleString("fr")} cr. ajoutés !`,"success")}et(t,e,i)}))}function vi(t,e,i=!1,o=null){var d,l;const a=t.creator_id===e,n=t.pot||0,r=((l=(d=t.mini_league_members)==null?void 0:d[0])==null?void 0:l.count)||0,s=o&&o.amount>0&&!o.claimed;return`<div data-league-id="${t.id}" style="background:${Jt};border:1px solid ${s?"rgba(212,160,23,0.5)":Qt};border-radius:12px;padding:14px 16px;cursor:pointer;margin-bottom:8px">
  ...
  </div>`}function Wr(t,e,i){const o=[];return t.length&&(o.push(`<div style="font-size:11px;font-weight:700;color:${Ze};text-transform:uppercase;letter-spacing:1px">Mes leagues en attente</div>`),o.push(...t.map(a=>vi(a,i,!1)))),e.length&&(o.push(`<div style="font-size:11px;font-weight:700;color:${Ze};text-transform:uppercase;letter-spacing:1px;margin-top:4px">Rejoindre</div>`),o.push(...e.map(a=>vi(a,i,!0)))),o.length?o.join(""):`<div style="text-align:center;padding:40px;color:${Ze}">🏆<br>Aucune mini league.<br>Crée la première !</div>`}function Xr(t,e){return t.length?t.map(i=>vi(i,e)).join(""):`<div style="text-align:center;padding:40px;color:${Ze}">Aucune mini league en cours.</div>`}function Jr(t,e,i={}){return t.length?t.map(o=>vi(o,e,!1,i[o.id])).join(""):`<div style="text-align:center;padding:40px;color:${Ze}">Aucune mini league archivée.</div>`}function Qr(t,e){const i=document.createElement("div");i.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px",i.innerHTML=`<div style="background:#fff;border-radius:16px;width:100%;max-width:400px;max-height:90vh;overflow-y:auto">
    ...
    <div style="padding:20px;display:flex;flex-direction:column;gap:14px">
    ...
    </div></div>`,document.body.appendChild(i),i.querySelector("#ml-form-close").addEventListener("click",()=>i.remove()),i.querySelectorAll('input[name="ml-type"]').forEach(o=>o.addEventListener("change",()=>{document.getElementById("ml-pwd-block").style.display=o.value==="private"?"block":"none"})),i.querySelector("#ml-create-confirm").addEventListener("click",async()=>{var g,p,u,y;const{toast:o}=e,a=document.getElementById("ml-name").value.trim(),n=parseInt(document.getElementById("ml-max").value)||6,r=parseInt(document.getElementById("ml-fee").value)||500,s=((g=i.querySelector('input[name="ml-type"]:checked'))==null?void 0:g.value)||"public",d=((p=i.querySelector('input[name="ml-mode"]:checked'))==null?void 0:p.value)||"aller",l=((y=(u=document.getElementById("ml-pwd"))==null?void 0:u.value)==null?void 0:y.trim())||null;if(!a){o("Le nom est obligatoire","error");return}if(r<100){o("Mise minimum : 100 crédits","error");return}if(s==="private"&&!l){o("Mot de passe requis","error");return}const{data:c,error:f}=await v.from("mini_leagues").insert({name:a,creator_id:e.state.profile.id,type:s,password:l,mode:d,max_players:n,entry_fee:r}).select().single();if(f){o("Erreur : "+f.message,"error");return}const{data:b}=await v.from("users").select("credits").eq("id",e.state.profile.id).single();if(((b==null?void 0:b.credits)||0)<r){await v.from("mini_leagues").delete().eq("id",c.id),o(`Crédits insuffisants pour la mise (${r.toLocaleString("fr")} cr.)`,"error");return}await v.from("users").update({credits:b.credits-r}).eq("id",e.state.profile.id),await v.from("mini_leagues").update({pot:r}).eq("id",c.id),await v.from("mini_league_members").insert({league_id:c.id,user_id:e.state.profile.id}),e.state.profile&&(e.state.profile.credits=b.credits-r);const x=document.getElementById("nav-credits");x&&(x.textContent=`💰 ${(e.state.profile.credits||0).toLocaleString("fr")}`),i.remove(),o(`Mini League créée ! ${r.toLocaleString("fr")} cr. déduits.`,"success"),ct(t,e,c.id)})}function Zr(){return new Promise(t=>{const e=document.createElement("div");e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9500;display:flex;align-items:center;justify-content:center;padding:20px",e.innerHTML=`
      ...
      </div>`,document.body.appendChild(e);const i=e.querySelector("#ml-pwd-input"),o=a=>{e.remove(),t(a)};setTimeout(()=>i==null?void 0:i.focus(),80),e.querySelector("#pwd-cancel").addEventListener("click",()=>o(null)),e.querySelector("#pwd-ok").addEventListener("click",()=>o(i.value.trim())),i.addEventListener("keydown",a=>{a.key==="Enter"&&o(i.value.trim())})})}async function sn(t,e,i,o){var f;const{toast:a,state:n}=e,r=n.profile.id;let s=null;if(o==="private"&&(s=await Zr(),s===null))return;const{data:d,error:l}=await v.rpc("join_mini_league",{p_league_id:i,p_user_id:r,p_password:s});if(l||!(d!=null&&d.success)){console.error("[MiniLeague] join_mini_league:",l||d),a((d==null?void 0:d.error)||"Erreur lors de l'inscription","error");return}if(typeof e.refreshProfile=="function")await e.refreshProfile();else{const{data:b}=await v.from("users").select("credits").eq("id",r).single();n.profile&&b&&(n.profile.credits=b.credits)}const c=document.getElementById("nav-credits");c&&(c.textContent=`💰 ${(((f=n.profile)==null?void 0:f.credits)||0).toLocaleString("fr")}`),a("✅ Inscrit ! Pot mis à jour.","success"),ct(t,e,i)}async function ea(t,e,i,o){const{toast:a,state:n}=e,r=n.profile.id;if(!await new Promise(b=>{const x=document.createElement("div");x.className="modal-overlay",x.style.zIndex="2100",x.innerHTML=`<div class="modal" style="max-width:360px">
    ...
    </div>`,document.body.appendChild(x),x.querySelector("#lv-ok").onclick=()=>{x.remove(),b(!0)},x.querySelector("#lv-cancel").onclick=()=>{x.remove(),b(!1)}}))return;const{data:d,error:l}=await v.rpc("leave_mini_league",{p_league_id:i,p_user_id:r});if(l||!(d!=null&&d.success)){a((d==null?void 0:d.error)||"Erreur lors de la désinscription","error");return}const c=d.refund||o;n.profile&&(n.profile.credits=(n.profile.credits||0)+c);const f=document.getElementById("nav-credits");f&&(f.textContent=`💰 ${(n.profile.credits||0).toLocaleString("fr")}`),a(`↩️ Désinscrit · +${c.toLocaleString("fr")} cr. remboursés`,"success"),et(t,e,"waiting")}async function Ri(t,e,i,o,a){const{toast:n}=e;if(!confirm(`Supprimer définitivement "${o}" et tous ses matchs/membres ? Action irréversible.`))return;await v.from("mini_league_matches").delete().eq("league_id",i),await v.from("mini_league_members").delete().eq("league_id",i);const{error:r}=await v.from("mini_leagues").delete().eq("id",i);if(r){n("Erreur suppression ("+r.message+")","error");return}n("Mini League supprimée avec succès","success"),et(t,e,a)}async function ta(t,e,i){await v.from("mini_leagues").update({is_archived:!0}).eq("id",i),e.toast("Mini League archivée","success"),et(t,e,"archived")}async function ct(t,e,i){var L,m,M,E,I,D,O,Y,J;const{state:o,toast:a}=e,n=o.profile.id,[{data:r},{data:s},{data:d}]=await Promise.all([v.from("mini_leagues").select("*").eq("id",i).single(),v.from("mini_league_members").select("*, user:users(id,pseudo,club_name,club_color1,club_color2)").eq("league_id",i),v.from("mini_league_matches").select("*").eq("league_id",i).order("matchday").order("created_at")]);if(!r){a("Introuvable","error"),await et(t,e);return}const l=(s||[]).some(A=>A.user_id===n),c=r.creator_id===n,f=(s||[]).map(A=>A.user).filter(Boolean),b=aa(f,d||[]),x=(d||[]).filter(A=>A.matchday===r.current_day),g=r.pot||0,p=r.entry_fee||100,u=x.length>0&&x.every(A=>A.status==="finished"||A.status==="bye"),y=r.current_day>=r.total_days,w=(s||[]).find(A=>A.user_id===n);t.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        ${f.map(A=>`
          <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid ${It}">
            <div style="width:36px;height:36px;border-radius:50%;background:${A.club_color2||rn};display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:${A.club_color1||"#fff"}">${(A.pseudo||"?").slice(0,2).toUpperCase()}</div>
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${b.map((A,_)=>{const H=g>0&&r.status==="finished"?_===0?Math.floor(g*.7):_===1?Math.floor(g*.2):_===2?Math.floor(g*.1):0:0;return`<tr style="border-bottom:1px solid ${It};${A.userId===n?"background:rgba(74,222,128,0.08);":""}">
              <td style="padding:7px 3px 7px 0;font-weight:700;color:${_===0?dt:_<3?"#4ade80":Tt}">${["🥇","🥈","🥉"][_]||_+1}</td>
              ...
              <td style="text-align:center;color:${Tt}">${A.played}</td><td style="text-align:center;color:${Tt}">${A.won}-${A.drawn}-${A.lost}</td>
              ...
              ${g>0&&r.status==="finished"?`<td style="text-align:right;font-weight:700;color:${dt}">${H?H.toLocaleString("fr")+" cr.":"—"}</td>`:""}
            </tr>`}).join("")}</tbody>
        </table>
      </div>`:""}
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,r.status==="active"?r.current_day-1:r.current_day)},(A,_)=>_+1).reverse().map(A=>{const _=(d||[]).filter(H=>H.matchday===A);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${Ze};margin-bottom:6px">Journée ${A}</div>${_.map(H=>Eo(H,f,n,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  </div>`}async function ia(t,e,i,o){const{toast:a,state:n}=e,r=ra(o.map(l=>l.id),i.mode),s=[];r.forEach((l,c)=>l.forEach(f=>s.push({league_id:i.id,matchday:c+1,home_id:f.home||f.bye,away_id:f.away||null,is_bye:!!f.bye,status:f.bye?"bye":"pending"})));const{error:d}=await v.from("mini_league_matches").insert(s);if(d){a("Erreur calendrier : "+d.message,"error");return}await v.from("mini_leagues").update({status:"active",current_day:1,total_days:r.length}).eq("id",i.id),a(`🚀 Lancée ! Pot : ${(i.pot||0).toLocaleString("fr")} cr.`,"success"),ct(t,e,i.id)}async function oa(t,e,i){const{data:o}=await v.from("mini_leagues").select("current_day,total_days,pot").eq("id",i).single(),a=(o.current_day||0)+1;if(a>(o.total_days||0)){ct(t,e,i);return}await v.from("mini_leagues").update({current_day:a}).eq("id",i),e.toast(`Journée ${a} commencée !`,"success"),ct(t,e,i)}async function na(t,e,i,o,a){var b,x;const{state:n,toast:r}=e,s=[Math.floor((i.pot||0)*.7),Math.floor((i.pot||0)*.2),Math.floor((i.pot||0)*.1)],d=["🥇","🥈","🥉"][a],l=o.prize_amount||s[a]||0,c=o.prize_claimed,f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",f.innerHTML=`
    <div style="background:linear-gradient(160deg,#1a0a2e,#3b1a6e);border-radius:20px;padding:32px 24px;max-width:320px;width:100%;text-align:center;color:#fff;box-shadow:0 8px 40px rgba(0,0,0,0.6)">
      ...
      <div style="background:rgba(212,160,23,0.2);border:2px solid ${dt};border-radius:14px;padding:16px;margin-bottom:24px">
        ...
        <div style="font-size:32px;font-weight:900;color:${dt}">${l.toLocaleString("fr")} cr.</div>
      ...
      ${c?'<div style="font-size:13px;color:#86efac;margin-bottom:16px">✅ Crédits déjà récupérés</div>':`<button id="claim-prize-btn" style="width:100%;padding:14px;border-radius:12px;border:none;background:${dt};color:#111;font-size:16px;font-weight:900;cursor:pointer;margin-bottom:12px">💰 Récupérer ${l.toLocaleString("fr")} cr.</button>`}
      <button id="prize-close" style="background:rgba(255,255,255,0.1);border:none;color:rgba(255,255,255,0.7);padding:10px 24px;border-radius:10px;font-size:14px;cursor:pointer">Fermer</button>
    </div>`,document.body.appendChild(f),(b=f.querySelector("#prize-close"))==null||b.addEventListener("click",()=>f.remove()),(x=f.querySelector("#claim-prize-btn"))==null||x.addEventListener("click",async g=>{const p=g.currentTarget;p.disabled=!0,p.textContent="...";const{data:u,error:y}=await v.rpc("claim_mini_league_prize",{p_league_id:i.id,p_user_id:n.profile.id});if(y||!(u!=null&&u.success)){console.error("[MiniLeague] claim_mini_league_prize:",y||u),r((u==null?void 0:u.error)||"Erreur lors de la récupération","error"),p.disabled=!1,p.textContent=`💰 Récupérer ${l.toLocaleString("fr")} cr.`;return}if(u.already_claimed)r("Déjà récupéré précédemment","info");else{const w=(n.profile.credits||0)+u.prize;n.profile&&(n.profile.credits=w);const h=document.getElementById("nav-credits");h&&(h.textContent=`💰 ${w.toLocaleString("fr")}`),r(`💰 +${u.prize.toLocaleString("fr")} cr. ajoutés à ton solde !`,"success")}f.remove(),ct(t,e,i.id)})}function ra(t,e){const o=t.length%2===0?[...t]:[...t,null],a=o.length;let n=o.slice(1);const r=[];for(let s=0;s<a-1;s++){const d=[],l=[o[0],...n];for(let c=0;c<a/2;c++){const f=l[c],b=l[a-1-c];f===null?d.push({bye:b}):b===null?d.push({bye:f}):d.push({home:f,away:b})}r.push(d),n=[n[n.length-1],...n.slice(0,-1)]}return e==="aller-retour"?[...r,...r.map(s=>s.map(d=>d.bye?d:{home:d.away,away:d.home}))]:r}function aa(t,e){const i={};return t.forEach(o=>{i[o.id]={userId:o.id,pseudo:o.pseudo,clubName:o.club_name||o.pseudo,played:0,won:0,drawn:0,lost:0,goalsFor:0,goalsAgainst:0,goalDiff:0,points:0}}),e.filter(o=>o.status==="finished"&&!o.is_bye&&o.home_score!=null).forEach(o=>{const a=i[o.home_id],n=i[o.away_id];!a||!n||(a.played++,n.played++,a.goalsFor+=o.home_score,a.goalsAgainst+=o.away_score,n.goalsFor+=o.away_score,n.goalsAgainst+=o.home_score,o.home_score>o.away_score?(a.won++,a.points+=3,n.lost++):o.home_score<o.away_score?(n.won++,n.points+=3,a.lost++):(a.drawn++,a.points++,n.drawn++,n.points++),a.goalDiff=a.goalsFor-a.goalsAgainst,n.goalDiff=n.goalsFor-n.goalsAgainst)}),Object.values(i).sort((o,a)=>a.points-o.points||a.goalDiff-o.goalDiff||a.goalsFor-o.goalsFor)}async function Lo(t,e){const{state:i,navigate:o,toast:a}=e,n=i.params||{},r=n.leagueId||null,s=n.mlMatchId||null,d=i.user.id;if(!s||!r){a("Match introuvable","error"),o("mini-league");return}const{data:l,error:c}=await v.from("mini_league_matches").select("id, league_id, home_id, away_id, status, match_id").eq("id",s).single();if(c||!l){a("Match introuvable","error"),o("mini-league",{openLeagueId:r});return}if(l.home_id!==d&&l.away_id!==d){a("Vous ne faites pas partie de ce match","error"),o("mini-league",{openLeagueId:r});return}const f=l.home_id===d;if(l.match_id){await Ct(t,e,l.match_id,f,{mlLeagueId:r,mlMatchId:s});return}await $i(t,e,"mini_league",async({deckId:b,gcCardsEnriched:x,gcDefs:g,stadiumDef:p})=>{lt(t);const u=async y=>{const{data:w,error:h}=await v.rpc("start_mini_league_match",{p_ml_match_id:s,p_user_id:d,p_deck_id:b});if(h||!(w!=null&&w.success)){console.error("[MiniLeague] start_mini_league_match error:",h||w),a((w==null?void 0:w.error)||"Impossible de lancer le match","error"),o("mini-league",{openLeagueId:r});return}if(w.matched){await Ct(t,e,w.match_id,f,{mlLeagueId:r,mlMatchId:s,myGC:y||[],gcDefs:g,stadiumDef:p});return}await sa(t,e,{mlMatchId:s,leagueId:r,amIHome:f,chosenGC:y,gcDefs:g,stadiumDef:p})};if(!(x!=null&&x.length)){await u([]);return}_i(t,x,u)})}function sa(t,e,{mlMatchId:i,leagueId:o,amIHome:a,chosenGC:n,gcDefs:r,stadiumDef:s}){return new Promise(d=>{var p;const{navigate:l}=e;t.innerHTML=`
    <div style="min-height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;background:linear-gradient(135deg,#0a1a2e,#0d3d1e);color:#fff;padding:32px;text-align:center;gap:20px">
      <div style="font-size:36px">🏆</div>
    ...
    </div>`;let c=!1;const f=async u=>{if(!c){if(c=!0,clearInterval(g),x)try{x.unsubscribe()}catch{}u&&await Ct(t,e,u,a,{mlLeagueId:o,mlMatchId:i,myGC:n||[],gcDefs:r,stadiumDef:s}),d()}};(p=document.getElementById("wait-back"))==null||p.addEventListener("click",()=>{if(c=!0,clearInterval(g),x)try{x.unsubscribe()}catch{}l("mini-league",{openLeagueId:o}),d()});const b=async()=>{if(c)return;const{data:u}=await v.from("mini_league_matches").select("match_id").eq("id",i).single();u!=null&&u.match_id&&f(u.match_id)},x=v.channel(`ml_match_${i}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"mini_league_matches",filter:`id=eq.${i}`},u=>{var y;(y=u.new)!=null&&y.match_id&&f(u.new.match_id)}).subscribe(),g=setInterval(b,3e3);b()})}const la="/",da=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function ca(t,e,i){let o=0;const a=document.createElement("div");a.id="tutorial-overlay",a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const n=()=>{var c,f,b;const s=e[o],d=o===e.length-1,l=Math.round((o+1)/e.length*100);a.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div id="mkt-content" style="padding:10px 16px;display:flex;flex-direction:column;gap:8px"></div>
    ...
    </div>`}function b(p){var I,D,O,Y;const u=(I=p.card)==null?void 0:I.player;if(!u)return"";const y=((D=p.card)==null?void 0:D.evolution_bonus)||0,w=ui(u,u.job,y),h=u.job2?ui(u,u.job2,y):0,L=p.status==="sold",m=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,M=To[u.job]||"#bbb",E=u.job2?To[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${L?"opacity:0.7":""}">
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:10px;color:${L?"#22c55e":"#999"};margin-top:1px">
          ${L?`✅ Vendu à ${((Y=p.buyer)==null?void 0:Y.pseudo)||"—"} · ${p.sold_at?new Date(p.sold_at).toLocaleDateString("fr"):""}`:`🟢 En vente depuis le ${new Date(p.listed_at).toLocaleDateString("fr")}`}
        </div>
      </div>
      <div style="text-align:right;flex-shrink:0">
    ...
    </div>`}function x(){const p=document.getElementById("mkt-content"),u=document.getElementById("mkt-filters");if(p){if(u.style.display=d==="buy"?"flex":"none",d==="buy"){const y=c(r);p.innerHTML=y.length?`<div class="mkt-buy-grid">${y.map(f).join("")}</div>`:'<div style="text-align:center;color:#aaa;padding:40px">Aucune carte trouvée.</div>'}else{const y=s.filter(h=>h.status==="active").sort((h,L)=>new Date(L.listed_at)-new Date(h.listed_at)),w=s.filter(h=>h.status==="sold").sort((h,L)=>new Date(L.sold_at||L.listed_at)-new Date(h.sold_at||h.listed_at));p.innerHTML=(y.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:4px 0 6px;text-transform:uppercase;letter-spacing:1px">🟢 En vente (${y.length})</div>`+y.map(b).join(""):"")+(w.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:12px 0 6px;text-transform:uppercase;letter-spacing:1px">✅ Ventes réussies (${w.length})</div>`+w.map(b).join(""):"")+(!y.length&&!w.length?'<div style="text-align:center;color:#aaa;padding:40px">Aucune vente pour le moment.</div>':"")}p.querySelectorAll("[data-buy]").forEach(y=>y.addEventListener("click",()=>fa(y.dataset.buy,r,t,e))),p.querySelectorAll("[data-cancel]").forEach(y=>y.addEventListener("click",()=>ga(y.dataset.cancel,t,e)))}}t.querySelectorAll(".mkt-tab").forEach(p=>{p.addEventListener("click",()=>{d=p.dataset.tab,t.querySelectorAll(".mkt-tab").forEach(u=>{const y=u===p;u.style.background=y?"var(--green)":"var(--tile-bg)",u.style.color=y?"#fff":"var(--tile-fg-dim)",u.style.borderColor=y?"var(--green)":"var(--tile-border)"}),x()})});let g;["flt-name","flt-club","flt-country","flt-job","flt-rarity","flt-note1","flt-note2"].forEach(p=>{var u;(u=document.getElementById(p))==null||u.addEventListener("input",()=>{clearTimeout(g),g=setTimeout(x,250)})}),x()}async function fa(t,e,i,o){const{state:a,toast:n,refreshProfile:r}=o,s=e.find(c=>c.id===t);if(!s)return;const d=s.price;if((a.profile.credits||0)<d){n("Crédits insuffisants","error");return}ma(s,async()=>{const{error:c}=await v.rpc("buy_market_card",{p_listing_id:t,p_buyer_id:a.profile.id});if(c){n("Erreur achat : "+c.message,"error");return}await r();const f=document.getElementById("nav-credits");f&&(f.textContent=`💰 ${(a.profile.credits||0).toLocaleString("fr")}`),n("✅ Carte achetée !","success"),await io(i,o)})}function ma(t,e){var r;const i=(r=t.card)==null?void 0:r.player,o=i?`${i.firstname} ${i.surname_real}`:"cette carte",a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",a.innerHTML=`
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      <div style="display:flex;gap:10px">
        ...
        <button id="buy-ok" style="flex:1;padding:12px;border-radius:10px;border:none;background:var(--green);color:#fff;font-size:14px;font-weight:900;cursor:pointer">Confirmer</button>
    ...
    </div>`,document.body.appendChild(a);const n=s=>{a.remove(),s&&e()};a.querySelector("#buy-cancel").addEventListener("click",()=>n(!1)),a.querySelector("#buy-ok").addEventListener("click",()=>n(!0)),a.addEventListener("click",s=>{s.target===a&&n(!1)})}async function ga(t,e,i){const{toast:o}=i,{data:a}=await v.from("market_listings").select("card_id").eq("id",t).single();if(await v.from("market_listings").update({status:"cancelled"}).eq("id",t),a!=null&&a.card_id){const{count:n}=await v.from("market_listings").select("id",{count:"exact",head:!0}).eq("card_id",a.card_id).eq("status","active");n||await v.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",a.card_id)}o("Annonce retirée","success"),io(e,i)}async function xa(t,e){var x,g,p,u,y;const{state:i,navigate:o}=e,a=((g=(x=e==null?void 0:e.state)==null?void 0:x.params)==null?void 0:g.tab)||"ranked";t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:var(--tile-fg-dim)">⚽ Chargement...</div>';const[{data:n},{data:r},{data:s}]=await Promise.all([v.from("users").select("id,pseudo,club_name,mmr,rank_tier,ranked_wins,ranked_losses,ranked_draws,placement_matches").gte("placement_matches",1).order("mmr",{ascending:!1}).limit(100),v.rpc("get_mini_league_leaderboard"),v.rpc("get_random_leaderboard")]);let d=a;const l=w=>w<3?["#D4A017","#a0a0a0","#cd7f32"][w]:"var(--green)",c=w=>w<3?"#000":"#fff";function f(){var L,m,M;const w=document.getElementById("rankings-legend");w&&(w.innerHTML=d==="random"?'<div style="font-size:11px;color:var(--tile-fg-dim);text-align:center;margin-bottom:10px">📊 = score de classement fiable (tient compte du nombre de matchs joués, pas seulement du %)</div>':"");const h=document.getElementById("rankings-list");if(h){if(d==="ranked"){const E=n||[];h.innerHTML=E.length>0?E.map((I,D)=>{const O=wi(I.mmr??1e3),Y=(I.ranked_wins||0)+(I.ranked_losses||0)+(I.ranked_draws||0),J=Y>0?Math.round((I.ranked_wins||0)/Y*100):0,A=I.id===i.profile.id,_=(I.placement_matches||0)<10;return`
          <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${A?"border:2px solid var(--yellow)":""}">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:11px;color:var(--tile-fg-dim)">${I.club_name} · ${O.label}</div>
            </div>
            <div style="text-align:right;flex-shrink:0">
          ...
          </div>`}).join(""):'<div style="text-align:center;color:var(--tile-fg-dim);padding:40px">Aucun joueur classé.</div>'}else if(d==="mini-league"){const E=r||[];h.innerHTML=E.length>0?E.map((I,D)=>`
        <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${I.user_id===i.profile.id?"border:2px solid var(--yellow)":""}">
          <div style="width:32px;height:32px;border-radius:50%;background:${l(D)};color:${c(D)};display:flex;align-items:center;justify-content:center;font-weight:900;flex-shrink:0;font-size:${D<3?"16":"13"}px">${D<3?["🥇","🥈","🥉"][D]:D+1}</div>
          <div style="flex:1;min-width:0">
      ...
      `).join(""):`<div style="text-align:center;color:var(--tile-fg-dim);padding:40px">Aucun résultat de Mini League pour l'instant.</div>`}else{const E=s||[];h.innerHTML=E.length>0?E.map((I,D)=>`
        <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${I.user_id===i.profile.id?"border:2px solid var(--yellow)":""}">
          <div style="width:32px;height:32px;border-radius:50%;background:${l(D)};color:${c(D)};display:flex;align-items:center;justify-content:center;font-weight:900;flex-shrink:0;font-size:${D<3?"16":"13"}px">${D<3?["🥇","🥈","🥉"][D]:D+1}</div>
          <div style="flex:1;min-width:0">
            <div style="font-weight:700;color:var(--tile-fg-on-page);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${I.pseudo}</div>
  ...
  <div class="page">
    ...
    </div>`,(M=document.getElementById("ranked-back"))==null||M.addEventListener("click",()=>o("home"));return}const d=s.mmr??1e3,l=s.mmr_deviation??350,c=s.mmr_volatility??.06,f=s.placement_matches??0,b=f<10,x=Math.max(0,10-f),g=wi(d),p=So(d),u=Ii.findIndex(O=>O.id===g.id),y=Ii[u+1]||null,w={bronze:"linear-gradient(160deg,#3d1c00,#7a3e00)",silver:"linear-gradient(160deg,#1a1a2e,#3a3a5e)",gold:"linear-gradient(160deg,#1a1200,#4a3500)",platinum:"linear-gradient(160deg,#001a20,#003040)",diamond:"linear-gradient(160deg,#001030,#1a2860)",master:"linear-gradient(160deg,#1a0030,#3d0070)"},h=(s.ranked_wins||0)+(s.ranked_losses||0)+(s.ranked_draws||0),L=h>0?Math.round((s.ranked_wins||0)/h*100):0,m=Ii.map(O=>`
  ...
  </div>`}mn(xn);const ye={user:null,profile:null,page:"home",params:{}};function Ft(t,e="info",i=3e3){const o=document.getElementById("toast");o&&(o.textContent=t,o.className=`show ${e}`,clearTimeout(o._t),o._t=setTimeout(()=>{o.className=""},i))}function ha(t,e,i=""){document.getElementById("modal-title").textContent=t,document.getElementById("modal-body").innerHTML=e,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function Oi(){document.getElementById("modal-overlay").classList.add("hidden")}async function Zt(){if(!ye.user)return;const{data:t}=await v.from("users").select("*").eq("id",ye.user.id).single();t&&(ye.profile=t)}const ln="mw_theme";function dn(){return localStorage.getItem(ln)||"dark"}function va(t){var e;localStorage.setItem(ln,t),wa(t),(e=ye.profile)!=null&&e.id&&v.from("users").update({theme:t}).eq("id",ye.profile.id).then(()=>{})}function wa(t){document.documentElement.setAttribute("data-theme",t)}function Pt(t,e={}){ye.page=t,ye.params=e,cn()}async function cn(){var o,a,n,r;const t=document.getElementById("page-content");if(!t)return;document.querySelectorAll(".bottom-nav a").forEach(s=>{s.classList.toggle("active",s.dataset.page===ye.page)});const e=document.getElementById("nav-credits");e&&ye.profile&&(e.textContent=`💰 ${(ye.profile.credits||0).toLocaleString("fr")}`);const i={state:ye,navigate:Pt,toast:Ft,openModal:ha,closeModal:Oi,refreshProfile:Zt};switch(t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',ye.page){case"home":await Si(t,i);break;case"home2":await Si(t,i);break;case"game":await Pn(t,i);break;case"settings":await Go(t,i);break;case"collection":await Kn(t,i);break;case"decks":await Bi(t,i);break;case"boosters":await rr(t,i);break;case"ranked":await ba(t,i);break;case"match":{const s=ye.params&&ye.params.matchMode||"vs_ai_easy";s==="random"?await Ni(t,i,!1):s==="ranked"?await Ni(t,i,!0):s==="friend"?await Hr(t,i,(o=ye.params)==null?void 0:o.friendId,(a=ye.params)==null?void 0:a.friendName):s==="mini_league"||s==="mini-league"?await Lo(t,i,(n=ye.params)==null?void 0:n.mlMatchId,(r=ye.params)==null?void 0:r.leagueId):await vr(t,i);break}case"market":await ua(t,i);break;case"rankings":await xa(t,i);break;case"matches":await ya(t,i);break;case"friends":await kn(t,i);break;case"mini-league":await Yr(t,i);break;case"match-mini-league":{const s=ye.params||{};await Lo(t,i,s.mlMatchId,s.leagueId);break}default:await Si(t,i)}}function _a(){var o,a;const t=document.getElementById("app"),e=ye.profile;if(!e)return;const i="/icons/";t.innerHTML=`
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.26-1839" style="cursor:pointer">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(n=>{n.addEventListener("click",r=>{r.preventDefault(),Pt(n.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>Pt("home")),document.getElementById("nav-credits").addEventListener("click",()=>Pt("boosters")),(o=document.getElementById("journal-btn"))==null||o.addEventListener("click",()=>ka()),(a=document.getElementById("settings-btn"))==null||a.addEventListener("click",()=>Pt("settings"))}async function ka(){const{data:t}=await v.from("patch_notes").select("*").eq("is_published",!0).order("published_at",{ascending:!1}).limit(20),e=document.createElement("div");e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px";const i=(t||[]).map(o=>{const a=new Date(o.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"long",year:"numeric"});return`<div style="padding:14px 0;border-bottom:1px solid #f0f0f0">
      ${o.image_url?`<img src="${o.image_url}" style="width:100%;max-height:160px;object-fit:cover;border-radius:8px;margin-bottom:10px">`:""}
      <div style="font-size:12px;color:#999;margin-bottom:4px">${a}</div>
      <div style="font-size:15px;font-weight:900;margin-bottom:6px">${o.title}</div>
      <div style="font-size:13px;color:#444;line-height:1.6;white-space:pre-line">${o.description}</div>
  ...
  <div id="public-landing">
  ...
  </div>`,(o=t.querySelector("#pl-play-btn"))==null||o.addEventListener("click",e)}async function Ea(){document.documentElement.setAttribute("data-theme",dn()),document.getElementById("modal-overlay").addEventListener("click",i=>{i.target===i.currentTarget&&Oi()}),document.getElementById("modal-close").addEventListener("click",Oi);const{data:{session:t}}=await v.auth.getSession();if(!t){Io(),$a(document.getElementById("app"),{onPlay:()=>po(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Ft})});return}ye.user=t.user,await Zt(),Io();try{const{data:i}=await v.from("formation_links_overrides").select("formation, links"),o={};(i||[]).forEach(a=>{o[a.formation]=a.links}),gn(o)}catch(i){console.warn("Impossible de charger les overrides de formation:",i)}if(!ye.profile){yn(document.getElementById("app"),{state:ye,navigate:async()=>{await Zt(),Ci()},toast:Ft,refreshProfile:Zt});return}const e=Array.isArray(ye.profile.pending_boosters)?ye.profile.pending_boosters:[];if(!ye.profile.onboarding_done&&e.length>0){fr(document.getElementById("app"),{state:ye,navigate:()=>Ci(),toast:Ft,refreshProfile:Zt});return}Ci(),setTimeout(()=>pa(ye.profile,Pt,Ft),800),v.auth.onAuthStateChange(async(i,o)=>{i==="SIGNED_OUT"&&(ye.user=null,ye.profile=null,document.getElementById("app").innerHTML="",po(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Ft}))})}function La(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function Ei(){const t=document.getElementById("app");t&&(t.style.height=La()+"px")}window.addEventListener("resize",Ei);window.addEventListener("orientationchange",()=>setTimeout(Ei,150));window.visualViewport&&window.visualViewport.addEventListener("resize",Ei);function Ci(){const t=()=>{var i;(i=ye.user)!=null&&i.id&&v.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",ye.user.id).then(()=>{})};t(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(t,6e4);const e=document.getElementById("app");e.style.display="flex",e.style.flexDirection="column",Ei(),_a(),cn()}function Io(){const t=document.getElementById("app-loader"),e=document.getElementById("app");e&&(e.style.display=""),t&&(t.classList.add("zoom-out"),setTimeout(()=>t.style.display="none",500))}function pn(t){var o;const e=document.getElementById("app-loader");if(e&&(e.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
    ...
    <button id="boot-retry" style="margin-top:8px;padding:12px 30px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer">Réessayer</button>`,document.body.appendChild(i),(o=document.getElementById("boot-retry"))==null||o.addEventListener("click",()=>window.location.reload())}Ea().catch(t=>{console.error("Échec du démarrage:",t),pn()});setTimeout(()=>{const t=document.getElementById("app-loader");t&&t.style.display!=="none"&&!t.classList.contains("zoom-out")&&!document.getElementById("boot-error")&&pn("Le serveur met trop de temps à répondre.")},12e3);
