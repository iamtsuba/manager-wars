...
import{s as v,l as Ht,m as Bi,F as qi,r as Ce,j as ii,h as Di,n as Kn,o as Vn,T as bi,k as Yn,p as Wn,b as Xn}from"./formation-links-Bb-Ce4un.js";const Jn="/";function Qi(e,{navigate:t,toast:i}){let n="login";const r=()=>{var a,s,d,c,l,m;const o=n==="login";e.innerHTML=`
  ...
  `;function s(){var x;const c=document.getElementById("logo-preview"),l=document.getElementById("logo-initials"),m=((x=document.getElementById("setup-club"))==null?void 0:x.value)||"MW",y=m.trim().split(" ").filter(Boolean),g=y.length>=2?(y[0][0]+y[1][0]).toUpperCase():m.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=o),l&&(l.textContent=g,l.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function d(c){document.querySelectorAll(".setup-step").forEach(l=>l.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("step1-error");if(l.textContent="",c.length<3){l.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:m}=await v.from("users").select("id").eq("pseudo",c).maybeSingle();if(m){l.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),l=document.getElementById("step2-error");if(l.textContent="",c.length<2){l.textContent="Nom trop court (min. 2 caractères).";return}const{data:m}=await v.from("users").select("id").eq("club_name",c).maybeSingle();if(m){l.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("setup-club").value.trim(),m=document.getElementById("step3-error"),y=document.getElementById("step3-finish");m.textContent="",y.disabled=!0,y.textContent="Création en cours…";try{const{error:g}=await v.from("users").insert({id:t.user.id,pseudo:c,club_name:l,club_color1:o,club_color2:a,credits:1e4});if(g)throw g;await Zn(t.user.id),await r(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(g){m.textContent=g.message,y.disabled=!1,y.textContent="🚀 Créer mon profil !"}})}async function Zn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await v.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const eo="modulepreload",to=function(e){return"/"+e},Zi={},mn=function(t,i,n){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(d=>{if(d=to(d),d in Zi)return;Zi[d]=!0;const c=d.endsWith(".css"),l=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${l}`))return;const m=document.createElement("link");if(m.rel=c?"stylesheet":eo,c||(m.as="script"),m.crossOrigin="",m.href=d,s&&m.setAttribute("nonce",s),document.head.appendChild(m),c)return new Promise((y,g)=>{m.addEventListener("load",y),m.addEventListener("error",()=>g(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return r.then(a=>{for(const s of a||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},Xt="#1A6B3C",Jt="#cc2222",io="#D4A017",en="#888";async function no(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </style>`}const so="2026.07.20-2253";async function tn(e,{state:t,navigate:i,toast:n}){var o,a;const r=t.profile;r&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
      ...
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:0 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;align-items:center"></div>
        ...
        <div style="display:flex;justify-content:flex-end">
          ...
          <div style="height:150px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${Le?`<img src="${Le}" style="max-width:120px;max-height:120px;object-fit:contain;border-radius:6px">`:`<span style="font-size:64px">${M.icon}</span>`}
          </div>
        ...
        </div>`},({type:R,gc:M,def:N,owned:ce})=>{const ue=window.innerWidth>=768?.76:.54,we={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},ye={purple:"#9b59b6",light_blue:"#00bcd4"},ze=we[N==null?void 0:N.color]||we.purple,Ie=ye[N==null?void 0:N.color]||ye.purple,ke=N!=null&&N.image_url?`/icons/${N.image_url}`:null,Ee=Math.round(140*657/507);return ce?`<div style="display:inline-block;zoom:${ue};line-height:0;pointer-events:none"><div style="width:140px;height:${Ee}px;border-radius:8px;background:${ze};border:1px solid ${Ie};display:flex;flex-direction:column;overflow:hidden"><div style="height:34px;background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center"><span style="font-size:10px;font-weight:900;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:130px">${R}</span></div><div style="flex:1;display:flex;align-items:center;justify-content:center;min-height:0">${ke?`<img src="${ke}" style="max-width:110px;max-height:100px;object-fit:contain">`:`<span style="font-size:28px">${M.icon}</span>`}</div><div style="height:30px;display:flex;align-items:center;justify-content:center;padding:0 4px"><span style="font-size:8px;color:rgba(255,255,255,0.7);text-align:center;line-height:1.2">${((N==null?void 0:N.effect)||M.desc||"").slice(0,26)}</span></div></div></div>`:`<div style="display:inline-block;zoom:${ue};line-height:0;pointer-events:none"><div style="width:140px;height:${Ee}px;border-radius:8px;background:#eee;border:1px solid #ddd;display:flex;flex-direction:column;align-items:center;justify-content:center;filter:grayscale(1);opacity:0.45"><span style="font-size:24px">${M.icon}</span><span style="font-size:9px;color:#aaa;margin-top:4px;text-align:center;padding:0 6px">${R}</span></div></div>`},({type:R,owned:M,def:N})=>{M&&zo(R,N,o)})}function U(j){const F="#E87722",H="/";if(!y.length){j.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const Q={};y.forEach(M=>{const N=M.stadium_id||"?";(Q[N]=Q[N]||[]).push(M)});const R=Object.entries(Q).map(([M,N])=>({sid:M,def:u[M]||null,count:N.length,card:N[0]}));oe(R,({def:M,count:N})=>{var Ie,ke;const ce=(M==null?void 0:M.name)||"?",ue=((Ie=M==null?void 0:M.club)==null?void 0:Ie.encoded_name)||(M==null?void 0:M.country_code)||"—",we=M!=null&&M.image_url?`${H}icons/${M.image_url}`:((ke=M==null?void 0:M.club)==null?void 0:ke.logo_url)||(M!=null&&M.country_code?`https://flagsapi.com/${M.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),ye=we?`<img src="${we}" style="width:90px;height:90px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<div style="font-size:56px">🏟️</div>',ze=N>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${N}</div>`:"";return`<div style="position:relative;width:140px;border-radius:12px;border:3px solid #c45a00;background:linear-gradient(160deg,${F},#c45a00);display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${F}66">
          ${ze}
        ...
        </div>`},({def:M,count:N})=>{var ke,Ee;const ce=window.innerWidth>=768?.76:.54,ue=(M==null?void 0:M.name)||"?",we=((ke=M==null?void 0:M.club)==null?void 0:ke.encoded_name)||(M==null?void 0:M.country_code)||"—",ye=M!=null&&M.image_url?`${H}icons/${M.image_url}`:((Ee=M==null?void 0:M.club)==null?void 0:Ee.logo_url)||(M!=null&&M.country_code?`https://flagsapi.com/${M.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),ze=ye?`<img src="${ye}" style="width:44px;height:44px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<span style="font-size:22px">🏟️</span>',Ie=Math.round(140*657/507);return`<div style="display:inline-block;zoom:${ce};line-height:0;pointer-events:none">
          <div style="width:140px;height:${Ie}px;border-radius:8px;background:linear-gradient(160deg,${F},#c45a00);border:1px solid #c45a00;display:flex;flex-direction:column;overflow:hidden">
            <div style="height:36px;background:rgba(0,0,0,0.25);display:flex;align-items:center;justify-content:center;flex-direction:column;padding:2px 6px">
              ...
              <div style="font-size:9px;font-weight:900;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:130px;margin-top:1px">${ue}</div>
            </div>
            ...
            <div style="height:42px;display:flex;flex-direction:column;align-items:center;justify-content:center;background:rgba(0,0,0,0.3);padding:3px;gap:2px">
              <div style="font-size:8px;font-weight:700;color:rgba(255,255,255,0.9);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:126px;text-align:center">${we}</div>
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(me=document.getElementById("close-detail"))==null||me.addEventListener("click",s);let Y=new Set;const oe=()=>{const O=Y.size,V=document.getElementById("sell-action-panel");if(!V)return;V.style.display=O>0?"block":"none",document.getElementById("sell-selected-count").textContent=O;const U=document.getElementById("evolve-btn");U&&(U.textContent=`⬆️ Évoluer ${O>1?"(+"+O+")":""}`)};document.querySelectorAll(".expl-check").forEach(O=>{O.addEventListener("change",()=>{const V=O.dataset.id;O.checked?Y.add(V):Y.delete(V);const U=O.closest(".exemplaire-row");U&&(U.style.borderColor=O.checked?"#1A6B3C":"#eee"),oe()})}),document.querySelectorAll(".exemplaire-row").forEach(O=>{O.addEventListener("click",V=>{if(V.target.closest("button")||V.target.tagName==="INPUT")return;const U=O.querySelector(".expl-check");U&&!U.disabled&&(U.checked=!U.checked,U.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(O=>{O.addEventListener("click",V=>{V.stopPropagation();const U=document.querySelector(`.expl-hist[data-hist="${O.dataset.idx}"]`);U&&(U.style.display=U.style.display==="none"?"flex":"none")})}),(S=document.getElementById("evolve-btn"))==null||S.addEventListener("click",async()=>{if(y<=1)return;const O=[...Y];if(!O.length)return;if(Y.has(e.id)){const R=document.createElement("div");R.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",R.innerHTML=`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const S=window.innerWidth>=700,W=(R,M,N)=>{var qe,je;const ce=(t.gcDefs||[]).find(Ye=>Ye.name===R.gc_type),ue={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ce==null?void 0:ce.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",we={purple:"#b06ce0",light_blue:"#00d4ef"}[ce==null?void 0:ce.color]||"#b06ce0",ye=(ce==null?void 0:ce.name)||R.gc_type,ze=(ce==null?void 0:ce.effect)||((qe=De[R.gc_type])==null?void 0:qe.desc)||"",Ie=ce!=null&&ce.image_url?`/icons/${ce.image_url}`:null,ke=((je=De[R.gc_type])==null?void 0:je.icon)||"⚡",Ee=Math.round(N*.22),He=Math.round(N*.22),$e=N-Ee-He,Le=ye.length>12?7:9;return`<div class="gc-mini" data-gc-id="${R.id}" data-gc-type="${R.gc_type}"
          ...
          <div style="height:${Ee}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
            <span style="font-size:${Le}px;font-weight:900;color:#fff;text-align:center;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${M-6}px">${ye}</span>
          ...
          <div style="height:${$e}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${Ie?`<img src="${Ie}" style="max-width:${M-10}px;max-height:${$e-6}px;object-fit:contain">`:`<span style="font-size:${Math.round($e*.55)}px">${ke}</span>`}
          </div>
  ...
  <div id="match-history-panel">
      ...
      ${gi(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            <div style="background:rgba(255,255,255,0.08);border-radius:14px;padding:14px 20px;text-align:center;min-width:220px">
              ...
              ${Ae.id!==nt.id?`<div style="font-size:20px;font-weight:900;color:${Ae.color}">
                    ${Ne>J.mmr?"📈":"📉"} ${nt.emoji} ${nt.label} → ${Ae.emoji} ${Ae.label}
                  </div>
                  <div style="font-size:13px;color:rgba(255,255,255,0.5);margin-top:4px">${Ne>J.mmr?"Promotion !":"Rétrogradation"}</div>`:`<div style="font-size:36px">${Se>=0?"📈":"📉"}</div>
    ...
    </div>`,rt=f[p+"Team"],Un=(()=>{var ge,Me,_e,jt,at;if(P&&((ge=f.pendingAttack)!=null&&ge.players)){const Pe=f.pendingAttack;let Oe="";if(K.length>0){const Be=K.map(Ge=>{const ft=(rt[Ge._role]||[]).find(yi=>yi.cardId===Ge.cardId)||Ge,st=rt[Ge._role]||[],Ct=st.findIndex(yi=>yi.cardId===Ge.cardId),Bt=xt(st.length),gt=Ct>=0?Bt[Ct]:ft._col??1;return{...ft,_line:Ge._role,_col:gt}}),kt=Zt(Be,((Me=f.modifiers)==null?void 0:Me[p])||{},f[p+"Formation"]);Oe=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      </div>`,(Ne=te.querySelector("#psub-close"))==null||Ne.addEventListener("click",()=>te.remove()),(Ae=te.querySelector("#pout-up"))==null||Ae.addEventListener("click",()=>{P>0&&(P--,J())}),(We=te.querySelector("#pout-down"))==null||We.addEventListener("click",()=>{P<G.length-1&&(P++,J())}),(Ue=te.querySelector("#pin-up"))==null||Ue.addEventListener("click",()=>{ee>0&&(ee--,J())}),(nt=te.querySelector("#pin-down"))==null||nt.addEventListener("click",()=>{ee<q.length-1&&(ee++,J())});const Se=(Fe,Xe,Ke,pt)=>{const ot=te.querySelector("#"+Fe);if(!ot)return;let At=0;ot.addEventListener("touchstart",ut=>{At=ut.touches[0].clientY},{passive:!0}),ot.addEventListener("touchend",ut=>{const _t=ut.changedTouches[0].clientY-At;if(Math.abs(_t)<30)return;const rt=Xe();_t<0&&rt<pt-1?(Ke(rt+1),J()):_t>0&&rt>0&&(Ke(rt-1),J())},{passive:!0})};Se("pin-panel",()=>ee,Fe=>ee=Fe,q.length),Se("pout-panel",()=>P,Fe=>P=Fe,G.length),(Ut=te.querySelector("#psub-confirm"))==null||Ut.addEventListener("click",async Fe=>{if(Fe.preventDefault(),Fe.stopPropagation(),re)return;re=!0;const Xe=G[P],Ke=q[ee];if(!Xe||!Ke)return;const pt=Xe._line,ot=(z[pt]||[]).findIndex(_t=>_t.cardId===Xe.cardId);if(ot===-1){x("Erreur : joueur introuvable","error"),te.remove();return}const At={...Ke,_line:pt,position:Xe.position,used:!1,boost:0};z[pt].splice(ot,1,At);const ut=[...A,Ke.cardId];te.remove(),Q(Xe,Ke,async()=>{await C({[p+"Team"]:z,[u+"Team"]:f[u+"Team"],["usedSubIds_"+p]:ut})})})}document.body.appendChild(te),J()}function Q(_,E,z){const A={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},L=document.createElement("div");L.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const G=(K,ee,re)=>`<div style="text-align:center">
  ...
  <div class="page">
    ...
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.20-2253" style="cursor:pointer">
