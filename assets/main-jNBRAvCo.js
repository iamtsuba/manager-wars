...
import{s as T,F as $i,i as Ut,k as Vi,l as ot,m as Ii,f as Nt,n as vn,T as li,j as wn,o as _n,b as $n}from"./formation-links-BZe0pGr4.js";const kn="/";function Mi(e,{navigate:t,toast:i}){let o="login";const d=()=>{var r,l,u,a,n,x;const s=o==="login";e.innerHTML=`
    ...
    </style>`,(r=document.getElementById("tab-login-btn"))==null||r.addEventListener("click",()=>{o="login",d()}),(l=document.getElementById("tab-reg-btn"))==null||l.addEventListener("click",()=>{o="register",d()}),s?((u=document.getElementById("login-password"))==null||u.addEventListener("keydown",f=>{var h;f.key==="Enter"&&((h=document.getElementById("login-btn"))==null||h.click())}),(a=document.getElementById("login-btn"))==null||a.addEventListener("click",async()=>{const f=document.getElementById("login-email").value.trim(),h=document.getElementById("login-password").value,_=document.getElementById("login-error");if(_.textContent="",!f||!h){_.textContent="Remplissez tous les champs.";return}const p=document.getElementById("login-btn");p.textContent="⏳ Connexion…",p.disabled=!0;const{error:c}=await T.auth.signInWithPassword({email:f,password:h});if(p.textContent="⚽ Se connecter",p.disabled=!1,c){_.textContent=c.message.includes("Invalid")?"Email ou mot de passe incorrect.":c.message;return}window.location.reload()})):((n=document.getElementById("reg-confirm"))==null||n.addEventListener("keydown",f=>{var h;f.key==="Enter"&&((h=document.getElementById("reg-btn"))==null||h.click())}),(x=document.getElementById("reg-btn"))==null||x.addEventListener("click",async()=>{const f=document.getElementById("reg-email").value.trim(),h=document.getElementById("reg-password").value,_=document.getElementById("reg-confirm").value,p=document.getElementById("reg-error");if(p.textContent="",!f||!h||!_){p.textContent="Remplissez tous les champs.";return}if(h.length<6){p.textContent="Mot de passe trop court (min. 6 caractères).";return}if(h!==_){p.textContent="Les mots de passe ne correspondent pas.";return}const c=document.getElementById("reg-btn");c.textContent="⏳ Création…",c.disabled=!0;const{error:A}=await T.auth.signUp({email:f,password:h});if(c.textContent="🚀 Créer mon compte",c.disabled=!1,A){p.textContent=A.message;return}i("Compte créé ! Connecte-toi pour commencer.","success",4e3),o="login",d(),setTimeout(()=>{const m=document.getElementById("login-email");m&&(m.value=f)},50)}))};d()}function En(e,{state:t,navigate:i,toast:o,refreshProfile:d}){let s="#1A6B3C",r="#D4A017";e.innerHTML=`
  ...
  `;function l(){var _;const a=document.getElementById("logo-preview"),n=document.getElementById("logo-initials"),x=((_=document.getElementById("setup-club"))==null?void 0:_.value)||"MW",f=x.trim().split(" ").filter(Boolean),h=f.length>=2?(f[0][0]+f[1][0]).toUpperCase():x.slice(0,2).toUpperCase();a&&(a.style.background=r,a.style.borderColor=s),n&&(n.textContent=h,n.style.color=s)}document.getElementById("color1").addEventListener("input",a=>{s=a.target.value,document.getElementById("swatch1").style.background=s,l()}),document.getElementById("color2").addEventListener("input",a=>{r=a.target.value,document.getElementById("swatch2").style.background=r,l()});function u(a){document.querySelectorAll(".setup-step").forEach(n=>n.classList.remove("active")),document.getElementById(`step-${a}`).classList.add("active"),document.getElementById("step-num").textContent=a,document.getElementById("progress-fill").style.width=`${Math.round(a/3*100)}%`,a===3&&l()}document.getElementById("step1-next").addEventListener("click",async()=>{const a=document.getElementById("setup-pseudo").value.trim(),n=document.getElementById("step1-error");if(n.textContent="",a.length<3){n.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:x}=await T.from("users").select("id").eq("pseudo",a).maybeSingle();if(x){n.textContent="Ce pseudo est déjà pris.";return}u(2)}),document.getElementById("step2-back").addEventListener("click",()=>u(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const a=document.getElementById("setup-club").value.trim(),n=document.getElementById("step2-error");if(n.textContent="",a.length<2){n.textContent="Nom trop court (min. 2 caractères).";return}const{data:x}=await T.from("users").select("id").eq("club_name",a).maybeSingle();if(x){n.textContent="Ce nom de club est déjà pris.";return}u(3)}),document.getElementById("step3-back").addEventListener("click",()=>u(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const a=document.getElementById("setup-pseudo").value.trim(),n=document.getElementById("setup-club").value.trim(),x=document.getElementById("step3-error"),f=document.getElementById("step3-finish");x.textContent="",f.disabled=!0,f.textContent="Création en cours…";try{const{error:h}=await T.from("users").insert({id:t.user.id,pseudo:a,club_name:n,club_color1:s,club_color2:r,credits:1e4});if(h)throw h;await Tn(t.user.id),await d(),o(`Bienvenue ${a} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(h){x.textContent=h.message,f.disabled=!1,f.textContent="🚀 Créer mon profil !"}})}async function Tn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await T.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const zn="modulepreload",Sn=function(e){return"/"+e},ji={},oi=function(t,i,o){let d=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),l=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));d=Promise.allSettled(i.map(u=>{if(u=Sn(u),u in ji)return;ji[u]=!0;const a=u.endsWith(".css"),n=a?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${u}"]${n}`))return;const x=document.createElement("link");if(x.rel=a?"stylesheet":zn,a||(x.as="script"),x.crossOrigin="",x.href=u,l&&x.setAttribute("nonce",l),document.head.appendChild(x),a)return new Promise((f,h)=>{x.addEventListener("load",f),x.addEventListener("error",()=>h(new Error(`Unable to preload CSS for ${u}`)))})}))}function s(r){const l=new Event("vite:preloadError",{cancelable:!0});if(l.payload=r,window.dispatchEvent(l),!l.defaultPrevented)throw r}return d.then(r=>{for(const l of r||[])l.status==="rejected"&&s(l.reason);return t().catch(s)})},Rt="#1A6B3C",Ot="#cc2222",An="#D4A017",Ci="#888";async function Ln(e,t){const{state:i,toast:o}=t;e.innerHTML=`
    ...
    </div>`,a.querySelectorAll("[data-stats]").forEach(h=>{h.addEventListener("click",()=>jn(e,h.dataset.stats,h.dataset.friendName))}),a.querySelectorAll("[data-match]").forEach(h=>{h.addEventListener("click",()=>{const _=h.dataset.friendId,p=h.dataset.friendName;console.log("[Friends] clic match",{fid:_,fname:p,hasNavigate:typeof o}),typeof o=="function"?o("match",{matchMode:"friend",friendId:_,friendName:p}):t("Erreur navigation","error")})})}function In(e,t){const i=e.club_name||e.pseudo||"?",o=e.pseudo||"",d=(o||i).slice(0,2).toUpperCase(),s=e.club_color2||Rt,r=e.club_color1||"#ffffff",l=e.last_seen_at?new Date(e.last_seen_at):null,u=l&&Date.now()-l.getTime()<3*60*1e3;return`
    ...
    ${Ei()}`,document.body.appendChild(a);const x=()=>a.remove();a.querySelector("#pending-close").addEventListener("click",x),a.addEventListener("click",f=>{f.target===a&&x()}),a.querySelectorAll("[data-accept]").forEach(f=>{f.addEventListener("click",async()=>{const{error:h}=await T.from("friendships").update({status:"accepted"}).eq("id",f.dataset.accept);if(h){t("Erreur : "+h.message,"error");return}f.closest("div[style]").remove(),t("✅ Ami accepté !","success"),Yi(e,t,o),i&&i()})}),a.querySelectorAll("[data-decline]").forEach(f=>{f.addEventListener("click",async()=>{await T.from("friendships").delete().eq("id",f.dataset.decline),f.closest("div[style]").remove(),t("Demande refusée","info"),i&&i()})})}async function jn(e,t,i){const o=e.user.id,[d,s]=[o,t].sort(),r=o===d,{data:l}=await T.from("friend_match_stats").select("*").eq("player1_id",d).eq("player2_id",s).single(),u=e.profile.club_name||e.profile.pseudo||"Moi",a=l||{},n=r?a.wins_p1||0:a.wins_p2||0,x=r?a.wins_p2||0:a.wins_p1||0,f=a.draws||0,h=r?a.goals_p1||0:a.goals_p2||0,_=r?a.goals_p2||0:a.goals_p1||0,p=r?a.gc_used_p1||0:a.gc_used_p2||0,c=r?a.gc_used_p2||0:a.gc_used_p1||0,A=a.matches_total||0,m=(B,C,P,j=Rt,N=Ot)=>`
    ...
    <div class="popup-box" style="max-width:380px">
      ...
      ${A===0?`
        ...
        ${m("GC utilisés ⚡",p,c,"#7a28b8","#7a28b8")}
    ...
    </style>`}async function Ji({player1Id:e,player2Id:t,score1:i,score2:o,gc1:d,gc2:s}){const[r,l]=[e,t].sort(),u=e!==r,a=u?o:i,n=u?i:o,x=u?s:d,f=u?d:s,h=a>n?1:0,_=n>a?1:0,p=a===n?1:0,{data:c}=await T.from("friend_match_stats").select("*").eq("player1_id",r).eq("player2_id",l).single();c?await T.from("friend_match_stats").update({wins_p1:c.wins_p1+h,wins_p2:c.wins_p2+_,draws:c.draws+p,goals_p1:c.goals_p1+a,goals_p2:c.goals_p2+n,gc_used_p1:c.gc_used_p1+x,gc_used_p2:c.gc_used_p2+f,matches_total:c.matches_total+1}).eq("player1_id",r).eq("player2_id",l):await T.from("friend_match_stats").insert({player1_id:r,player2_id:l,wins_p1:h,wins_p2:_,draws:p,goals_p1:a,goals_p2:n,gc_used_p1:x,gc_used_p2:f,matches_total:1})}const Cn="2026.07.07-2249";async function qi(e,{state:t,navigate:i,toast:o}){var s,r;const d=t.profile;d&&(e.innerHTML=`
  <style>
  ...
  <div style="position:absolute;left:${se}px;top:${we}px;width:${ve}px;height:${_e}px;z-index:3;
    ...
    ${bt(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),a=(l||[]).filter(O=>O.card_type==="player"&&O.player),n=(l||[]).filter(O=>O.card_type==="game_changer"),x=(l||[]).filter(O=>O.card_type==="formation"),f=(l||[]).filter(O=>O.card_type==="stadium"),{data:h}=await T.from("gc_definitions").select("name,gc_type,color,effect,image_url"),_={};(h||[]).forEach(O=>{_[O.name]=O});const{data:p}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),c={};(p||[]).forEach(O=>{c[O.id]=O}),f.forEach(O=>{O.stadium_def&&(c[O.stadium_id]=O.stadium_def)});const A=Object.keys($i),m=Object.keys(Oe),I={};a.forEach(O=>{const ee=O.player.id;I[ee]=(I[ee]||0)+1}),new Set(Object.keys(I).map(O=>String(O)));const B=new Set(x.map(O=>O.formation)),C=new Set(n.map(O=>O.gc_type));let P="player",j="Tous",N="",V=!1;function K(){return[...a].sort((O,ee)=>{const xe=Wt.indexOf(O.player.job),le=Wt.indexOf(ee.player.job);return xe!==le?xe-le:(O.player.surname_encoded||"").localeCompare(ee.player.surname_encoded||"")})}function oe(){return[...u||[]].sort((O,ee)=>{const xe=Wt.indexOf(O.job),le=Wt.indexOf(ee.job);return xe!==le?xe-le:(O.surname_encoded||"").localeCompare(ee.surname_encoded||"")})}function ie(){return K().filter(O=>{const ee=O.player,xe=j==="Tous"||ee.job===j,le=!N||`${ee.firstname} ${ee.surname_encoded}`.toLowerCase().includes(N);return xe&&le})}function Q(){return oe().filter(O=>{const ee=j==="Tous"||O.job===j,xe=!N||`${O.firstname} ${O.surname_encoded}`.toLowerCase().includes(N);return ee&&xe})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Jt=1e3;function to(e,t,i,o){var _,p,c;const{state:d,toast:s,closeModal:r,navigate:l,refreshProfile:u}=i,a=e.formation,n={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function x(){const A=Ut[a]||{},m=$i[a]||[],I=290,B=360,C=20;function P(N){const V=A[N];return V?{x:V.x*I,y:V.y*B}:null}let j=`<svg width="${I}" height="${B}" viewBox="0 0 ${I} ${B}" xmlns="http://www.w3.org/2000/svg">`;for(const[N,V]of m){const K=P(N),oe=P(V);!K||!oe||(j+=`<line x1="${K.x}" y1="${K.y}" x2="${oe.x}" y2="${oe.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const N of Object.keys(A)){const V=P(N);if(!V)continue;const K=N.replace(/\d+/,""),oe=n[K]||"#555";j+=`<circle cx="${V.x}" cy="${V.y}" r="${C}" fill="${oe}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,j+=`<text x="${V.x}" y="${V.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${K}</text>`}return j+="</svg>",j}const f=t.filter(A=>A.formation===a);f.length;const h=!e.is_for_sale;o(`Formation ${a}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(_=document.getElementById("direct-sell-form-btn"))==null||_.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${a} pour ${Jt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const A=f.find(I=>!I.is_for_sale)||f[0];if(!A){s("Aucune carte à vendre","error");return}await T.from("market_listings").delete().eq("card_id",A.id),await T.from("transfer_history").delete().eq("card_id",A.id);const{error:m}=await T.from("cards").delete().eq("id",A.id);if(m){s(m.message,"error");return}await T.from("users").update({credits:(d.profile.credits||0)+Jt}).eq("id",d.profile.id),await u(),s(`+${Jt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),r(),l("collection")}),(p=document.getElementById("market-sell-form-btn"))==null||p.addEventListener("click",async()=>{const A=parseInt(document.getElementById("sell-price-form").value);if(!A||A<1){s("Prix invalide","error");return}await T.from("cards").update({is_for_sale:!0,sale_price:A}).eq("id",e.id),await T.from("market_listings").insert({seller_id:d.profile.id,card_id:e.id,price:A}),s("Carte mise en vente sur le marché !","success"),r(),l("collection")}),(c=document.getElementById("cancel-sell-form-btn"))==null||c.addEventListener("click",async()=>{await T.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await T.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),s("Annonce retirée","success"),r(),l("collection")})}async function Ni(e,t,i,o){var me,_e,ve,we,Ee;const{state:d,toast:s,openModal:r,closeModal:l,navigate:u,refreshProfile:a}=o,n=e.player,x=t.filter(se=>se.player.id===n.id),f=x.length,h=e.evolution_bonus||0,_=Math.max((Number(n.note_g)||0)+(n.job==="GK"||n.job2==="GK"?h:0),(Number(n.note_d)||0)+(n.job==="DEF"||n.job2==="DEF"?h:0),(Number(n.note_m)||0)+(n.job==="MIL"||n.job2==="MIL"?h:0),(Number(n.note_a)||0)+(n.job==="ATT"||n.job2==="ATT"?h:0)),p=n.rarity||"normal",{data:c}=await T.from("sell_price_configs").select("*").eq("rarity",p).lte("note_min",_).gte("note_max",_).order("note_min",{ascending:!1}).limit(1);((me=c==null?void 0:c[0])==null?void 0:me.price)??Yn[p];const A=n.rarity!=="legende";Qn(n);const m=((n.rarity==="pepite"||n.rarity==="papyte")&&e.current_note!=null?e.current_note:pi(n,n.job))+h,I=n.rarity==="pepite"||n.rarity==="papyte",B=n.job2?(I?nn(e,Jn(n.job2)):pi(n,n.job2))+(pi(n,n.job2)>0?h:0):null;ci[n.job],n.job2&&ci[n.job2];const C=Kn[n.rarity]||"#ccc";Xn[n.country_code]||n.country_code;const P=e.evolution_bonus||0,N=m+P,V=B||0,K=V>0?V+P:0,oe=x.map(se=>se.id);let ie={};if(oe.length){const{data:se}=await T.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",oe).order("transferred_at",{ascending:!0});(se||[]).forEach(pe=>{ie[pe.card_id]||(ie[pe.card_id]=[]),ie[pe.card_id].push(pe)})}const Q=se=>{const pe=se.transferred_at?new Date(se.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",de=se.source==="booster"?"Booster":se.price?se.price.toLocaleString("fr"):"—";return`
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(_e=document.getElementById("close-detail"))==null||_e.addEventListener("click",l);let D=new Set;const re=()=>{const se=D.size,pe=document.getElementById("sell-action-panel");if(!pe)return;pe.style.display=se>0?"block":"none",document.getElementById("sell-selected-count").textContent=se;const de=document.getElementById("evolve-btn");de&&(de.textContent=`⬆️ Évoluer ${se>1?"(+"+se+")":""}`)};document.querySelectorAll(".expl-check").forEach(se=>{se.addEventListener("change",()=>{const pe=se.dataset.id;se.checked?D.add(pe):D.delete(pe);const de=se.closest(".exemplaire-row");de&&(de.style.borderColor=se.checked?"#1A6B3C":"#eee"),re()})}),document.querySelectorAll(".exemplaire-row").forEach(se=>{se.addEventListener("click",pe=>{if(pe.target.closest("button")||pe.target.tagName==="INPUT")return;const de=se.querySelector(".expl-check");de&&!de.disabled&&(de.checked=!de.checked,de.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(se=>{se.addEventListener("click",pe=>{pe.stopPropagation();const de=document.querySelector(`.expl-hist[data-hist="${se.dataset.idx}"]`);de&&(de.style.display=de.style.display==="none"?"flex":"none")})}),(ve=document.getElementById("evolve-btn"))==null||ve.addEventListener("click",async()=>{if(f<=1)return;const se=[...D];if(!se.length)return;if(D.has(e.id)){const ge=document.createElement("div");ge.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",ge.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;padding-bottom:80px;background:var(--page-bg)">
    ...
    <div class="page-body">
      ...
      stadium_def:stadium_definitions(id, name, club_id, country_code, image_url,
        club:clubs(encoded_name, logo_url))`).eq("owner_id",o.profile.id).eq("card_type","stadium"),u=(r||[]).filter(A=>A.card_type==="player"&&A.player),a=(r||[]).filter(A=>A.card_type==="formation"),n=(l||[]).filter(A=>A.card_type==="stadium"),x=[...new Set(n.map(A=>A.stadium_id).filter(Boolean))];let f={};if(n.forEach(A=>{A.stadium_def&&A.stadium_id&&(f[A.stadium_id]=A.stadium_def)}),x.length&&Object.keys(f).length<x.length){const{data:A}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",x);(A||[]).forEach(m=>{f[m.id]=m})}const h=a.map(A=>A.formation).filter(Boolean),{data:_}=await T.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let p=s.formation||"4-4-2";h.length>0&&!h.includes(p)&&(p=h[0]);const c={deckId:e,name:s.name,formation:p,formationCardId:s.formation_card_id,stadiumCardId:s.stadium_card_id||null,slots:{},subs:[],playerCards:u,formationCards:a,stadiumCards:n,stadDefMap:f,availableFormations:h};(_||[]).forEach(A=>{A.is_starter?c.slots[A.position]=A.card_id:c.subs.includes(A.card_id)||c.subs.push(A.card_id)}),ht(t,c,i)}function ht(e,t,i){var n,x,f,h;const{navigate:o}=i;ei[t.formation];const d=Oi(t.formation),s=d.filter(_=>t.slots[_]).length,r=t.availableFormations.length>0?t.availableFormations:Object.keys(ei),l=(n=t.stadiumCards)==null?void 0:n.find(_=>_.id===t.stadiumCardId),u=l&&((x=t.stadDefMap)==null?void 0:x[l.stadium_id])||null,a=t.subs.map(_=>t.playerCards.find(p=>p.id===_)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div style="height:100%;overflow-y:auto;padding-bottom:80px;background:var(--page-bg)">
      ...
      <div style="display:flex;gap:0;min-height:600px">
        ...
        <div style="width:140px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:16px 8px;gap:12px;border-right:1px solid rgba(255,255,255,0.1)">
          ...
          <div id="add-stad-btn" style="cursor:pointer">
            ${l?(()=>{var c;const _=t.stadDefMap[l.stadium_id],p=((c=_==null?void 0:_.club)==null?void 0:c.logo_url)||null;return`<div style="width:100px;height:130px;border-radius:8px;background:linear-gradient(135deg,#1a3a6b,#0a1a3a);border:2px solid #E87722;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px">
          ...
          <div style="display:flex;flex-direction:column;gap:6px;align-items:center" id="subs-list">
            ${a.map(_=>{const p={..._.player,_evolution_bonus:_.evolution_bonus||0};return`<div style="position:relative;flex-shrink:0;overflow:visible">
  ...
  </div>`,io(e,t,d,i),document.getElementById("builder-back").addEventListener("click",()=>o("decks")),document.getElementById("formation-select").addEventListener("change",_=>{t.formation=_.target.value;const p=Oi(t.formation),c={};p.forEach(A=>{t.slots[A]&&(c[A]=t.slots[A])}),t.slots=c,ht(e,t,i)}),(f=document.getElementById("add-stad-btn"))==null||f.addEventListener("click",()=>{no(t,e,i)}),document.getElementById("save-deck").addEventListener("click",()=>ao(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(_=>{_.addEventListener("click",()=>{t.subs=t.subs.filter(p=>p!==_.dataset.removeSub),ht(e,t,i)})}),(h=document.getElementById("add-sub-btn"))==null||h.addEventListener("click",()=>{ro(t,e,i)})}function io(e,t,i,o){var I,B;const d=window.innerWidth>=768,s=e.querySelector(d?"#deck-field-pc":"#deck-field-mobile");if(!s)return;const r=(I=t.stadiumCards)==null?void 0:I.find(C=>C.id===t.stadiumCardId),l=r&&((B=t.stadDefMap)==null?void 0:B[r.stadium_id])||null,u=Ut[t.formation]||{},a=Vi(t.formation)||[],n={};for(const C of i){const P=t.slots[C],j=P?t.playerCards.find(N=>N.id===P):null;j!=null&&j.player?n[C]={...j.player,_evolution_bonus:j.evolution_bonus||0,face:j.player.face||null}:n[C]=null}const x=window.innerWidth>=768,f=x?window.innerWidth-180:window.innerWidth-20,h=x?Math.min(f,800):f,_=Math.round(x?h*1.3:h*1.35),p=x?80:44;let c="";for(const[C,P]of a){const j=u[C],N=u[P];if(!j||!N)continue;const V=j.x*h,K=j.y*_,oe=N.x*h,ie=N.y*_,Q=n[C],R=n[P],D=ot(Q,R);D==="#ff3333"||D==="#cc2222"?c+=`<line x1="${V.toFixed(1)}" y1="${K.toFixed(1)}" x2="${oe.toFixed(1)}" y2="${ie.toFixed(1)}" stroke="${D}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(c+=`<line x1="${V.toFixed(1)}" y1="${K.toFixed(1)}" x2="${oe.toFixed(1)}" y2="${ie.toFixed(1)}" stroke="${D}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,c+=`<line x1="${V.toFixed(1)}" y1="${K.toFixed(1)}" x2="${oe.toFixed(1)}" y2="${ie.toFixed(1)}" stroke="${D}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let A="";const m=Math.round(p*657/507);for(const C of i){const P=u[C];if(!P)continue;const j=n[C],N=P.x*h,V=P.y*_,K=Math.round(N-p/2),oe=Math.round(V-m/2);if(j){const ie=bt({...j,_evolution_bonus:j._evolution_bonus||0},{width:p,showStad:!0,stadDef:l});A+=`<div style="position:absolute;left:${K}px;top:${oe}px;cursor:pointer;z-index:2" class="deck-slot-hit" data-pos="${C}">${ie}</div>`}else{const ie=C.replace(/\d+/,"");A+=`<div style="position:absolute;left:${K}px;top:${oe}px;width:${p}px;height:${m}px;
    ...
    </div>`),(r=document.getElementById("stad-none"))==null||r.addEventListener("click",()=>{e.stadiumCardId=null,d(),ht(t,e,i)}),document.querySelectorAll(".stad-choice").forEach(l=>{l.addEventListener("click",()=>{e.stadiumCardId=l.dataset.stadId,d(),ht(t,e,i)})})}function oo(e,t,i,o){var f,h,_;const{openModal:d,closeModal:s}=o,r=e.replace(/\d+/,""),l=t.slots[e],u=l?t.playerCards.find(p=>p.id===l):null;(f=u==null?void 0:u.player)==null||f.id;const a=new Set;Object.entries(t.slots).forEach(([p,c])=>{var m;if(p===e||!c)return;const A=t.playerCards.find(I=>I.id===c);(m=A==null?void 0:A.player)!=null&&m.id&&a.add(A.player.id)}),t.subs.forEach(p=>{var A;const c=t.playerCards.find(m=>m.id===p);(A=c==null?void 0:c.player)!=null&&A.id&&a.add(c.player.id)});const n=new Set,x=t.playerCards.filter(p=>{const c=p.player;return!(c.job===r||c.job2===r)||a.has(c.id)||n.has(c.id)?!1:(n.add(c.id),!0)});x.sort((p,c)=>{const A=p.evolution_bonus||0,m=c.evolution_bonus||0,I=(r==="GK"?p.player.note_g:r==="DEF"?p.player.note_d:r==="MIL"?p.player.note_m:p.player.note_a)+(r===p.player.job||r===p.player.job2?A:0);return(r==="GK"?c.player.note_g:r==="DEF"?c.player.note_d:r==="MIL"?c.player.note_m:c.player.note_a)+(r===c.player.job||r===c.player.job2?m:0)-I}),d(`Choisir ${r} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${x.length>0?x.map(p=>{var C,P;const c=p.player,A=p.evolution_bonus||0,m=(r==="GK"?c.note_g:r==="DEF"?c.note_d:r==="MIL"?c.note_m:c.note_a)+(r===c.job||r===c.job2?A:0),I=rn(c),B={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[c.rarity];return`<div class="player-option" data-card-id="${p.id}"
  ...
  <div class="page">
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${s.map(_=>{const p=_.cost===0||d>=_.cost,c=n[_.id]===!0;return`<div class="booster-card ${!p||c?"disabled":""}" data-booster="${_.id}" style="position:relative">
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px 16px">
        ${s.map(_=>{const p=r.find(c=>c.gc_type===_.gc_type);return l(_,!!p)}).join("")}
      </div>
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(_=>{_.addEventListener("click",()=>{const p=_.dataset.id,c=s.find(m=>m.id===p);if(!c)return;const A=r.findIndex(m=>m.gc_type===c.gc_type);A>-1?r.splice(A,1):r.length<3&&r.push(c),a()})}),(x=e.querySelector("#gc-launch"))==null||x.addEventListener("click",()=>{n&&u(r)}),(f=e.querySelector("#gc-no-gc"))==null||f.addEventListener("click",()=>u([])),(h=e.querySelector("#gc-reset"))==null||h.addEventListener("click",()=>{r.length&&(r=[],a())})}a()}function _o(e){var o;const t=((o=e==null?void 0:e.state)==null?void 0:o.params)||{},i=t.matchMode||"vs_ai_easy";return i==="friend"?`Match vs ${t.friendName||"Ami"}`:i==="random"?"Match vs Random":`Match vs IA — ${i.replace("vs_ai_","").toUpperCase()}`}async function $o(e,t,i){const{state:o,navigate:d}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await T.from("decks").select(`id,name,is_active,formation,stadium_card_id,
      stadium_card:cards!stadium_card_id(id,stadium_id,
        stadium_def:stadium_definitions(id,name,club_id,country_code,image_url,
          club:clubs(encoded_name,logo_url)))`).eq("owner_id",o.profile.id).order("created_at",{ascending:!1});if(!s||s.length===0){Et(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>d("decks"));return}const r=s.map(f=>f.id),{data:l}=await T.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",r).order("slot_order"),u=[...new Set((l||[]).filter(f=>{var h,_;return((h=f.card)==null?void 0:h.card_type)==="stadium"&&((_=f.card)==null?void 0:_.stadium_id)}).map(f=>f.card.stadium_id))],a={};if(u.length){const{data:f}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",u);(f||[]).forEach(h=>{a[h.id]=h}),(l||[]).forEach(h=>{var _,p;((_=h.card)==null?void 0:_.card_type)==="stadium"&&((p=h.card)!=null&&p.stadium_id)&&(h.card._stadiumDef=a[h.card.stadium_id]||null)})}let n=0;function x(){var C,P,j,N,V,K,oe;const f=s[n],h=(l||[]).filter(ie=>ie.deck_id===f.id),_=h.filter(ie=>{var Q;return ie.is_starter&&((Q=ie.card)==null?void 0:Q.player)}).map(ie=>hi(ie.card,ie.position)),p=h.find(ie=>{var Q;return((Q=ie.card)==null?void 0:Q.card_type)==="formation"}),c=f.formation||((C=p==null?void 0:p.card)==null?void 0:C.formation)||"4-4-2";let A=_.length>=11?vt(_,c):null,m=((P=f.stadium_card)==null?void 0:P.stadium_def)||null;m&&A&&(A=Tt(A,m));const I=_.length>=11;et(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;align-items:center;gap:8px;padding:6px 14px;background:linear-gradient(90deg,rgba(232,119,34,0.3),rgba(196,90,0,0.2));border-top:1px solid rgba(232,119,34,0.4);flex-shrink:0">
        ...
        <div style="font-size:11px;font-weight:900;color:#FFD700">+10 aux joueurs ${((j=m.club)==null?void 0:j.encoded_name)||m.country_code||""}</div>
      </div>`:""}
      ...
      <div style="padding:10px 14px 16px;flex-shrink:0;display:flex;flex-direction:column;gap:8px;background:rgba(0,0,0,0.2)">
        ...
        stroke="${K}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const h=48,_=64,p=13,c=16,A={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[B,C]of Object.entries(a)){const P=x(B);if(!P||!C)continue;const j=B.replace(/[0-9]/g,""),N=Ht[j]||"#555",V=r.includes(C.cardId),K=i==="attack"&&(["MIL","ATT"].includes(j)||V)&&!C.used||i==="defense"&&["GK","DEF","MIL"].includes(j)&&!C.used,oe=o.includes(C.cardId);let ie;i==="attack"?ie=V?Math.max(1,Number(C.note_a)||0):j==="MIL"?Number(C.note_m)||0:Number(C.note_a)||0:i==="defense"?ie=j==="GK"?Number(C.note_g)||0:j==="MIL"?Number(C.note_m)||0:Number(C.note_d)||0:ie=Number(j==="GK"?C.note_g:j==="DEF"?C.note_d:j==="MIL"?C.note_m:C.note_a)||0,ie=ie+(C.boost||0),C.stadiumBonus&&(i==="attack"&&(j==="ATT"||j==="MIL")||i==="defense"&&(j==="GK"||j==="DEF"||j==="MIL")?ie+=10:i||(ie+=10));const Q=(P.x-h/2).toFixed(1),R=(P.y-_/2).toFixed(1),D=A[C==null?void 0:C.rarity]||A.normal;if(C.used){const se=typeof import.meta<"u"&&"/"||"/",de=`${typeof window<"u"&&((I=window.location)==null?void 0:I.origin)||""}${se}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");f+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="#161616"/>`,f+=`<image href="${de}" xlink:href="${de}" x="${Q}" y="${R}" width="${h}" height="${_}" preserveAspectRatio="xMidYMid slice"/>`,f+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="none" stroke="${D}" stroke-width="2" opacity="0.7"/>`,f+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${C.cardId}" data-role="${j}" style="cursor:pointer"/>`;continue}const re=oe?.45:1,me=oe?"#FFD700":D,_e=oe?3:(C==null?void 0:C.rarity)==="legende"||(C==null?void 0:C.rarity)==="légende"||(C==null?void 0:C.rarity)==="pepite"||(C==null?void 0:C.rarity)==="pépite"?2.5:2,ve=_-p-c;f+=`<defs><clipPath id="cp-${B}"><rect x="${Q}" y="${(P.y-_/2+p).toFixed(1)}" width="${h}" height="${ve}"/></clipPath></defs>`,f+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="${N}" opacity="${re}"/>`;const we=He(C);we&&(f+=`<image href="${we}" xlink:href="${we}" x="${Q}" y="${(P.y-_/2+p).toFixed(1)}" width="${h}" height="${ve}" clip-path="url(#cp-${B})" preserveAspectRatio="xMidYMin slice"/>`),f+=`<rect x="${Q}" y="${R}" width="${h}" height="${p}" rx="4" fill="rgba(255,255,255,0.92)"/>`,f+=`<text x="${P.x.toFixed(1)}" y="${(P.y-_/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(C.name||"").slice(0,9)}</text>`;const Ee=(P.y+_/2-c).toFixed(1);f+=`<rect x="${Q}" y="${Ee}" width="${h}" height="${c}" fill="rgba(255,255,255,0.92)"/>`;{const se=Eo(C.country_code);se?f+=`<image href="${se}" xlink:href="${se}" x="${(P.x-20).toFixed(1)}" y="${(P.y+_/2-c+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:f+=`<text x="${(P.x-13).toFixed(1)}" y="${(P.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${To(C.country_code)}</text>`;const pe=C.stadiumBonus?"#E87722":"#111";f+=`<text x="${P.x.toFixed(1)}" y="${(P.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="${pe}" font-family="Arial Black">${ie}</text>`;const de=ko(C);de?f+=`<image href="${de}" xlink:href="${de}" x="${(P.x+h/2-14).toFixed(1)}" y="${(P.y+_/2-c+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:C.clubName&&(f+=`<text x="${(P.x+14).toFixed(1)}" y="${(P.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(C.clubName||"").slice(0,3).toUpperCase()}</text>`),C.stadiumBonus&&(f+=`<rect x="${(P.x-h/2).toFixed(1)}" y="${(P.y-_/2).toFixed(1)}" width="${h}" height="${_}" rx="5" fill="none" stroke="#E87722" stroke-width="2" opacity="0.8"/>`)}f+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="${oe?"rgba(255,255,255,0.12)":"none"}" stroke="${me}" stroke-width="${_e}" opacity="${re}"/>`,K&&(f+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${oe?"selected":""}" data-card-id="${C.cardId}" data-role="${j}" style="cursor:pointer"/>`)}const m=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-m} ${-m} ${d+m*2} ${s+m*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${f}
  ...
  </div>`}async function si(e,t,i,o){var P;const{state:d,navigate:s,toast:r}=t;et(e);const l=d.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!l.deckId)return $o(e,t,i);const u=l.deckId;let a,n,x,f;try{const j=await Promise.all([T.from("decks").select("formation,name,stadium_card_id").eq("id",u).single(),T.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",u).order("slot_order")]);a=j[0].data,x=j[0].error,n=j[1].data,f=j[1].error}catch(j){console.error("[Match] Exception chargement deck:",j),Et(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>s("home"));return}if(x||f){console.error("[Match] Erreur Supabase:",x||f),Et(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>s("home"));return}const h=(n||[]).filter(j=>{var N;return j.is_starter&&((N=j.card)==null?void 0:N.player)}).map(j=>hi(j.card,j.position)),_=(n||[]).filter(j=>{var N;return!j.is_starter&&((N=j.card)==null?void 0:N.player)}).map(j=>hi(j.card,j.position));if(h.length<11){Et(e,"⚠️",`Deck incomplet (${h.length}/11).`,"Compléter",()=>s("decks"));return}const p=(n||[]).find(j=>{var N;return((N=j.card)==null?void 0:N.card_type)==="formation"}),c=(a==null?void 0:a.formation)||((P=p==null?void 0:p.card)==null?void 0:P.formation)||"4-4-2",{data:A,error:m}=await T.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",d.profile.id).eq("card_type","game_changer"),{data:I}=await T.from("gc_definitions").select("*").eq("is_active",!0),B=(A||[]).map(j=>({...j,_gcDef:(I==null?void 0:I.find(N=>N.name===j.gc_type||N.id===j.gc_definition_id))||null}));let C=null;if(a!=null&&a.stadium_card_id){const{data:j}=await T.from("cards").select("stadium_id").eq("id",a.stadium_card_id).single();if(j!=null&&j.stadium_id){const{data:N}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",j.stadium_id).single();C=N||null}}o({deckId:u,formation:c,starters:h,subsRaw:_,gcCardsEnriched:B,gcDefs:I||[],stadiumDef:C})}async function zo(e,t){const{state:i}=t,d=(i.params||{}).matchMode||"vs_ai_easy",s=d.replace("vs_ai_",""),r=d;await si(e,t,d,async({deckId:l,formation:u,starters:a,subsRaw:n,gcCardsEnriched:x,gcDefs:f,stadiumDef:h})=>{try{let _=vt(a,u);h&&(_=Tt(_,h),jt(n,h));const p=await So(u,s),c=p.lines||p,A=async m=>{try{const{data:I,error:B}=await T.from("matches").insert({home_id:i.profile.id,away_id:null,mode:r,home_deck_id:l,status:"in_progress"}).select().single();if(B){console.error("[MatchIA] Erreur création match:",B),Et(e,"⚠️","Impossible de créer le match ("+B.message+").","Retour",()=>t.navigate("home"));return}const C=p.stadiumDef||null;C&&c&&(Tt(c,C),jt(p.subs||[],C));const P={gcDefs:f||[],matchId:I==null?void 0:I.id,mode:r,difficulty:s,formation:u,homeTeam:_,aiTeam:c,homeSubs:n,subsUsed:0,maxSubs:Math.min(n.length,3),aiSubs:p.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((p.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:p.gcCards||[],aiUsedGc:[],aiStadiumDef:C,homeScore:0,aiScore:0,gcCards:m,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Lo(e,P,t)}catch(I){console.error("[MatchIA] Exception launchMatch:",I),Et(e,"⚠️","Erreur au lancement du match : "+I.message,"Retour",()=>t.navigate("home"))}};if(!x.length){A([]);return}ai(e,x,A)}catch(_){console.error("[MatchIA] Exception setup:",_),Et(e,"⚠️","Erreur de préparation du match : "+_.message,"Retour",()=>t.navigate("home"))}})}async function So(e,t){var c;const{data:i}=await T.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:Ao(e),subs:[],gcCards:[],stadiumDef:null};const o=Mt[e]||Mt["4-4-2"],d={GK:[],DEF:[],MIL:[],ATT:[]},s=new Set;function r(A,m,I){var B,C;return s.add(A.id),{cardId:"ai-"+A.id+"-"+I,id:A.id,firstname:A.firstname,name:A.surname_encoded,country_code:A.country_code,club_id:A.club_id,job:A.job,job2:A.job2,note_g:Number(A.note_g)||0,note_d:Number(A.note_d)||0,note_m:Number(A.note_m)||0,note_a:Number(A.note_a)||0,rarity:A.rarity,skin:A.skin,hair:A.hair,hair_length:A.hair_length,clubName:((B=A.clubs)==null?void 0:B.encoded_name)||null,clubLogo:((C=A.clubs)==null?void 0:C.logo_url)||null,boost:0,used:!1,_line:m}}for(const A of["GK","DEF","MIL","ATT"]){const m=i.filter(j=>j.job===A&&!s.has(j.id)),I=i.filter(j=>j.job!==A&&!s.has(j.id)),B=[...m,...I],C=[];for(let j=0;j<o[A];j++){const N=B[j];N&&C.push(r(N,A,j))}const P=st(C.length);C.forEach((j,N)=>{j._col=P[N]}),d[A]=C}const u=i.filter(A=>!s.has(A.id)).slice(0,5).map((A,m)=>r(A,A.job,100+m)),x=Object.keys(Oe).sort(()=>Math.random()-.5).slice(0,3).map((A,m)=>{var I,B;return{id:"ai-gc-"+m,gc_type:A,name:((I=Oe[A])==null?void 0:I.name)||A,icon:((B=Oe[A])==null?void 0:B.icon)||"⚡"}}),f=Object.values(d).flat(),h={};f.forEach(A=>{A.club_id&&(h[A.club_id]=(h[A.club_id]||0)+1)});const _=(c=Object.entries(h).sort((A,m)=>m[1]-A[1])[0])==null?void 0:c[0];let p=null;if(_){const{data:A}=await T.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",_).single();A&&(p={club_id:A.id,country_code:null,name:A.encoded_name+" Stadium",club:{encoded_name:A.encoded_name,logo_url:A.logo_url}})}return{lines:d,subs:u,gcCards:x,stadiumDef:p}}function Ao(e){const t=Mt[e]||Mt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},o=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let d=0;for(const s of["GK","DEF","MIL","ATT"]){const r=[];for(let u=0;u<t[s];u++){const a=3+Math.floor(Math.random()*5);r.push({cardId:"fake-"+d,id:"fake-"+d,firstname:"IA",name:o[d%o.length],country_code:"XX",club_id:null,job:s,job2:null,note_g:s==="GK"?a:2,note_d:s==="DEF"?a:2,note_m:s==="MIL"?a:2,note_a:s==="ATT"?a:2,rarity:"normal",boost:0,used:!1,_line:s}),d++}const l=st(r.length);r.forEach((u,a)=>{u._col=l[a]}),i[s]=r}return i}function Lo(e,t,i){var o;e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>Io(e,t,i),5e3)}function Io(e,t,i){const o=t.homeTeam.MIL||[],d=t.aiTeam.MIL||[],s=t.stadiumDef||null,r=t.aiStadiumDef||null;function l(m,I){const B=Se(m,"MIL"),C=m.evolution_bonus||m._evolution_bonus||0,P=m.job==="MIL"||m.job2==="MIL"?C:0,j=m.stadiumBonus||I&&(I.club_id&&String(m.club_id)===String(I.club_id)||I.country_code&&m.country_code===I.country_code)?10:0;return B+P+j}function u(m,I){return m.reduce((B,C)=>B+l(C,I),0)}function a(m){let I=0;for(let B=0;B<m.length-1;B++){const C=ot(m[B],m[B+1]);C==="#00ff88"?I+=2:C==="#FFD700"&&(I+=1)}return I}const n=u(o,s)+a(o),x=u(d,r)+a(d),f=n>=x;function h(m,I,B,C,P){return`<div id="duel-row-${C}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${m.map((j,N)=>{const V=N<m.length-1?ot(j,m[N+1]):null,K=!V||V==="#ff3333"||V==="#cc2222",oe=V==="#00ff88"?"+2":V==="#FFD700"?"+1":"";l(j,P),j.stadiumBonus||P&&(P.club_id&&String(j.club_id)===String(P.club_id)||P.country_code&&(j.country_code,P.country_code));const ie=j.evolution_bonus||j._evolution_bonus||0;return`
          ...
          ${N<m.length-1?`<div class="duel-link duel-link-${C}" data-idx="${N}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${K?"rgba(255,255,255,0.12)":V};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${K?"none":`0 0 8px ${V}`}">
      ...
      <div class="duel-score-line duel-score-line-${C}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
        Score: ${u(m,P)} + ${a(m)} liens = <b style="color:#fff">${u(m,P)+a(m)}</b>
      </div>
    </div>`}e.innerHTML=`
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,N.style.transition="opacity .45s ease",N.style.opacity="1",N.style.pointerEvents="auto",(V=document.getElementById("start-match-btn"))==null||V.addEventListener("click",A))},600)},700)},2800)}function Mo(e){if(e.type==="duel"&&(e.homeTotal!=null||e.aiTotal!=null)){const t=(e.homeTotal||0)>=(e.aiTotal||0);return`
  ...
  `,document.body.appendChild(o);let n=!1;const x=()=>{n||(n=!0,o.remove(),i())};o.addEventListener("click",x),setTimeout(x,2e3)}function qt(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function fi(e,t,i,o=null,d=null){var h,_;if(t.phase!=="attack"){qt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){qt(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const s=Object.entries(t.homeTeam).flatMap(([p,c])=>(c||[]).filter(A=>A.used).map(A=>({...A,_line:A._line||p}))),r=t.homeSubs.filter(p=>!t.usedSubIds.includes(p.cardId));if(!s.length){qt("Aucun joueur utilisé à remplacer");return}if(!r.length){qt("Aucun remplaçant disponible");return}let l=Math.max(0,s.findIndex(p=>p.cardId===d));const u=((h=s[l])==null?void 0:h._line)||((_=s[l])==null?void 0:_.job);let a=o?Math.max(0,r.findIndex(p=>p.cardId===o)):Math.max(0,r.findIndex(p=>p.job===u)),n=!1;const x=document.createElement("div");x.id="sub-overlay",x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function f(){var C,P,j,N,V,K;const p=s[l],c=r[a],A=Math.min(130,Math.round((window.innerWidth-90)/2)),m=Math.round(A*1.35),I=oe=>`background:rgba(255,255,255,0.12);border:none;color:${oe?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${oe?"default":"pointer"};flex-shrink:0`;x.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="out-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
        ...
        <div>${p?Xe({...p,used:!1,boost:0},A,m):"<div>—</div>"}</div>
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
      ${e.map(x=>{const f=x._line||x.job||"MIL",h={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[f]||"#555",_=Se(x,f)+(x.boost||0),p=l.find(c=>c.cardId===x.cardId);return`<div class="gc-pick-item" data-cid="${x.cardId}"
          style="width:80px;border-radius:8px;border:2.5px solid ${p?"#FFD700":"rgba(255,255,255,0.25)"};background:${h};overflow:hidden;cursor:pointer;flex-shrink:0;${x.used?"opacity:0.3;pointer-events:none":""}">
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(s);let l=!1;const u=()=>{l||(l=!0,s.remove(),setTimeout(()=>d(),50))};s.addEventListener("click",u),setTimeout(u,3500)}async function Pt(e,t,i){var n,x;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:o}=i,d=t.homeScore>t.aiScore,s=t.homeScore===t.aiScore,r=d?"victoire":s?"nul":"defaite",l=Hn(t.mode,r);t.matchId&&await T.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:d?o.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const u={credits:(o.profile.credits||0)+l,matches_played:(o.profile.matches_played||0)+1};d?u.wins=(o.profile.wins||0)+1:s?u.draws=(o.profile.draws||0)+1:u.losses=(o.profile.losses||0)+1,await T.from("users").update(u).eq("id",o.profile.id),await i.refreshProfile();const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",a.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const B=async(N=!0)=>{_=!0,p&&(T.removeChannel(p),p=null),N&&await T.rpc("cancel_matchmaking",{p_user_id:x.profile.id}).catch(()=>{})};(j=document.getElementById("mm-cancel"))==null||j.addEventListener("click",async()=>{try{await B(!0)}catch{}Ve(e),f("home")});const C=async(N,V,K)=>{if(_)return;_=!0,p&&(T.removeChannel(p),p=null);const oe=document.getElementById("mm-status");oe&&(oe.textContent="Adversaire trouvé !"),await new Promise(ie=>setTimeout(ie,600)),e.isConnected&&gn(e,t,N,K,r,l,a,n,u)};if(a){const N=[{range:50,delay:0},{range:100,delay:15e3},{range:200,delay:3e4},{range:400,delay:45e3},{range:800,delay:6e4}];let V=0,K=!1;const oe=async()=>{if(_||K)return;const{range:ie}=N[V],Q=document.getElementById("mm-range");Q&&(Q.textContent=`Plage MMR : ±${ie}`);const{data:R,error:D}=await T.rpc("try_matchmake_ranked",{p_user_id:x.profile.id,p_deck_id:i,p_mmr:c,p_range:ie});if(console.log("[Ranked] try_matchmake_ranked result:",R,"error:",JSON.stringify(D)),_)return;if(D||!(R!=null&&R.success)){h("Erreur matchmaking ranked","error"),Ve(e),f("home");return}if(R.matched){K=!0,C(R.match_id,R.opponent_id,!1);return}const re=document.getElementById("mm-status");if(re&&(re.textContent="En attente d'un adversaire..."),p||(p=T.channel("matchmaking-"+x.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${x.profile.id}`},me=>{const _e=me.new;_e.status==="matched"&&_e.match_id&&!K&&(K=!0,C(_e.match_id,_e.matched_with,!0))}).subscribe()),V<N.length-1){const me=N[V+1].delay-N[V].delay;V++,setTimeout(()=>{!_&&!K&&oe()},me)}};await oe()}else{const{data:N,error:V}=await T.rpc("try_matchmake",{p_user_id:x.profile.id,p_deck_id:i});if(V||!(N!=null&&N.success)){h("Erreur de matchmaking","error"),Ve(e),f("home");return}if(N.matched){C(N.match_id,N.opponent_id,!1);return}const K=document.getElementById("mm-status");K&&(K.textContent="En attente d'un autre joueur..."),p=T.channel("matchmaking-"+x.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${x.profile.id}`},oe=>{const ie=oe.new;ie.status==="matched"&&ie.match_id&&C(ie.match_id,ie.matched_with,!0)}).subscribe()}}async function Ko(e,t,i){const{state:o,navigate:d,toast:s}=t;try{(T.getChannels?T.getChannels():[]).forEach(a=>{const n=a.topic||"";(n.includes("matchmaking")||n.includes("pvp-match")||n.includes("friend-invite"))&&T.removeChannel(a)})}catch{}const{data:r}=await T.from("matches").select("home_id, away_id, status").eq("id",i).single();if(!r){s("Match introuvable","error"),d("home");return}if(r.status==="finished"){s("Ce match est terminé","info"),d("home");return}const l=r.home_id===o.profile.id;gn(e,t,i,l,[],[])}async function gn(e,t,i,o,d=[],s=[],r=!1,l=null,u=null){const{state:a,navigate:n,toast:x}=t,f=o?"p1":"p2",h=o?"p2":"p1",_=(d||[]).map($=>$.id),p=(d||[]).map($=>({id:$.id,gc_type:$.gc_type,_gcDef:$._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:c}=await T.from("matches").select("*").eq("id",i).single();if(!c){x("Match introuvable","error"),n("home");return}async function A(){var W,te;console.log("[PvP] buildGameState match:",{id:c.id,home_deck_id:c.home_deck_id,away_deck_id:c.away_deck_id,mode:c.mode,is_ranked:c.is_ranked});const[{data:$,error:g},{data:y,error:b},{data:v},{data:w}]=await Promise.all([T.rpc("get_deck_for_match",{p_deck_id:c.home_deck_id}),T.rpc("get_deck_for_match",{p_deck_id:c.away_deck_id}),T.from("users").select("id,pseudo,club_name").eq("id",c.home_id).single(),T.from("users").select("id,pseudo,club_name").eq("id",c.away_id).single()]);console.log("[PvP] get_deck_for_match p1:",g==null?void 0:g.message,"p2:",b==null?void 0:b.message,"p1D:",(W=$==null?void 0:$.starters)==null?void 0:W.length,"p2D:",(te=y==null?void 0:y.starters)==null?void 0:te.length);const k=Z=>{const fe=Number(Z.evolution_bonus)||0;return{cardId:Z.card_id,position:Z.position,id:Z.id,firstname:Z.firstname,name:Z.surname_encoded,country_code:Z.country_code,club_id:Z.club_id,job:Z.job,job2:Z.job2,note_g:(Number(Z.note_g)||0)+(Z.job==="GK"||Z.job2==="GK"&&Number(Z.note_g)>0?fe:0),note_d:(Number(Z.note_d)||0)+(Z.job==="DEF"||Z.job2==="DEF"&&Number(Z.note_d)>0?fe:0),note_m:(Number(Z.note_m)||0)+(Z.job==="MIL"||Z.job2==="MIL"&&Number(Z.note_m)>0?fe:0),note_a:(Number(Z.note_a)||0)+(Z.job==="ATT"||Z.job2==="ATT"&&Number(Z.note_a)>0?fe:0),evolution_bonus:fe,rarity:Z.rarity,skin:Z.skin,hair:Z.hair,hair_length:Z.hair_length,clubName:Z.club_encoded_name||null,clubLogo:Z.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},z=(($==null?void 0:$.starters)||[]).map(Z=>k(Z)),E=((y==null?void 0:y.starters)||[]).map(Z=>k(Z)),L=($==null?void 0:$.formation)||"4-4-2",S=(y==null?void 0:y.formation)||"4-4-2";let M=vt(z,L),F=vt(E,S);const Y=(($==null?void 0:$.subs)||[]).map(Z=>k(Z)),J=((y==null?void 0:y.subs)||[]).map(Z=>k(Z)),U=($==null?void 0:$.stadium_def)||(o?u:null),G=(y==null?void 0:y.stadium_def)||(o?null:u);return U&&(M=Tt(M,U),jt(Y,U)),G&&(F=Tt(F,G),jt(J,G)),{p1Team:M,p2Team:F,p1Subs:Y,p2Subs:J,p1Formation:L,p2Formation:S,p1Name:(v==null?void 0:v.club_name)||(v==null?void 0:v.pseudo)||"Joueur 1",p2Name:(w==null?void 0:w.club_name)||(w==null?void 0:w.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:o?_:[],gc_p2:o?[]:_,gcCardsFull_p1:o?p:[],gcCardsFull_p2:o?[]:p,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let m=c.game_state&&Object.keys(c.game_state).length?c.game_state:null;if(console.log("[PvP] init - amIHome:",o,"gameState exists:",!!m,"match.status:",c.status,"home_id:",c.home_id,"away_id:",c.away_id,"myId:",a.profile.id),!m)if(o){m=await A(),console.log("[PvP] home - gameState built, p1Team keys:",Object.keys((m==null?void 0:m.p1Team)||{}));const{data:$}=await T.from("matches").select("game_state").eq("id",i).single();!($!=null&&$.game_state)||!Object.keys($.game_state).length?await T.from("matches").update({game_state:m,turn_user_id:c.home_id}).eq("id",i):m=$.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>',console.log("[PvP] away - waiting for home to write game_state...");for(let $=0;$<30&&!m;$++){await new Promise(y=>setTimeout(y,400));const{data:g}=await T.from("matches").select("game_state").eq("id",i).single();g!=null&&g.game_state&&Object.keys(g.game_state).length&&(m=g.game_state),$%5===0&&console.log("[PvP] away - poll",$,"game_state:",!!(g!=null&&g.game_state))}if(!m){x("Erreur de synchronisation","error"),n("home");return}m.gc_p2=_,m.gcCardsFull_p2=p,await T.from("matches").update({game_state:m}).eq("id",i)}let I=!1,B=null,C=!1;const P=new Set,j=new Set;async function N($){var S,M;try{T.removeChannel(V)}catch{}const g=m[f+"Score"]||0,y=m[h+"Score"]||0;let b,v;$.winner_id?(b=$.winner_id===a.profile.id,v=!1):$.forfeit?(b=g>y,v=!1):(v=g===y,b=g>y);let w="";if(r&&o)try{const{data:F}=await T.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",c.home_id).single(),{data:Y}=await T.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",c.away_id).single();if(F&&Y){const J=b?1:v?.5:0,U=1-J,G=F.placement_matches<10,W=Y.placement_matches<10,te=Ii(F.mmr,F.mmr_deviation,F.mmr_volatility,Y.mmr,Y.mmr_deviation,J===1?1:J===0?0:.5,G),Z=Ii(Y.mmr,Y.mmr_deviation,Y.mmr_volatility,F.mmr,F.mmr_deviation,U===1?1:U===0?0:.5,W);await T.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:v?null:b?c.home_id:c.away_id,p_home_id:c.home_id,p_away_id:c.away_id,p_home_delta:te.delta,p_away_delta:Z.delta,p_home_new_rd:te.newRd,p_away_new_rd:Z.newRd,p_home_new_vol:te.newSigma,p_away_new_vol:Z.newSigma});const fe=te.delta,ye=te.newMmr,be=Nt(ye),ke=fe>=0?"+":"",Be=fe>=0?"#4caf50":"#ff6b6b",Fe=Nt(F.mmr);w=`
      ...
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${r?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(k),(M=k.querySelector("#pvp-end-home"))==null||M.addEventListener("click",()=>{k.remove(),Ve(e),n(r?"ranked":"home")})}const V=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},$=>{const g=$.new;try{if(g.status==="finished"||g.forfeit){if(I)return;I=!0,B&&(clearInterval(B),B=null),g.game_state&&(m=g.game_state),N(g);return}if(g.game_state){const y=m;m=g.game_state;const b=m._lastGC;if(b&&b.seq&&!j.has(b.seq)&&(j.add(b.seq),b.by!==f)){se(b.type,b.by,()=>me());return}const v=y[f+"Score"]||0,w=y[h+"Score"]||0,k=m[f+"Score"]||0,z=m[h+"Score"]||0,E=k>v,L=z>w;if((E||L)&&!P.has(m.round)){P.add(m.round);const S=[...m.log||[]].reverse().find(F=>F.isGoal),M=((S==null?void 0:S.homePlayers)||[]).map(F=>({name:F.name,note:F.note,portrait:F.portrait,job:F.job}));Ee(M,k,z,E,()=>me());return}me()}}catch(y){console.error("[PvP] crash:",y)}}).subscribe();async function K($){Object.assign(m,$),m.lastActionAt=new Date().toISOString();const{error:g}=await T.from("matches").update({game_state:m}).eq("id",i);g&&x("Erreur de synchronisation","error");try{me()}catch(y){console.error("[PvP] renderPvpScreen crash:",y)}}async function oe(){if(I)return;I=!0,B&&(clearInterval(B),B=null);const $=o?c.away_id:c.home_id,g=o?"p2":"p1",y=o?"p1":"p2",b={...m,[g+"Score"]:3,[y+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:$,home_score:b.p1Score||0,away_score:b.p2Score||0,game_state:b}).eq("id",i)}catch{}try{T.removeChannel(V)}catch{}setTimeout(()=>{Ve(e),n("home")},800)}const ie={BOOST_STAT:({value:$=1,count:g=1,roles:y=[]},b,v)=>{const w=b[f+"Team"],k=Object.entries(w).filter(([z])=>!y.length||y.includes(z)).flatMap(([z,E])=>E.filter(L=>!L.used).map(L=>({...L,_line:z})));if(!k.length){b.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),v(b);return}Q(k,g,`Choisir ${g} joueur(s) à booster (+${$})`,z=>{z.forEach(E=>{const L=(w[E._line]||[]).find(S=>S.cardId===E.cardId);L&&(L.boost=(L.boost||0)+$,b.log.push({text:`⚡ +${$} sur ${L.name}`,type:"info"}))}),b[f+"Team"]=w,v(b)})},DEBUFF_STAT:({value:$=1,count:g=1,roles:y=[],target:b="ai"},v,w)=>{const k=b==="home"?f:h,z=v[k+"Team"],E=b==="home"?"allié":"adverse",L=Object.entries(z).filter(([S])=>!y.length||y.includes(S)).flatMap(([S,M])=>M.map(F=>({...F,_line:S})));if(!L.length){v.log.push({text:`🎯 Aucun joueur ${E}`,type:"info"}),w(v);return}Q(L,g,`Choisir ${g} joueur(s) ${E}(s) (-${$})`,S=>{S.forEach(M=>{const F=(z[M._line]||[]).find(Y=>Y.cardId===M.cardId);F&&(F.boost=(F.boost||0)-$,v.log.push({text:`🎯 -${$} sur ${F.name}`,type:"info"}))}),v[k+"Team"]=z,w(v)})},GRAY_PLAYER:({count:$=1,roles:g=[],target:y="ai"},b,v)=>{const w=y==="home"?f:h,k=b[w+"Team"],z=y==="home"?"allié":"adverse",E=Object.entries(k).filter(([L])=>!g.length||g.includes(L)).flatMap(([L,S])=>S.filter(M=>!M.used).map(M=>({...M,_line:L})));if(!E.length){b.log.push({text:`❌ Aucun joueur ${z}`,type:"info"}),v(b);return}Q(E,$,`Choisir ${$} joueur(s) ${z}(s) à exclure`,L=>{const S="usedCardIds_"+w,M=new Set(b[S]||[]);L.forEach(F=>{const Y=(k[F._line]||[]).find(J=>J.cardId===F.cardId);Y&&(Y.used=!0,M.add(F.cardId),b.log.push({text:`❌ ${Y.name} exclu !`,type:"info"}))}),b[S]=[...M],b[w+"Team"]=k,v(b)})},REVIVE_PLAYER:({count:$=1,roles:g=[]},y,b)=>{const v=y[f+"Team"],w=Object.entries(v).filter(([k])=>!g.length||g.includes(k)).flatMap(([k,z])=>z.filter(E=>E.used).map(E=>({...E,_line:k})));if(!w.length){y.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),b(y);return}Q(w,$,`Choisir ${$} joueur(s) à ressusciter`,k=>{k.forEach(z=>{const E=(v[z._line]||[]).find(L=>L.cardId===z.cardId);E&&(E.used=!1,y.log.push({text:`💫 ${E.name} ressuscité !`,type:"info"}))}),y[f+"Team"]=v,b(y)})},REMOVE_GOAL:({},$,g)=>{const y=h+"Score";$[y]>0?($[y]--,$.log.push({text:"🚫 Dernier but annulé !",type:"info"})):$.log.push({text:"🚫 Aucun but à annuler",type:"info"}),g($)},ADD_GOAL_DRAW:({},$,g)=>{$[f+"Score"]===$[h+"Score"]?($[f+"Score"]++,$.log.push({text:"🎯 But bonus !",type:"info"})):$.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),g($)},ADD_SUB:({value:$=1},g,y)=>{g.maxSubs=(g.maxSubs||3)+$,g.log.push({text:`🔄 +${$} remplacement(s)`,type:"info"}),y(g)},CUSTOM:({},$,g)=>g($)};function Q($,g,y,b){const v=document.createElement("div");v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let w=[];function k(){var E,L;const z=$.map(S=>{const M={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[S._line]||"#555",F=Se(S,S._line)+(S.boost||0),J=w.find(G=>G.cardId===S.cardId)?"#FFD700":"rgba(255,255,255,0.25)",U=S.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${S.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${J};background:${M};overflow:hidden;cursor:pointer;${U}">
        ...
        </div>`,(E=v.querySelector("#pp-close"))==null||E.addEventListener("click",()=>v.remove()),v.querySelectorAll(".pp-item").forEach(S=>{S.addEventListener("click",()=>{const M=S.dataset.cid,F=$.find(J=>J.cardId===M),Y=w.findIndex(J=>J.cardId===M);F&&(Y>-1?w.splice(Y,1):w.length<g&&w.push(F),k())})}),(L=v.querySelector("#pp-confirm"))==null||L.addEventListener("click",()=>{v.remove(),b(w)})}k(),document.body.appendChild(v)}async function R($,g){const b=(m["gcCardsFull_"+f]||[]).find(z=>z.id===$),v=(b==null?void 0:b._gcDef)||(m.gcDefs||[]).find(z=>{var E;return z.name===g||((E=z.name)==null?void 0:E.toLowerCase().trim())===(g==null?void 0:g.toLowerCase().trim())}),w=[...m["usedGc_"+f]||[],$],k={type:g,by:f,seq:(m._gcAnimSeq||0)+1};j.add(k.seq),se(g,f,async()=>{if(v!=null&&v.effect_type&&v.effect_type!=="CUSTOM"){const E=ie[v.effect_type];if(E){const L={...m};E(v.effect_params||{},L,async S=>{S["usedGc_"+f]=w,S._lastGC=k,S._gcAnimSeq=k.seq,await K(S)});return}}const z={...m};switch(g){case"Remplacement+":z.maxSubs=(z.maxSubs||3)+1,z.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const E=h+"Score";z[E]>0&&(z[E]--,z.log.push({text:"🚫 But annulé",type:"info"}));break}}z["usedGc_"+f]=w,z._lastGC=k,z._gcAnimSeq=k.seq,await K(z)})}function D($,g){const y="usedCardIds_"+$,b=new Set(m[y]||[]);g.forEach(v=>b.add(v)),m[y]=[...b]}function re(){for(const $ of["p1","p2"]){const g=new Set(m["usedCardIds_"+$]||[]),y=m[$+"Team"];if(y)for(const b of["GK","DEF","MIL","ATT"])(y[b]||[]).forEach(v=>{v.used=g.has(v.cardId)})}}function me(){var We,nt,pt,ut,ft,ce;if(m.phase==="reveal")return _e();if(m.phase==="midfield")return we();if(m.phase==="finished")return q();const $=m[f+"Team"],g=m[h+"Team"];re();const y=m[f+"Score"],b=m[h+"Score"],v=m[f+"Name"],w=m[h+"Name"],k=m.phase===f+"-attack",z=m.phase===f+"-defense",E=Array.isArray(m["selected_"+f])?m["selected_"+f]:[],L=E.map(X=>X.cardId),S=window.innerWidth>=700,M=m[f+"Subs"]||[],F=m["usedSubIds_"+f]||[],Y=M.filter(X=>!F.includes(X.cardId)),J=m["gcCardsFull_"+f]||[],U=m["usedGc_"+f]||[],G=J.filter(X=>!U.includes(X.id)),W=m.boostOwner===f&&!m.boostUsed,te=!["GK","DEF","MIL","ATT"].some(X=>(g[X]||[]).some(H=>!H.used)),Z=[...$.MIL||[],...$.ATT||[]].filter(X=>!X.used),fe=k&&te&&Z.length===0?[...$.GK||[],...$.DEF||[]].filter(X=>!X.used).map(X=>X.cardId):[];function ye(X,H,ae){var kt,Ct;const he=X._gcDef,$e={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[he==null?void 0:he.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Qe={purple:"#b06ce0",light_blue:"#00d4ef"}[he==null?void 0:he.color]||"#b06ce0",Ze=(he==null?void 0:he.name)||X.gc_type,tt=(he==null?void 0:he.effect)||((kt=Oe[X.gc_type])==null?void 0:kt.desc)||"",rt=he!=null&&he.image_url?`/icons/${he.image_url}`:null,_t=((Ct=Oe[X.gc_type])==null?void 0:Ct.icon)||"⚡",$t=Math.round(ae*.22),dt=Math.round(ae*.22),gt=ae-$t-dt,mt=Ze.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${X.id}" data-gc-type="${X.gc_type}"
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ct($,m[f+"Formation"],Ne,L,300,300,fe)}
      </div>
        ...
        </div>`}return`<div style="font-size:11px;color:${X.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${X.type==="goal"?700:400};padding:3px 2px">${X.text||""}</div>`}const Ke=(()=>{var H,ae;if(z&&((H=m.pendingAttack)!=null&&H.players)){const he=m.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(w);let E=!1;const L=()=>{E||(E=!0,w.remove(),setTimeout(()=>v(),50))};w.addEventListener("click",L),setTimeout(L,3500)}function se($,g,y){var U,G;const b=(m.gcDefs||[]).find(W=>{var te;return W.name===$||((te=W.name)==null?void 0:te.toLowerCase().trim())===($==null?void 0:$.toLowerCase().trim())}),v={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[b==null?void 0:b.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",w={purple:"#b06ce0",light_blue:"#00d4ef"}[b==null?void 0:b.color]||"#b06ce0",k=(b==null?void 0:b.name)||$,z=(b==null?void 0:b.effect)||((U=Oe[$])==null?void 0:U.desc)||"",E=b!=null&&b.image_url?`/icons/${b.image_url}`:null,L=((G=Oe[$])==null?void 0:G.icon)||"⚡",M=g===f?"Vous":m[g+"Name"]||"Adversaire",F=document.createElement("div");F.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",F.innerHTML=`
      <style>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      </div>`,(R=document.getElementById("lobby-cancel"))==null||R.addEventListener("click",async()=>{_=!0,p&&(T.removeChannel(p),p=null),ie&&await T.from("friend_match_invites").update({status:"declined"}).eq("id",ie),Ve(e),x("home")})},A=async(K,oe)=>{_=!0,p&&(T.removeChannel(p),p=null),await new Promise(ie=>setTimeout(ie,600)),e.isConnected&&Jo(e,t,K,oe,r,l)},{data:m}=await T.from("friend_match_invites").select("*").eq("status","pending").or(`and(inviter_id.eq.${h},invitee_id.eq.${u}),and(inviter_id.eq.${u},invitee_id.eq.${h})`).order("created_at",{ascending:!1}).limit(1).maybeSingle();let I,B;if(m&&m.inviter_id===u)B=!1,I=m.id,await T.from("friend_match_invites").update({invitee_deck_id:i,invitee_ready:!0,status:"accepted"}).eq("id",I);else{B=!0;const{data:K,error:oe}=await T.from("friend_match_invites").insert({inviter_id:h,invitee_id:u,friend_id:u,inviter_deck_id:i,inviter_ready:!0,status:"pending"}).select().single();if(oe||!K){f("Erreur création invitation","error"),Ve(e),x("home");return}I=K.id}if(c(!0,!B,I),!B){const K={home_id:u,away_id:h,home_deck_id:m.inviter_deck_id,away_deck_id:i,status:"active",mode:"friend"},oe=await T.from("matches").insert(K).select().single(),ie=oe.data;if(!ie){f("Erreur création match: "+(((V=oe.error)==null?void 0:V.message)||""),"error"),Ve(e),x("home");return}await T.from("friend_match_invites").update({match_id:ie.id,status:"matched"}).eq("id",I),A(ie.id,!1);return}let C=!1;const P=K=>{_||C||(K.status==="matched"&&K.match_id?(C=!0,clearInterval(j),clearInterval(N),A(K.match_id,!0)):K.status==="declined"?(clearInterval(j),clearInterval(N),f(`${a} a décliné l'invitation`,"warning"),Ve(e),x("home")):K.invitee_ready&&c(!0,!0,I))},j=setInterval(()=>{if(_){clearInterval(j);return}T.from("matchmaking_queue").delete().eq("user_id",h).then(()=>{},()=>{})},3e3),N=setInterval(async()=>{if(_||C){clearInterval(N);return}const{data:K}=await T.from("friend_match_invites").select("*").eq("id",I).maybeSingle();K&&P(K)},1200);p=T.channel("friend-invite-"+I).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${I}`},K=>P(K.new)).subscribe()}async function Jo(e,t,i,o,d=[],s=[]){const{state:r,navigate:l,toast:u}=t,a=o?"p1":"p2",n=o?"p2":"p1",x=(d||[]).map(q=>q.id),f=(d||[]).map(q=>({id:q.id,gc_type:q.gc_type,_gcDef:q._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:h}=await T.from("matches").select("*").eq("id",i).single();if(!h){u("Match introuvable","error"),l("home");return}async function _(){const[{data:q},{data:$},{data:g},{data:y}]=await Promise.all([T.rpc("get_deck_for_match",{p_deck_id:h.home_deck_id}),T.rpc("get_deck_for_match",{p_deck_id:h.away_deck_id}),T.from("users").select("id,pseudo,club_name").eq("id",h.home_id).single(),T.from("users").select("id,pseudo,club_name").eq("id",h.away_id).single()]),b=E=>{const L=Number(E.evolution_bonus)||0;return{cardId:E.card_id,position:E.position,id:E.id,firstname:E.firstname,name:E.surname_encoded,country_code:E.country_code,club_id:E.club_id,job:E.job,job2:E.job2,note_g:(Number(E.note_g)||0)+(E.job==="GK"||E.job2==="GK"&&Number(E.note_g)>0?L:0),note_d:(Number(E.note_d)||0)+(E.job==="DEF"||E.job2==="DEF"&&Number(E.note_d)>0?L:0),note_m:(Number(E.note_m)||0)+(E.job==="MIL"||E.job2==="MIL"&&Number(E.note_m)>0?L:0),note_a:(Number(E.note_a)||0)+(E.job==="ATT"||E.job2==="ATT"&&Number(E.note_a)>0?L:0),evolution_bonus:L,rarity:E.rarity,skin:E.skin,hair:E.hair,hair_length:E.hair_length,clubName:E.club_encoded_name||null,clubLogo:E.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},v=((q==null?void 0:q.starters)||[]).map(E=>b(E)),w=(($==null?void 0:$.starters)||[]).map(E=>b(E)),k=(q==null?void 0:q.formation)||"4-4-2",z=($==null?void 0:$.formation)||"4-4-2";return{p1Team:(()=>{const E=vt(v,k);return stadiumDef?Tt(E,stadiumDef):E})(),p2Team:vt(w,z),p1Subs:(()=>{const E=((q==null?void 0:q.subs)||[]).map(L=>b(L));return stadiumDef&&jt(E,stadiumDef),E})(),p2Subs:(($==null?void 0:$.subs)||[]).map(E=>b(E)),p1Formation:k,p2Formation:z,p1Name:(g==null?void 0:g.club_name)||(g==null?void 0:g.pseudo)||"Joueur 1",p2Name:(y==null?void 0:y.club_name)||(y==null?void 0:y.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:o?x:[],gc_p2:o?[]:x,gcCardsFull_p1:o?f:[],gcCardsFull_p2:o?[]:f,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let p=h.game_state&&Object.keys(h.game_state).length?h.game_state:null;if(!p)if(o){p=await _();const{data:q}=await T.from("matches").select("game_state").eq("id",i).single();!(q!=null&&q.game_state)||!Object.keys(q.game_state).length?await T.from("matches").update({game_state:p,turn_user_id:h.home_id}).eq("id",i):p=q.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let q=0;q<60&&!p;q++){await new Promise(g=>setTimeout(g,400));const{data:$}=await T.from("matches").select("game_state").eq("id",i).single();$!=null&&$.game_state&&Object.keys($.game_state).length&&(p=$.game_state)}if(!p){u("Erreur de synchronisation","error"),l("home");return}p.gc_p2=x,p.gcCardsFull_p2=f,await T.from("matches").update({game_state:p}).eq("id",i)}let c=!1,A=!1,m=!1,I=null,B=!1;const C=new Set,P=new Set;function j(q){var E,L;try{T.removeChannel(N)}catch{}const $=p[a+"Score"]||0,g=p[n+"Score"]||0;if(!m){m=!0;const S=q.winner_id||($>g?r.profile.id:g>$?"opp":null),M=S===r.profile.id?"win":S?"loss":null;M&&oi(()=>Promise.resolve().then(()=>on),void 0).then(F=>F.applyOwnEvolution(r.profile.id,M)).catch(()=>{})}if(!A){const S=p.p1Score||0,M=p.p2Score||0,F=(p.usedGc_p1||[]).length,Y=(p.usedGc_p2||[]).length,J=q.winner_id||(S>M?h.home_id:M>S?h.away_id:null);(J?r.profile.id===J:r.profile.id<(o?h.away_id:h.home_id))&&(A=!0,Ji({player1Id:h.home_id,player2Id:h.away_id,score1:S,score2:M,gc1:F,gc2:Y}).catch(G=>console.warn("[FriendMatch] updateStats:",G)))}let y,b;q.winner_id?(y=q.winner_id===r.profile.id,b=!1):q.forfeit?(y=$>g,b=!1):(b=$===g,y=$>g),(E=document.getElementById("pvp-end-overlay"))==null||E.remove();const v=document.createElement("div");v.id="pvp-end-overlay",v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const w=b?"🤝":y?"🏆":"😞",k=b?"MATCH NUL":y?"VICTOIRE !":"DÉFAITE",z=b?"#fff":y?"#FFD700":"#ff6b6b";v.innerHTML=`
      <div style="font-size:64px">${w}</div>
      <div style="font-size:26px;font-weight:900;color:${z}">${k}</div>
      <div style="font-size:18px">${p[a+"Name"]} ${$} – ${g} ${p[n+"Name"]}</div>
      ${q.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${y?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(v),(L=v.querySelector("#pvp-end-home"))==null||L.addEventListener("click",()=>{v.remove(),Ve(e),l("home")})}const N=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},q=>{const $=q.new;try{if($.status==="finished"||$.forfeit){if(c)return;c=!0,I&&(clearInterval(I),I=null),$.game_state&&(p=$.game_state),j($);return}if($.game_state){const g=p;p=$.game_state;const y=p._lastGC;if(y&&y.seq&&!P.has(y.seq)&&(P.add(y.seq),y.by!==a)){Ee(y.type,y.by,()=>re());return}const b=g[a+"Score"]||0,v=g[n+"Score"]||0,w=p[a+"Score"]||0,k=p[n+"Score"]||0,z=w>b,E=k>v;if((z||E)&&!C.has(p.round)){C.add(p.round);const L=[...p.log||[]].reverse().find(M=>M.isGoal),S=((L==null?void 0:L.homePlayers)||[]).map(M=>({name:M.name,note:M.note,portrait:M.portrait,job:M.job}));we(S,w,k,z,()=>re());return}re()}}catch(g){console.error("[PvP] crash:",g)}}).subscribe();async function V(q){Object.assign(p,q),p.lastActionAt=new Date().toISOString();const{error:$}=await T.from("matches").update({game_state:p}).eq("id",i);$&&u("Erreur de synchronisation","error");try{re()}catch(g){console.error("[PvP] renderPvpScreen crash:",g)}}async function K(){if(c)return;c=!0,I&&(clearInterval(I),I=null);const q=o?h.away_id:h.home_id,$=o?"p2":"p1",g=o?"p1":"p2",y={...p,[$+"Score"]:3,[g+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:q,home_score:y.p1Score||0,away_score:y.p2Score||0,game_state:y}).eq("id",i)}catch{}try{T.removeChannel(N)}catch{}setTimeout(()=>{Ve(e),l("home")},800)}const oe={BOOST_STAT:({value:q=1,count:$=1,roles:g=[]},y,b)=>{const v=y[a+"Team"],w=Object.entries(v).filter(([k])=>!g.length||g.includes(k)).flatMap(([k,z])=>z.filter(E=>!E.used).map(E=>({...E,_line:k})));if(!w.length){y.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),b(y);return}ie(w,$,`Choisir ${$} joueur(s) à booster (+${q})`,k=>{k.forEach(z=>{const E=(v[z._line]||[]).find(L=>L.cardId===z.cardId);E&&(E.boost=(E.boost||0)+q,y.log.push({text:`⚡ +${q} sur ${E.name}`,type:"info"}))}),y[a+"Team"]=v,b(y)})},DEBUFF_STAT:({value:q=1,count:$=1,roles:g=[],target:y="ai"},b,v)=>{const w=y==="home"?a:n,k=b[w+"Team"],z=y==="home"?"allié":"adverse",E=Object.entries(k).filter(([L])=>!g.length||g.includes(L)).flatMap(([L,S])=>S.map(M=>({...M,_line:L})));if(!E.length){b.log.push({text:`🎯 Aucun joueur ${z}`,type:"info"}),v(b);return}ie(E,$,`Choisir ${$} joueur(s) ${z}(s) (-${q})`,L=>{L.forEach(S=>{const M=(k[S._line]||[]).find(F=>F.cardId===S.cardId);M&&(M.boost=(M.boost||0)-q,b.log.push({text:`🎯 -${q} sur ${M.name}`,type:"info"}))}),b[w+"Team"]=k,v(b)})},GRAY_PLAYER:({count:q=1,roles:$=[],target:g="ai"},y,b)=>{const v=g==="home"?a:n,w=y[v+"Team"],k=g==="home"?"allié":"adverse",z=Object.entries(w).filter(([E])=>!$.length||$.includes(E)).flatMap(([E,L])=>L.filter(S=>!S.used).map(S=>({...S,_line:E})));if(!z.length){y.log.push({text:`❌ Aucun joueur ${k}`,type:"info"}),b(y);return}ie(z,q,`Choisir ${q} joueur(s) ${k}(s) à exclure`,E=>{const L="usedCardIds_"+v,S=new Set(y[L]||[]);E.forEach(M=>{const F=(w[M._line]||[]).find(Y=>Y.cardId===M.cardId);F&&(F.used=!0,S.add(M.cardId),y.log.push({text:`❌ ${F.name} exclu !`,type:"info"}))}),y[L]=[...S],y[v+"Team"]=w,b(y)})},REVIVE_PLAYER:({count:q=1,roles:$=[]},g,y)=>{const b=g[a+"Team"],v=Object.entries(b).filter(([w])=>!$.length||$.includes(w)).flatMap(([w,k])=>k.filter(z=>z.used).map(z=>({...z,_line:w})));if(!v.length){g.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),y(g);return}ie(v,q,`Choisir ${q} joueur(s) à ressusciter`,w=>{w.forEach(k=>{const z=(b[k._line]||[]).find(E=>E.cardId===k.cardId);z&&(z.used=!1,g.log.push({text:`💫 ${z.name} ressuscité !`,type:"info"}))}),g[a+"Team"]=b,y(g)})},REMOVE_GOAL:({},q,$)=>{const g=n+"Score";q[g]>0?(q[g]--,q.log.push({text:"🚫 Dernier but annulé !",type:"info"})):q.log.push({text:"🚫 Aucun but à annuler",type:"info"}),$(q)},ADD_GOAL_DRAW:({},q,$)=>{q[a+"Score"]===q[n+"Score"]?(q[a+"Score"]++,q.log.push({text:"🎯 But bonus !",type:"info"})):q.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),$(q)},ADD_SUB:({value:q=1},$,g)=>{$.maxSubs=($.maxSubs||3)+q,$.log.push({text:`🔄 +${q} remplacement(s)`,type:"info"}),g($)},CUSTOM:({},q,$)=>$(q)};function ie(q,$,g,y){const b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let v=[];function w(){var z,E;const k=q.map(L=>{const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[L._line]||"#555",M=Se(L,L._line)+(L.boost||0),Y=v.find(U=>U.cardId===L.cardId)?"#FFD700":"rgba(255,255,255,0.25)",J=L.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${L.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${Y};background:${S};overflow:hidden;cursor:pointer;${J}">
        ...
        </div>`,(z=b.querySelector("#pp-close"))==null||z.addEventListener("click",()=>b.remove()),b.querySelectorAll(".pp-item").forEach(L=>{L.addEventListener("click",()=>{const S=L.dataset.cid,M=q.find(Y=>Y.cardId===S),F=v.findIndex(Y=>Y.cardId===S);M&&(F>-1?v.splice(F,1):v.length<$&&v.push(M),w())})}),(E=b.querySelector("#pp-confirm"))==null||E.addEventListener("click",()=>{b.remove(),y(v)})}w(),document.body.appendChild(b)}async function Q(q,$){const y=(p["gcCardsFull_"+a]||[]).find(k=>k.id===q),b=(y==null?void 0:y._gcDef)||(p.gcDefs||[]).find(k=>{var z;return k.name===$||((z=k.name)==null?void 0:z.toLowerCase().trim())===($==null?void 0:$.toLowerCase().trim())}),v=[...p["usedGc_"+a]||[],q],w={type:$,by:a,seq:(p._gcAnimSeq||0)+1};P.add(w.seq),Ee($,a,async()=>{if(b!=null&&b.effect_type&&b.effect_type!=="CUSTOM"){const z=oe[b.effect_type];if(z){const E={...p};z(b.effect_params||{},E,async L=>{L["usedGc_"+a]=v,L._lastGC=w,L._gcAnimSeq=w.seq,await V(L)});return}}const k={...p};switch($){case"Remplacement+":k.maxSubs=(k.maxSubs||3)+1,k.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const z=n+"Score";k[z]>0&&(k[z]--,k.log.push({text:"🚫 But annulé",type:"info"}));break}}k["usedGc_"+a]=v,k._lastGC=w,k._gcAnimSeq=w.seq,await V(k)})}function R(q,$){const g="usedCardIds_"+q,y=new Set(p[g]||[]);$.forEach(b=>y.add(b)),p[g]=[...y]}function D(){for(const q of["p1","p2"]){const $=new Set(p["usedCardIds_"+q]||[]),g=p[q+"Team"];if(g)for(const y of["GK","DEF","MIL","ATT"])(g[y]||[]).forEach(b=>{b.used=$.has(b.cardId)})}}function re(){var Ge,We,nt,pt,ut,ft;if(p.phase==="reveal")return me();if(p.phase==="midfield")return ve();if(p.phase==="finished")return ze();const q=p[a+"Team"],$=p[n+"Team"];D();const g=p[a+"Score"],y=p[n+"Score"],b=p[a+"Name"],v=p[n+"Name"],w=p.phase===a+"-attack",k=p.phase===a+"-defense",z=Array.isArray(p["selected_"+a])?p["selected_"+a]:[],E=z.map(ce=>ce.cardId),L=window.innerWidth>=700,S=p[a+"Subs"]||[],M=p["usedSubIds_"+a]||[],F=S.filter(ce=>!M.includes(ce.cardId)),Y=p["gcCardsFull_"+a]||[],J=p["usedGc_"+a]||[],U=Y.filter(ce=>!J.includes(ce.id)),G=p.boostOwner===a&&!p.boostUsed,W=!["GK","DEF","MIL","ATT"].some(ce=>($[ce]||[]).some(X=>!X.used)),te=[...q.MIL||[],...q.ATT||[]].filter(ce=>!ce.used),Z=w&&W&&te.length===0?[...q.GK||[],...q.DEF||[]].filter(ce=>!ce.used).map(ce=>ce.cardId):[];function fe(ce,X,H){var mt,kt;const ae=ce._gcDef,he={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ae==null?void 0:ae.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",$e={purple:"#b06ce0",light_blue:"#00d4ef"}[ae==null?void 0:ae.color]||"#b06ce0",Qe=(ae==null?void 0:ae.name)||ce.gc_type,Ze=(ae==null?void 0:ae.effect)||((mt=Oe[ce.gc_type])==null?void 0:mt.desc)||"",tt=ae!=null&&ae.image_url?`/icons/${ae.image_url}`:null,rt=((kt=Oe[ce.gc_type])==null?void 0:kt.icon)||"⚡",_t=Math.round(H*.22),$t=Math.round(H*.22),dt=H-_t-$t,gt=Qe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${ce.id}" data-gc-type="${ce.gc_type}"
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ct(q,p[a+"Formation"],Ae,E,300,300,Z)}
      </div>
        ...
        </div>`}return`<div style="font-size:11px;color:${ce.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${ce.type==="goal"?700:400};padding:3px 2px">${ce.text||""}</div>`}const Ye=(()=>{var X,H;if(k&&((X=p.pendingAttack)!=null&&X.players)){const ae=p.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
        ...
        </div>`}if(w&&((H=p.pendingAttack)!=null&&H.players)){const ae=p.pendingAttack;return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #00ff88">
          <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
          ${it((ae.players||[]).map(he=>({...he,used:!1})),"#00ff88",ae.total)}
        </div>`}const ce=(p.log||[]).slice(-1)[0];return ce?'<div style="padding:2px 4px">'+Ue(ce)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})(),Ke=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
        <button id="pvp-quit" style="width:34px;height:34px;border-radius:50%;background:rgba(220,50,50,0.7);border:none;color:#fff;font-size:16px;cursor:pointer;flex-shrink:0">✕</button>
        <div style="flex:1;display:flex;align-items:center;justify-content:center;gap:8px">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(v);let z=!1;const E=()=>{z||(z=!0,v.remove(),setTimeout(()=>b(),50))};v.addEventListener("click",E),setTimeout(E,3500)}function Ee(q,$,g){var J,U;const y=(p.gcDefs||[]).find(G=>{var W;return G.name===q||((W=G.name)==null?void 0:W.toLowerCase().trim())===(q==null?void 0:q.toLowerCase().trim())}),b={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[y==null?void 0:y.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",v={purple:"#b06ce0",light_blue:"#00d4ef"}[y==null?void 0:y.color]||"#b06ce0",w=(y==null?void 0:y.name)||q,k=(y==null?void 0:y.effect)||((J=Oe[q])==null?void 0:J.desc)||"",z=y!=null&&y.image_url?`/icons/${y.image_url}`:null,E=((U=Oe[q])==null?void 0:U.icon)||"⚡",S=$===a?"Vous":p[$+"Name"]||"Adversaire",M=document.createElement("div");M.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",M.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
      ...
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(fe=S.querySelector("#psub-close"))==null||fe.addEventListener("click",()=>S.remove()),(ye=S.querySelector("#pout-up"))==null||ye.addEventListener("click",()=>{k>0&&(k--,M())}),(be=S.querySelector("#pout-down"))==null||be.addEventListener("click",()=>{k<v.length-1&&(k++,M())}),(ke=S.querySelector("#pin-up"))==null||ke.addEventListener("click",()=>{E>0&&(E--,M())}),(Be=S.querySelector("#pin-down"))==null||Be.addEventListener("click",()=>{E<w.length-1&&(E++,M())});const Z=(Me,je,Ae,Ne)=>{const Ue=S.querySelector("#"+Me);if(!Ue)return;let Ye=0;Ue.addEventListener("touchstart",Ke=>{Ye=Ke.touches[0].clientY},{passive:!0}),Ue.addEventListener("touchend",Ke=>{const Re=Ke.changedTouches[0].clientY-Ye;if(Math.abs(Re)<30)return;const Ge=je();Re<0&&Ge<Ne-1?(Ae(Ge+1),M()):Re>0&&Ge>0&&(Ae(Ge-1),M())},{passive:!0})};Z("pin-panel",()=>E,Me=>E=Me,w.length),Z("pout-panel",()=>k,Me=>k=Me,v.length),(Fe=S.querySelector("#psub-confirm"))==null||Fe.addEventListener("click",async Me=>{if(Me.preventDefault(),Me.stopPropagation(),L)return;L=!0;const je=v[k],Ae=w[E];if(!je||!Ae)return;const Ne=je._line,Ue=(g[Ne]||[]).findIndex(Re=>Re.cardId===je.cardId);if(Ue===-1){u("Erreur : joueur introuvable","error"),S.remove();return}const Ye={...Ae,_line:Ne,position:je.position,used:!1,boost:0};g[Ne].splice(Ue,1,Ye);const Ke=[...y,Ae.cardId];S.remove(),O(je,Ae,async()=>{await V({[a+"Team"]:g,[n+"Team"]:p[n+"Team"],["usedSubIds_"+a]:Ke})})})}document.body.appendChild(S),M()}function O(q,$,g){const y={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const v=(z,E,L)=>`<div style="text-align:center">
      ...
      </div>`,document.body.appendChild($),(y=$.querySelector("#pvp-hist-close"))==null||y.addEventListener("click",()=>$.remove())}async function le(){if(p.phase!==a+"-attack")return;const q=a==="p1"?"p2":"p1",$=(p.round||0)+1,g=[...p.log||[]];g.push({type:"info",text:`⏭️ ${p[a+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const y=Te(p),b=Ce(q),v=y||!b?"finished":q+"-attack";await V({["selected_"+a]:[],modifiers:{...p.modifiers,[a]:{}},pendingAttack:null,phase:v,attacker:q,round:$,log:g}),v==="finished"&&await Pe(p)}async function ge(){const q=p[a+"Team"],$=!["GK","DEF","MIL","ATT"].some(v=>(p[n+"Team"][v]||[]).some(w=>!w.used)),g=(p["selected_"+a]||[]).map(v=>{const w=(q[v._role]||[]).find(M=>M.cardId===v.cardId)||v,k=$&&["GK","DEF"].includes(v._role),z=q[v._role]||[],E=z.findIndex(M=>M.cardId===v.cardId),L=st(z.length),S=E>=0?L[E]:w._col??1;return{...w,_line:v._role,_col:S,...k?{note_a:Math.max(1,Number(w.note_a)||0)}:{}}});if(!g.length)return;const y=Kt(g,p.modifiers[a]||{});R(a,g.map(v=>v.cardId)),g.forEach(v=>{const w=(q[v._role]||[]).find(k=>k.cardId===v.cardId);w&&(w.used=!0)}),p["selected_"+a]=[],re();const b=[...p.log||[]];if($){const v=(p[a+"Score"]||0)+1,w=g.map(S=>({name:S.name,note:Se(S,S._line||"ATT"),portrait:He(S),job:S.job}));b.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:w,homeTotal:y.total,aiTotal:0});const k=(p.round||0)+1,z=a==="p1"?"p2":"p1",E={...p,[a+"Team"]:q,[a+"Score"]:v},L=Te(E);C.add(k),we(w,v,p[n+"Score"]||0,!0,async()=>{await V({[a+"Team"]:q,[a+"Score"]:v,["selected_"+a]:[],modifiers:{...p.modifiers,[a]:{}},pendingAttack:null,phase:L?"finished":z+"-attack",attacker:z,round:k,log:b}),L&&await Pe({...p,[a+"Score"]:v})});return}b.push({type:"pending",text:`⚔️ ${p[a+"Name"]} attaque (${y.total})`}),await V({[a+"Team"]:q,[n+"Team"]:p[n+"Team"],pendingAttack:{...y,players:g,side:a},["selected_"+a]:[],modifiers:{...p.modifiers,[a]:{}},phase:n+"-defense",log:b})}async function ne(){const q=p[a+"Team"],$=(p["selected_"+a]||[]).map(J=>{const U=(q[J._role]||[]).find(fe=>fe.cardId===J.cardId)||J,G=q[J._role]||[],W=G.findIndex(fe=>fe.cardId===J.cardId),te=st(G.length),Z=W>=0?te[W]:U._col??1;return{...U,_line:J._role,_col:Z}}),g=Vt($,p.modifiers[a]||{});R(a,$.map(J=>J.cardId)),$.forEach(J=>{const U=(q[J._role]||[]).find(G=>G.cardId===J.cardId);U&&(U.used=!0)}),p["selected_"+a]=[],re();const y=Yt(p.pendingAttack.total,g.total,p.modifiers[a]||{}),b=p.pendingAttack.side,v=y==="attack"||(y==null?void 0:y.goal),w=b==="p1"?"p2":"p1",k=(p.round||0)+1,z=(p.pendingAttack.players||[]).map(J=>({name:J.name,note:Se(J,J._line||"ATT"),portrait:He(J),job:J.job})),E=[...p.log||[]];E.push({type:"duel",isGoal:v,homeScored:v&&b===a,text:v?`⚽ BUT de ${p[b+"Name"]} ! (${p.pendingAttack.total} vs ${g.total})`:`✋ Attaque stoppée (${p.pendingAttack.total} vs ${g.total})`,homePlayers:z,aiPlayers:$.map(J=>({name:J.name,note:Se(J,J._line||"DEF"),portrait:He(J),job:J.job})),homeTotal:p.pendingAttack.total,aiTotal:g.total});const L=v?(p[b+"Score"]||0)+1:p[b+"Score"]||0,S={...p,[a+"Team"]:q,[b+"Score"]:L},M=Te(S),F=M?"finished":w+"-attack",Y=async()=>{await V({[a+"Team"]:q,[n+"Team"]:p[n+"Team"],[b+"Score"]:L,["selected_"+a]:[],modifiers:{...p.modifiers,[a]:{}},pendingAttack:null,phase:F,attacker:w,round:k,log:E}),(F==="finished"||M)&&await Pe({...p,[b+"Score"]:L})};if(v){const J=b===a,U=J?L:p[a+"Score"]||0,G=J?p[n+"Score"]||0:L;C.add(k),we(z,U,G,J,Y)}else await Y()}function ue(q){return["MIL","ATT"].some($=>(q[$]||[]).some(g=>!g.used))}function Le(q){return["GK","DEF","MIL","ATT"].some($=>(q[$]||[]).some(g=>!g.used))}function Ie(q){return Le(q)&&!ue(q)}function Ce(q){const $=p[q+"Team"],g=p[(q==="p1"?"p2":"p1")+"Team"];return!!(ue($)||!Le(g)&&Ie($))}function Te(q){const $=["MIL","ATT"].some(E=>(q.p1Team[E]||[]).some(L=>!L.used)),g=["MIL","ATT"].some(E=>(q.p2Team[E]||[]).some(L=>!L.used)),y=Le(q.p1Team),b=Le(q.p2Team);return!$&&!(!b&&y)&&(!g&&!(!y&&b))}function qe(q){const $=q.p1Score||0,g=q.p2Score||0;return $===g?null:$>g?h.home_id:h.away_id}async function Pe(q){try{const $=qe(q),g=$?h.home_id===$?h.away_id:h.home_id:null;await T.from("matches").update({status:"finished",winner_id:$,home_score:q.p1Score||0,away_score:q.p2Score||0}).eq("id",i),$&&g&&zi($,g).catch(()=>{})}catch($){console.error("[PvP] finishMatch:",$)}}function ze(){var b;const q=p[a+"Score"],$=p[n+"Score"],g=q>$,y=q===$;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?Qo(f,p,d):i==="active"?Zo(h,d):er(_,d)}
    </div>
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        <div style="font-size:12px;color:#888;margin-bottom:10px">💰 ${p} cr./joueur → Pot estimé : ${(p*x.length).toLocaleString("fr")} cr. (🥇${Math.floor(p*x.length*.7).toLocaleString("fr")} · 🥈${Math.floor(p*x.length*.2).toLocaleString("fr")} · 🥉${Math.floor(p*x.length*.1).toLocaleString("fr")})</div>
        ${x.map(Q=>`
        ...
        ${n&&x.length>=2?`<button id="ml-start-btn" class="btn btn-primary" style="width:100%;margin-top:14px;padding:12px">🚀 Lancer (prélève ${p} cr. × ${x.length})</button>`:""}
    ...
    <div style="background:linear-gradient(160deg,#1a0a2e,#3b1a6e);border-radius:20px;padding:32px 24px;max-width:320px;width:100%;text-align:center;color:#fff;box-shadow:0 8px 40px rgba(0,0,0,0.6)">
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏆 Retour à la Mini League</button>`,document.body.appendChild(k),(M=k.querySelector("#pvp-end-home"))==null||M.addEventListener("click",()=>{k.remove(),Ve(e),a?l("mini-league",{openLeagueId:a}):l("mini-league")})}const V=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},g=>{const y=g.new;try{if(y.status==="finished"||y.forfeit){if(A||(A=!0,B&&(clearInterval(B),B=null),y.game_state&&(c=y.game_state),c.phase==="finished"&&!y.forfeit&&document.getElementById("pvp-home")))return;N(y);return}if(y.game_state){const b=c;c=y.game_state;const v=c._lastGC;if(v&&v.seq&&!j.has(v.seq)&&(j.add(v.seq),v.by!==n)){se(v.type,v.by,()=>me());return}const w=b[n+"Score"]||0,k=b[x+"Score"]||0,z=c[n+"Score"]||0,E=c[x+"Score"]||0,L=z>w,S=E>k;if((L||S)&&!P.has(c.round)){P.add(c.round);const M=[...c.log||[]].reverse().find(Y=>Y.isGoal),F=((M==null?void 0:M.homePlayers)||[]).map(Y=>({name:Y.name,note:Y.note,portrait:Y.portrait,job:Y.job}));Ee(F,z,E,L,()=>me());return}me()}}catch(b){console.error("[PvP] crash:",b)}}).subscribe();async function K(g){Object.assign(c,g),c.lastActionAt=new Date().toISOString();const{error:y}=await T.from("matches").update({game_state:c}).eq("id",i);y&&u("Erreur de synchronisation","error");try{me()}catch(b){console.error("[PvP] renderPvpScreen crash:",b)}}async function oe(){if(A)return;A=!0,B&&(clearInterval(B),B=null);const g=o?_.away_id:_.home_id,y=o?"p2":"p1",b=o?"p1":"p2",v={...c,[y+"Score"]:3,[b+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:g,home_score:v.p1Score||0,away_score:v.p2Score||0,game_state:v}).eq("id",i)}catch{}try{T.removeChannel(V)}catch{}setTimeout(()=>{Ve(e),l("mini-league")},800)}const ie={BOOST_STAT:({value:g=1,count:y=1,roles:b=[]},v,w)=>{const k=v[n+"Team"],z=Object.entries(k).filter(([E])=>!b.length||b.includes(E)).flatMap(([E,L])=>L.filter(S=>!S.used).map(S=>({...S,_line:E})));if(!z.length){v.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),w(v);return}Q(z,y,`Choisir ${y} joueur(s) à booster (+${g})`,E=>{E.forEach(L=>{const S=(k[L._line]||[]).find(M=>M.cardId===L.cardId);S&&(S.boost=(S.boost||0)+g,v.log.push({text:`⚡ +${g} sur ${S.name}`,type:"info"}))}),v[n+"Team"]=k,w(v)})},DEBUFF_STAT:({value:g=1,count:y=1,roles:b=[],target:v="ai"},w,k)=>{const z=v==="home"?n:x,E=w[z+"Team"],L=v==="home"?"allié":"adverse",S=Object.entries(E).filter(([M])=>!b.length||b.includes(M)).flatMap(([M,F])=>F.map(Y=>({...Y,_line:M})));if(!S.length){w.log.push({text:`🎯 Aucun joueur ${L}`,type:"info"}),k(w);return}Q(S,y,`Choisir ${y} joueur(s) ${L}(s) (-${g})`,M=>{M.forEach(F=>{const Y=(E[F._line]||[]).find(J=>J.cardId===F.cardId);Y&&(Y.boost=(Y.boost||0)-g,w.log.push({text:`🎯 -${g} sur ${Y.name}`,type:"info"}))}),w[z+"Team"]=E,k(w)})},GRAY_PLAYER:({count:g=1,roles:y=[],target:b="ai"},v,w)=>{const k=b==="home"?n:x,z=v[k+"Team"],E=b==="home"?"allié":"adverse",L=Object.entries(z).filter(([S])=>!y.length||y.includes(S)).flatMap(([S,M])=>M.filter(F=>!F.used).map(F=>({...F,_line:S})));if(!L.length){v.log.push({text:`❌ Aucun joueur ${E}`,type:"info"}),w(v);return}Q(L,g,`Choisir ${g} joueur(s) ${E}(s) à exclure`,S=>{const M="usedCardIds_"+k,F=new Set(v[M]||[]);S.forEach(Y=>{const J=(z[Y._line]||[]).find(U=>U.cardId===Y.cardId);J&&(J.used=!0,F.add(Y.cardId),v.log.push({text:`❌ ${J.name} exclu !`,type:"info"}))}),v[M]=[...F],v[k+"Team"]=z,w(v)})},REVIVE_PLAYER:({count:g=1,roles:y=[]},b,v)=>{const w=b[n+"Team"],k=Object.entries(w).filter(([z])=>!y.length||y.includes(z)).flatMap(([z,E])=>E.filter(L=>L.used).map(L=>({...L,_line:z})));if(!k.length){b.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),v(b);return}Q(k,g,`Choisir ${g} joueur(s) à ressusciter`,z=>{z.forEach(E=>{const L=(w[E._line]||[]).find(S=>S.cardId===E.cardId);L&&(L.used=!1,b.log.push({text:`💫 ${L.name} ressuscité !`,type:"info"}))}),b[n+"Team"]=w,v(b)})},REMOVE_GOAL:({},g,y)=>{const b=x+"Score";g[b]>0?(g[b]--,g.log.push({text:"🚫 Dernier but annulé !",type:"info"})):g.log.push({text:"🚫 Aucun but à annuler",type:"info"}),y(g)},ADD_GOAL_DRAW:({},g,y)=>{g[n+"Score"]===g[x+"Score"]?(g[n+"Score"]++,g.log.push({text:"🎯 But bonus !",type:"info"})):g.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),y(g)},ADD_SUB:({value:g=1},y,b)=>{y.maxSubs=(y.maxSubs||3)+g,y.log.push({text:`🔄 +${g} remplacement(s)`,type:"info"}),b(y)},CUSTOM:({},g,y)=>y(g)};function Q(g,y,b,v){const w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let k=[];function z(){var L,S;const E=g.map(M=>{const F={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[M._line]||"#555",Y=Se(M,M._line)+(M.boost||0),U=k.find(W=>W.cardId===M.cardId)?"#FFD700":"rgba(255,255,255,0.25)",G=M.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${M.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${U};background:${F};overflow:hidden;cursor:pointer;${G}">
        ...
        </div>`,(L=w.querySelector("#pp-close"))==null||L.addEventListener("click",()=>w.remove()),w.querySelectorAll(".pp-item").forEach(M=>{M.addEventListener("click",()=>{const F=M.dataset.cid,Y=g.find(U=>U.cardId===F),J=k.findIndex(U=>U.cardId===F);Y&&(J>-1?k.splice(J,1):k.length<y&&k.push(Y),z())})}),(S=w.querySelector("#pp-confirm"))==null||S.addEventListener("click",()=>{w.remove(),v(k)})}z(),document.body.appendChild(w)}async function R(g,y){const v=(c["gcCardsFull_"+n]||[]).find(E=>E.id===g),w=(v==null?void 0:v._gcDef)||(c.gcDefs||[]).find(E=>{var L;return E.name===y||((L=E.name)==null?void 0:L.toLowerCase().trim())===(y==null?void 0:y.toLowerCase().trim())}),k=[...c["usedGc_"+n]||[],g],z={type:y,by:n,seq:(c._gcAnimSeq||0)+1};j.add(z.seq),se(y,n,async()=>{if(w!=null&&w.effect_type&&w.effect_type!=="CUSTOM"){const L=ie[w.effect_type];if(L){const S={...c};L(w.effect_params||{},S,async M=>{M["usedGc_"+n]=k,M._lastGC=z,M._gcAnimSeq=z.seq,await K(M)});return}}const E={...c};switch(y){case"Remplacement+":E.maxSubs=(E.maxSubs||3)+1,E.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const L=x+"Score";E[L]>0&&(E[L]--,E.log.push({text:"🚫 But annulé",type:"info"}));break}}E["usedGc_"+n]=k,E._lastGC=z,E._gcAnimSeq=z.seq,await K(E)})}function D(g,y){const b="usedCardIds_"+g,v=new Set(c[b]||[]);y.forEach(w=>v.add(w)),c[b]=[...v]}function re(){for(const g of["p1","p2"]){const y=new Set(c["usedCardIds_"+g]||[]),b=c[g+"Team"];if(b)for(const v of["GK","DEF","MIL","ATT"])(b[v]||[]).forEach(w=>{w.used=y.has(w.cardId)})}}function me(){var nt,pt,ut,ft,ce,X;if(c.phase==="reveal")return _e();if(c.phase==="midfield")return we();if(c.phase==="finished")return q();const g=c[n+"Team"],y=c[x+"Team"];re();const b=c[n+"Score"],v=c[x+"Score"],w=c[n+"Name"],k=c[x+"Name"],z=c.phase===n+"-attack",E=c.phase===n+"-defense",L=Array.isArray(c["selected_"+n])?c["selected_"+n]:[],S=L.map(H=>H.cardId),M=window.innerWidth>=700,F=c[n+"Subs"]||[],Y=c["usedSubIds_"+n]||[],J=F.filter(H=>!Y.includes(H.cardId)),U=c["gcCardsFull_"+n]||[],G=c["usedGc_"+n]||[],W=U.filter(H=>!G.includes(H.id)),te=c.boostOwner===n&&!c.boostUsed,Z=!["GK","DEF","MIL","ATT"].some(H=>(y[H]||[]).some(ae=>!ae.used)),fe=[...g.MIL||[],...g.ATT||[]].filter(H=>!H.used),ye=z&&Z&&fe.length===0?[...g.GK||[],...g.DEF||[]].filter(H=>!H.used).map(H=>H.cardId):[];function be(H,ae,he){var Ct,Li;const $e=H._gcDef,Qe={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[$e==null?void 0:$e.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Ze={purple:"#b06ce0",light_blue:"#00d4ef"}[$e==null?void 0:$e.color]||"#b06ce0",tt=($e==null?void 0:$e.name)||H.gc_type,rt=($e==null?void 0:$e.effect)||((Ct=Oe[H.gc_type])==null?void 0:Ct.desc)||"",_t=$e!=null&&$e.image_url?`/icons/${$e.image_url}`:null,$t=((Li=Oe[H.gc_type])==null?void 0:Li.icon)||"⚡",dt=Math.round(he*.22),gt=Math.round(he*.22),mt=he-dt-gt,kt=tt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${H.id}" data-gc-type="${H.gc_type}"
      ...
      </div>`;function We(){const H=e.querySelector(".match-screen");if(!H)return;const ae=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);H.style.height=ae+"px",H.style.minHeight=ae+"px",H.style.maxHeight=ae+"px",H.style.overflow="hidden";const he=e.querySelector("#mobile-action-bar"),$e=e.querySelector("#mobile-play-area");he&&$e&&($e.style.paddingBottom=he.offsetHeight+"px")}if(We(),setTimeout(We,120),setTimeout(We,400),C||(C=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",We),window.visualViewport.addEventListener("scroll",We)),window.addEventListener("resize",We)),function(){const ae=e.querySelector(".terrain-wrapper svg");ae&&(ae.removeAttribute("width"),ae.removeAttribute("height"),ae.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ae.setAttribute("viewBox","-26 -26 352 352"),ae.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(H=>{H.addEventListener("click",()=>{if(!z&&!E)return;const ae=H.dataset.cardId,he=H.dataset.role,$e=(g[he]||[]).find(rt=>rt.cardId===ae);if(!$e||$e.used)return;const Qe=ye.includes(ae);if(z&&!["MIL","ATT"].includes(he)&&!Qe)return;Array.isArray(c["selected_"+n])||(c["selected_"+n]=[]);const Ze=c["selected_"+n],tt=Ze.findIndex(rt=>rt.cardId===ae);tt>-1?Ze.splice(tt,1):Ze.length<3&&Ze.push({...$e,_role:he}),me()})}),e.querySelectorAll(".match-used-hit").forEach(H=>{H.addEventListener("click",()=>O(H.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(H=>{H.addEventListener("click",()=>O())}),(nt=e.querySelector("#pvp-sub-open"))==null||nt.addEventListener("click",()=>O()),e.querySelectorAll(".pvp-gc-mini").forEach(H=>{H.addEventListener("click",()=>pe(H.dataset.gcId,H.dataset.gcType))}),(pt=e.querySelector("#pvp-boost-card"))==null||pt.addEventListener("click",()=>de()),(ut=e.querySelector("#pvp-action"))==null||ut.addEventListener("click",H=>{z?H.currentTarget.dataset.pass==="1"||!Te(n)?ge():ne():E&&ue()}),(ft=e.querySelector("#pvp-quit"))==null||ft.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&oe()}),(ce=e.querySelector("#pvp-view-opp"))==null||ce.addEventListener("click",()=>xe()),(X=e.querySelector("#pvp-toggle-history"))==null||X.addEventListener("click",()=>le()),B&&(clearInterval(B),B=null),(z||E)&&!A){let H=30,ae=!1;const he=()=>document.getElementById("pvp-timer"),$e=()=>{he()&&(he().textContent=H+"s",he().style.color=ae?"#ff4444":"#fff")};$e(),B=setInterval(()=>{H--,H<0?ae?(clearInterval(B),B=null,z&&!Te(n)?ge():oe()):(ae=!0,H=15,$e()):$e()},1e3)}}function _e(){et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      </div>`,document.body.appendChild(b),(v=b.querySelector("#bp-close"))==null||v.addEventListener("click",()=>b.remove()),b.querySelectorAll(".bp-item").forEach(w=>{w.addEventListener("click",async()=>{const k=w.dataset.cid,z=y.find(L=>L.cardId===k);if(!z)return;const E=(g[z._line]||[]).find(L=>L.cardId===k);E&&(E.boost=(E.boost||0)+c.boostValue),b.remove(),await K({[n+"Team"]:g,boostUsed:!0})})})}function O(g=null){var J,U;if(!(c.phase===n+"-attack")){u("Remplacement uniquement avant votre attaque","warning");return}const b=c[n+"Team"],v=c["usedSubIds_"+n]||[],w=c.maxSubs||3;if(v.length>=w){u(`Maximum ${w} remplacements atteint`,"warning");return}const k=Object.entries(b).flatMap(([G,W])=>(W||[]).filter(te=>te.used).map(te=>({...te,_line:G}))),z=(c[n+"Subs"]||[]).filter(G=>!v.includes(G.cardId));if(!k.length){u("Aucun joueur utilisé à remplacer","warning");return}if(!z.length){u("Aucun remplaçant disponible","warning");return}let E=Math.max(0,k.findIndex(G=>G.cardId===g));const L=((J=k[E])==null?void 0:J._line)||((U=k[E])==null?void 0:U.job);let S=Math.max(0,z.findIndex(G=>G.job===L)),M=!1;const F=document.createElement("div");F.id="pvp-sub-overlay",F.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function Y(){var be,ke,Be,Fe,Me,je;const G=k[E],W=z[S],te=Math.min(130,Math.round((window.innerWidth-90)/2)),Z=Math.round(te*1.35),fe=Ae=>`background:rgba(255,255,255,0.12);border:none;color:${Ae?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Ae?"default":"pointer"};flex-shrink:0`;F.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
    ...
    `,d.querySelectorAll("ul,ol").forEach(h=>{h.style.paddingLeft="20px",h.style.marginTop="6px",h.style.marginBottom="6px"}),d.querySelectorAll("li").forEach(h=>{h.style.marginBottom="4px"}),d.querySelectorAll("p").forEach(h=>{h.style.marginBottom="8px"}),(n=d.querySelector("#tuto-prev"))==null||n.addEventListener("click",()=>{o--,s()}),(x=d.querySelector("#tuto-next"))==null||x.addEventListener("click",()=>{u?r():(o++,s())}),(f=d.querySelector("#tuto-skip"))==null||f.addEventListener("click",()=>{confirm("Passer le tutoriel ? Tu pourras le revoir plus tard depuis les paramètres.")&&r()})},r=async()=>{d.remove(),e!=null&&e.id&&await T.from("users").update({tutorial_done:!0}).eq("id",e.id),i==null||i()};document.body.appendChild(d),s()}async function gr(e,t,i){if(!e||e.tutorial_done)return;let o=[];const{data:d,error:s}=await T.rpc("get_tutorial_steps");if(!s&&(d==null?void 0:d.length)>0)o=d,console.log(`[Tutorial] RPC OK → ${o.length} étapes`);else{const{data:l,error:u}=await T.from("tutorial_steps").select("*").eq("is_active",!0).order("step_order");!u&&(l==null?void 0:l.length)>0?(o=l,console.log(`[Tutorial] Direct OK → ${o.length} étapes`)):(console.warn(`[Tutorial] Aucune étape DB (RPC: ${s==null?void 0:s.message}, Direct: ${u==null?void 0:u.message})`),i&&i("[Tutorial] DB vide ou inaccessible — tuto local utilisé","warning",5e3))}const r=o.length>0?o.map(l=>({emoji:l.emoji,title:l.title,color:l.color,content:l.content,image_url:l.image_url||null})):ur;fr(e,r,()=>t("boosters"))}const Qt={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function St(e,t,i=0){return t?(Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0)+(t===e.job||t===e.job2?i:0):0}async function mr(e,t){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',await Ai(e,t)}async function Ai(e,t){const{state:i,toast:o}=t,{data:d}=await T.from("market_listings").select(`id, price, status, listed_at, seller_id,
      ...
      card:cards(id, card_type, current_note, evolution_bonus,
        player:players(id, firstname, surname_encoded, country_code, job, job2,
          ...
          clubs(encoded_name, logo_url)))`).eq("seller_id",i.profile.id).in("status",["active","sold"]).order("listed_at",{ascending:!1}).limit(100),r=(d||[]).filter(p=>p.seller_id!==i.profile.id),l=s||[];e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <!-- Header -->
    ...
    <div id="mkt-content" style="padding:10px 16px;display:flex;flex-direction:column;gap:8px"></div>
  </div>`;let u="buy";const a=()=>{var p,c,A,m,I,B,C;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((c=document.getElementById("flt-club"))==null?void 0:c.value)||"").toLowerCase().trim(),country:(((A=document.getElementById("flt-country"))==null?void 0:A.value)||"").toLowerCase().trim(),job:((m=document.getElementById("flt-job"))==null?void 0:m.value)||"",rarity:((I=document.getElementById("flt-rarity"))==null?void 0:I.value)||"",note1:parseInt((B=document.getElementById("flt-note1"))==null?void 0:B.value)||0,note2:parseInt((C=document.getElementById("flt-note2"))==null?void 0:C.value)||0}};function n(p){const c=a();return p.filter(A=>{var V,K,oe;const m=(V=A.card)==null?void 0:V.player;if(!m)return!1;const I=`${m.firstname} ${m.surname_encoded}`.toLowerCase(),B=(((K=m.clubs)==null?void 0:K.encoded_name)||"").toLowerCase(),C=(m.country_code||"").toLowerCase(),P=((oe=A.card)==null?void 0:oe.evolution_bonus)||0,j=St(m,m.job,P),N=m.job2?St(m,m.job2,P):0;return!(c.name&&!I.includes(c.name)||c.club&&!B.includes(c.club)||c.country&&!C.includes(c.country)||c.job&&m.job!==c.job||c.rarity&&m.rarity!==c.rarity||c.note1&&j<c.note1||c.note2&&N<c.note2)})}function x(p){var N,V,K,oe;const c=(N=p.card)==null?void 0:N.player;if(!c)return"";const A=((V=p.card)==null?void 0:V.evolution_bonus)||0,m=St(c,c.job,A),I=c.job2?St(c,c.job2,A):0,B=(i.profile.credits||0)>=p.price,C=c.country_code?`https://flagsapi.com/${c.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,P=Qt[c.job]||"#bbb",j=c.job2?Qt[c.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
      ...
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${P};display:flex;align-items:center;justify-content:center">
          <span style="font-size:14px;font-weight:900;color:${P};font-family:Arial Black,Arial">${m}</span>
        </div>
      ...
      <div style="text-align:right;flex-shrink:0">
        ...
        <button class="btn btn-primary btn-sm" data-buy="${p.id}" ${B?"":"disabled"} style="margin-top:4px;font-size:11px;padding:4px 10px">${B?"Acheter":"Trop cher"}</button>
      </div>
    </div>`}function f(p){var N,V,K,oe;const c=(N=p.card)==null?void 0:N.player;if(!c)return"";const A=((V=p.card)==null?void 0:V.evolution_bonus)||0,m=St(c,c.job,A),I=c.job2?St(c,c.job2,A):0,B=p.status==="sold",C=c.country_code?`https://flagsapi.com/${c.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,P=Qt[c.job]||"#bbb",j=c.job2?Qt[c.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${B?"opacity:0.7":""}">
      ${C?`<img src="${C}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(K=c.clubs)!=null&&K.logo_url?`<img src="${c.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${P};display:flex;align-items:center;justify-content:center">
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const A=r||[];_.innerHTML=A.length>0?A.map((m,I)=>{const B=Nt(m.mmr??1e3),C=(m.ranked_wins||0)+(m.ranked_losses||0)+(m.ranked_draws||0),P=C>0?Math.round((m.ranked_wins||0)/C*100):0,j=m.id===i.profile.id,N=(m.placement_matches||0)<10;return`
          <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${j?"border:2px solid var(--yellow)":""}">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:11px;color:var(--gray-600)">${m.club_name} · ${B.label}</div>
            </div>
            <div style="text-align:right;flex-shrink:0">
  ...
  <div style="min-height:100%;background:${m[_.id]};padding:16px;overflow-y:auto;display:flex;flex-direction:column;gap:16px">
    ...
    <div style="background:rgba(0,0,0,0.3);border-radius:12px;padding:12px 16px">
      ...
      <div style="background:rgba(255,255,255,0.1);border-radius:6px;height:10px;overflow:hidden">
        <div style="height:100%;width:${p}%;background:linear-gradient(90deg,${_.color},${_.color}aa);border-radius:6px;transition:width 0.8s ease"></div>
      </div>
      <div style="text-align:center;font-size:11px;color:rgba(255,255,255,0.4);margin-top:4px">${p}% vers ${A?A.label:"Maître"}</div>
    </div>`:""}
    ...
    <div style="display:flex;flex-direction:column;gap:10px;margin-top:auto;padding-top:8px">
      ...
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${d.id},away_id.eq.${d.id}`).order("created_at",{ascending:!1}).limit(50),r={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},l=(s||[]).filter(n=>n.status==="finished"),u=(s||[]).filter(n=>n.status==="in_progress");function a(n){const x=n.home_id===d.id;x?n.home_score:n.away_score,x?n.away_score:n.home_score;const f=n.winner_id===d.id,h=n.home_score===n.away_score&&n.status==="finished",_=n.status!=="finished"?"…":h?"N":f?"V":"D",p=n.status!=="finished"||h?"#888":f?"#1A6B3C":"#c0392b";let c=r[n.mode]||n.mode;n.away_id===null&&!c.startsWith("IA")&&(c="IA");const m=n.home_id===d.id?n.away:n.home;let I;n.away_id===null?I=c:m?I=`${m.club_name||m.pseudo} (${m.pseudo})`:I="Adversaire";let B="";n.status==="in_progress"&&Date.now()-new Date(n.created_at).getTime()>3600*1e3&&(B=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const C=new Date(n.created_at),P=C.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+C.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),j=n.status==="finished"?`${n.home_score} - ${n.away_score}`:`${n.home_score||0} - ${n.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
      ...
      <div style="display:flex;align-items:center;gap:8px">
        ...
        <span style="background:${p};color:#fff;width:22px;height:22px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:900">${_}</span>
      </div>
    </div>`}e.innerHTML=`
  <div class="page">
    <div class="page-header">
