import{s as w,l as J,F as we,b as ge,c as Pe,d as lt}from"./formation-links-CDBKB_z4.js";function ze(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  `,e.querySelectorAll("[data-nav]").forEach(r=>{r.addEventListener("click",l=>{l.preventDefault(),i(r.dataset.nav)})}),(d=document.getElementById("nav-rankings"))==null||d.addEventListener("click",()=>i("rankings")),(s=document.getElementById("nav-matches"))==null||s.addEventListener("click",()=>i("matches")),e.querySelectorAll("[data-action]").forEach(r=>{r.addEventListener("click",()=>{if(r.classList.add("tapped"),setTimeout(()=>r.classList.remove("tapped"),200),r.dataset.action==="match-ai"){ft(i);return}n("Bientôt disponible","info")})}),document.getElementById("logout-btn").addEventListener("click",async()=>{await w.auth.signOut(),window.location.reload()})}function ft(e){const t=[{mode:"vs_ai_easy",label:"Facile",sub:"Gain 500 cr.",icon:"🟢"},{mode:"vs_ai_medium",label:"Moyen",sub:"Gain 1 000 cr.",icon:"🟡"},{mode:"vs_ai_hard",label:"Difficile",sub:"Gain 1 500 cr.",icon:"🟠"},{mode:"vs_ai_club",label:"Club",sub:"Gain 2 500 cr.",icon:"🔴"}],i=document.createElement("div");i.className="modal-overlay",i.style.zIndex="2000",i.innerHTML=`
  ...
  </div>`}function mt(e){const t=e.job||"ATT",i=oe(e,t),n=$e[e.country_code]||e.country_code||"";return`
  <div style="
    width:140px;border-radius:12px;padding:6px;
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
  ...
  </div>`}async function xt(e,t){const{state:i,navigate:n,toast:a,openModal:o,closeModal:d}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await w.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),l=(s||[]).filter(I=>I.card_type==="player"&&I.player),c=(s||[]).filter(I=>I.card_type==="game_changer"),p=(s||[]).filter(I=>I.card_type==="formation"),f=Object.keys(we),u=Object.keys(ie),v={};l.forEach(I=>{const L=I.player.id;v[L]=(v[L]||0)+1});const _=new Set(Object.keys(v).map(I=>String(I))),x=new Set(p.map(I=>I.formation)),b=new Set(c.map(I=>I.gc_type));let g="player",y="Tous",h="",k=!1;function m(){return[...l].sort((I,L)=>{const M=re.indexOf(I.player.job),B=re.indexOf(L.player.job);return M!==B?M-B:(I.player.surname_encoded||"").localeCompare(L.player.surname_encoded||"")})}function $(){return[...r||[]].sort((I,L)=>{const M=re.indexOf(I.job),B=re.indexOf(L.job);return M!==B?M-B:(I.surname_encoded||"").localeCompare(L.surname_encoded||"")})}function E(){return m().filter(I=>{const L=I.player,M=y==="Tous"||L.job===y,B=!h||`${L.firstname} ${L.surname_encoded}`.toLowerCase().includes(h);return M&&B})}function T(){return $().filter(I=>{const L=y==="Tous"||I.job===y,M=!h||`${I.firstname} ${I.surname_encoded}`.toLowerCase().includes(h);return L&&M})}e.innerHTML=`
  <div class="page">
    ...
    <div class="page-body">
      <div style="display:flex;flex-wrap:wrap;gap:10px;justify-content:flex-start" id="col-grid"></div>
          ...
          ${gt.map(L=>`
            <button class="filter-btn" data-job="${L}"
              style="flex-shrink:0;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;cursor:pointer;
                border:1.5px solid ${L===y?"var(--green)":"var(--gray-200)"};
                background:${L===y?"var(--green)":"#fff"};
                color:${L===y?"#fff":"var(--gray-600)"}">
              ${L}
            </button>`).join("")}
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",L=>{h=L.target.value.toLowerCase(),z()}),e.querySelectorAll(".filter-btn").forEach(L=>{L.addEventListener("click",()=>{y=L.dataset.job,A(),z()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{k=!k,A(),z()})):(I.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{k=!k,A(),z()})))}function z(){const I=document.getElementById("col-grid");I&&(g==="player"?C(I):g==="formation"?G(I):q(I))}function C(I){if(k){const L=T();if(!L.length){I.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}I.innerHTML=L.map(M=>{if(_.has(String(M.id))){const S=l.find(H=>H.player.id===M.id),F=v[M.id]||1,O=F>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${F}</div>`:"",K=l.filter(H=>H.player.id===M.id&&H.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Be(S,O+K)}return mt(M)}).join("")}else{const L=E();if(!L.length){I.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const M={},B=[];L.forEach(S=>{const F=S.player.id;M[F]||(M[F]=!0,B.push(S))}),I.innerHTML=B.map(S=>{const F=v[S.player.id]||1,O=F>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${F}</div>`:"",K=l.filter(H=>H.player.id===S.player.id&&H.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Be(S,O+K)}).join("")}I.querySelectorAll("[data-card-id]").forEach(L=>{L.addEventListener("click",()=>{const M=l.find(B=>B.id===L.dataset.cardId);M&&bt(M,l,v,t)})})}function G(I){const L=k?f:[...x];if(!L.length){I.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}I.innerHTML=L.map(M=>{if(x.has(M)){const S=p.find(P=>P.formation===M),F=p.filter(P=>P.formation===M).length,O=F>1?`<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${F}</div>`:"";return`
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const ee=1e3;function ht(e,t,i,n){var _,x,b;const{state:a,toast:o,closeModal:d,navigate:s,refreshProfile:r}=i,l=e.formation,c={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function p(){const g=ge[l]||{},y=we[l]||[],h=290,k=360,m=20;function $(T){const A=g[T];return A?{x:A.x*h,y:A.y*k}:null}let E=`<svg width="${h}" height="${k}" viewBox="0 0 ${h} ${k}" xmlns="http://www.w3.org/2000/svg">`;for(const[T,A]of y){const z=$(T),C=$(A);!z||!C||(E+=`<line x1="${z.x}" y1="${z.y}" x2="${C.x}" y2="${C.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const T of Object.keys(g)){const A=$(T);if(!A)continue;const z=T.replace(/\d+/,""),C=c[z]||"#555";E+=`<circle cx="${A.x}" cy="${A.y}" r="${m}" fill="${C}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,E+=`<text x="${A.x}" y="${A.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${z}</text>`}return E+="</svg>",E}const f=t.filter(g=>g.formation===l),u=f.length,v=!e.is_for_sale;n(`Formation ${l}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(_=document.getElementById("direct-sell-form-btn"))==null||_.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${l} pour ${ee.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const g=f.find(h=>!h.is_for_sale)||f[0];if(!g){o("Aucune carte à vendre","error");return}const{error:y}=await w.from("cards").delete().eq("id",g.id);if(y){o(y.message,"error");return}await w.from("users").update({credits:(a.profile.credits||0)+ee}).eq("id",a.profile.id),await r(),o(`+${ee.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),d(),s("collection")}),(x=document.getElementById("market-sell-form-btn"))==null||x.addEventListener("click",async()=>{const g=parseInt(document.getElementById("sell-price-form").value);if(!g||g<1){o("Prix invalide","error");return}await w.from("cards").update({is_for_sale:!0,sale_price:g}).eq("id",e.id),await w.from("market_listings").insert({seller_id:a.profile.id,card_id:e.id,price:g}),o("Carte mise en vente sur le marché !","success"),d(),s("collection")}),(b=document.getElementById("cancel-sell-form-btn"))==null||b.addEventListener("click",async()=>{await w.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await w.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),d(),s("collection")})}function bt(e,t,i,n){var m,$,E,T,A,z;const{state:a,toast:o,openModal:d,closeModal:s,navigate:r,refreshProfile:l}=n,c=e.player,p=t.filter(C=>C.player.id===c.id),f=p.length,u=yt[c.rarity]||1e3,v=c.rarity!=="legende",_=Je(c),x=oe(c,c.job),b=c.job2?oe(c,c.job2):null,g=pe[c.job]||"#1A6B3C",y=c.job2?pe[c.job2]:null,h=We[c.rarity]||"#ccc",k=$e[c.country_code]||c.country_code||"";d(`${c.firstname} ${c.surname_encoded}`,`<div style="display:flex;gap:16px;flex-wrap:wrap;justify-content:center">
      ...
      <div style="width:140px;border-radius:12px;padding:6px;background:${h};flex-shrink:0">
        <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
          ...
          <div style="background:#f2e8d2;padding:3px 6px;display:flex;align-items:center;justify-content:space-between;min-height:24px">
            ...
            ${(m=c.clubs)!=null&&m.logo_url?`<img src="${c.clubs.logo_url}" style="width:22px;height:16px;object-fit:contain">`:`<div style="background:#1a3a7a;color:#fff;border-radius:2px;padding:1px 3px;font-size:6px;font-weight:800">${((($=c.clubs)==null?void 0:$.encoded_name)||"").slice(0,6)}</div>`}
          </div>
        </div>
      </div>
    ...
    </div>`:""}`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(E=document.getElementById("close-detail"))==null||E.addEventListener("click",s),(T=document.getElementById("direct-sell-btn"))==null||T.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte ${c.surname_encoded} pour ${u.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const C=p.find(q=>!q.is_for_sale)||p[0];if(!C){o("Aucune carte à vendre","error");return}const{error:G}=await w.from("cards").delete().eq("id",C.id);if(G){o(G.message,"error");return}await w.from("users").update({credits:(a.profile.credits||0)+u}).eq("id",a.profile.id),await l(),o(`+${u.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),s(),r("collection")}),(A=document.getElementById("market-sell-btn"))==null||A.addEventListener("click",async()=>{const C=parseInt(document.getElementById("sell-price").value);if(!C||C<1){o("Prix invalide","error");return}await w.from("cards").update({is_for_sale:!0,sale_price:C}).eq("id",e.id),await w.from("market_listings").insert({seller_id:a.profile.id,card_id:e.id,price:C}),o("Carte mise en vente sur le marché !","success"),s(),r("collection")}),(z=document.getElementById("cancel-sell-btn"))==null||z.addEventListener("click",async()=>{await w.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await w.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),s(),r("collection")})}const fe={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},V={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function _e(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}async function ye(e,t){const{state:i,navigate:n,toast:a}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await w.from("decks").select("id,name,formation_card_id").eq("owner_id",i.profile.id).order("created_at",{ascending:!1});e.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>Mes decks</h2>
    ...
    <div class="page-body">
      <div style="display:grid;gap:10px">
        ${(o==null?void 0:o.length)>0?o.map(d=>`
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const d=prompt("Nom du deck :",`Deck ${((o==null?void 0:o.length)||0)+1}`);if(!d)return;const{data:s,error:r}=await w.from("decks").insert({owner_id:i.profile.id,name:d}).select().single();if(r){a(r.message,"error");return}a("Deck créé !","success"),Se(s.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(d=>{d.addEventListener("click",()=>Se(d.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(d=>{d.addEventListener("click",async()=>{const s=prompt("Nouveau nom :",d.dataset.name);if(!s||s===d.dataset.name)return;const{error:r}=await w.from("decks").update({name:s}).eq("id",d.dataset.rename);if(r){a(r.message,"error");return}a("Deck renommé !","success"),ye(e,t)})}),e.querySelectorAll("[data-delete]").forEach(d=>{d.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${d.dataset.name}" ? Cette action est irréversible.`))return;await w.from("deck_cards").delete().eq("deck_id",d.dataset.delete);const{error:s}=await w.from("decks").delete().eq("id",d.dataset.delete);if(s){a(s.message,"error");return}a("Deck supprimé.","success"),ye(e,t)})})}async function Se(e,t,i){const{state:n,toast:a}=i;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await w.from("decks").select("*").eq("id",e).single(),{data:d}=await w.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",n.profile.id),s=(d||[]).filter(u=>u.card_type==="player"&&u.player),r=(d||[]).filter(u=>u.card_type==="formation"),l=r.map(u=>u.formation).filter(Boolean),{data:c}=await w.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let p=o.formation||"4-4-2";l.length>0&&!l.includes(p)&&(p=l[0]);const f={deckId:e,name:o.name,formation:p,formationCardId:o.formation_card_id,slots:{},subs:[],playerCards:s,formationCards:r,availableFormations:l};(c||[]).forEach(u=>{u.is_starter?f.slots[u.position]=u.card_id:f.subs.includes(u.card_id)||f.subs.push(u.card_id)}),X(t,f,i)}function X(e,t,i){var r;const{navigate:n}=i;fe[t.formation];const a=Fe(t.formation),o=a.filter(l=>t.slots[l]).length,d=t.availableFormations.length>0?t.availableFormations:Object.keys(fe),s=t.subs.map(l=>t.playerCards.find(c=>c.id===l)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div class="page">
    ...
    <div style="padding:10px 16px;background:#fff;border-bottom:1px solid var(--gray-200)">
      ...
      <select id="formation-select" style="width:100%;padding:7px;border-radius:6px;border:1.5px solid var(--gray-200)">
        ${d.map(l=>`<option value="${l}" ${l===t.formation?"selected":""}>${l}</option>`).join("")}
      </select>
    </div>
    ...
    <div style="padding:10px 12px;background:rgba(0,0,0,0.25);border-top:1px solid rgba(255,255,255,0.1)">
      ...
      <div style="display:flex;gap:8px;align-items:flex-end;overflow-x:auto;padding-bottom:4px" id="subs-list">
        ${s.map(l=>{const c=l.player;return`<div style="position:relative;flex-shrink:0">
  ...
  </div>`,wt(e,t,a,i),document.getElementById("builder-back").addEventListener("click",()=>n("decks")),document.getElementById("formation-select").addEventListener("change",l=>{t.formation=l.target.value;const c=Fe(t.formation),p={};c.forEach(f=>{t.slots[f]&&(p[f]=t.slots[f])}),t.slots=p,X(e,t,i)}),document.getElementById("save-deck").addEventListener("click",()=>kt(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(l=>{l.addEventListener("click",()=>{t.subs=t.subs.filter(c=>c!==l.dataset.removeSub),X(e,t,i)})}),(r=document.getElementById("add-sub-btn"))==null||r.addEventListener("click",()=>{_t(t,e,i)})}function wt(e,t,i,n){const a=e.querySelector("#deck-field");if(!a)return;const o=ge[t.formation]||{},d=Pe(t.formation)||[],s={};for(const b of i){const g=t.slots[b],y=g?t.playerCards.find(h=>h.id===g):null;s[b]=y?y.player:null}const r=300,l=300,c=48,p=64,f=13,u=16,v=38;function _(b){const g=o[b];return g?{x:g.x*r,y:g.y*l}:null}let x="";for(const[b,g]of d){const y=_(b),h=_(g);if(!y||!h)continue;const k=s[b]?{...s[b],club_id:s[b].club_id,country_code:s[b].country_code,rarity:s[b].rarity}:null,m=s[g]?{...s[g],club_id:s[g].club_id,country_code:s[g].country_code,rarity:s[g].rarity}:null,$=J(k,m);$==="#ff3333"||$==="#cc2222"?x+=`<line x1="${y.x.toFixed(1)}" y1="${y.y.toFixed(1)}" x2="${h.x.toFixed(1)}" y2="${h.y.toFixed(1)}" stroke="${$}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(x+=`<line x1="${y.x.toFixed(1)}" y1="${y.y.toFixed(1)}" x2="${h.x.toFixed(1)}" y2="${h.y.toFixed(1)}" stroke="${$}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,x+=`<line x1="${y.x.toFixed(1)}" y1="${y.y.toFixed(1)}" x2="${h.x.toFixed(1)}" y2="${h.y.toFixed(1)}" stroke="${$}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const b of i){const g=_(b);if(!g)continue;const y=s[b],h=b.replace(/\d+/,""),k=V[h]||"#555",m=(g.x-c/2).toFixed(1),$=(g.y-p/2).toFixed(1),E={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[y==null?void 0:y.rarity]||"#aaa";if(y){const T=_e(y),A=getClubLogo(y),z=flagImgUrl(y.country_code),C=Number(h==="GK"?y.note_g:h==="DEF"?y.note_d:h==="MIL"?y.note_m:y.note_a)||0,G=p-f-u;x+=`<defs><clipPath id="dcp-${b}"><rect x="${m}" y="${(g.y-p/2+f).toFixed(1)}" width="${c}" height="${G}"/></clipPath></defs>`,x+=`<rect x="${m}" y="${$}" width="${c}" height="${p}" rx="5" fill="${k}"/>`,T&&(x+=`<image href="${T}" x="${m}" y="${(g.y-p/2+f).toFixed(1)}" width="${c}" height="${G}" clip-path="url(#dcp-${b})" preserveAspectRatio="xMidYMin slice"/>`),x+=`<rect x="${m}" y="${$}" width="${c}" height="${f}" fill="rgba(255,255,255,0.93)"/>`,x+=`<text x="${g.x.toFixed(1)}" y="${(g.y-p/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(y.surname_encoded||"").slice(0,9)}</text>`;const q=(g.y+p/2-u).toFixed(1);x+=`<rect x="${m}" y="${q}" width="${c}" height="${u}" fill="rgba(255,255,255,0.93)"/>`,z&&(x+=`<image href="${z}" x="${(g.x-21).toFixed(1)}" y="${(g.y+p/2-u+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),x+=`<text x="${g.x.toFixed(1)}" y="${(g.y+p/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${C}</text>`,A&&(x+=`<image href="${A}" x="${(g.x+c/2-14).toFixed(1)}" y="${(g.y+p/2-u+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),x+=`<rect x="${m}" y="${$}" width="${c}" height="${p}" rx="5" fill="none" stroke="${E}" stroke-width="2"/>`}else x+=`<rect x="${m}" y="${$}" width="${c}" height="${p}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,x+=`<text x="${g.x.toFixed(1)}" y="${g.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,x+=`<text x="${g.x.toFixed(1)}" y="${(g.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${h}</text>`;x+=`<rect x="${m}" y="${$}" width="${c}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${b}" style="cursor:pointer"/>`}a.innerHTML=`<svg viewBox="${-v} ${-v} ${r+v*2} ${l+v*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${x}</svg>`,a.querySelectorAll(".deck-slot-hit").forEach(b=>{b.addEventListener("click",()=>$t(b.dataset.pos,t,e,n))})}function $t(e,t,i,n){var f,u,v;const{openModal:a,closeModal:o}=n,d=e.replace(/\d+/,""),s=t.slots[e],r=s?t.playerCards.find(_=>_.id===s):null;(f=r==null?void 0:r.player)==null||f.id;const l=new Set;Object.entries(t.slots).forEach(([_,x])=>{var g;if(_===e||!x)return;const b=t.playerCards.find(y=>y.id===x);(g=b==null?void 0:b.player)!=null&&g.id&&l.add(b.player.id)}),t.subs.forEach(_=>{var b;const x=t.playerCards.find(g=>g.id===_);(b=x==null?void 0:x.player)!=null&&b.id&&l.add(x.player.id)});const c=new Set,p=t.playerCards.filter(_=>{const x=_.player;return!(x.job===d||x.job2===d)||l.has(x.id)||c.has(x.id)?!1:(c.add(x.id),!0)});p.sort((_,x)=>{const b=d==="GK"?_.player.note_g:d==="DEF"?_.player.note_d:d==="MIL"?_.player.note_m:_.player.note_a;return(d==="GK"?x.player.note_g:d==="DEF"?x.player.note_d:d==="MIL"?x.player.note_m:x.player.note_a)-b}),a(`Choisir ${d} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${p.length>0?p.map(_=>{var h,k;const x=_.player,b=d==="GK"?x.note_g:d==="DEF"?x.note_d:d==="MIL"?x.note_m:x.note_a,g=_e(x),y={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[x.rarity];return`<div class="player-option" data-card-id="${_.id}"
          style="display:flex;align-items:center;gap:10px;padding:8px;border:1.5px solid var(--gray-200);border-radius:10px;cursor:pointer">
          <!-- Portrait -->
          <div style="width:44px;height:44px;border-radius:8px;overflow:hidden;flex-shrink:0;background:#dde;border:2px solid ${V[d]}">
            ${g?`<img src="${g}" style="width:100%;height:100%;object-fit:cover">`:`<div style="width:100%;height:100%;background:${V[d]};display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:700">${d}</div>`}
          </div>
          <!-- Infos -->
          ...
          <!-- Note -->
          <div style="width:36px;height:36px;border-radius:8px;background:${V[d]};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:16px;border:2px solid ${y};flex-shrink:0">
            ${b}
          </div>
        </div>`}).join(""):'<div style="text-align:center;color:var(--gray-600);padding:20px">Aucun joueur pour ce poste.<br><small>Ouvre des boosters !</small></div>'}
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(u=document.getElementById("close-selector"))==null||u.addEventListener("click",o),(v=document.getElementById("remove-player"))==null||v.addEventListener("click",()=>{delete t.slots[e],o(),X(i,t,n)}),document.querySelectorAll(".player-option").forEach(_=>{_.addEventListener("click",()=>{t.slots[e]=_.dataset.cardId,o(),X(i,t,n)})})}function _t(e,t,i){var r;const{openModal:n,closeModal:a}=i,o=new Set;Object.values(e.slots).filter(Boolean).forEach(l=>{var p;const c=e.playerCards.find(f=>f.id===l);(p=c==null?void 0:c.player)!=null&&p.id&&o.add(c.player.id)}),e.subs.forEach(l=>{var p;const c=e.playerCards.find(f=>f.id===l);(p=c==null?void 0:c.player)!=null&&p.id&&o.add(c.player.id)});const d=new Set,s=e.playerCards.filter(l=>{var c,p,f;return o.has((c=l.player)==null?void 0:c.id)||d.has((p=l.player)==null?void 0:p.id)?!1:(d.add((f=l.player)==null?void 0:f.id),!0)});n("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${s.length>0?s.map(l=>{var u;const c=l.player,p=_e(c),f=c.job==="GK"?c.note_g:c.job==="DEF"?c.note_d:c.job==="MIL"?c.note_m:c.note_a;return`<div class="player-option" data-card-id="${l.id}"
          style="display:flex;align-items:center;gap:10px;padding:8px;border:1.5px solid var(--gray-200);border-radius:10px;cursor:pointer">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-sub-selector">Fermer</button>'),(r=document.getElementById("close-sub-selector"))==null||r.addEventListener("click",a),document.querySelectorAll(".player-option").forEach(l=>{l.addEventListener("click",()=>{e.subs.push(l.dataset.cardId),a(),X(t,e,i)})})}async function kt(e,t){const{state:i,toast:n,navigate:a}=t,o=e.formationCards.find(r=>r.formation===e.formation),d=(o==null?void 0:o.id)||e.formationCardId;await w.from("decks").update({formation:e.formation,formation_card_id:d||null}).eq("id",e.deckId),await w.from("deck_cards").delete().eq("deck_id",e.deckId);const s=[];if(Object.entries(e.slots).forEach(([r,l],c)=>{s.push({deck_id:e.deckId,card_id:l,position:r,is_starter:!0,slot_order:c})}),e.subs.forEach((r,l)=>{s.push({deck_id:e.deckId,card_id:r,position:`SUB${l+1}`,is_starter:!1,slot_order:100+l})}),s.length>0){const{error:r}=await w.from("deck_cards").insert(s);if(r){n(r.message,"error");return}}n("Deck enregistré ✅","success"),a("decks")}function Fe(e){const t=fe[e]||fe["4-4-2"],i=["GK1"];for(let n=1;n<=t.DEF;n++)i.push(`DEF${n}`);for(let n=1;n<=t.MIL;n++)i.push(`MIL${n}`);for(let n=1;n<=t.ATT;n++)i.push(`ATT${n}`);return i}const Et="modulepreload",It=function(e){return"/manager-wars/"+e},De={},At=function(t,i,n){let a=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const d=document.querySelector("meta[property=csp-nonce]"),s=(d==null?void 0:d.nonce)||(d==null?void 0:d.getAttribute("nonce"));a=Promise.allSettled(i.map(r=>{if(r=It(r),r in De)return;De[r]=!0;const l=r.endsWith(".css"),c=l?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${r}"]${c}`))return;const p=document.createElement("link");if(p.rel=l?"stylesheet":Et,l||(p.as="script"),p.crossOrigin="",p.href=r,s&&p.setAttribute("nonce",s),document.head.appendChild(p),l)return new Promise((f,u)=>{p.addEventListener("load",f),p.addEventListener("error",()=>u(new Error(`Unable to preload CSS for ${r}`)))})}))}function o(d){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=d,window.dispatchEvent(s),!s.defaultPrevented)throw d}return a.then(d=>{for(const s of d||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})};async function Xe(){const{data:e}=await w.from("booster_configs").select("*").eq("is_active",!0).order("sort_order");if(!(e!=null&&e.length))return[];const{data:t}=await w.from("booster_drop_rates").select("*").in("booster_id",e.map(i=>i.id)).order("sort_order");return e.map(i=>({...i,rates:(t||[]).filter(n=>n.booster_id===i.id)}))}function Tt(e){if(!(e!=null&&e.length))return null;const t=e.reduce((n,a)=>n+Number(a.percentage),0);let i=Math.random()*t;for(const n of e)if(i-=Number(n.percentage),i<=0)return n;return e[e.length-1]}const Lt=Object.freeze(Object.defineProperty({__proto__:null,loadActiveBoosters:Xe,rollDropRate:Tt},Symbol.toStringTag,{value:"Module"})),Qe=()=>Object.keys(ge),Mt=[{id:"players_std",img:"/manager-wars/icons/booster-players.png",name:"Players",sub:"5 cartes joueurs",cost:5e3,costLabel:"5 000 crédits",cardCount:5,type:"player"},{id:"players_pub",img:"/manager-wars/icons/booster-silver.png",name:"Players (pub)",sub:"3 cartes joueurs",cost:0,costLabel:"1 pub",cardCount:3,type:"player"},{id:"game_changer",img:"/manager-wars/icons/booster-gamechanger.png",name:"Game Changer",sub:"3 cartes spéciales",cost:1e4,costLabel:"10 000 crédits",cardCount:3,type:"game_changer"},{id:"formation",img:"/manager-wars/icons/booster-formation.png",name:"Formation",sub:"1 carte formation",cost:1e4,costLabel:"10 000 crédits",cardCount:1,type:"formation"}],Ze={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé."},"Double attaque":{icon:"⚡",desc:"La prochaine attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la prochaine action IA."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function zt(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";if(!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}const Ct={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},jt={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};function Bt(e){var a;const t={};(e.rates||[]).forEach(o=>{t[o.card_type]=(t[o.card_type]||0)+Number(o.percentage||0)});const i=((a=Object.entries(t).sort((o,d)=>d[1]-o[1])[0])==null?void 0:a[0])||"player",n=e.image_url||"booster-players.png";return{id:e.id,img:"/manager-wars/icons/"+n,name:e.name,sub:`${e.card_count} carte(s)`,cost:e.price_type==="credits"&&e.price_credits||0,costLabel:e.price_type==="credits"?`${(e.price_credits||0).toLocaleString("fr")} crédits`:e.price_type==="pub"?"1 pub":"Gratuit",cardCount:e.card_count||5,type:i,isPub:e.price_type==="pub",rates:e.rates||[],_raw:e}}async function St(e,{state:t,navigate:i,toast:n}){var d;const a=((d=t.profile)==null?void 0:d.credits)||0;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⏳ Chargement...</div>';let o=[];try{o=(await Xe()).map(Bt)}catch(s){console.warn("Erreur chargement boosters DB, fallback hardcodé",s)}o.length||(o=Mt.map(s=>({...s,rates:[],isPub:s.id==="players_pub"}))),e.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📦 Boosters</h2>
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${o.map(s=>{const r=s.cost===0||a>=s.cost;return`<div class="booster-card ${r?"":"disabled"}" data-booster="${s.id}" style="position:relative">
  ...
  </div>`,e.querySelectorAll(".booster-card:not(.disabled)").forEach(s=>{s.addEventListener("click",async()=>{const r=o.find(l=>l.id===s.dataset.booster);if(r){s.style.opacity="0.5",s.style.pointerEvents="none";try{await Ft(r,{state:t,toast:n,navigate:i,container:e})}catch(l){n(l.message,"error"),s.style.opacity="",s.style.pointerEvents=""}}})}),e.querySelectorAll(".booster-info-btn").forEach(s=>{s.addEventListener("click",r=>{r.stopPropagation();const l=o.find(c=>c.id===s.dataset.boosterId);Kt(l)})})}async function Ft(e,{state:t,toast:i,navigate:n,container:a}){if(e.cost>0&&t.profile.credits<e.cost){i("Crédits insuffisants","error");return}if(e.isPub)try{await Vt()}catch(p){i(p.message||"Regardez la pub entièrement pour ouvrir le booster","error");return}const{data:o}=await w.from("cards").select("card_type, player_id, formation").eq("owner_id",t.profile.id),d=new Set((o||[]).filter(p=>p.card_type==="player").map(p=>p.player_id)),s=new Set((o||[]).filter(p=>p.card_type==="formation").map(p=>p.formation));let r=[];const l=e.type||"player";l==="player"?r=await Rt(t.profile,e.cardCount,e.cost):l==="game_changer"?r=await qt(t.profile,e.cardCount,e.cost):l==="formation"?r=await Ot(t.profile,e.cost):r=await Nt(t.profile,e),r.forEach(p=>{p.card_type==="player"&&p.player?p.isDuplicate=d.has(p.player.id):p.card_type==="formation"&&(p.isDuplicate=s.has(p.formation))});const{data:c}=await w.from("users").select("*").eq("id",t.profile.id).single();c&&(t.profile=c),Pt(r,e,n)}function Dt(){const e=Math.random()*100;return e<.5?"legende":e<2?"special":e<10?"normal_high":"normal_low"}function U(e){return Math.max(Number(e.note_g)||0,Number(e.note_d)||0,Number(e.note_m)||0,Number(e.note_a)||0)}function Gt(e,t){let i;switch(t){case"legende":i=e.filter(n=>n.rarity==="legende"),i.length||(i=e.filter(n=>n.rarity==="pepite"||n.rarity==="papyte")),i.length||(i=e.filter(n=>U(n)>=6));break;case"special":i=e.filter(n=>n.rarity==="pepite"||n.rarity==="papyte"),i.length||(i=e.filter(n=>U(n)>=6));break;case"normal_high":i=e.filter(n=>n.rarity==="normal"&&U(n)>=6),i.length||(i=e.filter(n=>U(n)>=6));break;default:i=e.filter(n=>n.rarity==="normal"&&U(n)>=1&&U(n)<=5),i.length||(i=e.filter(n=>n.rarity==="normal"));break}return i.length||(i=e),i[Math.floor(Math.random()*i.length)]}async function Nt(e,t){if(t.cost>0){const{error:a}=await w.from("users").update({credits:e.credits-t.cost}).eq("id",e.id);if(a)throw a}const{rollDropRate:i}=await At(async()=>{const{rollDropRate:a}=await Promise.resolve().then(()=>Lt);return{rollDropRate:a}},void 0),n=[];for(let a=0;a<(t.cardCount||5);a++){const o=i(t.rates);if(o){if(o.card_type==="player"){let d=w.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);o.rarity&&(d=d.eq("rarity",o.rarity));const{data:s}=await d;let r=s||[];if((o.note_min||o.note_max)&&(r=r.filter(p=>{const f=Math.max(Number(p.note_g)||0,Number(p.note_d)||0,Number(p.note_m)||0,Number(p.note_a)||0);return(!o.note_min||f>=o.note_min)&&(!o.note_max||f<=o.note_max)})),r.length||(r=s||[]),!r.length)continue;const l=r[Math.floor(Math.random()*r.length)],{data:c}=await w.from("cards").insert({owner_id:e.id,player_id:l.id,card_type:"player"}).select().single();c&&n.push({...c,player:l})}else if(o.card_type==="game_changer"){const d=["Ressusciter","Double attaque","Bouclier","Vol de note","Gel","Remplacement+"],s=d[Math.floor(Math.random()*d.length)],{data:r}=await w.from("cards").insert({owner_id:e.id,card_type:"game_changer",gc_type:s}).select().single();r&&n.push(r)}else if(o.card_type==="formation"){const d=Qe(),s=d[Math.floor(Math.random()*d.length)],{data:r}=await w.from("cards").insert({owner_id:e.id,card_type:"formation",formation:s}).select();r!=null&&r[0]&&n.push(r[0])}}}return n}async function Rt(e,t,i){if(i>0){const{error:l}=await w.from("users").update({credits:e.credits-i}).eq("id",e.id);if(l)throw l}const{data:n}=await w.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(n!=null&&n.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const a=n.filter(l=>l.job==="GK"),o=n.filter(l=>l.job!=="GK"),d=!e.first_booster_opened&&a.length>0,s=[];for(let l=0;l<t;l++){const c=l===0&&d?a:l===0?o:n,p=Dt(),f=Gt(c,p);f&&s.push(f)}d&&await w.from("users").update({first_booster_opened:!0}).eq("id",e.id);const{data:r}=await w.from("cards").insert(s.map(l=>({owner_id:e.id,player_id:l.id,card_type:"player"}))).select();return s.map((l,c)=>({...r[c],player:l}))}async function qt(e,t,i){const{error:n}=await w.from("users").update({credits:e.credits-i}).eq("id",e.id);if(n)throw n;const a=Object.keys(Ze),o=Array.from({length:t},()=>a[Math.floor(Math.random()*a.length)]),{data:d}=await w.from("cards").insert(o.map(s=>({owner_id:e.id,card_type:"game_changer",gc_type:s}))).select();return d}async function Ot(e,t){const{error:i}=await w.from("users").update({credits:e.credits-t}).eq("id",e.id);if(i)throw i;const n=Qe(),a=n[Math.floor(Math.random()*n.length)],{data:o}=await w.from("cards").insert({owner_id:e.id,card_type:"formation",formation:a}).select();return o}function Pt(e,t,i){const n=document.createElement("div");n.id="booster-anim-overlay",n.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(n);let a=!1;document.getElementById("pack-visual").addEventListener("click",()=>{if(a)return;a=!0;const f=document.getElementById("pack-visual");f.classList.add("shaking"),setTimeout(()=>{f.classList.add("pack-open"),setTimeout(()=>{document.getElementById("pack-phase").style.display="none",d(0)},600)},500)});let o=0;function d(f){o=f;const u=document.getElementById("reveal-phase");u.style.display="flex",s(f)}function s(f){var k;const u=e[f],v=document.getElementById("card-counter"),_=document.getElementById("single-card-slot"),x=document.getElementById("card-tap-hint");v&&(v.textContent=`Carte ${f+1} / ${e.length}`),x&&(x.textContent=f<e.length-1?"Appuie pour continuer →":"Appuie pour voir toutes tes cartes");const b=Ge(u),g=u.card_type==="player"&&((k=u.player)==null?void 0:k.rarity)==="legende";_.innerHTML=`
      ...
      </div>`,g&&c();const y=document.getElementById("current-card-wrap");let h=!1;y.addEventListener("click",()=>{if(h)return;h=!0;const m=f+1;m<e.length?(y.style.transition="all 0.25s ease",y.style.transform="translateX(-120%) rotate(-15deg)",y.style.opacity="0",setTimeout(()=>s(m),250)):r()})}function r(){p(),document.getElementById("reveal-phase").style.display="none";const f=document.getElementById("recap-phase");f.style.display="flex";const u=document.getElementById("recap-grid");u.innerHTML=e.map((v,_)=>`
      ...
      <div style="background:#fff;border-radius:16px;padding:20px;max-width:360px;width:100%;max-height:80vh;overflow-y:auto">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 12px;border-radius:8px;background:#f5f5f5;margin-bottom:6px">
          <div>
            <span style="font-weight:700;font-size:13px">${o[d.card_type]||d.card_type}</span>
            ${d.rarity?`<span style="margin-left:6px;padding:1px 6px;border-radius:8px;background:${a[d.rarity]||"#eee"};color:#fff;font-size:10px;font-weight:700">${d.rarity}</span>`:""}
      ...
      </div>`,document.body.appendChild(n),n.addEventListener("click",d=>{d.target===n&&n.remove()}),(i=document.getElementById("odds-close"))==null||i.addEventListener("click",()=>n.remove());return}Ht()}function Ht(){const e=document.createElement("div");e.style.cssText=`position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const Ut="simulation",et="VOTRE_ZONE_ID";function Vt(){switch(Ut){case"propellerads":return Yt();case"adsense":return Wt();default:return ke()}}function ke(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
  ...
  </div>`,(o=document.getElementById("msg-btn"))==null||o.addEventListener("click",a)}function N(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function me(e){var i,n;const t=e.player;return{cardId:e.id,id:t.id,firstname:t.firstname,name:t.surname_encoded,country_code:t.country_code,club_id:t.club_id,job:t.job,job2:t.job2,note_g:Number(t.note_g)||0,note_d:Number(t.note_d)||0,note_m:Number(t.note_m)||0,note_a:Number(t.note_a)||0,rarity:t.rarity,skin:t.skin,hair:t.hair,hair_length:t.hair_length,clubName:((i=t.clubs)==null?void 0:i.encoded_name)||null,clubLogo:((n=t.clubs)==null?void 0:n.logo_url)||null,boost:0,used:!1,_line:null,_col:null}}function Ee(e){return e===1?[1]:e===2?[0,2]:e===3?[0,1,2]:e===4?[0,1,1,2]:e===5?[0,1,1,1,2]:[1]}function Jt(){const e=Math.random()*100;return e<.1?4:e<5?3:e<20?2:1}function it(e,t){const i=Q[t]||Q["4-4-2"],n={GK:[],DEF:[],MIL:[],ATT:[]},a=[...e];for(const o of["GK","DEF","MIL","ATT"]){const d=[];for(let r=0;r<i[o];r++){let l=a.findIndex(c=>c.job===o);if(l===-1&&(l=a.findIndex(c=>c.job2===o)),l===-1&&(l=0),a[l]){const c={...a[l],_line:o};d.push(c),a.splice(l,1)}}const s=Ee(d.length);d.forEach((r,l)=>{r._col=s[l]}),n[o]=d}return n}async function Xt(e,t){const{data:i}=await w.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length").eq("is_active",!0).limit(60);if(!i||i.length<11)return Qt(e);const n=Q[e]||Q["4-4-2"],a={GK:[],DEF:[],MIL:[],ATT:[]},o=[...i];for(const d of["GK","DEF","MIL","ATT"]){const s=o.filter(f=>f.job===d),r=o.filter(f=>f.job!==d),l=[...s,...r],c=[];for(let f=0;f<n[d];f++){const u=l[f]||o[f];u&&c.push({cardId:"ai-"+u.id+"-"+f,id:u.id,firstname:u.firstname,name:u.surname_encoded,country_code:u.country_code,club_id:u.club_id,job:u.job,job2:u.job2,note_g:Number(u.note_g)||0,note_d:Number(u.note_d)||0,note_m:Number(u.note_m)||0,note_a:Number(u.note_a)||0,rarity:u.rarity,skin:u.skin,hair:u.hair,hair_length:u.hair_length,boost:0,used:!1,_line:d})}const p=Ee(c.length);c.forEach((f,u)=>{f._col=p[u]}),a[d]=c}return a}function Qt(e){const t=Q[e]||Q["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let a=0;for(const o of["GK","DEF","MIL","ATT"]){const d=[];for(let r=0;r<t[o];r++){const l=3+Math.floor(Math.random()*5);d.push({cardId:"fake-"+a,id:"fake-"+a,firstname:"IA",name:n[a%n.length],country_code:"XX",club_id:null,job:o,job2:null,note_g:o==="GK"?l:2,note_d:o==="DEF"?l:2,note_m:o==="MIL"?l:2,note_a:o==="ATT"?l:2,rarity:"normal",boost:0,used:!1,_line:o}),a++}const s=Ee(d.length);d.forEach((r,l)=>{r._col=s[l]}),i[o]=d}return i}async function Zt(e,t){var k;const{state:i,navigate:n,toast:a}=t,o=i.params||{};e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const d=o.matchMode||"vs_ai_easy",s=d.replace("vs_ai_",""),r=d;if(!o.deckId)return ei(e,t,d);const l=o.deckId,[{data:c},{data:p}]=await Promise.all([w.from("decks").select("formation,name").eq("id",l).single(),w.from("deck_cards").select(`position, is_starter, slot_order,
        card:cards(id, card_type, formation,
          player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
            note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
            clubs(encoded_name,logo_url)))`).eq("deck_id",l).order("slot_order")]),f=(p||[]).filter(m=>{var $;return m.is_starter&&(($=m.card)==null?void 0:$.player)}).map(m=>me(m.card)),u=(p||[]).filter(m=>{var $;return!m.is_starter&&(($=m.card)==null?void 0:$.player)}).map(m=>me(m.card));if(f.length<11){tt(e,"⚠️",`Deck incomplet (${f.length}/11).`,"Compléter",()=>n("decks"));return}const v=(p||[]).find(m=>{var $;return(($=m.card)==null?void 0:$.card_type)==="formation"}),_=(c==null?void 0:c.formation)||((k=v==null?void 0:v.card)==null?void 0:k.formation)||"4-4-2",{data:x}=await w.from("cards").select("id,gc_type").eq("owner_id",i.profile.id).eq("card_type","game_changer"),b=it(f,_),g=await Xt(_),{data:y}=await w.from("matches").insert({home_id:i.profile.id,away_id:null,mode:r,home_deck_id:l,status:"in_progress"}).select().single(),h={matchId:y==null?void 0:y.id,mode:r,difficulty:s,formation:_,homeTeam:b,aiTeam:g,homeSubs:u,subsUsed:0,maxSubs:Math.min(u.length,3),homeScore:0,aiScore:0,gcCards:x||[],usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};ti(e,h,t)}async function ei(e,t,i){const{state:n,navigate:a}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await w.from("decks").select("id,name,is_active,formation").eq("owner_id",n.profile.id).order("created_at",{ascending:!1});if(!o||o.length===0){tt(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>a("decks"));return}const d=o.map(c=>c.id),{data:s}=await w.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",d).order("slot_order");let r=0;function l(){var g,y,h,k,m;const c=o[r],p=(s||[]).filter($=>$.deck_id===c.id),f=p.filter($=>{var E;return $.is_starter&&((E=$.card)==null?void 0:E.player)}).map($=>me($.card)),u=p.find($=>{var E;return((E=$.card)==null?void 0:E.card_type)==="formation"}),v=c.formation||((g=u==null?void 0:u.card)==null?void 0:g.formation)||"4-4-2",_=f.length>=11?it(f,v):null,x=f.length>=11;e.innerHTML=`
    ...
    </div>`,(y=document.getElementById("prev-deck"))==null||y.addEventListener("click",()=>{r>0&&(r--,l())}),(h=document.getElementById("next-deck"))==null||h.addEventListener("click",()=>{r<o.length-1&&(r++,l())}),(k=document.getElementById("validate-deck"))==null||k.addEventListener("click",()=>{x&&t.navigate("match",{matchMode:i,deckId:c.id})}),(m=document.getElementById("cancel-deck-select"))==null||m.addEventListener("click",()=>a("home"));const b=document.getElementById("deck-swipe-zone");if(b){let $=0,E=0;b.addEventListener("touchstart",T=>{$=T.touches[0].clientX,E=T.touches[0].clientY},{passive:!0}),b.addEventListener("touchend",T=>{const A=T.changedTouches[0].clientX-$,z=T.changedTouches[0].clientY-E;Math.abs(A)<40||Math.abs(A)<Math.abs(z)||(A<0&&r<o.length-1?(r++,l()):A>0&&r>0&&(r--,l()))},{passive:!0})}}l()}function ti(e,t,i){e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;min-height:100vh;gap:12px;padding:12px 16px;background:#0a3d1e">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>ii(e,t,i),5e3)}function ii(e,t,i){const n=t.homeTeam.MIL||[],a=t.aiTeam.MIL||[];function o(p){return p.reduce((f,u)=>f+D(u,"MIL"),0)}function d(p){let f=0;for(let u=0;u<p.length-1;u++){const v=J(p[u],p[u+1]);v==="#00ff88"?f+=2:v==="#FFD700"&&(f+=1)}return f}const s=o(n)+d(n),r=o(a)+d(a),l=s>=r;function c(p,f,u){return`<div style="text-align:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${p.map((v,_)=>{const x=N(v),b=_<p.length-1?J(v,p[_+1]):null,g=b&&b!=="#ff3333"&&b!=="#cc2222";return`
          ...
          ${b?`<div style="width:14px;height:4px;border-radius:2px;background:${b};flex-shrink:0;opacity:${g?.9:.3}"></div>`:""}
          `}).join("")}
      </div>
      <div style="margin-top:6px;font-size:11px;color:rgba(255,255,255,0.5)">
        Score: ${o(p)} + ${d(p)} liens = <b style="color:#fff">${o(p)+d(p)}</b>
      </div>
    </div>`}e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;min-height:100vh;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${p}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${a?`<img src="${a}" style="width:${p-4}px;height:${p-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,p-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
  ...
  </div>`}function ae(e,t,i){if(!(e!=null&&e.length))return"";const n=e.slice(0,5);let a='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return n.forEach((o,d)=>{if(a+=Y(o,40,52),d<n.length-1){const s=J(o,n[d+1]);a+=`<div style="width:7px;height:3px;background:${s==="#ff3333"||s==="#cc2222"?"rgba(255,255,255,0.12)":s};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(a+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),a+="</div>",a}function xe(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function nt(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function Ie(e,t,i,n,a=310,o=310){const d=ge[t]||{},s=Pe(t)||we[t]||[],r={},l=["ATT","MIL","DEF","GK"];for(const g of l)(e[g]||[]).forEach((h,k)=>{r[`${g}${k+1}`]=h});function c(g){const y=d[g];return y?{x:y.x*a,y:y.y*o}:null}let p="";for(const[g,y]of s){const h=c(g),k=c(y);if(!h||!k)continue;const m=r[g],$=r[y],E=J(m,$);E==="#00ff88"||E==="#FFD700"?(p+=`<line x1="${h.x.toFixed(1)}" y1="${h.y.toFixed(1)}" x2="${k.x.toFixed(1)}" y2="${k.y.toFixed(1)}"
        ...
        stroke="${E}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const f=48,u=64,v=13,_=16,x={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[g,y]of Object.entries(r)){const h=c(g);if(!h||!y)continue;const k=g.replace(/[0-9]/g,""),m=Z[k]||"#555",$=i==="attack"&&["MIL","ATT"].includes(k)&&!y.used||i==="defense"&&["GK","DEF","MIL"].includes(k)&&!y.used,E=n.includes(y.cardId);let T;i==="attack"?T=k==="MIL"?Number(y.note_m)||0:Number(y.note_a)||0:i==="defense"?T=k==="GK"?Number(y.note_g)||0:k==="MIL"?Number(y.note_m)||0:Number(y.note_d)||0:T=Number(k==="GK"?y.note_g:k==="DEF"?y.note_d:k==="MIL"?y.note_m:y.note_a)||0,T=T+(y.boost||0);const A=(h.x-f/2).toFixed(1),z=(h.y-u/2).toFixed(1),C=y.used?.25:1,G=x[y==null?void 0:y.rarity]||x.normal,q=E?"#ff3030":G,I=E?3:(y==null?void 0:y.rarity)==="légende"||(y==null?void 0:y.rarity)==="pépite"?2.5:2,L=u-v-_;p+=`<defs><clipPath id="cp-${g}"><rect x="${A}" y="${(h.y-u/2+v).toFixed(1)}" width="${f}" height="${L}"/></clipPath></defs>`,p+=`<rect x="${A}" y="${z}" width="${f}" height="${u}" rx="5" fill="${m}" opacity="${C}"/>`;const M=N(y);M&&!y.used&&(p+=`<image href="${M}" x="${A}" y="${(h.y-u/2+v).toFixed(1)}" width="${f}" height="${L}" clip-path="url(#cp-${g})" preserveAspectRatio="xMidYMin slice"/>`),p+=`<rect x="${A}" y="${z}" width="${f}" height="${v}" rx="4" fill="rgba(255,255,255,0.92)"/>`,p+=`<text x="${h.x.toFixed(1)}" y="${(h.y-u/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(y.name||"").slice(0,9)}</text>`;const B=(h.y+u/2-_).toFixed(1);if(p+=`<rect x="${A}" y="${B}" width="${f}" height="${_}" fill="rgba(255,255,255,0.92)"/>`,y.used)p+=`<text x="${h.x.toFixed(1)}" y="${(h.y+u/2-_/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="13" font-weight="900" fill="rgba(0,0,0,0.4)" font-family="Arial Black">–</text>`;else{const S=xe(y.country_code);S?p+=`<image href="${S}" x="${(h.x-20).toFixed(1)}" y="${(h.y+u/2-_+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:p+=`<text x="${(h.x-13).toFixed(1)}" y="${(h.y+u/2-_/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${nt(y.country_code)}</text>`,p+=`<text x="${h.x.toFixed(1)}" y="${(h.y+u/2-_/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${T}</text>`;const F=ot(y);F?p+=`<image href="${F}" x="${(h.x+f/2-14).toFixed(1)}" y="${(h.y+u/2-_+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:y.clubName&&(p+=`<text x="${(h.x+14).toFixed(1)}" y="${(h.y+u/2-_/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(y.clubName||"").slice(0,3).toUpperCase()}</text>`),y.boost&&(p+=`<rect x="${(h.x+f/2-12).toFixed(1)}" y="${(h.y-u/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,p+=`<text x="${(h.x+f/2-5).toFixed(1)}" y="${(h.y-u/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${y.boost}</text>`)}p+=`<rect x="${A}" y="${z}" width="${f}" height="${u}" rx="5" fill="${E?"rgba(255,255,255,0.12)":"none"}" stroke="${q}" stroke-width="${I}" opacity="${C}"/>`,$&&(p+=`<rect x="${A}" y="${z}" width="${f}" height="${u}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${E?"selected":""}" data-card-id="${y.cardId}" data-role="${k}" style="cursor:pointer"/>`)}const b=38;return`<svg viewBox="${-b} ${-b} ${a+b*2} ${o+b*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${p}
  ...
  <div style="text-align:center;flex-shrink:0;width:38px">
    ...
    <div style="display:flex;align-items:center;gap:4px;${t?"flex-direction:row-reverse":""};background:rgba(255,255,255,0.04);border-radius:8px;padding:5px 8px;border:1px solid rgba(255,255,255,0.07)">
      ...
      ${se(e.inPlayer||{})}
    </div>`}return`<div style="font-size:11px;color:${e.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${e.type==="goal"?700:400};padding:3px 2px">${e.text||""}</div>`}function R(e,t,i){var v,_,x,b,g,y,h,k;const n=t.selected.map(m=>m.cardId),a=t.usedSubIds||[],o=t.homeSubs.filter(m=>!a.includes(m.cardId));Object.values(t.homeTeam).flat().filter(m=>m.used).length>0&&o.length>0&&t.subsUsed<t.maxSubs,t.log[t.log.length-1];const s=t.phase==="ai-attack"||t.phase==="ai-defense",r=t.phase==="attack",l=t.phase==="defense",c=t.phase==="finished",p=t.gcCards.filter(m=>!t.usedGc.includes(m.id)),f=t.boostCard&&!t.boostUsed;if(e.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;height:calc(100dvh - 130px);max-height:calc(100dvh - 130px);overflow:hidden;background:#0a3d1e;position:relative">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:100px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const $=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
          ...
          </div>`}const m=t.log[t.log.length-1];return m?'<div style="padding:2px 4px">'+oi(m)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    <div style="display:flex;flex-shrink:0;overflow:hidden">
      ...
      <div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:50px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
        ${o.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':o.map(m=>`
              <div class="sub-btn-col" data-sub-id="${m.cardId}" title="${m.firstname} ${m.name}" style="cursor:pointer;flex-shrink:0">
                ${Y(m,44,58)}
              </div>`).join("")}
      </div>
      ...
      <div style="flex:1;display:grid;grid-template-columns:repeat(4,1fr);gap:3px;align-content:start">
        ${p.map(m=>{var $;return`
          <div class="gc-mini" data-gc-id="${m.id}" data-gc-type="${m.gc_type}"
            ...
            <div style="font-size:16px">${(($=ie[m.gc_type])==null?void 0:$.icon)||"⚡"}</div>
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(m=>{var $,E,T;if(m.type==="duel"){const A=m.isGoal,z=m.homeScored?"#FFD700":A?"#ff6b6b":"rgba(255,255,255,0.3)",C=m.homeScored?"⚽ BUT !":A?"⚽ BUT IA !":($=m.homePlayers)!=null&&$.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${A?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${z};margin-bottom:4px">
                <div style="font-size:9px;color:${z};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${C}</div>
                ${(E=m.homePlayers)!=null&&E.length?`<div style="margin-bottom:3px">${ae(m.homePlayers,"rgba(255,255,255,0.7)",m.homeTotal)}</div>`:""}
                ${(T=m.aiPlayers)!=null&&T.length?`<div style="opacity:0.7">${ae(m.aiPlayers,"#ff6b6b",m.aiTotal)}</div>`:""}
                ...
                <div style="display:flex;align-items:center;gap:8px">
                  ${m.outPlayer?Y({...m.outPlayer,used:!0,_line:m.outPlayer.job,rarity:"normal"},38,50):""}
                  <span style="color:rgba(255,255,255,0.4);font-size:18px">→</span>
                  ${m.inPlayer?Y({...m.inPlayer,_line:m.inPlayer.job,rarity:"normal"},38,50):""}
                </div>
  ...
  </div>`,t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase==="attack"||t.phase==="defense"){let m=!1,$=30;const E=()=>document.getElementById("match-timer"),T=()=>{const A=E();if(!A)return;const z=String(Math.floor($/60)).padStart(2,"0"),C=String($%60).padStart(2,"0");A.textContent=` ${z}:${C}`,A.style.color=m?"#ff2222":"#ff9500",A.style.fontWeight="900"};T(),t._timerInt=setInterval(()=>{if($--,$<0)if(!m)m=!0,$=15,T();else{clearInterval(t._timerInt),t._timerInt=null,t.homeScore=0,t.aiScore=3;const A=document.createElement("div");A.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",A.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(A),setTimeout(()=>{A.remove(),W(e,t,i)},2500)}else T()},1e3)}(v=document.getElementById("match-quit"))==null||v.addEventListener("click",()=>{confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,W(e,t,i))}),(_=document.getElementById("view-ai"))==null||_.addEventListener("click",()=>fi(t,i)),(x=document.getElementById("toggle-history"))==null||x.addEventListener("click",()=>{var m;(m=document.getElementById("match-history-panel"))==null||m.classList.add("open")}),(b=document.getElementById("close-history"))==null||b.addEventListener("click",()=>{var m;(m=document.getElementById("match-history-panel"))==null||m.classList.remove("open")}),(g=document.getElementById("btn-action"))==null||g.addEventListener("click",()=>{t.selected.length!==0&&(r?ri(e,t,i):l&&ai(e,t,i))}),(y=document.getElementById("btn-results"))==null||y.addEventListener("click",()=>W(e,t,i)),e.querySelectorAll(".match-slot-hit").forEach(m=>{m.addEventListener("click",()=>ni(m,t,e,i))}),e.querySelectorAll(".gc-mini").forEach(m=>{m.addEventListener("click",()=>li(m.dataset.gcId,m.dataset.gcType,e,t,i))}),(h=document.getElementById("boost-card"))==null||h.addEventListener("click",()=>pi(e,t,i)),e.querySelectorAll(".sub-btn-col").forEach(m=>{m.addEventListener("click",()=>Ne(e,t,i,m.dataset.subId))}),(k=document.getElementById("sub-btn-main"))==null||k.addEventListener("click",()=>Ne(e,t,i))}function ni(e,t,i,n){const a=e.dataset.cardId,o=e.dataset.role,d=t.selected.findIndex(s=>s.cardId===a);if(d!==-1)t.selected.splice(d,1);else{if(t.selected.length>=3){n.toast("Maximum 3 joueurs","error");return}const s=(t.homeTeam[o]||[]).find(r=>r.cardId===a);s&&t.selected.push({...s,_role:o,_line:o})}R(i,t,n)}function Ae(e,t,i){e.matchId&&w.from("matches").update({last_player_id:i}).eq("id",e.matchId).then(()=>{})}function ri(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),Ae(t,i,i.state.profile.id);const n=t.selected.map(o=>({...o,_line:o._role})),a=He(n,t.modifiers.home);t.pendingAttack={...a,players:[...t.selected],side:"home"},t.selected.forEach(o=>{const d=(t.homeTeam[o._role]||[]).find(s=>s.cardId===o.cardId);d&&(d.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${a.total} (base ${a.base}${a.links?` +${a.links} liens`:""}) — ${t.selected.map(o=>o.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",R(e,t,i),setTimeout(()=>si(e,t,i),1200)}function ai(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),Ae(t,i,i.state.profile.id);const n=t.selected.map(s=>({...s,_line:s._role})),a=Ue(n,t.modifiers.home);t.selected.forEach(s=>{const r=(t.homeTeam[s._role]||[]).find(l=>l.cardId===s.cardId);r&&(r.used=!0)});const o=Ve(t.pendingAttack.total,a.total,t.modifiers.home),d={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(s=>({name:s.name,note:s._line==="MIL"?s.note_m:s.note_a,portrait:N(s),job:s.job,country_code:s.country_code,rarity:s.rarity,clubName:s.clubName,clubLogo:s.clubLogo})),homePlayers:t.selected.map(s=>{const r=(t.homeTeam[s._role]||[]).find(l=>l.cardId===s.cardId)||s;return{name:r.name,note:r._line==="GK"?r.note_g:r._line==="MIL"?r.note_m:r.note_d,portrait:N(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo}}),homeTotal:a.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(o.shielded)d.text="🛡️ Bouclier ! But annulé.",t.log.push(d);else if(o.goal){t.aiScore++,d.isGoal=!0,d.homeScored=!1,d.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${a.total})`,t.log.push(d),t.selected=[],t.modifiers.home={},t.pendingAttack=null,R(e,t,i),Te(d.aiPlayers,t.homeScore,t.aiScore,!1,()=>{ne(e,t,i,"home-attack")});return}else d.text=`🧤 Défense réussie ! (${a.total} ≥ ${t.pendingAttack.total})`,t.log.push(d);t.selected=[],t.modifiers.home={},t.pendingAttack=null,ne(e,t,i,"home-attack")}function ve(e,t,i){Ae(t,i,null);const n=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]],a=Ye(n,"attack",t.difficulty);if(!a.length){st(e,t,i);return}const o=He(a,t.modifiers.ai);t.pendingAttack={...o,players:a,side:"ai"},a.forEach(l=>{l.used=!0}),t.log.push({text:`🤖 IA attaque : ${o.total} (${a.map(l=>l.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const d=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(l=>!l.used),r=(t.homeSubs||[]).filter(l=>!(t.usedSubIds||[]).includes(l.cardId)).length>0&&t.subsUsed<t.maxSubs;if(d.length===0&&!r){t.aiScore++;const l={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:a.map(c=>({name:c.name,note:c._line==="MIL"?c.note_m:c.note_a,portrait:N(c),job:c.job,country_code:c.country_code,rarity:c.rarity,clubName:c.clubName,clubLogo:c.clubLogo})),aiTotal:o.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(l),t.pendingAttack=null,R(e,t,i),Te(l.aiPlayers,t.homeScore,t.aiScore,!1,()=>{ne(e,t,i,"home-attack")});return}t.phase="defense",R(e,t,i)}function si(e,t,i){const n=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],a=Ye(n,"defense",t.difficulty),o=a.length>0?Ue(a,t.modifiers.ai).total:0;a.forEach(r=>{r.used=!0});const d=Ve(t.pendingAttack.total,o,t.modifiers.ai),s={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(r=>({name:r.name,note:r._line==="MIL"?r.note_m:r.note_a,portrait:N(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo})),aiPlayers:a.map(r=>({name:r.name,note:r._line==="GK"?r.note_g:r._line==="MIL"?r.note_m:r.note_d,portrait:N(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo})),homeTotal:t.pendingAttack.total,aiTotal:o,isGoal:!1,homeScored:!1,text:""};if(d.shielded)s.text="🛡️ Bouclier IA !",t.log.push(s);else if(d.goal){t.homeScore++,s.isGoal=!0,s.homeScored=!0,s.text=`⚽ BUT ! (${t.pendingAttack.total} > ${o})`,t.log.push(s),t.modifiers.ai={},t.pendingAttack=null,R(e,t,i),Te(s.homePlayers,t.homeScore,t.aiScore,!0,()=>{ne(e,t,i,"ai-attack")});return}else s.text=`🧤 IA défend (${o} ≥ ${t.pendingAttack.total})`,t.log.push(s);t.modifiers.ai={},t.pendingAttack=null,ne(e,t,i,"ai-attack")}function ne(e,t,i,n){if(t.round++,at(t)){W(e,t,i);return}if(n==="home-attack"){if(![...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(o=>!o.used).length){if(![...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(d=>!d.used).length){W(e,t,i);return}t.phase="ai-attack",R(e,t,i),setTimeout(()=>ve(e,t,i),800);return}t.phase="attack",R(e,t,i)}else{if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(o=>!o.used).length){st(e,t,i);return}t.phase="ai-attack",R(e,t,i),setTimeout(()=>ve(e,t,i),800)}}function at(e){const t=["MIL","ATT","GK","DEF"].some(n=>(e.homeTeam[n]||[]).some(a=>!a.used)),i=["MIL","ATT","GK","DEF"].some(n=>(e.aiTeam[n]||[]).some(a=>!a.used));return!t&&!i}function st(e,t,i){at(t)?W(e,t,i):(t.phase="attack",R(e,t,i))}function di(e,t,i){const n=document.createElement("div");n.style.cssText=`
    position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:800;
    display:flex;flex-direction:column;align-items:center;justify-content:center;gap:20px;
    animation:subFadeIn 0.2s ease;
  `;const a=N(e),o=N(t),d=Z[e.job]||"#555",s=Z[t.job]||"#555",r=e.job==="GK"?e.note_g:e.job==="DEF"?e.note_d:e.job==="MIL"?e.note_m:e.note_a,l=t.job==="GK"?t.note_g:t.job==="DEF"?t.note_d:t.job==="MIL"?t.note_m:t.note_a;n.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(n);let c=!1;const p=()=>{c||(c=!0,n.remove(),i())};n.addEventListener("click",p),setTimeout(p,2e3)}function te(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function Ne(e,t,i,n=null){if(t.phase!=="attack"){te("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){te(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const a=Object.values(t.homeTeam).flat().filter(p=>p.used),o=t.homeSubs.filter(p=>!t.usedSubIds.includes(p.cardId));if(!a.length){te("Aucun joueur utilisé à remplacer");return}if(!o.length){te("Aucun remplaçant disponible");return}let d=0,s=Math.max(0,o.findIndex(p=>p.cardId===n)),r=!1;const l=document.createElement("div");l.id="sub-overlay",l.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function c(){var b,g,y,h,k,m;const p=a[d],f=o[s],u=Math.min(130,Math.round((window.innerWidth-90)/2)),v=Math.round(u*1.35),_=$=>`background:rgba(255,255,255,0.12);border:none;color:${$?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${$?"default":"pointer"};flex-shrink:0`;l.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <button id="in-down" style="${_(s>=o.length-1)}" ${s>=o.length-1?"disabled":""}>▼</button>
      ...
      <div id="out-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
        ...
        <button id="out-down" style="${_(d>=a.length-1)}" ${d>=a.length-1?"disabled":""}>▼</button>
        <div style="font-size:10px;color:rgba(255,255,255,0.35)">${d+1}/${a.length}</div>
      </div>
    </div>
    <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
      <button id="sub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
    </div>`,(b=l.querySelector("#sub-close"))==null||b.addEventListener("click",()=>l.remove()),(g=l.querySelector("#out-up"))==null||g.addEventListener("click",()=>{d>0&&(d--,c())}),(y=l.querySelector("#out-down"))==null||y.addEventListener("click",()=>{d<a.length-1&&(d++,c())}),(h=l.querySelector("#in-up"))==null||h.addEventListener("click",()=>{s>0&&(s--,c())}),(k=l.querySelector("#in-down"))==null||k.addEventListener("click",()=>{s<o.length-1&&(s++,c())});const x=($,E,T,A)=>{const z=l.querySelector("#"+$);if(!z)return;let C=0;z.addEventListener("touchstart",G=>{C=G.touches[0].clientY},{passive:!0}),z.addEventListener("touchend",G=>{const q=G.changedTouches[0].clientY-C;if(Math.abs(q)<30)return;const I=E();q<0&&I<A-1?(T(I+1),c()):q>0&&I>0&&(T(I-1),c())},{passive:!0})};x("in-panel",()=>s,$=>s=$,o.length),x("out-panel",()=>d,$=>d=$,a.length),(m=l.querySelector("#sub-confirm"))==null||m.addEventListener("click",$=>{if($.preventDefault(),$.stopPropagation(),r)return;r=!0;const E=a[d],T=o[s];if(!E||!T)return;let A=null,z=-1;for(const[G,q]of Object.entries(t.homeTeam)){const I=(q||[]).findIndex(L=>L.cardId===E.cardId);if(I!==-1){A=G,z=I;break}}if(z===-1||!A){te("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),l.remove();return}const C={...T,_line:A,_col:E._col||0,used:!1,boost:0};t.homeTeam[A].splice(z,1,C),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(T.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:E.name,firstname:E.firstname,note:D(E,A),portrait:N(E),job:E.job,country_code:E.country_code,rarity:E.rarity,clubName:E.clubName,clubLogo:E.clubLogo},inPlayer:{name:T.name,firstname:T.firstname,note:D(T,A),portrait:N(T),job:T.job,country_code:T.country_code,rarity:T.rarity,clubName:T.clubName,clubLogo:T.clubLogo},text:`🔄 ${T.firstname} ${T.name} remplace ${E.firstname} ${E.name}`}),l.remove(),di(E,T,()=>R(e,t,i))})}document.body.appendChild(l),c()}function li(e,t,i,n,a){var s,r;const o=ie[t]||{icon:"⚡",desc:"Carte spéciale."},d=document.createElement("div");d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;align-items:center;justify-content:center;padding:24px",d.innerHTML=`
    ...
    </div>`,document.body.appendChild(d),(s=d.querySelector("#gc-back"))==null||s.addEventListener("click",()=>d.remove()),(r=d.querySelector("#gc-use"))==null||r.addEventListener("click",()=>{d.remove(),ci(e,t,i,n,a)})}function ci(e,t,i,n,a){if(!n.usedGc.includes(e)){switch(n.usedGc.push(e),t){case"Double attaque":n.modifiers.home.doubleAttack=!0,n.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":n.modifiers.home.shield=!0,n.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{let o=!1;for(const d of["ATT","MIL","DEF","GK"]){const s=(n.homeTeam[d]||[]).find(r=>r.used);if(s){s.used=!1,o=!0;break}}n.log.push({text:o?"💫 Joueur ressuscité !":"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":n.modifiers.ai.stolenNote=(n.modifiers.ai.stolenNote||0)+1,n.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const o=[...n.aiTeam.ATT||[],...n.aiTeam.MIL||[]].filter(d=>!d.used);if(o.length){const d=o.sort((s,r)=>D(r,"ATT")-D(s,"ATT"))[0];d.used=!0,n.log.push({text:`❄️ ${d.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":n.maxSubs++,n.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}w.from("cards").delete().eq("id",e).then(()=>{}),R(i,n,a)}}function pi(e,t,i){const n=Object.values(t.homeTeam).flat().filter(a=>!a.used);if(!n.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(a=>{a.addEventListener("click",()=>{const o=a.dataset.cardId;for(const d of["GK","DEF","MIL","ATT"]){const s=(t.homeTeam[d]||[]).find(r=>r.cardId===o);if(s){s.boost=(s.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${s.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),R(e,t,i)})})}function Te(e,t,i,n,a){const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const d=Array.from({length:10},(l,c)=>`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(o);let s=!1;const r=()=>{s||(s=!0,o.remove(),setTimeout(()=>a(),50))};o.addEventListener("click",r),setTimeout(r,3500)}async function W(e,t,i){var c,p;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,a=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,d=a?"victoire":o?"nul":"defaite",s=ut(t.mode,d);t.matchId&&await w.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:a?n.profile.id:null,home_credits_reward:s,played_at:new Date().toISOString()}).eq("id",t.matchId);const r={credits:(n.profile.credits||0)+s,matches_played:(n.profile.matches_played||0)+1};a?r.wins=(n.profile.wins||0)+1:o?r.draws=(n.profile.draws||0)+1:r.losses=(n.profile.losses||0)+1,await w.from("users").update(r).eq("id",n.profile.id),await i.refreshProfile();const l=document.createElement("div");l.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",l.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      card:cards(id, card_type,
        player:players(id, firstname, surname_encoded, country_code, job, job2,
          note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length, clubs(encoded_name, logo_url)))`).eq("status","active").order("listed_at",{ascending:!1}).limit(60),o=(a||[]).filter(r=>r.seller_id===i.profile.id),d=(a||[]).filter(r=>r.seller_id!==i.profile.id);e.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `;function s(r){const l=document.getElementById("mkt-content"),c=r==="buy"?d:o;if(c.length===0){l.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${c.map(p=>{var x,b,g;const f=(x=p.card)==null?void 0:x.player;if(!f)return"";const u=f.job==="GK"?f.note_g:f.job==="DEF"?f.note_d:f.job==="MIL"?f.note_m:f.note_a,v=ui[f.rarity],_=(i.profile.credits||0)>=p.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="font-size:11px;color:var(--gray-600)">${f.country_code} · ${((b=f.clubs)==null?void 0:b.encoded_name)||"—"} · ${f.rarity} · ${f.job}</div>
    ...
    </div>`,l.querySelectorAll("[data-buy]").forEach(p=>{p.addEventListener("click",()=>yi(p.dataset.buy,c,e,t))}),l.querySelectorAll("[data-cancel]").forEach(p=>{p.addEventListener("click",()=>mi(p.dataset.cancel,e,t))})}s("buy"),e.querySelectorAll(".mkt-tab").forEach(r=>{r.addEventListener("click",()=>{e.querySelectorAll(".mkt-tab").forEach(l=>{const c=l===r;l.style.background=c?"var(--green)":"#fff",l.style.color=c?"#fff":"var(--gray-600)",l.style.borderColor=c?"var(--green)":"var(--gray-200)"}),s(r.dataset.tab)})})}async function yi(e,t,i,n){const{state:a,toast:o,refreshProfile:d}=n,s=t.find(r=>r.id===e);if(s){if((a.profile.credits||0)<s.price){o("Crédits insuffisants","error");return}if(confirm(`Acheter ${s.card.player.firstname} ${s.card.player.surname_encoded} pour ${s.price.toLocaleString("fr")} crédits ?`))try{const{error:r}=await w.from("cards").update({owner_id:a.profile.id,is_for_sale:!1,sale_price:null}).eq("id",s.card.id);if(r)throw r;await w.from("market_listings").update({status:"sold",buyer_id:a.profile.id,sold_at:new Date().toISOString()}).eq("id",e),await w.from("users").update({credits:(a.profile.credits||0)-s.price}).eq("id",a.profile.id);const{data:l}=await w.from("users").select("credits").eq("id",s.seller_id).single();l&&await w.from("users").update({credits:(l.credits||0)+s.price}).eq("id",s.seller_id),await w.from("notifications").insert({user_id:s.seller_id,type:"card_sold",message:`Ta carte ${s.card.player.surname_encoded} a été vendue pour ${s.price} crédits !`,data:{card_id:s.card.id,price:s.price}}),await d(),o("Carte achetée ! ✅","success"),Le(i,n)}catch(r){o("Erreur : "+r.message,"error")}}}async function mi(e,t,i){const{toast:n}=i,{data:a}=await w.from("market_listings").select("card_id").eq("id",e).single();await w.from("market_listings").update({status:"cancelled"}).eq("id",e),a&&await w.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",a.card_id),n("Annonce retirée","success"),Le(t,i)}function xi(e){return{GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[e]||"#888"}async function vi(e,{state:t,navigate:i,toast:n}){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:a}=await w.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100);e.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `}async function hi(e,{state:t,navigate:i,toast:n}){const a=t.profile;if(!a)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await w.from("matches").select("id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at").or(`home_id.eq.${a.id},away_id.eq.${a.id}`).order("created_at",{ascending:!1}).limit(50),d={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},s=(o||[]).filter(c=>c.status==="finished"),r=(o||[]).filter(c=>c.status==="in_progress");function l(c){const p=c.home_id===a.id;p?c.home_score:c.away_score,p?c.away_score:c.home_score;const f=c.winner_id===a.id,u=c.home_score===c.away_score&&c.status==="finished",v=c.status!=="finished"?"…":u?"N":f?"V":"D",_=c.status!=="finished"||u?"#888":f?"#1A6B3C":"#c0392b";let x=d[c.mode]||c.mode;c.away_id===null&&!x.startsWith("IA")&&(x="IA");let b="";c.status==="in_progress"&&Date.now()-new Date(c.created_at).getTime()>3600*1e3&&(b=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const g=new Date(c.created_at),y=g.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+g.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),h=c.status==="finished"?`${c.home_score} - ${c.away_score}`:`${c.home_score||0} - ${c.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
        <div style="font-size:13px;font-weight:600">${x}${b}</div>
        <div style="font-size:11px;color:var(--gray-600)">${y}${c.status==="in_progress"?" · en cours":""}</div>
      </div>
      <div style="display:flex;align-items:center;gap:8px">
