...
import{s as w,F as si,r as Ae,j as St,l as Pi,m as yt,h as di,n as gn,o as mn,T as Yt,k as xn,p as yn,b as bn}from"./formation-links-D0diUT_D.js";const hn="/";function Li(e,{navigate:t,toast:i}){let n="login";const a=()=>{var r,s,d,l,c,u;const o=n==="login";e.innerHTML=`
  ...
  `;function s(){var f;const l=document.getElementById("logo-preview"),c=document.getElementById("logo-initials"),u=((f=document.getElementById("setup-club"))==null?void 0:f.value)||"MW",g=u.trim().split(" ").filter(Boolean),x=g.length>=2?(g[0][0]+g[1][0]).toUpperCase():u.slice(0,2).toUpperCase();l&&(l.style.background=r,l.style.borderColor=o),c&&(c.textContent=x,c.style.color=o)}document.getElementById("color1").addEventListener("input",l=>{o=l.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",l=>{r=l.target.value,document.getElementById("swatch2").style.background=r,s()});function d(l){document.querySelectorAll(".setup-step").forEach(c=>c.classList.remove("active")),document.getElementById(`step-${l}`).classList.add("active"),document.getElementById("step-num").textContent=l,document.getElementById("progress-fill").style.width=`${Math.round(l/3*100)}%`,l===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("step1-error");if(c.textContent="",l.length<3){c.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await w.from("users").select("id").eq("pseudo",l).maybeSingle();if(u){c.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const l=document.getElementById("setup-club").value.trim(),c=document.getElementById("step2-error");if(c.textContent="",l.length<2){c.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await w.from("users").select("id").eq("club_name",l).maybeSingle();if(u){c.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),g=document.getElementById("step3-finish");u.textContent="",g.disabled=!0,g.textContent="Création en cours…";try{const{error:x}=await w.from("users").insert({id:t.user.id,pseudo:l,club_name:c,club_color1:o,club_color2:r,credits:1e4});if(x)throw x;await wn(t.user.id),await a(),n(`Bienvenue ${l} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(x){u.textContent=x.message,g.disabled=!1,g.textContent="🚀 Créer mon profil !"}})}async function wn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await w.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const _n="modulepreload",kn=function(e){return"/"+e},Ti={},Ri=function(t,i,n){let a=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),s=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));a=Promise.allSettled(i.map(d=>{if(d=kn(d),d in Ti)return;Ti[d]=!0;const l=d.endsWith(".css"),c=l?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${c}`))return;const u=document.createElement("link");if(u.rel=l?"stylesheet":_n,l||(u.as="script"),u.crossOrigin="",u.href=d,s&&u.setAttribute("nonce",s),document.head.appendChild(u),l)return new Promise((g,x)=>{u.addEventListener("load",g),u.addEventListener("error",()=>x(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(r){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=r,window.dispatchEvent(s),!s.defaultPrevented)throw r}return a.then(r=>{for(const s of r||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},Et="#1A6B3C",Lt="#cc2222",$n="#D4A017",zi="#888";async function En(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </style>`}const In="2026.07.12-1741";async function Ii(e,{state:t,navigate:i,toast:n}){var o,r;const a=t.profile;a&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="home-dark" id="home-dark">
    ...
    ${Ae(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),l=(s||[]).filter(T=>T.card_type==="player"&&T.player),c=(s||[]).filter(T=>T.card_type==="game_changer"),u=(s||[]).filter(T=>T.card_type==="formation"),g=(s||[]).filter(T=>T.card_type==="stadium"),{data:x}=await w.from("gc_definitions").select("name,gc_type,color,effect,image_url"),f={};(x||[]).forEach(T=>{f[T.name]=T});const{data:p}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),m={};(p||[]).forEach(T=>{m[T.id]=T}),g.forEach(T=>{T.stadium_def&&(m[T.stadium_id]=T.stadium_def)});const b=Object.keys(si),v=Object.keys(Se),h={};l.forEach(T=>{const S=T.player.id;h[S]=(h[S]||0)+1}),new Set(Object.keys(h).map(T=>String(T)));const $=new Set(u.map(T=>T.formation)),y=new Set(c.map(T=>T.gc_type));let C="player",E="Tous",G="",N=!1;function te(){return[...l].sort((T,S)=>{const R=jt.indexOf(T.player.job),H=jt.indexOf(S.player.job);return R!==H?R-H:(T.player.surname_real||"").localeCompare(S.player.surname_real||"")})}function ne(){return[...d||[]].sort((T,S)=>{const R=jt.indexOf(T.job),H=jt.indexOf(S.job);return R!==H?R-H:(T.surname_real||"").localeCompare(S.surname_real||"")})}function ee(){return te().filter(T=>{const S=T.player,R=E==="Tous"||S.job===E,H=!G||`${S.firstname} ${S.surname_real}`.toLowerCase().includes(G);return R&&H})}function j(){return ne().filter(T=>{const S=E==="Tous"||T.job===E,R=!G||`${T.firstname} ${T.surname_real}`.toLowerCase().includes(G);return S&&R})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",S=>{G=S.target.value.toLowerCase(),k()}),e.querySelectorAll(".filter-btn").forEach(S=>{S.addEventListener("click",()=>{E=S.dataset.job,I(),k()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{N=!N,I(),k()})):(T.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{N=!N,I(),k()})))}function k(){const T=document.getElementById("col-grid");T&&(C==="player"?_e(T):C==="formation"?U(T):C==="stadium"?Y(T):V(T))}function F(T,S,R,H,K){const B=document.getElementById("col-grid"),O=document.getElementById("col-big");if(!B||!O)return;var Ee=0;function ke(){const we=window.innerWidth>=768,me=document.getElementById("col-big"),Le=document.getElementById("col-grid");we&&me&&(me.style.minHeight="420px",me.style.flex="1 1 auto"),we&&Le&&(Le.style.height=Math.round(310*.76+16)+"px",Le.style.flexShrink="0",Le.style.overflowX="auto",Le.style.overflowY="hidden",Le.style.alignItems="center",Le.style.padding="8px 16px"),O.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+S(T[Ee])+"</div>";var $e=O.querySelector("[data-card-id],[data-form-id],[data-gc-id]");$e&&$e.addEventListener("click",function(){H(T[Ee])}),requestAnimationFrame(function(){var he=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!he||!O)){var Ce=O.clientHeight-8,je=O.clientWidth-24,Fe=he.offsetHeight,_=he.offsetWidth;if(Fe>0&&_>0&&Ce>40){var L=we?2.2:1.6,A=Math.min(Ce/Fe,je/_,L);he.style.transform="scale("+A.toFixed(3)+")",he.style.transformOrigin="top center"}}}),B.innerHTML=T.map(function(he,Ce){var je=Ce===Ee,Fe="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(je?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+Ce+'" style="'+Fe+'">'+R(he,je)+"</div>"}).join(""),B.querySelectorAll(".col-mini-item").forEach(function(he){he.addEventListener("click",function(){Ee=Number(he.dataset.idx),ke(),he.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}ke()}function de(T){var S=window.innerWidth>=768?.76:.54,R;if(!T||T._fake){var H=T?T.player:null;if(!H)return"";R=Mi(H)}else R=Jt(T,"");return'<div style="display:inline-block;zoom:'+S+';pointer-events:none;line-height:0">'+R+"</div>"}function pe(T,S,R){S=S||100,R=R||140;var H=St[T]||{},K={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},B=Math.max(3,Math.round(S*.06)),O=Object.entries(H).map(function(ke){var we=ke[0],me=ke[1],Le=we.replace(/\d+$/,""),$e=K[Le]||"#888",he=Math.round(me.x*S),Ce=Math.round(me.y*R);return'<circle cx="'+he+'" cy="'+Ce+'" r="'+B+'" fill="'+$e+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),Ee=Math.max(1,Math.round(S/50));return'<svg viewBox="0 0 '+S+" "+R+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+S+'" height="'+R+'" fill="#1A6B3C"/><rect x="'+Math.round(S*.2)+'" y="'+Math.round(R*.02)+'" width="'+Math.round(S*.6)+'" height="'+Math.round(R*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+Ee+'"/><line x1="0" y1="'+Math.round(R*.5)+'" x2="'+S+'" y2="'+Math.round(R*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+Ee+'"/><ellipse cx="'+Math.round(S*.5)+'" cy="'+Math.round(R*.5)+'" rx="'+Math.round(S*.18)+'" ry="'+Math.round(R*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+Ee+'"/><rect x="'+Math.round(S*.2)+'" y="'+Math.round(R*.82)+'" width="'+Math.round(S*.6)+'" height="'+Math.round(R*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+Ee+'"/>'+O+"</svg>"}function le(T,S,R){var H=R>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+R+"</div>":"",K=S?'data-form-id="'+S.id+'"':"",B=T.length>7?14:T.length>5?16:19,O=!!S;return"<div "+K+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(O?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(O?"":"filter:grayscale(1);opacity:0.5")+'">'+H+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(O?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+B+"px;font-weight:900;color:"+(O?"#1A6B3C":"#aaa")+';line-height:1">'+T+'</div></div><div style="flex:1;overflow:hidden;background:'+(O?"#1A6B3C":"#ccc")+'">'+pe(T,140,220)+"</div></div>"}function ae(T,S){var R=window.innerWidth>=768?.76:.54,H=140,K=305,B=Math.round(K*.22),O=K-B,Ee=T.length>7?10:13,ke=pe(T,H,O),we=S?"1.5px solid #2a7a40":"1px solid #ddd",me=S?"":"filter:grayscale(1);opacity:0.45;",Le=S?"#1A6B3C":"#bbb",$e="#fff";return'<div style="display:inline-block;zoom:'+R+';line-height:0;pointer-events:none"><div style="width:'+H+"px;height:"+K+"px;border-radius:6px;border:"+we+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+me+'"><div style="height:'+B+"px;background:"+Le+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+Ee+"px;font-weight:900;color:"+$e+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(H-4)+'px">'+T+'</span></div><div style="height:'+O+'px;overflow:hidden;flex-shrink:0">'+ke+"</div></div></div>"}function _e(T){if(N){const S=j();if(!S.length){T.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const R=S.map(H=>l.find(K=>K.player.id===H.id)||{_fake:!0,player:H,id:"fake-"+H.id});F(R,H=>H._fake?Mi(H.player):Jt(H,""),H=>H._fake?de({player:H.player,id:"x",_fake:!0}):de(H),H=>{H._fake||Ci(H,l,h,t)})}else{const S=ee();if(!S.length){T.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const R={};S.forEach(K=>{const B=K.player.id;R[B]||(R[B]=[]),R[B].push(K)});const H=Object.values(R).map(K=>Pn(K));F(H,K=>{const B=h[K.player.id]||1,O=B>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${B}</div>`:"",ke=l.filter(we=>we.player.id===K.player.id&&we.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Jt(K,O+ke)},K=>de(K),K=>Ci(K,l,h,t))}}function U(T){const S=N?b:[...$];if(!S.length){T.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const R=S.map(H=>({formation:H,card:u.find(K=>K.formation===H)||null,owned:$.has(H)}));F(R,({formation:H,card:K,owned:B})=>le(H,B?K:null,B?u.filter(O=>O.formation===H).length:0),({formation:H,owned:K})=>ae(H,K),({card:H,owned:K})=>{K&&H&&Kn(H,u,t,o)})}function V(T){const S=Object.keys(f),R=N?S.length?S:v:[...y];if(!R.length){T.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const H=R.map(K=>({type:K,gc:Se[K]||{icon:"⚡",desc:""},def:f[K]||null,owned:y.has(K),card:c.find(B=>B.gc_type===K)||null}));F(H,({type:K,gc:B,def:O,owned:Ee,card:ke})=>{var L;const we=Ee?c.filter(A=>A.gc_type===K).length:0,me=we>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${we}</div>`:"",Le=(O==null?void 0:O.gc_type)==="ultra_game_changer",$e={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},he={purple:"#b06ce0",light_blue:"#00d4ef"},Ce=$e[O==null?void 0:O.color]||$e.purple,je=he[O==null?void 0:O.color]||he.purple,Fe=(O==null?void 0:O.effect)||B.desc||"",_=O!=null&&O.image_url?`/icons/${O.image_url}`:((L=O==null?void 0:O.club)==null?void 0:L.logo_url)||(O!=null&&O.country_code?`https://flagsapi.com/${O.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);return Ee&&ke?`<div data-gc-id="${ke.id}" data-gc-type="${K}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${je};background:${Ce};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${je}66;cursor:pointer">
          ${me}
        ...
        </div>`},({def:B,count:O})=>{var $e,he;const Ee=window.innerWidth>=768?.76:.54,ke=(B==null?void 0:B.name)||"?",we=(($e=B==null?void 0:B.club)==null?void 0:$e.encoded_name)||(B==null?void 0:B.country_code)||"—",me=B!=null&&B.image_url?`${R}icons/${B.image_url}`:((he=B==null?void 0:B.club)==null?void 0:he.logo_url)||(B!=null&&B.country_code?`https://flagsapi.com/${B.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Le=me?`<img src="${me}" style="width:64px;height:64px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<span style="font-size:32px">🏟️</span>';return`<div style="display:inline-block;zoom:${Ee};line-height:0;pointer-events:none">
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(T=>{T.addEventListener("click",()=>{C=T.dataset.tab,E="Tous",G="",N=!1,e.querySelectorAll(".col-tab-btn").forEach(S=>{const R=S.dataset.tab===C;S.style.borderBottomColor=R?"var(--green)":"transparent",S.style.color=R?"var(--green)":"var(--gray-600)"}),I(),k()})}),I(),k()}function Un(e,t,i){const n=Se[e]||{icon:"⚡",desc:"Effet spécial."},a=(t==null?void 0:t.gc_type)==="ultra_game_changer",o={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},r={purple:"#b06ce0",light_blue:"#00d4ef"},s=o[t==null?void 0:t.color]||o.purple,d=r[t==null?void 0:t.color]||r.purple,l=(t==null?void 0:t.name)||e,c=(t==null?void 0:t.effect)||n.desc,u=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${u.map((U,V)=>{const Y=ee[U.id]||[],T=U.is_for_sale,S=Y.length?Y[Y.length-1]:null,H=(c.rarity==="pepite"||c.rarity==="papyte")&&U.current_note!=null?` (☆${U.current_note})`:"";return`
            <div data-card-id="${U.id}" data-card-idx="${V}" class="exemplaire-row"
              ...
              <div style="display:flex;align-items:center;gap:10px">
                ...
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:700">Exemplaire ${V+1}${H}${T?" 🏷️ En vente":""}</div>
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;background:#f0fdf4;border-radius:10px;padding:10px 14px">
          <div>
            ...
            <div style="font-size:11px;color:#555;margin-top:2px">
              ${(e.evolution_bonus||0)>0?`+${e.evolution_bonus} appliqué${e.evolution_bonus>1?"s":""} · Note : ${G}${c.job2&&N>0?` / ${te}`:""}`:`Note actuelle : ${G}${c.job2&&N>0?` / ${te}`:""}`}
            </div>
          </div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",c.note_g],["DEF",c.note_d],["MIL",c.note_m],["ATT",c.note_a]].map(([U,V])=>{const Y=Wt[U],T=U==="GK"?"#fff":Y,S=U===c.job||U===c.job2,R=(Number(V)||0)+(S&&C>0?C:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
    ...
    ${I}
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(pe=document.getElementById("close-detail"))==null||pe.addEventListener("click",s);let k=new Set;const F=()=>{const U=k.size,V=document.getElementById("sell-action-panel");if(!V)return;V.style.display=U>0?"block":"none",document.getElementById("sell-selected-count").textContent=U;const Y=document.getElementById("evolve-btn");Y&&(Y.textContent=`⬆️ Évoluer ${U>1?"(+"+U+")":""}`)};document.querySelectorAll(".expl-check").forEach(U=>{U.addEventListener("change",()=>{const V=U.dataset.id;U.checked?k.add(V):k.delete(V);const Y=U.closest(".exemplaire-row");Y&&(Y.style.borderColor=U.checked?"#1A6B3C":"#eee"),F()})}),document.querySelectorAll(".exemplaire-row").forEach(U=>{U.addEventListener("click",V=>{if(V.target.closest("button")||V.target.tagName==="INPUT")return;const Y=U.querySelector(".expl-check");Y&&!Y.disabled&&(Y.checked=!Y.checked,Y.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(U=>{U.addEventListener("click",V=>{V.stopPropagation();const Y=document.querySelector(`.expl-hist[data-hist="${U.dataset.idx}"]`);Y&&(Y.style.display=Y.style.display==="none"?"flex":"none")})}),(le=document.getElementById("evolve-btn"))==null||le.addEventListener("click",async()=>{if(g<=1)return;const U=[...k];if(!U.length)return;if(k.has(e.id)){const K=document.createElement("div");K.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",K.innerHTML=`
        ...
        <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.3)">
          ...
          <div style="background:#f0fdf4;border-radius:10px;padding:12px;margin-bottom:16px;font-size:13px;color:#333">
            ...
            📈 Note : <strong>${v+(e.evolution_bonus||0)}</strong> → <strong style="color:#1A6B3C">${v+(e.evolution_bonus||0)+V.length*Y}</strong>
            ${$&&$>0?`<br>📈 Note 2 : <strong>${$+(e.evolution_bonus||0)}</strong> → <strong style="color:#1A6B3C">${$+(e.evolution_bonus||0)+V.length*Y}</strong>`:""}
            ${c.rarity==="legende"?`<br><span style="color:#7a28b8;font-size:11px;font-weight:700">✨ Légende : +${Y} par copie sacrifiée</span>`:""}
          </div>
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    </div>`;m("⚽ Choisir une formation",v),document.querySelectorAll("#modal-body [data-forma]").forEach(h=>{h.addEventListener("click",()=>{t.formation=h.dataset.forma;const $=Bi(t.formation),y={};$.forEach(C=>{t.slots[C]&&(y[C]=t.slots[C])}),t.slots=y,b(),Ze(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(m=>m.addEventListener("click",x)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(m=>m.addEventListener("click",()=>Yn(t,e,i))),e.querySelectorAll("#save-deck").forEach(m=>m.addEventListener("click",()=>Jn(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(m=>{m.addEventListener("click",()=>{t.subs=t.subs.filter(b=>b!==m.dataset.removeSub),Ze(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(m=>m.addEventListener("click",()=>Xn(t,e,i)))}function Vn(e,t,i,n){var h,$;const a=window.innerWidth>=900,o=e.querySelector(a?"#deck-field-pc":"#deck-field-mobile");if(!o)return;const r=(h=t.stadiumCards)==null?void 0:h.find(y=>y.id===t.stadiumCardId),s=r&&(($=t.stadDefMap)==null?void 0:$[r.stadium_id])||null,d=St[t.formation]||{},l=Pi(t.formation)||[],c={};for(const y of i){const C=t.slots[y],E=C?t.playerCards.find(G=>G.id===C):null;E!=null&&E.player?c[y]={...E.player,_evolution_bonus:E.evolution_bonus||0,face:E.player.face||null}:c[y]=null}const u=window.innerWidth>=900,g=u?window.innerWidth-280:window.innerWidth-20,x=u?Math.min(g,860):g,f=Math.round(u?x*.82:x*.85),p=u?84:44;let m="";for(const[y,C]of l){const E=d[y],G=d[C];if(!E||!G)continue;const N=E.x*x,te=Math.round(.03*f+E.y*.85*f),ne=G.x*x,ee=Math.round(.03*f+G.y*.85*f),j=c[y],I=c[C],k=yt(j,I);k==="#ff3333"||k==="#cc2222"?m+=`<line x1="${N.toFixed(1)}" y1="${te.toFixed(1)}" x2="${ne.toFixed(1)}" y2="${ee.toFixed(1)}" stroke="${k}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(m+=`<line x1="${N.toFixed(1)}" y1="${te.toFixed(1)}" x2="${ne.toFixed(1)}" y2="${ee.toFixed(1)}" stroke="${k}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,m+=`<line x1="${N.toFixed(1)}" y1="${te.toFixed(1)}" x2="${ne.toFixed(1)}" y2="${ee.toFixed(1)}" stroke="${k}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let b="";const v=Math.round(p*657/507);for(const y of i){const C=d[y];if(!C)continue;const E=c[y],G=C.x*x;C.y*f;const N=Math.round(.03*f+C.y*(.85*f)),te=Math.round(G-p/2),ne=Math.round(N-v/2);if(E){const ee=y.replace(/\d+/,""),j=s&&(s.club_id&&String(E.club_id)===String(s.club_id)||s.country_code&&E.country_code===s.country_code),I=Ae({...E,_evolution_bonus:E._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:ee});b+=`<div style="position:absolute;left:${te}px;top:${ne}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${y}">
        <div style="position:relative">${j?'<div style="position:absolute;top:-18px;left:0;right:0;text-align:center;font-size:14px;z-index:5;line-height:1">🏟️</div>':""}${I}</div>
      </div>`}else{const ee=y.replace(/\d+/,"");b+=`<div style="position:absolute;left:${te}px;top:${ne}px;width:${p}px;height:${v}px;
  ...
  `,document.body.appendChild(a);let o=!1;const r=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let d=!1;const l=I=>I.touches&&I.touches[0]?I.touches[0].clientX:I.clientX;function c(I){o||(d=!0,s.style.opacity="1",u(I))}function u(I){if(!d||o)return;const k=r.getBoundingClientRect(),F=l(I)-k.left,de=Math.max(0,Math.min(1,F/k.width));s.style.width=de*k.width+"px",de>=.82&&x()}function g(){o||(d=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function x(){var k;if(o)return;o=!0,d=!1,s.style.width="100%",s.style.opacity="1",(k=document.getElementById("cut-flash"))==null||k.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const I=document.getElementById("cut-hint");I&&(I.style.opacity="0"),r.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",b(0)},620)}r.addEventListener("pointerdown",c),window.addEventListener("pointermove",u),window.addEventListener("pointerup",g),r.addEventListener("touchstart",c,{passive:!0}),window.addEventListener("touchmove",u,{passive:!0}),window.addEventListener("touchend",g);let f=0,p=!1;const m=new Set;function b(I){f=I,document.getElementById("reveal-phase").style.display="flex",v(),h(I,0),G()}function v(){const I=document.getElementById("card-dots");I&&(I.innerHTML=e.map((k,F)=>`<div class="card-dot" data-i="${F}" style="width:8px;height:8px;border-radius:50%;background:${F===f?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),I.querySelectorAll(".card-dot").forEach(k=>k.addEventListener("click",()=>y(parseInt(k.dataset.i)))))}function h(I,k){var Y,T;const F=e[I],de=document.getElementById("card-counter"),pe=document.getElementById("card-track");de&&(de.textContent=`Carte ${I+1} / ${e.length}`);const le=document.getElementById("reveal-btns");le&&(le.style.display=I===e.length-1?"flex":"none");const ae=F.card_type==="player"&&((Y=F.player)==null?void 0:Y.rarity)==="legende",_e=!m.has(I);m.add(I);let U=0;if(F.card_type==="player"&&F.player){const S=F.player,R=S.job||"ATT";U=Number(R==="GK"?S.note_g:R==="DEF"?S.note_d:R==="MIL"?S.note_m:S.note_a)||0}const V=S=>{pe.innerHTML=`
        ...
        </div>`;const R=document.getElementById("current-card-wrap");k!==0?(R.style.transition="none",R.style.transform=`translateX(${k>0?100:-100}%)`,requestAnimationFrame(()=>{R.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",R.style.transform="translateX(0)"})):R.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),S||ae?te():ne(),v()};_e&&(((T=F.player)==null?void 0:T.rarity)==="legende"||U>=8)&&F.card_type==="player"&&F.player?$(F,()=>V(!0)):V(!1)}function $(I,k){var we;p=!0;const F=I.player,de=`https://flagsapi.com/${F.country_code}/flat/64.png`,pe=(we=F.clubs)==null?void 0:we.logo_url,le=F.face?"/"+F.face.replace(/^public\//,"").replace(/^\//,""):null,ae=F.job||"ATT",_e=Number(ae==="GK"?F.note_g:ae==="DEF"?F.note_d:ae==="MIL"?F.note_m:F.note_a)||0,U=I.evolution_bonus||0,V=_e+U,Y=F.rarity==="legende",T=V>=18,S=document.getElementById("walkout-overlay"),R=document.getElementById("walkout-stage");if(!S||!R){p=!1,k();return}let H=null;Y&&(H=new Audio("/sounds/Legendary.mp3"),H.volume=.8,H.play().catch(()=>{})),S.style.display="flex";const K=()=>{const me=R.firstElementChild;me&&(me.classList.remove("wo-in"),me.classList.add("wo-out"))},B=1800,O=400;R.innerHTML=`<img class="wo-in" src="${de}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(K,B),setTimeout(()=>{var me;R.innerHTML=pe?`<img class="wo-in" src="${pe}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((me=F.clubs)==null?void 0:me.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},B+O),setTimeout(K,B*2+O),setTimeout(()=>{R.innerHTML=le?`<img class="wo-in" src="${le}" style="height:200px;border-radius:50%;box-shadow:0 0 40px rgba(255,255,255,0.3);object-fit:cover;object-position:top">`:'<div class="wo-in" style="font-size:80px">👤</div>',navigator.vibrate&&navigator.vibrate(30)},(B+O)*2),setTimeout(K,(B+O)*2+B);const ke=T?"#FFD700":{GK:"#c0c0c0",DEF:"#e03030",MIL:"#D4A017",ATT:"#3fbf5f"}[ae]||"#fff";setTimeout(()=>{R.innerHTML=`<div class="wo-in" style="
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div id="deck-top-bar" style="flex-shrink:0">
        ...
        <div style="display:flex;align-items:center;gap:8px;padding:5px 14px;background:linear-gradient(90deg,rgba(232,119,34,0.3),transparent);border-top:1px solid rgba(232,119,34,0.3)">
          ...
          <span style="font-size:11px;color:#FFD700;margin-left:auto">+10 aux joueurs ${((E=v.club)==null?void 0:E.encoded_name)||v.country_code||""}</span>
        </div>`:""}
      </div>
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:visible;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center">
        ${b?`<div class="deck-preview-wrap" style="overflow:visible">
               ${At(b,m,null,[],300,300)}
             </div>`:`<div style="opacity:.4;text-align:center"><div style="font-size:32px">⚠️</div><div>Deck incomplet (${f.length}/11)</div></div>`}
      </div>

      <!-- Pagination -->
      ${o.length>1?`<div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">${o.map((ee,j)=>`<div style="width:6px;height:6px;border-radius:50%;background:${j===c?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}</div>`:""}
      ...
      <div id="deck-bottom-bar" style="flex-shrink:0;padding:10px 14px 14px;display:flex;flex-direction:column;gap:8px;background:rgba(0,0,0,0.2)">
    ...
    </div>`,requestAnimationFrame(function(){var V;const j=e.querySelector(".deck-preview-wrap svg"),I=e.querySelector(".deck-preview-wrap"),k=e.querySelector("#deck-swipe-zone");if(!j||!I||!k)return;const F=k.clientHeight-8,de=k.clientWidth-8,pe=30,le=(V=j.getAttribute("viewBox"))==null?void 0:V.split(" ").map(Number),ae=le?le[3]/le[2]:1.1;let _e=de,U=_e*ae;U>F&&(U=F,_e=U/ae),j.removeAttribute("width"),j.removeAttribute("height"),j.style.cssText="display:block;overflow:visible",j.setAttribute("preserveAspectRatio","xMidYMid meet"),j.setAttribute("width",_e),j.setAttribute("height",U),I.style.cssText=`width:${_e}px;height:${U}px;overflow:visible;margin-top:${pe}px`}),(G=document.getElementById("prev-deck"))==null||G.addEventListener("click",()=>{c>0&&(c--,u())}),(N=document.getElementById("next-deck"))==null||N.addEventListener("click",()=>{c<o.length-1&&(c++,u())}),(te=document.getElementById("validate-deck"))==null||te.addEventListener("click",()=>{if(!h)return;const ee=t.state.params||{};t.navigate("match",{...ee,matchMode:ee.matchMode||i,deckId:g.id})}),(ne=document.getElementById("cancel-deck-select"))==null||ne.addEventListener("click",()=>{Ue(e),a("home")});const $=document.getElementById("deck-swipe-zone");if($){let ee=0,j=0;$.addEventListener("touchstart",I=>{ee=I.touches[0].clientX,j=I.touches[0].clientY},{passive:!0}),$.addEventListener("touchend",I=>{const k=I.changedTouches[0].clientX-ee,F=I.changedTouches[0].clientY-j;Math.abs(k)<40||Math.abs(k)<Math.abs(F)||(k<0&&c<o.length-1?(c++,u()):k>0&&c>0&&(c--,u()))},{passive:!0})}}u()}function Je(e,t=44,i=58,n=null){return Ae(e,{width:t,showStad:!!n,stadDef:n,used:e==null?void 0:e.used})}function ut(e,t,i){if(!(e!=null&&e.length))return"";const n=e.slice(0,5);let a='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return n.forEach((o,r)=>{const s=o._line||o.job||"MIL";if(a+=Ae(o,{width:40,role:s,extraNote:o.boost||0}),r<n.length-1){const d=yt(o,n[r+1]);a+=`<div style="width:7px;height:3px;background:${d==="#ff3333"||d==="#cc2222"?"rgba(255,255,255,0.12)":d};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(a+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),a+="</div>",a}function It(e,t,i,n,a=310,o=310,r=[]){const s=St[t]||{},d=Pi(t)||si[t]||[],l={},c=["ATT","MIL","DEF","GK"];for(const m of c)(e[m]||[]).forEach((v,h)=>{l[`${m}${h+1}`]=v});function u(m){const b=s[m];return b?{x:b.x*a,y:b.y*o}:null}let g="";for(const[m,b]of d){const v=u(m),h=u(b);if(!v||!h)continue;const $=l[m],y=l[b],C=yt($,y);C==="#00ff88"||C==="#FFD700"?(g+=`<line x1="${v.x.toFixed(1)}" y1="${v.y.toFixed(1)}" x2="${h.x.toFixed(1)}" y2="${h.y.toFixed(1)}"
        ...
        stroke="${C}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const x=Math.max(52,Math.round(a*.18)),f=Math.round(x*657/507);for(const[m,b]of Object.entries(l)){const v=u(m);if(!v||!b)continue;const h=m.replace(/[0-9]/g,""),$=r.includes(b.cardId),y=i==="attack"&&(["MIL","ATT"].includes(h)||$)&&!b.used||i==="defense"&&["GK","DEF","MIL"].includes(h)&&!b.used,C=n.includes(b.cardId);let E=b.boost||0;b.stadiumBonus&&(i==="attack"&&(h==="ATT"||h==="MIL")||i==="defense"&&(h==="GK"||h==="DEF"||h==="MIL")?E+=10:i||(E+=10));const G=Math.round(v.x-x/2),N=Math.round(v.y-f/2);if(b.used){g+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${G}" y="${N}" width="${x}" height="${f}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${b.cardId}" data-role="${h}" style="cursor:pointer"/>`;continue}const te=Ae({...b,_evolution_bonus:0},{width:x,showStad:!0,stadDef:null,role:h,extraNote:E}),ne=C?"outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;opacity:0.75;":"";g+=`<foreignObject x="${G-2}" y="${N-30}" width="${x+8}" height="${f+60}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="${ne}position:relative">
        ${te}
      </div>
    </foreignObject>`,y&&(g+=`<rect x="${G}" y="${N}" width="${x}" height="${f}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${C?"selected":""}" data-card-id="${b.cardId}" data-role="${h}" style="cursor:pointer"/>`)}const p=Math.round(Math.max(x*.7,80));return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-p} ${-p} ${a+p*2} ${o+p*2}" width="100%" style="display:block;width:100%;margin:0 auto">
    ${g}
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const F=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
            ...
            ${ut((F.players||[]).map(de=>({...de,used:!1})),"#ff6b6b",F.total)}
            ...
            ${ut((F.players||[]).map(de=>({...de,used:!1})),"#00ff88",F.total)}
    ...
    ${(()=>{const k=window.innerWidth>=700,F=(T,S,R)=>{var Ce,je;const H=(t.gcDefs||[]).find(Fe=>Fe.name===T.gc_type),K={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[H==null?void 0:H.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",B={purple:"#b06ce0",light_blue:"#00d4ef"}[H==null?void 0:H.color]||"#b06ce0",O=(H==null?void 0:H.name)||T.gc_type,Ee=(H==null?void 0:H.effect)||((Ce=Se[T.gc_type])==null?void 0:Ce.desc)||"",ke=H!=null&&H.image_url?`/icons/${H.image_url}`:null,we=((je=Se[T.gc_type])==null?void 0:je.icon)||"⚡",me=Math.round(R*.22),Le=Math.round(R*.22),$e=R-me-Le,he=O.length>12?7:9;return`<div class="gc-mini" data-gc-id="${T.id}" data-gc-type="${T.gc_type}"
          style="box-sizing:border-box;width:${S}px;height:${R}px;border-radius:10px;border:2px solid ${B};background:${K};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
          ...
          <div style="height:${$e}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${ke?`<img src="${ke}" style="max-width:${S-10}px;max-height:${$e-6}px;object-fit:contain">`:`<span style="font-size:${Math.round($e*.55)}px">${we}</span>`}
          </div>
        ...
        </div>`},de=(T,S)=>{var R;return`<div id="boost-card"
          ...
          </div>`},pe=(T,S)=>S?de(130,175):F(T,130,175),le=(T,S)=>S?de(68,95):F(T,68,95),ae=k?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",_e=f?`<button id="btn-results" style="${ae};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:u?`<div style="${ae};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:b?`<button id="btn-pass" style="${ae};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER (plus d'attaquants)</button>`:g?`<button id="btn-action" style="${ae};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:x?`<button id="btn-action" style="${ae};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${ae};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,U=g||x?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",V=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${k?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(k=>{var F,de,pe;if(k.type==="duel"){const le=k.isGoal,ae=k.homeScored?"#FFD700":le?"#ff6b6b":"rgba(255,255,255,0.3)",_e=k.homeScored?"⚽ BUT !":le?"⚽ BUT IA !":(F=k.homePlayers)!=null&&F.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${le?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${ae};margin-bottom:4px">
                <div style="font-size:9px;color:${ae};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${_e}</div>
                ${(de=k.homePlayers)!=null&&de.length?`<div style="margin-bottom:3px">${ut(k.homePlayers,"rgba(255,255,255,0.7)",k.homeTotal)}</div>`:""}
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${It(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            <div style="background:rgba(255,255,255,0.08);border-radius:14px;padding:14px 20px;text-align:center;min-width:220px">
              ...
              ${Me.id!==Pe.id?`<div style="font-size:20px;font-weight:900;color:${Me.color}">
                  ...
                  <div style="font-size:13px;color:rgba(255,255,255,0.5);margin-top:4px">${ze>Q.mmr?"Promotion !":"Rétrogradation"}</div>`:`<div style="font-size:36px">${be>=0?"📈":"📉"}</div>
            ...
            </div>`}}catch(Q){console.error("[Ranked] MMR update error:",Q)}(J=document.getElementById("pvp-end-overlay"))==null||J.remove();const q=document.createElement("div");q.id="pvp-end-overlay",q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const P=z?"🤝":M?"🏆":"😞",W=z?"MATCH NUL":M?"VICTOIRE !":"DÉFAITE",X=z?"#fff":M?"#FFD700":"#ff6b6b";q.innerHTML=`
      ...
      ${D}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${r?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(q),(re=q.querySelector("#pvp-end-home"))==null||re.addEventListener("click",()=>{q.remove(),Ue(e),x(r?"ranked":"home")})}const ee=w.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},_=>{const L=_.new;try{if(L.status==="finished"||L.forfeit){if(C)return;C=!0,E&&(clearInterval(E),E=null),L.game_state&&(y=L.game_state),ne(L);return}if(L.game_state){const A=y;y=L.game_state;const M=y._lastGC;if(M&&M.seq&&!te.has(M.seq)&&(te.add(M.seq),M.by!==p)){Y(M.type,M.by,()=>ae());return}const z=A[p+"Score"]||0,D=A[m+"Score"]||0,q=y[p+"Score"]||0,P=y[m+"Score"]||0,W=q>z,X=P>D;if((W||X)&&!N.has(y.round)){N.add(y.round);const J=[...y.log||[]].reverse().find(Q=>Q.isGoal),re=((J==null?void 0:J.homePlayers)||[]).map(Q=>({name:Q.name,note:Q.note,portrait:Q.portrait,job:Q.job}));st(re,q,P,W,()=>ae());return}ae()}}catch(A){console.error("[PvP] crash:",A)}}).subscribe();async function j(_){Object.assign(y,_),y.lastActionAt=new Date().toISOString();const{error:L}=await w.from("matches").update({game_state:y}).eq("id",i);L&&f("Erreur de synchronisation","error");try{ae()}catch(A){console.error("[PvP] renderPvpScreen crash:",A)}}async function I(){if(C)return;C=!0,E&&(clearInterval(E),E=null);const _=n?h.away_id:h.home_id,L=n?"p2":"p1",A=n?"p1":"p2",M={...y,[L+"Score"]:3,[A+"Score"]:0,phase:"finished"};try{await w.from("matches").update({status:"finished",forfeit:!0,winner_id:_,home_score:M.p1Score||0,away_score:M.p2Score||0,game_state:M}).eq("id",i)}catch{}try{w.removeChannel(ee)}catch{}setTimeout(()=>{Ue(e),x("home")},800)}const k={BOOST_STAT:({value:_=1,count:L=1,roles:A=[]},M,z)=>{const D=M[p+"Team"],q=Object.entries(D).filter(([P])=>!A.length||A.includes(P)).flatMap(([P,W])=>W.filter(X=>!X.used).map(X=>({...X,_line:P})));if(!q.length){M.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),z(M);return}F(q,L,`Choisir ${L} joueur(s) à booster (+${_})`,P=>{P.forEach(W=>{const X=(D[W._line]||[]).find(J=>J.cardId===W.cardId);X&&(X.boost=(X.boost||0)+_,M.log.push({text:`⚡ +${_} sur ${X.name}`,type:"info"}))}),M[p+"Team"]=D,z(M)})},DEBUFF_STAT:({value:_=1,count:L=1,roles:A=[],target:M="ai"},z,D)=>{const q=M==="home"?p:m,P=z[q+"Team"],W=M==="home"?"allié":"adverse",X=Object.entries(P).filter(([J])=>!A.length||A.includes(J)).flatMap(([J,re])=>re.map(Q=>({...Q,_line:J})));if(!X.length){z.log.push({text:`🎯 Aucun joueur ${W}`,type:"info"}),D(z);return}F(X,L,`Choisir ${L} joueur(s) ${W}(s) (-${_})`,J=>{J.forEach(re=>{const Q=(P[re._line]||[]).find(ce=>ce.cardId===re.cardId);Q&&(Q.boost=(Q.boost||0)-_,z.log.push({text:`🎯 -${_} sur ${Q.name}`,type:"info"}))}),z[q+"Team"]=P,D(z)})},GRAY_PLAYER:({count:_=1,roles:L=[],target:A="ai"},M,z)=>{const D=A==="home"?p:m,q=M[D+"Team"],P=A==="home"?"allié":"adverse",W=Object.entries(q).filter(([X])=>!L.length||L.includes(X)).flatMap(([X,J])=>J.filter(re=>!re.used).map(re=>({...re,_line:X})));if(!W.length){M.log.push({text:`❌ Aucun joueur ${P}`,type:"info"}),z(M);return}F(W,_,`Choisir ${_} joueur(s) ${P}(s) à exclure`,X=>{const J="usedCardIds_"+D,re=new Set(M[J]||[]);X.forEach(Q=>{const ce=(q[Q._line]||[]).find(se=>se.cardId===Q.cardId);ce&&(ce.used=!0,re.add(Q.cardId),M.log.push({text:`❌ ${ce.name} exclu !`,type:"info"}))}),M[J]=[...re],M[D+"Team"]=q,z(M)})},REVIVE_PLAYER:({count:_=1,roles:L=[]},A,M)=>{const z=A[p+"Team"],D=Object.entries(z).filter(([q])=>!L.length||L.includes(q)).flatMap(([q,P])=>P.filter(W=>W.used).map(W=>({...W,_line:q})));if(!D.length){A.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),M(A);return}F(D,_,`Choisir ${_} joueur(s) à ressusciter`,q=>{q.forEach(P=>{const W=(z[P._line]||[]).find(X=>X.cardId===P.cardId);W&&(W.used=!1,A.log.push({text:`💫 ${W.name} ressuscité !`,type:"info"}))}),A[p+"Team"]=z,M(A)})},REMOVE_GOAL:({},_,L)=>{const A=m+"Score";_[A]>0?(_[A]--,_.log.push({text:"🚫 Dernier but annulé !",type:"info"})):_.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L(_)},ADD_GOAL_DRAW:({},_,L)=>{_[p+"Score"]===_[m+"Score"]?(_[p+"Score"]++,_.log.push({text:"🎯 But bonus !",type:"info"})):_.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L(_)},ADD_SUB:({value:_=1},L,A)=>{L.maxSubs=(L.maxSubs||3)+_,L.log.push({text:`🔄 +${_} remplacement(s)`,type:"info"}),A(L)},CUSTOM:({},_,L)=>L(_)};function F(_,L,A,M){const z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let D=[];function q(){var W,X;const P=_.map(J=>{const re={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[J._line]||"#555",Q=qe(J,J._line)+(J.boost||0),se=D.find(ue=>ue.cardId===J.cardId)?"#FFD700":"rgba(255,255,255,0.25)",oe=J.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${J.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${se};background:${re};overflow:hidden;cursor:pointer;${oe}">
        ...
        </div>`,(W=z.querySelector("#pp-close"))==null||W.addEventListener("click",()=>z.remove()),z.querySelectorAll(".pp-item").forEach(J=>{J.addEventListener("click",()=>{const re=J.dataset.cid,Q=_.find(se=>se.cardId===re),ce=D.findIndex(se=>se.cardId===re);Q&&(ce>-1?D.splice(ce,1):D.length<L&&D.push(Q),q())})}),(X=z.querySelector("#pp-confirm"))==null||X.addEventListener("click",()=>{z.remove(),M(D)})}q(),document.body.appendChild(z)}async function de(_,L){const M=(y["gcCardsFull_"+p]||[]).find(P=>P.id===_),z=(M==null?void 0:M._gcDef)||(y.gcDefs||[]).find(P=>{var W;return P.name===L||((W=P.name)==null?void 0:W.toLowerCase().trim())===(L==null?void 0:L.toLowerCase().trim())}),D=[...y["usedGc_"+p]||[],_],q={type:L,by:p,seq:(y._gcAnimSeq||0)+1};te.add(q.seq),Y(L,p,async()=>{if(z!=null&&z.effect_type&&z.effect_type!=="CUSTOM"){const W=k[z.effect_type];if(W){const X={...y};W(z.effect_params||{},X,async J=>{J["usedGc_"+p]=D,J._lastGC=q,J._gcAnimSeq=q.seq,await j(J)});return}}const P={...y};switch(L){case"Remplacement+":P.maxSubs=(P.maxSubs||3)+1,P.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const W=m+"Score";P[W]>0&&(P[W]--,P.log.push({text:"🚫 But annulé",type:"info"}));break}}P["usedGc_"+p]=D,P._lastGC=q,P._gcAnimSeq=q.seq,await j(P)})}function pe(_,L){const A="usedCardIds_"+_,M=new Set(y[A]||[]);L.forEach(z=>M.add(z)),y[A]=[...M]}function le(){for(const _ of["p1","p2"]){const L=new Set(y["usedCardIds_"+_]||[]),A=y[_+"Team"];if(A)for(const M of["GK","DEF","MIL","ATT"])(A[M]||[]).forEach(z=>{z.used=L.has(z.cardId)})}}function ae(){var Ve,nt,yi,bi,hi,vi;if(y.phase==="reveal")return _e();if(y.phase==="midfield")return V();if(y.phase==="finished")return Fe();const _=y[p+"Team"],L=y[m+"Team"];le();const A=y[p+"Score"],M=y[m+"Score"],z=y[p+"Name"],D=y[m+"Name"],q=y.phase===p+"-attack",P=y.phase===p+"-defense",W=Array.isArray(y["selected_"+p])?y["selected_"+p]:[],X=W.map(ie=>ie.cardId),J=window.innerWidth>=700,re=y[p+"Subs"]||[],Q=y["usedSubIds_"+p]||[],ce=re.filter(ie=>!Q.includes(ie.cardId)),se=y["gcCardsFull_"+p]||[],oe=y["usedGc_"+p]||[],ue=se.filter(ie=>!oe.includes(ie.id)),fe=y.boostOwner===p&&!y.boostUsed,xe=!["GK","DEF","MIL","ATT"].some(ie=>(L[ie]||[]).some(ge=>!ge.used)),Z=[..._.MIL||[],..._.ATT||[]].filter(ie=>!ie.used),be=q&&xe&&Z.length===0?[..._.GK||[],..._.DEF||[]].filter(ie=>!ie.used).map(ie=>ie.cardId):[];function ze(ie,ge,Ie){var $i,Ei;const ye=ie._gcDef,ot={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ye==null?void 0:ye.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",lt={purple:"#b06ce0",light_blue:"#00d4ef"}[ye==null?void 0:ye.color]||"#b06ce0",ht=(ye==null?void 0:ye.name)||ie.gc_type,vt=(ye==null?void 0:ye.effect)||(($i=Se[ie.gc_type])==null?void 0:$i.desc)||"",wi=ye!=null&&ye.image_url?`/icons/${ye.image_url}`:null,un=((Ei=Se[ie.gc_type])==null?void 0:Ei.icon)||"⚡",_i=Math.round(Ie*.22),ki=Math.round(Ie*.22),Vt=Ie-_i-ki,fn=ht.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${ie.id}" data-gc-type="${ie.gc_type}"
      ...
      </div>`}const Ne=(ie,ge)=>ge?Me(130,175):ze(ie,130,175),Ge=(ie,ge)=>ge?Me(68,95):ze(ie,68,95),Pe=J?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",bt=q?$e(p)?`<button id="pvp-action" style="${Pe};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${W.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${Pe};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:P?`<button id="pvp-action" style="${Pe};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${W.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${D}</div>`,Be=q&&!$e(p)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':q||P?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${W.length}/3 sélectionné(s)</div>`:"",Oe=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${J?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ${ce.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':ce.map(ie=>`<div class="pvp-sub-btn" data-sub-id="${ie.cardId}" style="cursor:pointer;flex-shrink:0">${Je(ie,J?76:44,J?100:58)}</div>`).join("")}
      ...
      </div>`;function De(){const ie=e.querySelector(".match-screen");if(!ie)return;const ge=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);ie.style.height=ge+"px",ie.style.minHeight=ge+"px",ie.style.maxHeight=ge+"px",ie.style.overflow="hidden";const Ie=e.querySelector("#mobile-action-bar"),ye=e.querySelector("#mobile-play-area");Ie&&ye&&(ye.style.paddingBottom=Ie.offsetHeight+"px")}if(De(),setTimeout(De,120),setTimeout(De,400),G||(G=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",De),window.visualViewport.addEventListener("scroll",De)),window.addEventListener("resize",De)),function(){const ge=e.querySelector(".terrain-wrapper svg");ge&&(ge.removeAttribute("width"),ge.removeAttribute("height"),ge.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ge.setAttribute("viewBox","-26 -26 352 352"),ge.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(ie=>{ie.addEventListener("click",()=>{if(!q&&!P)return;const ge=ie.dataset.cardId,Ie=ie.dataset.role,ye=(_[Ie]||[]).find(vt=>vt.cardId===ge);if(!ye||ye.used)return;const ot=be.includes(ge);if(q&&!["MIL","ATT"].includes(Ie)&&!ot)return;Array.isArray(y["selected_"+p])||(y["selected_"+p]=[]);const lt=y["selected_"+p],ht=lt.findIndex(vt=>vt.cardId===ge);ht>-1?lt.splice(ht,1):lt.length<3&&lt.push({...ye,_role:Ie}),ae()})}),e.querySelectorAll(".match-used-hit").forEach(ie=>{ie.addEventListener("click",()=>R(ie.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(ie=>{ie.addEventListener("click",()=>R())}),(Ve=e.querySelector("#pvp-sub-open"))==null||Ve.addEventListener("click",()=>R()),e.querySelectorAll(".pvp-gc-mini").forEach(ie=>{ie.addEventListener("click",()=>T(ie.dataset.gcId,ie.dataset.gcType))}),(nt=e.querySelector("#pvp-boost-card"))==null||nt.addEventListener("click",()=>S()),(yi=e.querySelector("#pvp-action"))==null||yi.addEventListener("click",ie=>{q?ie.currentTarget.dataset.pass==="1"||!$e(p)?O():Ee():P&&ke()}),(bi=e.querySelector("#pvp-quit"))==null||bi.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&I()}),(hi=e.querySelector("#pvp-view-opp"))==null||hi.addEventListener("click",()=>K()),(vi=e.querySelector("#pvp-toggle-history"))==null||vi.addEventListener("click",()=>B()),E&&(clearInterval(E),E=null),(q||P)&&!C){let ie=30,ge=!1;const Ie=()=>document.getElementById("pvp-timer"),ye=()=>{Ie()&&(Ie().textContent=ie+"s",Ie().style.color=ge?"#ff4444":"#fff")};ye(),E=setInterval(()=>{ie--,ie<0?ge?(clearInterval(E),E=null,q&&!$e(p)?O():I()):(ge=!0,ie=15,ye()):ye()},1e3)}}function _e(){He(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      ...
      <div style="width:min(90vw,420px)">${It(y[m+"Team"],y[m+"Formation"],null,[],We(),rt())}</div>
    </div>`,p==="p1"&&setTimeout(async()=>{await j({phase:"midfield"})},5e3)}let U=!1;function V(){if(U)return;const _=y[p+"Team"].MIL||[],L=y[m+"Team"].MIL||[],A=milScore(_)+milLinks(_),M=milScore(L)+milLinks(L),z=A>=M;function D(se,oe,ue){return`<div id="duel-row-${ue}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${se.map((fe,xe)=>{const Z=xe<se.length-1?yt(fe,se[xe+1]):null,be=!Z||Z==="#ff3333"||Z==="#cc2222",ze=Z==="#00ff88"?"+2":Z==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${ue}" data-idx="${xe}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(Q);let ce=!1;const se=()=>{ce||(ce=!0,Q.remove(),setTimeout(()=>A&&A(),50))};Q.addEventListener("click",se),setTimeout(se,3e3)}function T(_,L){var Q,ce,se,oe;const M=(y["gcCardsFull_"+p]||[]).find(ue=>ue.id===_),z=M==null?void 0:M._gcDef,D={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[z==null?void 0:z.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",q={purple:"#b06ce0",light_blue:"#00d4ef"}[z==null?void 0:z.color]||"#b06ce0",P=(z==null?void 0:z.name)||L,W=(z==null?void 0:z.effect)||((Q=Se[L])==null?void 0:Q.desc)||"Carte spéciale.",X=z!=null&&z.image_url?`/icons/${z.image_url}`:null,J=((ce=Se[L])==null?void 0:ce.icon)||"⚡",re=document.createElement("div");re.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",re.innerHTML=`
      ...
      </div>`,document.body.appendChild(re),(se=re.querySelector("#pvp-gc-back"))==null||se.addEventListener("click",()=>re.remove()),(oe=re.querySelector("#pvp-gc-use"))==null||oe.addEventListener("click",()=>{re.remove(),de(_,L)})}function S(){var M;const _=y[p+"Team"],L=Object.entries(_).flatMap(([z,D])=>(D||[]).filter(q=>!q.used).map(q=>({...q,_line:z})));if(!L.length)return;const A=document.createElement("div");A.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",A.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">⚡ Choisir un joueur pour +${y.boostValue}</div>
        <button id="bp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${L.map(z=>{const D={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[z._line]||"#555",q=qe(z,z._line)+(z.boost||0);return`<div class="bp-item" data-cid="${z.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${D};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(A),(M=A.querySelector("#bp-close"))==null||M.addEventListener("click",()=>A.remove()),A.querySelectorAll(".bp-item").forEach(z=>{z.addEventListener("click",async()=>{const D=z.dataset.cid,q=L.find(W=>W.cardId===D);if(!q)return;const P=(_[q._line]||[]).find(W=>W.cardId===D);P&&(P.boost=(P.boost||0)+y.boostValue),A.remove(),await j({[p+"Team"]:_,boostUsed:!0})})})}function R(_=null){var ce,se;if(!(y.phase===p+"-attack")){f("Remplacement uniquement avant votre attaque","warning");return}const A=y[p+"Team"],M=y["usedSubIds_"+p]||[],z=y.maxSubs||3;if(M.length>=z){f(`Maximum ${z} remplacements atteint`,"warning");return}const D=Object.entries(A).flatMap(([oe,ue])=>(ue||[]).filter(fe=>fe.used).map(fe=>({...fe,_line:oe}))),q=(y[p+"Subs"]||[]).filter(oe=>!M.includes(oe.cardId));if(!D.length){f("Aucun joueur utilisé à remplacer","warning");return}if(!q.length){f("Aucun remplaçant disponible","warning");return}let P=Math.max(0,D.findIndex(oe=>oe.cardId===_));const W=((ce=D[P])==null?void 0:ce._line)||((se=D[P])==null?void 0:se.job);let X=Math.max(0,q.findIndex(oe=>oe.job===W)),J=!1;const re=document.createElement("div");re.id="pvp-sub-overlay",re.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function Q(){var ze,Me,Ne,Ge,Pe,bt;const oe=D[P],ue=q[X],fe=Math.min(130,Math.round((window.innerWidth-90)/2)),xe=Math.round(fe*1.35),Z=Be=>`background:rgba(255,255,255,0.12);border:none;color:${Be?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Be?"default":"pointer"};flex-shrink:0`;re.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      </div>`,(ze=re.querySelector("#psub-close"))==null||ze.addEventListener("click",()=>re.remove()),(Me=re.querySelector("#pout-up"))==null||Me.addEventListener("click",()=>{P>0&&(P--,Q())}),(Ne=re.querySelector("#pout-down"))==null||Ne.addEventListener("click",()=>{P<D.length-1&&(P++,Q())}),(Ge=re.querySelector("#pin-up"))==null||Ge.addEventListener("click",()=>{X>0&&(X--,Q())}),(Pe=re.querySelector("#pin-down"))==null||Pe.addEventListener("click",()=>{X<q.length-1&&(X++,Q())});const be=(Be,Oe,Re,tt)=>{const Ke=re.querySelector("#"+Be);if(!Ke)return;let it=0;Ke.addEventListener("touchstart",De=>{it=De.touches[0].clientY},{passive:!0}),Ke.addEventListener("touchend",De=>{const Ve=De.changedTouches[0].clientY-it;if(Math.abs(Ve)<30)return;const nt=Oe();Ve<0&&nt<tt-1?(Re(nt+1),Q()):Ve>0&&nt>0&&(Re(nt-1),Q())},{passive:!0})};be("pin-panel",()=>X,Be=>X=Be,q.length),be("pout-panel",()=>P,Be=>P=Be,D.length),(bt=re.querySelector("#psub-confirm"))==null||bt.addEventListener("click",async Be=>{if(Be.preventDefault(),Be.stopPropagation(),J)return;J=!0;const Oe=D[P],Re=q[X];if(!Oe||!Re)return;const tt=Oe._line,Ke=(A[tt]||[]).findIndex(Ve=>Ve.cardId===Oe.cardId);if(Ke===-1){f("Erreur : joueur introuvable","error"),re.remove();return}const it={...Re,_line:tt,position:Oe.position,used:!1,boost:0};A[tt].splice(Ke,1,it);const De=[...M,Re.cardId];re.remove(),H(Oe,Re,async()=>{await j({[p+"Team"]:A,[m+"Team"]:y[m+"Team"],["usedSubIds_"+p]:De})})})}document.body.appendChild(re),Q()}function H(_,L,A){const M={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const D=(W,X,J)=>`<div style="text-align:center">
      ...
      <div style="width:70px;height:70px;border-radius:50%;background:${M[W.job]||"#555"};border:3px solid ${X};position:relative;overflow:hidden;margin:0 auto">
        ${Tt(W)?`<img src="${Tt(W)}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover">`:""}
      </div>
      ...
      </div>`,document.body.appendChild(L),(A=L.querySelector("#pvp-hist-close"))==null||A.addEventListener("click",()=>L.remove())}async function O(){if(y.phase!==p+"-attack")return;const _=p==="p1"?"p2":"p1",L=(y.round||0)+1,A=[...y.log||[]];A.push({type:"info",text:`⏭️ ${y[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const M=he(y),z=$e(_),D=M||!z?"finished":_+"-attack";await j({["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:D,attacker:_,round:L,log:A}),D==="finished"&&await je(y)}async function Ee(){const _=y[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(D=>(y[m+"Team"][D]||[]).some(q=>!q.used)),A=(y["selected_"+p]||[]).map(D=>{const q=(_[D._role]||[]).find(se=>se.cardId===D.cardId)||D,P=L&&["GK","DEF"].includes(D._role),W=_[D._role]||[],X=W.findIndex(se=>se.cardId===D.cardId),J=xt(W.length),re=X>=0?J[X]:q._col??1,Q=y.stadiumDef||y.homeStadiumDef||null,ce=q.stadiumBonus||Q&&(Q.club_id&&String(q.club_id)===String(Q.club_id)||Q.country_code&&q.country_code===Q.country_code)||!1;return{...q,_line:D._role,_col:re,stadiumBonus:ce,...P?{note_a:Math.max(1,Number(q.note_a)||0)}:{}}});if(!A.length)return;const M=pi(A,y.modifiers[p]||{});pe(p,A.map(D=>D.cardId)),A.forEach(D=>{const q=(_[D._role]||[]).find(P=>P.cardId===D.cardId);q&&(q.used=!0)}),y["selected_"+p]=[],ae();const z=[...y.log||[]];if(L){const D=(y[p+"Score"]||0)+1,q=A.map(re=>histPlayer(re));z.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:q,homeTotal:M.total,aiTotal:0});const P=(y.round||0)+1,W=p==="p1"?"p2":"p1",X={...y,[p+"Team"]:_,[p+"Score"]:D},J=he(X);N.add(P),st(q,D,y[m+"Score"]||0,!0,async()=>{await j({[p+"Team"]:_,[p+"Score"]:D,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:J?"finished":W+"-attack",attacker:W,round:P,log:z}),J&&await je({...y,[p+"Score"]:D})});return}z.push({type:"pending",text:`⚔️ ${y[p+"Name"]} attaque (${M.total})`}),await j({[p+"Team"]:_,[m+"Team"]:y[m+"Team"],pendingAttack:{...M,players:A,side:p},["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},phase:m+"-defense",log:z})}async function ke(){const _=y[p+"Team"],L=(y["selected_"+p]||[]).map(oe=>{const ue=(_[oe._role]||[]).find(ze=>ze.cardId===oe.cardId)||oe,fe=_[oe._role]||[],xe=fe.findIndex(ze=>ze.cardId===oe.cardId),Z=xt(fe.length),be=xe>=0?Z[xe]:ue._col??1;return{...ue,_line:oe._role,_col:be}}),A=ui(L,y.modifiers[p]||{});pe(p,L.map(oe=>oe.cardId)),L.forEach(oe=>{const ue=(_[oe._role]||[]).find(fe=>fe.cardId===oe.cardId);ue&&(ue.used=!0)}),y["selected_"+p]=[],ae();const M=fi(y.pendingAttack.total,A.total,y.modifiers[p]||{}),z=y.pendingAttack.side,D=M==="attack"||(M==null?void 0:M.goal),q=z==="p1"?"p2":"p1",P=(y.round||0)+1,W=(y.pendingAttack.players||[]).map(oe=>histPlayer(oe)),X=[...y.log||[]];X.push({type:"duel",isGoal:D,homeScored:D&&z===p,text:D?`⚽ BUT de ${y[z+"Name"]} ! (${y.pendingAttack.total} vs ${A.total})`:`✋ Attaque stoppée (${y.pendingAttack.total} vs ${A.total})`,homePlayers:W,aiPlayers:L.map(oe=>histPlayer(oe)),homeTotal:y.pendingAttack.total,aiTotal:A.total});const J=D?(y[z+"Score"]||0)+1:y[z+"Score"]||0,re={...y,[p+"Team"]:_,[z+"Score"]:J},Q=he(re),ce=Q?"finished":q+"-attack",se=async()=>{await j({[p+"Team"]:_,[m+"Team"]:y[m+"Team"],[z+"Score"]:J,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:ce,attacker:q,round:P,log:X}),(ce==="finished"||Q)&&await je({...y,[z+"Score"]:J})};if(D){const oe=z===p,ue=oe?J:y[p+"Score"]||0,fe=oe?y[m+"Score"]||0:J;N.add(P),st(W,ue,fe,oe,se)}else await se()}function we(_){return["MIL","ATT"].some(L=>(_[L]||[]).some(A=>!A.used))}function me(_){return["GK","DEF","MIL","ATT"].some(L=>(_[L]||[]).some(A=>!A.used))}function Le(_){return me(_)&&!we(_)}function $e(_){const L=y[_+"Team"],A=y[(_==="p1"?"p2":"p1")+"Team"];return!!(we(L)||!me(A)&&Le(L))}function he(_){const L=["MIL","ATT"].some(X=>(_.p1Team[X]||[]).some(J=>!J.used)),A=["MIL","ATT"].some(X=>(_.p2Team[X]||[]).some(J=>!J.used)),M=me(_.p1Team),z=me(_.p2Team);return!L&&!(!z&&M)&&(!A&&!(!M&&z))}function Ce(_){const L=_.p1Score||0,A=_.p2Score||0;return L===A?null:L>A?h.home_id:h.away_id}async function je(_){try{const L=Ce(_),A=L?h.home_id===L?h.away_id:h.home_id:null;await w.from("matches").update({status:"finished",winner_id:L,home_score:_.p1Score||0,away_score:_.p2Score||0}).eq("id",i),L&&A&&updateEvolutiveCards(L,A).catch(()=>{})}catch(L){console.error("[PvP] finishMatch:",L)}}function Fe(){var z;const _=y[p+"Score"],L=y[m+"Score"],A=_>L,M=_===L;He(e),e.innerHTML=`
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${g.map((j,I)=>{const k=f>0&&r.status==="finished"?I===0?Math.floor(f*.7):I===1?Math.floor(f*.2):I===2?Math.floor(f*.1):0:0;return`<tr style="border-bottom:1px solid #f5f5f5;${j.userId===o?"background:#f0fdf4;":""}">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,r.status==="active"?r.current_day-1:r.current_day)},(j,I)=>I+1).reverse().map(j=>{const I=(d||[]).filter(k=>k.matchday===j);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${j}</div>${I.map(k=>Di(k,u,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let d="buy";const l=()=>{var p,m,b,v,h,$,y;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((m=document.getElementById("flt-club"))==null?void 0:m.value)||"").toLowerCase().trim(),country:(((b=document.getElementById("flt-country"))==null?void 0:b.value)||"").toLowerCase().trim(),job:((v=document.getElementById("flt-job"))==null?void 0:v.value)||"",rarity:((h=document.getElementById("flt-rarity"))==null?void 0:h.value)||"",note1:parseInt(($=document.getElementById("flt-note1"))==null?void 0:$.value)||0,note2:parseInt((y=document.getElementById("flt-note2"))==null?void 0:y.value)||0}};function c(p){const m=l();return p.filter(b=>{var N,te,ne;const v=(N=b.card)==null?void 0:N.player;if(!v)return!1;const h=`${v.firstname} ${v.surname_real}`.toLowerCase(),$=(((te=v.clubs)==null?void 0:te.encoded_name)||"").toLowerCase(),y=(v.country_code||"").toLowerCase(),C=((ne=b.card)==null?void 0:ne.evolution_bonus)||0,E=ct(v,v.job,C),G=v.job2?ct(v,v.job2,C):0;return!(m.name&&!h.includes(m.name)||m.club&&!$.includes(m.club)||m.country&&!y.includes(m.country)||m.job&&v.job!==m.job||m.rarity&&v.rarity!==m.rarity||m.note1&&E<m.note1||m.note2&&G<m.note2)})}function u(p){var G,N,te,ne;const m=(G=p.card)==null?void 0:G.player;if(!m)return"";const b=((N=p.card)==null?void 0:N.evolution_bonus)||0,v=ct(m,m.job,b),h=m.job2?ct(m,m.job2,b):0,$=(i.profile.credits||0)>=p.price,y=m.country_code?`https://flagsapi.com/${m.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,C=Dt[m.job]||"#bbb",E=m.job2?Dt[m.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
      ${y?`<img src="${y}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      <!-- Logo club -->
      ${(te=m.clubs)!=null&&te.logo_url?`<img src="${m.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <!-- Notes -->
    ...
    </div>`}function g(p){var G,N,te,ne;const m=(G=p.card)==null?void 0:G.player;if(!m)return"";const b=((N=p.card)==null?void 0:N.evolution_bonus)||0,v=ct(m,m.job,b),h=m.job2?ct(m,m.job2,b):0,$=p.status==="sold",y=m.country_code?`https://flagsapi.com/${m.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,C=Dt[m.job]||"#bbb",E=m.job2?Dt[m.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${$?"opacity:0.7":""}">
      ${y?`<img src="${y}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(te=m.clubs)!=null&&te.logo_url?`<img src="${m.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${C};display:flex;align-items:center;justify-content:center">
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const b=r||[];f.innerHTML=b.length>0?b.map((v,h)=>{const $=di(v.mmr??1e3),y=(v.ranked_wins||0)+(v.ranked_losses||0)+(v.ranked_draws||0),C=y>0?Math.round((v.ranked_wins||0)/y*100):0,E=v.id===i.profile.id,G=(v.placement_matches||0)<10;return`
