  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      ...
      <div style="background:#fff3cd;border-radius:10px;padding:10px 14px;width:100%">
        ...
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const W of Object.keys(I)){const de=F(W);if(!de)continue;const re=W.replace(/\d+/,""),N=o[re]||"#555";j+=`<circle cx="${de.x}" cy="${de.y}" r="${C}" fill="${N}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,j+=`<text x="${de.x}" y="${de.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${re}</text>`}return j+="</svg>",j}const b=t.filter(I=>I.formation===n);b.length;const u=!e.is_for_sale;s(`Formation ${n}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${m}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${l?`<img src="${l}" style="width:${m-4}px;height:${m-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,m-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
        ...
        stroke="${re}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const u=48,A=64,r=13,p=16,I={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[H,C]of Object.entries(n)){const F=m(H);if(!F||!C)continue;const j=H.replace(/[0-9]/g,""),W=Mt[j]||"#555",de=a.includes(C.cardId),re=i==="attack"&&(["MIL","ATT"].includes(j)||de)&&!C.used||i==="defense"&&["GK","DEF","MIL"].includes(j)&&!C.used,N=s.includes(C.cardId);let X;i==="attack"?X=de?Math.max(1,Number(C.note_a)||0):j==="MIL"?Number(C.note_m)||0:Number(C.note_a)||0:i==="defense"?X=j==="GK"?Number(C.note_g)||0:j==="MIL"?Number(C.note_m)||0:Number(C.note_d)||0:X=Number(j==="GK"?C.note_g:j==="DEF"?C.note_d:j==="MIL"?C.note_m:C.note_a)||0,X=X+(C.boost||0),C.stadiumBonus&&(i==="attack"&&(j==="ATT"||j==="MIL")||i==="defense"&&(j==="GK"||j==="DEF"||j==="MIL")?X+=10:i||(X+=10));const U=(F.x-u/2).toFixed(1),P=(F.y-A/2).toFixed(1),ne=I[C==null?void 0:C.rarity]||I.normal;if(C.used){const Ae=typeof import.meta<"u"&&"/"||"/",xe=`${typeof window<"u"&&((B=window.location)==null?void 0:B.origin)||""}${Ae}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");b+=`<rect x="${U}" y="${P}" width="${u}" height="${A}" rx="5" fill="#161616"/>`,b+=`<image href="${xe}" xlink:href="${xe}" x="${U}" y="${P}" width="${u}" height="${A}" preserveAspectRatio="xMidYMid slice"/>`,b+=`<rect x="${U}" y="${P}" width="${u}" height="${A}" rx="5" fill="none" stroke="${ne}" stroke-width="2" opacity="0.7"/>`,b+=`<rect x="${U}" y="${P}" width="${u}" height="${A}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${C.cardId}" data-role="${j}" style="cursor:pointer"/>`;continue}const pe=N?.45:1,ye=N?"#FFD700":ne,Se=N?3:(C==null?void 0:C.rarity)==="legende"||(C==null?void 0:C.rarity)==="légende"||(C==null?void 0:C.rarity)==="pepite"||(C==null?void 0:C.rarity)==="pépite"?2.5:2,we=A-r-p;b+=`<defs><clipPath id="cp-${H}"><rect x="${U}" y="${(F.y-A/2+r).toFixed(1)}" width="${u}" height="${we}"/></clipPath></defs>`,b+=`<rect x="${U}" y="${P}" width="${u}" height="${A}" rx="5" fill="${W}" opacity="${pe}"/>`;const ve=Pe(C);ve&&(b+=`<image href="${ve}" xlink:href="${ve}" x="${U}" y="${(F.y-A/2+r).toFixed(1)}" width="${u}" height="${we}" clip-path="url(#cp-${H})" preserveAspectRatio="xMidYMin slice"/>`),b+=`<rect x="${U}" y="${P}" width="${u}" height="${r}" rx="4" fill="rgba(255,255,255,0.92)"/>`,b+=`<text x="${F.x.toFixed(1)}" y="${(F.y-A/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(C.name||"").slice(0,9)}</text>`;const _e=(F.y+A/2-p).toFixed(1);b+=`<rect x="${U}" y="${_e}" width="${u}" height="${p}" fill="rgba(255,255,255,0.92)"/>`;{const Ae=Zt(C.country_code);Ae?b+=`<image href="${Ae}" xlink:href="${Ae}" x="${(F.x-20).toFixed(1)}" y="${(F.y+A/2-p+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:b+=`<text x="${(F.x-13).toFixed(1)}" y="${(F.y+A/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${bo(C.country_code)}</text>`;const Ie=C.stadiumBonus?"#E87722":"#111";b+=`<text x="${F.x.toFixed(1)}" y="${(F.y+A/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="${Ie}" font-family="Arial Black">${X}</text>`;const xe=yo(C);xe?b+=`<image href="${xe}" xlink:href="${xe}" x="${(F.x+u/2-14).toFixed(1)}" y="${(F.y+A/2-p+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:C.clubName&&(b+=`<text x="${(F.x+14).toFixed(1)}" y="${(F.y+A/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(C.clubName||"").slice(0,3).toUpperCase()}</text>`),C.stadiumBonus&&(b+=`<rect x="${(F.x-u/2).toFixed(1)}" y="${(F.y-A/2).toFixed(1)}" width="${u}" height="${A}" rx="5" fill="none" stroke="#E87722" stroke-width="2" opacity="0.8"/>`)}b+=`<rect x="${U}" y="${P}" width="${u}" height="${A}" rx="5" fill="${N?"rgba(255,255,255,0.12)":"none"}" stroke="${ye}" stroke-width="${Se}" opacity="${pe}"/>`,re&&(b+=`<rect x="${U}" y="${P}" width="${u}" height="${A}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${N?"selected":""}" data-card-id="${C.cardId}" data-role="${j}" style="cursor:pointer"/>`)}const L=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-L} ${-L} ${l+L*2} ${d+L*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${b}
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(U,P)=>P+1).reverse().map(U=>{const P=(g||[]).filter(ne=>ne.matchday===U);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${U}</div>${P.map(ne=>Ri(ne,m,d,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
