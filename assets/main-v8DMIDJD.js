import{s as E,l as Qe,j as Ti,F as oi,h as St,i as Qi,k as Zi,b as en}from"./formation-links-CEzN0rcz.js";function mi(e,{navigate:t,toast:n}){e.innerHTML=`
  ...
  `;function l(){var k;const i=document.getElementById("logo-preview"),a=document.getElementById("logo-initials"),f=((k=document.getElementById("setup-club"))==null?void 0:k.value)||"MW",m=f.trim().split(" ").filter(Boolean),x=m.length>=2?(m[0][0]+m[1][0]).toUpperCase():f.slice(0,2).toUpperCase();i&&(i.style.background=d,i.style.borderColor=s),a&&(a.textContent=x,a.style.color=s)}document.getElementById("color1").addEventListener("input",i=>{s=i.target.value,document.getElementById("swatch1").style.background=s,l()}),document.getElementById("color2").addEventListener("input",i=>{d=i.target.value,document.getElementById("swatch2").style.background=d,l()});function c(i){document.querySelectorAll(".setup-step").forEach(a=>a.classList.remove("active")),document.getElementById(`step-${i}`).classList.add("active"),document.getElementById("step-num").textContent=i,document.getElementById("progress-fill").style.width=`${Math.round(i/3*100)}%`,i===3&&l()}document.getElementById("step1-next").addEventListener("click",async()=>{const i=document.getElementById("setup-pseudo").value.trim(),a=document.getElementById("step1-error");if(a.textContent="",i.length<3){a.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await E.from("users").select("id").eq("pseudo",i).maybeSingle();if(f){a.textContent="Ce pseudo est déjà pris.";return}c(2)}),document.getElementById("step2-back").addEventListener("click",()=>c(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const i=document.getElementById("setup-club").value.trim(),a=document.getElementById("step2-error");if(a.textContent="",i.length<2){a.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await E.from("users").select("id").eq("club_name",i).maybeSingle();if(f){a.textContent="Ce nom de club est déjà pris.";return}c(3)}),document.getElementById("step3-back").addEventListener("click",()=>c(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const i=document.getElementById("setup-pseudo").value.trim(),a=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),m=document.getElementById("step3-finish");f.textContent="",m.disabled=!0,m.textContent="Création en cours…";try{const{error:x}=await E.from("users").insert({id:t.user.id,pseudo:i,club_name:a,club_color1:s,club_color2:d,credits:1e4});if(x)throw x;await nn(t.user.id),await p(),r(`Bienvenue ${i} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(x){f.textContent=x.message,m.disabled=!1,m.textContent="🚀 Créer mon profil !"}})}async function nn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await E.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(n){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",n)}}const Lt="#1A6B3C",zt="#cc2222",on="#D4A017",xi="#888";async function rn(e,t){const{state:n,toast:r}=t;e.innerHTML=`
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          ...
          clubs(encoded_name,logo_url)))`).in("deck_id",d).order("slot_order");let c=0;function i(){var M,j,O,F,S;const a=s[c],f=(l||[]).filter(C=>C.deck_id===a.id),m=f.filter(C=>{var Z;return C.is_starter&&((Z=C.card)==null?void 0:Z.player)}).map(C=>Xt(C.card,C.position)),x=f.find(C=>{var Z;return((Z=C.card)==null?void 0:Z.card_type)==="formation"}),k=a.formation||((M=x==null?void 0:x.card)==null?void 0:M.formation)||"4-4-2",o=m.length>=11?pt(m,k):null,A=m.length>=11;We(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${s.map((C,Z)=>`<div style="width:7px;height:7px;border-radius:50%;background:${Z===c?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
    ...
    </div>`,function(){const Z=e.querySelector(".deck-preview-wrap svg");Z&&(Z.removeAttribute("width"),Z.removeAttribute("height"),Z.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",Z.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(j=document.getElementById("prev-deck"))==null||j.addEventListener("click",()=>{c>0&&(c--,i())}),(O=document.getElementById("next-deck"))==null||O.addEventListener("click",()=>{c<s.length-1&&(c++,i())}),(F=document.getElementById("validate-deck"))==null||F.addEventListener("click",()=>{if(!A)return;const C=t.state.params||{};t.navigate("match",{...C,matchMode:C.matchMode||n,deckId:a.id})}),(S=document.getElementById("cancel-deck-select"))==null||S.addEventListener("click",()=>{Ne(e),p("home")});const z=document.getElementById("deck-swipe-zone");if(z){let C=0,Z=0;z.addEventListener("touchstart",ae=>{C=ae.touches[0].clientX,Z=ae.touches[0].clientY},{passive:!0}),z.addEventListener("touchend",ae=>{const se=ae.changedTouches[0].clientX-C,B=ae.changedTouches[0].clientY-Z;Math.abs(se)<40||Math.abs(se)<Math.abs(B)||(se<0&&c<s.length-1?(c++,i()):se>0&&c>0&&(c--,i()))},{passive:!0})}}i()}function Si(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function Ue(e,t=44,n=58){if(!e)return`<div style="width:${t}px;height:${n}px;border:1.5px dashed rgba(255,255,255,0.2);border-radius:5px"></div>`;const r=typeof e.portrait=="string"&&e.portrait.startsWith("http")?e.portrait:Ie(e),p=Si(e),s=e._line||e.job||"MIL",d=ct[s]||"#555",l={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",c=e.note!==void 0?Number(e.note)||0:(Number(be(e,s))||0)+(e.boost||0),i=Ci(e==null?void 0:e.country_code),a=Math.round(n*.19),f=Math.round(n*.25),m=n-Math.round(n*.19)-Math.round(n*.25),x=e!=null&&e.used?.28:1;return`<div style="width:${t}px;height:${n}px;border-radius:5px;border:2px solid ${l};background:${d};position:relative;overflow:hidden;flex-shrink:0;opacity:${x}">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${f}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${p?`<img src="${p}" style="width:${f-4}px;height:${f-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,f-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
        ...
        stroke="${se}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const x=48,k=64,o=13,A=16,z={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[O,F]of Object.entries(i)){const S=f(O);if(!S||!F)continue;const C=O.replace(/[0-9]/g,""),Z=ct[C]||"#555",ae=d.includes(F.cardId),se=n==="attack"&&(["MIL","ATT"].includes(C)||ae)&&!F.used||n==="defense"&&["GK","DEF","MIL"].includes(C)&&!F.used,B=r.includes(F.cardId);let Y;n==="attack"?Y=ae?Math.max(1,Number(F.note_a)||0):C==="MIL"?Number(F.note_m)||0:Number(F.note_a)||0:n==="defense"?Y=C==="GK"?Number(F.note_g)||0:C==="MIL"?Number(F.note_m)||0:Number(F.note_d)||0:Y=Number(C==="GK"?F.note_g:C==="DEF"?F.note_d:C==="MIL"?F.note_m:F.note_a)||0,Y=Y+(F.boost||0);const te=(S.x-x/2).toFixed(1),D=(S.y-k/2).toFixed(1),Q=z[F==null?void 0:F.rarity]||z.normal;if(F.used){const N=typeof import.meta<"u"&&"/manager-wars/"||"/",P=`${typeof window<"u"&&((j=window.location)==null?void 0:j.origin)||""}${N}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");m+=`<rect x="${te}" y="${D}" width="${x}" height="${k}" rx="5" fill="#161616"/>`,m+=`<image href="${P}" xlink:href="${P}" x="${te}" y="${D}" width="${x}" height="${k}" preserveAspectRatio="xMidYMid slice"/>`,m+=`<rect x="${te}" y="${D}" width="${x}" height="${k}" rx="5" fill="none" stroke="${Q}" stroke-width="2" opacity="0.7"/>`,m+=`<rect x="${te}" y="${D}" width="${x}" height="${k}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${F.cardId}" data-role="${C}" style="cursor:pointer"/>`;continue}const K=B?.45:1,re=B?"#FFD700":Q,ge=B?3:(F==null?void 0:F.rarity)==="legende"||(F==null?void 0:F.rarity)==="légende"||(F==null?void 0:F.rarity)==="pepite"||(F==null?void 0:F.rarity)==="pépite"?2.5:2,me=k-o-A;m+=`<defs><clipPath id="cp-${O}"><rect x="${te}" y="${(S.y-k/2+o).toFixed(1)}" width="${x}" height="${me}"/></clipPath></defs>`,m+=`<rect x="${te}" y="${D}" width="${x}" height="${k}" rx="5" fill="${Z}" opacity="${K}"/>`;const xe=Ie(F);xe&&(m+=`<image href="${xe}" xlink:href="${xe}" x="${te}" y="${(S.y-k/2+o).toFixed(1)}" width="${x}" height="${me}" clip-path="url(#cp-${O})" preserveAspectRatio="xMidYMin slice"/>`),m+=`<rect x="${te}" y="${D}" width="${x}" height="${o}" rx="4" fill="rgba(255,255,255,0.92)"/>`,m+=`<text x="${S.x.toFixed(1)}" y="${(S.y-k/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(F.name||"").slice(0,9)}</text>`;const R=(S.y+k/2-A).toFixed(1);m+=`<rect x="${te}" y="${R}" width="${x}" height="${A}" fill="rgba(255,255,255,0.92)"/>`;{const N=Jt(F.country_code);N?m+=`<image href="${N}" xlink:href="${N}" x="${(S.x-20).toFixed(1)}" y="${(S.y+k/2-A+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:m+=`<text x="${(S.x-13).toFixed(1)}" y="${(S.y+k/2-A/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${Ci(F.country_code)}</text>`,m+=`<text x="${S.x.toFixed(1)}" y="${(S.y+k/2-A/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${Y}</text>`;const W=Si(F);W?m+=`<image href="${W}" xlink:href="${W}" x="${(S.x+x/2-14).toFixed(1)}" y="${(S.y+k/2-A+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:F.clubName&&(m+=`<text x="${(S.x+14).toFixed(1)}" y="${(S.y+k/2-A/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(F.clubName||"").slice(0,3).toUpperCase()}</text>`),F.boost&&(m+=`<rect x="${(S.x+x/2-12).toFixed(1)}" y="${(S.y-k/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,m+=`<text x="${(S.x+x/2-5).toFixed(1)}" y="${(S.y-k/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${F.boost}</text>`)}m+=`<rect x="${te}" y="${D}" width="${x}" height="${k}" rx="5" fill="${B?"rgba(255,255,255,0.12)":"none"}" stroke="${re}" stroke-width="${ge}" opacity="${K}"/>`,se&&(m+=`<rect x="${te}" y="${D}" width="${x}" height="${k}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${B?"selected":""}" data-card-id="${F.cardId}" data-role="${C}" style="cursor:pointer"/>`)}const M=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-M} ${-M} ${p+M*2} ${s+M*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${m}
  ...
  <div style="text-align:center;flex-shrink:0;width:38px">
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const x=async(j=!0)=>{f=!0,m&&(E.removeChannel(m),m=null),j&&await E.rpc("cancel_matchmaking",{p_user_id:c.profile.id}).catch(()=>{})};(M=document.getElementById("mm-cancel"))==null||M.addEventListener("click",async()=>{try{await x(!0)}catch{}Ne(e),i("home")});const k=async(j,O,F)=>{if(f)return;f=!0,m&&(E.removeChannel(m),m=null);const S=document.getElementById("mm-status");S&&(S.textContent="Adversaire trouvé !"),await new Promise(C=>setTimeout(C,600)),e.isConnected&&ji(e,t,j,F,d,l)},{data:o,error:A}=await E.rpc("try_matchmake",{p_user_id:c.profile.id,p_deck_id:n});if(A||!(o!=null&&o.success)){a("Erreur de matchmaking","error"),Ne(e),i("home");return}if(o.matched){k(o.match_id,o.opponent_id,!1);return}const z=document.getElementById("mm-status");z&&(z.textContent="En attente d'un autre joueur..."),m=E.channel("matchmaking-"+c.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${c.profile.id}`},j=>{const O=j.new;O.status==="matched"&&O.match_id&&k(O.match_id,O.matched_with,!0)}).subscribe()}async function gn(e,t,n){const{state:r,navigate:p,toast:s}=t;try{(E.getChannels?E.getChannels():[]).forEach(i=>{const a=i.topic||"";(a.includes("matchmaking")||a.includes("pvp-match")||a.includes("friend-invite"))&&E.removeChannel(i)})}catch{}const{data:d}=await E.from("matches").select("home_id, away_id, status").eq("id",n).single();if(!d){s("Match introuvable","error"),p("home");return}if(d.status==="finished"){s("Ce match est terminé","info"),p("home");return}const l=d.home_id===r.profile.id;ji(e,t,n,l,[],[])}async function ji(e,t,n,r,p=[],s=[]){const{state:d,navigate:l,toast:c}=t,i=r?"p1":"p2",a=r?"p2":"p1",f=(p||[]).map(u=>u.id),m=(p||[]).map(u=>({id:u.id,gc_type:u.gc_type,_gcDef:u._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:x}=await E.from("matches").select("*").eq("id",n).single();if(!x){c("Match introuvable","error"),l("home");return}async function k(){const[{data:u},{data:g},{data:b},{data:h}]=await Promise.all([E.rpc("get_deck_for_match",{p_deck_id:x.home_deck_id}),E.rpc("get_deck_for_match",{p_deck_id:x.away_deck_id}),E.from("users").select("id,pseudo,club_name").eq("id",x.home_id).single(),E.from("users").select("id,pseudo,club_name").eq("id",x.away_id).single()]),y=$=>({cardId:$.card_id,position:$.position,id:$.id,firstname:$.firstname,name:$.surname_encoded,country_code:$.country_code,club_id:$.club_id,job:$.job,job2:$.job2,note_g:Number($.note_g)||0,note_d:Number($.note_d)||0,note_m:Number($.note_m)||0,note_a:Number($.note_a)||0,rarity:$.rarity,skin:$.skin,hair:$.hair,hair_length:$.hair_length,clubName:$.club_encoded_name||null,clubLogo:$.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),v=((u==null?void 0:u.starters)||[]).map($=>y($)),w=((g==null?void 0:g.starters)||[]).map($=>y($)),_=(u==null?void 0:u.formation)||"4-4-2",I=(g==null?void 0:g.formation)||"4-4-2";return{p1Team:pt(v,_),p2Team:pt(w,I),p1Subs:((u==null?void 0:u.subs)||[]).map($=>y($)),p2Subs:((g==null?void 0:g.subs)||[]).map($=>y($)),p1Formation:_,p2Formation:I,p1Name:(b==null?void 0:b.club_name)||(b==null?void 0:b.pseudo)||"Joueur 1",p2Name:(h==null?void 0:h.club_name)||(h==null?void 0:h.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:r?f:[],gc_p2:r?[]:f,gcCardsFull_p1:r?m:[],gcCardsFull_p2:r?[]:m,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let o=x.game_state&&Object.keys(x.game_state).length?x.game_state:null;if(!o)if(r){o=await k();const{data:u}=await E.from("matches").select("game_state").eq("id",n).single();!(u!=null&&u.game_state)||!Object.keys(u.game_state).length?await E.from("matches").update({game_state:o,turn_user_id:x.home_id}).eq("id",n):o=u.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let u=0;u<30&&!o;u++){await new Promise(b=>setTimeout(b,400));const{data:g}=await E.from("matches").select("game_state").eq("id",n).single();g!=null&&g.game_state&&Object.keys(g.game_state).length&&(o=g.game_state)}if(!o){c("Erreur de synchronisation","error"),l("home");return}o.gc_p2=f,o.gcCardsFull_p2=m,await E.from("matches").update({game_state:o}).eq("id",n)}let A=!1,z=null,M=!1;const j=new Set,O=new Set;function F(u){var $,L;try{E.removeChannel(S)}catch{}const g=o[i+"Score"]||0,b=o[a+"Score"]||0;let h,y;u.winner_id?(h=u.winner_id===d.profile.id,y=!1):u.forfeit?(h=g>b,y=!1):(y=g===b,h=g>b),($=document.getElementById("pvp-end-overlay"))==null||$.remove();const v=document.createElement("div");v.id="pvp-end-overlay",v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const w=y?"🤝":h?"🏆":"😞",_=y?"MATCH NUL":h?"VICTOIRE !":"DÉFAITE",I=y?"#fff":h?"#FFD700":"#ff6b6b";v.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(v),(L=v.querySelector("#pvp-end-home"))==null||L.addEventListener("click",()=>{v.remove(),Ne(e),l("home")})}const S=E.channel("pvp-match-"+n).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${n}`},u=>{const g=u.new;try{if(g.status==="finished"||g.forfeit){if(A)return;A=!0,z&&(clearInterval(z),z=null),g.game_state&&(o=g.game_state),F(g);return}if(g.game_state){const b=o;o=g.game_state;const h=o._lastGC;if(h&&h.seq&&!O.has(h.seq)&&(O.add(h.seq),h.by!==i)){me(h.type,h.by,()=>D());return}const y=b[i+"Score"]||0,v=b[a+"Score"]||0,w=o[i+"Score"]||0,_=o[a+"Score"]||0,I=w>y,$=_>v;if((I||$)&&!j.has(o.round)){j.add(o.round);const L=[...o.log||[]].reverse().find(U=>U.isGoal),q=((L==null?void 0:L.homePlayers)||[]).map(U=>({name:U.name,note:U.note,portrait:U.portrait,job:U.job}));ge(q,w,_,I,()=>D());return}D()}}catch(b){console.error("[PvP] crash:",b)}}).subscribe();async function C(u){Object.assign(o,u),o.lastActionAt=new Date().toISOString();const{error:g}=await E.from("matches").update({game_state:o}).eq("id",n);g&&c("Erreur de synchronisation","error");try{D()}catch(b){console.error("[PvP] renderPvpScreen crash:",b)}}async function Z(){if(A)return;A=!0,z&&(clearInterval(z),z=null);const u=r?x.away_id:x.home_id,g=r?"p2":"p1",b=r?"p1":"p2",h={...o,[g+"Score"]:3,[b+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:u,home_score:h.p1Score||0,away_score:h.p2Score||0,game_state:h}).eq("id",n)}catch{}try{E.removeChannel(S)}catch{}setTimeout(()=>{Ne(e),l("home")},800)}const ae={BOOST_STAT:({value:u=1,count:g=1,roles:b=[]},h,y)=>{const v=h[i+"Team"],w=Object.entries(v).filter(([_])=>!b.length||b.includes(_)).flatMap(([_,I])=>I.filter($=>!$.used).map($=>({...$,_line:_})));if(!w.length){h.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),y(h);return}se(w,g,`Choisir ${g} joueur(s) à booster (+${u})`,_=>{_.forEach(I=>{const $=(v[I._line]||[]).find(L=>L.cardId===I.cardId);$&&($.boost=($.boost||0)+u,h.log.push({text:`⚡ +${u} sur ${$.name}`,type:"info"}))}),h[i+"Team"]=v,y(h)})},DEBUFF_STAT:({value:u=1,count:g=1,roles:b=[],target:h="ai"},y,v)=>{const w=h==="home"?i:a,_=y[w+"Team"],I=h==="home"?"allié":"adverse",$=Object.entries(_).filter(([L])=>!b.length||b.includes(L)).flatMap(([L,q])=>q.filter(U=>!U.used).map(U=>({...U,_line:L})));if(!$.length){y.log.push({text:`🎯 Aucun joueur ${I}`,type:"info"}),v(y);return}se($,g,`Choisir ${g} joueur(s) ${I}(s) (-${u})`,L=>{L.forEach(q=>{const U=(_[q._line]||[]).find(oe=>oe.cardId===q.cardId);U&&(U.boost=(U.boost||0)-u,y.log.push({text:`🎯 -${u} sur ${U.name}`,type:"info"}))}),y[w+"Team"]=_,v(y)})},GRAY_PLAYER:({count:u=1,roles:g=[],target:b="ai"},h,y)=>{const v=b==="home"?i:a,w=h[v+"Team"],_=b==="home"?"allié":"adverse",I=Object.entries(w).filter(([$])=>!g.length||g.includes($)).flatMap(([$,L])=>L.filter(q=>!q.used).map(q=>({...q,_line:$})));if(!I.length){h.log.push({text:`❌ Aucun joueur ${_}`,type:"info"}),y(h);return}se(I,u,`Choisir ${u} joueur(s) ${_}(s) à exclure`,$=>{$.forEach(L=>{const q=(w[L._line]||[]).find(U=>U.cardId===L.cardId);q&&(q.used=!0,h.log.push({text:`❌ ${q.name} exclu !`,type:"info"}))}),h[v+"Team"]=w,y(h)})},REVIVE_PLAYER:({count:u=1,roles:g=[]},b,h)=>{const y=b[i+"Team"],v=Object.entries(y).filter(([w])=>!g.length||g.includes(w)).flatMap(([w,_])=>_.filter(I=>I.used).map(I=>({...I,_line:w})));if(!v.length){b.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),h(b);return}se(v,u,`Choisir ${u} joueur(s) à ressusciter`,w=>{w.forEach(_=>{const I=(y[_._line]||[]).find($=>$.cardId===_.cardId);I&&(I.used=!1,b.log.push({text:`💫 ${I.name} ressuscité !`,type:"info"}))}),b[i+"Team"]=y,h(b)})},REMOVE_GOAL:({},u,g)=>{const b=a+"Score";u[b]>0?(u[b]--,u.log.push({text:"🚫 Dernier but annulé !",type:"info"})):u.log.push({text:"🚫 Aucun but à annuler",type:"info"}),g(u)},ADD_GOAL_DRAW:({},u,g)=>{u[i+"Score"]===u[a+"Score"]?(u[i+"Score"]++,u.log.push({text:"🎯 But bonus !",type:"info"})):u.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),g(u)},ADD_SUB:({value:u=1},g,b)=>{g.maxSubs=(g.maxSubs||3)+u,g.log.push({text:`🔄 +${u} remplacement(s)`,type:"info"}),b(g)},CUSTOM:({},u,g)=>g(u)};function se(u,g,b,h){const y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let v=[];function w(){var I,$;const _=u.map(L=>{const q={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[L._line]||"#555",U=be(L,L._line)+(L.boost||0),ce=v.find(H=>H.cardId===L.cardId)?"#FFD700":"rgba(255,255,255,0.25)",G=L.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${L.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${ce};background:${q};overflow:hidden;cursor:pointer;${G}">
        ...
        </div>`,(I=y.querySelector("#pp-close"))==null||I.addEventListener("click",()=>y.remove()),y.querySelectorAll(".pp-item").forEach(L=>{L.addEventListener("click",()=>{const q=L.dataset.cid,U=u.find(ce=>ce.cardId===q),oe=v.findIndex(ce=>ce.cardId===q);U&&(oe>-1?v.splice(oe,1):v.length<g&&v.push(U),w())})}),($=y.querySelector("#pp-confirm"))==null||$.addEventListener("click",()=>{y.remove(),h(v)})}w(),document.body.appendChild(y)}async function B(u,g){const h=(o["gcCardsFull_"+i]||[]).find(_=>_.id===u),y=(h==null?void 0:h._gcDef)||(o.gcDefs||[]).find(_=>{var I;return _.name===g||((I=_.name)==null?void 0:I.toLowerCase().trim())===(g==null?void 0:g.toLowerCase().trim())}),v=[...o["usedGc_"+i]||[],u],w={type:g,by:i,seq:(o._gcAnimSeq||0)+1};O.add(w.seq),me(g,i,async()=>{if(y!=null&&y.effect_type&&y.effect_type!=="CUSTOM"){const I=ae[y.effect_type];if(I){const $={...o};I(y.effect_params||{},$,async L=>{L["usedGc_"+i]=v,L._lastGC=w,L._gcAnimSeq=w.seq,await C(L)});return}}const _={...o};switch(g){case"Remplacement+":_.maxSubs=(_.maxSubs||3)+1,_.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const I=a+"Score";_[I]>0&&(_[I]--,_.log.push({text:"🚫 But annulé",type:"info"}));break}}_["usedGc_"+i]=v,_._lastGC=w,_._gcAnimSeq=w.seq,await C(_)})}function Y(u,g){const b="usedCardIds_"+u,h=new Set(o[b]||[]);g.forEach(y=>h.add(y)),o[b]=[...h]}function te(){for(const u of["p1","p2"]){const g=new Set(o["usedCardIds_"+u]||[]),b=o[u+"Team"];if(b)for(const h of["GK","DEF","MIL","ATT"])(b[h]||[]).forEach(y=>{y.used=g.has(y.cardId)})}}function D(){var Oe,ft,gt,mt,xt,yt;if(o.phase==="reveal")return Q();if(o.phase==="midfield")return re();if(o.phase==="finished")return Ke();const u=o[i+"Team"],g=o[a+"Team"];te();const b=o[i+"Score"],h=o[a+"Score"],y=o[i+"Name"],v=o[a+"Name"],w=o.phase===i+"-attack",_=o.phase===i+"-defense",I=Array.isArray(o["selected_"+i])?o["selected_"+i]:[],$=I.map(T=>T.cardId),L=window.innerWidth>=700,q=o[i+"Subs"]||[],U=o["usedSubIds_"+i]||[],oe=q.filter(T=>!U.includes(T.cardId)),ce=o["gcCardsFull_"+i]||[],G=o["usedGc_"+i]||[],H=ce.filter(T=>!G.includes(T.id)),J=o.boostOwner===i&&!o.boostUsed,ne=!["GK","DEF","MIL","ATT"].some(T=>(g[T]||[]).some(V=>!V.used)),de=[...u.MIL||[],...u.ATT||[]].filter(T=>!T.used),fe=w&&ne&&de.length===0?[...u.GK||[],...u.DEF||[]].filter(T=>!T.used).map(T=>T.cardId):[];function le(T,V,pe){var wt,$t;const ee=T._gcDef,He={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ee==null?void 0:ee.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Ye={purple:"#b06ce0",light_blue:"#00d4ef"}[ee==null?void 0:ee.color]||"#b06ce0",Xe=(ee==null?void 0:ee.name)||T.gc_type,Je=(ee==null?void 0:ee.effect)||((wt=Be[T.gc_type])==null?void 0:wt.desc)||"",ht=ee!=null&&ee.image_url?`/manager-wars/icons/${ee.image_url}`:null,Ut=(($t=Be[T.gc_type])==null?void 0:$t.icon)||"⚡",bt=Math.round(pe*.22),vt=Math.round(pe*.22),nt=pe-bt-vt,Kt=Xe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${T.id}" data-gc-type="${T.gc_type}"
    ...
    </div>`,(y=document.getElementById("pvp-home"))==null||y.addEventListener("click",()=>{try{E.removeChannel(S)}catch{}Ne(e),l("home")})}D()}const mn="2026.06.27-2305";async function hi(e,{state:t,navigate:n,toast:r}){var s,d;const p=t.profile;p&&(e.innerHTML=`
  <div class="page">
    <div class="page-body">
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),i=(l||[]).filter(R=>R.card_type==="player"&&R.player),a=(l||[]).filter(R=>R.card_type==="game_changer"),f=(l||[]).filter(R=>R.card_type==="formation"),{data:m}=await E.from("gc_definitions").select("name,gc_type,color,effect,image_url"),x={};(m||[]).forEach(R=>{x[R.name]=R});const k=Object.keys(oi),o=Object.keys(Be),A={};i.forEach(R=>{const N=R.player.id;A[N]=(A[N]||0)+1}),new Set(Object.keys(A).map(R=>String(R)));const z=new Set(f.map(R=>R.formation)),M=new Set(a.map(R=>R.gc_type));let j="player",O="Tous",F="",S=!1;function C(){return[...i].sort((R,N)=>{const W=Ct.indexOf(R.player.job),P=Ct.indexOf(N.player.job);return W!==P?W-P:(R.player.surname_encoded||"").localeCompare(N.player.surname_encoded||"")})}function Z(){return[...c||[]].sort((R,N)=>{const W=Ct.indexOf(R.job),P=Ct.indexOf(N.job);return W!==P?W-P:(R.surname_encoded||"").localeCompare(N.surname_encoded||"")})}function ae(){return C().filter(R=>{const N=R.player,W=O==="Tous"||N.job===O,P=!F||`${N.firstname} ${N.surname_encoded}`.toLowerCase().includes(F);return W&&P})}function se(){return Z().filter(R=>{const N=O==="Tous"||R.job===O,W=!F||`${R.firstname} ${R.surname_encoded}`.toLowerCase().includes(F);return N&&W})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const _t=1e3;function kn(e,t,n,r){var o,A,z;const{state:p,toast:s,closeModal:d,navigate:l,refreshProfile:c}=n,i=e.formation,a={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function f(){const M=St[i]||{},j=oi[i]||[],O=290,F=360,S=20;function C(ae){const se=M[ae];return se?{x:se.x*O,y:se.y*F}:null}let Z=`<svg width="${O}" height="${F}" viewBox="0 0 ${O} ${F}" xmlns="http://www.w3.org/2000/svg">`;for(const[ae,se]of j){const B=C(ae),Y=C(se);!B||!Y||(Z+=`<line x1="${B.x}" y1="${B.y}" x2="${Y.x}" y2="${Y.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const ae of Object.keys(M)){const se=C(ae);if(!se)continue;const B=ae.replace(/\d+/,""),Y=a[B]||"#555";Z+=`<circle cx="${se.x}" cy="${se.y}" r="${S}" fill="${Y}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,Z+=`<text x="${se.x}" y="${se.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${B}</text>`}return Z+="</svg>",Z}const m=t.filter(M=>M.formation===i),x=m.length,k=!e.is_for_sale;r(`Formation ${i}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(o=document.getElementById("direct-sell-form-btn"))==null||o.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${i} pour ${_t.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const M=m.find(O=>!O.is_for_sale)||m[0];if(!M){s("Aucune carte à vendre","error");return}const{error:j}=await E.from("cards").delete().eq("id",M.id);if(j){s(j.message,"error");return}await E.from("users").update({credits:(p.profile.credits||0)+_t}).eq("id",p.profile.id),await c(),s(`+${_t.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),d(),l("collection")}),(A=document.getElementById("market-sell-form-btn"))==null||A.addEventListener("click",async()=>{const M=parseInt(document.getElementById("sell-price-form").value);if(!M||M<1){s("Prix invalide","error");return}await E.from("cards").update({is_for_sale:!0,sale_price:M}).eq("id",e.id),await E.from("market_listings").insert({seller_id:p.profile.id,card_id:e.id,price:M}),s("Carte mise en vente sur le marché !","success"),d(),l("collection")}),(z=document.getElementById("cancel-sell-form-btn"))==null||z.addEventListener("click",async()=>{await E.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await E.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),s("Annonce retirée","success"),d(),l("collection")})}async function vi(e,t,n,r){var se,B,Y,te,D,Q;const{state:p,toast:s,openModal:d,closeModal:l,navigate:c,refreshProfile:i}=r,a=e.player,f=t.filter(K=>K.player.id===a.id),m=f.length,x=wn[a.rarity]||1e3,k=a.rarity!=="legende",o=qi(a),A=Mt(a,a.job),z=a.job2?Mt(a,a.job2):null,M=Tt[a.job]||"#1A6B3C",j=a.job2?Tt[a.job2]:null,O=Di[a.rarity]||"#ccc",F=ci[a.country_code]||a.country_code||"",S=f.map(K=>K.id);let C={};if(S.length){const{data:K}=await E.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",S).order("transferred_at",{ascending:!0});(K||[]).forEach(re=>{C[re.card_id]||(C[re.card_id]=[]),C[re.card_id].push(re)})}const Z=K=>`
      ...
      <div style="width:140px;border-radius:12px;padding:6px;background:${O};flex-shrink:0">
        <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
          ...
          <div style="background:#f2e8d2;padding:3px 6px;display:flex;align-items:center;justify-content:space-between;min-height:24px">
            ...
            ${(se=a.clubs)!=null&&se.logo_url?`<img src="${a.clubs.logo_url}" style="width:22px;height:16px;object-fit:contain">`:`<div style="background:#1a3a7a;color:#fff;border-radius:2px;padding:1px 3px;font-size:6px;font-weight:800">${(((B=a.clubs)==null?void 0:B.encoded_name)||"").slice(0,6)}</div>`}
          </div>
        </div>
      </div>
  ...
  </div>`,An(e,t,p,n),document.getElementById("builder-back").addEventListener("click",()=>r("decks")),document.getElementById("formation-select").addEventListener("change",i=>{t.formation=i.target.value;const a=$i(t.formation),f={};a.forEach(m=>{t.slots[m]&&(f[m]=t.slots[m])}),t.slots=f,ut(e,t,n)}),document.getElementById("save-deck").addEventListener("click",()=>Ln(t,n)),e.querySelectorAll("[data-remove-sub]").forEach(i=>{i.addEventListener("click",()=>{t.subs=t.subs.filter(a=>a!==i.dataset.removeSub),ut(e,t,n)})}),(c=document.getElementById("add-sub-btn"))==null||c.addEventListener("click",()=>{In(t,e,n)})}function An(e,t,n,r){const p=e.querySelector("#deck-field");if(!p)return;const s=St[t.formation]||{},d=Ti(t.formation)||[],l={};for(const z of n){const M=t.slots[z],j=M?t.playerCards.find(O=>O.id===M):null;l[z]=j?j.player:null}const c=300,i=300,a=48,f=64,m=13,x=16,k=38;function o(z){const M=s[z];return M?{x:M.x*c,y:M.y*i}:null}let A="";for(const[z,M]of d){const j=o(z),O=o(M);if(!j||!O)continue;const F=l[z]?{...l[z],club_id:l[z].club_id,country_code:l[z].country_code,rarity:l[z].rarity}:null,S=l[M]?{...l[M],club_id:l[M].club_id,country_code:l[M].country_code,rarity:l[M].rarity}:null,C=Qe(F,S);C==="#ff3333"||C==="#cc2222"?A+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${O.x.toFixed(1)}" y2="${O.y.toFixed(1)}" stroke="${C}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(A+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${O.x.toFixed(1)}" y2="${O.y.toFixed(1)}" stroke="${C}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,A+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${O.x.toFixed(1)}" y2="${O.y.toFixed(1)}" stroke="${C}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const z of n){const M=o(z);if(!M)continue;const j=l[z],O=z.replace(/\d+/,""),F=rt[O]||"#555",S=(M.x-a/2).toFixed(1),C=(M.y-f/2).toFixed(1),Z={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[j==null?void 0:j.rarity]||"#aaa";if(j){const ae=Ot(j),se=Ni(j),B=Gi(j.country_code),Y=Number(O==="GK"?j.note_g:O==="DEF"?j.note_d:O==="MIL"?j.note_m:j.note_a)||0,te=f-m-x;A+=`<defs><clipPath id="dcp-${z}"><rect x="${S}" y="${(M.y-f/2+m).toFixed(1)}" width="${a}" height="${te}"/></clipPath></defs>`,A+=`<rect x="${S}" y="${C}" width="${a}" height="${f}" rx="5" fill="${F}"/>`,ae&&(A+=`<image href="${ae}" x="${S}" y="${(M.y-f/2+m).toFixed(1)}" width="${a}" height="${te}" clip-path="url(#dcp-${z})" preserveAspectRatio="xMidYMin slice"/>`),A+=`<rect x="${S}" y="${C}" width="${a}" height="${m}" fill="rgba(255,255,255,0.93)"/>`,A+=`<text x="${M.x.toFixed(1)}" y="${(M.y-f/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(j.surname_encoded||"").slice(0,9)}</text>`;const D=(M.y+f/2-x).toFixed(1);A+=`<rect x="${S}" y="${D}" width="${a}" height="${x}" fill="rgba(255,255,255,0.93)"/>`,B&&(A+=`<image href="${B}" x="${(M.x-21).toFixed(1)}" y="${(M.y+f/2-x+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),A+=`<text x="${M.x.toFixed(1)}" y="${(M.y+f/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${Y}</text>`,se&&(A+=`<image href="${se}" x="${(M.x+a/2-14).toFixed(1)}" y="${(M.y+f/2-x+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),A+=`<rect x="${S}" y="${C}" width="${a}" height="${f}" rx="5" fill="none" stroke="${Z}" stroke-width="2"/>`}else A+=`<rect x="${S}" y="${C}" width="${a}" height="${f}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,A+=`<text x="${M.x.toFixed(1)}" y="${M.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,A+=`<text x="${M.x.toFixed(1)}" y="${(M.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${O}</text>`;A+=`<rect x="${S}" y="${C}" width="${a}" height="${f}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${z}" style="cursor:pointer"/>`}p.innerHTML=`<svg viewBox="${-k} ${-k} ${c+k*2} ${i+k*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${A}</svg>`,p.querySelectorAll(".deck-slot-hit").forEach(z=>{z.addEventListener("click",()=>Tn(z.dataset.pos,t,e,r))})}function Tn(e,t,n,r){var m,x,k;const{openModal:p,closeModal:s}=r,d=e.replace(/\d+/,""),l=t.slots[e],c=l?t.playerCards.find(o=>o.id===l):null;(m=c==null?void 0:c.player)==null||m.id;const i=new Set;Object.entries(t.slots).forEach(([o,A])=>{var M;if(o===e||!A)return;const z=t.playerCards.find(j=>j.id===A);(M=z==null?void 0:z.player)!=null&&M.id&&i.add(z.player.id)}),t.subs.forEach(o=>{var z;const A=t.playerCards.find(M=>M.id===o);(z=A==null?void 0:A.player)!=null&&z.id&&i.add(A.player.id)});const a=new Set,f=t.playerCards.filter(o=>{const A=o.player;return!(A.job===d||A.job2===d)||i.has(A.id)||a.has(A.id)?!1:(a.add(A.id),!0)});f.sort((o,A)=>{const z=d==="GK"?o.player.note_g:d==="DEF"?o.player.note_d:d==="MIL"?o.player.note_m:o.player.note_a;return(d==="GK"?A.player.note_g:d==="DEF"?A.player.note_d:d==="MIL"?A.player.note_m:A.player.note_a)-z}),p(`Choisir ${d} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${f.length>0?f.map(o=>{var O,F;const A=o.player,z=d==="GK"?A.note_g:d==="DEF"?A.note_d:d==="MIL"?A.note_m:A.note_a,M=Ot(A),j={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[A.rarity];return`<div class="player-option" data-card-id="${o.id}"
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;align-items:center;gap:6px;margin-top:3px">
              ...
              ${(O=A.clubs)!=null&&O.logo_url?`<img src="${A.clubs.logo_url}" style="width:18px;height:18px;object-fit:contain">`:`<span style="font-size:10px;color:var(--gray-600)">${((F=A.clubs)==null?void 0:F.encoded_name)||"—"}</span>`}
              <span style="font-size:10px;color:var(--gray-600)">${A.country_code}</span>
            </div>
          </div>
          <!-- Note -->
          <div style="width:36px;height:36px;border-radius:8px;background:${rt[d]};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:16px;border:2px solid ${j};flex-shrink:0">
            ${z}
          </div>
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${o.map((j,O)=>{const F=O<o.length-1?Qe(j,o[O+1]):null,S=!F||F==="#ff3333"||F==="#cc2222",C=F==="#00ff88"?"+2":F==="#FFD700"?"+1":"";return`
          ...
          ${O<o.length-1?`<div class="duel-link duel-link-${M}" data-idx="${O}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${S?"rgba(255,255,255,0.12)":F};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${S?"none":`0 0 8px ${F}`}">
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,F.style.transition="opacity .45s ease",F.style.opacity="1",F.style.pointerEvents="auto",(S=document.getElementById("start-match-btn"))==null||S.addEventListener("click",k))},600)},700)},2800)}function eo(e){if(e.type==="duel"&&(e.homeTotal!=null||e.aiTotal!=null)){const t=(e.homeTotal||0)>=(e.aiTotal||0);return`
    ...
    <div style="display:flex;align-items:center;gap:4px;${t?"flex-direction:row-reverse":""};background:rgba(255,255,255,0.04);border-radius:8px;padding:5px 8px;border:1px solid rgba(255,255,255,0.07)">
      ...
      ${tt(e.inPlayer||{})}
    </div>`}return`<div style="font-size:11px;color:${e.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${e.type==="goal"?700:400};padding:3px 2px">${e.text||""}</div>`}function ze(e,t,n){var j,O,F,S,C,Z,ae,se;const r=t.selected.map(B=>B.cardId),p=t.usedSubIds||[],s=t.homeSubs.filter(B=>!p.includes(B.cardId)),l=Object.values(t.homeTeam).flat().filter(B=>B.used).length>0&&s.length>0&&t.subsUsed<t.maxSubs,c=!["GK","DEF","MIL","ATT"].some(B=>(t.aiTeam[B]||[]).some(Y=>!Y.used)),i=[...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(B=>!B.used),a=t.phase==="attack"&&c&&i.length===0?[...t.homeTeam.GK||[],...t.homeTeam.DEF||[]].filter(B=>!B.used).map(B=>B.cardId):[];t.log[t.log.length-1];const f=t.phase==="ai-attack"||t.phase==="ai-defense",m=t.phase==="attack",x=t.phase==="defense",k=t.phase==="finished",o=t.gcCards.filter(B=>!t.usedGc.includes(B.id)),A=t.boostCard&&!t.boostUsed;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column",e.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const Y=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
          ...
          </div>`}const B=t.log[t.log.length-1];return B?'<div style="padding:2px 4px">'+eo(B)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(s);let l=!1;const c=()=>{l||(l=!0,s.remove(),setTimeout(()=>p(),50))};s.addEventListener("click",c),setTimeout(c,3500)}async function It(e,t,n){var a,f;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:r}=n,p=t.homeScore>t.aiScore,s=t.homeScore===t.aiScore,d=p?"victoire":s?"nul":"defaite",l=ln(t.mode,d);t.matchId&&await E.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:p?r.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const c={credits:(r.profile.credits||0)+l,matches_played:(r.profile.matches_played||0)+1};p?c.wins=(r.profile.wins||0)+1:s?c.draws=(r.profile.draws||0)+1:c.losses=(r.profile.losses||0)+1,await E.from("users").update(c).eq("id",r.profile.id),await n.refreshProfile();const i=document.createElement("div");i.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",i.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      </div>`,(B=document.getElementById("lobby-cancel"))==null||B.addEventListener("click",async()=>{k=!0,o&&(E.removeChannel(o),o=null),ae&&await E.from("friend_match_invites").update({status:"declined"}).eq("id",ae),Ne(e),f("home")})},z=async(C,Z)=>{k=!0,o&&(E.removeChannel(o),o=null),await new Promise(ae=>setTimeout(ae,600)),e.isConnected&&go(e,t,C,Z,d,l)},{data:M}=await E.from("friend_match_invites").select("*").eq("status","pending").or(`and(inviter_id.eq.${x},invitee_id.eq.${c}),and(inviter_id.eq.${c},invitee_id.eq.${x})`).order("created_at",{ascending:!1}).limit(1).maybeSingle();let j,O;if(M&&M.inviter_id===c)O=!1,j=M.id,await E.from("friend_match_invites").update({invitee_deck_id:n,invitee_ready:!0,status:"accepted"}).eq("id",j);else{O=!0;const{data:C,error:Z}=await E.from("friend_match_invites").insert({inviter_id:x,invitee_id:c,friend_id:c,inviter_deck_id:n,inviter_ready:!0,status:"pending"}).select().single();if(Z||!C){m("Erreur création invitation","error"),Ne(e),f("home");return}j=C.id}if(A(!0,!O,j),!O){let C=null;const Z={home_id:c,away_id:x,home_deck_id:M.inviter_deck_id,away_deck_id:n,status:"active"};let ae=await E.from("matches").insert({...Z,mode:"friend"}).select().single();if(ae.error&&(console.warn("[FriendMatch] insert avec mode échoué, retry sans mode:",ae.error.message),ae=await E.from("matches").insert(Z).select().single()),C=ae.data,!C){m("Erreur création match: "+(((S=ae.error)==null?void 0:S.message)||""),"error"),Ne(e),f("home");return}await E.from("friend_match_invites").update({match_id:C.id,status:"matched"}).eq("id",j),z(C.id,!1);return}const F=setInterval(()=>{if(k){clearInterval(F);return}E.from("matchmaking_queue").delete().eq("user_id",x).then(()=>{},()=>{})},3e3);o=E.channel("friend-invite-"+j).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${j}`},async C=>{if(k)return;const Z=C.new;Z.status==="matched"&&Z.match_id?(clearInterval(F),z(Z.match_id,!0)):Z.status==="declined"?(clearInterval(F),m(`${i} a décliné l'invitation`,"warning"),Ne(e),f("home")):Z.invitee_ready&&A(!0,!0,j)}).subscribe()}async function go(e,t,n,r,p=[],s=[]){const{state:d,navigate:l,toast:c}=t,i=r?"p1":"p2",a=r?"p2":"p1",f=(p||[]).map(u=>u.id),m=(p||[]).map(u=>({id:u.id,gc_type:u.gc_type,_gcDef:u._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:x}=await E.from("matches").select("*").eq("id",n).single();if(!x){c("Match introuvable","error"),l("home");return}async function k(){const[{data:u},{data:g},{data:b},{data:h}]=await Promise.all([E.rpc("get_deck_for_match",{p_deck_id:x.home_deck_id}),E.rpc("get_deck_for_match",{p_deck_id:x.away_deck_id}),E.from("users").select("id,pseudo,club_name").eq("id",x.home_id).single(),E.from("users").select("id,pseudo,club_name").eq("id",x.away_id).single()]),y=$=>({cardId:$.card_id,position:$.position,id:$.id,firstname:$.firstname,name:$.surname_encoded,country_code:$.country_code,club_id:$.club_id,job:$.job,job2:$.job2,note_g:Number($.note_g)||0,note_d:Number($.note_d)||0,note_m:Number($.note_m)||0,note_a:Number($.note_a)||0,rarity:$.rarity,skin:$.skin,hair:$.hair,hair_length:$.hair_length,clubName:$.club_encoded_name||null,clubLogo:$.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),v=((u==null?void 0:u.starters)||[]).map($=>y($)),w=((g==null?void 0:g.starters)||[]).map($=>y($)),_=(u==null?void 0:u.formation)||"4-4-2",I=(g==null?void 0:g.formation)||"4-4-2";return{p1Team:pt(v,_),p2Team:pt(w,I),p1Subs:((u==null?void 0:u.subs)||[]).map($=>y($)),p2Subs:((g==null?void 0:g.subs)||[]).map($=>y($)),p1Formation:_,p2Formation:I,p1Name:(b==null?void 0:b.club_name)||(b==null?void 0:b.pseudo)||"Joueur 1",p2Name:(h==null?void 0:h.club_name)||(h==null?void 0:h.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:r?f:[],gc_p2:r?[]:f,gcCardsFull_p1:r?m:[],gcCardsFull_p2:r?[]:m,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let o=x.game_state&&Object.keys(x.game_state).length?x.game_state:null;if(!o)if(r){o=await k();const{data:u}=await E.from("matches").select("game_state").eq("id",n).single();!(u!=null&&u.game_state)||!Object.keys(u.game_state).length?await E.from("matches").update({game_state:o,turn_user_id:x.home_id}).eq("id",n):o=u.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let u=0;u<30&&!o;u++){await new Promise(b=>setTimeout(b,400));const{data:g}=await E.from("matches").select("game_state").eq("id",n).single();g!=null&&g.game_state&&Object.keys(g.game_state).length&&(o=g.game_state)}if(!o){c("Erreur de synchronisation","error"),l("home");return}o.gc_p2=f,o.gcCardsFull_p2=m,await E.from("matches").update({game_state:o}).eq("id",n)}let A=!1,z=null,M=!1;const j=new Set,O=new Set;function F(u){var $,L;try{E.removeChannel(S)}catch{}const g=o[i+"Score"]||0,b=o[a+"Score"]||0;let h,y;u.winner_id?(h=u.winner_id===d.profile.id,y=!1):u.forfeit?(h=g>b,y=!1):(y=g===b,h=g>b),($=document.getElementById("pvp-end-overlay"))==null||$.remove();const v=document.createElement("div");v.id="pvp-end-overlay",v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const w=y?"🤝":h?"🏆":"😞",_=y?"MATCH NUL":h?"VICTOIRE !":"DÉFAITE",I=y?"#fff":h?"#FFD700":"#ff6b6b";v.innerHTML=`
      <div style="font-size:64px">${w}</div>
      <div style="font-size:26px;font-weight:900;color:${I}">${_}</div>
      <div style="font-size:18px">${o[i+"Name"]} ${g} – ${b} ${o[a+"Name"]}</div>
      ${u.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${h?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      <div style="font-size:32px;font-weight:900;color:#FFD700">${u} - ${g}</div>
      <button id="pvp-home" style="padding:16px 40px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏠 Retour</button>
    </div>`,(y=document.getElementById("pvp-home"))==null||y.addEventListener("click",()=>{try{E.removeChannel(S)}catch{}Ne(e),l("home")})}D()}const mo={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function xo(e,t){const{state:n,toast:r}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await fi(e,t)}async function fi(e,t){const{state:n,toast:r}=t,{data:p}=await E.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  `}async function $o(e,{state:t,navigate:n,toast:r}){const p=t.profile;if(!p)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await E.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
      ...
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${p.id},away_id.eq.${p.id}`).order("created_at",{ascending:!1}).limit(50),d={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},l=(s||[]).filter(a=>a.status==="finished"),c=(s||[]).filter(a=>a.status==="in_progress");function i(a){const f=a.home_id===p.id;f?a.home_score:a.away_score,f?a.away_score:a.home_score;const m=a.winner_id===p.id,x=a.home_score===a.away_score&&a.status==="finished",k=a.status!=="finished"?"…":x?"N":m?"V":"D",o=a.status!=="finished"||x?"#888":m?"#1A6B3C":"#c0392b";let A=d[a.mode]||a.mode;a.away_id===null&&!A.startsWith("IA")&&(A="IA");const M=a.home_id===p.id?a.away:a.home;let j;a.away_id===null?j=A:M?j=`${M.club_name||M.pseudo} (${M.pseudo})`:j="Adversaire";let O="";a.status==="in_progress"&&Date.now()-new Date(a.created_at).getTime()>3600*1e3&&(O=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const F=new Date(a.created_at),S=F.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+F.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),C=a.status==="finished"?`${a.home_score} - ${a.away_score}`:`${a.home_score||0} - ${a.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
