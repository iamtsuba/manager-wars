import{s as $,F as et,b as Me,c as Ft,l as ge,d as ai}from"./formation-links-CDBKB_z4.js";function _t(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),n=(d||[]).filter(_=>_.card_type==="player"&&_.player),r=(d||[]).filter(_=>_.card_type==="game_changer"),p=(d||[]).filter(_=>_.card_type==="formation"),{data:g}=await $.from("gc_definitions").select("name,gc_type,color,effect,image_url"),f={};(g||[]).forEach(_=>{f[_.name]=_});const y=Object.keys(et),u=Object.keys(de),h={};n.forEach(_=>{const b=_.player.id;h[b]=(h[b]||0)+1}),new Set(Object.keys(h).map(_=>String(_)));const T=new Set(p.map(_=>_.formation)),v=new Set(r.map(_=>_.gc_type));let x="player",k="Tous",D="",M=!1;function m(){return[...n].sort((_,b)=>{const j=je.indexOf(_.player.job),E=je.indexOf(b.player.job);return j!==E?j-E:(_.player.surname_encoded||"").localeCompare(b.player.surname_encoded||"")})}function A(){return[...l||[]].sort((_,b)=>{const j=je.indexOf(_.job),E=je.indexOf(b.job);return j!==E?j-E:(_.surname_encoded||"").localeCompare(b.surname_encoded||"")})}function G(){return m().filter(_=>{const b=_.player,j=k==="Tous"||b.job===k,E=!D||`${b.firstname} ${b.surname_encoded}`.toLowerCase().includes(D);return j&&E})}function q(){return A().filter(_=>{const b=k==="Tous"||_.job===k,j=!D||`${_.firstname} ${_.surname_encoded}`.toLowerCase().includes(D);return b&&j})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function N(){const _=document.getElementById("col-filters");_&&(x==="player"?(_.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",b=>{D=b.target.value.toLowerCase(),O()}),e.querySelectorAll(".filter-btn").forEach(b=>{b.addEventListener("click",()=>{k=b.dataset.job,N(),O()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{M=!M,N(),O()})):(_.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{M=!M,N(),O()})))}function O(){const _=document.getElementById("col-grid");_&&(x==="player"?S(_):x==="formation"?P(_):C(_))}function Y(_,b,j,E,w){w=w||"#7a28b8";const F=document.getElementById("col-grid"),I=document.getElementById("col-big");if(!F||!I)return;var R=0;function B(){I.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+b(_[R])+"</div>";var K=I.querySelector("[data-card-id],[data-form-id],[data-gc-id]");K&&K.addEventListener("click",function(){E(_[R])}),requestAnimationFrame(function(){var V=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!V||!I)){var J=I.clientHeight-8,te=I.clientWidth-24,re=V.offsetHeight,ae=V.offsetWidth;if(re>0&&ae>0&&J>40){var se=Math.min(J/re,te/ae,1);V.style.transform="scale("+se.toFixed(3)+")",V.style.transformOrigin="top center"}}}),F.innerHTML=_.map(function(V,J){return'<div class="col-mini-item" data-idx="'+J+'" style="flex-shrink:0;cursor:pointer;border-radius:8px;border:1.5px solid '+(J===R?w:"transparent")+';transition:border-color .15s;overflow:hidden">'+j(V,J===R)+"</div>"}).join(""),F.querySelectorAll(".col-mini-item").forEach(function(V){V.addEventListener("click",function(){R=Number(V.dataset.idx),B(),V.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}B()}function U(_){var b=.54,j=Math.round(140*b),E=Math.round(310*b),w;if(!_||_._fake){var F=_?_.player:null;if(!F)return"";w=At(F)}else w=Ue(_,"");return'<div style="width:'+j+"px;height:"+E+'px;overflow:hidden;position:relative;flex-shrink:0"><div style="transform:scale('+b+');transform-origin:top left;position:absolute;top:0;left:0;pointer-events:none">'+w+"</div></div>"}function Q(_,b,j){b=b||100,j=j||140;var E=Me[_]||{},w={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},F=Math.max(3,Math.round(b*.06)),I=Object.entries(E).map(function(B){var K=B[0],V=B[1],J=K.replace(/\d+$/,""),te=w[J]||"#888",re=Math.round(V.x*b),ae=Math.round(V.y*j);return'<circle cx="'+re+'" cy="'+ae+'" r="'+F+'" fill="'+te+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),R=Math.max(1,Math.round(b/50));return'<svg viewBox="0 0 '+b+" "+j+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+b+'" height="'+j+'" fill="#1A6B3C"/><rect x="'+Math.round(b*.2)+'" y="'+Math.round(j*.02)+'" width="'+Math.round(b*.6)+'" height="'+Math.round(j*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+R+'"/><line x1="0" y1="'+Math.round(j*.5)+'" x2="'+b+'" y2="'+Math.round(j*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+R+'"/><ellipse cx="'+Math.round(b*.5)+'" cy="'+Math.round(j*.5)+'" rx="'+Math.round(b*.18)+'" ry="'+Math.round(j*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+R+'"/><rect x="'+Math.round(b*.2)+'" y="'+Math.round(j*.82)+'" width="'+Math.round(b*.6)+'" height="'+Math.round(j*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+R+'"/>'+I+"</svg>"}function z(_,b,j){var E=j>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+j+"</div>":"",w=b?'data-form-id="'+b.id+'"':"",F=_.length>7?14:_.length>5?16:19,I=!!b;return"<div "+w+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(I?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(I?"":"filter:grayscale(1);opacity:0.5")+'">'+E+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(I?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+F+"px;font-weight:900;color:"+(I?"#1A6B3C":"#aaa")+';line-height:1">'+_+'</div></div><div style="flex:1;overflow:hidden;background:'+(I?"#1A6B3C":"#ccc")+'">'+Q(_,140,220)+"</div></div>"}function L(_,b){var j=.54,E=Math.round(140*j),w=Math.round(305*j),F=Math.round(w*.22),I=w-F,R=_.length>7?5:7,B=Q(_,E,I),K=b?"1.5px solid #2a7a40":"1px solid #ddd",V=b?"":"filter:grayscale(1);opacity:0.45;",J=b?"#1A6B3C":"#bbb",te="#fff";return'<div style="width:'+E+"px;height:"+w+"px;border-radius:6px;border:"+K+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+V+'"><div style="height:'+F+"px;background:"+J+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+R+"px;font-weight:900;color:"+te+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(E-4)+'px">'+_+'</span></div><div style="height:'+I+'px;overflow:hidden;flex-shrink:0">'+B+"</div></div>"}function S(_){if(M){const b=q();if(!b.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const j=b.map(E=>n.find(w=>w.player.id===E.id)||{_fake:!0,player:E,id:"fake-"+E.id});Y(j,E=>E._fake?At(E.player):Ue(E,""),E=>E._fake?U({player:E.player,id:"x",_fake:!0}):U(E),E=>{E._fake||It(E,n,h,t)},"#1A6B3C")}else{const b=G();if(!b.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const j={},E=[];b.forEach(w=>{j[w.player.id]||(j[w.player.id]=!0,E.push(w))}),Y(E,w=>{const F=h[w.player.id]||1,I=F>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${F}</div>`:"",B=n.filter(K=>K.player.id===w.player.id&&K.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Ue(w,I+B)},w=>U(w),w=>It(w,n,h,t),"#1A6B3C")}}function P(_){const b=M?y:[...T];if(!b.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const j=b.map(E=>({formation:E,card:p.find(w=>w.formation===E)||null,owned:T.has(E)}));Y(j,({formation:E,card:w,owned:F})=>z(E,F?w:null,F?p.filter(I=>I.formation===E).length:0),({formation:E,owned:w})=>L(E,w),({card:E,owned:w})=>{w&&E&&mi(E,p,t,o)},"#1A6B3C")}function C(_){const b=Object.keys(f),j=M?b.length?b:u:[...v];if(!j.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const E=j.map(w=>({type:w,gc:de[w]||{icon:"⚡",desc:""},def:f[w]||null,owned:v.has(w),card:r.find(F=>F.gc_type===w)||null}));Y(E,({type:w,gc:F,def:I,owned:R,card:B})=>{const K=R?r.filter(ze=>ze.gc_type===w).length:0,V=K>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${K}</div>`:"",J=(I==null?void 0:I.gc_type)==="ultra_game_changer",te={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},re={purple:"#b06ce0",light_blue:"#00d4ef"},ae=te[I==null?void 0:I.color]||te.purple,se=re[I==null?void 0:I.color]||re.purple,xe=(I==null?void 0:I.effect)||F.desc||"",me=I!=null&&I.image_url?`/manager-wars/icons/${I.image_url}`:null;return R&&B?`<div data-gc-id="${B.id}" data-gc-type="${w}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${se};background:${ae};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${se}66;cursor:pointer">
          ${V}
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const $e=1e3;function mi(e,t,i,s){var u,h,T;const{state:c,toast:o,closeModal:a,navigate:d,refreshProfile:l}=i,n=e.formation,r={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function p(){const v=Me[n]||{},x=et[n]||[],k=290,D=360,M=20;function m(G){const q=v[G];return q?{x:q.x*k,y:q.y*D}:null}let A=`<svg width="${k}" height="${D}" viewBox="0 0 ${k} ${D}" xmlns="http://www.w3.org/2000/svg">`;for(const[G,q]of x){const N=m(G),O=m(q);!N||!O||(A+=`<line x1="${N.x}" y1="${N.y}" x2="${O.x}" y2="${O.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const G of Object.keys(v)){const q=m(G);if(!q)continue;const N=G.replace(/\d+/,""),O=r[N]||"#555";A+=`<circle cx="${q.x}" cy="${q.y}" r="${M}" fill="${O}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,A+=`<text x="${q.x}" y="${q.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${N}</text>`}return A+="</svg>",A}const g=t.filter(v=>v.formation===n),f=g.length,y=!e.is_for_sale;s(`Formation ${n}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const Oi="simulation",Kt="VOTRE_ZONE_ID";function Hi(){switch(Oi){case"propellerads":return Ki();case"adsense":return Ui();default:return rt()}}function rt(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order");let l=0;function n(){var v,x,k,D,M;const r=o[l],p=(d||[]).filter(m=>m.deck_id===r.id),g=p.filter(m=>{var A;return m.is_starter&&((A=m.card)==null?void 0:A.player)}).map(m=>We(m.card,m.position)),f=p.find(m=>{var A;return((A=m.card)==null?void 0:A.card_type)==="formation"}),y=r.formation||((v=f==null?void 0:f.card)==null?void 0:v.formation)||"4-4-2",u=g.length>=11?Ne(g,y):null,h=g.length>=11;Vt(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${p}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${c?`<img src="${c}" style="width:${p-4}px;height:${p-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,p-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
        ...
        stroke="${A}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=48,f=64,y=13,u=16,h={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[v,x]of Object.entries(l)){const k=r(v);if(!k||!x)continue;const D=v.replace(/[0-9]/g,""),M=_e[D]||"#555",m=i==="attack"&&["MIL","ATT"].includes(D)&&!x.used||i==="defense"&&["GK","DEF","MIL"].includes(D)&&!x.used,A=s.includes(x.cardId);let G;i==="attack"?G=D==="MIL"?Number(x.note_m)||0:Number(x.note_a)||0:i==="defense"?G=D==="GK"?Number(x.note_g)||0:D==="MIL"?Number(x.note_m)||0:Number(x.note_d)||0:G=Number(D==="GK"?x.note_g:D==="DEF"?x.note_d:D==="MIL"?x.note_m:x.note_a)||0,G=G+(x.boost||0);const q=(k.x-g/2).toFixed(1),N=(k.y-f/2).toFixed(1),O=x.used?.25:1,Y=h[x==null?void 0:x.rarity]||h.normal,U=A?"#ff3030":Y,Q=A?3:(x==null?void 0:x.rarity)==="légende"||(x==null?void 0:x.rarity)==="pépite"?2.5:2,z=f-y-u;p+=`<defs><clipPath id="cp-${v}"><rect x="${q}" y="${(k.y-f/2+y).toFixed(1)}" width="${g}" height="${z}"/></clipPath></defs>`,p+=`<rect x="${q}" y="${N}" width="${g}" height="${f}" rx="5" fill="${M}" opacity="${O}"/>`;const L=oe(x);L&&!x.used&&(p+=`<image href="${L}" x="${q}" y="${(k.y-f/2+y).toFixed(1)}" width="${g}" height="${z}" clip-path="url(#cp-${v})" preserveAspectRatio="xMidYMin slice"/>`),p+=`<rect x="${q}" y="${N}" width="${g}" height="${y}" rx="4" fill="rgba(255,255,255,0.92)"/>`,p+=`<text x="${k.x.toFixed(1)}" y="${(k.y-f/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(x.name||"").slice(0,9)}</text>`;const S=(k.y+f/2-u).toFixed(1);if(p+=`<rect x="${q}" y="${S}" width="${g}" height="${u}" fill="rgba(255,255,255,0.92)"/>`,x.used)p+=`<text x="${k.x.toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="13" font-weight="900" fill="rgba(0,0,0,0.4)" font-family="Arial Black">–</text>`;else{const P=Je(x.country_code);P?p+=`<image href="${P}" x="${(k.x-20).toFixed(1)}" y="${(k.y+f/2-u+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:p+=`<text x="${(k.x-13).toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${Jt(x.country_code)}</text>`,p+=`<text x="${k.x.toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${G}</text>`;const C=Wt(x);C?p+=`<image href="${C}" x="${(k.x+g/2-14).toFixed(1)}" y="${(k.y+f/2-u+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:x.clubName&&(p+=`<text x="${(k.x+14).toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(x.clubName||"").slice(0,3).toUpperCase()}</text>`),x.boost&&(p+=`<rect x="${(k.x+g/2-12).toFixed(1)}" y="${(k.y-f/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,p+=`<text x="${(k.x+g/2-5).toFixed(1)}" y="${(k.y-f/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${x.boost}</text>`)}p+=`<rect x="${q}" y="${N}" width="${g}" height="${f}" rx="5" fill="${A?"rgba(255,255,255,0.12)":"none"}" stroke="${U}" stroke-width="${Q}" opacity="${O}"/>`,m&&(p+=`<rect x="${q}" y="${N}" width="${g}" height="${f}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${A?"selected":""}" data-card-id="${x.cardId}" data-role="${D}" style="cursor:pointer"/>`)}const T=38;return`<svg viewBox="${-T} ${-T} ${c+T*2} ${o+T*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${p}
  ...
  </div>`}async function Xt(e,t,i,s){var D;const{state:c,navigate:o,toast:a}=t;Vt(e);const d=c.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!d.deckId)return Vi(e,t,i);const l=d.deckId;let n,r,p,g;try{const M=await Promise.all([$.from("decks").select("formation,name").eq("id",l).single(),$.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",l).order("slot_order")]);n=M[0].data,p=M[0].error,r=M[1].data,g=M[1].error}catch(M){console.error("[Match] Exception chargement deck:",M),ue(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>o("home"));return}if(p||g){console.error("[Match] Erreur Supabase:",p||g),ue(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>o("home"));return}const f=(r||[]).filter(M=>{var m;return M.is_starter&&((m=M.card)==null?void 0:m.player)}).map(M=>We(M.card,M.position)),y=(r||[]).filter(M=>{var m;return!M.is_starter&&((m=M.card)==null?void 0:m.player)}).map(M=>We(M.card,M.position));if(f.length<11){ue(e,"⚠️",`Deck incomplet (${f.length}/11).`,"Compléter",()=>o("decks"));return}const u=(r||[]).find(M=>{var m;return((m=M.card)==null?void 0:m.card_type)==="formation"}),h=(n==null?void 0:n.formation)||((D=u==null?void 0:u.card)==null?void 0:D.formation)||"4-4-2",{data:T,error:v}=await $.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",c.profile.id).eq("card_type","game_changer"),{data:x}=await $.from("gc_definitions").select("*").eq("is_active",!0),k=(T||[]).map(M=>({...M,_gcDef:(x==null?void 0:x.find(m=>m.name===M.gc_type||m.id===M.gc_definition_id))||null}));s({deckId:l,formation:h,starters:f,subsRaw:y,gcCardsEnriched:k,gcDefs:x||[]})}async function Yi(e,t){const{state:i}=t,c=(i.params||{}).matchMode||"vs_ai_easy",o=c.replace("vs_ai_",""),a=c;await Xt(e,t,c,async({deckId:d,formation:l,starters:n,subsRaw:r,gcCardsEnriched:p,gcDefs:g})=>{try{const f=Ne(n,l),y=await Wi(l,o),u=async h=>{try{const{data:T,error:v}=await $.from("matches").insert({home_id:i.profile.id,away_id:null,mode:a,home_deck_id:d,status:"in_progress"}).select().single();if(v){console.error("[MatchIA] Erreur création match:",v),ue(e,"⚠️","Impossible de créer le match ("+v.message+").","Retour",()=>t.navigate("home"));return}const x={gcDefs:g||[],matchId:T==null?void 0:T.id,mode:a,difficulty:o,formation:l,homeTeam:f,aiTeam:y,homeSubs:r,subsUsed:0,maxSubs:Math.min(r.length,3),homeScore:0,aiScore:0,gcCards:h,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Xi(e,x,t)}catch(T){console.error("[MatchIA] Exception launchMatch:",T),ue(e,"⚠️","Erreur au lancement du match : "+T.message,"Retour",()=>t.navigate("home"))}};if(!p.length){u([]);return}Yt(e,p,u)}catch(f){console.error("[MatchIA] Exception setup:",f),ue(e,"⚠️","Erreur de préparation du match : "+f.message,"Retour",()=>t.navigate("home"))}})}async function Wi(e,t){var a,d;const{data:i}=await $.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return Ji(e);const s=we[e]||we["4-4-2"],c={GK:[],DEF:[],MIL:[],ATT:[]},o=[...i];for(const l of["GK","DEF","MIL","ATT"]){const n=o.filter(y=>y.job===l),r=o.filter(y=>y.job!==l),p=[...n,...r],g=[];for(let y=0;y<s[l];y++){const u=p[y]||o[y];u&&g.push({cardId:"ai-"+u.id+"-"+y,id:u.id,firstname:u.firstname,name:u.surname_encoded,country_code:u.country_code,club_id:u.club_id,job:u.job,job2:u.job2,note_g:Number(u.note_g)||0,note_d:Number(u.note_d)||0,note_m:Number(u.note_m)||0,note_a:Number(u.note_a)||0,rarity:u.rarity,skin:u.skin,hair:u.hair,hair_length:u.hair_length,clubName:((a=u.clubs)==null?void 0:a.encoded_name)||null,clubLogo:((d=u.clubs)==null?void 0:d.logo_url)||null,boost:0,used:!1,_line:l})}const f=Re(g.length);g.forEach((y,u)=>{y._col=f[u]}),c[l]=g}return c}function Ji(e){const t=we[e]||we["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},s=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let c=0;for(const o of["GK","DEF","MIL","ATT"]){const a=[];for(let l=0;l<t[o];l++){const n=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+c,id:"fake-"+c,firstname:"IA",name:s[c%s.length],country_code:"XX",club_id:null,job:o,job2:null,note_g:o==="GK"?n:2,note_d:o==="DEF"?n:2,note_m:o==="MIL"?n:2,note_a:o==="ATT"?n:2,rarity:"normal",boost:0,used:!1,_line:o}),c++}const d=Re(a.length);a.forEach((l,n)=>{l._col=d[n]}),i[o]=a}return i}function Xi(e,t,i){e.innerHTML=`
  ...
  </div>`,setTimeout(()=>Zi(e,t,i),5e3)}function Zi(e,t,i){const s=t.homeTeam.MIL||[],c=t.aiTeam.MIL||[];function o(p){return p.reduce((g,f)=>g+ie(f,"MIL"),0)}function a(p){let g=0;for(let f=0;f<p.length-1;f++){const y=ge(p[f],p[f+1]);y==="#00ff88"?g+=2:y==="#FFD700"&&(g+=1)}return g}const d=o(s)+a(s),l=o(c)+a(c),n=d>=l;function r(p,g,f){return`<div style="text-align:center">
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:100px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const A=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
            ...
            ${Ee((A.players||[]).map(G=>({...G,used:!1})),"#ff6b6b",A.total)}
            ...
            ${Ee((A.players||[]).map(G=>({...G,used:!1})),"#00ff88",A.total)}
    ...
    ${(()=>{const m=window.innerWidth>=700,A=(L,S,P)=>{var V,J;const C=(t.gcDefs||[]).find(te=>te.name===L.gc_type),_={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[C==null?void 0:C.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",b={purple:"#b06ce0",light_blue:"#00d4ef"}[C==null?void 0:C.color]||"#b06ce0",j=(C==null?void 0:C.name)||L.gc_type,E=(C==null?void 0:C.effect)||((V=de[L.gc_type])==null?void 0:V.desc)||"",w=C!=null&&C.image_url?`/manager-wars/icons/${C.image_url}`:null,F=((J=de[L.gc_type])==null?void 0:J.icon)||"⚡",I=Math.round(P*.22),R=Math.round(P*.22),B=P-I-R,K=j.length>12?7:9;return`<div class="gc-mini" data-gc-id="${L.id}" data-gc-type="${L.gc_type}"
          style="box-sizing:border-box;width:${S}px;height:${P}px;border-radius:10px;border:2px solid ${b};background:${_};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
          ...
          <div style="height:${B}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${w?`<img src="${w}" style="max-width:${S-10}px;max-height:${B-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(B*.55)}px">${F}</span>`}
          </div>
        ...
        </div>`},G=(L,S)=>{var P;return`<div id="boost-card"
          style="box-sizing:border-box;width:${L}px;height:${S}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(S*.04)}px;text-align:center;flex-shrink:0">
            ...
            <div style="font-size:${Math.round(S*.09)}px;color:#000;font-weight:900">+${(P=t.boostCard)==null?void 0:P.value}</div>
          </div>`},q=(L,S)=>S?G(110,150):A(L,110,150),N=(L,S)=>S?G(68,95):A(L,68,95),O=m?"padding:14px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:10px 8px;border-radius:12px;font-size:13px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Y=r?`<button id="btn-results" style="${O};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:d?`<div style="${O};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:l?`<button id="btn-action" style="${O};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:n?`<button id="btn-action" style="${O};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${O};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,U=l||n?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",Q=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${m?56:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
        ...
        <div style="display:flex;flex:0.8;min-height:0;overflow:hidden">
          ...
          <div style="width:140px;flex-shrink:0;display:flex;flex-direction:column;padding:10px 8px;background:rgba(0,0,0,0.2)">
            ...
            <div style="width:100%;flex-shrink:0;padding-top:8px">${Y}${U}</div>
          </div>
        </div>`:`
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(m=>{var A,G,q;if(m.type==="duel"){const N=m.isGoal,O=m.homeScored?"#FFD700":N?"#ff6b6b":"rgba(255,255,255,0.3)",Y=m.homeScored?"⚽ BUT !":N?"⚽ BUT IA !":(A=m.homePlayers)!=null&&A.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${N?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${O};margin-bottom:4px">
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(o);let d=!1;const l=()=>{d||(d=!0,o.remove(),setTimeout(()=>c(),50))};o.addEventListener("click",l),setTimeout(l,3500)}async function ve(e,t,i){var r,p;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:s}=i,c=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,a=c?"victoire":o?"nul":"defaite",d=pi(t.mode,a);t.matchId&&await $.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?s.profile.id:null,home_credits_reward:d,played_at:new Date().toISOString()}).eq("id",t.matchId);const l={credits:(s.profile.credits||0)+d,matches_played:(s.profile.matches_played||0)+1};c?l.wins=(s.profile.wins||0)+1:o?l.draws=(s.profile.draws||0)+1:l.losses=(s.profile.losses||0)+1,await $.from("users").update(l).eq("id",s.profile.id),await i.refreshProfile();const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",n.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const f=()=>document.getElementById("mm-status"),y=async(x=!0)=>{p=!0,g&&($.removeChannel(g),g=null),x&&await $.rpc("cancel_matchmaking",{p_user_id:l.profile.id}).catch(()=>{})};(v=document.getElementById("mm-cancel"))==null||v.addEventListener("click",async()=>{try{await y(!0)}catch(x){console.warn("[Matchmaking] cleanup error:",x)}le(e),n("home")});const u=async(x,k,D)=>{p||(p=!0,g&&($.removeChannel(g),g=null),f()&&(f().textContent="Adversaire trouvé !"),await new Promise(M=>setTimeout(M,600)),e.isConnected!==!1&&uo(e,t,x,D,a,d))},{data:h,error:T}=await $.rpc("try_matchmake",{p_user_id:l.profile.id,p_deck_id:i});if(T||!(h!=null&&h.success)){r("Erreur de matchmaking","error"),le(e),n("home");return}if(h.matched){u(h.match_id,h.opponent_id,!1);return}f()&&(f().textContent="En attente d'un autre joueur..."),g=$.channel("matchmaking-"+l.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${l.profile.id}`},x=>{const k=x.new;k.status==="matched"&&k.match_id&&u(k.match_id,k.matched_with,!0)}).subscribe()}async function uo(e,t,i,s,c=[],o=[]){const{state:a,navigate:d,toast:l}=t,n=s?"p1":"p2",r=s?"p2":"p1",p=(c||[]).map(z=>z.id),g=(c||[]).map(z=>({id:z.id,gc_type:z.gc_type,_gcDef:z._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation du match...</div>';const{data:f}=await $.from("matches").select("*").eq("id",i).single();if(!f){l("Match introuvable","error"),d("home");return}async function y(){const[{data:z,error:L},{data:S,error:P},{data:C},{data:_}]=await Promise.all([$.rpc("get_deck_for_match",{p_deck_id:f.home_deck_id}),$.rpc("get_deck_for_match",{p_deck_id:f.away_deck_id}),$.from("users").select("id,pseudo,club_name").eq("id",f.home_id).single(),$.from("users").select("id,pseudo,club_name").eq("id",f.away_id).single()]);L&&console.error("[PvP] get_deck_for_match p1 error:",L),P&&console.error("[PvP] get_deck_for_match p2 error:",P);function b(B,K){return{cardId:B.card_id,position:K||B.position,id:B.id,firstname:B.firstname,name:B.surname_encoded,country_code:B.country_code,club_id:B.club_id,job:B.job,job2:B.job2,note_g:Number(B.note_g)||0,note_d:Number(B.note_d)||0,note_m:Number(B.note_m)||0,note_a:Number(B.note_a)||0,rarity:B.rarity,skin:B.skin,hair:B.hair,hair_length:B.hair_length,clubName:B.club_encoded_name||null,clubLogo:B.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}}const j=((z==null?void 0:z.starters)||[]).map(B=>b(B,B.position)),E=((S==null?void 0:S.starters)||[]).map(B=>b(B,B.position)),w=((z==null?void 0:z.subs)||[]).map(B=>b(B,B.position)),F=((S==null?void 0:S.subs)||[]).map(B=>b(B,B.position)),I=(z==null?void 0:z.formation)||"4-4-2",R=(S==null?void 0:S.formation)||"4-4-2";return console.log("[PvP] p1Formation:",I,"p1 starters:",j.length,"positions:",j.map(B=>B.position)),console.log("[PvP] p2Formation:",R,"p2 starters:",E.length,"positions:",E.map(B=>B.position)),{p1Team:Ne(j,I),p2Team:Ne(E,R),p1Subs:w,p2Subs:F,p1Formation:I,p2Formation:R,p1Name:(C==null?void 0:C.club_name)||(C==null?void 0:C.pseudo)||"Joueur 1",p2Name:(_==null?void 0:_.club_name)||(_==null?void 0:_.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:s?p:[],gc_p2:s?[]:p,gcCardsFull_p1:s?g:[],gcCardsFull_p2:s?[]:g,gcReady_p1:!0,gcReady_p2:!0,usedGc_p1:[],usedGc_p2:[],lastActionAt:new Date().toISOString()}}let u=f.game_state&&Object.keys(f.game_state).length?f.game_state:null;if(!u)if(s){u=await y();const{data:z}=await $.from("matches").select("game_state").eq("id",i).single();!(z!=null&&z.game_state)||!Object.keys(z.game_state).length?await $.from("matches").update({game_state:u,turn_user_id:f.home_id}).eq("id",i):u=z.game_state}else{e.innerHTML=`<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation avec l'adversaire...</div>`;for(let z=0;z<30&&!u;z++){await new Promise(S=>setTimeout(S,400));const{data:L}=await $.from("matches").select("game_state").eq("id",i).single();L!=null&&L.game_state&&Object.keys(L.game_state).length&&(u=L.game_state)}if(!u){l("Erreur de synchronisation","error"),d("home");return}u.gc_p2=p,u.gcCardsFull_p2=g,await $.from("matches").update({game_state:u}).eq("id",i)}let h=!1;function T(z){var j;try{$.removeChannel(v)}catch{}const L=a.profile.id,S=z.winner_id===L,P=!!z.forfeit,C=u[n+"Score"]??0,_=u[r+"Score"]??0,b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center",b.innerHTML=`
      ...
      ${P?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${S?"L'adversaire a quitté la partie":"Match perdu par forfait"}</div>`:""}
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(b),(j=b.querySelector("#pvp-end-home"))==null||j.addEventListener("click",()=>{b.remove(),le(e),d("home")})}const v=$.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},z=>{const L=z.new;try{if(L.status==="finished"||L.forfeit){if(h)return;h=!0,u._timerInt&&(clearInterval(u._timerInt),u._timerInt=null),L.game_state&&(u=L.game_state),T(L);return}L.game_state&&(u=L.game_state,D())}catch(S){console.error("[PvP] Realtime render crash:",S,"gameState:",u)}}).subscribe();async function x(z){Object.assign(u,z),u.lastActionAt=new Date().toISOString();const{error:L}=await $.from("matches").update({game_state:u}).eq("id",i);L&&(console.error("[PvP] pushState DB error:",L.message,L),l("Erreur de synchronisation : "+L.message,"error"));try{D()}catch(S){console.error("[PvP] renderPvpScreen crash après pushState:",S),l("Erreur d'affichage : "+S.message,"error")}}async function k(){if(h)return;h=!0,u._timerInt&&(clearInterval(u._timerInt),u._timerInt=null);const z=s?f.away_id:f.home_id;try{await $.from("matches").update({status:"finished",forfeit:!0,winner_id:z}).eq("id",i)}catch(L){console.warn("[PvP] forfeit DB error:",L)}try{$.removeChannel(v)}catch{}le(e),d("home")}function D(){var ut,ft,gt,xt,mt;const z=u[n+"Team"];u[r+"Team"];const L=u[n+"Score"],S=u[r+"Score"],P=u[n+"Name"],C=u[r+"Name"];if(u.phase==="gc-select")return M();if(u.phase==="reveal")return m();if(u.phase==="midfield")return A();if(u.phase==="finished")return Y();const _=u.phase===n+"-attack",b=u.phase===n+"-defense",j=u.phase===r+"-attack"||u.phase===r+"-defense",E=Array.isArray(u["selected_"+n])?u["selected_"+n]:[],w=E.map(H=>H.cardId);e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const F=window.innerWidth>=700,I=u[n+"Subs"]||[],R=u["usedSubIds_"+n]||[],B=I.filter(H=>!R.includes(H.cardId)),K=u["gcCardsFull_"+n]||[],V=u["usedGc_"+n]||[],J=K.filter(H=>!V.includes(H.id)),te=u.boostOwner===n&&!u.boostUsed,re=(H,X,ee)=>{var bt,wt;const W=H._gcDef,ce={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[W==null?void 0:W.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Ce={purple:"#b06ce0",light_blue:"#00d4ef"}[W==null?void 0:W.color]||"#b06ce0",ye=(W==null?void 0:W.name)||H.gc_type,oi=(W==null?void 0:W.effect)||((bt=de[H.gc_type])==null?void 0:bt.desc)||"",yt=W!=null&&W.image_url?`/manager-wars/icons/${W.image_url}`:null,ni=((wt=de[H.gc_type])==null?void 0:wt.icon)||"⚡",ht=Math.round(ee*.22),vt=Math.round(ee*.22),Ke=ee-ht-vt,ri=ye.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${H.id}" data-gc-type="${H.gc_type}"
        style="width:${X}px;height:${ee}px;border-radius:10px;border:2px solid ${Ce};background:${ce};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
      ...
      </div>`},ae=(H,X)=>`<div id="pvp-boost-card" style="width:${H}px;height:${X}px;flex-shrink:0;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px">
      ...
      <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
        ${Te(z,u[n+"Formation"],_?"attack":b?"defense":"idle",w,300,300)}
          ...
          ${Ee(H.players.map(ce=>({...ce,used:!1})),"#ff6b6b",H.total)}
        </div>`;const X=Array.isArray(u.log)?u.log:[],ee=X[X.length-1];return ee?`<div style="padding:7px 10px;border-left:3px solid ${ee.type==="goal"?"#FFD700":ee.type==="stop"?"#00ff88":ee.type==="attack"?"#ff6b6b":"rgba(255,255,255,0.6)"};font-size:12px;color:#fff">${ee.text||""}</div>`:'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})(),ii=(Array.isArray(u.log)?u.log:[]).length,pt=`
      <!-- Score -->
      <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:rgba(0,0,0,0.25);flex-shrink:0">
        ...
        <div style="flex:1;text-align:center;color:#fff;font-size:14px;font-weight:700">
          ${P} <span style="color:#FFD700;font-size:18px">${L}</span> – <span style="color:#FFD700;font-size:18px">${S}</span> ${C}
        </div>
      ...
      <div style="text-align:center;padding:4px;background:rgba(0,0,0,0.15);font-size:11px;color:${j?"rgba(255,255,255,0.4)":"#FFD700"};font-weight:700;flex-shrink:0">
        ${j?`⏳ Tour de ${C}`:_?"⚔️ À vous d'attaquer !":b?"🛡️ À vous de défendre !":""}
      </div>
      <!-- Zone actions -->
      ...
      </div>`;const H=e.querySelector(".terrain-wrapper");H&&(H.innerHTML=Te(z,u[n+"Formation"],_?"attack":b?"defense":"idle",w,300,300))}if(e.querySelectorAll(".match-slot-hit").forEach(H=>{H.addEventListener("click",()=>{if(!_&&!b)return;const X=H.dataset.cardId,ee=H.dataset.role,W=(z[ee]||[]).find(ye=>ye.cardId===X);if(!W||W.used)return;Array.isArray(u["selected_"+n])||(u["selected_"+n]=[]);const ce=u["selected_"+n],Ce=ce.findIndex(ye=>ye.cardId===X);Ce>-1?ce.splice(Ce,1):ce.length<3&&ce.push({...W,_role:ee}),D()})}),(ut=e.querySelector("#pvp-boost-card"))==null||ut.addEventListener("click",()=>{G()}),e.querySelectorAll(".pvp-gc-mini").forEach(H=>{H.addEventListener("click",()=>{O(H.dataset.gcId,H.dataset.gcType)})}),(ft=e.querySelector("#pvp-action"))==null||ft.addEventListener("click",()=>{_?U():b&&Q()}),(gt=e.querySelector("#pvp-quit"))==null||gt.addEventListener("click",()=>{confirm("Quitter le match ? Vous perdrez par forfait.")&&k()}),(xt=e.querySelector("#pvp-view-opp"))==null||xt.addEventListener("click",()=>{N()}),(mt=e.querySelector("#pvp-toggle-history"))==null||mt.addEventListener("click",()=>{q()}),function(){const X=e.querySelector(".terrain-wrapper svg");X&&(X.removeAttribute("width"),X.removeAttribute("height"),X.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",X.setAttribute("viewBox","-26 -26 352 352"),X.setAttribute("preserveAspectRatio","xMidYMid meet"));const ee=e.querySelector(".match-screen");if(ee){const W=e.clientHeight;W>50&&(ee.style.height=W+"px",ee.style.maxHeight=W+"px",ee.style.overflow="hidden")}}(),u._timerInt&&(clearInterval(u._timerInt),u._timerInt=null),(_||b)&&!h){let H=30,X=!1;const ee=()=>document.getElementById("pvp-timer"),W=()=>{ee()&&(ee().textContent=H+"s",ee().style.color=X?"#ff4444":"#fff")};W(),u._timerInt=setInterval(()=>{H--,H<0?X?(clearInterval(u._timerInt),u._timerInt=null,k()):(X=!0,H=15,W()):W()},1e3)}}function M(){var b;const z=u["gcReady_"+n],L=u["gcReady_"+r];Array.isArray(u["gc_"+n])||(u["gc_"+n]=[]);let S=u["gc_"+n];const P=3;function C(j,E){const w=j._gcDef,F={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},I={purple:"#9b59b6",light_blue:"#00bcd4"},R=F[w==null?void 0:w.color]||F.purple,B=I[w==null?void 0:w.color]||I.purple;return`<div class="pvp-gc-card" data-id="${j.id}"
        style="width:100px;border-radius:10px;border:3px solid ${E?"#FFD700":B};background:${R};
          display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0;position:relative;
    ...
    <div id="pvp-gc-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      <div style="text-align:center;padding:12px 16px 8px;flex-shrink:0">
        ...
        <div style="font-size:20px;font-weight:900;color:#fff">Choisir ses Game Changers</div>
        <div style="font-size:13px;color:rgba(255,255,255,0.5);margin-top:3px">
          Sélectionne <b style="color:#FFD700">${P}</b> cartes · ${S.length}/${P}
        </div>
      </div>
      ${z?`
    ...
    </div>`,z)return;const _=n==="p1"?f.home_id:f.away_id;$.from("cards").select("id,gc_type").eq("owner_id",_).eq("card_type","game_changer").then(async({data:j})=>{const{data:E}=await $.from("gc_definitions").select("*").eq("is_active",!0),w=new Set,F=(j||[]).filter(R=>w.has(R.gc_type)?!1:(w.add(R.gc_type),!0)).map(R=>({...R,_gcDef:(E==null?void 0:E.find(B=>B.name===R.gc_type))||null})),I=document.getElementById("pvp-gc-grid");if(I){if(!F.length){I.innerHTML='<div style="color:rgba(255,255,255,0.4);font-size:13px;text-align:center;margin-top:30px">Aucune carte Game Changer disponible.</div>';return}I.innerHTML=F.map(R=>C(R,S.includes(R.id))).join(""),I.querySelectorAll(".pvp-gc-card").forEach(R=>{R.addEventListener("click",()=>{const B=R.dataset.id,K=S.indexOf(B);K>-1?S.splice(K,1):S.length<P&&S.push(B),D()})})}}),(b=e.querySelector("#pvp-gc-validate"))==null||b.addEventListener("click",async()=>{const j=n==="p1"?f.home_id:f.away_id,{data:E}=await $.from("cards").select("id,gc_type").eq("owner_id",j).eq("card_type","game_changer").in("id",S),{data:w}=await $.from("gc_definitions").select("*").eq("is_active",!0),F=(E||[]).map(I=>({...I,_gcDef:(w==null?void 0:w.find(R=>R.name===I.gc_type))||null}));await x({["gcReady_"+n]:!0,["gcCardsFull_"+n]:F}),u["gcReady_"+r]&&await x({phase:"reveal"})})}function m(){e.style.overflow="hidden",e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      ...
      <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
    </div>`,n==="p1"&&setTimeout(async()=>{await x({phase:"midfield"})},5e3)}function A(){const z=u[n+"Team"].MIL||[],L=u[r+"Team"].MIL||[];function S(E){return E.reduce((w,F)=>w+ie(F,"MIL"),0)}function P(E){let w=0;for(let F=0;F<E.length-1;F++){const I=ge(E[F],E[F+1]);I==="#00ff88"?w+=2:I==="#FFD700"&&(w+=1)}return w}const C=S(z)+P(z),_=S(L)+P(L),b=C>=_;function j(E,w,F){return`<div style="text-align:center">
        <div style="font-size:10px;color:rgba(255,255,255,0.5);letter-spacing:2px;margin-bottom:8px;text-transform:uppercase">${w}</div>
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${E.map((I,R)=>{const B=oe(I),K=R<E.length-1?ge(I,E[R+1]):null,V=K&&K!=="#ff3333"&&K!=="#cc2222";return`
            <div style="width:52px;height:52px;border-radius:8px;background:${F};position:relative;flex-shrink:0;overflow:hidden;border:2px solid rgba(255,255,255,0.3)">
        ...
        <div style="margin-top:6px;font-size:11px;color:rgba(255,255,255,0.5)">
          Score: ${S(E)} + ${P(E)} liens = <b style="color:#fff">${S(E)+P(E)}</b>
        </div>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div id="pvp-midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff;max-width:320px"></div>
    </div>`,setTimeout(()=>{const E=document.getElementById("pvp-score-me"),w=document.getElementById("pvp-score-opp"),F=document.getElementById("pvp-midfield-result");E&&w&&(b?(E.style.fontSize="80px",E.style.color="#FFD700",w.style.opacity="0.25"):(w.style.fontSize="80px",w.style.color="#ff6b6b",E.style.opacity="0.25"));const I=u.p1Team.MIL||[],R=u.p2Team.MIL||[],B=S(I)+P(I),K=S(R)+P(R),V=B>=K?"p1":"p2";u[V+"Name"];let J=u.boostValue;J==null&&(J=Ut(),u.boostValue=J,u.boostOwner=V,u.boostUsed=!1),F&&(F.style.opacity="1",F.innerHTML=`<div style="font-size:20px;font-weight:900;margin-bottom:10px">
        ...
        <div style="background:rgba(135,206,235,0.15);border:2px solid #87CEEB;border-radius:14px;padding:14px 24px;display:inline-block;margin-top:4px" id="pvp-boost-display">
          ...
          <div style="font-size:10px;color:rgba(135,206,235,0.7)">Applicable sur n'importe quel joueur</div>
        </div>`:""}`),setTimeout(async()=>{if(n!=="p1")return;const te=V;await x({phase:te+"-attack",attacker:te,round:1,boostValue:u.boostValue,boostUsed:!1,boostOwner:te})},1800)},600)}function G(){var P;const z=u[n+"Team"],L=Object.entries(z).flatMap(([C,_])=>(_||[]).filter(b=>!b.used).map(b=>({...b,_line:C})));if(!L.length)return;const S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",S.innerHTML=`
      ...
      </div>`,document.body.appendChild(S),(P=S.querySelector("#boost-picker-close"))==null||P.addEventListener("click",()=>S.remove()),S.querySelectorAll(".boost-pick-item").forEach(C=>{C.addEventListener("click",async()=>{const _=C.dataset.cid,b=L.find(w=>w.cardId===_);if(!b)return;const j=u[n+"Team"],E=(j[b._line]||[]).find(w=>w.cardId===_);E&&(E.boost=(E.boost||0)+u.boostValue),S.remove(),await x({[n+"Team"]:j,boostUsed:!0})})})}function q(){var S;const z=Array.isArray(u.log)?u.log:[],L=document.createElement("div");L.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",L.innerHTML=`
      <div style="display:flex;align-items:center;padding:14px 16px;border-bottom:1px solid rgba(255,255,255,0.1);flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">📋 Historique du match</div>
        <button id="pvp-hist-close" style="background:none;border:none;color:rgba(255,255,255,0.6);font-size:20px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
        ${z.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...z].reverse().map(P=>`<div style="padding:8px 10px;border-radius:8px;background:rgba(255,255,255,0.04);border-left:3px solid ${P.type==="goal"?"#FFD700":P.type==="defense"?"#00ff88":"rgba(255,255,255,0.5)"}">
                <div style="font-size:12px;color:#fff">${P.text||""}</div>
              </div>`).join("")}
      </div>`,document.body.appendChild(L),(S=L.querySelector("#pvp-hist-close"))==null||S.addEventListener("click",()=>L.remove())}function N(){var L;const z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",z.innerHTML=`
      <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-transform:uppercase">Équipe adverse</div>
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${u[r+"Name"]}</div>
      <div style="width:min(90vw,420px)">${Ie(u[r+"Team"],u[r+"Formation"],null,[],300,300)}</div>
      <button id="pvp-view-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(z),(L=z.querySelector("#pvp-view-opp-close"))==null||L.addEventListener("click",()=>z.remove())}function O(z,L){var B,K;const P=(u["gcCardsFull_"+n]||[]).find(V=>V.id===z),C=P==null?void 0:P._gcDef,_=de[L]||{icon:"⚡",desc:"Carte spéciale."},b={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[C==null?void 0:C.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",j={purple:"#b06ce0",light_blue:"#00d4ef"}[C==null?void 0:C.color]||"#b06ce0",E=(C==null?void 0:C.name)||L,w=(C==null?void 0:C.effect)||_.desc,F=C!=null&&C.image_url?`/manager-wars/icons/${C.image_url}`:null,I=_.icon||"⚡",R=document.createElement("div");R.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",R.innerHTML=`
      ...
      </div>`,document.body.appendChild(R),(B=R.querySelector("#pvp-gc-back"))==null||B.addEventListener("click",()=>R.remove()),(K=R.querySelector("#pvp-gc-use"))==null||K.addEventListener("click",async()=>{R.remove();const V=u["usedGc_"+n]||[];V.push(z),await x({["usedGc_"+n]:V})})}function Y(){var C;const z=u[n+"Score"],L=u[r+"Score"],S=z>L,P=z===L;e.style.overflow="hidden",e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      <div style="font-size:64px">${S?"🏆":P?"🤝":"😤"}</div>
      <div style="font-size:24px;font-weight:900;color:#fff">${S?"Victoire !":P?"Match nul":"Défaite"}</div>
    ...
    </div>`,(C=document.getElementById("pvp-home"))==null||C.addEventListener("click",()=>{$.removeChannel(v),le(e),d("home")})}async function U(){const z=(u["selected_"+n]||[]).map(P=>({...P,_line:P._role}));if(!z.length)return;const L=tt(z,u.modifiers[n]||{});z.forEach(P=>{const C=(u[n+"Team"][P._role]||[]).find(_=>_.cardId===P.cardId);C&&(C.used=!0)});const S=Array.isArray(u.log)?u.log:[];S.push({type:"attack",text:`⚔️ ${u[n+"Name"]} attaque (${L.total})`,players:z,total:L.total,side:n}),await x({pendingAttack:{...L,players:z,side:n},["selected_"+n]:[],modifiers:{...u.modifiers,[n]:{}},phase:r+"-defense",log:S})}async function Q(){const z=(u["selected_"+n]||[]).map(F=>({...F,_line:F._role})),L=it(z,u.modifiers[n]||{});z.forEach(F=>{const I=(u[n+"Team"][F._role]||[]).find(R=>R.cardId===F.cardId);I&&(I.used=!0)});const S=ot(u.pendingAttack.total,L.total,u.modifiers[n]||{}),P=u.pendingAttack.side,C=S==="attack",_=u[P+"Score"]+(C?1:0),b=P==="p1"?"p2":"p1",j=(u.round||0)+1,E=j>10,w=Array.isArray(u.log)?u.log:[];w.push({type:"defense",text:`🛡️ ${u[n+"Name"]} défend (${L.total})`,players:z,total:L.total,side:n}),w.push({type:C?"goal":"stop",text:C?`⚽ BUT de ${u[P+"Name"]} ! (${u.pendingAttack.total} vs ${L.total})`:`✋ Attaque stoppée (${u.pendingAttack.total} vs ${L.total})`}),await x({[P+"Score"]:_,["selected_"+n]:[],modifiers:{...u.modifiers,[n]:{}},pendingAttack:null,phase:E?"finished":b+"-attack",attacker:b,round:j,log:w}),E&&await $.from("matches").update({status:"finished"}).eq("id",i)}D()}const fo={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function go(e,t){const{state:i,toast:s}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await dt(e,t)}async function dt(e,t){const{state:i,toast:s}=t,{data:c}=await $.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  `;function l(n){const r=document.getElementById("mkt-content"),p=n==="buy"?a:d;if(p.length===0){r.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${g.map(f=>{var x,k,D,M;const y=(x=f.card)==null?void 0:x.player;if(!y)return"";const u=y.job==="GK"?y.note_g:y.job==="DEF"?y.note_d:y.job==="MIL"?y.note_m:y.note_a,h=fo[y.rarity],T=(i.profile.credits||0)>=f.price,v=f.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${v?"opacity:0.65":""}">
