...
import{s as v,j as hi,q as zn,t as Ht,u as Ni,F as Oi,o as tt,n as ei,g as At,r as Te,m as Mt,v as Ai,w as uo,T as Li,p as fo,x as mo,d as go}from"./special-cards-lEGt-tGs.js";const xo="/";function cn(e,{navigate:t,toast:i}){let n="login";const o=()=>{var a,s,d,l,c,f,b;const r=n==="login";if(e.innerHTML=`
    <div style="
  ...
  `;function s(){var g;const l=document.getElementById("logo-preview"),c=document.getElementById("logo-initials"),f=((g=document.getElementById("setup-club"))==null?void 0:g.value)||"MW",b=f.trim().split(" ").filter(Boolean),x=b.length>=2?(b[0][0]+b[1][0]).toUpperCase():f.slice(0,2).toUpperCase();l&&(l.style.background=a,l.style.borderColor=r),c&&(c.textContent=x,c.style.color=r)}document.getElementById("color1").addEventListener("input",l=>{r=l.target.value,document.getElementById("swatch1").style.background=r,s()}),document.getElementById("color2").addEventListener("input",l=>{a=l.target.value,document.getElementById("swatch2").style.background=a,s()});function d(l){document.querySelectorAll(".setup-step").forEach(c=>c.classList.remove("active")),document.getElementById(`step-${l}`).classList.add("active"),document.getElementById("step-num").textContent=l,document.getElementById("progress-fill").style.width=`${Math.round(l/3*100)}%`,l===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("step1-error");if(c.textContent="",l.length<3){c.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await v.from("users").select("id").eq("pseudo",l).maybeSingle();if(f){c.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const l=document.getElementById("setup-club").value.trim(),c=document.getElementById("step2-error");if(c.textContent="",l.length<2){c.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await v.from("users").select("id").eq("club_name",l).maybeSingle();if(f){c.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),b=document.getElementById("step3-finish");f.textContent="",b.disabled=!0,b.textContent="Création en cours…";try{const{error:x}=await v.from("users").insert({id:t.user.id,pseudo:l,club_name:c,club_color1:r,club_color2:a,credits:1e4});if(x)throw x;await yo(t.user.id),await o(),n(`Bienvenue ${l} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(x){f.textContent=x.message,b.disabled=!1,b.textContent="🚀 Créer mon profil !"}})}async function yo(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await v.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const ho="modulepreload",vo=function(e){return"/"+e},pn={},Hi=function(t,i,n){let o=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));o=Promise.allSettled(i.map(d=>{if(d=vo(d),d in pn)return;pn[d]=!0;const l=d.endsWith(".css"),c=l?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${c}`))return;const f=document.createElement("link");if(f.rel=l?"stylesheet":ho,l||(f.as="script"),f.crossOrigin="",f.href=d,s&&f.setAttribute("nonce",s),document.head.appendChild(f),l)return new Promise((b,x)=>{f.addEventListener("load",b),f.addEventListener("error",()=>x(new Error(`Unable to preload CSS for ${d}`)))})}))}function r(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return o.then(a=>{for(const s of a||[])s.status==="rejected"&&r(s.reason);return t().catch(r)})},ti="#1A6B3C",ii="#cc2222",wo="#D4A017",un="#888";async function _o(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </div>`,await Tn(i,n,t),document.getElementById("btn-add-friend").addEventListener("click",()=>$o(i,n)),document.getElementById("btn-accept-friend").addEventListener("click",()=>Ui(i,n,null,t))}async function Tn(e,t,i={}){const{navigate:n}=i,o=e.user.id,{data:r,error:a}=await v.from("friendships").select("id, requester_id, addressee_id").eq("status","accepted").or(`requester_id.eq.${o},addressee_id.eq.${o}`),{count:s}=await v.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",o).eq("status","pending"),d=document.getElementById("pending-badge");d&&(s>0?(d.style.display="flex",d.textContent=s):d.style.display="none");const l=document.getElementById("friends-list");if(!l)return;if(a){console.error("[Friends] Erreur:",a),l.innerHTML=`<div style="color:${ii};text-align:center;padding:16px">Erreur chargement : ${a.message}</div>`;return}const c=(r||[]).map(x=>x.requester_id===o?x.addressee_id:x.requester_id);let f={};if(c.length){const{data:x}=await v.from("users").select("id, pseudo, club_name, last_seen_at, club_color1, club_color2").in("id",c);(x||[]).forEach(g=>{f[g.id]=g})}const b=(r||[]).map(x=>({friendshipId:x.id,friend:f[x.requester_id===o?x.addressee_id:x.requester_id]||{pseudo:"?"}}));if(!b.length){l.innerHTML=`
  ...
  <div class="home-dark" id="home-dark">
    ...
    `,document.head.appendChild(l)}const o=Co.map(l=>`
    <a class="home2-chrome-tab" data-route="${l.route}" data-key="${l.key}">
    ...
    </a>`).join("");let r=document.getElementById("home2-chrome-header");r||(r=document.createElement("div"),r.id="home2-chrome-header",r.className="home2-chrome-header",r.innerHTML=`
      ...
      <div class="home2-chrome-tabs">${o}</div>
    ...
    `,document.body.appendChild(r),r.querySelector("#home2-chrome-settings-btn").addEventListener("click",()=>e("settings")),r.querySelector("#home2-chrome-credits").addEventListener("click",()=>e("boosters")));let a=document.getElementById("home2-mobile-top");a||(a=document.createElement("div"),a.id="home2-mobile-top",a.className="home2-mobile-top",a.innerHTML=`
      <div class="home2-chrome-logo"><img src="${n}logo-withname.png" alt="Manager Wars"></div>
      <div class="home2-chrome-right">
    ...
    </div>`,document.body.appendChild(t),t.querySelector("#journal-close-v2").addEventListener("click",()=>t.remove()),t.addEventListener("click",n=>{n.target===t&&t.remove()})}async function Ro(e,{state:t,navigate:i,toast:n}){var m,C,$,S,G,Q,ie;const o=t.profile;if(!o)return;const r="/icons/";Bn(i,o,"home2",r);const a=o.mmr??1e3,s=hi(a),d=qo(a,s),l=zn(a),c=isFinite(s.max)?s.max+1:null,[{list:f,iAmInTop:b,myPosition:x},g,p]=await Promise.all([Do(o.id),Fo(5),Po()]),u=(N,I)=>`
    <div class="rk-row ${N.id===o.id?"rk-row-me":""}">
      <div class="rk-pos ${I<3?"rk-pos-top"+(I+1):""}">${I<3?["🥇","🥈","🥉"][I]:I+1}</div>
      <div class="rk-name ${N.id===o.id?"rk-name-me":""}">${N.pseudo}</div>
      <div class="rk-rp">${(N.mmr??0).toLocaleString("fr")} RP</div>
    </div>`,y=N=>`
    <div class="news-item">
      ...
      <div class="news-body">
        ...
        <div class="news-time">${Bo(N.published_at)}</div>
      </div>
    </div>`;let w=0;const h=N=>`
    <div class="promo-icon-wrap">
      <img src="${r}${N.icon||"nav-boosters.png"}" class="promo-icon" onerror="this.style.display='none'">
    </div>
      ...
      <div class="home2-dash">
        ...
        <div class="home2-col-center">
          ...
          <div class="promo-banner" id="promo-banner">
            ...
            ${p.length>1?`<div class="promo-dots">${p.map((N,I)=>`<div class="promo-dot ${I===0?"active":""}" data-dot="${I}"></div>`).join("")}</div>`:""}
          </div>`:""}
        </div>
  ...
  </div>`,requestAnimationFrame(()=>{var de,me;const N=((de=window.visualViewport)==null?void 0:de.height)||window.innerHeight,I=window.innerWidth<1024;let z=0;if(I){const j=document.getElementById("home2-mobile-top"),V=document.getElementById("home2-mobile-bottom");z=((j==null?void 0:j.offsetHeight)||0)+((V==null?void 0:V.offsetHeight)||0)}else z=((me=document.getElementById("home2-chrome-header"))==null?void 0:me.offsetHeight)||0;const K=N-z,W=e.querySelector(".home-dark");W&&(W.style.minHeight=K+"px")});const E=()=>{const N=document.getElementById("mode-indicator");N&&(N.textContent=window.innerWidth>=1024?"🖥️ Mode PC":"📱 Mode Mobile")};if(E(),window.addEventListener("resize",E),(m=document.getElementById("nav-profile-btn"))==null||m.addEventListener("click",()=>i("settings")),(C=document.getElementById("nav-rankings-link"))==null||C.addEventListener("click",()=>i("rankings")),($=document.getElementById("nav-rankings-cta"))==null||$.addEventListener("click",()=>i("rankings")),(S=document.getElementById("rank-inline-link-btn"))==null||S.addEventListener("click",()=>i("rankings")),(G=document.getElementById("nav-journal-link"))==null||G.addEventListener("click",()=>Go()),e.querySelectorAll("[data-action]").forEach(N=>{N.addEventListener("click",()=>{N.style.transform="scale(.96)",setTimeout(()=>N.style.transform="",180);const I=N.dataset.action;if(I==="match-ai"){Fn(i);return}if(I==="match-random"){i("match",{matchMode:"random"});return}if(I==="match-friend"){i("friends");return}if(I==="mini-league"){i("mini-league");return}if(I==="ranked"){i("ranked");return}n("Bientôt disponible","info")})}),p.length>1){const N=document.getElementById("promo-slide-content"),I=()=>Array.from(document.querySelectorAll(".promo-dot")),z=K=>{w=K,N&&(N.innerHTML=h(p[w])),I().forEach((W,de)=>W.classList.toggle("active",de===w))};setInterval(()=>z((w+1)%p.length),5e3),document.querySelectorAll(".promo-dot").forEach(K=>{K.addEventListener("click",()=>z(Number(K.dataset.dot)))})}(Q=document.getElementById("promo-cta-btn"))==null||Q.addEventListener("click",()=>i("boosters")),(ie=document.getElementById("back-to-v1-btn"))==null||ie.addEventListener("click",()=>{jo(),i("home")}),Dn(t,n),Uo(t,n,i),qn(t,n,i),No(t,n)}async function No(e,t){const i=e.profile.id,{data:n}=await v.from("mini_league_members").select("league_id, prize_amount, mini_leagues(name)").eq("user_id",i).eq("prize_claimed",!1).gt("prize_amount",0);if(!(n!=null&&n.length))return;const o=n.reduce((a,s)=>a+(s.prize_amount||0),0),r=document.createElement("div");r.className="modal-overlay",r.style.zIndex="2200",r.innerHTML=`<div class="modal" style="max-width:400px">
    <div class="modal-header"><h2 style="color:#1A6B3C">🏆 Gains à récupérer</h2></div>
    <div class="modal-body" style="padding:18px 20px">
      <p style="font-size:14px;color:#555;margin:0 0 14px">Tu as terminé sur le podium de ${n.length>1?"plusieurs mini leagues":"une mini league"} !</p>
      <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:16px">
      ...
      <button data-resume="${d.id}" data-mini="${f?"1":""}" style="width:38px;height:38px;border-radius:50%;border:none;background:#22c55e;color:#fff;font-size:18px;cursor:pointer;flex-shrink:0">⚽</button>
      <button data-abandon="${d.id}" data-opp="${l}" style="width:38px;height:38px;border-radius:50%;border:none;background:#cc2222;color:#fff;font-size:18px;cursor:pointer;flex-shrink:0">✕</button>
    </div>`}).join(""),n.querySelectorAll("[data-resume]").forEach(d=>{d.addEventListener("click",async()=>{const l=document.getElementById("page-content")||document.getElementById("app");if(d.dataset.mini==="1"){const{data:c}=await v.from("mini_league_matches").select("id, league_id").eq("match_id",d.dataset.resume).single();c?i("match-mini-league",{mlMatchId:c.id,leagueId:c.league_id}):i("mini-league")}else{const{resumePvpMatch:c}=await Hi(async()=>{const{resumePvpMatch:f}=await Promise.resolve().then(()=>io);return{resumePvpMatch:f}},void 0);c(l,{state:e,navigate:i,toast:t,openModal:null,closeModal:null,refreshProfile:null},d.dataset.resume)}})}),n.querySelectorAll("[data-abandon]").forEach(d=>{d.addEventListener("click",()=>{Ho(async()=>{await Oo(d.dataset.abandon,d.dataset.opp,o),t("Match abandonné (défaite 3-0)","info"),qn(e,t,i)})})})}async function Oo(e,t,i){It();const{data:n}=await v.from("matches").select("home_id, away_id, game_state, mode").eq("id",e).single();if(!n)return;const o=n.home_id===i,r=o?0:3,a=o?3:0,s=n.game_state||{};s.p1Score=r,s.p2Score=a,s.phase="finished",s.forfeit=!0,await v.from("matches").update({status:"finished",forfeit:!0,winner_id:t,home_score:r,away_score:a,game_state:s}).eq("id",e),n.mode==="mini_league"&&await v.from("mini_league_matches").update({status:"finished",home_score:r,away_score:a}).eq("match_id",e)}function Ho(e){const t=document.createElement("div");t.className="modal-overlay",t.innerHTML=`<div class="modal" style="max-width:340px">
    <div class="modal-body" style="padding:26px 22px 20px;text-align:center">
      <div style="font-size:40px;margin-bottom:8px">⚠️</div>
      <div style="font-size:17px;font-weight:900;margin-bottom:6px;color:#1a1a1a">Abandonner le match ?</div>
      <div style="font-size:13px;color:#666;margin-bottom:18px">Tu perdras par forfait <b>3-0</b>.</div>
  ...
  </div>`,(l=document.getElementById("match-inv-accept"))==null||l.addEventListener("click",()=>{n.innerHTML="",i("match",{matchMode:"friend",friendId:a,friendName:r,isAccepting:!0})}),(c=document.getElementById("match-inv-decline"))==null||c.addEventListener("click",async()=>{await v.from("friend_match_invites").update({status:"declined"}).eq("id",o.id),n.innerHTML="",t("Invitation refusée","info")})}async function Dn(e,t){const i=document.getElementById("friend-requests-banner");if(!i)return;const{data:n,error:o}=await v.from("friendships").select("id, requester:users!requester_id(pseudo, club_name)").eq("addressee_id",e.user.id).eq("status","pending");if(o||!(n!=null&&n.length)){i.innerHTML="";return}const r=n.length,a=n.slice(0,2).map(d=>{var l;return((l=d.requester)==null?void 0:l.pseudo)||"?"}).join(", "),s=r>2?` +${r-2}`:"";i.innerHTML=`<div id="friend-req-btn" style="display:flex;align-items:center;gap:10px;background:linear-gradient(135deg,rgba(26,107,60,0.6),rgba(42,157,92,0.4));color:#fff;border-radius:12px;padding:12px 16px;border:1px solid rgba(26,107,60,0.4);cursor:pointer;box-shadow:0 3px 12px rgba(26,107,60,0.25)">
    <div style="background:rgba(255,255,255,0.15);border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0">👥</div>
    <div style="flex:1;min-width:0"><div style="font-size:13px;font-weight:900">${r} demande${r>1?"s":""} d'ami${r>1?"s":""}</div><div style="font-size:11px;opacity:0.85;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${a}${s}</div></div>
    <div style="font-size:20px;flex-shrink:0">›</div>
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:0 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;align-items:center"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",M=>{S=M.target.value.toLowerCase(),K()}),e.querySelectorAll(".filter-btn").forEach(M=>{M.addEventListener("click",()=>{$=M.dataset.job,z(),K()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{G=!G,z(),K()})):(k.innerHTML=`
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(g=document.getElementById("direct-sell-form-btn"))==null||g.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${l} pour ${li.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const y=b.find(h=>!h.is_for_sale)||b[0];if(!y){r("Aucune carte à vendre","error");return}await v.from("market_listings").delete().eq("card_id",y.id),await v.from("transfer_history").delete().eq("card_id",y.id);const{error:w}=await v.from("cards").delete().eq("id",y.id);if(w){r(w.message,"error");return}await v.from("users").update({credits:(o.profile.credits||0)+li}).eq("id",o.profile.id),await d(),r(`+${li.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),s("collection")}),(p=document.getElementById("market-sell-form-btn"))==null||p.addEventListener("click",async()=>{const y=parseInt(document.getElementById("sell-price-form").value);if(!y||y<1){r("Prix invalide","error");return}await v.from("cards").update({is_for_sale:!0,sale_price:y}).eq("id",e.id),await v.from("market_listings").insert({seller_id:o.profile.id,card_id:e.id,price:y}),r("Carte mise en vente sur le marché !","success"),a(),s("collection")}),(u=document.getElementById("cancel-sell-form-btn"))==null||u.addEventListener("click",async()=>{await v.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await v.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),r("Annonce retirée","success"),a(),s("collection")})}async function yn(e,t,i,n){var me,j,V,ue,pe,ce;const{state:o,toast:r,openModal:a,closeModal:s,navigate:d,refreshProfile:l}=n,c=e.player,f=t.filter(k=>k.player.id===c.id),b=f.length,x=e.evolution_bonus||0,g=Math.max((Number(c.note_g)||0)+(c.job==="GK"||c.job2==="GK"?x:0),(Number(c.note_d)||0)+(c.job==="DEF"||c.job2==="DEF"?x:0),(Number(c.note_m)||0)+(c.job==="MIL"||c.job2==="MIL"?x:0),(Number(c.note_a)||0)+(c.job==="ATT"||c.job2==="ATT"?x:0)),p=c.rarity||"normal",{data:u}=await v.from("sell_price_configs").select("*").eq("rarity",p).lte("note_min",g).gte("note_max",g).order("note_min",{ascending:!1}).limit(1);((me=u==null?void 0:u[0])==null?void 0:me.price)??Xo[p];const y=c.rarity!=="legende";At(c);const w=di(c,c.job)+x,h=c.job2?di(c,c.job2)+(di(c,c.job2)>0?x:0):null;Ii[c.job],c.job2&&Ii[c.job2];const E=Yo[c.rarity]||"#ccc";Qo[c.country_code]||c.country_code;const m=e.evolution_bonus||0,$=w+m,S=h||0,G=S>0?S+m:0,Q=f.map(k=>k.id);let ie={};if(Q.length){const{data:k}=await v.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",Q).order("transferred_at",{ascending:!0});(k||[]).forEach(M=>{ie[M.card_id]||(ie[M.card_id]=[]),ie[M.card_id].push(M)})}const N=Q.length?`
    <div style="margin-top:16px;border-top:1px solid var(--tile-border);padding-top:14px">
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px">
        ${f.filter(k=>k.id!==e.id).map((k,M)=>{const O=ie[k.id]||[],F=k.is_for_sale,Y=O.length?O[O.length-1]:null,q=k.evolution_bonus||0,H=Y?Y.source==="booster"?"Booster":Y.price?Y.price.toLocaleString("fr")+" cr.":"—":"—",le=Y?new Date(Y.transferred_at).toLocaleDateString("fr",{day:"2-digit",month:"2-digit",year:"numeric"}):"",ne=80,fe=Math.round(ne*657/507),we=Te({...c,_evolution_bonus:q},{width:ne});return`
      ...
      <!-- Carte visuelle -->
      ${Te({...c,_evolution_bonus:x},{width:160})}
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",c.note_g],["DEF",c.note_d],["MIL",c.note_m],["ATT",c.note_a]].map(([k,M])=>{const O=Ii[k],F=k==="GK"?"#fff":O,Y=k===c.job||k===c.job2,q=(Number(M)||0)+(Y&&m>0?m:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
    ...
    ${N}
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(j=document.getElementById("close-detail"))==null||j.addEventListener("click",s);let I=new Set;const z=c.rarity==="pepite"?1.3:c.rarity==="papyte"?.7:1;function K(){let k=0;return document.querySelectorAll(".expl-check:checked").forEach(M=>{if(M.dataset.id===e.id)return;const F=Number(M.dataset.evo)||0,Y=Number(M.dataset.note)||0;k+=Y+F}),Math.round(k*z)}const W=()=>{const k=I.size,M=document.getElementById("sell-action-panel");if(!M)return;M.style.display=k>0?"block":"none",document.getElementById("sell-selected-count").textContent=k;const O=document.getElementById("evolve-btn");if(O){const F=K();O.textContent=F>0?`⬆️ Évoluer (+${F})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach(k=>{k.addEventListener("change",()=>{const M=k.dataset.id;k.checked?I.add(M):I.delete(M);const O=k.closest(".exemplaire-row");if(O){const F=O.querySelector(".expl-sel-overlay"),Y=O.querySelector(".expl-sel-check");F&&(F.style.display=k.checked?"block":"none"),Y&&(Y.style.display=k.checked?"flex":"none"),O.style.transform=k.checked?"scale(1.05)":"scale(1)"}W()})}),document.querySelectorAll(".exemplaire-row").forEach(k=>{k.addEventListener("click",M=>{if(M.target.tagName==="INPUT")return;const O=k.querySelector(".expl-check");O&&!O.disabled&&(O.checked=!O.checked,O.dispatchEvent(new Event("change")))})}),(V=document.getElementById("evolve-btn"))==null||V.addEventListener("click",async()=>{if(b<=1)return;const k=[...I];if(!k.length)return;if(I.has(e.id)){const ne=document.createElement("div");ne.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",ne.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;const c=window.innerWidth>=900,f=e.querySelector(".deck-pc-layout"),b=e.querySelector(".deck-mobile-layout");f&&(f.style.display=c?"block":"none"),b&&(b.style.display=c?"none":"block"),nr(e,t,o,i),e.querySelectorAll("#builder-back").forEach(u=>u.addEventListener("click",()=>n("decks")));const x=()=>{const{openModal:u,closeModal:y}=i,w=`<div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px">
    ...
    </div>`;u("⚽ Choisir une formation",w),document.querySelectorAll("#modal-body [data-forma]").forEach(h=>{h.addEventListener("click",()=>{t.formation=h.dataset.forma;const E=wn(t.formation),m={};E.forEach(C=>{t.slots[C]&&(m[C]=t.slots[C])}),t.slots=m,y(),wt(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",x)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>or(t,e,i))),e.querySelectorAll("#save-deck").forEach(u=>u.addEventListener("click",()=>sr(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{t.subs=t.subs.filter(y=>y!==u.dataset.removeSub),wt(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>ar(t,e,i)))}function nr(e,t,i,n){var h,E,m;const o=window.innerWidth>=900,r=e.querySelector(o?"#deck-field-pc":"#deck-field-mobile");if(!r)return;const a=(h=t.stadiumCards)==null?void 0:h.find(C=>C.id===t.stadiumCardId),s=a&&((E=t.stadDefMap)==null?void 0:E[a.stadium_id])||null,d=Mt[t.formation]||{},l=Ni(t.formation)||[],c={};for(const C of i){const $=t.slots[C],S=$?t.playerCards.find(G=>G.id===$):null;S!=null&&S.player?c[C]={...S.player,_evolution_bonus:S.evolution_bonus||0,face:S.player.face||null}:c[C]=null}const f=window.innerWidth>=900,b=f?window.innerWidth-280:window.innerWidth-20,x=f?Math.min(b,860):b,g=Math.round(f?x*.82:x*.85),p=f?84:44;let u="";for(const[C,$]of l){const S=d[C],G=d[$];if(!S||!G)continue;const Q=S.x*x,ie=Math.round(.03*g+S.y*.85*g),N=G.x*x,I=Math.round(.03*g+G.y*.85*g),z=c[C],K=c[$],W=Ht(z,K);W==="#ff3333"||W==="#cc2222"?u+=`<line x1="${Q.toFixed(1)}" y1="${ie.toFixed(1)}" x2="${N.toFixed(1)}" y2="${I.toFixed(1)}" stroke="${W}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(u+=`<line x1="${Q.toFixed(1)}" y1="${ie.toFixed(1)}" x2="${N.toFixed(1)}" y2="${I.toFixed(1)}" stroke="${W}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,u+=`<line x1="${Q.toFixed(1)}" y1="${ie.toFixed(1)}" x2="${N.toFixed(1)}" y2="${I.toFixed(1)}" stroke="${W}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let y="";const w=Math.round(p*657/507);for(const C of i){const $=d[C];if(!$)continue;const S=c[C],G=$.x*x;$.y*g;const Q=Math.round(.03*g+$.y*(.85*g)),ie=Math.round(G-p/2),N=Math.round(Q-w/2);if(S){const I=C.replace(/\d+/,""),z=s&&(s.club_id&&String(S.club_id)===String(s.club_id)||s.country_code&&S.country_code===s.country_code),K=Te({...S,_evolution_bonus:S._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:I});z&&((m=s.club)!=null&&m.logo_url||s.image_url),y+=`<div style="position:absolute;left:${ie}px;top:${N}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${C}">
        <div style="position:relative">${K}</div>
      </div>`}else{const I=C.replace(/\d+/,"");y+=`<div style="position:absolute;left:${ie}px;top:${N}px;width:${p}px;height:${w}px;
  ...
  <div class="page">
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${r.map(g=>{const p=g.cost===0||o>=g.cost,u=c[g.id]===!0;return`<div class="booster-card ${!p||u?"disabled":""}" data-booster="${g.id}" style="position:relative">
  ...
  `,document.body.appendChild(o);let r=!1;const a=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let d=!1;const l=z=>z.touches&&z.touches[0]?z.touches[0].clientX:z.clientX;function c(z){r||(d=!0,s.style.opacity="1",f(z))}function f(z){if(!d||r)return;const K=a.getBoundingClientRect(),W=l(z)-K.left,de=Math.max(0,Math.min(1,W/K.width));s.style.width=de*K.width+"px",de>=.82&&x()}function b(){r||(d=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{r||(s.style.transition="")},220))}function x(){var K;if(r)return;r=!0,d=!1,s.style.width="100%",s.style.opacity="1",(K=document.getElementById("cut-flash"))==null||K.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const z=document.getElementById("cut-hint");z&&(z.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",y(0)},620)}a.addEventListener("pointerdown",c),window.addEventListener("pointermove",f),window.addEventListener("pointerup",b),a.addEventListener("touchstart",c,{passive:!0}),window.addEventListener("touchmove",f,{passive:!0}),window.addEventListener("touchend",b);let g=0,p=!1;const u=new Set;function y(z){g=z,document.getElementById("reveal-phase").style.display="flex",w(),h(z,0),S()}function w(){const z=document.getElementById("card-dots");z&&(z.innerHTML=e.map((K,W)=>`<div class="card-dot" data-i="${W}" style="width:8px;height:8px;border-radius:50%;background:${W===g?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),z.querySelectorAll(".card-dot").forEach(K=>K.addEventListener("click",()=>m(parseInt(K.dataset.i)))))}function h(z,K){var k,M;const W=e[z],de=document.getElementById("card-counter"),me=document.getElementById("card-track");de&&(de.textContent=`Carte ${z+1} / ${e.length}`);const j=document.getElementById("reveal-btns");j&&(j.style.display=z===e.length-1?"flex":"none");const V=W.card_type==="player"&&((k=W.player)==null?void 0:k.rarity)==="legende",ue=!u.has(z);u.add(z);let pe=0;if(W.card_type==="player"&&W.player){const O=W.player,F=O.job||"ATT";pe=(Number(F==="GK"?O.note_g:F==="DEF"?O.note_d:F==="MIL"?O.note_m:O.note_a)||0)+(W.evolution_bonus||0)}const ce=O=>{me.innerHTML=`
        ...
        ${ce}
      </div>`,M&&navigator.vibrate&&navigator.vibrate([50,30,100,30,200])},(H+le)*3),setTimeout(q,(H+le)*3+H),setTimeout(()=>{O.style.display="none",F.innerHTML="",p=!1,Y&&!k&&Y.pause(),navigator.vibrate&&navigator.vibrate([40,30,80]),K()},(H+le)*4)}function m(z){if(p||z<0||z>=e.length||z===g)return;const K=z>g?1:-1;g=z,h(z,K)}function C(){m(g+1)}function $(){m(g-1)}function S(){const z=document.getElementById("card-viewport");if(!z||z._swipeBound)return;z._swipeBound=!0;let K=0,W=0,de=0,me=!1;const j=k=>k.touches?k.touches[0].clientX:k.clientX,V=k=>k.touches?k.touches[0].clientY:k.clientY,ue=k=>{me=!0,K=j(k),W=V(k),de=0},pe=k=>{if(!me)return;de=j(k)-K;const M=V(k)-W;if(Math.abs(de)<Math.abs(M))return;const O=document.getElementById("current-card-wrap");O&&(O.style.transition="none",O.style.transform=`translateX(${de*.6}px) rotate(${de*.02}deg)`)},ce=()=>{if(!me)return;me=!1;const k=document.getElementById("current-card-wrap"),M=55;de<=-M&&g<e.length-1?C():de>=M&&g>0?$():k&&(k.style.transition="transform .2s ease",k.style.transform="translateX(0)")};z.addEventListener("pointerdown",ue),z.addEventListener("pointermove",pe),z.addEventListener("pointerup",ce),z.addEventListener("pointercancel",ce),z.addEventListener("touchstart",ue,{passive:!0}),z.addEventListener("touchmove",pe,{passive:!0}),z.addEventListener("touchend",ce),z.addEventListener("click",k=>{if(Math.abs(de)>8)return;const M=z.getBoundingClientRect();k.clientX-M.left>M.width/2?C():$()})}let G=null;function Q(){const z=document.getElementById("fireworks-canvas");if(!z)return;z.width=window.innerWidth,z.height=window.innerHeight;const K=z.getContext("2d"),W=[];function de(){const j=Math.random()*z.width,V=Math.random()*z.height*.6,ue=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],pe=ue[Math.floor(Math.random()*ue.length)];for(let ce=0;ce<60;ce++){const k=Math.PI*2/60*ce,M=2+Math.random()*5;W.push({x:j,y:V,vx:Math.cos(k)*M,vy:Math.sin(k)*M,alpha:1,color:pe,size:2+Math.random()*3})}}de(),G=setInterval(de,600);function me(){if(document.getElementById("fireworks-canvas")){K.clearRect(0,0,z.width,z.height);for(let j=W.length-1;j>=0;j--){const V=W[j];if(V.x+=V.vx,V.y+=V.vy+.08,V.vy*=.98,V.alpha-=.018,V.alpha<=0){W.splice(j,1);continue}K.globalAlpha=V.alpha,K.fillStyle=V.color,K.beginPath(),K.arc(V.x,V.y,V.size,0,Math.PI*2),K.fill()}K.globalAlpha=1,(G!==null||W.length>0)&&requestAnimationFrame(me)}}me()}function ie(){G!==null&&(clearInterval(G),G=null);const z=document.getElementById("fireworks-canvas");z&&z.getContext("2d").clearRect(0,0,z.width,z.height)}if(n){const z=document.getElementById("reveal-btns");z&&(z.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(I=document.getElementById("reveal-next"))==null||I.addEventListener("click",()=>{ie(),o.remove(),n()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{ie(),o.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{ie(),o.remove(),i("boosters")})}function xr(e){var t,i,n,o;if(e.card_type==="player"&&e.player)return pr(e);if(e.card_type==="game_changer"){const r=e._gcDef,a=(r==null?void 0:r.name)||e.gc_type||"Game Changer",s=(r==null?void 0:r.effect)||((t=ji[e.gc_type])==null?void 0:t.desc)||"",d=r!=null&&r.image_url?`/icons/${r.image_url}`:null,l=((i=ji[e.gc_type])==null?void 0:i.icon)||"⚡";return tt(a,d,l,s,{width:170})}if(e.card_type==="formation")return Ai(e.formation,Mt[e.formation],{width:160});if(e.card_type==="stadium"){const r=e._stadiumDef,a=(r==null?void 0:r.name)||"Stade",s=((n=r==null?void 0:r.club)==null?void 0:n.encoded_name)||(r==null?void 0:r.country_code)||"—",d=r!=null&&r.image_url?`/icons/${r.image_url}`:((o=r==null?void 0:r.club)==null?void 0:o.logo_url)||(r!=null&&r.country_code?`https://flagsapi.com/${r.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),l=`${s}<br>+10 ⭐ joueurs alliés`;return ei(a,d,l,{width:170})}return'<div style="width:140px;height:200px;background:#333;border-radius:12px"></div>'}function br(e){var t,i;if((t=e==null?void 0:e.rates)!=null&&t.length){const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;align-items:center;justify-content:center;z-index:4000;padding:16px";const o={normal:"#ccc",pepite:"#D4A017",pépite:"#D4A017",papyte:"#909090",legende:"#7a28b8",légende:"#7a28b8"},r={player:"Joueur",formation:"Formation",game_changer:"Game Changer",game_helper:"Game Helper"};n.innerHTML=`
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <div id="deck-top-bar" style="flex-shrink:0">
        ...
        <div style="display:flex;align-items:center;gap:8px;padding:6px 8px">
          ...
          <button id="next-deck" style="width:40px;height:40px;border-radius:50%;background:rgba(255,255,255,${c===r.length-1?"0.05":"0.15"});border:2px solid rgba(255,255,255,${c===r.length-1?"0.1":"0.3"});color:${c===r.length-1?"rgba(255,255,255,0.2)":"#fff"};font-size:18px;cursor:${c===r.length-1?"default":"pointer"};flex-shrink:0">▶</button>
        </div>
        ${w?`
      ...
      <!-- Pagination -->
      ${r.length>1?`<div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">${r.map((N,I)=>`<div style="width:6px;height:6px;border-radius:50%;background:${I===c?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}</div>`:""}
    ...
    </div>`,requestAnimationFrame(()=>requestAnimationFrame(function N(){const I=e.querySelector(".deck-preview-wrap"),z=e.querySelector("#deck-swipe-zone");if(!I||!z||!y)return;const K=z.clientWidth>=900,W=Math.max(200,z.clientHeight-(K?60:40)),me=Math.max(200,z.clientWidth-16),j=K?Math.min(117,Math.max(52,Math.round(me*.22))):Math.max(44,Math.round(me*.168));if(W<220||me<220){requestAnimationFrame(N);return}const V=K?null:Math.round(j*.55);I.innerHTML=Nt(y,u,null,[],me,W,[],V),I.style.cssText=`width:${me}px;height:${W}px;overflow:hidden;margin:${K?0:60}px auto 0`;const ue=I.querySelector("svg");ue&&(ue.style.cssText="display:block;width:100%;height:100%",ue.setAttribute("preserveAspectRatio",K?"xMidYMid meet":"none"))})),(S=document.getElementById("prev-deck"))==null||S.addEventListener("click",()=>{c>0&&(c--,f())}),(G=document.getElementById("next-deck"))==null||G.addEventListener("click",()=>{c<r.length-1&&(c++,f())}),(Q=document.getElementById("validate-deck"))==null||Q.addEventListener("click",()=>{if(!h)return;const N=t.state.params||{};t.navigate("match",{...N,matchMode:N.matchMode||i,deckId:b.id})}),(ie=document.getElementById("cancel-deck-select"))==null||ie.addEventListener("click",()=>{it(e),o("home")});const E=document.getElementById("deck-swipe-zone");if(E){let N=0,I=0;E.addEventListener("touchstart",z=>{N=z.touches[0].clientX,I=z.touches[0].clientY},{passive:!0}),E.addEventListener("touchend",z=>{const K=z.changedTouches[0].clientX-N,W=z.changedTouches[0].clientY-I;Math.abs(K)<40||Math.abs(K)<Math.abs(W)||(K<0&&c<r.length-1?(c++,f()):K>0&&c>0&&(c--,f()))},{passive:!0})}}f()}function vt(e,t=44,i=58,n=null){return Te(e,{width:t,showStad:!!n,stadDef:n,used:e==null?void 0:e.used})}function Je(e,t,i,n,o){if(!(e!=null&&e.length))return"";const r=e.slice(0,5);let a='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return r.forEach((s,d)=>{const l=s._line||s.job||"MIL";let c=s.boost||0;if(s.stadiumBonus&&(n==="attack"&&(l==="ATT"||l==="MIL")||n==="defense"&&(l==="GK"||l==="DEF"||l==="MIL")?c+=10:n||(c+=10)),a+=Te(s,{width:40,role:l,extraNote:c}),d<r.length-1){const f=r[d+1],b=Gn(s,f,o)?Ht(s,f):null;a+=`<div style="width:7px;height:3px;background:${!b||b==="#ff3333"||b==="#cc2222"?"rgba(255,255,255,0.12)":b};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(a+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),a+="</div>",a}function wi(e,t,i,n,o=310,r=310,a=[],s=null){const d=Mt[t]||{},l=Ni(t)||Oi[t]||[],c={},f=["ATT","MIL","DEF","GK"];for(const y of f)(e[y]||[]).forEach((h,E)=>{c[`${y}${E+1}`]=h});function b(y){const w=d[y];return w?{x:w.x*o,y:w.y*r}:null}let x="";for(const[y,w]of l){const h=b(y),E=b(w);if(!h||!E)continue;const m=c[y],C=c[w],$=Ht(m,C);$==="#00ff88"||$==="#FFD700"?(x+=`<line x1="${h.x.toFixed(1)}" y1="${h.y.toFixed(1)}" x2="${E.x.toFixed(1)}" y2="${E.y.toFixed(1)}"
    ...
    ${wi(e,t,i,n,o,r,a,s)}
          ...
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_real,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",d).order("slot_order")]);l=$[0].data,f=$[0].error,c=$[1].data,b=$[1].error}catch($){console.error("[Match] Exception chargement deck:",$),zt(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>r("home"));return}if(f||b){console.error("[Match] Erreur Supabase:",f||b),zt(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>r("home"));return}const x=(c||[]).filter($=>{var S;return $.is_starter&&((S=$.card)==null?void 0:S.player)}).map($=>qi($.card,$.position)),g=(c||[]).filter($=>{var S;return!$.is_starter&&((S=$.card)==null?void 0:S.player)}).map($=>qi($.card,$.position));if(x.length<11){zt(e,"⚠️",`Deck incomplet (${x.length}/11).`,"Compléter",()=>r("decks"));return}const p=(c||[]).find($=>{var S;return((S=$.card)==null?void 0:S.card_type)==="formation"}),u=(l==null?void 0:l.formation)||((C=p==null?void 0:p.card)==null?void 0:C.formation)||"4-4-2",{data:y,error:w}=await v.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",o.profile.id).eq("card_type","game_changer"),{data:h}=await v.from("gc_definitions").select("*").eq("is_active",!0),E=(y||[]).map($=>({...$,_gcDef:(h==null?void 0:h.find(S=>S.name===$.gc_type||S.id===$.gc_definition_id))||null}));let m=null;if(l!=null&&l.stadium_card_id){const{data:$}=await v.from("cards").select("stadium_id").eq("id",l.stadium_card_id).single();if($!=null&&$.stadium_id){const{data:S}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",$.stadium_id).single();m=S||null}}n({deckId:d,formation:u,starters:x,subsRaw:g,gcCardsEnriched:E,gcDefs:h||[],stadiumDef:m})}function Tt(){return Math.min(window.innerWidth-40,860)}function Gt(){return Math.round(Tt()*1.1)}function Jn(e){var o,r;if(!e)return null;const t=e._line||e.job||"MIL",i=t==="GK"?e.note_g||0:t==="DEF"?e.note_d||0:t==="MIL"?e.note_m||0:e.note_a||0,n=e.stadiumBonus?10:0;return{name:e.name,firstname:e.firstname||"",note:i+(e.boost||0)+n,note_g:e.note_g||0,note_d:e.note_d||0,note_m:e.note_m||0,note_a:e.note_a||0,_evolution_bonus:0,stadiumBonus:e.stadiumBonus||!1,boost:e.boost||0,job:e.job,job2:e.job2||null,_line:e._line||e.job,_col:e._col,country_code:e.country_code,club_id:e.club_id,rarity:e.rarity,clubName:e.clubName||((o=e.clubs)==null?void 0:o.encoded_name)||null,clubLogo:e.clubLogo||((r=e.clubs)==null?void 0:r.logo_url)||null,face:e.face||null,portrait:At(e)}}function Er(e,t,i,n="Adversaire"){An("/sounds/match-opening.mp3",.3);const o=Tt(),r=Gt();return`
    ...
    </div>`}function Qn(e){var r,a,s;if(!e)return"";const t=d=>d?Te({...d,_evolution_bonus:0},{width:52,role:d._line||d.job,showStad:!!d.stadiumBonus,extraNote:d.boost||0}):"",n={goal:"⚽","goal-home":"⚽","goal-ai":"⚽",duel:"⚔️",midfield:"🎯",sub:"🔄",gc:"⚡",boost:"💥","defense-won":"🛡️","attack-won":"⚔️","defense-lost":"😓","attack-lost":"😓"}[e.type]||"📋";return`
    ...
    pointer-events:none;white-space:nowrap;max-width:90vw;text-align:center`,o.textContent=e,!document.getElementById("game-toast-style")){const r=document.createElement("style");r.id="game-toast-style",r.textContent=`
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${e.map((r,a)=>{const s=a<e.length-1?Ht(r,e[a+1]):null,d=!s||s==="#ff3333"||s==="#cc2222",l=s==="#00ff88"?"+10":s==="#FFD700"?"+5":"";return Zn(r,o),r.stadiumBonus||o&&(o.club_id&&String(r.club_id)===String(o.club_id)||o.country_code&&(r.country_code,o.country_code)),`
    ...
    </div>`}async function Ir(e,t){const{state:i}=t,o=(i.params||{}).matchMode||"vs_ai_easy",r=o.replace("vs_ai_",""),a=o;await _i(e,t,o,async({deckId:s,formation:d,starters:l,subsRaw:c,gcCardsEnriched:f,gcDefs:b,stadiumDef:x})=>{try{let g=gi(l,d);x&&(g=ri(g,x),mi(c,x));const p=await zr(d,r),u=p.lines||p,y=async w=>{try{const h=a==="vs_ai_club"?"club":a,{data:E,error:m}=await v.from("matches").insert({home_id:i.profile.id,away_id:null,mode:h,home_deck_id:s,status:"in_progress"}).select().single();if(m){console.error("[MatchIA] Erreur création match:",m),zt(e,"⚠️","Impossible de créer le match ("+m.message+").","Retour",()=>t.navigate("home"));return}const C=p.stadiumDef||null;C&&u&&(ri(u,C),mi(p.subs||[],C));const $={gcDefs:b||[],matchId:E==null?void 0:E.id,mode:a,difficulty:r,formation:d,homeTeam:g,aiTeam:u,homeSubs:c,subsUsed:0,maxSubs:Math.min(c.length,3),aiSubs:p.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((p.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:p.gcCards||[],aiUsedGc:[],aiStadiumDef:C,homeScore:0,aiScore:0,gcCards:w,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Sr(e,$,t)}catch(h){console.error("[MatchIA] Exception launchMatch:",h),zt(e,"⚠️","Erreur au lancement du match : "+h.message,"Retour",()=>t.navigate("home"))}};if(!f.length){y([]);return}vi(e,f,y)}catch(g){console.error("[MatchIA] Exception setup:",g),zt(e,"⚠️","Erreur de préparation du match : "+g.message,"Retour",()=>t.navigate("home"))}})}async function zr(e,t){var u;const{data:i}=await v.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:Tr(e),subs:[],gcCards:[],stadiumDef:null};const n=Rt[e]||Rt["4-4-2"],o={GK:[],DEF:[],MIL:[],ATT:[]},r=new Set;function a(y,w,h){var E,m;return r.add(y.id),{cardId:"ai-"+y.id+"-"+h,id:y.id,firstname:y.firstname,name:y.surname_real,country_code:y.country_code,club_id:y.club_id,job:y.job,job2:y.job2,note_g:Number(y.note_g)||0,note_d:Number(y.note_d)||0,note_m:Number(y.note_m)||0,note_a:Number(y.note_a)||0,rarity:y.rarity,skin:y.skin,hair:y.hair,hair_length:y.hair_length,clubName:((E=y.clubs)==null?void 0:E.encoded_name)||null,clubLogo:((m=y.clubs)==null?void 0:m.logo_url)||null,boost:0,used:!1,_line:w}}for(const y of["GK","DEF","MIL","ATT"]){const w=i.filter($=>$.job===y&&!r.has($.id)),h=i.filter($=>$.job!==y&&!r.has($.id)),E=[...w,...h],m=[];for(let $=0;$<n[y];$++){const S=E[$];S&&m.push(a(S,y,$))}const C=xt(m.length);m.forEach(($,S)=>{$._col=C[S]}),o[y]=m}const d=i.filter(y=>!r.has(y.id)).slice(0,5).map((y,w)=>a(y,y.job,100+w)),f=Object.keys(je).sort(()=>Math.random()-.5).slice(0,3).map((y,w)=>{var h,E;return{id:"ai-gc-"+w,gc_type:y,name:((h=je[y])==null?void 0:h.name)||y,icon:((E=je[y])==null?void 0:E.icon)||"⚡"}}),b=Object.values(o).flat(),x={};b.forEach(y=>{y.club_id&&(x[y.club_id]=(x[y.club_id]||0)+1)});const g=(u=Object.entries(x).sort((y,w)=>w[1]-y[1])[0])==null?void 0:u[0];let p=null;if(g){const{data:y}=await v.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",g).single();y&&(p={club_id:y.id,country_code:null,name:y.encoded_name+" Stadium",club:{encoded_name:y.encoded_name,logo_url:y.logo_url}})}return{lines:o,subs:d,gcCards:f,stadiumDef:p}}function Tr(e){const t=Rt[e]||Rt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let o=0;for(const r of["GK","DEF","MIL","ATT"]){const a=[];for(let d=0;d<t[r];d++){const l=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+o,id:"fake-"+o,firstname:"IA",name:n[o%n.length],country_code:"XX",club_id:null,job:r,job2:null,note_g:r==="GK"?l:2,note_d:r==="DEF"?l:2,note_m:r==="MIL"?l:2,note_a:r==="ATT"?l:2,rarity:"normal",boost:0,used:!1,_line:r}),o++}const s=xt(a.length);a.forEach((d,l)=>{d._col=s[l]}),i[r]=a}return i}function Sr(e,t,i){var o;An("/sounds/match-opening.mp3",.3);const n=`<div style="position:relative;width:22px;height:22px;flex-shrink:0;display:flex;align-items:center;justify-content:center">
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const j=window.innerWidth>=700,V=(q,H,le)=>{var Se,Be;const ne=(t.gcDefs||[]).find($e=>$e.name===q.gc_type),fe=(ne==null?void 0:ne.name)||q.gc_type,we=(ne==null?void 0:ne.effect)||((Se=je[q.gc_type])==null?void 0:Se.desc)||"",he=ne!=null&&ne.image_url?`/icons/${ne.image_url}`:null,Ie=((Be=je[q.gc_type])==null?void 0:Be.icon)||"⚡";return`<div class="gc-mini" data-gc-id="${q.id}" data-gc-type="${q.gc_type}" style="flex-shrink:0;cursor:pointer">
          ${tt(fe,he,Ie,we,{width:H})}
        ...
        <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
          ${Nt(t.homeTeam,t.formation,t.phase,n,j?1300:Tt(),j?600:Gt(),g)}
        </div>
      </div>`;return j?`
  ...
  <div id="match-history-panel">
    ...
    </div>`,(m=f.querySelector("#sub-close"))==null||m.addEventListener("click",()=>f.remove()),(C=f.querySelector("#out-up"))==null||C.addEventListener("click",()=>{s>0&&(s--,b())}),($=f.querySelector("#out-down"))==null||$.addEventListener("click",()=>{s<r.length-1&&(s++,b())}),(S=f.querySelector("#in-up"))==null||S.addEventListener("click",()=>{l>0&&(l--,b())}),(G=f.querySelector("#in-down"))==null||G.addEventListener("click",()=>{l<a.length-1&&(l++,b())});const E=(ie,N,I,z)=>{const K=f.querySelector("#"+ie);if(!K)return;let W=0;K.addEventListener("touchstart",de=>{W=de.touches[0].clientY},{passive:!0}),K.addEventListener("touchend",de=>{const me=de.changedTouches[0].clientY-W;if(Math.abs(me)<30)return;const j=N();me<0&&j<z-1?(I(j+1),b()):me>0&&j>0&&(I(j-1),b())},{passive:!0})};E("in-panel",()=>l,ie=>l=ie,a.length),E("out-panel",()=>s,ie=>s=ie,r.length),(Q=f.querySelector("#sub-confirm"))==null||Q.addEventListener("click",ie=>{if(ie.preventDefault(),ie.stopPropagation(),c)return;c=!0;const N=r[s],I=a[l];if(!N||!I)return;let z=null,K=-1;for(const[de,me]of Object.entries(t.homeTeam)){const j=(me||[]).findIndex(V=>V.cardId===N.cardId);if(j!==-1){z=de,K=j;break}}if(K===-1||!z){$t("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),f.remove();return}const W={...I,_line:z,_col:N._col||0,used:!1,boost:0};t.homeTeam[z].splice(K,1,W),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(I.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:N.name,firstname:N.firstname,note:Pe(N,z),portrait:At(N),job:N.job,country_code:N.country_code,rarity:N.rarity,clubName:N.clubName,clubLogo:N.clubLogo},inPlayer:{name:I.name,firstname:I.firstname,note:Pe(I,z),portrait:At(I),job:I.job,country_code:I.country_code,rarity:I.rarity,clubName:I.clubName,clubLogo:I.clubLogo},text:`🔄 ${I.firstname} ${I.name} remplace ${N.firstname} ${N.name}`}),f.remove(),Lr(N,I,()=>ke(e,t,i))})}document.body.appendChild(f),b()}function Fr(e,t,i,n,o){var b,x;const r=(n.gcDefs||[]).find(g=>g.name===t),a=je[t]||{icon:"⚡",desc:"Carte spéciale."},s=(r==null?void 0:r.name)||t,d=(r==null?void 0:r.effect)||a.desc,l=r!=null&&r.image_url?`/icons/${r.image_url}`:null,c=a.icon||"⚡",f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",f.innerHTML=`
    ${tt(s,l,c,d,{width:190})}
    <!-- Boutons -->
    <div style="display:flex;gap:12px;width:190px">
      <button id="gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
      <button id="gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
    </div>`,document.body.appendChild(f),(b=f.querySelector("#gc-back"))==null||b.addEventListener("click",()=>f.remove()),(x=f.querySelector("#gc-use"))==null||x.addEventListener("click",()=>{f.remove(),Gr(e,t,i,n,o)})}function ci(e,t,i,n,o,r){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let s=[];function d(){var l,c;a.innerHTML=`
    ...
    </div>`,(l=a.querySelector("#gc-picker-close"))==null||l.addEventListener("click",()=>a.remove()),a.querySelectorAll(".gc-pick-item").forEach(f=>{f.addEventListener("click",()=>{const b=f.dataset.cid,x=e.find(p=>p.cardId===b);if(!x)return;const g=s.findIndex(p=>p.cardId===b);g>-1?s.splice(g,1):s.length<t&&s.push(x),d()})}),(c=a.querySelector("#gc-picker-confirm"))==null||c.addEventListener("click",()=>{a.remove(),r(s)})}d(),document.body.appendChild(a)}const Pr={BOOST_STAT:({value:e=1,count:t=1,roles:i=[]},n,o,r)=>{const a=Object.entries(n.homeTeam).filter(([s])=>!i.length||i.includes(s)).flatMap(([s,d])=>d.filter(l=>!l.used).map(l=>({...l,_line:s})));return a.length?(ci(a,t,`Choisir ${t} joueur(s) à booster (+${e})`,o,n,s=>{s.forEach(d=>{const l=(n.homeTeam[d._line]||[]).find(c=>c.cardId===d.cardId);l&&(l.boost=(l.boost||0)+e,n.log.push({text:`⚡ +${e} sur ${l.name}`,type:"info"}))}),ke(o,n,r)}),!0):(n.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),ke(o,n,r),!0)},DEBUFF_STAT:({value:e=1,count:t=1,roles:i=[],target:n="ai"},o,r,a)=>{const s=n==="home"?o.homeTeam:o.aiTeam,d=n==="ai"?"adverse":"allié",l=Object.entries(s).filter(([c])=>!i.length||i.includes(c)).flatMap(([c,f])=>f.filter(b=>!b.used).map(b=>({...b,_line:c})));return l.length?(ci(l,t,`Choisir ${t} joueur(s) ${d}(s) à débuffer (-${e})`,r,o,c=>{c.forEach(f=>{const x=((n==="home"?o.homeTeam:o.aiTeam)[f._line]||[]).find(g=>g.cardId===f.cardId);x&&(x.boost=(x.boost||0)-e,o.log.push({text:`🎯 -${e} sur ${x.name}${n==="ai"?" (IA)":""}`,type:"info"}))}),ke(r,o,a)}),!0):(o.log.push({text:`🎯 Aucun joueur ${d} disponible`,type:"info"}),ke(r,o,a),!0)},GRAY_PLAYER:({count:e=1,roles:t=[],target:i="ai"},n,o,r)=>{const a=i==="home"?n.homeTeam:n.aiTeam,s=i==="ai"?"adverse":"allié",d=Object.entries(a).filter(([l])=>!t.length||t.includes(l)).flatMap(([l,c])=>c.filter(f=>!f.used).map(f=>({...f,_line:l})));return d.length?(ci(d,e,`Choisir ${e} joueur(s) ${s}(s) à exclure`,o,n,l=>{l.forEach(c=>{const b=((i==="home"?n.homeTeam:n.aiTeam)[c._line]||[]).find(x=>x.cardId===c.cardId);b&&(b.used=!0,n.log.push({text:`❌ ${b.name}${i==="ai"?" (IA)":""} exclu !`,type:"info"}))}),ke(o,n,r)}),!0):(n.log.push({text:`❌ Aucun joueur ${s} à exclure`,type:"info"}),ke(o,n,r),!0)},REVIVE_PLAYER:({count:e=1,roles:t=[]},i,n,o)=>{const r=Object.entries(i.homeTeam).filter(([a])=>!t.length||t.includes(a)).flatMap(([a,s])=>s.filter(d=>d.used).map(d=>({...d,_line:a})));return r.length?(ci(r,e,`Choisir ${e} joueur(s) à ressusciter`,n,i,a=>{a.forEach(s=>{const d=(i.homeTeam[s._line]||[]).find(l=>l.cardId===s.cardId);d&&(d.used=!1,i.log.push({text:`💫 ${d.name} ressuscité !`,type:"info"}))}),ke(n,i,o)}),!0):(i.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),ke(n,i,o),!0)},REMOVE_GOAL:({},e)=>(e.aiScore>0?(e.aiScore--,e.log.push({text:"🚫 Dernier but IA annulé !",type:"info"})):e.log.push({text:"🚫 Aucun but à annuler",type:"info"}),!1),ADD_GOAL_DRAW:({},e)=>(e.homeScore===e.aiScore?(e.homeScore++,e.log.push({text:"🎯 But bonus (match nul) !",type:"info"})):e.log.push({text:"🎯 But bonus : non applicable (pas de match nul)",type:"info"}),!1),ADD_SUB:({value:e=1},t)=>(t.maxSubs=(t.maxSubs||3)+e,t.log.push({text:`🔄 +${e} remplacement(s) débloqué(s)`,type:"info"}),!1),CUSTOM:()=>!1};function Gr(e,t,i,n,o){n.usedGc.push(e);const r=n.gcDefs||[],a=r.find(d=>d.name===t)||r.find(d=>{var l;return((l=d.name)==null?void 0:l.toLowerCase().trim())===(t==null?void 0:t.toLowerCase().trim())});let s=!1;if(a!=null&&a.effect_type&&a.effect_type!=="CUSTOM"){const d=Pr[a.effect_type];d?d(a.effect_params||{},n,i,o)||(s=!0):(o.toast(`Effet "${a.effect_type}" non implémenté`,"error"),s=!0)}else{switch(t){case"Double attaque":n.modifiers.home.doubleAttack=!0,n.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":n.modifiers.home.shield=!0,n.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const d=Object.entries(n.homeTeam).flatMap(([l,c])=>(c||[]).filter(f=>f.used).map(f=>({...f,_line:l})));d.length?(d[0].used=!1,n.log.push({text:`💫 ${d[0].name} ressuscité !`,type:"info"})):n.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":n.modifiers.ai.stolenNote=(n.modifiers.ai.stolenNote||0)+1,n.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const d=[...n.aiTeam.ATT||[],...n.aiTeam.MIL||[]].filter(l=>!l.used);if(d.length){const l=d.sort((c,f)=>Pe(f,"ATT")-Pe(c,"ATT"))[0];l.used=!0,n.log.push({text:`❄️ ${l.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":n.maxSubs++,n.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}s=!0}v.from("cards").delete().eq("id",e).then(()=>{}),s&&ke(i,n,o)}function Rr(e,t,i){const n=Object.values(t.homeTeam).flat().filter(o=>!o.used);if(!n.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(o=>{o.addEventListener("click",()=>{const r=o.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const s=(t.homeTeam[a]||[]).find(d=>d.cardId===r);if(s){s.boost=(s.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${s.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),ke(e,t,i)})})}async function ht(e,t,i){var c,f;It(),Ue(),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,o=t.homeScore>t.aiScore,r=t.homeScore===t.aiScore,a=o?"victoire":r?"nul":"defaite",s=Vo(t.mode,a);t.matchId&&await v.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:o?n.profile.id:null,home_credits_reward:s,played_at:new Date().toISOString()}).eq("id",t.matchId);const d={credits:(n.profile.credits||0)+s,matches_played:(n.profile.matches_played||0)+1};o?d.wins=(n.profile.wins||0)+1:r?d.draws=(n.profile.draws||0)+1:d.losses=(n.profile.losses||0)+1,await v.from("users").update(d).eq("id",n.profile.id),await i.refreshProfile();const l=document.createElement("div");l.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",l.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${wi(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            </div>`}}catch(Z){console.error("[Ranked] MMR update error:",Z)}(te=document.getElementById("pvp-end-overlay"))==null||te.remove();const D=document.createElement("div");D.id="pvp-end-overlay",D.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const P=T?"🤝":B?"🏆":"😞",U=T?"MATCH NUL":B?"VICTOIRE !":"DÉFAITE",J=T?"#fff":B?"#FFD700":"#ff6b6b";D.innerHTML=`
      ...
      ${R}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${c?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(D),(ee=D.querySelector("#pvp-end-home"))==null||ee.addEventListener("click",()=>{D.remove(),it(e),c?x("mini-league",{openLeagueId:c}):x(a?"ranked":"home")})}const N=v.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},_=>{var A;const L=_.new;try{if(L.status==="finished"||L.forfeit){if(C)return;C=!0,$&&(clearInterval($),$=null),Ue(),L.game_state&&(m=L.game_state),ie(L);return}if(L.game_state){const B=m;m=L.game_state;const T=m._lastGC;if(T&&T.seq&&!Q.has(T.seq)&&(Q.add(T.seq),T.by!==p)){k(T.type,T.by,()=>V());return}const R=B[p+"Score"]||0,D=B[u+"Score"]||0,P=m[p+"Score"]||0,U=m[u+"Score"]||0,J=P>R,te=U>D;if((J||te)&&!G.has(m.round)){G.add(m.round);const ee=[...m.log||[]].reverse().find(ae=>ae.isGoal),Z=((ee==null?void 0:ee.homePlayers)||[]).map(ae=>({name:ae.name,note:ae.note,portrait:ae.portrait,job:ae.job}));!!((A=ee==null?void 0:ee.text)!=null&&A.includes("DERNIER CORNER"))?we().then(()=>{Qe(Z,P,U,J,()=>V())}):Qe(Z,P,U,J,()=>V());return}V()}}catch(B){console.error("[PvP] crash:",B)}}).subscribe();async function I(_){Object.assign(m,_),m.lastActionAt=new Date().toISOString();const{error:L}=await v.from("matches").update({game_state:m}).eq("id",i);L&&g("Erreur de synchronisation","error");try{V()}catch(A){console.error("[PvP] renderPvpScreen crash:",A)}}async function z(){if(C)return;C=!0,It(),Ue(),$&&(clearInterval($),$=null);const _=n?h.away_id:h.home_id,L=n?"p2":"p1",A=n?"p1":"p2",B={...m,[L+"Score"]:3,[A+"Score"]:0,phase:"finished"},T=B.p1Score||0,R=B.p2Score||0;try{await v.from("matches").update({game_state:B}).eq("id",i);const{data:D,error:P}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:_,p_home_score:T,p_away_score:R,p_forfeit:!0});if(P&&console.error("[PvP] finish_pvp_match (forfeit):",P),c)try{await v.rpc("check_and_finish_mini_league",{p_league_id:c})}catch(U){console.error("[PvP] check_and_finish_mini_league (forfeit):",U)}try{const{data:U}=await v.rpc("apply_match_rewards",{p_match_id:i});U!=null&&U.success&&!(U!=null&&U.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(U){console.error("[PvP] apply_match_rewards (forfeit):",U)}}catch{}try{v.removeChannel(N)}catch{}setTimeout(()=>{it(e),c?x("mini-league",{openLeagueId:c}):x("home")},800)}const K={BOOST_STAT:({value:_=1,count:L=1,roles:A=[]},B,T)=>{const R=B[p+"Team"],D=Object.entries(R).filter(([P])=>!A.length||A.includes(P)).flatMap(([P,U])=>U.filter(J=>!J.used).map(J=>({...J,_line:P})));if(!D.length){B.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),T(B);return}W(D,L,`Choisir ${L} joueur(s) à booster (+${_})`,P=>{P.forEach(U=>{const J=(R[U._line]||[]).find(te=>te.cardId===U.cardId);J&&(J.boost=(J.boost||0)+_,B.log.push({text:`⚡ +${_} sur ${J.name}`,type:"info"}))}),B[p+"Team"]=R,T(B)})},DEBUFF_STAT:({value:_=1,count:L=1,roles:A=[],target:B="ai"},T,R)=>{const D=B==="home"?p:u,P=T[D+"Team"],U=B==="home"?"allié":"adverse",J=Object.entries(P).filter(([te])=>!A.length||A.includes(te)).flatMap(([te,ee])=>ee.map(Z=>({...Z,_line:te})));if(!J.length){T.log.push({text:`🎯 Aucun joueur ${U}`,type:"info"}),R(T);return}W(J,L,`Choisir ${L} joueur(s) ${U}(s) (-${_})`,te=>{te.forEach(ee=>{const Z=(P[ee._line]||[]).find(re=>re.cardId===ee.cardId);Z&&(Z.boost=(Z.boost||0)-_,T.log.push({text:`🎯 -${_} sur ${Z.name}`,type:"info"}))}),T[D+"Team"]=P,R(T)})},GRAY_PLAYER:({count:_=1,roles:L=[],target:A="ai"},B,T)=>{const R=A==="home"?p:u,D=B[R+"Team"],P=A==="home"?"allié":"adverse",U=Object.entries(D).filter(([J])=>!L.length||L.includes(J)).flatMap(([J,te])=>te.filter(ee=>!ee.used).map(ee=>({...ee,_line:J})));if(!U.length){B.log.push({text:`❌ Aucun joueur ${P}`,type:"info"}),T(B);return}W(U,_,`Choisir ${_} joueur(s) ${P}(s) à exclure`,J=>{const te="usedCardIds_"+R,ee=new Set(B[te]||[]);J.forEach(Z=>{const re=(D[Z._line]||[]).find(ae=>ae.cardId===Z.cardId);re&&(re.used=!0,ee.add(Z.cardId),B.log.push({text:`❌ ${re.name} exclu !`,type:"info"}))}),B[te]=[...ee],B[R+"Team"]=D,T(B)})},REVIVE_PLAYER:({count:_=1,roles:L=[]},A,B)=>{const T=A[p+"Team"],R=Object.entries(T).filter(([D])=>!L.length||L.includes(D)).flatMap(([D,P])=>P.filter(U=>U.used).map(U=>({...U,_line:D})));if(!R.length){A.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),B(A);return}W(R,_,`Choisir ${_} joueur(s) à ressusciter`,D=>{D.forEach(P=>{const U=(T[P._line]||[]).find(J=>J.cardId===P.cardId);U&&(U.used=!1,A.log.push({text:`💫 ${U.name} ressuscité !`,type:"info"}))}),A[p+"Team"]=T,B(A)})},REMOVE_GOAL:({},_,L)=>{const A=u+"Score";_[A]>0?(_[A]--,_.log.push({text:"🚫 Dernier but annulé !",type:"info"})):_.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L(_)},ADD_GOAL_DRAW:({},_,L)=>{_[p+"Score"]===_[u+"Score"]?(_[p+"Score"]++,_.log.push({text:"🎯 But bonus !",type:"info"})):_.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L(_)},ADD_SUB:({value:_=1},L,A)=>{L.maxSubs=(L.maxSubs||3)+_,L.log.push({text:`🔄 +${_} remplacement(s)`,type:"info"}),A(L)},CUSTOM:({},_,L)=>L(_)};function W(_,L,A,B){const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let R=[];function D(){var U,J;const P=_.map(te=>{const ee=te._line||te.job||"MIL",Z=R.find(ae=>ae.cardId===te.cardId),re=Te({...te,_evolution_bonus:0},{width:90,showStad:!0,role:ee,extraNote:te.boost||0});return`<div class="pp-item" data-cid="${te.cardId}"
        ...
        </div>`,(U=T.querySelector("#pp-close"))==null||U.addEventListener("click",()=>T.remove()),T.querySelectorAll(".pp-item").forEach(te=>{te.addEventListener("click",()=>{const ee=te.dataset.cid,Z=_.find(ae=>ae.cardId===ee),re=R.findIndex(ae=>ae.cardId===ee);Z&&(re>-1?R.splice(re,1):R.length<L&&R.push(Z),D())})}),(J=T.querySelector("#pp-confirm"))==null||J.addEventListener("click",()=>{T.remove(),B(R)})}D(),document.body.appendChild(T)}async function de(_,L){const B=(m["gcCardsFull_"+p]||[]).find(P=>P.id===_),T=(B==null?void 0:B._gcDef)||(m.gcDefs||[]).find(P=>{var U;return P.name===L||((U=P.name)==null?void 0:U.toLowerCase().trim())===(L==null?void 0:L.toLowerCase().trim())}),R=[...m["usedGc_"+p]||[],_],D={type:L,by:p,seq:(m._gcAnimSeq||0)+1};Q.add(D.seq),k(L,p,async()=>{if(T!=null&&T.effect_type&&T.effect_type!=="CUSTOM"){const U=K[T.effect_type];if(U){const J={...m};U(T.effect_params||{},J,async te=>{te["usedGc_"+p]=R,te._lastGC=D,te._gcAnimSeq=D.seq,await I(te)});return}}const P={...m};switch(L){case"Remplacement+":P.maxSubs=(P.maxSubs||3)+1,P.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const U=u+"Score";P[U]>0&&(P[U]--,P.log.push({text:"🚫 But annulé",type:"info"}));break}}P["usedGc_"+p]=R,P._lastGC=D,P._gcAnimSeq=D.seq,await I(P)})}function me(_,L){const A="usedCardIds_"+_,B=new Set(m[A]||[]);L.forEach(T=>B.add(T)),m[A]=[...B]}function j(){for(const _ of["p1","p2"]){const L=new Set(m["usedCardIds_"+_]||[]),A=m[_+"Team"];if(A)for(const B of["GK","DEF","MIL","ATT"])(A[B]||[]).forEach(T=>{T.used=L.has(T.cardId)})}}function V(){var on,rn,an,sn,dn,ln;if(m.phase==="reveal")return ue();if(m.phase==="midfield")return ce();if(m.phase==="finished")return Ge();const _=m[p+"Team"],L=m[u+"Team"];j();const A=m[p+"Score"],B=m[u+"Score"],T=m[p+"Name"],R=m[u+"Name"],D=m.phase===p+"-attack",P=m.phase===p+"-defense",U=Array.isArray(m["selected_"+p])?m["selected_"+p]:[],J=U.map(X=>X.cardId),te=window.innerWidth>=700,ee=m[p+"Subs"]||[],Z=m["usedSubIds_"+p]||[],re=ee.filter(X=>!Z.includes(X.cardId)),ae=m["gcCardsFull_"+p]||[],xe=m["usedGc_"+p]||[],se=ae.filter(X=>!xe.includes(X.id)),be=m.boostOwner===p&&!m.boostUsed,ve=[..._.MIL||[],..._.ATT||[]].filter(X=>!X.used),oe=[...L.MIL||[],...L.ATT||[]].filter(X=>!X.used),Le=!$e(L),Re=ve.length===0&&oe.length===0&&!Le,ze=(_.DEF||[]).filter(X=>!X.used),Ve=(_.GK||[]).filter(X=>!X.used);let Oe=[];D&&ve.length===0&&(Le?(Oe=ze.map(X=>X.cardId),ze.length===0&&(Oe=Oe.concat(Ve.map(X=>X.cardId)))):Re&&(Oe=[...ze,...Ve].map(X=>X.cardId)));function nt(X,ge,Ae){var Me,ft;const _e=X._gcDef,qt=(_e==null?void 0:_e.name)||X.gc_type,at=(_e==null?void 0:_e.effect)||((Me=je[X.gc_type])==null?void 0:Me.desc)||"",Fe=_e!=null&&_e.image_url?`/icons/${_e.image_url}`:null,Ne=((ft=je[X.gc_type])==null?void 0:ft.icon)||"⚡";return`<div class="pvp-gc-mini" data-gc-id="${X.id}" data-gc-type="${X.gc_type}" style="flex-shrink:0;cursor:pointer">
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-play-area" style="flex:1;min-height:0;display:flex;overflow:hidden">
          <div id="match-field" style="flex:1;min-width:0;min-height:0;overflow:hidden">
            <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
              ${Nt(_,m[p+"Formation"],ut,J,Tt(),Gt(),Oe)}
            </div>
          </div>
        </div>
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
      ...
      </div>`;function Bt(){const X=e.querySelector(".match-screen");if(!X)return;const ge=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);X.style.height=ge+"px",X.style.minHeight=ge+"px",X.style.maxHeight=ge+"px",X.style.overflow="hidden";const Ae=e.querySelector("#mobile-action-bar"),_e=e.querySelector("#mobile-play-area");Ae&&_e&&(_e.style.paddingBottom=Ae.offsetHeight+"px")}if(Bt(),setTimeout(Bt,120),setTimeout(Bt,400),S||(S=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Bt),window.visualViewport.addEventListener("scroll",Bt)),window.addEventListener("resize",Bt)),function(){const ge=e.querySelector("#match-field .terrain-wrapper svg")||e.querySelector(".terrain-wrapper svg");if(!ge)return;const Ae=ge.closest("#match-terrain-wrap");Ae&&(Ae.style.cssText="position:relative;width:100%;height:100%;padding:0"),ge.removeAttribute("width"),ge.removeAttribute("height"),ge.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ge.setAttribute("preserveAspectRatio","xMidYMid meet")}(),m._pvpResizeBound||(m._pvpResizeBound=!0,window.addEventListener("resize",()=>{const X=e.querySelector(".terrain-wrapper svg");if(X){const ge=X.closest("#match-terrain-wrap");ge&&(ge.style.cssText="position:relative;width:100%;height:100%;padding:0"),X.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0"}})),e.querySelectorAll(".match-slot-hit").forEach(X=>{X.addEventListener("click",()=>{if(!D&&!P)return;const ge=X.dataset.cardId,Ae=X.dataset.role,_e=(_[Ae]||[]).find(Ne=>Ne.cardId===ge);if(!_e||_e.used)return;const qt=Oe.includes(ge);if(D&&!["MIL","ATT"].includes(Ae)&&!qt)return;Array.isArray(m["selected_"+p])||(m["selected_"+p]=[]);const at=m["selected_"+p],Fe=at.findIndex(Ne=>Ne.cardId===ge);Fe>-1?at.splice(Fe,1):at.length<3&&at.push({..._e,_role:Ae}),V()})}),e.querySelectorAll(".match-used-hit").forEach(X=>{X.addEventListener("click",()=>F(X.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(X=>{X.addEventListener("click",()=>F())}),(on=e.querySelector("#pvp-sub-open"))==null||on.addEventListener("click",()=>F()),e.querySelectorAll(".pvp-gc-mini").forEach(X=>{D?X.addEventListener("click",()=>M(X.dataset.gcId,X.dataset.gcType)):(X.style.opacity="0.35",X.style.cursor="default",X.addEventListener("click",()=>$t("⚡ Les Game Changers ne sont utilisables qu'en attaque","rgba(180,100,0,0.9)")))}),(rn=e.querySelector("#pvp-boost-card"))==null||rn.addEventListener("click",()=>O()),(an=e.querySelector("#pvp-action"))==null||an.addEventListener("click",X=>{D?X.currentTarget.dataset.pass==="1"||!ct(p)?le():Ie():P&&Se()}),(sn=e.querySelector("#pvp-quit"))==null||sn.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&z()}),(dn=e.querySelector("#pvp-view-opp"))==null||dn.addEventListener("click",()=>q()),(ln=e.querySelector("#pvp-toggle-history"))==null||ln.addEventListener("click",()=>H()),$&&(clearInterval($),$=null),Ue(),(D||P)&&!C){let X=30,ge=!1;const Ae=()=>document.getElementById("pvp-timer"),_e=()=>{Ae()&&(Ae().textContent=X+"s",Ae().style.color=ge?"#ff4444":"#fff")};_e(),$=setInterval(()=>{X--,X<0?ge?(clearInterval($),$=null,Ue(),D&&!ct(p)?le():z()):(ge=!0,X=15,_e(),Mn("/sounds/timer-urgent.mp3",.6)):_e()},1e3)}}function ue(){st(e),e.innerHTML=`
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${L.map(T=>{const R={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[T._line]||"#555",D=Pe(T,T._line)+(T.boost||0);return`<div class="bp-item" data-cid="${T.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${R};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(A),(B=A.querySelector("#bp-close"))==null||B.addEventListener("click",()=>A.remove()),A.querySelectorAll(".bp-item").forEach(T=>{T.addEventListener("click",async()=>{const R=T.dataset.cid,D=L.find(U=>U.cardId===R);if(!D)return;const P=(_[D._line]||[]).find(U=>U.cardId===R);P&&(P.boost=(P.boost||0)+m.boostValue),A.remove(),await I({[p+"Team"]:_,boostUsed:!0})})})}function F(_=null){var re,ae;if(!(m.phase===p+"-attack")){g("Remplacement uniquement avant votre attaque","warning");return}const A=m[p+"Team"],B=m["usedSubIds_"+p]||[],T=m.maxSubs||3;if(B.length>=T){g(`Maximum ${T} remplacements atteint`,"warning");return}const R=Object.entries(A).flatMap(([xe,se])=>(se||[]).filter(be=>be.used).map(be=>({...be,_line:xe}))),D=(m[p+"Subs"]||[]).filter(xe=>!B.includes(xe.cardId));if(!R.length){g("Aucun joueur utilisé à remplacer","warning");return}if(!D.length){g("Aucun remplaçant disponible","warning");return}let P=Math.max(0,R.findIndex(xe=>xe.cardId===_));const U=((re=R[P])==null?void 0:re._line)||((ae=R[P])==null?void 0:ae.job);let J=Math.max(0,D.findIndex(xe=>xe.job===U)),te=!1;const ee=document.createElement("div");ee.id="pvp-sub-overlay",ee.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function Z(){var Re,ze,Ve,Oe,nt,Ut;const xe=R[P],se=D[J],be=Math.min(130,Math.round((window.innerWidth-90)/2)),ve=Math.round(be*1.35),oe=De=>`background:rgba(255,255,255,0.12);border:none;color:${De?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${De?"default":"pointer"};flex-shrink:0`;ee.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      </div>`,(Re=ee.querySelector("#psub-close"))==null||Re.addEventListener("click",()=>ee.remove()),(ze=ee.querySelector("#pout-up"))==null||ze.addEventListener("click",()=>{P>0&&(P--,Z())}),(Ve=ee.querySelector("#pout-down"))==null||Ve.addEventListener("click",()=>{P<R.length-1&&(P++,Z())}),(Oe=ee.querySelector("#pin-up"))==null||Oe.addEventListener("click",()=>{J>0&&(J--,Z())}),(nt=ee.querySelector("#pin-down"))==null||nt.addEventListener("click",()=>{J<D.length-1&&(J++,Z())});const Le=(De,Ye,He,pt)=>{const ot=ee.querySelector("#"+De);if(!ot)return;let jt=0;ot.addEventListener("touchstart",ut=>{jt=ut.touches[0].clientY},{passive:!0}),ot.addEventListener("touchend",ut=>{const kt=ut.changedTouches[0].clientY-jt;if(Math.abs(kt)<30)return;const rt=Ye();kt<0&&rt<pt-1?(He(rt+1),Z()):kt>0&&rt>0&&(He(rt-1),Z())},{passive:!0})};Le("pin-panel",()=>J,De=>J=De,D.length),Le("pout-panel",()=>P,De=>P=De,R.length),(Ut=ee.querySelector("#psub-confirm"))==null||Ut.addEventListener("click",async De=>{if(De.preventDefault(),De.stopPropagation(),te)return;te=!0;const Ye=R[P],He=D[J];if(!Ye||!He)return;const pt=Ye._line,ot=(A[pt]||[]).findIndex(kt=>kt.cardId===Ye.cardId);if(ot===-1){g("Erreur : joueur introuvable","error"),ee.remove();return}const jt={...He,_line:pt,position:Ye.position,used:!1,boost:0};A[pt].splice(ot,1,jt);const ut=[...B,He.cardId];ee.remove(),Y(Ye,He,async()=>{await I({[p+"Team"]:A,[u+"Team"]:m[u+"Team"],["usedSubIds_"+p]:ut})})})}document.body.appendChild(ee),Z()}function Y(_,L,A){const B={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const R=(U,J,te)=>`<div style="text-align:center">
      ...
      <div style="width:min(90vw,420px)">${wi(m[u+"Team"],m[u+"Formation"],null,[],Tt(),Gt())}</div>
      ...
      </div>`,document.body.appendChild(L),(A=L.querySelector("#pvp-hist-close"))==null||A.addEventListener("click",()=>L.remove())}async function le(){if(m.phase!==p+"-attack")return;const _=p==="p1"?"p2":"p1",L=(m.round||0)+1,A=[...m.log||[]];A.push({type:"info",text:`⏭️ ${m[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const B=Ee(m),T=ct(_),R=B||!T?"finished":_+"-attack";await I({["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:R,attacker:_,round:L,log:A}),R==="finished"&&await qe(m)}function ne(_){const L=(_.GK||[]).some(B=>!B.used),A=["DEF","MIL","ATT"].some(B=>(_[B]||[]).some(T=>!T.used));return L&&!A}function fe(_){return(_.p1Score||0)!==(_.p2Score||0)?null:ne(_.p1Team)&&!$e(_.p2Team)?{side:"p1",gk:(_.p1Team.GK||[]).find(L=>!L.used)}:ne(_.p2Team)&&!$e(_.p1Team)?{side:"p2",gk:(_.p2Team.GK||[]).find(L=>!L.used)}:null}async function we(){const _=document.createElement("div");_.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(_);const L=(A,B)=>new Promise(T=>{_.innerHTML=`<div style="font-size:32px;font-weight:900;color:${B};letter-spacing:2px;animation:lcFade 1.4s ease both">${A}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(T,1400)});await L("⚽ DERNIER CORNER","#FFD700"),await L("🧤 LE GARDIEN MONTE !","#4fc3f7"),_.remove()}async function he(_,L){const{side:A,gk:B}=_;if(!B)return;const T=A+"Score",R=(L[T]||0)+1;B.used=!0;const D=[...L.log||[]];D.push({type:"duel",isGoal:!0,homeScored:A===p,homePlayers:A==="p1"?[Dt(B)]:[],aiPlayers:A==="p2"?[Dt(B)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${L[A+"Name"]} marque !`});const P=(L.round||0)+1,U=A==="p1"?"p2":"p1",J={...L,[T]:R},te=Ee(J);G.add(P),await we();const ee=A===p?R:L[p+"Score"]||0,Z=A===p?L[u+"Score"]||0:R;Qe([Dt(B)],ee,Z,A===p,async()=>{await I({[T]:R,log:D,round:P,pendingAttack:null,phase:te?"finished":U+"-attack",attacker:U,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}}}),te&&await qe({...m,[T]:R})})}async function Ie(){const _=m[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(R=>(m[u+"Team"][R]||[]).some(D=>!D.used)),A=(m["selected_"+p]||[]).map(R=>{const D=(_[R._role]||[]).find(ae=>ae.cardId===R.cardId)||R,P=["GK","DEF"].includes(R._role),U=_[R._role]||[],J=U.findIndex(ae=>ae.cardId===R.cardId),te=xt(U.length),ee=J>=0?te[J]:D._col??1,Z=m.stadiumDef||m.homeStadiumDef||null,re=D.stadiumBonus||Z&&(Z.club_id&&String(D.club_id)===String(Z.club_id)||Z.country_code&&D.country_code===Z.country_code)||!1;return{...D,_line:R._role,_col:ee,stadiumBonus:re,...P?{note_a:Math.max(1,Number(D.note_a)||0)}:{}}});if(!A.length)return;const B=ni(A,m.modifiers[p]||{},m[p+"Formation"]);me(p,A.map(R=>R.cardId)),A.forEach(R=>{const D=(_[R._role]||[]).find(P=>P.cardId===R.cardId);D&&(D.used=!0)}),m["selected_"+p]=[],V();const T=[...m.log||[]];if(L){if(A.length===1&&(A[0]._line==="GK"||A[0].job==="GK")&&(m.p1Score||0)===(m.p2Score||0)){await he({side:p,gk:A[0]},{...m,[p+"Team"]:_,log:T});return}const D=(m[p+"Score"]||0)+1,P=A.map(Z=>Dt(Z));T.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:P,homeTotal:B.total,aiTotal:0});const U=(m.round||0)+1,J=p==="p1"?"p2":"p1",te={...m,[p+"Team"]:_,[p+"Score"]:D},ee=Ee(te);G.add(U),Qe(P,D,m[u+"Score"]||0,!0,async()=>{await I({[p+"Team"]:_,[p+"Score"]:D,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:ee?"finished":J+"-attack",attacker:J,round:U,log:T}),ee&&await qe({...m,[p+"Score"]:D})});return}T.push({type:"pending",text:`⚔️ ${m[p+"Name"]} attaque (${B.total})`}),await I({[p+"Team"]:_,[u+"Team"]:m[u+"Team"],pendingAttack:{...B,players:A,side:p},["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},phase:u+"-defense",log:T})}async function Se(){const _=m[p+"Team"],L=(m["selected_"+p]||[]).map(se=>{const be=(_[se._role]||[]).find(ze=>ze.cardId===se.cardId)||se,ve=_[se._role]||[],oe=ve.findIndex(ze=>ze.cardId===se.cardId),Le=xt(ve.length),Re=oe>=0?Le[oe]:be._col??1;return{...be,_line:se._role,_col:Re}}),A=oi(L,m.modifiers[p]||{},m[p+"Formation"]);me(p,L.map(se=>se.cardId)),L.forEach(se=>{const be=(_[se._role]||[]).find(ve=>ve.cardId===se.cardId);be&&(be.used=!0)}),m["selected_"+p]=[],V();const B=Ji(m.pendingAttack.total,A.total,m.modifiers[p]||{}),T=m.pendingAttack.side,R=B==="attack"||(B==null?void 0:B.goal),D=T==="p1"?"p2":"p1",P=(m.round||0)+1,U=(m.pendingAttack.players||[]).map(se=>Dt(se)),J=[...m.log||[]];J.push({type:"duel",isGoal:R,homeScored:R&&T===p,text:R?`⚽ BUT de ${m[T+"Name"]} ! (${m.pendingAttack.total} vs ${A.total})`:`✋ Attaque stoppée (${m.pendingAttack.total} vs ${A.total})`,homePlayers:U,aiPlayers:L.map(se=>Dt(se)),homeTotal:m.pendingAttack.total,aiTotal:A.total});const te=R?(m[T+"Score"]||0)+1:m[T+"Score"]||0,ee={...m,[p+"Team"]:_,[T+"Score"]:te},Z=Ee(ee),re=Z?"finished":D+"-attack",ae=!Z&&!R?fe(ee):null,xe=async()=>{if(ae){await he(ae,ee);return}await I({[p+"Team"]:_,[u+"Team"]:m[u+"Team"],[T+"Score"]:te,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:re,attacker:D,round:P,log:J}),(re==="finished"||Z)&&await qe({...m,[T+"Score"]:te})};if(R){const se=T===p,be=se?te:m[p+"Score"]||0,ve=se?m[u+"Score"]||0:te;G.add(P),Qe(U,be,ve,se,xe)}else await xe()}function Be(_){return["MIL","ATT"].some(L=>(_[L]||[]).some(A=>!A.used))}function $e(_){return["GK","DEF","MIL","ATT"].some(L=>(_[L]||[]).some(A=>!A.used))}function _t(_){return $e(_)&&!Be(_)}function ct(_){const L=m[_+"Team"],A=m[(_==="p1"?"p2":"p1")+"Team"];return!!(Be(L)||_t(L)&&!$e(A)||_t(L)&&!Be(A)&&$e(A))}function Ee(_){return!$e(_.p1Team)&&!$e(_.p2Team)}function Ce(_){const L=_.p1Score||0,A=_.p2Score||0;return L===A?null:L>A?h.home_id:h.away_id}async function qe(_){It(),Ue();try{const L=Ce(_),A=L?h.home_id===L?h.away_id:h.home_id:null,B=_.p1Score||0,T=_.p2Score||0,{data:R,error:D}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:L,p_home_score:B,p_away_score:T,p_forfeit:!1});if(D&&console.error("[PvP] finish_pvp_match:",D),c)try{await v.rpc("check_and_finish_mini_league",{p_league_id:c})}catch(P){console.error("[PvP] check_and_finish_mini_league:",P)}L&&A&&Ur(L,A).catch(()=>{});try{const{data:P}=await v.rpc("apply_match_rewards",{p_match_id:i});P!=null&&P.success&&!(P!=null&&P.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(P){console.error("[PvP] apply_match_rewards:",P)}if(typeof l=="function")try{await l({homeScore:B,awayScore:T,winnerId:L,homeId:h.home_id,awayId:h.away_id,matchId:i})}catch(P){console.error("[PvP] onMatchEnd:",P)}}catch(L){console.error("[PvP] finishMatch:",L)}}function Ge(){var T;const _=m[p+"Score"],L=m[u+"Score"],A=_>L,B=_===L;st(e),e.innerHTML=`
    ...
    </div>`,(T=document.getElementById("pvp-home"))==null||T.addEventListener("click",()=>{try{v.removeChannel(N)}catch{}it(e),c?x("mini-league",{openLeagueId:c}):x("home")})}V()}async function Pi(e,t,i=!1){var s,d;const{state:n,navigate:o,toast:r}=t,a=i&&((d=(s=t==null?void 0:t.state)==null?void 0:s.params)==null?void 0:d.rankedData)||null;await _i(e,t,i?"ranked":"random",async({deckId:l,formation:c,starters:f,subsRaw:b,gcCardsEnriched:x,gcDefs:g,stadiumDef:p})=>{st(e);const u=async y=>{await Yr(e,t,l,c,f,b,y||[],g,p,i,a)};if(!(x!=null&&x.length)){await u([]);return}vi(e,x,u)})}async function Vr(e,t,i){return Kr(e,t,i)}async function Yr(e,t,i,n,o,r,a=[],s=[],d=null,l=!1,c=null){var ie,N;const{state:f,navigate:b,toast:x}=t,g=(c==null?void 0:c.mmr)??((ie=f.profile)==null?void 0:ie.mmr)??1e3,p=l?hi(g):null,u=l?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",y=l?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let w=!1,h=null,E=null;e.innerHTML=`
    ...
    </div>`;const m=async()=>{w=!0,h&&(h.unsubscribe(),h=null),E&&(clearInterval(E),E=null);try{await v.rpc("cancel_matchmaking",{p_user_id:f.profile.id})}catch(I){console.error("[Matchmaking] cancel error:",I)}};(N=document.getElementById("cancel-mm"))==null||N.addEventListener("click",async()=>{await m(),it(e),b("home")});const C=async(I,z)=>{w||(w=!0,h&&(h.unsubscribe(),h=null),E&&(clearInterval(E),E=null),await Ct(e,t,I,z,{myGC:a,gcDefs:s,stadiumDef:d,isRanked:l,rankedData:c,onMatchEnd:l?Wr:null}))},$=f.user.id,{data:S,error:G}=l?await v.rpc("try_matchmake_ranked",{p_user_id:$,p_deck_id:i,p_mmr:g}):await v.rpc("try_matchmake",{p_user_id:$,p_deck_id:i});if(G||!(S!=null&&S.success)){console.error("[Matchmaking] try_matchmake error:",G||S),x("Erreur de matchmaking","error"),it(e),b("home");return}if(S.matched){await C(S.match_id,!1);return}const Q=async()=>{if(w)return;const{data:I}=await v.from("matchmaking_queue").select("status, match_id").eq("user_id",$).maybeSingle();(I==null?void 0:I.status)==="matched"&&I.match_id&&await C(I.match_id,!0)};h=v.channel(`mm_${$}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${$}`},I=>{const z=I.new;z.status==="matched"&&z.match_id&&C(z.match_id,!0)}).subscribe(),E=setInterval(Q,3e3),setTimeout(()=>{w||m()},12e4)}async function Wr(e,t){const{state:i,toast:n}=t,{winnerId:o,homeId:r,awayId:a,homeScore:s,awayScore:d,matchId:l}=e,c=i.user.id,f=r===c,b=o===c,{data:x}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",c).single(),{data:g}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",f?a:r).single();if(x&&g){const p=uo(x.mmr,x.mmr_rd,x.mmr_v,g.mmr,g.mmr_rd,b?1:0);await v.from("users").update({mmr:p.mmr,mmr_rd:p.rd,mmr_v:p.v,mmr_wins:b?v.sql`mmr_wins + 1`:void 0,mmr_losses:b?void 0:v.sql`mmr_losses + 1`}).eq("id",c),n(b?`+MMR ↑ ${Math.round(p.mmr-x.mmr)}`:`-MMR ↓ ${Math.round(x.mmr-p.mmr)}`,b?"success":"error",4e3)}}const io=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:Pi,resumePvpMatch:Vr},Symbol.toStringTag,{value:"Module"}));async function Xr(e,t){const{state:i,navigate:n,toast:o}=t,r=i.params||{},a=r.friendId||null,s=r.friendName||"Ami",d=!!r.isAccepting;await _i(e,t,"friend",async({deckId:l,formation:c,starters:f,subsRaw:b,gcCardsEnriched:x,gcDefs:g,stadiumDef:p})=>{st(e);const u=async y=>{const w=y||[];d?await Qr(e,t,l,w,g,p):await Jr(e,t,l,w,g,p,a,s)};if(!(x!=null&&x.length)){await u([]);return}vi(e,x,u)})}async function Jr(e,t,i,n,o,r,a,s){var w;const{state:d,navigate:l,toast:c}=t,f=d.user.id,{data:b,error:x}=await v.from("friend_match_invites").insert({inviter_id:f,invitee_id:a,friend_id:a,status:"pending",inviter_deck_id:i,expires_at:new Date(Date.now()+2*60*1e3).toISOString()}).select("id").single();if(x||!b){console.error("[Friend] Erreur création invitation:",x),c("Impossible de créer l'invitation","error"),l("home");return}e.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`,document.body.appendChild(x),x.querySelector("#lv-ok").onclick=()=>{x.remove(),b(!0)},x.querySelector("#lv-cancel").onclick=()=>{x.remove(),b(!1)}}))return;const{data:d,error:l}=await v.rpc("leave_mini_league",{p_league_id:i,p_user_id:a});if(l||!(d!=null&&d.success)){o((d==null?void 0:d.error)||"Erreur lors de la désinscription","error");return}const c=d.refund||n;r.profile&&(r.profile.credits=(r.profile.credits||0)+c);const f=document.getElementById("nav-credits");f&&(f.textContent=`💰 ${(r.profile.credits||0).toLocaleString("fr")}`),o(`↩️ Désinscrit · +${c.toLocaleString("fr")} cr. remboursés`,"success"),et(e,t,"waiting")}async function Gi(e,t,i,n,o){const{toast:r}=t;if(!confirm(`Supprimer définitivement "${n}" et tous ses matchs/membres ? Action irréversible.`))return;await v.from("mini_league_matches").delete().eq("league_id",i),await v.from("mini_league_members").delete().eq("league_id",i);const{error:a}=await v.from("mini_leagues").delete().eq("id",i);if(a){r("Erreur suppression ("+a.message+")","error");return}r("Mini League supprimée avec succès","success"),et(e,t,o)}async function sa(e,t,i){await v.from("mini_leagues").update({is_archived:!0}).eq("id",i),t.toast("Mini League archivée","success"),et(e,t,"archived")}async function lt(e,t,i){var E,m,C,$,S,G,Q,ie,N;const{state:n,toast:o}=t,r=n.profile.id,[{data:a},{data:s},{data:d}]=await Promise.all([v.from("mini_leagues").select("*").eq("id",i).single(),v.from("mini_league_members").select("*, user:users(id,pseudo,club_name,club_color1,club_color2)").eq("league_id",i),v.from("mini_league_matches").select("*").eq("league_id",i).order("matchday").order("created_at")]);if(!a){o("Introuvable","error"),await et(e,t);return}const l=(s||[]).some(I=>I.user_id===r),c=a.creator_id===r,f=(s||[]).map(I=>I.user).filter(Boolean),b=ua(f,d||[]),x=(d||[]).filter(I=>I.matchday===a.current_day),g=a.pot||0,p=a.entry_fee||100,u=x.length>0&&x.every(I=>I.status==="finished"||I.status==="bye"),y=a.current_day>=a.total_days,w=(s||[]).find(I=>I.user_id===r);e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Xt};border:1px solid ${Jt};border-radius:12px;padding:16px">
        ...
        ${f.map(I=>`
          <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid ${Lt}">
            <div style="width:36px;height:36px;border-radius:50%;background:${I.club_color2||no};display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:${I.club_color1||"#fff"}">${(I.pseudo||"?").slice(0,2).toUpperCase()}</div>
      ...
      <div style="background:${Xt};border:1px solid ${Jt};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${b.map((I,z)=>{const K=g>0&&a.status==="finished"?z===0?Math.floor(g*.7):z===1?Math.floor(g*.2):z===2?Math.floor(g*.1):0:0;return`<tr style="border-bottom:1px solid ${Lt};${I.userId===r?"background:rgba(74,222,128,0.08);":""}">
              <td style="padding:7px 3px 7px 0;font-weight:700;color:${z===0?dt:z<3?"#4ade80":Et}">${["🥇","🥈","🥉"][z]||z+1}</td>
              ...
              <td style="text-align:center;color:${Et}">${I.played}</td><td style="text-align:center;color:${Et}">${I.won}-${I.drawn}-${I.lost}</td>
      ...
      <div style="background:${Xt};border:1px solid ${Jt};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(I,z)=>z+1).reverse().map(I=>{const z=(d||[]).filter(K=>K.matchday===I);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${Ze};margin-bottom:6px">Journée ${I}</div>${z.map(K=>$n(K,f,r,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
    ...
    </div>`;let c=!1;const f=async u=>{if(!c){if(c=!0,clearInterval(g),x)try{x.unsubscribe()}catch{}u&&await Ct(e,t,u,o,{mlLeagueId:n,mlMatchId:i,myGC:r||[],gcDefs:a,stadiumDef:s}),d()}};(p=document.getElementById("wait-back"))==null||p.addEventListener("click",()=>{if(c=!0,clearInterval(g),x)try{x.unsubscribe()}catch{}l("mini-league",{openLeagueId:n}),d()});const b=async()=>{if(c)return;const{data:u}=await v.from("mini_league_matches").select("match_id").eq("id",i).single();u!=null&&u.match_id&&f(u.match_id)},x=v.channel(`ml_match_${i}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"mini_league_matches",filter:`id=eq.${i}`},u=>{var y;(y=u.new)!=null&&y.match_id&&f(u.new.match_id)}).subscribe(),g=setInterval(b,3e3);b()})}const ma="/",ga=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function xa(e,t,i){let n=0;const o=document.createElement("div");o.id="tutorial-overlay",o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const r=()=>{var c,f,b;const s=t[n],d=n===t.length-1,l=Math.round((n+1)/t.length*100);o.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div id="mkt-content" style="padding:10px 16px;display:flex;flex-direction:column;gap:8px"></div>
      ...
      ${Te({...u,_evolution_bonus:y},{width:140})}
      ...
      <div style="display:flex;gap:10px">
        ...
        <button id="buy-ok" style="flex:1;padding:12px;border-radius:10px;border:none;background:var(--green);color:#fff;font-size:14px;font-weight:900;cursor:pointer">Confirmer</button>
          ...
          <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${I?"border:2px solid var(--yellow)":""}">
            ...
            <div style="text-align:right;flex-shrink:0">
              <div style="font-size:20px">${z?"❓":Q.emoji}</div>
              <div style="font-size:11px;font-weight:700;color:${Q.color}">${z?"Placement":Q.label}</div>
              ${z?"":`<div style="font-size:10px;color:var(--tile-fg-dim)">${N}% WR</div>`}
            </div>
  ...
  <div class="page">
    ...
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.26-0040" style="cursor:pointer">
