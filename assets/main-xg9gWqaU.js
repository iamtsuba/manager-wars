...
import{s as v,n as Ot,o as Ni,F as Ri,l as et,k as Qt,r as Ie,j as St,p as Si,h as Oi,q as Qn,t as Zn,T as ki,m as eo,u as to,b as io}from"./special-cards-CiJC3HtT.js";const no="/";function rn(e,{navigate:t,toast:i}){let n="login";const r=()=>{var a,s,d,c,l,f;const o=n==="login";e.innerHTML=`
  ...
  `;function s(){var y;const c=document.getElementById("logo-preview"),l=document.getElementById("logo-initials"),f=((y=document.getElementById("setup-club"))==null?void 0:y.value)||"MW",x=f.trim().split(" ").filter(Boolean),g=x.length>=2?(x[0][0]+x[1][0]).toUpperCase():f.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=o),l&&(l.textContent=g,l.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function d(c){document.querySelectorAll(".setup-step").forEach(l=>l.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("step1-error");if(l.textContent="",c.length<3){l.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await v.from("users").select("id").eq("pseudo",c).maybeSingle();if(f){l.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),l=document.getElementById("step2-error");if(l.textContent="",c.length<2){l.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await v.from("users").select("id").eq("club_name",c).maybeSingle();if(f){l.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),x=document.getElementById("step3-finish");f.textContent="",x.disabled=!0,x.textContent="Création en cours…";try{const{error:g}=await v.from("users").insert({id:t.user.id,pseudo:c,club_name:l,club_color1:o,club_color2:a,credits:1e4});if(g)throw g;await ro(t.user.id),await r(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(g){f.textContent=g.message,x.disabled=!1,x.textContent="🚀 Créer mon profil !"}})}async function ro(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await v.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const ao="modulepreload",so=function(e){return"/"+e},an={},_n=function(t,i,n){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(d=>{if(d=so(d),d in an)return;an[d]=!0;const c=d.endsWith(".css"),l=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${l}`))return;const f=document.createElement("link");if(f.rel=c?"stylesheet":ao,c||(f.as="script"),f.crossOrigin="",f.href=d,s&&f.setAttribute("nonce",s),document.head.appendChild(f),c)return new Promise((x,g)=>{f.addEventListener("load",x),f.addEventListener("error",()=>g(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return r.then(a=>{for(const s of a||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},Zt="#1A6B3C",ei="#cc2222",lo="#D4A017",sn="#888";async function co(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </style>`}const $n="mw_sound_muted";function ci(){return localStorage.getItem($n)==="1"}function mo(e){localStorage.setItem($n,e?"1":"0")}function go(e,t=1){if(ci())return null;try{const i=new Audio(e);return i.volume=t,i.play().catch(()=>{}),i}catch{return null}}let $t=null,Ai=null;function yo(e,t=.3){if(!($t&&Ai===e&&!$t.paused)&&(Ft(),!ci()))try{const i=new Audio(e);i.loop=!0,i.volume=t,i.play().catch(()=>{}),$t=i,Ai=e}catch{}}function Ft(){if($t)try{$t.pause(),$t.currentTime=0}catch{}$t=null,Ai=null}const xo="2026.07.21-1712";async function dn(e,{state:t,navigate:i,toast:n}){var o,a;const r=t.profile;r&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(fe=document.getElementById("close-detail"))==null||fe.addEventListener("click",s);let K=new Set;const X=()=>{const G=K.size,O=document.getElementById("sell-action-panel");if(!O)return;O.style.display=G>0?"block":"none",document.getElementById("sell-selected-count").textContent=G;const $=document.getElementById("evolve-btn");$&&($.textContent=`⬆️ Évoluer ${G>1?"(+"+G+")":""}`)};document.querySelectorAll(".expl-check").forEach(G=>{G.addEventListener("change",()=>{const O=G.dataset.id;G.checked?K.add(O):K.delete(O);const $=G.closest(".exemplaire-row");$&&($.style.borderColor=G.checked?"#1A6B3C":"#eee"),X()})}),document.querySelectorAll(".exemplaire-row").forEach(G=>{G.addEventListener("click",O=>{if(O.target.closest("button")||O.target.tagName==="INPUT")return;const $=G.querySelector(".expl-check");$&&!$.disabled&&($.checked=!$.checked,$.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(G=>{G.addEventListener("click",O=>{O.stopPropagation();const $=document.querySelector(`.expl-hist[data-hist="${G.dataset.idx}"]`);$&&($.style.display=$.style.display==="none"?"flex":"none")})}),(M=document.getElementById("evolve-btn"))==null||M.addEventListener("click",async()=>{if(x<=1)return;const G=[...K];if(!G.length)return;if(K.has(e.id)){const B=document.createElement("div");B.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",B.innerHTML=`
  ...
  <div class="page">
    ...
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.21-1712" style="cursor:pointer">
