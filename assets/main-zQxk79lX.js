import{s as $,l as Y,F as ee,b as be,c as ue,d as et}from"./formation-links-BXiJqmnj.js";function Ee(t,{navigate:e,toast:i}){t.innerHTML=`
  ...
  `;function d(){var m;const l=document.getElementById("logo-preview"),c=document.getElementById("logo-initials"),f=((m=document.getElementById("setup-club"))==null?void 0:m.value)||"MW",p=f.trim().split(" ").filter(Boolean),u=p.length>=2?(p[0][0]+p[1][0]).toUpperCase():f.slice(0,2).toUpperCase();l&&(l.style.background=a,l.style.borderColor=n),c&&(c.textContent=u,c.style.color=n)}document.getElementById("color1").addEventListener("input",l=>{n=l.target.value,document.getElementById("swatch1").style.background=n,d()}),document.getElementById("color2").addEventListener("input",l=>{a=l.target.value,document.getElementById("swatch2").style.background=a,d()});function s(l){document.querySelectorAll(".setup-step").forEach(c=>c.classList.remove("active")),document.getElementById(`step-${l}`).classList.add("active"),document.getElementById("step-num").textContent=l,document.getElementById("progress-fill").style.width=`${Math.round(l/3*100)}%`,l===3&&d()}document.getElementById("step1-next").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("step1-error");if(c.textContent="",l.length<3){c.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await $.from("users").select("id").eq("pseudo",l).maybeSingle();if(f){c.textContent="Ce pseudo est déjà pris.";return}s(2)}),document.getElementById("step2-back").addEventListener("click",()=>s(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const l=document.getElementById("setup-club").value.trim(),c=document.getElementById("step2-error");if(c.textContent="",l.length<2){c.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await $.from("users").select("id").eq("club_name",l).maybeSingle();if(f){c.textContent="Ce nom de club est déjà pris.";return}s(3)}),document.getElementById("step3-back").addEventListener("click",()=>s(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),p=document.getElementById("step3-finish");f.textContent="",p.disabled=!0,p.textContent="Création en cours…";try{const{error:u}=await $.from("users").insert({id:e.user.id,pseudo:l,club_name:c,club_color1:n,club_color2:a,credits:1e4});if(u)throw u;await it(e.user.id),await r(),o(`Bienvenue ${l} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(u){f.textContent=u.message,p.disabled=!1,p.textContent="🚀 Créer mon profil !"}})}async function it(t){const{data:e}=await $.from("players").select("id,job,firstname,surname_encoded,country_code,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,sell_price").eq("is_active",!0);if(!e||e.length===0)return;const i=e,o=i.filter(s=>s.job==="GK"),r=i.filter(s=>s.job!=="GK"),n=[];for(let s=0;s<5;s++){let l=[];if(s===0&&o.length>0){const c=o[Math.floor(Math.random()*o.length)];l.push(c);const f=Ie([...r]).slice(0,3);l.push(...f)}else l=Ie([...i]).slice(0,4);l.forEach(c=>{n.push({owner_id:t,player_id:c.id,card_type:"player"})})}["Ressusciter","Double attaque","Bouclier"].forEach(s=>{n.push({owner_id:t,card_type:"game_changer",gc_type:s})});const d=["4-4-2","4-3-3","3-4-3","3-5-2","5-3-2"];n.push({owner_id:t,card_type:"formation",formation:d[Math.floor(Math.random()*d.length)]}),n.length>0&&await $.from("cards").insert(n),await $.from("users").update({first_booster_opened:!0}).eq("id",t)}function Ie(t){for(let e=t.length-1;e>0;e--){const i=Math.floor(Math.random()*(e+1));[t[e],t[i]]=[t[i],t[e]]}return t}async function Ae(t,{state:e,navigate:i,toast:o}){var a;const r=e.profile;if(!r)return;const{data:n}=await $.from("matches").select("id,home_score,away_score,status,mode,winner_id,created_at").or(`home_id.eq.${r.id},away_id.eq.${r.id}`).eq("status","finished").order("created_at",{ascending:!1}).limit(3);t.innerHTML=`
  <div class="page">
    <div class="page-body">
      ...
      <div>
        ...
        <div class="card-panel" style="padding:0">
          ${n.map(d=>{const s=d.winner_id===r.id,l=d.home_score===d.away_score,c=l?"N":s?"V":"D",f=l?"#888":s?"#1A6B3C":"#c0392b",p={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat"}[d.mode]||d.mode,m=new Date(d.created_at).toLocaleDateString("fr",{month:"short",day:"numeric"});return`<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;border-bottom:1px solid var(--gray-200)">
              <div>
  ...
  `,document.body.appendChild(i);const o=()=>i.remove();document.getElementById("diff-cancel").addEventListener("click",o),i.addEventListener("click",r=>{r.target===i&&o()}),i.querySelectorAll("[data-mode]").forEach(r=>{r.addEventListener("click",()=>{o(),t("match",{matchMode:r.dataset.mode})})})}const de={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé pour ce match."},"Double attaque":{icon:"⚡",desc:"La note d'attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la note d'un joueur adverse."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function D(t,e){if(!t)return 0;switch(e){case"GK":return Number(t.note_g)||0;case"DEF":return Number(t.note_d)||0;case"MIL":return Number(t.note_m)||0;case"ATT":return Number(t.note_a)||0;default:return 0}}function Ge(t){let e=0;for(let i=0;i<t.length-1;i++){const o=t[i],r=t[i+1];if(!o||!r)continue;const n=Y(o,r);n==="#00ff88"?e+=2:n==="#FFD700"&&(e+=1)}return e}function Ne(t,e={}){const i=t.reduce((n,a)=>{const d=a._line||a.job;return n+(Number(d==="MIL"?a.note_m:a.note_a)||0)},0),o=Ge(t);let r=i+o;return e.doubleAttack&&(r*=2),e.stolenNote&&(r-=e.stolenNote),{base:i,links:o,total:Math.max(0,r)}}function Re(t,e={}){const i=t.reduce((n,a)=>{const d=a._line||a.job;let s=0;return d==="GK"?s=Number(a.note_g)||0:d==="MIL"?s=Number(a.note_m)||0:s=Number(a.note_d)||0,n+s},0),o=Ge(t);let r=i+o;return e.stolenNote&&(r-=e.stolenNote),{base:i,links:o,total:Math.max(0,r)}}function qe(t,e,i={}){return i.shield?{goal:!1,shielded:!0}:{goal:t>e,shielded:!1}}function Oe(t,e,i="easy"){const o=t.filter(a=>!a.used);if(!o.length)return[];const r=[...o].sort((a,d)=>{const s=e==="attack"?D(a,"ATT"):a._line==="GK"?D(a,"GK"):D(a,"DEF");return(e==="attack"?D(d,"ATT"):d._line==="GK"?D(d,"GK"):D(d,"DEF"))-s});let n=i==="easy"?1+Math.floor(Math.random()*2):i==="medium"?2+Math.floor(Math.random()*2):3;return r.slice(0,Math.min(n,r.length,3))}function nt(t,e){const i={vs_ai_easy:{victoire:500,nul:250,defaite:50},vs_ai_medium:{victoire:1e3,nul:500,defaite:50},vs_ai_hard:{victoire:1500,nul:750,defaite:100},vs_ai_club:{victoire:2500,nul:1250,defaite:100}};return(i[t]||i.vs_ai_easy)[e]||0}const Ke={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},le={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},oe=["GK","DEF","MIL","ATT"],rt=["Tous","GK","DEF","MIL","ATT"],at={normal:1e3,pepite:5e3,papyte:5e3,legende:1e4},he={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function Pe(t){const e="https://fcnwclxlkytdfjotqkta.supabase.co";if(!(t!=null&&t.skin)||!(t!=null&&t.hair))return null;const i=t.hair==="chauve"?`${t.skin}-chauve-rase`:`${t.skin}-${t.hair}-${t.hair_length}`;return`${e}/storage/v1/object/public/assets/tetes/${i}.png`}function te(t,e){return t&&Number(e==="GK"?t.note_g:e==="DEF"?t.note_d:e==="MIL"?t.note_m:t.note_a)||0}function Te(t,e=""){var f,p;const i=t.player;if(!i)return"";const o=i.job||"ATT",r=le[o],n=Ke[i.rarity]||"#ccc",a=te(i,o),d=i.job2?te(i,i.job2):null,s=i.job2?le[i.job2]:null,l=Pe(i),c=he[i.country_code]||i.country_code||"";return`
  <div style="
  ...
  </div>`}function st(t){const e=t.job||"ATT",i=te(t,e),o=he[t.country_code]||t.country_code||"";return`
  <div style="
    width:140px;border-radius:12px;padding:6px;
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
  ...
  </div>`}async function dt(t,e){const{state:i,navigate:o,toast:r,openModal:n,closeModal:a}=e;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await $.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),l=(d||[]).filter(I=>I.card_type==="player"&&I.player),c=(d||[]).filter(I=>I.card_type==="game_changer"),f=(d||[]).filter(I=>I.card_type==="formation"),p=Object.keys(ee),u=Object.keys(de),m={};l.forEach(I=>{const A=I.player.id;m[A]=(m[A]||0)+1});const w=new Set(Object.keys(m).map(I=>String(I))),h=new Set(f.map(I=>I.formation)),k=new Set(c.map(I=>I.gc_type));let b="player",g="Tous",y="",v=!1;function x(){return[...l].sort((I,A)=>{const T=oe.indexOf(I.player.job),C=oe.indexOf(A.player.job);return T!==C?T-C:(I.player.surname_encoded||"").localeCompare(A.player.surname_encoded||"")})}function E(){return[...s||[]].sort((I,A)=>{const T=oe.indexOf(I.job),C=oe.indexOf(A.job);return T!==C?T-C:(I.surname_encoded||"").localeCompare(A.surname_encoded||"")})}function _(){return x().filter(I=>{const A=I.player,T=g==="Tous"||A.job===g,C=!y||`${A.firstname} ${A.surname_encoded}`.toLowerCase().includes(y);return T&&C})}function M(){return E().filter(I=>{const A=g==="Tous"||I.job===g,T=!y||`${I.firstname} ${I.surname_encoded}`.toLowerCase().includes(y);return A&&T})}t.innerHTML=`
  <div class="page">
    ...
    <div class="page-body">
      <div style="display:flex;flex-wrap:wrap;gap:10px;justify-content:flex-start" id="col-grid"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",A=>{y=A.target.value.toLowerCase(),z()}),t.querySelectorAll(".filter-btn").forEach(A=>{A.addEventListener("click",()=>{g=A.dataset.job,L(),z()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{v=!v,L(),z()})):(I.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{v=!v,L(),z()})))}function z(){const I=document.getElementById("col-grid");I&&(b==="player"?B(I):b==="formation"?H(I):X(I))}function B(I){if(v){const A=M();if(!A.length){I.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}I.innerHTML=A.map(T=>{if(w.has(String(T.id))){const F=l.find(P=>P.player.id===T.id),N=m[T.id]||1,q=N>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${N}</div>`:"",K=l.filter(P=>P.player.id===T.id&&P.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Te(F,q+K)}return st(T)}).join("")}else{const A=_();if(!A.length){I.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const T={},C=[];A.forEach(F=>{const N=F.player.id;T[N]||(T[N]=!0,C.push(F))}),I.innerHTML=C.map(F=>{const N=m[F.player.id]||1,q=N>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${N}</div>`:"",K=l.filter(P=>P.player.id===F.player.id&&P.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Te(F,q+K)}).join("")}I.querySelectorAll("[data-card-id]").forEach(A=>{A.addEventListener("click",()=>{const T=l.find(C=>C.id===A.dataset.cardId);T&&pt(T,l,m,e)})})}function H(I){const A=v?p:[...h];if(!A.length){I.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}I.innerHTML=A.map(T=>{if(h.has(T)){const F=f.find(O=>O.formation===T),N=f.filter(O=>O.formation===T).length,q=N>1?`<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${N}</div>`:"";return`
        ...
        <div style="
          ...
          <div style="font-weight:900;font-size:18px">${T}</div>
        </div>`}).join(""),I.querySelectorAll("[data-form-id]").forEach(T=>{T.addEventListener("click",()=>{const C=f.find(F=>F.id===T.dataset.formId);C&&ct(C,f,e,n)})})}function X(I){const A=v?u:[...k];if(!A.length){I.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}I.innerHTML=A.map(T=>{const C=k.has(T),F=de[T]||{icon:"⚡"};if(C){const N=c.find(K=>K.gc_type===T),q=c.filter(K=>K.gc_type===T).length,O=q>1?`<div style="position:absolute;top:4px;right:4px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${q}</div>`:"";return`
          <div data-gc-id="${N.id}" data-gc-type="${T}" style="
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Q=1e3;function ct(t,e,i,o){var w,h,k;const{state:r,toast:n,closeModal:a,navigate:d,refreshProfile:s}=i,l=t.formation,c={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function f(){const b=be[l]||{},g=ee[l]||[],y=290,v=360,x=20;function E(M){const L=b[M];return L?{x:L.x*y,y:L.y*v}:null}let _=`<svg width="${y}" height="${v}" viewBox="0 0 ${y} ${v}" xmlns="http://www.w3.org/2000/svg">`;for(const[M,L]of g){const z=E(M),B=E(L);!z||!B||(_+=`<line x1="${z.x}" y1="${z.y}" x2="${B.x}" y2="${B.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const M of Object.keys(b)){const L=E(M);if(!L)continue;const z=M.replace(/\d+/,""),B=c[z]||"#555";_+=`<circle cx="${L.x}" cy="${L.y}" r="${x}" fill="${B}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,_+=`<text x="${L.x}" y="${L.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${z}</text>`}return _+="</svg>",_}const p=e.filter(b=>b.formation===l),u=p.length,m=!t.is_for_sale;o(`Formation ${l}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(w=document.getElementById("direct-sell-form-btn"))==null||w.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${l} pour ${Q.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const b=p.find(y=>!y.is_for_sale)||p[0];if(!b){n("Aucune carte à vendre","error");return}const{error:g}=await $.from("cards").delete().eq("id",b.id);if(g){n(g.message,"error");return}await $.from("users").update({credits:(r.profile.credits||0)+Q}).eq("id",r.profile.id),await s(),n(`+${Q.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),d("collection")}),(h=document.getElementById("market-sell-form-btn"))==null||h.addEventListener("click",async()=>{const b=parseInt(document.getElementById("sell-price-form").value);if(!b||b<1){n("Prix invalide","error");return}await $.from("cards").update({is_for_sale:!0,sale_price:b}).eq("id",t.id),await $.from("market_listings").insert({seller_id:r.profile.id,card_id:t.id,price:b}),n("Carte mise en vente sur le marché !","success"),a(),d("collection")}),(k=document.getElementById("cancel-sell-form-btn"))==null||k.addEventListener("click",async()=>{await $.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",t.id),await $.from("market_listings").update({status:"cancelled"}).eq("card_id",t.id).eq("status","active"),n("Annonce retirée","success"),a(),d("collection")})}function pt(t,e,i,o){var x,E,_,M,L,z;const{state:r,toast:n,openModal:a,closeModal:d,navigate:s,refreshProfile:l}=o,c=t.player,f=e.filter(B=>B.player.id===c.id),p=f.length,u=at[c.rarity]||1e3,m=c.rarity!=="legende",w=Pe(c),h=te(c,c.job),k=c.job2?te(c,c.job2):null,b=le[c.job]||"#1A6B3C",g=c.job2?le[c.job2]:null,y=Ke[c.rarity]||"#ccc",v=he[c.country_code]||c.country_code||"";a(`${c.firstname} ${c.surname_encoded}`,`<div style="display:flex;gap:16px;flex-wrap:wrap;justify-content:center">
      ...
      <div style="width:140px;border-radius:12px;padding:6px;background:${y};flex-shrink:0">
        <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
          ...
          <div style="background:#f2e8d2;padding:3px 6px;display:flex;align-items:center;justify-content:space-between;min-height:24px">
            ...
            ${(x=c.clubs)!=null&&x.logo_url?`<img src="${c.clubs.logo_url}" style="width:22px;height:16px;object-fit:contain">`:`<div style="background:#1a3a7a;color:#fff;border-radius:2px;padding:1px 3px;font-size:6px;font-weight:800">${(((E=c.clubs)==null?void 0:E.encoded_name)||"").slice(0,6)}</div>`}
          </div>
        </div>
      </div>
  ...
  <div class="page">
    ...
    <div class="page-body">
      <button class="btn btn-primary" id="save-deck" style="width:100%" ${n<11?"disabled":""}>
  ...
  </div>`,ft(t,e,r,i),document.getElementById("builder-back").addEventListener("click",()=>o("decks")),document.getElementById("formation-select").addEventListener("change",l=>{e.formation=l.target.value;const c=Me(e.formation),f={};c.forEach(p=>{e.slots[p]&&(f[p]=e.slots[p])}),e.slots=f,J(t,e,i)}),document.getElementById("save-deck").addEventListener("click",()=>yt(e,i)),t.querySelectorAll("[data-remove-sub]").forEach(l=>{l.addEventListener("click",()=>{e.subs=e.subs.filter(c=>c!==l.dataset.removeSub),J(t,e,i)})}),(s=document.getElementById("add-sub-btn"))==null||s.addEventListener("click",()=>{gt(e,t,i)})}function ft(t,e,i,o){const r=document.getElementById("deck-field");if(!r)return;const n=e.formation,a=be[n]||{},d=ue?ue(n)||ee[n]||[]:ee[n]||[],s={};i.forEach(g=>{const y=e.slots[g],v=y?e.playerCards.find(x=>x.id===y):null;s[g]=(v==null?void 0:v.player)||null});const l=i.filter(g=>g.startsWith("MIL"));let c=0,f=0;l.forEach(g=>{const y=s[g];y&&(c+=Number(y.note_m)||0)}),d.forEach(([g,y])=>{if(!g.startsWith("MIL")||!y.startsWith("MIL"))return;Y(s[g],s[y])!=="#ff3333"&&f++});const p=320,u=320,m=28,w=4;function h(g){const y=a[g];return y?{x:y.x*p,y:y.y*u}:null}let k="";for(const[g,y]of d){const v=h(g),x=h(y);if(!v||!x)continue;const E=s[g],_=s[y],M=Y(E,_);if(M==="#00ff88"||M==="#FFD700")k+=`<line x1="${v.x}" y1="${v.y}" x2="${x.x}" y2="${x.y}"
        stroke="${M}" stroke-width="${w*3}" stroke-linecap="round" opacity="0.25"/>`,k+=`<line x1="${v.x}" y1="${v.y}" x2="${x.x}" y2="${x.y}"
        stroke="${M}" stroke-width="${w}" stroke-linecap="round" opacity="0.95"/>`;else{const z=M==="#ff3333"||M==="#cc2222"?.75:.9;k+=`<line x1="${v.x}" y1="${v.y}" x2="${x.x}" y2="${x.y}"
        stroke="${M}" stroke-width="${w}" stroke-linecap="round" opacity="${z}"/>`}}for(const g of i){const y=h(g);if(!y)continue;const v=s[g],x=g.replace(/\d+/,""),_={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[x]||"#555",M=v?Number(x==="GK"?v.note_g:x==="DEF"?v.note_d:x==="MIL"?v.note_m:v.note_a)||0:null,L=v?(v.surname_encoded||"").slice(0,8).toUpperCase():"";if(v){const z=we(v);z&&(k+=`
          <defs>
            <clipPath id="clip-${g}">
        ...
        <circle cx="${y.x}" cy="${y.y}" r="${m}" fill="${z?"transparent":_}"
          stroke="${_}" stroke-width="2" />
        <circle cx="${y.x}" cy="${y.y}" r="${m}" fill="${_}" opacity="${z?"0.5":"1"}"
    ...
    <svg viewBox="0 0 ${p} ${u}" width="100%" style="display:block;max-width:380px;margin:0 auto">
      ${b}
      ${k}
    </svg>`,r.querySelectorAll(".deck-slot-hit").forEach(g=>{g.addEventListener("click",()=>ut(g.dataset.pos,e,t,o))})}function ut(t,e,i,o){var p,u,m;const{openModal:r,closeModal:n}=o,a=t.replace(/\d+/,""),d=e.slots[t],s=d?e.playerCards.find(w=>w.id===d):null;(p=s==null?void 0:s.player)==null||p.id;const l=new Set;Object.entries(e.slots).forEach(([w,h])=>{var b;if(w===t||!h)return;const k=e.playerCards.find(g=>g.id===h);(b=k==null?void 0:k.player)!=null&&b.id&&l.add(k.player.id)}),e.subs.forEach(w=>{var k;const h=e.playerCards.find(b=>b.id===w);(k=h==null?void 0:h.player)!=null&&k.id&&l.add(h.player.id)});const c=new Set,f=e.playerCards.filter(w=>{const h=w.player;return!(h.job===a||h.job2===a)||l.has(h.id)||c.has(h.id)?!1:(c.add(h.id),!0)});f.sort((w,h)=>{const k=a==="GK"?w.player.note_g:a==="DEF"?w.player.note_d:a==="MIL"?w.player.note_m:w.player.note_a;return(a==="GK"?h.player.note_g:a==="DEF"?h.player.note_d:a==="MIL"?h.player.note_m:h.player.note_a)-k}),r(`Choisir ${a} — ${t}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${e.slots[t]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${f.length>0?f.map(w=>{var y,v;const h=w.player,k=a==="GK"?h.note_g:a==="DEF"?h.note_d:a==="MIL"?h.note_m:h.note_a,b=we(h),g={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[h.rarity];return`<div class="player-option" data-card-id="${w.id}"
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(u=document.getElementById("close-selector"))==null||u.addEventListener("click",n),(m=document.getElementById("remove-player"))==null||m.addEventListener("click",()=>{delete e.slots[t],n(),J(i,e,o)}),document.querySelectorAll(".player-option").forEach(w=>{w.addEventListener("click",()=>{e.slots[t]=w.dataset.cardId,n(),J(i,e,o)})})}function gt(t,e,i){var s;const{openModal:o,closeModal:r}=i,n=new Set;Object.values(t.slots).filter(Boolean).forEach(l=>{var f;const c=t.playerCards.find(p=>p.id===l);(f=c==null?void 0:c.player)!=null&&f.id&&n.add(c.player.id)}),t.subs.forEach(l=>{var f;const c=t.playerCards.find(p=>p.id===l);(f=c==null?void 0:c.player)!=null&&f.id&&n.add(c.player.id)});const a=new Set,d=t.playerCards.filter(l=>{var c,f,p;return n.has((c=l.player)==null?void 0:c.id)||a.has((f=l.player)==null?void 0:f.id)?!1:(a.add((p=l.player)==null?void 0:p.id),!0)});o("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${d.length>0?d.map(l=>{var u;const c=l.player,f=we(c),p=c.job==="GK"?c.note_g:c.job==="DEF"?c.note_d:c.job==="MIL"?c.note_m:c.note_a;return`<div class="player-option" data-card-id="${l.id}"
          style="display:flex;align-items:center;gap:10px;padding:8px;border:1.5px solid var(--gray-200);border-radius:10px;cursor:pointer">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-sub-selector">Fermer</button>'),(s=document.getElementById("close-sub-selector"))==null||s.addEventListener("click",r),document.querySelectorAll(".player-option").forEach(l=>{l.addEventListener("click",()=>{t.subs.push(l.dataset.cardId),r(),J(e,t,i)})})}async function yt(t,e){const{state:i,toast:o,navigate:r}=e,n=t.formationCards.find(s=>s.formation===t.formation),a=(n==null?void 0:n.id)||t.formationCardId;await $.from("decks").update({formation:t.formation,formation_card_id:a||null}).eq("id",t.deckId),await $.from("deck_cards").delete().eq("deck_id",t.deckId);const d=[];if(Object.entries(t.slots).forEach(([s,l],c)=>{d.push({deck_id:t.deckId,card_id:l,position:s,is_starter:!0,slot_order:c})}),t.subs.forEach((s,l)=>{d.push({deck_id:t.deckId,card_id:s,position:`SUB${l+1}`,is_starter:!1,slot_order:100+l})}),d.length>0){const{error:s}=await $.from("deck_cards").insert(d);if(s){o(s.message,"error");return}}o("Deck enregistré ✅","success"),r("decks")}function Me(t){const e=ce[t]||ce["4-4-2"],i=["GK1"];for(let o=1;o<=e.DEF;o++)i.push(`DEF${o}`);for(let o=1;o<=e.MIL;o++)i.push(`MIL${o}`);for(let o=1;o<=e.ATT;o++)i.push(`ATT${o}`);return i}const ze=[{id:"players_std",img:"/manager-wars/icons/booster-players.png",name:"Players",sub:"5 cartes joueurs",cost:5e3,costLabel:"5 000 crédits",cardCount:5,type:"player"},{id:"players_pub",img:"/manager-wars/icons/booster-silver.png",name:"Players (pub)",sub:"3 cartes joueurs",cost:0,costLabel:"1 pub",cardCount:3,type:"player"},{id:"game_changer",img:"/manager-wars/icons/booster-gamechanger.png",name:"Game Changer",sub:"3 cartes spéciales",cost:1e4,costLabel:"10 000 crédits",cardCount:3,type:"game_changer"},{id:"formation",img:"/manager-wars/icons/booster-formation.png",name:"Formation",sub:"1 carte formation",cost:1e4,costLabel:"10 000 crédits",cardCount:1,type:"formation"}],He={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé."},"Double attaque":{icon:"⚡",desc:"La prochaine attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la prochaine action IA."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function mt(t){const e="https://fcnwclxlkytdfjotqkta.supabase.co";if(!(t!=null&&t.skin)||!(t!=null&&t.hair))return null;const i=t.hair==="chauve"?`${t.skin}-chauve-rase`:`${t.skin}-${t.hair}-${t.hair_length}`;return`${e}/storage/v1/object/public/assets/tetes/${i}.png`}const xt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},vt={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function bt(t,{state:e,navigate:i,toast:o}){var n;const r=((n=e.profile)==null?void 0:n.credits)||0;t.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📦 Boosters</h2>
  ...
  `,document.body.appendChild(o);let r=!1;document.getElementById("pack-visual").addEventListener("click",()=>{if(r)return;r=!0;const p=document.getElementById("pack-visual");p.classList.add("shaking"),setTimeout(()=>{p.classList.add("pack-open"),setTimeout(()=>{document.getElementById("pack-phase").style.display="none",a(0)},600)},500)});let n=0;function a(p){n=p;const u=document.getElementById("reveal-phase");u.style.display="flex",d(p)}function d(p){var v;const u=t[p],m=document.getElementById("card-counter"),w=document.getElementById("single-card-slot"),h=document.getElementById("card-tap-hint");m&&(m.textContent=`Carte ${p+1} / ${t.length}`),h&&(h.textContent=p<t.length-1?"Appuie pour continuer →":"Appuie pour voir toutes tes cartes");const k=je(u),b=u.card_type==="player"&&((v=u.player)==null?void 0:v.rarity)==="legende";w.innerHTML=`
      ...
      </div>`,b&&c();const g=document.getElementById("current-card-wrap");let y=!1;g.addEventListener("click",()=>{if(y)return;y=!0;const x=p+1;x<t.length?(g.style.transition="all 0.25s ease",g.style.transform="translateX(-120%) rotate(-15deg)",g.style.opacity="0",setTimeout(()=>d(x),250)):s()})}function s(){f(),document.getElementById("reveal-phase").style.display="none";const p=document.getElementById("recap-phase");p.style.display="flex";const u=document.getElementById("recap-grid");u.innerHTML=t.map((m,w)=>`
      ...
      </div>`).join("")}let l=null;function c(){const p=document.getElementById("fireworks-canvas");if(!p)return;p.width=window.innerWidth,p.height=window.innerHeight;const u=p.getContext("2d"),m=[];function w(){const k=Math.random()*p.width,b=Math.random()*p.height*.6,g=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],y=g[Math.floor(Math.random()*g.length)];for(let v=0;v<60;v++){const x=Math.PI*2/60*v,E=2+Math.random()*5;m.push({x:k,y:b,vx:Math.cos(x)*E,vy:Math.sin(x)*E,alpha:1,color:y,size:2+Math.random()*3})}}w(),l=setInterval(w,600);function h(){if(document.getElementById("fireworks-canvas")){u.clearRect(0,0,p.width,p.height);for(let k=m.length-1;k>=0;k--){const b=m[k];if(b.x+=b.vx,b.y+=b.vy+.08,b.vy*=.98,b.alpha-=.018,b.alpha<=0){m.splice(k,1);continue}u.globalAlpha=b.alpha,u.fillStyle=b.color,u.beginPath(),u.arc(b.x,b.y,b.size,0,Math.PI*2),u.fill()}u.globalAlpha=1,(l!==null||m.length>0)&&requestAnimationFrame(h)}}h()}function f(){l!==null&&(clearInterval(l),l=null);const p=document.getElementById("fireworks-canvas");p&&p.getContext("2d").clearRect(0,0,p.width,p.height)}document.getElementById("reveal-collection").addEventListener("click",()=>{f(),o.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{f(),o.remove(),i("boosters")})}function je(t){var e,i;if(t.card_type==="player"&&t.player){const o=t.player,r=o.job||"ATT",n=xt[r]||"#1A6B3C",a=vt[o.rarity]||"#ccc",d=r==="GK"?o.note_g:r==="DEF"?o.note_d:r==="MIL"?o.note_m:o.note_a,s=mt(o),l={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL"}[o.country_code]||o.country_code;return`<div style="width:140px;height:200px;background:#f2e8d2;border-radius:12px;border:3px solid ${a};overflow:hidden;display:flex;flex-direction:column">
      <!-- Nom -->
  ...
  `,document.body.appendChild(t),t.addEventListener("click",e=>{e.target===t&&t.remove()}),document.getElementById("odds-close").addEventListener("click",()=>t.remove())}function Tt(){return new Promise(t=>{const e=document.createElement("div");e.style.cssText="position:fixed;inset:0;background:#000;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:16px";let i=5;e.innerHTML=`<div style="font-size:48px">📺</div>
      ...
      <div style="font-size:36px;font-weight:900" id="ad-cd">5</div>`,document.body.appendChild(e);const o=setInterval(()=>{i--;const r=document.getElementById("ad-cd");r&&(r.textContent=i),i<=0&&(clearInterval(o),e.remove(),t(!0))},1e3)})}const W={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},R={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Ue(t,e,i,o,r){var n;t.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
    <div style="text-align:center;padding:40px;color:#fff">
      <div style="font-size:48px;margin-bottom:16px">${e}</div>
      <p style="margin-bottom:16px">${i}</p>
      <button class="btn btn-primary" id="msg-btn">${o}</button>
    </div>
  </div>`,(n=document.getElementById("msg-btn"))==null||n.addEventListener("click",r)}function S(t){const e=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!e||!(t!=null&&t.skin)||!(t!=null&&t.hair))return null;const i=t.hair==="chauve"?`${t.skin}-chauve-rase`:`${t.skin}-${t.hair}-${t.hair_length}`;return`${e}/storage/v1/object/public/assets/tetes/${i}.png`}function ye(t){var i,o;const e=t.player;return{cardId:t.id,id:e.id,firstname:e.firstname,name:e.surname_encoded,country_code:e.country_code,club_id:e.club_id,job:e.job,job2:e.job2,note_g:Number(e.note_g)||0,note_d:Number(e.note_d)||0,note_m:Number(e.note_m)||0,note_a:Number(e.note_a)||0,rarity:e.rarity,skin:e.skin,hair:e.hair,hair_length:e.hair_length,clubName:((i=e.clubs)==null?void 0:i.encoded_name)||null,clubLogo:((o=e.clubs)==null?void 0:o.logo_url)||null,boost:0,used:!1,_line:null,_col:null}}function $e(t){return t===1?[1]:t===2?[0,2]:t===3?[0,1,2]:t===4?[0,1,1,2]:t===5?[0,1,1,1,2]:[1]}function Lt(){const t=Math.random()*100;return t<.1?4:t<5?3:t<20?2:1}function Ve(t,e){const i=W[e]||W["4-4-2"],o={GK:[],DEF:[],MIL:[],ATT:[]},r=[...t];for(const n of["GK","DEF","MIL","ATT"]){const a=[];for(let s=0;s<i[n];s++){let l=r.findIndex(c=>c.job===n);if(l===-1&&(l=r.findIndex(c=>c.job2===n)),l===-1&&(l=0),r[l]){const c={...r[l],_line:n};a.push(c),r.splice(l,1)}}const d=$e(a.length);a.forEach((s,l)=>{s._col=d[l]}),o[n]=a}return o}async function Mt(t,e){const{data:i}=await $.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length").eq("is_active",!0).limit(60);if(!i||i.length<11)return zt(t);const o=W[t]||W["4-4-2"],r={GK:[],DEF:[],MIL:[],ATT:[]},n=[...i];for(const a of["GK","DEF","MIL","ATT"]){const d=n.filter(p=>p.job===a),s=n.filter(p=>p.job!==a),l=[...d,...s],c=[];for(let p=0;p<o[a];p++){const u=l[p]||n[p];u&&c.push({cardId:"ai-"+u.id+"-"+p,id:u.id,firstname:u.firstname,name:u.surname_encoded,country_code:u.country_code,club_id:u.club_id,job:u.job,job2:u.job2,note_g:Number(u.note_g)||0,note_d:Number(u.note_d)||0,note_m:Number(u.note_m)||0,note_a:Number(u.note_a)||0,rarity:u.rarity,skin:u.skin,hair:u.hair,hair_length:u.hair_length,boost:0,used:!1,_line:a})}const f=$e(c.length);c.forEach((p,u)=>{p._col=f[u]}),r[a]=c}return r}function zt(t){const e=W[t]||W["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},o=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let r=0;for(const n of["GK","DEF","MIL","ATT"]){const a=[];for(let s=0;s<e[n];s++){const l=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+r,id:"fake-"+r,firstname:"IA",name:o[r%o.length],country_code:"XX",club_id:null,job:n,job2:null,note_g:n==="GK"?l:2,note_d:n==="DEF"?l:2,note_m:n==="MIL"?l:2,note_a:n==="ATT"?l:2,rarity:"normal",boost:0,used:!1,_line:n}),r++}const d=$e(a.length);a.forEach((s,l)=>{s._col=d[l]}),i[n]=a}return i}async function jt(t,e){var v;const{state:i,navigate:o,toast:r}=e,n=i.params||{};t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const a=n.matchMode||"vs_ai_easy",d=a.replace("vs_ai_",""),s=a;if(!n.deckId)return Ct(t,e,a);const l=n.deckId,[{data:c},{data:f}]=await Promise.all([$.from("decks").select("formation,name").eq("id",l).single(),$.from("deck_cards").select(`position, is_starter, slot_order,
        card:cards(id, card_type, formation,
          player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
            note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
            clubs(encoded_name,logo_url)))`).eq("deck_id",l).order("slot_order")]),p=(f||[]).filter(x=>{var E;return x.is_starter&&((E=x.card)==null?void 0:E.player)}).map(x=>ye(x.card)),u=(f||[]).filter(x=>{var E;return!x.is_starter&&((E=x.card)==null?void 0:E.player)}).map(x=>ye(x.card));if(p.length<11){Ue(t,"⚠️",`Deck incomplet (${p.length}/11).`,"Compléter",()=>o("decks"));return}const m=(f||[]).find(x=>{var E;return((E=x.card)==null?void 0:E.card_type)==="formation"}),w=(c==null?void 0:c.formation)||((v=m==null?void 0:m.card)==null?void 0:v.formation)||"4-4-2",{data:h}=await $.from("cards").select("id,gc_type").eq("owner_id",i.profile.id).eq("card_type","game_changer"),k=Ve(p,w),b=await Mt(w),{data:g}=await $.from("matches").insert({home_id:i.profile.id,away_id:null,mode:s,home_deck_id:l,status:"in_progress"}).select().single(),y={matchId:g==null?void 0:g.id,mode:s,difficulty:d,formation:w,homeTeam:k,aiTeam:b,homeSubs:u,subsUsed:0,maxSubs:Math.min(u.length,3),homeScore:0,aiScore:0,gcCards:h||[],usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Bt(t,y,e)}async function Ct(t,e,i){const{state:o,navigate:r}=e;t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await $.from("decks").select("id,name,is_active,formation").eq("owner_id",o.profile.id).order("created_at",{ascending:!1});if(!n||n.length===0){Ue(t,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>r("decks"));return}const a=n.map(c=>c.id),{data:d}=await $.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order");let s=0;function l(){var b,g,y,v,x;const c=n[s],f=(d||[]).filter(E=>E.deck_id===c.id),p=f.filter(E=>{var _;return E.is_starter&&((_=E.card)==null?void 0:_.player)}).map(E=>ye(E.card)),u=f.find(E=>{var _;return((_=E.card)==null?void 0:_.card_type)==="formation"}),m=c.formation||((b=u==null?void 0:u.card)==null?void 0:b.formation)||"4-4-2",w=p.length>=11?Ve(p,m):null,h=p.length>=11;t.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:calc(100dvh - 130px);overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;align-items:center;gap:8px;padding:8px;flex-shrink:0">
        ...
        <div style="flex:1;text-align:center">
          ...
          <div style="font-size:11px;opacity:.6;margin-top:2px">${m} · ${p.length}/11 ${c.is_active?"· ⭐ Actif":""}</div>
        </div>
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:hidden;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center">
        ${w?`<div style="width:min(98vw, calc(100dvh - 400px));overflow:hidden;flex-shrink:0">${Je(w,m,null,[],285,285)}</div>`:`<div style="display:flex;align-items:center;justify-content:center;height:100%;opacity:.4;flex-direction:column;gap:8px">
    ...
    </div>`,(g=document.getElementById("prev-deck"))==null||g.addEventListener("click",()=>{s>0&&(s--,l())}),(y=document.getElementById("next-deck"))==null||y.addEventListener("click",()=>{s<n.length-1&&(s++,l())}),(v=document.getElementById("validate-deck"))==null||v.addEventListener("click",()=>{h&&e.navigate("match",{matchMode:i,deckId:c.id})}),(x=document.getElementById("cancel-deck-select"))==null||x.addEventListener("click",()=>r("home"));const k=document.getElementById("deck-swipe-zone");if(k){let E=0,_=0;k.addEventListener("touchstart",M=>{E=M.touches[0].clientX,_=M.touches[0].clientY},{passive:!0}),k.addEventListener("touchend",M=>{const L=M.changedTouches[0].clientX-E,z=M.changedTouches[0].clientY-_;Math.abs(L)<40||Math.abs(L)<Math.abs(z)||(L<0&&s<n.length-1?(s++,l()):L>0&&s>0&&(s--,l()))},{passive:!0})}}l()}function Bt(t,e,i){const o=e.homeTeam.MIL||[],r=e.aiTeam.MIL||[];function n(f){return f.reduce((p,u)=>p+D(u,"MIL"),0)}function a(f){let p=0;for(let u=0;u<f.length-1;u++){const m=Y(f[u],f[u+1]);m!=="#ff3333"&&m!=="#cc2222"&&p++}return p}const d=n(o)+a(o),s=n(r)+a(r),l=d>=s;function c(f,p,u){return`<div style="text-align:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${f.map((m,w)=>{const h=S(m),k=w<f.length-1?Y(m,f[w+1]):null,b=k&&k!=="#ff3333"&&k!=="#cc2222";return`
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;gap:24px;padding:24px;background:#0a3d1e">
    ...
    <div id="midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff;max-width:320px"></div>
  </div>`,setTimeout(()=>{const f=document.getElementById("score-home"),p=document.getElementById("score-ai"),u=document.getElementById("midfield-result");if(f&&p&&(l?(f.style.fontSize="80px",f.style.color="#FFD700",p.style.opacity="0.25"):(p.style.fontSize="80px",p.style.color="#ff6b6b",f.style.opacity="0.25")),u){const m=Lt();u.style.opacity="1",l&&e.clubName,u.innerHTML=`
      ...
      `,l&&(e.boostCard={value:m})}e.attacker=l?"home":"ai",e.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:o.map(m=>({name:m.name,note:D(m,"MIL"),portrait:S(m),job:m.job})),aiPlayers:r.map(m=>({name:m.name,note:D(m,"MIL"),portrait:S(m),job:m.job})),homeTotal:d,aiTotal:s,text:`Duel milieu : ${e.clubName} ${d} – ${s} IA → ${l?e.clubName+" attaque":"IA attaque"}`}),setTimeout(()=>{e.phase=e.attacker==="home"?"attack":"ai-attack",G(t,e,i),e.attacker==="ai"&&setTimeout(()=>me(t,e,i),1e3)},2800)},1200)}function St(t){if(!t||t.length<2)return"🌍";try{return String.fromCodePoint(127462+t.charCodeAt(0)-65)+String.fromCodePoint(127462+t.charCodeAt(1)-65)}catch{return"🌍"}}function Ye(t,e,i,o,r=310,n=310){const a=be[e]||{},d=ue(e)||ee[e]||[],s={},l=["ATT","MIL","DEF","GK"];for(const b of l)(t[b]||[]).forEach((y,v)=>{s[`${b}${v+1}`]=y});function c(b){const g=a[b];return g?{x:g.x*r,y:g.y*n}:null}let f="";for(const[b,g]of d){const y=c(b),v=c(g);if(!y||!v)continue;const x=s[b],E=s[g],_=Y(x,E);_==="#00ff88"||_==="#FFD700"?(f+=`<line x1="${y.x.toFixed(1)}" y1="${y.y.toFixed(1)}" x2="${v.x.toFixed(1)}" y2="${v.y.toFixed(1)}"
        stroke="${_}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,f+=`<line x1="${y.x.toFixed(1)}" y1="${y.y.toFixed(1)}" x2="${v.x.toFixed(1)}" y2="${v.y.toFixed(1)}"
        stroke="${_}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):f+=`<line x1="${y.x.toFixed(1)}" y1="${y.y.toFixed(1)}" x2="${v.x.toFixed(1)}" y2="${v.y.toFixed(1)}"
        stroke="${_}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const p=48,u=64,m=13,w=16,h={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[b,g]of Object.entries(s)){const y=c(b);if(!y||!g)continue;const v=b.replace(/[0-9]/g,""),x=R[v]||"#555",E=i==="attack"&&["MIL","ATT"].includes(v)&&!g.used||i==="defense"&&["GK","DEF","MIL"].includes(v)&&!g.used,_=o.includes(g.cardId);let M;i==="attack"?M=v==="MIL"?Number(g.note_m)||0:Number(g.note_a)||0:i==="defense"?M=v==="GK"?Number(g.note_g)||0:v==="MIL"?Number(g.note_m)||0:Number(g.note_d)||0:M=Number(v==="GK"?g.note_g:v==="DEF"?g.note_d:v==="MIL"?g.note_m:g.note_a)||0,M=M+(g.boost||0);const L=(y.x-p/2).toFixed(1),z=(y.y-u/2).toFixed(1),B=g.used?.25:1,H=h[g==null?void 0:g.rarity]||h.normal,X=_?"#ffffff":H,I=_?3:(g==null?void 0:g.rarity)==="légende"||(g==null?void 0:g.rarity)==="pépite"?2.5:2,A=u-m-w;f+=`<defs><clipPath id="cp-${b}"><rect x="${L}" y="${(y.y-u/2+m).toFixed(1)}" width="${p}" height="${A}"/></clipPath></defs>`,f+=`<rect x="${L}" y="${z}" width="${p}" height="${u}" rx="5" fill="${x}" opacity="${B}"/>`;const T=S(g);T&&!g.used&&(f+=`<image href="${T}" x="${L}" y="${(y.y-u/2+m).toFixed(1)}" width="${p}" height="${A}" clip-path="url(#cp-${b})" preserveAspectRatio="xMidYMin slice"/>`),f+=`<rect x="${L}" y="${z}" width="${p}" height="${m}" rx="4" fill="rgba(255,255,255,0.92)"/>`,f+=`<text x="${y.x.toFixed(1)}" y="${(y.y-u/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(g.name||"").slice(0,9)}</text>`;const C=(y.y+u/2-w).toFixed(1);f+=`<rect x="${L}" y="${C}" width="${p}" height="${w}" fill="rgba(255,255,255,0.92)"/>`,g.used?f+=`<text x="${y.x.toFixed(1)}" y="${(y.y+u/2-w/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="13" font-weight="900" fill="rgba(0,0,0,0.4)" font-family="Arial Black">–</text>`:(f+=`<text x="${(y.x-13).toFixed(1)}" y="${(y.y+u/2-w/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${St(g.country_code)}</text>`,f+=`<text x="${y.x.toFixed(1)}" y="${(y.y+u/2-w/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${M}</text>`,g.clubName&&(f+=`<text x="${(y.x+14).toFixed(1)}" y="${(y.y+u/2-w/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(g.clubName||"").slice(0,3).toUpperCase()}</text>`),g.boost&&(f+=`<rect x="${(y.x+p/2-12).toFixed(1)}" y="${(y.y-u/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,f+=`<text x="${(y.x+p/2-5).toFixed(1)}" y="${(y.y-u/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${g.boost}</text>`)),f+=`<rect x="${L}" y="${z}" width="${p}" height="${u}" rx="5" fill="${_?"rgba(255,255,255,0.12)":"none"}" stroke="${X}" stroke-width="${I}" opacity="${B}"/>`,E&&(f+=`<rect x="${L}" y="${z}" width="${p}" height="${u}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${_?"selected":""}" data-card-id="${g.cardId}" data-role="${v}" style="cursor:pointer"/>`)}const k=38;return`<svg viewBox="${-k} ${-k} ${r+k*2} ${n+k*2}" width="100%" style="display:block;width:100%;max-width:380px;margin:0 auto">
    ${f}
  ...
  <div style="text-align:center;flex-shrink:0;width:38px">
    ...
    <div style="display:flex;align-items:center;gap:4px;${e?"flex-direction:row-reverse":""};background:rgba(255,255,255,0.04);border-radius:8px;padding:5px 8px;border:1px solid rgba(255,255,255,0.07)">
      ...
      ${ne(t.inPlayer||{})}
    </div>`}return`<div style="font-size:11px;color:${t.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${t.type==="goal"?700:400};padding:3px 2px">${t.text||""}</div>`}function G(t,e,i){var m,w,h,k,b,g,y,v;const o=e.selected.map(x=>x.cardId),r=e.usedSubIds||[],n=e.homeSubs.filter(x=>!r.includes(x.cardId)),d=Object.values(e.homeTeam).flat().filter(x=>x.used).length>0&&n.length>0&&e.subsUsed<e.maxSubs;e.log[e.log.length-1];const s=e.phase==="ai-attack"||e.phase==="ai-defense",l=e.phase==="attack",c=e.phase==="defense",f=e.phase==="finished",p=e.gcCards.filter(x=>!e.usedGc.includes(x.id)),u=e.boostCard&&!e.boostUsed;t.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;height:calc(100dvh - 130px);overflow:hidden;background:#0a3d1e;position:relative">
      ...
      ${(()=>{if(e.phase==="defense"&&e.pendingAttack){const E=e.pendingAttack;return`<div style="padding:6px 8px;background:rgba(180,30,30,0.25);border-left:3px solid #ff6b6b">
            ...
            <div style="display:flex;align-items:center;gap:6px">
              <div style="display:flex;gap:5px;flex:1">
                ${(E.players||[]).map(_=>{const M=_._line==="MIL"?_.note_m:_.note_a,L=S(_);return'<div style="text-align:center"><div style="width:36px;height:36px;border-radius:6px;background:'+(R[_.job]||"#555")+';position:relative;overflow:hidden;border:1.5px solid rgba(255,255,255,0.3)">'+(L?'<img src="'+L+'" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover">':"")+'<div style="position:absolute;bottom:0;left:0;right:0;background:rgba(0,0,0,0.7);font-size:9px;color:#fff;font-weight:900;text-align:center">'+M+'</div></div><div style="font-size:6px;color:rgba(255,255,255,0.5)">'+(_.name||"").slice(0,6)+"</div></div>"}).join("")}
              </div>
          ...
          </div>`}const x=e.log[e.log.length-1];return x?'<div style="padding:2px 4px">'+Ft(x)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    <div style="display:flex;flex:1;min-height:0;overflow:hidden">
      ...
      <div style="display:flex;flex-direction:column;gap:5px;padding:6px 3px;width:42px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
        ${n.length===0?'<div style="font-size:8px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':n.map(x=>{const E=S(x),_=R[x.job]||"#555",M=x.job==="GK"?x.note_g:x.job==="DEF"?x.note_d:x.job==="MIL"?x.note_m:x.note_a;return`
      ...
      <div style="flex:1;display:grid;grid-template-columns:repeat(4,1fr);gap:3px;align-content:start">
        ${p.map(x=>{var E;return`
          <div class="gc-mini" data-gc-id="${x.id}" data-gc-type="${x.gc_type}"
            ...
            <div style="font-size:16px">${((E=de[x.gc_type])==null?void 0:E.icon)||"⚡"}</div>
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${e.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...e.log].reverse().map((x,E)=>`
  ...
  </div>`,requestAnimationFrame(()=>{var L;const x=t.querySelector(".match-screen");if(!x)return;const E=x.getBoundingClientRect().top,_=((L=document.querySelector('.bottom-nav, nav, [class*="nav"]'))==null?void 0:L.offsetHeight)||60,M=Math.round(window.innerHeight-E-_);M>150&&(x.style.height=M+"px")}),(m=document.getElementById("match-quit"))==null||m.addEventListener("click",()=>{confirm("Abandonner ? Résultat : défaite 3-0")&&(e.homeScore=0,e.aiScore=3,ie(t,e,i))}),(w=document.getElementById("view-ai"))==null||w.addEventListener("click",()=>Pt(e,i)),(h=document.getElementById("toggle-history"))==null||h.addEventListener("click",()=>{var x;(x=document.getElementById("match-history-panel"))==null||x.classList.add("open")}),(k=document.getElementById("close-history"))==null||k.addEventListener("click",()=>{var x;(x=document.getElementById("match-history-panel"))==null||x.classList.remove("open")}),(b=document.getElementById("btn-action"))==null||b.addEventListener("click",()=>{e.selected.length!==0&&(l?Gt(t,e,i):c&&Nt(t,e,i))}),(g=document.getElementById("btn-results"))==null||g.addEventListener("click",()=>ie(t,e,i)),t.querySelectorAll(".match-slot-hit").forEach(x=>{x.addEventListener("click",()=>Dt(x,e,t,i))}),t.querySelectorAll(".gc-mini").forEach(x=>{x.addEventListener("click",()=>Ot(x.dataset.gcId,x.dataset.gcType,t,e,i))}),(y=document.getElementById("boost-card"))==null||y.addEventListener("click",()=>Kt(t,e,i)),t.querySelectorAll(".sub-btn-col").forEach(x=>{x.addEventListener("click",()=>Be(t,e,i,x.dataset.subId))}),(v=document.getElementById("sub-btn-main"))==null||v.addEventListener("click",()=>Be(t,e,i))}function Dt(t,e,i,o){const r=t.dataset.cardId,n=t.dataset.role,a=e.selected.findIndex(d=>d.cardId===r);if(a!==-1)e.selected.splice(a,1);else{if(e.selected.length>=3){o.toast("Maximum 3 joueurs","error");return}const d=(e.homeTeam[n]||[]).find(s=>s.cardId===r);d&&e.selected.push({...d,_role:n,_line:n})}G(i,e,o)}function Gt(t,e,i){const o=e.selected.map(n=>({...n,_line:n._role})),r=Ne(o,e.modifiers.home);e.pendingAttack={...r,players:[...e.selected],side:"home"},e.selected.forEach(n=>{const a=(e.homeTeam[n._role]||[]).find(d=>d.cardId===n.cardId);a&&(a.used=!0)}),e.log.push({text:`⚔️ Vous attaquez : ${r.total} (base ${r.base}${r.links?` +${r.links} liens`:""}) — ${e.selected.map(n=>n.name).join(", ")}`,type:"info"}),e.selected=[],e.modifiers.home={},e.phase="ai-defense",G(t,e,i),setTimeout(()=>Rt(t,e,i),1200)}function Nt(t,e,i){const o=e.selected.map(d=>({...d,_line:d._role})),r=Re(o,e.modifiers.home);e.selected.forEach(d=>{const s=(e.homeTeam[d._role]||[]).find(l=>l.cardId===d.cardId);s&&(s.used=!0)});const n=qe(e.pendingAttack.total,r.total,e.modifiers.home),a={type:"duel",title:"Défense",aiPlayers:(e.pendingAttack.players||[]).map(d=>({name:d.name,note:d._line==="MIL"?d.note_m:d.note_a,portrait:S(d),job:d.job})),homePlayers:e.selected.map(d=>{const s=(e.homeTeam[d._role]||[]).find(l=>l.cardId===d.cardId)||d;return{name:s.name,note:s._line==="GK"?s.note_g:s._line==="MIL"?s.note_m:s.note_d,portrait:S(s),job:s.job}}),homeTotal:r.total,aiTotal:e.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(n.shielded)a.text="🛡️ Bouclier ! But annulé.",e.log.push(a);else if(n.goal){e.aiScore++,a.isGoal=!0,a.homeScored=!1,a.text=`⚽ BUT IA ! (${e.pendingAttack.total} > ${r.total})`,e.log.push(a),e.selected=[],e.modifiers.home={},e.pendingAttack=null,G(t,e,i),Qe(a.aiPlayers,e.homeScore,e.aiScore,!1,()=>{pe(t,e,i,"home-attack")});return}else a.text=`🧤 Défense réussie ! (${r.total} ≥ ${e.pendingAttack.total})`,e.log.push(a);e.selected=[],e.modifiers.home={},e.pendingAttack=null,pe(t,e,i,"home-attack")}function me(t,e,i){const o=[...e.aiTeam.MIL||[],...e.aiTeam.ATT||[]],r=Oe(o,"attack",e.difficulty);if(!r.length){Xe(t,e,i);return}const n=Ne(r,e.modifiers.ai);e.pendingAttack={...n,players:r,side:"ai"},r.forEach(a=>{a.used=!0}),e.log.push({text:`🤖 IA attaque : ${n.total} (${r.map(a=>a.name).join(", ")})`,type:"info"}),e.modifiers.ai={},e.phase="defense",G(t,e,i)}function Rt(t,e,i){const o=[...e.aiTeam.GK||[],...e.aiTeam.DEF||[],...e.aiTeam.MIL||[]],r=Oe(o,"defense",e.difficulty),n=r.length>0?Re(r,e.modifiers.ai).total:0;r.forEach(s=>{s.used=!0});const a=qe(e.pendingAttack.total,n,e.modifiers.ai),d={type:"duel",title:"Attaque",homePlayers:(e.pendingAttack.players||[]).map(s=>({name:s.name,note:s._line==="MIL"?s.note_m:s.note_a,portrait:S(s),job:s.job})),aiPlayers:r.map(s=>({name:s.name,note:s._line==="GK"?s.note_g:s._line==="MIL"?s.note_m:s.note_d,portrait:S(s),job:s.job})),homeTotal:e.pendingAttack.total,aiTotal:n,isGoal:!1,homeScored:!1,text:""};if(a.shielded)d.text="🛡️ Bouclier IA !",e.log.push(d);else if(a.goal){e.homeScore++,d.isGoal=!0,d.homeScored=!0,d.text=`⚽ BUT ! (${e.pendingAttack.total} > ${n})`,e.log.push(d),e.modifiers.ai={},e.pendingAttack=null,G(t,e,i),Qe(d.homePlayers,e.homeScore,e.aiScore,!0,()=>{pe(t,e,i,"ai-attack")});return}else d.text=`🧤 IA défend (${n} ≥ ${e.pendingAttack.total})`,e.log.push(d);e.modifiers.ai={},e.pendingAttack=null,pe(t,e,i,"ai-attack")}function pe(t,e,i,o){if(e.round++,We(e)){ie(t,e,i);return}if(o==="home-attack"){if(![...e.homeTeam.MIL||[],...e.homeTeam.ATT||[]].filter(n=>!n.used).length){if(![...e.homeTeam.GK||[],...e.homeTeam.DEF||[],...e.homeTeam.MIL||[]].filter(a=>!a.used).length){ie(t,e,i);return}e.phase="ai-attack",G(t,e,i),setTimeout(()=>me(t,e,i),800);return}e.phase="attack",G(t,e,i)}else{if(![...e.aiTeam.MIL||[],...e.aiTeam.ATT||[]].filter(n=>!n.used).length){Xe(t,e,i);return}e.phase="ai-attack",G(t,e,i),setTimeout(()=>me(t,e,i),800)}}function We(t){const e=["MIL","ATT","GK","DEF"].some(o=>(t.homeTeam[o]||[]).some(r=>!r.used)),i=["MIL","ATT","GK","DEF"].some(o=>(t.aiTeam[o]||[]).some(r=>!r.used));return!e&&!i}function Xe(t,e,i){We(e)?ie(t,e,i):(e.phase="attack",G(t,e,i))}function qt(t,e,i){const o=document.createElement("div");o.style.cssText=`
    position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:800;
    display:flex;flex-direction:column;align-items:center;justify-content:center;gap:20px;
    animation:subFadeIn 0.2s ease;
  `;const r=S(t),n=S(e),a=R[t.job]||"#555",d=R[e.job]||"#555",s=t.job==="GK"?t.note_g:t.job==="DEF"?t.note_d:t.job==="MIL"?t.note_m:t.note_a,l=e.job==="GK"?e.note_g:e.job==="DEF"?e.note_d:e.job==="MIL"?e.note_m:e.note_a;o.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(o);let c=!1;const f=()=>{c||(c=!0,o.remove(),setTimeout(()=>i(),50))};o.addEventListener("click",f),setTimeout(f,2e3)}function Z(t,e="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${e};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=t,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function Ce(t){const e=S(t),i=R[t.job]||"#555",o={normal:"rgba(255,255,255,0.2)",pépite:"#D4A017",papyte:"#909090",légende:"#7a28b8"}[t.rarity]||"rgba(255,255,255,0.2)",r=t.job==="GK"?t.note_g:t.job==="DEF"?t.note_d:t.job==="MIL"?t.note_m:t.note_a;return`
  <div style="width:72px;overflow:hidden;border-radius:8px;border:2px solid ${o};background:rgba(0,0,0,0.5);flex-shrink:0">
  ...
  </div>`}function Be(t,e,i,o=null){if(e.phase!=="attack"){Z("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(e.usedSubIds||(e.usedSubIds=[]),e.subsUsed>=e.maxSubs){Z(`Maximum ${e.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const r=Object.values(e.homeTeam).flat().filter(c=>c.used),n=e.homeSubs.filter(c=>!e.usedSubIds.includes(c.cardId));if(!r.length){Z("Aucun joueur utilisé à remplacer");return}if(!n.length){Z("Aucun remplaçant disponible");return}let a=null,d=o||null;const s=document.createElement("div");s.id="sub-overlay",s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:700;display:flex;flex-direction:column;overflow:hidden";function l(){var c,f;s.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:14px">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:10px">
        ...
        <div class="sub-pick" data-id="${p.cardId}"
          style="cursor:pointer;border-radius:10px;border:2px solid ${d===p.cardId?"#00ff88":"rgba(255,255,255,0.1)"};overflow:hidden;transition:border-color 0.15s">
          ${Ce(p)}
        </div>`).join("")}
      </div>
    </div>
    <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
      <button id="sub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;
        background:${a&&d?"#1A6B3C":"rgba(255,255,255,0.08)"};
        color:${a&&d?"#fff":"rgba(255,255,255,0.25)"};
        font-size:15px;font-weight:900;cursor:${a&&d?"pointer":"default"}">
        ${a&&d?"✅ Confirmer le remplacement":"Sélectionnez un joueur sortant et entrant"}
      </button>
    </div>`,(c=document.getElementById("sub-close"))==null||c.addEventListener("click",()=>s.remove()),s.querySelectorAll(".grayed-pick").forEach(p=>{p.addEventListener("click",()=>{a=p.dataset.id,p.dataset.line,l()})}),s.querySelectorAll(".sub-pick").forEach(p=>{p.addEventListener("click",()=>{d=p.dataset.id,l()})}),(f=document.getElementById("sub-confirm"))==null||f.addEventListener("click",()=>{if(!a||!d)return;let p=null,u=-1;for(const[k,b]of Object.entries(e.homeTeam)){const g=(b||[]).findIndex(y=>y.cardId===a);if(g!==-1){p=k,u=g;break}}const m=e.homeSubs.find(k=>k.cardId===d);if(u===-1||!p||!m){Z("Erreur remplacement — réessaie","rgba(180,0,0,0.9)"),s.remove(),G(t,e,i);return}const w={...e.homeTeam[p][u]},h={...m,_line:p,_col:w._col,used:!1,boost:0};e.homeTeam[p].splice(u,1,h),e.usedSubIds||(e.usedSubIds=[]),e.usedSubIds.push(d),e.subsUsed++,e.selected=[],e.log.push({type:"sub",subSide:"home",outPlayer:{name:w.name,firstname:w.firstname,note:D(w,p),portrait:S(w),job:w.job},inPlayer:{name:m.name,firstname:m.firstname,note:D(m,p),portrait:S(m),job:m.job},text:`🔄 ${m.firstname} ${m.name} remplace ${w.firstname} ${w.name}`}),s.remove(),qt(w,m,()=>G(t,e,i))})}l(),document.body.appendChild(s)}function Ot(t,e,i,o,r){if(!o.usedGc.includes(t)){switch(o.usedGc.push(t),e){case"Double attaque":o.modifiers.home.doubleAttack=!0,o.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":o.modifiers.home.shield=!0,o.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{let n=!1;for(const a of["ATT","MIL","DEF","GK"]){const d=(o.homeTeam[a]||[]).find(s=>s.used);if(d){d.used=!1,n=!0;break}}o.log.push({text:n?"💫 Joueur ressuscité !":"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":o.modifiers.ai.stolenNote=(o.modifiers.ai.stolenNote||0)+1,o.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const n=[...o.aiTeam.ATT||[],...o.aiTeam.MIL||[]].filter(a=>!a.used);if(n.length){const a=n.sort((d,s)=>D(s,"ATT")-D(d,"ATT"))[0];a.used=!0,o.log.push({text:`❄️ ${a.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":o.maxSubs++,o.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}$.from("cards").delete().eq("id",t).then(()=>{}),G(i,o,r)}}function Kt(t,e,i){const o=Object.values(e.homeTeam).flat().filter(r=>!r.used);if(!o.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ${o.map(r=>`
        <div class="player-boost-opt" data-card-id="${r.cardId}"
          ...
          <div style="width:32px;height:32px;background:${R[r.job]||"#888"};border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:13px">${D(r,r._line||r.job)}</div>
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(r=>{r.addEventListener("click",()=>{const n=r.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const d=(e.homeTeam[a]||[]).find(s=>s.cardId===n);if(d){d.boost=(d.boost||0)+e.boostCard.value,e.log.push({text:`⚡ Boost +${e.boostCard.value} appliqué à ${d.name}`,type:"info"});break}}e.boostUsed=!0,i.closeModal(),G(t,e,i)})})}function Qe(t,e,i,o,r){const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const a=Array.from({length:10},(l,c)=>`
  ...
  <div style="display:flex;gap:10px;animation:scoreIn 0.4s ease 1s both;position:relative;z-index:1">
    ${t.map(l=>`
    <div style="text-align:center">
      <div style="width:50px;height:50px;border-radius:50%;background:${R[l.job]||"#555"};border:2px solid #FFD700;position:relative;overflow:hidden;margin:0 auto">
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(n);let d=!1;const s=()=>{d||(d=!0,n.remove(),setTimeout(()=>r(),50))};n.addEventListener("click",s),setTimeout(s,3500)}async function ie(t,e,i){var c,f;e.phase="finished";const{state:o}=i,r=e.homeScore>e.aiScore,n=e.homeScore===e.aiScore,a=r?"victoire":n?"nul":"defaite",d=nt(e.mode,a);e.matchId&&await $.from("matches").update({status:"finished",home_score:e.homeScore,away_score:e.aiScore,winner_id:r?o.profile.id:null,home_credits_reward:d,played_at:new Date().toISOString()}).eq("id",e.matchId);const s={credits:(o.profile.credits||0)+d,matches_played:(o.profile.matches_played||0)+1};r?s.wins=(o.profile.wins||0)+1:n?s.draws=(o.profile.draws||0)+1:s.losses=(o.profile.losses||0)+1,await $.from("users").update(s).eq("id",o.profile.id),await i.refreshProfile();const l=document.createElement("div");l.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",l.innerHTML=`
    ...
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Ht={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function Ut(t,e){const{state:i,toast:o}=e;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await ke(t,e)}async function ke(t,e){const{state:i,toast:o}=e,{data:r}=await $.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  `;function d(s){const l=document.getElementById("mkt-content"),c=s==="buy"?a:n;if(c.length===0){l.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${c.map(f=>{var h,k,b;const p=(h=f.card)==null?void 0:h.player;if(!p)return"";const u=p.job==="GK"?p.note_g:p.job==="DEF"?p.note_d:p.job==="MIL"?p.note_m:p.note_a,m=Ht[p.rarity],w=(i.profile.credits||0)>=f.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px">
          <div style="width:44px;height:44px;border-radius:8px;background:${Jt(p.job)};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:18px;border:2px solid ${m};flex-shrink:0">${u}</div>
          <div style="flex:1;min-width:0">
    ...
    </div>`,l.querySelectorAll("[data-buy]").forEach(f=>{f.addEventListener("click",()=>Vt(f.dataset.buy,c,t,e))}),l.querySelectorAll("[data-cancel]").forEach(f=>{f.addEventListener("click",()=>Yt(f.dataset.cancel,t,e))})}d("buy"),t.querySelectorAll(".mkt-tab").forEach(s=>{s.addEventListener("click",()=>{t.querySelectorAll(".mkt-tab").forEach(l=>{const c=l===s;l.style.background=c?"var(--green)":"#fff",l.style.color=c?"#fff":"var(--gray-600)",l.style.borderColor=c?"var(--green)":"var(--gray-200)"}),d(s.dataset.tab)})})}async function Vt(t,e,i,o){const{state:r,toast:n,refreshProfile:a}=o,d=e.find(s=>s.id===t);if(d){if((r.profile.credits||0)<d.price){n("Crédits insuffisants","error");return}if(confirm(`Acheter ${d.card.player.firstname} ${d.card.player.surname_encoded} pour ${d.price.toLocaleString("fr")} crédits ?`))try{const{error:s}=await $.from("cards").update({owner_id:r.profile.id,is_for_sale:!1,sale_price:null}).eq("id",d.card.id);if(s)throw s;await $.from("market_listings").update({status:"sold",buyer_id:r.profile.id,sold_at:new Date().toISOString()}).eq("id",t),await $.from("users").update({credits:(r.profile.credits||0)-d.price}).eq("id",r.profile.id);const{data:l}=await $.from("users").select("credits").eq("id",d.seller_id).single();l&&await $.from("users").update({credits:(l.credits||0)+d.price}).eq("id",d.seller_id),await $.from("notifications").insert({user_id:d.seller_id,type:"card_sold",message:`Ta carte ${d.card.player.surname_encoded} a été vendue pour ${d.price} crédits !`,data:{card_id:d.card.id,price:d.price}}),await a(),n("Carte achetée ! ✅","success"),ke(i,o)}catch(s){n("Erreur : "+s.message,"error")}}}async function Yt(t,e,i){const{toast:o}=i,{data:r}=await $.from("market_listings").select("card_id").eq("id",t).single();await $.from("market_listings").update({status:"cancelled"}).eq("id",t),r&&await $.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",r.card_id),o("Annonce retirée","success"),ke(e,i)}function Jt(t){return{GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[t]||"#888"}async function Wt(t,{state:e,navigate:i,toast:o}){t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:r}=await $.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100);t.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `}const j={user:null,profile:null,page:"home",params:{}};function re(t,e="info",i=3e3){const o=document.getElementById("toast");o&&(o.textContent=t,o.className=`show ${e}`,clearTimeout(o._t),o._t=setTimeout(()=>{o.className=""},i))}function Xt(t,e,i=""){document.getElementById("modal-title").textContent=t,document.getElementById("modal-body").innerHTML=e,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function xe(){document.getElementById("modal-overlay").classList.add("hidden")}async function ae(){if(!j.user)return;const{data:t}=await $.from("users").select("*").eq("id",j.user.id).single();t&&(j.profile=t)}const _e="mw_theme";function fe(){return localStorage.getItem(_e)||"light"}function Qt(t){var e;localStorage.setItem(_e,t),ve(t),(e=j.profile)!=null&&e.id&&$.from("users").update({theme:t}).eq("id",j.profile.id).then(()=>{})}function ve(t){document.documentElement.setAttribute("data-theme",t)}function se(t,e={}){j.page=t,j.params=e,Ze()}async function Ze(){const t=document.getElementById("page-content");if(!t)return;document.querySelectorAll(".bottom-nav a").forEach(o=>{o.classList.toggle("active",o.dataset.page===j.page)});const e=document.getElementById("nav-credits");e&&j.profile&&(e.textContent=`💰 ${(j.profile.credits||0).toLocaleString("fr")}`);const i={state:j,navigate:se,toast:re,openModal:Xt,closeModal:xe,refreshProfile:ae};switch(t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',j.page){case"home":await Ae(t,i);break;case"collection":await dt(t,i);break;case"decks":await ge(t,i);break;case"boosters":await bt(t,i);break;case"match":await jt(t,i);break;case"market":await Ut(t,i);break;case"rankings":await Wt(t,i);break;default:await Ae(t,i)}}function Zt(){const t=document.getElementById("app"),e=j.profile;if(!e)return;const i="/manager-wars/icons/";t.innerHTML=`
    <nav class="top-nav">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(o=>{o.addEventListener("click",r=>{r.preventDefault(),se(o.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>se("home")),document.getElementById("nav-credits").addEventListener("click",()=>se("boosters")),document.getElementById("theme-toggle").addEventListener("click",()=>{const r=fe()==="dark"?"light":"dark";Qt(r),Se(r)}),Se(fe())}function Se(t){const e=document.getElementById("theme-icon");e&&(e.textContent=t==="dark"?"☀️":"🌙")}async function ei(){ve(fe()),document.getElementById("modal-overlay").addEventListener("click",e=>{e.target===e.currentTarget&&xe()}),document.getElementById("modal-close").addEventListener("click",xe);const{data:{session:t}}=await $.auth.getSession();if(!t){De(),Ee(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:re});return}j.user=t.user,await ae(),De();try{const{data:e}=await $.from("formation_links_overrides").select("formation, links"),i={};(e||[]).forEach(o=>{i[o.formation]=o.links}),et(i)}catch(e){console.warn("Impossible de charger les overrides de formation:",e)}if(!j.profile){tt(document.getElementById("app"),{state:j,navigate:async()=>{await ae(),Fe()},toast:re,refreshProfile:ae});return}j.profile.theme&&j.profile.theme!==fe()&&(localStorage.setItem(_e,j.profile.theme),ve(j.profile.theme)),Fe(),$.auth.onAuthStateChange(async(e,i)=>{e==="SIGNED_OUT"&&(j.user=null,j.profile=null,document.getElementById("app").innerHTML="",Ee(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:re}))})}function Fe(){const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",Zt(),Ze()}function De(){const t=document.getElementById("app-loader"),e=document.getElementById("app");e&&(e.style.display=""),t&&(t.classList.add("zoom-out"),setTimeout(()=>t.style.display="none",500))}ei();
