...
export async function pageImportExport(container, { toast }) {
  ...
  document.getElementById('export-players-template').addEventListener('click', () => {
    downloadCSV('players_template.csv',
      'id,firstname,surname_real,lastname_reel,country_code,club_id,club_encoded_name,job,job2,note_g,note_d,note_m,note_a,rarity,note_min,note_max,skin,hair,hair_length,sell_price,face,ethnie\n' +
  ...
  document.getElementById('export-players').addEventListener('click', async () => {
    const { data: players, error } = await supabase
      .from('players')
      .select('id,firstname,surname_real,lastname_reel,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,note_min,note_max,skin,hair,hair_length,sell_price,face,ethnie,clubs(encoded_name)')
      .order('surname_real')
    if (error) { toast(error.message, 'error'); return }
    if (!players || players.length === 0) { toast('Aucun joueur à exporter', 'info'); return }

    let csv = 'id,firstname,surname_real,lastname_reel,country_code,club_id,club_encoded_name,job,job2,note_g,note_d,note_m,note_a,rarity,note_min,note_max,skin,hair,hair_length,sell_price,face,ethnie\n'
    players.forEach(p => {
      csv += [
        p.id, p.firstname, p.surname_real, p.lastname_reel || '', p.country_code,
        p.club_id || '', p.clubs?.encoded_name || '', p.job, p.job2 || '',
  ...
  document.getElementById('import-players').addEventListener('change', async (e) => {
    ...
    try {
      ...
      // Charger les clubs pour mapper encoded_name → id (utilisé seulement
      // si la ligne CSV ne fournit pas déjà club_id directement)
      ...
      for (const row of rows) {
        if (!row.firstname || !row.surname_real || !row.country_code || !row.job) {
          skipped++; errorMsgs.push(`Ligne ignorée (champs requis manquants): ${row.firstname || '?'}`)
          continue
        }
        if (!validJobs.includes(row.job)) {
          skipped++; errorMsgs.push(`${row.firstname}: job invalide "${row.job}"`)
          continue
        }

        const clubId = row.club_id || (row.club_encoded_name ? clubMap[row.club_encoded_name.toUpperCase()] || null : null)

        const payload = {
          firstname: row.firstname,
          surname_real: row.surname_real,
          lastname_reel: row.lastname_reel || null,
          country_code: row.country_code.toUpperCase().slice(0,2),
          club_id: clubId,
          job: row.job,
          ...
          // skin : texte libre désormais (ex: "Skin/ATT1.png"), plus de validation par enum
          skin: row.skin || null,
          ...
          face: row.face || null,
          ethnie: row.ethnie || null,
        }

        if (row.id) { toUpdate.push({ id: row.id, ...payload }) }
        else { toInsert.push(payload) }
      }

      // ── Envoi par lots (au lieu d'une requête par ligne, trop lent/fragile
      // sur plusieurs milliers de lignes) ──
      const BATCH_SIZE = 500
      let updated = 0, inserted = 0, batchErrors = 0

      async function runBatches(list, fn) {
        for (let i = 0; i < list.length; i += BATCH_SIZE) {
          const batch = list.slice(i, i + BATCH_SIZE)
          const { error, count } = await fn(batch)
          if (error) {
            batchErrors += batch.length
            errorMsgs.push(`Lot ${i}-${i + batch.length}: ${error.message}`)
          } else {
            fn._ok = (fn._ok || 0) + batch.length
          }
        }
        return fn._ok || 0
      }

      updated = await runBatches(toUpdate, (batch) =>
        supabase.from('players').upsert(batch, { onConflict: 'id' })
      )
      inserted = await runBatches(toInsert, (batch) =>
        supabase.from('players').insert(batch)
      )

      resultEl.innerHTML = `<div style="color:var(--green)">✅ ${inserted} créés, ${updated} mis à jour</div>
        ${skipped + batchErrors > 0 ? `<div style="color:#c0392b">❌ ${skipped} lignes invalides, ${batchErrors} en erreur sur les lots</div><div style="font-size:11px;color:var(--gray-600);max-height:100px;overflow-y:auto">${errorMsgs.slice(0,10).join('<br>')}</div>` : ''}`
      toast(`${inserted} créés, ${updated} mis à jour`, 'success')
    } catch (err) {
      resultEl.innerHTML = `<span style="color:#c0392b">Erreur : ${err.message}</span>`
    }
    e.target.value = ''
