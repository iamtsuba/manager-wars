...
export async function renderMatchMiniLeague(container, ctx, mlMatchId, leagueId) {
  ...
  await loadMatchSetup(container, ctx, 'random', ({ deckId, formation, starters, subsRaw, gcCardsEnriched, gcDefs, stadiumDef }) => {
    const start = (selectedGC) => showMiniLeagueLobby(container, ctx, deckId, formation, starters, subsRaw, selectedGC, gcDefs || [], mlMatchId, leagueId)
    if (!gcCardsEnriched.length) { start([]); return }
    showGCSelection(container, gcCardsEnriched, start)
  })
}

async function showMiniLeagueLobby(container, ctx, deckId, formation, starters, subsRaw, myGC = [], gcDefs = [], mlMatchId, leagueId) {
  ...
  // Étanchéité : quitter la file random
  ...
  // Charger le match ML pour connaître les 2 joueurs
  const { data: mlMatch } = await supabase
    .from('mini_league_matches')
    .select('*, home:users!home_id(id,pseudo,club_name), away:users!away_id(id,pseudo,club_name)')
    .eq('id', mlMatchId).single()

  if (!mlMatch) { toast('Match introuvable', 'error'); _showBottomNav(container); navigate('mini-league'); return }

  // Vérifier que l'utilisateur est bien l'un des 2 joueurs
  if (mlMatch.home_id !== uid && mlMatch.away_id !== uid) {
    toast('Tu ne fais pas partie de ce match', 'error')
    _showBottomNav(container); navigate('mini-league'); return
  }

  const amIHome = mlMatch.home_id === uid
  const oppUser = amIHome ? mlMatch.away : mlMatch.home
  const oppId   = amIHome ? mlMatch.away_id : mlMatch.home_id

  const renderLobby = (myReady, oppReady) => {
    container.innerHTML = `
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:20px;background:linear-gradient(180deg,#0a1628,#1a0a2e);padding:24px;text-align:center;color:#fff">
        ...
        <div style="font-size:14px;color:rgba(255,255,255,0.6)">Journée ${mlMatch.matchday}</div>
        <div style="display:flex;gap:24px;align-items:center">
          ...
          <div style="display:flex;flex-direction:column;align-items:center;gap:6px">
            <div style="width:52px;height:52px;border-radius:50%;background:${oppReady?'#1A6B3C':'rgba(255,255,255,0.1)'};display:flex;align-items:center;justify-content:center;font-size:22px;border:2px solid ${oppReady?'#22c55e':'rgba(255,255,255,0.2)'}">
              ${oppReady?'✅':'⏳'}
            </div>
            <div style="font-size:12px;font-weight:700">${oppUser?.club_name||oppUser?.pseudo||'?'}</div>
            <div style="font-size:11px;color:${oppReady?'#22c55e':'#aaa'}">${oppReady?'Prêt':'En attente'}</div>
          </div>
        </div>
    ...
    document.getElementById('lobby-cancel')?.addEventListener('click', () => {
      cancelled = true; clearInterval(pollInterval)
      _showBottomNav(container); navigate('mini-league')
    })
  }

  const startMatch = async (matchId, isHome) => {
    cancelled = true; clearInterval(pollInterval)
    await new Promise(r => setTimeout(r, 600))
    if (!container.isConnected) return
    renderPvpMatch(container, ctx, matchId, isHome, myGC, gcDefs)
  }

  renderLobby(true, false)

  // ── Home crée le match, Away le rejoint ──────────────────────────────
  if (amIHome) {
    // Vérifier si le match n'existe pas déjà (cas reprise)
    let existingMatchId = mlMatch.match_id
    if (!existingMatchId) {
      const { data: newMatch, error: mErr } = await supabase.from('matches').insert({
        home_id: uid, away_id: oppId,
        home_deck_id: deckId, away_deck_id: null,
        status: 'active', mode: 'mini_league'
      }).select().single()
      if (mErr || !newMatch) { toast('Erreur création match', 'error'); _showBottomNav(container); navigate('mini-league'); return }
      existingMatchId = newMatch.id
      await supabase.from('mini_league_matches').update({ match_id: existingMatchId, status: 'playing' }).eq('id', mlMatchId)
    }
    // Polling : attendre que away_deck_id soit rempli
    const finalMatchId = existingMatchId
    pollInterval = setInterval(async () => {
      if (cancelled) { clearInterval(pollInterval); return }
      const { data: m } = await supabase.from('matches').select('away_deck_id').eq('id', finalMatchId).single()
      if (m?.away_deck_id) { clearInterval(pollInterval); startMatch(finalMatchId, true) }
    }, 1500)
  } else {
    // Away : attendre que home crée le match puis rejoindre
    let joinedMatchId = mlMatch.match_id
    pollInterval = setInterval(async () => {
      if (cancelled) { clearInterval(pollInterval); return }
      const { data: freshMl } = await supabase.from('mini_league_matches').select('match_id').eq('id', mlMatchId).single()
      if (freshMl?.match_id) {
        clearInterval(pollInterval)
        joinedMatchId = freshMl.match_id
        // Enregistrer mon deck dans le match
        await supabase.from('matches').update({ away_deck_id: deckId }).eq('id', joinedMatchId)
        startMatch(joinedMatchId, false)
      }
    }, 1500)
  }
  ...
  // Le lobby polling est géré plus haut (home crée / away rejoint)
}
...
async function renderPvpMatch(container, ctx, matchId, amIHome, myGC = [], gcDefs = []) {
  ...
  if (!match) { toast('Match introuvable', 'error'); navigate('mini-league'); return }
  ...
  if (!gameState) {
    if (amIHome) {
      ...
      if (!gameState) { toast('Erreur de synchronisation', 'error'); navigate('mini-league'); return }
  ...
  function showPvpEndScreen(row) {
    ...
    // le gagnant enregistre. En cas de nul, le joueur dont l'ID est le plus petit.
    if (!_statsRecorded) {
      ...
      const winnerId = row.winner_id || (p1Score > p2Score ? match.home_id : p2Score > p1Score ? match.away_id : null)
      const iAmAuthority = winnerId
        ? state.profile.id === winnerId           // le gagnant enregistre
        : state.profile.id < (amIHome ? match.away_id : match.home_id) // nul : plus petit ID
      if (iAmAuthority) {
        _statsRecorded = true
  ...
  async function forfeitMatch() {
    ...
    setTimeout(() => { _showBottomNav(container); navigate('mini-league') }, 800)
  }
  ...
  function renderPvpScreen() {
    ...
    const terrainHTML = `<div style="overflow:hidden;min-width:0;flex:1;min-height:0;display:flex;flex-direction:column" id="match-field">
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${renderTeam(myTeam, gameState[myRole+'Formation'], phase, selectedIds,Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.05), extraSelectableIds)}
      </div>
    </div>`
  ...
  function renderPvpReveal() {
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="width:min(90vw,420px)">${buildTeamSVG(gameState[oppRole+'Team'],gameState[oppRole+'Formation'],null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.05))}</div>
    </div>`
  ...
  async function finishMatch(finalState) {
    try {
      ...
      const p1Score = finalState.p1Score||0
      const p2Score = finalState.p2Score||0

      await supabase.from('matches').update({
        status: 'finished', winner_id: winnerId, home_score: p1Score, away_score: p2Score
      }).eq('id', matchId)

      // ── Mise à jour du match ML ──────────────────────────────────────────
      // Récupérer le mlMatchId depuis la table mini_league_matches via match_id
      ...
      if (mlM) {
        // home_score = home_id score (p1 si home), away_score = away_id score
        await supabase.from('mini_league_matches').update({
          home_score: p1Score, away_score: p2Score, status: 'finished'
        }).eq('id', mlM.id)

        // Vérifier si TOUS les matchs de la journée sont terminés → avancer automatiquement
        const { data: dayMatches } = await supabase
          .from('mini_league_matches')
          .select('id, status')
          .eq('league_id', mlM.league_id)
          .eq('matchday', mlM.matchday)

        const allDone = (dayMatches||[]).every(m => m.status==='finished' || m.status==='bye')
        if (allDone) {
          const { data: league } = await supabase
            .from('mini_leagues').select('current_day,total_days').eq('id', mlM.league_id).single()
          if (league) {
            const next = (league.current_day||0) + 1
            const finished = next > (league.total_days||0)
            await supabase.from('mini_leagues').update({
              current_day: finished ? league.total_days : next,
    ...
    } catch(e) { console.error('[ML] finishMatch:', e) }
  }
